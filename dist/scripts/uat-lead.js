...
import { startUATRun, upsertUATResult, createUATDefect, getRunStats, closeUATRun, getOpenDefects, exportRunResults } from '../api/uat/handlers';
class UATLead {
    constructor() {
    ...
    async createNewRun() {
        ...
        const result = await startUATRun(env_url, app_version || undefined, browser || undefined, role || undefined, notes || undefined);
        if (result.error) {
                ...
                const gateColor = stats.gate_status === 'GREEN' ? chalk.green :
                    stats.gate_status === 'YELLOW' ? chalk.yellow :
                        stats.gate_status === 'RED' ? chalk.red :
                            chalk.gray;
                console.log(chalk.white(`${statusIcon} ${parsed.case_id}: ${parsed.status}`) +
     ...
     */
    async autoCreateDefect(payload) {
    ...
    async showStatus() {
        ...
        console.log(`Gate Status: ${stats.gate_status === 'GREEN' ? chalk.green(stats.gate_status) :
    ...
    async closeRun() {
        ...
        console.log(chalk.cyan('\n=== UAT Run Complete ==='));
        console.log(`Gate Status: ${result.gate_status === 'GREEN' ? chalk.green.bold('GREEN - GO') :
    ...
    async exportResults() {
        ...
        const results = await exportRunResults(this.activeRun, format);
