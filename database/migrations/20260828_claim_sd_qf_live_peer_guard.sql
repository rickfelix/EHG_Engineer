...
-- SD-LEO-INFRA-QF-CLAIM-PEER-GUARD-001
--
-- RCA: SD-LEO-FIX-CLAIM-RPC-REFUSE-001 (20260717_claim_sd_phantom_session_guard.sql) added a
-- "live-foreign-claim" guard for the SD branch only: if strategic_directives_v2.claiming_session_id
-- points at a session that is still alive (heartbeat < 900s) but whose own claude_sessions.sd_key
-- pointer has drifted away from p_sd_id, claim_sd() refuses to stomp that live peer. The QF branch
-- (v_is_qf = TRUE) never got the equivalent guard: quick_fixes.claiming_session_id is read once
-- for the terminal-status check and only ever written by the final UPDATE -- a QF whose
-- claiming_session_id names a still-live session can be silently claimed out from under it the
-- moment that session's claude_sessions.sd_key drifts (peer parks/switches away without releasing
-- the QF row itself). This migration adds the QF-side counterpart, mirroring the SD-side guard
-- variable-for-variable (v_qf_claiming_id / v_qf_claim_hb_age vs. v_sd_claiming_id / v_sd_claim_hb_age).
--
-- Second, independent fix in the same migration (same SD, both FRs land together per the SD's
-- own title): quick_fixes.started_at exists and is stamped once by scripts/create-quick-fix.js
-- at creation time when a QF is auto-claimed by its creator, but claim_sd()'s QF UPDATE branch
-- never touches it -- a QF claimed later via the normal claim_sd RPC path (qf-start.js,
-- worker-checkin.cjs, lib/sd-creation/source-adapters/qf.js) never gets started_at populated,
-- so read-quick-fix.js's "Started:" display and any started_at-based age/duration calculation
-- silently show nothing for the common re-claim path. Stamped as COALESCE(started_at, NOW()) so
-- a takeover/re-claim never resets an already-recorded original start time.
--
-- Signature is UNCHANGED (still 5 args, same defaults) -- CREATE OR REPLACE is safe here, no DROP
-- needed (same precedent as 20260717_claim_sd_phantom_session_guard.sql).
--
-- The body below is the LIVE production claim_sd() definition (database/migrations/
-- 20260717_claim_sd_phantom_session_guard.sql) with exactly the two additive changes described
-- above, both marked below. Every existing guard, comment, and provenance note is preserved
-- verbatim so this migration is a pure additive change, not a silent documentation-loss regression
-- on the fleet's single most critical RPC.
--
-- Out of scope (same-class asymmetries noted during LEAD-phase investigation, not fixed here):
-- the armed-silence-window guard (v_respect_inflight / expected_silence_until) and the
-- sd_conflict_matrix blocking-conflict check are also SD-only. Both are pre-existing, intentional
-- scope boundaries this SD does not touch -- flagged for a future SD if QF silence-window /
-- conflict-matrix parity is ever wanted.
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'claim_sd: exactly one overload confirmed (5-arg, QF live-peer guard + started_at stamp added).';
END $$;
