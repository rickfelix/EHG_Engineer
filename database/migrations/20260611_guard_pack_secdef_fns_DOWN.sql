-- DOWN migration for 20260611_guard_pack_secdef_fns.sql
-- SD: SD-MAN-FIX-SECURITY-GUARD-PACK-001
-- @approved-by: codestreetlabs@gmail.com
--
-- Restores the 8 pre-guard function bodies VERBATIM as captured live on
-- 2026-06-11 (pg_get_functiondef, .claude/pre-guard-functiondefs.sql) and
-- restates the captured EXECUTE ACL ({postgres,service_role,authenticated}).
--
-- WARNING: applying this DOWN removes the per-venture authorization guards
-- and re-opens F-3 of the 2026-06-10 security posture review.

-- ---- advance_venture_stage(uuid,integer,integer,text) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- advance_venture_to_stage(uuid,integer,text,text) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- bootstrap_venture_workflow(uuid) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- create_eva_conversation(uuid,text,jsonb) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- eva_circuit_allows_request(text) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- record_eva_failure(text,text,jsonb) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- record_eva_success(text) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ---- rescan_stage_20(uuid) | secdef=true | proacl={postgres=X/postgres,service_role=X/postgres,authenticated=X/postgres} ----
...
-- ============================================================================
-- EXECUTE grants — captured ACL restated (CREATE OR REPLACE preserves ACLs;
-- belt-and-suspenders).
-- ============================================================================
