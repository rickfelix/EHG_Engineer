-- Migration: Fix action_items validation for JSONB type
-- Issue: SD-RETRO-ENHANCE-001 (final fix)
-- Created: 2025-10-16
--
-- Problem: Trigger tries to use TRIM() on action_items which is JSONB, not TEXT
-- Error: function pg_catalog.btrim(jsonb) does not exist
--
-- Root Cause: action_items column is JSONB but validation treats it as TEXT
--
-- Solution: Check if action_items JSONB is non-empty instead of TEXT trimming

BEGIN;

-- Drop and recreate trigger function with JSONB-aware validation
...
BEGIN
  ...
  -- Validation 2: PUBLISHED retrospectives must have action_items
  -- ONLY enforce when status is being changed TO published
  -- FIXED: Handle JSONB type properly (check if NULL or empty)
  IF is_status_changing_to_published THEN
    -- Check if action_items is NULL or empty JSONB
    ...
    -- If action_items is a string, check if it's empty after trimming
  ...
  -- Validation 3: PUBLISHED retrospectives must have quality_score >= 70
  -- ONLY enforce when status is being changed TO published
  ...
  -- Validation 4: CODE_TRACEABILITY - validate file extensions
...
'SD-RETRO-ENHANCE-001 Layer 2: Enhanced trigger with CONDITIONAL quality enforcement (JSONB-aware validation)';
...
BEGIN
  ...
  END IF;
  RAISE NOTICE '✅ Fixed trigger verified (JSONB-aware action_items validation)';
END $$;
...
-- Test 1: Embedding update should now work (JSONB handled correctly)
...
-- Test 4: Change status to PUBLISHED without action_items should fail
-- UPDATE retrospectives
-- SET status = 'PUBLISHED', action_items = NULL
-- WHERE id = '[UUID]' AND status != 'PUBLISHED';
-- Expected: ERROR - must have non-empty action_items
