-- Migration: LLM Registry Ollama Integration (Fixed)
-- SD: SD-LEO-INFRA-INTELLIGENT-LOCAL-LLM-001B
-- Purpose: Add Ollama provider and local model for Haiku replacement
-- Date: 2026-02-05
-- Fix: Added unique constraint on (provider_id, model_key) before using ON CONFLICT
...
-- Step 2: Add leo_tier column to llm_models for LEO Protocol tier mapping
...
-- Step 3: Add is_local flag to distinguish local vs cloud models
ALTER TABLE llm_models
ADD COLUMN IF NOT EXISTS is_local BOOLEAN DEFAULT FALSE;

-- Step 4: Add Ollama provider
...
-- Step 5: Add qwen3-coder:30b as local Haiku replacement
...
-- Step 6: Add/update current Anthropic Claude models with LEO tiers
-- Claude Haiku 3.5
INSERT INTO llm_models (
  provider_id,
  model_key,
...
-- Step 7: Add provider_source column to model_usage_log for tracking local vs cloud
...
-- Step 8: Create index for efficient tier lookups
...
-- Step 9: Create view for model registry lookup (used by client-factory.js)
