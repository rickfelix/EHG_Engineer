-- Fix type mismatches in match_sub_agents_hybrid function
-- Issue: RETURNS TABLE declares varchar but SELECT returns text
...
RETURNS TABLE (
  id uuid,
  code text,  -- Changed from varchar(50) to text
  name text,  -- Changed from varchar(255) to text
  priority integer,
...
-- Also fix match_sub_agents_semantic
...
RETURNS TABLE (
  id uuid,
  code text,  -- Changed from varchar(50)
