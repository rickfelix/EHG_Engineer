...
-- SD-LEO-INFRA-SIDE-CLAIM-ELIGIBILITY-001 (FR-2)
--
-- Adds a client-stamped gate-version column + a new claim_sd() parameter so the
-- observe-only trigger (next migration) has something to compare against a floor.
--
-- CRITICAL: this is NOT a naive `CREATE OR REPLACE FUNCTION claim_sd(..., p_client_gate_version
-- integer DEFAULT NULL)`. Postgres treats a changed argument-type list as a SEPARATE overload --
-- that would leave BOTH claim_sd(text,text,text,boolean) and claim_sd(text,text,text,boolean,integer)
-- in the catalog. Every production caller (lib/claim-guard.mjs, lib/session-conflict-checker.mjs,
-- scripts/qf-start.js, scripts/worker-checkin.cjs) passes exactly 3 named args with all trailing
-- params defaulted -- with two matching overloads, PostgREST returns PGRST203 "could not choose
-- the best candidate function" and EVERY claim fails, reproducing the exact fleet-stall this SD
-- exists to prevent (risk-agent finding, sub_agent_execution_results 5a7bfbd9). Verified via
-- pg_depend that no view/rule/other function depends on claim_sd, so DROP+CREATE is safe.
--
-- The body below is the LIVE production claim_sd() definition (fetched via pg_get_functiondef
-- immediately before authoring this migration) with exactly ONE behavioral addition (the
-- claim_gate_client_version stamp, marked below) -- every existing guard, comment, and provenance
-- note is preserved verbatim so this migration is a pure additive change, not a silent
-- documentation-loss regression on the fleet's single most critical RPC.

ALTER TABLE strategic_directives_v2 ADD COLUMN IF NOT EXISTS claim_gate_client_version integer;
...
-- Prompt PostgREST to reload its schema cache promptly so the new 5-arg signature is
-- recognized without waiting for the next automatic cache refresh (risk-agent finding).
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'claim_sd: exactly one overload confirmed (5-arg, with p_client_gate_version).';
END $$;
