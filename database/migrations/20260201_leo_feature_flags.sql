-- Migration: Leo Feature Flags Foundation
-- SD: SD-LEO-SELF-IMPROVE-001D
-- Purpose: Create feature flag infrastructure for Phase 1.5
-- Author: Claude Code (SD-LEO-SELF-IMPROVE-001D)
...
CREATE INDEX IF NOT EXISTS idx_leo_feature_flags_flag_key
...
CREATE TABLE IF NOT EXISTS leo_feature_flag_policies (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_leo_feature_flag_policies_flag_env
...
CREATE TABLE IF NOT EXISTS leo_kill_switches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Insert CONST-009 kill switch for feature flags
...
-- Audit trail for flag changes (optional but recommended)
-- =============================================================================

CREATE TABLE IF NOT EXISTS leo_feature_flag_audit_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  flag_key TEXT NOT NULL,
  action TEXT NOT NULL, -- 'created', 'updated', 'deleted', 'policy_updated', 'kill_switch_activated', etc.
...
CREATE INDEX IF NOT EXISTS idx_leo_feature_flag_audit_log_flag_key
  ON leo_feature_flag_audit_log(flag_key, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_leo_feature_flag_audit_log_created_at
