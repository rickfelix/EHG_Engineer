...
-- Fixed: 2025-12-05 - Resolved aggregate function + set-returning function incompatibility
...
CREATE INDEX IF NOT EXISTS idx_ai_assessments_content ON ai_quality_assessments(content_type, content_id);
CREATE INDEX IF NOT EXISTS idx_ai_assessments_score ON ai_quality_assessments(weighted_score);
CREATE INDEX IF NOT EXISTS idx_ai_assessments_time ON ai_quality_assessments(assessed_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_assessments_model ON ai_quality_assessments(model);
CREATE INDEX IF NOT EXISTS idx_ai_assessments_passed ON ai_quality_assessments(content_type, (weighted_score >= 70));
...
-- FIXED: Use LATERAL join to avoid set-returning function in aggregate
...
FROM ai_quality_assessments,
  LATERAL jsonb_each(scores) AS criterion(criterion_name, criterion_data),
  LATERAL (SELECT (criterion_data->>'score')::numeric AS score) AS score_extract
...
SELECT
  content_type,
  criterion_name,
  COUNT(*) FILTER (WHERE score < 5) as fail_count,
  COUNT(*) as total_count,
  ROUND(100.0 * COUNT(*) FILTER (WHERE score < 5) / NULLIF(COUNT(*), 0), 1) as fail_rate_pct,
  ARRAY_AGG(DISTINCT content_id) FILTER (WHERE score < 5) as failing_content_ids
FROM ai_quality_assessments,
  LATERAL jsonb_each(scores) AS criterion(criterion_name, criterion_data),
  LATERAL (SELECT (criterion_data->>'score')::numeric AS score) AS score_extract
GROUP BY content_type, criterion_name
HAVING COUNT(*) FILTER (WHERE score < 5) > 0
...
DROP POLICY IF EXISTS "Allow read for authenticated" ON ai_quality_assessments;
DROP POLICY IF EXISTS "Allow all for service role" ON ai_quality_assessments;
DROP POLICY IF EXISTS "Allow read for anon" ON ai_quality_assessments;
