-- Migration: Fix retrospective trigger for embedding generation
-- Issue: SD-2025-1016-EMBEDDING-FIX
-- Created: 2025-10-16
--
-- Purpose: Fix auto_populate_retrospective_fields() trigger to allow embedding updates
-- Problems:
-- 1. Trigger references severity_level without NULL check
-- 2. Trigger enforces affected_components validation on ALL updates (blocks embedding generation)
-- 3. Trigger enforces quality_score validation on updates (blocks embedding updates)
--
-- Solution: Make validations conditional - only enforce on INSERT or when fields are being changed

BEGIN;

-- Drop and recreate trigger function with fixed validation logic
...
BEGIN
  ...
  -- Validation 2: CRITICAL/HIGH severity must have tags
  -- ONLY enforce if severity_level field exists and is being set to CRITICAL/HIGH
  -- Use EXISTS check for column to prevent "no field" errors
  IF TG_OP = 'INSERT' AND
     NEW.severity_level IS NOT NULL AND
     NEW.severity_level IN ('CRITICAL', 'HIGH') AND
...
'SD-RETRO-ENHANCE-001 Layer 2: Enhanced trigger with CONDITIONAL quality enforcement (fixed for embedding generation)';
...
BEGIN
  ...
  END IF;
  RAISE NOTICE '✅ Fixed trigger verified';
END $$;
...
-- Test 1: Embedding update should now work (doesn't change status or learning_category)
...
-- DROP TRIGGER IF EXISTS trigger_auto_populate_retrospective_fields ON retrospectives;
-- -- Then re-run original migration: 20251016_retrospective_quality_enforcement_layers_1_2.sql
