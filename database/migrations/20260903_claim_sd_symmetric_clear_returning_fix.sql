...
-- UPDATE sets `sd_key = NULL` and then reads `sd_key` back via `RETURNING sd_key INTO
-- v_evicted_sd_key` on the SAME statement. PostgreSQL's RETURNING clause always
-- reflects the row's values AFTER the UPDATE has applied them -- so `v_evicted_sd_key`
-- was NULL on every single call, structurally, regardless of what the row's sd_key had
-- been beforehand. The symmetric-clear block that depends on it (`IF v_evicted_sd_key
-- IS NOT NULL THEN ...`) could therefore never fire. Measured: 0 of 55 real claim
-- switches since the guard shipped (Solomon predicate addition 01982cf5, 2026-09-02).
-- CONFIRMED against the LIVE function via `pg_get_functiondef('public.claim_sd'::regproc)`
-- fetched fresh this session (.artifacts/claim_sd.live.current.sql) -- NOT the 9-day-stale
-- database/evidence/claim-surface-sync/claim_sd.before.sql snapshot, which VALIDATION
-- (LEAD phase) caught this migration's first draft had wrongly relied on: two guards
-- shipped by SD-LEO-INFRA-QF-CLAIM-PEER-GUARD-001 (2026-08-28, after that snapshot) --
-- the QF live-foreign-peer guard and quick_fixes.started_at stamp -- plus the
-- phantom-session guard (present in both the stale snapshot AND live, but dropped from
-- this migration's first draft by a plain copy error -- caught before this file was
-- applied anywhere) were all missing from that draft. This version is built from the
-- fresh live fetch with ONLY the two intended changes below; every other guard, branch,
-- and comment is preserved byte-for-byte.
--
-- FIX: capture the OLD sd_key with a plain SELECT, using the IDENTICAL predicate,
-- immediately BEFORE the destructive UPDATE -- not from the UPDATE's own RETURNING.
-- PostgreSQL 17.4 (this project's live version) has no `RETURNING OLD.col` syntax, so a
-- preceding SELECT is the correct, minimal fix.
--
-- OBSERVABILITY (success criterion #3 on this SD): a symmetric clear had NO durable
-- trail at all -- 0 of 53,034 session_lifecycle_events rows since 2026-02-01 record it,
-- and no event_type name mentions "switch"/"evict". Without an observable trail this
-- corrective is unverifiable in production after it ships (a passing unit test proves
-- the SQL shape, not that it ever actually ran). Adds one INSERT, gated on the clear
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'claim_sd: exactly one overload confirmed (5-arg, symmetric-clear RETURNING fix applied).';
END $$;
