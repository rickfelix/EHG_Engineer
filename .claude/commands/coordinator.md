...
6. **Chairman exec email every 30 minutes (default-on)**: `cron: "*/30 * * * *"`, `prompt: "node scripts/adam-exec-summary.mjs"`, `recurring: true` — operator is usually away; the chairman gets ONE Adam-lens exec summary (north-star + needs-you + 24h value + self-improvement + canary, with the coordinator fleet-health card embedded). Skips the send when nothing material changed (`.adam-email-last.json` signature). QF-20260609-024 retired the standalone `coordinator-email-summary.mjs` cron (that script remains as a dependency — `adam-exec-summary.mjs` reuses it in dry-run for the fleet card). **Live cutover** (one-time, coordinator session): `CronDelete` the old `node scripts/coordinator-email-summary.mjs` loop, then `CronCreate` this one.
