...
**DELEGATES TO**: `/sd-create` skill (`.claude/commands/sd-create.md`)

Parse the arguments and invoke the `sd-create` skill using the Skill tool:
```
Skill tool: skill="sd-create", args="<remaining arguments after 'create'>"
```

The sd-create skill handles vision readiness rubric, type inference, all 6 flag modes (interactive, --from-uat, --from-learn, --from-feedback, --from-plan, --child), and SD key generation via canonical scripts.
...
**DELEGATES TO**: `/leo-resume` skill (`.claude/commands/leo-resume.md`)

Invoke the `leo-resume` skill using the Skill tool:
```
Skill tool: skill="leo-resume"
```

The leo-resume skill handles session state restoration via the canonical `scripts/leo-resume.js` script, then loads the appropriate phase context.
