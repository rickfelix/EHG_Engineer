...
- No args → **Interactive menu** (context-aware main menu)
- `status` → Show current proving run status
- `report <venture-id>` → Generate report for a venture
- `discover` → Jump directly to opportunity discovery
- `resume` → Resume most recent proving run
...
ALL user interaction MUST use AskUserQuestion. NEVER auto-advance past a gate — the Chairman always decides.
...
(async () => {
  ...
  let stratScores = null;
  try { const res = await supabase.rpc('get_discovery_strategy_scores'); stratScores = res.data; } catch(e) {}
