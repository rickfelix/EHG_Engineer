...
hooks:
  PreToolUse:
    - once: true
      command: node /mnt/c/_EHG/EHG_Engineer/scripts/hooks/session-init.cjs
    - once: true
      command: node /mnt/c/_EHG/EHG_Engineer/scripts/hooks/capture-baseline-test-state.cjs
  PostToolUse:
    - command: node /mnt/c/_EHG/EHG_Engineer/scripts/hooks/model-tracking.cjs
    - command: node /mnt/c/_EHG/EHG_Engineer/scripts/hooks/persist-session-state.cjs
---
...
**Migration Note**: This agent has been migrated to .claude/skills/ for hot-reload support (SD-CLAUDE-CODE-2.1.0-LEO-001).
...
Use Playwright MCP for fast, interactive E2E testing during EXEC phase.
