...
(async () => {
  const { data, error } = await sb.from('sub_agent_execution_results').select('*')
    .eq('sd_id','SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-D').order('created_at',{ascending:true});
  ...
  for (const r of data) {
    ...
    console.log([r.phase||'-', r.sub_agent_code, r.verdict, (r.created_at||'').slice(0,19),
      'sha=' + (r.evaluated_commit_sha ? String(r.evaluated_commit_sha).slice(0,11) : (md.evaluated_commit_sha? String(md.evaluated_commit_sha).slice(0,11):'-')),
