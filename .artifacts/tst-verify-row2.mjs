...
const r = await s.from('sub_agent_execution_results')
  .select('id, sub_agent_code, verdict, phase, created_at, confidence_score, metadata')
...
for (const row of (r.data||[])) {
  console.log('---');
  console.log('id', row.id, '| verdict', row.verdict, '| phase', row.phase, '| created_at', row.created_at, '| conf', row.confidence_score);
...
const v = await s.from('v_sub_agent_repo_compliance').select('*').eq('sd_id','aaf65001-7031-46aa-882f-9f51281dc572').limit(3);
console.log('\ncompliance view:', v.error ? 'ERR '+v.error.message : JSON.stringify(v.data,null,1));
