...
const COMMIT = '9be8d42c39668a0e42c42d56a456279872a759e2';
...
// --- runner artifacts -------------------------------------------------------
const shippedPath = R + 'michael-d-shipped-v2.json';
const fullPath = R + 'unit-full-v2.json';
const floorPath = R + 'baseline-floor-v2.json';
const lintPath = R + 'eslint-shipped-v2.json';
...
let results = {
  ...
  summary,
  justification:
    'PASS: every shipped test file is green. 14 of 14 files, 250 of 250 tests passed with zero failures and zero skips, The full unit tier ran 48,796 tests with 48,583 passed and 2 failed; both failures are 60-second timeouts in tests/unit/eva/complexity-scorer.test.js, which touches nothing this child ships and passes 7 of 7 in 11.67 seconds in isolation at this same commit, so no shipped change regressed anything. ESLint is clean across all 28 shipped source and test files. 17 of the 19 PRD scenarios map to a named, labelled, passing test; TS-4 is covered in substance by six assertions across five files without a TS-4 label; TS-15 is a process smoke that no automated test performs, so it was measured directly with a runner script and passed on the clauses that are host-independent (exactly one JSON object on stdout, no API factory constructed) while its exit-0 clause is host-conditional for the two credentialed Google feeders. Neither shortfall is a failing test, and neither blocks EXEC-TO-PLAN: TS-15 is by construction the chairman post-migration host smoke.',
  findings: [
  ...
  metadata: {
    ...
    regression_full_unit_tier: {
      ...
      console_log_path: R + 'unit-full-v2.log',
