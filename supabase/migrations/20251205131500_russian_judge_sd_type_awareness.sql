-- Migration: Russian Judge SD Type Awareness (FIXED)
...
-- Fix: Changed created_at to assessed_at (correct column name)
...
WITH threshold_stats AS (
  ...
  FROM ai_quality_assessments
  WHERE sd_type IS NOT NULL
    AND assessed_at >= NOW() - INTERVAL '4 weeks'  -- Last 4 weeks (FIXED: assessed_at not created_at)
  GROUP BY sd_type, content_type, pass_threshold
)
SELECT
  sd_type,
