...
// ---------------------------------------------------------------------------
// TS-5 — the env-var arm counts READS, not WRITES
// (SD-LEO-INFRA-LOOP-STATE-AWAITING-001)
//
// A test that SETS SUPABASE_URL to a value it supplies has overridden the
// ambient credentials, which is the opposite of depending on them — it cannot
// hang against the test.invalid.local sentinel it just replaced. Three
// pre-existing unit tests were flagged for exactly this, one of them for
// setting the sentinel itself.
// ---------------------------------------------------------------------------
describe('TS-5: assignment is not a credential read', () => {
  it('does NOT flag env supplied to a child process', () => {
    const src = [
      "import { spawn } from 'node:child_process';",
      "describe('hook', () => { it('runs', () => {",
      '  spawn(node, [hook], { env: { ...process.env, SUPABASE_URL: stubUrl, SUPABASE_SERVICE_ROLE_KEY: fake } });',
      '}); });',
    ].join('\n');
    expect(classifyTestSource(src)).toBe('CLEAN');
  });

  it('does NOT flag an env var blanked out to force a failure path', () => {
    expect(classifyTestSource("const env = { SUPABASE_URL: '', SUPABASE_SERVICE_ROLE_KEY: '' };")).toBe('CLEAN');
  });

  // THE ARM THAT MUST STILL BITE. Without these three, "stop matching the env
  // vars entirely" would satisfy the two assertions above — the exclusion has
  // to be narrow enough that every real credential read is still caught.
  it('STILL flags a direct read of ambient credentials', () => {
    expect(classifyTestSource('const u = process.env.SUPABASE_URL;')).toBe('UNGUARDED');
    expect(classifyTestSource('if (!process.env.SUPABASE_SERVICE_ROLE_KEY) return;')).toBe('UNGUARDED');
  });

  it('STILL flags a comparison, which is a read and not a write', () => {
    // `==` must survive the lookahead — only a single `=` is an assignment.
    expect(classifyTestSource('if (SUPABASE_URL == other) {}')).toBe('UNGUARDED');
  });

  it('STILL flags a test that writes the env AND builds a real client', () => {
    // The factory arm is independent of the env arm, so overriding credentials
    // buys nothing if the file then constructs a live client in-process.
    const src = [
      "import { createSupabaseServiceClient } from '../../lib/supabase-client.js';",
      'const env = { SUPABASE_URL: mine };',
      'const c = createSupabaseServiceClient();',
    ].join('\n');
    expect(classifyTestSource(src)).toBe('UNGUARDED');
  });
