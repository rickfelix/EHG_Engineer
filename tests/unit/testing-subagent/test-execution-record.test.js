...
describe('buildTestExecution', () => {
  it('builds the canonical 10-field shape', () => {
    const te = buildTestExecution({ executed: 10, passed: 8, failed: 2, skipped: 0, artifactSha: 'abc123', runner: 'vitest', artifactPath: '/tmp/report.json', source: 'fresh', mappedCandidates: 4, foundFiles: 3 });
    expect(te).toEqual({ tests_executed: 10, tests_passed: 8, tests_failed: 2, tests_skipped: 0, artifact_sha: 'abc123', runner: 'vitest', artifact_path: '/tmp/report.json', source: 'fresh', mapped_candidates: 4, found_files: 3 });
  });

  it('defaults to a zeroed, non-measured record when called with no args', () => {
    const te = buildTestExecution();
    expect(te).toEqual({ tests_executed: 0, tests_passed: 0, tests_failed: 0, tests_skipped: 0, artifact_sha: null, runner: null, artifact_path: null, source: null, mapped_candidates: null, found_files: null });
