...
describe('gemini-pin-lint: full-sweep sanity against the real repo', () => {
  it('reports exactly the 23 KNOWN, not-yet-consolidated routing pins (allowlist covers the other 11 non-routing occurrences)', () => {
    // 23 is not "0 violations" -- that is the point at THIS stage of the SD. SD-LEO-ORCH-
    // GEMINI-MODEL-SCAN-001-E consolidated the creative-gen pin (lib/creative/providers/gemini.js),
    // dropping the count from 24 to 23. The remaining sibling subsystem-consolidation SDs
    // (SD-LEO-ORCH-GEMINI-MODEL-SCAN-001-D, -F, -G) migrate the rest onto model-config.js; the CI
    // gate flips from allow-fail to blocking only once this count reaches 0 (tracked on
    // SD-LEO-ORCH-GEMINI-MODEL-SCAN-001-G). A regression that ADDS a new unallowlisted pin, or an
    // allowlist entry drifting stale, changes this number -- which is exactly what this
    // pinned-count test is designed to catch.
    let output = '';
    try {
    ...
    expect(output).toMatch(/23 unallowlisted violation/);
  });
