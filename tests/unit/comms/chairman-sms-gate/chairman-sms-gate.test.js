...
// SD-LEO-INFRA-WIRE-CHAIRMAN-SMS-001: the new pre-send consult gate is an ORTHOGONAL concern to
// everything this file tests (rubric, transport, quiet-hours, parked-row resolution). A pass-
// through stub keeps these tests exercising only their own concern -- same precedent as this
// file's existing resolveChairmanZone/classifyOverAsk injectable seams. Harmless for tests that
// never reach the gate (blocked/held earlier for unrelated reasons).
...
describe('chairman-sms-gate sendChairmanSMS()', () => {
  ...
  it('TS-2: a well-formed sms-authority decision is sent exactly once via the injected sender', async () => {
    const sender = makeSender();
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { sender, console: silentConsole, runPreSendConsultLane });
  ...
  it('TS-3: a console-authority (spend) decision routes to console, not SMS', async () => {
    const sender = makeSender();
    const res = await sendChairmanSMS(wellFormedDecision({ authority: 'spend' }), DAYTIME, { sender, console: silentConsole, runPreSendConsultLane });
    expect(res.sent).toBe(false);
  ...
  it('TS-4: rubric unavailable (throws) fail-closes a decision — held + console-logged, never sent', async () => {
    ...
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { sender, evaluate: throwingEvaluate, console: silentConsole, runPreSendConsultLane });
    expect(res.sent).toBe(false);
    expect(res.held).toBe(true);
  ...
  it('TS-6: default sender DELEGATES to the -B durable path and fails SOFT (no throw) — SD-LEO-INFRA-ADAM-OUTBOUND-WIRE-LIVE-001 FR-2', async () => {
    ...
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { console: silentConsole, fallbackSend, runPreSendConsultLane });
  ...
  it('TS-7 (QF-20260719-509): an injected sender reporting the live-incident softFailed shape returns sent:false and fires the fallback', async () => {
    ...
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { sender, console: silentConsole, fallbackSend, runPreSendConsultLane });
    expect(res.sent).toBe(false);
    expect(res.transportFailed).toBe(true);
    expect(res.fallbackFired).toBe(true);
...
describe('FR-1/FR-4 (SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-141): chairmanZone resolved at the choke point, not per-caller', () => {
  ...
  it("omitting chairmanZone AND nowHourET resolves via the real choke-point fallback and is held on the default zone's quiet hours", async () => {
    ...
    const res = await sendChairmanSMS(wellFormedDecision(), ctx, { sender, console: silentConsole, resolveChairmanZone: stub, runPreSendConsultLane });
    expect(res.sent).toBe(false);
  ...
  it('supplying nowHourET (the shape all TS-1..TS-8 tests above use) short-circuits the fallback -- zero calls to resolveChairmanZone', async () => {
    const sender = makeSender();
    const stub = vi.fn(async () => ({ zone: 'America/Los_Angeles', source: 'test_stub' }));
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { sender, console: silentConsole, resolveChairmanZone: stub, runPreSendConsultLane });
    expect(res.sent).toBe(true);
...
describe('QF-20260818-263: a reply-class send auto-resolves parked chairman inbound', () => {
  ...
  it('a successful send with replyToInbound resolves all outstanding parked rows via the injected supabase', async () => {
    ...
    const res = await sendChairmanSMS(
      wellFormedDecision(), { ...DAYTIME, replyToInbound: true }, { sender, console: silentConsole, supabase: sb, runPreSendConsultLane }
    );
    expect(res.sent).toBe(true);
  ...
  it('a successful send WITHOUT replyToInbound never touches sms_relay_staging', async () => {
    const sender = makeSender();
    const sb = makeFakeSupabase();
    const res = await sendChairmanSMS(wellFormedDecision(), DAYTIME, { sender, console: silentConsole, supabase: sb, runPreSendConsultLane });
    expect(res.sent).toBe(true);
  ...
  it('a resolve failure is fail-soft — the send still reports sent:true, error logged not thrown', async () => {
    ...
    const res = await sendChairmanSMS(
      wellFormedDecision(), { ...DAYTIME, replyToInbound: true }, { sender, console: silentConsole, supabase: throwingSupabase, runPreSendConsultLane }
    );
    expect(res.sent).toBe(true);
