...
describe('extractAndPopulateDeliverables (QF-20260911-793 coordination_only threading)', () => {
  ...
  // class this SD exists to fix. These tests close that gap.
  //
  // PR #8703 adversarial review finding: the flag alone is author-writable JSONB with no
  // provenance -- any PRD author could exempt a real requirement by naming it one of the 3
  // reserved titles and setting coordination_only:true. Closed by additionally requiring the
  // WHOLE PRD's own metadata.generation_method match the one auto-generator that legitimately
  // emits these FRs. All fixtures below carry that provenance field except the dedicated
  // spoof-rejection test, which deliberately omits it.
  const PARENT_ORCHESTRATOR_PRD_METADATA = { generation_method: 'ParentOrchestratorHandler.generateParentPRD' };

  it('an FR with coordination_only:true, on a PRD generated by ParentOrchestratorHandler, produces a deliverable row carrying metadata.coordination_only:true', async () => {
    const supabase = makeSupabase();
    const frs = [{ id: 'FR-ORCHESTRATE-001', title: 'Child SD Orchestration', coordination_only: true }];
    const prd = { exec_checklist: DEFAULT_CHECKLIST, functional_requirements: frs, metadata: PARENT_ORCHESTRATOR_PRD_METADATA };
  ...
  it('an FR with no coordination_only key produces a deliverable row with metadata: {} (no accidental flag leakage)', async () => {
    const supabase = makeSupabase();
    const prd = { exec_checklist: DEFAULT_CHECKLIST, functional_requirements: FOUR_FRS, metadata: PARENT_ORCHESTRATOR_PRD_METADATA };
  ...
  it('an FR with coordination_only:true is NOT honored when the PRD was not generated by ParentOrchestratorHandler (closes the per-FR-flag spoof)', async () => {
    const supabase = makeSupabase();
    // Same exact reserved title + flag an attacker would use, but on a PRD with no
    // generation_method provenance (e.g. a human- or LLM-authored PRD via add-prd-to-database.js).
    const frs = [{ id: 'FR-SPOOF-001', title: 'Progress Tracking', coordination_only: true }];
    const prd = { exec_checklist: DEFAULT_CHECKLIST, functional_requirements: frs };

    const result = await extractAndPopulateDeliverables(SD_ID, prd, supabase, { silent: true, skipIfExists: true });

    expect(result.success).toBe(true);
    expect(result.deliverables[0].metadata).toEqual({});
  });
});
