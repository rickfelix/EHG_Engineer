...
describe('QF-20260815-288: the DR-rehearsal workflow sets the bypass on its push', () => {
  it('the "Commit runbook update" step exports EHG_ALLOW_MAIN_PUSH=1 on the git push line', () => {
    ...
    expect(pushLine).toContain('EHG_ALLOW_MAIN_PUSH=1');
    // The fallback must survive -- a future, unrelated push failure (network, conflict) should
    // still not fail the whole job.
    expect(pushLine).toContain('|| echo "Push skipped"');
  });
});
