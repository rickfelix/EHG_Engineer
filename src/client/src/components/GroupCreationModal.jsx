...
const GroupCreationModal = ({ isOpen = true, onClose, selectedSubmissions, combineMethod: initialMethod = 'intelligent', onCreateGroup, onSuccess }) => {
  const [groupName, setGroupName] = useState('');
  const [combineMethod, setCombineMethod] = useState(initialMethod);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create group using direct API call
      const response = await fetch('/api/sdip/create-group', {
        method: 'POST',
        headers: {
      ...
      if (onSuccess) {
        ...
        await onCreateGroup(result);
      }

      onClose();
      // Reset form
      setGroupName('');
      setCombineMethod(initialMethod);
    } catch (err) {
  ...
  const combineOptions = [
    { value: 'intelligent', label: '🧠 Intelligent', desc: 'AI-driven optimal combination' },
    { value: 'chronological', label: '📅 Chronological', desc: 'Keep time order' },
    { value: 'merge', label: '🔄 Merge & Dedupe', desc: 'Remove duplicates' },
    { value: 'priority', label: '⭐ Priority-based', desc: 'Highest strategic impact first' },
