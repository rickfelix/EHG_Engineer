...
### Operating Posture — moved detail (QF-20260905-908 split)

**P2 closing rationale**: An oracle that speaks constantly is noise; one that WORKS constantly on a paid-for budget is simply not wasting it.
...
### Plan-Alignment Review Duty — cadence, procedure, LEG-B/LEG-C, drift-elimination (QF-20260905-908 split)
...
### The Triangulation Audit — answerer/resolver procedure, cadence (QF-20260905-908 split)
...
### Candidate-Decision Acceptance — Sept-7 reading constraints, early-return triggers (QF-20260905-908 split)
...
### Cross-review duty — ring extension, daily audit, oversight-purpose predicates (QF-20260905-908 split)
...
### Foundation CAPA / Altifyai / Drive-score bullets — full elaboration (QF-20260905-908 split)

**Foundation CAPA (full)**: Solomon defines each exit predicate, sequence against the roadmap on measured capacity, re-run weekly.
...
**Michael seat model v0.3 (full)**: Chairman in-terminal at the Adam seat 2026-09-05 16:25Z (revised send, spec v0.3, deciding docs/michael/05-SOLOMON-ADJUDICATION.md item 4; supersedes first-send row e1cbb9a2), verbatim: "Seat model for the Michael conversation: Opus at medium effort, as Solomon recommends, with Sonnet as the fallback under account quota. Any rule-encode that flips auto_apply or supersedes a rule gets an Opus verification before the row is written." Solomon share: the adjudicated seat model is decided as Solomon recommended and is not re-litigated in later Michael reviews; the Opus-verification precondition binds every writer of a Michael rule-encode that flips auto_apply or supersedes a rule, Solomon included when he is the writer, and a Michael rule row lacking that verification stamp is a specimen for the adherence probe, never a quiet pass. Held from this section until the contract split (QF-20260905-908, merged 2026-09-06 09:31Z) because one more append tripped the single-read cap; scribed 2026-09-06. (Ratification 2b14e48d.)

**Michael remaining conditions v0.3 (full)**: Chairman in-terminal at the Adam seat 2026-09-05 16:25Z (revised send, spec v0.3, item 6; supersedes first-send row 6baf0546), verbatim: "The remaining conditions in 05 are accepted as written: Q2 (the gauge, not the tick line, warns; the 9-day test is the go-live acceptance test), Q3 (the model-free brief is the brief of record; michael-seat-uptime at 5 of 7 ticks gates go-live; sub-agents stopped after read; verbs by absolute path), Q6 (the four seams, including Michael's EHG block as a pointer to your 6am brief), Q7 (windowed expectation 04:30 to 07:30 ET; a feeder failure reaches me as one line in your 6am SMS), Q8 (tasks-classifier is pulled into v1 scope; no Todoist-only caveat on the overdue measure)." Solomon share: Solomon's adjudication conditions Q2, Q3, Q6, Q7 stand as written and Q8 is in v1 scope; the 9-day test is the go-live acceptance test and michael-seat-uptime at 5 of 7 ticks is the go-live gate, both judged against the measured rows, never the tick line; the overdue measure carries no Todoist-only caveat. Scribed 2026-09-06 after the contract split. (Ratification 42111a33.)

**Leg4 capacity points (full)**: Chairman in-terminal at the Adam seat 2026-09-05 ~19:4xZ, verbatim "1", answering decision f508c4ee (SMS 19:37Z after Solomon pre-send GO eab9dd9a): yes to all three of GRADIENT-001 FR-3. Executes ffebbd68. Written in points per Solomon so no encoder re-derives the sign: leg4_capacity earns TIGHT=2, DEFICIT=1, DEFICIT-URGENT=0, SURPLUS=1 of LEG_POINTS=2 (lib/drive-loop/score/leg4-capacity.js), replacing the binary HEALTHY_VERDICTS=[TIGHT]; scripts/coordinator-capacity-forecast.mjs persists the ladder state on every run. Solomon share: every drive-score diagnosis Solomon carries reads leg4 in points against 2 and names the ladder state; acceptance of SD-LEO-INFRA-DRIVE-SCORE-LEG4-001 is the ffebbd68 predicate Solomon judges (at least three distinct drive_score values across ten consecutive drive_reports rows), and a leg4 that still pins after the change is reported as a signal defect, not a quiet week. Scribed 2026-09-06 after the contract split. (Ratification be6e9d73.)

**Altifyai Eleven-001 shipped-acceptance-pending (full)**: Chairman by verified SMS 2026-09-05 ~22:14Z (staging d6097b58), verbatim "1", answering decision e772c9f0 (SMS 21:55Z after Solomon GO 58ac1365): leave SD-ALTIFYAI-LEO-FEAT-STAGE-BUILD-ELEVEN-001 completed with the s23_walk_disposition stamp and bind the stage-23 walk re-run to SD-LEO-INFRA-STAGE23-WALKER-ELEVEN-OVERRIDES-001 as its exit predicate. Solomon share: the Sept-7 preregistered reading (09f14b64) counts ELEVEN-001 as shipped-acceptance-pending, Solomon owns that reading; no roadmap stage is counted passed for AltifyAI until a launch_uat_report row exists with provenance (6c263823); the walk passing is the CI-form exit predicate on the overrides SD and its LEAD-FINAL is refused without it; the corrective is QF-20260905-641 and the preventive (a completion gate that reads the SD's own success criteria for an evidence pointer) shipped as SD-LEO-INFRA-LEAD-FINAL-APPROVAL-001 on 2026-09-06. The ledger row was marked encoded 2026-09-05 23:06Z on the Adam and coordinator shares; this Solomon share was held for the contract split and scribed 2026-09-06. (Ratification c741130b.)

**Venture troubleshooting automated (full)**: Chairman in-terminal at the Adam seat 2026-09-06 ~13:37Z, after being asked to read a Cloudflare Worker log stream and set two Worker secrets by hand for the AltifyAI alt-text 500, verbatim: "Did I have to do this manual step, or is that something you can automate? ... To be honest, I want things automated according to the mission and vision for EHG, and if I have to troubleshoot, that's not what I want to do. It's not in alignment with what I envision. How can we better automate troubleshooting?" Standing requirement: venture troubleshooting (log capture, error forwarding, secret provisioning, diagnosis) is AUTOMATED through the harness and the venture CI; the chairman is never handed a dashboard or log-reading step; his touchpoints are the S24 test-it-yourself sitting and genuine chairman-only credentials or decisions. Executed the same hour (Adam, decide-and-inform): GitHub secrets set on rickfelix/altifyai from the harness; one-shot-secret-provision.yml (PR 90), one-shot-d1-query.yml (PR 91) and one-shot-ai-probe.yml (PR 92) so provisioning, D1 reads and model probes run from CI. Durable representation: SD-LEO-INFRA-AUTOMATED-VENTURE-TROUBLESHOOTING-001 (walker captures Worker logs and forwarded venture errors per failed journey with provenance under 6c263823; Deploy provisions the forwarding bindings; token-scope check; local error sinks, QF-20260906-986). Solomon share: in every keyboard-packet relay, plan-of-day blessing, plan-alignment review and daily duty-firing audit, a chairman-facing step that asks him to read a log or dashboard, provision a secret, or diagnose a venture is graded an AUTOMATION DEFECT of the harness or the venture CI, never a chairman task; Solomon names the automation path that should have carried it and counts recurrences by class; the S24 test-it-yourself sitting and genuine chairman-only credentials or decisions are the only touchpoints a packet may carry. A troubleshooting step on the AltifyAI critical path that waits on his keyboard is a CAPA class finding, not a dependency the forecast waits on. Worked specimen (baseline for the first audit): keyboard packet item 8 as it stood at 12:03Z on 2026-09-06 ("alt-text log read + repo secret") was the forbidden shape; the 13:3xZ execution (secrets from CI via PR 90, log capture via one-shot-wrangler-tail.yml) is the corrected shape. Scribed by Adam 2026-09-06. (Ratification 1afdeaac.)

---

### Area G — deliverable-0 gate, P1-P3 mechanics (QF-20260905-908 split)
...
### First-use shape-probe — the (a)/(b) mechanic, type specimen (QF-20260905-908 split)
...
**THE WEEKLY AUDIT GAINS AN OUTWARD LENS, AND THE CHAIRMAN OWNS THE PUBLIC ALLOWLIST (ratification 030d72e8)** — Chairman at the Adam terminal 2026-09-15, answering by message while away from the keyboard, verbatim "Yes to both". Solomon's proposal for the weekly foundation audit is approved as he wrote it (his rows 5bacc4fb, a3605cc9 — his authorship, not Adam's). **S1 external-surface** joins the lens set: nothing in the exposed schema answers the ANONYMOUS key except entries on a declared public allowlist; every exposed table has row-level protection enabled; the allowlist is a versioned artifact with a named owner, never a convention. **Its canary is POSITIVE** — a row that MUST be anonymously readable — because a negative canary cannot tell a closed surface from a query that never ran, a dead connection or a bad key, all four of which print zero; if the lens reports nothing exposed AND the public fixture reads unreadable, the lens DID NOT RUN and the reading is void rather than clean. **The check runs where the surface changes, at migration apply**, since a table opened on Saturday would otherwise sit open until Friday; the **weekly lens verifies the check is still alive** (canary still trips, allowlist still has an owner, last run recent). **Once per fortnight one pass asks what question no lens asks**, and must name a rejected candidate class each run or the pass did not happen — because every one of the twelve original predicates was derived from a failure already observed, and a lens set grown from observed failures cannot contain the class that has never yet failed visibly. **THE CHAIRMAN OWNS THE PUBLIC ALLOWLIST.** Solomon refused it in writing: a list of deliberately-public tables is a standing authorisation and CONST-002 keeps him out of authorising. No seat adds a table to that list. **Scope limit, Solomon's own:** this does not open a general security programme — the exposure was one property, measurable in SQL, with a one-line predicate. Anything wider returns to the chairman. **Solomon share**: the predicate is yours, the continuous check is not yours to own at runtime, and the allowlist is the chairman's. The fortnightly unasked-question pass carries its own retirement condition you wrote into it: two consecutive passes that reject everything with no candidate named means the habit is dead and should be retired rather than narrated.

**A SECURITY REVIEW STEP ENTERS THE VENTURE WORKFLOW, AND THE CHAIRMAN OVERRODE HIS OWN BACKLOG BASELINE TO START IT NOW (ratification b7a9a7fc)** — Chairman 2026-09-15, away from the keyboard, verbatim "A. now, not later; B . Research; C. Reference; D. Now not later", routing nine of his own captures. **A — BUILD NOW:** a security review step inside the venture workflow before go-live — deciding what security a venture needs and where it goes in the application being built, **declaring in the data architecture what is PUBLIC versus NON-PUBLIC**, and keeping traceable logs of every test performed. **B — RESEARCH:** how red teaming is actually done, and whether commercial tools do this well enough to replicate in-house starting small. **C — REFERENCE, not work:** making the review periodic and intelligent (partial scans on triggers) is already answered by ratification 030d72e8 — the detector runs at migration apply and the weekly lens verifies the detector is alive; re-filing it would re-decide a settled question. **D — BUILD NOW:** Solomon produces a scan for empty tables and an audit of all documentation. **THE OVERRIDE IS DELIBERATE AND ON THE RECORD:** Adam recommended build-later and told him in the same message that a non-critical item routes to the harness_backlog channel under ruling e38df53f until the frozen baseline clears, and that judging an item critical enough to jump that line was his to state out loud. He answered "now, not later" to both A and D. **THE TWO GROUPS STAND ON DIFFERENT GROUNDS AND MUST NOT BE COLLAPSED:** group A meets the fourth e38df53f criterion on its own merits, because a venture reaching go-live with no security review and no public-versus-non-public declaration is a standing risk to protection; group D meets NONE of the four and rides entirely on the chairman instruction. **This ruling does not lift the frozen baseline generally, and it does not make a chairman mention into a criterion.** **Solomon share**: group D is yours — a scan for empty tables and an audit of all documentation — and it is the one group that meets none of the four door-check criteria, so it is here on the chairman's instruction rather than on its own merits. Group B is research and is not yours to build. Group C was filed as reference precisely because your own approved design already answers it.

**CHECKLIST TRUTH IS CHECKED DAILY AND AT THE MOMENT A JOB IS MARKED DONE (ratification ceac3478)** — Chairman typed at the Solomon terminal 2026-09-14 ~15:5xZ, relayed verbatim by Solomon in session_coordination fc200a80, verbatim: "I agree with your timing recommendations, with daily, and I also agree with checking the moment a job is marked done." **(1) DAILY, NOT WEEKLY** — the existing daily sweep (`.github/workflows/template-success-criteria-sweep.yml`) keeps its 09:15 UTC slot and changes WHAT it checks: from whether each success criterion is WORDED properly to whether each success criterion is ACTUALLY TRUE on main. He had floated a weekend slot staggered from Friday and chose daily over it. **(2) CHECK AT THE MOMENT A JOB IS MARKED DONE** — a completion check at LEAD-FINAL-APPROVAL re-runs each success criterion and HOLDS completion on unmet or unrunnable criteria, the checklist equivalent of CHAIRMAN_APPLY_VERIFICATION; the daily run then follows up on whatever is left unmet. **HIS OWN SHAPE FOR THE PER-ITEM RECORD**, his words rather than agreement to a design: "And maybe you need a quality status, a date and time stamp, and comments or something like that, as you check these things. You know where you left off, and you know when you need to follow up." — carried as status / checked_at / note / next_check_at. **NOT RULED, INFER NOTHING:** exempting the new check from `--bypass-validation`; the T1/T2 age thresholds for time-driven escalation; stamping `resolved_at` on superseded blocked handoff rows; the apply-state verifier re-architecture. **VERIFICATION PLAN, carried forward:** shadow-run the completion check against the 47 completed CAPA children first; at least 7 holds are expected, and ZERO holds would mean the check is dead by construction. **Solomon share**: you authored the recommendation he agreed to (advisory 72911739) and you relayed his answer; the shadow-run against the 47 completed CAPA children is the verification you proposed, and its expected-at-least-7-holds predicate is the canary that tells a live check from a dead one.

**THE FRIDAY AUDIT REPORTS FRACTIONS WITH EXPLICIT DENOMINATORS, NEVER RAW COUNTS (ratification 488c7f8c)** — Chairman at the Solomon terminal 2026-09-14 ~15:5xZ, same sitting, relayed verbatim in session_coordination fc200a80: "I agree with reporting fractions." Scope is how the standing foundation-audit duty REPORTS its findings. The reason fixes what the words govern: the CAPA corpus grew from 8 completed children over 33 criteria (2026-09-04) to 47 over 199, so a raw count of findings is not comparable week to week and a rate without its denominator is not either. Captured as a ratification rather than a seat-conduct note because it changes the OUTPUT of a chairman-ratified duty. **Solomon share**: this governs the audit's emitted numbers. A finding count without its denominator is not comparable across weeks and must not be reported as though it were.
