...
| `acknowledged_at` | Never answer a signal/message; assert `acknowledged_at` does not stamp on the row a reader would actually query. | Recipe only — deferred, investigation narrowed (not resolved) during QF-20260815-748. Several legitimate same-row claim-marker usages of `acknowledged_at` exist (`lib/coordinator/relay-queue.cjs` drain-claim, `lib/coordinator/signal-router.cjs` route-ack) that are NOT instances of this defect. `lib/adam/outbound-silence-watchdog.js` (Adam's own outbound-row breach detector) reads `acknowledged_at` on `sender_type='adam'` rows keyed by `target_session` — the exact "sender's own outbound row" shape the original finding named — but is ALREADY hardened against the most likely failure mode: `hasCorrelatedReply()` (added for `SD-LEO-INFRA-ACKSTAMP-FALSE-METRICS-C6-001`) treats a reply arriving as a NEW row (sent BY the target) as satisfying the check regardless of whether `acknowledged_at` itself was ever stamped. What remains open and NOT yet checked: whether anything ever legitimately stamps BOTH `read_at` AND `acknowledged_at` together on one of Adam's own outbound rows for the non-correlated-reply case (`isBreaching`'s `read_at` set + `acknowledged_at` unset branch implies some writer is expected to exist) — if no such writer exists, a read-but-fire-and-forget-acked message would breach forever; if one exists, confirm it targets the row the watchdog actually queries. |
