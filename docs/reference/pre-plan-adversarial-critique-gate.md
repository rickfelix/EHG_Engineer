...
## Severity aggregation — sufficiency threshold + decision-authority anchoring (SD-LEO-INFRA-CRITIQUE-GATE-NON-001)

Measured live: `plan_critiques` had **0 PASS verdicts in 375 runs** (2026-04-07..2026-09-03), 359 block (96%). Pure max()-over-findings meant a single low-stakes finding (LLM-labeled `block` even for an addressable gap like a missing acceptance criterion) force-blocked every handoff. `deriveCombinedSeverity()` (`pre-plan-critique.js`) now downgrades a solo block finding to `warn` UNLESS either (a) every co-occurring block finding is categorized `contradiction` or `missing_rollback` — the two categories the LLM's own rewritten rubric (`lib/eva/devils-advocate.js`) anchors to genuine decision-authority cost (would PLAN/EXEC need to escalate, or can they just fix it?) — or (b) there are ≥2 distinct (deduped) block findings. Anything NOT explicitly recognized as low-authority (`missing_criteria`/`scope_incoherence`/`reuse_opportunity`) — including an off-vocabulary category, a missing/malformed one, or a deterministic invariant-library finding — stays block-eligible by default (deny-by-default, fail-closed). This does NOT touch the PR #6927 anti-laundering rule: a block verdict with EMPTY findings never downgrades. Success criterion for this fix was DISCRIMINATION (a known-good plan passes, a known-bad plan still blocks), not the pass rate — validated against 3 live LLM-critique runs and, at production scale, by replaying all 359 historical block rows through the new logic: 219 (61%) still block, 140 (39%) downgrade.
