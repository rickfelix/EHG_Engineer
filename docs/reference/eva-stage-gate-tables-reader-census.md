...
| `eva-orchestrator.js` taste-gate block | `:1269` area (post-edit) | writer call site | Same outcome vocabulary; also fixed the pre-existing `details:`/`criteria:` param-name typo (TESTING F11) so `eva_stage_gate_results.gate_criteria` stops being silently empty for taste gates too. |
