...
- `idx_sub_agent_results_created_at`
  ...
  ```
- `idx_sub_agent_results_metadata_gin`
  ```sql
  CREATE INDEX idx_sub_agent_results_metadata_gin ON public.sub_agent_execution_results USING gin (metadata)
  ```
...
### 1. Allow read access to all users (SELECT)

- **Roles**: {public}
- **Using**: `true`

### 2. Allow service_role to delete sub_agent_execution_results (DELETE)
...
- **Timing**: BEFORE INSERT
- **Action**: `EXECUTE FUNCTION reject_control_chars_in_subagent_evidence()`
...
### trg_validate_testing_test_execution_ins

- **Timing**: BEFORE INSERT
- **Action**: `EXECUTE FUNCTION validate_testing_test_execution()`

### trg_validate_testing_test_execution_upd
