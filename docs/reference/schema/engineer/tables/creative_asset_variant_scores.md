...
- **Using**: `(EXISTS ( SELECT 1
  ...
  WHERE ((ca.id = creative_asset_variant_scores.creative_asset_id) AND (ca.venture_id IN ( SELECT v.id
           FROM ventures v
          WHERE (v.company_id IN ( SELECT user_company_access.company_id
                   FROM user_company_access
                  WHERE (user_company_access.user_id = auth.uid()))))) AND cavs_variant_matches_venture(creative_asset_variant_scores.variant_id, ca.venture_id))))`
