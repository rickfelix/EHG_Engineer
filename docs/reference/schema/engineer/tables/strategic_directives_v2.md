...
- `ck_strategic_directives_worktree_commit_pin_provenance`: CHECK (((worktree_commit_pin IS NULL) OR (worktree_commit_pin ~ '^.+@[0-9a-f]{7,40}$'::text) OR (worktree_commit_pin ~ '^.+@[0-9a-f]{7,40}~asof:\d{4}-\d{2}-\d{2}T'::text) OR (worktree_commit_pin ~ '^HISTORICAL:.+$'::text)))
...
### 3. strategic_directives_v2_service_role_access (ALL)

- **Roles**: {authenticated}
- **Using**: `fn_is_service_role()`
- **With Check**: `fn_is_service_role()`

### 4. venture_insert_strategic_directives_v2 (INSERT)

- **Roles**: {authenticated}
- **With Check**: `((venture_id IS NULL) OR fn_user_has_venture_access(venture_id))`

### 5. venture_select_strategic_directives_v2 (SELECT)

- **Roles**: {authenticated}
- **Using**: `((venture_id IS NULL) OR fn_user_has_venture_access(venture_id))`

### 6. venture_update_strategic_directives_v2 (UPDATE)
...
### tr_sd_completed_event

- **Timing**: AFTER UPDATE
- **Action**: `EXECUTE FUNCTION fn_emit_sd_completed_event()`

### trg_aaa_sync_type_change_reason
...
- **Timing**: AFTER UPDATE
- **Action**: `EXECUTE FUNCTION fn_auto_close_feedback_on_sd_completion()`

### trg_auto_close_quick_fixes_on_sd_completion

- **Timing**: AFTER UPDATE
- **Action**: `EXECUTE FUNCTION fn_auto_close_quick_fixes_on_sd_completion()`
...
### trg_enforce_sd_quality_advancement

- **Timing**: BEFORE UPDATE
- **Action**: `EXECUTE FUNCTION enforce_sd_quality_on_advancement()`
...
### trg_reset_patterns_on_sd_cancel

- **Timing**: AFTER UPDATE
- **Action**: `EXECUTE FUNCTION trg_fn_reset_patterns_on_sd_cancel()`

### trg_resolve_patterns_on_sd_complete

- **Timing**: AFTER UPDATE
- **Action**: `EXECUTE FUNCTION trg_fn_resolve_patterns_on_sd_complete()`
