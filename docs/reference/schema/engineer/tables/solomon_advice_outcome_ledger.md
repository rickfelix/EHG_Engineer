...
- `solomon_advice_outcome_ledger_decision_check`: CHECK ((decision = ANY (ARRAY['pending'::text, 'accepted'::text, 'rejected'::text, 'partial'::text, 'deferred'::text, 'superseded'::text])))
- `solomon_advice_outcome_ledger_defer_trigger_required`: CHECK (((decision <> 'deferred'::text) OR (defer_trigger IS NOT NULL)))
- `solomon_advice_outcome_ledger_outcome_check`: CHECK ((outcome = ANY (ARRAY['unknown'::text, 'shipped_clean'::text, 'reverted'::text, 'caused_rework'::text, 'not_applicable'::text, 'unmeasurable'::text])))
