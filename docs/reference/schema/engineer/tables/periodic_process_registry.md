...
- `periodic_process_registry_expected_window_et_shape_check`: CHECK (((expected_window_et IS NULL) OR ((jsonb_typeof(expected_window_et) = 'object'::text) AND ((expected_window_et ->> 'start'::text) ~ '^[0-2][0-9]:[0-5][0-9]$'::text) AND ((expected_window_et ->> 'end'::text) ~ '^[0-2][0-9]:[0-5][0-9]$'::text))))
