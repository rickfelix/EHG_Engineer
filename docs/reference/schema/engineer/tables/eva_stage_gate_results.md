...
| resolved_outcome | `text` | YES | - | SD-LEO-INFRA-S3-SOFT-GATE-REDESIGN-001 FR-5: eventual resolved outcome of this gate verdict (data foundation for empirical kill-threshold calibration; do not tighten before >=~50 resolved outcomes). |
| outcome_resolved_at | `timestamp with time zone` | YES | - | SD-LEO-INFRA-S3-SOFT-GATE-REDESIGN-001 FR-5: timestamp when resolved_outcome was set. |
...
- `eva_stage_gate_results_stage_number_check`: CHECK (((stage_number >= 1) AND (stage_number <= 27)))
