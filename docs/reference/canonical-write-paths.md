...
## Related: DB-side enforcement for strategic_directives_v2 (SD-LEO-INFRA-STRATEGIC-DIRECTIVES-CANONICAL-001)

The `strategic_directives_v2` row above documents the JS-side convention (handoff.js as the
declared canonical writer). SD-LEO-INFRA-STRATEGIC-DIRECTIVES-CANONICAL-001 adds an independent,
complementary DB-side backstop for the same table's lifecycle columns
(`status`/`current_phase`/`completion_date`): a BEFORE UPDATE trigger pair rejecting any write to
those columns that doesn't carry a registry-valid `lifecycle_write_token` stamp (custom SQLSTATE
`SDCW1`), with the registry allowlist in `public.sd_canonical_writer_policy()` — modeled on this
same file's convention, but enforced by Postgres itself rather than by JS-side discipline alone.

- Migration (staged, chairman-gated, **APPROVED but APPLY HELD** pending the 13 remaining writers
  being wired): `database/chairman-gated/20260824_strategic_directives_canonical_writer_choke.sql`
- Deploy sequence and operational runbook: `database/chairman-gated/README.md`
- Writer inventory (all 15 sites originally enumerated; 2 later superseded by QF-20260824-641's
  dead-code deletion, leaving 13 real stamped sites): `database/evidence/SD-LEO-INFRA-STRATEGIC-DIRECTIVES-CANONICAL-001-writer-inventory.md`
- This registry's `exempt_writers` list for `strategic_directives_v2` and the DB-side registry's
  allowlist are two SEPARATE, independently-maintained lists covering the same table from different
  layers (JS-side convention vs. DB-side enforcement) — a writer added to one is not automatically
