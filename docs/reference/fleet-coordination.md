...
**Session-scoped operator actions + model/effort chip** (SD-LEO-INFRA-LEO-COMPLETION-001-E): each worker row also shows a `Model/Effort` chip (e.g. `sonnet/xhigh`) sourced from `claude_sessions.metadata.model`/`.effort` (written by `lib/checkin/steps/model-effort-merge.cjs`; `-/-` when absent). `fleet-dashboard.cjs` also accepts three session-scoped commands, routed BEFORE the section dispatch since they take a second positional argument (session-id or callsign): `node scripts/fleet-dashboard.cjs detail <id>` (prints `lib/fleet/session-detail-view.js`'s context-usage/last-tool/attach-state view), `browser-request <id>` (prints `lib/fleet/browser-control.js`'s launch options — never launches a browser itself), and `takeover <id>` (pauses agent-driven browser actions for that session). These wire two previously-dead-code modules (built by completed SD-LEO-INFRA-SESSION-VIEW-BROWSER-001) into their first real production callers.
