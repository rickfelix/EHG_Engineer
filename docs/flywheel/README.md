...
| Sourcing engine is built (10/10 children) | **LIVE (built) + 1 of 3 CRONS ACTIVE (corrected 2026-08-24)** — SD-LEO-INFRA-SOURCING-ENGINE-CONSUMPTION-001 live-verified deployed GitHub Actions state via the API (not the local `process.env` probe this doc previously blamed for a "false dormant" — that theory was wrong): **gauge-gap-miner and deferred-watcher are `disabled_manually`**; only **auto-refill** is `active`. `sourcing_engine_activation_state` DB rows say `enabled=true` for all 3 — a real DB-vs-deployment SSOT inversion. Full breakdown: `docs/sourcing-engine-activation-runbook.md`'s FR-6 table. Re-enabling the 2 disabled arms is a chairman-gated decision, not yet made. |
