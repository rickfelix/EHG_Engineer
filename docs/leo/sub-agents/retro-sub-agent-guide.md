...
**Fixed (SD-LEO-INFRA-RETRO-PROMOTION-PATH-001)**: the RETRO sub-agent's enhance-path
(`lib/sub-agents/retro/db-operations.js`) previously refused `rich_existing_content` for ANY
sufficiently-long existing retro, regardless of whether it was auto-generated boilerplate or
genuinely manual content — measured against a live 150-row `retro_type=HANDOFF` sample, this
incorrectly refused ~99.3% of the population. `classifyRetro()`
(`scripts/modules/handoff/lib/retro-clobber-guard.js`) now only refuses when the existing content
is BOTH rich AND not auto-generated (`rich && !isAutoGenerated`); auto-generated content, however
long, is promotable. A second, independently-discovered bug was fixed in the same SD:
`isSafeToWriteRetro` always classified the SD's most-recently-created retro row rather than the
specific row `enhanceRetrospective` was about to overwrite — it now accepts an optional
`targetRowId` to classify the exact row in play (backward-compatible; the 7 other production
callers that don't pass it are unaffected). `scripts/generate-retrospective.js`'s fresh-INSERT
path remains a valid fallback but should no longer be needed for this failure mode.
...
|---------|-----|------|---------|
| 3.3.0 | SD-LEO-INFRA-RETRO-PROMOTION-PATH-001 | 2026-08-24 | Fixed `classifyRetro()` provenance/length conflation (`rich && !isAutoGenerated`) and `isSafeToWriteRetro`'s row-selection bug (`targetRowId` option) — closes the "Known gap" this doc previously described |
...
*Last Updated: 2026-08-24 | LEO Protocol v4.4.1*
