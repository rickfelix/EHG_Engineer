...
- **CONSUMES:** S-4 gauges; demand engine EXECUTE rail (assets are channel INPUTS); the `generateAsset` provider abstraction per the recovered dedicated spec (`visual-video-generation-satellite-spec.md`, component-SSOT): **RunwayML PRIMARY for both image and video (chairman ruling 07-10 ~4 PM), Gemini as the live fallback behind the abstraction** — never a direct provider call at a call site; S17 design-system artifacts as brand source. [§3.2 provider line corrected 07-11 per reconciliation — an earlier revision of this section named Gemini as the ratified image path, which predated recovery of the ruling.]
