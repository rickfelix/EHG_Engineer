...
- **Child C — Execution rail** (D2): wire publisher/provisioning to production; SEO-basics template requirements; outreach kit; graduated-autonomy + injection-floor rails. Depends on A (measure before you drive traffic).
  ...
  - **Shipped** (`SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001`): closes a dead-by-construction gap in the ladder above — `recordPublishOutcome`/`evaluateGraduation` had ZERO production callers before this SD, so no channel could ever graduate or demote regardless of real-world post outcomes. Root cause: `publish()` rebuilt a fresh `idempotencyKey` every call and used it to key both `checkPublishAuthorization`'s `correlationId` param AND the `campaign_content` dedup/upsert, so on an approval-gated attempt #2+ the ledger's "already accepted" branch returned the ORIGINAL propose-time `correlation_id` while the dedup key was a different, freshly-rebuilt one — the join this whole SD depends on (`venture_channel_publish_ledger.correlation_id = campaign_content.idempotency_key`) was unreachable. Fixed in `publisher/index.js` (`dispatchKey = authCheck.correlationId || idempotencyKey`). New scheduled step `scripts/cron/publish-outcome-observer.mjs` (30-min cadence, `.github/workflows/publish-outcome-observer-cron.yml`) sweeps `outcome='unknown'` ledger rows, joins to `campaign_content`, and classifies each via a real X/Bluesky lookup (`lib/marketing/observer/observe-outcome.js`) — never self-reported by `publish()` itself, per the ledger's own anti-pattern warning. Classification is a strict 3-way "never guess": a real X `GET /2/tweets/:id` returns HTTP 200 with an `errors[]` array (no 404) for a deleted post, and AT Protocol returns HTTP 400 + `RecordNotFound` (not a plain 404) — both adapters were hardened to recognize their platform's actual not-found shape rather than asserting `exists:true` from HTTP-ok alone. A fixed 24h `OBSERVATION_WINDOW_MS` keeps a join-miss `'unknown'` (retried) rather than a premature terminal `'unmeasurable'`, since the approval-gated path's chairman-approval-to-publish() retry is human-paced and can lag by hours. `evaluateGraduation` is now mode-scoped (`execution_mode='mock'|'live'`, fail-closed on any non-`'live'`/non-omitted value) so a mock-mode outcome can never graduate or demote the real `venture_channel_autonomy` row — closing a SECURITY finding where the mode-guard's first fix attempt was itself fail-open for `null`/`undefined`/`'dry_run'`.
