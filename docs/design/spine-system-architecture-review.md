...
## 10. §7 verify-first — CLOSED (2026-07-11, SD-LEO-ORCH-OPERATING-COMPANY-SPINE-001-A)

The §7 run-before-you-build proof is complete: one seeded end-to-end thread (fixture CEO+4 VPs+14 crew created via the EXISTING, unmodified `venture-ceo-factory.js` path, one `agent_messages` row claimed, one real budget-checked decision, one real calibration row written and resolved) ran against real live tables. Two hard gaps this proof surfaced along the way — `agent_budgets`/`agent_budget_logs` were referenced by `budget-manager.js` but never migrated, and `truth-layer.js`'s calibration writer was a deliberate in-memory no-op — are fixed with additive migrations, not by inventing an identity/authority substrate (that stays Child B's scope; zero diff inside `_createAgent()`/`_grantTools()`, confirmed against `origin/main`). PR #5969 (deep-tier adversarial review caught and fixed a CRITICAL RLS gap on the two new tables before merge). The factory's good bones are now proven real — Children C-H (the six satellites) may take a dependency on the fold-in.
