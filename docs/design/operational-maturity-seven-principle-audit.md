...
**Provenance note (re-materialized):** the shared-tree original was destroyed in the 2026-07-11 evening untracked-file loss; this copy is re-emitted verbatim from the authoring Solomon session, including the P4 RESOLVED stamp applied in-tree by the SD-LEO-INFRA-VENTURE-OPS-ACTUALS-001 implementing worker. Any other post-authoring in-tree stamps applied after ~15:30Z may have been lost with the original.
...
| 4 | Wire ops collectors + fix service_telemetry writer/reader mismatch + external uptime probe on venture URLs | P4 | Small SD | **SHIPPED — SD-LEO-INFRA-VENTURE-OPS-ACTUALS-001 / PR #5924** |
