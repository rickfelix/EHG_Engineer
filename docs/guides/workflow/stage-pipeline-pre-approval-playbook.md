...
These nine rules are non-negotiable. They surfaced during S18–S19 work and will be applied on every subsequent stage.
...
9. **Constrain LLM capability-decision steps to portfolio-default capabilities.** When a stage prompts an LLM to enumerate venture capabilities (Stage 19 sprint planning today; potentially future stages like S20/S22), the prompt must inject the EHG portfolio-default capabilities as MANDATORY items — not leave them implicit. **Why:** the *Quality Lifecycle System Vision* (`docs/reference/vision/quality-lifecycle-system.md`) mandates that every venture ship with (a) a feedback widget, (b) error capture middleware, and (c) API integration to the central `feedback` table — so chairman-level cross-portfolio quality visibility actually works. Without prompt-level enforcement, Stage 19 produces sprint plans that omit these capabilities entirely (verified 2026-04-28 against LexiGuard's regenerated sprint plan, artifact `7f107870-0314-4860-a2cc-4197f0fa31ad` — zero items related to feedback or error capture). One blind spot per venture compounds into portfolio-wide opacity. **What to verify in pre-approval review:** (a) any LLM-driven capability list has a hard constraint pointing at the default-capabilities config; (b) deviation requires an explicit `override_reason` (e.g., B2B-only venture with no end-user UI); (c) the §1.0 pre-approval check includes "default-capabilities adherence" alongside house-stack adherence; (d) the Plan Mode binding contract surfaces `docs/monitoring.md` (a path within the seeded venture repo, not this repo) so Replit Agent reads the existing Sentry+feedback integration spec when planning. **Sibling pattern to Rule 8** — same vision-exists-but-prompt-silent defect class. Tracked: **SD-LEO-ENH-CONSTRAIN-STAGE-EMIT-001** (status=draft, awaiting LEAD approval).
...
- [ ] **Default-capabilities adherence verified.** For Stage 19 (Sprint Planning) artifacts, confirm `sprintItems` contains both `EHG_VENTURE_DEFAULT_CAPABILITIES` entries (feedback widget + error capture middleware) defined in `lib/eva/config/venture-default-capabilities.js`, OR `default_capabilities_override.<capability_id>.override_reason` is populated for each missing entry (non-empty trimmed string — null/empty fails closed). See Rule 9 of Section 0 for rationale. Use `node scripts/one-off/audit-venture-default-capabilities.mjs` for a portfolio-wide read-only audit. The Plan Mode binding contract surfaces `docs/monitoring.md` (venture-repo path) automatically, so Replit Agent reads the Sentry+feedback integration spec when planning.
...
- 6 methodology rules from §0 surfaced during S18 debugging; Rule 7 (*ask the consumer*) was retro-added during S19 Replit Agent kickoff; Rule 8 (*constrain LLM stack decisions to a house tech stack*) was retro-added 2026-04-28 reviewing LexiGuard's Stage 14 output; Rule 9 (*constrain LLM capability decisions to portfolio defaults*) was retro-added 2026-04-28 after re-deriving LexiGuard's Stage 19 sprint plan and observing zero feedback/error-capture items despite the Quality Lifecycle System Vision mandating them.
...
- **GitHub repo seeding (`lib/eva/bridge/replit-repo-seeder.js`)** packages (all `docs/*` paths below are within the seeded venture repo, not this repo): `docs/designs/<screen>.html` from `stage_17_approved_desktop` (one HTML file per approved S17 design), `docs/marketing-copy.md` from all 9 S18 `marketing_*` artifacts, `docs/product-roadmap.md`, `docs/gtm-strategy.md`, `docs/color-palette.md`, plus the main `replit.md` body built from idea-brief / persona / competitive-analysis / wireframes. **The marketing-first contract is enforced at the seeder layer** — Replit Agent receives the chairman-approved copy AND the approved HTML designs as binding reference material.
