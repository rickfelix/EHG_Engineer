...
| **Spend guardrails** | The 8-point spend-guardrail policy (`lib/venture-deploy/spend-guardrails.js`) is a **hard precondition** before a Cloudflare-default venture goes live (D1 has no hard dollar cap — runaway-invoice risk). Enforced as the fail-closed S19 exit gate **`spend guardrails ready`** (see Enforcement). | n/a | — |
...
- **Fail-closed S19 spend-guardrail gate (SD-LEO-INFRA-VENTURE-CLOUDFLARE-DEFAULT-001-D):** the 8-point spend-guardrail policy is carried forward from the Cloudflare research's *conditional* go (no provider hard dollar cap; D1 runaway-invoice risk) as a **hard precondition** at go-live. It is enforced as the S19 exit gate string **`spend guardrails ready`** (`venture_stages.metadata.gates.exit`, migration `20260627_s19_spend_guardrails_exit_gate.sql`), which `lib/eva/lifecycle/exit-gate-enforcer.js` dispatches to the fail-closed `verifySpendGuardrailsReady` verifier (`lib/eva/lifecycle/exit-gate-verifiers.js`). A venture cannot advance past S19 unless a `venture_guardrail_state` row exists for **all 8** canonical guardrails AND every `decision='allow'` AND no kill-switch is open — closing the gap where the verifier existed but was never dispatched (the precondition had only been enforced implicitly inside `lib/venture-deploy/publish.js`).
