...
| `compounding` | The Compounding-Prerequisite Director | Does it unlock downstream / is it a prerequisite | Is this a keystone other work depends on? Does shipping it raise a weak gauge capability? Or is it a leaf with no compounding? | **COMPOSE** `lib/adam/gauge-lens.js` (`readCapabilityGaps` → per-capability build%). |
