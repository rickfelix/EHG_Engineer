...
5. **Cross-Session Marker Scoping** (SD-LEO-INFRA-FIX-SESSION-REGISTER-001): a distinct hook, `scripts/hooks/session-register.cjs` (the `SessionStart` upsert-into-`claude_sessions` hook, separate from `resolveOwnSession()` above), previously fell back to the most-recently-modified file under `.claude/session-identity/*.json` with NO hostname/PID scoping whenever `CLAUDE_SESSION_ID` was unset in-process — the normal case for `SessionStart:compact`. This let one session's compact-hook read an unrelated session's marker and smear its own hostname/tty onto that foreign `session_id`, which a downstream ad-hoc remediation then judged "phantom" and full-REPLACED the row's `metadata`, wiping `fleet_identity`/`tier_rank`/`auto_proceed` and incorrectly stranding the session as `non_fleet`-blocked. The hook now delegates to `lib/hooks/session-id.cjs`'s `resolveSessionId()` (stdin → env → PID-scoped marker → mtime-fallback-as-last-resort), matching the same PID-scoping discipline as `resolveOwnSession()`'s Strategy 2 above. `lib/session-writer.cjs::markSessionNonFleet()` is now the canonical merge-preserving write path for non_fleet remediation (never a full metadata replace), and `lib/claim/build-forbidden-session.cjs::isBuildForbiddenSession()` logs an anomaly warning on a bare `non_fleet:true` with no `role` key — the fingerprint of this incident class.
