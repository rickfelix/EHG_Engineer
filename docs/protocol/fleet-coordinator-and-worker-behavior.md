...
**UPDATE (SD-LEO-INFRA-FLEET-DOWN-PAGER-001): the overcount above is now partly closed, and the claim-based workaround this paragraph recommends was the wrong discriminator.** Gating on `sd_key` fails exactly when it matters: the stale-session sweep RELEASES a frozen seat's claim, so a claim-derived filter goes blind as the condition matures (measured: claim-derived predicates agreed with ground truth 0/6 and 1/6 on the six specimens behind `lib/fleet/stuck-seat-predicate.cjs`). The honest discriminator is **tool advancement** — `last_tool_at` is the only tick-immune activity clock, because the tick daemon described above cannot forge it. `liveFleetWorkers()` (`lib/fleet/genuine-worker.mjs`) now excludes a seat that is *positively known wedged*: tool-silent past `FREEZE_CUT_MINUTES` (default 120, env `FLEET_FREEZE_CUT_MINUTES`, floored at 15) **and** either `loop_state='active'` (stuck mid-iteration) or `loop_state='awaiting_tick'` with its recorded wake deadline already passed. The verdict is delegated to the shipped `lib/fleet/stuck-seat-predicate.cjs` — required by module path, since `scripts/reconcile-seats.cjs:62` exports a *different* `classifySeat` that actuates via `markReleased`.

**Why this matters for the pager, and the trap it removes:** `scripts/fleet-down-alert.mjs` pages only when `active_count` reaches **0**. While the count was heartbeat-derived, zero was reachable only by sessions *vanishing*, never by them *freezing* — so a total freeze, the outage the alerter exists for, would have recorded a full fleet and paged nobody.

**The term FAILS OPEN and only some callers opt in.** An absent/unparseable `last_tool_at`, absent `loop_state`, missing wake deadline, or thrown verdict all leave the seat counted — only a positive STUCK verdict removes one. Consequently a caller that does not SELECT those two columns silently keeps the old heartbeat-only behaviour. Freeze-aware today: `scripts/fleet-worker-pulse.mjs` (and therefore the pager), `scripts/adam-exec-summary.mjs`, plus `adam-coordinator-health.mjs` / `coordinator-idle-qf-hint.mjs` incidentally via `select('*')`. **Still column-blind**: `coordinator-audit.mjs`, `coordinator-email-summary.mjs`, `live-fleet-sessions.cjs`, `tier-ladder.cjs`, `tier-backlog.cjs`. To opt one in, spread `FREEZE_TERM_COLUMNS` from `lib/fleet/genuine-worker.mjs` into its select — nothing enforces this today, as `scripts/lint/fleet-liveness-select-lint.mjs` checks row-cap bounding only and never inspects column lists.

**UPDATE (SD-LEO-INFRA-PERMISSION-FREEZE-STUCK-001): a STUCK verdict now carries a name, not just a session_id prefix, and reaches the coordinator's owed-actions surface, not only the pull-only `fleet-dashboard.cjs` CLI.** `classifySeat()` gained a pure identity passthrough (`window_handle`, `fleet_identity` — read straight from `session.metadata`, never a classification input, so this does not reintroduce the claim-derived-field defect this module exists to avoid). `lib/governance/drive-state/axes/fleet-health.cjs`'s `fleet_health` axis citation now lists ALL stuck seats (not the prior `slice(0,3)`) with resolved identity — `fleet_identity` is an OBJECT on live rows (`{role, color, callsign, ...}`, confirmed 28/28), so the citation reads `.callsign`/`.display_name` via `lib/fleet/fleet-identity-label.cjs`, never the raw object. A new `lib/fleet/stuck-seat-keystroke-packet.cjs` renders a numbered, chairman-ready recovery line per stuck seat, appended (unmangled, bypassing the shared 160-char `safeCitation()` cap) to the existing `recover_stuck_seat` owed-action lane row's body — the same `adam_action_required` surface `lib/governance/drive-state/owed-actions.cjs` already registers, not a new axis or act type. Separately, `scripts/hooks/pre-tool-enforce.cjs`'s `current_tool` PreToolUse telemetry writer — disabled since 2026-08-01 by a session-id derivation bug — was repaired, so `current_tool` now actually populates during a live tool call instead of silently no-op'ing.
...
| Worker-signal receipts: EVERY worker signal (not just severity=high `stuck`) gets a coordinator inbox receipt with a rendered disposition line (`routed:harness_backlog` \| `needs-decision:coordinator` \| `queued`), so a worker checking its own outstanding signals can see the coordinator has enumerated it, distinct from having answered it (SD-LEO-INFRA-COORDINATOR-RECEIPTS-BROADCAST-CONSTRAINTS-001 FR-1) | `scripts/fleet-dashboard.cjs` (`writeSignalReceipts`, `signalReceiptDisposition`) → `lib/fleet/outstanding-signals.cjs` (`received` field, `countUnreceiptedOverdue` gauge input) |
| Capped-pool broadcast: when the worktree pool has been over cap for >30 minutes, ONE directed INFO row per live seat fans out fleet-wide (dedupe is emission-recency-based via a state file, never acknowledged_at-based, so one seat acking never triggers a resend), replacing the originally-specified but confirmed-dead `target_session='broadcast'` mechanism (SD-LEO-INFRA-COORDINATOR-RECEIPTS-BROADCAST-CONSTRAINTS-001 FR-2) | `lib/coordinator/capped-pool-broadcast.cjs` (`decide`/`tick`/`emitToLiveSeats`) → wired into `scripts/stale-session-sweep.cjs`'s sweep tick |
| WORK_ASSIGNMENT CONSTRAINTS block: every WORK_ASSIGNMENT dispatch is stamped with a `CONSTRAINTS:` block naming the target SD's ratifications, resolvable holds, governance-note metadata, and any Solomon ruling text, so a worker reads them BEFORE the first edit rather than discovering a load-bearing rule after a draft has already leaned the other way (SD-LEO-INFRA-COORDINATOR-RECEIPTS-BROADCAST-CONSTRAINTS-001 FR-3/FR-4) | `lib/coordinator/dispatch.cjs` (`stampConstraintsBlock`) — reads the four governance-note keys DIRECTLY, never via the shared `resolveHoldProvenance` (that function's own key set is unchanged; see its docblock in `lib/fleet/claim-eligibility.cjs` for why widening it was reverted) |
