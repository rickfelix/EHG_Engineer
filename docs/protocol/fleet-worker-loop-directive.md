...
5. AUTO-PROCEED is ON — do not stop for confirmation; only pause on the canonical pause points in CLAUDE.md. THE LOOP'S UNIT OF WORK IS ONE COMPLETE WORK ITEM — a strategic directive driven through ALL its handoffs (LEAD → PLAN → EXEC → PLAN_VERIFICATION → LEAD_FINAL) to completion, OR a quick fix worked to completion — NEVER a single handoff or phase. A phase boundary is NOT a pause point and is NOT the end of an iteration: flow from each handoff straight into the next IN THE SAME TURN, and do NOT arm a ScheduleWakeup between the handoffs of an active item. ScheduleWakeup is an idle/wait mechanism (step 6), not a pacing mechanism for active work — arm it ONLY at a genuine boundary: the item is complete AND the belt is empty, or you are blocked waiting on an external event (CI, a coordinator reply). When you hold a claimed item with an immediately-actionable next handoff, keep working — never end the turn there.
6. On completion, /signal a FLEET-RETRO, run the post-completion tail, then claim the next workable SD IN THIS SAME TURN and start building it — never park between SDs while the belt is non-empty (KPI: median completion→next-claim ≤3 min, p90 ≤8 min). Park ONLY when the belt is genuinely EMPTY: **first PUSH any WIP** — before arming the ScheduleWakeup, commit your in-progress work on the claim-bound branch AND `git push` it (or run `node scripts/prepark-wip.cjs`) so a sweep-driven claim release + peer re-route can RESUME from your branch instead of orphaning the unpushed partial commit (the worktree survives, but a fresh re-route worktree can't see an unpushed commit) — THEN ScheduleWakeup ~5 min (≈300s crash-recovery heartbeat, NOT the work trigger) and re-check (do NOT stop to ask — there is no human in the loop window). This same-turn rule is mirrored in the `[ROLE]` WORKER block (`scripts/hooks/session-role-orient.cjs`) — keep the two in sync. On loop stop (no work / told to stop / winding down), /signal feedback "offline — <reason>" so the coordinator reflects the exit instead of a silent disappearance. (SD-FDBK-INFRA-AUTO-PUSH-WIP-001, SD-MAN-INFRA-SAME-TURN-NEXT-001)
7. WIND-DOWN HANDSHAKE (before you finish an SD or go idle — operator 2026-06-10). Two hard rules so you never strand work or vanish without warning: (a) NEVER drop an in-progress SD to claim a different one — FINISH it through LEAD_FINAL or hand it off explicitly; a half-done SD left `in_progress` + unclaimed is an orphan the next worker can't safely resume (live example: REVIVE-EVA-MASTER stranded at EXEC 87%). (b) Before going quiet, give the coordinator a GRACE WINDOW: `/signal feedback "winding down — finished <SD>, anything queued for me? idling ~180s"`, then arm a SHORT `ScheduleWakeup` (~180s) and on that next tick RE-CHECK your inbox for a coordinator reply/assignment BEFORE you settle into the ~5min (≈300s) idle cadence. Announce → grace → idle; never disappear mid-stream. Enforced (backstop) by the Stop hook `stop-loop-wakeup-reminder.cjs`, which now also fires for a claim-holding worker whose loop_state never entered the machine.
