...
### Write sites (14) — already-correct or exempt, each cited
...
| `scripts/fleet-dashboard.cjs` `printInbox` (coordinator-audience signal inbox render) | already-correct (idempotency gate added 2026-09-07) | `read_at`-only, defers ack to explicit ack commands. Was UNCONDITIONAL (no `read_at IS NULL` gate) until SD-LEO-INFRA-READ-WRITTEN-UNSCOPED-001 — every render overwrote `read_at` to now(), drifting it from a first-delivery timestamp to a most-recently-rendered one, inconsistent with every other site in this table. Verified safe to gate: the SELECT feeding this write already gates on `acknowledged_at IS NULL`, not `read_at` (the drain-on-display RCA this write's header comment cites was fixed via that SELECT column, not the UPDATE's unconditionality), so adding the gate cannot reopen it. |
| `scripts/fleet-dashboard.cjs` `printWorkerInbox` (worker-audience own-inbox render) | already-correct | genuinely read-only — no write of any kind; advisory path additionally documents the `payload.actioned_at` dual-marker (see receipt-contract table above) |
...
| ~~`lib/coordinator/signal-router.cjs` `stampRouted`~~ REMOVED (SD-LEO-INFRA-SIGNAL-LANE-PER-001, 2026-08-24) | n/a — file no longer writes `acknowledged_at` | This row's original claim ("acks on genuine promotion-to-`feedback` action") was already stale when written: `stampRouted()` never touched `acknowledged_at` — that write lived in a SEPARATE, then-dormant function, `stampRoutedToCoordinator()` (SD-LEO-INFRA-SIGNAL-ROUTER-AUTO-001, FR-7), which this census apparently conflated with `stampRouted`. FR-4 fixed `stampRoutedToCoordinator` (made it a non-disposing provenance marker, mirroring `stampRouted`'s already-correct shape, and fixed the `loadRecentSignals` select that made its own idempotency guard unreachable) but did NOT give it a production caller — it still has none, deliberately (that decision belongs to whichever future SD wires it in, given the sweep-legacy-twin-parity test surface). It never writes `acknowledged_at` either way. A genuine FR-1 disposition is written by `scripts/coordinator-ack-signal.cjs` alone (already listed above); the one narrow, explicitly-scoped exception is `scripts/one-off/signal-lane-backfill-001.mjs` (FR-3's one-time historical backfill), which stamps `acknowledged_at` but always pairs it with `disposition:null` + `isRetention:true` — never a real FR-1 disposition value — and is idempotent, so it cannot become an ongoing parallel closing mechanism. |

| `scripts/michael-inbox.cjs` `drainInbox` | already-correct (new write, added 2026-09-07) | SD-LEO-INFRA-MICHAEL-ADAM-COMMS-001 (FR-1) — `drainInbox()` previously read unread rows (`read_at IS NULL`) but never wrote back any lifecycle column, so the same rows resurfaced on every invocation forever despite the function existing and being called from Michael's own quiet-tick guidance. Fix: stamps `read_at` on every recognized-kind row (mirrors `scripts/solomon-advisory.cjs`'s `stampSurfaced`), and deliberately never stamps orphaned/unrecognized-kind rows, which must stay visible per the file's own "surface, never silently consume" contract. `scripts/michael-quiet-tick.mjs`'s inbox-nudge counter (FR-2) was independently keyed on `acknowledged_at` — switched to `read_at` so the two agree; Michael has no two-stage ack model (unlike Solomon/Adam) and no background/cron split (unlike Adam's `SD-LEO-INFRA-ADAM-INBOX-SURFACE-NOT-STAMP-001`), so `read_at` is the single correct lifecycle column for this role. |

| `lib/coordinator/adam-advisory-store.cjs` `stampActioned` | already-correct (new write, added 2026-08-30) | QF-20260830-084 — `stampActioned` previously wrote only `payload.actioned_at`, while the lane-pending gauge (`lib/coordination/lane-pending-gauge.cjs` `summarizePendingLane`) and other `acknowledged_at`-null-scoped readers used a DIFFERENT retirement predicate on the same rows — two predicates on one row, by construction. Measured: 46 fully-actioned `adam_advisory` rows read permanently pending in the lane counter, up to 29h. Fix: `stampActioned` now also writes `acknowledged_at` (same update call, same timestamp) so every downstream `acknowledged_at` reader sees the same retirement moment as the advisory-lane's own `payload.actioned_at` selector — this does NOT change the canonical advisory selector above, which still reads `payload->>actioned_at IS NULL` unchanged. |
