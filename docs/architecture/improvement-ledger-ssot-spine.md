...
- **Loop E scope gap (opened 2026-07-04, SD-LEO-INFRA-ROLE-RUBRIC-SCORE-001)**: this view's `WHERE category = 'adam_self_assessment'` predicate is now under-scoped. That SD extended the role-self-review pattern to two more roles, each emitting the same common score schema under its own category: `coordinator_self_assessment` (`scripts/coordinator-self-review.mjs`, flag `COORD_SELF_SCORE_V1`) and `solomon_self_assessment` (`scripts/solomon-self-assessment-writer.cjs`, flag `SOLOMON_SELF_SCORE_CADENCE`) — both currently default-OFF, so this gap is dormant until the paired live-enablement follow-up flips those flags (plus the 3 `*_self_score_age` gauge-registry entries) together. When that follow-up ships, this view's three `SELECT ... FROM feedback WHERE category = 'adam_self_assessment'` branches (`database/migrations/20260703_improvement_ledger_views.sql`) need widening to `category IN ('adam_self_assessment', 'coordinator_self_assessment', 'solomon_self_assessment')` — otherwise Loop E silently undercounts two of the three roles it is meant to track, the exact failure mode this spine exists to prevent.
