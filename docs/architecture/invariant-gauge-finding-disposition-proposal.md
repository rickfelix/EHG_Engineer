...
| Distinct finding titles behind the 5,419 rows | 12 (roughly 450x duplicate amplification per title) |
| Oldest outstanding row | ~51.5 days |

This is the single channel this SD's census (FR-1) confirms as genuinely neglected — not a
throughput/backlog-size story like `harness_backlog` (16.1% closing rate, actively used) or
`completion_flag` (85.7%), but a channel whose designed disposition mechanism has been used
...
defined cadence (e.g. weekly), dispositioning each of the 12 distinct finding titles as
...
- **Con:** 5,419 rows behind only 12 distinct titles suggests most of the volume is duplicate
...
bulk of the 12 distinct titles (after confirming which ones dominate), or change the write cadence
...
Option B, scoped narrowly: investigate the 12 distinct titles first (a cheap, single query) to
