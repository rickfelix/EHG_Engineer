...
| 18 | `sync_ventures_to_eva_ventures_update()` | `database/migrations/20260315_fix_eva_ventures_status_sync.sql` (original), also reproduced verbatim in `database/migrations/20260821_eva_scheduler_queue_kill_time_teardown.sql` / `_DOWN.sql` (SD-LEO-INFRA-EVA-SCHEDULER-HYGIENE-001 FR-4, which adds an unrelated `eva_scheduler_queue` teardown block to the SAME function via `CREATE OR REPLACE` and must preserve this pre-existing line byte-identical, per that migration's own post-assert) | **SAME DISPOSITION AS #17 (writes `eva_ventures.current_lifecycle_stage`, not `ventures`)** | `UPDATE eva_ventures SET current_lifecycle_stage = NEW.current_lifecycle_stage ... WHERE venture_id = NEW.id` -- a TRIGGER on `ventures` (`trg_ventures_update_sync_eva`) that READS `ventures.current_lifecycle_stage` (already gated by whichever path #1-15 wrote it) and MIRRORS it into `eva_ventures`; it never writes `ventures` itself, so it is not an advancement path in this census's sense -- it is the sync leg for the same open #17 question (is `eva_ventures.current_lifecycle_stage` an independently-relied-upon store, or purely a downstream mirror of an already-gated write?). Pre-existing since 2026-03-15, unrelated to and untouched in substance by the 20260821 migration that merely reproduces it. |
