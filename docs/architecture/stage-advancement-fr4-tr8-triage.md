...
| 17 | `lib/proving-companion/stage-config.js` (generated; `getGateStages()`/`getKillGateStages()`) | **GENERATED, not hand-maintained** — `scripts/generate-stage-config.cjs` emits this file's `gateType` field straight from `venture_stages.gate_type` per row (raw, no work_type filter), with a CI drift guard (`--check`, 0-diff enforced; verified in sync during this SD's EXEC). This satisfies FR-3/AC#3's "published stage/gate map artifact... generated, not hand-maintained prose" requirement — a second, independent generator reading the same raw column would itself be a competing SSOT, so `stage-governance.js` does not duplicate it; see that file's own header cross-reference. |
