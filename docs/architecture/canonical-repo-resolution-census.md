...
| `lib/sub-agents/repo-target-resolver.js` (`computeReposForSD`; literal `rickfelix/ehg` / `rickfelix/EHG_Engineer` at ~line 40-41) | SD-LEO-INFRA-E2E-VERIFICATION-ROBUSTNESS-001-F (FR-1): `computeReposForSD` extracted here verbatim from the entry formerly listed below as a Deferred-with-owner, High-risk site (`gates.js`) — the literal content moved, but did not change, so this is the same pre-existing anchor relocated to a new canonical home (also importable from `lib/`), not new drift. `gates.js` itself now delegates here via a local function-declaration wrapper (see below) and no longer contains the literal. |
...
| Site | Notes |
|---|---|
| ~~`scripts/modules/handoff/executors/lead-final-approval/gates.js` (`computeReposForSD`)~~ **RESOLVED** by SD-LEO-INFRA-E2E-VERIFICATION-ROBUSTNESS-001-F (FR-1) | The literal implementation was extracted verbatim to `lib/sub-agents/repo-target-resolver.js` (new self-anchor entry above) so `lib/`-layer consumers could resolve target repos without importing `scripts/modules/handoff/executors/` (layering inversion). `gates.js` now delegates via a local function-declaration wrapper importing from that module — chosen specifically because a plain re-export (`export { X } from '...'` or `import { X } from '...'; export { X };`) both silently broke under vitest despite passing `node --check`, caught by this repo's own `pr-merge-verification.test.js` CI run (15 test failures) and confirmed via a minimal reproduction. No behavior change — verified against `tests/unit/lead-final-pr-merge-verification-cross-repo.test.js`, `tests/unit/venture-aware-completion-gates.test.js`, and `scripts/modules/handoff/executors/lead-final-approval/gates/pr-merge-verification.test.js` (all passing, 118+ tests). |
