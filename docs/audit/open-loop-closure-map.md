...
| 66 dormancy-telemetry rows sitting in the chairman view | **OBSOLETE as decisions** — machine telemetry mis-routed into a human queue (class C7); the underlying dormancy bug family is already tracked (QF-20260703-076 lineage) | Coordinator + Adam | Filter/reroute: severity-based feedback→chairman mapping must exclude auto-filed telemetry; batch-clear the 66 from the view's scope | Sonnet | Next-week belt |
| Remaining 9 flag_review rows | VERDICT PENDING SWEEP (fold into S1 sweep; checkin-own-claim row is CLOSED-UNDOCUMENTED — fix merged; MarketLens F4 row: verify patch state, then either close-with-citation or surface to chairman) | Sonnet seat | Same enumeration sweep | Sonnet | Next-week belt |
| `shouldAutoEscalate` predicate only fires on `raised_by='adam'` — stage-gate inserts structurally bypass it | **REAL-OPEN** | Adam (source) | Surfacing SD: any blocking pending decision escalates regardless of raiser; stage_gate rows escalate by class | Sonnet | **Before Discovery run** |
| SLA enforcer/timeout modules have zero production call sites (24h fallback everyone assumes never fires) | **REAL-OPEN** (class C2) | Adam (source, same SD) | Wire into a scheduled runner + add `stage_gate` SLA key (already flagged as feedback `3acb9cdd`) | Sonnet | **Before Discovery run** |
