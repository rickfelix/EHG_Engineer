...
**PARTIALLY ADDRESSED** by SD-LEO-INFRA-RESEARCH-INTELLIGENCE-OPERATOR-001-C (PR #6247, `modeling.js`): the forecast leg gains a grounding path — `generateForecast` now accepts injected reference data (`deps.referenceData`, or a `deps.supabase` read via `loadReferenceData`) from Child A's versioned `research_intelligence_reference` product (economic families `market_size`/`unit_economics`/`comparables`, `is_current`), and the prompt carries an estimation-method rules block (ranges-not-bare-points, Fermi decomposition, explicit assumptions, reference-class base rates). This wires the primary grounding mechanism the C2 finding calls for. NOT yet closed: (a) the standing reference is populated only once the RESEARCH_INTELLIGENCE_OPERATOR is armed (Child A ships it defined-but-unarmed), so with an empty table the forecast still runs unguided (honest-idle by design); (b) per-number `evidence`/`source_grade` provenance fields on the forecast output are still absent — the injection improves the estimate but does not yet stamp weakest-link grades. Backward compatible: with no reference data supplied, behavior is unchanged.
...
- **M4** `synthesis/tech-trajectory.js:42,47,147` — declared `dataFeed` external source has no caller anywhere; `data_feed_active` always false yet score carries weight. (Deliberately out of scope for SD-LEO-INFRA-SYNTHESIS-SCORING-HARDENING-001 — deferred, see PRD TR-2.)
