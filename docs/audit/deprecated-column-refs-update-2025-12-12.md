...
- **Table Rename**: `prds` → `product_requirements_v2`

## Sections Updated

### Section 269: "📚 Automated PRD Enrichment (MANDATORY)"
**Type**: knowledge_retrieval
**Changes**:
- Replaced 4 occurrences of `<SD_UUID>` → `<SD_ID>` in CLI examples
...
**Changes**:
- 1 occurrence: `strategic_directives_v2.uuid_id` → `strategic_directives_v2.id`
- 2 occurrences: `sd_uuid` → `sd_id`
- 1 occurrence: `prds` → `product_requirements_v2`
...
**Changes**:
- 3 occurrences: `sd_uuid` → `sd_id`
- 2 occurrences: `prds` → `product_requirements_v2`
...
1. `strategic_directives_v2\.uuid_id` → `strategic_directives_v2.id`
2. `--sd-uuid` → `--sd-id` (CLI flags)
3. `<SD_UUID>` → `<SD_ID>` (documentation placeholders)
4. `${SD_UUID}` → `${SD_ID}` (template variables)
5. `\bsd_uuid\b` → `sd_id` (column references)
6. `\bprds\b` → `product_requirements_v2` (table names)
