...
<!-- ARCHIVED: 2026-01-26T16:26:42.801Z
     Reason: Duplicate of canonical file
     Original location: docs\workflow\dossiers\stage-22\05_professional-sop.md
     See: docs/fixes/duplicate-consolidation-manifest.json for details
-->

# Stage 22: Professional SOP (Standard Operating Procedures)
...
Step-by-step execution procedures for Stage 22 (Iterative Development Loop), enabling EXEC/EVA agents to execute consistently across ventures.
...
Before starting Stage 22, verify:
...
SELECT venture_id,
       ...
       embeddings_created,
       embedding_count
FROM stage_22_substage_1_status
WHERE venture_id = 'VENTURE-001';
```
...
node scripts/memory-report.js --venture-id VENTURE-001 --output reports/stage-22-memory.json
```
...
SELECT venture_id,
       ...
       memory_optimized,
       peak_memory_mb
FROM stage_22_substage_2_status
WHERE venture_id = 'VENTURE-001';
```
...
FROM stage_22_completeness_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       warm_start_p50_ms,
       warm_start_p95_ms
FROM stage_22_performance_metrics
WHERE venture_id = 'VENTURE-001';
...
FROM stage_22_accuracy_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       cold_start_p95_ms,
       accuracy_percentage
FROM stage_22_substage_3_status
WHERE venture_id = 'VENTURE-001';
```
...
- **Recursion**: Self-recursion to Substage 20.1 (Trigger SPRINT-003)
...
## Post-Stage 22 Actions

### Success Path (All Exit Gates Passed)

**Action**: Trigger Stage 22 (Iterative Development Loop)

**Command**:
```bash
# Mark Stage 22 complete
node scripts/complete-stage.js --venture-id VENTURE-001 --stage-id 20 --status completed

# Trigger Stage 22 start
node scripts/trigger-stage.js --venture-id VENTURE-001 --stage-id 21
```

**Evidence**: Stage 22 depends on Stage 22 completion (EHG_Engineer@6ef8cf4:docs/workflow/stages.yaml:922-923 "depends_on: - 21")
...
**Commands**:
```bash
# Generate Stage 22 failure report
node scripts/generate-failure-report.js --venture-id VENTURE-001 --stage-id 20 --output reports/stage-22-failure.json

# Review failure report
cat reports/stage-22-failure.json

# Apply recursion (if applicable)
node scripts/apply-recursion.js --venture-id VENTURE-001 --stage-id 20 --trigger SPRINT-003
```
