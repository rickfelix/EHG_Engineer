...
<!-- ARCHIVED: 2026-01-26T16:26:42.888Z
     Reason: Duplicate of canonical file
     Original location: docs\workflow\dossiers\stage-21\05_professional-sop.md
     See: docs/fixes/duplicate-consolidation-manifest.json for details
-->

# Stage 21: Professional SOP (Standard Operating Procedures)
...
Step-by-step execution procedures for Stage 21 (Final Pre-Flight Check), enabling EXEC/EVA agents to execute consistently across ventures.
...
Before starting Stage 21, verify:
...
SELECT venture_id,
       ...
       embeddings_created,
       embedding_count
FROM stage_21_substage_1_status
WHERE venture_id = 'VENTURE-001';
```
...
node scripts/memory-report.js --venture-id VENTURE-001 --output reports/stage-21-memory.json
```
...
SELECT venture_id,
       ...
       memory_optimized,
       peak_memory_mb
FROM stage_21_substage_2_status
WHERE venture_id = 'VENTURE-001';
```
...
FROM stage_21_completeness_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       warm_start_p50_ms,
       warm_start_p95_ms
FROM stage_21_performance_metrics
WHERE venture_id = 'VENTURE-001';
...
FROM stage_21_accuracy_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       cold_start_p95_ms,
       accuracy_percentage
FROM stage_21_substage_3_status
WHERE venture_id = 'VENTURE-001';
```
...
- **Recursion**: Self-recursion to Substage 20.1 (Trigger PREFLIGHT-003)
...
## Post-Stage 21 Actions

### Success Path (All Exit Gates Passed)

**Action**: Trigger Stage 21 (Final Pre-Flight Check)

**Command**:
```bash
# Mark Stage 21 complete
node scripts/complete-stage.js --venture-id VENTURE-001 --stage-id 20 --status completed

# Trigger Stage 21 start
node scripts/trigger-stage.js --venture-id VENTURE-001 --stage-id 21
```

**Evidence**: Stage 21 depends on Stage 21 completion (EHG_Engineer@6ef8cf4:docs/workflow/stages.yaml:922-923 "depends_on: - 20")
...
**Commands**:
```bash
# Generate Stage 21 failure report
node scripts/generate-failure-report.js --venture-id VENTURE-001 --stage-id 20 --output reports/stage-21-failure.json

# Review failure report
cat reports/stage-21-failure.json

# Apply recursion (if applicable)
node scripts/apply-recursion.js --venture-id VENTURE-001 --stage-id 20 --trigger PREFLIGHT-003
```
