<!-- ARCHIVED: 2026-01-26T16:26:42.623Z
     Reason: Duplicate of canonical file
     Original location: docs\workflow\dossiers\stage-24\05_professional-sop.md
...
Before starting Stage 24, verify:
...
SELECT venture_id,
       ...
       embeddings_created,
       embedding_count
FROM stage_24_substage_1_status
WHERE venture_id = 'VENTURE-001';
```
...
node scripts/memory-report.js --venture-id VENTURE-001 --output reports/stage-24-memory.json
```
...
SELECT venture_id,
       ...
       memory_optimized,
       peak_memory_mb
FROM stage_24_substage_2_status
WHERE venture_id = 'VENTURE-001';
```
...
FROM stage_24_completeness_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       warm_start_p50_ms,
       warm_start_p95_ms
FROM stage_24_performance_metrics
WHERE venture_id = 'VENTURE-001';
...
FROM stage_24_accuracy_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       cold_start_p95_ms,
       accuracy_percentage
FROM stage_24_substage_3_status
WHERE venture_id = 'VENTURE-001';
```
...
- **Recursion**: Self-recursion to Substage 20.1 (Trigger ITERATION-003)
...
## Post-Stage 24 Actions

### Success Path (All Exit Gates Passed)

**Action**: Trigger Stage 24 (MVP Engine: Automated Feedback Iteration)

**Command**:
```bash
# Mark Stage 24 complete
node scripts/complete-stage.js --venture-id VENTURE-001 --stage-id 20 --status completed

# Trigger Stage 24 start
node scripts/trigger-stage.js --venture-id VENTURE-001 --stage-id 21
```

**Evidence**: Stage 24 depends on Stage 24 completion (EHG_Engineer@6ef8cf4:docs/workflow/stages.yaml:922-923 "depends_on: - 23")
...
**Commands**:
```bash
# Generate Stage 24 failure report
node scripts/generate-failure-report.js --venture-id VENTURE-001 --stage-id 20 --output reports/stage-24-failure.json

# Review failure report
cat reports/stage-24-failure.json

# Apply recursion (if applicable)
node scripts/apply-recursion.js --venture-id VENTURE-001 --stage-id 20 --trigger ITERATION-003
```
