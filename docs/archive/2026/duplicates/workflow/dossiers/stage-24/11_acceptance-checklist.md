...
cat ../stage-25/07_recursion-blueprint.md | grep -E "Trigger ITERATION-[0-9]+" | wc -l
```
...
cat ../stage-07/10_gaps-backlog.md | grep -E "SD-[A-Z-]+-[0-9]+" | sort -u | wc -l
```
...
cat ../stage-25/06_agent-orchestration.md | grep -E "Agent [0-9]+:" | wc -l
```
...
cat ../stage-15/09_metrics-monitoring.md | grep -E "SELECT|FROM|WHERE" | wc -l
```
...
cat ../stage-07/10_gaps-backlog.md | grep -E "## Gap [0-9]+:" | wc -l
```
...
```bash
# Check SOP structure
cat ../stage-25/05_professional-sop.md | grep -E "### Substage 20\.[0-9]+" | wc -l
```
...
cat ../stage-07/08_configurability-matrix.md | grep -E "\*\*Parameter [0-9]+\*\*" | wc -l
```
...
TRIGGERS=$(cat ../stage-25/07_recursion-blueprint.md | grep -E "Trigger ITERATION-[0-9]+" | wc -l)
echo "Recursion Triggers: $TRIGGERS (target: 3)"

# 3. SD references
SD_REFS=$(cat ../stage-07/10_gaps-backlog.md | grep -E "SD-[A-Z-]+-[0-9]+" | sort -u | wc -l)
echo "SD Cross-References: $SD_REFS (target: ≥7)"

# 4. Agent count
AGENTS=$(cat ../stage-25/06_agent-orchestration.md | grep -E "Agent [0-9]+:" | wc -l)
echo "Agents Defined: $AGENTS (target: ≥4)"

# 5. SQL queries
SQL_LINES=$(cat ../stage-15/09_metrics-monitoring.md | grep -E "SELECT|FROM|WHERE" | wc -l)
echo "SQL Query Lines: $SQL_LINES (target: ≥9)"

# 6. Gaps mapped
GAPS=$(cat ../stage-07/10_gaps-backlog.md | grep -E "## Gap [0-9]+:" | wc -l)
echo "Gaps Mapped: $GAPS (target: 5)"

# 7. Substages in SOP
SUBSTAGES=$(cat ../stage-25/05_professional-sop.md | grep -E "### Substage 20\.[0-9]+" | wc -l)
echo "SOP Substages: $SUBSTAGES (target: 3)"

# 8. Configurable parameters
PARAMS=$(cat ../stage-07/08_configurability-matrix.md | grep -E "\*\*Parameter [0-9]+\*\*" | wc -l)
