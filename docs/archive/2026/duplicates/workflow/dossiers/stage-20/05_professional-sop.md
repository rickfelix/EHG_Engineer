...
<!-- ARCHIVED: 2026-01-26T16:26:42.974Z
     Reason: Duplicate of canonical file
     Original location: docs\workflow\dossiers\stage-20\05_professional-sop.md
     See: docs/fixes/duplicate-consolidation-manifest.json for details
-->

# Stage 20: Professional SOP (Standard Operating Procedures)
...
Step-by-step execution procedures for Stage 20 (Enhanced Context Loading), enabling EXEC/EVA agents to execute consistently across ventures.
...
Before starting Stage 20, verify:
...
SELECT venture_id,
       ...
       embeddings_created,
       embedding_count
FROM stage_20_substage_1_status
WHERE venture_id = 'VENTURE-001';
```
...
node scripts/memory-report.js --venture-id VENTURE-001 --output reports/stage-20-memory.json
```
...
SELECT venture_id,
       ...
       memory_optimized,
       peak_memory_mb
FROM stage_20_substage_2_status
WHERE venture_id = 'VENTURE-001';
```
...
FROM stage_20_completeness_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       warm_start_p50_ms,
       warm_start_p95_ms
FROM stage_20_performance_metrics
WHERE venture_id = 'VENTURE-001';
...
FROM stage_20_accuracy_metrics
WHERE venture_id = 'VENTURE-001';
...
SELECT venture_id,
       ...
       cold_start_p95_ms,
       accuracy_percentage
FROM stage_20_substage_3_status
WHERE venture_id = 'VENTURE-001';
```
...
- **Recursion**: Self-recursion to Substage 20.1 (Trigger CONTEXT-003)
...
**Evidence**: Stage 21 depends on Stage 20 completion (EHG_Engineer@6ef8cf4:docs/workflow/stages.yaml:922-923 "depends_on: - 20")
...
**Commands**:
```bash
# Generate Stage 20 failure report
node scripts/generate-failure-report.js --venture-id VENTURE-001 --stage-id 20 --output reports/stage-20-failure.json

# Review failure report
cat reports/stage-20-failure.json

# Apply recursion (if applicable)
node scripts/apply-recursion.js --venture-id VENTURE-001 --stage-id 20 --trigger CONTEXT-003
```
