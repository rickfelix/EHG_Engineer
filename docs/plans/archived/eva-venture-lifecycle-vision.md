...
- [2. Core Philosophy](#2-core-philosophy)
  - [Capital-Efficient Venture Creation via AI Orchestration](#capital-efficient-venture-creation-via-ai-orchestration)
  - [Maximum Automation, Minimum Human Intervention](#maximum-automation-minimum-human-intervention)
  - [The Chairman Governance Model](#the-chairman-governance-model)
  - [Unlimited Compute](#unlimited-compute)
  - [CLI as Authoritative Interface](#cli-as-authoritative-interface)
  - [Active Intelligence, Not Passive Collection](#active-intelligence-not-passive-collection)
  - [Design for Exit](#design-for-exit)
- [3. Automation Architecture](#3-automation-architecture)
  - [Three Layers of Automated Control](#three-layers-of-automated-control)
  - [Decision Filter Engine](#decision-filter-engine)
  - [Reality Gates (Non-Negotiable)](#reality-gates-non-negotiable)
  - [What the Chairman Actually Sees](#what-the-chairman-actually-sees)
  - [Advisory Checkpoints (Optional, Non-Blocking)](#advisory-checkpoints-optional-non-blocking)
  - [Conditional Outcome Resolution](#conditional-outcome-resolution)
  - [Sprint Cadence](#sprint-cadence)
  - [EVA Runtime Model: Hybrid Reactive + Scheduled](#eva-runtime-model-hybrid-reactive-scheduled)
  - [Ground Truth: Web-Grounded AnalysisSteps](#ground-truth-web-grounded-analysissteps)
  - [Reality Gate Failure Recovery](#reality-gate-failure-recovery)
- [4. Phase Structure](#4-phase-structure)
  - [The Six Phases](#the-six-phases)
  - [Phase Boundaries and Gate Types](#phase-boundaries-and-gate-types)
  - [Gate Design Principle](#gate-design-principle)
- [5. Stage Inventory](#5-stage-inventory)
  - [Linear Progression: Stages 1-23](#linear-progression-stages-1-23)
  - [Recurring Operations Loop: Stages 24-25](#recurring-operations-loop-stages-24-25)
  - [Cross-Stage Data Contracts](#cross-stage-data-contracts)
  - [Immutable Stage Versioning](#immutable-stage-versioning)
  - [The analysisStep Pattern](#the-analysisstep-pattern)
- [6. Venture Lifecycle Flow](#6-venture-lifecycle-flow)
  - [End-to-End Flow](#end-to-end-flow)
  - [Chairman Decision Points (Complete List)](#chairman-decision-points-complete-list)
  - [Fully Automated Decision Points](#fully-automated-decision-points)
  - [How Pivots Work](#how-pivots-work)
  - [How Expand Decisions Are Scoped](#how-expand-decisions-are-scoped)
  - [How Ventures Enter the Lifecycle](#how-ventures-enter-the-lifecycle)
  - [How Ventures Exit the Lifecycle](#how-ventures-exit-the-lifecycle)
- [7. Key Differentiators from Gemini Vision](#7-key-differentiators-from-gemini-vision)
  - [1. Engineering Execution Delegated to LEO](#1-engineering-execution-delegated-to-leo)
  - [2. GTM as First-Class Stage(s)](#2-gtm-as-first-class-stages)
  - [3. Brand/Identity Retained and Broadened](#3-brandidentity-retained-and-broadened)
  - [4. Exit Designed-For but Deferred](#4-exit-designed-for-but-deferred)
  - [5. Investment as Automated Portfolio Optimization](#5-investment-as-automated-portfolio-optimization)
  - [6. Phase Structure](#6-phase-structure)
- [8. Venture Decision Taxonomy](#8-venture-decision-taxonomy)
  - [Gate Decisions (Within Phases)](#gate-decisions-within-phases)
  - [Venture Decisions (Lifecycle-Level)](#venture-decisions-lifecycle-level)
  - [Decision Filter Outputs (Any Stage)](#decision-filter-outputs-any-stage)
  - [Chairman Decisions (Override Authority)](#chairman-decisions-override-authority)
  - [Decision Design Principles](#decision-design-principles)
- [9. The SD Bridge: Lifecycle to Engineering](#9-the-sd-bridge-lifecycle-to-engineering)
  - [How It Works](#how-it-works)
  - [Why This Matters](#why-this-matters)
- [10. Post-Launch Operations](#10-post-launch-operations)
  - [Decided](#decided)
  - [Needs Deep Research (Step 6)](#needs-deep-research-step-6)
  - [Design Principle](#design-principle)
- [11. Success Metrics](#11-success-metrics)
  - [Venture Health (5 Dimensions)](#venture-health-5-dimensions)
  - [Lifecycle Effectiveness](#lifecycle-effectiveness)
  - [The Compounding Intelligence Signal](#the-compounding-intelligence-signal)
- [12. Portfolio Intelligence](#12-portfolio-intelligence)
  - [Cross-Venture Knowledge Transfer](#cross-venture-knowledge-transfer)
  - [Venture Templates](#venture-templates)
  - [Portfolio Prioritization](#portfolio-prioritization)
  - [Inter-Venture Dependencies](#inter-venture-dependencies)
  - [Resource Contention Management](#resource-contention-management)
  - [Chairman Dashboard & Notifications](#chairman-dashboard-notifications)
- [Appendix A: Stage 0 (Pre-Lifecycle)](#appendix-a-stage-0-pre-lifecycle)
- [Appendix B: Golden Nuggets](#appendix-b-golden-nuggets)
- [Appendix C: Scoring Models](#appendix-c-scoring-models)
  - [Stage 3: Hybrid Kill Gate Scoring](#stage-3-hybrid-kill-gate-scoring)
  - [Stage 5: Financial Kill Gate](#stage-5-financial-kill-gate)
- [Appendix D: Enum Reference](#appendix-d-enum-reference)
  - [Decision Enums](#decision-enums)
  - [Categorization Enums](#categorization-enums)
...
The **EHG App** serves as the Chairman's governance interface -- a dashboard extending the existing React/TypeScript application with a decision queue, portfolio health monitoring, and escalation handling. The Chairman approves, rejects, and reviews decisions through this interface. The CLI remains authoritative for venture progression; the dashboard is how the Chairman exercises governance authority, not how ventures advance through stages.

**GUI deprecation**: The EHG App previously included 25 individual stage components (GUI-driven stage data entry). These are being removed as the CLI takes over all stage progression. The remaining EHG App role is Chairman governance: decision queue, portfolio health heatmap, event feed, and escalation panels. No per-stage GUI will remain.
...
### EVA Runtime Model: Hybrid Reactive + Scheduled

EVA operates as a **hybrid runtime** -- combining real-time event reactions with scheduled batch operations. This is not a manual, request-driven system; EVA continuously monitors and acts.

**Reactive layer (event-driven):**
- **Database change listeners** (Supabase Realtime) detect state changes: venture stage completions, Chairman decisions recorded, DFE escalation triggers, metric threshold breaches
- **Immediate response**: When a Chairman approves a Stage 10 brand decision, EVA detects the `chairman_decisions` row update and advances the venture to Stage 11 without delay
- **Escalation routing**: DFE trigger events are immediately surfaced to the Chairman's decision queue in the EHG App

**Scheduled layer (cron-driven):**
- **Planned venture advancement**: Batch processing of ventures ready for their next stage (e.g., nightly runs to advance all ventures with completed analysis)
- **Ops cycle cadence**: Stage 24-25 recurring loops execute on their risk-adaptive schedule (weekly/monthly/quarterly)
- **Release scheduling**: Future enhancement releases can be planned for specific dates, with EVA preparing all prerequisites in advance
- **Portfolio health sweeps**: Periodic cross-venture health assessments and knowledge transfer operations

**Why hybrid**: Pure event-driven misses planned cadences (scheduled releases, portfolio reviews). Pure scheduled misses urgency (Chairman decision at 2pm shouldn't wait until the nightly batch). The hybrid model ensures ventures advance at the speed of decisions while maintaining predictable operational rhythms.
...
Every stage outcome feeds a **portfolio knowledge base** that accelerates future ventures. Knowledge retrieval uses **hybrid semantic search** -- combining vector similarity (via local embeddings generated by Ollama) with keyword matching (BM25) -- to surface the most relevant patterns regardless of exact terminology. A venture exploring "subscription pricing for developer tools" automatically retrieves patterns from prior ventures that used "recurring revenue for API products" even though the phrasing differs. This is fully automated -- the system gets smarter without Chairman involvement.
...
| **Successful patterns** | Architecture, pricing, and GTM patterns from successful ventures seed future ones (semantic similarity matching) | SaaS venture #2 inherits proven architecture patterns from venture #1 |
| **Failure signals** | Failed ventures contribute anti-patterns that future stages flag (vector search surfaces related failures) | "Marketplace pricing in B2B SaaS" flagged as risky based on prior failure |
...
| **Issue pattern reuse** | LEO Protocol `issue_patterns` table entries are indexed for vector search, enabling cross-venture debugging intelligence | A database connection issue solved in venture A is surfaced when venture B hits a similar symptom |
...
The Chairman interacts with the portfolio through the **EHG App** (React/TypeScript) -- extending the existing application's Chairman interface with smart notifications. The EHG App already includes a Chairman's Glass Cockpit, DecisionsInbox (approve/reject with evidence), and EscalationPanel. These existing components are wired to the CLI governance plane so that Chairman decisions recorded through the dashboard flow back to the `chairman_decisions` table where the EVA runtime's reactive layer (Supabase Realtime) detects them and advances ventures accordingly.

**Dashboard Views:**
- **Decision Queue** (extends existing DecisionsInbox): Pending decisions ranked by venture priority. Shows decision type, venture name, urgency, and recommended action. Chairman approves/rejects directly.
- **Health Heatmap** (extends existing PortfolioSummary): All ventures at a glance, color-coded by health score across 5 dimensions.
...
- **Escalation Panel** (extends existing EscalationPanel): Evidence cards with AI recommendations for DFE-escalated decisions.
...
*Version 4.7 revision: 8 OpenClaw-informed decisions applied (D1-D8). D1: Borrow ideas / replicate in-house — no external dependency. D2: Hybrid reactive+scheduled runtime (Supabase Realtime + cron). D3: Chairman interacts via EHG App dashboard (extend existing Glass Cockpit). D4: Per-agent tool profiles (architecture plan). D5: SKILL.md format evolution (architecture plan). D6: Hybrid semantic search for cross-venture learning (vector + BM25 via Ollama). D7: Model failover deferred. D8: Phase C as standalone body of work (architecture plan). Also: 25-stage GUI components deprecated in favor of CLI-only progression; EHG App retained for Chairman governance only.*
...
*OpenClaw research: architecture analysis, replication candidate matrix, threat model (2026-02-12)*
