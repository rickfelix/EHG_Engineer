#!/usr/bin/env node
// EHG Backlog Import Script with Lifecycle Management
...
function extractSDData(row) {
  return {
    sd_id: trimToNull(row['SD ID']),
    sequence_rank: parseInt(row['Sequence Rank']) || 0,
    sd_title: trimToNull(row['SD Title']),
    page_category: trimToNull(row['Page Category']),
...
async function main() {
  ...
  for (const [sdId, group] of Object.entries(sdGroups)) {
    ...
    // Store original SD ID if we generated one
    if (sd.sd_id && sd.sd_id.startsWith('SD-GEN-')) {
      sd.previous_sd_id = group.sd_data.sd_id || 'NONE';
    }
    
    // Validation
    const expectedTotal = sd.h_count + sd.m_count + sd.l_count + sd.future_count;
    if (expectedTotal !== sd.total_items) {
      warnings.push({
        sd_id: sdId,
        type: 'count_mismatch',
        message: `Priority counts (H:${sd.h_count}+M:${sd.m_count}+L:${sd.l_count}+F:${sd.future_count}=${expectedTotal}) != total items (${sd.total_items})`
      });
    }
    
    if (sd.must_have_count + sd.wish_list_count > sd.total_items) {
      warnings.push({
        sd_id: sdId,
        type: 'tag_overflow',
        message: `Tag counts (must:${sd.must_have_count}+nice:${sd.wish_list_count}) exceed total items (${sd.total_items})`
      });
    }
  ...
  if (warnings.length > 0) {
    // Dedupe warnings by type and message
  ...
  topMustHave.forEach(({ sd }) => {
    const title = sd.sd_title ? sd.sd_title.substring(0, 25) : 'N/A';
    console.log(`  ${sd.sd_id} | ${String(sd.sequence_rank).padStart(3)} | ${title.padEnd(25)} | ${String(sd.must_have_pct).padStart(5)}% | ${sd.h_count}/${sd.m_count}/${sd.l_count}/${sd.future_count}`);
  });
  ...
  if (processedSDs.length > 0) {
    ...
    console.log('='.repeat(60));
    console.log(`SD ID:        ${example.sd.sd_id}`);
    console.log(`Title:        ${example.sd.sd_title || 'N/A'}`);
    console.log(`Sequence:     ${example.sd.sequence_rank}`);
    console.log(`Page:         ${example.sd.page_category || 'N/A'} / ${example.sd.page_title || 'N/A'}`);
    ...
    console.log(`Must-have:    ${example.sd.must_have_count}/${example.sd.total_items} (${example.sd.must_have_pct}%)`);
...
async function importToDatabase(processedSDs, warnings, checksum, filePath) {
  ...
  // First, mark all existing records as not present in latest import
  ...
  if (resetSDError && resetSDError.code !== '42P01') {
  ...
  for (const { sd, items } of processedSDs) {
    try {
      ...
      const { error: sdError } = await supabase
        .from('strategic_directives_backlog')
        .upsert(sd, { onConflict: 'sd_id' });
      
      if (sdError) {
        console.error(`❌ Failed to import SD ${sd.sd_id}:`, sdError.message);
        errorCount++;
        continue;
      }
      
      // Upsert items with composite key
      if (items.length > 0) {
        const itemsWithSdId = items.map(item => ({ sd_id: sd.sd_id, ...item }));
        const { error: itemError } = await supabase
          .from('sd_backlog_map')
          .upsert(itemsWithSdId, { onConflict: 'sd_id,backlog_id' });
        
        if (itemError) {
          console.error(`❌ Failed to import items for SD ${sd.sd_id}:`, itemError.message);
          errorCount++;
        } else {
          successCount++;
          console.log(`✅ Imported SD ${sd.sd_id} with ${items.length} items`);
        }
      } else {
        successCount++;
        console.log(`✅ Imported SD ${sd.sd_id} (no items)`);
      }
    } catch (err) {
      console.error(`❌ Unexpected error for SD ${sd.sd_id}:`, err.message);
      errorCount++;
    }
  }
