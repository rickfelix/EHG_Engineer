 ...
 *   2. Replace SD-ROUTE-AUDIT-VENTURES with your SD ID (e.g., SD-AUTH-001)
...
const SD_ID = 'SD-ROUTE-AUDIT-VENTURES'; // TODO: Replace with your SD ID (e.g., 'SD-AUTH-001')
const PRD_TITLE = 'Route Assessment: Ventures Section'; // TODO: Replace with your PRD title
...
async function createPRD() {
  ...
  const prdData = {
    ...
    // Executive & Context
    executive_summary: `
      Assess all 9 routes in the Ventures navigation section for design consistency, accessibility
      compliance (WCAG 2.1 AA), performance metrics (LCP/FID/CLS), and security posture. This section
      is CRITICAL as it contains core venture management functionality including list view, detail view,
      create/edit flows, and venture-specific sub-pages.

      Sub-agents (DESIGN, SECURITY, PERFORMANCE) will evaluate each route. All issues will be
      categorized by severity (P0-P3) and documented with remediation recommendations.

      Output: Assessment findings recorded for aggregation into parent audit report.
    `.trim(),

    business_context: `
      Ventures section is the primary interaction point for users managing their venture portfolio.
      Quality issues here directly impact user satisfaction and platform adoption. Assessment will
      establish baseline quality metrics and identify high-priority improvements.
    `.trim(),

    technical_context: `
      Routes to assess: /ventures (list), /ventures/[id] (detail), /ventures/new (create),
      /ventures/[id]/edit (edit), plus sub-pages for stages, analytics, settings.
      React components with Zustand state, React Query data fetching, Shadcn UI primitives.
    `.trim(),
    ...
    functional_requirements: [
      {
        id: 'FR-1',
        requirement: 'Navigate and assess all 9 Ventures routes',
        description: 'Visit each route, capture state, invoke sub-agents',
        priority: 'CRITICAL',
        acceptance_criteria: [
          'All 9 routes visited and evaluated',
          'Each route has DESIGN sub-agent assessment',
          'Each route has PERFORMANCE metrics captured'
        ]
      },
      {
        id: 'FR-2',
        requirement: 'Categorize all findings by severity',
        description: 'Apply P0-P3 classification to identified issues',
        priority: 'HIGH',
        acceptance_criteria: [
          'All issues have severity classification',
          'P0/P1 issues have detailed remediation notes'
        ]
      },
      {
        id: 'FR-3',
        requirement: 'Execute DESIGN sub-agent evaluation',
        description: 'Assess UI consistency, visual hierarchy, responsive behavior',
        priority: 'HIGH',
        acceptance_criteria: [
      ...
      {
        id: 'FR-4',
        requirement: 'Execute ACCESSIBILITY assessment',
        description: 'Evaluate WCAG 2.1 AA compliance for each route',
        priority: 'HIGH',
        acceptance_criteria: [
      ...
      {
        id: 'FR-5',
        requirement: 'Execute PERFORMANCE sub-agent evaluation',
        description: 'Capture Core Web Vitals metrics',
        priority: 'MEDIUM',
        acceptance_criteria: [
          'LCP measured for each route',
          'FID measured for interactive routes',
          'CLS measured for dynamic content routes'
        ]
      }
    ],
    ...
    non_functional_requirements: [
      {
        ...
        target_metric: '9/9 Ventures routes assessed'
      },
      {
        type: 'quality',
        requirement: 'Issue documentation quality',
        target_metric: 'Each issue has description, location, severity, recommendation'
      }
    ],
    ...
    technical_requirements: [
      {
        ...
        description: 'Invoke DESIGN, SECURITY, PERFORMANCE sub-agents per route',
        dependencies: ['design-agent', 'security-agent', 'performance-agent']
      }
    ],
