 ...
 *   2. Replace SD-VS-RESEARCH-ARM-001 with your SD ID (e.g., SD-AUTH-001)
...
const SD_ID = 'SD-VS-RESEARCH-ARM-001'; // TODO: Replace with your SD ID (e.g., 'SD-AUTH-001')
const PRD_TITLE = 'Research Arm Pipeline Integration'; // TODO: Replace with your PRD title
...
async function createPRD() {
  ...
  const prdData = {
    ...
    // Executive & Context
    executive_summary: `
      Integrate the research arm pipeline with the venture selection framework. This includes
      connecting the CrewAI research agents, implementing weekly research digest generation,
      and creating hooks for triggering research workflows from the UI.

      The pipeline will automate venture research by leveraging AI agents to analyze market
      opportunities and provide structured research outputs that feed into the scoring engine.
    `.trim(),

    business_context: `
      - User pain point: Manual research is time-consuming and inconsistent
      - Business objective: Automate venture research with AI-powered analysis
      - Success metrics: Research digest delivered weekly, pipeline success rate >90%
    `.trim(),

    technical_context: `
      - Existing systems: CrewAI research crews (Venture Research, Quick Validation)
      - Architecture patterns: Queue-based processing, webhook integration
    ...
    functional_requirements: [
      {
        id: 'FR-1',
        requirement: 'Create research pipeline trigger hooks',
        description: 'API endpoints to trigger research workflows from UI',
        priority: 'CRITICAL',
        acceptance_criteria: [
          'POST endpoint to trigger research for a venture',
          'Queue management for pending research jobs',
          'Status polling endpoint for job progress'
        ]
      },
      {
        id: 'FR-2',
        requirement: 'Implement weekly research digest',
        description: 'Automated weekly summary of research findings',
        priority: 'HIGH',
        acceptance_criteria: [
          'Scheduled job runs every Monday',
          'Aggregates research from past 7 days',
          'Outputs structured digest to database'
        ]
      },
      {
        id: 'FR-3',
        requirement: 'Connect CrewAI research output to scoring',
        description: 'Feed research results into scoring engine',
        priority: 'HIGH',
        acceptance_criteria: [
          'Research results stored in structured format',
          'Scoring engine can read research metrics',
          'Pattern match scores derived from research'
        ]
      }
    ],
