 ...
 *   2. Replace SD-VS-SCORING-RUBRIC-001 with your SD ID (e.g., SD-AUTH-001)
...
const SD_ID = 'SD-VS-SCORING-RUBRIC-001'; // TODO: Replace with your SD ID (e.g., 'SD-AUTH-001')
const PRD_TITLE = 'Venture Opportunity Scoring Engine'; // TODO: Replace with your PRD title
...
async function createPRD() {
  ...
  const prdData = {
    ...
    // Executive & Context
    executive_summary: `
      Implement a weighted scoring engine for venture opportunities that applies the Chairman's
      configurable scoring rubric. This engine will calculate composite scores based on 6 dimensions:
      Feedback Speed (25%), Pattern Match (20%), Market Demand (20%), Unit Economics (15%),
      Distribution Fit (10%), and Strategic Unlock (10%).

      The scoring engine enables objective evaluation of venture opportunities against the
      Chairman's investment criteria stored in chairman_settings, providing data-driven
      prioritization for the venture pipeline.
    `.trim(),

    business_context: `
      - User pain point: Manual evaluation of ventures is subjective and time-consuming
      - Business objective: Automate consistent scoring against defined criteria
      - Success metrics: All opportunities scored within 24h, scoring variance <5%
    `.trim(),

    technical_context: `
      - Existing systems: chairman_settings table (from SD-VS-CHAIRMAN-SETTINGS-001)
      - Architecture patterns: Supabase stored procedures, React hooks
      - Integration points: venture_opportunities table, chairman_settings table
    `.trim(),
    ...
    functional_requirements: [
      {
        id: 'FR-1',
        requirement: 'Create venture_opportunity_scores table',
        description: 'Database table to store scoring results with all 6 dimension scores',
        priority: 'CRITICAL',
        acceptance_criteria: [
          'Table has columns for all 6 scoring dimensions',
          'Foreign key to venture_opportunities table',
          'RLS policies implemented for data protection'
        ]
      },
      {
        id: 'FR-2',
        requirement: 'Implement scoring calculation function',
        description: 'Supabase function that calculates weighted composite score',
        priority: 'CRITICAL',
        acceptance_criteria: [
          'Applies weights from chairman_settings',
          'Returns composite score 0-100',
          'Handles missing dimension values gracefully'
        ]
      },
      {
        id: 'FR-3',
        requirement: 'Create useVentureScoring React hook',
        description: 'Hook for fetching and calculating scores in UI',
        priority: 'HIGH',
        acceptance_criteria: [
