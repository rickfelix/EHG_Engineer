...
export const ADAM_LOOPS = [
  ...
  },
  {
    ...
    cron: '14 * * * *',
    prompt: 'Adam heartbeat-sms tick: REQUIRED FIRST STEP (coordinator ruling e1b923a2, durable fix of the 08-18 69-min lapse) — query sms_outbound_obligations for the most recent kind=\'heartbeat_status\' row (order by created_at desc, limit 1) and compute the real elapsed gap since its created_at. Never decide from session recall of when you last sent — a session restart loses that memory entirely, which is exactly how the lapse happened. Only proceed once the MEASURED gap is >=~55min and the chairman is awake; otherwise stay silent this tick. If the gap has been reached: if there is truly nothing plan-relevant to report, stay SILENT (silence-by-default) — otherwise compose ONE short (1-2 sentence), professional-casual, plan-relevant status line and run node scripts/adam-chairman-sms.mjs --kind heartbeat_status --body "<line>" (quiet hours and rate caps are enforced by the send gate itself — do not skip the call to pre-empt them). Never send a false all-good; if something is actually wrong, send the decision/alert email instead (node scripts/adam-decision-email.mjs) rather than this heartbeat.',
  },
  {
