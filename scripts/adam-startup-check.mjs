...
export const ADAM_LOOPS = [
  {
    ...
    prompt: 'Run `node scripts/adam-quiet-tick.mjs`. It prints ONE QUIET_TICK summary line and self-paces. If the output contains NO QUIET_TICK_PING / QUIET_TICK_STALL_ALERT / QUIET_TICK_VENTURE_STALL_ALERT / QUIET_TICK_VENTURE_REAL_BUILD_STALL_ALERT / QUIET_TICK_BOARD_STALE / QUIET_TICK_OUTBOUND_PROBE / QUIET_TICK_INBOX_DIRECTIVE / QUIET_TICK_INBOX_ITEM / QUIET_TICK_INBOX_CAP / QUIET_TICK_INBOX_BACKLOG_BREACH / QUIET_TICK_SMS_INBOUND / QUIET_TICK_SMS_PARKED / QUIET_TICK_RATIFICATION_STALE / QUIET_TICK_OVERSIGHT_OVERDUE / QUIET_TICK_SELFSCORE_OVERDUE / QUIET_TICK_HEARTBEAT_OVERDUE / QUIET_TICK_STANDING_PRIORITY_UNSERVED / QUIET_TICK_CHAIRMAN_GATED_STAMP_ERROR / QUIET_TICK_CLASSIFIER_DENIAL_STAMP_ERROR / QUIET_TICK_IDLE_BESIDE_CLAIMABLE / QUIET_TICK_VENTURE_TERMINAL_FLIP / QUIET_TICK_COMPLETION_READY_PARENT / QUIET_TICK_COMPLETION_READY_PARENT_ERROR / QUIET_TICK_CRON_PARITY_MISSING / QUIET_TICK_CRON_PARITY_ERROR / QUIET_TICK_PARENT_ROLLUP_SYNC_ERROR / QUIET_TICK_OUTPUT_FLOW_STALL / QUIET_TICK_DURATION_BASELINE_BREACH / QUIET_TICK_PROBE_TIMEOUT / QUIET_TICK_VENTURE_DEPLOY_RED / QUIET_TICK_ERROR lines, this turn is a NO-OP: arm ScheduleWakeup(nextWakeSeconds from the output) and emit nothing else. Otherwise act on the flagged lines (QUIET_TICK_INBOX_DIRECTIVE lines are HARD interrupts — process the directed row, then `node scripts/adam-advisory.cjs ack <id>`; QUIET_TICK_INBOX_ITEM lines are directed inbox rows to action or deliberately leave pending; QUIET_TICK_VENTURE_STALL_ALERT flags a stalled venture to investigate/escalate, mirroring QUIET_TICK_STALL_ALERT; QUIET_TICK_VENTURE_REAL_BUILD_STALL_ALERT flags a venture that is divergent (past simulation-OK, real build never started) AND stalled beyond its resolved clock — investigate/escalate: start the real build or record a gating decision; QUIET_TICK_BOARD_STALE flags manual PM-board child item(s) that sat untouched past 7 days or whose review_by passed (QF-20260830-690, the chairman found ten stale before the seat did) — a HARD interrupt: groom each named item now (close/route/re-date/fence it, touching status or blocker clears the line) via `node scripts/adam-pm-board.mjs` (prints the review_by column); QUIET_TICK_INBOX_CAP means the inbox fetch hit its cap — more rows may remain beyond this tick, re-run the drain; QUIET_TICK_INBOX_BACKLOG_BREACH means unacked inbound rows are past their breach threshold EVEN IF the surfaced list above looks short — that mismatch is the witnessed incident shape (21 unacked behind a displayed "1-5"), so drain the FULL lane with `node scripts/adam-advisory.cjs inbox`, which applies no correlation filter; QUIET_TICK_SMS_INBOUND flags an undrained chairman SMS reply — a HARD interrupt: drain + reply per the CHAIRMAN SMS CHANNEL DUTY, `node scripts/sms-relay-drain.cjs`; QUIET_TICK_SMS_PARKED flags a PARKED chairman SMS (already terminal-drained as no_match/rate_limited on the verified chairman number, so it will never show up as QUIET_TICK_SMS_INBOUND) — a HARD interrupt distinct from that one: address it, then explicitly disposition it with `node scripts/resolve-parked-chairman-sms.cjs <id>` (the id is printed in the line) or it re-fires every tick; QUIET_TICK_RATIFICATION_STALE flags a chairman ratification unencoded >=24h (SD-LEO-INFRA-CHAIRMAN-RATIFICATION-LEDGER-001) — a HARD interrupt: encode the ratified contract change and call lib/chairman/ratification-writer.mjs markRatificationEncoded() to clear it, or it re-fires every tick; QUIET_TICK_RATIFICATION_MARKER_INVALID is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260901-107): it flags an already-encoded row whose marker_text was never actually present in the live section content at encode time (a writer defect predating markRatificationEncoded()\'s fail-closed check, not a reverted clause) — re-encoding cannot fix it; correction rides a separate chairman-gated data-repair migration outside your reach, so a tick whose only output is this line is STILL a NO-OP. QUIET_TICK_RATIFICATION_CONTRACT_UNVERIFIED is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (SD-LEO-ORCH-CAPA-CONTRACT-TRUTH-001-B), for the same reason as the MARKER_INVALID sibling directly above: it flags an already-encoded row whose ruling NAMES target contracts that do not carry marker_text at the encode-time pin, meaning a ruling encoded at only SOME of the contracts it claimed. That is a historical shortfall, NOT a reverted clause, and it is unrepairable where you stand: the append-only freeze trigger on the ledger permits only the NULL-to-set transition, so re-encoding these rows is rejected outright and correction rides the same chairman-gated data-repair path. A tick whose only output is this line is STILL a NO-OP. EXPECTED VOLUME IS A MOVING TARGET, NOT A FIXED NUMBER -- do not treat any hard-coded count here as current. Originally measured 2026-09-03 at 21 rows of 53 (.artifacts/testing/quiet-tick-multi-target-dryrun.json); SD-LEO-INFRA-RATIFICATION-ENCODE-VERIFICATION-001 re-measured 2026-09-11 at 64 of 96 rows carrying a manifest_hash that is not a git object (up from 24 of 53), and that same SD corrected markRatificationEncoded\'s own pin-resolution call (it previously omitted relPath/encoded_at, making its approximate pin tier unreachable), which changes the true pin-resolvable count again going forward. Re-measure live via node scripts/adam-quiet-tick.mjs rather than trusting this sentence\'s number; a growing or shifting count on its own is expected drift, not a regression. QUIET_TICK_RATIFICATION_CONTRACT_UNPINNABLE_BACKLOG is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260912-125): it is a standing count of encoded rulings naming target contracts whose commit pin could not be derived at all, so the CONTRACT_UNVERIFIED check directly above never ran for them at all — these rows are unchecked-until-repaired, not misses, and the count exists so that population is visible rather than silently absent from every report. No action is needed beyond awareness. A tick whose only output is this line is STILL a NO-OP. QUIET_TICK_SMS_PARKED_STALE is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (SD-LEO-INFRA-PARKED-CHAIRMAN-SMS-001): it never fires without its sibling QUIET_TICK_SMS_PARKED already in the gate on the same row, so it adds no new interrupt — it only distinguishes a parked row that has sat unresolved for >=24h from routine parked traffic, so a genuinely-overdue row is grep-able instead of indistinguishable noise; when you see it, prioritize disposition of that row first; QUIET_TICK_OVERSIGHT_OVERDUE / QUIET_TICK_SELFSCORE_OVERDUE flag a lost/failing deliberate-check cron — run the named check NOW per the line, QF-20260719-825; QUIET_TICK_HEARTBEAT_OVERDUE means the measured gap since the last kind=\'heartbeat_status\' send has reached >=55min (the durable hourly cron only checks this once/hour at :14, so a send just after :14 could otherwise slip to ~90min before the next check — QF-20260823-131) — send the hourly heartbeat NOW per the line (`node scripts/adam-chairman-sms.mjs --kind heartbeat_status --body "<short status line>"`, or stay silent this send if there is truly nothing plan-relevant to report; quiet hours/rate caps are enforced by the send gate itself); QUIET_TICK_HEARTBEAT_SUPPRESSED is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260906-942), mirroring QUIET_TICK_STALL_SUPPRESSED / QUIET_TICK_SMS_SUPPRESSED: it records that the heartbeat cadence is breached but the send gate would drop the send anyway (within the 22:00-06:00 ET quiet window), so no actionable instruction is printed. A tick whose only output is this line is STILL a NO-OP. Its actionable sibling is QUIET_TICK_HEARTBEAT_OVERDUE, in the gate above. QUIET_TICK_STANDING_PRIORITY_UNSERVED means a standing priority is SET and nothing is routed into it — the tick is NOT a no-op even if everything else is quiet: route work to the priority or clear it deliberately, never leave it set and unserved, SD-LEO-INFRA-ADAM-DURABLE-STANDING-001); QUIET_TICK_CHAIRMAN_GATED_STAMP_ERROR flags a failed metadata stamp or record insert in the chairman-gated-SD decision-row guard (SD-LEO-INFRA-CHAIRMAN-GATED-SD-DECISION-ROW-GUARD-001) — a HARD interrupt: investigate the named sd_key/error (the SD stays selectable next tick, but a recurring stamp error means the guard itself is stuck), then arm the wakeup. QUIET_TICK_IDLE_BESIDE_CLAIMABLE flags live-idle worker seat(s) sitting beside claimable draft SD(s) on the belt at the same time (QF-20260828-922, witnessed 08-28: the chairman caught this before any role did) — a HARD interrupt: dispatch the idle capacity now via `node scripts/worker-checkin.cjs`, or investigate why it is not self-claiming, then arm the wakeup. QUIET_TICK_VENTURE_TERMINAL_FLIP flags a venture that was under active watch and flipped to cancelled/killed by a writer that is NOT a chairman-approved kill/cancel/sunset decision (QF-20260829-232, witnessed AltifyAI 2026-08-29 10:44Z — a rejected veto\'s own reasoning was mechanically written in as the kill_reason) — a HARD interrupt: investigate who/what flipped the status and whether it was legitimate, escalate to the chairman if not. QUIET_TICK_CHAIRMAN_GATED_UNSURFACED=n is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above, mirroring QUIET_TICK_STALL_SUPPRESSED / QUIET_TICK_SMS_SUPPRESSED: it confirms n chairman-gated SDs were found and auto-recorded this tick (the decision itself reaches you through the normal chairman decision channel, not through this tick) — a tick whose only output is this line is STILL a NO-OP; waking you to be told the guard is working as designed is the alert fatigue QF-20260725-638 removed. Its actionable sibling is QUIET_TICK_CHAIRMAN_GATED_STAMP_ERROR, in the gate above. QUIET_TICK_CLASSIFIER_DENIAL_GUARD=n is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260906-881), mirroring QUIET_TICK_CHAIRMAN_GATED_UNSURFACED: it confirms n ungated migration-apply WAIT verdicts / classifier-denied worker signals were found and auto-recorded as chairman_approval rows this tick (lib/chairman/classifier-denial-guard.mjs) — the decision reaches you through the normal chairman decision channel (chairman-decisions.mjs list / exec-summary NEEDS-YOU), not through this tick. A tick whose only output is this line is STILL a NO-OP. Its actionable sibling is QUIET_TICK_CLASSIFIER_DENIAL_STAMP_ERROR, in the gate above (a failed record/stamp — investigate the named sd_key/error). QUIET_TICK_STANDING_PRIORITY (status=served) is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (SD-LEO-INFRA-ADAM-DURABLE-STANDING-001): it records that the standing priority IS being served — work is routed into it — which is the healthy state. Waking you to be told nothing is wrong is the alert fatigue QF-20260725-638 removed, so a tick whose only output is a served-priority line is STILL a NO-OP. Only its sibling QUIET_TICK_STANDING_PRIORITY_UNSERVED is actionable. QUIET_TICK_STALL_SUPPRESSED is INFORMATIONAL ONLY and is likewise deliberately absent from the NO-OP gate above (QF-20260725-639): it records that a ledger row was EXCLUDED from the stall alarm because it is not work — a parent-tier ANCHOR (a forward-list/rollup entry that never advances by design; the child carries the work) or an advisory_thread comms row. A tick whose only output is stall-suppression lines is STILL a NO-OP. Read it only when auditing why a known board row is quiet. QUIET_TICK_VENTURE_PARK_SUPPRESSED is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above: it records that a venture was EXCLUDED from the stall alarm because a gating decision deliberately parks it (audit trail so the exclusion is never silent). A tick whose only output is park-suppression lines is STILL a NO-OP — treating it as actionable would wake you to be told nothing is wrong, rebuilding the alert fatigue QF-20260725-638 removed. Read it only when auditing why a known-parked venture is quiet, or when its recorded unpark trigger has come due. QUIET_TICK_SMS_SUPPRESSED is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260808-834 + the QF-20260808-673 count-vs-surface class): it records an undrained chairman SMS that was NOT raised as a hard interrupt because SMS_RELAY_DRAIN_ENABLED is unset, so the remediation it would point at (scripts/sms-relay-drain.cjs) is inert. It exists so `sms=N` in the summary can never coexist with zero surfaced rows — the mismatch that would let a reader treat the tick as a NO-OP and miss the chairman. DO NOT ADD IT TO THE ALLOWLIST: allowlisting it re-arms an interrupt toward a remediation nobody can perform, which is the alert-fatigue loop QF-20260808-834 removed. Its actionable sibling is QUIET_TICK_SMS_INBOUND, emitted once the drain flag is set. QUIET_TICK_SMS_WATCHDOG is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260810-590): it records an undrained inbound whose body exactly matches the canonical automated are-you-still-there watchdog text, not a genuine chairman message — the remediation is checking your own outbound cadence, never replying to it as if it were the chairman (that reply burns the measured-presence quiet-hours window on a bot). Any body that does not exactly match still falls through to QUIET_TICK_SMS_INBOUND / QUIET_TICK_SMS_SUPPRESSED unchanged (fail-toward-chairman on ambiguous bodies). QUIET_TICK_HASH_SKIP is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260829-373, A7 burn-lever): it records that this tick\'s cheap state hash (inbox max row id + claims count + sms undrained count + escalation count) matched the prior tick\'s, so the heavy task_ledger/venture/ratification-regression enumerations were skipped — every hard-interrupt surface above (inbox directives, chairman SMS, etc.) still ran unconditionally, so nothing actionable is ever hidden behind this line. A tick whose only output is this line is STILL a NO-OP. QUIET_TICK_COMPLETION_READY_PARENT flags a completion-ready orchestrator parent (all children terminal, PLAN-TO-LEAD accepted, unclaimed, no accepted LEAD-FINAL yet) that has sat >=2h without a parent_completion WORK_ASSIGNMENT — ~10 minutes of work invisible to dispatchable_backlog_size by construction (QF-20260830-933, live specimen SD-LEO-FEAT-MEDIA-PRODUCTION-CAPABILITY-001 sat 28h unowned) — a HARD interrupt: dispatch the named SD(s) now (`node scripts/run-parent-completion.mjs <SD-KEY>`, worker-executed only), then arm the wakeup. QUIET_TICK_COMPLETION_READY_PARENT_ERROR means the completion-ready-parents probe itself failed (advisory KPI, non-fatal to the tick) — investigate the named error; it does not by itself indicate a stuck parent. QUIET_TICK_COMPLETION_READY_PARENT_HELD is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above (QF-20260901-357): it records completion-ready orchestrator parent(s) EXCLUDED from the interrupt above because they carry a documented dispatch hold (metadata.needs_coordinator_review or a still-non-terminal metadata.blocked_by_sd_key) — no action needed, the hold itself is the authorization state. A tick whose only output is this line is STILL a NO-OP. QUIET_TICK_CRON_PARITY_MISSING flags a required loop (ADAM_LOOPS, the SAME predicate the startup check uses — never a second list) absent from the armed set THIS TICK, not just at the last /adam startup (QF-20260830-590: a cron lost mid-session to 7-day harness expiry, CronDelete, or a restart was previously silent until the next startup) — a HARD interrupt: re-arm the named key(s) via CronCreate (see `node scripts/adam-startup-check.mjs` for the exact spec), then arm the wakeup. Silent (never printed) when no armed set was supplied to this tick — advisory only, matching the startup check\'s own tolerance. QUIET_TICK_CRON_PARITY_ERROR means the parity check itself failed (non-fatal) — investigate the named error. QUIET_TICK_PARENT_ROLLUP_SYNC_ERROR (QF-20260912-408) means syncParentRollupStatus (lib/adam/task-ledger.js) failed to persist a rolled-up parent status this tick — a HARD interrupt: investigate the named detail (a DB error, not a stuck parent by itself); the tick falls through to the pre-fix, possibly-stale read rather than aborting. QUIET_TICK_OUTPUT_FLOW_STALL flags origin/main HEAD not advancing for the stated hours while the fleet is active (SD-LEO-INFRA-ACTIVATE-INERT-STALL-001-C, RCA 9a02a76d: the 2026-09-01 14.6h stall read "2 active workers, 1 build" all night with zero merges, invisible to every input/liveness gauge above) — a HARD interrupt: check for a blocked reaper/merge chain (`node scripts/safe-root-resync.mjs`), then arm the wakeup. Never fires during a legitimately quiescent fleet. QUIET_TICK_DURATION_BASELINE_BREACH flags an in-flight SD running past its sd_type\'s historical p95 completion duration (chairman-specified 2026-09-01) — a FLAG for worker-health investigation, never a verdict: check on the named sd_key; `escalated=true` means this is the 2nd consecutive tick past baseline (Solomon-visible) and warrants closer attention, but neither tier auto-kills or auto-fails the item. Never fires on a type with too few historical samples to trust. QUIET_TICK_PROBE_TIMEOUT flags a per-probe wall-clock budget exceeded (QF-20260911-888, five consecutive ticks hung past a 150s external timeout on 2026-09-11 post-quota-freeze) — a HARD interrupt: the named probe likely hit an unbounded query or a DB lock wait; investigate that probe\'s query directly (the tick itself already recovered and continued past it, since the timeout folds into the probe\'s own fail-soft error path). QUIET_TICK_PROBE_ELAPSED is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above: it is diagnostic per-probe timing, printed to stderr only under --verbose (never present in normal cron output), so its absence from a normal tick is expected, not a regression. QUIET_TICK_VENTURE_DEPLOY_RED flags a venture\'s deploy-workflow run with a red step naming no secret/permission (QF-20260912-924: rickfelix/altifyai\'s deploy.yml was red on every run 09-06 through 09-11, unseen for six days because adam-github-assessment.mjs is hard-scoped to one repo and flag-gated off, and synthetic-actor-guard.js only reads its named UAT step at stage-advance time) — a HARD interrupt: source a venture QF for the named repo/run/step, then arm the wakeup. QUIET_TICK_VENTURE_DEPLOY_KEYSTROKE is INFORMATIONAL ONLY and is deliberately absent from the NO-OP gate above, mirroring QUIET_TICK_CHAIRMAN_GATED_UNSURFACED/QUIET_TICK_CLASSIFIER_DENIAL_GUARD: it confirms a credential-class red deploy step (the ::error:: text itself named the missing secret/permission) was found and auto-recorded as a durable chairman keystroke this tick (lib/adam/venture-deploy-watcher.mjs) — the decision reaches you through the normal chairman decision channel, not through this tick. A tick whose only output is this line is STILL a NO-OP. QUIET_TICK_VENTURE_DEPLOY_ERROR means the venture-deploy watcher itself failed for one venture (unparseable repo_url, a gh/API read failure) — advisory, non-fatal to the tick and never blocking another venture\'s read; investigate the named repo/error.',
  },
  {
    key: 'governance-scan',
  ...
  {
    // QF-20260905-121 (Solomon audit #7 item 2, 0df6a79f; ratification 63ff6ef2 explicitly
    // RETAINED the 21:30 ET bandwidth-forecast duty, but it had NO durable trigger — it fired
    // only when a session happened to remember at 01:30Z, which two consecutive sessions did
    // not (measured 2026-09-05: no forecast row in sms_outbound_obligations on either 09-03 or
    // 09-04). Same session-fragile class CLAUDE_ADAM.md 5i exists to close, one duty short of
    // parity. ACCEPTANCE (Solomon STEP-0 reply d5df599d 2026-09-05): the silent path must ALSO
    // leave a durable trace — a silent tick is otherwise indistinguishable from a dead one.
    key: 'bandwidth-forecast',
    label: '21:30 ET presleep bandwidth-capacity-forecast SMS (silence-by-default; durable trace on BOTH paths)',
    script: 'account-usage-paste-projection.mjs',
    cron: '30 21 * * *', // 21:30 local (ET) — the retained duty\'s own slot (CLAUDE_ADAM.md 5g c3)
    prompt: 'Adam bandwidth-forecast tick: run `node scripts/account-usage-paste-projection.mjs` for the active account. Query the `feedback` table for the most recent row with category=\'adam_duty_log\' AND metadata.duty=\'bandwidth_forecast\' (order by created_at desc, limit 1) to find the last-sent-forecast timestamp — never decide from session memory of when you last sent; a session restart loses that memory entirely, which is exactly how this duty went dark for two nights running. If the projection reflects a usage paste NEWER than that stamp (a genuinely new /usage paste since the last send): compose ONE terse capacity-projection line and send it via `node scripts/adam-chairman-sms.mjs --kind bandwidth_forecast --body "<line>"` (quiet hours 22:00-06:00 ET and rate caps are enforced by the send gate itself — 21:30 is inside the awake window — never skip the call to pre-empt them), THEN record the durable marker below with silent:false. If there is NO new paste since that stamp: stay SILENT on SMS (silence-by-default, CLAUDE_ADAM.md 5g c3) but ALWAYS still call emitFeedback({supabase, title:"bandwidth-forecast: no new usage paste", description:`no new usage paste since ${lastPasteIso}`, severity:"low", category:"adam_duty_log", source_type:"manual_feedback", dedup_key:`bandwidth-forecast-<YYYY-MM-DD ET>`, metadata:{duty:"bandwidth_forecast", silent:true, last_paste_iso:lastPasteIso}}) from lib/governance/emit-feedback.js. This durable trace is NOT optional cosmetic logging — it is the QF-20260905-121 acceptance criterion itself: it is what lets a daily duty-firing audit tell fired-quiet from absent instead of two indistinguishable silences.',
  },
];
