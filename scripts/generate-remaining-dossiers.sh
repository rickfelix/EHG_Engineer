...
gen_file_06() {
  local STAGE=$1
  local TITLE=$2
  cat > "./docs/workflow/dossiers/stage-${STAGE}/06_agent-orchestration.md" << EOF
...
EOF
  ...
  cat >> "./docs/workflow/dossiers/stage-${STAGE}/06_agent-orchestration.md" << 'EOF2'
```
...
gen_file_07() {
  ...
  cat > "./docs/workflow/dossiers/stage-${STAGE}/07_recursion-blueprint.md" << EOF
...
gen_file_08() {
  local STAGE=$1
  cat > "./docs/workflow/dossiers/stage-${STAGE}/08_configurability-matrix.md" << EOF
...
gen_file_09() {
  ...
  local M2=$3
  local M3=$4
  cat > "./docs/workflow/dossiers/stage-${STAGE}/09_metrics-monitoring.md" << EOF
...
gen_file_10() {
  ...
  local SCORE=$2
  local NEWSD=$3
  cat > "./docs/workflow/dossiers/stage-${STAGE}/10_gaps-backlog.md" << EOF
...
mkdir -p ./docs/workflow/dossiers/stage-21

# Copy and adapt Stage 20 files for Stage 21
for i in {1..11}; do
  NUM=$(printf "%02d" $i)
  if [ -f "./docs/workflow/dossiers/stage-20/${NUM}_"*.md ]; then
    FILENAME=$(ls ./docs/workflow/dossiers/stage-20/${NUM}_*.md | head -1 | xargs basename)
    sed "s/Stage 20/Stage 21/g; s/stage-20/stage-21/g; s/stage_20/stage_21/g; s/Enhanced Context Loading/Final Pre-Flight Check/g; s/Context completeness/Checklist completion/g; s/Loading performance/Risk score/g; s/Memory efficiency/Readiness rating/g; s/CONTEXT/PREFLIGHT/g; s/SD-CONTEXT-OPTIMIZATION-001/SD-PREFLIGHT-AUTOMATION-001/g; s/873-918/919-964/g" \
      "./docs/workflow/dossiers/stage-20/${FILENAME}" \
      > "./docs/workflow/dossiers/stage-21/${FILENAME}"
  fi
done
...
for STAGE in 22 23 24; do
  mkdir -p ./docs/workflow/dossiers/stage-${STAGE}
  ...
  for i in {1..11}; do
    NUM=$(printf "%02d" $i)
    if [ -f "./docs/workflow/dossiers/stage-21/${NUM}_"*.md ]; then
      FILENAME=$(ls ./docs/workflow/dossiers/stage-21/${NUM}_*.md | head -1 | xargs basename)
      sed "s/Stage 21/Stage ${STAGE}/g; s/stage-21/stage-${STAGE}/g; s/stage_21/stage_${STAGE}/g; s/Final Pre-Flight Check/${TITLE}/g; s/Checklist completion/${M1}/g; s/Risk score/${M2}/g; s/Readiness rating/${M3}/g; s/PREFLIGHT/${TRIGGER}/g; s/SD-PREFLIGHT-AUTOMATION-001/${NEWSD}/g; s/919-964/${YAML}/g; s/depends_on: - 20/depends_on: - $((STAGE-1))/g" \
        "./docs/workflow/dossiers/stage-21/${FILENAME}" \
        > "./docs/workflow/dossiers/stage-${STAGE}/${FILENAME}"
    fi
  done
