...
async function updateQADirector() {
  console.log('🔄 Updating QA Engineering Director sub-agent in database...\n');

  try {
    // First, verify the sub-agent exists
    ...
    if (fetchError) {
      console.error('❌ Error fetching existing sub-agent:', fetchError);
      process.exit(1);
    }

    if (!existing) {
      console.error('❌ QA Engineering Director sub-agent not found (code: TESTING)');
      process.exit(1);
    }

    console.log('✅ Found existing sub-agent:');
    console.log(`   Name: ${existing.name}`);
    console.log(`   Code: ${existing.code}`);
    console.log(`   Priority: ${existing.priority}`);
    console.log(`   Columns: ${Object.keys(existing).join(', ')}`);
    console.log('');

    // Update the sub-agent (only fields that exist in the table)
    ...
    console.log('✅ Successfully updated QA Engineering Director sub-agent!');
    console.log('');
    console.log('📊 Updated fields:');
    console.log('   - description: Updated to Testing-First Edition (comprehensive E2E)');
    console.log('   - capabilities: Updated with 10 core capabilities');
    ...
    console.log('   2. Review updated CLAUDE.md to verify changes');
    console.log('   3. Update qa-engineering-director-enhanced.js script to match new workflow');
    console.log('');
