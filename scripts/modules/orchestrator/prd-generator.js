/**
 * PRD Generator for Orchestrator
 *
 * Functions for generating Product Requirements Documents during LEO Protocol execution.
 * Supports both standard SDs and consolidated SDs (with backlog items).
 *
 ...
 */
export async function isConsolidatedSD(sdId, supabase) {
 ...
 */
export async function fetchBacklogItems(sdId, supabase) {
  ...
  if (error) {
    console.log(chalk.yellow(`   No backlog items found for ${sdId}`));
    return [];
  }
 ...
 *
 * @param {Array} backlogItems - Array of backlog items
 * @returns {Object} - Evidence mapping by backlog ID
 */
...
export function generateUserStoriesFromBacklog(backlogItems, sd) {
  ...
  if (stories.length !== backlogItems.length) {
    console.log(chalk.yellow(`   Story count mismatch: ${stories.length} stories from ${backlogItems.length} items`));
  }
 ...
 *
 * @returns {Array<string>} - Default acceptance criteria
 */
 ...
 */
export async function generatePRD(sd, supabase) {
  const prdId = `PRD-${sd.id}-${Date.now()}`;

  // Check if this is a consolidated SD
  const isConsolidated = await isConsolidatedSD(sd.id, supabase);
  let backlogItems = [];

  if (isConsolidated) {
    console.log(chalk.cyan('   Detected consolidated SD - fetching backlog items...'));
    backlogItems = await fetchBacklogItems(sd.id, supabase);
  ...
  if (isConsolidated) {
    ...
    if (expectedCount > 0 && backlogItems.length !== expectedCount) {
      console.log(chalk.yellow(`   Item count mismatch: Found ${backlogItems.length}, expected ${expectedCount}`));
    }
  }
