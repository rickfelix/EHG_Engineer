...
class CLAUDEMDGeneratorV3 {
  ...
  getFileSpecs(digestMetadata) {
    const specs = [
      ...
      // SD-LEO-INFRA-STATIC-PREFIX-DIET-001 (burn-lever A4): the reference companion that starts
      // clawing CLAUDE_CORE.md back toward the Read tool's 25k single-call cap. Rules, prohibitions
      // and every behavioral clause stay in the gated file — this carries pure reference/procedure.
      ['CLAUDE_CORE_MANUAL.md', (d) => generateCoreManual(d, this.fileMapping), 'full'],
      // SD-LEO-FIX-CLAUDE-ADAM-SPLIT-001 (FR-4): the dated-rationale companion for CORE, in the
      // Adam/Solomon PROVENANCE shape — carries the WHY carved out of rule sections, never a rule.
      ['CLAUDE_CORE_PROVENANCE.md', (d) => generateCoreProvenance(d, this.fileMapping), 'full'],
      ...
      ['CLAUDE_PLAN_MANUAL.md', (d) => generatePlanManual(d, this.fileMapping), 'full'],
      ['CLAUDE_EXEC.md', (d) => generateExec(d, this.fileMapping), 'full'],
      // SD-LEO-FIX-CLAUDE-ADAM-SPLIT-001 (FR-3): EXEC was the only phase file with no companion of
      // either kind at 43,069 tokens. The manual carries reference/procedure (allow-listed in the
      // mapping) plus the generated schema-constraints and process-scripts blocks; the provenance
      // carries the retrospective evidence behind the rules. Rules stay in CLAUDE_EXEC.md.
      ['CLAUDE_EXEC_MANUAL.md', (d) => generateExecManual(d, this.fileMapping), 'full'],
      ['CLAUDE_EXEC_PROVENANCE.md', (d) => generateExecProvenance(d, this.fileMapping), 'full'],
...
// CLAUDE_EXEC.md added by SD-LEO-FIX-CLAUDE-ADAM-SPLIT-001 (FR-3) — that SD landed: the split to
// CLAUDE_EXEC_MANUAL.md / CLAUDE_EXEC_PROVENANCE.md took it from 43,069 to ~19,000 tokens.
// CLAUDE_CORE.md and CLAUDE_ADAM.md are deliberately NOT here: CORE measures fits:null (23,851 —
// under the raw cap but inside the marginal band, FR-4's stated target) and ADAM is fits:false
// pending a section-choice ruling on a binding companion; adding either would wedge every encode.
export const MUST_FIT_SINGLE_READ = ['CLAUDE_LEAD.md', 'CLAUDE_PLAN.md', 'CLAUDE_SOLOMON.md', 'CLAUDE_MICHAEL.md', 'CLAUDE_EXEC.md'];
...
// CLAUDE_EXEC.md ADDED by SD-LEO-FIX-CLAUDE-ADAM-SPLIT-001 (FR-3): measured ~19,024 tokens after the
// companion split (was 43,069), comfortably under the confirmed-fit threshold, so the SD that makes
// it fit has landed and this list's own rule says to add it. CLAUDE_CORE.md (FR-4) is NOT added: it
// measures 23,851 — under the raw cap but inside the marginal band — so the instrument cannot yet
// confirm it fits, and adding it would throw SINGLE_READ_FIT_UNCONFIRMED on the next regeneration.
export const MUST_CONFIRM_SINGLE_READ_FIT = ['CLAUDE_LEAD.md', 'CLAUDE_EXEC.md'];
...
export const KNOWN_GENERATED_FILES = [
  'CLAUDE.md', 'CLAUDE_CORE.md', 'CLAUDE_CORE_MANUAL.md', 'CLAUDE_CORE_PROVENANCE.md', 'CLAUDE_LEAD.md', 'CLAUDE_LEAD_MANUAL.md', 'CLAUDE_PLAN.md', 'CLAUDE_PLAN_MANUAL.md', 'CLAUDE_EXEC.md', 'CLAUDE_EXEC_MANUAL.md', 'CLAUDE_EXEC_PROVENANCE.md', 'CLAUDE_ADAM.md', 'CLAUDE_ADAM_MANUAL.md', 'CLAUDE_ADAM_PROVENANCE.md', 'CLAUDE_COORDINATOR.md', 'CLAUDE_COORDINATOR_MANUAL.md', 'CLAUDE_COORDINATOR_PROVENANCE.md', 'CLAUDE_SOLOMON.md', 'CLAUDE_SOLOMON_MANUAL.md', 'CLAUDE_SOLOMON_PROVENANCE.md', 'CLAUDE_SOLOMON_MODEL_POSTURE.md', 'CLAUDE_MICHAEL.md', 'CLAUDE_MICHAEL_MODEL_POSTURE.md',
