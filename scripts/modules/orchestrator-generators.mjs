/**
 * Orchestrator Generators Module
 ...
 *
 * Contains PRD and user story generation methods.
 * @module OrchestratorGenerators
 * @version 1.0.0
 */
...
// =============================================================================
// CONSOLIDATED SD HELPERS
// =============================================================================

/**
 * Check if SD is a consolidated SD (multiple backlog items)
 * @param {Object} supabase - Supabase client
 * @param {string} sdId - Strategic Directive ID
 * @returns {Promise<boolean>} True if consolidated SD
 */
export async function isConsolidatedSD(supabase, sdId) {
  const { data: sd } = await supabase
    .from('strategic_directives_v2')
    .select('consolidation_type, metadata')
    .eq('id', sdId)
    .single();

  if (!sd) return false;

  // Check for consolidation type or metadata indicators
  const isConsolidated = sd.consolidation_type === 'full_backlog' ||
                        sd.consolidation_type === 'filtered_backlog' ||
                        (sd.metadata && sd.metadata.consolidated_from);

  return isConsolidated;
}

/**
 * Fetch backlog items for a consolidated SD
 * @param {Object} supabase - Supabase client
 * @param {string} sdId - Strategic Directive ID
...
export async function fetchBacklogItems(supabase, sdId) {
  ...
  if (error) {
    console.log(chalk.yellow(`Warning: No backlog items found for ${sdId}`));
    return [];
  }
...
export async function generatePRD(supabase, sd) {
  ...
  if (consolidated) {
    ...
    if (expectedCount > 0 && backlogItems.length !== expectedCount) {
      console.log(chalk.yellow(`Warning: Item count mismatch: Found ${backlogItems.length}, expected ${expectedCount}`));
    }
  }
  ...
  return {
    ...
    metadata: {
      ...
      sd_priority: sd.priority || 'medium',
      is_consolidated: consolidated,
      backlog_items: consolidated ? backlogItems.map(i => i.backlog_id) : []
    }
  };
}
 ...
 * @param {Array} backlogItems - Backlog items array
...
export function generateUserStoriesFromBacklog(backlogItems, sd) {
  ...
  if (stories.length !== backlogItems.length) {
    console.log(chalk.yellow(`Warning: Story count mismatch: ${stories.length} stories from ${backlogItems.length} items`));
  }
 ...
 * @param {Object} _sd - Strategic Directive (currently unused)
 * @returns {Array<string>} Acceptance criteria
 */
