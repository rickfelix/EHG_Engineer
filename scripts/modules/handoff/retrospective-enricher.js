...
export async function enrichRetrospectivePreGate(supabase, sdId, sd) {
  ...
  // SD-LEO-INFRA-BACKEND-WRITE-SAFETY-001 (formerly SD-FDBK-INFRA-HANDOFF-RETRO-GENERATORS-001, cancelled) (FR-3): consult shared
  // retro-clobber-guard. Replaces the prior inline guards (3 cases:
  // explicit-manual, rich-existing-content, generated_by-null-with-rich-content
  // — the last was the actual leak path for witness retro 84ada45e).
  const { isSafeToWriteRetro } = await import('./lib/retro-clobber-guard.js');
  const guard = await isSafeToWriteRetro(supabase, sdId);
  if (!guard.safe) {
    console.warn(`[ENFORCE] skipped retro enrichment for sdId=${sdId} reason=${guard.reason}`);
    return result;
  }
