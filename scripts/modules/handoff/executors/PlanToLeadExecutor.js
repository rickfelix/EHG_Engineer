...
export class PlanToLeadExecutor extends BaseExecutor {
  ...
  getRequiredGates(sd, options) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // US-002: USER STORY EXISTENCE GATE (SD-LEO-COMPLETION-GATES-001)
    // ═══════════════════════════════════════════════════════════════════════════
    // Root Cause: SD-STAGE-ARCH-001-P10 completed without fulfilling US-003
    // (creating follow-up SDs in database). User stories existed but weren't validated.
    //
    // This gate ensures:
    // 1. If SD type requires user stories (requires_user_stories=true), they MUST exist
    // 2. Shift-left: Catch missing stories at PLAN-TO-LEAD, not at completion
    // 3. Prevents "Silent Success" anti-pattern
    // ═══════════════════════════════════════════════════════════════════════════
    gates.push({
      ...
      validator: async (ctx) => {
        ...
        // For PLAN-TO-LEAD, we just need stories to EXIST
        // The actual validation happens via calculate_sd_progress
        console.log('   ✅ User stories exist - gate passed');

        // Warning if stories are not complete
        const warnings = [];
