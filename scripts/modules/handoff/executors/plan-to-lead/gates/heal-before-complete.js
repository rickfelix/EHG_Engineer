...
export function createHealBeforeCompleteGate(supabase) {
  return {
    ...
    validator: async (ctx) => {
      ...
      let threshold = await loadHealThreshold(supabase);
      ...
      try {
        ...
        if (sdRecord?.metadata?.vision_origin_score_id) {
          ...
          console.log(`   📏 Using GRADE.A (${GRADE.A}) threshold instead of default`);
        }
      } catch (err) {
        ...
        console.log(`   📏 Using standard threshold (${threshold})`);
      }
