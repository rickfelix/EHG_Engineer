...
export const STANDARD_LOOPS = [
  ...
  // SD-LEO-INFRA-SILENT-HOLDER-AUDIT-001: this duty is prompt-only (script:null), so the prompt IS
  // the contract — pinned by tests/unit/silent-holder-visibility.test.js against the column names
  // and discriminator phrases below. The prior text enumerated sd_key claims only (the MIRROR), so
  // QF holders were invisible to it by instruction (measured: a 4-day frozen QF holder), and its
  // signal-recency clock cleared a seat whose last signal WAS its last tool call.
  { key: 'silent-holder-audit', label: 'Silent-claim-holder audit — status-or-release on frozen holders from BOTH claim tables', script: null, cron: '23 * * * *',
    prompt: 'Silent-holder audit: enumerate holders from BOTH authoritative claim tables — strategic_directives_v2.claiming_session_id AND quick_fixes.claiming_session_id (claude_sessions.sd_key is only a MIRROR of a past claim decision and is NULL for QF holders; never enumerate from it). For each holder, discriminate silence by RAW last_tool_at compared across TWO samples a few minutes apart — byte-identical raw timestamps = frozen; an advancing clock = working. Admissible evidence rule (two-sided): a field counts only if it re-stamps on healthy seats AND stops on frozen ones — last_tool_at passes; heartbeat_at, wake_armed_at/expected_wake_at, wind_down and window_owner_pid all fail it, do not consult them. Judge work product BY PHASE: commits/PR at EXEC, product_requirements_v2 rows at PLAN, handoff evidence at LEAD — a commits test at PLAN misreads normal state as abandonment. For each frozen holder with no phase-appropriate work product, send ONE status-or-release coordinator_request via the dispatch choke (skip if an unanswered one is pending), and note the revival class: frozen with FRESH heartbeat = nudgeable (paste a loop prompt); frozen with DEAD heartbeat = needs a fresh session. Release decisions stay with the sweep; this loop only asks.' },
