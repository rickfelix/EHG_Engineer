...
export const SOLOMON_LOOPS = [
  ...
  },
  {
    ...
    prompt: `Solomon deep-sweep tick: (1) FIRST enforce the per-sweep task_budget at ENTRY, BEFORE any Read/Grep; if refused, STOP and SAY SO. YOU are the enforcer here - nothing calls enforceSweepBudget for you, so skipping this step silently removes the budget entirely. THE EXTENT IS PER-SWEEP AND ELAPSED IS DERIVED FOR YOU (QF-20260903-418): pass the epoch-ms at which THIS SWEEP started and the answers/tokens spent DURING THIS SWEEP - at sweep entry those are the sweep-start timestamp and ZERO, NOT your day-scoped or seat-lifetime totals. Feeding a day-scoped number into a per-sweep ceiling is exactly what took four consecutive ticks dark (09-02 04:28Z count 26/5, 09-02 10:26Z count 96/5, 09-02 17:4xZ tokens 5.58M/200k, 09-03 04:37Z count 24/5) - every one of them actually in budget. The call CANNOT rubber-stamp any more: it FAILS CLOSED on a missing extent and REFUSES the retired count/elapsedMs/tokens keys, so an argument-less call now returns withinBudget:false with verdict:extent_unmeasurable instead of a false pass (QF-20260729-221). Run: node -e "const m=require('./scripts/solomon-advisory.cjs');console.log(JSON.stringify(m.enforceSweepBudget(null,{sweepStartedAtMs:<epoch ms when THIS sweep started>,answersThisSweep:<answers sent in THIS sweep>,tokensThisSweep:<tokens spent in THIS sweep>})))". If you cannot supply real sweep-scoped numbers, say so and STOP rather than guessing. A REFUSAL IS NEVER SILENT: when withinBudget is false, report the returned verdict and reason in this tick's output, so a refused sweep is DISTINGUISHABLE from a clean one that simply had nothing to say - silence-by-default covers findings, never budget refusals. (2) Resolve the sweep MODE at TICK TIME from the DESIGN-OF-RECORD policy (fail-safe to the live pin) and log it: node -e "import('./scripts/solomon-startup-check.mjs').then(m=>m.resolveSolomonSweepMode()).then(x=>process.stdout.write(x))". (3a) If mode===proactive (always-sweep policy of record, or a Fable pin fallback): pull ONE §4 backlog item, investigate the LIVE codebase with deep analysis, and surface EXACTLY ONE propose-only finding via node scripts/solomon-advisory.cjs send "<finding>" (dedup IS enforced; QUOTA IS MEASURED, NOT ENFORCED — nothing will stop you at the cap, so silence-by-default is YOURS to keep). (3b) Else (consult mode, default): drain the consult inbox and answer the highest-value open solomon_consult with deep analysis via node scripts/solomon-advisory.cjs send "<answer>" --reply-to <consult-correlation> (dedup IS enforced; QUOTA IS MEASURED, NOT ENFORCED — nothing will stop you at the cap). Propose, NEVER execute/build in either mode (CONST-002). DECISION_REQUESTED (SD-ALTIFYAI-LEO-FIX-SOLOMON-ADVICE-LEDGER-001): every advisory you send is admitted to the ledger as a workload someone must dispose of UNLESS you pass --informational. Add --informational when the send is a status update, an FYI finding with no requested action, or an answer that closes the loop with nothing further needed from the recipient. OMIT --informational (the default) when you are asking the recipient to actually decide something — accept, reject, or otherwise dispose of what you sent. When in doubt, omit it: a decision-requiring row wrongly marked informational disappears from the actionable queue silently, which is worse than an informational row wrongly left as a workload item.`,
  },
