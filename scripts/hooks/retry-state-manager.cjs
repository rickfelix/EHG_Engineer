...
// destructive primaries rather than dropping `find` outright, so the common read-only case
// (`find . -name x`) keeps its exemption.
//
// Adversarial review (PR #7185): the -delete/-exec[dir]/-ok[dir] set alone still missed
// find's -f* WRITE primaries — -fprint/-fprint0/-fls write to a named file argument, and
// -fprintf additionally accepts a caller-controlled format string — all with zero shell
// metacharacters, the exact same bypass class this QF exists to close. Added.
const READ_ONLY_LEADING_RE =
  /^(?:git\s+(?:status|log|diff|show|branch|rev-parse|describe|remote(?:\s+-v)?|config\s+--get|cat-file|ls-files|for-each-ref)\b|ls|ll|pwd|cat|head|tail|grep|rg|find(?![\s\S]*-(?:delete|execdir|exec|okdir|ok|fprintf|fprint0|fprint|fls)\b)|wc|stat|echo|whoami|date|env|printenv|which|type|node\s+--version|npm\s+(?:run\s+)?(?:ls|list|view|outdated)\b|gh\s+(?:run\s+(?:list|view|watch)|pr\s+(?:list|view|diff|checks|status)|workflow\s+(?:list|view)|issue\s+(?:list|view))\b|gh\s+api\b(?![\s\S]*(?:(?:^|\s)-X[=\s]*['"]?(?:POST|PUT|PATCH|DELETE)\b|--method[=\s]+['"]?(?:POST|PUT|PATCH|DELETE)\b|(?:^|\s)-[fF](?:\s|=)|--field\b|--raw-field\b|--input\b)))/i;
