...
const success_criteria = [
  ...
  { criterion: 'AC-6 (CI proof): the SD\'s own PR runs the now-blocking workflow green, and a throwaway commit adding execSync(`git ${x}`) to a backlog file turns it red (then is dropped).', measure: 'gh pr checks shows shell-injection-argv-lint pass on the PR head; the negative probe run URL is recorded in the PR body.' },
];
...
const smoke_test_steps = [
  ...
  { step_number: 3, instruction: 'On the same branch add a line execSync(`git ${x}`) to that file, commit, and re-run diff mode.', expected_outcome: 'Exit 1; exactly one new violation (the added line) is reported; the moved site is still pre-existing.' },
