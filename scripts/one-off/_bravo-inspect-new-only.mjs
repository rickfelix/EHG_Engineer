...
const PAGE=1000; let from=0; let data=[];
for(;;){ const {data:page,error}=await supabase.from('product_requirements_v2').select('id, sd_id, functional_requirements').range(from,from+PAGE-1); if(error) throw error; if(!page||page.length===0) break; data=data.concat(page); if(page.length<PAGE) break; from+=PAGE; }
...
for (const row of data) {
  ...
  for (const req of funcReqs) {
    ...
    if (whichPattern(newText) && !whichPattern(oldText)) {
      count++;
      console.log('---', count, row.id, req.id, '--- has req.requirement field?', 'requirement' in (req||{}), JSON.stringify(req.requirement||null).slice(0,80));
    }
  }
}
console.log('total NEW-only:', count);
