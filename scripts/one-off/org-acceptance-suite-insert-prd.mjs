#!/usr/bin/env node
/**
 * SD-LEO-INFRA-ORGANIZATION-ACCEPTANCE-SUITE-001 -- PRD insertion (inline-mode content).
 */
...
async function main() {
  ...
  const content = JSON.parse(readFileSync(new URL('./org-acceptance-suite-prd-content.json', import.meta.url), 'utf8'));

  const prdRow = {
    id: `PRD-${SD_KEY}`,
    sd_id: sdRow.id,
    title: 'Organization Acceptance Suite -- MAST-Seeded Fixtures, Pass/Catch Rate, Provenance',
    status: 'approved',
