...
// specified at each original site. Idempotent: guarded by the pointer markers. APPLIED 2026-09-04 13:1xZ.
...
async function save(supabase, id, before, after) {
...
export async function main() {
  ...
  const c611 = await load(supabase, 611);
  const c629 = await load(supabase, 629);
  let n611 = c611;
  let n629 = c629;
  const moved = [];
  ...
  await save(supabase, 611, c611, n611);
  await save(supabase, 629, c629, n629);
...
if (isMainModule(import.meta.url)) {
  main().catch((e) => { console.error('MOVE FAILED:', e.message); process.exit(1); });
}
