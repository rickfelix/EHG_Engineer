 ...
 * Version 2: Corrected schema (id instead of sd_id)
 *
...
async function main() {
  ...
  try {
    ...
    if (tableExists) {
      // Check schema for sub_agent_execution_results
      console.log('\n   Checking sub_agent_execution_results schema...');
      const schemaResult = await client.query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'sub_agent_execution_results'
        ORDER BY ordinal_position;
      `);
      const columns = schemaResult.rows.map(r => r.column_name);
      console.log(`   Columns: ${columns.join(', ')}`);

      // Determine the correct SD ID column name
      const sdColumnName = columns.includes('sd_id') ? 'sd_id' : 'strategic_directive_id';
        ...
        try {
          await client.query(`
            INSERT INTO sub_agent_execution_results (
              ${sdColumnName}, agent_type, phase, status, confidence_score,
              summary, recommendations, execution_timestamp
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
            ON CONFLICT (${sdColumnName}, agent_type, phase) DO NOTHING
          `, [
            SD_ID,
            agent.agent_type,
            agent.phase,
    ...
    if (deliverablesTableExists) {
      // Check schema for sd_deliverables
      console.log('\n   Checking sd_deliverables schema...');
      const delivSchemaResult = await client.query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'sd_deliverables'
        ORDER BY ordinal_position;
      `);
      const delivColumns = delivSchemaResult.rows.map(r => r.column_name);
      console.log(`   Columns: ${delivColumns.join(', ')}`);

      const delivSdColumnName = delivColumns.includes('sd_id') ? 'sd_id' : 'strategic_directive_id';
        ...
        try {
          await client.query(`
            INSERT INTO sd_deliverables (
              ${delivSdColumnName}, deliverable_type, file_path, description,
              lines_of_code, status, created_at
            ) VALUES ($1, $2, $3, $4, $5, $6, NOW())
            ON CONFLICT (${delivSdColumnName}, file_path) DO NOTHING
          `, [
            SD_ID,
    ...
    // Check if user_stories table exists and its schema
    const userStoriesCheck = await client.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = 'public'
    ...
    if (userStoriesCheck.rows[0].exists) {
      const userStoriesSchemaResult = await client.query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'user_stories'
        ORDER BY ordinal_position;
      `);
      const userStoryColumns = userStoriesSchemaResult.rows.map(r => r.column_name);
      const userStorySdColumn = userStoryColumns.includes('sd_id') ? 'sd_id' : 'strategic_directive_id';

      const storiesResult = await client.query(
        `SELECT * FROM user_stories WHERE ${userStorySdColumn} = $1`,
        [SD_ID]
      );
      ...
      console.log('   ⚠️  user_stories table does not exist');
    }
