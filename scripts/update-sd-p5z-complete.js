import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';
dotenv.config();

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
);

async function markP5ZComplete() {
  try {
    console.log('\n=== MARKING SD-2025-1013-P5Z AS COMPLETE ===\n');

    // Get current data
    const { data: currentSD, error: fetchError } = await supabase
      .from('strategic_directives_v2')
      .select('*')
      .eq('id', 'SD-2025-1013-P5Z')
      .single();

    if (fetchError) {
      console.error('Error fetching SD:', fetchError.message);
      return;
    }

    if (!currentSD) {
      console.error('SD-2025-1013-P5Z not found in database');
      return;
    }

    console.log('Current status:');
    console.log(`  Title: ${currentSD.title}`);
    console.log(`  Status: ${currentSD.status}`);
    console.log(`  Progress: ${currentSD.progress}%`);
    console.log(`  Phase: ${currentSD.phase}`);

    // Update to complete
    const updatedMetadata = {
      ...(currentSD.metadata || {}),
      completion_status: 'complete',
      completion_date: new Date().toISOString(),
      completed_work: 'Consolidated theme toggle - removed duplication',
      approved_by: 'LEAD'
    };

    const { data: updated, error: updateError } = await supabase
      .from('strategic_directives_v2')
      .update({
        progress: 100,
        metadata: updatedMetadata,
        updated_at: new Date().toISOString()
      })
      .eq('id', 'SD-2025-1013-P5Z')
      .select();

    if (updateError) {
      console.error('Error updating SD:', updateError.message);
      return;
    }

    console.log('\n✅ Successfully updated SD-2025-1013-P5Z:');
    console.log('  Progress: 100%');
    console.log('  Completion status: complete');
    console.log('  Completion date:', updatedMetadata.completion_date);

  } catch (err) {
    console.error('Failed to update SD:', err.message);
  }
}

markP5ZComplete();
