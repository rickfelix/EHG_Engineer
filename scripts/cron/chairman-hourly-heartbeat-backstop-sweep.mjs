...
// A single named constant, shared by both branches of the status-decision table (PLAN-phase
// TESTING sub-agent finding G4): "how long is a stuck live-owed row, or a failed backstop
// attempt, allowed to sit before the backstop treats the hour as still unfilled?" Also acts as
// the GHA-lag grace floor (finding G10): gives the live path right-of-first-refusal early in
// the hour rather than racing it on the very first tick.
...
/**
 * Reads the most recent obligation row for `kind` created within [startIso, endIso).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 */
async function fetchLatestRowForKind(supabase, kind, startIso, endIso) {
...
export async function main(argv = process.argv, deps = {}) {
  ...
  // sendChairmanSMS's rubric gate below, using the SAME chairmanZone just resolved.
  const { startIso, endIso, hourKey } = etHourWindowUtc(now, chairmanZone);
  ...
  const [{ row: liveRow, error: liveErr }, { row: backstopRow, error: backstopErr }] = await Promise.all([
    fetchRow(supabase, LIVE_KIND, startIso, endIso),
    fetchRow(supabase, BACKSTOP_KIND, startIso, endIso),
  ]);
  ...
  const message = { type: 'status', body, kind: BACKSTOP_KIND, dedupeKey };
  const context = { now, allowQuietHours, chairmanZone, recipientPhone };
