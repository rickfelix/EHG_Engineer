...
export async function main(argv = process.argv, deps = {}) {
  ...
  try {
    ...
    let released = 0, refused = 0, heldStill = 0, skipped = 0, auditWriteFailed = 0;
    const outcomes = [];
    for (const row of heldRows || []) {
      ...
      else if (outcome.action === 'hold') heldStill += 1;
    ...
    logger.log?.(`[chairman-held-sends-release] ${JSON.stringify({
      ...
      checked: (heldRows || []).length, released, refused, held_still: heldStill, skipped, audit_write_failed: auditWriteFailed,
    })}`);
    return { exitCode: EXIT_OK, summary: { checked: (heldRows || []).length, released, refused, heldStill, skipped, auditWriteFailed, outcomes } };
  } catch (err) {
