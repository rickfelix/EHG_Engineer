...
import { discoverAllProcesses, findOrphanedActiveCrons } from '../lib/periodic-liveness/enumerate-processes.mjs';
...
async function main() {
  ...
  const { data: rows, error } = await supabase
    ...
    .select('process_key, owner, process_type, currently_expected_active');
  ...
  const orphans = findOrphanedActiveCrons(discovered, rows);
  const orphanKeys = new Set(orphans.map((r) => r.process_key));
  const extras = (rows || []).filter((r) => !discoveredKeys.has(r.process_key) && !orphanKeys.has(r.process_key));
  if (extras.length > 0) {
  ...
  // QF-20260911-080: the reverse direction — an ACTIVE standalone_cron registry row (the
  // registry itself says it should be running) with NO discovered invoker anywhere (STANDARD_LOOPS,
  // a GHA cron, or scripts/cron/*). standard_loop:drain-inventory sat here for months.
  if (orphans.length > 0) {
    console.log(`\nORPHANED (registry says active, no invoker found) — ${orphans.length} process(es):`);
    for (const o of orphans) console.log(`  - ${o.process_key}`);
    console.log('\nFix: add a STANDARD_LOOPS entry (scripts/coordinator-startup-check.mjs) or a .github/workflows/*-cron.yml for each.');
