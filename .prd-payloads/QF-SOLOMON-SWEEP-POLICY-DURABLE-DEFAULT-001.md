...
**ADDENDUM (Solomon verification c513ac81, Fable-code-verified — STRENGTHENS + corrects):** `solomonSweepMode()` derives mode from the CONFIGURED pin (`getClaudeModel('solomon')` → MODEL_DEFAULTS/CLAUDE_MODEL_SOLOMON = claude-opus-4-8), NOT the live serving model — a `/model` switch never reaches the resolver, so the mode returns CONSULT even while the session is genuinely serving Fable-5. Correction to the original advisory: the override does NOT auto-revert when a Fable pin returns via /model; only a config/env pin change would. Implication: the chairman's env override is load-bearing in ALL states, and pin-derivation-vs-serving-model divergence is a second structural argument for encoding the policy in the design-of-record (this QF) rather than deriving mode from a pin string. (scripts/solomon-startup-check.mjs L41-44 + lib/config/model-config.js.)
