...
**CHAIRMAN ADDITION 2026-07-17 (same session):** *"Occasionally I might send you a text message and ask you for a project status update as it relates to the road map."* → NEW FR-0: CHAIRMAN INBOUND FREE-TEXT ROUTES TO ADAM. In the drain path (`drainSmsRelayStaging`/`handleInboundSmsReply`), when an inbound resolves `no_match` AND `from_phone` equals the chairman's configured number, do NOT drop it — insert a directed `session_coordination` row to the active Adam session (`payload.kind='chairman_sms_inbound'`, body = the text, from, received_at) so Adam's inbox surfaces it as a chairman request; Adam answers via his own send (roadmap-grounded status per the plan-first lens: wave positions + Slipped/Committing/Done, SMS-sized). **TONE (chairman-directed 2026-07-17): all chairman-facing SMS bodies use PROFESSIONAL-CASUAL plain English — complete sentences, no protocol shorthand/abbreviations/telegraphic labels ("DONE:", "N=80"), reads like a sharp chief-of-staff texting; concise but human.** Unknown numbers keep the current silent-drop (no enumeration oracle). **LATENCY LESSON (live witness 2026-07-17 20:39-20:47Z):** the chairman's first real status-request sat UNDRAINED 8+ min because the GHA */5 drain cron lags (GitHub scheduler looseness — known behavior). Therefore FR-0 must route chairman inbound at a LOW-LATENCY point: either (a) hook at RELAY/staging-write time (preferred — a DB trigger or the staging RPC notifies), or (b) supplement the GHA cron with a server-side 60s interval running scripts/sms-relay-drain.cjs on the EHG_Engineer server. Do NOT rely on GHA cron punctuality for chairman-facing latency. Adam-side interim until this ships: Adam's quiet-tick watches `sms_relay_staging` DIRECTLY (undrained rows from the chairman's number — pre-drain, so cron lag can't hide them) and triggers the drain via workflow_dispatch when rows are found.
