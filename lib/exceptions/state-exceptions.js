/**
 * State & Agent Exception Classes
 * Centralized state machine and agent exceptions for LEO Protocol
 *
 * Extracted from:
 * - lib/agents/venture-ceo-runtime.js
 * - lib/agents/base-sub-agent.js
 */

/**
 * VentureRequiredException - Thrown when agent is instantiated without ventureId
 * SD-HARDENING-V2-004: Legacy Mode Elimination
 * THE LAW: Every sub-agent MUST belong to a venture. NO EXCEPTIONS.
 */
export class VentureRequiredException extends Error {
  constructor(agentName) {
    super(`Venture governance required: Agent "${agentName}" cannot be instantiated without ventureId. Legacy mode has been eliminated.`);
    this.name = 'VentureRequiredException';
    this.isRetryable = false;
    this.agentName = agentName;
  }
}

/**
 * CircuitBreakerException - Thrown when iteration/loop limit exceeded
 * Industrial Hardening v3.0: Prevents runaway token consumption
 */
export class CircuitBreakerException extends Error {
  constructor(agentId, reason, iterationCount) {
    super(`Circuit breaker triggered for agent ${agentId}. Reason: ${reason}. Iterations: ${iterationCount}`);
    this.name = 'CircuitBreakerException';
    this.isRetryable = false;
    this.agentId = agentId;
    this.reason = reason;
    this.iterationCount = iterationCount;
  }
}

/**
 * UnauthorizedCapabilityError - Thrown when agent lacks required capability
 */
export class UnauthorizedCapabilityError extends Error {
  constructor(capability, agentId) {
    super(`Agent ${agentId} is not authorized to use capability: ${capability}`);
    this.name = 'UnauthorizedCapabilityError';
    this.capability = capability;
    this.agentId = agentId;
    this.isRetryable = false;
  }
}

/**
 * StateStalenessError - Thrown when state is too old
 * Prevents operating on stale state that may have been modified
 */
export class StateStalenessError extends Error {
  constructor(ventureId, lastKnownVersion, currentVersion) {
    super(`State staleness detected for venture ${ventureId}. Local: ${lastKnownVersion}, Remote: ${currentVersion}`);
    this.name = 'StateStalenessError';
    this.ventureId = ventureId;
    this.lastKnownVersion = lastKnownVersion;
    this.currentVersion = currentVersion;
    this.isRetryable = true;
  }
}

/**
 * GoldenNuggetValidationError - Validation failure for stage transitions
 * THE LAW: No stage transition without passing Golden Nugget validation
 */
export class GoldenNuggetValidationError extends Error {
  constructor(stageId, failedGates) {
    super(`Golden Nugget validation failed for stage ${stageId}: ${failedGates.join(', ')}`);
    this.name = 'GoldenNuggetValidationError';
    this.stageId = stageId;
    this.failedGates = failedGates;
    this.isRetryable = false;
  }
}

/**
 * StageGateValidationError - Thrown when stage gate criteria not met
 */
export class StageGateValidationError extends Error {
  constructor(stageId, gateId, reason) {
    super(`Stage gate validation failed for stage ${stageId}, gate ${gateId}: ${reason}`);
    this.name = 'StageGateValidationError';
    this.stageId = stageId;
    this.gateId = gateId;
    this.reason = reason;
    this.isRetryable = false;
  }
}

/**
 * BusinessHypothesisValidationError - Thrown when business hypothesis fails validation
 */
export class BusinessHypothesisValidationError extends Error {
  constructor(ventureId, hypothesis, reason) {
    super(`Business hypothesis validation failed for venture ${ventureId}: ${reason}`);
    this.name = 'BusinessHypothesisValidationError';
    this.ventureId = ventureId;
    this.hypothesis = hypothesis;
    this.reason = reason;
    this.isRetryable = false;
  }
}
