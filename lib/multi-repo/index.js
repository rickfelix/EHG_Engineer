...
// =============================================================================
// CONFIGURATION (exported for use by other modules)
// =============================================================================

// Base directory containing all EHG repos (parent of EHG_Engineer).
// FR-2 (SD-LEO-INFRA-SHIP-PREFLIGHT-REPORTS-001): a raw `resolve(__dirname,
// '../../..')` resolves wrong when this module loads from inside a
// `.worktrees/<sd>` checkout (which is where SD work actually runs) — __dirname
// is then under .worktrees, so the 3-levels-up guess lands on .worktrees itself.
// getRepoRoot() already corrects for this (stripWorktreeSuffix), and returns the
// EHG_Engineer root itself, not its parent — hence the extra resolve(..., '..') hop.
export const EHG_BASE_DIR = resolve(getRepoRoot(), '..');
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (stat.isDirectory() && existsSync(gitPath)) {
          // FR-3 (SD-LEO-INFRA-SHIP-PREFLIGHT-REPORTS-001): a `.git` FILE means
          // this is a linked worktree (its `.git` is a `gitdir:` pointer into
          // the main repo's `.git/worktrees/<name>`), not an independent repo
          // -- exclude it. Without this check, every sibling worktree under
          // EHG_BASE_DIR was counted as its own repo (measured: two consecutive
          // scans of the same commit produced different repo counts/verdicts).
...
/**
 * Get all active repositories from the registry.
 * Returns all repos (platform + ventures), not just ehg + EHG_Engineer.
 * SD-LEO-INFRA-VENTURE-DEVWORKFLOW-AWARENESS-001-G: Expanded from 2-repo to N-repo.
 * @returns {Object} Map of all active repos
 */
export function getPrimaryRepos() {
  // Return all discovered repos — ventures are first-class citizens
...
export function getAllReposStatus(primaryOnly = true) {
  ...
  for (const [_name, info] of Object.entries(repos)) {
    results.push({
      ...info,
...
export function findSDBranches(sdId) {
  ...
  // Branch prefixes for reference (matching done via SD ID in branch name)
  const _branchPrefixes = ['feat/', 'fix/', 'docs/', 'test/', 'chore/', 'refactor/'];
