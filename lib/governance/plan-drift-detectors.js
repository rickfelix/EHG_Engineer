...
export async function findOpenFinding(supabase, gaugeId) {
  const { data, error } = await supabase
    .from('feedback')
    // first_seen: QF-20260911-515 -- a re-emission is now recorded as a NEW row (feedback is
    // append-only), which needs the ORIGINAL first_seen carried forward rather than reset.
