...
// SD-LEO-ORCH-CAPA-RECORD-TRUTH-002-D (FR-6): the predicate TYPE vocabulary is owned by the
// evaluator, so isStructuredPredicate below validates against it rather than re-listing the types
// here — a second copy of the vocabulary would drift the moment a type is added.
import { PREDICATE_TYPE } from './release-condition-predicate.js';
 ...
 */
/**
 * SD-LEO-ORCH-CAPA-RECORD-TRUTH-002-D (FR-6) — is this a STRUCTURED, machine-evaluable predicate?
 *
 * PROSE IS REJECTED, DELIBERATELY. Release conditions in the wild embed structured data inside free
 * text (a live example carries `review_at=` as prose), so a parser that tried to read them would
 * appear to work while silently mis-reading anything phrased differently — manufacturing a new lying
 * instrument inside the workstream whose whole purpose is removing them. A condition is either an
 * explicit predicate object this function accepts, or it stays prose and is treated as judgment-only.
 * Fail-closed, matching evaluate()'s own contract: anything unrecognized is NOT a predicate.
 * @param {unknown} p
 * @returns {boolean}
 */
export function isStructuredPredicate(p) {
  // SEC-D-3: wrapped because a THROWING GETTER on `type` made this throw rather than return false,
  // and logHoldStateViolation's outer try then swallowed it — silently dropping the ENTIRE violation
  // record (reason, owner, review_at, prose release_condition), letting a caller suppress its own
  // contract-violation log. A shape guard must answer false, never propagate.
  try {
    if (!p || typeof p !== 'object' || Array.isArray(p)) return false;
    if (!Object.values(PREDICATE_TYPE).includes(p.type)) return false;
    // SEC-D-1: `params: null` was ADMITTED here and then crashed the evaluator. `const { params = {} }`
    // applies the ES default only for undefined, NEVER for null, and JSONB round-trips null faithfully
    // — so a single malformed stored row could throw inside a consumer. Absent params is fine
    // (evaluate treats it as empty); null or a non-object is not.
    if (p.params === undefined) return true;
    return !!p.params && typeof p.params === 'object' && !Array.isArray(p.params);
  } catch {
    return false; // an object that throws on inspection is, by definition, not a valid predicate
  }
}
...
export async function logHoldStateViolation(supabaseClient, { surface, stamp, errors }) {
  ...
  try {
    // FR-6: persist the machine-evaluable predicate ALONGSIDE the prose, not instead of it. Before
    // this, hold_state_contract_violations.release_condition_predicate
    // (20260718_release_condition_predicate.sql:18) had no writer and no reader anywhere in the tree
    // — a column and a fail-closed evaluator, both shipped and both dead. This is the producer half;
    // the consumer half lands in the same commit, because a producer without a consumer is precisely
    // the failure this module already committed once.
    const predicate = stamp && stamp.release_condition_predicate;
    await supabaseClient.from('hold_state_contract_violations').insert({
      ...
      release_condition_predicate: isStructuredPredicate(predicate) ? predicate : null,
      errors,
    });
  } catch (_e) {
