 ...
 *
 * AEGIS Integration (Phase 3):
 * This module now supports routing through the AEGIS unified governance system.
...
async function getAegisAdapter(supabase) {
  ...
  try {
    const module = await import('./aegis/adapters/FourOathsAdapter.js');
    FourOathsAdapter = module.FourOathsAdapter;
    aegisAdapter = new FourOathsAdapter({ supabase });
    return aegisAdapter;
  } catch (err) {
    console.warn('[FourOathsEnforcement] AEGIS adapter not available:', err.message);
    return null;
  }
}
...
export class FourOathsEnforcement {
   ...
   * @returns {Object|Promise<Object>} Combined validation result
   */
  validateAllOaths(agentAction) {
    // AEGIS Integration: Route through unified governance system if enabled
    if (this.useAegis && !agentAction._skipAegis) {
      return this._validateAllOathsViaAegis(agentAction);
    }
  ...
  /**
   * Validate all oaths via AEGIS adapter
   * @private
   */
  async _validateAllOathsViaAegis(agentAction) {
    try {
      const adapter = await getAegisAdapter(this.supabase);
      if (adapter) {
        console.log('[FourOathsEnforcement] Using AEGIS for validation');
        return await adapter.validateAllOaths(agentAction);
      }
      console.warn('[FourOathsEnforcement] AEGIS adapter unavailable, falling back to legacy');
    } catch (err) {
      console.warn('[FourOathsEnforcement] AEGIS validation failed, falling back:', err.message);
    }

    // Fallback to legacy - mark to skip AEGIS to prevent recursion
    return this.validateAllOaths({ ...agentAction, _skipAegis: true });
  }
 ...
 * @returns {boolean|Promise<boolean>} True if valid, false if any violation
 */
export async function isOathCompliant(agentAction) {
  const enforcement = getFourOathsEnforcement();
  const result = await enforcement.validateAllOaths(agentAction);
  return result.valid;
}
