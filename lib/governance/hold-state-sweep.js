...
const SURFACES = [
  ...
  // SD-LEO-INFRA-TIERED-SOURCING-CLAIM-001 (FR-7): without this 4th surface, the oracle_read_pending
  // hold's review_at stamp (lib/fleet/hold-writer.js writeSdOracleHold) has NO CONSUMER — a held
  // item whose consult never resolves and whose release cron is disabled would be invisible by
  // construction, since only the 3 surfaces above were read by this sweep (TESTING finding T-1).
  {
    surface: 'oracle_read_pending',
    reviewAtKey: 'oracle_read_pending_review_at',
    isActive: (row) => (row.metadata || {}).requires_human_action_reason === 'oracle_read_pending',
  },
];
