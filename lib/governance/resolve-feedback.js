...
import { fetchLatestFeedback, buildFeedbackCorrection } from './feedback-correction.js';
 ...
 * @param {string} [args.resolutionType] - free-text (feedback.resolution_type has no CHECK enum)
 * @returns {Promise<{ updated: boolean, id?: string, reason?: string, error?: string }>}
 */
export async function resolveFeedback({ supabase, feedbackId, quickFixId, sdId, notes, resolutionType } = {}) {
  ...
  if (quickFixId) changes.quick_fix_id = quickFixId;
  if (sdId) changes.resolution_sd_id = sdId;
  if (notes) changes.resolution_notes = notes;
  if (resolutionType) changes.resolution_type = resolutionType;

  try {
    // SD-LEO-INFRA-AUDIT-FIX-FEEDBACK-001-A: public.feedback is append-only -- the UPDATE
    // above is rejected unconditionally. Resolve to the LATEST row in the correction chain
    // (never trust the raw feedbackId row, which may already be stale) before deciding
    // whether this is a no-op; a fetchLatestFeedback error is fail-closed, not silently
    // treated as "not yet resolved".
    const latestResult = await fetchLatestFeedback(supabase, feedbackId);
    if (latestResult.error) {
      return { updated: false, error: latestResult.error };
    }
    if (latestResult.row.status === 'resolved') {
      return { updated: false, id: feedbackId, reason: 'no_row_or_already_resolved' };
    }

    const payload = buildFeedbackCorrection(latestResult.row, changes);
