...
export function decideDemand(reading, floor, meta = {}) {
  ...
  return {
    ...
    // QF-20260908-019: this line hardcoded the word "dispatchable" for every caller, but the
    // number is only ever "dispatchable" for the DEFAULT gauge (countDispatchableBacklog). A
    // gauge-injecting caller like qf-mint-gate.js (countClaimableQuickFixes) still rendered
    // "dispatchable N" -- self-describing and wrong about the population it counted. Naming
    // `engine` instead (already required, already unique per caller) fixes both the printed
    // verdict line (formatDemandDecision embeds this string) and the audit_log row
    // (recordDemandDecision persists the whole decision, reason included) with one change,
    // with no change to the object's shape or any field name.
    reason: sourced
      ? `${engine} ${reading.value} <= floor ${floor} — real demand, producing`
      : `${engine} ${reading.value} > floor ${floor} — belt already has work, staying quiet (this is healthy, not a fault)`,
    measured_at,
  };
}
