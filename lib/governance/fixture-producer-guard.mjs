 ...
 */
export const SANCTIONED_PERMANENT_NAMES = Object.freeze(new Set([CANARY_NAME]));
...
export function evaluateDeclaration(row, classification) {
  ...
  const named = typeof row?.name === 'string' && SANCTIONED_PERMANENT_NAMES.has(row.name);
  if (!named) {
    return {
      ok: false,
      tripsCanonical,
 ...
 * @returns {Promise<any>} the supabase insert result, unchanged
 */
export async function insertGuarded(supabase, table, row, decl) {
