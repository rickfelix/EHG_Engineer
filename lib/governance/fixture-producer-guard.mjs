...
/**
 * QF-20260830-177: a REAL-DB integration test whose process crashes before `afterAll` runs
 * leaves its DELIBERATELY_REAL ventures (and their chairman_decisions/venture_artifacts) as
 * permanent residue — 38 such rows accumulated across a month of interrupted CI runs of the
 * three tests/integration/eva/*-realdb.test.js suites, inflating the chairman's pending
 * sitting-depth gauge (P3) 20x. This does not (and cannot) prevent a crash. Called from each
 * suite's `beforeAll` before it creates new ventures, it bounds the class: any PRIOR run's
 * leftovers matching THIS suite's own name prefix are swept first, so at most one crashed
 * run's worth of residue can ever exist at a time instead of accumulating unboundedly.
 *
 * @param {object} supabase
 * @param {{namePrefix: string, olderThanMs?: number, now?: number, logger?: {log?:Function, warn?:Function}}} opts
 * @returns {Promise<{purged: number}>}
 */
export async function purgeStaleRealDbResidue(supabase, { namePrefix, olderThanMs = 60 * 60 * 1000, now = Date.now(), logger = console } = {}) {
  if (!namePrefix) throw new Error('[fixture-producer-guard] purgeStaleRealDbResidue requires namePrefix');
  ...
  const { data: stale, error } = await supabase
    ...
    .limit(500);
  if (error) {
    logger.warn?.(`[fixture-producer-guard] purgeStaleRealDbResidue: lookup failed (non-fatal): ${error.message}`);
    return { purged: 0 };
  }
  const ids = (stale || []).map((v) => v.id);
  if (!ids.length) return { purged: 0 };
  for (const table of ['chairman_decisions', 'venture_artifacts', 'venture_stage_work', 'venture_stage_transitions']) {
    await supabase.from(table).delete().in('venture_id', ids);
  }
  await supabase.from('ventures').delete().in('id', ids);
  logger.log?.(`[fixture-producer-guard] purgeStaleRealDbResidue: purged ${ids.length} stale ${namePrefix}* venture(s)`);
  return { purged: ids.length };
}

/**
 * SD-LEO-INFRA-ORG-TEST-TEARDOWN-001 FR-3: the sibling problem one level down from
 * purgeStaleRealDbResidue() above. org_agent_identities has no FK to ventures (confirmed live
 * via pg_constraint) and no created_at-based staleness column that matters here -- the failure
 * mode isn't "old", it's "orphaned": a crashed test process leaves an org_agent_identities row
 * whose venture_id points at a ventures row that teardownRun()'s own venture delete already
 * removed (or would have, had the crash not pre-empted it).
 *
 * DELIBERATELY A SEPARATE FUNCTION, not an extension of purgeStaleRealDbResidue() above: that
 * function's query shape (ventures.name ILIKE + created_at cutoff) does not fit
 * org_agent_identities (keyed by display_name, no direct staleness cutoff makes sense for a row
 * whose parent is already gone) -- extending it would either bend its contract for one caller or
 * risk the shared ventures-sweep path the 3 existing real-DB suites already depend on. A second,
 * narrowly-scoped function keeps that path byte-identical.
 *
 * NEVER touches a venture_id IS NULL row (the 5 real holdco operators) -- orphan detection only
 * fires for a NON-NULL venture_id that fails to resolve; a null venture_id can never be
 * "orphaned" by this predicate.
 *
 * @param {object} supabase
 * @param {{namePrefix: string, logger?: {log?:Function, warn?:Function}}} opts
 * @returns {Promise<{purged: number}>}
 */
export async function purgeOrphanedOrgAgentIdentities(supabase, { namePrefix, logger = console } = {}) {
  if (!namePrefix) throw new Error('[fixture-producer-guard] purgeOrphanedOrgAgentIdentities requires namePrefix');
  const { data: candidates, error } = await supabase
    .from('org_agent_identities')
    .select('id, venture_id, display_name')
    .ilike('display_name', `${namePrefix}%`)
    .not('venture_id', 'is', null)
    .limit(500);
  if (error) {
    logger.warn?.(`[fixture-producer-guard] purgeOrphanedOrgAgentIdentities: lookup failed (non-fatal): ${error.message}`);
    return { purged: 0 };
  }
  if (!candidates || candidates.length === 0) return { purged: 0 };

  const ventureIds = [...new Set(candidates.map((c) => c.venture_id))];
  // count-truncation-diff-lint: explicitly bounded by construction, not just in practice --
  // ventureIds is a deduplicated subset of `candidates`, itself capped at 500 by the .limit(500)
  // above, so ventureIds.length can never exceed 500 either; the explicit .limit(500) here makes
  // that bound visible to the query itself rather than relying on the caller's invariant.
  const { data: liveVentures, error: ventureErr } = await supabase
    .from('ventures')
    .select('id')
    .in('id', ventureIds)
    .limit(500);
  if (ventureErr) {
    logger.warn?.(`[fixture-producer-guard] purgeOrphanedOrgAgentIdentities: venture lookup failed (non-fatal): ${ventureErr.message}`);
    return { purged: 0 };
  }
  const liveVentureIds = new Set((liveVentures || []).map((v) => v.id));
  const orphanIds = candidates.filter((c) => !liveVentureIds.has(c.venture_id)).map((c) => c.id);
  if (!orphanIds.length) return { purged: 0 };

  const { error: deleteErr } = await supabase.from('org_agent_identities').delete().in('id', orphanIds);
  if (deleteErr) {
    logger.warn?.(`[fixture-producer-guard] purgeOrphanedOrgAgentIdentities: delete failed (non-fatal): ${deleteErr.message}`);
    return { purged: 0 };
  }
  logger.log?.(`[fixture-producer-guard] purgeOrphanedOrgAgentIdentities: purged ${orphanIds.length} orphaned ${namePrefix}* org_agent_identities row(s)`);
  return { purged: orphanIds.length };
}
...
export function insertGuarded(supabase, table, row, decl) {
  ...
  // SD-LEO-INFRA-FIXTURE-VENTURES-IDENTIFIED-001 (FR-2): stamp is_demo:true on every FIXTURE
  // insert, IN PLACE (never a spread copy -- a copy breaks the reference-identity guarantee this
  // guard exists to provide, see the header). This MUST happen strictly AFTER evaluateDeclaration
  // has already run and returned ok:true -- isFixtureVenture is flag-first, so stamping any
  // earlier would make the assert above vacuously true for every row and silently void it.
  if (classification === CLASSIFICATION.FIXTURE) {
    row.is_demo = true;
  }
