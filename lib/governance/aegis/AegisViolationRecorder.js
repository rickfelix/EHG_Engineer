...
export class AegisViolationRecorder {
  ...
  /**
   * Record a gate bypass event with complete audit context (GOV-008)
   * @param {Object} bypassEvent - Gate bypass details
   * @param {string} bypassEvent.gate_name - Name of the gate that was bypassed
   * @param {string} bypassEvent.sd_key - SD key associated with the bypass
   * @param {string} bypassEvent.severity - Severity level (critical, high, medium, low)
   * @param {string} bypassEvent.reason - Why the gate was bypassed
   * @param {string} [bypassEvent.session_id] - Session that triggered the bypass
   * @param {Object} [bypassEvent.context] - Additional context
   * @returns {Promise<Object>} Recorded audit entry
   */
  async recordGateBypass(bypassEvent) {
    if (!this.supabase) {
      console.warn('[AegisViolationRecorder] No Supabase client, gate bypass not recorded');
      return { recorded: false, error: 'no_supabase_client' };
    }

    const auditEntry = {
      event_type: 'gate_bypass',
      gate_name: bypassEvent.gate_name,
      sd_key: bypassEvent.sd_key,
      severity: bypassEvent.severity || 'medium',
      reason: bypassEvent.reason || 'unspecified',
      session_id: bypassEvent.session_id || null,
      context: bypassEvent.context || {},
    ...
    try {
      ...
      if (error) {
        console.error(`[AegisViolationRecorder] Gate bypass audit failed: ${error.message}`);
        return { recorded: false, error: error.message };
      }

      console.log(`[AegisViolationRecorder] Gate bypass recorded: ${data.id} (gate=${bypassEvent.gate_name}, sd=${bypassEvent.sd_key})`);
      return { recorded: true, id: data.id };
    } catch (err) {
      console.error(`[AegisViolationRecorder] Gate bypass audit exception: ${err.message}`);
      return { recorded: false, error: err.message };
    }
  }

  /**
   * Get bypass audit completeness percentage
   * @param {string} sdKey - SD key to check
   * @returns {Promise<Object>} Completeness report
   */
  async getBypassCompleteness(sdKey) {
    if (!this.supabase) {
      return { completeness: 0, total: 0, logged: 0 };
    }
    ...
    try {
      ...
      if (error) {
        return { completeness: 0, total: 0, logged: 0, error: error.message };
      }

      const entries = data || [];
      const complete = entries.filter(e => e.gate_name && e.severity && e.reason);

      return {
        completeness: entries.length > 0 ? Math.round((complete.length / entries.length) * 100) : 100,
        total: entries.length,
        logged: complete.length,
        incomplete: entries.filter(e => !e.gate_name || !e.severity || !e.reason)
      };
    } catch (err) {
      return { completeness: 0, total: 0, logged: 0, error: err.message };
    }
  }
