...
// CLI: manual run (npm run gov:run-probes). Reports degraded until the chairman
// applies the STAGED registry; at apply the armed cadence calls runProbeRegistry
// with a real fact resolver (operator_contract_waiver documents that deferral).
if (process.argv[1]?.replace(/\\/g, '/').endsWith('probe-runner.mjs')) {
  const { createClient } = await import('@supabase/supabase-js');
  await import('dotenv/config');
  const supabase = createClient(process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
  const run = await runProbeRegistry(supabase);
  if (run.degraded) {
    console.log(`Probe registry DEGRADED: ${run.reason}`);
  } else {
    console.log(`Probe registry: ${run.results.length} probe(s) evaluated`);
    for (const r of run.results) console.log(`  ${r.probe_key.padEnd(40)} ${r.verdict.padEnd(8)} ${r.detail}`);
  }
}
