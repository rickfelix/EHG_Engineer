...
/**
 * QF-20260904-983: the single source of truth for "does this telemetry row carry a real
 * witness verdict" (as opposed to merely existing). A reconcile-sweep row exists with
 * rungs=[] to record that the sweep looked, not that anything was verified -- treating row
 * existence alone as witnessed (as lib/ship/witness-adoption.mjs's classifyMerges previously
 * did) let that placeholder silence the unwitnessed-merge gauge. Exported so every reader of
 * merge_witness_telemetry uses the same predicate rather than each inventing its own.
 * @param {{rungs?: Array}} telemetryRow
 * @returns {boolean}
 */
export function hasRealVerdicts(telemetryRow) {
  return Array.isArray(telemetryRow?.rungs) && telemetryRow.rungs.length > 0;
}
...
export async function computeL1Outcome(supabase, workKey) {
  ...
  } else if (!hasRealVerdicts(telemetryRow)) {
