...
async function getAegisAdapter() {
  ...
  try {
    const module = await import('./aegis/adapters/HardHaltAdapter.js');
    HardHaltAdapter = module.HardHaltAdapter;
    aegisAdapter = new HardHaltAdapter();
    return aegisAdapter;
  } catch (err) {
    console.warn('[HardHaltProtocol] AEGIS adapter not available:', err.message);
    return null;
  }
}
...
export class HardHaltProtocol {
  ...
  async getHaltStatus() {
    ...
    try {
      ...
      // Map value_json to expected format
      return {
        isHalted: data.value_json?.enabled === true,
        haltReason: data.value_json?.reason,
        triggeredAt: data.value_json?.since,
        ...data.value_json
      };
    } catch (err) {
  ...
  async checkDeadManSwitch() {
    ...
    // AEGIS Integration: Route validation through AEGIS if enabled
    if (this.useAegis) {
      try {
        const adapter = await getAegisAdapter();
        if (adapter) {
          console.log('[HardHaltProtocol] Using AEGIS for dead-man switch validation');
          const result = await adapter.validateDeadManSwitch({
            hoursSinceActivity,
            warningThreshold,
            haltThreshold
          });
        ...
        console.warn('[HardHaltProtocol] AEGIS validation failed, using legacy:', err.message);
      }
    }
    ...
    if (hoursSinceActivity >= haltThreshold) {
      ...
      return { shouldHalt: true, hoursSinceActivity, aegis_enabled: false };
    }

    return { shouldHalt: false, hoursSinceActivity, aegis_enabled: false };
  }
  ...
  async _storeHaltStatus(haltData) {
    ...
    try {
      // Map haltData to value_json format
      const valueJson = {
        enabled: haltData.isHalted,
        reason: haltData.haltReason || haltData.reason || null,
        since: haltData.triggeredAt || (haltData.isHalted ? new Date().toISOString() : null)
      };
      ...
      const { error } = await this.supabase
        ...
        .update({
          ...
          updated_by: haltData.triggeredBy || haltData.restoredBy || 'system'
        })
        .eq('key', this.config.haltStatusKey);
 ...
 */
export async function ensureOperationAllowed(agentLevel, operationType = 'unknown') {
  ...
  // AEGIS Integration: Route through adapter if enabled
  if (protocol.useAegis) {
    try {
      const adapter = await getAegisAdapter();
      if (adapter) {
        const result = await adapter.validateOperationAllowed({
          agentLevel,
          isHalted: halted,
          operationType
        });
        ...
        if (!result.allowed) {
          throw new HardHaltError(
            'OPERATION_BLOCKED',
            result.issues[0] || `System is in Hard Halt state. ${agentLevel} operations are suspended.`
          );
        }
        return;
      }
    } catch (err) {
      if (err instanceof HardHaltError) throw err;
      console.warn('[HardHaltProtocol] AEGIS check failed, using legacy:', err.message);
    }
  }
