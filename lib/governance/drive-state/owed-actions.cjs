 ...
 * @returns {ReadonlyArray<{axis, act, since, runs, truncated, run_id, citation, keystroke_packets}>}
 */
function deriveOwedActions({ verdict, spans = null } = {}) {
  ...
  for (const e of verdict.axes) {
    ...
    out.push(Object.freeze({
      ...
      citation: e.citation,
      // SD-LEO-INFRA-PERMISSION-FREEZE-STUCK-001 FR-4/D2: an OPTIONAL passthrough, not a new
      // per-axis field every axis must populate (5 of 6 axes never set it, so this is `null`
      // there) — fleet_health is the only axis that ships one today. This is display data only,
      // never read by deriveOwedActions itself, so it cannot affect the closed act-set contract.
      keystroke_packets: Array.isArray(e.keystroke_packets) ? e.keystroke_packets : null,
    }));
  }
