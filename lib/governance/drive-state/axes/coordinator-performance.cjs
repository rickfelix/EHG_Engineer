 ...
 *
 * IT STILL DOES NOT RESCUE action_taken, FOR A REASON THE SCHEMA MAKES STRUCTURAL. What survives is
 * the fact that a breach was raised and that something stamped it retired. session_coordination has
 * FIFTEEN columns and none of them is actioned_at, actioned_by, or acknowledged_by (verified live).
 * Retirement is a key written into the payload JSONB — no constraint, no type check, no index — and
 * the retirement predicate at adam-advisory-store.cjs:49 is the entire mechanism:
 * `.is('payload->>actioned_at', null)`. So a durable row plus an unconstrained self-written key
 * still yields no answer to "who acted, and what did they do".
 *
