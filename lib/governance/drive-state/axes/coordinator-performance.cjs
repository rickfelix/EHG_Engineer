 ...
 *   All 21 are retired — actioned_at on 21 of 21, exactly as on the 61 delivered ones.
 *   delivered_at is NULL on all 21, while read_at is SET on all 21.
 *   They were never re-routed: target_session still equals original_target, a dead session.
 *   Dead-lettered rows retire FASTER than delivered ones — median 3.1m against 8.8m.
 *   actioned_by appears on 0 of 21 dead-lettered, against 4 of 61 delivered.
 *
 * So the retirement predicate cannot distinguish "a human-equivalent read this and acted" from "this
 * was addressed to a corpse and cleared 3 minutes later", and the undeliverable quarter is cleared
 * SOONEST. Worse, read_at is stamped on 21 rows that delivered_at says never arrived, so the one
 * column a reader would reach for to check receipt affirms receipt that did not happen. This is the
 * SD's whole thesis in its purest available form — a control emitting a success signal for work that
 * could not have occurred — and it is why action_taken is UNVERIFIABLE rather than merely unattributed.
 *
