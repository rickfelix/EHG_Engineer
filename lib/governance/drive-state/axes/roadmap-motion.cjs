 ...
 *
 * NOTE (SD-LEO-INFRA-SOURCING-ENGINE-CONSUMPTION-001, 2026-08-24): this comment previously named
 * a specific "unblocking child" SD that was never actually created — confirmed by exact-key and
 * ILIKE search against strategic_directives_v2, both zero rows (do not re-add a specific SD-key
 * citation here without first confirming the row exists). No replacement tracking SD has been
 * filed as of this fix; the per-item commitment-clock work described above remains real and
 * unclaimed. classifyMotion() below is left unwired (classify() still unconditionally returns
 * UNMEASURABLE) pending that future work.
 */
...
const BLOCKED_CITATION =
  ...
  'cadence are required to difference two observations. No tracking SD/QF filed as of 2026-08-24 ' +
  '(SD-LEO-INFRA-SOURCING-ENGINE-CONSUMPTION-001 found and removed a dangling citation to a child ' +
  'SD that was never actually created).';
