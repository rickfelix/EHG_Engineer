...
/**
 * SD-LEO-INFRA-PERMISSION-PROMPT-BLOCKED-001 (VALIDATION finding F1, evidence 9fb7b181): the
 * awaiting-approval and loop-dead axes previously had NO automatic reader — only the hand-run
 * scripts/one-off/_coord-silent-holder-audit.cjs surfaced them, so a stuck seat was only found
 * once a human already suspected a problem. Folding both predicates into this axis's `stuck[]`
 * makes them reach the SAME coordinator-visible surface (fleet_health owed-action) as the
 * pre-existing tool-silence signal, with no new axis/owed-action type registered.
 *
 * Dedup by session_id, tool-silence first: it is the most-measured signal (TOOL_SILENCE_CUT_MINUTES
 * calibration above) and a seat already flagged by it does not need a second, less-measured reason.
 */
function extraStuckSeats(seats, nowMs, predicate, minutesOf, reason) {
  return seats.filter((row) => predicate(row, nowMs)).map((row) => ({
    session_id: row.session_id,
    silent: Math.max(0, Math.round((nowMs - minutesOf(row)) / 60000)),
    window_handle: (row.metadata || {}).window_handle ?? null,
    fleet_identity: (row.metadata || {}).fleet_identity ?? null,
    reason
  }));
}

/** Independent rollback switch for FR-3 (SD-LEO-INFRA-PERMISSION-PROMPT-BLOCKED-001): the hook-side
 *  LEO_AWAITING_APPROVAL_STAMP=off only stops NEW stamps from being written and does nothing for
 *  isLoopDead (which never depends on the stamp). Setting LEO_FLEET_HEALTH_NEW_AXES=off removes
 *  BOTH new predicates from fetch()'s stuck[] merge in one step, with no code redeploy, reverting
 *  this axis to its pre-SD behavior (tool_silence only) if either proves noisy in production. */
function newAxesDisabled() {
  const v = String(process.env.LEO_FLEET_HEALTH_NEW_AXES || 'on').toLowerCase();
...
async function fetch(supabase, { now } = {}) {
  ...
  const toolSilent = results.filter((r) => r.verdict === VERDICT.STUCK).map((r) => ({
  ...
  const awaitingApproval = newAxesDisabled() ? [] : extraStuckSeats(seats, nowMs, isAwaitingApprovalStale,
    (row) => Date.parse(row.metadata.awaiting_approval_since), 'awaiting_approval');
  const loopDead = newAxesDisabled() ? [] : extraStuckSeats(seats, nowMs, isLoopDead,
    (row) => Date.parse(row.last_tool_at), 'loop_dead');

  const bySessionId = new Map();
  for (const s of [...toolSilent, ...awaitingApproval, ...loopDead]) {
    if (!bySessionId.has(s.session_id)) bySessionId.set(s.session_id, s);
  }
...
function renderStuckSeat(s) {
  ...
  // reason defaults to tool_silence for pre-existing callers that never set it (e.g. tests
  // asserting the pre-SD-LEO-INFRA-PERMISSION-PROMPT-BLOCKED-001 stuck-entry shape).
  if (s.reason && s.reason !== 'tool_silence') identity.push(`reason=${s.reason}`);
