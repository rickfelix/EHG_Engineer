...
function classify(state, clock) {
  ...
  if (stalled.length > 0) {
    return {
      ...
      // ventures.name is DB free text and this citation reaches the --json consumer path, where
      // render-time safeCitation does not apply. Wrapped at the AXIS level for parity with
      // chairman-decisions. ventures is not anon-writable (probed), so this is defence in depth.
      citation: safeCitation(`venture-stage-motion: ${stalled.length} of ${state.evaluated.length} active ventures divergent-and-stalled past their tier clock — ` +
        stalled.slice(0, 3).map((v) => `${v.name || v.id}@${v.elapsed_days}d/${v.clock_days}d`).join(', '), 400),
