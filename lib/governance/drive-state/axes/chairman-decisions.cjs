...
// QF-20260902-427: chairman_pending_decisions is a UNION whose Source 5 reads the feedback
// table's flag_review telemetry (auto-filed, dashboard-visibility-only, never a decision awaiting
// a chairman ruling) — 47 of 47 live rows on 2026-09-02 carried decision_type='flag_review',
// aged 8+ days, and were read here as pending DECISIONS. Mirrors the exclusion the ratified
// get_pending_chairman_items (20260710_create_get_pending_chairman_items.sql) already applies at
// its own consumer boundary for the identical reason. Excluding at THIS consumer (not the view)
// is deliberate: the view's Source 5 legitimately still surfaces flag_review to the chairman
// DASHBOARD, and narrowing the view itself would strip that surface and require a chairman-gated
// migration for a JS-only fix.
const EXCLUDED_DECISION_TYPES = Object.freeze(['flag_review']);

/** Exported so classify()'s fixture tests can exercise the exact exclusion predicate without a
 *  live DB — the same array-level filter fetch() applies to real rows below. */
function excludeUntrustedDecisionTypes(rows) {
  return (rows || []).filter((row) => !EXCLUDED_DECISION_TYPES.includes(row && row.decision_type));
}
...
async function fetch(supabase, { now } = {}) {
  ...
  // .limit(500): the header above already documents this population as attacker-reachable via an
  // unrestricted anon INSERT policy on the feedback table (Source 5) — a genuine, not decorative,
  // cap. A legitimate pending-decision queue is never in the hundreds (the 47/77-row contamination
  // this QF fixes was already read as anomalous), so 500 bounds a flood without truncating any
  // real population.
  const { data, error } = await supabase
    .from('chairman_pending_decisions')
    .select('id, title, priority, created_at, blocking, decision_type')
    .limit(500);
  if (error) throw new Error('chairman-decisions: query failed: ' + error.message);
  // QF-20260902-427: exclusion applied BEFORE classify()'s zero-population guard sees the
  // result, so an all-flag_review view correctly reduces to the real-decision count rather
  // than being counted first and only excluded during classification.
  return { pending: excludeUntrustedDecisionTypes(data), now };
}
...
function classify(state, clock) {
  ...
  if (state.pending.length === 0) {
    // ZERO PENDING IS UNMEASURABLE, NOT CLEAR — AND THIS AXIS FAILED OPEN HERE UNTIL A REVIEW
    // PROBED IT. A wrong-source fetch returns [], and returning CLEAR from that is a confident
    // all-clear from a blind probe: exactly the defect this SD exists to remove, in the axis that
    // opens it. Demonstrated by execution — correct source yielded STALLED with 1 pending, a wrong
    // source yielded CLEAR with "0 pending decisions" and nothing pinned the difference.
    //
    // Axes 2 and 5 already carried this guard (a zero population means the probe could not see a
    // set that certainly exists); axis 1 did not, and its own header even warned that querying the
    // underlying table instead of the view returns 0 rows — the author knew zero was reachable and
    // still rendered it as health.
    //
    // A genuinely empty queue is indistinguishable HERE from a failed read, so this reports the
    // honest thing rather than guessing. The distinction belongs to the caller that can prove the
    // read succeeded, and no such proof exists on this path today.
    return {
      axis: AXIS, state: STATE.UNMEASURABLE, reason: 'empty_decision_population',
      citation: 'chairman-decisions: 0 pending rows returned — an empty queue and a failed/mis-targeted ' +
        'read are indistinguishable on this path, so this is NOT reported as clear',
      owed: null, in_motion: null, stalled: null, action_taken: ACTION.UNVERIFIABLE
    };
  }
...
module.exports = { AXIS, fetch, classify, STALE_AFTER_HOURS, UNMEASURABLE_SUBSTATES, EXCLUDED_DECISION_TYPES, excludeUntrustedDecisionTypes };
