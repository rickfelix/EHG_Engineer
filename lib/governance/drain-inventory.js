...
/**
 * SD-LEO-INFRA-CAPTURE-CHANNEL-DISPOSITION-001 (FR-2). A LIFETIME-ever closing-path use is not the
 * same as an ADEQUATE closing-path RATE: the live invariant-gauge-finding channel carries exactly 1
 * lifetime disposition against 5,419 undrained rows (a 0.018% rate) and, before this constant existed,
 * that single disposition permanently disarmed exceedsBacklogThreshold regardless of population size.
 * 0.01 (1%) sits with wide margin between the live failing case (0.018%) and the pre-existing
 * ACCEPT-CONTROL fixture below (53/848 = 6.2%), so the threshold discriminates both without
 * re-litigating either.
 */
export const DEFAULT_RATE_FLOOR = 0.01;
 ...
 * counts while the closing path is being used AT AN ADEQUATE RATE.
 *
 * SD-LEO-INFRA-CAPTURE-CHANNEL-DISPOSITION-001 (FR-2) CORRECTED a defect in the ORIGINAL "not
 * draining" predicate: a STRICT `closingPathUses === 0` treated ANY nonzero lifetime use (even a
 * single disposition, ever) as proof of active draining, regardless of the outstanding population —
 * this is what let invariant-gauge-finding read PASS with 5,419 undrained rows against exactly 1
 * lifetime disposition. `closingPathUses` is now normalized (null/undefined/NaN -> 0, the SAFE
 * direction) and, for a genuinely nonzero value, checked as a RATE (uses/count) against
 * DEFAULT_RATE_FLOOR — but ONLY once the population is already backlog-sized, so a small, low-rate
 * queue that isn't a backlog by depth or age still passes untouched (unchanged from before this fix).
 *
 ...
 * @param {{count?:number, oldestAgeMs?:number|null, closingPathUses?:number|null}} reading
 * @returns {boolean}
 */
export function exceedsBacklogThreshold(descriptor, reading) {
  ...
  const isBacklogSized = count >= countLimit || age >= ageLimit;

  // A genuinely unmeasured closing path (null/undefined/NaN, e.g. a descriptor with no closing-path
  // reading wired up at all) is normalized to 0 -- never silently read as "some disposition exists".
  const uses = Number.isFinite(reading.closingPathUses) ? reading.closingPathUses : 0;
  if (uses === 0) return isBacklogSized;

  // uses is a LIFETIME total, count is the CURRENT open population -- not a bounded [0,1] ratio
  // (a well-managed channel can have uses > count), but a rate below the floor still discriminates
  // real neglect (0.018%) from every known-healthy fixture (6.2%, 25.7%, 100%+).
  const rateFloor = Number.isFinite(descriptor?.rateFloor) ? descriptor.rateFloor : DEFAULT_RATE_FLOOR;
  const rate = count > 0 ? uses / count : Infinity;
  return isBacklogSized && rate < rateFloor;
}
...
export function classifyObserved(descriptor, reading) {
  ...
  // 4,235 outstanding invariant_gauge_finding rows. Normalized the same way exceedsBacklogThreshold
  // does (null/undefined/NaN -> 0) so a genuinely-unmeasured closing path never reads as PASS by
  // falling through both checks on a small population.
  const uses = Number.isFinite(reading.closingPathUses) ? reading.closingPathUses : 0;
  if (uses === 0 && (reading.count || 0) > 0) {
...
export function buildInventoryRow(id, descriptor, reading) {
  ...
  return {
    ...
    // SD-LEO-INFRA-CAPTURE-CHANNEL-DISPOSITION-001 (FR-1): sla is a static descriptor fact (days,
    // optional severityIn); drainRate is a live-computed fact (uses/count) so a reader never has to
    // recompute it from the raw fields to answer "how healthy is this channel's closing rate".
    sla: descriptor?.sla ?? null,
    drainRate: (Number.isFinite(reading?.closingPathUses) && Number.isFinite(reading?.count) && reading.count > 0)
      ? reading.closingPathUses / reading.count
      : null,
  };
}
