...
// QF-20260830-086: the description text drifted from current_value (read "2 of 6" while the number
// said 5) because no writer ever rewrote it — recomputeKrGov31 wrote the number, never the prose.
// Derive both from the SAME per-layer result so they can never disagree again.
const LAYER_LABELS = { mission: 'Mission', constitution: 'Constitution', vision: 'Vision', strategy: 'Strategy', okr: 'OKRs', sd: 'SDs' };
export function buildKrGov31Description(layers, passingCount) {
  const passing = layers.filter((l) => l.pass).map((l) => LAYER_LABELS[l.key || l.layer]);
  const failing = layers.filter((l) => !l.pass).map((l) => LAYER_LABELS[l.key || l.layer]);
  const pendingNote = failing.length ? `; pending: ${failing.join(', ')}` : '';
  return `Mission + Constitution + Vision + Strategy + OKRs + SDs all accessible via CLI and connected in pipeline. Currently ${passingCount} of ${layers.length} layers active (${passing.join(', ') || 'none'})${pendingNote}.`;
}
...
export async function recomputeKrGov31(opts) {
  ...
  if (apply) {
    const { error } = await supabase
      .from('key_results')
      .update({
        current_value: passingCount, status, last_updated_by: RECOMPUTE_WRITER, updated_at: now,
        description: buildKrGov31Description(health.layers, passingCount),
      })
      .eq('code', KR_CODE);
...
export default { CASCADE_LAYERS, checkLayerHealth, computeCascadeHealth, recomputeKrGov31, buildKrGov31Description, KR_CODE, TARGET_LAYERS };
