...
// SD-FDBK-INFRA-SOLOMON-SCORECARD-MEASURES-001 FR-3: below this fraction of dimensions
// carrying a real signal, a cycle is flagged as too thin to read as a grade. Stated
// explicitly rather than implied, and overridable per role via config.coverageFloor.
// 0.5 is the point at which most of the rubric is guessing — Solomon at 2-of-5 (0.4)
// is exactly the case that motivated it.
...
function computeCoverage(dimensions, config, inconclusive) {
  ...
  // SD-FDBK-INFRA-SOLOMON-SCORECARD-MEASURES-001 FR-3: the coverage FLOOR.
  //
  // Saying the coverage out loud (shipped 2026-07-28) made a thin score honest but not
  // ACTIONABLE — nothing anywhere consumed coverage.scored, so a 1-of-5 cycle rendered its
  // caveat and passed on by. This adds the missing judgement: below the floor, the cycle is
  // flagged.
  //
  // SOFT-FLAG BY RULING, NOT A REFUSAL, and the reason is not stylistic. The writer's stated
  // invariant is fail-open/never-break-a-tick; a gate that can break the tick manufactures
  // pressure to bypass it, and a bypassed gate is WORSE than an advisory one because it still
  // looks enforcing. So this is additive metadata only — verify-score-contract's blocking
  // rules are deliberately NOT extended, and `overallString` is deliberately unchanged so the
  // already-shipped headline is neither rebuilt nor regressed.
  const floor = Number.isFinite(config && config.coverageFloor) ? config.coverageFloor : DEFAULT_COVERAGE_FLOOR;
  const ratio = total > 0 ? scored / total : 0;
  return {
    scored,
    total,
    unmeasured: Array.isArray(inconclusive) ? [...inconclusive] : [],
    floor,
    below_floor: total > 0 && ratio < floor,
  };
}
