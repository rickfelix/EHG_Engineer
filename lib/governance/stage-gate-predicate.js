 ...
 *   (e) normal comparison: blocked = (currentStage < requiredStage) OR (launch_mode !== 'live').
 *       SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-2 added the launch_mode arm: a venture's
 *       stage and its launch_mode are set by two independent writers (stage advancement
 *       vs. the chairman-gated go-live authorization in lib/eva/launch-mode.js), so a
 *       stage-only comparison would pass a venture that reached requiredStage through
 *       some OTHER path (a migration, a manual stage bump) without ever receiving that
 *       separate authorization. This does not change rule (c) — is_demo ventures remain
 *       OUT_OF_SCOPE regardless of launch_mode.
 *
 ...
 *
 * ATOMIC claim, not check-then-act (ship-gate adversarial review finding): when
 * shouldConsume is true, this issues ONE UPDATE ... WHERE consumed_at IS NULL ... RETURNING,
 * not a separate SELECT followed by an unconditional UPDATE. Two concurrent calls sharing
 * the same overrideKey+ventureId (a real scenario: email-campaigns.js's actorId is
 * enrollment.campaign_id, shared across every enrollment being processed in that campaign)
 * would otherwise both read the override as active before either commits its consumed_at
 * write, double-spending a single one-shot override. With the WHERE clause on the UPDATE
 * itself, only the first caller's write can match; the second's UPDATE affects zero rows
 * and correctly reports no active override.
 *
...
async function hasActiveOverride(supabase, overrideKey, ventureId, shouldConsume = false) {
  ...
  try {
    ...
    if (shouldConsume) {
      // Atomic claim: the WHERE clause (including .is('consumed_at', null)) is evaluated
      // by the DB as part of the same UPDATE, so a concurrent racer's identical UPDATE
      // cannot also match once this one commits.
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) {
        // SECURITY finding M8: an unreadable claim result must not be silently swallowed --
        // fail closed (never suppress) AND surface the fault, since a false "no override"
        // here is otherwise indistinguishable from "there was genuinely no override".
        console.warn(`[stage-gate-predicate] override atomic-claim update failed (fail-closed, override NOT applied this call): ${error.message || error}`);
        return false;
      }
      return !!data;
    }
    // Shadow mode: read-only existence check, never claims/consumes anything, so the
    // shadow verdict/audit row can accurately report "this WOULD be suppressed if armed"
    // without spending the one-shot on an evaluation that changes no real behavior.
    const { data, error } = await supabase
      .from('chairman_decisions')
      .select('id')
      .eq('override_key', overrideKey)
...
async function writeAuditRow(supabase, { entityType, entityId, ventureId, requiredStage, actualStage, verdict, armed, reason }) {
  ...
  try {
    const { error } = await supabase.from('audit_log').insert({
      ...
      entity_id: entityId,
      // `reason` (ship-gate adversarial review finding): without it, a chairman-override
      // PASS was indistinguishable in audit_log from a genuine "venture reached the
      // required stage" PASS -- exactly the event this predicate's most security-sensitive
      // path (an operator bypassing an external-contact gate) most needs to be traceable.
      metadata: { venture_id: ventureId, required_stage: requiredStage, actual_stage: actualStage, verdict, armed, reason: reason ?? null },
    });
    if (error) {
 ...
 * @returns {Promise<{ inScope: boolean, blocked: boolean, verdict: 'PASS'|'BLOCK'|'OUT_OF_SCOPE', reason: string|null, armed: boolean, venture: {is_demo:boolean,current_lifecycle_stage:number,launch_mode:string,status:string}|null }>}
 */
export async function checkStageGate({ supabase, ventureId, requiredStage, actorType, actorId, armed }) {
  ...
  if (!ventureId) {
    return { inScope: false, blocked: false, verdict: VERDICT.OUT_OF_SCOPE, reason: 'no_venture_id', armed: false, venture: null };
  }
  ...
  if (!Number.isInteger(requiredStage) || requiredStage < 1 || requiredStage > 26) {
    await writeAuditRow(supabase, { entityType: actorType, entityId: actorId, ventureId, requiredStage, actualStage: null, verdict: VERDICT.BLOCK, armed: resolvedArmed, reason: 'invalid_required_stage' });
    return { inScope: true, blocked: true, verdict: VERDICT.BLOCK, reason: 'invalid_required_stage', armed: resolvedArmed, venture: null };
  }
  ...
  try {
    const { data, error } = await supabase
      .from('ventures')
      .select('is_demo, current_lifecycle_stage, launch_mode, status')
      .eq('id', ventureId)
      .maybeSingle();
  ...
  //
  // `venture` (SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-5): the raw snapshot is surfaced on
  // every return path where it was resolved, so a caller building an independent positive
  // predicate on top (e.g. assertOutreachAuthorized below) does not need a second query.
  // This is additive only -- it does not change this function's own blocked/verdict/reason
  // semantics for SD/QF gating.
  if (venture && venture.is_demo === true) {
    return { inScope: false, blocked: false, verdict: VERDICT.OUT_OF_SCOPE, reason: 'is_demo', armed: false, venture };
  }
  ...
  if (!venture || actualStage === null || actualStage === undefined) {
    await writeAuditRow(supabase, { entityType: actorType, entityId: actorId, ventureId, requiredStage, actualStage: null, verdict: VERDICT.BLOCK, armed: resolvedArmed, reason: 'unresolvable_stage' });
    return { inScope: true, blocked: true, verdict: VERDICT.BLOCK, reason: 'unresolvable_stage', armed: resolvedArmed, venture };
  }
  ...
  //
  // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-2: launch_mode='live' is an ADDITIONAL
  // AND-condition here, belt-and-suspenders against a venture's stage advancing past
  // requiredStage through a path OTHER than the proper go-live authorization (the sole
  // writer of launch_mode='live' is lib/eva/launch-mode.js, chairman-gated). Rule (c)
  // above (is_demo) is unchanged — this does not touch demo-venture handling.
  const stageBlocked = actualStage < requiredStage;
  const launchModeBlocked = venture.launch_mode !== 'live';
  const rawBlocked = stageBlocked || launchModeBlocked;
  ...
  await writeAuditRow(supabase, { entityType: actorType, entityId: actorId, ventureId, requiredStage, actualStage, verdict, armed: resolvedArmed, reason });

  return { inScope: true, blocked, verdict, reason, armed: resolvedArmed, venture };
}
...
/** Reasons assertOutreachAuthorized() can refuse a send. */
export const OUTREACH_REFUSAL_REASON = Object.freeze({
  NO_VENTURE_ID: 'no_venture_id',
  UNRESOLVABLE_VENTURE: 'unresolvable_venture',
  IS_DEMO: 'is_demo',
  NOT_ACTIVE: 'not_active',
  BELOW_STAGE: 'below_stage',
  LAUNCH_MODE_NOT_LIVE: 'launch_mode_not_live',
});

/**
 * The ONE choke point for OUTREACH authorization (a real send/publish to an external
 * human) — distinct from checkStageGate()'s SD/QF-gating semantics above.
 *
 * SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-2/FR-3 (Solomon's amended design, coordinator
 * directive 7c1c6622, 2026-09-12): checkStageGate()'s rule (a) (no venture id ->
 * OUT_OF_SCOPE) and rule (c) (is_demo -> OUT_OF_SCOPE) are CORRECT for SD/QF gating —
 * most SDs/QFs legitimately have no venture link at all, and a demo venture should never
 * be audited as a real stage-gate subject. Both are WRONG for outreach: a send with no
 * venture linkage, or a send against a demo venture, must be REFUSED, not silently
 * treated as "not applicable, so it proceeds".
 *
 * The authorization predicate, stated POSITIVELY (per the design note: a boolean that
 * defaults to permit-on-absence decays the way a negatively-phrased flag does — see
 * evaluateGraduation()'s own comment on this same principle):
 *
 *   authorized iff the venture row resolves
 *     AND is_demo = false
 *     AND status = 'active'
 *     AND current_lifecycle_stage >= 24
 *     AND launch_mode = 'live'
 *
 * Deliberately NEVER "launch_mode != 'simulated'": both non-demo ventures ever measured
 * past stage 24 (DataDistill S27, MarketLens S25) are status='cancelled' — a stage-only
 * or launch-mode-only test would silently pass them.
 *
 * checkStageGate() is still invoked here (with armed forced true) so its audit_log
 * instrumentation and the one-shot chairman override remain the SOLE chairman touchpoint
 * for this gate too — but this function does NOT use checkStageGate()'s own
 * blocked/verdict as the authorization decision (that would inherit rules (a)/(c)'s
 * wrong-for-outreach OUT_OF_SCOPE semantics). It reads the venture snapshot instead and
 * evaluates the positive predicate above directly, honoring only a genuine
 * `reason:'chairman_override'` result as an override of that predicate.
 *
 * The shadow STAGE_GATE_PREDICATE_ARMED flag keeps its existing SD-gating role
 * (checkStageGate's OWN default `armed` resolution) and must never be read by this
 * function as a reason to permit a send — armed:true is passed explicitly, unconditionally,
 * every call, specifically so a disabled/absent flag can never turn this gate into a
 * silent pass-through for real customer outreach.
 *
 * @param {object} params
 ...
 * @returns {Promise<{ authorized: boolean, mode: 'live'|'mock', reason: string|null, snapshot: { stage: number|null, is_demo: boolean|null, status: string|null, launch_mode: string|null } }>}
 */
export async function assertOutreachAuthorized({ supabase, ventureId, actorType, actorId }) {
  if (!ventureId) {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.NO_VENTURE_ID, snapshot: { stage: null, is_demo: null, status: null, launch_mode: null } };
  }

  const result = await checkStageGate({ supabase, ventureId, requiredStage: 24, actorType, actorId, armed: true });

  const snapshot = {
    stage: result.venture ? result.venture.current_lifecycle_stage : null,
    is_demo: result.venture ? result.venture.is_demo : null,
    status: result.venture ? result.venture.status : null,
    launch_mode: result.venture ? result.venture.launch_mode : null,
  };

  // The one-shot chairman override is the sole chairman touchpoint for this gate: a
  // genuine override result authorizes regardless of the raw snapshot (e.g. a below-stage
  // venture the chairman has explicitly cleared for this specific action).
  if (result.reason === 'chairman_override') {
    return { authorized: true, mode: 'live', reason: 'chairman_override', snapshot };
  }

  if (!result.venture) {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.UNRESOLVABLE_VENTURE, snapshot };
  }
  if (result.venture.is_demo === true) {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.IS_DEMO, snapshot };
  }
  if (result.venture.status !== 'active') {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.NOT_ACTIVE, snapshot };
  }
  if (!(result.venture.current_lifecycle_stage >= 24)) {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.BELOW_STAGE, snapshot };
  }
  if (result.venture.launch_mode !== 'live') {
    return { authorized: false, mode: 'mock', reason: OUTREACH_REFUSAL_REASON.LAUNCH_MODE_NOT_LIVE, snapshot };
  }

  return { authorized: true, mode: 'live', reason: null, snapshot };
}

export default { checkStageGate, shouldEnforceBlock, assertOutreachAuthorized, OUTREACH_REFUSAL_REASON, VERDICT, STAGE_GATE_ARMED_FLAG_KEY };
