...
export const DRAIN_DESCRIPTORS = Object.freeze({
  ...
  'wind-down-survey': {
    ...
    // QF-20260803-503: rows now insert with status='resolved' (never 'new'/'triaged') — the
    // human-triage-lane flood this predicate used to describe (~14k residue, ~41-55/hour) is
    // the defect this QF fixed, not the intended design. A distribution-over-time reader
    // filters by category + metadata.reason, which is status-agnostic (FR-3's actual
    // acceptance criterion); it was never load-bearing on status='new'/'triaged'.
    predicate: 'feedback rows with category=wind_down_survey (status=resolved by design; query by metadata.reason for the distribution, not by status)',
    fieldQuestion: 'Answers "why did a worker stop", intended to feed fleet-attrition analysis.',
    evidence: 'Writer scripts/hooks/stop-loop-wakeup-reminder.cjs:200. Self-resolving since QF-20260803-503 (paired one-shot disposition of the pre-fix residue: QF-20260802-966). Still no dedicated reader script/gauge as of this writing — write-only sink, now out of the human lane by construction rather than by a status a reader would have queried on.',
  },
