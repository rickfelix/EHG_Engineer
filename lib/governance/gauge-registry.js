 ...
 *   ownerRole     — who a tripped finding is routed to (adam | solomon | michael | coordinator | chairman)
...
export const GAUGE_REGISTRY = [
  ...
  },
  {
    ...
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'adam',
    remediation: 'Confirm node scripts/adam-self-assessment-writer.cjs --force is firing on cadence (adam-startup-check.mjs self-score cron); check the fail-open catch path for a swallowed error.',
    // QF-20260911-404: verified live -- 174 rows, most recent within hours (well under the
    // gauge's 48h staleness threshold). Flipped on alongside solomon_self_score_age and the
    // coordinator's own gauge+writer fix in the same change (Adam disposition, Friday
    // Foundation Audit #2 item 6): a gauge with a demonstrated writer is not left enabled:false.
  ...
  },
  {
    ...
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'coordinator',
    remediation: 'Confirm node scripts/coordinator-self-review.mjs --force is firing on cadence (coordinator-startup-check.mjs self-review cron); check the fail-open catch path for a swallowed error.',
    // QF-20260911-404: root cause was 0 rows EVER -- COORD_SELF_SCORE_V1 was never set anywhere
    // (machine/user env, .env, CI all checked), and unlike Adam/Solomon's parallel flags this
    // writer shipped with no --force bypass at all, so no chairman-directed override could ever
    // reach it. Fixed in this change: added --force to coordinator-self-review.mjs (mirrors
    // Adam/Solomon's own pattern exactly) and wired it into the cron prompt
    // (coordinator-startup-check.mjs). NOT yet flipped on: this writer's insert is behind a
    // single-writer mutation guard (guardMutation/resolveOwnSessionId) that only the live
    // coordinator SESSION may satisfy -- a worker/QF seat cannot manufacture the first row (nor
    // should it try; that guard exists precisely to stop a non-coordinator session performing
    // this write). Enable this gauge once the coordinator re-arms its self-review cron with the
    // new --force prompt and scripts/lint/self-score-gauge-writer-lint.mjs confirms a live row.
    prevent: 'This gauge itself is the durable lapsed-self-score detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    ...
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'solomon',
    remediation: 'Confirm node scripts/solomon-self-assessment-writer.cjs --force is firing on cadence (the deep-sweep tick); check the fail-open catch path for a swallowed error.',
    // QF-20260911-404: verified live -- 84 rows, most recent within hours (well under the
    // gauge's 48h staleness threshold). Flipped on alongside adam_self_score_age and the
    // coordinator's own gauge+writer fix in the same change (Adam disposition, Friday
    // Foundation Audit #2 item 6): a gauge with a demonstrated writer is not left enabled:false.
    prevent: 'This gauge itself is the durable lapsed-self-score detector.',
    enabled: true,
  ...
  // QF-20260831-191: an escalated QF with no linked SD sits in NO lane -- the QF lane treats it
  // as done, no SD exists yet, so nothing reads it. Witnessed specimens QF-20260831-310 and -373
  // both sat this way until hand-linked. Complements (not replaces) SD-LEO-INFRA-TIERED-SOURCING-
  // CLAIM-001's F3, which is a point-in-time build-phase assertion; this is the standing detector.
  {
    id: 'orphaned-escalated-qf',
    name: 'Quick-fixes escalated with no linked SD (escalated_to_sd_id NULL) past grace window',
    detectorFn: 'orphaned-escalated-qf',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'adam',
    remediation: 'Hand-link each flagged QF to its escalated SD (set quick_fixes.escalated_to_sd_id), or if no SD was actually created yet, source one.',
    prevent: 'This gauge itself is the standing detection that a QF escalated with no SD link surfaces within one sweep cycle instead of sitting invisible in no lane.',
    enabled: true,
    tracesToLayer: null,
  },
  {
  ...
  // SD-LEO-INFRA-TIERED-SOURCING-CLAIM-001 (FR-2): QF-SCOPED ONLY (TESTING finding T-3) — an open
  // QF whose routing_tier is NULL bypassed the canonical triage-gate.js -> work-item-router.js
  // classification path entirely (the incident class behind QF-20260830-901). Baseline measured
  // pre-EXEC: 45 total QFs carry NULL routing_tier, but 0 are open, so tripWhen>0 on the open lane
  // is a clean starting signal, not a permanently-tripped one.
  {
    id: 'off-canonical-qf-mint',
    name: 'Open QF minted outside the canonical routing_tier assignment path',
    detectorFn: 'off-canonical-qf-mint',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'coordinator',
    remediation: 'Investigate how the flagged QF(s) reached open status without a routing_tier: confirm they were minted outside scripts/modules/triage-gate.js (direct insert, repair script) and either backfill routing_tier via the canonical classifier or escalate per work-item-router.js if a risk/schema signal is present.',
    prevent: 'SD-LEO-INFRA-TIERED-SOURCING-CLAIM-001 FR-2 — this gauge itself is the durable off-canonical-mint detection; a genuine fix is every QF creator routing through triage-gate.js, which is already the case for create-quick-fix.js and convergence-remediation-writers.js.',
    enabled: true,
    tracesToLayer: null,
  },
  // SD-LEO-INFRA-RESTORE-AGENT-TOOL-001 (FR-4): task-subagent-recorder.cjs had written ZERO rows
  // in its ENTIRE history before this SD (verified by direct DB query + independent Explore
  // re-verification) -- the Task->Agent tool rename left every hook matcher naming only 'Task',
  // AND two separate, pre-existing field-name bugs (reading tool_result/result and
  // tool_call_id/call_id instead of the verified tool_response/tool_use_id contract,
  // scripts/hooks/__tests__/session-id-propagation-canary.test.js:13-16) meant the hook would
  // have stayed a silent no-op even once the matcher/guard were fixed. Nothing detected this for
  // the hook's whole life. This gauge is the standing detection so a future regression of the
  // same class (a harness tool rename, a payload-shape drift) surfaces within one sweep cycle.
  {
    id: 'agent-tool-hook-liveness',
    name: 'task-subagent-recorder.cjs hook liveness (source=task_hook row freshness)',
    detectorFn: 'agent-tool-hook-liveness',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'coordinator',
    remediation: 'Zero sub_agent_execution_results rows with source=task_hook in the trailing 7 days while sessions were active in that window -- re-verify .claude/settings.json\'s Task|Agent matchers (PostToolUse ~:183, PreToolUse ~:316/:336) and task-subagent-recorder.cjs\'s guard/field reads (~:399-441) have not regressed.',
    prevent: 'SD-LEO-INFRA-RESTORE-AGENT-TOOL-001 -- this gauge itself is the durable liveness detection; a genuine fix is the matcher/guard/field-name fix staying correct across future harness tool-name or payload-shape changes.',
    enabled: true,
    tracesToLayer: null,
  },
  {
  ...
  {
    id: 'experience-review-coverage',
    name: 'Experience-review coverage for active ventures at stage>=20 (X3)',
    detectorFn: 'experience-review-coverage',
    thresholdConfig: { tripWhen: (result) => result?.alarmed === true },
    ownerRole: 'solomon',
    remediation: 'At least one active venture at stage 20 or later has zero venture_experience_review_runs rows -- run the experience-review pipeline (scripts/experience-review/gather-context.mjs + record-review.mjs, out-of-band annex if the venture already passed Stage 20) against the uncovered venture(s).',
    prevent: 'SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-D (X3) -- PREVENTIVE-CLASS per ratification 49656c8c/D5: this gauge is the durable detection that the experience-review pilot does not silently stop running once C3.1\'s manual AltifyAI run is no longer the only data point; the parent SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001\'s own tracking (not this gauge\'s pass/fail) applies the two-consecutive-weekly-clear-readings graduation bar. Solomon-owned miss (feedback 3b22d053).',
    enabled: true,
    tracesToLayer: null,
  },
  // SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-G: the 11 spec-§9 gauges (docs/michael/02-SPEC.md),
  // registered as STUBs (enabled:false, a real string detectorFn key with no resolver wired yet in
  // scripts/gauge-runner.mjs -- the runner skips an unresolved key non-fatally). Detection logic
  // depends on live feeder/seat data sibling Michael children own; registering the slot now avoids
  // a build-order deadlock. michael-token-budget is explicitly deferred per spec and NOT registered.
  {
    id: 'michael-brief-landed',
    name: "Michael's brief-of-record missing or unverified after the morning window",
    detectorFn: 'michael-brief-landed',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: 'select 1 from michael_brief_runs where et_date = today_et() and verified empty after 06:30 ET -- check the assemble/finalize feeders and render/verify logs.',
    prevent: 'This gauge itself is the durable never-claim-more-than-landed detector for the brief pipeline.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-feeder-health',
    name: 'A Michael feeder failed today, or degraded on three consecutive dates',
    detectorFn: 'michael-feeder-health',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: 'Any michael_feeder_runs row today with status=failed, or the same feeder degraded on three consecutive dates -- check the failing feeder\'s host/GHA logs.',
    prevent: 'This gauge itself is the durable per-feeder health detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-seat-uptime',
    name: 'Michael seat missed too many mornings in the trailing week (go-live gate)',
    detectorFn: 'michael-seat-uptime',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) >= 2 },
    ownerRole: 'michael',
    remediation: 'Count of dates in the last 7 with no michael_feeder_runs row where venue=seat trips at 2 -- go-live requires <=2 in the parallel-read week; check the seat\'s morning activation.',
    prevent: 'This gauge itself is the durable go-live seat-uptime detector (spec §10).',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-oauth-health',
    name: "Michael's Google OAuth grant invalid or expiring soon",
    detectorFn: 'michael-oauth-health',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: "michael_credentials.last_error='invalid_grant' or expires_at < now() + interval '48 hours' -- re-run the OAuth consent flow.",
    prevent: 'This gauge itself is the durable re-consent-due detector; the OAuth warning is deliberately a gauge, not a tick line, since the seat itself may be dead (spec §1.4).',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-classifier-drift',
    name: 'Sonnet/Opus classifier disagreement rate exceeds threshold',
    detectorFn: 'michael-classifier-drift',
    thresholdConfig: { tripWhen: (result) => (result?.rate || 0) > 0.05 },
    ownerRole: 'michael',
    remediation: '7-day share of sampled items where verified_by disagrees exceeds 5% -- review the disagreeing sample and correct the classifier rules.',
    prevent: 'This gauge itself is the durable classifier-drift detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-gmail-modify-ceiling',
    name: 'A Gmail-triage run hit its threads_modified ceiling',
    detectorFn: 'michael-gmail-modify-ceiling',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: 'Any run today with counts.threads_modified > ceiling -- the run is degraded by design; review the seeding inbox for a burst that warrants raising the ceiling.',
    prevent: 'This gauge itself is the durable modify-ceiling detector protecting against runaway Gmail writes.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-surface-rate',
    name: 'Chairman-surfaced needs_you count exceeds the chairman-set ceiling',
    detectorFn: 'michael-surface-rate',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > (result?.ceiling ?? 5) },
    ownerRole: 'michael',
    remediation: '7-day mean of needs_you count above a chairman-set ceiling (default 5) -- review recent surfaced items for a rule that should auto-apply instead.',
    prevent: 'This gauge itself is the durable over-surfacing detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-reopen-rate',
    name: 'A thread archived by an auto_apply rule was reopened (revoke trigger)',
    detectorFn: 'michael-reopen-rate',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) >= 1 },
    ownerRole: 'michael',
    remediation: 'Count of reopened_at in the last 7 days on threads archived by an auto_apply rule trips at 1 -- the rule is revoked per spec §7; review via --unarchive.',
    prevent: 'This gauge itself is the durable auto_apply-safety reopen detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-overdue-cleared',
    name: 'A proposed Todoist action sat un-actioned past the conversation day',
    detectorFn: 'michael-overdue-cleared',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: 'Any michael_todoist_snapshot row for a conversation day with proposed_action set and chosen_action null -- review with the chairman in the next morning conversation.',
    prevent: 'This gauge itself is the durable overdue-decision detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-ledger-gap',
    name: 'A brief was surfaced with no corresponding feedback-ledger entry',
    detectorFn: 'michael-ledger-gap',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) > 0 },
    ownerRole: 'michael',
    remediation: 'A date with surfaced_at set and no michael_feedback_ledger row -- check brief-finalize\'s ledger write path.',
    prevent: 'This gauge itself is the durable ledger-completeness detector.',
    enabled: false,
    tracesToLayer: null,
  },
  {
    id: 'michael-account-independence',
    name: 'Michael seat has run under one account_profile for 14+ consecutive registrations',
    detectorFn: 'michael-account-independence',
    thresholdConfig: { tripWhen: (result) => (result?.count || 0) >= 14 },
    ownerRole: 'michael',
    remediation: '14 consecutive seat registrations on one account_profile -- rotate the seat to a different Max-plan account profile.',
    prevent: 'This gauge itself is the durable account-independence detector for the three-account rotation discipline.',
    enabled: false,
    tracesToLayer: null,
  },
];
...
export const DRAIN_DESCRIPTORS = Object.freeze({
  ...
  'roadmap-link-exception': {
    ...
    consumer: 'scripts/adam-coordinator-health.mjs (countRoadmapLinkExceptionsLive, all-corpus + non_terminal split); scripts/ci/reasonless-roadmap-link-non-terminal.mjs (SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-D, non_terminal-scoped CI predicate, npm run ci:reasonless-roadmap-links)',
    closingPath: 'without_reason -> with_reason via the canonical builder; non-terminal repairs via scripts/sourcing-engine/repair-reasonless-roadmap-links.mjs',
    predicate: 'exceptions where reason_supplied !== true, scoped to NON-TERMINAL SDs by the CI predicate (the historical corpus is reported, never asserted on)',
    shapeContract: 'buildRoadmapLinkException emits {sd_key, operator_reason, reason_supplied, recorded_at}; countRoadmapLinkExceptions tests reason_supplied === true STRICTLY. A hand-authored {reason, recorded_by} row counts as without-reason no matter how good the reason. Measured live 2026-09-06: a bare STRING value (written by .artifacts/michael-002-fences-20260905.mjs:30, bypassing the builder) is the concrete instance of this disagreement — classifyExceptionShape names it bare_string.',
    accumulationSignal: 'Present for the NON-TERMINAL scope only, as a point-in-time count (no oldest-unresolved age) — the historical (terminal) corpus has no accumulation signal by design, since it is explicitly not a target to drive to zero.',
    fieldQuestion: 'Answers "was a reason supplied THROUGH THE CANONICAL BUILDER", not "was a reason supplied".',
    evidence: 'The writer/reader shape-disagreement variant: drains fine and reports a confident WRONG number. More dangerous than an undrained queue because it looks healthy and precise, and both parties are correct inside their own frame. Repaired 2026-09-06: 13 live non-terminal rows (9 bare_string, 4 no_reason_marker) via SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-D; the CI predicate now holds non_terminal.without_reason at 0 going forward for the live belt.',
  },
  ...
  // SD-LEO-INFRA-HARNESS-BACKLOG-PER-001 (FR-5): this descriptor's single-literal
  // source.category filter narrows correctly BY CONSTRUCTION once FR-2's write-time routing
  // and the FR-3/FR-4 backfill move real per-flag findings to category='completion_flag_finding'
  // — no code change needed here; do not "fix" this into an enumeration.
  'harness-backlog': {
    ...
    // QF-20260903-333: harness_backlog carries no SLA_CATEGORIES entry (retired — no
    // automated route meets a 7-day deadline). SLA_CATEGORIES.harness_backlog is `undefined`
    // here by design; drain-inventory.js's `descriptor?.sla ?? null` already handles it.
    ...
    evidence: 'Live measured (2026-08-24, TESTING-corrected): 7,236 total rows, 6,258 open (new/triaged), 996 closed via genuinely TERMINAL statuses (resolved/wont_fix/invalid, NOT the complement of open -- backlog/in_progress are deliberately non-terminal) plus the promoter\'s metadata flag (15.9% rate) — the terminal-status trigger alone accounts for 976 rows; only ~259-464 rows have EVER carried an SD/QF link at all, so the residual gap is TRIAGE INTAKE (most rows are never linked to anything), not a missing disposition mechanism. feedback-sla-gauge.cjs\'s SLA_CATEGORIES entry for this category is additionally scoped to severityIn:[high,critical], leaving ~94% (medium/low/NULL severity) invisible to active alarming (FR-3).',
  },
  'completion-flag': {
    ...
    evidence: 'Live measured (2026-08-24, TESTING-corrected): 520 total rows, 280 open (new/triaged), 185 closed via genuinely TERMINAL statuses (resolved/wont_fix/invalid -- backlog is deliberately non-terminal, per lib/coordination/unified-inbox-builder.js\'s own status mapping) -- 66.1% rate, still a healthy, actively-working channel. The chairman-facing "~98% unconsumed" figure cited at this SD\'s authoring measures a DIFFERENT, narrower extent (the trailing 30-day window, where 98.1% of recently-created rows are still open because triage activity paused after 2026-08-17) — both figures are correct, measuring different time extents; see FR-1 AC-5.',
  },
});
