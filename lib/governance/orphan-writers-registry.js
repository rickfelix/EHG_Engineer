...
export const ORPHAN_ENTRIES = Object.freeze([
  ...
  // --- QF-20260906-590: singleton role-seat dead-process guard reads hostname/pid from four
  // identity fetches that never selected them, so isSeatProcessDead's fail-closed input guards
  // (row.hostname/row.pid) can never fire from these callers. ---
  Object.freeze({
    id: 'role-seat-liveness-hostname-pid-not-selected',
    entry_type: 'wired-but-blind',
    writer: { kind: 'column', table: 'claude_sessions', column: 'hostname, pid', description: 'scripts/hooks/capture-session-id.cjs:460-469 writes top-level hostname and pid = metadata.cc_pid on every claude_sessions row.' },
    reader: { file: 'lib/coordinator/role-seat-liveness.cjs:42-48', description: 'isSeatProcessDead requires row.hostname/row.pid to prove a prior seat process is gone (Adam+Solomon singleton guards), but its four callers -- adam-identity.cjs fetchFreshAdams (:187) and fetchAllAdamsStrict (:218), solomon-identity.cjs fetchFreshSolomons (:148) and fetchAllSolomonsStrict (:174) -- never selected hostname/pid, so every row arrives with both fields undefined and the guard returns false (cannot say) by construction.' },
    predicate: { description: 'The four select() lists cited above omit hostname and pid; isSeatProcessDead\'s row.hostname/row.pid type checks (role-seat-liveness.cjs:42,44) fail on every row from these callers regardless of the real process state.' },
    evidence: 'Adam, 2026-09-06 11:09-11:25Z, live Adam restart: successor 1fd18159 refused 3x by prior d1140357 (cc_pid 36844, confirmed NOT FOUND via PowerShell Get-Process) until the unrelated 15-minute tool-silence cut retired the seat. QF-20260906-590.',
  }),
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // QF-20260913-811: Adam's first session-armed run of scripts/drain-inventory.mjs
  // (coordinator request 22976f55, 2026-09-13 14:31Z) reported 14 detectors, 8 FAILING —
  // none registered here, so the chairman's weekly known-orphan number (ratification
  // 2ab4b4bc) could not count them. Each referenced by id into DRAIN_DESCRIPTORS
  // (gauge-registry.js) rather than re-declared (FR-6, same pattern as
  // 'feedback-sla-categories' above). A ninth detector named in the run,
  // feedback-sla-breach, is NOT added here — it is already represented by the existing
  // 'feedback-sla-categories' entry above (refs_drain_descriptor: 'feedback-sla-breach'),
  // and getReaderClassification/scripts/orphan-writers-count.mjs both key on entry_type
  // alone, so adding a second entry for the same DRAIN_DESCRIPTORS key would duplicate
  // one specimen's count, not add a new one.
  // ═══════════════════════════════════════════════════════════════════════

  // --- writer-with-no-reader: NO_CONSUMER ---
  Object.freeze({
    id: 'relay-drop',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'relay-drop',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CONSUMER, n=20, oldest 22.8d. DRAIN_DESCRIPTORS[\'relay-drop\'] declares no `consumer` (writer scripts/coordinator-relay-drop-gauge.cjs:56) — registered as a gauge with ownerRole=coordinator and still undrained.',
  }),
  Object.freeze({
    id: 'wind-down-survey',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'wind-down-survey',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CONSUMER. DRAIN_DESCRIPTORS[\'wind-down-survey\'] declares no `consumer` (writer scripts/hooks/stop-loop-wakeup-reminder.cjs recordWindDown()) — worker_wind_down_events remains a write-only sink.',
  }),
  Object.freeze({
    id: 'quick-fixes-stranded',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'quick-fixes-stranded',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CONSUMER (naive-timestamp skew, SD-LEO-INFRA-NAIVE-TIMESTAMP-SKEW-001 cross-referenced). DRAIN_DESCRIPTORS[\'quick-fixes-stranded\'] declares no `consumer` — scripts/orphan-qf-reaper.mjs:229 requires .not(claiming_session_id,is,null), reconciling the OPPOSITE population; nothing owns quick_fixes rows with status=in_progress AND claiming_session_id IS NULL.',
  }),
  Object.freeze({
    id: 'worktree-reaper-refusals',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'worktree-reaper-refusals',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CONSUMER. DRAIN_DESCRIPTORS[\'worktree-reaper-refusals\'] declares no `consumer` — writer worktree-reaper-tick.cjs:285, its only invoker ignores the return value; the accumulation counter (.claude/worktree-reaper-state.json, consecutive_refusals) is present but undrained.',
  }),

  // --- writer-with-no-reader: NO_CLOSING_PATH (a consumer/reader is named, but nothing
  // marks a row addressed -- QF-20260913-811's fix shape groups this with NO_CONSUMER
  // under writer-with-no-reader, since getReaderClassification's two-bucket rule has no
  // separate NO_CLOSING_PATH bucket) ---
  Object.freeze({
    id: 'adam-adherence-drift',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'adam-adherence-drift',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CLOSING_PATH, n=191, oldest 83.7d. DRAIN_DESCRIPTORS[\'adam-adherence-drift\'] names a consumer (feedback-sla-gauge, counts it) but no closingPath -- the gauge\'s only response is to write ANOTHER feedback row (feedback_sla_breach) which is itself undrained. A reminder is not a drain.',
  }),
  Object.freeze({
    id: 'adam-adherence-ledger',
    entry_type: 'writer-with-no-reader',
    refs_drain_descriptor: 'adam-adherence-ledger',
    known_orphan: true,
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: NO_CLOSING_PATH, n=371, oldest 83.7d. DRAIN_DESCRIPTORS[\'adam-adherence-ledger\'] names a real consumer (lib/vision/rung-health-convergence.mjs:126) but closingPath is structurally ABSENT: the adam_adherence_ledger table has no status column at all (run_id/probe/duty/verdict/detail/remediation_ref/created_at) -- a verdict=fail row can never be marked addressed. A named consumer is not sufficient when the table cannot express DONE.',
  }),

  // --- wired-but-blind: a reader/closing-path exists and is genuinely never used
  // (CLOSING_PATH_UNEXERCISED, distinct from NO_CONSUMER/NO_CLOSING_PATH above) ---
  Object.freeze({
    id: 'invariant-gauge-finding',
    entry_type: 'wired-but-blind',
    refs_drain_descriptor: 'invariant-gauge-finding',
    evidence: 'Adam, drain-inventory run 22976f55, 2026-09-13 14:31Z: UNDRAINED, n=5687, oldest 72.1d. DRAIN_DESCRIPTORS[\'invariant-gauge-finding\'] names a real closing path (a gauge_finding_dispositions row per finding, shipped by QF-20260911-425 PR #8715) that has NEVER been exercised -- gauge_finding_dispositions is live with ZERO rows ever written despite this being the largest single undrained category in the database. The designed drain exists and is not running or not draining, which is wired-but-blind, not writer-with-no-reader (no known_orphan needed: DRAIN_DESCRIPTORS declares a real consumer).',
  }),
]);
