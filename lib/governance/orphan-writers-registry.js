...
export const ORPHAN_ENTRIES = Object.freeze([
  ...
  Object.freeze({
    ...
    predicate: { description: '2,494 open feedback rows in category=harness_backlog, oldest 89 days, none routed or retired by any reader.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 1 [owner QF-333; route or retire].',
  }),
  // --- specimen 2: writer-with-no-reader ---
  Object.freeze({
    id: 'sub-agent-results-storage-unpersisted-warning',
    entry_type: 'writer-with-no-reader',
    writer: { file: 'lib/sub-agent-executor/results-storage.js', description: 'When a caller field cannot be persisted, the store console.warns and then returns success — the warning is the only record of the drop.' },
    reader: { kind: 'none', description: 'Nothing consumes the console.warn output; the store\'s success return is what every caller and every downstream gate actually observes.' },
    predicate: { description: 'A caller field that failed to persist produces a console-only warning with no row, alert, or gauge reading it — the store reports success regardless.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 2 [SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H].',
  }),
  // --- specimen 3: writer-with-no-reader ---
  Object.freeze({
    id: 'presend-consult-lane-choke-error-swallowed',
    entry_type: 'writer-with-no-reader',
    writer: { file: 'presend-consult-lane.cjs', description: 'Lines 116-121 catch and hold the choke error text that caused a chairman send to be held, then discard it.' },
    reader: { kind: 'none', description: 'The cause of the held send is never persisted anywhere a reader — a row, a log sink, a seat — could later consult it.' },
    predicate: { description: 'A held chairman send has no queryable record of WHY it was held; the choke error text exists only transiently inside the catch block.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 3 [Adam fix shape 2cd77e87].',
  }),
  // --- specimen 4: reads-before-the-writer ---
  Object.freeze({
    id: 'worktree-reaper-stagereclaim-placeholder',
    entry_type: 'reads-before-the-writer',
    writer: { kind: 'field', description: '_reclaimCandidate.auditAccepted — the real audit-acceptance writer that would set this field to a meaningful value does not exist yet.' },
    reader: { file: 'scripts/worktree-reaper.mjs', description: 'stageReclaim (~:1754) filters on _reclaimCandidate, which is derived from the still-placeholder auditAccepted:false field.' },
    predicate: { description: 'stageReclaim always evaluates auditAccepted:false because no writer has ever set it to true — the filter always returns empty, structurally, because the reader runs before any real writer exists.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 4 [QF-20260904-508].',
  }),
  ...
  // --- specimen 7: wired-but-blind ---
  Object.freeze({
    id: 'claim-eligibility-hold-provenance-coalescer',
    entry_type: 'wired-but-blind',
    writer: { kind: 'fields', description: 'Ten hold-provenance fields are stamped by their respective writers across the claim-eligibility surface.' },
    reader: { file: 'lib/fleet/claim-eligibility.cjs', description: 'resolveHoldProvenance (:711) coalesces only six of those keys; writers stamp four others that the coalescer never reads.' },
    predicate: { description: 'Two chairman-ceremony fences render as bare boolean flags because their provenance keys are among the four the coalescer omits — the reader is wired but blind to those four fields.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 7 [QF-724].',
  }),
  // --- specimen 8: reads-before-the-writer ---
  Object.freeze({
    id: 'solomon-advisory-already-answered-backpressure-parked',
    entry_type: 'reads-before-the-writer',
    writer: { kind: 'field', table: 'session_coordination', column: 'backpressure_parked', description: 'A row is marked backpressure_parked before the real answer/delivery writer ever runs against it.' },
    reader: { file: 'scripts/solomon-advisory.cjs', description: 'alreadyAnswered and the CC-idempotence query key on reply_to, and read a backpressure-parked row as already delivered.' },
    predicate: { description: 'A backpressure-parked row is indistinguishable, to alreadyAnswered, from a genuinely delivered one — the guard reads the row before the delivery writer has actually run against it.' },
    evidence: 'Adam, 2026-09-04 ~22:4xZ, Solomon deep-sweep finding 4b662adb specimen 8 [board c81e8d43].',
  }),
  ...
  Object.freeze({
    ...
    entry_type: 'wired-but-blind',
    writer: { file: 'lib/coordinator/dispatch.cjs', description: 'Line ~1100: the backpressure counter increments over every session_coordination row sent to a target, including INFORMATIONAL_KINDS such as roll_call.' },
    reader: { file: 'lib/coordinator/dispatch.cjs', description: 'The same counter is read as a backpressure signal — a target is treated as backed up once its count crosses the floor.' },
    predicate: { description: 'roll_call rows never ack, so any target that receives them sits at a permanent counted floor (coordinator measured: 328 unacked rows, all roll_call) — the counter cannot discriminate real backpressure from informational noise. Solomon 47cd9f79: reads-before-the-writer is explicitly NOT the mechanism here; the reader and writer are the same live counter, just counting the wrong population.' },
    evidence: 'Solomon 47cd9f79, 2026-09-04 23:17Z, measured at lib/coordinator/dispatch.cjs:1100 [Tier-1 QF filed 2026-09-04 ~23:2xZ: counter filter fix].',
  }),
  // --- specimen 13: writer-with-no-reader ---
  Object.freeze({
    id: 'session-coordination-backpressure-parked-no-reader',
    entry_type: 'writer-with-no-reader',
    writer: { kind: 'column', table: 'session_coordination', column: 'backpressure_parked', description: 'Dispatch writers set backpressure_parked on rows they hold back under load.' },
    reader: { kind: 'none', description: 'Grep over lib/ and scripts/ (writers excluded) finds no reader anywhere that consumes backpressure_parked.' },
    predicate: { description: 'session_coordination.backpressure_parked is written but has zero consuming readers in the codebase.' },
    evidence: 'Solomon 47cd9f79, 2026-09-04 23:17Z, measured by grep over lib/ and scripts/ [Tier-1 QF filed 2026-09-04 ~23:2xZ: role-seat roll-call skip].',
  }),
  ...
  // --- detector-with-no-sink: first specimen ---
  Object.freeze({
    id: 'stale-session-sweep-conflicts-console-only',
    entry_type: 'detector-with-no-sink',
    writer: { file: 'scripts/stale-session-sweep.cjs', description: 'Line ~4189: the holds-N-claims CONFLICTS detector computes and prints its finding.' },
    reader: { kind: 'none', description: 'The finding is console-only — no row, file, or seat persists it, so nothing downstream can ever discover a past CONFLICTS finding.' },
    predicate: { description: 'The CONFLICTS line reaches no reader because it was never durably written anywhere in the first place: a detector with no sink.' },
    evidence: 'Solomon deep-sweep finding 84677786 item 2, Adam scribe, 2026-09-05 10:5xZ, measured at scripts/stale-session-sweep.cjs:4189.',
  }),
