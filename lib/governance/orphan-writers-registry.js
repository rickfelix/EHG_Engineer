...
export const ORPHAN_ENTRIES = Object.freeze([
  ...
  Object.freeze({
    ...
    reader: { file: 'scripts/audit-retrospectives-audit-actor-provenance.mjs', description: 'FR-3(c) scheduled check counting retrospectives_audit rows with changed_by IS NULL OR NOT IN the canonical set. Also: tests/unit/governance/retrospectives-writer-census.test.js (FR-3(b), the writer allowlist this trigger will eventually gate) and lib/eva/__tests__/retro-clobber-guard.test.js (the 8 isSafeToWriteRetro wire-in sites, one of which -- restore_from_audit -- is now also a registered retro_canonical_writer_policy identity).' },
