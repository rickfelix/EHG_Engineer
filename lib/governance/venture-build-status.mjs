...
import { formatCauseLine } from './venture-cause-line.js';
...
/**
 * SD-LEO-INFRA-AUTOMATED-VENTURE-TROUBLESHOOTING-001 FR-5: the most recent captured_cause
 * for a venture's stage-23 journey walk, rendered as a single status-line string, or null
 * when there is no failed walk with a capture to show (a clean walk, or no walk at all --
 * both render as "no cause to show", never as an error).
 *
 * Uses the SAME lookup shape as lib/eva/quality-findings/db-sourced-findings.js's
 * produceJourneyWalkFindings: orchestrator rows are matched via `metadata->>venture_id`,
 * NOT the incompletely-backfilled top-level `venture_id` column (DESIGN evidence 1a71cf03).
 * Fail-soft to null on ANY read error -- a cause-line failure must never affect the
 * BUILD_STATUS verdict itself, which is why this is a SEPARATE function called only from
 * the IO wrappers below, never from the pure deriveVentureBuildStatus().
 * @param {object} supabase
 * @param {string} ventureId
 * @returns {Promise<string|null>}
 */
export async function fetchCauseLineForVenture(supabase, ventureId) {
  ...
  try {
    const { data: orchestrators, error } = await supabase
      ...
      .limit(5);
    if (error || !Array.isArray(orchestrators)) return null;

    const withFailedCause = orchestrators.find(
      (o) => o?.metadata?.journey_walk_result?.status === 'fail' && o?.metadata?.journey_walk_result?.captured_cause
    );
    if (!withFailedCause) return null;

    return formatCauseLine(withFailedCause.metadata.journey_walk_result.captured_cause);
  } catch {
    return null;
  }
}
 ...
 * @param {object} supabase
 * @param {string} ventureId
 * @returns {Promise<{status:string, evidence:object, measured_at:string, cause?:string|null}>}
 */
export async function fetchVentureBuildStatus(supabase, ventureId) {
  ...
  try {
    ...
    const derived = deriveVentureBuildStatus(ventureRow, { hasRoutedDeployment });
    // FR-5: additive only -- deriveVentureBuildStatus's own status/evidence are untouched,
    // this is a sibling key populated by a second, independently-failing IO read.
    const cause = await fetchCauseLineForVenture(supabase, ventureId);
    return cause ? { ...derived, cause } : derived;
  } catch (e) {
...
export async function fetchVentureBuildStatusBatch(supabase, ventureIds) {
  ...
  // FR-5: ONE batched orchestrator query for the whole set (same one-query-per-set
  // convention as the two reads above), never one query per venture.
  let causeById = new Map();
  try {
    const { data: orchestrators, error } = await supabase
      .from('strategic_directives_v2')
      .select('metadata')
      .eq('sd_type', 'orchestrator')
      .in('metadata->>venture_id', ids)
      .order('created_at', { ascending: false })
      .limit(ids.length * 5);
    if (!error && Array.isArray(orchestrators)) {
      for (const o of orchestrators) {
        const vId = o?.metadata?.venture_id;
        if (!vId || causeById.has(vId)) continue; // most-recent-first: first hit per venture wins
        const jwr = o?.metadata?.journey_walk_result;
        if (jwr?.status === 'fail' && jwr?.captured_cause) {
          causeById.set(vId, formatCauseLine(jwr.captured_cause));
        }
      }
    }
  } catch {
    // fail-soft: cause line degrades to absent per-venture, not a batch failure (same
    // convention as the venture_deployments read above).
  }
  ...
  for (const id of ids) {
    ...
    const derived = deriveVentureBuildStatus(byId.get(id), { hasRoutedDeployment: routedDeploymentIds.has(id) });
    const cause = causeById.get(id);
    result.set(id, cause ? { ...derived, cause } : derived);
  }
  return result;
}
