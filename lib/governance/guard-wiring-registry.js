...
export const GUARD_REGISTRY = deepFreeze([
  {
    ...
    definedAt: 'scripts/solomon-advisory.cjs:322',
    gatedInput: 'spent',
    permissiveMeans: 'withinBudget:true — the per-sweep cost ceiling does not apply',
    expectedWired: false,
    note: 'The Solomon per-sweep cost ceiling. STILL ZERO executable production callers; the only '
      + 'caller-shaped reference is a PROMPT STRING at scripts/solomon-startup-check.mjs:187, so it '
      + 'stays enforced by operator judgment alone and expectedWired stays false. QF-20260903-418 '
      + 'changed what the permissive branch COSTS, not the wiring: the extent is now DERIVED '
      + '(sweepStartedAtMs -> elapsed) instead of caller-documented, the retired scope-ambiguous keys '
      + 'are refused, and an unmeasurable extent FAILS CLOSED, so the argument-less rubber stamp of '
      + 'QF-20260729-221 is gone. What is left unwired is the CALL, not the check.',
  },
  {
