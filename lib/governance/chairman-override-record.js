...
export function buildOverrideRecord({
  ...
  return {
    ...
    // THIS IS AN AUDIT RECORD OF SOMETHING ALREADY DECIDED — NOT AN OPEN DECISION.
    // severity:'high' + status:'new' + resolved_at:null is exactly the predicate the
    // chairman_all_decision_signals view uses, and that view has NO category filter. A historical
    // override therefore surfaced as a decision AWAITING the chairman, and relayed the
    // operator-supplied rationale into a chairman-facing queue it was never written for. The
    // chairman already made this call; re-asking is both noise and a small disclosure.
    severity: 'medium',
    status: 'resolved',
    resolved_at: at,
    resolution_notes: `Chairman-directed override, recorded for audit. Boundary: ${b}`.slice(0, 1000),
...
export async function recordChairmanOverride(supabase, input, { logger = console } = {}) {
  ...
  try {
    // SOURCE_TYPE IS PART OF THE DEDUP IDENTITY, and that is a security property, not tidiness.
    // The `telegram_bot_insert_feedback` policy constrains ONLY source_type, so an anon actor can
    // insert a row with category='chairman_override' and any metadata it likes. Deduping on
    // (category, override_key) alone let a PRE-SEEDED FORGERY collide with a genuine crossing: the
    // real writer would return {deduped:true} with the FORGERY'S id, suppressing the authentic
    // record and reporting the forgery as though it were ours. A forged governance record is worse
    // than a missing one. Scoping the read to our own source_type means a forgery can add noise but
    // can never silence the real thing.
