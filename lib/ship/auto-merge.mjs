...
/**
 * QF-20260815-128: a PR's headRefOid is a stable historical fact — the last commit that was on
 * the branch, unaffected by the merge method or whether the branch was later deleted. Read once
 * right before the merge call and once right after a confirmed merge; if they differ, a push
 * landed in the gap between this function's two callers and the merge may not carry the commit
 * that was intended (measured incident, PR #7060, Golf-2 signal 02302c4d).
 * Returns null on lookup failure — advisory only, never blocks a merge that already succeeded.
 * @returns {string|null}
 */
export function fetchHeadRefOid(prNumber, repoOwner, repoName, runner = defaultRunner) {
  if (!repoOwner || !repoName) return null;
  const r = runner(['pr', 'view', String(prNumber), '-R', `${repoOwner}/${repoName}`, '--json', 'headRefOid', '--jq', '.headRefOid']);
  if (r.code !== 0) return null;
  const trimmed = r.stdout.trim();
  return trimmed || null;
}
...
export async function attemptAutoMerge({
  ...
  // QF-20260815-128: snapshot the head we're ABOUT to merge, right before the merge call, so a
  // push landing in the (small but real) gap between here and the confirmed-merge check below is
  // detectable rather than silently absorbed.
  const intendedHead = fetchHeadRefOid(prNumber, repoOwner, repoName, runner);
  ...
  if (merge.code === 0) {
    if (verifyMerged(prNumber, repoOwner, repoName, runner)) {
      // QF-20260815-128: headRefOid is a stable historical fact about the PR (unaffected by merge
      // method), so re-reading it post-merge and comparing against the pre-merge snapshot detects
      // exactly the class of race the measured incident (PR #7060) hit: a push landing after the
      // intended head was read. Advisory-only — the merge already happened, so this can only
      // surface the mismatch loudly, not prevent it (unlike gh-merge-safe.mjs's atomic sha-pinned
      // merge, this CLI subcommand has no equivalent test-and-set parameter).
      const mergedHead = fetchHeadRefOid(prNumber, repoOwner, repoName, runner);
      if (intendedHead && mergedHead && intendedHead !== mergedHead) {
        logger.error(
          `[MERGE_HEAD_MISMATCH] PR #${prNumber}: intended head ${intendedHead}, but the branch's ` +
          `head was ${mergedHead} by the time the merge was confirmed — a push landed after this ` +
          `merge was armed. Verify main contains the intended commit; open a recovery PR for any delta.`,
        );
      }
    ...
    logger.warn(
      `⚠️  gh pr merge exited 0 but mergedAt is null — verifying PR state before declaring success`, // gh-merge-guard-exempt: diagnostic log inside this module's own already-hardened merge call, not a worker-facing instruction
    );
  }
  ...
  if (verifyMerged(prNumber, repoOwner, repoName, runner)) {
    logger.info(
      `ℹ️  gh pr merge returned ${merge.code} but PR is already MERGED (concurrent-merge race) — continuing`, // gh-merge-guard-exempt: diagnostic log inside this module's own already-hardened merge call, not a worker-facing instruction
    );
    await observeMergeWorkLadder({
      prNumber, repoOwner, repoName, workKey, tier, runner, merged: true, verifyResult: { ok: true },
      lookupWorkKeyReal, fetchReviewFinding, supabase: witnessSupabase, lane, logger,
  ...
  const reason = silentSuccess
    ? `silent-success: gh pr merge exit 0 but mergedAt null, state=${state ?? 'unknown'}` // gh-merge-guard-exempt: diagnostic string inside this module's own already-hardened merge call, not a worker-facing instruction
    : (merge.stderr.trim() || `merge exit ${merge.code}, state=${state ?? 'unknown'}`);
  logger.error(
    silentSuccess
      ? `❌ gh pr merge silent-success regression (exit 0, mergedAt null, state=${state ?? 'unknown'}) — /ship HARD-FAIL` // gh-merge-guard-exempt: diagnostic log inside this module's own already-hardened merge call, not a worker-facing instruction
      : `❌ gh pr merge failed (exit ${merge.code}, PR state=${state ?? 'unknown'}) — /ship HARD-FAIL`, // gh-merge-guard-exempt: diagnostic log inside this module's own already-hardened merge call, not a worker-facing instruction
  );
  logger.error(
    `   Manual recovery: gh pr ready ${prNumber} && gh pr merge ${prNumber} --merge --delete-branch --admin`, // gh-merge-guard-exempt: --admin is unsupported by gh-merge-safe.mjs's parseArgs; this is a last-resort manual-recovery path after the hardened auto-merge already failed
  );
  await observeMergeWorkLadder({
