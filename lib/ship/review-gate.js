 ...
 *
 * QF-20260904-868 (RCA, following the coordinator's own signal-557d3939 ruling): the
 * ticket that reported this false positive attributed it to a bypassed addedLinesOnly
 * filter -- verified empirically that premise is WRONG (addedLinesOnly already
 * correctly excludes the cited context line). The real cause: a diff "line" can be a
 * machine-generated blob with NO embedded newlines (e.g. a JSON.stringify test-evidence
 * artifact), sometimes exceeding a million characters. Every CRIT pattern is written
 * assuming a human-scale source line; an unconstrained '.*' proximity pattern (one of
 * this same closed enumeration's own auth-bypass-class checks) trivially co-occurs two
 * unrelated substrings by chance alone across such a line (measured: one test title's
 * imperative kill-switch wording and, ~1MB later on the SAME line, an unrelated test
 * title's unrelated review-passed wording). A legitimate source line is never this long
 * -- excluded past a generous sanity threshold, rather than special-casing every
 * generated-file path that could
 * produce one (a path-based exemption list would need updating for every future
 * generator; a length cap needs none).
 * @param {string} body
 * @param {number} [width]
 * @returns {string}
 */
const MAX_SCANNABLE_LINE_LENGTH = 5000;
...
function addedLinesOnly(body, width = 1) {
  return body
    ...
    .filter((line) => {
      ...
      if (prefix.length !== width || !prefix.includes('+')) return false;
      return line.length <= MAX_SCANNABLE_LINE_LENGTH;
    })
