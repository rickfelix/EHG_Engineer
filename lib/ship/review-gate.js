 ...
 * `diff --git … b/<path>` / `+++ b/<path>` headers, and its polarity-column
 * `width` from its hunk header. Content before any header (e.g. a bare
 * snippet passed by a unit test) has a null path, width 1, and is scanned
 * as one non-exempt segment.
 *
 * QF-20260818-024 (RCA, following QF-20260818-651): a unified-diff hunk
 * header is `@@ ... @@` (2 '@', one polarity column per line: '+'/'-'/' ').
 * A COMBINED diff (git's `--cc`/`-c` format, e.g. `git show <merge-sha>`
 * for a 2+-parent merge) uses `@@@ ... @@@` (N+1 '@' for an N-parent merge,
 * one polarity column PER PARENT). `/^@@ /` never matches `@@@`, so a
 * combined-diff hunk was never recognized as "in a hunk" -- reopening the
 * `+++`-marker path-spoof class this file's own header comment documents,
 * for the whole file. Widened to `/^(@@+) /`, a strict superset that also
 * captures the header's own '@' run length to compute the per-file
 * polarity-column width (captured length - 1).
...
/**
 * Reduce a diff segment body down to ADDED content only, width-aware (the
 * `+++ b/...` file-header line is always excluded regardless of width, since
 * extended diff headers are never polarity-column-prefixed).
