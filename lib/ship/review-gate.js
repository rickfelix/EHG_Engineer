...
/**
 * Reduce a diff segment body down to ADDED lines only (unified-diff `+` prefix,
 * excluding the `+++ b/...` file-header line), with the leading `+` stripped.
...
export function checkCriticalFindings(diffContent) {
  ...
  const segments = splitDiffByFile(diffContent).map((segment) => ({
    ...segment,
    addedBody: addedLinesOnly(segment.body),
  }));
