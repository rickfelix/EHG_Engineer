...
// QF-20260904-983: the same real-verdicts predicate lib/governance/l1-work-outcome.js already
// uses, imported rather than re-invented, so a reconcile-sweep row's empty rungs=[] placeholder
// can never again silence classifyMerges() the way it silenced the unwitnessed-merge gauge.
import { hasRealVerdicts } from '../governance/l1-work-outcome.js';
 ...
 * QF-20260904-983: a row counts as witnessed only when it carries real per-rung verdicts
 * (hasRealVerdicts) -- row EXISTENCE alone previously qualified, which let a reconcile-sweep
 * row's empty rungs=[] placeholder silence the unwitnessed-merge gauge without anything ever
 * being verified.
 */
export function classifyMerges(merges, telemetryRows) {
  const validRows = (telemetryRows || []).filter((t) => !isInvalidated(t) && hasRealVerdicts(t));
