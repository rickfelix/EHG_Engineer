 ...
 * @param {{promptCount:number, modelCount:number, samplesPerCell:number, estimatedInputTokens?:number, estimatedOutputTokens?:number, env?:object}} params
...
export async function preflightBudgetCheck({
  ...
  const cellCount = promptCount * modelCount * samplesPerCell;
  const perCallCost = estimateCost(AUDIT_PROVIDER_KEY, estimatedInputTokens, estimatedOutputTokens);
  const estimatedCostUsd = perCallCost * cellCount;
...
export const _internal = { capFromEnv, today };
