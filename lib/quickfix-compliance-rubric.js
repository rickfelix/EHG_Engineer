    ...
    criteria: [
      {
        ...
        points: 15,
        check: async (context) => {
          // QF-20260604-479 / PAT-QF-EMPTY-DIFF-FALSE-COMPLETION-001 (defense-in-depth):
          // an empty diff (0 files / 0 LOC) changed nothing, so the original error cannot have
          // been resolved BY THIS QF — fail closed instead of passing vacuously on an empty
          // errorsAfterFix list. (Primary guard is the orchestrator's terminal empty-diff check.)
          const noFiles = !Array.isArray(context.filesChanged) || context.filesChanged.length === 0;
          if (noFiles && (Number(context.actualSourceLoc ?? context.actualLoc) || 0) === 0) {
            return { score: 0, passed: false, evidence: 'Empty diff — no code changed; original error cannot have been resolved by this QF' };
          }
