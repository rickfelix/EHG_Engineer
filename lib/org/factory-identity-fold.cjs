 ...
 *
 * REGRESSION sub-agent finding (PLAN_VERIFICATION): this file -> canonical-role-titles.mjs ->
 * venture-ceo-factory.js -> this file is an import CYCLE, inert only because the import()
 * below is dynamic and deferred to call time (after venture-ceo-factory has finished
 * evaluating). DO NOT convert it to a static import/require -- that would put
 * STANDARD_VENTURE_TEMPLATE in TDZ, the catch below would swallow the error, and every title
 * would silently degrade to the bare role_key -- the exact silent-clobber failure mode this
 * SD exists to stop.
 */
