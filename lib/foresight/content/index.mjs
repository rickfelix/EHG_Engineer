...
// Case-insensitive (sentence-initial "My …" is the common first-person opener),
// plural forms included, tolerant of dash/colon/single-quote/bracket prefixes.
const FIRST_PERSON_RE = /(^|[\s"“(:—–'‘[])(i|i'm|i've|my|mine|myself|we|our|ours)\b/i;
// Case-SENSITIVE proper-noun tail so generic prose ("whether you are building
// products…") never false-positives; covers the contraction and act-as/respond-as/
// answer-as/pretend-to-be prompt forms, tolerates 0–2 interposed title/adjective
// tokens ("the renowned", "Dr.") and ALL-CAPS name spellings.
const SPEAK_AS_RE = /\b(?:[Yy]ou(?:\s+are|'re)|[Aa]ct\s+as|[Rr]espond\s+as|[Ss]peak\s+as|[Ww]rite\s+as|[Aa]nswer\s+as|[Pp]retend\s+to\s+be)\s+(?:[\w.']+\s+){0,2}[A-ZÁÉÍÓÚÀ-ÿ][\wà-ÿ'-]*(?:\s+[A-ZÁÉÍÓÚÀ-ÿ][\wà-ÿ'-]+)+/;
// The provenance disclaimer legitimately says "not an endorsement by …" — strip
// negated forms (including bare-verb "does not endorse") before testing so the
// guard can cover endorsed/endorsement/endorsing/worked/partners without failing
// every authored profile.
const ENDORSEMENT_NEGATION_RE = /\b(?:does|do|did)\s+not\s+endorse\b|\bnot\s+an?\s+endorsement\b|\bnot\s+(?:endorsed|endorsing|affiliated|partnered)\b/gi;
const ENDORSEMENT_RE = /\b(?:endorse[sd]?|endorsements?|endorsing|approved by|backed by EHG|consultant to EHG|work(?:s|ed|ing)? (?:with|for) EHG|partner(?:s|ed|ship)? with EHG|affiliated with EHG)\b/i;
...
export function validateProfile(p) {
  ...
  } else if (p && !Array.isArray(p.sources) && Number(p.source_count) > 0) {
    problems.push(`profile ${p.perspective_id}: source_count ${p.source_count} but sources is missing/not an array`);
  }
...
export function validateBoard(councils, profiles) {
  ...
  // Same contract on the council axis: no duplicate council_ids, and every §5
  // roster council must be present — a board with a duplicated council standing
  // in for a missing one otherwise ships CI-green with orphaned profiles.
  const councilIds = new Set();
  for (const c of councils || []) {
    if (councilIds.has(c?.council_id)) problems.push(`duplicate council_id ${c.council_id}`);
    councilIds.add(c?.council_id);
  }
  for (const key of Object.keys(SPEC_ROSTER)) {
    if (!councilIds.has(key)) problems.push(`missing §5 council ${key}`);
  }
