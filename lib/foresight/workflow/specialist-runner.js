...
export function buildSpecialistPrompt(profile, council, ventureCandidate) {
  const system = [
    ...
    'Respond with ONLY a JSON object matching this shape: {"perspective_id": string, "venture_candidate_id": string, "executive_conclusion": string, "observations": [{"statement": string, "classification": "verified_observation"|"strong_inference"|"weak_signal"|"forecast"|"speculation", "confidence": number (0.0 to 1.0, not a percentage), "source_ids": string[]}], "opportunities": string[], "risks": string[], "assumptions": string[], "minority_or_counter_view": string, "recommended_action": "reject"|"monitor"|"research"|"prototype"|"validate"|"incubate"|"launch"|"scale", "recommended_experiment": string, "overall_confidence": number (0.0 to 1.0, not a percentage)}',
  ].join('\n');
 ...
 * @returns {Promise<{ parseError: boolean, id?: string, output?: Object, reason?: string }>}
 *   `output` is the full parsed section-11 response -- callers (run-venture-screen.js)
 *   MUST forward it to the adjudicator, not the static specialist profile, or the
 *   adjudicator receives a persona bio instead of actual findings (adversarial PR
 *   review finding, PR #6174).
 */
export async function runSpecialistAssessment({ profile, council, ventureCandidate, supabase, llmClient }) {
  ...
  return { parseError: false, id: data.assessment_id, output };
}
