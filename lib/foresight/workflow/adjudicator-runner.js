 ...
 * @param {Array} params.specialistAssessments - the 3 specialist assessment OUTPUTS (from
 *   runSpecialistAssessment's `output` field) for this council -- must be the actual parsed
 *   findings, not the static specialist profiles (adversarial PR review finding, PR #6174).
 ...
 *   `output` is the full parsed section-12 response -- callers MUST forward it to EVA
 *   synthesis, not a bare {adjudication_id, council_id} pair.
 */
export async function runAdjudication({ adjudicatorProfile, council, specialistAssessments, ventureCandidateId, supabase, llmClient }) {
  ...
  return { parseError: false, id: data.adjudication_id, output };
}
