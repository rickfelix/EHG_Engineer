...
/**
 * QF-20260828-884 (QF-20260828-188 leg 4, chairman-evidenced): the mechanical formatter for any
 * chairman-facing timestamp. Adam's own answer text read a UTC stamp as ET by hand (9:02pm for
 * 5:02pm) and the chairman corrected him live -- hand UTC->ET conversion is banned in favor of
 * this. Renders "h:mm AM/PM ET" (zone-label is always the literal "ET" for the chairman channel,
 * matching every existing chairman-facing surface's convention, regardless of the IANA `zone`
 * passed in -- a caller passing a non-ET zone gets that zone's wall-clock time under the "ET"
 * label deliberately, since this formatter's contract is "the chairman's own clock", not a
 * general-purpose zone abbreviation resolver).
 * @param {Date|number|string} instant
 * @param {string} [zone] IANA zone, default 'America/New_York'
 * @returns {string} e.g. "5:02 PM ET"
 */
export function formatChairmanTimestamp(instant, zone = TZ) {
  const d = instant instanceof Date ? instant : new Date(instant);
  if (Number.isNaN(d.getTime())) throw new Error(`formatChairmanTimestamp(): not a valid instant — got ${JSON.stringify(instant)}`);
  const p = etParts(d, zone);
  const period = p.hour >= 12 ? 'PM' : 'AM';
  const hour12 = p.hour % 12 === 0 ? 12 : p.hour % 12;
  return `${hour12}:${String(p.minute).padStart(2, '0')} ${period} ET`;
}

/**
 * QF-20260905-680: ratification 7010e20f fixed the routine-heartbeat cadence to SET-SCHEDULE
 * ET slots (6am/9am/12pm/3pm/6pm/9pm), superseding the "every ~3h since last send" gap rule
 * (ratification 9eebe200) that let the cadence silently drift ~1h/day when a catch-up tick's
 * OWN send moved the measured gap's anchor. Pure + DST-aware (reuses this module's own
 * etWallClockUtc double-pass arithmetic) so it is directly unit-testable across a DST edge.
 */
export const HEARTBEAT_ET_SLOT_HOURS = [6, 9, 12, 15, 18, 21];

/**
 * The next fixed ET heartbeat slot at or after `now` (inclusive of an exact boundary hit).
 * Wraps to tomorrow's first slot (6:00 ET) when `now` is past today's last slot (21:00 ET).
 * @param {Date|number} now
 * @param {string} [zone] IANA zone, default 'America/New_York'
 * @returns {string} ISO UTC instant of the next slot
 */
export function nextHeartbeatSlotIso(now, zone = TZ) {
  const d = now instanceof Date ? now : new Date(now);
  const p = etParts(d, zone);
  const upcoming = HEARTBEAT_ET_SLOT_HOURS.find((h) => h > p.hour || (h === p.hour && p.minute === 0 && p.second === 0));
  if (upcoming !== undefined) return etWallClockUtc(d, upcoming, 0, zone).toISOString();
  const tomorrow = new Date(d.getTime() + 24 * 60 * 60 * 1000);
  return etWallClockUtc(tomorrow, HEARTBEAT_ET_SLOT_HOURS[0], 0, zone).toISOString();
}

/**
 * The most recent fixed ET heartbeat slot at or before `now` — the slot a catch-up tick
 * checks "has this been sent yet?" against. Falls back to yesterday's last slot (21:00 ET)
 * when `now` is before today's first slot.
 * @param {Date|number} now
 * @param {string} [zone] IANA zone, default 'America/New_York'
 * @returns {string} ISO UTC instant of the current/most-recent slot
 */
export function currentHeartbeatSlotIso(now, zone = TZ) {
  const d = now instanceof Date ? now : new Date(now);
  const p = etParts(d, zone);
  const passed = HEARTBEAT_ET_SLOT_HOURS.filter((h) => h <= p.hour);
  if (passed.length > 0) return etWallClockUtc(d, passed[passed.length - 1], 0, zone).toISOString();
  const yesterday = new Date(d.getTime() - 24 * 60 * 60 * 1000);
  return etWallClockUtc(yesterday, HEARTBEAT_ET_SLOT_HOURS[HEARTBEAT_ET_SLOT_HOURS.length - 1], 0, zone).toISOString();
}
...
/**
 * QF-20260911-252: the live heartbeat sender's own explicit per-slot dedupe_key (when present)
 * is a STRONGER, unambiguous coverage signal than a created_at-based lookback window -- it is
 * the live path's own self-declared slot stamp, correct regardless of how early or late the send
 * actually landed (measured specimen: a live send 43 minutes before its slot, carrying dedupe_key
 * 'slot-2100-20260911'). Shared here (this module has zero deps) so both the sweep
 * (scripts/cron/chairman-hourly-heartbeat-backstop-sweep.mjs) and the drain-time re-verify
 * (lib/chairman/sms-outbound-worker.js) recognize the exact same set of formats without
 * duplicating -- and drifting -- the format list between them. Measured live, the key FORMAT has
 * already changed shape once (2026-09-07/08: 'adam-slot-YYYY-MM-DD-HHMMet'; 2026-09-11/12:
 * 'slot-HHMM-YYYYMMDD') and is sometimes absent entirely -- a row with no key or an unrecognized
 * format simply falls through to each caller's own time-window-based fallback, never a hard
 * failure.
 * @param {number} dueSlotHour
 * @param {string} etDateKey - "YYYY-MM-DD" in the chairman's zone
 * @returns {string[]} every known-format dedupe_key this slot's live send could carry
 */
export function buildLiveSlotDedupeKeyCandidates(dueSlotHour, etDateKey) {
  const hh = String(dueSlotHour).padStart(2, '0');
  const dateDigits = etDateKey.replace(/-/g, '');
  return [
    `slot-${hh}00-${dateDigits}`,        // 2026-09-11/12 convention
    `adam-slot-${etDateKey}-${hh}00et`,  // 2026-09-07/08 convention (superseded, still recognized)
  ];
}
