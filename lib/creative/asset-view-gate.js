...
const DEFAULT_VIEW_URL_TTL_SECONDS = 300;
...
function overrideKeyFor(ventureId) {
...
export async function checkAssetViewAuthorized({ supabase, ventureId }) {
  ...
  // earlier approval, not coexist with it as a permanent green light. Ties broken by created_at
  // DESC as a defensive secondary sort (attempt_number is DB-defaulted to 1 and uniquely
  // constrained per venture+stage+decision_type today, but a future writer inserting an
  // explicit null would otherwise sort first under a bare DESC).
