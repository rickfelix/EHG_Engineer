...
// storagePath returned a valid signed URL for the unrelated venture's asset). Also rejects any
// '..' path-traversal segment outright, defense-in-depth against a bucket-literal-eating
// normalization at the storage-client layer (SECURITY finding SEC-B2).
function assertStoragePathBelongsToVenture(storagePath, ventureId) {
  ...
  if (storagePath.split('/').includes('..')) {
    throw new TaskFailedError('asset-view-gate: refusing a storagePath containing a path-traversal segment', { code: 'STORAGE_PATH_TRAVERSAL' });
  }
