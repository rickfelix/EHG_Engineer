...
//   S25 — current_lifecycle_stage >= 25 (go-live, SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-3
//         repointed this from 24), via lib/governance/stage-gate-predicate.js.
...
// so it authorizes exactly one mintAssetViewUrl call, never standing access -- though the signed
// URL that single mint produces is itself a bearer token, replayable by anyone who obtains it
// until its (capped, short) TTL expires.
//
// mintAssetViewUrl additionally binds the requested storagePath to the authorized ventureId
// (asset-storage.js's own `${ventureId}/...` write convention) and rejects path-traversal
// segments outright -- checkAssetViewAuthorized alone only authorizes a SUBJECT (the venture);
// without this second check a caller could pass an authorized venture's id alongside a
// DIFFERENT venture's storagePath and receive a valid signed URL for someone else's asset.
...
// SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-3: repointed 24 -> 25 for the 2026-08-28 27-stage
// renumbering. Stage 24 is now launch_readiness (the KILL gate, formerly stage 23); go-live
// moved to stage 25 (is_irreversible=true). This constant was meant to mean go-live all along.
const REQUIRED_LIFECYCLE_STAGE = 25;
...
// asset-storage.js#persistAssetPrivately writes every object under `${ventureId}/...` (its own
// literal path template) -- this is the ONLY invariant tying a storage object to the venture that
// is authorized to view it. checkAssetViewAuthorized authorizes a SUBJECT (ventureId); without
// this check, mintAssetViewUrl would sign whatever RESOURCE (storagePath) the caller names,
// regardless of which venture actually owns it (SECURITY EXEC_TO_PLAN finding, row
// 9c36f751-5460-4b55-b546-afdba145c473: an authorized venture's id + an unrelated venture's
// storagePath returned a valid signed URL for the unrelated venture's asset).
//
// Every '/'-delimited segment is validated against a POSITIVE allowlist, not a '..' denylist
// (SECURITY re-verification finding, row f878598d-4774-4884-9d3b-dc0128f8ec0b: a bare denylist on
// the literal string '..' still admits percent-encoded ('%2e%2e'), backslash, or non-ASCII
// lookalike separators, since @supabase/storage-js does not encode/normalize the path before it
// reaches the wire). An allowlist has no such gap: anything outside [A-Za-z0-9._-] per segment is
// rejected outright, and a lone '.' or '..' segment is rejected even though both characters are
// individually allowed (a real filename may contain a literal '.', e.g. an extension).
const STORAGE_PATH_SEGMENT_RE = /^[A-Za-z0-9._-]+$/;

function assertStoragePathBelongsToVenture(storagePath, ventureId) {
  if (typeof storagePath !== 'string' || !storagePath) {
    throw new TaskFailedError('asset-view-gate: storagePath is required', { code: 'INVALID_STORAGE_PATH' });
  }
  const expectedPrefix = `${ventureId}/`;
  if (!storagePath.startsWith(expectedPrefix)) {
    throw new TaskFailedError('asset-view-gate: storagePath does not belong to the authorized venture', { code: 'STORAGE_PATH_VENTURE_MISMATCH' });
  }
  const segments = storagePath.split('/');
  const allSegmentsSafe = segments.every(
    (segment) => !/^\.+$/.test(segment) && STORAGE_PATH_SEGMENT_RE.test(segment)
  );
  if (!allSegmentsSafe) {
    throw new TaskFailedError('asset-view-gate: refusing a storagePath with an invalid or path-traversal-shaped segment', { code: 'STORAGE_PATH_TRAVERSAL' });
  }
}

/**
 * Mints a short-lived signed URL for an already-persisted creative-assets-private object, after
 * confirming the owning venture is authorized to have it viewed AND that the requested
 * storagePath actually belongs to that venture. Never persists the URL.
...
export async function mintAssetViewUrl(supabase, { ventureId, storagePath, ttlSeconds }) {
  ...
  assertStoragePathBelongsToVenture(storagePath, ventureId);
