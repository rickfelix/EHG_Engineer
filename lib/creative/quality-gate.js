...
export function screenForFabrication(generationResult) {
  ...
  // Fail CLOSED, not open, when there is nothing to screen — an absent prompt must never read
  // identical to "screened and clean". Reachable once generate-asset.js's deps.routes injection
  // seam is used with a provider that omits provenance.prompt (both in-repo providers always
  // set it today, but the seam does not guarantee that for a future/injected one).
  if (typeof generationResult?.provenance?.prompt !== 'string' || generationResult.provenance.prompt.trim() === '') {
    return { pass: false, reason: 'PROMPT_UNAVAILABLE' };
  }
