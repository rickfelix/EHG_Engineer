...
/**
 * SD-LEO-FEAT-MEDIA-PRODUCTION-CAPABILITY-001-A: ventureId is mandatory (format-checked here;
 * existence is verified by the caller, which already holds a DB client — see
 * creative-brief.js#requestCreativeAsset) so the future HARD FENCE (child B) always has a
 * venture to key its S23+S24 check on. deps is forwarded to the provider unmodified (this was
 * previously hardcoded to `{}`, which meant NO caller could ever reach a live provider call
 * regardless of testMode) and `deps.routes` may override the frozen ROUTES table for a single
 * call — the injection seam unit tests use instead of `vi.mock`-ing a frozen module import.
 * @param {string} ventureId
 * @param {'image'|'video'} capability
 * @param {{prompt: string}} spec
 * @param {object} [constraints]
 * @param {object} [deps] — forwarded to the provider; `deps.routes` overrides ROUTES for testing
 * @returns {Promise<{asset: object, provenance: object, cost: number|null}>}
 * @throws {TaskFailedError} missing ventureId, unknown capability, or every configured provider attempt failed
 * @throws {ProviderNotConfiguredError} no provider for this capability has a usable credential
 */
export async function generateAsset(ventureId, capability, spec, constraints = {}, deps = {}) {
  if (!ventureId) {
    throw new TaskFailedError('generateAsset requires a ventureId', { code: 'VENTURE_ID_REQUIRED' });
  }
  ...
  for (const provider of route) {
    ...
    try {
      return await provider.generate({ capability, spec, constraints }, deps);
    } catch (err) {
