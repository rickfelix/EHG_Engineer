...
function createMockSupabase({ venture, productReview, override = null } = {}) {
  ...
  // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-2: checkStageGate() rule (e) now also requires
  // launch_mode='live'. Defaulted here (not on every call site above) so the many existing
  // `{is_demo, current_lifecycle_stage}` fixtures keep describing an authorized venture
  // unless a test explicitly overrides launch_mode to exercise the new condition.
  const ventureRow = venture ? { launch_mode: 'live', ...venture } : null;
  return {
...
describe('checkAssetViewAuthorized', () => {
  ...
  it('TS-2: blocks when no S23 product_review row exists', async () => {
    const supabase = createMockSupabase({ productReview: null, venture: { is_demo: false, current_lifecycle_stage: 25 } });
  ...
  it('TS-3: blocks when the latest S23 attempt is a send_back (rejected), even if an earlier attempt was approved', async () => {
    ...
    const supabase = createMockSupabase({ productReview: { status: 'rejected' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const result = await checkAssetViewAuthorized({ supabase, ventureId: VENTURE_ID });
    expect(result).toEqual({ allowed: false, reason: 'product_review_not_approved' });
  });
  ...
  it("SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-2: S23 approved, stage 25 satisfied, but launch_mode!='live' is STILL blocked", async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 25, launch_mode: 'simulated' } });
    const result = await checkAssetViewAuthorized({ supabase, ventureId: VENTURE_ID });
    expect(result).toEqual({ allowed: false, reason: 'lifecycle_stage_gate_blocked' });
  });

  it('TS-6: S23 approved and go-live (S25) satisfied -> allowed', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const result = await checkAssetViewAuthorized({ supabase, ventureId: VENTURE_ID });
    expect(result).toEqual({ allowed: true, reason: null });
  });

  // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-3: REQUIRED_LIFECYCLE_STAGE repointed 24 -> 25
  // (stage 24 is now launch_readiness, the KILL gate; go-live is 25 under the 27-stage scheme).
  // Regression test at the exact new boundary.
  it('FR-3: S23 approved but venture only at stage 24 (launch_readiness, not yet go-live) is STILL blocked', async () => {
  ...
  it('TS-12: latest S23 attempt is pending (not yet decided by the chairman)', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'pending' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const result = await checkAssetViewAuthorized({ supabase, ventureId: VENTURE_ID });
    expect(result).toEqual({ allowed: false, reason: 'product_review_not_approved' });
  });
});
...
describe('mintAssetViewUrl', () => {
  ...
  it('SECURITY regression (row 9c36f751-5460-4b55-b546-afdba145c473): refuses to mint a URL for a storagePath belonging to a DIFFERENT venture than the authorized one, even though checkAssetViewAuthorized on its own only checks the subject (ventureId)', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const createSignedUrlSpy = vi.spyOn(supabase.storage.from(), 'createSignedUrl');
    await expect(
      mintAssetViewUrl(supabase, { ventureId: VENTURE_ID, storagePath: 'venture-VICTIM/image-secret' })
    ).rejects.toMatchObject({ code: 'STORAGE_PATH_VENTURE_MISMATCH' });
    expect(createSignedUrlSpy).not.toHaveBeenCalled();
  });

  it('SECURITY regression (SEC-B2, and re-verification row f878598d-4774-4884-9d3b-dc0128f8ec0b): refuses every path-traversal-shaped segment, not just the literal string ".." -- a positive allowlist, not a denylist, so percent-encoded/backslash/lookalike separators are rejected by construction rather than enumerated', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const createSignedUrlSpy = vi.spyOn(supabase.storage.from(), 'createSignedUrl');
    const hostileSuffixes = [
      '../chairman-docs/roadmap.pdf',
      '....//chairman-docs/roadmap.pdf',
      '%2e%2e/chairman-docs/roadmap.pdf',
      '..%2fchairman-docs/roadmap.pdf',
      '..\\chairman-docs\\roadmap.pdf',
      '．．/chairman-docs/roadmap.pdf', // fullwidth dot lookalike
    ];
    for (const suffix of hostileSuffixes) {
      await expect(
        mintAssetViewUrl(supabase, { ventureId: VENTURE_ID, storagePath: `${VENTURE_ID}/${suffix}` })
      ).rejects.toMatchObject({ code: 'STORAGE_PATH_TRAVERSAL' });
    }
    expect(createSignedUrlSpy).not.toHaveBeenCalled();
  });

  it('still permits a legitimate storagePath with a real filename extension (a lone "." char in a segment is fine; only an ALL-dots segment is rejected)', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 25 } });
    const result = await mintAssetViewUrl(supabase, { ventureId: VENTURE_ID, storagePath: `${VENTURE_ID}/image-1.png` });
