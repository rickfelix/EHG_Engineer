...
describe('mintAssetViewUrl', () => {
  ...
  it('SECURITY regression (SEC-B2): refuses a storagePath containing a path-traversal segment even when it starts with the authorized venture prefix', async () => {
    const supabase = createMockSupabase({ productReview: { status: 'approved' }, venture: { is_demo: false, current_lifecycle_stage: 24 } });
    const createSignedUrlSpy = vi.spyOn(supabase.storage.from(), 'createSignedUrl');
    await expect(
      mintAssetViewUrl(supabase, { ventureId: VENTURE_ID, storagePath: `${VENTURE_ID}/../chairman-docs/roadmap.pdf` })
