...
describe('screenForFabrication (AC-1)', () => {
  ...
  it('SECURITY correction: fails CLOSED (never open) when provenance.prompt is absent — reachable via the deps.routes injection seam', () => {
    const noPrompt = { provenance: { testMode: false, generator: 'fake' }, asset: { kind: 'generated' } };
    const result = screenForFabrication(noPrompt);
    expect(result.pass).toBe(false);
    expect(result.reason).toBe('PROMPT_UNAVAILABLE');
  });

  it('SECURITY correction: fails CLOSED when provenance.prompt is an empty/whitespace string', () => {
    const blankPrompt = { provenance: { testMode: false, prompt: '   ' }, asset: { kind: 'generated' } };
    expect(screenForFabrication(blankPrompt).reason).toBe('PROMPT_UNAVAILABLE');
  });
});
