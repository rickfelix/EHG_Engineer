...
describe('generateAsset() provider-abstraction primitive', () => {
  ...
  it('routes image requests to Gemini fallback when Runway is unconfigured (test-mode, no live call)', async () => {
    ...
    const result = await generateAsset('v1', 'image', { prompt: 'a hero image' });
  ...
  it('throws ProviderNotConfiguredError for video when Runway is unconfigured (no fallback for video)', async () => {
    ...
    await expect(generateAsset('v1', 'video', { prompt: 'a demo video' })).rejects.toThrow(ProviderNotConfiguredError);
  });
  ...
  it('throws ProviderNotConfiguredError when no provider for the capability is configured at all', async () => {
    ...
    await expect(generateAsset('v1', 'image', { prompt: 'x' })).rejects.toThrow(ProviderNotConfiguredError);
  });

  it('throws TaskFailedError (never a silent empty asset) on unknown capability', async () => {
    await expect(generateAsset('v1', 'audio', { prompt: 'x' })).rejects.toThrow(TaskFailedError);
  });

  it('throws TaskFailedError when ventureId is missing (SD-...-001-A FR-2) — before any provider is attempted', async () => {
    process.env.GEMINI_API_KEY = 'test-key';
    await expect(generateAsset(undefined, 'image', { prompt: 'x' })).rejects.toThrow(TaskFailedError);
    await expect(generateAsset('', 'image', { prompt: 'x' })).rejects.toMatchObject({ code: 'VENTURE_ID_REQUIRED' });
  });

  it('forwards deps to the provider — deps that were previously hardcoded to {} — and supports a deps.routes override for injection-based testing', async () => {
    const fakeProvider = {
      name: 'fake',
      isConfigured: () => true,
      generate: vi.fn().mockResolvedValue({ asset: { kind: 'generated' }, provenance: { generator: 'fake' }, cost: 1 }),
    };
    const result = await generateAsset('v1', 'image', { prompt: 'x' }, {}, { routes: { image: [fakeProvider] }, testMode: false, custom: 'marker' });
    expect(result.provenance.generator).toBe('fake');
    expect(fakeProvider.generate).toHaveBeenCalledWith(
      expect.objectContaining({ capability: 'image' }),
      expect.objectContaining({ testMode: false, custom: 'marker' })
    );
  });
