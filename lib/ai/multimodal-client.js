...
Respond in JSON format:
{
  ...
  async callGemini(imageData, prompt) {
    ...
    const primaryModel = this.config.model || 'gemini-2.5-flash';  // Cost control (2026-06-05)
    const fallbackModels = this.config.fallbackModels || ['gemini-2.5-flash'];
  ...
  getRecommendedModel(useCase = 'default') {
    const recommendations = {
      ...
      'default': 'gemini-2.5-flash'  // Cost control (2026-06-05): use 'high-accuracy' for Pro
    };
