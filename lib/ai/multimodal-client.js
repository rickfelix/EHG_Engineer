...
// SD-LEO-ORCH-GEMINI-MODEL-SCAN-001-F: Gemini model selection has exactly one SSOT
// (lib/config/model-config.js's getGoogleModel(purpose)). The literals this used to hardcode
// are byte-identical to the vision/reasoning purpose defaults today (verified this session),
// so this is env-overridable and centrally upgradable now, with no behavior change.
...
class MultimodalClient {
  constructor(config = {}) {
    this.config = {
      ...
      model: config.model || process.env.AI_MODEL || getGoogleModel('vision'),  // Cost control (2026-06-05): default to Flash; pass config.model/useCase 'high-accuracy' for Pro
    ...
    this.tokenPricing = {
      ...
      'gpt-5.4-mini': { input: 0.15, output: 0.60 },  // GPT-5.4 mini - cheap/fast
      'gpt-5.4-nano': { input: 0.05, output: 0.20 },  // GPT-5.4 nano - ultra-cheap
      ...
      'gemini-2.5-flash': { input: 0.30, output: 2.50 },  // Gemini 2.5 Flash (current 2026 pricing)
...
Respond in JSON format:
{
  ...
  async callGemini(imageData, prompt) {
    ...
    const primaryModel = this.config.model || getGoogleModel('vision');  // Cost control (2026-06-05)
    const fallbackModels = this.config.fallbackModels || [getGoogleModel('vision')];
  ...
  getRecommendedModel(useCase = 'default') {
    const recommendations = {
      'high-accuracy': getGoogleModel('reasoning'),  // Best accuracy, multimodal native
      'balanced': getGoogleModel('vision'),  // Good balance of cost and performance
      'low-cost': getGoogleModel('vision'),  // Cheapest option
      'complex-reasoning': 'claude-opus-4-8',  // Complex analysis
      'fast-screening': getGoogleModel('vision'),  // Quick checks
      'default': getGoogleModel('vision')  // Cost control (2026-06-05): use 'high-accuracy' for Pro
    };
