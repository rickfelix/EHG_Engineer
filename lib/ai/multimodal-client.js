...
class MultimodalClient {
  constructor(config = {}) {
    this.config = {
      ...
      model: config.model || process.env.AI_MODEL || 'gpt-5.1',
    ...
    this.tokenPricing = {
      'gpt-5.1': { input: 1.25, output: 10.00 },  // GPT-5.1 (same pricing as GPT-5)
      'gpt-5.1-chat-latest': { input: 1.25, output: 10.00 },  // GPT-5.1 (uses GPT-5 pricing)
      'gpt-5': { input: 1.25, output: 10.00 },  // GPT-5 standard
      'gpt-5-mini': { input: 0.50, output: 5.00 },  // GPT-5 mini tier
      'gpt-5-nano': { input: 0.15, output: 1.50 },  // GPT-5 nano tier
      ...
      'claude-sonnet-4-5': { input: 3.00, output: 15.00 },  // Claude Sonnet 4.5 (Sept 2025)
      'claude-haiku-4-5': { input: 1.00, output: 5.00 },  // Claude Haiku 4.5 (Oct 2025)
      'claude-opus-4-1': { input: 15.00, output: 75.00 },  // Claude Opus 4.1 (Aug 2025)
      'local': { input: 0, output: 0 }
    };

    // Embedding model pricing (per 1M tokens)
    this.embeddingPricing = {
      'text-embedding-3-small': { standard: 0.02, batch: 0.01 },  // 1536 dimensions
      'text-embedding-3-large': { standard: 0.13, batch: 0.065 }, // 3072 dimensions
      'text-embedding-ada-002': { standard: 0.10, batch: 0.05 }   // 1536 dimensions (legacy)
    };
...
Respond in JSON format:
{
  ...
  getAvailableModels() {
    const models = {
      openai: ['gpt-5.1', 'gpt-5.1-chat-latest', 'gpt-5-pro', 'gpt-5-mini', 'gpt-5-nano', 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini'],
  ...
  getRecommendedModel(useCase = 'default') {
    const recommendations = {
      'high-accuracy': 'gpt-5.1',  // Best accuracy, advanced reasoning
      'balanced': 'gpt-5-mini',  // Good balance of cost and performance
      'low-cost': 'gpt-5-nano',  // Cheapest option
      'complex-reasoning': 'claude-opus-4-1',  // Complex analysis, agentic tasks
