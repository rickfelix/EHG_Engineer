 ...
 * Exported (SD-LEO-FEAT-MARKETLENS-OWNED-AUDIENCE-001) so callers that need their own
 * review-gate table (not marketing_content/marketing_content_variants) can reuse the
 * prompt/LLM logic without going through generateContent()'s side effects.
 */
export function buildGenerationPrompt(ventureContext, contentType, platform) {
 ...
 * Exported (SD-LEO-FEAT-MARKETLENS-OWNED-AUDIENCE-001) — see buildGenerationPrompt.
 */
export async function callLLMForVariants(prompt) {
  try {
