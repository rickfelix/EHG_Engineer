...
export async function evaluateGraduation({ supabase, ventureId, channelType, requiredStreak = DEFAULT_GRADUATION_STREAK, mode }) {
  ...
  if (mode === 'mock') {
    return { success: true, autonomyState: null, cleanStreak, autonomyWriteSkipped: 'mock-mode evaluation never writes venture_channel_autonomy (no mode dimension on that table)' };
  }
