...
// S24 (Go Live) -- fixed constant, not per-channel.
...
export async function checkPublishAuthorization({ supabase, ventureId, channelType, contentId, correlationId, send = {}, logger }) {
  ...
  // SD-LEO-INFRA-STAGE-GATE-PREDICATE-001 (FR-3): action-time re-check at the confirmed
  // single architectural chokepoint for all outbound marketing channel publishes.
  // Additive, independent of the autonomy_state logic below -- does not replace it.
  // Shadow mode (armed=false, the default) still audit-logs the real verdict but never
  // blocks, per the build-now/arm-later program sequencing.
  ...
  if (shouldEnforceBlock(stageGate)) {
    return { allowed: false, reason: `STAGE_GATE_BLOCKED (fail-closed): ${stageGate.reason}` };
  }
