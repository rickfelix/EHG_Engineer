...
import { executionModeField, invalidateExecutionModeProbeCache, probeExecutionModeExists } from './ledger-execution-mode-probe.js';
...
// the probe's process-lifetime cache is safe for the analogous stage_write_token pattern
// (a soft-degrading UPDATE with no NOT NULL constraint), but execution_mode is NOT NULL with
// no default BY DESIGN -- a process that cached "column absent" before the chairman applies
// the FR-4 migration, and is still running after, would omit the now-required column on every
// subsequent INSERT and hard-fail with 23502, reproducing SEC-H1's exact symptom without a
// process restart. Self-heals: on that specific failure, invalidate the cache and retry once
// with a freshly-probed stamp before giving up.
function isExecutionModeNotNullViolation(error) {
  return error?.code === '23502' && /execution_mode/.test(error?.message || '');
}
...
async function insertLedgerRowSelfHealing(supabase, baseRow, mode) {
  ...
  if (result.error && Object.keys(stamp).length === 0 && isExecutionModeNotNullViolation(result.error)) {
...
export async function checkPublishAuthorization({ supabase, ventureId, channelType, contentId, correlationId, send = {}, logger }) {
  ...
  // 7c1c6622): action-time re-check at the confirmed single architectural chokepoint for
  // all outbound marketing channel publishes. Additive, independent of the autonomy_state
  // logic below -- does not replace it. UNCONDITIONALLY ENFORCED (armed:true inside
  // assertOutreachAuthorized) -- this gate does NOT wait on STAGE_GATE_PREDICATE_ARMED,
  // which keeps its own separate SD/QF-gating role. A refusal here never writes a ledger
  // row (the autonomy_state/ledger logic below never runs), and mode:'mock' is threaded
  // through the allow path below so the caller (publisher/index.js) knows never to invoke
  // a real adapter even on a nominal "allowed" result outside this gate.
  const outreach = await assertOutreachAuthorized({
    supabase,
    ventureId,
    actorType: 'channel_publish',
    actorId: `${channelType}:${contentId}`,
  });
  if (!outreach.authorized) {
    return { allowed: false, reason: `OUTREACH_NOT_AUTHORIZED (fail-closed): ${outreach.reason}`, mode: outreach.mode };
  }
  ...
  if (autonomyState === 'autonomous') {
    ...
    // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-4 (SECURITY finding SEC-H1): probed, not
    // unconditional -- execution_mode does not exist in the live schema until the chairman
    // applies the FR-4 migration; an unconditional stamp would PGRST204/42703 every insert.
    // Always 'live' when present, since this line only executes once outreach.authorized is
    // confirmed true. insertLedgerRowSelfHealing (SEC-H1-R) retries once, with a fresh probe,
    // if the omitted stamp turns out to violate a NOT NULL constraint that now exists.
    ...
    return { allowed: true, mode: outreach.mode, autonomyState, ledgerEntryId: inserted?.id, correlationId: autoCorrelationId };
  }
  ...
  if (accepted) {
    return { allowed: true, mode: outreach.mode, autonomyState, ledgerEntryId: accepted.id, correlationId: accepted.correlation_id };
  }
  ...
  if (!existingPending) {
    // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-4: same self-healing probed stamp as the
    // autonomous-tier insert above (SECURITY SEC-H1 / SEC-H1-R) -- this only executes once
    // outreach.authorized is confirmed true.
    const { data: inserted, error: proposeError } = await insertLedgerRowSelfHealing(supabase, {
      venture_id: ventureId,
      channel_type: channelType,
      content_ref: contentId,
...
export async function recordPublishOutcome({ supabase, correlationId, outcome, outcomeRef }) {
  // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 FR-6: widened to include 'unmeasurable'
  // (a row with no traceable artifact to ever determine a real outcome -- mirrors the
  // sibling solomon_advice_outcome_ledger precedent, database/migrations/
  // 20260828_solomon_ledger_outcome_unmeasurable.sql). The SQL CHECK constraint must be
  // widened in the SAME change (TR-8: migration before/atomic with this code) or every
  // 'unmeasurable' write here will reach the DB and fail with 23514 -- handled explicitly
  // below, never silently swallowed.
  if (!['shipped_clean', 'reverted', 'caused_rework', 'unmeasurable'].includes(outcome)) {
    throw new Error(`recordPublishOutcome: invalid outcome '${outcome}'`);
  }

  // NOTE: no execution_mode predicate on this UPDATE -- correlation_id carries a UNIQUE
  // constraint (20260710 migration), so a correlation_id-keyed update can only ever touch
  // the one row it identifies; there is no cross-mode row it could mistakenly hit. Select
  // execution_mode from the updated row so evaluateGraduation's candidate window can be
  // scoped to the SAME mode as the row just recorded (FR-5) -- mode isolation lives on the
  // READ side, not here.
  //
  // The column is PROBED, never a hardcoded literal: execution_mode is added by a
  // chairman-gated migration that may ship un-applied for an indeterminate period (see
  // lib/marketing/ledger-execution-mode-probe.js's own header). A literal
  // `.select('venture_id, channel_type, execution_mode')` would error on EVERY call to this
  // function -- the only production caller of recordPublishOutcome -- until the chairman
  // applies that migration, since PostgREST rejects a returning-clause naming an unrecognized
  // column. Reusing the SAME probe executionModeField() already uses for the INSERT path
  // (SECURITY finding SEC-H1) degrades this SELECT the same way: the column is simply
  // omitted, so `data.execution_mode` comes back undefined, which evaluateGraduation's own
  // mode guard already treats as the legacy/backward-compatible "mode omitted" case.
  const selectCols = (await probeExecutionModeExists(supabase))
    ? 'venture_id, channel_type, execution_mode'
    : 'venture_id, channel_type';
  const { data, error } = await supabase
    .from('venture_channel_publish_ledger')
  ...
  if (error) {
    // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 FR-4/TR-9: a 23514 (check_violation) on
    // an 'unmeasurable' write means the SQL migration hasn't landed yet -- EXPECTED,
    // pre-migration, and must be classified as such, never conflated with a genuine
    // failure or silently absorbed into a classification-only counter upstream.
    if (error.code === '23514' && outcome === 'unmeasurable') {
      return { success: false, error: error.message, reason: 'expected-pre-migration' };
    }
  ...
  return evaluateGraduation({ supabase, ventureId: data.venture_id, channelType: data.channel_type, mode: data.execution_mode });
}
 ...
 */
export async function evaluateGraduation({ supabase, ventureId, channelType, requiredStreak = DEFAULT_GRADUATION_STREAK, mode }) {
  // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-4: a mock-discriminated row must never contribute to
  // the clean streak (a mock publish earning real, unsupervised graduation authority is the exact
  // harm this SD exists to prevent). execution_mode IS applied in the live schema (confirmed by
  // direct query, SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 VALIDATION/TESTING re-verification) --
  // the 42703 fallback below is now dead code in production but is left in place (not removed;
  // out of scope for this SD) as a defensive fallback for any environment where the column is
  // genuinely absent.
  //
  // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 FR-5: when `mode` is supplied, the candidate window
  // is filtered to rows of that SAME execution_mode at the QUERY level -- a mock-mode call never
  // even sees a live-mode row and vice versa. This replaces the old behavior of merely breaking
  // the streak loop when a mismatched-mode row was encountered (fail-safe, but not true isolation).
  // `mode` is optional for backward compatibility with existing direct callers that don't track
  // it; the loop below still contains its own mode-mismatch break as defense-in-depth regardless.
  //
  // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 FR-6: the outcome exclusion now covers BOTH
  // 'unknown' AND 'unmeasurable' -- widening the outcome domain to add 'unmeasurable' must not
  // let those rows newly enter the streak window (where they would be treated as streak-breakers,
  // silently demoting channels the moment an unmeasurable row appears). An unmeasurable row must
  // be exactly as invisible to this safety valve as an unknown row already is.
  let recent;
  let recentError;
  let executionModeAvailable = true;
  {
    let query = supabase
      .from('venture_channel_publish_ledger')
      .select('decision, outcome, execution_mode, created_at') // schema-lint-disable-line
      .eq('venture_id', ventureId)
      .eq('channel_type', channelType)
      .not('outcome', 'in', '(unknown,unmeasurable)');
    if (mode) query = query.eq('execution_mode', mode);
    ({ data: recent, error: recentError } = await query
      .order('created_at', { ascending: false })
      .limit(requiredStreak));
  }

  if (recentError && (recentError.code === '42703' || /execution_mode/.test(recentError.message || ''))) {
    executionModeAvailable = false;
    ({ data: recent, error: recentError } = await supabase
      .from('venture_channel_publish_ledger')
  ...
  for (const row of recent || []) {
    if (executionModeAvailable && row.execution_mode === 'mock') break;
  ...
  // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 SECURITY finding SEC-2: venture_channel_autonomy
  // has NO execution_mode dimension -- it is the single, real, per-(venture,channel) autonomy
  // state. When mode='mock', the candidate window above is filtered to mock-only rows, so the
  // loop's own `row.execution_mode === 'mock'` break fires unconditionally on the very first
  // row, making streakEarned ALWAYS false for a mock-mode call. Writing the resulting
  // 'propose_and_approve' unconditionally would DEMOTE a channel's real (possibly live-
  // graduated) autonomy_state based solely on a mock outcome having been recorded -- exactly
  // the cross-mode harm this SD exists to prevent, and newly reachable because this SD is the
  // first production caller of recordPublishOutcome. A mock-mode evaluation must never touch
  // venture_channel_autonomy in either direction (neither graduate nor demote).
  //
  // TESTING finding (EXEC phase, HIGH): the original `mode === 'mock'` check alone was
  // fail-OPEN for every other value -- null (a real ledger row whose execution_mode was
  // never stamped), undefined-but-explicitly-passed, or any unexpected string all fell
  // through to the write below, reproducing the exact SEC-2 harm. Fail CLOSED instead: only
  // two values may proceed to the write --
  //   - mode === 'live' (the confirmed-live production path recordPublishOutcome takes), and
  //   - mode === undefined, i.e. the `mode` KEY WAS NEVER PASSED AT ALL -- preserved
  //     deliberately for backward compatibility with callers that predate mode-awareness
  //     (this file's own test suite calls evaluateGraduation directly without a mode
  //     argument in several places).
  // Any other value -- including an EXPLICITLY passed null/'mock'/'dry_run'/anything else --
  // is treated as "not confirmed live" and skips the write. A plain (non-default)
  // destructured parameter is `undefined` only when the caller omits the key entirely (or
  // passes `undefined` itself) -- an explicit `{mode: null}` yields `null`, not `undefined`
  // -- so this correctly distinguishes "caller never mentioned mode" from "caller told us
  // it's not live" without extra bookkeeping.
  if (mode !== undefined && mode !== 'live') {
    return { success: true, autonomyState: null, cleanStreak, autonomyWriteSkipped: `evaluation mode '${mode}' is not confirmed-live; venture_channel_autonomy is never written except for a confirmed-live evaluation or a legacy caller that omits mode entirely` };
  }
