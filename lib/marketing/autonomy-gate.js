...
import { evaluateCrackGateStatus, recordCrackGateObservation } from '../eva/lifecycle/crack-gate-evaluator.js';
...
async function checkCrackGateObserveOnly({ supabase, ventureId, logger }) {
  try {
    ...
    logger?.warn?.(`[autonomy-gate] crack-gate observe-only check failed (non-fatal, publish unaffected): ${err.message}`);
    return null;
  }
}
