...
import { executionModeField } from './ledger-execution-mode-probe.js';
...
export async function checkPublishAuthorization({ supabase, ventureId, channelType, contentId, correlationId, send = {}, logger }) {
  ...
  if (autonomyState === 'autonomous') {
    ...
    // confirmed true.
    const executionModeStamp = await executionModeField(supabase, outreach.mode);
  ...
  if (!existingPending) {
    // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-4: same probed stamp as the autonomous-tier
    // insert above (SECURITY SEC-H1) -- this only executes once outreach.authorized is
    // confirmed true.
    const executionModeStamp = await executionModeField(supabase, outreach.mode);
