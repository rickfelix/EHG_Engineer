...
export async function evaluateGraduation({ supabase, ventureId, channelType, requiredStreak = DEFAULT_GRADUATION_STREAK }) {
  ...
  // harm this SD exists to prevent). execution_mode does not exist in the live schema yet (its
  // migration is chairman-gated and staged, not applied) -- select it defensively and fall back to
  // the pre-existing query on an undefined_column error, so this function is not broken by its own
  // forward-looking column until that migration lands.
  let recent;
  let recentError;
