...
export function createEmailCampaigns(deps) {
  ...
  return {
     ...
     *
     * SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-5: gated independently of processStep() --
     * a caller that invokes sendEmail() directly (bypassing processStep()'s consent +
     * stage-gate checks) must not get a free pass. ventureId is REQUIRED (throws if
     * omitted, a caller-contract violation, not a runtime authorization decision) so an
     * optional/omittable param can never fail OPEN into an ungated send.
     *
     * @param {object} params
     * @param {string} params.ventureId - Venture this send is on behalf of (REQUIRED)
     ...
     * @returns {Promise<{success: boolean, messageId?: string, error?: string, attempts: number, refused?: boolean, refusalReason?: string}>}
     */
    async sendEmail(params) {
      const { ventureId, to, subject, html, from = 'noreply@ehg.ai', tags } = params;
      if (!ventureId) throw new Error('sendEmail requires ventureId');

      const actorId = tags?.campaign ? `campaign:${tags.campaign}` : `direct:${to}`;
      const outreach = await assertOutreachAuthorized({ supabase, ventureId, actorType: 'campaign', actorId });
      if (!outreach.authorized) {
        return {
          success: false,
          refused: true,
          refusalReason: outreach.reason,
          error: `SEND_REFUSAL (fail-closed): ${outreach.reason}`,
          attempts: 0,
        };
      }
    ...
    async processStep(enrollment, steps) {
      ...
      // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 (Solomon's amended design, coordinator directive
      // 7c1c6622): the action-time stage-gate re-check happens inside sendEmail() below, NOT
      // as a second call here. SECURITY finding SEC-M3 (sub_agent_execution_results
      // 3ed447ec-de8c-4798-9fdc-5a0c814623a5): an earlier draft called assertOutreachAuthorized()
      // at BOTH this call site and inside sendEmail() for the same logical send, with two bugs
      // as a result -- (1) KEY MISMATCH: this call site used bare enrollment.campaign_id as
      // actorId, while sendEmail() derives `campaign:${tags.campaign}` from the tags it sets
      // just below, so a chairman override recorded against one actorId would never match a
      // lookup against the other; (2) DOUBLE-CONSUME: assertOutreachAuthorized() forces
      // armed:true unconditionally, so a chairman override's one-shot claim would be consumed
      // by the FIRST gate and then find itself already spent at the SECOND, authorizing
      // nothing while still burning the override and inflating the override-rate metric.
      // sendEmail() alone is now the sole outreach-authorization chokepoint for this send --
      // it already independently refuses a direct caller that bypasses processStep() entirely,
      // which was this check's actual purpose; a second call added no additional safety, only
      // the two bugs above.
      ...
      if (!sendResult.success) {
        // SECURITY finding SEC-M3: a stage-gate refusal (sendResult.refused) is not a transport
        // failure -- conflating them under action:'failed' hid the real reason and made a
        // refused/suppressed send indistinguishable from a genuine Resend error.
        if (sendResult.refused) {
          return { action: 'suppressed', reason: 'stage_gate', detail: sendResult.refusalReason };
        }
