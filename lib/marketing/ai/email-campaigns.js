...
export function createEmailCampaigns(deps) {
  ...
  return {
    ...
    async processStep(enrollment, steps) {
      ...
      // 7c1c6622): action-time re-check, additive alongside the consent check above --
      // independent gate, does not replace it. Unconditionally enforced (assertOutreachAuthorized
      // forces armed:true internally) -- this is NOT the old shadow-mode checkStageGate/
      // shouldEnforceBlock pairing, which treated is_demo/no-venture as OUT_OF_SCOPE (correct for
      // SD-gating, wrong for outreach). Also redundant with sendEmail()'s OWN gate below (belt-
      // and-suspenders): a caller that bypasses this method and calls sendEmail() directly still
      // gets refused there.
      const outreach = await assertOutreachAuthorized({
        supabase,
        ventureId: enrollment.venture_id,
        actorType: 'campaign',
      ...
      if (!outreach.authorized) {
        return { action: 'suppressed', reason: 'stage_gate', detail: outreach.reason };
      }
