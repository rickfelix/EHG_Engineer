...
export class XAdapter {
  ...
  async getTweet(tweetId) {
    ...
    // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 TESTING finding (EXEC phase, CRITICAL):
    // HTTP-ok alone was asserted as exists:true without inspecting the payload. X API v2
    // GET /2/tweets/:id returns HTTP 200 WITH an errors[] array and NO data key for a
    // deleted/unavailable tweet -- the prior code wrote a deleted post to the ledger as
    // shipped_clean, a wrong TERMINAL outcome that could earn autonomy graduation.
    // Positive evidence (data.id) is required for exists:true; a 200-with-errors[] body
    // is the documented not-found shape (exists:false); anything else (unparseable, or
    // neither data nor errors) is never guessed -- stays transient.
