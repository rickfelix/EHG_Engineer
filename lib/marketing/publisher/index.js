...
export async function publish({ supabase, content, platform, options = {}, ventureId, campaignId, send = {} }) {
  ...
  if (existing && existing.length > 0) {
    // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-3: mode:'real' here is safe -- campaign_content
    // is only ever upserted on the real adapter-dispatch success path below (never on a
    // dry-run or an auth deny), so a dedup hit against it can only reflect a prior REAL send.
    return { success: true, postId: existing[0].external_post_id, mode: 'real', deduplicated: true };
  }
  ...
  if (!authCheck.allowed) {
    return { success: false, error: authCheck.reason, mode: authCheck.mode, blockedBy: 'autonomy-gate' };
  }
  ...
  if (!resolvedCredentials) {
    return {
      ...
      postUrl: null,
      dryRun: true,
      // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-3: mode is authCheck.mode, NOT a hardcoded
      // 'mock' — an authorized (mode:'live') autonomous-tier send that already wrote an
      // 'accepted' ledger row (autonomy-gate.js) but then hits a missing-credentials
      // dry-run here must still be flagged so a caller can reconcile that ledger row via
      // recordPublishOutcome(), never silently treat it as a completed real send.
      mode: authCheck.mode,
