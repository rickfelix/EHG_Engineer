...
export async function publish({ supabase, content, platform, options = {}, ventureId, campaignId, send = {} }) {
  ...
  // MUST run before adapter construction, and before the dedup check below (SECURITY
  // finding SEC-M2, sub_agent_execution_results 3ed447ec: a dedup HIT must never be
  // treated as implying authorization -- campaign_content is also written, ungated, by
  // lib/marketing/content-generator.js at content-creation time, not only by the real-
  // dispatch success path further below, so a dedup hit alone proves nothing about
  // whether THIS attempt is authorized). This is the enforcement chokepoint every caller
  // (content-pipeline.js, owned-audience-content-loop.js) passes through. No log-only
  // branch: a missing/errored read here denies the publish attempt.
  ...
  if (!authCheck.allowed) {
    // SECURITY finding SEC-M1 (sub_agent_execution_results 3ed447ec): `authMode` (not
    // `mode`) carries assertOutreachAuthorized()'s 'live'|'mock' AUTHORIZATION vocabulary.
    // `mode` itself is reserved exclusively for the DISPATCH vocabulary further below
    // ('real' = adapter.publish() actually ran) — the two must never share one field, or a
    // future check that treats them as synonyms (e.g. `mode !== 'mock'`) could silently
    // credit a send that never happened.
    return { success: false, error: authCheck.reason, authMode: authCheck.mode, blockedBy: 'autonomy-gate' };
  }

  // SD-LEO-INFRA-PUBLISH-OUTCOME-OBSERVER-001 FR-1: use the ledger's own durable
  // correlationId (authCheck.correlationId), NOT the function-local idempotencyKey
  // rebuilt fresh on every call. On an approval-gated attempt #2+ (the fail-closed
  // default), checkPublishAuthorization's "already accepted" branch returns the
  // ORIGINAL propose-time correlation_id (autonomy-gate.js's accepted-row lookup),
  // which differs from this call's freshly-built idempotencyKey. Keying
  // campaign_content on the local key instead of authCheck.correlationId made the
  // ledger-to-campaign_content join dead by construction on that path (found by
  // TESTING sub-agent review, evidence 26462519). authCheck.allowed is guaranteed
  // true here, and both allowed:true return sites in checkPublishAuthorization
  // (autonomous and already-accepted) always populate correlationId.
  const dispatchKey = authCheck.correlationId || idempotencyKey;

  // Check for duplicate dispatch
  const { data: existing } = await supabase
    .from('campaign_content')
    .select('id, external_post_id')
    .eq('idempotency_key', dispatchKey)
    .limit(1);

  if (existing && existing.length > 0) {
    // SEC-M2 fix: no `mode:'real'` claim here — campaign_content is written by more than
    // one path (see comment above), so a dedup hit cannot prove the original write was a
    // genuine dispatch. The authorization check above has already run for THIS attempt
    // regardless of this early return.
  ...
  if (!resolvedCredentials) {
    return {
      ...
      // adapter.publish() actually ran) is deliberately omitted here — a value meaning
      // "authorized for real outreach" must never label a send that did not occur.
      // `authMode` carries authCheck.mode (assertOutreachAuthorized()'s 'live'|'mock')
      // instead: an authorized (authMode:'live') autonomous-tier send that already wrote
      // an 'accepted' ledger row (autonomy-gate.js) but then hits missing credentials here
      // must still be flagged so a caller can reconcile that ledger row via
      // recordPublishOutcome(), never silently treated as a completed real send.
      authMode: authCheck.mode,
  ...
  try {
    ...
    //
    // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-3: mode:'real' here is the ONLY path that
    // actually reached adapter.publish() -- callers (content-pipeline.js,
    // owned-audience-content-loop.js) must check .mode, never bare .success, before
    // counting a publish as real (a dry-run below and a stage-gate deny above both
    // return success:true/false without ever reaching this line).
    return { ...result, mode: 'real', ledgerCorrelationId: authCheck.correlationId };
  } catch (error) {
