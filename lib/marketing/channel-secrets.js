...
/**
 * Adapter to organic-channel-provisioning.js's expected DI shape:
 * deps.storeSecret: (ventureId, channelType, credentials) => Promise<string>. storeSecret
 * above takes a single options object (matching this module's other exports) — this
 * binds a supabase client and returns a positionally-called wrapper so
 * provisionOrganicChannel() can be handed a real implementation instead of leaving the
 * DI slot unfilled (SD-LEO-INFRA-VENTURE-DEMAND-DISTRIBUTION-001-C FR-2).
 */
export function createStoreSecretDep({ supabase, provider } = {}) {
  return (ventureId, channelType, credentials) =>
    storeSecret({ supabase, ventureId, channelType, provider, credentials });
}

export default { storeSecret, resolveSecret, resolveChannelCredentials, createStoreSecretDep };
