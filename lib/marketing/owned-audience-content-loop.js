...
export async function publishApprovedItem({ queueItemId, ventureId, platform }, deps = {}) {
  ...
  // SD-LEO-INFRA-DEMAND-ENGINE-FAIL-001 FR-3: result.success alone is true for a
  // dry-run/mock result too (publisher/index.js's contract) -- only mode:'real' means
  // adapter.publish() actually ran. A forced-mock or below-go-live-denied publish must
  // never be persisted as a completed real publish.
  if (!result?.success || result.mode !== 'real') {
    return { ok: false, reason: result?.error || (result?.success ? `not_real_mode:${result?.mode}` : 'publish_failed') };
  }
