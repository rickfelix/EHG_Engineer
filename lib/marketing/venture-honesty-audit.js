...
export async function buildHonestyAudit({ supabase, ventureId }) {
  ...
  // this is the instrument whose entire job is reporting whether a venture really contacted
  // real humans, so it must not fold a mock send into the same total as a live one.
  // execution_mode does not exist in the live schema yet (its migration is chairman-gated and
  // staged, not applied) -- select it defensively and fall back to the pre-existing query on an
  // undefined_column error, mirroring evaluateGraduation()'s fallback in autonomy-gate.js.
  let ledgerRows;
  let ledgerError;
  let executionModeAvailable = true;
  ({ data: ledgerRows, error: ledgerError } = await supabase
    .from('venture_channel_publish_ledger')
    .select('channel_type, decision, outcome, execution_mode, created_at') // schema-lint-disable-line
    .eq('venture_id', ventureId)
    .order('created_at', { ascending: false })
    .limit(500));

  if (ledgerError && (ledgerError.code === '42703' || /execution_mode/.test(ledgerError.message || ''))) {
  ...
  if (ledgerError) {
  ...
  } else if (!executionModeAvailable) {
    audit.sends = {
      readable: true,
      total: (ledgerRows || []).length,
      execution_mode_known: false,
    };
    audit.gaps.push(
      'sends total cannot be split into live vs. mock: execution_mode does not exist in the live schema yet (its migration is chairman-gated and unapplied)'
    );
  } else {
    ...
    audit.sends = {
      readable: true,
      total: rows.length,
      execution_mode_known: true,
      live: rows.filter((r) => r.execution_mode !== 'mock').length,
      mock: rows.filter((r) => r.execution_mode === 'mock').length,
    };
  }
...
export function renderHonestyAudit(audit) {
  ...
  lines.push('SENDS:');
  if (!s.readable) {
    lines.push(`  UNREADABLE — ${s.detail}`);
  } else if (s.execution_mode_known) {
    lines.push(`  publish-ledger entries: ${s.total} (live: ${s.live}, mock: ${s.mock})`);
  } else {
    lines.push(`  publish-ledger entries: ${s.total} (live/mock split unknown — execution_mode not yet in schema)`);
  }
  lines.push('');
...
/**
 * OPERATOR CONTRACT — the armed-cadence leg.
 *
 * The gate named a real gap and it was right: this audit shipped as a bare CLI, and by this SD's
 * own argument an instrument nobody invokes on a cadence is indistinguishable from an absent one.
 * A CLI is a thing a human might run; an ARMED registration is a thing the fleet will notice the
 * absence of.
 *
 * ARMED rather than a live cron, deliberately: the audit reads venture_demand_verdicts and
 * venture_consent_events, and those migrations are merged but NOT YET APPLIED (chairman-approval
 * gated). Registering a firing cron today would schedule a job whose every run reports
 * VERDICT_UNREADABLE. The ARMED shape carries a NAMED activation trigger instead, so the
 * periodic-liveness watcher owns the armed-but-never-fired case and the gap stays visible rather
 * than being either hidden or noisy.
 *
 * Fail-soft: registration is bookkeeping and must never block an audit run — mirrors
 * registerProvisioningOwner in lib/venture-email/provision-venture-email.js.
 */
export async function registerHonestyAuditOwner(supabase) {
  try {
    const { registerArmedMachinery } = await import('../machinery-class/armed-registration.js');
    return await registerArmedMachinery(supabase, { sd_key: 'SD-LEO-FEAT-VENTURE-DEMAND-VALIDATION-001' }, {
      owner: 'venture-demand-validation',
      activationTrigger: 'the venture_demand_verdicts and venture_consent_events migrations are APPLIED (chairman @approved-by header + coordinator apply), after which the per-venture honesty audit runs on the chairman review cadence',
...
export default { AUDIT_STATUS, STATUS_MEANING, buildHonestyAudit, renderHonestyAudit, registerHonestyAuditOwner };
