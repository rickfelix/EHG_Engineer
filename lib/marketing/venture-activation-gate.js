...
// SD-LEO-GEN-NEED-ABLE-CONTINUALLY-001 FR-2: computeCpaGaugeState is a SEPARATE, non-gating
// signal. Its result is added to the persisted `rungs` object AFTER decideActivationVerdict()
// and buildPathToPass() have already run on the original 4-rung array (see computeActivationVerdict
// below) — it is NEVER passed into either function. Adding it to that array (even with no ratified
// floor) would still change buildPathToPass()'s "RATIFIED FLOOR MISSING for: ..." text, breaking
// byte-identical verdict/citation/path_to_pass output for every venture. CPA stays informational
// until/unless a future SD obtains chairman floor ratification for it — do NOT add 'cpa' to
// ACTIVATION_RUNGS or RATIFIED_FLOORS without that ratification.
import { computeCpaGaugeState } from '../telemetry/cpa-gauge.mjs';
// SD-LEO-GEN-NEED-ABLE-CONTINUALLY-001 count-truncation-diff-lint fix: a plain .select() on
// daily_rollups is silently capped at PostgREST's 1000-row max (see
// lib/db/fetch-all-paginated.mjs's own docstring for the live 2026-07-19 incident this module
// exists to prevent) -- a venture with many platforms/days of data could silently undercount
// spend/conversions without this.
...
/** Default lookback window for the CPA gauge (TR-2). Explicit, overridable — never hardcoded
 *  inside computeCpaGaugeState() itself (TR-1 keeps that function pure/I-O-free). */
export const DEFAULT_CPA_LOOKBACK_DAYS = 30;

/**
 * Resolve the CPA rung — SD-LEO-GEN-NEED-ABLE-CONTINUALLY-001 FR-2.
 *
 * NOT part of the ACTIVATION_RUNGS ladder. Queries daily_rollups (summed across ALL platforms
 * for this venture, per TR-2 — computeActivationVerdict has no platform parameter) and delegates
 * the honest-measurement computation entirely to computeCpaGaugeState, exactly as
 * resolveTelemetryRungs/resolvePaidRung delegate to funnel-gauge.mjs. Fails closed on a query
 * error (TR-4): "I could not look" is reported as no_writer_yet with the error message, never
 * thrown, so one additional rung can never crash the whole verdict computation.
 */
export async function resolveCpaRung({ supabase, ventureId, lookbackDays = DEFAULT_CPA_LOOKBACK_DAYS, now = new Date() }) {
  ...
  try {
    ...
    // TR-4: fail closed, never let a paginated-read error crash the whole verdict computation.
    return { rung: 'cpa', state: 'no_writer_yet', value_cents_per_conversion: null, reason: `daily_rollups query failed: ${err.message}` };
  }

  return { rung: 'cpa', ...computeCpaGaugeState({ dailyRollupRows: rows }) };
}
...
export function buildPathToPass(rungs, floors = RATIFIED_FLOORS) {
  ...
  for (const r of rungs) {
    ...
    } else if (!Number.isFinite(floors?.[r.rung]?.minimum)) {
      steps.push(`${r.rung}: measured at ${r.value} but there is NO RATIFIED FLOOR to judge it against`);
    }
  }

  // THE STEP THAT WAS MISSING, found by running FR-6 for real: when every rung is UNMEASURABLE the
  // loop above never reaches the floor branch, so the path named the absent telemetry writer and
  // said nothing about the absent floor. That is an INCOMPLETE PATH — someone would instrument
  // telemetry, re-run, and only then discover the gate is still inert because no floor is ratified.
  // A path to pass has to name EVERY blocker, not just the first one you would hit.
  const judgeableRungs = rungs.filter((r) => !r.declared_unfiltered);
  const unratified = judgeableRungs.filter((r) => !Number.isFinite(floors?.[r.rung]?.minimum)).map((r) => r.rung);
  if (unratified.length > 0) {
    steps.push(
      `RATIFIED FLOOR MISSING for: ${unratified.join(', ')} — even once these rungs are measured, no venture can reach PASS until a chairman/Adam decision names a floor. The circulating 300-visitor figure is codified nowhere and contradicts this venture's own stored 100/5%/10 metadata, so it is not usable as one`
    );
  }
...
export async function computeActivationVerdict({ supabase, ventureId, floors = RATIFIED_FLOORS, now = new Date() }) {
  ...
  if (telErr) {
    ...
    // SD-LEO-GEN-NEED-ABLE-CONTINUALLY-001 FR-2: cpa is resolved and attached AFTER
    // buildPathToPass(blind, floors) has already run on the original (non-cpa) array — it is
    // never passed in, so the blind-path text this venture would have seen without this SD is
    // byte-identical to the text it sees with it.
    const blindRungsObject = Object.fromEntries(blind.map((r) => [r.rung, r]));
    blindRungsObject.cpa = await resolveCpaRung({ supabase, ventureId, now });
    return {
      venture_id: ventureId,
      verdict: ACTIVATION_VERDICT.NO_DATA,
      rungs: blindRungsObject,
  ...
  const pathToPass = buildPathToPass(rungs, floors);

  // SD-LEO-GEN-NEED-ABLE-CONTINUALLY-001 FR-2: cpa is computed and attached to the persisted
  // rungs object ONLY AFTER verdict/why/pathToPass have already been derived from the original
  // 4-rung `rungs` array above. It is deliberately never passed into decideActivationVerdict or
  // buildPathToPass — see the resolveCpaRung docstring and the import comment at the top of this
  // file for why (adding it to that array, even unfloored, would change buildPathToPass's
  // "RATIFIED FLOOR MISSING for: ..." text for every venture).
  const rungsObject = Object.fromEntries(rungs.map((r) => [r.rung, r]));
  rungsObject.cpa = await resolveCpaRung({ supabase, ventureId, now });
