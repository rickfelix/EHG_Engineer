 ...
 *   finding SEC-3).
 * @returns {Promise<{outcome: 'shipped_clean'|'reverted'|'unknown'|'unmeasurable', outcomeRef: string|null, reason: string}>}
 */
export async function observeOutcome({ supabase, correlationId, adapters = ADAPTERS, resolveCredentials = resolveChannelCredentials }) {
  const { data: ledgerRow, error: ledgerError } = await supabase
    .from('venture_channel_publish_ledger')
    .select('id, venture_id, channel_type, correlation_id')
    .eq('correlation_id', correlationId)
    .maybeSingle();

  if (ledgerError) {
    return { outcome: 'unknown', outcomeRef: null, reason: `ledger read failed: ${ledgerError.message}` };
  }
  if (!ledgerRow) {
    return { outcome: 'unmeasurable', outcomeRef: null, reason: 'no ledger row for this correlationId' };
  }
  ...
  if (contentError) {
    return { outcome: 'unknown', outcomeRef: null, reason: `campaign_content read failed: ${contentError.message}` };
  }
  ...
  if (!externalPostId || DRY_RUN_SENTINEL_RE.test(externalPostId)) {
    return { outcome: 'unmeasurable', outcomeRef: null, reason: 'no joinable real post id (missing campaign_content row, null external_post_id, or a dry-run sentinel)' };
  }

  const platform = contentRow.platform || ledgerRow.channel_type;
  const AdapterClass = adapters[platform];
  if (!AdapterClass) {
    return { outcome: 'unmeasurable', outcomeRef: externalPostId, reason: `no adapter for platform '${platform}' (out of scope for this SD — FR-9)` };
  }
  ...
  if (!credentials) {
    return { outcome: 'unknown', outcomeRef: externalPostId, reason: `no per-venture credential resolvable for platform '${platform}' — never falls back to a shared identity` };
  }
  ...
  if (lookup.transient) {
    return { outcome: 'unknown', outcomeRef: externalPostId, reason: 'transient lookup failure — retry next scheduled run' };
  }
  if (lookup.exists === false) {
    return { outcome: 'reverted', outcomeRef: externalPostId, reason: 'post confirmed absent (deleted or never existed)' };
  }
  return { outcome: 'shipped_clean', outcomeRef: externalPostId, reason: 'post confirmed present' };
}
