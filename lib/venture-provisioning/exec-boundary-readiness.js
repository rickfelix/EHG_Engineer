 ...
 * sk_live key is refused unconditionally from a fleet/CI context (see
 * lib/payments/stripe-client.js). This function never creates, rotates, or reads a key
 * value itself; env.STRIPE_SECRET_KEY must already be a chairman/Adam-provisioned sk_test
 ...
 * instead of an uncaught guard exception. (Prefix spelled without its trailing separator
 * throughout this file: CRIT-001's source-substring scanner has no test/comment exemption
 * by design -- see the coordinator-ratified fix in this SD's retrospective.)
 *
 * IDEMPOTENT (testing-agent finding F1, fixed same-session): this module has no persisted
 * ventures.stripe_account_id column to look up before creating, so account.create() passes
 * a deterministic Stripe Idempotency-Key instead -- Stripe's API guarantees a retry with the
 * SAME key returns the SAME account object rather than creating a duplicate, without a
 * schema change or an extra list+filter round-trip. Never throws (matches the rest of this
 * module's contract, and testing-agent's F2): a Stripe API error is caught and reported as a
 * clean {ok:false} result, not an uncaught exception that would abort FR-1/2/4 too.
...
export async function provisionPaymentAccountSetup({ ventureId, ventureName }, deps = {}) {
  ...
  try {
    account = await stripe.accounts.create(
      ...
      { idempotencyKey: `venture-payment-account-setup-${ventureId}` }
    );
  } catch (err) {
    return { ok: false, reason: `stripe_error: ${err.message}` };
  }
 ...
 *
 * Carries accountId/chargesEnabled/detailsSubmitted through on success (security-agent
 * finding SEC-003, fixed same-session): the prior version dropped them, leaving a created
 * Stripe resource with no audit trail in the persisted report -- neither the artifact, the
 * ventures row, nor stdout recorded which account was created.
 * @param {{ok: boolean, accountId?: string, chargesEnabled?: boolean, detailsSubmitted?: boolean, reason?: string}} provisionResult
 * @returns {{ paymentAccountProvisioned: boolean, accountId: string|null, chargesEnabled: boolean|null, detailsSubmitted: boolean|null, decisionPoint: object|null }}
 */
export function assessPaymentAccountReadiness(provisionResult) {
  if (provisionResult.ok) {
    return {
      paymentAccountProvisioned: true,
      accountId: provisionResult.accountId,
      chargesEnabled: provisionResult.chargesEnabled,
      detailsSubmitted: provisionResult.detailsSubmitted,
      decisionPoint: null,
    };
  }
  return {
    ...
    decisionPoint: {
      ...
      blockedOn: [
        'no sk_test Stripe key is configured in the fleet environment',
      ...
      note: `provisionPaymentAccountSetup() reason: ${provisionResult.reason || 'unknown'}. Code path is built + unit-tested (Connect Express account.create via the sanctioned getStripeForVenture() guard) but cannot run live without a chairman/Adam-provisioned sk_test key -- this module never creates or rotates keys itself.`,
    },
  };
}
...
export async function buildProvisioningReadinessReport({ supabase, ventureId, deploymentUrl, ventureName }, deps = {}) {
  ...
  // dryRun skips BOTH side-effecting provisioning calls entirely (security-agent finding
  // SEC-002, fixed same-session): provisionOrganicChannel() inserts venture_distribution_channels
  // + seeds channel_budgets, and provisionPaymentAccountSetup() creates a real Stripe account
  // when a test key is configured -- neither is something a "preview only" flag should allow
  // to run. Previously only the FINAL persist step was gated, so --dry-run against a venture
  // with no existing distribution/payment state would silently create real rows/resources.
  let distribution;
  let payment;
  if (deps.dryRun) {
    distribution = assessDistributionReadiness(channelConfigArtifact, { ok: false, reason: 'dry_run_not_attempted' });
    payment = assessPaymentAccountReadiness({ ok: false, reason: 'dry_run_not_attempted' });
  } else {
    ...
    distribution = assessDistributionReadiness(channelConfigArtifact, provisionResult);
    ...
    payment = assessPaymentAccountReadiness(paymentResult);
  }
...
/**
 * Pure. Infers the Cloudflare deployment target from the deployment URL's hostname shape.
 *
 * Adversarial-review finding (deep-tier /ship gate): the prior version hardcoded
 * `deployment_target: 'cloudflare_workers'` for every venture regardless of what was
 * actually observed -- wrong to assume (this module is venture-agnostic; not every venture
 * is on Cloudflare Workers) AND wrong spelling (the codebase's canonical enum,
 * lib/venture-deploy/stack-descriptor.js DEPLOYMENT_TARGETS, uses the hyphenated
 * 'cloudflare-workers'; 'cloudflare_workers' with an underscore appears nowhere else in the
 * repo and ventures.deployment_target has no CHECK constraint to catch it). Returns null
 * (never a guess) when the URL shape doesn't match a known pattern -- callers must not write
 * a target this function can't actually infer, consistent with this module's design (record
 * REAL observed state, never fabricate).
 * @param {string} url
 * @returns {'cloudflare-workers'|'cloudflare-pages'|null}
 */
export function inferDeploymentTarget(url) {
  if (!url) return null;
  let hostname;
  try {
...
/**
 * Pure. Ranks health_status by severity so a correction can never SILENTLY UNDERSTATE a
 * fresher, worse reading.
 *
 * Adversarial-review finding: the original carry-forward always preferred the correction's
 * `to` value outright, so once a venture had a correction on file (e.g. a known partial
 * defect, healthy->warning), a LATER genuine full outage (deploy.reachable:false, which
 * toVentureHealthStatus maps to 'critical') would still persist as the stale, less-severe
 * 'warning' -- the one column meant to reflect real-time health would under-report an actual
 * outage as an already-triaged partial issue. Correction still wins when it is EQUALLY or
 * MORE severe than the fresh naive reading (that's the whole point of a correction -- the
 * naive check structurally can't see the defect the correction documents); it never wins
 * when the fresh reading is MORE severe, since that's new information the correction
 * couldn't have anticipated.
 * @param {'healthy'|'warning'|'critical'} correctionTo
 * @param {'healthy'|'warning'|'critical'} naive
 * @returns {'healthy'|'warning'|'critical'}
 */
export function resolveHealthStatus(correctionTo, naive) {
  const SEVERITY = { healthy: 0, warning: 1, critical: 2 };
  if (!correctionTo) return naive;
  return SEVERITY[naive] > SEVERITY[correctionTo] ? naive : correctionTo;
}
 ...
 *
 * CORRECTION CARRY-FORWARD (incident, resolved same-session): toVentureHealthStatus() is a
 * naive function of {reachable, assetsVerified} -- an asset-existence check that structurally
 * cannot observe a client-side-rendered functional defect (e.g. a broken auth config). A
 * one-off correction (scripts/venture-provisioning/correct-altifyai-health-status.mjs)
 * annotated a prior artifact row with `health_status_correction` to record exactly that gap
 * -- but the NEXT unrelated re-run of this function (e.g. refreshing FR-3) silently
 * overwrote ventures.health_status back to the naive value, since nothing read the
 * annotation. Fixed by reading the prior current artifact's health_status_correction before
 * writing, using its `to` value in place of the naive one, and carrying the annotation
 * forward into the new row -- so a correction survives every future re-run until someone
 * explicitly clears it (e.g. once the referenced defect SD ships a real fix).
...
export async function recordProvisioningReadiness({ supabase, ventureId, report }) {
  const { data: priorArtifact } = await supabase
    ...
    .eq('is_current', true)
    .maybeSingle();
  const priorCorrection = priorArtifact?.artifact_data?.health_status_correction || null;
  const healthStatus = resolveHealthStatus(priorCorrection?.to, toVentureHealthStatus(report.deploy));

  const ventureUpdatePayload = {
    deployment_url: report.deploy.url,
    health_status: healthStatus,
  };
  // Only write deployment_target when it can be confidently inferred from the URL shape --
  // never fabricate a value for a target this function can't actually observe.
  const inferredTarget = inferDeploymentTarget(report.deploy.url);
  if (inferredTarget) ventureUpdatePayload.deployment_target = inferredTarget;

  const { data: updatedVentureRows, error: ventureUpdateError } = await supabase
    .from('ventures')
    .update(ventureUpdatePayload)
    .eq('id', ventureId)
    .select('id');
  ...
  const artifactData = priorCorrection ? { ...report, health_status_correction: priorCorrection } : report;
  const artifactRow = {
    venture_id: ventureId,
    lifecycle_stage: lifecycleStage,
    artifact_type: READINESS_ARTIFACT_TYPE,
    title: 'OPCO-A provisioning readiness report (FR-1/FR-2/FR-3/FR-4 decision-points)',
    is_current: true,
  ...
  if (isUniqueViolation) {
    // Adversarial-review finding (deep-tier /ship gate): idx_unique_current_artifact is a
    // PARTIAL index (WHERE is_current=true) -- without an is_current filter here, this WHERE
    // clause matches every historical (is_current=false) row for this stage too, not just the
    // race's winning row. Setting is_current:true on more than one matched row would collide
    // with the SAME partial unique index this fallback exists to recover from. The race
    // winner's row is always the one already is_current=true, so that's the only row this
    // fallback may touch.
