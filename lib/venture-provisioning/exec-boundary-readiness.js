 ...
 * FR-4 analytics wiring).
 *
 * WHY A READINESS REPORT, NOT THREE SEPARATE FEATURES: investigation found each FR has
 * a real chairman-reserved boundary sitting at the edge of what it literally asks for:
 ...
 * otherwise only reads. FR-4 only reads. No publish, no live payment, no DNS mutation.
 *
...
/**
 * Orchestrates FR-1/FR-2/FR-4 assessment for one venture. I/O-heavy; injectable deps.
 * @param {{ supabase: object, ventureId: string, deploymentUrl: string }} params
 * @param {{ fetchImpl?: typeof fetch, provisionOrganicChannel?: Function, now?: () => string }} [deps]
 * @returns {Promise<object>} the full readiness report
 */
export async function buildProvisioningReadinessReport({ supabase, ventureId, deploymentUrl }, deps = {}) {
  ...
  const decisionPoints = [distribution.decisionPoint, analytics.decisionPoint].filter(Boolean);
