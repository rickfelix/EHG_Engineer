...
/**
 * Registrar-scope preflight (FR-2, QF-20260912-746): before composing a packet the
 * chairman can approve but the executor cannot honor, confirm the registrar token can
 * actually (a) list registered domains, (b) check one candidate domain, and (c) read
 * the billing profile registration ultimately needs. A missing grant is caught HERE,
 * named, and refused — not discovered after chairman approval (the live specimen:
 * a Registrar-Admin-only token that could search/check domains but got a 403 on the
 * billing-profile read, surfaced only after the chairman had already approved).
 *
 * Only runs when a live registrar is injected — plan-mode (registrar=null) has
 * nothing to preflight; quoteShortlist's existing degrade-to-'unknown' already
 * handles that case without blocking.
 *
 * listDomains/checkDomain block ONLY on a clear permission-denial signal (HTTP
 * 403/401) — a transient fault there (rate limit, timeout) must NOT block packet
 * composition, matching quoteShortlist's own established "degrade, don't
 * fabricate" posture for per-domain checks. The billing-profile read is the one
 * new safety net this FR adds, so it blocks on ANY failure, not just a 403 —
 * an unconfirmed billing profile means the purchase cannot be confirmed executable.
 *
 * @returns {Promise<{blocked: true, reason: string}|null>}
 */
export async function preflightRegistrarScope(registrar, probeDomain) {
  if (!registrar) return null;
  const isPermissionDenial = (err) => err?.registrarStatus === 403 || err?.registrarStatus === 401;
  try {
    await registrar.listDomains();
  } catch (err) {
    if (isPermissionDenial(err)) return { blocked: true, reason: `registrar domain list failed: ${err.message}` };
  }
  if (probeDomain) {
    try {
      await registrar.checkDomain(probeDomain);
    } catch (err) {
      if (isPermissionDenial(err)) return { blocked: true, reason: `registrar domain check failed for "${probeDomain}": ${err.message}` };
    }
  }
  try {
    await registrar.checkBillingProfile();
  } catch (err) {
    const grant = err.registrarStatus === 403 ? 'Account > Billing > Edit or Read' : null;
    return {
      blocked: true,
      reason: grant
        ? `registrar token lacks the ${grant} grant (billing-profile read returned HTTP 403: ${err.message})`
        : `registrar billing-profile read failed: ${err.message}`,
    };
  }
  return null;
}
 ...
 * @returns {Promise<{status: 'created'|'existing'|'no_shortlist'|'registrar_scope_missing'|'pending_conflict', decision?: object, unblock?: string}>}
 */
export async function composeAcquisitionPacket(supabase, ventureId, deps = {}) {
  ...
  const preflight = await preflightRegistrarScope(registrar, shortlist[0]?.domain);
  if (preflight?.blocked) {
    return { status: 'registrar_scope_missing', unblock: preflight.reason };
  }
...
export default { composeAcquisitionPacket, readDomainShortlist, quoteShortlist, pickRecommended, preflightRegistrarScope, resolvePurchaseCeilingUsd, PACKET_KIND, PACKET_VERSION, DEFAULT_DOMAIN_PURCHASE_CEILING_USD };
