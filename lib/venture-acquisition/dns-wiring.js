 ...
 *
 * CREDENTIAL CONTRACT (SD-FDBK-ENH-COMPLETION-FLAG-HARNESS-001): prefers
 * CLOUDFLARE_DNS_API_TOKEN (measured working for /zones/{id}/dns_records
 * calls); falls back to CLOUDFLARE_REGISTRAR_API_TOKEN — the registrar
 * adapter's own token (TR-5) — for environments that only provision the
 * single combined-scope credential. The registrar token measurably 403s
 * "Authentication error" (code 10000) on dns_records calls in this
 * environment, so the fallback is a compatibility path, not the happy path.
 * The DNS adapter factory returns null without either token — plan mode.
 *
 * KNOWN LIMITATION: neither token can createZone (POST /zones 403s on both,
 * missing the zone.create permission) — an ops/permissions gap, not fixable
 * by picking a different token. wireDomainDns()'s createZone call below is
 * left as-is (throws uncaught) rather than silently downgraded to plan mode,
 * so a brand-new venture domain's zone-creation failure stays visible.
 *
...
export function createDnsAdapter(env = process.env, { fetchImpl } = {}) {
  const token = env.CLOUDFLARE_DNS_API_TOKEN || env.CLOUDFLARE_REGISTRAR_API_TOKEN;
...
export async function wireDomainDns(dns, domain, target) {
  ...
  // KNOWN LIMITATION (SD-FDBK-ENH-COMPLETION-FLAG-HARNESS-001): createZone requires the
  // account's zone.create permission, which neither CLOUDFLARE_DNS_API_TOKEN nor
  // CLOUDFLARE_REGISTRAR_API_TOKEN currently holds (measured: both 403). A brand-new venture
  // domain with no existing zone will THROW here uncaught — deliberately not downgraded to
  // 'blocked_on_credentials' (that status is reserved for the no-adapter/no-token case above).
  // Fixing this requires an ops action (grant zone.create on one of the tokens), not a code change.
