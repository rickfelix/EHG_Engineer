...
import { recordDisposition, updateDispositionStatus, getDispositionBySubject, computeQuestionKey, deleteDisposition } from '../decision-binding/disposition.js';
...
export async function executeAcquisition(supabase, decisionId, deps = {}) {
  ...
  try {
    ...
    const msg = String(e?.message || e).slice(0, 500);
    // FR-3 (QF-20260912-746): registrar-adapter.js now throws .registrarErrors
    // (the raw errors[] array) and .registrarStatus (HTTP status) alongside the
    // joined message -- surface both, not just the string, so an operator sees
    // the registrar's real error code instead of re-deriving it from prose.
    const registrarErrors = Array.isArray(e?.registrarErrors) ? e.registrarErrors.slice(0, 10) : [];
    const registrarStatus = typeof e?.registrarStatus === 'number' ? e.registrarStatus : null;
    const persist = await persistOnDecision(supabase, decision, 'acquisition_error', {
      step: 'register', reason: msg, registrar_errors: registrarErrors, registrar_status: registrarStatus, at: now().toISOString(),
    });
    ...
    // resolveStuckAcquisition below is the operator-facing resolution path).
    return { status: 'failed', reason: 'register_failed', error: msg, registrarErrors, registrarStatus, errorPersisted: persist.persisted };
  }
...
/**
 * Resolve a disposition stuck at `in_flight_or_unknown_outcome` (FR-4, QF-20260912-746).
 * Prior to this, the refusal text named no actual tool — an operator had to hand-edit
 * the `system_events` row. This NEVER trusts the caller's belief on its own: it always
 * re-checks the LIVE registrar via the (now-corrected, FR-1) adapter before acting.
 *
 * - Registrar confirms the domain is STILL registrable (not purchased): the earlier
 *   attempt genuinely failed before registering. The stuck row is DELETED so a fresh
 *   executeAcquisition retry can record a brand-new disposition and proceed normally.
 * - Registrar reports the domain is NOT registrable (already taken/registered): the
 *   earlier attempt may actually have succeeded. REFUSE — do not delete a row that
 *   could be the only durable record of a real purchase; a human must investigate.
 *
 * @param {object} supabase - service-role client
 * @param {string} decisionId - chairman_decisions.id (the same id executeAcquisition takes)
 * @param {{registrar: object}} deps - a live registrar adapter is REQUIRED; this is
 *   never a plan-mode operation (fabricating a domain-availability answer would be
 *   exactly the unverified-claim problem this function exists to avoid).
 * @returns {Promise<{status: 'resolved'|'refused', reason?: string, domain?: string}>}
 */
export async function resolveStuckAcquisition(supabase, decisionId, deps = {}) {
  const { registrar } = deps;
  if (!registrar) {
    return { status: 'refused', reason: 'registrar_adapter_required: cannot verify domain status without a live registrar check' };
  }
  ...
  const subject = { venture_id: ventureId, domain };
  const questionKey = computeQuestionKey('domain_acquisition', subject);
  const existing = await getDispositionBySubject(supabase, 'domain_acquisition', subject);
  if (!existing) return { status: 'refused', reason: 'no_stuck_disposition_found', domain };
  if (existing.payload?.status !== 'awaiting_disposition') {
    return { status: 'refused', reason: `disposition_not_stuck: status is "${existing.payload?.status}", not awaiting_disposition`, domain };
  }
  ...
  try {
    ...
    return { status: 'refused', reason: `registrar_check_failed: ${String(e?.message || e).slice(0, 500)}`, domain };
  }

  if (live.registrable !== true) {
    return {
      status: 'refused',
      reason: 'registrar_reports_domain_registered: the prior attempt may have succeeded -- this disposition was NOT deleted; a human must verify ownership before any further action',
      domain,
    };
  }

  await deleteDisposition(supabase, questionKey);
  return { status: 'resolved', reason: 'registrar_confirms_not_registered: stuck disposition cleared, a retry can proceed', domain };
}

export default { executeAcquisition, resolveStuckAcquisition, planAcquisitionActions, PACKET_QUOTE_TTL_MS };
