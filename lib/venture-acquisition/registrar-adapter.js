 ...
 * CLOUDFLARE_REGISTRAR_API_TOKEN (originally intended to carry Registrar
 * write + DNS edit scopes, least-privilege) + CLOUDFLARE_ACCOUNT_ID. The
 * factory returns NULL when either is absent — null adapter IS the
 * plan-mode activation gate downstream (blocked_on_credentials, zero live
 * calls). The token value is captured in a closure and never logged,
 * echoed, thrown, or persisted; error messages carry only sanitized
 * registrar error text.
 *
 * CORRECTION (SD-FDBK-ENH-COMPLETION-FLAG-HARNESS-001, measured 2026-09-12):
 * the deployed CLOUDFLARE_REGISTRAR_API_TOKEN does NOT carry working DNS
 * scope — it 403s on /zones/{id}/dns_records. lib/venture-acquisition/
 * dns-wiring.js's createDnsAdapter() no longer relies on this token alone;
 * it prefers a separate CLOUDFLARE_DNS_API_TOKEN. This adapter itself is
 * unaffected (it never calls /zones or /dns_records, only registrar/
 * domain-search endpoints), so no behavior change here — doc-only fix.
 *
 * PROVISIONING (chairman bootstrap step, NOT an EXEC blocker): create a CF API
 * token scoped to Registrar:Edit on the platform account (see dns-wiring.js
 * for the separate DNS:Edit token), then set the env var(s) in the platform
 * secret store. Until then every consumer plans.
 *
