...
// KNOWN_PEERS set (adam, solomon, coordinator) does not include chairman, so both eva-scheduler
// and chairman-fleet owner labels resolve to an IDENTICAL {kind:'coordinator', resolvedPeer:null}
// fallback shape -- this raw-label check is the only way to distinguish them, applied to the
// normalized (trim+lowercase) label so a stray trailing space can't slip past an anchored regex
// (security-agent finding C2). Deliberately NOT added to owner-target-resolver.mjs itself: that
// module's KNOWN_PEERS/contract is owned by SD-LEO-INFRA-OPERATIVE-AGENT-OWNERSHIP-001-B.
