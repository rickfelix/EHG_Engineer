...
export async function emitCoordinatorRung(supabase, row, ownerTarget, deps = {}) {
  ...
  try {
    ...
    // SD-LEO-INFRA-LANE-HYGIENE-OVER-001: sender_session + a one-line body, mirroring the
    // FR-5 fix already applied to this function's sibling in scripts/periodic-liveness-
    // watcher.mjs -- this insert was missing both, so it counted as empty_sender_row AND
    // bodyless_row (kind periodic_liveness_ladder is in neither exemption list).
    // eslint-disable-next-line session-coordination-insert-classguard/no-raw-session-coordination-insert -- pre-existing site (classguard backlog), swept into this diff's scan only because SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A updates the KNOWN_PEERS ownership comment above. target_session is a freshly-resolved coordinator id or the documented sentinel.
...
// SD-LEO-INFRA-LIVENESS-LADDER-OWNER-ROUTING-001 / FR-2(b): owner-target-resolver.mjs's
// KNOWN_PEERS set (adam, solomon, coordinator, michael) does not include chairman, so both
// eva-scheduler and chairman-fleet owner labels resolve to an IDENTICAL {kind:'coordinator',
// resolvedPeer:null} fallback shape -- this raw-label check is the only way to distinguish them,
// applied to the normalized (trim+lowercase) label so a stray trailing space can't slip past an
// anchored regex (security-agent finding C2). Deliberately NOT added to owner-target-resolver.mjs
// itself: that module's KNOWN_PEERS/contract was established by SD-LEO-INFRA-OPERATIVE-AGENT-
// OWNERSHIP-001-B (completed); michael joined it under SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A.
const CHAIRMAN_OWNER_RE = /^chairman(-fleet)?$/;

/**
 * FR-1/FR-1b/FR-2 routing decision for one laddered (or ladder-eligible, for the FR-1b timeout
 * check) periodic_process_registry row. Pure -- no IO -- so it is the single seam both the
 * watcher and unit tests use for the composite routing decision (the watcher's own tick loop is
 * unexported, per testing-agent's finding that TS-1/TS-2/TS-5/TS-6-style scenarios had nothing to
 * call directly without this function).
 *
 * Gate on `ownerTarget.live === true`, NEVER on `ownerTarget.target` truthiness --
 * resolveOwnerTarget's fallback always returns a truthy target (`coordinatorId ||
 * 'broadcast-coordinator'`), a sentinel string that is truthy even when no live coordinator
 * exists (security-agent finding).
 *
 * FR-1b: a row whose consecutive-miss count has reached LADDER_THRESHOLD+3 with its owner
 * directive still unacked is treated as owner-not-responsive and falls through to the
 * chairman_awareness path -- checked OUTSIDE the one-shot `climb.laddered` gate (climbLadder only
 * returns laddered:true on the exact threshold-crossing tick), reusing climbLadder's own atomic
 * counter rather than a wall-clock/read_at-based timer.
 *
 * @param {{rawOwner:string, ownerTarget:{live:boolean, target:string}, climb:{count:number}, directiveActioned?:boolean}} args
 * @returns {{route:'owner_directive'|'chairman_awareness', target?:string, reason:string}}
 */
export function decideLadderRoute({ rawOwner, ownerTarget, climb, directiveActioned = false }) {
  const normalizedOwner = String(rawOwner ?? '').trim().toLowerCase();
  if (CHAIRMAN_OWNER_RE.test(normalizedOwner)) {
    return { route: 'chairman_awareness', reason: 'chairman_owned' };
  }

  const count = Number.isFinite(climb?.count) ? climb.count : 0;
  const timedOut = !directiveActioned && count >= LADDER_THRESHOLD + 3;
  if (timedOut) {
    return { route: 'chairman_awareness', reason: 'owner_directive_unacked_timeout' };
  }

  if (ownerTarget?.live === true) {
    return { route: 'owner_directive', target: ownerTarget.target, reason: 'live_owner' };
  }

  return { route: 'chairman_awareness', reason: 'dead_owner' };
}
...
async function findRecentlyDismissedSignatures(supabase) {
  ...
  try {
    ...
    const { data } = await supabase
      ...
      // Adversarial-review finding (PR #7300): an unordered LIMIT gives Postgres/PostgREST no
      // row-selection guarantee -- if qualifying rows ever exceed the cap, ORDER BY is what makes
      // "keep the 50 most recent" true rather than an arbitrary, non-deterministic 50. The cap
      // itself is still a defensive ceiling (cooldown window is short, 15min default), not a real
      // limit in practice -- but only WITH ordering does that claim actually hold.
...
export async function emitLadderDigest(supabase, candidates, deps = {}) {
  ...
  // QF-20260823-965: the ladder digest is what actually reaches the chairman queue -- a
  // single-process escalation names its required invocation (when its row has one) so the
  // chairman-facing text reads "run all", not a bare threshold_exceeded.
  const soleRequiredInvocation = escalating.length === 1 ? escalating[0].required_invocation : null;
  const title = escalating.length === 1
    ? `${DIGEST_PREFIX} ${escalating[0].display_name || escalating[0].process_key}${soleRequiredInvocation ? ` (requires invocation: '${soleRequiredInvocation}')` : ''}`
