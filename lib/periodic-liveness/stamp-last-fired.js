...
export async function stampFromGithubActionsRun(supabase, processKey, ranAtIso) {
  ...
  // SD-LEO-FIX-GHA-CRON-LIVENESS-001: MONOTONIC GUARD. MEASURED: this repo's own
  // fetchScheduledRuns() occasionally receives a stale GitHub Actions API batch (a distinct
  // backend replica, not a client cache -- observed up to a month stale). Before this guard,
  // an unconditional UPDATE meant a single stale fetch could regress last_fired_at BACKWARD,
  // silently undoing a genuinely fresher stamp already recorded. This is the ONLY writer of
  // this column besides the self_stamped path above (which is always `new Date()` and
  // therefore already monotonic by construction) -- no caller anywhere backfills or corrects
  // last_fired_at, so a strict newer-only filter has no legitimate override case to preserve.
  const { data, error } = await supabase
    .from('periodic_process_registry')
    .update({ last_fired_at: ranAtIso, updated_at: new Date().toISOString() })
    .eq('process_key', processKey)
    .eq('liveness_source', 'github_actions_api')
    .or(`last_fired_at.is.null,last_fired_at.lt.${ranAtIso}`)
    .select('process_key');
  ...
  // 0 rows: either the process_key isn't registered at all, or (the monotonic-guard filter's
  // doing) it IS registered but already carries a last_fired_at >= ranAtIso -- a correctly
  // skipped no-op-backward, not an error. Disambiguate with one cheap existence check rather
  // than conflating both under the same "not registered" warning.
  const { data: existing } = await supabase
    ...
    .maybeSingle();
  if (existing) {
    return { stamped: false, reason: 'not_newer_than_current_stamp' };
  }
