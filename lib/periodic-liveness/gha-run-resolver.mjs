...
/**
 * PURE: QF-20260830-795 -- residual of QF-20260830-694. A run that CONCLUDES (success OR
 * failure) is an OBSERVED fact and must stamp last_fired_at; only 'no_data' (this cycle's fetch
 * found no run for the workflow at all) has nothing to stamp. Before this, only 'stamp' called
 ...
 * @param {{decision: 'stamp'|'overdue'|'no_data', ranAtIso?: string}} classified
 * @returns {boolean}
 */
export function shouldStampDecision({ decision, ranAtIso } = {}) {
  return (decision === 'stamp' || decision === 'overdue') && !!ranAtIso;
}
