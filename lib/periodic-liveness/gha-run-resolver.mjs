...
export function observedGapStats(runs) {
  ...
  for (const [file, timestamps] of byFile) {
    ...
    stats.set(file, { maxGapMs, medianGapMs: medianOf(gaps), sampleCount: timestamps.length });
  }
  return stats;
}

function medianOf(gapsMs) {
  if (gapsMs.length === 0) return 0;
...
/**
 * PURE: is this workflow's MEDIAN observed gap more than `thresholdMultiple` times its configured
 * interval? QF-20260903-060: the max-gap floor above is DELIBERATELY self-adjusting (its own doc
 * comment above), so a workflow that is uniformly slow every cycle never breaches its own
 * worst-observed-gap -- systemic drift stays invisible even though every run succeeds. The median
 * is far more robust to that self-inflation than the max. This is a SEPARATE, non-self-adjusting
 * check that does NOT feed overdueThresholdMs's grace floor, so it cannot destabilize the
 * anti-jitter tolerance QF-20260823-631/QF-20260824-373 tuned -- it only surfaces the gap.
 * Requires >=4 samples (this ticket's own measurement floor) so one pair of runs can't trip it.
 */
export function medianRatioAlarm(row, ghaGapStats, thresholdMultiple = 3) {
  if (!ghaGapStats || !(row.expected_interval_seconds > 0)) return null;
  ...
  if (!observed || observed.sampleCount < 4) return null;
  const ratio = observed.medianGapMs / (row.expected_interval_seconds * 1000);
  if (ratio <= thresholdMultiple) return null;
  return { ratio, medianGapMs: observed.medianGapMs, sampleCount: observed.sampleCount };
}
