...
// QF-20260823-965: a multi-section CLI (e.g. `fleet-dashboard.cjs all` vs `... workers`) is only
// PROVEN alive by the one section its liveness stamp is wired to -- other sections running fine
// still read OVERDUE. Deriving the required trailing arg from the loop's own documented `prompt`
// (rather than hardcoding a per-tool table) surfaces that contract on the registry row and in
// escalation text, so an operator sees "run all", not a bare threshold_exceeded. A prompt with no
// argument beyond the script path (single-purpose scripts) has nothing ambiguous to name.
export function deriveRequiredInvocation(prompt) {
  if (!prompt) return null;
  const tokens = prompt.trim().split(/\s+/); // tokens[0]='node', tokens[1]=script path
  return tokens.length < 3 ? null : tokens[tokens.length - 1];
}
...
export function parseStandardLoops(sourceText) {
  ...
  while ((m = entryRe.exec(block)) !== null) {
    ...
    const promptMatch = /prompt:\s*'([^']+)'/.exec(entry);
    const scriptMatch = /script:\s*'([^']+)'/.exec(entry);
    out.push({
      ...
      required_invocation: deriveRequiredInvocation(promptMatch ? promptMatch[1] : null),
 ...
 *
 * SD-LEO-INFRA-ACTIVATE-INERT-STALL-001-A: a scripts/cron/*.mjs file that is ALSO invoked by a
 * STANDARD_LOOPS entry (e.g. index-jam-detector.mjs, wired in as standard_loop:index-jam-detector)
 * was previously double-discovered — once as cron_script:<file> (no cadence, no activation_note,
 * self_stamped, currently_expected_active forced true on every reseed) and once as
 * standard_loop:<key> (the real, hand-owned registration with the real cadence). The generic
 * cron_script:* row is a confusing SECOND source of truth for a process a STANDARD_LOOPS entry
 * already owns, so it is excluded here whenever a standard_loop entry names that same script file
 * — dropping cron_script:index-jam-detector.mjs's specimen and any future one in the same shape,
 * not just this one instance.
 */
export function discoverAllProcesses(repoRoot) {
  const standardLoops = discoverStandardLoops(repoRoot);
  const coveredScripts = new Set(standardLoops.map((l) => l.script).filter(Boolean));
  const cronScripts = discoverCronScripts(repoRoot)
    .filter((p) => !coveredScripts.has(p.process_key.slice('cron_script:'.length)));
  return [
...
// Registry process_key prefixes that are never backed by a discoverable invoker (role/session
// bookkeeping rows, not recurring jobs) — mirrors the pre-existing informational filter in
// scripts/enumerate-periodic-processes.mjs so the two can't drift apart.
export const EXEMPT_REGISTRY_PREFIXES = ['__', 'role_session:', 'scheduler_round:', 'g3-armed-'];

/**
 * QF-20260911-080 — the reverse direction of the shadow sweep above. discoverAllProcesses finds
 * every invoker and checks it's REGISTERED; this checks every row the registry EXPECTS to be
 * running (process_type='standalone_cron', currently_expected_active=true) actually has a
 * discovered invoker. standard_loop:drain-inventory was exactly this gap: a registry row nobody
 * ever scheduled, discovered only because a human ran it by hand often enough to keep it off
 * OVERDUE.
 */
export function findOrphanedActiveCrons(discovered, registryRows) {
  const discoveredKeys = new Set(discovered.map((p) => p.process_key));
  return (registryRows || []).filter((r) =>
    r.process_type === 'standalone_cron'
    && r.currently_expected_active === true
    && !discoveredKeys.has(r.process_key)
    && !EXEMPT_REGISTRY_PREFIXES.some((prefix) => r.process_key.startsWith(prefix)));
}
