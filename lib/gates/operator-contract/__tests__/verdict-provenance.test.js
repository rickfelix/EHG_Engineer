...
describe('GAP-1 producer side — the verdict pins its ENDPOINTS, not just its tree', () => {
  /**
   * Mutation M-GAP1b (producer stops pinning, range falls back to `${baseRef}...HEAD`) SURVIVED
   * the whole suite on first run: the reader-side test proved the reader USES base_sha, and
   * nothing proved the producer EMITS it. Producer verified, consumer verified, wire unverified
   * — for the third time in this SD. Both halves are pinned here.
   */
  it('collectSdDiff resolves real endpoint shas against a real repo', async () => {
    const { collectSdDiff } = await import('../harness-adapter.js');
    const d = collectSdDiff({ appPath: process.cwd() });
    expect(d.base_sha).toMatch(/^[0-9a-f]{40}$/);
    expect(d.head_sha).toMatch(/^[0-9a-f]{40}$/);
  });

  it('the shas reach details on a real verdict — the wire, not just the value', async () => {
    const res = await createOperatorContractGate(supabase, sd, process.cwd()).validator({ sd });
    expect(res.details.base_sha).toMatch(/^[0-9a-f]{40}$/);
    expect(res.details.head_sha).toMatch(/^[0-9a-f]{40}$/);
  });

  it('an unpinnable verdict reports null rather than a stale or invented sha', async () => {
    // The fail-open path: git threw, so there are no endpoints. null says "not reproducible",
    // which is true. Inventing one would be worse than admitting it.
    const res = await createOperatorContractGate(supabase, sd, 'C:/definitely/not/a/repo/xyz').validator({ sd });
    expect(res.details.base_sha).toBeNull();
  });
});
