...
describe('GAP-1 — the pinned range survives the base MOVING', () => {
  /**
   * The crux of GAP-1, and the one thing a static fixture cannot show: a pinned range and a
   * `ref...HEAD` range are IDENTICAL until the ref moves. So this fixture moves it — which is
   * precisely what happens when this PR merges into main.
   *
   * VALIDATION measured the unpinned behaviour flipping reproduced:true -> false on a
   * byte-identical tree. Here the assertion is the other direction: after the base advances to
   * include the SD's own commit, the PINNED range still reports the SD's files.
   */
  it('after the base branch advances past HEAD, the pinned diff is UNCHANGED', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'oc-moving-base-'));
    dirs.push(dir);
    git(dir, ['init', '-q', '-b', 'main']);
    git(dir, ['config', 'user.email', 'test@example.invalid']);
    git(dir, ['config', 'user.name', 'Test']);
    ...
    git(dir, ['branch', '-f', 'basebranch']);
    fs.writeFileSync(path.join(dir, 'payload.sql'), 'CREATE TABLE widgets (id uuid);\n');
    git(dir, ['add', '-A']); git(dir, ['commit', '-q', '-m', 'sd work']);

    const before = collectSdDiff({ appPath: dir, baseRef: 'basebranch' });
    expect(before.createdTables).toContain('widgets');
    expect(before.base_sha).toMatch(/^[0-9a-f]{40}$/);

    // THE MERGE: the base now contains the SD's own commit. An unpinned `basebranch...HEAD`
    // collapses to an EMPTY diff from here — the verdict silently means something else.
    git(dir, ['branch', '-f', 'basebranch', 'HEAD']);
    const moving = execFileSync('git', ['diff', '--name-only', 'basebranch...HEAD'], { cwd: dir, encoding: 'utf8' }).trim();
    expect(moving, 'the ARM: an unpinned range really does go empty once the base moves').toBe('');

    // Re-running against the RECORDED base sha — what the reader does — still sees the work.
    const after = collectSdDiff({ appPath: dir, baseRef: before.base_sha });
    expect(after.createdTables).toContain('widgets');
    expect(after.migrations.map((m) => m.path)).toEqual(before.migrations.map((m) => m.path));
  });
});
