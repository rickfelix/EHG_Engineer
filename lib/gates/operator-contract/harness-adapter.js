...
import { safeQuery } from '../../db/safe-query.mjs';
...
export function collectSdDiff({ appPath, baseRef = 'origin/main' } = {}) {
  ...
  // SEC (SECURITY row 4a1d302b, SD-LEO-INFRA-OPERATOR-CONTRACT-GATE-002): argv-safety was
  // never the only sink here. THE CWD IS A TRUST SINK IN ITS OWN RIGHT, because git reads the
  // TARGET repo's .git/config before doing anything. Demonstrated, not theorised: a fixture
  // repo with `diff.external` set in its own config EXECUTED that command when collectSdDiff
  // ran against it, and the spawned child INHERITED process.env — SUPABASE_SERVICE_ROLE_KEY
  // confirmed present inside it. The same run reported createdTables=[] and unreadable=0: a
  // clean read of a file it never read, which is the identical silent-blinding class as the
  // pathspec finding above, reached through config instead of argv.
  //
  // This became reachable only when SD-LEO-INFRA-OPERATOR-CONTRACT-GATE-002 introduced the
  // first DATA-FLOW into appPath (the read-back CLI passes a path out of the database).
  // Before that it was always ENGINEER_ROOT, derived from module location, so no attacker
  // data ever reached this parameter. The sink is inherited; the reachability is ours, so the
  // fix belongs here in the runner rather than at each call site — same placement argument as
  // --literal-pathspecs.
  // `--no-ext-diff`, NOT `-c diff.external=`. The config form was my first attempt and it
  // BROKE CONTENT READS: git treats the empty value as a CONFIGURED external driver, so
  // enumeration still worked while every per-file read came back empty — a clean report of a
  // file never read, which is the identical silent-blinding class this whole function was
  // hardened against. Caught by the RCE SD's own fixture tests, which is exactly what they are
  // for; a mutation-proved guard from another SD stopped me shipping a silent regression into
  ...
  // GAP-1 (VALIDATION row 419fe764): PIN THE ENDPOINTS. `origin/main` is a MOVING REF, and both
  // the producer and the read-back reader defaulted to it — so a recorded verdict silently
  // changed meaning as main advanced. Measured with the tree BYTE-IDENTICAL: advancing
  // origin/main past the SD's own commit flips the same verdict from reproduced:true to false,
  // which is exactly what happens when this PR merges. A `reproduced:false` would then be
  // indistinguishable from a genuine wrong-tree or tampering finding — the same silence class
  // FR-5 documents one layer down, at the other end of the same feature.
  //
  // Resolved ONCE and reused, so the shas reported are the shas diffed. Reading them afterwards
  // would re-resolve a ref that may have moved in between.
  let baseSha = null;
  let headSha = null;
  try {
    baseSha = run(['merge-base', baseRef, 'HEAD']).trim() || null;
    headSha = run(['rev-parse', 'HEAD']).trim() || null;
  } catch { /* fail-open: an unpinnable verdict is still a verdict, just not reproducible */ }
  // Diff from the merge-base COMMIT rather than the ref. `a...b` already means merge-base..b, so
  // this is identical today and stable tomorrow — which is the entire point.
  const range = baseSha && headSha ? `${baseSha}..${headSha}` : `${baseRef}...HEAD`;
  const nameOnly = run(['diff', '--name-only', '-z', range]).split('\0').filter(Boolean);
  const changedFiles = [];
  const migrations = [];
  const createdTables = [];
  ...
  for (const path of nameOnly) {
    ...
    try {
      ...
      // Same PINNED range as the enumeration above — enumerating one range and reading another
      // would let the two disagree the moment main moves.
      const patch = run(['diff', range, '--', path]);
  ...
  return { changedFiles, migrations, createdTables: [...new Set(createdTables)], unreadable, base_sha: baseSha, head_sha: headSha };
}
...
export async function resolveOperatorContract({ sd, appPath, supabase, now = new Date(), diff }) {
  ...
  const { changedFiles, migrations, createdTables, unreadable = [], base_sha: baseSha = null, head_sha: headSha = null } = diff || collectSdDiff({ appPath });
  ...
  try {
    const data = await safeQuery(
      ...
      { site: 'harness-adapter:periodic_process_registry' }
    );
    registryRows = data || [];
  } catch (e) {
    // NOT permissive despite the old "fail-open" label: an empty registryRows makes cadence_armed
    // false below, which fails the operator-contract gate -- a query fault is treated as "no
    // process armed" (fail-closed), still a real verdict rather than an unhandled crash.
    console.debug('[harness-adapter] periodic_process_registry query suppressed:', e?.message || e);
  }
  ...
  return {
    // FR-1: `creator` was READ at the gate factory (details.creator_kinds) and returned by NONE
    // of these three paths, so the key was always undefined and JSON.stringify dropped it —
    // emitted in source, absent in data. Measured at the persistence layer: creator_kinds is
    // MISSING, not null, from the stored details of the 2026-08-08T01:34:51 rejection.
    creator,
    base_sha: baseSha, head_sha: headSha,
...
export function createOperatorContractGate(supabase, sd, appPath, { diff } = {}) {
  return {
    ...
    validator: async (ctx) => {
      ...
      // FR-2: THIS is the value that gets emitted, not the executor-side appPath. The
      // `|| process.cwd()` fallback lets the two diverge, and repoPath is the only name for
      // the tree git actually ran against (it becomes collectSdDiff's cwd). A verdict that
      // names the wrong tree is worse than one that names none.
      //
      // FR-5, recorded so the next person does not re-derive it: resolveTargetRepository
      // NEVER reads sd.worktree_path. It goes through getRepoPath -> ENGINEER_ROOT, and
      // lib/repo-paths.js defines ENGINEER_ROOT via path.resolve(__dirname, '..') — MODULE
      // LOCATION, not the SD. Measured three ways: module-in-main + cwd-in-main -> main;
      // module-in-main + cwd-in-WORKTREE -> MAIN, the wrong tree for a worktree SD;
      // module-in-worktree -> the worktree. Sibling asymmetry: repo-paths getRepoRoot()
      // strips the worktree suffix, resolveRepoPath does not. No live wrong-tree verdict is
      // proven, so nothing here changes the resolver — but the failure would be SILENT in the
      // worst direction: wrong tree -> empty merge-base diff -> is_creator false ->
      // OPERATOR_CONTRACT_NOT_APPLICABLE -> PASS, indistinguishable from a genuine
      // non-creator SD. This emitted field is the instrument that would settle it.
      ...
      try {
        ...
        if (result.verdict === 'pass') {
          ...
          return {
            ...
            details: {
              ...
              repo_path: repoPath, base_sha: result.base_sha, head_sha: result.head_sha, creator_kinds: result.creator?.creator_kinds,
        ...
        return {
          ...
          issues: [result.reason],
          warnings: [],
          details: { reason: result.reason, missing: result.missing, creator_kinds: result.creator?.creator_kinds, repo_path: repoPath, base_sha: result.base_sha, head_sha: result.head_sha },
        };
      } catch (err) {
        ...
        if (enforced) {
          ...
          return {
            ...
            warnings: [],
            details: { fail_open: false, enforced: true, error: err.message, repo_path: repoPath, base_sha: null, head_sha: null },
          };
        }
        ...
        return {
          ...
          details: { fail_open: true, enforced: false, error: err.message, repo_path: repoPath, base_sha: null, head_sha: null },
        };
      }
    },
  };
