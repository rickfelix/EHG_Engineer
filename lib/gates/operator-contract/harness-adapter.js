...
export function collectSdDiff({ appPath, baseRef = 'origin/main' } = {}) {
  ...
  const run = (rawArgs) => {
    const args = rawArgs[0] === 'diff' ? ['diff', '--no-ext-diff', ...rawArgs.slice(1)] : rawArgs;
    ...
    validator: async (ctx) => {
      ...
      try {
        ...
        if (result.verdict === 'pass') {
          ...
          return {
            ...
            details: {
              ...
              repo_path: repoPath, creator_kinds: result.creator?.creator_kinds,
        ...
        return {
          ...
          issues: [result.reason],
          warnings: [],
          details: { reason: result.reason, missing: result.missing, creator_kinds: result.creator?.creator_kinds, repo_path: repoPath },
        };
      } catch (err) {
        ...
        if (enforced) {
          ...
          return {
            ...
            warnings: [],
            details: { fail_open: false, enforced: true, error: err.message, repo_path: repoPath },
          };
        }
        ...
        return {
          ...
          details: { fail_open: true, enforced: false, error: err.message, repo_path: repoPath },
        };
      }
    },
  };
