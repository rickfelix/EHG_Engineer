...
export async function send({ to, body, mediaUrl, identity }) {
  ...
  // SD-LEO-INFRA-MICHAEL-TIER2-CHECKPOINT-SEND-001 SECURITY SEC-M3: without a timeout, a hung
  // request can still be in flight when the next scheduled fire starts (Michael's checkpoint-send
  // windows catch two 15-minute fires each) -- a bounded timeout closes that overlap window for
  // every caller, not just the identity-bearing one.
