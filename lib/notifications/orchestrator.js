...
import { etLocalHour, etLocalMinute, isValidCanonicalZone } from '../time/chairman-et-wall-clock.js';

// QF-20260816-091: default zone for a chairman with no explicit quiet_hours.timezone set.
const DEFAULT_QUIET_HOURS_ZONE = 'America/New_York';
...
export async function sendImmediateNotification(supabase, params) {
  ...
  // 2. Check quiet hours
  // QF-20260906-177: params.now is an optional test seam (defaults to isInQuietHours' own
  // `new Date()` when omitted) -- production behavior unchanged.
  const inQuietHours = await isInQuietHours(supabase, params.chairmanUserId, params.now);
  if (inQuietHours) {
...
export async function sendDailyDigest(supabase, params) {
  ...
  const { events, trueCount } = await gatherDigestEvents(supabase, windowStart, windowEnd);
  ...
  const { data: notification, error: insertError } = await supabase
    ...
    .insert({
      ...
      email_metadata: { event_count: events.length, true_count: trueCount, window_start: windowStart, window_end: windowEnd }
    })
    .select('id')
    .single();
 ...
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {{ sdKey: string, sdTitle: string, totalScore: number, dimensionScores: Object, scoreId: string, recipientEmail: string, chairmanUserId: string }} params
 * @returns {Promise<{ notificationId: string | null, status: string }>}
 */
export async function sendVisionScoreNotification(supabase, params) {
  ...
  const chairmanUserId = params.chairmanUserId || process.env.CHAIRMAN_USER_ID;
...
/**
 * QF-20260906-177: `now` is injectable (like channel-health-recorder.js's computeHealthUpdate /
 * evaluateAlarmTransition / checkCanaryFreshness) so a test can pin a deterministic instant
 * instead of depending on the real wall clock and the runner's timezone. Defaults to `new Date()`
 * -- production callers that omit it are unaffected.
 */
export async function isInQuietHours(supabase, chairmanUserId, now = new Date()) {
  ...
  // QF-20260816-091: quiet hours are a chairman-LOCAL concept — comparing against the
  // server's wall clock (getHours/getMinutes) silently used the container's UTC time
  // instead, shifting the enforced window by the chairman's UTC offset.
  //
  // Adversarial review (PR #7180): a present-but-invalid prefs.timezone (typo, legacy
  // alias) must not degrade silently and indistinguishably from "never configured" — that
  // exact silent-fallback gap is what let the chairman_preferences upsert bug hide behind
  // quiet-hours-extension.js's deriveChairmanZone before its 'invalid_fallback' signal was
  // added (SEC-QW-03). Mirror that precedent here with a one-line warn.
  if (prefs.timezone && !isValidCanonicalZone(prefs.timezone)) {
    console.warn(`[quiet-hours] invalid prefs.timezone '${prefs.timezone}' for chairman_id=${chairmanUserId} — falling back to ${DEFAULT_QUIET_HOURS_ZONE}`);
  }
  const zone = isValidCanonicalZone(prefs.timezone) ? prefs.timezone : DEFAULT_QUIET_HOURS_ZONE;
  const currentHour = etLocalHour(now, zone);
  const currentMinute = etLocalMinute(now, zone);
...
// QF-20260816-214: the rendering query below stays capped at 50 (an unbounded digest email
// is its own UX defect), but the cap must never silently masquerade as the true day count --
// gatherWeeklyMetrics already uses fetchAllPaginated for its own truncation-discipline needs
// (SD-LEO-INFRA-COUNT-TRUNCATION-DISCIPLINE-001); a rendered list has a different, correct
// answer -- keep the cap, but also report trueCount via a lightweight exact head-count on the
// SAME filters, so callers can render "...and N more" instead of a silently wrong total.
async function gatherDigestEvents(supabase, windowStart, windowEnd) {
  const events = [];
  const EVENT_TYPES = ['stage_completion', 'dfe_auto_approval', 'venture_health_change'];
  ...
  if (events.length === 50) {
    const { count } = await supabase
      ...
      .lte('created_at', windowEnd)
      .in('event_type', EVENT_TYPES);
    if (typeof count === 'number') trueCount = count;
  }

  return { events, trueCount };
}
