 ...
 * @param {{ date: string, timezone: string, events: Array<{ type: string, ventureName: string, description: string, timestamp: string, deepLink: string }>, trueCount?: number }} data
 * @returns {{ html: string, text: string, subject: string }}
 */
export function dailyDigestTemplate(data) {
  // QF-20260816-214: trueCount is the real 24h total; events is capped at 50 for rendering.
  // Falls back to events.length when the caller doesn't supply trueCount (back-compat).
  const totalCount = typeof data.trueCount === 'number' ? data.trueCount : data.events.length;
  const truncated = totalCount > data.events.length;
  const moreNote = truncated ? ` (showing ${data.events.length}, ${totalCount - data.events.length} more not shown)` : '';
  const subject = `[Daily Digest] ${data.date} - ${totalCount} portfolio events`;
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #16213e; padding: 24px; color: #e0e0e0;">
        <p style="font-size: 16px; margin-top: 0;">${totalCount} events in the last 24 hours${escapeHtml(moreNote)}:</p>
        ${eventSections}
      </div>
      <div style="background: #1a1a2e; padding: 12px 20px; border-radius: 0 0 8px 8px; text-align: center;">
