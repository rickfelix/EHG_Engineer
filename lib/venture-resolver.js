...
import { resolveLocalPath } from './repo-paths.js';
...
export function getVenturePath(targetApp) {
  ...
  // resolveLocalPath() (lib/repo-paths.js), not a bare path.resolve(): a registry local_path
  // can be relative (e.g. "../ehg", SD-LEO-INFRA-REPO-HYGIENE-PATH-001 FR-1) -- a bare
  // path.resolve() on a relative value resolves against process.cwd(), the caller's ambient
  // working directory at invocation time, which is unpredictable and almost never correct.
  // Found live-broken during EXEC-TO-PLAN review of that same SD: getVenturePath('ehg') from
  // this worktree returned a nonexistent ...\.worktrees\ehg before this fix.
  for (const app of Object.values(apps)) {
    if (app.name?.toLowerCase() === needle && app.local_path) {
      return resolveLocalPath(app.local_path);
    }
  }
...
export function getVentureConfig(targetApp) {
  ...
  for (const app of Object.values(apps)) {
    ...
    if (appNormalized === needleNormalized) {
      return { ...app, local_path: app.local_path ? resolveLocalPath(app.local_path) : null };
    }
  }
...
export function listVentures() {
  ...
  return Object.values(apps)
    ...
    .map(app => ({
      ...app,
      local_path: app.local_path ? resolveLocalPath(app.local_path) : null
    }));
}
