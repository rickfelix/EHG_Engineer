...
async function assertWorkerTierAllowed(supabase, row, logger = console) {
  try {
    ...
    // QF-20260830-737 (Adam ruling, this row is its record): a WORKER-side tier_rank with no
    // recorded metadata.tier_rank_source is a number derived from an absence -- the identical
    // principle that makes an unprovenanced SD-side min_tier_rank ADVISORY (ruling 1B,
    // SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001 FR-3) applies symmetrically here. Before this fix
    // one seat in six (Charlie 2b9045cc, tier_rank=4, tier_rank_source empty) could be hard-
    // refused tier-1 work on a value nobody could source (measured: 17:16Z REFUSED WORK_ASSIGNMENT
    // on a stranded parent unowned 27h) while the SD-side twin of the same defect logged advisory.
    // Gates BOTH throw sites below -- above_worker_tier/tier_stamp_missing and the reserved-
    // no-lower-backlog refusal -- since both depend on workerRank being a JUSTIFIED number.
    const hasWorkerProvenance = !!(sess.metadata && sess.metadata.tier_rank_source);
    const { data: sd } = await supabase
      .from('strategic_directives_v2')
      .select('metadata')
      .eq('sd_key', sdKey)
    ...
    if (verdict === 'above_worker_tier' || verdict === 'tier_stamp_missing') {
      if (!hasWorkerProvenance) {
        logger && logger.info && logger.info(
          `[dispatch] ADVISORY (not blocking): worker tier_rank ${workerRank} has no tier_rank_source; `
          + `dispatching ${sdKey} (min_tier_rank ${minRank}) anyway per QF-20260830-737 (worker-side twin of ruling 1B). `
          + `Stamp metadata.tier_rank_source to restore enforcement for this seat.`
        );
      } else {
        const e = new Error(
    ...
    if (minRank < workerRank) {
      ...
      if (backlogData && !lowerTierBacklog(minRank, backlogData)) {
        if (!hasWorkerProvenance) {
          logger && logger.info && logger.info(
            `[dispatch] ADVISORY (not blocking): worker tier_rank ${workerRank} has no tier_rank_source; `
            + `dispatching ${sdKey} (tier_rank ${minRank}, no lower backlog) anyway per QF-20260830-737. `
            + `Stamp metadata.tier_rank_source to restore the reserved-capability refusal for this seat.`
          );
        } else {
          const e = new Error(
...
async function assertSendBackpressure(supabase, row, logger = console) {
  try {
    ...
    if (count >= BACKPRESSURE_UNANSWERED_LIMIT) {
      const e = new Error(
        `[dispatch] DISPATCH_BACKPRESSURE: target ${row.target_session} already has ${count} unanswered directed row(s) (limit ${BACKPRESSURE_UNANSWERED_LIMIT}) — refusing another routine send. Use an exempt payload.kind (${[...BACKPRESSURE_EXEMPT_KINDS].join(', ')}) only if this delivery genuinely cannot wait.`
      );
