...
async function assertWorkerTierAllowed(supabase, row, logger = console) {
  try {
    ...
    // QF-20260831-419: WORK-DOWN-NEVER-UP enforcement retired to advisory-only (see below) --
    // provenance no longer gates a throw vs. an advisory log, both branches now always log.
    const { data: sd } = await supabase
      .from('strategic_directives_v2')
      .select('metadata')
      .eq('sd_key', sdKey)
    ...
    // SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001 (FR-3): a floor without metadata.min_tier_rank_reason
    // is advisory. tierRankVerdict returns 'above_worker_tier_advisory' in that case (distinct from
    // the blocking 'above_worker_tier'), which is neither thrown here nor equal to any check below
    // -- dispatch proceeds, logged so the advisory is visible rather than a silent pass-through.
    const hasProvenance = !!(sd && sd.metadata && sd.metadata.min_tier_rank_reason);
    const verdict = tierRankVerdict(workerRank, minRank, { hasProvenance });
    if (verdict === 'above_worker_tier' || verdict === 'tier_stamp_missing') {
      // QF-20260831-419 (chairman order, ratified by adam a78170fa): WORK-DOWN-NEVER-UP
      // enforcement retired -- tier mismatches are now logged advisory only, never refused.
      logger && logger.info && logger.info(
        `[dispatch] ADVISORY (not blocking): worker tier_rank ${workerRank} `
        + `${verdict === 'tier_stamp_missing' ? 'is missing/unresolved' : `is below min_tier_rank ${minRank}`} for ${sdKey}; `
        + `dispatching anyway per QF-20260831-419 (tier enforcement retired to advisory).`
      );
    }
    if (verdict === 'above_worker_tier_advisory') {
      logger && logger.info && logger.info(
        `[dispatch] ADVISORY (not blocking): ${sdKey} carries min_tier_rank ${minRank} with no min_tier_rank_reason; `
        + `dispatching to tier_rank ${workerRank} worker anyway per ruling 1B. Set TIER_FLOOR_PROVENANCE_ADVISORY=off to restore binding-regardless-of-provenance.`
      );
    }
    ...
    if (minRank < workerRank) {
      ...
      // lowerTierBacklog returns false -> CONFIRMED reserved, refuse -- UNLESS the worker's own
      // rank is unprovenanced (QF-20260830-737), in which case this degrades to advisory too.
      if (backlogData && !lowerTierBacklog(minRank, backlogData)) {
        // QF-20260831-419: reserved-capability refusal retired to advisory.
        logger && logger.info && logger.info(
          `[dispatch] ADVISORY (not blocking): ${sdKey} (tier_rank ${minRank}) has no backlog at/below that `
          + `tier — dispatching to tier_rank ${workerRank} worker anyway per QF-20260831-419 (tier enforcement retired to advisory).`
        );
      }
    }
  } catch (e) {
