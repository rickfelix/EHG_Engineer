...
async function assertSendBackpressure(supabase, row, logger = console) {
  try {
    ...
    if (count >= BACKPRESSURE_UNANSWERED_LIMIT) {
      ...
      const e = new Error(
        `[dispatch] DISPATCH_BACKPRESSURE: target ${row.target_session} already has ${count} unanswered directed row(s) (limit ${BACKPRESSURE_UNANSWERED_LIMIT}) — refusing another routine send (content PARKED, not lost). Use an exempt payload.kind (${[...BACKPRESSURE_EXEMPT_KINDS].join(', ')}) only if this delivery genuinely cannot wait.`
      );
