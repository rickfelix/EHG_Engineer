...
async function assertSendBackpressure(supabase, row, logger = console) {
  try {
    ...
    // QF-20260902-962: the WORKER_SIGNAL friction lane (scripts/worker-signal.cjs) never sets
    // payload.kind — it keys on payload.signal_type, exactly the same "kind-only scheme misses
    // the friction channel" trap lib/fleet/worker-status.cjs's classifyCoordinationRow already
    // documents and defends against on the READ side. A friction report (stuck/need-sweep/
    // prd-ambiguous/gate-bug/spec-conflict/harness-bug/feedback/unfit/other) is the mechanism
    // that prevents a bypass, so it must never be throttled by the coordinator's own backlog —
    // a refused signal was witnessed becoming a --bypass-validation false completion instead.
