...
// QF-20260830-454: SEAT_BUSY_RESERVATION payload kind, sole representation of "this seat is
// busy on directed work" for both seat-busy-fence.cjs (worker-side self-claim suppression) and
// capacity gauges (coordinator-idle-qf-hint.mjs) — see stampSeatBusyReservation below.
const { PAYLOAD_KINDS, INFORMATIONAL_KINDS } = require('../fleet/worker-status.cjs');
// QF-20260904-695: INFORMATIONAL_KINDS rows (roll_call chief among them) are undrained by
// design and never set acknowledged_at -- a target with a high-volume periodic sender (e.g. the
// coordinator, fed continuous worker roll_calls) accumulates these past BACKPRESSURE_UNANSWERED_LIMIT
// forever, permanently blocking every OTHER routine send to that target. Excluded from the
// counted population only (see assertSendBackpressure below) -- NOT added to
// BACKPRESSURE_EXEMPT_KINDS, since that set controls whether the OUTGOING send skips the check
// entirely, and an informational row being individually parked is harmless (it still delivers).
const INFORMATIONAL_KINDS_SET = new Set(INFORMATIONAL_KINDS);
const { CORRECTION_KIND_SET, DISPOSITION_KIND } = require('./message-kinds.cjs');
// SD-LEO-FIX-COORDINATOR-RULING-REVERSES-001 (FR-1): a caller can already set payload.urgency
// directly (payload is unrestricted JSONB), but that leaves the value a re-typed string literal
// with no shared source of truth -- the same drift already visible on `topic`. opts.urgency below
// gives it one validated, discoverable name instead.
const { isKnownUrgency } = require('./urgency-levels.cjs');
// QF-20260831-560: send backpressure — never let a 4th unanswered directed row queue up on one
// target. Exempt classes that must always get through regardless of backlog: collision warnings
// (coordination-conflict alerts), amend-sd notices, and every existing protocol-mandated
// delivery (corrections + the terminal disposition verdict — the SAME set assertCorrelationNotDisposed
// already treats as un-lockable, reused rather than re-invented).
// QF-20260906-162: a signal_receipt is a coordinator-authored delivery receipt, not a routine
...
const { resolveAssignmentTargetKey, describeUnreadableAssignment, describeUndirectableAssignment } = require('../fleet/assignment-target.cjs');
...
/**
 * QF-20260907-463 rollout switch, separate from isAssignmentTargetGuardBinding above — a
 * DIFFERENT, narrower check (the 'directed' profile) with its own calibration window, per this
 * same file's Observe-Only-First precedent. A row can pass the broader worker-profile guard
 * (unreadable) yet still fail this one (undirectable): sharing one switch would either promote
 * both checks together before either has been independently reviewed, or block this one on the
 * older check's already-reviewed volume — neither is the calibration this file's own convention
 * calls for.
 * @returns {boolean} true when the directed-target guard should REFUSE rather than warn
 */
function isDirectedTargetGuardBinding() {
  return String(process.env.DISPATCH_DIRECTED_TARGET_GUARD || '').toLowerCase() === 'block';
}
...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: the michael singleton's sentinel + live-id resolution.
const { getActiveMichaelId } = require('./michael-identity.cjs');
...
// QF-20260829-513: the SAME writer the auto-promote refill path already uses (reserve-sd.cjs) —
// do NOT build a second soft-lock mechanism; the shipped one has a live consumer
// (drain-reservations.cjs/seat-busy-fence.cjs) and a single existing producer this QF extends.
const { reserveSd } = require('./reserve-sd.cjs');

// Sentinel target → role, for the target-drain warn. A target with no entry here (e.g. the
// now-removed bare 'broadcast' — QF-20260911-753) resolves to null via the isFullUuid
// fallback below: no single drain set applies, so the check stays silent.
const SENTINEL_ROLES = Object.freeze({
  'broadcast-coordinator': 'coordinator',
  'broadcast-solomon': 'solomon',
  'broadcast-adam': 'adam',
  'broadcast-michael': 'michael',
});
...
async function resolveTargetRole(supabase, target, targetRoleHint) {
  ...
  // QF-20260911-753: the explicit target === 'broadcast' check was removed -- 'broadcast' is no
  // longer a value that can reach here on a legitimate insert (assertValidTarget rejects it
  // first), and !isFullUuid(target) already covers it as a fallback either way.
  if (!isFullUuid(target)) return null;
  try {
    if (!_roleIdsCache.ids || (Date.now() - _roleIdsCache.at) > ROLE_IDS_TTL_MS) {
      const [adamId, solomonId, coordinatorId, michaelId] = await Promise.all([
        getActiveAdamId(supabase, {}).catch(() => null),
        getActiveSolomonId(supabase, {}).catch(() => null),
        getActiveCoordinatorId(supabase).catch(() => null),
        getActiveMichaelId(supabase, {}).catch(() => null),
      ]);
      _roleIdsCache = { at: Date.now(), ids: { adamId, solomonId, coordinatorId, michaelId } };
    }
    const { adamId, solomonId, coordinatorId, michaelId } = _roleIdsCache.ids;
    if (target === adamId) return 'adam';
    if (target === solomonId) return 'solomon';
    if (target === coordinatorId) return 'coordinator';
    if (target === michaelId) return 'michael';
    return null;
  } catch {
    return null;
  }
...
// Documented non-UUID targets that are intentionally allowed. broadcast-coordinator =
// worker→coordinator; broadcast-solomon = worker/Adam→Solomon consult lane
// (SD-LEO-INFRA-SOLOMON-CONSULT-001C — inert until SOLOMON_CONSULT_V1); broadcast-adam =
// Solomon→Adam direct lane fallback (SD-LEO-INFRA-ROLE-BASED-COMMS-ROUTING-PROTOCOL-001-B —
// inert until ADAM_SOLOMON_TWOWAY_V1), symmetric with broadcast-solomon so a
// transiently-unresolvable Adam never throws DISPATCH_TARGET_INVALID on the reverse leg.
// Sentinels short-circuit the live-session lookup (not a single row).
//
// QF-20260911-753: the bare 'broadcast' sentinel was REMOVED. It had no reader that ever
// acknowledges it — measured live: 91 rows on target_session='broadcast', 0 EVER
// acknowledged, across all history. Every other sentinel here has a confirmed drain
// (broadcast-coordinator: the /coordinator start flow; the others: their named role's
// inbox). A sentinel every writer may address and no reader drains is a silent sink for
// directives (Adam disposition, Friday Foundation Audit #2 item 12(a), feedback 89b3be6a).
// Writers that previously targeted bare 'broadcast' now get DISPATCH_TARGET_INVALID and
// must name a real role sentinel or a live session id instead.
const SENTINEL_TARGETS = Object.freeze(['broadcast-coordinator', 'broadcast-solomon', 'broadcast-adam', 'broadcast-michael']);
 ...
 */
async function assertValidTarget(supabase, target, logger = console, opts = {}) {
  ...
  const { data, error } = await supabase
    ...
    .select('session_id, heartbeat_at, sd_key')
  ...
  // unread by design (lib/checkin/steps/index.cjs's claim slot gates every acquisition tier
  // including directed-assignment), so it waits until a human re-targets it. Refuse when an
  // idle seat exists instead, unless the caller explicitly opts in (the walk-worktree owner is
  // a legitimate deliberate busy target).
  if (opts.isWorkAssignment && data.sd_key && !opts.allowBusyTarget) {
    // QF-20260913-509: the prior raw `.is('sd_key', null)` scan named ANY claim-less, fresh
    // session as the re-target suggestion -- including role seats (adam/solomon/coordinator,
    // never valid targets for a WORKER item) and quick-fix holders (a QF claim leaves sd_key
    // null, so a seat mid-QF read as idle capacity here). Reuse the shared, canonical idle
    // predicate (lib/fleet/seat-idle-predicate.mjs) instead of a bespoke definition -- one
    // eligible-target notion, fixed once, every caller uses it.
    const { seatIdleVerdict } = await import('../fleet/seat-idle-predicate.mjs');
    const { data: qfHolders } = await supabase
      ...
      .limit(500);
    const qfHolderSessionIds = new Set((qfHolders || []).map((r) => r.claiming_session_id).filter(Boolean));
    const { data: candidates } = await supabase
      .from('claude_sessions')
      .select('session_id, sd_key, metadata, heartbeat_at')
      .gte('heartbeat_at', new Date(Date.now() - LIVE_HEARTBEAT_CUTOFF_MS).toISOString())
      .limit(50);
    const idleSeat = (candidates || []).find(
      (s) => seatIdleVerdict(s, { sdHolderSessionIds: null, qfHolderSessionIds }).idle
    );
    if (idleSeat) {
      const msg = `[dispatch] REFUSED insert: target_session ${target} holds claim ${data.sd_key} `
        + `and at least one idle seat exists (${idleSeat.session_id}) — a directed WORK_ASSIGNMENT `
        + `to a busy seat sits unread until a human re-targets it. Re-target to the idle seat, or `
        + `pass { allowBusyTarget: true } if this seat is the deliberate addressee.`;
      logger && logger.error && logger.error(msg);
      const e = new Error(msg);
      e.code = 'DISPATCH_BUSY_TARGET_HAS_CLAIM';
      throw e;
    }
  }
...
/**
 * QF-20260728-944: which role a payload DECLARES its addressee to be, or null if the payload
 * makes no such declaration. Scoped to exactly the lanes measured mis-delivered — do not widen
 * without a measured occurrence (the ticket explicitly warns against sizing/guessing).
 * SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-G adds 'michael_handoff' per the chairman-ratified
 * Solomon Q6 adjudication (docs/michael/05-SOLOMON-ADJUDICATION.md), which names this function
 * explicitly and cites a real measured incident (an untyped row landing at 13:56Z, 2026-09-05) as
 * the occurrence this scoping comment asks for.
 * PURE.
 * @param {object|null} payload
 * @returns {'solomon'|'adam'|'michael'|null}
 */
function declaredAddresseeRole(payload) {
  if (!payload || typeof payload !== 'object') return null;
  if (payload.topic === 'solomon_responsibilities') return 'solomon';
  if (payload.topic === 'adam_responsibilities') return 'adam';
  if (payload.kind === 'solomon_consult') return 'solomon';
  if (payload.kind === 'michael_handoff') return 'michael';
  return null;
}

/**
 * QF-20260728-944 (chairman-approved): refuse an insert whose payload declares an addressee role
 * that CONFIRMABLY differs from the target session's resolved role. Fail-open on an unresolvable
 * target role (precision over recall — same posture as the drain-set warn check) and fail-open on
 * any error in this guard itself; fail-CLOSED only on a confirmed mismatch.
 * @throws {Error} code DISPATCH_ROLE_TOPIC_MISMATCH on a confirmed mismatch
 */
async function assertAddresseeRoleMatch(supabase, row, opts, logger = console) {
  try {
    const payload = row.payload && typeof row.payload === 'object' ? row.payload : null;
    const declaredRole = declaredAddresseeRole(payload);
    if (!declaredRole) return;
    const targetRole = await resolveTargetRole(supabase, row.target_session, opts && opts.targetRoleHint);
    if (!targetRole || targetRole === declaredRole) return;
    const msg = `[dispatch] REFUSED insert: payload declares addressee role '${declaredRole}' `
      + `(topic=${JSON.stringify(payload.topic)}, kind=${JSON.stringify(payload.kind)}) but `
      + `target_session ${row.target_session} resolves to role '${targetRole}' — role/topic `
      + `mismatch, message would misdeliver (QF-20260728-944).`;
    logger && logger.error && logger.error(msg);
    const e = new Error(msg);
    e.code = 'DISPATCH_ROLE_TOPIC_MISMATCH';
    throw e;
  } catch (e) {
    if (e && e.code === 'DISPATCH_ROLE_TOPIC_MISMATCH') throw e; // fail CLOSED on a confirmed mismatch
    logger && logger.warn && logger.warn(`[dispatch] addressee-role check skipped (fail-open): ${e.message}`);
  }
}
 ...
 * @private
 */
async function stampReleaseRequest(supabase, row, logger = console, mergeMetadataKeysFn = null) {
  try {
    if (row.message_type !== 'WORK_ASSIGNMENT') return;
    const payload = row.payload && typeof row.payload === 'object' ? row.payload : {};
    ...
    // SD-LEO-FIX-STRATEGIC-DIRECTIVES-UPDATED-001: this used to spread held.metadata whole and
    // full-blob .update({ metadata: {...} }) -- a TOCTOU race that can silently clobber a key a
    // concurrent stamper (claim-stamp.cjs, another dispatch) wrote between this read and this
    // write. mergeMetadataKeys() writes ONLY release_request via an atomic JSONB `||` merge.
    const mergeFn = mergeMetadataKeysFn || (await import('./safe-metadata-merge.mjs')).mergeMetadataKeys;
    await mergeFn(heldSd, {
      release_request: {
...
// QF-20260829-513: default lifetime for a directed-dispatch COORDINATOR_RESERVATION fence.
// Bounds the SAME risk the acceptance criteria names -- a dispatch nobody acts on must not
// strand the SD permanently -- while comfortably exceeding a worker's normal /checkin cadence
// (~15min TTL claims, checked in well under this) so a live assignee's own claim always lands
// before the fence would expire out from under it.
const DISPATCH_RESERVATION_TTL_SECONDS = 720; // 12 minutes

/**
 * QF-20260829-513: fence a directed WORK_ASSIGNMENT's target SD against a competing self-claim
 * for the reservation's lifetime.
 *
 * THE GAP THIS CLOSES: lib/coordinator/reserve-sd.cjs (reserveSd) writes a COORDINATOR_RESERVATION
 * row consumed worker-side by drain-reservations.cjs/seat-busy-fence.cjs, but its only producer was
 * the auto-promote refill path (lib/sourcing-engine/refill-auto-promote.js softReserveLeaf) — the
 * DIRECTED dispatch path (a coordinator handing a named SD to a named seat via this very choke
 * point) wrote no fence at all. A race between the directive being sent and the assignee acting on
 * it let a different seat self-claim the SD out from under the intended assignee (live-caught:
 * SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001, session 2b9045cc lost the race and burned cycles on a
 * pool-fix for work someone else already held).
 *
 * TWO-SIDED BY CONSTRUCTION (the QF's own acceptance bar): reserves ONLY the named SD, for ONLY
 * the reservation's TTL, for ONLY this dispatch's target session — an unreserved SD, or this same
 * SD once the fence expires, remains freely self-claimable exactly as before. Reuses the EXISTING
 * writer rather than a parallel mechanism (a second soft-lock would give the fence two owners).
 *
 * Fail-soft, like every sibling stamp at this choke point: a reservation-write fault never blocks
 * the dispatch itself — the assignment still sends, just without the race-window fence.
 *
 * `reserveFn` is injectable (defaults to the real reserveSd) — same DI shape as
 * refill-auto-promote.js's softReserveLeaf(deps.reserveFn), so unit fixtures can assert the
 * wiring (which inputs call it, with what arguments) without depending on reserve-sd.cjs's own
 * live-coordinator resolution (covered separately by tests/unit/coordinator/reserve-sd.test.js).
 * @private
 */
async function stampCoordinatorReservation(supabase, row, logger = console, reserveFn = reserveSd) {
  try {
    if (row.message_type !== 'WORK_ASSIGNMENT') return;
    if (!row.target_session) return;
    const sdKey = resolveAssignmentTargetKey(row, { profile: 'dispatchStamp' });
    if (!sdKey) return; // a WORK_ASSIGNMENT with no named SD (e.g. a generic nudge) — nothing to fence
    const expiresAt = new Date(Date.now() + DISPATCH_RESERVATION_TTL_SECONDS * 1000).toISOString();
    const { error } = await reserveFn(supabase, { targetSd: sdKey, reservedForSession: row.target_session, expiresAt });
    if (error) {
      logger && logger.warn && logger.warn(`[dispatch] coordinator_reservation write skipped for ${sdKey} (fail-soft): ${error}`);
    }
  } catch (e) {
    logger && logger.warn && logger.warn(`[dispatch] coordinator_reservation stamp skipped: ${e.message}`);
  }
}

// QF-20260830-454: default lifetime for the SEAT_BUSY_RESERVATION written on every WORK_ASSIGNMENT
// dispatch. Generous vs. a directed task's typical duration; fails open (seat reads idle again)
// once expired rather than requiring an explicit clear — same posture as DISPATCH_RESERVATION_TTL.
const SEAT_BUSY_DISPATCH_TTL_SECONDS = 1800; // 30 minutes

/**
 * QF-20260830-454: give a dispatched WORK_ASSIGNMENT a representation capacity gauges can see.
 *
 * THE GAP THIS CLOSES: dispatch writes no row on either claim table, so an executing seat is
 * indistinguishable from an idle one to coordinator-idle-qf-hint.mjs's eligibleIdleWorkers() and
 * every other capacity surface keyed off claude_sessions/quick_fixes — worst on exactly the items
 * urgent enough to DISPATCH rather than mint (measured: Hotel-5, directive 98f2a4b5, released both
 * his QFs to run a dispatched AltifyAI stage-23 UAT and immediately received a competing idle hint).
 *
 * Reuses the EXISTING seat_busy_reservation payload kind (already defined + already drained by
 * lib/checkin/steps/seat-busy-fence.cjs, but with no producer anywhere in the codebase) rather than
 * a parallel representation — a second "seat is busy" mechanism would give gauges two sources to
 * reconcile. Fail-soft, like every sibling stamp at this choke point: a write fault never blocks
 * the dispatch itself.
 * @private
 */
async function stampSeatBusyReservation(supabase, row, logger = console) {
  try {
    if (row.message_type !== 'WORK_ASSIGNMENT') return;
    if (!row.target_session || !row.sender_session) return;
    const expiresAt = new Date(Date.now() + SEAT_BUSY_DISPATCH_TTL_SECONDS * 1000).toISOString();
    const { error } = await supabase.from('session_coordination').insert({
      ...
      // QF-20260901-039: session_coordination.subject is NOT NULL. This insert omitted it
      // entirely, so every attempt failed the column constraint and the seat-busy guard has
      // never actually held a reservation on any WORK_ASSIGNMENT dispatch since QF-20260830-454
      // introduced this stamp — the fail-soft catch below only ever warned, silently.
      subject: '[SEAT_BUSY_RESERVATION] executing a dispatched WORK_ASSIGNMENT',
      payload: { kind: PAYLOAD_KINDS.SEAT_BUSY_RESERVATION, reason: 'executing a dispatched WORK_ASSIGNMENT' },
      expires_at: expiresAt,
    });
    if (error) {
      logger && logger.warn && logger.warn(`[dispatch] seat_busy_reservation write skipped (fail-soft): ${error.message || error}`);
    }
  } catch (e) {
    logger && logger.warn && logger.warn(`[dispatch] seat_busy_reservation stamp skipped: ${e.message}`);
  }
}
 ...
 *
 * WRITES row.body DIRECTLY (TST-P2), and row.payload.body too WHEN row.payload already exists
 * (adversarial post-merge review, PR #8356, INFO finding: never invent row.payload on a
 * payload-less row -- this file's OTHER fill-if-absent stampers rely on some rows being
 * payload-less by design; see the two body mirrors below, payload.body->body then
 * body->payload.body). MUST run before either mirror.
 *
 * FAILS SOFT on an oversized render (TST-P1): the merged body is length-checked against
 * BODY_HARD_CAP directly (never capBodySafe over the FULL merged body -- see the WARN-1 finding
 * note inline below on why redaction is scoped to only the newly appended block). On an oversized
 * render the stamp is DROPPED (never truncated, never thrown) with a loud warning naming the SD
 * and the byte length -- the un-stamped WORK_ASSIGNMENT still inserts, matching every other
 * stamper's fail-soft contract.
 * @private
 */
async function stampConstraintsBlock(supabase, row, logger = console) {
  try {
    ...
    // SECURITY (sec-receipts-exec, evidence 3483a986; hardened post-merge-review PR #8356 finding
    // CRITICAL): resolveHoldProvenance/formatHoldProvenance already strip unsafe render chars
    // (claim-eligibility.cjs uses the SAME lib/shared/sanitize-render-text.cjs helper as this
    // file), but that protection covers ONLY the Hold line -- the three fields this stamper reads
    // directly (ratifications_cited, the solomon_* keys, forbidden_framing(s)) were rendered
    // with no equivalent stripping. A crafted metadata value could embed a Unicode line/paragraph
    // separator plus a forged "- Hold: ..." bullet (indistinguishable from a genuine sanitized
    // hold line under a `/gm` match), a bidi override char, or ANSI escape sequences that survive
    // into the rendered body. SAME shared sanitizer as claim-eligibility.cjs, applied here to
    // every field THIS function renders that resolveHoldProvenance does not already sanitize.
    const { sanitizeRenderText: sanitize } = require('../shared/sanitize-render-text.cjs');
    ...
    // SECURITY (adversarial post-merge review, PR #8356, WARNING finding): redact ONLY the newly
    // appended, metadata-sourced block -- never the caller's pre-existing body. capBodySafe's
    // redact() runs unconditionally over whatever string it is given (it was designed for the
    // worker-signal/aggregation hops, where redaction is the intended behavior); running it over
    // the FULL merged body rewrote ordinary caller-authored instruction prose containing
    // "api_key:"/"secret:"/"token:" (routine in an instructional WORK_ASSIGNMENT body) into
    // [REDACTED:CREDENTIAL] -- a silent content mutation no prior stamper on this row performed.
    const { redact, BODY_HARD_CAP } = require('../shared/body-cap.cjs');
    const safeBlock = redact(block);
    const merged = existingBody ? `${existingBody}\n\n${safeBlock}` : safeBlock;
    if (merged.length > BODY_HARD_CAP) {
    ...
    row.body = merged;
    // Adversarial post-merge review (PR #8356, INFO finding): only stamp INTO an existing payload
    // object -- never invent one on a payload-less row, matching the invariant this file's own
    // fill-if-absent stampers rely on ~100 lines below ("some rows are payload-less by design").
    if (row.payload && typeof row.payload === 'object') {
      row.payload = { ...payload, body: merged };
    }
  } catch (e) {
 ...
 * choke point, sibling to stampEffortRecommendation. FR-3: a fable-tier recommendation with no resolvable evidence
...
async function stampModelRecommendation(supabase, row, logger = console, mergeMetadataKeysFn = null) {
  try {
    ...
    const { data: sd } = await supabase
      .from('strategic_directives_v2')
      .select('title, description, scope, key_changes, sd_type, metadata')
      .eq('sd_key', sdKey)
      .maybeSingle();
    if (!sd) return;
...
// assertWorkerTierAllowed (WORK-DOWN-NEVER-UP dispatch enforcement) was deleted here per
// SD-FDBK-INFRA-RETIRE-SEAT-TIER-001, chairman ratification 20dc072b: seat-tier dispatch
// enforcement is retired fleet-wide; any seat may take any belt item. tier_rank/min_tier_rank
// stamps remain as advisory data (read by other, still-live mechanisms elsewhere in the fleet).
...
/**
 * SD-LEO-INFRA-INSERTCOORDINATIONROW-NOT-SIGNAL-001 FR-1: the single predicate for "this
 * outcome is a genuine delivery, not a failure" across both the DISPATCH_BACKPRESSURE and
 * DISPATCH_ALREADY_DELIVERED shapes (returned objects post-fix; also accepts a caught exception
 * defensively during any caller's migration window, since the shape is identical either way).
 *
 * DELIBERATELY VALUE-KEYED (checks `landed === true`), NEVER CODE-KEYED (checking `.code`
 * membership alone). DISPATCH_BACKPRESSURE's `landed` flag is VALUE-COMPUTED (dispatch.cjs's own
 * `e.landed = parkedRowId != null`): the backpressure park insert is best-effort and can fail
 * (logged as BACKPRESSURE_PARK_FAILED/BACKPRESSURE_PARK_THREW, "content may be lost"), which
 * yields `landed: false` on a MATCHING code. A code-keyed predicate would misreport that
 * genuinely lost message as delivered — worse than the defect this SD fixes (today a lost
 * message at least throws loudly). DISPATCH_ALREADY_DELIVERED is unconditionally landed:true,
 * so this same check covers it too without a second branch.
 *
 * @param {{landed?: boolean}|Error|null|undefined} errorOrResult
 * @returns {boolean}
 */
function isDeliveredDispatchError(errorOrResult) {
  return !!(errorOrResult && errorOrResult.landed === true);
}

/**
 * QF-20260831-560: real send backpressure, replacing the copy-pasted-x7 scratch-script
 * conditional (Solomon disposition: the heuristic is sound, the hand-rolled substitute is not).
 * Refuses a routine directed send when the target already has >= BACKPRESSURE_UNANSWERED_LIMIT
 * unacknowledged, unexpired directed rows — UNLESS payload.kind is in BACKPRESSURE_EXEMPT_KINDS,
 * in which case the send proceeds and the exemption is logged LOUDLY (a quiet exemption is how
 * the guard's own exempt lane becomes the new unguarded default path).
 */
async function assertSendBackpressure(supabase, row, logger = console) {
  try {
    if (!row.target_session) return;
    const kind = row.payload && typeof row.payload === 'object' ? row.payload.kind : null;
    if (kind && BACKPRESSURE_EXEMPT_KINDS.has(kind)) {
      const warn = (logger && logger.warn) || console.warn;
      warn(`[dispatch] BACKPRESSURE_EXEMPT: kind='${kind}' bypasses the unanswered-directed-row limit for target ${row.target_session}.`);
      return;
    }
    // QF-20260901-047: the advisory lanes (Adam/Solomon) carry a correction as payload.message_kind
    // (a sub-discriminator on the adam_advisory leg) while payload.kind is force-stamped to
    // 'adam_advisory' — so a retraction/amend/supersede never matched the kind-keyed exemption above
    // and was parked behind the cap like a routine send. Keyed on CORRECTION_KIND_SET specifically,
    // never the full MESSAGE_KIND_SET, so 'disposition' never exempts itself (message-kinds.cjs's own
    // documented trap) and the CLI cannot self-grant a bypass via --kind (Solomon's explicit ruling).
    const messageKind = row.payload && typeof row.payload === 'object' ? row.payload.message_kind : null;
    if (messageKind && CORRECTION_KIND_SET.has(messageKind)) {
      const warn = (logger && logger.warn) || console.warn;
      warn(`[dispatch] BACKPRESSURE_EXEMPT: message_kind='${messageKind}' bypasses the unanswered-directed-row limit for target ${row.target_session}.`);
      return;
    }
    // QF-20260902-962: the WORKER_SIGNAL friction lane (scripts/worker-signal.cjs) keys on
    // payload.signal_type — exactly the same "kind-only scheme misses the friction channel"
    // trap lib/fleet/worker-status.cjs's classifyCoordinationRow already documents and defends
    // against on the READ side. This exemption keys on signal_type ON PURPOSE (not on kind
    // absence): as of SD-LEO-INFRA-LANE-HYGIENE-MACHINE-WRITERS-001, worker-signal.cjs ALSO
    // stamps payload.kind='worker_signal' beside signal_type (additive, registered in
    // DRAIN_SETS), so a WORKER_SIGNAL row is no longer kind-less — but this exemption must stay
    // keyed on signal_type regardless, since that is the field that actually identifies a
    // friction report. A friction report (stuck/need-sweep/prd-ambiguous/gate-bug/spec-conflict/
    // harness-bug/feedback/unfit/other) is the mechanism that prevents a bypass, so it must
    // never be throttled by the coordinator's own backlog — a refused signal was witnessed
    // becoming a --bypass-validation false completion instead.
    const signalType = row.payload && typeof row.payload === 'object' ? row.payload.signal_type : null;
    if (signalType) {
      const warn = (logger && logger.warn) || console.warn;
      warn(`[dispatch] BACKPRESSURE_EXEMPT: signal_type='${signalType}' bypasses the unanswered-directed-row limit for target ${row.target_session}.`);
      return;
    }
    // QF-20260831-769: count only genuinely UNANSWERED rows. Two classes were previously
    // counted despite never being "unanswered" in any meaningful sense: (1) rows whose own
    // payload.kind is itself backpressure-exempt (the exemption only guarded the OUTGOING
    // side, not the counted population — Adam's exempt sends were filling the coordinator's
    // cap and choking Adam's own routine lane harder the more urgent traffic flowed); (2) a
    // solicited correlated REPLY (payload.kind='coordinator_reply' or payload.reply_to set)
    // — a reply IS an answer, not a fresh unanswered ask.
    // count-truncation-diff-lint: explicitly bounded, not the PostgREST default cap. 200 is
    // far above any observed peak (measured fleet-wide 24h peak against one target: 44) and
    // still provably correct against BACKPRESSURE_UNANSWERED_LIMIT=3 — a truncated fetch here
    // can only ever OVER-count toward the refuse verdict, never under-count into a false ADMIT.
    const { data: candidateRows, error } = await supabase
      ...
      .limit(200);
    if (error) {
      logger && logger.warn && logger.warn(`[dispatch] backpressure check skipped (fail-open): ${error.message}`);
      return;
    }
    const count = (candidateRows || []).filter((r) => {
      const p = r && r.payload;
      const k = p && typeof p === 'object' ? p.kind : null;
      // QF-20260904-695: a high-volume informational sender (roll_call chief among them) never
      // sets acknowledged_at by design -- left uncounted, these permanently occupy the cap and
      // block every OTHER routine send to the same target regardless of real backlog.
      if (k && INFORMATIONAL_KINDS_SET.has(k)) return false;
      if (k && BACKPRESSURE_EXEMPT_KINDS.has(k)) return false;
      const mk = p && typeof p === 'object' ? p.message_kind : null;
      if (mk && CORRECTION_KIND_SET.has(mk)) return false;
      // QF-20260902-962: mirror the outgoing exemption above into the counted population --
      // an exempt friction signal already sitting unanswered must not itself count toward
      // refusing the NEXT send, the same class of gap QF-20260831-769 fixed for kind-exempt rows.
      const st = p && typeof p === 'object' ? p.signal_type : null;
      if (st) return false;
      if (p && typeof p === 'object' && (k === 'coordinator_reply' || (p.reply_to != null && p.reply_to !== ''))) return false;
      // QF-20260901-023: a parked row is queued-for-later, not live pressure -- excluding it
      // from the count is what lets the cap actually clear once the target answers something.
      if (p && typeof p === 'object' && p.backpressure_parked === true) return false;
      return true;
    }).length;
    if (count >= BACKPRESSURE_UNANSWERED_LIMIT) {
      // QF-20260901-023: PARK, never discard. A refused send previously vanished with no
      // queryable trace -- the sender's content is durably written here (readable by the
      // recipient immediately, backpressure only ever gated the SEND, never the READ) instead
      // of being lost. Best-effort: if the park write itself fails, log loudly but still fail
      // the caller closed below (matches this guard's existing fail-closed contract).
      //
      // QF-20260901-039: PERSIST IS NOT RETURN, part 2 (measured live specimen, coordinator
      // directive 415d36d9/2135b402 thread: insert to 0dbe6227 threw DISPATCH_BACKPRESSURE yet
      // row e3a2d454 landed and was read by the target; a second specimen produced duplicate
      // rows 24cf1eb8/7b19e7e0). The insert above genuinely lands the content, but the
      // subsequent unconditional throw gave a caller no way to tell "parked, already landed,
      // do not resend" apart from "nothing landed, safe to retry" -- a caller that resends on
      // ANY thrown error duplicates a row that was never lost. Stamp the parked row's id and a
      // `landed:true` flag onto the thrown error so a caller can branch on it instead of
      // guessing.
      let parkedRowId = null;
      try {
        const parked = { ...row, payload: { ...(row.payload || {}), backpressure_parked: true, backpressure_parked_at: new Date().toISOString() } };
        const { data: parkedData, error: parkErr } = await supabase.from('session_coordination').insert(parked).select('id').single();
        if (parkErr) {
          logger && logger.warn && logger.warn(`[dispatch] BACKPRESSURE_PARK_FAILED (content may be lost): ${parkErr.message}`);
        } else {
          parkedRowId = parkedData && parkedData.id;
        }
      } catch (parkEx) {
        logger && logger.warn && logger.warn(`[dispatch] BACKPRESSURE_PARK_THREW (content may be lost): ${parkEx.message}`);
      }
      // QF-20260904-695: dropped the exempt-kind/correction-kind coaching -- it read as a
      // bypass invitation printed on every refusal (a sender steered toward an exempt kind
      // or a "correction" message_kind just to get through, when the row was never lost).
      const e = new Error(
        `[dispatch] DISPATCH_BACKPRESSURE: target ${row.target_session} already has ${count} unanswered directed row(s) (limit ${BACKPRESSURE_UNANSWERED_LIMIT}) — the row was inserted (PARKED, not lost${parkedRowId ? `, id=${parkedRowId}` : ''}) and will be delivered normally; the target is simply at ${count} unanswered right now.`
      );
      e.code = 'DISPATCH_BACKPRESSURE';
      e.landed = parkedRowId != null;
      e.parkedRowId = parkedRowId;
      throw e;
    }
  } catch (e) {
    if (e && e.code === 'DISPATCH_BACKPRESSURE') throw e; // fail CLOSED on a confirmed breach
    logger && logger.warn && logger.warn(`[dispatch] backpressure check skipped (fail-open): ${e.message}`);
  }
}

/**
 * QF-20260902-160: dedupe a retry on the SAME correlation_id instead of inserting a second row.
 * A caller that reads a DISPATCH_BACKPRESSURE/park refusal as "nothing landed" and resends the
 * identical ask duplicates content that already landed (parked or real) — measured live: Adam's
 * "supersedes my parked X" resends raised his own target's unanswered count and re-tightened the
 * same cap. Deliberately excludes multi-part sends (payload.part_index set): those legitimately
 * reuse one correlation_id across several distinct rows (adam-advisory.cjs --part N/M) and a
 * reply/coordinator_reply row (an ANSWER, not a duplicate ask). Fail-open on any lookup fault —
 * a dedupe-check fault must never block a genuine send.
 * @returns {Promise<string|null>} the existing row's id, or null if no dedupe applies
 * @private
 */
async function findExistingCorrelationRow(supabase, row, logger = console) {
  try {
    const payload = row.payload && typeof row.payload === 'object' ? row.payload : null;
    const corr = payload && payload.correlation_id;
    if (!corr || !row.target_session || payload.part_index != null) return null;
    const { data, error } = await supabase
      .from('session_coordination')
      .select('id, payload')
      .eq('target_session', row.target_session)
    ...
    const existing = data.find((r) => {
      ...
      if (p.kind === 'coordinator_reply') return false;
      if (p.reply_to != null && p.reply_to !== '') return false;
      if (p.part_index != null) return false;
      return true;
    });
    return existing ? existing.id : null;
  } catch (e) {
    logger && logger.warn && logger.warn(`[dispatch] correlation dedupe check skipped (fail-open): ${e.message}`);
    return null;
  }
}

/**
 * QF-20260912-283: persist an unresolvable-target occurrence to audit_log, durably, so the
 * "evidenced review of that window's output" the two guards above require before promotion to
 * binding has a queryable dataset instead of console.warn output scattered across whichever
 * process happened to be tailed. Measured: the diagnostic for the row that motivated this fix
 * (5ff49e27, event dispatch.directed_target_unresolvable) was already being emitted hours before
 * anyone connected it to a pattern spanning five seat-occurrences in one day — because it only
 * ever reached stdout.
 * Fire-and-forget: never blocks or fails the send; write failures route through the shared
 * audit-write-guard so a silent-drop is at least visible in logs, mirroring
 * scripts/modules/learning/index.js's emitFilterBypassAudit.
 */
async function auditUnresolvableTarget(supabase, eventType, metadata) {
  const payload = {
    event_type: eventType,
    entity_type: 'session_coordination_dispatch',
    entity_id: null,
    severity: 'warning',
    created_by: 'dispatch.cjs',
    metadata,
  };
  try {
    ...
    if (error) {
      ...
      logAuditWriteFailure('dispatch.auditUnresolvableTarget', error, payload);
    }
  } catch (err) {
    console.warn(`[audit] ${eventType} audit skipped (non-blocking): ${err.message}`);
  }
}

async function insertCoordinationRow(supabase, row, opts = {}) {
  const { logger = console, select = null, single = false, topicId = null, urgency = null } = opts;
  ...
  // SD-LEO-FIX-COORDINATOR-RULING-REVERSES-001 (FR-1): opts.urgency is additive and optional —
  // a caller that omits it produces a byte-identical row to before this change. Fail-open on an
  // unrecognized value (warn, still write it) rather than throw: this is the shared chokepoint for
  // every coordinator send (fleet-dashboard.cjs, capped-pool-broadcast.cjs, amend-sd.js), so a
  // throwing validation here has fleet-wide blast radius. Matches coordination-inbox.cjs's own
  // fail-open try/catch around its urgentRows fetch.
  if (urgency != null) {
    if (!isKnownUrgency(urgency)) {
      // SECURITY review (EXEC-TO-PLAN, c605ec77): JSON.stringify can itself throw (BigInt,
      // circular refs, a throwing toJSON()) — this warn must never be the reason a genuinely
      // fail-open path throws, and a pathological value must not blow up the log line either.
      let shown;
      try {
        shown = JSON.stringify(urgency);
      } catch {
        shown = '<unserializable>';
      }
      logger && logger.warn && logger.warn(
        `[dispatch] opts.urgency=${String(shown).slice(0, 200)} is not a recognized urgency level — ` +
        'writing it anyway (fail-open), but coordination-inbox.cjs will not treat it as an interrupt.'
      );
    }
    const payload = row.payload && typeof row.payload === 'object' ? row.payload : {};
    row.payload = { ...payload, urgency };
  }
  ...
  // QF-20260729-716: CLAIM_REMINDER/STALE_WARNING are valid coordination_message_type enum
  // members this choke otherwise accepts without complaint, but scripts/worker-checkin.cjs's
  // isCoordinatorPush() never surfaces either -- /checkin is the ONLY path that acks, so a row
  // sent on either type can never be acked by any path (measured: zero hits in
  // worker-checkin.cjs; confirmed live -- a CLAIM_REMINDER row sat 78.7 minutes unread on an
  // actively-ticking seat while a same-body INFO+coordinator_request row on the same inbox was
  // read in 11.9 minutes). It becomes permanent, unretirable residue in every report built on
  // "acknowledged_at IS NULL" -- refuse at send, the only point in the lifecycle where this row
  // can still be prevented, rather than let it become immortal and wrong. Direct raw inserts
  // (scripts/stale-session-sweep.cjs) bypass this function by design and are unaffected --
  // that sweep-authored path is a deliberately different, PostToolUse-hook-consumed lane.
  if (UNDRAINABLE_WORKER_MESSAGE_TYPES.includes(row.message_type)) {
    const e = new Error(
      `[dispatch] message_type='${row.message_type}' is not drainable by any worker /checkin path ` +
      "(scripts/worker-checkin.cjs's isCoordinatorPush() never surfaces it, so it can never be " +
      "acked) — refusing. Use message_type='INFO' with payload.kind='coordinator_request' (or " +
      "'coordinator_reminder') instead, which IS surfaced and consumed via /checkin."
    );
    e.code = 'DISPATCH_UNDRAINABLE_MESSAGE_TYPE';
    throw e;
  }
  ...
  if (unreadable) {
    ...
    const binding = isAssignmentTargetGuardBinding();
    await auditUnresolvableTarget(supabase, 'dispatch.assignment_target_unresolvable', {
      mode: binding ? 'blocked' : 'observe_only',
  ...
  // QF-20260907-463: DISTINCT from the unreadable check above (broader 'worker' profile) — a row
  // can be worker-readable (text scan finds a key) yet still fail the narrower 'directed' profile
  // extractDirectedSd actually uses, making it invisible to the fast/priority claim path. Measured
  // 58% of live WORK_ASSIGNMENT rows; one specimen sat unclaimed 12 hours despite naming its
  // target in prose. Own rollout switch (isDirectedTargetGuardBinding) — see its own docstring.
  const undirectable = describeUndirectableAssignment(row);
  if (undirectable) {
    const msg = `[dispatch] WORK_ASSIGNMENT has no structured directed target — ${undirectable.detail}.`;
    const directedBinding = isDirectedTargetGuardBinding();
    await auditUnresolvableTarget(supabase, 'dispatch.directed_target_unresolvable', {
      mode: directedBinding ? 'blocked' : 'observe_only',
      target_session: row.target_session,
      subject: String(row.subject || '').slice(0, 120),
      detail: undirectable.detail
    });
    if (directedBinding) {
      const e = new Error(msg + ' Refusing (DISPATCH_DIRECTED_TARGET_UNRESOLVABLE). Add payload.sd_key/target_sd, '
        + 'or stamp payload.kind=\'completion_nudge\'/payload.informational=true if this is a broadcast, not a directed assignment.');
      e.code = 'DISPATCH_DIRECTED_TARGET_UNRESOLVABLE';
      throw e;
    }
    logger && logger.warn && logger.warn(JSON.stringify({
      event: 'dispatch.directed_target_unresolvable',
      mode: 'observe_only',
      target_session: row.target_session,
      subject: String(row.subject || '').slice(0, 120),
      detail: undirectable.detail
  ...
  await assertValidTarget(supabase, row.target_session, logger, {
    isWorkAssignment: row.message_type === 'WORK_ASSIGNMENT',
    allowBusyTarget: !!opts.allowBusyTarget,
  });
  // QF-20260728-944 (chairman-approved 2026-07-28): a role-scoped message must not be deliverable
  // to a session resolving to a DIFFERENT role. Chairman-widened scope after THREE reminder-lane
  // occurrences (solomon_responsibilities -> an Adam session) plus a solomon_consult-lane
  // mis-delivery in the same window (a two-part consult Solomon never received). INSERT-SIDE
  // precondition, not a reader-side check: session_coordination has no updated_at, so once
  // written, wrong-at-insert and retargeted-after-insert are indistinguishable — the write is the
  // only point where this is checkable at all. Fires ONLY on a CONFIDENT mismatch (target role
  // resolvable AND differs from the declared role) — same precision-over-recall posture as the
  // sibling kind-vs-drain-set warn check below, promoted to fail-CLOSED per the chairman's explicit
  // approval (that one stays warn-only; this one is authorized to block).
  await assertAddresseeRoleMatch(supabase, row, opts, logger);
  ...
  // SD-LEO-INFRA-COMPLEXITY-TIERED-WORKER-ASSIGNMENT-001 (FR-4)'s WORK-DOWN-NEVER-UP dispatch
  // enforcement call was here; retired by SD-FDBK-INFRA-RETIRE-SEAT-TIER-001 (ratification 20dc072b).
  ...
  // to the two stamps above — MUST run before the body/payload.body mirrors below. It always
  // writes row.body directly; row.payload.body only when row.payload already exists (round-2
  // post-merge review fix: never invent a payload on a payload-less row — see the function's own
  // docblock). Both mirrors are themselves payload-guarded the same way, so ordering relative to
  // them is still irrelevant — but this must precede any caller-visible use of row.body for THIS
  // insert regardless.
  ...
  // QF-20260829-513: fence the directed WORK_ASSIGNMENT's target SD for the dispatch-to-claim
  // race window — the directed path was the one census surface with a live consumer and no producer.
  await stampCoordinatorReservation(supabase, row, logger);
  // QF-20260830-454: give the dispatch itself a capacity-gauge-visible representation.
  await stampSeatBusyReservation(supabase, row, logger);
  ...
  // QF-20260902-160: a retry sharing the SAME correlation_id as an already-landed row (parked or
  // real) is DELIVERED, not refused. MUST run BEFORE the fill-if-absent stamp below — a bare send
  // with no caller-supplied correlation_id gets a FRESH random one on every attempt, so it could
  // never dedupe-match a prior attempt if this ran after the stamp; only a genuinely repeated
  // correlation_id (the caller's own explicit retry marker) is ever dedupe-eligible.
  const dupeId = await findExistingCorrelationRow(supabase, row, logger);
  if (dupeId) {
    // SD-LEO-INFRA-INSERTCOORDINATIONROW-NOT-SIGNAL-001 FR-1: a delivered outcome is signalled
    // by an ADDITIVE RETURN, not a throw — a caller that treated any thrown error as "nothing
    // landed" resent content that was never lost. Not behind a separately-exported guard (unlike
    // assertSendBackpressure below), so this converts directly at its own site. Never restructure
    // {data,error} — only add fields — since durable callers depend on that shape unchanged.
    logger && logger.warn && logger.warn(
      `[dispatch] DISPATCH_ALREADY_DELIVERED: target ${row.target_session} already holds a DELIVERED row for this correlation (id=${dupeId}) — not resending.`
    );
    return { data: null, error: null, landed: true, parkedRowId: dupeId, code: 'DISPATCH_ALREADY_DELIVERED' };
  }
  ...
  // SD-LEO-INFRA-SEND-TIME-TARGET-001 / FR-2: kind-vs-target-drain-set contract. Fires only on
  // a CONFIDENT mismatch: resolvable target role + a non-terminal payload.kind absent from that
  // role's recognized set. Callers that know their target's role statically pass
  // opts.targetRoleHint (e.g. adam-advisory `--to solomon` -> 'solomon') so a UUID target needs
  // no identity lookup. Any AMBIGUOUS case (unresolvable role, registry lookup error) stays
  // fail-open — refusing on uncertainty would dead-letter legitimate sends on a flaky read.
  //
  // QF-20260913-426: WARN -> REFUSE tightening. SD-LEO-INFRA-SEND-TIME-TARGET-001's own
  // docblock named this the explicit follow-on "after burn-in" — a warning on a fire-and-forget
  // lane is indistinguishable from success to a reader who only checks the id line, so two
  // real sends orphaned silently (measured at the Adam seat, 2026-09-13). A CONFIDENT mismatch
  // now REFUSES the insert (throw, never lands) and names one recognized kind that would work,
  // matching this function's existing REFUSED-insert convention (assertValidTarget et al.)
  // rather than adding a second refusal idiom.
  try {
    const kind = row.payload && typeof row.payload === 'object' ? row.payload.kind : null;
    if (kind) {
      const targetRole = await resolveTargetRole(supabase, row.target_session, opts.targetRoleHint);
      const { checkKindDrainedByRole } = await import('../fleet/drain-set-registry.js');
      const check = await checkKindDrainedByRole({ supabase, targetRole, kind });
      if (check.applicable && !check.drained) {
        const alt = check.recognized[0];
        const msg = `[dispatch] REFUSED insert: kind '${kind}' is not in role '${targetRole}' drain set `
          + `(recognized: ${check.recognized.join(', ') || 'none'}) — this delivery would orphan at the `
          + `target, unread by construction.`
          + (alt ? ` Use '${alt}' (or another recognized kind) instead.` : '');
        logger && logger.error && logger.error(msg);
        const e = new Error(msg);
        e.code = 'DISPATCH_UNDRAINED_KIND';
        throw e;
      }
    }
  } catch (e) {
    if (e && e.code === 'DISPATCH_UNDRAINED_KIND') throw e; // fail CLOSED on a confident mismatch
  ...
  // QF-20260728-642: fill-if-absent default TTL, same precedent as the body/correlation_id
  // mirrors above (never overwrites a caller-supplied value). Without an explicit expires_at,
  // the session_coordination.expires_at column DEFAULT is only 60 minutes
  // (`now() + '01:00:00'::interval`) — every coordinator dispatch through this choke point
  // silently inherited that short window while the advisory lane (scripts/adam-advisory.cjs
  // advisoryExpiresAt) has used a 24h durable TTL since SD-LEO-INFRA-ADAM-ADVISORY-COMMS-001.
  // Measured live: a directed WORK_ASSIGNMENT whose own body said "this row CANNOT reach you
  // through self-claim; it has to be handed to you" expired unread in one hour, twice.
  const DISPATCH_TTL_MS = 24 * 60 * 60_000; // 24h — matches the advisory lane's durable window
  if (row.expires_at == null) {
    row.expires_at = new Date(Date.now() + DISPATCH_TTL_MS).toISOString();
  }
  // QF-20260831-560: backpressure ahead of the disposition-lock guard — a choked send should
  // never even reach the correlation check.
  //
  // SD-LEO-INFRA-INSERTCOORDINATIONROW-NOT-SIGNAL-001 FR-1: assertSendBackpressure itself stays
  // UNCHANGED and keeps throwing — it is separately exported with its own ~20 tests, and other
  // callers of it directly must see no behavior change. Only THIS call site converts a confirmed
  // delivery (isDeliveredDispatchError(e), i.e. landed===true — the park insert actually
  // succeeded) into an additive return. A FAILED park (landed:false, dispatch.cjs's own
  // BACKPRESSURE_PARK_FAILED/BACKPRESSURE_PARK_THREW warns) means the message is genuinely
  // lost and MUST keep propagating as a throw — converting that case too would silently drop
  // content, which is worse than today's defect (today a lost message at least throws loudly).
  try {
    await assertSendBackpressure(supabase, row, logger);
  } catch (e) {
    if (e && e.code === 'DISPATCH_BACKPRESSURE' && isDeliveredDispatchError(e)) {
      return { data: null, error: null, landed: true, parkedRowId: e.parkedRowId, code: e.code };
    }
    throw e;
  }
  ...
  // SD-LEO-INFRA-OPEN-COMMITMENTS-RECONCILED-001 / FR-3: a declared commitment (body carries a
  // [COMMIT: ...] tag) is written to the durable commitments table alongside the
  // session_coordination send. Fail-soft — never affects the row already inserted above.
  //
  // QF-20260901-039: PERSIST IS NOT RETURN. By this point the row has already landed (res.error
  // is falsy) — the caller's own request is DONE regardless of what happens next. An unguarded
  // `await` on a post-insert side effect would let any exception it raises propagate up and be
  // mistaken by the caller for "the send itself failed" (several callers in this codebase catch
  // insertCoordinationRow and log a THREW/FAILED outcome indistinguishable from a genuine
  // non-landing), risking a needless resend and duplicate rows for work that already landed.
  // writeCommitmentIfDeclared is itself internally fail-soft today, but this outer guard is the
  // one place that holds regardless of how that function is refactored later — belt AND braces
  // on the exact "landed row reported as a fresh failure" defect class this QF exists to close.
  if (!(res && res.error)) {
    try {
      const { writeCommitmentIfDeclared } = require('./commitment-writer.cjs');
      await writeCommitmentIfDeclared(supabase, row, logger);
    } catch (e) {
      logger && logger.warn && logger.warn(`[dispatch] post-insert commitment write failed (row already landed, not re-throwing): ${(e && e.message) || e}`);
    }
  }
  return res;
}
...
module.exports = {
  ...
  assertSendBackpressure, // QF-20260831-560 — exported for the choke-guard fixtures
  isDeliveredDispatchError, // SD-LEO-INFRA-INSERTCOORDINATIONROW-NOT-SIGNAL-001 FR-1 — exported for callers and unit fixtures
  findExistingCorrelationRow, // QF-20260902-160 — exported for unit fixtures
  BACKPRESSURE_EXEMPT_KINDS, // QF-20260831-560 — exported for unit fixtures
  BACKPRESSURE_UNANSWERED_LIMIT, // QF-20260831-560 — exported for unit fixtures
  declaredAddresseeRole, // QF-20260728-944 — exported for unit fixtures
  assertAddresseeRoleMatch, // QF-20260728-944 — exported for the choke-guard fixtures
};
