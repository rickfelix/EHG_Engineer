...
async function insertCoordinationRow(supabase, row, opts = {}) {
  ...
  if (dupeId) {
    const e = new Error(
      `[dispatch] DISPATCH_ALREADY_DELIVERED: target ${row.target_session} already holds a DELIVERED row for this correlation (id=${dupeId}) — do not resend.`
    );
    e.code = 'DISPATCH_ALREADY_DELIVERED';
    e.landed = true;
    e.parkedRowId = dupeId;
    throw e;
  }
