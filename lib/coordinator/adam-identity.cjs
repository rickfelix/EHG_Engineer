...
const ADAM_TOOL_SILENCE_CUT_FLOOR_MINUTES = 15;
...
/** True unless status is a KNOWN non-fresh-eligible value. Absent status is treated as
 ...
 *
 *  SECURITY evidence 46d5f420 (SEC-01): "absent" must cover BOTH `undefined` (JS callers/fixtures
 *  that never set the field) AND `null` (PostgREST's actual wire value for a genuinely NULL DB
 *  column — the status column is nullable and its CHECK constraint is satisfied by NULL). The
 *  original `status === undefined` check missed the null case, which meant it excluded (not
 *  deferred on) a NULL-status row from "fresh" — a fail-open widening of decideSingleAdamGuard's
 *  permissiveness toward retiring/re-registering, opposite of this field's own stated intent. Loose
 *  `== null` is intentional here (matches undefined AND null in one check) — do not "fix" to `===`. */
function isStatusFreshEligible(status) {
  return status == null || ADAM_FRESH_ELIGIBLE_STATUSES.has(status);
}
 ...
 * isKnownWedged and measured wrong there too -- see scripts/adam-register.cjs; FR-3 shipped with
 * no functional change instead.)
...
function isFreshAndActive(candidate, nowMs, freshMs) {
  ...
  const cutPointMinutes = Math.max(freshMs / 60000, ADAM_TOOL_SILENCE_CUT_FLOOR_MINUTES);
  try {
    ...
    // evidence d900a26a-8d14-48c4-ae35-def914e341e7): AC-3's original "UNKNOWN folds to NOT-fresh"
    // was WRONG, proven by a pre-existing, unmodified test (scripts/solomon-register.test.js's
    // "a FRESH prior Solomon => REFUSED") this SD had not yet run. THIS function evaluates a
    // session's OWN PEERS at registration time -- a freshly-registered Adam/Solomon that has not
    // yet made its first tool call legitimately has last_tool_at=null (UNKNOWN), exactly the same
    // "just spawned" shape checkNewSessionHealth already had to fold open for (F-1). Folding
    // UNKNOWN to NOT-fresh here meant a second registration racing in during that window would
    // incorrectly clear a genuinely-alive, seconds-old singleton. Unified with checkNewSessionHealth:
    // only a POSITIVELY CONFIRMED STUCK verdict fails; UNKNOWN passes exactly like HEALTHY. The
    // FR-6 Adam-673db833 specimen is unaffected -- it has a REAL, old last_tool_at (STUCK), not an
    // absent one (UNKNOWN).
    ...
    console.error(
      `[adam-identity] FR-2 tool-activity check failed for ${candidate && candidate.session_id}: ` +
...
async function fetchFreshAdams(supabase, { nowMs = Date.now(), freshMs = ADAM_FRESH_MS } = {}) {
  ...
  try {
    ...
    //
    // ADVERSARIAL REVIEW (PR #7369, INFO): `.in('status', [...])` alone disagrees with
    // isStatusFreshEligible(status) on NULL -- SQL's `status IN (...)` evaluates to NULL (never
    // true) for a NULL status, silently excluding it here, while the pure predicate deliberately
    // treats NULL as fresh-eligible (SECURITY evidence 46d5f420). `.or()` restores agreement: a
    // NULL status matches via `status.is.null` even though `.in()` alone would never match it.
    const data = await fapPaginate(() => supabase
      .from('claude_sessions')
      .select('session_id, heartbeat_at, metadata, hostname, pid')
      .gte('heartbeat_at', cutoff)
      .or('status.is.null,status.in.(active,idle,stale)')
...
async function fetchAllAdamsStrict(supabase) {
  ...
  try {
    ...
    const data = await fapPaginate(() => supabase
      .from('claude_sessions')
      .select('session_id, heartbeat_at, metadata, status, last_tool_at, hostname, pid')
      .filter('metadata->>role', 'eq', ADAM_ROLE)
      .order('session_id')); // unique-key tiebreaker for stable pagination
 ...
 *     landed in "stale" (F-7) so callers can re-validate racing-restart protection correctly —
 *     see the SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-C comment below.
 ...
 */
function decideSingleAdamGuard({ priorAdams, selfSessionId, nowMs = Date.now(), freshMs = ADAM_FRESH_MS, isProcessDead = () => false }) {
  ...
  // dead) so this stays pure; scripts/adam-register.cjs supplies lib/coordinator/role-seat-liveness.cjs
  // isSeatProcessDead, sound in the DEAD direction only (ESRCH) -- a live or recycled pid falls
  // through to the heartbeat classification below unchanged, so this can never kill a live Adam.
  const deadProcess = others.filter((a) => { try { return isProcessDead(a) === true; } catch { return false; } });
  const fresh = others.filter((a) => !deadProcess.includes(a) && isStatusFreshEligible(a.status) && isFreshAndActive(a, nowMs, freshMs));
  if (fresh.length > 0) {
    return {
      action: 'refuse',
      retire: [],
  ...
  // TWO DIFFERENT REASONS a prior lands in "stale," and the register script's retire re-check
  // must treat them differently. A prior with a genuinely OLD heartbeat is a candidate for the
  // "raced back to fresh since the decision" racing-restart protection (heartbeat re-check is the
  // right question). A prior with a STILL-FRESH heartbeat that failed only the tool-activity check
  // (isFresh(heartbeat)===true but isFreshAndActive===false -- a confirmed-STUCK heartbeating
  // shell) can NEVER "race back to fresh" by heartbeat, because its heartbeat was never the
  // discriminator and keeps ticking regardless -- re-checking heartbeat alone would (and, measured
  // pre-fix, DID) skip it forever, leaving a dead shell permanently role-tagged. Split the retire
  // set so the register script can re-validate each half with the question that actually applies
  // to it.
  // QF-20260905-201: a third bucket. A dead-process prior's heartbeat is frozen FRESH by construction
  // (nothing will ever age it inside the window), so the register script must re-validate it on the
  // PROCESS, never on heartbeat or tool activity -- keep it out of both older buckets.
