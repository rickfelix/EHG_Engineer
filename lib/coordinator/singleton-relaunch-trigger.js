...
const { getActiveMichaelId } = require('./michael-identity.cjs'); // SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A
...
const ROLE_ID_RESOLVERS = {
  ...
  michael: (supabase) => getActiveMichaelId(supabase),
...
export async function writeScheduleRecord(supabase, { role, senderSession, freshness, fleetActivity, targetLoopState, reason }) {
  ...
  // eslint-disable-next-line session-coordination-insert-classguard/no-raw-session-coordination-insert -- pre-existing site (part of the classguard's documented backlog), swept into this diff's scan only because SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A adds 'michael' to SINGLETON_ROLES/ROLE_ID_RESOLVERS above. target_session is the freshly-resolved coordinator id or its documented sentinel, never an echoed field.
  const { data, error } = await supabase
    .from('session_coordination')
    .insert({
