 ...
 *
 * QF-20260911-004: the fence column (session_coordination.target_sd, a plain string) has
 * never actually been SD-only -- it holds whatever key claim-eligibility.cjs's
 * coordinatorReservation() is asked to match against, which since this QF also accepts a
 * QF id (e.g. "QF-20260911-004"), not only an "SD-..." key. Renamed to reserveWorkItem to
 * stop the name implying a constraint the column never enforced; reserveSd is kept as a
 * thin alias so every existing SD caller/test is unaffected.
 *
 ...
 *   Promise<string|null> }} opts - targetSd is the fenced work item's key: an SD key or a
 *   QF id. expiresAt is an ISO-8601 string; the caller decides the reservation's lifetime,
 *   this writer never invents a default. resolveCoordinatorId defaults to the real
 *   getActiveCoordinatorId; override only for tests.
 * @returns {Promise<{ data: object|null, error: string|null }>}
 */
async function reserveWorkItem(supabase, { targetSd, reservedForSession, reservedForTier, lanePattern, expiresAt, resolveCoordinatorId } = {}) {
...
// Thin alias: every existing SD caller/test keeps working under the pre-QF-20260911-004 name.
const reserveSd = reserveWorkItem;

module.exports = { buildCoordinatorReservationPayload, reserveWorkItem, reserveSd };
