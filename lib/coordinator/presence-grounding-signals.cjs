...
async function getReadReceipts(supabase, sessionId, { sinceMs = DEFAULT_RECEIPT_WINDOW_MS, limit = 50 } = {}) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .limit(limit);
    if (!error && Array.isArray(data)) live = data;
  } catch { /* fail-open */ }

  // QF-20260727-554: cleanup_expired_coordination archives-and-deletes an acked, expired row
  // within SECONDS (measured live: 14s). retention_archive.row_data is the full to_jsonb() of
  // the original row, so a read-then-promptly-acked receipt is not lost, only unqueried here —
  // read it back rather than under-reporting. row_timestamp is populated from expires_at, NOT
  // created_at (SD-LEO-INFRA-RETENTION-DESTROYS-REPLY-001 FR-4), so the bound below is
  // deliberately widened by one expiry interval + slack rather than filtered on created_at.
  let archived = [];
  try {
    const coarseIso = new Date(Date.now() - sinceMs - 2 * 3600 * 1000).toISOString();
    const { data, error } = await supabase
    ...
    if (!error && Array.isArray(data)) {
      archived = data
        .map((row) => row && row.row_data)
        .filter((d) => d && d.id && d.sender_session === sessionId && d.read_at && d.created_at >= sinceIso)
        .map((d) => ({ id: d.id, target_session: d.target_session, read_at: d.read_at, created_at: d.created_at, subject: d.subject }));
    }
  } catch { /* fail-open */ }

  if (!live.length && !archived.length) return [];
  const byId = new Map();
  for (const r of [...live, ...archived]) byId.set(r.id, r);
  return [...byId.values()]
