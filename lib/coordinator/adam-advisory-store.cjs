...
async function stampActioned(supabase, advisoryRow, nowIso) {
  ...
  // QF-20260830-084: also stamp acknowledged_at, same action, same timestamp -- the lane
  // counter (summarizePendingLane) and every other consumer that reads acknowledged_at IS
  // NULL as "pending" used a DIFFERENT retirement predicate than this advisory tool's
  // payload.actioned_at, so a fully-actioned advisory read permanently pending on those
  // surfaces (measured: 46 rows, up to 29h). Writing both here is the single seam that
  // fixes every reader without touching each one -- one retirement predicate.
  const { error } = await supabase
    .from('session_coordination')
    .update({ payload: mergedPayload, acknowledged_at: nowIso })
