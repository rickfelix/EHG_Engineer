...
async function getCommsActivitySignals(supabase, sessionId, opts = {}) {
  ...
  try {
    ...
    // QF-20260703-642: a SENT row only counts as activity if it's part of a real exchange --
    // either the outstanding reply-requested ask (pendingSent) or a reply/continuation of one
    // (payload.reply_to set, the same convention repliedToIds above uses). A bare, unprompted
    // outbound row (e.g. the fleet-worker loop's per-tick "wakeup-armed" telemetry) is NOT
    // engagement -- counting it let a session pin ITSELF in tight-poll by signaling only to its
    // own telemetry, since a fresh self-sent row keeps recentActivity hot every tick forever.
    const reactiveSent = sentRows.filter(
      (r) => r === pendingSent ||
        (r && r.payload && typeof r.payload.reply_to === 'string' && r.payload.reply_to.length > 0)
    );

    const timestamps = []
      .concat(reactiveSent.slice(0, 1).map((r) => r.created_at))
      .concat(receivedRows.map((r) => r.created_at))
      .filter(Boolean)
      .map((t) => new Date(t).getTime());
    const lastActivityMs = timestamps.length ? Math.max(...timestamps) : undefined;

    // Cap anchor for the recent_activity branch too (from reactiveSent, not all sentRows): the
    // earliest known point of the live exchange among rows already fetched, so the 30-min cap in
    // computeAdaptiveCadence can actually fire for a genuine-but-endless thread instead of the
    // anchor silently defaulting to "now" every tick (which made now-openedAt always ~0).
    const reactiveTimestamps = []
      .concat(reactiveSent.map((r) => r.created_at))
      .concat(receivedRows.map((r) => r.created_at))
      .filter(Boolean)
      .map((t) => new Date(t).getTime());
    const threadOpenedAtMs = pendingSent
      ...
      : receivedUnactioned
        ...
        : reactiveTimestamps.length
          ? Math.min(...reactiveTimestamps)
          : undefined;
