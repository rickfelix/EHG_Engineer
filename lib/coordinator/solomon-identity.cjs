...
// QF-20260727-862: shared nil-UUID guard (see session-id-guard.cjs). Required here so the Solomon
// election cannot be won by the same unbacked ghost row that deposed the coordinator.
...
function pickCanonicalSolomon(rows) {
  ...
  const candidates = rows
    // QF-20260727-862: skip the nil UUID — the same 2026-07-27 ghost row that deposed the live
    // coordinator also outranked the real Solomon session (set_solomon_flag was applied to it too).
