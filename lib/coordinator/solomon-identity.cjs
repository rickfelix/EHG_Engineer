...
async function fetchAllSolomonsStrict(supabase) {
  ...
  try {
    ...
    const data = await fapPaginate(() => supabase
      .from('claude_sessions')
      .select('session_id, heartbeat_at, metadata, last_tool_at, hostname, pid')
      .filter('metadata->>role', 'eq', SOLOMON_ROLE)
      .order('session_id')); // unique-key tiebreaker for stable pagination
 ...
 */
function decideSingleSolomonGuard({ priorSolomons, selfSessionId, nowMs = Date.now(), freshMs = SOLOMON_FRESH_MS, isProcessDead = () => false }) {
  ...
  // dead) so this stays pure; scripts/solomon-register.cjs supplies lib/coordinator/role-seat-liveness.cjs
  // isSeatProcessDead, sound in the DEAD direction only (ESRCH) -- a live or recycled pid falls
  // through to the heartbeat classification below unchanged, so this can never kill a live Solomon.
