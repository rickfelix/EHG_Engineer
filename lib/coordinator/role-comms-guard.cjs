 ...
 * failure; prints the verification line on success (unchanged default). Sentinel targets
 * (broadcast-*) are buffered rows with no live session — reported, not role-verified. A direct
 * raw-session target (R1, expectedRole=null) gets a best-effort informational print only: R1
 * exists precisely to address arbitrary sessions, so no recipient class is enforced there.
 *
 * QF-20260902-100: `deferPrint` (opt-in, default false — every existing caller/test is
 * byte-identical unchanged) returns the success/informational line as a string instead of
 * printing it directly, so a caller can hold it until AFTER its own send actually lands (e.g.
 * printed alongside correlation_id once the row id exists) rather than printing "verified"
 * ahead of a send that a later gate can still refuse. The refusal path (exit 4) is unaffected —
 * it already prints its own ERROR line immediately, which is correct either way.
 */
async function assertTargetRole(supabase, { target, expectedRole, deferPrint = false }) {
  const emit = (msg) => { if (!deferPrint) console.log(msg); return msg; };
  if (typeof target === 'string' && target.startsWith('broadcast-')) {
    return emit(`  target-role verified: ${target} (buffered sentinel — no live session)`);
  }
  let actualRole;
  try {
    actualRole = await resolveSessionRole(supabase, target);
  } catch (e) {
    if (expectedRole === null) return emit(`  target-role: (direct target ${target} — role unresolved: ${e.message})`);
    console.error(`ERROR: [ROLE_GUARD] cannot verify target role (fail-closed): ${e.message}`);
    process.exit(4);
  }
  if (expectedRole === null) {
    return emit(`  target-role verified: ${actualRole || 'unregistered'} ${target} (direct target — no class constraint)`);
  }
  ...
  return emit(`  target-role verified: ${actualRole} ${target}`);
}
