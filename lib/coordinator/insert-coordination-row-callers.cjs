...
const FR2_CLI_ENTRYPOINTS = [
  { file: 'scripts/adam-advisory.cjs', line: 1392, disposition: 'migrated' },
  ...
  { file: 'scripts/worker-signal.cjs', line: 597, disposition: 'migrated' },
  { file: 'scripts/worker-signal.cjs', line: 762, disposition: 'migrated' },
  {
  ...
  {
    file: 'scripts/stale-session-sweep.cjs', line: 322,
    disposition: 'exempt-fire-and-forget',
    note: 'Bare `await insertCoordinationRow(...)` inside a sweep loop; no destructuring, no landed-dependent branch. process.exit elsewhere in the file is unrelated (CLI arg/mode handling).',
  },
  {
  ...
  {
    file: 'scripts/cron/batch-mint-sweep.mjs', line: 46,
    disposition: 'exempt-non-branching-destructure',
    note: 'Destructures {data,error} but the surrounding code does not branch process.exit on either — this is a fire-and-log path, not an exit-code decision.',
  },
  {
    file: 'scripts/coordinator-revive.cjs', line: 135,
    disposition: 'exempt-promise-catch-log-only',
    note: '`.then(...).catch((e) => console.warn(...))` — logs, never calls process.exit; the row insert above it (line 103) is also a fire-and-forget `.catch(console.warn)`.',
  },
  {
    file: 'scripts/adam-quiet-tick.mjs', line: 1434,
    disposition: 'exempt-fail-soft-try-catch',
    note: 'Both call sites (1434, 1437) sit inside `try { ... } catch { /* fail-open/fail-soft */ }` blocks that never call process.exit — they only gate an in-memory `acctNotified` flag.',
  },
  {
  ...
  {
    file: 'scripts/worker-checkin.cjs', line: 427,
    disposition: 'exempt-guarded-ternary',
...
const FR3_NON_CLI_CALLERS = [
  ...
  {
    file: 'lib/coordinator/coordination-events.cjs', line: 717,
    disposition: 'fixed-this-sd',
    note: 'emitNotificationWaitAlert: identical shape/fix to line 475.',
  },
  {
    file: 'lib/coordinator/coordination-events.cjs', line: 934,
    disposition: 'fixed-this-sd',
  ...
  {
    file: 'lib/adam/presend-consult-lane.cjs', line: 125,
    disposition: 'exempt-guarded-ternary',
  ...
  {
    file: 'lib/comms/adam-outbound/chairman-sms-gate/index.js', line: 534,
  ...
  {
    file: 'scripts/coordinator-self-review.mjs', line: 285,
  ...
  {
    file: 'scripts/coordinator-self-review.mjs', line: 299,
    disposition: 'exempt-outer-catch-log-only',
    note: 'Same shape/reasoning as line 285 (the bidirectional Adam-solicitation leg).',
  },
  {
