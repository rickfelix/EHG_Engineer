 ...
 * definition site) and lib/coordinator/dispatch.test.js. 27 files matched (later 28, after
 * QF-20260912-394 added a genuinely new caller — see that entry's own note); TWO were excluded
 ...
 * see EXCLUDED_FALSE_POSITIVES. The remaining 26 real callers (14 FR-2 files + 12 FR-3
 * files/14 call sites) are a complete partition: every entry appears in exactly one FR.
 *
 * `disposition` values:
...
const FR2_CLI_ENTRYPOINTS = [
  ...
  // reordered after the dedup check). QF-20260912-245 shifted it again, 1401 -> 1451 (new
  // recordFramingChairmanEscalation() function inserted above it in the same file).
  // QF-20260912-514 shifted adam-advisory.cjs 1451 -> 1462 (unclassified-recording comment
  // block added above) and solomon-advisory.cjs 1438 -> 1455 (new framingUnstampedWarning()
  // call + comment added above); QF-20260912-681 (concurrent) independently shifted
  // solomon-advisory.cjs 1438 -> 1449 (a probe-side filter added then removed). Merged: the
  // real post-merge line is 1466 (both changes' line-deltas apply on top of each other).
  { file: 'scripts/adam-advisory.cjs', line: 1462, disposition: 'migrated' },
  { file: 'scripts/solomon-advisory.cjs', line: 1466, disposition: 'migrated' },
  ...
  {
    file: 'scripts/adam-quiet-tick.mjs', line: 1552,
    ...
    // QF-20260912-924: shifted from (1537, 1540) — 15 lines added earlier in the file
    // (the new venture-deploy-watcher import + wiring + QUIET_TICK_VENTURE_DEPLOY_* lines).
    note: 'Both call sites (1552, 1555) sit inside `try { ... } catch { /* fail-open/fail-soft */ }` blocks that never call process.exit — they only gate an in-memory `acctNotified` flag.',
  },
  {
...
const FR3_NON_CLI_CALLERS = [
  ...
  },
  {
    ...
    // comment added above it in the same function). QF-20260912-901 shifted it again, 131 -> 145
    // (the measuredBy destructuring param + the measured_by[] provenance refusal check, both
    // added earlier in the same function, ahead of this call site).
    file: 'lib/adam/presend-consult-lane.cjs', line: 145,
    disposition: 'exempt-guarded-ternary',
  ...
  {
    // QF-20260913-173 shifted this call site from 1104 -> 1120 (the new 'no_open_question'
    // outcome branch added earlier in the same file, ahead of this call site).
    // SD-LEO-FIX-EVERY-CHAIRMAN-SMS-001 shifted it again, 1120 -> 1125 (the
    // UNMATCHED_OUTCOMES documenting comment added earlier in the same file).
    // SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 shifted it again, 1125 -> 1136 (the
    // stageDecisionSmsNotification self-heal try/catch block added earlier in the same file).
    file: 'lib/chairman/sms-bridge.js', line: 1136,
    disposition: 'exempt-fire-and-forget',
  ...
  },
  {
    ...
    // SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 shifted it again, 543 -> 525 (resolveChairmanUserId's
    // ~20-line local definition + docstring, earlier in the file, was extracted to its own module
    // and replaced with a 2-line import + re-export).
    file: 'lib/comms/adam-outbound/chairman-sms-gate/index.js', line: 525,
  ...
  {
    file: 'scripts/coordinator-self-review.mjs', line: 314,
    disposition: 'exempt-outer-catch-log-only',
    // QF-20260911-404: shifted from 308 -- added the FORCE_SELF_SCORE (--force) flag + comment
    // near the top of the file, for the coordinator self-score writer fix.
  ...
  {
    file: 'scripts/coordinator-self-review.mjs', line: 328,
    disposition: 'exempt-outer-catch-log-only',
    // QF-20260911-404: shifted from 322 -- same cause as the line-314 entry above.
    note: 'Same shape/reasoning as line 314 (the bidirectional Adam-solicitation leg). Line previously shifted from 299 by QF-20260906-891 (interruptibility filter added above the solicit loop).',
  },
  {
  ...
  {
    // QF-20260912-394: new call site, added by this QF (FR-4) to notify a carrier-filtered
    // SMS's originating seat -- not a pre-existing site whose line shifted.
    file: 'lib/chairman/sms-channel-health.js', line: 258,
    disposition: 'exempt-outer-catch-log-only',
    note: 'Wrapped in `try { ... } catch (e) { logger.warn(...) }` inside escalateCarrierFiltered\'s per-row loop -- a refused/failed notification (dead originator session, DISPATCH_TARGET_* errors) is caught and logged, never thrown out to the sweep caller, and never reverts the email-fallback stamp already written above it.',
  },
];
 ...
 * eslint-disable-next-line comment names inline. Recorded here so the census total (28 grep
 * hits - 2 false positives = 26 real callers = 14 FR-2 + 12 FR-3 files) is auditable rather than
 * silently dropped.
 */
