...
const FR2_CLI_ENTRYPOINTS = [
  ...
  {
    file: 'scripts/adam-quiet-tick.mjs', line: 1464,
    disposition: 'exempt-fail-soft-try-catch',
    // QF-20260906-601: shifted from (1440, 1443) — resolveMainRepoRoot() was added earlier in
    // the file (worktree-shared account-identity state file fix), pushing this block down.
    note: 'Both call sites (1464, 1467) sit inside `try { ... } catch { /* fail-open/fail-soft */ }` blocks that never call process.exit — they only gate an in-memory `acctNotified` flag.',
  },
  {
