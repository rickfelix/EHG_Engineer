...
const FR2_CLI_ENTRYPOINTS = [
  ...
  {
    file: 'scripts/adam-quiet-tick.mjs', line: 1483,
    disposition: 'exempt-fail-soft-try-catch',
    // QF-20260906-219: shifted from (1464, 1467) — resolveAccountSamplerIdentity() was extracted
    // earlier in the file (fleet-account-identity sampler-scope fix) and main()'s inline
    // getAccountIdentity() block was replaced with a single call to it, net-shifting this block.
    note: 'Both call sites (1483, 1486) sit inside `try { ... } catch { /* fail-open/fail-soft */ }` blocks that never call process.exit — they only gate an in-memory `acctNotified` flag.',
  },
  {
