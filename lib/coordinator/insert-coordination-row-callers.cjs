...
const FR2_CLI_ENTRYPOINTS = [
  ...
  {
    file: 'scripts/adam-quiet-tick.mjs', line: 1506,
    disposition: 'exempt-fail-soft-try-catch',
    // SD-LEO-INFRA-STAMP-CLAUDE-SESSIONS-001: shifted from (1483, 1486) — added
    // resolveAccountSamplerSourcePath() and its call site earlier in the file (LEAD-phase
    // prospective TESTING finding: saveLastAccountIdentity's source-path provenance fix).
    note: 'Both call sites (1506, 1509) sit inside `try { ... } catch { /* fail-open/fail-soft */ }` blocks that never call process.exit — they only gate an in-memory `acctNotified` flag.',
  },
  {
