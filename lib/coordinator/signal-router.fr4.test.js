...
describe('FR-4: ackAndRouteLoneSignal e2e — acks+routes lone medium WITHOUT insertFeedbackRow', () => {
  ...
  it('lone medium (single callsign) → routed=1, routed_to_coordinator stamped, NO feedback insert', async () => {
    const rows = [
    ...
    // SD-LEO-INFRA-SIGNAL-LANE-PER-001 (FR-4): stampRoutedToCoordinator is now NON-DISPOSING,
    // mirroring stampRouted()'s already-fixed shape — an automated route-to-coordinator action
    // must never itself close the row (that regressed the "9 critical signals silently vanished"
    // defect class). A genuine FR-1 disposition is written ONLY by coordinator-ack-signal.cjs
    // (the one narrow, explicitly-scoped exception is FR-3's historical backfill script, which
    // never pairs acknowledged_at with a real disposition value) -- this automated route-marking
    // path must NOT stamp acknowledged_at at all.
    expect(u.patch.acknowledged_at).toBeUndefined();
