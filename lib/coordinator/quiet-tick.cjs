 ...
 *
 * @param {{quiescent:boolean, partyOffsetS?:number, desiredQuiescentParkS?:number, desiredActiveS?:number, hasUnactionedDirective?:boolean, hasUndeliveredChairmanEscalation?:boolean}} s
 ...
 *
 * NOT YET WIRED into decideCadence(): shipping the actual [540,660] wake band
 * this predicate would gate is BLOCKED pending resolution of a structural finding
 * (FR-6): no preemption path exists anywhere in this codebase for an armed
 * ScheduleWakeup, and a parked worker seat runs no tools, so no PostToolUse hook
 * observes a session_coordination directive until the seat's own park interval
 * naturally expires. Live counter-evidence: seat 2b9045cc, parked at
 * wake_delay_seconds=3600, was unresponsive to two coordinator directives for
 * 27+ minutes (coordinator amendment_2, metadata.coordinator_review on
 * SD-LEO-INFRA-COORDINATOR-LOADED-QUIET-001). This function ships now as tested,
 * inert infrastructure for a follow-up SD once that gap is closed.
 *
