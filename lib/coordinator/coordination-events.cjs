  ...
  try {
    ...
    const body = churn
      ...
      : 'The worktree reaper has REFUSED to reap for ' + e.consecutive_refusals + ' consecutive due tick(s) while ' +
        ...
        'HARD-FAILS for every seat.\n\nFIX: bring the reaper source tree current (it self-refreshes; a persistent ' +
        'streak means that refresh is failing), or investigate why fast-forward is blocked.';
    const subject = churn
