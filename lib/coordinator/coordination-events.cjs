 ...
 * the day it is right.
 *
 * DELIBERATE, and stated because it cuts against fail-toward-alarm: an UNKNOWN pool (git census
 * failed, used===null) does NOT alarm. The census failing is already reported by its own path, and
 * firing a starvation alarm off a number we could not read would attribute a pool problem to a
 * reaper problem. The reason string stays distinct so the silence is greppable rather than blank.
 */
function detectReaperStarvation({ consecutiveRefusals, pool, threshold = REAPER_STARVATION_THRESHOLD } = {}) {
  ...
  if (used === null) return { matched: false, reason: 'pool_unknown', streak };
...
async function emitReaperStarvationAlert(supabase, evidence, opts) {
  ...
  try {
    ...
    const { data: dupes } = await supabase
      ...
      .select('id')
      .eq('message_type', 'INFO')
      .eq('payload->>kind', 'reaper_starvation_alert')
      .is('acknowledged_at', null)
      .gt('expires_at', nowIso)
      .limit(1);
    if (dupes && dupes.length > 0) return { ok: true, skipped: true, id: dupes[0].id };
    ...
    const row = {
      ...
      sender_type: 'sweep',
      message_type: 'INFO',
      subject: '[REAPER_STARVATION] ' + e.consecutive_refusals + ' consecutive refusals, pool ' + e.pool_used + '/' + e.pool_cap,
      body,
      payload: { kind: 'reaper_starvation_alert', severity: 'high', ...e },
      expires_at: expiresAt,
    };
    const { data, error } = await insertCoordinationRow(supabase, row, { select: 'id', single: true });
    if (error) {
...
async function runReaperStarvationSurfacing(supabase, outcome, opts) {
  ...
  try {
    ...
    const alert = await emitReaperStarvationAlert(supabase, res.evidence, { now: opts.now });
    return { ...res, alert };
  } catch (e) {
