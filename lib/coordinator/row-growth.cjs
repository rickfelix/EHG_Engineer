...
// SD-LEO-INFRA-BREAKAGE-DETECTOR-SURFACE-001-C: TOP-LEVEL (static) require of the fail-soft breakage
// boundary so the wire-check reachability graph unambiguously traces row-growth -> emit-breakage-alert
// -> alert-writer (the decomposition-root module whose exemption this child removes). The boundary +
// the writer are side-effect-free (pure function/const definitions), so hoisting is free at load time.
const { emitBreakageAlert } = require('../breakage/emit-breakage-alert.cjs');
...
/** Above this estimate, no exact count is taken (must not become the COUNT(*) cost estimates avoid). */
const EXACT_COUNT_CEILING = 250_000;
...
/** PURE: confirm/refute a candidate against a fresh exact count (null/over-ceiling => estimate_only;
 *  else compares against priorExactConfirmed 'exact' or candidate.prev 'prior_estimate'). */
function confirmRowGrowthAnomaly(candidate, exactCount, priorExactConfirmed, opts = {}) {
  const o = { ...DEFAULT_OPTS, exactCountCeiling: EXACT_COUNT_CEILING, ...opts };
  const base = { table: candidate.table, estimate: candidate.curr };
  if (candidate.curr > o.exactCountCeiling || !Number.isFinite(exactCount)) {
    return { ...base, status: 'estimate_only', measurement: 'estimate_only', exact: null, comparedAgainst: null, baseline: null, delta: null, factor: null, trigger: null };
  }
  const comparedAgainst = Number.isFinite(priorExactConfirmed) ? 'exact' : 'prior_estimate';
  const baseline = comparedAgainst === 'exact' ? priorExactConfirmed : candidate.prev;
  const delta = exactCount - baseline;
  const factor = baseline > 0 ? exactCount / baseline : null;
  const trips = delta >= o.absSpike || (baseline >= o.minRowsForFactor && factor !== null && factor >= o.growthFactor);
  const trigger = trips ? (delta >= o.absSpike ? 'abs_spike' : 'growth_factor') : null;
  return { ...base, status: trips ? 'confirmed' : 'refuted', measurement: 'exact', exact: exactCount, comparedAgainst, baseline, delta, factor, trigger };
}

/** IO (fail-soft): one exact COUNT(*) for a table, called at most once per anomaly candidate. */
async function readExactCount(sb, table) {
  try {
    const { count, error } = await sb.from(table).select('*', { count: 'exact', head: true });
    return !error && Number.isFinite(count) ? count : null;
  } catch { return null; }
}
 ...
 * @param {object} sb
 * @returns {Promise<{captured_at:string, tables:Object<string,number>, exact_confirmed:Object<string,number>}|null>}
 */
async function readLatestSnapshot(sb) {
  try {
    ...
    return { captured_at: p.captured_at || data[0].created_at, tables: p.tables, exact_confirmed: p.exact_confirmed || {} };
  } catch { return null; }
}

/**
 * FR-C3 (SD-LEO-INFRA-BREAKAGE-DETECTOR-SURFACE-001-C): surface a detected row-growth anomaly batch as
 * a single row-growth-anomaly system_alerts row (the writer dedups on source+break_class, so a recurring
 * anomaly does not flood). Routes through the shared FAIL-SOFT boundary, so a system_alerts write error
 * never breaks the snapshot CLI. No-op (no alert) when there are no anomalies. Consumes the frozen
 * child-A break_class — never re-encodes the taxonomy.
 * @param {object} sb - supabase client (the snapshot CLI's service client)
 * @param {Array<{table:string,prev:number,curr:number,delta:number,factor:number|null,trigger:string}>} anomalies
 * @returns {Promise<{ok:boolean, id?:string, deduped?:boolean, skipped?:string, error?:string}>}
 */
async function emitRowGrowthAnomalyAlert(sb, anomalies) {
  if (!Array.isArray(anomalies) || anomalies.length === 0) return { ok: false, skipped: 'no-anomalies' };
  const top = anomalies[0]; // detectRowGrowthAnomalies returns delta-desc, so [0] is the worst offender
  return emitBreakageAlert('row-growth-anomaly', 'row-growth', {
    message: `row-growth: ${anomalies.length} governance table(s) growing abnormally — top: ${top.table} +${top.delta}`,
    sourceEntityId: top.table,
    metadata: { anomaly_count: anomalies.length, top_table: top.table, anomalies },
  }, { supabase: sb });
}
