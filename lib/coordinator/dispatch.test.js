...
describe('dispatch guard — pure helpers', () => {
  ...
  it('sentinels include broadcast-coordinator but NOT bare broadcast (QF-20260911-753)', () => {
    expect(SENTINEL_TARGETS).toContain('broadcast-coordinator');
    expect(SENTINEL_TARGETS).not.toContain('broadcast');
    expect(isSentinelTarget('broadcast-coordinator')).toBe(true);
    expect(isSentinelTarget('broadcast')).toBe(false);
...
describe('insertCoordinationRow — accept/reject matrix (FR-2/3/4)', () => {
  ...
  it('REFUSES bare broadcast — no reader ever drains it (QF-20260911-753)', async () => {
    // Measured live: 91 rows on target_session='broadcast', 0 ever acknowledged. A guard
    // may decline a write, but a sentinel every writer may address and no reader drains is
    // a silent sink — removed from SENTINEL_TARGETS so writers fail loudly instead.
    const { sb, inserts } = makeFakeSupabase({ sessionRow: null });
    await expect(
      insertCoordinationRow(sb, { target_session: 'broadcast', message_type: 'COACHING' }, { logger: silent })
    ).rejects.toMatchObject({ code: 'DISPATCH_TARGET_INVALID' });
    expect(inserts).toHaveLength(0);
  });
