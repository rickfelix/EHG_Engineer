...
const os = require('os');
// QF-20260727-862: the nil UUID is a well-formed string, so it defeats every `typeof === 'string'`
// check below. Guard it explicitly at both the identity write-path and the election.
...
function pickCanonicalCoordinator(rows) {
  ...
  const candidates = rows
    // QF-20260727-862: skip the nil UUID. A ghost row carrying is_coordinator=true is already in the
    // DB, and '00000000-…' sorts FIRST in the session_id ASC tiebreak below — so without this filter
    // an unbacked row wins the election outright and deposes the live coordinator.
...
/**
 * QF-20260727-862: single sanitizing choke-point over EVERY resolution path below.
 * pickCanonicalCoordinator only guards the elected path — the pointer file, the legacy DB scan and
 * the self-ID handshake each return a session_id directly and would otherwise still hand back a nil
 * UUID. Callers treat null as "no live coordinator" and fall back to the `broadcast-coordinator`
 * sentinel, which IS drained and IS visible; a nil UUID is neither. Degrade loudly, never silently.
 */
...
async function setActiveCoordinator(supabase, sessionId) {
  ...
  // QF-20260727-862: the check above passes the nil UUID (a non-empty, well-formed string), which is
  // exactly how an absent CLAUDE_SESSION_ID reached the DB on 2026-07-27 and minted a coordinator
  // ghost row that deposed the live coordinator. Refuse the write at the source — a role flag on an
  // unbacked identity silently blackholes every signal the fleet sends.
  if (isNilUuid(sessionId)) {
    throw new Error(`setActiveCoordinator: refusing to flag the nil UUID (${NIL_UUID}) as coordinator — no process can back it, and it wins the election tiebreak. This means CLAUDE_SESSION_ID was unset/null at the call site. See QF-20260727-862.`);
  }
