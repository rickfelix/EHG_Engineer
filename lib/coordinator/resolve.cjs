...
function pickCanonicalCoordinator(rows) {
  ...
  const candidates = rows
    // QF-20260727-862: skip the nil UUID — a flagged ghost row is already in the DB, and
    // '00000000-…' sorts FIRST in the session_id ASC tiebreak below, so it would win outright.
...
async function setActiveCoordinator(supabase, sessionId) {
  ...
  // QF-20260727-862: the check above PASSES the nil UUID (non-empty, well-formed), which is exactly
  // how an absent CLAUDE_SESSION_ID minted the ghost coordinator row. A role flag on an unbacked
  // identity silently blackholes every signal the fleet sends — refuse the write at the source.
