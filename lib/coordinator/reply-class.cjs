...
async function alreadyAnswered(supabase, correlationId, opts) {
  ...
  try {
    ...
    // QF-20260904-225: a row parked by backpressure (payload.backpressure_parked) never
    // reached the originator, so it must never count as "answered" — otherwise a genuine
    // second reply on the same correlation is refused even though the first was never
    // delivered. Filtered in JS (not a DB-level clause) so the query SHAPE stays exactly
    // the 2-.eq()-then-.limit() the legacy-mock test pins (TS-1, correction-delivery-path);
    // the small limit buffer lets a real answer still surface if a parked row sits ahead of
    // it in the (unordered) result set.
    const { data, error } = await q.limit(20);
    if (error) return false;
    const genuine = (Array.isArray(data) ? data : []).filter((r) => !(r.payload && r.payload.backpressure_parked === true));
...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: broadcast-michael added beside its peers.
const UNPINGABLE_TARGETS = new Set(['broadcast', 'broadcast-coordinator', 'broadcast-solomon', 'broadcast-michael']);
...
async function checkAndPingOverdueReplies(supabase, { sessionId, senderType = 'worker', insert, now } = {}) {
  ...
  const { data: rows, error } = await supabase
    ...
    // A suppressed CC must RETIRE from this fetch like a pinged row does (adversarial ship
    // review PR #6942): without this, excluded CCs match the fetch forever on an unreaped
    // table, and once a CC-heavy sender accumulates >200 such rows the unordered limit-200
    // slice can permanently starve GENUINE overdue consults out of the sweep — the sweep
    // goes silently blind. Same terminal-window-over-unreaped-table hazard the
    // RECONCILE_HORIZON comment below defends against on the verdict path.
    .is('payload->>ping_suppressed_at', null)
    .limit(200);
  ...
  const overdueAll = findOverdueReplyNeeded(rows, nowMs, answered);
  const overdue = [];
  for (const r of overdueAll) {
    if (!isPreSendCc(r)) { overdue.push(r); continue; }
    // Retire the suppressed CC with an HONESTLY-NAMED stamp (ping_suppressed_at, never a
    // fake ping_sent_at) so the next sweep's fetch excludes it. Best-effort: a failed stamp
    // just means the row is re-excluded next tick — suppression correctness never depends
    // on the stamp landing.
    try {
      await supabase
        .from('session_coordination')
        .update({ payload: { ...r.payload, ping_suppressed_at: new Date(nowMs).toISOString() } })
        .eq('id', r.id);
    } catch { /* best-effort retirement — see comment above */ }
  }
