...
async function reconcileLateVerdicts(supabase, opts = {}) {
  ...
  const { data: rows, error } = await supabase
    ...
    // OLDEST-FIRST, explicitly. A bare .limit() is an ARBITRARY slice, and unreconciled rows
    // accumulate (only the answered fraction ever gets stamped), so an unordered window would
    // eventually stop surfacing the backlog's head and the sweep would go quietly blind — the same
    // count-truncation class this SD fixes elsewhere. created_at ascending drains oldest first;
    // id is the unique tiebreaker for a stable slice.
    .order('created_at', { ascending: true })
    .order('id', { ascending: true })
    .limit(limit);
  ...
  for (const row of rows) {
    ...
    // The disposition subject MUST be an object AND content-derived. computeQuestionKey throws on a
    // string, and disposition.js is explicit that a consult_answer key is derived from question
    // TEXT and "never on correlation_id/message_id, which rotate per session". Keying on corr would
    // therefore have been wrong even if it had been an object: a re-asked pre-send question in a new
    // session would mint a phantom second disposition instead of deduping onto the first.
    // Shape mirrors the canonical binder (scripts/consult-answer-bind.mjs) so BOTH paths compute the
    // SAME question_key — a content key that two writers disagree on does not dedup at all.
    const questionText = (row.payload && row.payload.body) || row.body || row.subject || null;
    if (!questionText) {
      // No text => no derivable content key. Skipping leaves the row un-stamped and retryable.
      // The alternative (a constant placeholder) would collapse every text-less consult onto ONE
      // question_key, where dedup returns the first row and the rest silently read as reconciled.
      unkeyable += 1;
      continue;
    }
    try {
      await recordDisposition({
        ...
        subject: { consult_purpose: 'pre_send', sd_key: (row.payload && row.payload.sd_key) || null, question_text: questionText },
  ...
  return { checked: rows.length, reconciled: reconciledIds.length, reconciledIds, nearMisses, unkeyable };
}
