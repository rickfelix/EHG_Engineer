 ...
 *   alreadyDispositioned:number, errors:number, firstError:string|null}>}
 *   `errors`/`firstError` exist so a lane that is FAILING on every write is distinguishable from a
 *   lane that is merely quiet — conflating the two is what hid SEC-1.
 */
async function reconcileLateVerdicts(supabase, opts = {}) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // created===false now means only "this exact row was already dispositioned by an earlier
      // sweep", so stamping is correct and idempotent. Counted anyway: a sudden run of them is the
      // signature of a key that has stopped discriminating, which is how the collapse above hid.
