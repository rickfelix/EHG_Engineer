...
// Adversarial-review finding: the message fallback previously required BOTH a phrasing match
// AND a separate "not (found|exist)" match, but PostgREST's real message ("Could not find the
// function ... in the schema cache") never contains "not found"/"not exist" as those exact
// substrings -- the AND could never be satisfied by the case it exists to catch. Each phrasing
// below is independently sufficient (verified against the real PostgREST and Postgres messages).
