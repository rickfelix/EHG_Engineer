...
import { classifyUnlinkedReason } from '../sd-creation/plan-linkage-classifier.js';
 ...
 * SD-LEO-INFRA-PLAN-LINKAGE-BELT-001 (FR-1b): the SAME atomic UPDATE also stamps a default
 * metadata.plan_linkage when one is not already present — the fence-lift door's tag-at-the-door
 * requirement — via a SQL CASE, so an SD reaching fence-lift with no prior stamp (e.g. a
 * pre-existing record created before this SD shipped) gets a safe classified default rather
 * than staying permanently unlabeled. An SD that already has a stamp (the common case, set at
 * creation by lib/sd-creation/plan-linkage-classifier.js) is left byte-identical — no read
 * required to know that, since the CASE evaluates server-side against the row being updated.
 * @param {string} sdKey
...
export function buildClearReviewQuery(sdKey) {
  ...
  const defaultUnlinkedReason = classifyUnlinkedReason(sdKey);
  return {
    sql: `UPDATE strategic_directives_v2
          SET metadata = COALESCE(metadata, '{}'::jsonb) || '{"needs_coordinator_review": false}'::jsonb
              || CASE WHEN metadata->'plan_linkage' IS NULL
                      THEN jsonb_build_object('plan_linkage', jsonb_build_object(
                             'linked', false,
                             'wave_id', NULL,
                             'wave_title', NULL,
                             'rung', NULL,
                             'unlinked_reason', $2::text,
                             'classified_at', now()::text,
                             'classified_by', 'fence_lift'
                           ))
                      ELSE '{}'::jsonb END
          WHERE sd_key = $1
            AND (
    ...
    params: [sdKey, defaultUnlinkedReason],
  };
}
