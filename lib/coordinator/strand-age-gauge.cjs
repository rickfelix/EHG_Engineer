...
async function planStrandAgeGauge(supabase, opts = {}) {
  ...
  for (const sd of candidates) {
    ...
    // updated_at looks unreliable when it's missing/unparseable or predates created_at
    // (never touched). pgTimestampMs() returns NaN (not null) for unparseable input, so the
    // guard must use Number.isFinite() — a `!== null` check would pass NaN through silently.
    ...
    if (updatedLooksReliable) {
      ...
      const { data: handoffs } = await supabase
        ...
        .limit(1);
      const handoffMs = pgTimestampMs(handoffs?.[0]?.resolved_at);
      if (Number.isFinite(handoffMs)) {
