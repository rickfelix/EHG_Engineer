...
function decideCommitments(commitments, opts = {}) {
  ...
  return (commitments || []).map((c) => {
    ...
    return {
      ...
      source: 'commitment',
      counterpartyLiveness: livenessOf ? livenessOf({ target_session: c.counterparty_session }) : undefined,
    };
  });
}
...
async function planRelayDrops(supabase, opts = {}) {
  ...
  try {
    ...
    const commitmentRowsForLiveness = openCommitments.map((c) => ({ target_session: c.counterparty_session }));
