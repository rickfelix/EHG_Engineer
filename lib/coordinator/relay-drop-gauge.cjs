...
async function planRelayDrops(supabase, opts = {}) {
  ...
  try {
    ...
    const counterpartySessions = await loadCounterpartySessions(supabase, inbound);
    const livenessOf = (row) => classifyCommitmentLiveness(
      row && row.target_session ? counterpartySessions[row.target_session] : null,
      { now }
    );
    const decisions = decideRelayDrops(inbound, outbound, { now, windowMs: resolveWindowMs(env), reviewWindowMs: resolveReviewWindowMs(env), livenessOf });
    return {
