...
const SLA_CATEGORIES = Object.freeze({
  ...
  // SD-LEO-INFRA-CAPTURE-CHANNEL-DISPOSITION-001 (FR-3): the severityIn:['high','critical'] filter
  // this entry carried previously left ~94% of the live harness_backlog population (medium/low/NULL
  // severity) structurally invisible to breach alarming, with no documented rationale for the
  // narrowing. Widened to all severities so the daily reminder (rate-limited to one per category per
  // day regardless of match count, see remindSlaBreaches) reflects the TRUE open population.
  harness_backlog: Object.freeze({ days: 7 }),
});
