...
// QF-20260903-333: harness_backlog is intentionally NOT an SLA category. Its only
// documented consumption path is manual — [MODE: campaign] sessions or `/leo audit`
// reading the raw table (CLAUDE.md "Session Mode Declaration"); the auto-refill cron
// promoter reads conversion_ledger-matched candidates only, and hand-promoting from
// harness_backlog is an explicitly forbidden anti-pattern (charter-audit-detectors.mjs
// detectAutoRefillBacklog). A 7-day breach reminder on a channel nothing consumes on a
// schedule declares a deadline that cannot be met — worse than declaring none (2471 open
// rows, oldest 87d, against the SLA this entry used to assert). The drain-gauge
// (drain-gauge.cjs) still gives honest, undated visibility into this category's depth.
// SD-LEO-INFRA-HARNESS-BACKLOG-PER-001 (FR-5): this single-literal-category filter narrows
// correctly BY CONSTRUCTION now that capture-completion-flags.js's real per-flag findings
// route to category='completion_flag_finding' instead of 'harness_backlog' (FR-2) — no code
// change needed here; the harness_backlog population simply shrinks to its genuine remainder
// as those rows and the FR-3/FR-4 backfill land. Do not "fix" this into an enumeration.
