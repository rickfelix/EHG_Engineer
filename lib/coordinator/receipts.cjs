 ...
 *
 * QF-20260830-482: a row the target already read (read_at set) is delivered, not
 * gone — dead_letter reflects a stale-session sweep opinion at flag time, not the
 * row's current delivery state. Re-sending a read row manufactures a duplicate
 * directive into an already-busy lane, so read rows are excluded here regardless
 * of the dead_letter flag.
 */
