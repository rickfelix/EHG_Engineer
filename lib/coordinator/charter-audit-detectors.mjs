...
export function detectUnstampedModel({ liveSessions = [] } = {}) {
  // SD-LEO-INFRA-FLEET-MODEL-REGISTRY-001 FR-2: this used to require
  // `effort_source === 'worker_self_report'` as a PROXY for model provenance, because no
  // model_source field existed anywhere in the codebase — the detector had nothing else to
  // read. That proxy is now wrong in both directions: a seat whose model was auto-observed
  // at registration but which never ran --effort was flagged as unstamped despite carrying a
  // perfectly good model, and a seat could satisfy the check on the strength of an EFFORT
  // stamp that says nothing about its model. Read the real provenance field, and keep
  // honouring the legacy effort_source proxy so rows written before this SD (the majority on
  // day one) are not suddenly flagged en masse.
  const hasModelProvenance = (m) => {
    if (typeof m.model_source === 'string' && m.model_source.trim()) return true;
    return m.effort_source === 'worker_self_report'; // legacy proxy, pre-model_source rows
  };
  const selfReportsModel = (s) => {
    const m = s && s.metadata;
    return !!(m && typeof m.model === 'string' && m.model.trim() && hasModelProvenance(m));
  };
