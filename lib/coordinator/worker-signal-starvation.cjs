...
function assertNonBinding(liveCount, archivedCount, cap) {
  ...
  return 'WORKER SIGNALS: TRUNCATED at MAX_SIGNALS=' + cap + ' (fetched ' + fetched
    + ') — both queries are ordered oldest-first so the aged starvation candidates stay visible, '
    + 'but some of the NEWEST signals in the window were discarded. '
...
async function reportWorkerSignalStarvation(supabase, opts = {}) {
  ...
  try {
    ...
    // QF-20260815-711: ordered OLDEST-first (ascending). Starvation IS age, so when the 30-day
    // window's population exceeds MAX_SIGNALS the cap must discard the NEWEST excess, not the
    // oldest — a descending order+cap silently drops exactly the aged rows this gauge exists to
    // find (the same anti-pattern the MAX_SIGNALS comment below already documents for the
    // 500->5000 raise; the ordering itself was never fixed alongside it).
    ...
    try {
      // QF-20260815-711: also ordered oldest-first, mirroring the live query above. The archive
      // exists to prove an OLD live signal was actually answered (excluding it from starved); a
      // descending order+cap would preferentially discard the oldest archived rows first — exactly
      // the ones needed to correctly clear the oldest, most-suspicious live candidates.
      ...
      const arch = await fetchPaginated(
        (from, to) => supabase
          ...
          .order('row_timestamp', { ascending: true })
          .range(from, to),
        MAX_SIGNALS
      );
      archived = arch
