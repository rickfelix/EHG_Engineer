...
const { getAliveCcPids, getMarkerReadState } = require('../fleet/cc-pid-liveness.cjs');
...
function decideQuiescence(s) {
  ...
  // take. If the CC-PID marker read could not determine liveness at all (marker directories
  // absent or unreadable), the liveActiveWorkers count below is heartbeat-only -- degraded,
  // not measured -- so this must never be allowed to read as a confirmed quiescent zero.
  if (s.couldNotDetermine) {
    return {
      quiescent: false,
      reason: 'could_not_determine — CC-PID marker read failed (absent/unreadable); liveActiveWorkers would be heartbeat-only, not measured',
    };
  }
  const quiescent =
...
async function assessFleetActivity(sb, opts) {
  ...
  const signals = { liveActiveWorkers: 0, inProgressBuilds: 0, recentTransitions: 0, couldNotDetermine: false, error: null };
  try {
    ...
    // QF-20260911-907: off-checkout (no markers reachable from THIS worktree/host) or a
    // read failure now surfaces explicitly as couldNotDetermine, rather than silently
    // degrading to heartbeat-only detection with no distinct signal — see decideQuiescence.
    const aliveCcPids = (opts && opts.aliveCcPids) || getAliveCcPids();
    const markerReadState = (opts && opts.markerReadState) || getMarkerReadState();
    signals.couldNotDetermine = markerReadState === 'unreadable' || markerReadState === 'absent';
