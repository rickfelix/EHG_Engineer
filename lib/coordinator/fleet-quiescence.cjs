...
async function assessFleetActivity(sb, opts) {
  ...
  try {
    ...
    // Off-checkout (no markers reachable from THIS worktree/host) → empty set → could-not-determine,
    // never a death verdict: degrades to heartbeat-only quiescence detection, never worse.
