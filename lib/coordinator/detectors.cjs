...
function detectStalledLoop(data) {
  ...
  for (const s of ((data && data.sessions) || [])) {
    ...
    // QF-20260728-112: the coordinator never claims an SD by design (sd_key is permanently null),
    // so once loop_state started reflecting live coordinators too (SD-LEO-INFRA-LOOP-STATE-AWAITING-001),
    // the no-claim + fresh-heartbeat conditions above were permanently satisfied for it — an
    // un-actionable false positive on a process behaving correctly, every sweep. Reuses sessionRole()
    // (the same coordinator identification detectDeployGap already uses) rather than re-deriving it.
    if (sessionRole(s) === 'coordinator') continue;
...
/**
 * NOTIFICATION_WAIT — a seat has been sitting on a Claude Code Notification event (most
 * commonly a permission prompt) for longer than the freshness window, per QF-20260905-346.
 * Reuses DEFAULT_STALLED_LOOP_FRESH_MS as N (per the QF's own scope: "N read from the
 * existing quiet-tick threshold table, not a new constant") rather than inventing a fresh
 * threshold for what is the same "does this still look alive" question the loop-liveness
 * detectors already answer. Input is PRE-DEDUPED to one (latest) wait per session by the
 * gatherer — this stays pure/no-I/O. Fail-open: a wait row with no usable created_at is
 * skipped, never falsely flagged.
 * @param {{ waits?: Array<{session_id:string, created_at:string}>, now?: number, thresholdMs?: number }} data
 * @returns {{ matched: boolean, reason: string, evidence: object }}
 */
function detectNotificationWait(data) {
  const now = (data && data.now) ?? Date.now();
  const thresholdMs = (data && data.thresholdMs) ?? DEFAULT_STALLED_LOOP_FRESH_MS;
  const waiting = [];
  for (const w of ((data && data.waits) || [])) {
    if (!w) continue;
    const ageMs = now - toMs(w.created_at);
    if (ageMs <= 0 || ageMs <= thresholdMs) continue;
    if (waiting.length < 10) {
      waiting.push({ session_id: w.session_id, age_ms: ageMs });
    }
  }
  if (waiting.length === 0) {
    return { matched: false, reason: 'no_stale_notification_waits', evidence: { threshold_ms: thresholdMs } };
  }
  return {
    matched: true,
    reason: 'seat_waiting_on_notification_past_threshold',
    evidence: { waiting_count: waiting.length, threshold_ms: thresholdMs, samples: waiting },
  };
}
