...
function detectStalledLoop(data) {
  ...
  for (const s of ((data && data.sessions) || [])) {
    ...
    // SD-LEO-INFRA-LOOP-STATE-AWAITING-001: a session that has NEVER participated in the fleet — no
    // claim, no worktree, no completed-SD counter — is not a worker that failed to pick up work. The
    // COORDINATOR is the standing example: it never claims an SD, so sd_key is permanently null and
    // its heartbeat is permanently fresh, satisfying every other condition here on every pass.
    // This guard could not have mattered before: loop_state was never reset to 'active' on resume,
    // so for live seats this loop body was structurally unreachable. Clearing the latch makes it
    // reachable, which is why it ships in the same change. MEASURED against the live table — of 8
    // live sessions, the only one this detector would newly flag was coordinator 880b5cc4, and it
    // would flag it forever. Mirrors the identical registration-ghost exclusion already carried by
    // detectIdleWithWork in lib/coordinator/charter-audit-detectors.mjs, which learned it the same way.
    if (!(s.claimed_at || s.worktree_path || Number(s.continuous_sds_completed) > 0)) continue;
