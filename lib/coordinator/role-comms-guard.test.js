...
describe('assert wrappers (fail-closed exits)', () => {
  ...
  // QF-20260902-100: deferPrint is opt-in — every call above omits it and keeps printing
  // immediately (unchanged default), pinned again here explicitly for the option's own contract.
  describe('deferPrint (QF-20260902-100) — hold the success line instead of printing it', () => {
    it('returns the verified line WITHOUT printing when deferPrint:true', async () => {
      // Delta-based, not .not.toHaveBeenCalled(): this file's console.log spy carries call
      // history across its own preceding tests (pre-existing, unrelated to this QF), so the
      // robust assertion is "no NEW calls from this invocation", not "zero calls ever".
      const log = vi.spyOn(console, 'log').mockImplementation(() => {});
      const before = log.mock.calls.length;
      const line = await assertTargetRole(mockSupabase({ data: [{ session_id: 't2', metadata: { role: 'coordinator' } }] }), { target: 't2', expectedRole: 'coordinator', deferPrint: true });
      expect(log.mock.calls.length).toBe(before);
      expect(line).toContain('target-role verified: coordinator t2');
    });

    it('returns the sentinel/direct-target lines too, still without printing, when deferPrint:true', async () => {
      const log = vi.spyOn(console, 'log').mockImplementation(() => {});
      const before = log.mock.calls.length;
      const sentinelLine = await assertTargetRole({ from: () => { throw new Error('must not query'); } }, { target: 'broadcast-coordinator', expectedRole: 'coordinator', deferPrint: true });
      expect(sentinelLine).toContain('broadcast-coordinator (buffered sentinel');
      const directLine = await assertTargetRole(mockSupabase({ data: [{ session_id: 't3', metadata: { role: 'worker' } }] }), { target: 't3', expectedRole: null, deferPrint: true });
      expect(directLine).toContain('direct target');
      expect(log.mock.calls.length).toBe(before);
    });

    it('a class-mismatch refusal still exits 4 and prints its own ERROR line immediately, deferPrint or not', async () => {
      const exit = exitAsThrow();
      await expect(assertTargetRole(mockSupabase({ data: [{ session_id: 't1', metadata: { role: 'worker' } }] }), { target: 't1', expectedRole: 'coordinator', deferPrint: true }))
        .rejects.toThrow('EXIT:4');
      expect(exit).toHaveBeenCalledWith(4);
    });

    it('omitting deferPrint keeps the exact pre-existing behavior (prints immediately)', async () => {
      const log = vi.spyOn(console, 'log').mockImplementation(() => {});
      await assertTargetRole(mockSupabase({ data: [{ session_id: 't2', metadata: { role: 'coordinator' } }] }), { target: 't2', expectedRole: 'coordinator' });
      expect(log).toHaveBeenCalledWith(expect.stringContaining('target-role verified: coordinator t2'));
    });
