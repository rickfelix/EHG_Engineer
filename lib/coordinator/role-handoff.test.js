...
// Coordinator pointer file path — used in the global writeFileSync interceptor below as
// defense-in-depth (SD-LEO-FIX-ENF-TRUSTS-FILE-001 already redirects resolve.cjs's own
// ACTIVE_COORDINATOR_FILE to a per-PID tmpdir path under VITEST). Sourced via a FRESH
// top-level require, NOT the `let resolve` variable below (line 22) — that is only assigned
// inside beforeEach and is undefined/TDZ at module-collection time, when this line runs.
const COORD_FILE_ABS = require('./resolve.cjs').ACTIVE_COORDINATOR_FILE;
