...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: the michael singleton (spec §1.2). Registered here
// BEFORE owner-target-resolver.mjs's KNOWN_PEERS gains 'michael' — resolvePeerTarget throws on an
// unknown key, and the liveness watcher passes the normalized KNOWN_PEERS key straight through.
const defaultMichaelIdentity = require('./michael-identity.cjs');
...
const PEER_KINDS = Object.freeze({
  ...
  michael: { class: 'session', sentinel: 'broadcast-michael' },
...
async function resolvePeerTarget(supabase, peer, opts = {}, deps = {}) {
  ...
  const {
    ...
    getActiveMichaelId = defaultMichaelIdentity.getActiveMichaelId,
    resolveMichaelReplyTarget = defaultMichaelIdentity.resolveMichaelReplyTarget,
  } = deps;
  ...
  if (key === 'michael') {
    if (originator) {
      const r = await resolveMichaelReplyTarget(supabase, originator, {});
      return { kind: 'session', target: r.target, sentinel: entry.sentinel, peer: key, live: Boolean(r.live), retargeted: r.retargeted, relayVia: null };
    }
    const live = await getActiveMichaelId(supabase, {});
    return { kind: 'session', target: live || entry.sentinel, sentinel: entry.sentinel, peer: key, live: Boolean(live), retargeted: false, relayVia: null };
  }
