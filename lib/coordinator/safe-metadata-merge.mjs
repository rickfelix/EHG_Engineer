 ...
 * jsonbColumn) combinations the generic core will ever interpolate into SQL. A caller
 * naming anything outside this set is refused — this is the only thing standing between a
 ...
 */
export const JSONB_MERGE_ALLOWLIST = {
  strategic_directives_v2: { keyColumns: ['sd_key', 'id'], jsonbColumn: 'metadata' },
  ...
  product_requirements_v2: { keyColumns: ['id'], jsonbColumn: 'metadata' },
};

function assertAllowedTarget(table, keyColumn, jsonbColumn) {
  const spec = JSONB_MERGE_ALLOWLIST[table];
  if (!spec) {
 ...
 * guard as mergeJsonbColumn. `extraGuardSql`/`extraGuardParams` let a caller append an
 * additional WHERE condition (e.g. a claim compare-and-swap) without the core needing to
 * know about claim semantics — extraGuardParams are appended after keyValue and key,
 * so any placeholders inside extraGuardSql must be numbered starting at $3.
 *
 * @param {object} args
 ...
 * @param {string} [args.extraGuardSql] - additional "AND ..." clause, e.g. "AND claiming_session_id = $3"
 * @param {Array} [args.extraGuardParams] - params for the extra guard, appended after [keyValue, key]
 * @returns {Promise<{rowCount: number}>}
 */
export async function removeJsonbColumnKey({ table, keyColumn, keyValue, jsonbColumn, key, client, extraGuardSql = '', extraGuardParams = [] }) {
  assertAllowedTarget(table, keyColumn, jsonbColumn);
  const sql = `UPDATE ${table}
       SET ${jsonbColumn} = COALESCE(${jsonbColumn}, '{}'::jsonb) - $2::text
       WHERE ${keyColumn} = $1${extraGuardSql ? ` ${extraGuardSql}` : ''}`;
  return client.query(sql, [keyValue, key, ...extraGuardParams]);
}
...
export async function removeMetadataKeyIfClaimedBy(id, key, claimingSessionId, opts = {}) {
  ...
  try {
    const result = await removeJsonbColumnKey({
      ...
      jsonbColumn: 'metadata', key, client,
      extraGuardSql: 'AND claiming_session_id = $3', extraGuardParams: [claimingSessionId],
    });
