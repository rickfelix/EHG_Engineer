...
export function triggerRankPass(opts = {}) {
  ...
  try {
    // QF-20260823-561: strip the caller's own CLAUDE_SESSION_ID before inheriting env — otherwise
    // the spawned ranker stamps THIS session's id into dispatch_rank_by for every row it re-ranks
    // (a mint/review-clearance/completion from Adam, Solomon, or a worker laundered into "this role
    // dispatched the whole belt"). The triggering session is preserved separately via
    // RANK_TRIGGERED_BY, which coordinator-backlog-rank.mjs records on its own dedicated column.
    const { CLAUDE_SESSION_ID: triggeringSessionId, ...inheritedEnv } = process.env;
    const child = spawnFn('node', [RANKER_SCRIPT], {
      detached: true,
      stdio: 'ignore',
      env: { ...inheritedEnv, RANK_EVENT_TRIGGER: '1', RANK_TRIGGERED_BY: triggeringSessionId || '' },
    });
    child.on('error', (err) => {
