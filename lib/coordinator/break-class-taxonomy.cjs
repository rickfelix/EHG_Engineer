 ...
 */
// @wire-check-exempt: dependency-root contract module for the BREAKAGE-DETECTOR decomposition;
// consumers are sibling children B-F (spine A->B->{C,D}->{E,F}) which are not yet built. Correctly
// unreferenced until child C/D/F imports it — NOT dead code. REMOVE this marker when a sibling
// imports the taxonomy (then it is genuinely reachable and must be wire-checked). Coordinator-
// sanctioned exemption for a WIRE_CHECK reachability false-positive on a decomposition root.
'use strict';
