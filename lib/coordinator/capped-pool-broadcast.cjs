...
async function tick(supabase, { repoRoot, coordinatorId, used, cap, statePath, seats } = {}) {
  ...
  if (result.written === 0) {
    if (decision.action === 'emit') {
      ...
      // ROUND-2 FIX (adversarial post-merge review, PR #8356): reverting ONLY
      // last_cleared_notice_at is inert -- decide()'s not-overCap branch ALWAYS sets
      // over_cap_since:null in nextState, and emittedForThisEpisode (decide()'s gate for even
      // considering a clear notice) requires over_cap_since != null. Without also restoring
      // over_cap_since here, a zero-seat clear fan-out permanently drops the episode marker,
      // so the clear notice can never be retried on any later tick. Restore BOTH so the next
      // tick sees the exact same "cleared, notice not yet delivered" state as this one did.
      nextState = { ...nextState, over_cap_since: state.over_cap_since, last_cleared_notice_at: state.last_cleared_notice_at };
    }
  }
