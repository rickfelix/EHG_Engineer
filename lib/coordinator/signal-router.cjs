...
async function loadRecentSignals(supabase, windowMin = WINDOW_MIN) {
  ...
  try {
    data = await fapPaginate(() => supabase
      ...
      // SD-LEO-INFRA-SIGNAL-LANE-PER-001 (FR-4): acknowledged_at was missing from this select
      // while stampRoutedToCoordinator() (below) and ackAndRouteLoneSignal() both branch on it —
      // undefined made the idempotency guard at stampRoutedToCoordinator unconditionally false
      // (re-stamping every tick, corrupting source_age_ms) and degraded the other guard to
      // routed_to_coordinator-only. Confirmed via TESTING's PLAN-TO-EXEC review (fd168314).
