...
async function loadRecentSignals(supabase, windowMin = WINDOW_MIN) {
  ...
  let data;
  // SD-LEO-INFRA-SIGNAL-LANE-PER-001 (FR-4): acknowledged_at was missing from this select while
  // stampRoutedToCoordinator() (below) and ackAndRouteLoneSignal() both branch on it — undefined
  // made the idempotency guard at stampRoutedToCoordinator unconditionally false (re-stamping
  // every tick, corrupting source_age_ms) and degraded the other guard to routed_to_coordinator-
  // only. Confirmed via TESTING's PLAN-TO-EXEC review (fd168314).
  try {
    data = await fapPaginate(() => supabase
      .from('session_coordination')
      .select('id, sender_session, target_session, payload, body, created_at, acknowledged_at')
...
/**
 * STILL NO PRODUCTION CALLER (SD-LEO-INFRA-SIGNAL-ROUTER-AUTO-001, FR-7; confirmed still true by
 * SD-LEO-INFRA-SIGNAL-LANE-PER-001's own TESTING pass at EXEC-TO-PLAN, bfb24a47) — this SD's FR-4
 * exercised the "retire" branch of its own PRD's explicit either/or (wire it live, or retire it),
 * not the "wire" branch: ackAndRouteLoneSignal() has NO caller in lib/sweep/passes/ or
 * lib/sweep/legacy-fallback.cjs, deliberately. Wiring a previously-fully-inert, live-DB-writing
 * function into the production sweep tick is a real behavior change (the coordinator would start
 * receiving a routed-to-coordinator marker for every medium+ severity lone signal, every tick) with
 * its own review surface (a parity test, tests/ci/sweep-legacy-twin-parity.test.js, tracks every
 * function call in both the pass and legacy-fallback twins) — out of scope for a dedup/disposition
 * SD to decide unilaterally. What THIS SD fixed is the two real bugs that would otherwise poison
 * whichever future SD DOES wire it in:
 *
 * *** THIS PATH USED TO DISPOSE, AND THAT WAS THE SAME BUG stampRouted() WAS FIXED FOR. ***
 * The original version of this function stamped acknowledged_at unconditionally alongside
 * routed_to_coordinator — an automated sweep marking a signal "handled" the instant it noticed
 * the signal, before any coordinator had actually acted on it. That is EXACTLY the class of bug
 * stampRouted()'s own docblock names ("9 rows, 100% of everything ever promoted, all
 * severity=critical... nothing re-raised them, because an acked row looks handled"). VALIDATION's
 * PLAN-TO-EXEC review (eb009c8e) flagged this dormant path as "the most likely place FR-4 gets
 * wired" and warned against reintroducing that exact defect. So: this function now ONLY
 * annotates provenance (routed_to_coordinator: true); it never touches acknowledged_at. The row
 * stays open/undispositioned — visible in FR-2's oldest-first surface — until a human
 * coordinator explicitly dispositions it via coordinator-ack-signal.cjs's FR-1 canonical writer,
 * which is the ONLY path in the codebase permitted to stamp acknowledged_at + a disposition
 * together (TR-4, the single-canonical-writer invariant).
 *
 * The idempotency check is now keyed purely on `routed_to_coordinator` (acknowledged_at is no
 * longer written by this function at all, so it can never be part of this function's own
 * idempotency condition) — this ALSO fixes the fixture-blind bug TESTING found (the old
 * `r.payload?.routed_to_coordinator && r.acknowledged_at` check was unconditionally false while
 * loadRecentSignals omitted acknowledged_at from its select, making the guard unreachable in
 * production and re-stamping every tick).
 */
async function stampRoutedToCoordinator(supabase, rows) {
  // Mark each contributing row routed-to-coordinator. Distinct from stampRouted(): NO
  // routed_to_feedback_id (no harness_backlog row exists for a lone signal).
  for (const r of rows) {
    if (r.payload?.routed_to_coordinator) continue; // idempotent
    const merged = {
      ...(r.payload || {}),
