    ...
    try {
      ...
      if (similar.length > 0 && similar[0].similarity > 0.4) {
        ...
        const created = await issueKB.createPattern({
          ...
          // SD-FDBK-ENH-LEARNING-LOOP-DESTROYS-001 / FR-4b — 'team_investigation' is not in
          // issue_patterns_source_check, so this raised 23514 and destroyed the finding. A DIFFERENT
          // error code from the PAT-001 collision (23505) on the SAME call, so either alone was
          // sufficient to lose the lesson and fixing one would have left it still broken.
          // 'manual' is the legal value closest in meaning; the agent identity is not lost, it is
          // already carried in related_sub_agents just above.
          source: 'manual',
        });
