...
export class SubAgentOrchestrator {
  ...
  async checkSynthesisReady(sdId, phase) {
    ...
    try {
      ...
      // QF-20260906-403: normalize the query param -- see results-storage.js's write-side fix.
      const { data: executions, error } = await this.supabase
        .from('sub_agent_execution_results')
        .select('*')
        .eq('sd_id', sdUuid)
        .eq('phase', normalizePhaseToken(phase))
  ...
  async getExecutionHistory(sdId, options = {}) {
    try {
      ...
      if (options.phase) {
        // QF-20260906-403: normalize the query param -- see results-storage.js's write-side fix.
        query = query.eq('phase', normalizePhaseToken(options.phase));
      }
