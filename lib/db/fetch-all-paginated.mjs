 ...
 * @param {() => any} queryFactory e.g. () => sb.from('feedback').select('id').eq(...)
...
export async function fetchAllPaginated(queryFactory, { pageSize = POSTGREST_MAX_ROWS, maxPages = 10000, maxRows = Infinity } = {}) {
  ...
  if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > POSTGREST_MAX_ROWS) {
    throw new Error(`fetchAllPaginated: pageSize must be an integer in [1, ${POSTGREST_MAX_ROWS}], got ${pageSize}`);
  }
  const all = [];
  ...
  // the would-be OOM hang into a diagnosable error.
  throw new Error(`fetchAllPaginated: exceeded ${maxPages} pages — query builder likely ignores .range()`);
}
