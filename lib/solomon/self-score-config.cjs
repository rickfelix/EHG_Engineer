...
const SCORERS = {
  ...
  // quota discipline: consult/audit ledger breach count, PLUS (QF-20260905-768) attached
  // in-process gatherer spend -- a seat can run gatherer agents idle past report and never touch
  // the consult ledger, so D3 must read BOTH signals or it is not reading the spend it claims to.
  // Inconclusive only when NEITHER signal is available; either alone still scores honestly.
  D3_silence_cost_discipline: (s) => {
    const quota = s.quota_breach_count;
    const flagged = s.attached_agent_flag_count;
    if (quota == null && flagged == null) return { score: null, provenance: 'no quota_breach_count or attached_agent_flag_count signal (inconclusive)' };
    const n = (quota || 0) + (flagged || 0);
    const score = n === 0 ? 5 : n === 1 ? 3 : 1;
    return {
      score,
      signal: `quota_breach_count=${quota ?? 'n/a'} attached_agent_flag_count=${flagged ?? 'n/a'}`,
      provenance: 'per-SD/per-day consult quota breaches plus idle/overrun attached gatherer agents this cycle',
      red_flag: n > 1,
    };
  },
