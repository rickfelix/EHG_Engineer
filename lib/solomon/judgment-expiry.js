 ...
 * grandfathered row. This classifier gates the runner's FIRST update attempt, which never touches
 * decision_by at all (round-3 note: the runner's bounded retry on a TRUE result from this classifier
 * DOES then touch decision_by, deliberately -- see violatesDecisionByIdentityCheck below and the
 * runner's dry-run disclosure of that rewrite before it happens). A violating row that is still
 * `decision='pending'` when it ages out would 23514 on this loop's first update forever without the
 * retry, and the catch-and-continue below already silently degrades stamped=N/M for any error --
 * exactly the failure class this table has shipped five times already (see this file's own header).
 * Pure classifier so the runner's retry decision is unit-testable without a DB.
 ...
 * self-healing retry (see the runner's main loop) performs the SAME destructive normalization that
 * irreversibly destroyed 1212 rows in the 2026-08-21 incident -- as an automatic side effect of an
 * unrelated aging stamp, with no operator authorization at the moment of destruction. The dry run
 * must disclose this BEFORE a live run, not just count it afterward. Mirrors the migration's CHECK
 * exactly: decision_by IS NULL OR (length(decision_by) <= 40 AND decision_by !~ '[[:space:]]').
 * @param {string|null|undefined} decisionBy
 * @returns {boolean}
 */
export function violatesDecisionByIdentityCheck(decisionBy) {
  if (decisionBy == null) return false;
  const s = String(decisionBy);
