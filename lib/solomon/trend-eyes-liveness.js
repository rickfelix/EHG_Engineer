...
export function checkTrendEyesLiveness(lastReceiptAt, nowMs, thresholdMs = STALE_RECEIPT_THRESHOLD_MS) {
  ...
  // A FUTURE-DATED receipt must alarm, not reassure. A negative age fails `> thresholdMs` for as
  // long as the timestamp stays ahead of the clock, so a single bad write — or a clock skew — would
  // pin this predicate to alarm:false PERMANENTLY, silencing the one alarm that notices a dead
  // sweep. A receipt from the future is evidence something is wrong, never evidence of freshness.
  if (ageMs < 0) {
    return { alarm: true, ageMs, reason: `receipt is dated ${Math.round(-ageMs / 60000)}min in the FUTURE — clock skew or a bad write, not a fresh run` };
  }
