...
export async function resolveSolomonConductFacts(supabase, { staleDays = DEFAULT_STALE_DAYS, now = new Date() } = {}) {
  ...
  try {
    ...
    // aging it.
    //
    // QF-20260912-681 considered adding a decision_requested=true filter here, but
    // tests/unit/solomon-conduct-probes-decision-requested-unaffected.test.js (TS-7,
    // SD-ALTIFYAI-LEO-FIX-SOLOMON-ADVICE-LEDGER-001) already ratifies the OPPOSITE: this probe must
    // NOT read decision_requested at all, so a stale informational row still counts here. That fix
    // is also unnecessary: scripts/solomon-advisory.cjs's captureLedgerRow now writes
    // decision='deferred' (never 'pending') for informational sends, so they never enter this
    // 'pending' count in the first place — no probe-side change needed, and TS-7 stays intact.
