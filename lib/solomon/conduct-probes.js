...
export async function resolveSolomonConductFacts(supabase, { staleDays = DEFAULT_STALE_DAYS, now = new Date() } = {}) {
  ...
  try {
    ...
    // aging it.
    //
    // QF-20260912-681: also requires decision_requested=true. This is an ADMISSION-SCOPE filter,
    // orthogonal to the anti-aging principle above (it never touches judgment_expired_at or lets a
    // row escape by aging) -- it excludes only rows that NEVER asked for a decision in the first
    // place (informational sends, captureLedgerRow decisionRequested:false). 20260821_solomon_
    // ledger_decision_requested.sql's column comment says this predicate is "DELIBERATELY NOT read
    // by" this probe "to not fold the two predicates into one helper" -- that guidance is about NOT
    // substituting decision_requested for expiry-tracking (the anti-aging axis); it does not cover
    // this narrower admission-scope use. Without this filter every informational send becomes a
    // permanent, unclosable FAIL the moment it ages past staleDays (measured live 2026-09-13: 54
    // stale-pending rows, 32 of them decision_requested=false and structurally unable to ever
    // receive a decision) -- an instrument that always reads FAIL carries no signal. Fails loud, not
    // silent, if the column is missing (same as the existing decision-column dependency above).
