...
export class PatternDetectionEngine {
  ...
  async createPattern(issue, similarIssues, sdId) {
    // SD-FDBK-ENH-LEARNING-LOOP-DESTROYS-001 / FR-7 — the SECOND copy of the same defect.
    //
    // Independently broken from the one in issue-knowledge-base.js and not named in the SD: same
    // /PAT-(\d+)/ against a queried maximum, ordered by created_at instead of pattern_id. The
    // ordering column differs; the failure does not. Whenever the newest row is not a bare
    // PAT-nnn — and 1650 of 1658 rows are not — the regex misses, patternNum stays 1, and the id
    // is PAT-001, which exists. Fixing only the other copy would have left this one live.
    //
    // FIXED RATHER THAN DELETED, deliberately, against the PRD's own suggestion. No importer
    // exists (verified: an import/require search finds zero, with a control proving the same
    // search finds importers of issue-knowledge-base). But NO IMPORTER FOUND IS NOT PROOF OF NO
    // CALLER — a dynamic import or a direct CLI invocation would not appear in that search, and
    // this session has already produced three wrong call-site claims from searches that looked
    // conclusive. Deleting 527 lines on a reachability inference risks far more than repairing
    // fifteen, and repairing satisfies the actual requirement — that no second broken copy
    // remains — just as well as deleting would.
    const patternId = `PAT-DET-${createHash('sha256')
      .update([issue.description || '', sdId || ''].join(FIELD_SEP))
      .digest('hex')
      .slice(0, 12)}`;
