 ...
 *
 * `now` names when the site was first seen. It defaults to (and, on an invalid/unparseable
 * value, FALLS BACK to) the current wall-clock time — the correct behavior for the real-time
 * RCA-trigger callers (lib/rca/rca-orchestrator.js), which never pass a 3rd/4th argument at
 * all. A caller backfilling a historical occurrence (e.g. from an old retrospective) may pass
 * its own Date to record the real historical first-seen time instead of "now" — SD-LEARN-FIX-
 * ADDRESS-PATTERN-LEARN-158. An invalid Date (NaN getTime()) is treated as "not supplied" so a
 * malformed historical timestamp can never produce an Invalid Date write or throw.
...
export function mergeSite(metadata, site, now = new Date()) {
  ...
  if (!exists) {
    sites.push({
      ...
      first_seen: effectiveNow.toISOString(),
    });
...
export async function escalateToClassSd(supabase, pattern, { now = new Date() } = {}) {
  try {
    ...
    // QF-20260911-015: createSD never invents its own key (pipeline.js SDKEY_REQUIRED
    // guard) -- every caller must generate one first. Mirrors pattern-alert-sd-creator.js's
    // PATTERN-source key generation, the precedent for this same automated-draft class.
    const { generateSDKey } = await import('../../scripts/modules/sd-key-generator.js');
    const input = buildClassSdInput(pattern);
    const generatedKey = await generateSDKey({ source: 'PATTERN', type: input.type, title: input.title });
    ...
    // enforced without breaking this flow. Placed after the spread so an explicit
    // input.criticality (none exist today) would still win.
    const created = await createSD({
      ...input,
      sdKey: generatedKey,
      criticality: input.criticality || 'later',
      criticalityReason: input.criticalityReason || 'auto-generated by class-escalation pattern-alert, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4',
    });
    ...
    // for this case, with no sd_key. Without this guard the `|| generatedKey` fallback below would
    // stamp assigned_sd_id with a key that was never actually inserted (TESTING finding, EXEC-TO-PLAN).
    if (created?.routedLater) {
      console.log(`[class-escalation] ${pattern.pattern_id} routed to harness_backlog (criticality=later), not stamping — pattern remains unescalated`);
      return null;
    }
    const sdKey = created?.sd_key || created?.sdKey || created?.id || generatedKey;
    if (!sdKey) {
