...
// REGRESSION (PLAN_VERIFICATION): resolved from this module's own location, not process.cwd().
// recordOccurrence() now always passes target_test_paths, so a caller invoked from a non-repo-root
// cwd would otherwise turn a previously-total call into a throwing one.
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');
const DEFAULT_MANIFEST_PATH = path.join(REPO_ROOT, 'tests', 'quarantine-manifest.json');
