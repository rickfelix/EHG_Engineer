...
export class IssueKnowledgeBase {
  ...
  async recordOccurrence(data) {
    const {
      ...
      target_test_paths = [],
      // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-158: additive, optional. When a caller is recording
      // a HISTORICAL occurrence (e.g. backfilling from an old retrospective), pass the real
      // event time here so the recorded site's first_seen reflects it instead of "now" (when the
      // backfill happened to run). Omitted/undefined/invalid -> byte-identical to before this SD
      // (site.first_seen defaults to current wall-clock time; see mergeSite's own fallback).
      occurred_at
    } = data;
    ...
    try {
      ...
      const occurredAtDate = occurred_at !== undefined && occurred_at !== null ? new Date(occurred_at) : undefined;
      await recordSiteAndMaybeEscalate(supabase, updated, { sd_id }, occurredAtDate ? { now: occurredAtDate } : {});
    } catch (escErr) {
  ...
  async createPattern(data) {
    const {
      ...
      source_feedback_ids = [],
      // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-158: additive, optional. See recordOccurrence's
      // occurred_at for the same rationale -- a brand-new pattern mined from an old, backlog-
      // drained retro should not be born with today's date as its created_at.
      occurred_at
    } = data;
    ...
    // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-158: an invalid/unparseable occurred_at is treated as
    // "not supplied" (same fallback contract as mergeSite) so a malformed historical timestamp
    // can never write an Invalid Date into created_at -- the DB's own now() default applies.
    const occurredAtDate = occurred_at !== undefined && occurred_at !== null ? new Date(occurred_at) : null;
    const validOccurredAt = occurredAtDate && !Number.isNaN(occurredAtDate.getTime()) ? occurredAtDate : null;
    ...
    const pattern = {
      ...
      ...(validOccurredAt ? { created_at: validOccurredAt.toISOString() } : {})
    };
