...
// QF-20260901-005: 28 pinned the pool below a six-seat fleet's real footprint (~4-6 trees/seat
// across qf/sd/adhoc), so the pool hit 28/28 and refilled to 30/28 within minutes of a hand
// reap. Raised with margin above 6*6=36; still a hard cap, not seat-derived, since seat count
// itself is not read here.
//
// QF-20260914-197: 40 filled again (measured composition: 18 adhoc + 10 SD + 8 QF + 1 docs +
// 1 reaper-source + 1 root = 39/40) while the reaper was deliberately held off the fleet
// (QF-20260914-380, a suspected worktree-content-destruction incident under investigation), so
// there was no documented unblock short of a code change. Raised again with margin, and made
// env-overridable (mirrors WORKTREE_DISK_FLOOR_BYTES below) so the next full pool does not
// require another code change while the reaper stays untrusted.
export const MAX_WORKTREE_COUNT = Math.max(1, Number(process.env.WORKTREE_MAX_COUNT) || 60);
 ...
 */
// Exported for unit testing (QF-20260829-412) — the `locked` line parsing is
// otherwise only reachable via a real `git worktree list --porcelain` shell-out.
export function parsePorcelain(raw) {
  ...
  for (const line of raw.split('\n')) {
    if (line.startsWith('worktree ')) {
      ...
      // QF-20260829-412: porcelain emits a bare `locked` line, or `locked <reason>`
      // when the worktree was locked with a reason string. Previously dropped
      // entirely, so callers had no way to distinguish a locked ghost registration
      // (directory gone, entry stuck) from a normal worktree.
      current.locked = true;
      const reason = line.slice('locked'.length).trim();
      if (reason) current.lockedReason = reason;
    }
  }
  if (current.path) worktrees.push(current);
  return worktrees;
 ...
 * @returns {Array<{path: string, branch?: string, head?: string, bare?: boolean, detached?: boolean, prunable?: boolean, locked?: boolean, lockedReason?: string}>}
 */
