...
export function validateSdExtras(payload) {
  ...
  for (const [i, m] of metrics.entries()) {
    ...
    if (matched) {
      // SECURITY evidence 52504464-fc59-44be-8178-f6a1cf0182b0 (SEC-LOG-ECHO-001): truncate,
      // mirroring the sibling smoke-step warning below — an untruncated echo of arbitrary
      // author-supplied text into console output is unnecessary here (only the matched
      // boilerplate pattern needs surfacing) and caps stderr volume against a pathological input.
      warnings.push({
        field: `success_metrics[${i}]`,
        expected: 'a concrete, SD-specific metric name and target',
        got: (metricText || targetText).slice(0, 60),
