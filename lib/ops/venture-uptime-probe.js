...
/**
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-F (C4.3): aggregate the most-recently-checked
 * probe status for a venture across all of its venture_deployments rows -- a venture can
 * have more than one deployment/URL, so this is NOT a .single() lookup. Rows with no
 * metadata.probe yet (freshly seeded via ensureDeploymentRows, never actually probed) are
 * ignored rather than treated as "unreachable" -- that would be a false negative on a
 * venture whose probe simply hasn't run yet. Returns null when the venture has no
 * deployment rows, or none carry probe data.
 *
 * @param {Object} supabase
 * @param {string} ventureId
 * @returns {Promise<{ url: string, reachable: boolean, status_code: number|null, last_error: string|null, consecutive_failures: number, surfaced: boolean, last_checked_at: string } | null>}
 */
export async function getLatestProbeStatus(supabase, ventureId) {
  if (!supabase || !ventureId) return null;

  // Bounded (not fetchAllPaginated): scoped to ONE venture's own deployment rows, not a
  // full-table scan -- 200 is far beyond any real venture's URL count and keeps this read
  // provably capped per count-truncation-diff-lint.
  ...
  const withProbe = data
    .map((row) => ({ url: row.url, probe: row.metadata?.probe }))
    .filter((r) => r.probe && typeof r.probe === 'object');

  if (withProbe.length === 0) return null;

  // Most-recently-checked wins. A missing/malformed last_checked_at sorts as oldest
  // (epoch, via NaN -> 0) rather than throwing or winning a comparison it has no real
  // claim to.
  const timeOf = (r) => {
    const t = r.probe.last_checked_at ? Date.parse(r.probe.last_checked_at) : NaN;
    return Number.isFinite(t) ? t : 0;
  };
  const latest = withProbe.reduce((best, r) => (timeOf(r) > timeOf(best) ? r : best));

  return { url: latest.url, ...latest.probe };
}
