...
describe('lib/worktree-quota', () => {
  ...
  describe('exports', () => {
    it('re-exports MAX_WORKTREE_COUNT as 60 by default (raised QF-20260914-197)', () => {
      expect(MAX_WORKTREE_COUNT).toBe(60);
    });

    it('MAX_WORKTREE_COUNT is overridable via WORKTREE_MAX_COUNT (QF-20260914-197)', async () => {
      const prev = process.env.WORKTREE_MAX_COUNT;
      try {
        process.env.WORKTREE_MAX_COUNT = '75';
        vi.resetModules();
        const reimported = await import('./worktree-quota.js?override-test');
        expect(reimported.MAX_WORKTREE_COUNT).toBe(75);
      } finally {
        if (prev === undefined) delete process.env.WORKTREE_MAX_COUNT;
        else process.env.WORKTREE_MAX_COUNT = prev;
      }
    });

    it('MAX_WORKTREE_COUNT falls back to the default on an invalid WORKTREE_MAX_COUNT', async () => {
      const prev = process.env.WORKTREE_MAX_COUNT;
      try {
        process.env.WORKTREE_MAX_COUNT = 'not-a-number';
        vi.resetModules();
        const reimported = await import('./worktree-quota.js?invalid-override-test');
        expect(reimported.MAX_WORKTREE_COUNT).toBe(60);
      } finally {
        if (prev === undefined) delete process.env.WORKTREE_MAX_COUNT;
        else process.env.WORKTREE_MAX_COUNT = prev;
      }
