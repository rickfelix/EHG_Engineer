...
// QF-20260901-879: the global 120s default is shorter than this repo's full E2E suite (~439s),
// so TESTING's evidence attempts always time out and EXEC-TO-PLAN blocks forever on
// SUBAGENT_EVIDENCE_MISSING. Per-code override, env-tunable per code, comfortably above
// measured worst-case suite runtime + margin -- other sub-agents keep the tighter global default.
const PER_CODE_TIMEOUT_MS = { TESTING: 600000 };

/** Pure, exported for unit testing: same precedence chain used at the real call site below. */
export function resolveSubAgentTimeoutMs(code, optionsTimeout, env = process.env) {
  return optionsTimeout
    || parseInt(env[`SUB_AGENT_TIMEOUT_MS_${code}`] || '', 10)
    || PER_CODE_TIMEOUT_MS[code]
    || parseInt(env.SUB_AGENT_TIMEOUT_MS || '120000', 10);
}

// SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-C: the only value of root_cause_reports.trigger_source
// this dispatch path is allowed to write -- the live root_cause_reports_trigger_source_check
// CHECK constraint accepts SUB_AGENT/MANUAL/TEST_FAILURE/CI_PIPELINE and rejects anything else
// (TESTING sub-agent EXEC-phase re-verify, evidence 28382f71: an earlier 'AGENT_DISPATCH' value
// silently violated this constraint on every real INSERT, making the whole fix a no-op in
// production while the mocked test suite stayed green). Also used to SCOPE the reuse lookup
// below so this mechanism only ever reuses an RCR it created itself, never an unrelated
// legitimate OPEN row from a different source (the auto-trigger orchestrator, a manual CLI
// trigger, etc.) -- reusing an unrelated row would let this dispatch overwrite someone else's
// in-progress incident analysis (same evidence: 79 SDs already carry >1 OPEN RCR).
const RCA_DISPATCH_TRIGGER_SOURCE = 'SUB_AGENT';

/**
 * SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-C (FR-C1): resolve the id to dispatch to lib/sub-agents/
 * rca.js's execute() -- an existing OPEN root_cause_reports row THIS mechanism created for this
 * SD if one exists, or a newly-created one. Exported, DI'd on the supabase client, for direct
 * unit testing without driving the full dispatch pipeline or touching the real rca.js module
 * file. Throws on a genuine creation failure (SECURITY/TESTING EXEC-phase re-verify) rather than
 * silently falling back to the SD UUID -- a silent fallback reproduces the exact pre-fix defect
 * this SD exists to close, invisibly.
 */
export async function resolveRcaDispatchTarget(rcaSupabase, sdUUID, sdKey, options = {}) {
  const { data: existingRcr, error: rcrLookupError } = await rcaSupabase
    .from('root_cause_reports')
    .select('id')
    .eq('sd_id', sdUUID)
    .eq('status', 'OPEN')
    .eq('trigger_source', RCA_DISPATCH_TRIGGER_SOURCE)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (rcrLookupError) {
    console.log(`   Warning: RCR lookup failed for RCA dispatch: ${rcrLookupError.message}`);
  }
  if (existingRcr?.id) {
    console.log(`   [RCA] Reusing existing OPEN root_cause_reports row: ${existingRcr.id}`);
    return existingRcr.id;
  }

  const { data: newRcr, error: rcrCreateError } = await rcaSupabase
    .from('root_cause_reports')
    .insert({
      scope_type: 'SD',
      scope_id: sdUUID,
      sd_id: sdUUID,
      trigger_source: RCA_DISPATCH_TRIGGER_SOURCE,
      trigger_tier: 4,
      failure_signature: `agent-dispatch:${sdUUID}:${Date.now()}`,
      problem_statement: options.objective || options.prompt || `Agent-invoked RCA for ${sdKey}`,
      observed: { description: 'Agent-invoked RCA dispatch' },
      ...
      status: 'OPEN',
      metadata: { created_by: 'agent_dispatch', dispatched_via: 'lib/sub-agent-executor/executor.js' },
    })
    .select('id')
    .single();
  if (rcrCreateError || !newRcr) {
    throw new Error(`resolveRcaDispatchTarget: failed to create root_cause_reports row: ${rcrCreateError?.message || 'no row returned'}`);
  }
  console.log(`   [RCA] Created new root_cause_reports row: ${newRcr.id}`);
  return newRcr.id;
}

// SD-LEO-INFRA-EXECUTOR-120S-1800S-001 / FR-1: a named sentinel so a timeout is identified
// unambiguously in the catch block below -- a Promise.race() rejection alone carries no
// information about which side lost, and matching on the message string is fragile.
class SubAgentTimeoutError extends Error {
  constructor(code, timeoutMs) {
    super(`Sub-agent ${code} timed out after ${timeoutMs}ms`);
...
export async function executeSubAgent(code, sdId, options = {}) {
  ...
  try {
    ...
    const modulePathSpecifier = `../sub-agents/${code.toLowerCase()}.js`;
    let timeoutHandle;
    try {
      const subAgentModule = await import(modulePathSpecifier);
      ...
      if (subAgentModule.execute) {
        ...
        // QF-20260901-879: a per-code override (env-tunable via SUB_AGENT_TIMEOUT_MS_<CODE>) wins
        // over the global default -- TESTING's E2E suite runtime exceeds 120s.
        const timeoutMs = resolveSubAgentTimeoutMs(code, options.timeout);
        // SD-LEO-INFRA-EXECUTOR-120S-1800S-001 / FR-2: capture the handle so it can be
        // cleared below on the fast path -- previously never cleared, leaking a pending
        // timer for the remaining duration in any in-process, non-exiting caller (the
        // phase-subagent-orchestrator, vitest). Invisible on the CLI path, which calls
        // process.exit() and force-kills all pending timers.
        ...
        // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-C (FR-C1): lib/sub-agents/rca.js is the ONLY
        // dispatchable sub-agent module whose execute() expects a root_cause_reports (RCR) row
        // id, not an SD id (RISK sub-agent evidence 0cae276e surveyed all 27 dispatchable
        // modules and confirmed this). Resolve an existing OPEN RCR for this SD, or create one,
        // before dispatching -- every other code's dispatch argument is unchanged.
        const dispatchTargetId = code === 'RCA' && sdUUID
          ? await resolveRcaDispatchTarget(await getSupabaseClient(), sdUUID, sdKey, options)
          : (sdUUID || sdId);
        results = await Promise.race([
          subAgentModule.execute(dispatchTargetId, subAgent, execOptions),
          timeoutPromise
        ]);
      ...
      clearTimeout(timeoutHandle);
      // SD-LEO-INFRA-EXECUTOR-120S-1800S-001 / FR-1: discriminate the real cause instead of
      // collapsing timeout, genuine error, and missing-module into one hardcoded label with
      // the exception discarded (metadata.error/stack were never populated). Verdict is
      // DELIBERATELY UNCHANGED for all 3 causes -- still 'MANUAL_REQUIRED', see FR-3: routing
      // through verdict='ERROR' would silently convert subagent-evidence-gate.js's advisory,
      // SUBAGENT_VERDICT_MODE-gated soft warning into an unconditional hard block on
      // PLAN-TO-EXEC/EXEC-TO-PLAN fleet-wide -- a blast radius out of scope for this fix.
      //
      // TESTING/SECURITY retrospective review (evidence ce5f58e3/2f9ab06f): a sub-agent
      // execute() rejecting with a non-Error (null, undefined, a bare string) made every
      // branch below's `err.message` dereference throw a TypeError INSIDE this catch --
      // escaping to the OUTER catch, which stores verdict='ERROR', reproducing exactly the
      // blast radius FR-3 exists to prevent. Normalizing here, before any branch touches it,
      // is what makes this catch block actually exception-safe for its own exception.
      const err = rawErr instanceof Error ? rawErr : new Error(String(rawErr));
      let failureCause;
      if (err instanceof SubAgentTimeoutError) {
        failureCause = 'timeout';
      } else {
        // TR-3: resolve the module specifier THE SAME WAY the dynamic import() above
        // resolves it (relative to this file's own URL) -- never a naive fs.existsSync()
        // against the raw relative specifier, which resolves against process.cwd() and is
        // confirmed wrong from every measured cwd (it would invert this fix, labeling every
        // failure 'missing_module'). Also never inferred from error.code alone: a
        // transitively-missing dependency inside an EXISTING module throws the identical
        // ERR_MODULE_NOT_FOUND code as a genuinely-missing top-level module.
      ...
      if (failureCause === 'missing_module') {
        ...
        console.log(`\nWarning: ${code} execution timed out`);
        results = {
          verdict: 'MANUAL_REQUIRED',
          confidence: 50,
          message: err.message,
          recommendations: [
            `Investigate why ${code}'s execute() did not resolve within the configured timeout`,
            'Consider whether SUB_AGENT_TIMEOUT_MS needs to be raised for this sub-agent'
          ],
          error: err.message,
          stack: err.stack,
          metadata: { failure_cause: failureCause }
        ...
        console.log(`\nWarning: ${code} execution threw an error: ${err.message}`);
        results = {
          verdict: 'MANUAL_REQUIRED',
          confidence: 50,
          message: `${code} execution failed: ${err.message}`,
          recommendations: [
            'Investigate the thrown error captured in metadata.error/metadata.stack',
            'Perform analysis according to sub-agent description'
          ],
          error: err.message,
          stack: err.stack,
    ...
    try {
      ...
      // SD-FDBK-ENH-RETRO-SUB-AGENT-001 FR-3: warnings (e.g. ambiguous_basename_match) do not
      // fail the check -- passed can be true -- so this logs independently of the pass/fail
      // branch above. Nesting it inside the failing branch would mean a passing-but-ambiguous
      // result is never logged at all, which is the exact gap PLAN-phase review found.
      if (hallucinationCheck.warnings && hallucinationCheck.warnings.length > 0) {
        console.log(`   Hallucination check warnings: ${hallucinationCheck.warnings.map(w => w.message || w.type || String(w)).join('; ')}`);
      }
    ...
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (SECURITY sub-agent, EXEC, S2): a payload-completeness
    // readback failure means storeSubAgentResults already wrote a row with a genuine verdict and
    // mostly-real content, and only THEN found one field missing on independent re-read — the
    // write itself succeeded. Falling through to the errorResult store below would, within the
    // 5-minute dedup window (same sd_id+code+phase), take the UPDATE branch and overwrite that
    // row with a content-free ERROR record: the control meant to catch dropped content would
    // itself destroy content that mostly survived. The already-written row stays as the record.
    if (error?.isPayloadCompletenessFailure) {
      throw error;
    }
    ...
    try {
      // SD-LEO-ORCH-CAPA-CONTRACT-TRUTH-001-A: thread the caller's phase into the ERROR row.
      //
      // This fallback previously passed NO options at all, unlike the primary write above which
      // threads { sdKey, phase }. With no phase, results-storage falls back to querying
      // strategic_directives_v2.current_phase — so EVERY error caught here was stamped with
      // wherever the SD happens to be, not the phase that was actually invoked. Observed live:
      // a run invoked with --phase PLAN_VERIFY produced an ERROR row stamped EXEC, which makes
      // an error-path row impossible to attribute to the run that produced it.
      //
      // Deliberately passes ONLY options.phase, not the primary's `options.phase ||
      // rawCurrentPhase`: sdKey and rawCurrentPhase are declared with const INSIDE the try block
      // and are NOT in scope here, so referencing them would throw a ReferenceError inside the
      // error handler — turning a recoverable failure into a lost row. options is a function
      // parameter and is always in scope. When options.phase is absent the existing
      // current_phase derivation still applies, so this strictly adds fidelity and removes none.
      await storeSubAgentResults(code, sdId, null, errorResult, { phase: options.phase });
    } catch (storeError) {
