 ...
 * rca.js's execute() -- an existing OPEN root_cause_reports row for this SD if one exists, or a
 * newly-created one. Exported, DI'd on the supabase client, for direct unit testing without
 * driving the full dispatch pipeline or touching the real rca.js module file.
 */
export async function resolveRcaDispatchTarget(rcaSupabase, sdUUID, sdKey, options = {}) {
  ...
  if (rcrCreateError || !newRcr) {
    console.log(`   Warning: failed to create RCR for RCA dispatch: ${rcrCreateError?.message}`);
    return sdUUID;
  }
