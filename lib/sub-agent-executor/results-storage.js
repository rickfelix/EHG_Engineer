...
export async function storeSubAgentResults(code, sdId, subAgent, results, options = {}) {
  ...
  if (unpersisted.length > 0) {
    throw new Error(`UNRECOGNIZED_FIELD_DROPPED (results-storage.storeSubAgentResults) [${code}]: caller field(s) not stored and not declared in PERSISTED_ELSEWHERE: ${unpersisted.join(', ')}. If the drop is intentional, add each to PERSISTED_ELSEWHERE with its reason; otherwise this is a typo'd or unmapped field that would otherwise vanish silently.`);
  }
