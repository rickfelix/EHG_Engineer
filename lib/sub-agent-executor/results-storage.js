...
export async function storeSubAgentResults(code, sdId, subAgent, results, options = {}) {
  ...
  // above has always been soft. A field/key-drop failure (ReadbackFieldMismatchError /
  // ReadbackKeyDropError) means the row WAS found and its content provably does not match what was
  // sent -- an unambiguous defect with no plausible test-environment explanation -- so that one
  // propagates out of storeSubAgentResults into executor.js's existing catch block (which already
  // re-throws to the CLI), failing the sub-agent run instead of being logged and ignored.
