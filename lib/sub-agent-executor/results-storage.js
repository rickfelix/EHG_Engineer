...
import { assertEvidenceHasContent } from './evidence-content-guard.js';
...
import { computeContentHash } from './evidence-provenance.js';
...
import { verifyReadback, ReadbackCheckError, ReadbackFieldMismatchError, ReadbackKeyDropError } from '../checkers/readback-checker.mjs';
...
import { validateTestExecutionShape } from './testing-verdict-guard.js';
...
export function mapVerdict(rawVerdict) {
  ...
  const verdictMap = {
    ...
    UNKNOWN: 'BLOCKED',
    // ESCALATE is the same "could not reach a verdict" case as UNKNOWN, not a crash and
    // not a classification failure — without this entry it fell through to the
    // TRULY-UNRECOGNISED branch below and returned 'ERROR', consuming the ERROR tripwire
    // this file's own comments (see below) document as reserved for that purpose.
    // QF-20260815-658, specimen security-agent row 6bcfa2b9 (self-corrected to FAIL).
    ESCALATE: 'BLOCKED',
  };
...
export function resolveEvaluatedCommitSha(repoPath, exec = execFileSync) {
  ...
  try {
    ...
    // execFileSync calls (deep-tier adversarial review finding) -- a `rev-parse HEAD`
    // is normally instant, but an unresponsive filesystem/mount must never hang this
    // hot, frequently-executed evidence-write path indefinitely.
    return exec('git', ['rev-parse', 'HEAD'], { cwd: repoPath, encoding: 'utf8', timeout: 5000 }).trim() || null;
  } catch {
    return null;
  }
}
...
/**
 * Resolves a commit's author-independent commit timestamp (ISO 8601), for the TESTING
 * sub-agent's artifact-freshness check (SD-FDBK-INFRA-TESTING-EVIDENCE-REUSE-001): an
 * artifact must not predate the commit it is being asked to certify. Sibling to
 * resolveEvaluatedCommitSha above -- same injectable-exec, same 5s bound, same
 * never-throws contract (a caller treats null as "cannot verify freshness", never as
 * "assume fresh").
 ...
 * @returns {string|null}
 */
export function resolveCommitTimestamp(sha, repoPath, exec = execFileSync) {
  if (!repoPath || !sha) return null;
  try {
    return exec('git', ['show', '-s', '--format=%cI', sha], { cwd: repoPath, encoding: 'utf8', timeout: 5000 }).trim() || null;
  } catch {
    return null;
  }
}
...
/**
 * SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1, third verification pass) — TOP-LEVEL RESULT
 * FIELDS THAT BELONG IN metadata, DECLARED ONCE.
 *
 * WHY A LIST AND NOT MORE HAND-WRITTEN PAIRS. The first two verification passes on this SD each
 * found real fields FR-1's throw would have refused (pass 1: the readback defect; pass 2:
 * options/metrics, then baseline_applied/mode), fixed them as individual metadata lines PLUS
 * individual PERSISTED_ELSEWHERE entries, and each time concluded the fleet was covered. It was
 * not: an exhaustive census of every module executor.js can resolve
 * (`../sub-agents/${code.toLowerCase()}.js`, executor.js:251) found EIGHTEEN more, across five
 * module families. The reason the same miss kept recurring is structural — "the field is written"
 * and "the field is exempt from the throw" lived in two separate places, so a reader could satisfy
 * one and believe they had satisfied both. Below, PERSISTED_ELSEWHERE is DERIVED from this list, so
 * a field is exempt IF AND ONLY IF it is also preserved. The two can no longer drift, and adding
 * the next sub-agent field is a one-line change in one place.
 *
 * THESE ARE ALL GENUINELY UNHANDLED TODAY, not merely undeclared. Each is a TOP-LEVEL key on the
 * result object; `safeMetadata` derives exclusively from `results.metadata`, so none of them has
 * ever reached the row by any path. They are silently discarded on every current run and would
 * hard-throw on every run once FR-1 lands. Verified live against sub_agent_execution_results: no
 * RISK row carries risk_domains (4,308 rows, 1,204 in the last 30 days, 439 in the last 7).
 *
 * WRITTEN CONDITIONALLY, not unconditionally-null like `message`/`hallucination_check` below.
 * Those four predate this list and stay as they are on purpose — their present-and-null form is
 * load-bearing for "the agent returned nothing" queries. Eighteen more unconditional nulls on
 * every row fleet-wide would be bloat with no query value, and the conditional spread matches this
 * same object's existing `...(phaseValue ? ... : {})` convention.
 */
export const TOP_LEVEL_FIELDS_PERSISTED_TO_METADATA = Object.freeze({
  // --- Every module that stamps its own measurement time (risk.js:47, rca.js:49,
  // venture_stack.js:40, modules/stories/execute.js:119, and all nine venture-stage modules).
  // Distinct from record.created_at, which is WRITE time: an evidence row that cannot tell
  // "when it was measured" from "when it was filed" cannot answer a freshness question.
  timestamp: "the sub-agent's own measurement time (distinct from record.created_at, which is write time)",

  // --- RISK (risk.js). The six fields below ARE the risk assessment; without them a RISK row
  // carries a verdict and nothing that justifies it.
  risk_domains: 'RISK per-domain scores/levels/rationales (risk.js:48, populated :111-:135)',
  overall_risk_score: 'RISK aggregate score (risk.js:162)',
  risk_level: 'RISK banding of the aggregate score (risk.js:163)',
  rationale: 'RISK verdict rationale from determineVerdict (risk.js:214)',
  assessment_id: 'RISK risk_assessments row id, the link to the full stored assessment (risk.js:253)',

  // --- RCA (rca.js). `analysis` is the entire forensic payload: root cause, category, causal
  // chain, contributing factors, pattern matches (rca.js:56-62).
  rcr_id: 'RCA root_cause_reports id under analysis (rca.js:46)',
  analysis: 'RCA root cause / causal chain / contributing factors / pattern matches (rca.js:56)',

  // --- STORIES (modules/stories/execute.js). Counters and arrays; several are legitimately
  // falsy (0, []), which is why the spread below tests key PRESENCE and never truthiness.
  stories_processed: 'STORIES count of stories examined (modules/stories/execute.js:120, :153)',
  stories_enhanced: 'STORIES count of stories enriched (modules/stories/execute.js:121)',
  context_added: 'STORIES stories that gained implementation context (modules/stories/execute.js:122)',
  already_complete: 'STORIES stories that needed no work (modules/stories/execute.js:123)',
  failed: 'STORIES stories that could not be processed (modules/stories/execute.js:124)',
  placeholders_removed: 'STORIES placeholder rows deleted (modules/stories/execute.js:242)',
  detected_gaps: 'STORIES requirement gaps found while generating (modules/stories/execute.js:335)',
  stories_created: 'STORIES rows created from acceptance criteria (modules/stories/execute.js:415)',

  // --- VENTURE_STACK + the venture-stage family (analytics, api, crm, dependency, financial,
  // launch, marketing, monitoring, pricing, sales, valuation). `blockers` is the highest-stakes
  // omission in this list: it is the list of reasons a venture-stage sub-agent says STOP, and it
  // is not the same column as `warnings` or `critical_issues`.
  blockers: 'venture-stage/VENTURE_STACK blocking reasons (venture_stack.js:107 and the nine venture-stage modules)',
  artifact: 'venture-stage generated artifact (BMC/GTM/pricing/etc., e.g. financial.js:238, marketing.js:252)',
});
...
export async function storeSubAgentResults(code, sdId, subAgent, results, options = {}) {
  ...
  // QF-20260906-403: the SOURCE-PRIORITY OR-chain is unchanged -- options.phase still wins
  // over results.phase, which still wins over results.metadata.phase. What changed is that
  // whichever source wins is now normalized (normalizePhaseToken) before being used, instead
  // of stored byte-identical. One SD carried five case/separator spellings of the same phase
  // across its own evidence rows (LEAD / PLAN_PRD / PLAN / EXEC / EXEC-TO-PLAN), each of which
  // the exact-match readers below treat as a DIFFERENT phase -- producing SUBAGENT_EVIDENCE_MISSING
  // false negatives for evidence that genuinely exists. No backfill: existing rows keep their
  // as-written labels; only writes from this point forward are canonicalized.
  let phaseValue = normalizePhaseToken(
  ...
  if (!phaseValue && normalizedSdId) {
    try {
      ...
      if (sdLookupError) {
        ...
        // FR-3: normalize the DERIVED value at this choke point too. QF-20260906-403:
        // explicitly-supplied caller phases (options/results/metadata, resolved above) are
        // now ALSO normalized at the point they're resolved -- this branch only runs when
        // none of those three sources supplied anything, so it stays a separate call site
        // rather than relying on a second pass over an already-normalized value.
  ...
  let metadata = {
    ...
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1): both were silently dropped on essentially
    // every executor.js run before this SD -- `message` is set on every MANUAL_REQUIRED branch
    // (executor.js ~333-372) and `hallucination_check` is set on every successful run
    // (executor.js:418) -- neither has ever had a column or a PERSISTED_ELSEWHERE entry, so
    // FR-1's new fail-loud unpersisted-field check would otherwise hard-block every such run.
    // Preserving the content (not just exempting the name) is the actual fix.
    message: typeof results.message === 'string' ? results.message : null,
    hallucination_check: results.hallucination_check ?? null,
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1, TESTING sub-agent re-verify pass): genuinely
    // unhandled before this SD (unlike options/metrics above, which were already written just
    // missing their PERSISTED_ELSEWHERE entry) -- security.js sets baseline_applied and
    // regression.js sets mode as top-level result fields with no prior column or metadata home.
    baseline_applied: results.baseline_applied ?? null,
    mode: typeof results.mode === 'string' ? results.mode : null,
    // Model routing metadata (added 2025-12-03)
    routing: subAgent?.routing || null,
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1, third verification pass): the eighteen
    // top-level fields declared in TOP_LEVEL_FIELDS_PERSISTED_TO_METADATA above -- RISK's whole
    // assessment, RCA's whole analysis, STORIES' counters, the venture-stage family's blockers
    // and artifact. Keyed on PRESENCE (`k in results`), never truthiness: `failed: []`,
    // `stories_processed: 0` and `overall_risk_score: 0` are all real, meaningful readings, and a
    // truthiness test would silently convert "zero failures" into "we did not record failures" --
    // which is the same claim-vs-reality gap this whole SD exists to close.
    ...Object.fromEntries(
      Object.keys(TOP_LEVEL_FIELDS_PERSISTED_TO_METADATA)
        .filter((k) => results && k in results)
        .map((k) => [k, results[k]])
    ),
    ...
    //
    // executed_from_cwd PREFERRED over repo_path — SECURITY caught this live during EXEC-TO-PLAN
    // review (evidence row 28b7921b-27c6-40bc-874e-8413209ca84d): resolve-repo.js's
    // toCanonicalRepoPath() DELIBERATELY strips the worktree segment from metadata.repo_path
    // (collapses `.worktrees/<sd>` to the main root, for cwd-leak-detection purposes elsewhere).
    // Preferring repo_path here stamped the REPO ROOT's HEAD instead of the SD's own worktree
    // branch HEAD, which would have made FR-3's staleness check false-positive on every
    // worktree-scoped SD (the default execution model fleet-wide). executed_from_cwd is
    // `process.cwd()` at write time (resolve-repo.js:235) — the actual location code ran, which
    // is exactly "what commit did this sub-agent evaluate".
    evaluated_commit_sha: resolveEvaluatedCommitSha(safeMetadata.executed_from_cwd || safeMetadata.repo_path),

    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-E: session_id is set AFTER the spread, same anti-clobber
    // rationale as original_verdict/evaluated_commit_sha above -- a caller-supplied
    // results.metadata.session_id must never survive over what THIS writer actually observed.
    // Explicit null (never an omitted key) when CLAUDE_SESSION_ID is unset, matching the existing
    // sentinel-value convention this file already uses for verdict (see note above).
  ...
  const record = {
    ...
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-A: source is a real column that this writer never set,
    // silently defaulting to the DB column default 'manual' -- indistinguishable from "no writer
    // claims this row". Set explicitly so a reader can tell this row came from a real runner.
    // Neither field is ever read from the caller's `results` object, so there is nothing a caller
    // could clobber -- no anti-clobber placement needed (unlike session_id/original_verdict above).
  ...
  // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-A: stamped AFTER `record` is fully built, over the row's
  // OWN final resolved values -- so a reader can re-derive this hash from the row it just read
  // and detect content altered post-write. `metadata` and `record.metadata` are the same object
  // reference, so this also updates `record` in place.
  metadata.content_hash = computeContentHash(record);

  // SD-FDBK-INFRA-TESTING-VERDICT-ROWS-001 FR-1: refuse a TESTING PASS/CONDITIONAL_PASS
  // write missing a genuine metadata.test_execution block. Placed above BOTH the dedup
  // update and insert branches below so neither can bypass it.
  validateTestExecutionShape(record);
  ...
  const PERSISTED_ELSEWHERE = Object.freeze({
    ...
    // SD-LEO-INFRA-EXECUTOR-120S-1800S-001 / FR-1 AC-5: caller-supplied top-level error/stack
    // ARE stored, just nested -- into metadata.error/metadata.stack (lines above), not a
    // top-level `record.error`/`record.stack` column. Without these two entries the
    // unpersisted-key check below reads `!('error' in record)` as true (record has no
    // top-level `error` key) and falsely warns they were dropped.
    error: 'written into metadata.error, not a top-level record column',
    stack: 'written into metadata.stack, not a top-level record column',
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1): the LIVE specimen found during LEAD due
    // diligence -- both were silently dropped on essentially every executor.js-routed run
    // (message on every MANUAL_REQUIRED branch, hallucination_check on every successful run)
    // before this SD. Now written into metadata (see above), not discarded.
    message: 'written into metadata.message, not a top-level record column',
    hallucination_check: 'written into metadata.hallucination_check, not a top-level record column',
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1, TESTING sub-agent re-verify pass): options and
    // metrics ARE already persisted (metadata.options/metadata.metrics, above) -- they were
    // simply missing from this list. Without this entry FR-1's fail-loud check refused ~32% of
    // live rows measured over 60 days: options is a top-level field on every one of at least 14
    // fleet sub-agent modules' own result objects (security.js, validation.js, retro/index.js,
    // regression.js, testing/index.js, database.js, design/index.js, docmon.js, github.js,
    // judge/index.js, performance.js, quickfix.js, uat.js).
    options: 'written into metadata.options, not a top-level record column',
    metrics: 'written into metadata.metrics, not a top-level record column',
    // baseline_applied (security.js) and mode (regression.js) were genuinely UNHANDLED before
    // this SD, unlike options/metrics above -- see the metadata object above where both are now
    // actually written, not merely exempted.
    baseline_applied: 'written into metadata.baseline_applied, not a top-level record column',
    mode: 'written into metadata.mode, not a top-level record column',
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1, third verification pass): DERIVED, never
    // retyped. Every key here is written into metadata by the spread above, from this same
    // frozen object -- so "exempt from the throw" and "actually preserved" are now the same
    // fact rather than two hand-maintained lists that drifted apart twice on this SD alone.
    // Retyping them would reintroduce exactly the drift this derivation exists to prevent.
    ...Object.fromEntries(
      Object.entries(TOP_LEVEL_FIELDS_PERSISTED_TO_METADATA)
        .map(([k, why]) => [k, `written into metadata.${k}, not a top-level record column — ${why}`])
    ),
  });
  // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-1): a warn-only diagnostic here was indistinguishable
  // from silence in practice -- nobody reads console.warn on a background sub-agent run, so a
  // genuine field-name mismatch (typo, renamed field, a new field the writer has not learned yet)
  // still produced a green PASS/CONDITIONAL_PASS row missing exactly the content it claimed to
  // carry. This is now fail-loud: a field neither mapped to a column nor exempted via
  // PERSISTED_ELSEWHERE refuses the write entirely, matching evidence-content-guard.js's existing
  // throwing convention rather than inventing a second error style.
  const unpersisted = Object.keys(results || {}).filter((k) => !(k in record) && !(k in PERSISTED_ELSEWHERE));
  if (unpersisted.length > 0) {
    // SECURITY sub-agent (EXEC, S3): `code` and each unpersisted key are object-key strings, not
    // free-form values, but a caller-controlled key could still carry control/ANSI bytes reaching
    // both a terminal and this row's own error text -- the same class store-sub-agent-repo-
    // evidence.js's safeValue() already bounds one layer over. Stripped and length-bounded here
    // rather than importing safeValue from a script into this lower-level module.
    const safeKeyList = (arr) => arr.map((k) => String(k).replace(/[\x00-\x1f\x7f]/g, '').slice(0, 200)).join(', ').slice(0, 2000);
    throw new Error(`UNRECOGNIZED_FIELD_DROPPED (results-storage.storeSubAgentResults) [${safeKeyList([code])}]: caller field(s) not stored and not declared in PERSISTED_ELSEWHERE: ${safeKeyList(unpersisted)}. If the drop is intentional, add each to PERSISTED_ELSEWHERE with its reason; otherwise this is a typo'd or unmapped field that would otherwise vanish silently.`);
  }
  ...
  // Empty-evidence guard. Placed BEFORE the dedup branch so it covers the UPDATE path too —
  // guarding only the insert would let a content-free result overwrite a good row within the
  // 5-minute dedup window, which is strictly worse than refusing to create one.
  assertEvidenceHasContent(record, { writer: 'results-storage.storeSubAgentResults' });
  ...
  // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-H (FR-2): the readback above only ever checked
  // verdict/sub_agent_code/sd_id and fails soft -- it would never have caught a write that landed
  // with the right verdict but a dropped payload (the specimen defect: a green row missing the
  // content it claimed to carry). This SECOND readback is scoped to the fields a caller actually
  // supplied.
  //
  // MEASURED CORRECTION (TESTING sub-agent, EXEC): an earlier version compared warnings/
  // recommendations via verifyReadback's expectedFields (JSON.stringify deep-equality).
  // readback-checker.mjs's own header already documents deepEqual's key-order sensitivity as an
  // accepted limitation "callers pass expectedFields already DB-shaped" -- but warnings/
  // recommendations are jsonb array-of-object columns, and Postgres does not preserve caller
  // object-key order through a jsonb round trip. A real write ({severity,issue,recommendation})
  // read back key-reordered ({issue,severity,recommendation}) -- semantically identical, byte
  // different -- and the exact-match comparison hard-failed a genuinely correct write. 182 call
  // sites in this repo push warning/recommendation objects, including the canonical
  // applySubAgentRepoVerdict itself, so this would have been self-breaking fleet-wide. Fixed by
  // checking PRESENCE, not exact equality, for these two fields -- order-insensitive, and still
  // exactly what the PRD's own acceptance criteria asked for: "every non-empty caller collection
  // is non-empty on the row," never byte-identical.
  //
  // detailed_analysis joins the presence-only group for a DIFFERENT reason (same TESTING pass,
  // same handoff): it is a TEXT column, but several sub-agents (security.js, validation.js,
  // retro/index.js, design) initialize it as an OBJECT ({}), and this file's own compression path
  // (RESULT_COMPRESSION_THRESHOLD, below) substitutes a compressed-artifact object for large
  // content. A TEXT column stringifies whatever is written -- `{}` reads back as the STRING "{}"
  // -- so an object-shaped `record.detailed_analysis` compared for exact equality against the
  // read-back string always mismatches, regardless of whether anything was actually dropped.
  // Only `summary` stays in expectedFields: it is unconditionally normalized to a plain string or
  // null before every write (see its own normalization above), so it has no such coercion hazard.
  //
  // HARD vs SOFT is decided by WHAT failed, not just THAT it failed. A rowcount/query failure
  // (ReadbackRowcountError / ReadbackQueryError / anything unexpected) means "could not
  // independently confirm the row" -- exactly what every test in this repo that mocks the write
  // client but not readback-checker.mjs produces, by design (verifyReadback deliberately never
  // accepts an injected client, see its own header), and that ambiguity is why the FIRST readback
  // above has always been soft. A field/key-drop failure, OR a presence check below finding the
  // row genuinely missing content the caller supplied, means the row WAS found and its content
  // provably does not match what was sent -- an unambiguous defect with no plausible
  // test-environment explanation -- so that one propagates out of storeSubAgentResults into
  // executor.js's existing catch block (which already re-throws to the CLI), failing the
  // sub-agent run instead of being logged and ignored.
  const metadataRequiredKeys = [];
  if (results.findings) metadataRequiredKeys.push('findings');
  if (results.metadata && results.metadata.findings) metadataRequiredKeys.push('_findings_had_keys');
  const hasArrayContent = (v) => Array.isArray(v) && v.length > 0;
  const hasScalarOrObjectContent = (v) => {
    if (typeof v === 'string') return v.trim().length > 0;
    if (v && typeof v === 'object') return Object.keys(v).length > 0;
    return Boolean(v);
  };
  try {
    const { row: readRow } = await verifyReadback({
      table: 'sub_agent_execution_results',
      match: { id: data.id },
      expectedFields: {
        summary: record.summary,
      },
      ...(metadataRequiredKeys.length > 0 ? { requiredKeys: { metadata: metadataRequiredKeys } } : {}),
    });
    for (const field of ['warnings', 'recommendations']) {
      if (hasArrayContent(record[field]) && !hasArrayContent(readRow[field])) {
        throw new ReadbackFieldMismatchError(
          `verifyReadback: field "${field}" on "sub_agent_execution_results" was non-empty when sent and empty on readback`,
          { table: 'sub_agent_execution_results', match: { id: data.id }, field }
        );
      }
    }
    if (hasScalarOrObjectContent(record.detailed_analysis) && !hasScalarOrObjectContent(readRow.detailed_analysis)) {
      throw new ReadbackFieldMismatchError(
        'verifyReadback: field "detailed_analysis" on "sub_agent_execution_results" was non-empty when sent and empty on readback',
        { table: 'sub_agent_execution_results', match: { id: data.id }, field: 'detailed_analysis' }
      );
    }
  } catch (completenessErr) {
    if (completenessErr instanceof ReadbackFieldMismatchError || completenessErr instanceof ReadbackKeyDropError) {
      // SECURITY sub-agent (EXEC, S2): this throw propagates into executor.js's catch block,
      // which — before this sentinel — would call storeSubAgentResults AGAIN with a content-free
      // errorResult. Because sd_id+code+phase match within the 5-minute dedup window, that second
      // call takes the UPDATE branch above and overwrites THIS row (which the write itself already
      // committed successfully, per the "row was found" precondition of a field-mismatch/key-drop
      // error) with an empty ERROR record — the control meant to catch dropped content instead
      // destroying content that mostly survived over one field that didn't. Marking the error
      // (same pattern as isBuiltinAgentRefusal, SD-LEO-INFRA-EXPLORE-UNREGISTERED-LEO-001) lets
      // executor.js skip that fallback store: the already-written row is a strictly better
      // artifact than what the recovery path would produce.
      completenessErr.isPayloadCompletenessFailure = true;
      throw completenessErr;
    }
    const kind = completenessErr instanceof ReadbackCheckError ? completenessErr.name : 'UnexpectedCheckerError';
    console.error(`   [PAYLOAD-COMPLETENESS-CHECK-FAILED] ${kind} for sub_agent_execution_results id=${data.id} sd_id=${normalizedSdId} sub_agent_code=${code}: ${completenessErr.message}`);
  }
