 ...
 * (all 15 live-observed spellings plus an unmapped case); pre-cutover rows never graded absent
...
describe('computeContentHash', () => {
  ...
  // QF-20260912-195: detailed_analysis TEXT-round-trip parity. results-storage.js stamps the
  // hash over the record it is ABOUT to insert (detailed_analysis may still be an object at that
  // point), but the column is TEXT-typed, so the row reads back with detailed_analysis as a
  // string. Without canonicalizing to the persisted (stringified) form here, the write-time hash
  // and the read-time re-derivation disagree on every row whose detailed_analysis was an object
  // — measured 25 of 40 live rows since 2026-09-10.
  it('an object-shaped detailed_analysis hashes the same as its JSON.stringify\'d (TEXT-persisted) form', () => {
    const analysisObject = { _compressed: true, artifact_id: 'art-1', summary: 'x', token_count: 10 };
    const preWrite = { verdict: 'PASS', confidence: 90, detailed_analysis: analysisObject };
    const postReadback = { verdict: 'PASS', confidence: 90, detailed_analysis: JSON.stringify(analysisObject) };
    expect(computeContentHash(preWrite)).toBe(computeContentHash(postReadback));
  });

  it('a null detailed_analysis is unaffected by the TEXT-round-trip canonicalization', () => {
    const a = { verdict: 'PASS', detailed_analysis: null };
    const b = { verdict: 'PASS' };
    expect(computeContentHash(a)).toBe(computeContentHash(b));
  });

  it('a string detailed_analysis (already in persisted form) is hashed as-is, unchanged behavior', () => {
    const payload = { verdict: 'PASS', detailed_analysis: 'plain text analysis' };
    expect(computeContentHash({ ...payload })).toBe(computeContentHash({ ...payload }));
  });
});
...
describe('normalisePhase', () => {
  const cases = [
    ...
    ['LEAD_FINAL', 'LEAD'],
    ['LEAD-FINAL-APPROVAL', 'LEAD'], // QF-20260905-723: was missing, hyphenated handoff-name spelling
    ['LEAD_FINAL_APPROVAL', 'LEAD'], // QF-20260905-723: was missing, underscore symmetry form
    ...
    ['PLAN_VERIFY', 'PLAN'], // QF-20260907-978: was missing, exec-to-plan sub-agent-orchestration.js's own spelling
    ['VERIFY', 'PLAN'], // QF-20260907-978: was missing, PLAN-TO-LEAD gate verification-pass spelling
    ['VERIFY (PLAN-TO-LEAD gate)', 'PLAN'], // QF-20260907-978: was missing, the same pass with a prose suffix
    ['PLAN_PRD', 'PLAN'],
    ['EXEC', 'EXEC'],
    ['EXEC_TO_PLAN', 'EXEC'],
...
describe('gradeProvenance', () => {
  ...
  // QF-20260912-195: reproduces the live defect end-to-end through gradeProvenance, not just
  // computeContentHash in isolation. The writer stamps content_hash over the record it is about
  // to insert (detailed_analysis still an object); the row as READ BACK has detailed_analysis as
  // a string (the TEXT column's persisted form). Before the fix this graded absent/
  // content_hash_mismatch on every such row (25 of 40 measured live since 2026-09-10).
  it('a row whose content_hash was stamped pre-write over an object detailed_analysis grades present after the TEXT round-trip', () => {
    const analysisObject = { _compressed: true, artifact_id: 'art-1', summary: 'x', token_count: 10 };
    const recordAtWriteTime = { ...postCutoverBase, detailed_analysis: analysisObject };
    const stampedHash = computeContentHash(recordAtWriteTime); // what results-storage.js:847 stamps
    const rowAsReadBack = { ...postCutoverBase, detailed_analysis: JSON.stringify(analysisObject), content_hash: stampedHash };
    expect(gradeProvenance(rowAsReadBack)).toEqual({ absent: false, preCutover: false });
  });
});
