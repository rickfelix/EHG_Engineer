...
/**
 * detailed_analysis's PERSISTED form: the column is TEXT-typed (see
 * migrations/*_create_sub_agent_execution_results*.sql), so an object/array value the writer
 * passes is stored as its JSON text and reads back as a string. Canonicalizing to that same
 * string form here — regardless of whether the caller passed an object (pre-persistence) or a
 * string (already read back from the DB) — is what lets the write-time stamp and the read-time
 * re-derivation agree. Mirrors results-storage.js's own compression-threshold stringify
 * convention (`typeof x === 'string' ? x : JSON.stringify(x)`).
 */
function persistedDetailedAnalysis(value) {
...
export function computeContentHash(payload) {
  const canonical = {
    ...
    detailed_analysis: persistedDetailedAnalysis(payload?.detailed_analysis),
 ...
 */
// QF-20260905-723: keys are underscore-only. normalisePhase canonicalizes
// a hyphenated handoff-name spelling (e.g. 'PLAN-TO-EXEC', copied verbatim from the handoff type)
// to its underscore form before lookup, so the two spellings can never drift apart again — a
// hyphenated duplicate entry here would be exactly the kind of second source of truth that let
// LEAD-FINAL-APPROVAL and three PLAN/EXEC crossings normalise to null (41 live rows since 09-04).
const PHASE_MAP = {
  LEAD: 'LEAD',
  LEAD_TO_PLAN: 'LEAD',
  LEAD_FINAL: 'LEAD',
  LEAD_FINAL_APPROVAL: 'LEAD',
  PLAN: 'PLAN',
  PLAN_TO_EXEC: 'PLAN',
  PLAN_TO_LEAD: 'PLAN',
  PLAN_VERIFICATION: 'PLAN',
  // QF-20260907-978: two more PLAN-verification spellings found by QF-20260905-723's own census
  // (30 rows since 09-04, all TESTING/SECURITY/VALIDATION/REGRESSION/RETRO evidence -- the exact
  // sub-agent set PLAN_VERIFICATION already carries). PLAN_VERIFY is exec-to-plan/gates/sub-agent-
  // orchestration.js's own phase label for this same evidence pass; VERIFY / 'VERIFY (PLAN-TO-LEAD
  // gate)' name the PLAN-TO-LEAD gate's verification pass, the same work PLAN_TO_LEAD above already
  // buckets to PLAN -- these are prose-labeled variants of it, not a distinct phase.
  PLAN_VERIFY: 'PLAN',
  VERIFY: 'PLAN',
  'VERIFY (PLAN_TO_LEAD gate)': 'PLAN',
  PLAN_PRD: 'PLAN',
  EXEC: 'EXEC',
  EXEC_TO_PLAN: 'EXEC',
...
export function normalisePhase(phase) {
  ...
  const key = typeof phase === 'string' ? phase.replace(/-/g, '_') : phase;
