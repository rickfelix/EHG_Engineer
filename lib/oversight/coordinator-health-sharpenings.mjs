...
// QF-20260902-542: a premature execute attempt isn't a real gate failure -- REWORK_CEILING now
// measures only the latter. Mirrors its value pending owner calibration; observability only.
export const PREMATURE_ATTEMPT_CEILING = 0.3;
// failed_gate (sd_phase_handoffs.validation_details.summary.failed_gate) values meaning "refused
// before real validation ran", not "validation ran and found a defect".
const PREMATURE_ATTEMPT_GATES = new Set(['SUBAGENT_EVIDENCE_MISSING', 'PREREQUISITE_PREFLIGHT_FAILED']);
...
export function deriveOutcomeFlow(sdRows, handoffRows, nowMs = Date.now(), windowDays = OUTCOME_WINDOW_DAYS) {
  ...
  if (!cohort.length) {
    return {
      status: 'no_cohort', window_days: windowDays, cohort_size: 0, conversion: null, median_latency_ms: null,
      rework_rate: null, premature_attempts: null, premature_attempt_rate: null, gate_retries: null,
    };
  }
  ...
  const rejectedHandoffs = cohortHandoffs.filter((h) => /reject|fail/i.test(h.status || ''));
  // QF-20260902-542: split the numerator -- a premature attempt and a real gate failure are
  // different worker-loop signals; collapsing them hid that 57.9% of "rework" was the former.
  const prematureAttempts = rejectedHandoffs.filter((h) => PREMATURE_ATTEMPT_GATES.has(h.validation_details?.summary?.failed_gate)).length;
  const gateRetries = rejectedHandoffs.length - prematureAttempts;
  return {
    status: 'measured',
    ...
    rework_rate: cohortHandoffs.length ? gateRetries / cohortHandoffs.length : 0,
    premature_attempts: prematureAttempts,
    premature_attempt_rate: cohortHandoffs.length ? prematureAttempts / cohortHandoffs.length : 0,
  ...
  for (let i = 0; i < cohortIds.length; i += 100) {
    ...
    try {
      hs = await fetchAllPaginated(() => supabase
        // validation_details added for QF-20260902-542's premature-attempt vs gate-retry split
        .from('sd_phase_handoffs').select('sd_id, status, validation_details').in('sd_id', chunk)
        .order('id', { ascending: true })); // stable page boundaries (FR-6)
    } catch (e) {
