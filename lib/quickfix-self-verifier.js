 ...
 */
export async function verifyTestCoverage(qf, context) {
  ...
  // not take. --skip-tests means no suite executed this run (testsPass is a trusted
  // default/override, not a measured result) — report that truthfully instead of falling
  // through to "confirmed passing".
  if (context.testsSkipped) {
    return {
      passed: true,
      message: 'Tests skipped — trusted CI',
      details: '--skip-tests was used; no unit or E2E suite executed in this run. PR CI checks are the test evidence (gh pr checks <PR#>).'
    };
  }
  ...
  // QF-20260911-755: never report an E2E run that did not execute as "verified". The
  // orchestrator threads e2eNotRunnable when the prod-ref guard refused the Playwright run
  // before any test; the unit run + PR CI are what actually backed testsPass in that case.
  if (context.e2eNotRunnable) {
    return {
      passed: true,
      message: 'Unit tests confirmed passing; E2E not runnable here',
      details: 'E2E smoke was refused by the e2e-db-target guard before any test executed (no non-production ref on this host) — recorded as e2e_not_runnable_here; unit run + PR CI are the test evidence'
    };
  }
