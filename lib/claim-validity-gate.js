...
export function isOwnerProcessAlive(ownerSessionId) {
  ...
  try {
    ...
    // No entry for ownerSessionId is could-not-determine, not confirmed-dead: falls through to
    // `false` here, which this function's own fail-open contract already treats as "no added
    // protection" rather than a death verdict -- the caller's other checks still gate the claim.
  ...
  if (sd.claiming_session_id !== mySessionId) {
    ...
    if (!ownerLookupFailed && shouldReleaseStaleOwner({ ownerHasSdKeyDrifted, ownerIsDead, ownerIsSilenced, ownerPidAlive, ownerSdKeyMissing })) {
      ...
      await releaseClaimBothSurfaces(supabase, {
        ...
        // QF-20260912-175: explicit, not the default. A dead owner is genuinely retired
        // ('released'); a drifted owner has already moved to another sd_key and is presumably
        // still alive elsewhere ('idle') — the session-side CAS (session_id + sd_key=THIS sdKey)
        // naturally no-ops for a drifted owner regardless, but naming it removes the ambiguity.
        sessionStatus: ownerIsDead ? 'released' : 'idle',
      });
      ...
      // LEO_RELEASE_WORKITEM_RESET (default ON since QF-20260912-961; 'off' is the explicit
      // kill switch). This IS a hand-back: the owner is dead,
