...
async function runCoordinationDetectorsLegacy(ctx) {
  ...
  // QF-20260902-959: gathering + running the detectors is now UNCONDITIONAL. The
  // COORD_DETECTORS_V2 flag no longer gates whether this runs at all — it gates only
  // the DISPATCH effect inside runAndLogDetectors: OFF writes one read-only
  // DETECTORS_EVALUATED dry-run row per sweep (would-fire census, no per-match write);
  // ON logs one coordination_events row per match (+ SPLIT_BRAIN auto-resolve), as before.
  try {
    ...
    for (const m of coordMatches) {
      const tail = m.dryRun ? ' (dry-run)' : m.logged ? '' : ' (event-log-failed)';
      console.log('  COORD_DETECTOR: ' + m.event_type + ' [' + m.severity + '] ' + m.reason + tail);
    }
    if (coordMatches.length > 0) {
      const dryRun = coordMatches.every((m) => m.dryRun);
      console.log('COORD_DETECTORS: ' + coordMatches.length + (dryRun ? ' would-fire event(s) (dry-run census)' : ' anomaly event(s) flagged'));
    }
  } catch (coordDetErr) {
  ...
  try {
    const notifWait = await coordEventsModule.runNotificationWaitSurfacing(supabase, {});
    if (notifWait && notifWait.matched) {
      const a = notifWait.alert || {};
      const tail = a.skipped ? ' (alert deduped)' : a.ok ? ' - operator alert emitted' : ' (alert emit failed)';
      console.log('  NOTIFICATION_WAIT: ' + notifWait.waiting_count + ' seat(s) waiting past threshold' + tail);
    }
  } catch (notifErr) {
    console.log('NOTIFICATION_WAIT: ' + (notifErr && notifErr.message ? notifErr.message : 'unknown'));
  }
}
