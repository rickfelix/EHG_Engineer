...
// QF-20260830-076: a team stuck in 'stopping' (or 'active' with zero live workers)
// for longer than this never transitioned and is effectively dead — it should not
// render a full banner box on every dashboard run forever.
const STALE_TEAM_AGE_SECONDS = 24 * 60 * 60;

/** True when a loaded team is stale: no live workers and older than the threshold. */
function isStaleTeam(team) {
  return team.active_workers === 0 && (team.uptime_seconds || 0) > STALE_TEAM_AGE_SECONDS;
}
...
function printTeam(teams, bar, opts = {}) {
  ...
  const staleTeams = allTeams.filter(isStaleTeam);
  const safeTeams = allTeams.filter((t) => !isStaleTeam(t));

  if (staleTeams.length > 0) {
    const oldest = Math.max(...staleTeams.map((t) => t.uptime_seconds || 0));
    log('');
    log(`  (${staleTeams.length} dead team(s) hidden — stuck stale >24h, no live workers; oldest ${fmtUptime(oldest)})`);
  }
