...
export const RETENTION_POLICIES = [
  ...
  //
  // SD-LEO-INFRA-EVA-SCHEDULER-HYGIENE-001 (FR-1): that SD closed green while this table grew
  // 25x (140,302 -> 3.4M+) because the registered policy above was never actually convergent —
  // measured steady-state inflow is ~120-140K rows/day, and DEFAULT_PER_RUN_CAP (20,000) on the
  // shared WEEKLY cron drains only ~2.4% of a week's inflow. hotDays does not change whether the
  // policy converges (only when divergence starts), so it is left at DEFAULT_HOT_DAYS. The real
  // fix is perRunCap sized to exceed measured peak daily inflow, run on a DAILY cadence specific
  // to this table via `retention-enforce.js --table eva_scheduler_metrics` (see
  // .github/workflows/retention-enforce-eva-scheduler-metrics-cron.yml) — the shared weekly
  // cadence for every other table in this registry is unchanged.
  // QF-20260823-655: an exact `count: 'exact'` eligibility count on this ~3.4M-row table hit
  // Postgres's statement timeout on the shared weekly cron (2 consecutive fails, 08-16/08-23) —
  // a full-table COUNT(*) does a sequential scan regardless of the WHERE clause's selectivity.
  // countMode: 'estimated' uses Postgres's planner row-estimate (EXPLAIN-based, near-instant even
  // on huge tables) instead; every other table above keeps the exact count via the default.
  { table: 'eva_scheduler_metrics', timestampColumn: 'created_at',  hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: 200000, countMode: 'estimated' },
  ...
  // SD-LEO-INFRA-WIND-DOWN-SURVEY-001 (FR-3, operator-contract REAPER): worker_wind_down_events
  // is the dedicated replacement for the feedback(category='wind_down_survey') mirror this SD
  // removed — one row per worker Stop-hook firing, fleet-wide, unbounded. Given a table without
  // a reaper is the same class of slow leak every other trend/telemetry table above got one for,
  // it ships with this entry from day one rather than as a later follow-up. Timestamp column is
  // created_at, DB-DEFAULTED (DEFAULT now()) but the writer (recordWindDown) DOES pass its own
  // value explicitly — deliberately, to keep created_at, dedup_key, and metadata.at derived from
  // the SAME captured `at` rather than racing a second now() call at insert time (TESTING
  // evidence c6902d6e caught this registry's own header claim — "no writer supplies its own
  // value" — was FALSE for this specific entry; corrected here rather than left silently wrong).
  // A worker process cannot meaningfully "backdate" this column the way the header's threat
  // model is guarding against (an untrusted/anon write forging history): the writer is the
  // service-role hook itself, not an external actor, and the column's DEFAULT still applies if
  // the value is ever omitted. 90 days is safe: as of this SD, the table has zero readers
  // (matching the feedback-table mirror it replaces), so there is no consumer lookback window to
  // protect against.
  // QF-20260913-915: pendingFile names this table's creating migration so enforcePolicy() can
  // tell a genuinely-missing relation apart from one that is DEFERRED in the disposition ledger
  // (see resolvePendingRelationStatus in lib/retention/pending-relation.js).
  { table: 'worker_wind_down_events', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP, pendingFile: 'database/migrations/20260821_worker_wind_down_events.sql' },
  // SD-LEO-INFRA-SMS-DELIVERY-STATUS-001 (operator-contract REAPER): sms_status_staging is an
  // INSERT-only landing table for Twilio status callbacks (one row per status TRANSITION per
  // message), drained within minutes by drainSmsStatusStaging into sms_outbound_obligations —
  // the staged row itself carries no long-term value once drained_at is stamped, so it is the
  // same slow-leak-without-a-reaper class every other trend/staging table above got one for.
  // received_at is DB-defaulted (DEFAULT now(), never sent by the RPC — see
  // database/chairman-gated/20260824_sms_status_staging.sql), matching this registry's own
  // no-writer-can-backdate principle. Covered by the existing weekly
  // gha_cron:retention-enforce-cron.yml (no new cadence needed).
  { table: 'sms_status_staging', timestampColumn: 'received_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  // SD-LEO-FEAT-AGENT-READINESS-SERVICE-001: llm_txt_version was DELIBERATELY NOT given an entry
  // here despite being a table CREATOR (see metadata.operator_contract_waiver on the SD for the
  // full reaper reasoning across all three new tables). An age-only entry keyed on created_at
  // would be actively dangerous, not merely decorative: enforcePolicy() (see below in this file)
  // filters purely on `.lt(timestampColumn, cutoff)` with no awareness of published_at, and
  // retention-enforce-cron.yml already runs this array live on main weekly. A venture's LIVE,
  // publicly-served llm.txt (published_at IS NOT NULL, no automatic regeneration) would silently
  // archive-then-DELETE after 90 days of no new draft, breaking the public /llm.txt route with no
  // alert tied to the cause (caught by an adversarial review pass on PR #7113 before merge, not
  // discovered live). Correctly expressing "protect published rows" needs a policy shape with a
  // WHERE-clause/predicate field this registry does not have today -- that is real follow-up scope
  // (a distinct harness gap, not specific to this table), not something to force through here.
  // SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001 (operator-contract REAPER): a released/refused/expired
  // held-decision row has no ongoing service purpose after this window -- unlike llm_txt_version
  // above, there is no "live, publicly-served" state a plain age cutoff could wrongly destroy. A
  // row genuinely stuck in held/releasing surfaces via public.v_chairman_held_sends_unreconcilable
  // (24h horizon) well before the 90-day archive window, so this age-only policy cannot silently
  // erase an unresolved chairman decision -- by the time it's old enough to archive, it has already
  // reached a terminal status or been flagged loud for hours.
  { table: 'chairman_held_sends', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  // SD-LEO-FEAT-GUARDRAILED-BROWSER-ACTUATION-001 (operator-contract REAPER): a per-session
  // browser-actuation cap counter has no service purpose once the session it counted for has
  // gone cold -- read via scripts/fleet-dashboard.cjs browser-audit for the live/recent case,
  // never referenced by an aged-out session.
  // QF-20260913-915: pendingFile is under database/chairman-gated/, so a missing relation here
  // resolves to CEREMONY_PENDING (merged, awaiting the chairman's apply ceremony) rather than an
  // unexplained reaper error — same mechanism as worker_wind_down_events above.
  { table: 'browser_actuation_session_caps', timestampColumn: 'updated_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP, pendingFile: 'database/chairman-gated/20260826_browser_actuation_session_cap.sql' },
  // SD-LEO-INFRA-USAGE-PASTE-LEDGER-001 (operator-contract REAPER): account_usage_pastes gets
  // ~3-10 rows/day (one per chairman /usage paste) -- low volume, but a table shipped without a
  // reaper is a slow leak regardless of rate. hotDays is 730, NOT DEFAULT_HOT_DAYS: unlike its
  // sibling account_usage_snapshots (90 days is safe because its only consumer reads the single
  // most-recent row), this table's own consumer (account-usage-burn-projection.cjs) reads PAIRS
  // of historical rows to derive a burn slope -- a 90-day window risks silently truncating a
  // slow-burn account's usable history. created_at (insert time, DB-defaulted) is keyed rather
  // than pasted_at (caller-supplied, backdatable). Covered by the existing weekly
  // gha_cron:retention-enforce-cron.yml (no new cadence needed).
  { table: 'account_usage_pastes', timestampColumn: 'created_at', hotDays: 730, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  ...
  // ages out -- read live via lib/creative/variant-scoring-bridge.js and
  // scripts/eva/variant-scoring-cli.mjs, never referenced by a stale link.
  { table: 'creative_asset_variant_scores', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  ...
  // change, on a 5-minute sweep cadence, with no unique constraint and no existing cap --
  // measured live seat files run 4.5KB-407KB and one (solomon) grew 45% in three days, so this
  // is the same unbounded-growth class every other trend/staging table in this registry already
  // got a reaper for.
  //
  // TIMESTAMP COLUMN IS last_verified_at, DELIBERATELY NOT created_at (a real defect a second
  // SECURITY review pass caught in this exact entry): writeSeatCheckpoint (lib/fleet/seat-
  // checkpoint-mirror.cjs:114-121) UPDATEs ONLY last_verified_at on an unchanged-content row --
  // created_at is frozen at first insert forever. A stable/rarely-changing seat (e.g. Michael's,
  // the seat this SD most needs to protect) would therefore have a created_at that ages past
  // 90 days while last_verified_at keeps refreshing every 5 minutes -- keying on created_at would
  // let retention archive-and-DELETE that seat's only LIVE checkpoint out from under the read
  // side, producing a recurring false-stale reading on exactly the control this SD ships. Keying
  // on last_verified_at instead gives the right semantics: the live row's last_verified_at is
  // always fresh (refreshed every tick) and never crosses the cutoff while the sweep runs, while a
  // SUPERSEDED row (replaced by a newer content_hash) stops being touched and correctly ages out.
  { table: 'role_seat_checkpoints', timestampColumn: 'last_verified_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  // SD-LEO-INFRA-DEMAND-ENGINE-PART-001 (OPERATOR_CONTRACT gate, REAPER member of the operator
  // triple): mock_outreach_personas is synthetic test data authored only when a mock run is
  // explicitly declared -- one-shot by nature, never drained by a queue, so it has no natural
  // cleanup path of its own and would otherwise accumulate forever exactly like every other
  // table this registry already reapers. created_at is DB-defaulted (migration
  // database/migrations/20260913_mock_outreach_personas.sql), never caller-supplied.
  { table: 'mock_outreach_personas', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
  // SD-LEO-INFRA-MICHAEL-TIER2-CHECKPOINT-SEND-001 (operator-contract REAPER): both tables are
  // append-only ledgers (one row per checkpoint-send verb invocation / per enablement toggle) with
  // no natural drain, the same slow-leak-without-a-reaper class every table above already got one
  // for. created_at is DB-defaulted on both (never caller-supplied). pendingFile names the not-yet-
  // applied migration so enforcePolicy() reads a missing relation as CEREMONY_PENDING rather than an
  // unexplained error (same mechanism as worker_wind_down_events above).
  { table: 'michael_checkpoint_send_ledger', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP, pendingFile: 'database/migrations/20260914_michael_checkpoint_send.sql' },
  { table: 'michael_checkpoint_send_enabled', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP, pendingFile: 'database/migrations/20260914_michael_checkpoint_send.sql' },
];
...
export const SOAK_ENTRIES = [
  ...
  // SD-LEO-INFRA-EVA-SCHEDULER-HYGIENE-001 (FR-2, operator-contract REAPER): the same
  // snapshot/restore shape as management_reviews_quarantine_20260610 above, from the
  // chairman-gated eva_scheduler_queue killed-venture purge
  // (database/migrations/20260821_purge_killed_venture_scheduler_queue.sql). Both archive
  // tables are DELIBERATELY KEPT after apply/restore — they are the purge's own audit trail
  // and its DOWN migration's only data source — so an age-only auto-DELETE reaper would be
  // actively wrong (it would destroy the one thing that makes the purge reversible). A manual,
  // chairman-reviewed drop after a verification soak window is the correct disposition, same as
  // the 2026-06-10 precedent. 14-day soak window, matching that precedent exactly.
  {
    table: 'eva_scheduler_queue_qkilled20260821',
    mode: 'soak_until',
    eligibleAfter: '2026-09-04',
    action: 'manual chairman-reviewed drop once the restore path is no longer wanted (purge audit trail)',
    status: 'awaiting_chairman_apply',
  },
  // Only materializes if the paired DOWN migration is ever applied (break-glass restore) — see
  // 20260821_purge_killed_venture_scheduler_queue_DOWN.sql step 1. Listed here unconditionally so
  // the reaper disposition is on record BEFORE the table can ever exist, not authored reactively
  // after an incident. The soak-report loop in scripts/retention-enforce.js is unconditional over
  // SOAK_ENTRIES and does not require the table to exist, so listing it early is safe.
  {
    table: 'eva_scheduler_queue_qkilled20260821_interlopers',
    mode: 'soak_until',
    eligibleAfter: '2026-09-04',
    action: 'manual chairman-reviewed drop once the restore path is no longer wanted (rollback interlopers trail)',
    status: 'not_yet_created_pending_rollback',
  },
];
