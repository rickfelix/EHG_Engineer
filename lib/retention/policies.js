...
export const RETENTION_POLICIES = [
  ...
  // SD-LEO-FEAT-AGENT-READINESS-SERVICE-001 (operator-contract REAPER, staged-not-applied schema):
  // llm_txt_version gets one row per generated version per venture. Only agent_readiness_audit_run
  // and agent_readiness_audit_sample carry a hard BEFORE UPDATE OR DELETE freeze trigger (append-only
  // measurement evidence) -- llm_txt_version's own trigger (llm_txt_version_publish_only) guards only
  // content/venture_url/content_lint_passed on UPDATE and does not block DELETE at all, so a normal
  // archive-then-delete policy is genuinely functional here, unlike the other two (see metadata.
  // operator_contract_waiver on the SD for why those two are waived rather than given a decorative
  // entry that would perpetually fail on delete). created_at is DB-defaulted (DEFAULT now()), never
  // supplied by draftVersion()/publishVersion(). Table does not exist live yet -- migration is staged
  // at database/chairman-gated/20260816_agent_readiness_audit_schema.sql pending chairman ceremony.
  { table: 'llm_txt_version', timestampColumn: 'created_at', hotDays: DEFAULT_HOT_DAYS, mode: 'archive', perRunCap: DEFAULT_PER_RUN_CAP },
];
