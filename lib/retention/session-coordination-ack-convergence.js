...
export async function convergeAckTTL(supabase, { now = new Date() } = {}) {
  ...
  // SD-LEO-INFRA-ACTIVATE-INERT-STALL-001-C / RCA 9a02a76d: a severity=high row (the class
  // this SD's sibling child made deliverable and severity-preserving) must not be silently
  // mechanically acked just because nobody actioned it within ACK_TTL_DAYS -- that is exactly
  // "directives retiring unread". Age alone is not consent for a high-severity row; genuine
  // acknowledgment must come from the actor, not the clock.
  const eligible = candidates.filter((row) => !(row.payload && row.payload.severity === 'high'));
