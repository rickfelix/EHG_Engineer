...
function mockClaudeSessionsEnrichmentQuery(data, error = null) {
  return {
    ...
    update: vi.fn(() => updateChainStub())
  };
}

// SD-LEO-INFRA-CLAIM-LIFECYCLE-RELEASE-002 (FR-6): the SD reaffirm branch now compare-and-sets, so
// the update chain continues past .eq() into .or().select(). These stubs terminated AT .eq(), which
// is why CI reported "…eq(...).or is not a function" — a MOCK gap, not a defect in the change: the
// real supabase client has always exposed .or/.select here.
//
...
describe('claimGuard', () => {
  ...
  it('queries claude_sessions directly, not v_active_sessions', async () => {
    ...
    mockFrom.mockImplementation((table) => {
      ...
      return { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), maybeSingle: vi.fn().mockResolvedValue({ data: null, error: null }), update: vi.fn(() => updateChainStub()) };
    });
