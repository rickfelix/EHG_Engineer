...
export function lint(message = {}, context = {}) {
  ...
  // 3. explicit reply instruction (decisions only) -- QF-20260903-523: the instruction must
  // name the SAME reply format matchSmsOption() (lib/chairman/sms-bridge.js) actually accepts.
  // That matcher takes ONLY a 1-based numeric index or an exact label match -- free text like
  // "Reply A or B" satisfies neither when the labels aren't literally "A"/"B", so a chairman
  // reply in that exact shape is classified no_match and parks instead of routing (measured
  // live, ratification f6315dbf/31c75f74: a signature-valid chairman answer parked 329ms after
  // draining). Requiring the numbered form is the only format guaranteed correct regardless of
  // label content -- do not loosen the matcher instead, its fail-closed design is correct.
  if (isDecision) {
    const opts = Array.isArray(message.options) ? message.options : [];
    const ri = typeof message.replyInstruction === 'string' ? message.replyInstruction.trim() : '';
    const hasRi = ri.length > 0;
    const numbers = ri.match(/\d+/g) || [];
    const namesEveryIndex = opts.length > 0 && opts.every((_, i) => numbers.includes(String(i + 1)));
    const ok = hasRi && namesEveryIndex;
    add('reply_instruction', ok, true, ok ? 'ok' : hasRi
      ? `reply instruction must name each numbered option (1-${opts.length}) -- e.g. "Reply 1 or 2"; matchSmsOption() only accepts a 1-based index or an exact label match, and free text like "A"/"B" matches neither`
      : 'a decision needs an explicit reply instruction (how to answer)');
  }
