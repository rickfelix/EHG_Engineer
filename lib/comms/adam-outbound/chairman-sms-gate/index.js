...
export async function sendChairmanSMS(message = {}, context = {}, opts = {}) {
  ...
  // QF-20260728-077: decisions carry their options/replyInstruction/noReplyConsequence as
  // separate structured fields (needed for staging above); the actual wire text needs them
  // folded into body, or the chairman receives a recommendation with nothing to choose between.
  // Reused below for the soft-fail email fallback too, so a failed decision send doesn't lose
  // the same context a second time.
  const wireMessage = isDecision ? { ...message, body: composeDecisionSmsBody(message) } : message;
  let sendResult;
  try {
    sendResult = await sender.send(wireMessage);
  } catch (err) {
  ...
  if (sendResult && sendResult.softFailed) {
    ...
    const fb = await fallbackSend({ message: wireMessage, reason: sendResult.reason });
