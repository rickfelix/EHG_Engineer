...
import { stageDecisionSmsNotification, invalidateDecisionSmsStaging, updateNotificationStatus, resolveAllParkedChairmanSmsRows, TOKEN_TTL_MS } from '../../../chairman/sms-bridge.js';
import { smsQuietWindowReleaseIso } from '../../../time/chairman-et-wall-clock.js';

/**
 * SD-LEO-INFRA-WIRE-CHAIRMAN-SMS-001 FR-3: sender identity for the pre-send consult row when a
 * chairman-lane caller has no live interactive session (cron sweeps, decision-scheduler). Named
 * and exported so every non-interactive caller uses the SAME sentinel rather than each inventing
 * its own string. Safe against session_coordination.sender_session (text, nullable, no FK) --
 * verified at LEAD (evidence e81f1e7b-7875-4cc5-acca-20f0cf5ff5ec). Callers with a real session
 * id (message.sessionId) keep it; this is a fallback, never a universal default.
 */
export const CHAIRMAN_LANE_AUTOMATED_SENTINEL = 'chairman-lane-automated';
...
/**
 * SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001 (FR-3): DB-first, fail-loud chairman identity
 * resolution. Replaces the previous `message.chairmanUserId || process.env.CHAIRMAN_USER_ID`
 * fallback -- CHAIRMAN_USER_ID is unset repo-wide and no production caller of sendChairmanSMS
 * supplies message.chairmanUserId, so that fallback always evaluated to undefined and failed
 * chairman_notifications.chairman_user_id NOT NULL deep in the pipeline (23502), not at the
 * point of resolution. An explicit message.chairmanUserId is still honored as an override --
 * this is never a downgrade path, only a way to skip the RPC when the caller already knows.
 * Exported + injectable via opts.resolveChairmanUserId so unit tests can stub without a live DB.
...
/**
 * QF-20260728-077: message.options/replyInstruction/noReplyConsequence are validated by the
 * rubric gate above (lint.js checks 1/3/4) and separately used for web staging
 * (extractOptionLabels feeding stageDecisionSmsNotification), but neither of those uses ever
 * folds them into message.body -- the one field the transport actually transmits. The gate
 * enforced a contract the wire text never honoured: a decision could pass with well-formed
 * options and reply instructions while the chairman received only the free text and a bare
 * "Recommend: X" naming an option that was never defined. Compose the complete wire body once,
 * here at the send choke point, so every caller of sendChairmanSMS is covered.
 */
function composeDecisionSmsBody(message) {
  const parts = [message.body];
  const labels = extractOptionLabels(message.options);
  if (labels.length > 0) parts.push(labels.join('\n'));
  if (typeof message.replyInstruction === 'string' && message.replyInstruction.trim()) {
    parts.push(message.replyInstruction.trim());
  }
  if (typeof message.noReplyConsequence === 'string' && message.noReplyConsequence.trim()) {
    parts.push(message.noReplyConsequence.trim());
  }
 ...
 *
 * QF-20260902-034, LIVE INCIDENT 2026-09-03: a unit test whose sender throws and doesn't inject
 * opts.fallbackSend falls through to THIS function with the real (env-inherited) recipient,
 * emailing the chairman fixture text. This is the single choke point for every caller (real or
 * test) that skips the opts.fallbackSend seam, so the refusal belongs here, not per-test.
 */
async function defaultFallbackSend({ message, reason }) {
  try {
    if (process.env.VITEST || process.env.NODE_ENV === 'test') {
      return { fired: false, reason: 'refused_test_env' };
    }
...
async function evalOverAsk(message, context, opts) {
  ...
  // QF-20260831-281: a reply to a chairman inbound request is an ANSWER, not an ask — the
  // classifier keys on vocabulary (ruling/approve/recommendation), not speech-act, so a report
  // the chairman explicitly requested was misclassified as an over-ask purely on word choice
  // (two false holds in one night, both passed verbatim-in-substance after cosmetic reword).
  // context.replyToInbound is caller-declared, mirroring the existing chairmanInitiated exemption.
  if (context.replyToInbound === true) return { held: false, reason: 'reply to chairman inbound request' };
...
export async function sendChairmanSMS(message = {}, context = {}, opts = {}) {
  ...
  // QF-20260728-077: fold options/replyInstruction/noReplyConsequence into body HERE — after
  // evalOverAsk (which classifies the free-text body semantically; composing earlier fed it
  // reply-instruction/option-list boilerplate and made it misclassify well-formed decisions as
  // over-asks), but before the rubric gate below. Composing any later (e.g. just before dispatch)
  // would let the length (max 1600 chars, check 5) and no-secrets (check 6) rubric checks validate
  // the short pre-composition body while a longer, unvalidated body is what actually transmits —
  // the exact "gate validates a proxy, transport sends something else" shape this fix exists to
  // close. From here on every check and every send path (including the soft-fail email fallback)
  // works off the SAME body that reaches the chairman.
  //
  // SD-LEO-INFRA-CHAIRMAN-SMS-DECISION-002 (FR-4): opts.skipCompose lets a caller whose
  // message.body was ALREADY composed by a prior pass through this gate skip a SECOND
  // composition. The one caller that needs this is releaseHeldSend (lib/adam/chairman-held-
  // send-release.js): chairman_held_sends.body is persisted AFTER composition (this same line
  // ran once already, before the hold), so re-dispatching the held row through sendChairmanSMS
  // without this flag would fold message.options/replyInstruction/noReplyConsequence into the
  // body a SECOND time — a visibly duplicated message to the chairman (VALIDATION sub-agent
  // finding F1, LEAD-TO-PLAN). Every other caller omits the flag and composes exactly as before.
  if (isDecision && !opts.skipCompose) message = { ...message, body: composeDecisionSmsBody(message) };
  ...
  // Blocked verdict -> not sent (FR-2). QF-20260902-939: a DECISION blocked SOLELY by quiet_hours
  // is a QUEUE, not a drop -- it goes through the SAME chairman_held_sends lane the pre-send
  // consult hold below writes to (hold_reason='quiet_hour', consult_correlation_id left NULL --
  // none was ever sought), expiring at the next 06:00 local so the release sweep's
  // hold_expires_at-only flush (lib/adam/chairman-held-send-release.js
  // releaseExpiredQuietHourHold) can find and send it. Any OTHER rubric block on a decision (any
  // other failing check, alone or mixed with quiet_hours) means the packet itself is malformed and
  // must be revised -- it is DROPPED, never silently queued and never reported held:true without a
  // row (the defect this QF closes). A non-decision (status) send keeps the PRE-EXISTING
  // held:true/'blocked' shape byte-identical -- scripts/adam-chairman-sms.mjs's formatSendResult()
  // depends on that exact shape to print its own "DROPPED" wording (NON-GOAL: unchanged).
  if (result.verdict !== 'pass') {
    const blockingFailed = (result.lintFindings || []).filter((f) => f.blocking && !f.ok);
    const quietHoursOnly = blockingFailed.length === 1 && blockingFailed[0].check === 'quiet_hours';

    if (isDecision && quietHoursOnly) {
      const queuedFor = smsQuietWindowReleaseIso(context.now || Date.now(), context.chairmanZone)
        || new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
      try {
        const supabase = opts.supabase || await makeDefaultSupabaseClient();
        const resolveIdentity = typeof opts.resolveChairmanUserId === 'function' ? opts.resolveChairmanUserId : resolveChairmanUserId;
        const chairmanUserId = await resolveIdentity(supabase, message);
        const { error: holdInsertError } = await supabase.from('chairman_held_sends').insert({
          chairman_user_id: chairmanUserId,
          chairman_email: message.chairmanEmail || process.env.CHAIRMAN_EMAIL,
          recipient_phone: message.recipientPhone || process.env.CHAIRMAN_PHONE,
          decision_id: message.decisionId || null,
          subject: message.subject || '[CHAIRMAN SMS]',
          body: message.body,
          options: extractOptionLabels(message.options),
          reply_instruction: typeof message.replyInstruction === 'string' ? message.replyInstruction : null,
          reply_id: typeof message.replyId === 'string' ? message.replyId : null,
          no_reply_consequence: typeof message.noReplyConsequence === 'string' ? message.noReplyConsequence : null,
          message_kind: message.type || null,
          sender_callsign: message.senderCallsign || 'chairman-sms-gate',
          session_id: message.sessionId || CHAIRMAN_LANE_AUTOMATED_SENTINEL,
          hold_reason: 'quiet_hour',
          hold_expires_at: queuedFor,
        });
        if (holdInsertError) {
          log.error(`[chairman-sms-gate] ⚠ FAILED to persist quiet-hour hold (message will be lost): ${holdInsertError.message}`);
        }
      } catch (err) {
        log.error(`[chairman-sms-gate] ⚠ FAILED to persist quiet-hour hold (message will be lost): ${err.message}`);
      }
      log.warn(`[chairman-sms-gate] quiet-hours block, QUEUED for ${queuedFor}`);
      return { sent: false, queued_for: queuedFor, reason: 'quiet_hours_queued', verdict: result.verdict, blockedReasons: result.blockedReasons || [] };
    }

    if (isDecision) {
      log.warn(`[chairman-sms-gate] blocked by rubric, DROPPED: ${(result.blockedReasons || []).join('; ')}`);
      return { sent: false, dropped: true, reason: 'blocked', verdict: result.verdict, blockedReasons: result.blockedReasons || [] };
    }
  ...
  // SD-LEO-INFRA-WIRE-CHAIRMAN-SMS-001: PRE-SEND Solomon-consult gate, mirroring the coordinator
  // lane's wiring (scripts/adam-advisory.cjs:1218-1281, SD-LEO-INFRA-ADAM-PRE-SEND-001). The
  // chairman lane never had this: consequential Adam-to-chairman recommendations reached the
  // chairman unconsulted. Gated on isDecision (a status/heartbeat send never consults). Passes
  // isChairmanTargeted:true, activating the hold-and-surface branch in
  // lib/adam/should-consult-solomon.js:367-370 -- unlike the coordinator lane (which
  // process.exit(3)s from a CLI), this is a library function: a hold returns a held-shape result
  // instead of dispatching. Every other outcome (skip/send/pendingReconcile/degraded-without-hold)
  // falls through to dispatch unchanged. Fail-OPEN on any error in the consult mechanism itself --
  // a gate bug must never block a chairman send that already passed the rubric.
  if (isDecision) {
    try {
      // Injectable seam (opts.runPreSendConsultLane), matching this file's established pattern
      // for opts.presenceResolver/opts.classifyOverAsk/opts.resolveChairmanZone -- unit tests
      // stub the WHOLE consult mechanism here rather than mocking six separate dynamic imports.
      // Production omits the override and dynamically imports the real modules.
      const runPreSendConsultLane = typeof opts.runPreSendConsultLane === 'function'
        ? opts.runPreSendConsultLane
        : (await import('../../../adam/presend-consult-lane.cjs')).runPreSendConsultLane;
      const { evaluatePreSendConsult, performBoundedConsult } = await import('../../../adam/should-consult-solomon.js');
      const { getActiveSolomonId } = await import('../../../coordinator/solomon-identity.cjs');
      const { insertCoordinationRow } = await import('../../../coordinator/dispatch.cjs');
      const { buildSolomonConsultPayload } = await import('../../../../scripts/worker-signal.cjs');
      const { buildPreSendConsultBody } = await import('../../../../scripts/adam-advisory.cjs');
      const supabase = opts.supabase || await makeDefaultSupabaseClient();
      // QF-20260728-944 lineage: sender_session carries no FK constraint (verified at LEAD), so a
      // documented sentinel is safe for callers with no live interactive session (cron sweeps,
      // decision-scheduler). Callers that DO have a real session id (message.sessionId) keep it --
      // the sentinel is a fallback, never a universal default, so attribution is preserved
      // wherever it genuinely exists.
      const sessionId = message.sessionId || CHAIRMAN_LANE_AUTOMATED_SENTINEL;
      // QF-20260905-123: the RAISING session (the Adam seat that called sendChairmanSMS), kept
      // separate from `sessionId` above and NEVER defaulted to the sentinel -- Solomon's
      // courtesy-copy-to-originator resolves against this (payload.origin_session), and the
      // sentinel is not a real session id (assertValidTarget's target-shape check refuses it,
      // which is exactly why the courtesy copy was silently lost). Null when genuinely
      // unknown (no message.sessionId AND no live CLAUDE_SESSION_ID), which the consult lane
      // and resolveConsultOriginator both already treat as "no CC" (fail-open, unchanged).
      const originatorSessionId = message.sessionId || process.env.CLAUDE_SESSION_ID || null;
      const outcome = await runPreSendConsultLane(
        {
          subject: message.subject || '[CHAIRMAN SMS]',
          body: message.body,
          senderCallsign: message.senderCallsign || 'chairman-sms-gate',
          sessionId,
          originatorSessionId,
          repo: process.cwd(),
          // QF-20260905-194: session_coordination.expires_at is timestamptz -- an epoch-ms
          // NUMBER made Postgres reject the insert ('date/time field value out of range'),
          // so no consult row was ever written and the chairman-targeted hold could never
          // be released. Must be an ISO string.
          expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
          isChairmanTargeted: true,
          addressee: { role: 'chairman', sessionId: null },
          // QF-20260912-901: per-number provenance stamps a composer attached to this packet.
          measuredBy: message.measuredBy || null,
        },
        {
          ...
          insertCoordinationRow: (row, rowOpts) => insertCoordinationRow(supabase, row, rowOpts),
        }
      );
      // QF-20260912-901: a first-use number with no measured_by[] stamp refuses outright -- never
      // held (nothing was ever cited to release), never dispatched. The composer must measure and
      // stamp the named number, then re-send.
      if (outcome.action === 'refuse') {
        log.error(`[chairman-sms-gate] ⛔ PRE-SEND REFUSED: body cites "${outcome.unstampedNumber}" with no measured_by[] provenance stamp -- measure it and re-send (QF-20260912-901).`);
        return { sent: false, held: false, reason: 'unstamped_number_refused', unstampedNumber: outcome.unstampedNumber };
      }
      if (outcome.action === 'hold-and-surface') {
        log.error('[chairman-sms-gate] ⛔ PRE-SEND HOLD: consequential chairman-facing send held pending Solomon (degraded) -- not dispatched.');
        // QF-20260828-255 (Solomon advisory f02c00e7, held-send 5ce0c4b5): persist-vs-return --
        // the hold state used to persist unconditionally even when NO consult row existed to ever
        // resolve it. presend-consult-lane.cjs now verifies the row in the same action; surface
        // that verdict loudly here (distinct log + distinct hold_reason below) rather than letting
        // an unresolvable hold look identical to a normal, resolvable one.
        if (outcome.consultRowVerified !== true) {
          log.error(`[chairman-sms-gate] ⛔⛔ QF-20260828-255: hold entered WITHOUT a verified consult row (consultRowId=${outcome.consultRowId || 'none'}) -- this hold may be UNRESOLVABLE until reconciled manually.`);
        }
        // SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001 (FR-1): persist the held decision durably so a
        // later Solomon verdict can release it via the reconcile sweep. Carries MATERIALS only --
        // stageDecisionSmsNotification (which mints the live reply token) still runs ONLY on the
        // release path below, never here, preserving SD-LEO-INFRA-SMS-DECIDE-REPLY-MATCHABLE-001's
        // guarantee that a never-sent message never receives a live reply token. Best-effort: a
        // persistence failure must not change the hold outcome (the send is correctly held either
        // way), but it IS logged loudly -- an unpersisted hold can never be reconciled or released.
        try {
          const resolveIdentity = typeof opts.resolveChairmanUserId === 'function' ? opts.resolveChairmanUserId : resolveChairmanUserId;
          const supabaseForHold = opts.supabase || await makeDefaultSupabaseClient();
          const heldChairmanUserId = await resolveIdentity(supabaseForHold, message);
          const { error: holdInsertError } = await supabaseForHold.from('chairman_held_sends').insert({
            consult_correlation_id: outcome.correlationId || null,
            // SD-LEO-INFRA-CHAIRMAN-SMS-DECISION-002 (FR-1): the row id of the pre-send consult
            // this hold anchors to (Explore defect 1) — previously never written despite the
            // column existing, leaving every held row unreadback-verifiable against its own
            // consult insert. Sourced from runPreSendConsultLane's outcome (threaded through
            // performBoundedConsult's hold-and-surface arm), not re-derived here.
            consult_row_id: outcome.consultRowId || null,
            chairman_user_id: heldChairmanUserId,
            chairman_email: message.chairmanEmail || process.env.CHAIRMAN_EMAIL,
            recipient_phone: message.recipientPhone || process.env.CHAIRMAN_PHONE,
            decision_id: message.decisionId || null,
            subject: message.subject || '[CHAIRMAN SMS]',
            body: message.body,
            options: extractOptionLabels(message.options),
            // SD-LEO-INFRA-CHAIRMAN-SMS-DECISION-002 (FR-3): persist the rubric-required reply
            // fields so a release re-evaluation of the rubric (lint.js checks 3 and 9) doesn't
            // rubric-block on fields that were present on `message` at hold time but never
            // survived to the release-path reconstruction (Explore defect 3).
            reply_instruction: typeof message.replyInstruction === 'string' ? message.replyInstruction : null,
            reply_id: typeof message.replyId === 'string' ? message.replyId : null,
            no_reply_consequence: typeof message.noReplyConsequence === 'string' ? message.noReplyConsequence : null,
            consequence_level: outcome.consequence || 'high',
            message_kind: message.type || null,
            sender_callsign: message.senderCallsign || 'chairman-sms-gate',
            session_id: sessionId,
            // QF-20260828-255: distinguishable so a reconcile sweep can find unresolvable holds.
            hold_reason: outcome.consultRowVerified === true ? (outcome.reason || 'solomon-consult-hold') : 'solomon-consult-hold-unverified',
            // QF-20260905-123: no dedicated column exists for this (never added by a migration
            // here -- a Tier-1 fix does not alter schema); metadata is the documented free-form
            // column this table already carries for exactly this kind of provenance. Null when
            // genuinely unknown, matching originatorSessionId's own null-safe contract above.
            metadata: originatorSessionId ? { originator_session_id: originatorSessionId } : null,
          });
          if (holdInsertError) {
            log.error(`[chairman-sms-gate] ⚠ FAILED to persist held send (unreconcilable until fixed): ${holdInsertError.message}`);
          }
        } catch (err) {
          log.error(`[chairman-sms-gate] ⚠ FAILED to persist held send (unreconcilable until fixed): ${err.message}`);
        }
        return { sent: false, held: true, reason: 'pre_send_consult_hold', verdict: 'pass', authorityClass: result.authorityClass };
      }
      if (outcome.pendingReconcile) log.log(`[chairman-sms-gate] ✓ PRE-SEND CONSULT QUEUED (non-blocking) — Solomon verdict will be reconciled on arrival (corr: ${outcome.correlationId}).`);
      else if (outcome.degraded) log.warn('[chairman-sms-gate] ⚠ PRE-SEND DEGRADED-PROCEED: Solomon consult unavailable; proceeding with caution.');
      else if (outcome.action === 'send') log.log('[chairman-sms-gate] ✓ PRE-SEND CONSULT recorded — Solomon verdict received; sending.');
    } catch (err) {
  ...
  // status/heartbeat sends (TR-3) are untouched and no supabase client is constructed for them.
  // NOTE (SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001, REGRESSION sub-agent finding F3): decision sends
  // held above are NO LONGER untouched in that sense — the hold branch now best-effort constructs
  // its own client and issues an RPC + insert to persist the held decision (see above). This block
  // (staging BEFORE dispatch) still never runs for a held send; only the "no DB I/O at all" half of
  // the old claim no longer holds. Runs BEFORE dispatch; never throws out of sendChairmanSMS (TR-3)
  // — not every caller of this function wraps it in try/catch.
  //
  ...
  if (isDecision && message.decisionId) {
    try {
      ...
      // SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001 (FR-3): DB-first, fail-loud resolution -- the
      // previous `|| process.env.CHAIRMAN_USER_ID` fallback always evaluated to undefined (the env
      // var is unset repo-wide) and failed chairman_notifications.chairman_user_id NOT NULL deep in
      // stageDecisionSmsNotification (23502) rather than here. A throw here is caught by this
      // block's existing try/catch (below) and returns {sent:false, reason:'notification_stage_failed'}
      // BEFORE any DB write is attempted -- fail loud at the point of resolution, not mid-pipeline.
      const resolveIdentityForStaging = typeof opts.resolveChairmanUserId === 'function' ? opts.resolveChairmanUserId : resolveChairmanUserId;
      const chairmanUserId = await resolveIdentityForStaging(supabase, message);
  ...
  // QF-20260818-263: a confirmed reply-class send resolves whatever parked chairman inbound is
  // outstanding in the SAME write path — answered implies resolved, no separate manual
  // resolve-parked-chairman-sms.cjs step. Fail-soft, same shape as the notification-status update
  // above: never lets a bookkeeping failure downgrade an already-genuinely-successful send.
  if (context.replyToInbound === true) {
    try {
      const resolveSupabase = opts.supabase || await makeDefaultSupabaseClient();
      await resolveAllParkedChairmanSmsRows(resolveSupabase);
    } catch (err) {
      log.error(`[chairman-sms-gate] parked-inbound auto-resolve failed (non-fatal, send already succeeded): ${err.message}`);
    }
  }
