...
export function cite(spec = {}) {
  ...
  const out = {
    ...
    citation: {
      ...
      // GRAINS — a locator finer than a row id, for facts that live INSIDE a row.
      //
      // Added for FR-3, which requires citing an sd row-id PLUS the claim_history array index
      // PLUS the entry timestamp. Before this, the output object was CLOSED: extra spec keys were
      // silently dropped, so leg 2 could only emit SD ids and shipped a limitation string
      // explaining why the finer grain was unavailable.
      //
      // The VALIDATION sub-agent named what that actually was: A RATIONALE SHIPPED IN PLACE OF
      // THE ASSERTION. The ruling asked for the composite locator PRECISELY BECAUSE claim events
      // have no row id of their own — so citing only the SD id discards the two components that
      // locate the event, while the honest-sounding limitation made it read as though the ruling
      // had been honoured.
      //
      // This does not widen C4, it serves it: the invariant is that every number be re-derivable
      // from its cited query, and "which entry, at what time" is exactly what makes a claim-event
      // count re-derivable. Still citations and predicates only — no copied state.
      ...(Array.isArray(spec.grains) && spec.grains.length > 0 ? { grains: spec.grains } : {}),
