 ...
 *
 * ── AC-8 / FR-7 SHIPS UNMET. READ THIS BEFORE TRUSTING THE FUNCTION BELOW. ────────────────
 * Fixing the column name above did NOT close AC-8, and for a while this file's comments implied
 * it had. THIS FUNCTION HAS ZERO PRODUCTION CALLERS. A rule implemented in a function nobody
 * calls is not in force, so nothing here alarms on anything.
 *
 * The live path is scripts/periodic-liveness-watcher.mjs reading this row's last_fired_at and
 * grace_multiplier. AND THE TWO DISAGREE ON EXACTLY THE STATE THIS SD SHIPS IN: with the
 * drive_reports table absent the sweep returns early WITHOUT stamping, so last_fired_at stays
 * NULL — isSelfStale() calls that stale:true (it would alarm), while the watcher returns
 * UNVERIFIED (periodic-liveness-watcher.mjs:248, 'no_last_fired_data_available') and escalates
 * only past 7 CONTINUOUS DAYS, never at 2x cadence.
 *
 * So FR-7's alarm degrades in precisely the condition it exists to detect — a compensating
 * control failing under the condition it compensates for.
 *
 * PRECISION, because "the alarm does not work" would overstate it: AC-8 IS SATISFIED ONCE A
 * REPORT HAS SUCCEEDED ONCE. stamp() then writes last_fired_at AND grace_multiplier: 2 together,
 * and a >48h gap yields OVERDUE on the watcher's cron. The gap is exactly the NEVER-PRODUCED
 * report — which is the state at merge, and the most missing a report can be. A second, smaller
 * leg of the same defect: registerArmedMachinery never sets grace_multiplier at all, so until the
 * first success the row also carries the column default 3 rather than FR-7's 2. Setting it at
 * REGISTRATION would close that leg; it would NOT close the null-last_fired_at leg, which is the
 * watcher's ratified behaviour and needs the ruling described below.
 *
 * THIS IS NOT UNILATERALLY FIXABLE FROM THIS SD. The watcher's null-is-UNVERIFIED behaviour is
 * ANOTHER SD's ratified acceptance criterion ("degrade to today's exact state (UNVERIFIED),
 * never a false OVERDUE/OK alarm"). FR-7 as written CONFLICTS with it, and resolving that needs
 * a ruling on which requirement yields — not a quiet edit to a fleet-wide watcher. Carried into
 * PLAN-TO-LEAD as a NAMED UNMET CRITERION with that conflict stated, never as a limitation.
 */
