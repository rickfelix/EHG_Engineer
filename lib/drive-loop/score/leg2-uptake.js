...
/**
 * ⚠️ NOT RATIFIED, and no longer load-bearing. SD-LEO-FIX-DRIVE-SCORE-GRADIENT-001 replaced the
 * binary cliff this constant used to gate (fraction >= 0.8 or nothing) with a continuous score —
 * see `earned` below. Retained as an export for backward compatibility / historical reference
 * only; scoreLeg2() no longer reads it.
 */
export const UPTAKE_THRESHOLD = 0.8;

/**
 * ⚠️ NOT RATIFIED, same status as UPTAKE_THRESHOLD above. A rescale alone (fraction * LEG_POINTS)
 * lets a SINGLE qualifying claim out of a ranked top-5 of 1 report the same as a well-sampled 5-of-5
 * day — measured directly against drive_reports history captured during PLAN: grains=1 on 11 of 20
 * historical readings, every one of them at the fraction ceiling. GRAIN_FLOOR is a confidence floor
 * on the absolute claim COUNT (not the fraction): a reading with fewer qualifying grains than this
 * is scaled down, never scored to zero and never divided by zero (the floor is a fixed positive
 * constant, not the denominator). Exported and injectable precisely so ratifying a different number
 * is a one-line change that cannot disturb the measurement above.
 */
export const GRAIN_FLOOR = 3;
 ...
 * @param {number} [o.grainFloor]
 */
export function scoreLeg2({ rankedTop5 = [], nowMs, grainFloor = GRAIN_FLOOR } = {}) {
  ...
  // ONE pass: claimGrain runs exactly once per SD, and grains/claimed are both derived from
  // that same per-SD result — so the count and the citation cannot describe different sets.
  const perSd = rankedTop5.map((sd) => ({ sd, grain: claimGrain(sd, nowMs) }));
  const grains = perSd.map((r) => r.grain).filter(Boolean);
  const claimed = perSd.filter((r) => r.grain !== null).map((r) => r.sd);
  ...
  // GRAIN_FLOOR is a fixed positive constant, never the (possibly zero) denominator, so this
  // division can never be by zero. grains.length is 0 exactly when fraction is 0, so confidence
  // never manufactures points fraction alone would not already have earned.
  const confidence = Math.min(grains.length, grainFloor) / grainFloor;
  const earned = denominator > 0 ? fraction * LEG_POINTS * confidence : 0;
  ...
  return {
    ...
    points: cite({
      ...
      row_ids: claimed.map((sd) => sd.id),
      grains,
      predicate: `${LEG_POINTS} points * fraction * confidence, where confidence = `
        + `min(grains, ${grainFloor}) / ${grainFloor} — a continuous rescale (SD-LEO-FIX-DRIVE-SCORE-`
        + 'GRADIENT-001), not the binary cliff this leg used to apply. UPTAKE_THRESHOLD is retained '
        + 'as an inert historical export and no longer gates this value',
      limitation: 'THE RESCALE AND GRAIN_FLOOR ARE NOT RATIFIED. FR-3 specifies the fraction and is '
        + `silent on what earns the points; ${grainFloor} is a placeholder recorded here rather than `
        + `settled, and is injectable so ratification changes it without touching the measurement. ${limitation}`,
      source: 'lib/drive-loop/score/leg2-uptake.js scoreLeg2',
    }),
  };
}
