  ...
  const out = {
    score: cite({
      ...
      limitation: `Denominator RATIFIED at ${SPEC_LEG_COUNT} legs / ${SPEC_LEG_COUNT * POINTS_PER_LEG} `
        ...
        + 'zeroed. leg2 is now a continuous fraction*confidence score (SD-LEO-FIX-DRIVE-SCORE-'
        + 'GRADIENT-001); its GRAIN_FLOOR confidence floor remains provisional/injectable, not '
        + 'ratified by this SD',
    ...
    measured_legs: measured.map((l) => {
      ...
      return {
        ...
        // leg4's bidirectional capacity verdict (DEFICIT-URGENT/DEFICIT/TIGHT/SURPLUS). Absent for
        // every other leg, which carries no such field — never defaulted to a placeholder string,
        // so a consumer's absence check (leg lookup returning undefined) stays meaningful.
