...
export function resolveOwner(item = {}) {
  ...
  // QF-20260807-032: an sd.owner_lane fallback used to sit here, but that field exists NOWHERE
  // on strategic_directives_v2 (42703 undefined_column; absent from metadata) -- the branch could
  // never fire and silently implied an owner-attribution capability this function does not have.
  // Pruned. Deliberately NOT falling back to item.lane either: it would fill this field on nearly
  // every row with an intake-routing value and read as an owner.
  return { owner: null, basis: sd ? 'SD exists but is unclaimed' : 'no SD — the item is unsourced' };
}
...
export function resolveChain(waves = [], items = []) {
  ...
  // QF-20260807-032: an it.sd.unmet_dependencies check used to sit here, but that field exists
  // NOWHERE on strategic_directives_v2 -- pruned (see belt-diagnosis.js's classifyItem for the
  // measurement).
  ...
  return {
    ...
    blocker: blocker
      ? cite({
        ...
        table: 'roadmap_wave_items',
        row_ids: [blocker.id],
        predicate: 'the FIRST item in the chain that cannot proceed — a blocked-on-* lane or a blocked SD. NOT merely the first item: an unclaimed head is waiting, not blocked, and reporting waiting as blocked sends someone to unblock nothing',
        source: 'lib/drive-loop/sections/chain-to-gate.js resolveChain',
