...
export function classifyItem(item = {}) {
  ...
  // QF-20260807-032: sd.unmet_dependencies was checked here but exists NOWHERE on
  // strategic_directives_v2 (42703 undefined_column; absent from metadata) -- the branch could
  // never fire and silently implied a capability this classifier does not have. Pruned; blocked
  // status and the blocked-on-* lane are the two REAL blocked signals.
  const blockedLane = typeof item.lane === 'string' && item.lane.startsWith('blocked-on-');
  if (blockedLane || sd.status === 'blocked') {
...
export function buildBeltDiagnosis(items = []) {
  ...
  const PREDICATES = {
    ...
    [CASES.BLOCKED]: 'items whose SD is blocked, or sits on a blocked-on-* lane — something else must happen first. NOT implied by the absence of a claim',
