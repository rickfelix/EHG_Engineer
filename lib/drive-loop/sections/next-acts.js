 ...
 */
// QF-20260807-032: this set used to also carry AWAIT_DECISION ('blocked on a human ruling'),
// gated on sd.blocked_on_decision. That field exists NOWHERE on strategic_directives_v2 (42703
// undefined_column; absent from metadata) -- the branch producing AWAIT_DECISION could never
// fire, so the act was a phantom member of a "closed" set: listed as possible, never emitted.
// Removed rather than left dead, per the same measurement as belt-diagnosis.js/chain-to-gate.js.
...
export function nextAct(item = {}) {
  ...
  // QF-20260807-032: sd.unmet_dependencies was also checked here as a second UNBLOCK trigger,
  // but that field exists NOWHERE either -- pruned along with AWAIT_DECISION above.
  if (sd.status === 'blocked') return ACTS.UNBLOCK;
...
export function ownerOf(item = {}) {
  ...
  // QF-20260807-032: an sd.owner_lane fallback used to sit here; pruned (see chain-to-gate.js's
  // resolveOwner for the same fix and the measurement backing it).
