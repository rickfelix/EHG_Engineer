...
import { MARKER_FILENAME as FLEET_LOCK_HASH_MARKER_FILENAME } from './fleet-lock-hash.mjs';
...
/**
 * QF-20260903-765: the isolate success path built a REAL node_modules via `npm install`
 * but never wrote lib/fleet-lock-hash.mjs's marker into it, so sd-start's
 * evaluateInstallDecision (which only consults that marker) always saw "no hash marker"
 * on this worktree and re-ran a full install on an already-complete tree — one run hit
 * the 120s timeout. Synchronous (this whole module is deliberately sync/injectable for
 * testing without a real install), mirroring fleet-lock-hash.mjs's own writeMarker()
 * format (hash + timestamp + session id, first line is the hash) without making this
 * module's exported functions async, which would ripple into call sites whose existing
 * try/catch around a synchronous call would stop catching a promise rejection.
 * Best-effort: a missing/unreadable lockfile or node_modules leaves no marker, and the
 * NEXT sd-start simply re-installs (today's behavior) rather than crashing here.
 */
export function writeIsolatedFleetLockHashMarkerSync(worktreePath, sessionId) {
  try {
    const lockPath = path.join(worktreePath, 'package-lock.json');
    const nmDir = path.join(worktreePath, 'node_modules');
    if (!fs.existsSync(lockPath) || !fs.existsSync(nmDir)) return { written: false };
    const hash = createHash('sha256').update(fs.readFileSync(lockPath)).digest('hex').slice(0, 12);
    const markerPath = path.join(nmDir, FLEET_LOCK_HASH_MARKER_FILENAME);
...
export function provisionWorktreeNodeModules(worktreePath, options = {}) {
  const { repoRoot, mode = getIsolationMode(), activeSessionCount, freeDiskBytes, sessionId, deps = {} } = options;
  const {
    decide = decideWorktreeProvisionMode,
    symlink = symlinkNodeModules,
    runInstall = defaultRunInstall,
    writeMarker = defaultWriteMarker,
    writeLockHashMarker = writeIsolatedFleetLockHashMarkerSync,
  ...
  try {
    ...
    // QF-20260903-765: the tree this install just built is complete — record its
    // lockfile hash so the checker sd-start already consults (fleet-lock-hash.mjs's
    // readMarker/composeInstallDecision) can answer "already installed" next time,
    // instead of finding no marker and re-running a redundant install.
    writeLockHashMarker(worktreePath, sessionId);
    const hooks = ensureHuskyHooks(worktreePath);
    if (!hooks.ok) log(`[worktree-provision] husky hook re-link failed (non-fatal): ${hooks.error}`);
