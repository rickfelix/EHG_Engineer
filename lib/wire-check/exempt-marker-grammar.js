...
export const DATED_MARKER = /@wire-check-exempt\s+([\w.-]+)\s+expires:(\d{4}-\d{2}-\d{2})/;
export const BARE_MARKER = /@wire-check-exempt\b(?!\s+[\w.-]+\s+expires:)/;
...
export function isExemptionActive(marker, now = new Date()) {
  ...
  if (marker.kind === 'dated') {
    ...
    if (!Number.isFinite(expiresMs)) return false; // malformed expiry -- fail closed, no grace
...
export function isExemptionCompliant(marker, now = new Date()) {
  ...
  if (marker.kind === 'dated') {
    const expiresMs = Date.parse(marker.expiresAt);
    if (!Number.isFinite(expiresMs)) return false; // malformed expiry -- fail closed
