...
async function acquireLock(supabase, sessionId) {
  ...
  // Claim the lock
  // QF-20260911-753: target_session is NOT part of this mechanism's matching contract --
  // findActiveLock() above reads only payload.lock_type/payload.status, never target_session
  // (confirmed the sole documented protocol in lib/worktree-manager.js's own header). The prior
  // 'broadcast' value was vestigial addressing metadata with no functional reader: it was the
  // dominant live contributor to dispatch.cjs's now-retired bare-broadcast sentinel (91 rows,
  // 0 ever acknowledged, all-time). A NULL target_session was tried first but REJECTED: this
  // table's own valid_target CHECK constraint requires target_session IS NOT NULL OR target_sd
  // IS NOT NULL (database/schema-reference-snapshot.json), and this row has no SD context to
  // supply as target_sd either -- NULLing target_session alone would fail every future insert
  // here, silently disabling the whole mutex (caught by adversarial review, not by the existing
  // mocked test suite). A non-null, non-sentinel, non-UUID literal satisfies the constraint
  // without resurrecting the retired sentinel or appearing in any reader's own-session/sentinel
  // match (inbox-readonly.cjs, worker-checkin.cjs).
  const { error } = await supabase.from('session_coordination').insert({
    message_type: 'INFO',
    target_session: 'system:npm-install-lock',
    subject: 'NODE_MODULES_LOCK',
    body: `Session ${sessionId.slice(0, 8)} is running npm install`,
    payload: {
      // SD-LEO-INFRA-LANE-HYGIENE-MACHINE-WRITERS-001 (FR-6): payload-based matching
      // (lock_type/status) is unchanged and remains what findActiveLock() reads; kind
      // is purely additive so the lane-lint gauge stops counting this row as untyped_row.
