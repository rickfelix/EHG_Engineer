...
import { checkDesignSystem, validateCVAPatterns } from './design-system.js';
import { visualVerification } from './visual-verification.js';
import { executeVisualPolishGate, formatGateResult } from './visual-polish-gate.js';
...
class DesignSubAgent {
  ...
  async execute(options = {}) {
    ...
    // 9. CVA Pattern Enforcement (SD-LEO-ORCH-AESTHETIC-DESIGN-SYSTEM-001-B)
    console.log('🧩 Validating CVA pattern compliance...');
    const cvaCheck = await validateCVAPatterns(options.path || './src', fileOptions);
    results.consistency.cvaPatterns = cvaCheck;

    // 10. Visual Polish Gate with AI Slop Detection (SD-LEO-ORCH-AESTHETIC-DESIGN-SYSTEM-001-C)
    if (options.visualPolishGate || options.aiSlopDetection) {
      console.log('✨ Running Visual Polish Gate (AI Slop Detection)...');
      const visualPolishResult = await executeVisualPolishGate({
        content: options.contentToAnalyze || '',
        fileType: options.fileType || 'jsx',
        ventureStage: options.ventureStage || 'build',
        venturePersonality: options.venturePersonality || null,
        analysis: options.visualAnalysis || null
      });
      results.consistency.visualPolishGate = visualPolishResult;

      // Output formatted result
      if (options.verbose) {
        console.log(formatGateResult(visualPolishResult));
      }

      // Hard block if exit code is non-zero
      if (visualPolishResult.exitCode !== 0) {
        console.log('\n❌ VISUAL POLISH GATE: HARD BLOCK');
        console.log(`   ${visualPolishResult.aiSlop.violations.length} AI Slop pattern(s) detected`);
        console.log('   Launch-stage ventures cannot ship with AI Slop patterns');
      }
    }

    // 12. Visual Verification (if enabled via --visual-verify flag)
  ...
  calculateScore(results) {
    ...
    // CVA pattern violations (SD-LEO-ORCH-AESTHETIC-DESIGN-SYSTEM-001-B)
    if (results.consistency && results.consistency.cvaPatterns) {
      const cvaViolations = results.consistency.cvaPatterns.totalViolations || 0;
      // 2 points per CVA violation (arbitrary Tailwind should use variants)
      score -= cvaViolations * 2;
    }

    // Visual Polish Gate (SD-LEO-ORCH-AESTHETIC-DESIGN-SYSTEM-001-C)
    if (results.consistency && results.consistency.visualPolishGate) {
      const vpg = results.consistency.visualPolishGate;
      // Factor in the gate score (0-100)
      const vpgDeduction = Math.max(0, 100 - vpg.score) * 0.15; // 15% weight
      score -= vpgDeduction;

      // Additional deduction for AI Slop violations
      const aiSlopCount = vpg.aiSlop?.violations?.length || 0;
      score -= aiSlopCount * 3; // 3 points per AI Slop violation
    }
  ...
  generateReport(results) {
    ...
    // CVA Pattern Compliance (SD-LEO-ORCH-AESTHETIC-DESIGN-SYSTEM-001-B)
    if (results.consistency && results.consistency.cvaPatterns) {
      const cva = results.consistency.cvaPatterns;
      console.log(`\n🧩 CVA Pattern Compliance: ${cva.status}`);
      console.log(`   Files scanned: ${cva.filesScanned}`);
      console.log(`   Files with CVA: ${cva.filesWithCVA}`);
      if (cva.totalViolations > 0) {
        console.log(`   ❌ Violations: ${cva.totalViolations} arbitrary Tailwind className(s)`);
        cva.violations.slice(0, 3).forEach(v => {
          console.log(`      - ${v.file}: ${v.violations.length} violation(s)`);
        });
        if (cva.suggestions.length > 0) {
          console.log('\n   💡 Migration Suggestions:');
          console.log('      Use CVA variants instead of arbitrary className props.');
          console.log('      Example: <Button variant="primary"> instead of <Button className="bg-blue-500">');
        }
      } else {
        console.log('   ✅ No arbitrary Tailwind className violations');
      }
    }
    ...
    if (results.consistency && results.consistency.visualPolishGate) {
      const vpg = results.consistency.visualPolishGate;
      console.log(`\n✨ Visual Polish Gate: ${vpg.status}`);
      console.log(`   Score: ${vpg.score}/100`);

      // AI Slop detection results
      if (vpg.aiSlop && vpg.aiSlop.violations.length > 0) {
        console.log(`   ❌ AI Slop Detected: ${vpg.aiSlop.violations.length} pattern(s)`);
        vpg.aiSlop.violations.slice(0, 3).forEach(v => {
          console.log(`      - ${v.name} (${v.severity})`);
        });
        console.log(`   Enforcement: ${vpg.aiSlop.enforcement.action}`);
      } else {
        console.log('   ✅ No AI Slop patterns detected');
      }

      // Personality audit results
      if (vpg.personalityAudit) {
        console.log(`   🎨 Personality (${vpg.personalityAudit.personality}): ${vpg.personalityAudit.complianceScore}%`);
      }

      // Critique scores
      if (vpg.critique) {
        console.log('   📐 Impeccable Critique:');
        for (const [_key, dim] of Object.entries(vpg.critique.dimensions)) {
          const icon = dim.score >= 70 ? '✅' : dim.score >= 50 ? '⚠️' : '❌';
          console.log(`      ${icon} ${dim.name}: ${dim.score}/100`);
        }
      }
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const visualPolishGate = process.argv.includes('--visual-polish-gate') || process.argv.includes('--ai-slop');
  const verbose = process.argv.includes('--verbose');
  const urlArg = process.argv.find(arg => arg.startsWith('--url='));
  const previewUrl = urlArg ? urlArg.split('=')[1] : null;
  const stageArg = process.argv.find(arg => arg.startsWith('--stage='));
  const ventureStage = stageArg ? stageArg.split('=')[1] : 'build';
  const personalityArg = process.argv.find(arg => arg.startsWith('--personality='));
  const venturePersonality = personalityArg ? personalityArg.split('=')[1] : null;
