...
class ParallelExecutor {
   ...
   */
  async checkContextHealth(_phase) {
   ...
   */
  async streamResultsToMemory(results, _context) {
  ...
  /**
   * Run sub-agent using the executeSubAgent integration
   */
  async runSubAgent(agent, context) {
    const sdId = context.strategicDirectiveId || context.sdId;
    const agentCode = agent.code?.toUpperCase() || agent.code;

    try {
      const result = await executeSubAgent(agentCode, sdId, {
        prd_id: context.prdId,
        phase: context.phase || 'EXEC',
        priority: agent.priority,
        batch_mode: true
      });

      // Map executeSubAgent result to parallel-executor format
      return {
        success: result.verdict === 'PASS' || result.verdict === 'CONDITIONAL_PASS',
        agent: agent.code,
        verdict: result.verdict,
        confidence: result.confidence || 0,
        message: result.message || `${agent.code} verification completed`,
      ...
      console.error(`   ❌ Sub-agent ${agent.code} execution failed: ${error.message}`);
      return {
        success: false,
        agent: agent.code,
        verdict: 'ERROR',
        ...
        error: error.message,
        requiresManual: true,
        recommendations: ['Review sub-agent logs', 'Check database connectivity', 'Verify sub-agent is registered']
      };
    }
  }
