...
export class VentureStateMachine {
  ...
  async initialize() {
    ...
    const { data: stageWork } = await this.supabase
      ...
      .select('stage_id, stage_status, health_score')
      .eq('venture_id', this.ventureId);

    if (stageWork) {
      for (const sw of stageWork) {
        this.stageStates.set(sw.stage_id, {
  ...
  async _approveHandoff(handoff, ceo_notes) {
    ...
    if (error) {
      ...
      if (error.message.includes('does not exist')) {
        console.log('   ℹ️  fn_advance_venture_stage not found, using direct update');
    ...
    this.stageStates.set(handoff.from_stage, { status: 'completed', health_score: 100 });
  ...
  /**
   * Direct stage advance fallback (if function doesn't exist)
   * @private
   */
  async _directStageAdvance(handoff, ceo_notes) {
    // Update ventures table
    const { error: ventureError } = await this.supabase
      .from('ventures')
      .update({
        current_lifecycle_stage: handoff.to_stage,
        updated_at: new Date().toISOString()
      })
      .eq('id', this.ventureId);

    if (ventureError) {
      throw new Error(`Direct stage advance failed: ${ventureError.message}`);
    }
    ...
    await this.supabase
      ...
      .eq('venture_id', this.ventureId)
      .eq('stage_id', handoff.from_stage);
