...
class IntelligentMultiSelector {
  ...
  detectTaskPattern(prompt, analysis) {
    ...
    for (const [pattern, agents] of Object.entries(this.taskPatterns)) {
   ...
   */
  async applyIntelligentSelection(agents, taskPattern, prompt, context) {
    const selected = [];
  ...
  applyResourceConstraints(agents, context) {
    ...
    const adjustedBudget = this.config.complexity_budget * complexity;
