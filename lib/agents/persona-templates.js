 ...
 * Uses global FORBIDDEN_PERSONAS list by default.
 * For target-application-aware checking, use isForbiddenForApp() from persona-config-provider.js
 *
 * @param {string} persona - The persona to check
 * @param {string[]} [forbiddenList] - Optional custom forbidden list (overrides global)
 * @returns {boolean} True if the persona is forbidden
 */
export function isForbiddenPersona(persona, forbiddenList) {
  const normalized = persona.toLowerCase().trim();
  const list = forbiddenList || FORBIDDEN_PERSONAS;
  return list.some(fp =>
