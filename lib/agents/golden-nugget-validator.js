 ...
 * @param {Object} ventureContext - Optional venture data for DB-level checks (e.g., problem_statement, raw_chairman_intent)
...
export function validateExitGate(gateDescription, artifacts, ventureContext = null) {
  ...
  // Added by: IDEATION-GENESIS-AUDIT - Priority 2: Lock the Vision
  ...
  if (/chairman.*intent.*captured|immutable.*vision|raw.*intent/i.test(gateDescription)) {
    ...
    // Check if raw_chairman_intent is captured
