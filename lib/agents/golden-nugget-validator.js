 ...
 * - Validates artifact CONTENT against lifecycle_stage_config DB requirements
...
import { loadStagesConfig, getStageRequirements, getStagesById } from './modules/golden-nugget-validator/stage-config.js';
 ...
 */
export async function validateGoldenNuggets(stageId, artifacts, { supabase } = {}) {
  console.log(`\n[GoldenNuggetValidator] Validating Golden Nuggets for stage ${stageId}`);

  // SD-LEO-INFRA-EVA-STAGE-PIPELINE-002E: Lazy-init from lifecycle_stage_config DB
  if (getStagesById().size === 0) {
    const loaded = await loadStagesConfig(supabase);
    if (!loaded) {
      console.warn('[GoldenNuggetValidator] Failed to load config from DB — validation will use defaults');
    }
  }
