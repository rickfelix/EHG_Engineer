...
class DesignSubAgent {
  ...
  async execute(options = {}) {
    ...
    if (options.visualVerify) {
      ...
      const visualResults = await this.visualVerification(options.path || './src', options);
  ...
  async checkColorContrast(basePath) {
    ...
    for (const file of cssFiles) {
      ...
      const _colors = this.extractColors(content);
  ...
  async visualVerification(basePath, options = {}) {
    ...
    results.mcp_instructions = this.generateMCPInstructions(previewUrl, options);

    // Step 3: Generate markdown audit template
    results.markdown_summary = this.generateVisualAuditMarkdown(previewUrl, results);
   ...
   */
  generateMCPInstructions(previewUrl, _options = {}) {
   ...
   */
  generateVisualAuditMarkdown(previewUrl, results) {
...
`;
   ...
   */
  async loadGitDiffFiles(_basePath) {
    try {
