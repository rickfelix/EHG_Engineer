 ...
 *
 * SD-LEO-ENH-TARGET-APPLICATION-AWARE-001: Uses persona-config-provider for
 * forbidden persona list instead of hardcoded arrays.
 *
 * @module lib/agents/modules/golden-nugget-validator/design-fidelity
 */

import { APPROVED_PERSONAS } from '../../persona-templates.js';
 ...
 * @param {string} [options.targetApp] - Target application name
 * @param {string} [options.sdType] - SD type
 * @returns {Object} {passed, failures, persona_check, ux_check}
 */
export function checkDesignFidelity(artifact, options = {}) {
  const failures = [];
  ...
  // ===========================================================================
  const approvedPersonas = APPROVED_PERSONAS;
  const forbiddenPersonas = getForbiddenPersonasSync(
