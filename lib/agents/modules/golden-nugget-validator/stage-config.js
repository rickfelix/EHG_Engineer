 ...
 * Loads stage configuration from venture_stages DB table
...
/**
 * Load stage configuration from venture_stages DB table.
 *
...
export async function loadStagesConfig(supabase) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.warn('[GoldenNuggetValidator] venture_stages returned 0 rows');
      return false;
    }
    ...
    console.log(`[GoldenNuggetValidator] Loaded venture_stages: ${STAGES_BY_ID.size} stages indexed`);
    return true;
  } catch (error) {
    console.error(`[GoldenNuggetValidator] Failed to load venture_stages: ${error.message}`);
    return false;
  }
}

/**
 * Get stage requirements from venture_stages.
...
export async function reloadStagesConfig(supabase) {
  console.log('[GoldenNuggetValidator] Reloading venture_stages...');
  STAGES_BY_ID.clear();
  _configLoaded = false;
