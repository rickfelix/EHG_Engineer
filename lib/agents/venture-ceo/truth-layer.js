...
export class TruthLayer {
  ...
  async logPrediction(prediction) {
    ...
    // SD-LEO-ORCH-OPERATING-COMPANY-SPINE-001-A: real write to agent_predictions — a fresh,
    // intentional reintroduction (see database/migrations/20260712_agent_predictions.sql),
    // distinct from the phantom table SD-LEO-INFRA-PHANTOM-TABLE-REFERENCE-001-B removed.
    ...
    if (error) {
      throw new Error(`Prediction insert failed: ${error.message}`);
    }
  ...
  async logOutcome(predictionId, outcome) {
    ...
    // SD-LEO-ORCH-OPERATING-COMPANY-SPINE-001-A: real write to agent_predictions.
    const { data, error } = await this.supabase
      .from('agent_predictions')
      .update({
        status: 'resolved',
    ...
    if (error) {
      throw new Error(`Outcome update failed: ${error.message}`);
    }

    console.log(`   [TRUTH] Outcome logged: ${was_correct ? 'CORRECT' : 'INCORRECT'} for prediction ${predictionId}`);
    return data;
  }
