...
import { checkStageArtifactPrecondition } from '../../eva/stage-artifact-precondition.js';
import { stageWriteTokenField } from '../../eva/stage-write-token-probe.js';
 ...
 *
 * SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-5, RE-SPECIFIED per prospective TESTING B5): this write
 * is strictly forward (completedStage + 1), so a directional guard has zero yield here — the
 * actual defect (stage-advancement-path-census.md row #16) is that this site advances a venture
 * with NO check that the completed stage's required artifacts exist. Reuses
 * checkStageArtifactPrecondition, the same fail-open artifact-existence check
 * lib/eva/stage-execution-worker.js's `_advanceStage` choke-point already enforces for the
 * daemon-walk path, so this ad-hoc CEO-runtime advance path can no longer bypass it.
 * @private
 */
async function _updateVentureProgress(context, completedStage) {
  const { supabase, ventureId } = context;
  try {
    const precondition = await checkStageArtifactPrecondition(supabase, ventureId, completedStage);
    if (precondition.blocked) {
      console.warn(`   Refusing to advance venture ${ventureId} past stage ${completedStage} — missing required artifact(s): ${precondition.missingArtifacts.join(', ')} (source=${precondition.source})`);
      return;
    }
    await supabase
      .from('ventures')
      .update({
        ...
        ...(await stageWriteTokenField(supabase, 'venture-ceo-handlers.js')),
