...
const STANDARD_VENTURE_TEMPLATE = {
  ...
  ceo: {
    ...
    // SD-LEO-INFRA-REMOVE-EVERY-BINDING-001 (chairman ruling deb0818c): no venture AI agent
    // is bound to, owns, or advances a pre-go-live venture workflow stage -- the factory
    // runs the stages, the organization runs the live business. can_advance_stage and
    // requires_advisory_approval (a stage-number list) removed.
  ...
  executives: [
    ...
    },
    {
      ...
      // SD-LEO-INFRA-REMOVE-EVERY-BINDING-001 (chairman ruling deb0818c): stage_ownership
      // removed; the live-business mandate below is re-expressed by launch/live state,
      // not a stage ordinal.
      post_stage_mandate: 'live-iteration once the venture is live: weekly feedback/analytics review → backlog grooming (inbound-ingestion as intake); monthly roadmap iteration; per-release acceptance',
    ...
    },
    {
      ...
      // removed; the live-business mandate below is re-expressed by launch/live state,
      // not a stage ordinal.
      post_stage_mandate: 'keep-live/SRE once the venture is deployed: daily cert/renewal/DNS-drift/deploy-health watch; on-alert remediation (restart/rollback) via factory work orders; weekly dependency/CVE patch; deprovision-on-kill (harvest-before-teardown)',
    ...
    },
    {
      ...
      // removed; the live-business mandate below is re-expressed by launch/live state,
      // not a stage ordinal.
      post_stage_mandate: 'post-launch acquisition/analytics once the venture is live: continuous user-acquisition + weekly KPI/analytics rollups to the evidence fabric (mandate no longer terminates at launch)',
    ...
    },
    {
      ...
      // never bound to a pre-go-live workflow stage (SD-LEO-INFRA-REMOVE-EVERY-BINDING-001,
      // chairman ruling deb0818c).
    ...
    },
    {
      ...
      // audit). Continuous mandate, like VP_MARKETING -- never bound to a pre-go-live
      // workflow stage (SD-LEO-INFRA-REMOVE-EVERY-BINDING-001, chairman ruling deb0818c).
...
/**
 * SD-LEO-INFRA-INSTANTIATEVENTURE-REFUSES-VENTURE-001 (FR-1): thrown by instantiateVenture()
 * when called with a venture id that has no row in the ventures table. Mirrors the shape of
 * lib/creative/creative-brief.js's VentureNotFoundError (a proven precedent for this exact
 * problem in a different domain) -- not imported directly, since the message is domain-specific
 * and the two modules have no other coupling.
 */
export class VentureNotFoundError extends Error {
  constructor(ventureId) {
    super(`Venture ${ventureId || '(missing)'} does not exist — refusing to instantiate an organization for it`);
...
/**
 * SD-LEO-INFRA-INSTANTIATEVENTURE-REFUSES-VENTURE-001 (FR-1): the default venture-existence
 * resolver, injectable via deps.ventureExistsFn so tests never need a real DB round-trip
 * (mirrors creative-brief.js's defaultVentureExists exactly, including its 22P02 handling). A
 * malformed (non-UUID) ventureId is treated as "does not exist" rather than letting Postgres's
 * raw 22P02 invalid_text_representation escape the typed VentureNotFoundError contract. Exported
 * (unlike creative-brief.js's private original) because this SD's own tests call it directly.
 */
export async function defaultVentureExists(supabase, ventureId) {
...
export class VentureFactory {
   ...
   * @param {Object} [deps] - Injectable dependencies (SD-LEO-INFRA-INSTANTIATEVENTURE-REFUSES-VENTURE-001, FR-2)
   * @param {typeof defaultVentureExists} [deps.ventureExistsFn] - venture-existence resolver, for testing
   * @returns {Promise<InstantiationResult>}
   * @throws {VentureNotFoundError} options.ventureId is missing or has no row in the ventures table
   */
  async instantiateVenture(options, deps = {}) {
    const {
      ventureName,
      ventureId,
      templateId = 'standard',
    ...
    // SD-LEO-INFRA-INSTANTIATEVENTURE-REFUSES-VENTURE-001 (FR-2): refuse BEFORE any side effect
    // -- including the console.log calls below and the template lookup -- so a caller never
    // observes a partial instantiation for a venture id that turns out not to exist. Mirrors
    // creative-brief.js's requestCreativeAsset() caller-side short-circuit exactly: the falsy
    // check runs before any DB call, so a missing/undefined/null ventureId never reaches Supabase.
    const ventureExistsFn = deps.ventureExistsFn || defaultVentureExists;
    if (!ventureId || !(await ventureExistsFn(this.supabase, ventureId))) {
...
// VentureNotFoundError and defaultVentureExists are exported directly at their class Type
// declarations above (SD-LEO-INFRA-INSTANTIATEVENTURE-REFUSES-VENTURE-001, FR-1 AC-4).
