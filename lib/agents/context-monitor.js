...
class ContextMonitor {
  constructor(openaiApiKey, projectRoot = process.cwd()) {
    // provider forced to 'openai': analyzeWithAI() below calls the OpenAI-specific
    // chat.completions.create()/functions shape directly, which requires an OpenAI adapter
    // regardless of purpose-based routing. purpose is 'classification' (selecting sub-agents),
    // not 'validation' — the call site previously carried a conflicting raw model override.
    this.openai = getLLMClient({ provider: 'openai', purpose: 'classification' });
