...
export async function fetchManualRevenueTotal(supabase, targetMonth) {
  try {
    ...
    // Bound the read to the target month only -- summarizeManualRevenue() discards everything
    // else anyway, and an unbounded fetch would re-scan the entire venture_revenue_entries table
    // on every hourly feed run as it grows.
    const records = await mod.fetchAndRollup(supabase, { since: `${targetMonth}-01T00:00:00.000Z` });
