...
export async function applyDeductionAttestation({ supabase, periodMonth, livemode = true, attestationId } = {}) {
  ...
  // Require ALL THREE figures present + finite. Coercing a MISSING figure to 0 (num(undefined)=0) while
  // stamping a non-null ref would report unattested=false for a deduction the chairman never supplied —
  // a silent fabricated-zero that defeats the NULL=unattested invariant. A partial attestation is refused.
  const p = decision.brief_data || {};
  const retire = p.solo_401k ?? p.retirement_solo_401k;
  const present = (v) => v != null && Number.isFinite(Number(v));
  if (!present(p.ppo) || !present(retire) || !present(p.se_tax)) {
    console.error('applyDeductionAttestation: refused — incomplete deduction params (ppo, solo_401k, se_tax all required)');
    return null;
  }
  const update = {
    ppo: Number(p.ppo),
    retirement_solo_401k: Number(retire),
    se_tax: Number(p.se_tax),
