  ...
  try {
    ...
    try {
      res = await fetch(`${rdapBase}${encodeURIComponent(domain)}`, {
        // QF-20260912-463: rdap.org's Cloudflare edge 403s undici's default 'node' UA
        // before the request ever reaches the registry RDAP server.
        headers: { accept: 'application/rdap+json', 'user-agent': 'EHG-Engineer venture-domains/1.0 (+https://github.com/rickfelix/EHG_Engineer)' },
