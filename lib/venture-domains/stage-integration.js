...
/**
 * QF-20260912-463: a rung that returns 'unknown' with the SAME rdap_status_* reason for
 * EVERY result of a run is not honest uncertainty -- it's dead (e.g. an edge blocking the
 * request before it reaches the registry). Returns the shared status string (e.g. '403')
 * when detected, else null. Pure, so it's independently testable.
 */
export function detectDeadRdapRung(byCandidate) {
  const results = [...byCandidate.values()].flatMap((a) => a.results || []);
  if (results.length === 0) return null;
  const statuses = new Set(results.map((r) => (r.verdict === 'unknown' ? r.reason : null)));
  if (statuses.size !== 1) return null;
  const [reason] = statuses;
  const m = /^rdap_status_(\d+)$/.exec(reason || '');
  return m ? m[1] : null;
}
...
export async function assessCandidateAvailability(names, checker) {
  ...
  const deadStatus = detectDeadRdapRung(byCandidate);
  if (deadStatus) {
    console.warn(`[venture-domains] every domain-availability result this run was unknown(rdap_status_${deadStatus}) -- the RDAP rung looks dead, not honestly uncertain.`);
  }
  return byCandidate;
}
...
export default {
  ...
  resolveDomainAvailabilityChecker, domainAvailabilityRecordFor, detectDeadRdapRung,
};
