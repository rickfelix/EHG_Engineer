...
/**
 * Parse raw SKILL.md content (frontmatter + body) without touching the filesystem.
 * @param {string} raw - Full SKILL.md content (frontmatter block + body)
 * @param {{ filePath?: string, skillKey?: string }} [opts]
 *   filePath - source path, used only for the basename fallback and returned filePath field
 *   skillKey - fallback key when no filePath is given and frontmatter has no `name`
 * @returns {object|null} Parsed skill object or null if invalid
 */
export function parseSkillContent(raw, { filePath, skillKey } = {}) {
  ...
  const fallbackKey = filePath ? basename(filePath, '.md') : skillKey;

  return {
    skillKey: frontmatter.name || fallbackKey,
