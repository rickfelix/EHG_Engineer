...
/** A skill file name is one path segment: no '/', '\', or '..' -- rejects path traversal
 *  (e.g. '../../some/other/file') before it ever reaches a git pathspec. */
const SAFE_SKILL_FILE_NAME = /^[^/\\]+$/;

/**
 * Repo-relative path for a skill file living in .claude/skills/.
 * @throws {PinnedReadError} if skillFileName is not a single safe path segment
 */
function skillRelPath(skillFileName) {
  if (
    typeof skillFileName !== 'string' ||
    skillFileName === '' ||
    skillFileName === '..' ||
    !SAFE_SKILL_FILE_NAME.test(skillFileName)
  ) {
    throw new PinnedReadError(`skillFileName must be a single safe path segment, got: ${JSON.stringify(skillFileName)}`, {
      code: 'PINNED_READ_BAD_ARG',
    });
  }
