 ...
 *
 * Disposition semantics (FR-2 honest taxonomy, SD-LEO-INFRA-VISION-LADDER-V1-001):
 *   duplicate        — exact/Jaccard/already-materialized match to an existing SD (terminal)
 *   already_covered  — already solved by an existing capability (terminal)
 *   declined         — advisory/research, deliberately not pursued (terminal; dismiss_reason from the closed set)
 *   deferred_to_rung — valuable but committed to a NAMED later rung (terminal; set by a human, needs target_rung)
 *   built            — value LIVE for the active vision (terminal; set by a probe: linked SD completed + capability live)
 *   converted        — NON-terminal in-flight: novel + aligned, promote=true; the drain materializes the SD; a probe later promotes converted->built once the SD completes + the capability is live
 *   null             — un-dispositioned/backlog (incl. an ambiguous item needing human eyes — the RETIRED parking 'deferred')
 */
...
export function classify(item, ctx = {}) {
  ...
  if (extKey && existingSdKeys.has(extKey)) {
    return { ...base, disposition: 'duplicate', triage_verdict: 'already_materialized', dedup_match_sd_key: extKey, dedup_score: 1.0 };
  }

  // (a) Action-type advisory filters (Pool-1). These are deterministic non-SD classes.
  const action = (item.action_type || '').toLowerCase();
  if (action === 'review') {
    return { ...base, disposition: 'declined', triage_verdict: 'advisory_review', dismiss_reason: 'advisory_only' };
  }
  if (action === 'research') {
    return { ...base, disposition: 'declined', triage_verdict: 'research_directive', dismiss_reason: 'research_only' };
  }
  ...
  for (const sd of existingSds) {
    if (sd && normTitle(sd.title) === itemTitleNorm && itemTitleNorm.length > 0) {
      return { ...base, disposition: 'duplicate', triage_verdict: 'exact_duplicate', dedup_match_sd_key: sd.sd_key, dedup_score: 1.0 };
    }
  ...
  if (best.score >= JACCARD_DUP_THRESHOLD) {
    return { ...base, disposition: 'duplicate', triage_verdict: 'jaccard_duplicate', dedup_match_sd_key: best.key, dedup_score: Number(best.score.toFixed(3)) };
  }
  ...
  for (const cap of capabilities) {
    ...
    if (calculateSimilarity(itemKws, capKws) >= CAPABILITY_COVER_THRESHOLD) {
      return { ...base, disposition: 'already_covered', triage_verdict: 'covered_by_capability', dismiss_reason: 'already_covered_by_capability' };
    }
  }

  // (d) CONSERVATIVE guard — a generic/low-signal title that weakly matches many SDs is NOT
  // auto-promoted and NOT mass-classified. FR-2: the parking 'deferred' disposition is RETIRED —
  // an ambiguous item that needs human eyes stays UN-dispositioned (disposition=null), so it
  // correctly remains in the backlog gauge until a human gives it a terminal value.
  if (itemKws.size <= GENERIC_KEYWORD_MAX && weakMatches >= AMBIGUOUS_MIN_WEAK_MATCHES) {
    return { ...base, disposition: null, triage_verdict: 'ambiguous_generic_needs_human', dedup_score: best.key != null ? Number(best.score.toFixed(3)) : null };
  }

  // (e) Novel + aligned -> promote. Disposition 'converted' is a NON-terminal in-flight marker;
  // the drain materializes the SD; a probe later promotes converted->built once that SD completes
  // and the capability is LIVE (an in-flight/draft SD is NOT 'built' — honest-gauge).
