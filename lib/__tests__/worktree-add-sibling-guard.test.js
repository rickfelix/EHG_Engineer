...
// Platform-portable: path.resolve('/repo') resolves to the native absolute-path form on
// whichever OS runs the suite (POSIX '/repo', Windows e.g. 'C:\repo') — a HARDCODED
// Windows-style literal here broke on the Linux CI runner, since path.isAbsolute('C:\\repo\\...')
// is false on POSIX, silently flipping the "absolute in-tree path" test's expected outcome.
