...
// QF-20260911-466: pending_approval is workable by construction when the SD is sitting at
// one of these two pre-LEAD-FINAL phases (it is the normal, live state for an SD one handoff
// from shipped — e.g. SD-LEO-FIX-GOVERNANCE-AUDIT-LOG-001 sits here today). Scoped to exactly
// these two phases so the blanket-WORKABLE escalation risk the 8d7007a2 SECURITY review fixed
// (an unvalidated --restore writing status='completed' without passing LEAD-FINAL-APPROVAL)
// is never reopened for any other phase.
const PRE_FINAL_PHASES = ['LEAD_FINAL', 'PLAN_VERIFICATION'];

/** True when `status` is unconditionally workable, or is pending_approval at a pre-final phase. */
export function isWorkableForUnpark(status, currentPhase) {
  return WORKABLE.includes(status) || (status === 'pending_approval' && PRE_FINAL_PHASES.includes(currentPhase));
}
...
// QF-20260912-346: role seats whose park requires a human/coordinator decision to lift,
// not a worker following sd-start's own printed remedy.
...
/**
 * Pure helper (no DB) — is this hold someone else's to release? QF-20260912-346: a worker
 * following sd-start.js's own printed unpark remedy undid a coordinator's park three times
 * in 25 minutes. "Own" means the SAME session parked it (metadata.stamped_by_session
 * matches), or no session was recorded AND parked_by isn't a role seat (a bare CLI park
 * with no session context at all, e.g. local ad-hoc use) -- a session match always wins
 * over a role-seat name so a role-seat session unparking its own park is never refused.
 * @param {{metadata?:object}} sd
 * @param {{writingSessionId?:string|null}} opts
 * @returns {{refused:boolean, reason:string|null}}
 */
export function checkUnparkHold(sd, { writingSessionId } = {}) {
  const md = sd.metadata || {};
  const parkedBy = typeof md.parked_by === 'string' ? md.parked_by.toLowerCase() : null;
  const stampedSession = md.stamped_by_session || null;

  if (stampedSession) {
    if (stampedSession === writingSessionId) return { refused: false, reason: null };
    return { refused: true, reason: `parked by session ${stampedSession}` };
  }
  if (parkedBy && ROLE_SEAT_PARKERS.includes(parkedBy)) {
    return { refused: true, reason: `parked by role seat "${md.parked_by}"` };
  }
 ...
 * BOTH present AND workable (isWorkableForUnpark — WORKABLE, or pending_approval at
 * LEAD_FINAL/PLAN_VERIFICATION per QF-20260911-466). Any other case (missing, or
 * present-but-not-workable, e.g. an 'unknown' backfill sentinel or pending_approval at any
 * other phase) REQUIRES an explicit restoreStatus — it never silently falls back to 'draft'.
 * @param {{sd_key,status,metadata,current_phase}} sd
 * @param {string} reason
 * @param {string} actor
 * @param {string} [nowIso]
...
export function computeUnparkPlan(sd, reason, actor, nowIso, opts = {}) {
  ...
  // explicit --restore must ALSO be workable. Without this, --restore is an unvalidated
  // escalation path — an operator (or a typo) supplying --restore completed would write
  // status='completed' through the allowlisted sd-park.js writer, firing the full completion
  // cascade (tr_sd_completed_event, auto-close-deliverables, etc.) without ever passing
  // LEAD-FINAL-APPROVAL. FR-4 exists to make unpark SAFER, not to open a new, more-visible
  // escalation path than the fallback it replaces. QF-20260911-466: pending_approval is
  // accepted here too, but ONLY when sd.current_phase is LEAD_FINAL or PLAN_VERIFICATION
  // (isWorkableForUnpark) — any other phase still refuses it exactly as before.
  if (restoreStatus && !isWorkableForUnpark(restoreStatus, sd.current_phase)) {
    const err = new Error(
      `Cannot unpark ${sd.sd_key}: --restore "${restoreStatus}" is not a workable status ` +
      `(current_phase=${sd.current_phase || 'unknown'}). ` +
      `Must be one of: ${WORKABLE.join(', ')}` +
      `${restoreStatus === 'pending_approval' ? `, or "pending_approval" when current_phase is one of: ${PRE_FINAL_PHASES.join(', ')}` : ''}.`
    );
    err.code = 'UNPARK_RESTORE_STATUS_INVALID';
    throw err;
  }

  if (!restoreStatus && (isInferred || !isWorkableForUnpark(from, sd.current_phase))) {
    const err = new Error(
      `Cannot unpark ${sd.sd_key}: metadata.parked_from_status is ` +
      `${isInferred ? `"${from}" (INFERRED, source="${md.parked_from_status_source}", not a recorded fact)` : from ? `"${from}" (not a workable status at current_phase=${sd.current_phase || 'unknown'})` : 'missing'}. ` +
 ...
 */
export async function unpark(client, sdKey, { reason, actor = 'cli', restoreStatus, writingSessionId, force = false } = {}) {
  // QF-20260911-466: current_phase is needed so computeUnparkPlan can allow pending_approval
  // at LEAD_FINAL/PLAN_VERIFICATION without widening WORKABLE for every other phase.
  const { rows } = await client.query(
    'SELECT sd_key, status, current_phase, metadata FROM strategic_directives_v2 WHERE sd_key=$1',
    [sdKey]
  );
  if (!rows.length) throw new Error(`SD not found: ${sdKey}`);
  const sd = rows[0];

  // QF-20260912-346: refuse to lift a hold set by a different actor unless explicitly forced.
  if (!force) {
    const holdCheck = checkUnparkHold(sd, { writingSessionId });
    if (holdCheck.refused) {
      const err = new Error(
        `Cannot unpark ${sd.sd_key}: ${holdCheck.reason}, not this caller. ` +
        'Ask the coordinator, or pass --force (with --reason explaining why) to override.'
      );
      err.code = 'UNPARK_HOLD_NOT_YOURS';
      throw err;
    }
  }
