...
function validateRequirement(requirement, sd, options = {}) {
  ...
  // Support both 'title' and 'requirement' field names — but they are NOT synonyms:
  // generators emit `title` as a short label and `requirement` as the rich body. The old
  // `title || requirement` collapse dropped the body whenever a title was present, scoring
  // the FR on the label alone (~1-2% Jaccard → false QUALITY GATE FAILED; witnessed on
  // SD-LEO-INFRA-ADAM-EVA-SEAM-001, flag e8008b14). reqTitle keeps the DISPLAY semantics
  // (returned as requirement_title); reqText scores ALL distinct text the author wrote
  // (title + requirement + description — trimmed, deduped, stable order) so the author's
  // field-name choice cannot change the grounding verdict. SD-FDBK-FIX-FIX-PRD-GROUNDING-001.
  const reqTitle = requirement.title || requirement.requirement || '';
  const reqParts = [requirement.title, requirement.requirement, requirement.description]
    .map((s) => (typeof s === 'string' ? s.trim() : ''))
    .filter(Boolean);
  const reqText = [...new Set(reqParts)].join(' ');
  ...
  if (explorationSummary) {
    const exploreText = typeof explorationSummary === 'string'
      ? explorationSummary
      : JSON.stringify(explorationSummary);
    const exploreSimilarity = calculateTextSimilarity(reqText, exploreText);
