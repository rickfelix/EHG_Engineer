 ...
 * @param {string} options.source_type - Type of error source (default: 'error_capture'). As of
 *   SD-FDBK-FIX-SECURITY-ISUNTRUSTEDORIGIN-OMITS-001, 'error_capture' is classified untrusted-origin
 *   by lib/factory/content-sanitizer.js's isUntrustedOrigin() (it is also anon-writable via the
 *   record_venture_error RPC), so downstream consumers now sanitize this text before it reaches an
 *   LLM prompt or an SD description. A legitimate internal/harness caller that specifically needs
 *   its triage text left unsanitized (e.g. to preserve full stack-trace fidelity) can pass
 *   options.source_type='auto_capture' instead -- an already-trusted value with no code changes
 *   required.
