...
// this function already distinguishes the staged, not-yet-applied manual_revenue_usd columns
// (PGRST204/42703, degraded below by retrying without them) from a real upsert error (thrown
// below). The factory's default throw-on-42703 would fire before this code ever saw `error`.
export async function upsertSubstrateInputs(periodMonth, fields, supabase = createSupabaseServiceClient({ throwOnSchemaDrift: false }), nowIso = new Date().toISOString()) {
  const pm = periodMonth || periodMonthOf(Date.now());
  ...
  // SD-EHG-PRODUCT-FIRSTREV-SUBSTRATE-ROLLUP-001-B: honest breakout of the manual
  // (venture_revenue_entries) portion of revenue_usd, additive to the Stripe component the
  // caller already folded in above -- never a replacement for revenue_usd on its own.
  if ('manual_revenue_usd' in fields) {
    row.manual_revenue_usd = fields.manual_revenue_usd;
    row.manual_revenue_last_synced_at = nowIso;
  }
  const { data, error } = await supabase
    .from('operator_cash_burn_monthly')
    ...
    .single();
  if (error) {
    // SD-EHG-PRODUCT-FIRSTREV-SUBSTRATE-ROLLUP-001-B: manual_revenue_usd/manual_revenue_last_synced_at
    // are STAGED (not-yet-applied) columns. Until the chairman applies that migration, PostgREST
    // rejects the whole upsert with a schema-cache "column not found" error (PGRST204 / 42703)
    // whenever those keys are present -- which would otherwise break the ALREADY-LIVE revenue_usd
    // write too, not just the new manual component. Fail soft for just the optional columns.
    const mentionsManualRevenue = 'manual_revenue_usd' in row && /manual_revenue/i.test(error.message || '');
    const isSchemaCacheMiss = error.code === 'PGRST204' || error.code === '42703';
    if (mentionsManualRevenue && isSchemaCacheMiss) {
      console.warn(`[cash-burn-substrate] manual_revenue_usd/manual_revenue_last_synced_at columns not found (staged migration not yet chairman-applied) -- retrying upsert without them for ${pm}`);
      const { manual_revenue_usd, manual_revenue_last_synced_at, ...rowWithoutManualRevenue } = row;
      const retry = await supabase
        .from('operator_cash_burn_monthly')
        .upsert(rowWithoutManualRevenue, { onConflict: 'period_month' })
        .select()
        .single();
      if (retry.error) throw new Error(`operator_cash_burn_monthly upsert failed: ${retry.error.message}`);
      return retry.data;
    }
    throw new Error(`operator_cash_burn_monthly upsert failed: ${error.message}`);
  }
  return data;
}
