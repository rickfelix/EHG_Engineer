 ...
 *
 * SD-ALTIFYAI-LEO-ORCH-SPRINT-2026-001-G: readWorkersUsage REQUIRES a scriptName. The
 * underlying dataset (workersInvocationsAdaptive) is scoped to CLOUDFLARE_ACCOUNT_ID, one
 * account shared across every venture's Worker. Without a per-script filter, the query
 * returns the account's TOTAL usage — every venture fed from this adapter would receive
 * the identical account-wide number, a fabricated per-venture measurement (worse than the
 * honest-unattested state this substrate exists to guarantee). This was latent until a
 * second venture was ever wired in; closing it here, at the adapter, means a future caller
 * cannot reintroduce the bug by simply omitting the argument.
 *
...
export function createCloudflareCostAdapter(env = process.env, { fetchImpl } = {}) {
  ...
  return {
    /**
     * Workers usage counts (requests, errors, subrequests) for ONE venture's Worker
     * (scriptName-scoped) over a date range. Returns the raw GraphQL data shape — the
     * caller converts to a dollar estimate via published unit pricing. Never returns a
     * cost figure itself. scriptName is REQUIRED — see module docblock.
     */
    readWorkersUsage: (startDate, endDate, scriptName) => {
      if (!scriptName) {
        throw new Error(
          'readWorkersUsage requires scriptName — an account-wide query would misattribute '
          + 'every venture sharing this Cloudflare account to the identical usage total '
          + '(SD-ALTIFYAI-LEO-ORCH-SPRINT-2026-001-G)',
        );
      }
      return graphql(
        `query WorkersUsage($accountTag: String!, $start: Date!, $end: Date!, $scriptName: String!) {
          viewer {
            accounts(filter: { accountTag: $accountTag }) {
              workersInvocationsAdaptive(
                limit: 1000
                filter: { date_geq: $start, date_leq: $end, scriptName: $scriptName }
              ) {
        ...
        { accountTag: accountId, start: startDate, end: endDate, scriptName },
      );
    },
