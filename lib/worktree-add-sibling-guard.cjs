'use strict';
// @wire-check-exempt: required lazily inside scripts/hooks/pre-tool-enforce.cjs's
// ENFORCEMENT 12e block (`require('../../lib/worktree-add-sibling-guard.cjs')`), gated
// behind `if (extractTargetPath(...))` so the static wire-check cannot trace the call site
// (mirrors the already-live lib/worktree-remove-junction-guard.cjs pattern in the same file).
/**
