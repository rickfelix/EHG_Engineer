  ...
  return async function fallbackExecutor(page, persona, ctx) {
    ...
    if (codeChallenge) {
      ...
      if (authProviderTestMode === 'clerk_development') {
        // STEP 1 of QF-20260902-512: this is the documented-but-not-yet-locally-verified
        // Clerk test-mode convention -- fill the fixed code, never poll IMAP for a dev
        // instance that (per the fenced-mailbox census) has never delivered a Clerk email.
