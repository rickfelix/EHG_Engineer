...
// QF-20260906-282: stp-e3e6 has NEVER succeeded across 5 historical uat_test_results rows
// with several distinct failure modes, and the walker's fixed 1x1 transparent-pixel fixture
// (degenerate: 1x1, uniform/transparent pixels) cannot distinguish a degenerate-input
// artifact from a genuine model/binding defect on the venture's alt-text provider -- a real,
// small, non-uniform image separates the two. Checked in rather than generated inline so its
// bytes are reviewable and stable across runs.
const ALT_TEXT_FIXTURE_PATH = fileURLToPath(new URL('./fixtures/alt-text-sample.png', import.meta.url));
...
async function altifyaiGenerateAltText(page, persona, ctx, stepLabel) {
  ...
  const tmpPng = path.join(os.tmpdir(), `altifyai-${stepLabel}-fixture-${Date.now()}.png`);
  fs.copyFileSync(ALT_TEXT_FIXTURE_PATH, tmpPng);

  // QF-20260906-282: capture the provider response class (status, elapsed ms, error body
  // prefix) for the backend alt-text call, so a bare TIMEOUT/500 result carries WHICH layer
  // failed instead of nothing -- mirrors the network-capture pattern already established in
  // scripts/one-off/diagnose-altifyai-generation-hang-033.mjs. Never fabricated: null unless a
  // matching response was actually observed.
  let providerResponseClass = null;
  const requestObservedAt = Date.now();
  const onAltTextResponse = async (res) => {
    if (!/\/api\/alt-text/i.test(res.url())) return;
    const elapsedMs = Date.now() - requestObservedAt;
    const errorBodyPrefix = res.status() >= 400
      ? (await res.text().catch(() => '')).slice(0, 200)
      : null;
    providerResponseClass = { status: res.status(), elapsedMs, errorBodyPrefix };
  };
  page.on('response', onAltTextResponse);
  ...
  try {
    try {
      ...
      if (!statusSuccess) {
        const err = new Error(`${stepLabel}: no [data-testid="status-success"] within 15s of upload -- automatic-generation trigger did not fire`);
        err.providerResponseClass = providerResponseClass;
        throw err;
      }
    } finally {
    ...
    while (Date.now() < waitDeadline) {
      ...
      if (displayVisible && !loadingVisible) {
        ...
        return { resolved: true, isError: ALT_TEXT_ERROR_PATTERN.test(displayText), displayText, url: page.url(), providerResponseClass };
      }
      await new Promise((resolve) => setTimeout(resolve, 5000));
    }
    return { resolved: false, isError: false, displayText: null, url: page.url(), providerResponseClass };
  } finally {
    page.off('response', onAltTextResponse);
  }
}

async function altifyaiAutoGenerateStepOverride(page, persona, ctx) {
  const { resolved, isError, displayText, url, providerResponseClass } = await altifyaiGenerateAltText(page, persona, ctx, 'stp-e3e6');
  ...
  const err = new Error(`stp-e3e6: GENERATION_DID_NOT_RESOLVE -- ${resolved ? `backend surfaced an error state: "${displayText}"` : `still state-loading after ${ALT_TEXT_GENERATION_WAIT_MS / 1000}s, no error surfaced`} (measured product defect, not a walker gap)`);
  // QF-20260906-282: same never-fabricated, absent-unless-observed pass-through as
  // authForensics/clickedButtonText -- lets the run row carry WHICH layer failed.
  err.providerResponseClass = providerResponseClass;
  throw err;
}
