 ...
 */
export function getTestCredential(ventureKey) {
  const raw = process.env[`VENTURE_UAT_TEST_ACCOUNT_${(ventureKey || '').toUpperCase()}`];
  if (!raw) return null;
  try {
...
export function buildStepExecutor(step, ventureKey) {
  ...
  return async function fallbackExecutor(page, _persona, ctx) {
    ...
    const credential = getTestCredential(ventureKey);
    if (!credential) {
      throw new Error(`auth required for step "${step.goal || step.step_id}" -- no test credential configured (VENTURE_UAT_TEST_ACCOUNT_${ventureKey.toUpperCase()} unset)`);
    }
    await page.goto(`${ctx.baseUrl}/register`);
    const signInToggle = page.locator('text=Already have an account? Sign in');
    if (await signInToggle.count() > 0) await signInToggle.click();
