  ...
  preflightChecks: [
    {
      ...
      name: 'land',
      async run(page, ctx) {
        const selector = 'text=Start free';
        ...
        const rendered = await page.locator(selector)
          ...
          .catch(() => false);
        if (!rendered) {
          throw new Error(`land: no "Start free" call-to-action rendered (waited 10s, selector: ${selector})`);
        }
        return { url: ctx.baseUrl, renderedStateSummary: 'landing page rendered with Start free CTA', matchedSelector: selector, ...PREFLIGHT_VERIFICATION };
      },
    },
    {
