...
// ever reaching the ~121s POST /api/alt-text call this comment describes. Independently
// re-confirmed via diagnose-altifyai-generation-hang-033.mjs's network capture that the
// underlying flow still resolves to the SAME HTTP 500 once upload succeeds -- i.e. this is the
// SAME pre-existing, out-of-scope venture-side defect with worsened upload-latency, not a
// different failure. Left as a disclosed correction rather than rewriting the paragraph above,
// which remains an accurate record of what was true when QF-20260902-033 diagnosed it.
...
function buildExportAssertion(testId, label) {
  return async (page, stepLabel) => {
    ...
    return { url: page.url(), renderedStateSummary: `${label} export triggered, download suggestedFilename="${filename}"`, matchedSelector: `[data-testid="${testId}"]`, stepOverrideUsed: true };
  };
}
...
async function altifyaiCopyAltTextStepOverride(page, persona, ctx) {
  ...
  await page.context().grantPermissions(['clipboard-write', 'clipboard-read'], { origin: new URL(url).origin });
