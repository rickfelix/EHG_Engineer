  ...
  preflightChecks: [
    {
      ...
      async run(page, ctx) {
        ...
        const rendered = await page.locator('text=Start free')
          .waitFor({ state: 'visible', timeout: 10000 })
          .then(() => true)
          .catch(() => false);
        if (!rendered) {
          throw new Error('land: no "Start free" call-to-action rendered (waited 10s)');
        }
        return { url: ctx.baseUrl, renderedStateSummary: 'landing page rendered with Start free CTA', ...PREFLIGHT_VERIFICATION };
      },
    },
    {
      ...
      async run(page, ctx) {
        ...
        await page.goto(url);
        const rendered = await page.locator('input[name="emailAddress"], input[type="email"], input[name="identifier"]')
          .waitFor({ state: 'visible', timeout: 10000 })
          .then(() => true)
          .catch(() => false);
        if (!rendered) {
          throw new Error('signupFormRenders: no email/identifier field rendered on /register (waited 10s)');
        }
        return { url, renderedStateSummary: 'Clerk sign-up/sign-in form rendered', ...PREFLIGHT_VERIFICATION };
      },
    },
    {
      ...
      async run(page, ctx) {
        ...
        return { url, renderedStateSummary: `upload route reachable (status=${status})`, ...PREFLIGHT_VERIFICATION };
      },
    },
    {
      name: 'feedbackWidget',
