  ...
  preflightChecks: [
    {
      ...
      //
      // QF-20260901-455: even a 10s waitFor still raced a COLD Cloudflare Workers start
      // (witnessed live: land RED while the other two re-sourced checks passed). A comma-joined
      // 'text=X, #Y' locator was tried and measured NOT to behave as an OR across engines here
      // (verified live: the union wait timed out at 30s even though both markers existed
      // moments later) -- so this races two INDEPENDENT waitFor() calls instead, the same
      // Promise.race idiom this file already uses below for the code-challenge/authenticated-URL
      // race. '#root:not(:empty)' is a hydration-safe structural marker (fires the instant React
      // mounts ANY content, measured live at ~194ms on a warm start -- far earlier and less
      // copy-dependent than the specific CTA text) matched alongside the CTA text itself; timeout
      // raised from 10s to 30s per the ticket's own ask.
      name: 'land',
      async run(page, ctx) {
        const HYDRATION_TIMEOUT_MS = 30000;
        const ctaSelector = 'text=Start free';
        const rootSelector = '#root:not(:empty)';
        await page.goto(ctx.baseUrl);
        // Promise.any: resolves the instant EITHER waitFor fulfills (whichever renders first),
        // only rejecting (AggregateError) once BOTH have exhausted their own timeout -- unlike
        // Promise.all/race, a fast true never waits out a slow false, and a fast false never
        // wins over a true that arrives later within the shared window.
        const matchedSelector = await Promise.any([
          page.locator(ctaSelector).waitFor({ state: 'visible', timeout: HYDRATION_TIMEOUT_MS }).then(() => ctaSelector),
          page.locator(rootSelector).waitFor({ state: 'visible', timeout: HYDRATION_TIMEOUT_MS }).then(() => rootSelector),
        ]).catch(() => null);
        if (!matchedSelector) {
          throw new Error(`land: neither "${ctaSelector}" nor "${rootSelector}" rendered (waited ${HYDRATION_TIMEOUT_MS / 1000}s)`);
        }
        return { url: ctx.baseUrl, renderedStateSummary: `landing page rendered (matched ${matchedSelector})`, matchedSelector, ...PREFLIGHT_VERIFICATION };
      },
    },
    {
      ...
      async run(page, ctx) {
        const selector = 'input[name="emailAddress"], input[type="email"], input[name="identifier"]';
        const url = `${ctx.baseUrl}/register`;
        await page.goto(url);
        const rendered = await page.locator(selector)
          .waitFor({ state: 'visible', timeout: 10000 })
          .then(() => true)
          .catch(() => false);
        if (!rendered) {
          throw new Error(`signupFormRenders: no email/identifier field rendered on /register (waited 10s, selector: ${selector})`);
        }
        return { url, renderedStateSummary: 'Clerk sign-up/sign-in form rendered', matchedSelector: selector, ...PREFLIGHT_VERIFICATION };
      },
    },
    {
