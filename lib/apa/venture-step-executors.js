...
/**
 * Bounded poll for an authenticated-state URL signal: the page's URL path no longer looks
 * like a sign-in/login screen. Polls page.url() directly (matching this file's existing
 * synchronous page.url() usage) rather than page.waitForURL(), since not every navigation
 * driver in this codebase's test/execution paths implements that Playwright-specific API.
...
async function pollForAuthenticatedUrl(page, { timeoutMs = 15000, pollIntervalMs = 250 } = {}) {
  ...
  const isAuthedUrl = () => {
    try {
      return !NOT_YET_AUTHENTICATED_PATH_RE.test(new URL(page.url()).pathname);
    } catch {
      return false;
    }
  };
