...
export function buildStepExecutor(step, ventureKey) {
  ...
  return async function fallbackExecutor(page, persona, ctx) {
    ...
    // SECURITY finding SEC-001 (HIGH): page.locator(...).count() does NOT auto-wait -- it is
    // an instant snapshot, unlike page.fill()'s auto-waiting behavior. On a client-side-mounted
    // auth widget (measured: this toggle can take ~900ms to mount), count() reads 0 before the
    // toggle exists, the click is skipped, and fill()+click() below then submit real credentials
    // to the ACTUAL REGISTRATION FORM on /register -- a measured, reproducible violation of this
    // SD's hard PRD constraint ("never create a real account... never ad-hoc registration").
    // waitFor() genuinely waits (bounded by timeout) instead of taking an instant snapshot, so a
    // slow-to-mount toggle is correctly detected rather than raced past.
    const signInToggle = page.locator('text=Already have an account? Sign in');
    const toggleVisible = await signInToggle
      .waitFor({ state: 'visible', timeout: 10000 })
      .then(() => true)
      .catch(() => false);
    if (toggleVisible) await signInToggle.click();
