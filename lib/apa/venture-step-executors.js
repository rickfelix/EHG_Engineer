...
async function altifyaiCopyAltTextStepOverride(page, persona, ctx) {
  ...
  await copyBtn.click();
  // Asserts the button's own post-click UI state transition, never clipboard content
  // (PLAN-phase redesign: altifyai's handleCopy swallows a clipboard-permission rejection in a
  // bare catch{} -- a missing grant in headless Chromium is indistinguishable from a broken
  // product if the assertion targets the clipboard itself).
