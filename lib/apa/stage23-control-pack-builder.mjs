...
async function pullStepConclusion({ repo, workflowFile, stepName, githubToken, fetchImpl }) {
  ...
  // A workflow step with no explicit `name:` (altifyai's ci.yml `- run: npm test` is exactly
  // this shape) is rendered by GitHub Actions as "Run <command>" in the Jobs API -- confirmed
  // live against a real completed run (34727450324), where the naive `stepName` match found
  // nothing despite the step existing and having actually run. Match either spelling so this
  // keeps working if the workflow ever gains an explicit `name: npm test` too.
  for (const job of jobsBody.jobs || []) {
    const step = (job.steps || []).find((s) => s.name === stepName || s.name === `Run ${stepName}`);
    if (step) {
