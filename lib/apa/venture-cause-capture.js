 ...
 * `key=`/`token=`/`secret=`/`password=` query-string or JSON values (INCLUDING compound
 * env-var names like `SUPABASE_SERVICE_ROLE_KEY=` — SECURITY finding SEC-1, EXEC phase:
 * a leading `\b` word-boundary anchor does not fire between `_` and a keyword, since `_`
 * is itself a word character, so `\bkey=` alone never matched a real-world
 * `SOMETHING_KEY=` env dump), JWTs (three dot-separated base64URL segments, matched as
 * their own pattern BEFORE the generic base64 fallback below — SECURITY finding SEC-2: the
 * standard-alphabet-only fallback fragments a base64URL JWT at every `-`/`_`, letting
 * sub-32-char pieces escape), long hex/base64(URL) runs (>=32 chars) that are far more
 * likely to be a credential or session id than diagnostic text, and email addresses. Then
 * collapse whitespace and truncate. Applied to EVERY item this module stores, regardless
 * of source (TR-2 — no leg is exempt).
 * @param {string} text
 * @returns {string}
 */
export function redactText(text) {
  ...
  let out = text
    ...
    // SECURITY finding SEC-6 (EXEC-TO-PLAN review, evidence 6d46d0ee): HTTP Basic auth
    // ("Authorization: Basic <base64>") has neither a credential keyword nor (often) a
    // >=32-char base64 blob -- real Basic-auth values are frequently short -- so it escaped
    // every other rule here. Same space-separated shape as the Bearer case above.
    .replace(/\bbasic\s+\S+/gi, 'basic=[REDACTED]')
    // Matches the WHOLE `<name>=<value>` pair whenever <name> ends in a credential-shaped
    // keyword, e.g. `SUPABASE_SERVICE_ROLE_KEY=...`, `DB_PASSWORD=...`, `api_key=...`,
    // `token: ...` -- the leading `[\w.-]*` (no `\b` anchor) is what lets a compound,
    // underscore-joined env-var name match; the bare `key` alternative is deliberately
    // broad (over-redaction is the safe failure mode for a credential scrubber).
    .replace(/[\w.-]*(?:api[_-]?key|apikey|token|secret|password|passwd|pwd|key)\s*[:=]\s*\S+/gi, '[REDACTED]')
    // JWT: eyJ... . ... . ... (base64URL, three dot-separated segments). Matched BEFORE the
    // generic base64 fallback below so a JWT is redacted as one unit rather than fragmenting
    // at its own `-`/`_` characters into sub-threshold pieces that survive.
    .replace(/eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/g, '[REDACTED-JWT]')
    // Standard AND base64URL alphabets (+/  and  -_), so a non-JWT base64URL secret
    // (e.g. a raw session id) is still caught even without the JWT's dot structure.
...
const OBSERVABILITY_DATASET = 'cloudflare-workers';
const OBSERVABILITY_KEY_SERVICE = '$metadata.service';

/**
 * Extract the event array from a telemetry query response. Mirrors altifyai's own
 * `scripts/query-observability-logs.mjs` `extractEvents()` (QF-20260819-687, live-verified
 * 2026-08-22 against real account data): `result.events.events` is authoritative WHENEVER
 * present, even when empty — `result.events.series` is a same-shaped chart array that a
 * naive "largest array of objects" walk mistakes for events precisely when the real array
 * is empty. The fallback walk exists only for a response missing the canonical path.
 * @param {object} payload
 * @returns {Array}
 */
function extractObservabilityEvents(payload) {
  ...
  const walk = (node, depth) => {
    ...
    if (Array.isArray(node)) {
      if (node.length && typeof node[0] === 'object' && node[0] !== null) arrays.push(node);
      return;
    }
    for (const value of Object.values(node)) walk(value, depth + 1);
  };
  walk(payload?.result ?? payload, 0);
  return arrays.sort((a, b) => b.length - a.length)[0] ?? [];
}

/**
 * Cloudflare Workers Observability telemetry query API — real, credentialed HTTP call.
 * Request/response shape matches altifyai's OWN live-verified reader
 * (`scripts/query-observability-logs.mjs`, QF-20260819-687) byte-for-byte — same endpoint,
 * same `queryId`/`view`/`parameters.datasets`/`parameters.filters` body shape, same epoch-
 * MILLISECONDS timeframe (an ISO string or epoch-seconds silently returns an empty result
 * set rather than an error, per that script's own measured incident). Reusing the proven
 * shape here rather than re-deriving it is deliberate: this module has no live account to
 * verify against, and that script does.
 *
 ...
 *   window.from/to are ISO strings (this module's own contract); converted to epoch ms here.
  ...
  try {
    const res = await f(
      `${CLOUDFLARE_API_BASE}/accounts/${accountId}/workers/observability/telemetry/query`,
      {
        ...
        body: JSON.stringify({
          queryId: 'venture-troubleshooting-fr1-adhoc',
          view: 'events',
          limit: MAX_ITEMS_PER_LEG,
          timeframe: { from: Date.parse(window.from), to: Date.parse(window.to) },
          parameters: {
            datasets: [OBSERVABILITY_DATASET],
            filters: [{ key: OBSERVABILITY_KEY_SERVICE, operation: 'eq', type: 'string', value: workerName }],
          },
        }),
      },
    );
    ...
    const events = extractObservabilityEvents(json);
    const rawItems = events.map((e) => {
      const message = e?.message ?? e?.['$metadata']?.message ?? e?.['$metadata']?.error ?? '';
      const level = e?.['$metadata']?.level ?? '';
      return JSON.stringify({ level, message: typeof message === 'string' ? message : JSON.stringify(message) });
    });
...
export async function queryVentureD1({ databaseName, table = 'generated_alt_texts', window, env = process.env, fetchImpl }) {
  ...
  // interpolated into the SQL text itself (D1's parameterized `?` placeholders only bind
  // VALUES, not identifiers) -- every caller today passes the hardcoded default, but an
  // identifier allowlist here is cheap insurance against a future caller ever passing an
  // untrusted value, rather than leaving the injection-shaped surface open until one does.
