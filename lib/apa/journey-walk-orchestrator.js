 ...
 *   deps.canaryStep = {journeyId, run: async ({baseUrl, page}) => outcome} (SD-LEO-FIX-
 *   ALTIFYAI-STAGE-WALK-001): when supplied, this module runs it AFTER the main walk (outside
 *   journeySteps/recordResult/pass_rate) and merges {canaryJourneyId, journeyResults} into the
 *   auto-derived control-pack evidence — see lib/apa/altifyai-canary-step.mjs. Absent by
 *   default (canary_mutation_control stays not_attempted, matching prior behavior).
...
export async function runVentureJourneyWalk({ sdId, ventureId = null, stageNumber = null, ventureKey, baseUrl, journeySteps, persona = {}, workerName = null, databaseName = null, deps = {} }) {
  ...
  try {
    ...
    // SD-LEO-FIX-ALTIFYAI-STAGE-WALK-001: MERGE, not replace. The minimum-assertion-manifest
    // fields (manifest/executedJourneys/evidenceManifest) can only be derived AFTER the walk
    // itself has run (they depend on walkResult.outcomes, computed above) -- a caller supplying
    // deps.controlPackEvidence for the OTHER controls (live_deployment_binding, fence,
    // canary waiver) has no way to pre-compute these before calling this function. The prior `||`
    // full-object fallback silently dropped this control to not_attempted for any caller that
    // ever supplied a partial pack. Caller-supplied fields still win (e.g. a caller's own,
    // venture-accurate deploymentSha overrides this module's `git rev-parse HEAD`, which is
    // EHG_Engineer's own commit, not the walked venture's). No prior caller ever supplied
    // deps.controlPackEvidence at all, so this is purely additive -- zero behavior change for
    // every existing default-evidence run.
    const defaultControlPackEvidence = {
    ...
    // SD-LEO-FIX-ALTIFYAI-STAGE-WALK-001 (coordinator ruling, superseding an earlier
    // waiver-based plan): canary_mutation_control is a mutation test of THIS WALKER, not the
    // venture's product surface, so it needs no seeded venture journey and no waiver. Like
    // manifest/executedJourneys above, its journeyResults can only be derived AFTER the walk
    // runs (they include every real step's PASS/FAIL) -- deps.canaryStep lets a caller supply
    // the deliberately-failing probe (see lib/apa/altifyai-canary-step.mjs) without this shared
    // module inventing or hardcoding one. Run OUTSIDE runJourneyWalk's own loop (never inside
    // journeySteps) so its guaranteed FAIL can never trip browser-executor's stop-at-first-
    // failure behavior or count against pass_rate/completedAllSteps -- it is merged only into
    // controlPackEvidence.journeyResults, never into recordResult/uat_test_results. Absent when
    // deps.canaryStep is omitted (every existing caller today), so this is purely additive.
    let canaryControlPackFields = {};
    if (deps.canaryStep) {
      const canaryOutcome = await deps.canaryStep.run({ baseUrl, page });
      canaryControlPackFields = {
        canaryJourneyId: deps.canaryStep.journeyId,
        journeyResults: [
          ...walkResult.outcomes.map((o) => ({ journeyId: o.step, status: o.success ? 'PASS' : 'FAIL' })),
          { journeyId: deps.canaryStep.journeyId, status: canaryOutcome.status },
        ],
      };
    }

    const controlPackEvidence = deps.controlPackEvidence
      ? { ...defaultControlPackEvidence, ...canaryControlPackFields, ...deps.controlPackEvidence }
      : { ...defaultControlPackEvidence, ...canaryControlPackFields };
