 ...
 * @returns {Promise<{step: string, url: string|null, renderedStateSummary: string|null, success: boolean, failureReason: string|null, ctxUpdates: object, clickedButtonText: string|null}>}
 */
export async function executeJourneyStep(page, step, stepExecutors, persona, ctx) {
  ...
  try {
    ...
    return { step, url, renderedStateSummary, success: true, failureReason: null, ctxUpdates, clickedButtonText: null, authForensics: null, providerResponseClass: null };
  } catch (err) {
    ...
    // QF-20260902-206: a step executor may attach clickedButtonText to a thrown Error (the
    // way a preflight return value carries matchedSelector) so the run row shows which button
    // was pressed even on a failure path -- never fabricated, absent unless the executor set it.
    const clickedButtonText = err instanceof Error && err.clickedButtonText !== undefined ? err.clickedButtonText : null;
    // QF-20260906-282: same never-fabricated pass-through for the backend provider response
    // class (status/elapsedMs/errorBodyPrefix), so a bare TIMEOUT/FAIL result carries WHICH
    // layer failed instead of nothing.
    const providerResponseClass = err instanceof Error && err.providerResponseClass !== undefined ? err.providerResponseClass : null;
    // QF-20260902-512: same never-fabricated, absent-unless-set pass-through as
    // clickedButtonText, for the auth-challenge forensics the run row needs.
    const authForensics = err instanceof Error
      ? {
          challengeKind: err.challengeKind ?? null,
          retrievalPath: err.retrievalPath ?? null,
          authMode: err.authMode ?? null,
          mailboxCensus: err.mailboxCensus ?? null,
          // QF-20260902-614: which leg authenticated -- a sign_up recovery is PRODUCT-FLOW
          // evidence only, never a sign-in proof (Solomon gauge-honesty clause).
          authBranch: err.authBranch ?? null,
          // QF-20260902-093: which sign-up fields were actually filled -- absent unless the
          // sign-up branch ran, same never-fabricated pass-through as the rest of this object.
          signupFieldsFilled: err.signupFieldsFilled ?? null,
          // QF-20260902-952: which branch fired at the code step (auto_submit | clicked:<name>
          // | neither), and the redacted before/after button census -- same never-fabricated
          // pass-through as the rest of this object.
          authSubmitBranch: err.authSubmitBranch ?? null,
          codeStepSnapshot: err.codeStepSnapshot ?? null,
        }
      : { challengeKind: null, retrievalPath: null, authMode: null, mailboxCensus: null };
    return { step, url: null, renderedStateSummary: null, success: false, failureReason, ctxUpdates: {}, clickedButtonText, authForensics, providerResponseClass };
  }
}
...
export async function runJourneyWalk(page, persona, journeySteps, stepExecutors, { baseUrl }) {
  ...
  for (const step of journeySteps) {
    // QF-20260906-826: real per-step elapsed time, so a RED run's uat_test_results row can
    // carry duration_ms instead of leaving it permanently null (measured live against run
    // b29e63cc). Wraps the call rather than touching executeJourneyStep's two return sites.
