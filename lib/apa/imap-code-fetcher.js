 ...
 * @returns {Promise<string>} the extracted 6-digit code
 */
export async function fetchVerificationCode({
  ...
  throw new Error(
    ...
    `(searched since ${since.toISOString()}, ${attempts} poll attempt(s), last poll saw ${lastMessagesSeen} matching message(s))`
  );
}

export default { fetchVerificationCode, extractSixDigitCode };
