 ...
 * 6. Cascade priority: Anthropic → Google/Gemini → OpenAI → Ollama
 *
...
import { getOpenAIModel, getClaudeModel, getGoogleModel, MODEL_DEFAULTS } from '../config/model-config.js';
...
export function getLLMClient(options = {}) {
  ...
  if (isEffortLevel) {
    ...
    // Priority: Anthropic (thinking) → Google Gemini → OpenAI → inline stub
    // LLM_PROVIDER=google skips Anthropic even when key is present
    if (process.env.ANTHROPIC_API_KEY && preferredProvider !== 'google') {
    ...
    if (process.env.GEMINI_API_KEY || process.env.GOOGLE_AI_API_KEY) {
      // Use 3.1-pro for high effort (hard tasks), 2.5-pro for everything else
      const googleModel = effortOrTier === 'high'
        ? getGoogleModel('generation')
        : getGoogleModel('validation');
      console.log(`   🔷 LLM Factory: Using Google Gemini (${googleModel}) for ${label} [Anthropic unavailable]`);
