...
const FALLBACK_TIER_TO_MODEL = {
  haiku: getClaudeModel('fast'),
  sonnet: getClaudeModel('validation'),
  opus: getClaudeModel('generation')
};
 ...
 */
const THINKING_MODEL = getClaudeModel('generation');
...
async function getSupabaseClient() {
  try {
    ...
    console.error('[client-factory] Supabase client creation error:', err?.message || err);
    return null;
  }
}
...
export function getLLMClient(options = {}) {
  ...
  if (isEffortLevel) {
    ...
    if ((process.env.GEMINI_API_KEY || process.env.GOOGLE_AI_API_KEY) && preferredProvider !== 'anthropic') {
      const googleModel = effortOrTier === 'high'
        ? getGoogleModel('generation')
        : getGoogleModel('validation');
      console.log(`   🔷 LLM Factory: Using Google Gemini (${googleModel}) with ${effortOrTier} effort for ${label}`);
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      console.log(`   🧠 LLM Factory: Anthropic with ${effortOrTier} effort (${budgetTokens} thinking tokens) for ${label} [Google unavailable]`);
    ...
    console.log(`   📋 LLM Factory: No cloud API keys — inline processing required for ${label}`);
    return {
  ...
  // Legacy tier-to-purpose mapping for cloud provider selection.
  // COST CONTROL (2026-06-05): opus-tier (security/critical purposes) maps to the
  // reserved 'reasoning' purpose → gemini-2.5-pro, so deep reasoning is preserved
  // exactly where it matters. Everything else resolves to the cheap default (Flash).
  // To keep a specific gate on Pro, give its callsite purpose: 'security'/'critical'.
  const tierToPurpose = { opus: 'reasoning', sonnet: 'validation', haiku: 'fast' };
  const resolvedPurpose = tierToPurpose[effortOrTier] || 'validation';

  // Use Google Gemini as primary cloud provider
...
function getAdapterByProvider(provider, options = {}) {
  switch (provider.toLowerCase()) {
    ...
    default:
      // Note: explicit provider='unknown' falls back to Anthropic for safety.
      // Default routing (no explicit provider) goes through getLLMClient() which prefers Google.
 ...
 *
 * Supports three providers (cascade: Google → OpenAI → Ollama):
 * - Google gemini-embedding-001 (1536 dimensions) — primary, cloud
 * - OpenAI text-embedding-3-small (1536 dimensions) — secondary, cloud
...
export function getEmbeddingClient(options = {}) {
  ...
  // Google Gemini embeddings (primary cloud)
      ...
      async embed(input) {
        ...
        for (const text of texts) {
          ...
          if (!response.ok) {
            ...
            throw new Error(`Google embedding failed (${response.status}): ${err}`);
          }
          const data = await response.json();
  ...
  // OpenAI embeddings (secondary cloud fallback)
  ...
  return {
    ...
    async embed(input) {
      const texts = Array.isArray(input) ? input : [input];
      if (!openaiKey) {
        throw new Error('GEMINI_API_KEY or OPENAI_API_KEY is required for embedding generation');
      }
...
export function getRoutingStatus() {
  ...
  return {
    ...
    providers: {
      ...
      ollama: isLocalLLMEnabledInternal()
    },
    tierMapping: {
