...
// Cost control (2026-06-05): env-tunable. Long venture-build processes make 1000s of
// factory calls in a single process; a 200-entry cap evicted still-useful entries, so
// the default capacity is raised and the TTL is widenable. NOTE: this cache is
// IN-PROCESS only — each handoff.js/add-prd invocation starts cold, which is the main
// reason the cross-system hit rate is low. A persistent cross-process cache (DB/disk)
// is the larger follow-up; measure its impact via scripts/llm-cost-report.mjs.
const DEFAULT_TTL_MS = Number.parseInt(process.env.LLM_CACHE_TTL_MS || '', 10) || 30 * 60 * 1000; // 30 min default
const DEFAULT_MAX_ENTRIES = Number.parseInt(process.env.LLM_CACHE_MAX_ENTRIES || '', 10) || 500;
