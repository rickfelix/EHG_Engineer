...
/**
 * Infer tier from options (synced with client-factory getPurposeTier)
 *
 * SD-LEO-FIX-LLM-FACTORY-BYPASS-001: Synced with client-factory.js effort-based routing.
 * Canary routing only applies to haiku-tier requests; everything else goes to cloud.
 * Purposes that map to effort levels (medium/high) are treated as sonnet-tier for
 * canary purposes since they always use cloud Opus with thinking budgets.
 */
function inferTier(options) {
  if (options.tier) return options.tier;

  const purposeToTier = {
    // Haiku-tier (fast, cheap — canary-eligible)
    classification: 'haiku',
    fast: 'haiku',
    screening: 'haiku',
    triage: 'haiku',

    // These map to effort-based routing (medium/high) in the factory,
    // which always uses cloud Opus. Treat as sonnet for canary purposes.
    validation: 'sonnet',
    analysis: 'sonnet',
    design: 'sonnet',
    'quality-evaluation': 'sonnet',
    generation: 'sonnet',
    'content-generation': 'sonnet',

    // Opus-tier (always cloud, never canary)
    security: 'opus',
    critical: 'opus'
  };
