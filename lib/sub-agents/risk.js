...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    // PAT-LES-0bac630efd73: an infrastructure SD with meaningfully-detected data-migration
    // risk but a thin PRD risks[] array is exactly the retrospective-caught gap -- one risk
    // listed for a change touching multiple core production tables. Advisory (warnings, not
    // critical_issues) so it cannot newly block a handoff; does not alter any existing domain
    // score or threshold, only surfaces a distinct, additional finding.
    const completenessWarning = checkRiskAssessmentCompleteness(sd, prd, dataMigrationRisk.score);
    if (completenessWarning) {
      results.warnings.push(completenessWarning);
    }
...
const MIN_RISKS_FOR_INFRA_MIGRATION = 3;

/**
 * PAT-LES-0bac630efd73: retrospective-caught gap -- an infrastructure SD with a
 * meaningfully-detected data-migration risk (score >= 6, i.e. schema/table changes were
 * actually found by assessDataMigrationRisk's content scan) had only one risk listed.
 * Pure function (no I/O) so it is directly unit-testable without mocking Supabase.
 *
 * validation-learn154 F4: the origin retrospective (SD-LEO-FIX-MULTI-VENTURE-ISOLATION-001)
 * measured "only one risk listed" against the SD-LEVEL `risks` column (SD.risks=1), not the
 * PRD's -- that SD's PRD.risks actually had 2. Checking prd.risks ALONE would still have fired
 * on that origin case (2 < 3), but it reads a different artifact than the one the retro
 * counted. Take the max of both so this catches a thin risk list wherever it was authored.
 *
 * @param {{sd_type?: string, risks?: Array}} sd
 * @param {{risks?: Array}} [prd]
 * @param {number} dataMigrationRiskScore
 * @returns {Object|null} a warning entry, or null when the check doesn't apply / passes
 */
export function checkRiskAssessmentCompleteness(sd, prd, dataMigrationRiskScore) {
  if (sd?.sd_type !== 'infrastructure' || dataMigrationRiskScore < 6) return null;

  const sdRiskCount = Array.isArray(sd?.risks) ? sd.risks.length : 0;
  const prdRiskCount = Array.isArray(prd?.risks) ? prd.risks.length : 0;
  const riskCount = Math.max(sdRiskCount, prdRiskCount);
  if (riskCount >= MIN_RISKS_FOR_INFRA_MIGRATION) return null;

  return {
    domain: 'Risk Assessment Completeness',
    score: dataMigrationRiskScore,
    issue: `Risk assessment has ${riskCount} entr${riskCount === 1 ? 'y' : 'ies'} (SD.risks=${sdRiskCount}, PRD.risks=${prdRiskCount}), but this infrastructure SD's data-migration risk score (${dataMigrationRiskScore}/10) indicates schema/table changes. Infrastructure SDs affecting core production tables should document at least ${MIN_RISKS_FOR_INFRA_MIGRATION} risks covering data integrity, performance impact, and rollback strategy.`,
    monitoring_recommended: true
  };
}
