#!/usr/bin/env node
/**
 * 📝 STORIES Sub-Agent - User Story Context Engineering
 *
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    if (!userStories || userStories.length === 0) {
      ...
      results.warnings = ['No user stories to enhance. Create user stories first.'];
      return results;
    }
...
async function analyzeCodebasePatterns(sdId, prd) {
  ...
  try {
    ...
    if (existsSync(`${basePath}/src/components`)) {
      ...
      patterns.components = componentFiles.slice(0, 10).map(f => ({
    ...
    if (existsSync(`${basePath}/src/services`)) {
      ...
      patterns.services = serviceFiles.slice(0, 10).map(f => ({
        path: f.replace(basePath, ''),
    ...
    if (existsSync(`${basePath}/src/hooks`)) {
      ...
      patterns.hooks = hookFiles.slice(0, 10).map(f => ({
        path: f.replace(basePath, ''),
