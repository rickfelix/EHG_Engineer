...
export async function execute(sdId, subAgent, options = {}) {
  // Use options.sdKey for display, sdId (UUID) for database operations
  ...
  const results = {
    sd_id: sdId, // UUID for foreign key
  ...
  try {
    ...
    // ============================================
    // 1B. DETECT AND REMOVE PLACEHOLDER STORIES (PAT-STORIES-PLACEHOLDER-001)
    // Stories with "To be defined" titles or minimal context are placeholders
    // that should be replaced with proper stories from PRD acceptance criteria
    // ============================================
    if (userStories && userStories.length > 0) {
      const placeholderPatterns = [
        ...
        /placeholder/i,
        /tbd/i,
        /^us-\d+$/i  // Just story number with no content
      ];
      ...
      if (placeholderStories.length > 0) {
        console.log(`   ⚠️  Found ${placeholderStories.length} placeholder stories (PAT-STORIES-PLACEHOLDER-001)`);
        console.log('   🗑️  Removing placeholder stories to regenerate from PRD...');
        ...
        if (deleteError) {
          console.log(`   ⚠️  Warning: Could not delete placeholders: ${deleteError.message}`);
          // Continue anyway - we'll try to update instead
        } else {
          console.log(`   ✅ Removed ${placeholderStories.length} placeholder stories`);
          results.placeholders_removed = placeholderStories.length;
          // Clear the stories array to trigger regeneration
          userStories = userStories.filter(s => !placeholderIds.includes(s.id));
        }
      }
    }
    ...
    if (!userStories || userStories.length === 0) {
      ...
      // Use options.sdKey (original key) or sdId for PRD ID construction
      const prdId = options.prd_id || `PRD-${options.sdKey || sdId}`;
      ...
      for (let i = 0; i < prd.acceptance_criteria.length; i++) {
        ...
        // Use sdKey for human-readable story keys
        ...
        // Validate quality before inserting (async function)
        ...
        if (!qualityCheck.valid) {
          ...
          // Log quality issues but don't try to store in non-existent column
          // Quality validation is informational during PLAN phase
        } else {
          console.log(`   ✅ Quality check passed for ${storyKey} (score: ${qualityCheck.score}%)`);
        }
        // Note: quality_score column may not exist in all schemas - validation is logged above
...
function generateQualityStoryContent(criterion, prd, index) {
  // ============================================================================
  // MARKET-PULSE DIRECTIVE: Customer-Focused User Role Mapping
  // v3.2.0 Productization Package - Operation 'Strategic Sight'
  // THE LAW: Stories written for DBAs/Developers are BLOCKED.
  // ============================================================================
  const roleMap = {
    ...
    // REMOVED: 'admin' -> 'System Administrator' (FORBIDDEN PERSONA)
  };

  // Detect appropriate user role from criterion and PRD
  let userRole = 'Platform User'; // Default to customer-focused persona
  const criterionLower = criterion.toLowerCase();
  const prdTitle = (prd?.title || '').toLowerCase();
  const prdSummary = (prd?.executive_summary || '').toLowerCase();

  // ============================================================================
  // MARKET-PULSE GUARD: Reject forbidden personas
  // ============================================================================
  ...
  if (hasForbiddenPersona) {
    // Log warning but DON'T use the forbidden persona
    console.log('   ⚠️  MARKET-PULSE VIOLATION: Criterion mentions tech persona. Defaulting to customer persona.');
    // Continue with default customer persona instead of blocking
  }

  for (const [key, role] of Object.entries(roleMap)) {
    if (criterionLower.includes(key) || prdTitle.includes(key) || prdSummary.includes(key)) {
      // Double-check this role isn't forbidden
 ...
 */
function generateUserWant(criterion, _prd) {
  // Extract the core action from criterion
  const _criterionLower = criterion.toLowerCase();
 ...
 * Calculate story points based on complexity indicators
 */
function calculateStoryPoints(criterion, _prd) {
...
async function generateCodePatterns(story, _prd, _patterns) {
  const codePatterns = [];
...
async function generateTestingScenarios(story, _prd) {
  const scenarios = [];
