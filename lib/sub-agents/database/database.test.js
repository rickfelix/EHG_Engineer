...
describe('DATABASE Sub-Agent Modules', () => {
  ...
  describe('Function Behavior', () => {
    ...
    // SD-LEO-FIX-DATABASE-SCHEMA-VALIDATOR-001: staticFileValidation() previously
    // never discovered any migration file (glob@7.2.3's callback API was awaited as
    // if it returned a Promise, so allFiles was always []). Prior coverage only
    // asserted `typeof staticFileValidation === 'function'` — this asserts observed
    // behavior against real fixture files, not the array literal.
    describe('staticFileValidation (behavioral)', () => {
      const fixtures = [];

      afterEach(async () => {
        await Promise.all(fixtures.splice(0).map(f => unlink(f).catch(() => {})));
      });

      async function writeFixture(relDir, relFilename, sdId) {
        const dir = path.join(getRepoRoot(), relDir);
        const filePath = path.join(dir, relFilename);
        await writeFile(filePath, `-- ${sdId}\nCREATE TABLE IF NOT EXISTS __test_fixture__ (id int);\n`, 'utf-8');
        fixtures.push(filePath);
        return filePath;
      }

      it('finds a real migration file under database/migrations by SD-ID content match', async () => {
        const { staticFileValidation } = await import('./schema-validator.js');
        const sdId = 'SD-TEST-FIXTURE-DB-MIGRATIONS-001';
        const filePath = await writeFixture('database/migrations', '__test_fixture_db_schema_validator_migrations__.sql', sdId);

        const result = await staticFileValidation(sdId, {});

        expect(result.verdict).not.toBe('NOT_REQUIRED');
        expect(result.migration_files.map(p => path.resolve(p))).toContain(path.resolve(filePath));
      });

      it('finds a chairman-gated migration file by SD-ID content match (the originally-scoped gap)', async () => {
        const { staticFileValidation } = await import('./schema-validator.js');
        const sdId = 'SD-TEST-FIXTURE-CHAIRMAN-GATED-001';
        const filePath = await writeFixture('database/chairman-gated', '__test_fixture_db_schema_validator_chairman_gated__.sql', sdId);
      ...
      it('returns NOT_REQUIRED when no file mentions the SD ID', async () => {
        const { staticFileValidation } = await import('./schema-validator.js');

        const result = await staticFileValidation('SD-DOES-NOT-EXIST-ANYWHERE-999', {});
