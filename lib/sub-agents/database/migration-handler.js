...
// SD-LEO-INFRA-FLEET-WIDE-SUB-001: DATABASE_MIGRATION declares supports_cross_repo=false
// in lib/sub-agents/registry.json (only_repo=EHG_Engineer). Migrations live in this
// repo only — use worktree-aware getRepoRoot() instead of process.cwd().
...
export async function executeViaSupabaseCLI(sqlContent, description = 'SQL migration') {
  ...
  try {
    ...
    // SD-LEO-INFRA-FLEET-WIDE-SUB-001: getRepoRoot() instead of process.cwd() — worktree-aware
    // canonical EHG_Engineer root; .temp/ stays in main even when invoked from a .worktrees checkout.
    const tempFile = path.join(getRepoRoot(), '.temp', `migration_${Date.now()}.sql`);
