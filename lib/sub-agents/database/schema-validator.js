...
// SD-LEO-INFRA-FLEET-WIDE-SUB-001: DATABASE_SCHEMA declares supports_cross_repo=false
// in lib/sub-agents/registry.json (only_repo=EHG_Engineer). Schema docs live in this
// repo only — use worktree-aware getRepoRoot() instead of process.cwd() so the path
// resolves to the canonical main root from inside a .worktrees/<sd> checkout too.
import { getRepoRoot } from '../../repo-paths.js';

const { glob } = globPkg;
const execAsync = promisify(exec);
// SD-LEO-FIX-DATABASE-SCHEMA-VALIDATOR-001: glob@7.2.3's main export is callback-based,
// not thenable — `await glob(pattern)` previously resolved to the Glob EventEmitter
// instance itself (not the match list), so every migrationPaths search below silently
// found zero files, for every SD, always. promisify() gives the real async result.
...
export async function staticFileValidation(sdId, _options) {
  ...
  try {
    ...
    // absolute:true + cwd:getRepoRoot() so results are correct and directly
    // readFile()-able regardless of which worktree this runs from.
    let allFiles = [];
    for (const pattern of migrationPaths) {
      try {
        const files = await globAsync(pattern, { cwd: getRepoRoot(), absolute: true });
