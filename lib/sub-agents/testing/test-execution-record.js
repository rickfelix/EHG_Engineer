 ...
 *   source?: 'fresh'|'reused'|null, mappedCandidates?: number|null, foundFiles?: number|null}} args
 ...
 *   artifact_path: string|null, source: string|null, mapped_candidates: number|null, found_files: number|null}}
 */
export function buildTestExecution({ executed = 0, passed = 0, failed = 0, skipped = 0, artifactSha = null, runner = null, artifactPath = null, source = null, mappedCandidates = null, foundFiles = null } = {}) {
  return {
    ...
    source: source || null,
    // QF-20260905-797: how many candidate test files the scoped mapper considered vs how many it
    // resolved to real files, so a partial map (e.g. co-located-only, pre-fix) is visible in the
    // evidence rather than silent. null (not 0) when the caller never supplied a scoped mapping.
    mapped_candidates: Number.isFinite(mappedCandidates) ? mappedCandidates : null,
    found_files: Number.isFinite(foundFiles) ? foundFiles : null
  };
}
