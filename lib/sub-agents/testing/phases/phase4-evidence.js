...
// SD-FDBK-INFRA-TESTING-SUB-AGENT-001: single-source applicability, replacing the private
// per-file exemption list here (which disagreed with index.js's own private list — 5 vs 11 entries).
...
export async function verifyUserStories(sdId, supabase, options = {}) {
  ...
  // SD types that verify existing functionality rather than implementing new stories don't
  // require e2e_test_path mapping since they validate, not create. Single source of truth
  // (SD-FDBK-INFRA-TESTING-SUB-AGENT-001): the same predicate index.js's checkForNonUISdType
  // uses to decide E2E applicability, so this file can no longer silently diverge from it.
  const sdType = (options.sdType || '').toLowerCase();
  const requireE2EMapping = !isE2EApplicabilityExempt(sdType);
  ...
  // sd_type requiring E2E mapping this clause demanded something nine-tenths of the
  ...
  //
  // QF-20260903-222: this SAME signal is also the sanctioned disposition path
  // (scripts/dispose-user-story.js) for a story that is a generator artefact and must never
  // be implemented at all (a review finding, a scope estimate) — a different reason than
  // "genuinely gated on external action", but an identical exclusion need. The reason is
  // recorded in metadata.disposition, which this predicate does not read; it stays a pure
  // two-field check either way.
