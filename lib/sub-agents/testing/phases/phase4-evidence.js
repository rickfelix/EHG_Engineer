...
export async function verifyUserStories(sdId, supabase, options = {}) {
  ...
  // SD-LEO-FEAT-IDEATION-INGESTION-CONNECTORS-001 (EXEC review, harness-bug finding): status=
  // 'blocked' is a deliberate, explicit signal that a story is genuinely gated on external human
  // action outside EXEC's control (mirrors the same exclusion added to the EXEC-TO-PLAN
  // USER_STORY_COVERAGE gate, scripts/modules/handoff/executors/exec-to-plan/gates/
  // user-story-coverage.js). Reported distinctly, excluded from the completeness filter that
  // otherwise unconditionally requires status IN (completed, validated).
  const externallyBlocked = stories.filter(s => s.status === 'blocked');
  const evaluable = stories.filter(s => s.status !== 'blocked');
