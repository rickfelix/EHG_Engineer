...
export async function checkForNonUISdType(sdId, validationMode, options, branchContext, supabaseClient = supabase) {
  ...
  if (scopedTestFiles.length === 0) {
    ...
    return {
      ...
      metadata: { measured: false, test_execution: buildTestExecution({ mappedCandidates: scopedTestFiles.mappedCandidates ?? null, foundFiles: 0 }), applicability_rule: 'policy_non_applicable_code_no_scoped_test' },
      options
    };
  }
