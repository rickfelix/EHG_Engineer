 ...
 * @param {{tests_executed?: number, tests_passed?: number, failed_tests?: number, skipped_tests?: number}} phase3
...
export async function checkForNonUISdType(sdId, validationMode, options, branchContext, supabaseClient = supabase) {
  ...
  const repoPath = branchContext?.repoPath
    || (await resolveSubAgentRepo({ sdId, subAgentCode: 'TESTING', targetApplication: 'EHG_Engineer' })).repoPath;
  let changedFiles = [];
  try {
