...
// SD-LEO-FIX-EXEC-PLAN-ACCEPTED-001 (FR-8): post-merge re-verify mode for checkForNonUISdType.
import { parseDiffRange } from './diff-range.js';
// QF-20260911-350: fallback zero-UI measurements when the direct diff range is unresolvable or
// resolves empty (squash-merged branches) -- see checkForNonUISdType's resolveFallbackNonUiFiles.
import { computeReposForSD } from '../repo-target-resolver.js';
import { findMergedPrFileList } from '../../../scripts/modules/handoff/executors/lead-final-approval/chairman-apply-state.js';
...
// SD-LEO-FIX-EXEC-PLAN-HEALS-001 FR-1/FR-2: the evidence-gated reconciler (never the
// zero-evidence autoCompleteDeliverablesForSD fallback) and the canonical promoter, used to
// heal-then-measure for POST_IMPLEMENTATION callers only.
import { reconcileDeliverables } from '../../../scripts/modules/handoff/executors/exec-to-plan/gates/deliverables-completeness.js';
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    const { data: sdTypeData } = await supabase
      ...
      .single();
    const storyGateContext = resolveStoryGateContext(options);

    await healDeliverablesAndStories(sdId, supabase, storyGateContext);
    ...
    if (!phase4_5.verified && phase4_5.incomplete?.length > 0) {
      // QF-20260903-748: this escalation used to block UNCONDITIONALLY, which applied the
      // EXEC-TO-PLAN "implementation complete" bar while TESTING was running to satisfy
      // PLAN-TO-EXEC. required-subagents.js maps PLAN-TO-EXEC -> PLAN_PRD, so TESTING running
      // pre-implementation is BY DESIGN; demanding completed stories there is unsatisfiable.
      //
      // The tell was in the message itself: it recommended acting "before EXEC→PLAN handoff"
      // while executing for the EARLIER handoff.
      //
      // It also inverted the incentive. verifyUserStories short-circuits to verified:true when
      // an SD has ZERO stories (phase4-evidence.js), so an SD with NO stories PASSED and an SD
      // that correctly authored stories before EXEC was BLOCKED — the gate punished compliance
      // with the harness's own story-creation expectation. That is why it stayed hidden for six
      // days across 115 occurrences: the SDs that would have exposed it were the compliant ones.
      //
      // verifyUserStories itself is NOT changed — its completeness filter is correct for its
      // real (EXEC-TO-PLAN) use and is pinned by existing unit suites. Only the classification
      // of its result is made context-aware.
      const finding = {
        severity: storyGateContext.blocking ? 'CRITICAL' : 'MEDIUM',
        issue: `${phase4_5.incomplete.length} user stories not fully implemented`,
        stories: phase4_5.incomplete,
        handoff_context: storyGateContext.context,
        context_source: storyGateContext.source,
        recommendation: storyGateContext.blocking
          ? 'Complete implementation of all user stories before EXEC→PLAN handoff'
          : 'Informational at this phase: stories are authored but not yet implemented, which is the expected pre-EXEC state. Implementation is verified at EXEC-TO-PLAN, where this same check blocks.'
      };

      if (storyGateContext.blocking) {
        results.critical_issues.push(finding);
        results.verdict = 'BLOCKED';
    ...
    // QF-20260903-748 (b): when TESTING runs BEFORE implementation exists (PLAN-TO-EXEC /
    // PLAN_PRD), phase3 necessarily reports tests_executed === 0 and the prospective arm of
    // generateVerdict returns BLOCKED at confidence 100 with no critical issues — a blocking
    // verdict recording no measurement, indistinguishable from a real blocker. Assess the one
    // thing that IS knowable pre-implementation: whether the PRD declares a test strategy.
    // Reuses the same context resolver as the story gate, so both pre-implementation
    // behaviours are driven by one definition of "which handoff asked for this".
    if (!storyGateContext.blocking) {
      const prd = await fetchPrdForTestStrategy(sdId);
      results.findings.prd_test_strategy = {
        ...assessPrdTestStrategy(prd),
        pre_implementation: true
      };
      console.log(`   📐 Pre-implementation phase — judging PRD test strategy instead of E2E execution: ${results.findings.prd_test_strategy.reason}`);
    }
 ...
 */
/**
 * QF-20260903-748: decide whether an INCOMPLETE-user-stories finding should BLOCK.
 *
 * TESTING is a required sub-agent for BOTH PLAN-TO-EXEC (evidence phase PLAN_PRD, before any
 * implementation exists) and EXEC-TO-PLAN (after it does). "Stories not yet implemented" is the
 * expected state at the first and a genuine defect at the second, so the same finding must be
 * classified differently depending on which handoff asked for the evidence.
 *
 * FAILS CLOSED. An unresolvable context blocks, because the alternative — treating "I don't know
 * which handoff this is" as permission to pass — would let the EXEC-TO-PLAN check be silently
 * skipped by omitting a flag. Blocking on ambiguity is recoverable (pass --phase); passing on
 * ambiguity is not (unverified work ships).
 *
 * Deliberately reads ONLY the caller-supplied context. It does not fall back to the SD's stored
 * current_phase: that records where the SD *is*, not which handoff invoked this run, and the two
 * diverge during a re-run.
 *
 * @param {Object} options - Execution options; may carry handoffType and/or phase.
 * @returns {{blocking: boolean, context: string, source: string}}
 */
export function resolveStoryGateContext(options = {}) {
  const normalize = (v) => (typeof v === 'string' ? v.trim().toUpperCase().replace(/[_\s]+/g, '-') : null);

  // Pre-implementation: stories are authored but nothing has been built yet, by design.
  const PRE_IMPLEMENTATION = new Set(['PLAN-TO-EXEC', 'PLAN-PRD']);
  // Post-implementation: the completeness bar is meaningful and must keep blocking.
  const POST_IMPLEMENTATION = new Set(['EXEC-TO-PLAN', 'PLAN-VERIFY', 'EXEC', 'PLAN-TO-LEAD', 'LEAD-FINAL-APPROVAL']);

  const candidates = [
    { value: normalize(options.handoffType), source: 'options.handoffType' },
    { value: normalize(options.phase), source: 'options.phase' }
  ];

  for (const { value, source } of candidates) {
    if (!value) continue;
    if (PRE_IMPLEMENTATION.has(value)) return { blocking: false, context: value, source };
    if (POST_IMPLEMENTATION.has(value)) return { blocking: true, context: value, source };
    // A recognised-but-unmapped value is ambiguous, not permissive — keep looking, then fail closed.
  }

  return { blocking: true, context: 'UNRESOLVED', source: 'fail-closed-default' };
}

/**
 * SD-LEO-FIX-EXEC-PLAN-HEALS-001 FR-1/FR-2: heal-then-measure, but ONLY for
 * POST_IMPLEMENTATION callers (storyGateContext.blocking === true). Uses ONLY the
 * evidence-gated reconciler (gates/deliverables-completeness.js's reconcileDeliverables) --
 * NEVER autoCompleteDeliverablesForSD / autoCompleteDeliverables, whose unconditional
 * AUTO_COMPLETE_HANDOFF_CONTEXT fallback tier would let a standalone TESTING run fabricate
 * the very evidence it then measures against (the exact self-authored-evidence defect class
 * CLAUDE.md's gate-evidence-provenance rule forbids). autoValidateUserStories is a safe
 * no-op when deliverables remain incomplete, so this can never over-promote a story.
 * @param {string} sdId
 * @param {object} supabase - injected Supabase client
 * @param {{blocking: boolean}} storyGateContext - from resolveStoryGateContext()
 * @returns {Promise<void>}
 */
export async function healDeliverablesAndStories(sdId, supabase, storyGateContext) {
  if (!storyGateContext?.blocking) return;

  // count-truncation-diff-lint: bounded read -- an SD's own deliverable rows never approach
  // this cap; it exists to keep the read provably bounded, not to model expected volume.
  const { data: healDeliverables } = await supabase
    .from('sd_scope_deliverables')
    .select('id, deliverable_name, completion_status, metadata, created_by')
    .eq('sd_id', sdId)
    .limit(200);
  if (healDeliverables && healDeliverables.length > 0) {
    const { reconciled } = await reconcileDeliverables(healDeliverables, sdId, supabase);
    if (reconciled > 0) console.log(`   🔄 TESTING heal step: reconciled ${reconciled} deliverable(s) before measuring`);
  }
  // Safe no-op when deliverables remain incomplete -- never over-promotes.
  await autoValidateUserStories(sdId, supabase);
}

/**
 * QF-20260903-748 (b): judge the PRD's TEST STRATEGY when TESTING runs pre-implementation.
 *
 * At PLAN_PRD there is nothing to execute, so phase3 reports tests_executed === 0 and the
 * prospective arm of generateVerdict returns BLOCKED at confidence 100 with NO critical issues
 * — a blocking verdict that records no measurement, and one that is indistinguishable from a
 * genuine blocker. What IS knowable before implementation is whether the PRD declares a test
 * strategy worth building against, so that is what this assesses.
 *
 * Never returns BLOCKED. A thin or absent strategy is a CONDITIONAL_PASS with the gap named:
 * "the plan is weak" is a different claim from "the code is broken", and only the second should
 * stop a handoff into EXEC.
 *
 * @returns {{assessed: boolean, adequate: boolean, scenario_count: number, reason: string, gaps: string[]}}
 */
export function assessPrdTestStrategy(prd) {
  if (!prd) {
    return { assessed: false, adequate: false, scenario_count: 0, gaps: ['no PRD row found'],
      reason: 'No PRD to assess. GATE_PRD_EXISTS owns PRD presence; TESTING does not duplicate it.' };
  }
  ...
  // The PRD contract itself requires >= 5 test scenarios covering happy path, edge cases and
  // errors. Measuring against the contract the PRD was authored to, not an invented bar.
  const MIN_SCENARIOS = 5;
  if (count === 0) gaps.push('PRD declares no test_scenarios');
  else if (count < MIN_SCENARIOS) gaps.push(`PRD declares ${count} test scenario(s), below the PRD contract minimum of ${MIN_SCENARIOS}`);

  // A scenario is only executable if it says what is done and what is expected. A list of
  // titles reads as coverage while asserting nothing.
  const executable = scenarios.filter((s) => s && (s.then || s.expected) && (s.when || s.scenario));
  if (count > 0 && executable.length < count) {
    gaps.push(`${count - executable.length} of ${count} scenario(s) lack a when/then pair, so they are not executable as written`);
  }

  // Error and edge coverage is the half most often missing; happy paths are rarely forgotten.
  const blob = JSON.stringify(scenarios).toLowerCase();
  const coversFailure = /error|fail|invalid|refus|reject|missing|edge|empty|unreadable|stale/.test(blob);
  if (count > 0 && !coversFailure) gaps.push('no scenario appears to cover an error or edge condition');

  const adequate = count >= MIN_SCENARIOS && executable.length === count && coversFailure;
  return {
    assessed: true,
    adequate,
    ...
    gaps,
    reason: adequate
      ? `PRD declares ${count} executable test scenarios including failure/edge coverage.`
      : `PRD test strategy is usable but incomplete: ${gaps.join('; ')}.`
  };
}

/**
 * TESTING is invoked with either a uuid or an SD key, and product_requirements_v2 stores BOTH
 * under different columns: sd_id holds the uuid, directive_id holds the KEY (verified against
 * this SD's live row — a directive_id lookup with the uuid returns nothing). Trying only one
 * would silently report "no PRD" for half of all callers.
 *
 * Two sequential .eq() lookups rather than a single .or() template, matching the security note
 * on fetchSdRowForRepoResolution below: an .or() string is a raw PostgREST filter expression,
 * so an interpolated identifier can inject additional clauses.
 */
async function fetchPrdForTestStrategy(sdId, supabaseClient = supabase) {
  const byColumn = async (column) => {
  ...
  try {
    return (await byColumn('sd_id')) || (await byColumn('directive_id'));
  } catch {
    return null;
  }
}
...
// applicabilitySource values that mean "changedFiles is a real, measured, non-empty, non-UI
// file list" -- as opposed to 'declared_type' (a type-exemption with no file-level measurement).
const MEASURED_NON_UI_SOURCES = new Set(['measured_diff', 'files_to_modify', 'merged_pr_file_list']);

/**
 * QF-20260911-350: when the direct diff range is unresolvable (cwd-shared-root-refused) or
 * resolves empty (a squash-merged branch where main...HEAD is empty by construction), the
 * zero-UI-by-diff gate used to fail closed unconditionally -- three separate BLOCKED runs in one
 * session for genuinely zero-UI, already-merged SDs. Try a second measurement first: the SD's
 * own declared files_to_modify (cheap, no network), else the merged PR's actual file list
 * (findMergedPrFileList, already used by the chairman-apply gate for the same purpose).
 * Exported for unit testing (mirrors checkForNonUISdType's own supabaseClient injection).
 * @returns {Promise<{files: string[]|null, source: 'files_to_modify'|'merged_pr_file_list'|null}>}
 */
export async function resolveFallbackNonUiFiles(sdId, sdData, supabaseClient) {
  const declaredFiles = Array.isArray(sdData?.metadata?.files_to_modify)
    ? sdData.metadata.files_to_modify.filter((f) => typeof f === 'string' && f.trim())
    : [];
  if (declaredFiles.length > 0) return { files: declaredFiles, source: 'files_to_modify' };
  try {
    const reposWithPaths = computeReposForSD(sdData);
    const { files, error } = await findMergedPrFileList(sdId, reposWithPaths, supabaseClient);
    if (!error && Array.isArray(files) && files.length > 0) {
      return { files, source: 'merged_pr_file_list' };
    }
  } catch (e) {
    console.log(`   ⚠️  Merged-PR file-list fallback unavailable: ${e.message}`);
  }
...
export async function checkForNonUISdType(sdId, validationMode, options, branchContext, supabaseClient = supabase) {
  ...
  // QF-20260911-350: sd_key/target_application/metadata added -- needed by
  // resolveFallbackNonUiFiles (computeReposForSD + metadata.files_to_modify) below.
  const { data: sdData } = await supabaseClient
    .from('strategic_directives_v2')
    .select('sd_type, category, key_changes, scope, title, sd_key, target_application, metadata')
    .or(`sd_key.eq.${sdId},id.eq.${sdId}`)
    .single();
  ...
  let cwdSharedRootRefused = false;
  async function resolveChangedFiles() {
    ...
    try {
      ...
      // SD-LEO-FIX-EXEC-PLAN-ACCEPTED-001 (FR-8, coordinator scope note 5bd643ca): once this
      // SD's branch is merged, main...HEAD is empty (HEAD is now an ancestor of main), which
      // would otherwise fail this gate closed forever for an already-merged SD. --diff-range
      // supplies the SD's real pre-merge range explicitly for a post-merge re-verify run.
      // A malformed/unsafe value is rejected and falls back to the default range (never
      // interpolated into the shell command unvalidated).
      const explicitRange = parseDiffRange(options?.diff_range);
      if (options?.diff_range && !explicitRange) {
        console.log(`   ⚠️  --diff-range value rejected (unsafe/malformed): "${options.diff_range}" — falling back to main...HEAD`);
      }
      let range = explicitRange || 'main...HEAD';

      // QF-20260905-829: an unqualified main...HEAD is empty BY CONSTRUCTION when the executing
      // cwd is the shared root checked out on main itself (HEAD IS main there) -- not a genuine
      // "no changes" signal. Worktrees share one .git, so the SD's real branch is visible from
      // the shared root's local refs with no checkout needed -- diff against it instead. Only
      // when no branch is resolvable does this become genuinely unmeasurable (cwdSharedRootRefused
      // below), rather than silently computing an artifact-empty diff that fails closed to a
      // doomed full E2E run (SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-F specimen).
      if (!explicitRange) {
        try {
          const currentBranch = execSync('git rev-parse --abbrev-ref HEAD', { encoding: 'utf-8', cwd: repoPath, timeout: 5000 }).trim();
          if (currentBranch === 'main') {
            if (branchContext?.branch && branchContext.branch !== 'main') {
              range = `main...${branchContext.branch}`;
              console.log(`   ⚠️  executed_from_cwd is the shared root on main -- diffing against the SD's real branch instead: ${range}`);
            } else {
              cwdSharedRootRefused = true;
            }
          }
        } catch { /* fail-open to the default range on a resolution error */ }
      }

      changedFiles = cwdSharedRootRefused
        ? null // distinct from [] -- "could not measure", never treated as "no changes"
        : execSync(`git diff --name-only ${range}`, { encoding: 'utf-8', cwd: repoPath, timeout: 15000 })
  ...
  if (!isE2EApplicabilityExempt(effectiveType)) {
    ...
    // QF-20260911-350: the direct diff can be unresolvable (cwdSharedRootRefused -> null) or
    // genuinely empty (a squash-merged branch where main...HEAD is empty by construction) --
    // either way, try a SECOND measurement (the SD's declared files_to_modify, or the merged
    // PR's actual file list) BEFORE failing closed. Only when both this and the direct diff are
    // unavailable does the gate fail closed to the mandatory E2E flow, exactly as before.
    let fallbackSource = null;
    if (diffFiles === null || diffFiles.length === 0) {
      const fallback = await resolveFallbackNonUiFiles(sdId, sdData, supabaseClient);
      if (fallback.files) {
        const fbClassification = classifySDAsUITouching(fallback.files);
        if (!fbClassification.ui_touching) {
          changedFiles = fallback.files;
          applicabilitySource = fallback.source;
          fallbackSource = fallback.source;
          console.log(`\n🗄️  Zero-UI via ${fallback.source} fallback (direct diff unmeasurable): ${fallback.files.length} file(s), none match UI patterns`);
        }
      }
    }

    if (!fallbackSource) {
      // QF-20260905-829: a null diffFiles means the cwd-shared-root-on-main case was genuinely
      // unresolvable (no SD branch to diff against instead) -- REFUSE distinctly, never a silent
      // fail-closed into a doomed full E2E run that times out reporting 0 tests.
      if (diffFiles === null) {
        return {
          verdict: 'BLOCKED',
          confidence: 100,
          ...
          critical_issues: [
            'Cannot determine E2E applicability: executed_from_cwd is the shared root checked out on main (main...HEAD is empty by construction there), and no SD feature branch was resolvable to diff against instead.'
          ],
          warnings: [],
          recommendations: [{
            severity: 'CRITICAL',
            issue: 'TESTING sub-agent executed from the shared root on main',
            recommendation: `Re-run from the SD's own worktree (node scripts/sd-start.js ${sdId}), or supply --diff-range explicitly.`
          }],
          // QF-20260911-350: a non-empty summary naming which measurements failed -- prior BLOCKED
          // rows for this axis persisted with no summary at all, unattributable after the fact.
          summary: 'Cannot determine E2E applicability: direct diff range unresolvable, and the files_to_modify/merged-PR-file-list fallback found nothing usable either.',
          detailed_analysis: {
            sd_type: effectiveType,
            sd_type_source: sdType ? 'declared' : 'category_fallback',
            skip_reason: 'executed_from_cwd is the shared root on main; main...HEAD is empty by construction and no SD branch was resolvable',
            applicability_rule: 'cwd_shared_root_refused'
          },
          findings: {
            phase0_intelligence: { skipped: true, reason: 'cwd_shared_root_refused' },
            phase1_preflight: { skipped: true },
            phase2_test_generation: { skipped: true },
            phase3_execution: { skipped: true, reason: 'cwd_shared_root_refused' },
            phase4_evidence: { type: 'refused' }
          },
          metadata: { measured: false, test_execution: buildTestExecution(), applicability_rule: 'cwd_shared_root_refused' },
          options
        };
      }
  ...
  // sniffing on key_changes/scope) -- for any of the three MEASURED sources (a direct diff, or
  // the QF-20260911-350 files_to_modify/merged_pr_file_list fallbacks) we already have direct
  // proof of code production (a non-empty, non-UI file list), so that guess would wrongly say
  // "no code" for every non-infrastructure type reaching this point (QF-20260902-796).
  const codeDetection = MEASURED_NON_UI_SOURCES.has(applicabilitySource)
    ? { producesCode: true, reason: `measured non-empty, non-UI ${applicabilitySource === 'measured_diff' ? 'diff' : applicabilitySource} (${changedFiles.length} file(s))` }
  ...
  if (!codeDetection.producesCode) {
    ...
    return {
      ...
      metadata: { measured: false, test_execution: buildTestExecution(), applicability_rule: 'policy_non_applicable_no_code', zero_ui_source: applicabilitySource },
      options
    };
  }
  ...
  if (scopedTestFiles.length === 0) {
    ...
    return {
      ...
      metadata: { measured: false, test_execution: buildTestExecution({ mappedCandidates: scopedTestFiles.mappedCandidates ?? null, foundFiles: 0 }), applicability_rule: 'policy_non_applicable_code_no_scoped_test', zero_ui_source: applicabilitySource },
      options
    };
  }
  ...
  const testExecution = buildTestExecution({
    ...
    runner: 'vitest',
    mappedCandidates: scopedTestFiles.mappedCandidates ?? null,
    foundFiles: scopedTestFiles.length
  });
  ...
  return {
    ...
    metadata: {
      ...
      applicability_rule: 'policy_non_applicable_code_measured',
      zero_ui_source: applicabilitySource
    },
    options
  };
}
