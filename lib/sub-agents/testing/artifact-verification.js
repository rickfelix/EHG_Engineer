 ...
 *
 * Two-sided: artifactStartTime must be at/after commitTimestamp (a real floor -- tests run
 * after the commit they certify) AND not after `now` (a peer QA review noted the original
 * floor-only check let an artifact -- whose stats.startTime is attacker-controlled JSON
 * content, only type-checked as a string -- claim an arbitrary future timestamp and satisfy
 * the floor trivially for any past commit; this mirrors the unsigned/one-sided-age defect
 * class found upstream in test-evidence-ingest.js's own freshness gate, which this function
 * must not repeat).
 ...
 * @param {number} now - injectable current time in ms, defaults to Date.now()
 * @returns {boolean}
 */
export function isArtifactFresh(artifactStartTime, commitTimestamp, now = Date.now()) {
  ...
  if (artifactMs > now) return false;
