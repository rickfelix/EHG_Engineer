  ...
  try {
    ...
    if (!storyError && stories && stories.length > 0) {
      const incomplete = stories.filter(s =>
        ...
        !s.e2e_test_path ||
        s.e2e_test_status !== 'passing'  // Fixed: enum value is 'passing' not 'passed'
      );
      ...
      if (incomplete.length > 0) {
        ...
        incomplete.forEach(s => {
          ...
          console.log(`        Status: ${s.status || 'NULL'}, E2E: ${s.e2e_test_path || 'NOT MAPPED'}, Result: ${s.e2e_test_status || 'NOT RUN'}`);
        });
        results.critical_issues.push({
          severity: 'CRITICAL',
    ...
    if (options.full_e2e) {
      ...
      if (previousTest && !error) {
        ...
        if (isRecent) {
          ...
          const cachedData = previousTest.metadata?.findings?.phase3_execution || previousTest.metadata || {};
