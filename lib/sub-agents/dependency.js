...
// repo into findPackageJsonFiles so DEPENDENCY sub-agent scans the SD's actual repo.
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  // (previously execute() ignored _options entirely — now threaded for cross-repo support).
  let resolution = null;
  try {
    resolution = await resolveSubAgentRepo({
      sdId,
      targetApplication: options.target_application,
      subAgentCode: 'DEPENDENCY',
      supabase,
    });
  } catch (e) {
    console.log(`   ⚠️  Repo resolution failed (${e.message}); falling back to EHG default`);
  }
  ...
  try {
    ...
    // SD-LEO-INFRA-FLEET-WIDE-SUB-001: thread resolved repo through findPackageJsonFiles for cross-repo SDs
    const packageJsonPaths = findPackageJsonFiles(resolution?.repoPath);
 ...
 */
function findPackageJsonFiles(repoPath) {
  const foundPaths = [];
  // SD-LEO-INFRA-FLEET-WIDE-SUB-001: accept caller-resolved repoPath (cross-repo aware)
  // for the application's package.json; always include EHG_Engineer's package.json
  // as the orchestrator-repo audit target.
  const appPath = repoPath || resolveRepoPath('ehg');
  const searchPaths = [
