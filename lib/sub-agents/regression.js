...
// SD-LEO-INFRA-FLEET-WIDE-SUB-001: removed module-scope DEFAULT_REPO_PATH eager-resolution
// (was at L34); moved to lazy resolution inside execute() so cross-repo SDs scan their
// target_application's repo via the shared helper. Direct callers that bypass execute()
// would have lost DEFAULT_REPO_PATH access — verified no such callers exist (DEFAULT_REPO_PATH
// was not exported and only referenced once at L74).
...
// SD-LEO-INFRA-FLEET-WIDE-SUB-001: DEFAULT_REPO_PATH removed (was eager-resolved at import).
// resolveRepoPath('ehg') now called lazily inside execute() as fallback for the helper.
const DEFAULT_ENGINEER_PATH = path.resolve(__dirname, '../../..');

/**
 * SD-LEO-INFRA-SUB-AGENT-REPO-001: deep-tier adversarial review found this file has two OTHER
 * CONDITIONAL_PASS-setting branches (documented API changes, coverage decrease) besides the
 * repo-resolution downgrade -- same defect class as the SD's headline bug (a CONDITIONAL_PASS
 * with no conditions/justification violates check_conditions_required/check_justification_required,
 * 23514), in the same file this SD patches. Populate both fields here, mirroring
 * applySubAgentRepoVerdict's synthesis, ONLY when they are not already set (additive, never
 * overrides an already-valid value).
 *
 * @param {Object} results - sub-agent results object (mutated in place)
 * @param {string} reason - the degradation reason (becomes conditions[].action and feeds justification)
 */
function ensureConditionalPassEvidence(results, reason) {
  if (!Array.isArray(results.conditions) || results.conditions.length === 0) {
    results.conditions = [{ action: reason, priority: 'medium', blocking: false }];
  }
  if (typeof results.justification !== 'string' || results.justification.trim().length < 50) {
    results.justification = `CONDITIONAL_PASS recorded by REGRESSION: ${reason}`;
  }
}

/**
 * SD-LEO-INFRA-SUB-AGENT-REPO-001: pure precedence rule for resolveSubAgentRepo's
 * targetApplication input. A caller-supplied options.target_application always wins;
 * otherwise fall back to the SD's own target_application. Extracted so the exact
 * precedence a CLI invocation depends on (options.target_application is never populated
 * by the CLI entry point below) is unit-testable without a DB round-trip.
 *
 * @param {Object} options - execute()'s options argument
 * @param {Object|null} sdDetails - return value of getSDDetails
 * @returns {string|undefined}
 */
export function resolveTargetApplicationForRegression(options, sdDetails) {
  return options?.target_application || sdDetails?.target_application || undefined;
}
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  // SD-LEO-INFRA-SUB-AGENT-REPO-001: fetch SD details (incl. target_application) BEFORE
  // resolving the repo. A CLI invocation never passes options.target_application, so without
  // this the resolver's `candidate` (resolve-repo.js) is always undefined and every CLI run
  // unconditionally falls through to {repoPath:null, repoResolved:false,
  // registrySource:'fallback'} regardless of whether the worktree/registry would actually
  // resolve correctly.
  // SD-LEO-INFRA-SUB-AGENT-REPO-001: this fetch used to run INSIDE the main try block below, so
  // an "SD not found" here was caught by the same handler that resolves with verdict='ERROR' --
  // execute()'s documented contract (`@returns {Promise<Object>}`, no `@throws`) is that it never
  // rejects. Moving getSDDetails earlier (to resolve target_application before resolveSubAgentRepo)
  // must not silently change that contract for any future caller beyond today's CLI tail (which
  // happens to .catch() and still exit(1) either way) -- so this failure path resolves normally too.
  ...
  if (!sdDetails) {
    ...
    results.critical_issues.push({
      severity: 'CRITICAL',
      issue: `Regression validation failed: SD not found: ${sdId}`,
      recommendation: 'Verify the SD id/key is correct and the SD exists in strategic_directives_v2'
    });
    return results;
  }
  ...
  try {
    resolution = await resolveSubAgentRepo({
      sdId,
      targetApplication: resolveTargetApplicationForRegression(options, sdDetails),
      subAgentCode: 'REGRESSION',
      supabase,
    });
    // SD-LEO-INFRA-SUB-AGENT-REPO-001: apply the repo-resolution verdict/metadata ONCE, here,
    // immediately after resolution — not at the end of the function. storeResults() persists
    // `results` further down in this same try block; applying it here (before that persist)
    // is what makes the downgraded verdict and repo_path/conditions/justification metadata
    // actually reach the DB row, instead of being computed on an object that was already
    // written moments earlier.
    applySubAgentRepoVerdict(results, resolution);

    const repoPath = options.repo_path || resolution.repoPath || resolveRepoPath('ehg');
    ...
    if (results.mode === 'capture-baseline') {
      ...
      // applySubAgentRepoVerdict already ran once, immediately after resolution was obtained
      // above — its metadata is already on `results`; no need to re-apply it here.
      return results;
    }
    ...
    if (apiComparison.breaking_changes > 0) {
      ...
      if (results.verdict === 'PASS') {
        ...
        ensureConditionalPassEvidence(results, `${apiComparison.documented_changes} documented API change(s) detected but recorded in the Refactor Brief`);
      }
    } else {
    ...
    if (coverageComparison.decreased && coverageComparison.decrease_percentage > 5) {
      ...
      if (results.verdict === 'PASS') {
        ...
        ensureConditionalPassEvidence(results, `Test coverage decreased by ${coverageComparison.decrease_percentage}% (from ${coverageComparison.baseline_coverage} to ${coverageComparison.current_coverage})`);
      }
  ...
  // applySubAgentRepoVerdict already ran once, immediately after resolution was obtained —
  // its verdict/metadata were already in effect when storeResults() persisted the row above.
  return results;
}
...
async function getSDDetails(sdId) {
  ...
  // SD-LEO-INFRA-SUB-AGENT-REPO-001: normalize sdId (it may be an sd_key, not a UUID) — the
  // `id` column is uuid, so an unnormalized key lookup returns zero rows, which the caller
  // reads as "SD not found" and throws, and (before this fix) meant target_application could
  // never be resolved for a key-based invocation.
  const normalizedId = (await normalizeSDId(supabase, sdId).catch(() => null)) || sdId;

  const { data, error } = await supabase
    .from('strategic_directives_v2')
    .select('id, title, sd_type, intensity_level, status, target_application')
    .eq('id', normalizedId)
    .single();
...
/**
 * SD-LEO-INFRA-SUB-AGENT-REPO-001: pure payload builder, extracted so the previously-silent
 * omission (metadata/conditions/justification never reached the insert) is unit-testable
 * without a DB round-trip or the heavy execute() phase pipeline (baseline capture, test/API/
 * coverage comparison — several of which shell out to real subprocesses, disproportionate to
 * mock for this fix).
 *
 * @param {string|null} normalizedSdId
 * @param {Object} results
 * @param {string|null} phaseValue
 * @returns {Object} the exact row shape storeResults() inserts
 */
export function buildRegressionInsertPayload(normalizedSdId, results, phaseValue) {
  return {
    ...
    phase: phaseValue,
    // this hand-rolled insert previously omitted these three fields entirely, so
    // applySubAgentRepoVerdict's repo_path/repo_resolved metadata never reached the DB,
    // and a CONDITIONAL_PASS downgrade with no conditions/justification violated
    // check_conditions_required/check_justification_required (23514).
    metadata: results.metadata || null,
    conditions: results.conditions || null,
    justification: results.justification || null
  };
}
...
async function storeResults(sdId, results) {
  ...
  try {
    ...
    // SD-LEO-INFRA-SUB-AGENT-REPO-001: supabase-js RETURNS a CHECK-constraint violation
    // (23514) as { error } rather than throwing -- the previous `await ...insert({...})`
    // (no destructured error) meant a rejected row never reached the catch below at all.
    // Destructure and log it explicitly so an insert failure is visible instead of silent.
    const { error: insertError } = await supabase.from('sub_agent_execution_results')
      .insert(buildRegressionInsertPayload(normalizedSdId, results, phaseValue));
    if (insertError) {
      console.log(`   ⚠️  Could not store results: ${insertError.message}`);
    }
  } catch (error) {
