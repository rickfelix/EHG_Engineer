...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    if (step2.found) {
      ...
      console.log(`      Functional Reqs: ${step2.prd.functional_requirements?.length || 0}`);
      console.log(`      Non-Functional Reqs: ${step2.prd.non_functional_requirements?.length || 0}`);
    ...
    if (step3.count === 0) {
      // SD types that don't require backlog items (scope is defined elsewhere)
      const backlogExemptTypes = ['refactor', 'infrastructure', 'documentation', 'docs', 'bugfix'];
      const sdType = step1.sd_type || 'feature';
      const isExempt = backlogExemptTypes.includes(sdType.toLowerCase());

      if (isExempt) {
        console.log(`   ℹ️  No backlog items (OK for ${sdType} SD - scope defined in PRD/files)`);
      } else {
        ...
        // Only reduce confidence for non-exempt SD types
    ...
    if (indexStatus.available) {
      ...
      // No explicit matchThreshold: the default is the MODEL-CALIBRATED floor
      // (SIMILARITY_TIERS in validation-automation.js). This caller used to pin 0.65 —
      // which, after the gemini recalibration, sat ABOVE the measured true-positive band
      // (paraphrase hits at 0.60–0.64), so the ONE production consumer of the fed search
      // kept filtering out everything the recalibration existed to let through
      // (REGRESSION finding f43ecf51, verify-at-the-consumer).
    ...
    if (!step6.found) {
      // Test evidence is required for final approval, but not during EXEC phase
      // Refactor/infrastructure SDs in EXEC phase haven't completed changes yet
      const sdType = step1.sd_type || 'feature';
      const currentPhase = step1.current_phase || 'EXEC';
      const testExemptInExec = ['refactor', 'infrastructure', 'documentation', 'docs'];
      const isExemptInExec = testExemptInExec.includes(sdType.toLowerCase()) && currentPhase === 'EXEC';

      if (isExemptInExec) {
        console.log(`   ℹ️  No test evidence yet (OK for ${sdType} in ${currentPhase} - tests run after changes)`);
      } else {
...
async function querySDMetadata(sdId) {
  const { data: sd, error } = await supabase
    ...
    .select('id, title, status, priority, progress, current_phase, scope, category, target_application, sd_type')
    .eq('id', sdId)
    .single();
