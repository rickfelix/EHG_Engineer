...
function evaluateInfrastructure(sd, venture) {
...
function evaluateSecurity(sd, venture) {
  const result = { score: 6, recommendations: [], blockers: [] };
  const content = `${sd.title} ${sd.scope || ''} ${sd.description || ''}`.toLowerCase();
...
function evaluateQuality(sd, venture) {
...
function evaluateOperations(sd, venture) {
  const result = { score: 6, recommendations: [] };
  const content = `${sd.title} ${sd.scope || ''} ${sd.description || ''}`.toLowerCase();
