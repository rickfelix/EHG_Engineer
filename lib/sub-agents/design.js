  ...
  try {
    ...
    if (options.workflow_review !== false) {
      ...
      try {
        ...
        // First get SD UUID for PRD lookup
        const { data: sdForUuid } = await dbClient
        ...
        const { data: prdData, error: prdError } = await dbClient
          ...
          .eq('sd_uuid', sdForUuid?.uuid_id || sdId)
          .single();
 ...
 */
async function workflowReviewCapability(sdId, prdData, userStories, currentWorkflow) {
  ...
  try {
    // Step 1: Extract workflow from user stories
    ...
    // Step 3: Detect workflow issues
    const issues = detectWorkflowIssues(interactionGraph, currentWorkflow);

    // Step 4: Calculate UX impact score
    ...
    analysis.recommendations = generateWorkflowRecommendations(issues, uxScore, analysis);
...
/**
 * Detect workflow issues (dead ends, circular flows, regressions)
 */
function detectWorkflowIssues(interactionGraph, currentWorkflow) {
  const issues = {
    deadEnds: [],
    circularFlows: [],
    unreachableStates: [],
...
function generateWorkflowRecommendations(issues, uxScore, analysis) {
  ...
  issues.circularFlows.forEach((flow, index) => {
    recommendations.push({
      priority: 'CRITICAL',
      category: 'workflow',
...
function parseBaselineWorkflow(description) {
  ...
  if (routeMatches) {
    workflow.routes = [...new Set(routeMatches)]; // Deduplicate
  }
