...
export function normalizeSdContext(sdContext) {
  ...
  const normalized = {
    ...
    personas: getPersonaRulesForSdType(sdContext.sd_type || sdContext.sdType || 'feature')
  };
...
/**
 * Get persona rules based on SD type
 * @param {string} sdType - The SD type
 * @returns {Object} Persona rules for the SD type
 */
function getPersonaRulesForSdType(sdType) {
  const type = (sdType || 'feature').toLowerCase();
  return SD_TYPE_PERSONA_RULES[type] || SD_TYPE_PERSONA_RULES.feature;
}
