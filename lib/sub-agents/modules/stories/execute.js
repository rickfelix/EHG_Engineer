...
import { generateQualityStoryContent } from './quality-generation.js';
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    if (!userStories || userStories.length === 0) {
      userStories = await createStoriesFromPRD(supabase, sdId, sdKey, options, results);
      if (!userStories) {
        return results;
      }
    }
...
async function createStoriesFromPRD(supabase, sdId, sdKey, options, results) {
  ...
  const prdId = options.prd_id || `PRD-${sdKey}`;
