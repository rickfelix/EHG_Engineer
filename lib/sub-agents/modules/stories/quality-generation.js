 ...
 *
 * Generation Strategy:
 * 1. Try LLM-powered generation for semantic understanding
...
import { createLLMStoryGenerator, isLLMAvailable } from './llm-story-generator.js';
 ...
 */
export async function generateQualityStoryContentWithLLM(criterion, prd, index, _options = {}) {
  const generator = getLLMGenerator();

  // Try LLM generation first
  if (generator && generator.isEnabled()) {
    try {
      const llmResult = await generator.generateSingleStory(criterion, prd, index);

      if (llmResult) {
        // Validate LLM output against forbidden personas
        const userRole = validateUserRole(llmResult.user_role);
 ...
 */
export async function generateStoriesBatch(acceptanceCriteria, prd, options = {}) {
  const generator = getLLMGenerator();

  // Try LLM batch generation first
  if (generator && generator.isEnabled()) {
    const result = await generator.generateStoriesFromCriteria(acceptanceCriteria, prd, options);

    if (result.success && result.stories.length > 0) {
      // Validate and enrich LLM stories
      const validatedStories = result.stories.map((story, i) => ({
        ...story,
        user_role: validateUserRole(story.user_role),
        generated_by: 'LLM'
      }));
...
function validateUserRole(role) {
  ...
  // Check against forbidden personas
  if (isForbiddenPersona(role)) {
