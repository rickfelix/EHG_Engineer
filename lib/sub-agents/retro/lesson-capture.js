...
export async function captureLessonLearned(supabase, sdId, sdData, options, results) {
  ...
  if (existingRetros && existingRetros.length > 0) {
    ...
    // in retro_canonical_writer_policy() as 'retro_lesson_capture'.
    const { data, error } = await updateRetrospectiveWithToken(
      (payload) => supabase.from('retrospectives').update(payload).eq('id', existing.id).select().single(),
      {
        key_learnings: updatedLearnings,
        what_needs_improvement: updatedImprovements,
        updated_at: new Date().toISOString()
      },
      'retro_lesson_capture'
    );
