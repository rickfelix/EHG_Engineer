...
describe('FR-3/TS-5: enhanceRetrospective preserves existing content it does not explicitly replace', () => {
  ...
  it('preserves existing scalar field values when newRetro omits them (preserve-on-absence)', async () => {
    const existing = {
      id: 'r-1', sd_id: SD_ID, status: 'DRAFT', retro_type: null,
      generated_by: 'SUB_AGENT', key_learnings: THIN_LEARNINGS,
      what_went_well: [], what_needs_improvement: [], action_items: [],
      title: 'x', description: 'y',
      on_schedule: false, within_scope: true, team_satisfaction: 8,
    };
    const sb = fakeSupabase(existing);
    // newRetro omits on_schedule/within_scope/team_satisfaction entirely.
    const sparseNewRetro = { ...newRetro };
    delete sparseNewRetro.on_schedule; delete sparseNewRetro.within_scope; delete sparseNewRetro.team_satisfaction;
