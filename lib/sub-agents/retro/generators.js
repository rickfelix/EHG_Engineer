...
export function generateRetrospective(sdData, prdData, handoffs, subAgentResults, _options, testEvidence = null, deliverables = null, storyCoverage = null) {
  ...
  // QF-20260912-856: this item was pushed as a bare STRING while every other entry in
  // actionItems is a SMART object (action/owner/deadline/success_criteria/priority/
  // smart_format/source) -- formatActionItems() renders a string with none of that
  // context, so it read as generic boilerplate to the specificity grader and dragged
  // RETROSPECTIVE_QUALITY_GATE's score down (measured: 39-43/100) even when every other
  // item was well-formed. Shape it like the rest of the array instead.
  if (protocolImprovements.length > 0) {
    const categories = [...new Set(protocolImprovements.map(i => i.category))];
    actionItems.push({
      action: `Address ${protocolImprovements.length} protocol finding(s) in: ${categories.join(', ')}`,
      owner: 'LEO Protocol Team',
      deadline: 'Next SD cycle',
      success_criteria: `${categories.join(', ')} validation includes these findings`,
      priority: 'medium',
      smart_format: true,
      source: 'protocol_improvement_summary',
    });
  }
...
function extractSubAgentInsights(results, sdKey) {
  ...
  for (const [agentCode, agentResults] of Object.entries(byAgent)) {
    ...
    const _latestResult = agentResults[agentResults.length - 1];
 ...
 */
function extractFutureEnhancements(sdData, prdData, _subAgentResults) {
  const enhancements = [];
  const sdKey = sdData.sd_key || sdData.id?.substring(0, 8);
