...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    //
    // QF-20260903-529: sd_id/sub_agent_results/deliverables columns are always UUIDs,
    // but execute()'s documented call form (every usage example in execute-subagent.js)
    // passes an sd_key. sdData.id is the UUID gatherSDMetadata already resolved above
    // (accepting either form) -- filtering these three on the raw sdId instead silently
    // returned zero rows for a key-form caller, with no error to surface the mismatch.
    ...
    if (deliverables.found) {
      ...
      // QF-20260903-529: this used to assert "(legacy SD)" as the cause of an empty
      // result the code never established -- a reader given a plausible reason stops
      // looking, even when the true cause is a query bug. State the empty fact only.
      console.log('   ℹ️  No scope deliverables found for this SD');
    }
    ...
    if (!options.dry_run) {
      ...
      if (stored.success) {
        ...
        // SD-LEO-INFRA-WIRE-FEEDBACK-QUALITY-001: Resolve feedback when SD completes
        if (sdData.status === 'completed' || options.isCompletion) {
          const resolution = await resolveFeedbackForCompletedSD(supabase, sdId);
          if (resolution.resolved > 0) {
            results.recommendations.push(`Resolved ${resolution.resolved} feedback item(s) from completed SD`);
          }
        }
      } else {
