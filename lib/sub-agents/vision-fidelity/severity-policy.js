...
const SD_TYPE_POLICIES = {
  ...
  // SD-LEO-INFRA-FIX-JOURNEY-WALK-001 FR-2: an orchestrator parent's own PRD is a
  // delegated-completion PRD (no product acceptance_criteria) and it never carries a
  // branch_name (measured: 0 of 5955 SDs system-wide), so loadPRD()/readGitDiff() always
  // starve this gate of real implementation evidence for orchestrators -- a strict block
  // fabricates a FAIL from zero evidence, not a real product gap. warn, not skip: the
  // children-completeness check upstream already gates real delivery; this just stops the
  // vision-fidelity layer from independently blocking on evidence it structurally cannot see.
  orchestrator:   { mode: 'warn' }
};
