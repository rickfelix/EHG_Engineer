...
describe('severity-tier policy (FR-3)', () => {
  ...
  it('SD-LEO-INFRA-FIX-JOURNEY-WALK-001 FR-2: orchestrator with high counts → never blocks (warn-only, same as infrastructure)', () => {
    const o = classifyOutcome({ sdType: 'orchestrator', criticalMissing: 7, nonCriticalMissing: 99 });
    expect(o.passed).toBe(true);
    expect(o.verdict).toBe('WARNING');
    expect(o.mode).toBe('warn');
  });

  it('SD-LEO-INFRA-FIX-JOURNEY-WALK-001 FR-2: a non-orchestrator type is unaffected -- feature with the same counts still FAILs', () => {
    const o = classifyOutcome({ sdType: 'feature', criticalMissing: 7, nonCriticalMissing: 99 });
    expect(o.passed).toBe(false);
    expect(o.verdict).toBe('FAIL');
  });
