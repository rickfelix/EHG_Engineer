...
export class APIRelevanceClassifier {
    ...
    if (!this.llmClient) {
      try {
        this.llmClient = await getLLMClient({
          purpose: 'api-relevance-classification',
          subAgent: 'API',
          phase: 'PLAN'
        });
      } catch (_error) {
        console.warn('LLM client unavailable - API relevance will fall back to keyword detection');
        return null;
      }
    }
  ...
  async classify(sd, prd = null) {
    // Get LLM client (may return null if unavailable)
    const llmClient = await this.getLLMClient();

    // If LLM not available, fall back to keyword detection
    if (!llmClient) {
      return this.keywordFallback(sd, prd);
    }

    try {
      const result = await this.callOpenAI(sd, prd, llmClient);
   ...
   */
  async callOpenAI(sd, prd, llmClient) {
...
- Calling third-party APIs as a CLIENT (Stripe, OpenAI, Twilio)
...
- "Integrate with Stripe payments API" → consumes=true, should_run=false (client-side consumption)
- "Wire event bus to workflow stage transitions" → consumes=true, should_run=false (internal service wiring, not HTTP endpoint changes)
- "Hook advanceStage() to trigger notifications at designated stages" → consumes=true, should_run=false (lifecycle hooks, not endpoint changes)
...
Should the API sub-agent review this SD?`;
    ...
    // Need higher token limit to accommodate reasoning + JSON output
