 ...
 *
 * Activation: Conditional (AI-powered relevance classification)
...
import { getAPIRelevanceClassifier } from './api-relevance-classifier.js';

// SD-LEO-INFRA-FLEET-WIDE-SUB-001: cross-repo-aware helper threads target_application's
// repo into scanAPIFiles so API sub-agent scans the SD's actual repo, not always EHG.
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  // Get validation mode from options (default to retrospective for EXEC-TO-PLAN handoffs)
  const validationMode = options.validation_mode || 'retrospective';
  ...
  const results = {
    ...
    blockers: [],
    warnings: [],
    // Required for CONDITIONAL_PASS verdict (database constraint)
    justification: null,
    conditions: null
  };

  // SD-LEO-INFRA-FLEET-WIDE-SUB-001 FR-2: resolve target_application via shared helper.
  // Hoisted outside try so applySubAgentRepoVerdict on both return paths can emit
  // metadata.repo_path top-level (the gate's contract per FR-2).
  let resolution = null;
  try {
    ...
    // ============================================
    // 2. AI-POWERED RELEVANCE CHECK
    // ============================================
    console.log('\n🤖 Step 2: AI-powered relevance classification...');
    const classifier = getAPIRelevanceClassifier();
    const classification = await classifier.classify(sd, prd);

    // Store classification in results for transparency
    results.findings.classification = {
      method: classification.classification_method,
      creates_api_endpoints: classification.creates_api_endpoints,
      modifies_api_endpoints: classification.modifies_api_endpoints,
      consumes_external_api: classification.consumes_external_api,
      confidence: classification.confidence,
      reasoning: classification.reasoning
    };

    if (!classification.should_run_api_subagent) {
      console.log('   ℹ️  AI determined this SD does not create/modify API endpoints - skipping review');
      console.log(`      Reason: ${classification.reasoning}`);
      results.verdict = 'PASS';
      results.confidence_score = 100;
      results.summary = `API sub-agent not applicable: ${classification.reasoning}`;
      if (resolution) applySubAgentRepoVerdict(results, resolution);
      return results;
    }

    console.log(`   ✓ API relevance confirmed (${classification.confidence}% confidence)`);
    console.log(`   ✓ API Type: ${classification.api_type}`);

    // Build apiInfo from classification for downstream compatibility
    const apiInfo = {
      hasAPI: true,
      type: classification.api_type,
      endpoints: [] // Will be populated by codebase scan
    };
    ...
    // SD-LEO-INFRA-FLEET-WIDE-SUB-001: thread resolved repo through scanAPIFiles for cross-repo SDs
    const apiFiles = await scanAPIFiles(resolution?.repoPath);
    ...
    if (results.blockers.length > 0) {
      ...
      // CONDITIONAL_PASS requires justification and conditions (database constraint)
      // Build justification from scores and context
      const scoreBreakdown = `Design: ${designScore.score}/10, Performance: ${performanceScore.score}/10, Security: ${securityScore.score}/10, Documentation: ${documentationScore.score}/10`;
      results.justification = `API architecture meets minimum acceptable standards (${avgScore.toFixed(1)}/10 average). ${scoreBreakdown}. Recommendations are non-blocking improvements that should be addressed in future iterations.`;

      // Convert recommendations to actionable conditions
      results.conditions = results.recommendations.slice(0, 5).map(rec => ({
        action: rec,
        priority: 'low',
        blocking: false
      }));

      // Ensure conditions is non-empty (database constraint)
      if (results.conditions.length === 0) {
        results.conditions = [{
          action: 'Review API design for potential improvements',
          priority: 'low',
          blocking: false
        }];
      }
 ...
 */
function _detectAPIType(sd, prd) {
 ...
 */
async function scanAPIFiles(repoPath) {
  const apiFiles = [];
  // SD-LEO-INFRA-FLEET-WIDE-SUB-001: accept caller-resolved repoPath (from resolveSubAgentRepo
  // in execute()); fall back to EHG default for legacy direct invocations.
  const ehgPath = repoPath || resolveRepoPath('ehg');
 ...
 */
async function evaluateAPIDesign(apiFiles, apiInfo, _prd) {
 ...
 */
async function evaluateAPIPerformance(apiFiles, _apiInfo) {
 ...
 */
async function evaluateAPISecurity(apiFiles, apiInfo, _prd) {
 ...
 */
async function evaluateAPIDocumentation(apiFiles, _apiInfo) {
  const score = { score: 7, recommendations: [] };
