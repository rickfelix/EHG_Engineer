...
function evaluateSalesProcess(sd, _venture) {
...
function evaluateMetrics(sd, _venture) {
  const result = { score: 6, recommendations: [] };
  const content = `${sd.title} ${sd.scope || ''} ${sd.description || ''}`.toLowerCase();
...
function evaluateEnablement(sd, _venture) {
  const result = { score: 6, recommendations: [] };
  const content = `${sd.title} ${sd.scope || ''} ${sd.description || ''}`.toLowerCase();
...
`;
  ...
  return {
    type: ARTIFACT_TYPES.IDENTITY_GTM_SALES_STRATEGY,
    content,
