...
async function checkInputValidation(repoPath) {
  try {
    ...
    // Pattern: string concatenation with + in query context (not arithmetic)
    // Filter: exclude comments, strings, parameterized patterns
    const { stdout: sqlConcat } = await execAsync(
      `cd "${repoPath}" && grep -r "query.*+=\\|sql.*+=" src 2>/dev/null | ` +
      'grep -v "test.js\\|test.tsx\\|__tests__\\|//" | wc -l'
    );

    // Check for eval usage (actual XSS risk)
    // Pattern: eval() function calls (not dangerouslySetInnerHTML, which has valid use cases)
    // Filter: exclude comments and test files
