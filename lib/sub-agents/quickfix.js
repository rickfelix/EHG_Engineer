...
const EHG_APP_PATH = resolveRepoPath('ehg');
...
function verifyApplicationContext(issue) {
  ...
  // eslint-disable-next-line no-process-cwd-in-sub-agents -- LEGITIMATE: verifyApplicationContext compares the operator's actual working directory against expected target to warn about wrong-repo edits. Must be runtime cwd, not target_application path.
  const cwd = process.cwd();
  if (result.targetApp === 'ehg' && cwd.includes('EHG_Engineer')) {
    result.warning = `⚠️  Implementation should happen in ${EHG_APP_PATH}, not ${EHG_ENGINEER_PATH}`;
  } else if (result.targetApp === 'ehg_engineer' && !cwd.includes('EHG_Engineer')) {
    result.warning = `⚠️  This fix applies to EHG_Engineer, but you're in ${cwd}`;
  }
...
export async function execute(sdId, subAgent, options = {}) {
  ...
  try {
    ...
    console.log('📍 Application Context Verification\n');
    ...
    console.log(`   Path Verified: ${appContext.verified ? '✅ YES' : '❌ NO'}`);
    ...
    console.log('🔍 Step 0: Root Cause Analysis - Pattern Detection\n');
    ...
    if (patternAnalysis.isSystemic) {
      console.log('\n   ⚠️  SYSTEMIC PATTERN DETECTED - Recommending escalation\n');
      results.warnings.push({
        severity: 'HIGH',
    ...
    if (consoleError) {
      console.log('📸 Capturing Console Error Baseline\n');
