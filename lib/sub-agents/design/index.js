...
async function checkForNonUISdType(sdId, options) {
  try {
    ...
    return {
      ...
      // SD-LEO-INFRA-FLEET-WIDE-SUB-001 FR-6 fix: previously omitted repo_path entirely,
      // causing 100% of post-merge infra DESIGN rows to classify EXPLICIT_NULL/LEGACY at
      // the fleet gate (RCA-FR6-DESIGN-METADATA-DROP-2026-05-26). Thread the resolved
      // repo_path through so the metadata contract is honored on this fourth return path too.
      metadata: {
        repo_path: resolveDesignRepo(options).resolvedRepoPath,
        repo_resolved: true,
        components_dir_exists: true,
        skip_reason: `non_ui_sd_type:${effectiveType}`
      },
      options
    };
  } catch (error) {
