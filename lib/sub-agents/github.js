...
export async function execute(sdId, subAgent, options = {}) {
  ...
  const relaxedCiSdTypes = ['database', 'infrastructure', 'documentation', 'protocol'];
  ...
  if (relaxedCiSdTypes.includes(sdCategory)) {
    ...
    try {
      const { stdout: prList } = await execAsync(`gh pr list --state all --search "${sdId}" --json number,state,title 2>/dev/null || echo "[]"`);
