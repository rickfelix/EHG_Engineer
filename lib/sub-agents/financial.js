 ...
 */
function evaluateBMCCompleteness(sd, _venture) {
  const result = {
    score: 0,
    recommendations: [],
    blockers: [],
 ...
 */
function evaluateCostStructure(sd, _venture, _benchmarks) {
 ...
 */
function evaluateFinancialViability(sd, _venture, _benchmarks) {
  const result = { score: 7, recommendations: [], blockers: [] };

  const content = `${sd.title} ${sd.scope || ''} ${sd.description || ''}`.toLowerCase();
  const _metadata = sd.metadata || {};
...
`;
  ...
  return {
    type: ARTIFACT_TYPES.ENGINE_BUSINESS_MODEL_CANVAS,
    content: content,
