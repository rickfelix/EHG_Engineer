...
async function analyzeMemory(repoPath) {
  try {
    ...
    const { stdout: missingCleanup } = await execAsync(
      `cd "${repoPath}" && grep -r "useEffect" src 2>/dev/null | grep -v "return () =>" | wc -l`
    );

    // Event listeners without cleanup
    const { stdout: unboundListeners } = await execAsync(
      `cd "${repoPath}" && grep -r "addEventListener" src 2>/dev/null | grep -v "removeEventListener" | wc -l`
    );

    const memoryLeaks = parseInt(missingCleanup.trim()) + parseInt(unboundListeners.trim());

    return {
      checked: true,
      memory_leaks: memoryLeaks > 5 ? 1 : 0, // Only flag if significant number
      missing_cleanup: parseInt(missingCleanup.trim()),
      unbound_listeners: parseInt(unboundListeners.trim()),
      leak_patterns: memoryLeaks > 5 ? [
        `${missingCleanup.trim()} useEffect hooks may need cleanup`,
        `${unboundListeners.trim()} event listeners may need removeEventListener`
      ] : []
    };
  } catch (error) {
    return {
