...
// SD-LEO-FEAT-AUTHOR-VENTURE-DESIGN-001: 'fable' added — the design-generation purpose
// (claude-fable-5) is a heavy generation tier; without this it fell to the SHORT default
// (observed: resolvedTimeoutMs=30000 reason=default on the live smoke).
const LONG_TIMEOUT_MODEL_PATTERN = /(?:^|[-/])(?:pro|opus|o1|o3|thinking|fable)\b/i;
...
export class GoogleAdapter {
  ...
  constructor(options = {}) {
    ...
    // Cost control (2026-06-05): honor the configured/env default (Flash) instead of
    // hardcoding gemini-2.5-pro, so a bare `new GoogleAdapter()` no longer bypasses
    // the cheap default. Callers needing Pro pass options.model explicitly.
    this.defaultModel = options.model || getGoogleModel('validation');
    this.model = this.defaultModel;
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const generationConfig = {
          // Cost control (2026-06-05): cap gate/verdict outputs at 4096 (measured gate
          // outputs average <1K tokens); content-generation keeps the larger ceiling.
          maxOutputTokens: options.maxTokens || options.max_tokens || (options.purpose === 'content-generation' ? 16384 : 4096)
        };
        ...
        if (isGemini3x) {
          // QF-20260818-343: was `if (effortLevel && isGemini3x)` — a caller with no
          // effortLevel fell through this whole block with NO thinkingConfig set at all.
          // Gemini 3.x has no thinking-OFF state (min=LOW; unlike 2.5's thinkingBudget:0,
          // see GEMINI_THINKING_LEVELS above), so an unset config leaves the server-side
          // default in effect — measured truncating a 4096-token response on a simple
          // validation-role prompt. Defaulting to 'low' here is the same cost-conscious
          // posture the isGemini2_5 flash branch below already applies; existing callers
          // that already pass an explicit effortLevel are unaffected (still resolved first).
          const family = model.includes('flash') ? 'flash' : 'pro';
          const levelMap = GEMINI_THINKING_LEVELS[family] || GEMINI_THINKING_LEVELS.pro;
          generationConfig.thinkingConfig = {
            thinkingLevel: levelMap[effortLevel || 'low'] || 'low'
          };
        } else if (isGemini2_5) {
          // Gemini 2.5: thinking tokens are billed as OUTPUT and share the
          // maxOutputTokens budget. They were ~60% of the June cost spike.
          const isFlash = model.includes('flash');
          if (isFlash) {
            // COST CONTROL (2026-06-05): Flash is the high-volume cheap tier — disable
            // thinking by default so 100% of billed output is content, not hidden thoughts.
            // Opt back in per-deployment with GEMINI_FLASH_THINKING_BUDGET=<tokens>.
            const envBudget = Number.parseInt(process.env.GEMINI_FLASH_THINKING_BUDGET ?? '', 10);
            const flashBudget = Number.isFinite(envBudget) && envBudget >= 0 ? envBudget : 0;
            generationConfig.thinkingConfig = { thinkingBudget: flashBudget };
          } else {
            // Pro (reserved 'reasoning' tier): keep a capped thinking budget by effort.
            // Pro minimum is 128; never exceed 50% of maxOutputTokens so content has room.
            const budgetMap = { low: 512, medium: 2048, high: 8192 };
