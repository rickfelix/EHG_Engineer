...
import { getClaudeModel, getOpenAIModel, getGoogleModel } from '../../config/model-config.js';
...
export function createValidatedDebateConfig(proposerModel, options = {}) {
  ...
  const defaultEvaluators = [
    { persona: 'safety', model: getClaudeModel('validation') },
    { persona: 'value', model: getOpenAIModel('validation') },
    { persona: 'risk', model: getGoogleModel('validation') }
  ];
