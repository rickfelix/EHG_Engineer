...
const { isAutoStartableQF, isClaimableWithVerify } = require('./qf-auto-start.cjs');
...
//
// normalizeLane/requireResolvableLane/assertLaneUnambiguous were EXTRACTED to
// lib/fleet/lane-scope.cjs by SD-LEO-INFRA-ONE-BELT-CENSUS-001 (implementation step 1a) so
// lib/fleet/belt-census.cjs can share the exact same lane-scope logic instead of duplicating it.
// This file's own behavior, consumers, and exports are unchanged — see lane-scope.cjs for the
// full rationale docblocks (the "SAFE BY COINCIDENCE" ambiguity-guard note in particular).
...
// isAutoStartableQF now reads (see qf-auto-start.cjs). Long-established column, no fallback needed.
const AUTO_START_CANDIDATE_COLUMNS = 'id, status, pr_url, commit_sha, created_at, routing_tier, title, description, not_before, factory_lane, owner, release_condition, target_application, compliance_details';
...
async function fetchAutoStartCandidateRows(supabase, scope) {
  ...
  // factory_lane's) that can be absent from the SAME live table factory_lane is absent from --
  // both are missing today. This is a SEPARATE probe stage, layered AFTER the factory_lane
  // resolution above, so it never re-decides factory_lane's own inclusion -- it only ever adds
  // ", verified_at" onto whatever `columns` that resolution already settled on.
  const columnsWithVerifiedAt = `${columns}, verified_at`;
  const verifiedAtProbe = await applyFilters(supabase.from('quick_fixes').select(columnsWithVerifiedAt)).limit(1);
  if (verifiedAtProbe.error && verifiedAtProbe.error.code !== '42703') throw verifiedAtProbe.error;
  if (!verifiedAtProbe.error) {
    if (!Array.isArray(verifiedAtProbe.data)) {
      throw new Error(`countAutoStartableQuickFixes: verified_at probe returned data=${JSON.stringify(verifiedAtProbe.data)} with no error — refusing to treat an unreadable quick_fixes table as empty`);
    }
    columns = columnsWithVerifiedAt;
  }
  // else: verified_at genuinely absent (42703) -- `columns` stays at whatever the factory_lane
  // resolution above left it at. isAutoStartableQF degrades safely when qf.verified_at is
  // undefined (falls back to created_at-only; see lib/fleet/qf-auto-start.cjs).
...
// QF-20260821-032: COMPLEMENT of countAutoStartableQuickFixes on the staleness axis only --
// "verify-gated" supply, reported separately so a deficit verdict can tell it apart from
// genuinely-empty supply. Same candidate rows/fail-loud contract; deliberately not summed in.
async function countClaimableWithVerifyQuickFixes(supabase, scope) {
  if (scope !== undefined && scope !== null) {
    requireResolvableLane(scope);
    await assertLaneUnambiguous(supabase, 'quick_fixes', scope);
  }
  const rows = await fetchAutoStartCandidateRows(supabase, scope);
  const nowMs = Date.now();
  return (rows || []).filter((row) => isClaimableWithVerify(row, nowMs)).length;
}
...
module.exports = { countDispatchableBacklog, countClaimableQuickFixes, countAutoStartableQuickFixes, countClaimableWithVerifyQuickFixes, countBeltDepth, ELIGIBILITY_COLUMNS };
