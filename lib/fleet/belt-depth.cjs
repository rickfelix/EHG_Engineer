...
/**
 * QF-20260821-032: the COMPLEMENT of countAutoStartableQuickFixes on the staleness axis only —
 * QFs that clear every other auto-start gate but are held solely because they haven't cleared a
 * disposition-sweep re-verify yet. Reports "verify-gated" supply separately from "genuinely
 * empty" supply so a deficit verdict can tell them apart. Same candidate rows, same fail-loud
 * contract as countAutoStartableQuickFixes; deliberately NOT summed into it.
 *
 * @param {object} supabase
 * @returns {Promise<number>} count of quick_fixes rows isClaimableWithVerify admits right now
 */
