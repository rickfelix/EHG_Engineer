 ...
 *
 *  SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001: line below ALWAYS passes tiering_active:true to the gate
 *  (an explicit per-SD floor is an author reservation honored even when the GLOBAL tiering switch
 *  is off) -- that is a DIFFERENT lever from TIER_FLOOR_PROVENANCE_ADVISORY, which governs whether
 *  a provenance-free floor blocks at all. Do not conflate them: TIER_FLOOR_PROVENANCE_ADVISORY=off
 *  restores always-binding-regardless-of-provenance; it does not touch this hardcoded true. A
 *  provenance-free 'above_worker_tier_advisory' verdict matches neither string below and correctly
 *  falls through unblocked. */
