...
/**
 * Tier-axis dispatch enforcement retired fleet-wide (SD-FDBK-INFRA-RETIRE-SEAT-TIER-001, chairman
 * ratification 20dc072b). classifyDispatchIneligibility never emits 'above_worker_tier' or
 * 'tier_stamp_missing' as of this SD (the branches that produced them were deleted from
 * claim-eligibility.cjs's tierAxes), so this always returns false. Retained (not deleted) as a
 * no-op for call-site/export compatibility with claimableForTier below and its existing test
 * suite (tests/unit/fleet/tier-claimable-floor.test.js), which already documents/asserts this
 * always-false contract per the prior retirement step, QF-20260831-419.
 */
function tierBlocks() {
  return false;
}
...
/**
 * FR-2 (SD-LEO-INFRA-BELT-CLAIMABLE-ACCURACY-FLOOR-001): the repo analog of the tier axis. Of the
 * tier-reachable set, how many can a worker checked out in `cwd` actually claim FROM THIS REPO —
 * i.e. not cross-repo (target_application pinned to another checkout) and premise/precondition-open.
 * A repo-pinned EHG_Engineer worker otherwise sees a non-zero fleet-wide belt_claimable_at_my_tier
 * that is entirely MarketLens/venture SDs it cannot act on, and idles while the gauge shows work.
 *
 * REUSE, DON'T RE-DERIVE (TR-1): the repo/premise/precondition axes are the gate's — pass `cwd` so
 * classifyDispatchIneligibility applies them; count only rows for which it returns null (fully
 * claimable from this checkout). Fail-open: a classifier fault or absent cwd never under-reports.
 * @param {object[]} pool
 * @param {{ workerTierRank?: number, tieringActive?: boolean, cwd?: string, currentApp?: string, preFiltered?: boolean, depSatisfied?: Function }} opts
 * @returns {object[]}
 */
function claimableForRepo(pool, opts = {}) {
  const { workerTierRank, tieringActive, cwd, currentApp, preFiltered, depSatisfied } = opts;
  const tierClaimable = claimableForTier(pool, { workerTierRank, tieringActive, preFiltered, depSatisfied });
  if (!cwd && !currentApp) return tierClaimable; // no repo context => do not narrow (fail-open to tier count)
  return tierClaimable.filter((sd) => {
    try { return classifyDispatchIneligibility(sd, { cwd, currentApp }) === null; }
    catch { return true; } // classifier fault never narrows the repo count
  });
}
