...
export async function scanRecentQfMintsForBatches(supabaseClient, { lookbackMs = 60 * 60 * 1000, nowMs = Date.now() } = {}) {
  // Always paginated to completion — a QF mint-volume read has no natural cap, and an
  // unbounded/capped read here would silently under-detect a genuine batch (count-truncation
  // discipline, mirrors the neighbouring hold-state-overdue/off-canonical-mint-gauge detectors).
