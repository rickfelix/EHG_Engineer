...
/** Same instant, regardless of spelling ('...Z' vs '...+00:00') -- PostgREST returns a
 *  timestamptz with a numeric offset, never 'Z', so a string comparison against our own
 *  toISOString() output always disagrees even on a byte-identical persisted value. */
...
async function recordUsagePaste(reading, opts = {}) {
  ...
  const { data: readback, error: readErr } = await opts.supabase
    ...
    .eq('id', inserted.id)
    .single();
  if (readErr || !readback || readback.account_uuid8 !== row.account_uuid8 || !sameInstant(readback.pasted_at, row.pasted_at)) {
...
module.exports = { recordUsagePaste, sanitizePromoNote, sameInstant, TABLE, MAX_PROMO_NOTE_LENGTH };
