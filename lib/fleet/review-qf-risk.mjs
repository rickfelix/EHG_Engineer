...
export async function reviewQfRisk(supabase, { qfId, verdict, confidence, reasoning, repoPath, executedFromCwd }) {
  ...
  // That id is truthy, so stampQfRiskReviewed's `!subAgentRowId` guard alone would let a stamp
  // be written citing evidence that was never actually recorded -- exactly what ratification
  // 6c263823 forbids, triggerable by a plain DB timeout, no adversary required. Bail instead.
  if (stored?.storage_timeout) {
    return { ok: false, stamped: false, error: 'SECURITY review row failed to persist (storage timeout) -- refusing to stamp against unrecorded evidence' };
  }
