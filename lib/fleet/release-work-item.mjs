 ...
 *   added it to prevent.
 *
 * FLAGGING: the per-site conversion (FR-1b) sits behind LEO_RELEASE_WORKITEM_RESET — see
 *   isReleaseWorkItemResetEnabled. QF-20260912-961: default flipped ON (every wired site's
 *   behavior below was already resolved); 'off' is the explicit kill switch.
 *
...
/**
 * FR-1b gate. QF-20260912-961 (part a): default flipped ON — the sixteen-site ledger above
 * (WIRED (5) / EXEMPT (7) / NO-OP (4)) already resolved every call site's correct behavior;
 * leaving the reset OFF by default in the fleet's own .env meant every hand-back path that
 * calls releaseWorkItemOnSessionEnd was a documented no-op, which is exactly the orphaned-QF
 * class this ticket measured (QF-20260911-553/117 reopened by hand). 'off' is kept as the
 * explicit kill switch: any value other than the literal string 'off' enables the reset,
 * so an unset/absent value (the common case) is ON, not a silent no-op.
 */
export function isReleaseWorkItemResetEnabled(env = process.env) {
  return env.LEO_RELEASE_WORKITEM_RESET !== 'off';
}
