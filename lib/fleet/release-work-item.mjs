...
export async function releaseWorkItemOnSessionEnd(supabase, workItemKey, reason, opts = {}) {
  ...
  try {
    ...
    if (worktreePath) {
      ...
      // which is the question actually being asked: does this branch carry work someone would lose?
      ...
      if (probe.count > 0) {
        onLog(`SKIP_RESET: ${workItemKey} — ${reason} — ${probe.count} unpushed commit(s)`);
        return { ok: true, kind: 'sd', action: 'sd_no_reset', reason, detail: `${probe.count} unpushed commit(s) — real work, not reverted` };
      }
    }
