...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: the loop reads the exported list instead of a
// hard-coded ['adam','solomon'] pair, so a role added to the list is refused-tested automatically
// (TESTING evidence 629fc7e3: the hard-coded pair stayed green and blind when a role was missing).
describe('singleton role lists (SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A)', () => {
  it('michael is a refusable singleton beside adam and solomon; coordinator stays replaceable', () => {
    expect([...REFUSABLE_SINGLETON_ROLES].sort()).toEqual(['adam', 'michael', 'solomon']);
    expect([...SINGLETON_ROLES].sort()).toEqual(['adam', 'coordinator', 'michael', 'solomon']);
    expect(isSingletonRole('michael')).toBe(true);
  });
});

describe('FR4-REFUSE: a second Adam, Solomon or Michael is refused AT THE ROUTE, holder named', () => {
  for (const role of REFUSABLE_SINGLETON_ROLES) {
