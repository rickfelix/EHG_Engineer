 ...
 * @returns {Promise<{content: string|null, torn: boolean}>}
 */
...
async function mirrorSeat(supabase, claudeDir, seatName, liveHeartbeatBySuffix = new Map()) {
  try {
    ...
    const { content, torn } = await readStableFile(chosen);
...
module.exports = { pickNewestCandidate, readStableFile, writeSeatCheckpoint, mirrorSeat, TORN_READ_GAP_MS };
