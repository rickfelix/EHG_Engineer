...
/**
 * QF-20260831-587 — cap-recovery: a self-escalation this seat raised on ITSELF stays open
 * forever once the seat recovers (2026-08-31 08:02-09:53Z blackout: 8 empty-body broadcast
 * stuck signals aged into a false 96-minute SLA breach; the coordinator cleared them by hand).
 * RECOVERY PATHS MUST CLEAR WHAT OUTAGE PATHS EMIT. Pure — mirrors shouldSelfEscalate's shape
 * so the same fixtures cover both directions: escalate while stuck-on-the-SAME-deadline,
 * clear the instant that deadline is no longer the live overdue one (re-armed, or resolved).
 * @param {object} session - a claude_sessions row (needs last_tool_at, loop_state, metadata)
 * @param {number} nowMs - current time in ms (injectable clock)
 * @param {number} cutMinutes - see shouldSelfEscalate
 * @returns {{shouldClear: boolean, priorEscalatedForWakeAt: string|null}}
 */
function shouldClearSelfEscalation(session, nowMs, cutMinutes) {
  const priorEscalatedForWakeAt = (session && session.metadata && session.metadata.self_escalated_for_wake_at) || null;
  if (!priorEscalatedForWakeAt) return { shouldClear: false, priorEscalatedForWakeAt: null };
  const verdict = classifySeat(session, { cutPointMinutes: cutMinutes, now: nowMs });
  const stillStuckOnSameDeadline = verdict.verdict === VERDICT.STUCK && verdict.wake && verdict.wake.state === 'armed_overdue';
  return { shouldClear: !stillStuckOnSameDeadline, priorEscalatedForWakeAt };
}
...
module.exports = { shouldSelfEscalate, shouldClearSelfEscalation, buildSelfEscalationRow };
