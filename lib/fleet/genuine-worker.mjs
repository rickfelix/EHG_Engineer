 ...
 *
 * THREE alarm mechanisms (QF-20260904-650 engagement classifier, QF-20260904-790 outbound-
 * silence watchdog, and a fourth still-undiscovered reader FR-7 exists to catch) each read
 * heartbeat_at as a loop-liveness proxy — and a dead loop keeps heartbeating, because
 * heartbeat_at is stamped by a daemon separate from the loop itself. Rather than have every
 * reader re-derive its own composition of isKnownWedged/classifySeat (and re-introduce this
 * exact defect class a fourth time), this is ONE place that does it.
 *
 * TWO EXPORTS, DELIBERATELY, NOT ONE WITH A MODE FLAG — because they answer for DIFFERENT
 * consumers who must never be forced through the other's contract:
 *
 *   - classifyLoopLiveness (below): FAIL-OPEN, for ALARM consumers (engagement gauges,
 *     watchdogs, pagers). A false DEAD-LOOP costs a spurious alert; a false LIVE costs a
 *     missed outage. isKnownWedged's existing fail-open design is exactly right here.
 *   - classifyLoopLivenessStrict (below): NON-FAIL-OPEN, for ACTUATION consumers.
 *     lib/coordinator/adam-identity.cjs, michael-identity.cjs, and solomon-identity.cjs each
 *     carry an explicit comment that isKnownWedged must NEVER be used for singleton
 *     role-election because a fail-open verdict there would let a genuinely-dead incumbent
 *     keep "winning" the election forever. This export exposes classifySeat's own
 *     STUCK/HEALTHY/UNKNOWN verdict UNCHANGED, so an actuation consumer gets the raw signal,
 *     never a laundered "definitely live" answer.
 *
 * Both are pure and additive: neither changes isKnownWedged/liveFleetWorkers/classifySeat, and
 * neither is wired into any existing consumer by this SD.
 */

/**
 * Non-fail-open. Returns classifySeat's own verdict (STUCK/HEALTHY/UNKNOWN) directly, with no
 * fail-open coercion — for singleton-actuation consumers that must see the raw signal.
 *
 * Options-object signature, matching classifyLoopLiveness below — NOT positional. A prior
 * positional-args revision of this function let `classifyLoopLivenessStrict(s, {nowMs, cutMinutes})`
 * (the documented call shape) silently pass an object as `nowMs` to classifySeat, which is
 * neither a Date nor finite, so classifySeat fell back to the wall clock instead of the caller's
 * intended deterministic time — a genuinely HEALTHY row could misread STUCK or vice versa
 * depending on real-world clock drift at call time. TESTING sub-agent finding, EXEC-TO-PLAN.
 * @param {object} s - claude_sessions row
 * @param {object} [opts]
 * @param {number} [opts.nowMs]
 * @param {number} [opts.cutMinutes]
 * @returns {'STUCK'|'HEALTHY'|'UNKNOWN'}
 */
export const classifyLoopLivenessStrict = (s, { nowMs = Date.now(), cutMinutes = FREEZE_CUT_MINUTES } = {}) => {
  try {
    const { classifySeat, VERDICT } = loadPredicate();
    return classifySeat(s, { cutPointMinutes: cutMinutes, now: nowMs }).verdict;
  } catch {
    // A read/resolve failure is genuinely UNKNOWN, never a fabricated verdict in either
    // direction — an actuation consumer must be able to tell "could not classify" from
...
/**
 * Fail-open, 4-state, for ALARM consumers. UNKNOWN is returned explicitly (never silently
 * coerced to a live state) whenever last_tool_at or loop_state is absent — isKnownWedged alone
 * cannot distinguish "genuinely healthy" from "the input never carried this column", because
 * both fail open to "not wedged"; this wrapper adds that distinction back for callers (like the
 * FR-2 engagement gauge) that need to report UNKNOWN as its own bucket instead of silently
 * counting it as live.
 * @param {object} s - claude_sessions row
 * @param {object} [opts]
 * @param {(session: object) => boolean} [opts.isClaimed] - injected claim signal (same
 *   discipline as scripts/lib/engagement-buckets.mjs's classifySessionBucket: each caller
 *   supplies its own, this function never re-derives a claim mechanism itself)
 * @param {number} [opts.nowMs]
 * @param {number} [opts.cutMinutes]
 * @returns {'LIVE-ENGAGED'|'LIVE-IDLE'|'DEAD-LOOP'|'UNKNOWN'}
 */
export const classifyLoopLiveness = (s, { isClaimed, nowMs = Date.now(), cutMinutes = FREEZE_CUT_MINUTES } = {}) => {
  if (!s) return 'UNKNOWN';
  // Checked FIRST: a positive DEAD-LOOP verdict already proves both columns were present and
  // read (isKnownWedged only reaches classifySeat when loop_state is 'active'/'awaiting_tick',
  // and classifySeat only returns STUCK when last_tool_at parsed) — no need to re-check below.
  if (isKnownWedged(s, nowMs, cutMinutes)) return 'DEAD-LOOP';
  if (s.last_tool_at == null || s.loop_state == null) return 'UNKNOWN';
  return typeof isClaimed === 'function' && isClaimed(s) ? 'LIVE-ENGAGED' : 'LIVE-IDLE';
};
 ...
 * claimed_at, and worktree_path all NULLed by the dual-surface release path (see
 * database/migrations/20260816_claim_sd_tier_check.sql and lib/claim/release-claim-
 * both-surfaces.mjs), and continuous_sds_completed is not reliably incremented
 * either — so the four original signals all go false at the exact moment a worker
 * SUCCEEDS, making the cleanest releases the most invisible. released_at is the
 * missing durable signal: it is stamped NOW() by every release path and is ONLY
 * ever nulled again by claim_sd itself at the session's NEXT claim (at which point
 * sd_key/claimed_at/worktree_path are set fresh anyway) — so it stays true for
 * exactly the idle-but-previously-claimed window this filter was blind to.
 *
 * DO NOT edit this rationale without reading isDispatchableFleetMember's own divergence
 * rationale (lib/fleet/session-predicates.mjs:77-84) and the full consumer census
 * (docs/architecture/fleet-liveness-predicate-consumer-census.md) — the two predicates
 * diverge on purpose, for different reasons, and a well-meaning edit to unify them
 * reintroduces the exact false-negative that census exists to prevent.
 */
export const everClaimed = (s) =>
  !!(s.sd_key || s.claimed_at || s.worktree_path || s.released_at || (s.continuous_sds_completed > 0));
...
/**
 * Release-recency window, in milliseconds. NOT the same constant as liveFleetWorkers'
 * windowMs default above (also 15min) despite the coincidental value — that one gates
 * heartbeat FRESHNESS (is this session even still ticking), this one gates how recently a
 * session RELEASED a claim (is it mid-wind-down, not yet idle capacity). They are
 * independently tuned for unrelated purposes and must not be merged into one constant.
 *
 * SD-LEO-INFRA-UNIFY-FLEET-LIVENESS-001: released_at alone (see everClaimed above) has no
 * recency concept -- a session released 3 minutes ago and one released 3 weeks ago are
 * indistinguishable to everClaimed/isFleetWorker/liveFleetWorkers. That let
 * coordinator-idle-qf-hint.mjs deliver a claim hint into a released-but-still-heartbeating
 * shell window at 07:56:44Z (Hotel-5) -- the session had just finished work and had not yet
 * re-armed. Tuned against four self-resolving shell windows measured the same incident night,
 * 4.5-10.5 minutes; 15 minutes sits above that measured range with margin.
 */
export const RECENTLY_RELEASED_WINDOW_MS = 15 * 60 * 1000; // 15 min

/**
 * Is this session inside its post-release wind-down window? Additive discriminator -- does
 * NOT change everClaimed/isFleetWorker/liveFleetWorkers, which still treat a recently-released
 * session as a live, ever-claimed genuine worker (correct: it IS one). Callers that need to
 * distinguish "just released, still winding down" from "genuinely idle" (e.g.
 * coordinator-idle-qf-hint.mjs deciding whether to deliver a claim hint) should gate on this
 * IN ADDITION TO liveFleetWorkers, not instead of it.
 */
export const isRecentlyReleased = (s, nowMs, windowMs = RECENTLY_RELEASED_WINDOW_MS) => {
  if (!s?.released_at) return false;
  const releasedAt = Date.parse(s.released_at);
  if (!Number.isFinite(releasedAt)) return false;
  return nowMs - releasedAt < windowMs;
};
