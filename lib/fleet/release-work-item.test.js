...
describe('FR1-FLAG: LEO_RELEASE_WORKITEM_RESET defaults ON, "off" is the explicit kill switch', () => {
  it('QF-20260912-961: is ON when unset, ON for any other value, OFF only for exactly "off"', () => {
    expect(isReleaseWorkItemResetEnabled({})).toBe(true);
    expect(isReleaseWorkItemResetEnabled({ LEO_RELEASE_WORKITEM_RESET: '' })).toBe(true);
    expect(isReleaseWorkItemResetEnabled({ LEO_RELEASE_WORKITEM_RESET: 'true' })).toBe(true);
    expect(isReleaseWorkItemResetEnabled({ LEO_RELEASE_WORKITEM_RESET: '1' })).toBe(true);
    expect(isReleaseWorkItemResetEnabled({ LEO_RELEASE_WORKITEM_RESET: 'on' })).toBe(true);
    expect(isReleaseWorkItemResetEnabled({ LEO_RELEASE_WORKITEM_RESET: 'off' })).toBe(false);
  });
});
...
describe('FR1-SD: phase rewind, with the accepted-handoff guard CARRIED', () => {
  ...
  // A REAL git repo, built here rather than borrowed from the ambient checkout.
  //
  // This was `process.cwd()`, which passed locally for a reason that had nothing to do with the
  // assertion: my worktree happened to sit 6 commits ahead of a resolvable origin/main. CI checks
  // out shallow, origin/main does not resolve, the probe returned INDETERMINATE, and the test
  // failed there while staying green on every local run. An ambient fixture is not a fixture —
  // it is a reading of whatever machine you are standing on.
  //
  // Still real git, deliberately: SD-6 exists because a stubbed count returns whatever you
  // configured and confirms your own mistake back to you — that is precisely how the @{u}..HEAD
  // range bug survived reasoning and died to this test. So the repo is genuine and the COUNT is
  // pinned; only the ambient dependency is removed. origin/main is created as a real
  // remote-tracking ref via update-ref, so no network and no remote are involved.
  let WORKTREE_WITH_COMMITS;
  const git = (dir, ...args) => execFileSync('git', args, { cwd: dir, stdio: ['ignore', 'pipe', 'ignore'] });

  beforeAll(() => {
    WORKTREE_WITH_COMMITS = mkdtempSync(join(tmpdir(), 'rwi-ahead-'));
    git(WORKTREE_WITH_COMMITS, 'init', '-q');
    git(WORKTREE_WITH_COMMITS, 'config', 'user.email', 'test@example.invalid');
    git(WORKTREE_WITH_COMMITS, 'config', 'user.name', 'Test');
    git(WORKTREE_WITH_COMMITS, 'config', 'commit.gpgsign', 'false');
    git(WORKTREE_WITH_COMMITS, 'commit', '-q', '--allow-empty', '-m', 'base');
    // origin/main pinned at the base commit; HEAD then moves two ahead of it.
    git(WORKTREE_WITH_COMMITS, 'update-ref', 'refs/remotes/origin/main', 'HEAD');
    git(WORKTREE_WITH_COMMITS, 'commit', '-q', '--allow-empty', '-m', 'work 1');
    git(WORKTREE_WITH_COMMITS, 'commit', '-q', '--allow-empty', '-m', 'work 2');
    // Guard the fixture itself: if this ever stops reading 2, SD-6 is measuring something else.
    const ahead = String(git(WORKTREE_WITH_COMMITS, 'rev-list', '--count', 'origin/main..HEAD')).trim();
    if (ahead !== '2') throw new Error(`fixture did not build: expected 2 commits ahead, got "${ahead}"`);
  });

  afterAll(() => {
    // Windows holds locks on freshly-written .git files often enough that rmSync can EPERM.
    // A cleanup failure must not fail the suite — it would report a tmpdir quirk as a defect in
    // the code under test, which is the most confusing kind of red.
    try {
      if (WORKTREE_WITH_COMMITS) rmSync(WORKTREE_WITH_COMMITS, { recursive: true, force: true });
    } catch { /* leaked tmpdir; the OS reaps it */ }
  });
const allow = async () => ({ ok: true });
  ...
  it('SD-0: THE REWIND IS OPT-IN — default leaves the phase alone for resume', async () => {
    ...
    expect(sb.updates[0].patch).toEqual({ status: 'active', lifecycle_write_token: 'release-work-item.mjs' });
  ...
  it('SD-1: rewinds a mid-phase SD to its safe boundary when ASKED and the gate allows', async () => {
    ...
    expect(sb.updates).toEqual([{ patch: { status: 'active', lifecycle_write_token: 'release-work-item.mjs', current_phase: 'PLAN_PRD' } }]);
  ...
  it('SD-6: unpushed commits are REAL WORK — the row is not reverted', async () => {
    ...
    expect(r.action).toBe('sd_no_reset');
    // Asserts "not yet on main", NOT "unpushed": the range counts pushed commits too, and the
    // message must not claim otherwise. Pinned here so the honest wording cannot regress quietly.
    expect(r.detail).toMatch(/commit\(s\) not yet on main/);
    expect(r.detail).not.toMatch(/unpushed/);
    // The fixture is built to be exactly 2 ahead, so pin the NUMBER, not just the wording. Under
    // the old ambient fixture this could not be asserted at all — the count was whatever the
    // developer's checkout happened to be, which is why the wording was all it could check.
    expect(r.detail).toMatch(/^2 commit\(s\) not yet on main/);
