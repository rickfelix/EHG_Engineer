...
function scrubGitEnv(env) {
  ...
  // POSITIVE hardening, not just removal (R5-1). Deleting an override still leaves whatever the
  // machine's /etc/gitconfig or ~/.gitconfig says, and core.hooksPath / core.fsmonitor there are
  // equally command-execution primitives. Pin both off for the source-tree operations.
  out.GIT_CONFIG_NOSYSTEM = '1';
  out.GIT_CONFIG_GLOBAL = NONEXISTENT_GIT_CONFIG;
  return out;
}

/**
 * A path that deliberately does not exist. git treats an unreadable GIT_CONFIG_GLOBAL as "no global
 * config", which is what we want: no user-level core.hooksPath can reach these commands.
 */
const NONEXISTENT_GIT_CONFIG = path.join(
  require('node:os').tmpdir(), 'leo-source-tree-no-global-gitconfig-DOES-NOT-EXIST',
);
