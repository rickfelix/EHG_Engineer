...
function makeScrubbedGitRunner(cwd, {
  ...
  return (args, callOpts = {}) => {
    ...
    if (!r || r.status !== 0) {
      // A signal-killed or timed-out child (e.g. ETIMEDOUT) often has EMPTY stderr, so the old
      // `failed: ${stderr}` message rendered as literally "git ... failed: " with nothing after
      // the colon — QF-20260912-397. r.error/r.status/r.signal are spawnSync's own structural
      // fields and were never read here; folding them into the suffix (never the prefix, so any
      // existing "git ... failed: " prefix match is unaffected) makes a silent kill diagnosable.
      const stderrText = ((r && r.stderr) || '').toString().trim();
      const diag = [];
      if (!r) {
        diag.push('spawnSync returned no result');
      } else {
        diag.push(`exit ${r.status === null || r.status === undefined ? 'null' : r.status}`);
        if (r.signal) diag.push(`signal ${r.signal}`);
        if (r.error) diag.push(`spawn error: ${r.error.code || r.error.message || String(r.error)}`);
      }
      const suffix = stderrText ? `${stderrText} (${diag.join(', ')})` : `(no stderr -- ${diag.join(', ')})`;
