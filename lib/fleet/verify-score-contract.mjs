...
/**
 * SD-LEO-INFRA-ROLE-RUBRIC-SCORE-001 FR-5: scoped write-time refusal check.
 *
 * `validateScoreContract()`'s `violations` array mixes two unrelated concerns: Rule 1/2
 * ("INVALID: ...") are genuine contract breaches (vanity measurement / skipped verify step) —
 * a write-time refusal should key ONLY on these. Rule 3 ("ESCALATE: ...") is an escalation
 * signal, not a defect in the row being written; refusing on it would silently drop legitimate
 * escalation-eligible scores. `valid: violations.length === 0` conflates the two, so callers
 * must NOT gate refusal on the raw `valid` boolean — use this helper instead.
 *
 * @param {string[]} violations - the `violations` array from validateScoreContract()
 * @returns {boolean} true only when at least one Rule 1/2 ("INVALID:") violation is present
 */
export function hasBlockingViolation(violations) {
  return (violations || []).some((v) => typeof v === 'string' && v.startsWith('INVALID:'));
}
