...
// SD-LEO-FIX-SELF-CLAIM-PREDICATE-001: a risk-noun hit is excluded UNLESS a fresh
// security-agent risk-review stamp (content-hash-bound to the current title+description)
// says the text was already read and found not to be a real risk change.
const { getRiskReviewStampFreshness } = require('./qf-risk-review-stamp.cjs');
...
function isAutoStartableQF(qf, nowMs) {
  ...
  // QF-20260830-901: the bare-word TIER3_RISK_RE below (schema/migration in particular) fires on
  // rows that merely DISCUSS a migration (an audit/gating QF citing a real migration path as
  // evidence) exactly as readily as one that PERFORMS a DDL change -- measured live: 5 of 5 open
  // QFs were routing_tier=2 (already correctly classified as non-risky by the mint-time router,
  // lib/utils/work-item-router.js's routeWorkItem/checkRiskEscalation) yet this redundant rescan
  // disagreed and held every one, while two seats sat idle in front of them. Reuse the SAME
  // context-aware detectors the mint-time router already uses, instead of a second, cruder
  // bare-word duplicate: findSchemaKeywordWithVerbContext requires a change VERB near the noun
  // (schema/migration are frequently discussed without being changed); findRiskKeywordWithContext
  // keeps auth/rls/payments/credentials on default-escalate (a security SURFACE is a signal
  // regardless of grammatical role -- narrowing that one is explicitly documented as unsafe) and
  // only suppresses a few provably-safe noun contexts (SQL "SECURITY DEFINER", *.test.js paths,
  // "-adjacent"/"-related" compounds). Degrades to the OLD blind regex if the ESM import fails,
  // never silently unfiltered.
  const riskScanText = `${qf.title || ''} ${qf.description || ''}`;
  try {
    const { findSchemaKeywordWithVerbContext, findRiskKeywordWithContext } = require('../utils/work-item-router.js');
    // QF-20260901-269 / Solomon ruling 1dfd49bd: NEUTRALIZE sd_type/content_type ENUM-PAIR
    // mentions ("security/retrospective") BEFORE scanning -- they name a routing category, not
    // a risk change. Scoped to THIS re-scan path only, NOT the shared findRiskKeywordWithContext
    // (which must stay maximally fail-safe for its other callers, e.g. mint-time routing).
    // Gated on the text actually discussing the (sd_type, content_type) pair -- the exact
    // measured false-positive, QF-20260830-735 -- so unrelated "<word>/<word>" phrasing
    // (e.g. genuine "security/auth flow") is never silently neutralized.
    let scanText = riskScanText;
    if (/\bsd_type\b/i.test(scanText) && /\bcontent_type\b/i.test(scanText)) {
      try {
        const { CANONICAL_SD_TYPES } = require('../sd-type-enum.js');
        scanText = scanText.replace(/\b([a-z_]+)\/([a-z_]+)\b/gi, (m, a, b) => (
          CANONICAL_SD_TYPES.has(a.toLowerCase()) || CANONICAL_SD_TYPES.has(b.toLowerCase()) ? 'sd-type-pair' : m
        ));
      } catch (_e2) { /* degrade-safe: scan the original text unmodified */ }
    }
    const hit = findSchemaKeywordWithVerbContext(scanText) || findRiskKeywordWithContext(scanText);
    if (hit) {
      // Persisted-vs-rederived TIER DISAGREEMENT must be a VISIBLE gauge, not a silent
      // exclusion (Solomon amendment 2) -- the read-time guard itself is KEPT (amendment 1).
      const persistedTier = qf.routing_tier != null ? Number(qf.routing_tier) : null;
      if (persistedTier != null && persistedTier < 3) {
        // SECURITY EXEC finding (row 37e524f5, C2): this line used to always say "— excluding
        // from self-claim", which became false the moment the risk-review stamp check below can
        // allow the row through. Verdict is decided by that check, not this gauge line.
        console.error(`[qf-auto-start] TIER_DISAGREEMENT qf=${qf.id} persisted_tier=${persistedTier} rederived_hit="${hit}"`);
      }
      // SD-LEO-FIX-SELF-CLAIM-PREDICATE-001: a fresh, content-hash-bound security-agent
      // risk-review stamp overrides the hold -- a post-review edit to title/description
      // invalidates the stamp (status flips to 'stale'), dropping the row back to directed
      // dispatch until re-reviewed. This is for risk NOUNS in the text, not risk CHANGES:
      // a genuine auth-origin edit (QF-20260902-022's class) still escalates via the
      // persisted routing_tier check above, untouched by this stamp.
      const { status } = getRiskReviewStampFreshness(qf);
      if (status !== 'fresh') {
        console.error(`[qf-auto-start] RISK_NOUN_HIT qf=${qf.id} hit="${hit}" stamp=${status} — excluding from self-claim`);
        return false;
      }
      console.error(`[qf-auto-start] RISK_NOUN_HIT qf=${qf.id} hit="${hit}" stamp=fresh — allowing self-claim (security-reviewed)`);
    }
  } catch (e) {
    console.error(`[qf-auto-start] context-aware risk scan unavailable — falling back to blind keyword scan: ${e?.message || e}`);
    if (TIER3_RISK_RE.test(riskScanText)) return false;
  }
  ...
  const ageDays = (nowMs - ageSourceMs) / (24 * 60 * 60 * 1000);
  return ageDays < STALE_QF_DAYS;                       // exclude stale/verify-first QFs
}

// QF-20260821-032: staleness-only COMPLEMENT of isAutoStartableQF (gates duplicated, not
// extracted, to keep this diff minimal -- see that function for the rationale of each check).
// A row failing ONLY on age needs a re-verify, not "no supply". Does NOT widen STALE_QF_DAYS.
function isClaimableWithVerify(qf, nowMs) {
  ...
  if (qf.pr_url || qf.commit_sha) return false;
  if (qf.factory_lane) return false;
  if (qf.routing_tier != null && Number(qf.routing_tier) >= 3) return false;
  if (TIER3_RISK_RE.test(`${qf.title || ''} ${qf.description || ''}`)) return false;
  if (qf.not_before) {
    const notBefore = Date.parse(qf.not_before);
    if (Number.isFinite(notBefore) && notBefore > nowMs) return false;
  }
  ...
  const ageSourceMs = Math.max(...candidates);
  const ageDays = (nowMs - ageSourceMs) / (24 * 60 * 60 * 1000);
  return ageDays >= STALE_QF_DAYS;                      // the ONLY thing that differs from isAutoStartableQF
}

// SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-A: the worker self-claim PICKER's actual eligibility
// gate. Before this, the picker (scripts/worker-checkin.cjs selfClaimQuickFix) called
// isAutoStartableQF ALONE as its admission test, so isClaimableWithVerify -- despite existing
// in this file since QF-20260821-032 -- was wired to nothing but a coordinator gauge
// (scripts/coordinator-idle-qf-hint.mjs's summary.claimableWithVerify counter). That made the
// STALE_QF_DAYS cliff a hard, permanent exclusion for the picker: live-measured 2026-09-06, 11
// open QFs sat past it (oldest QF-20260705-022 at 62.3 days), unclaimable by ANY worker
// regardless of severity, forever, with no path back in short of a manual not_before/dispatch
// edit. A QF failing isAutoStartableQF ONLY on staleness is DEMOTED here to
// claimable-with-verify (needsVerify:true) instead of excluded outright; every other exclusion
// reason (fixture, in-flight, factory-lane, tier, risk, not_before, chairman-gate) still holds
// the row out entirely, unchanged. isAutoStartableQF and isClaimableWithVerify themselves are
// NOT modified by this change -- other callers (isCriticalQfJumpEligible's jump-the-queue gate,
// the coordinator gauge, the capacity forecaster) keep their existing, narrower meanings.
function getQfPickerVerdict(qf, nowMs) {
  if (isAutoStartableQF(qf, nowMs)) return { eligible: true, needsVerify: false };
  if (isClaimableWithVerify(qf, nowMs)) return { eligible: true, needsVerify: true };
  return { eligible: false, needsVerify: false };
}

module.exports = { isAutoStartableQF, isClaimableWithVerify, getQfPickerVerdict, TIER3_RISK_RE, STALE_QF_DAYS };
