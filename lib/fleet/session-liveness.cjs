...
//
// SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-E: the contract above is DELIBERATELY NARROWED for a session
// whose status is 'released' or 'stale'. ~24 release-path writers across the fleet set status to a
// terminal/stale value without ever writing is_alive:false in the same statement (that gap is the
// other half of this SD's fix), so a released/stale row's raw is_alive can be stale-true for its
// entire remaining lifetime. Trusting it there is not "never downgrading a real death" — it IS the
// false-positive this SD exists to close. The one-directional promise still holds for every OTHER
// status (active, idle, or any other non-terminal value): raw_is_alive is denied ONLY for
// released/stale, never elsewhere.
//
...
/**
 * SD-LEO-INFRA-STALE-SESSION-SWEEP-001 FR-1b — heartbeat age in SECONDS, or null if unknowable.
 *
 * ADDITIVE ONLY. isSessionAlive() and every rung above are untouched: nothing here is called by the
 * predicate, so no verdict can change. It exists because the hard-abandonment ceiling needs an age
 * and MUST NOT re-derive one — there is no single age column present at all five shouldHoldClaim
 * call sites. The sweep's v_active_sessions projection carries heartbeat_age_seconds and NOT
 * heartbeat_at; both compaction hooks carry heartbeat_at and NOT heartbeat_age_seconds. A ceiling
 * reading one form would be silently inert at two or three of five sites — the same field-set
 * blindness this SD exists to fix, recurring inside its own fix.
 *
 * The preference order deliberately mirrors hasFreshHeartbeat above (age_seconds first, then
 * heartbeat_at, then last_heartbeat); liveness-abandon-ceiling.test.js asserts the two agree at the
 * threshold in both row shapes, so they cannot drift apart.
 *
 * RETURNS null — NOT 0 and NOT Infinity — when no age field is present. That distinction is
 * load-bearing: 0 would mean "brand new" and hold a claim forever, Infinity would mean "ancient" and
 * evict unconditionally. Both are wrong answers wearing a number. The caller decides what to do with
 * "I cannot tell", and does so explicitly.
 */
function heartbeatAgeSec(session, nowMs = Date.now()) {
  if (!session) return null;
  if (typeof session.heartbeat_age_seconds === 'number') return session.heartbeat_age_seconds;
  const ts = session.heartbeat_at ?? session.last_heartbeat;
  if (ts == null) return null;
  const ms = _ageMs(ts, nowMs);
...
// SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-E FR-2: statuses whose raw is_alive is no longer trusted,
// since ~24 release-path writers can leave it stale-true here. Deliberately just these two —
// 'idle' is a legitimate alive-but-unclaimed state and must keep trusting a fresh raw is_alive.
const TERMINAL_STATUSES_DENY_RAW_ALIVE = new Set(['released', 'stale']);

/**
 * The READ-TIME liveness SSOT. Returns { alive, reason }.
 * alive = raw is_alive===true (denied for status IN ('released','stale') — see FR-2 note above)
 * OR fresh heartbeat OR live PID OR fresh tick OR armed-silence.
 * One-directional for every status OTHER than released/stale: a raw-alive session is alive (reason
 * 'raw_is_alive'); a parked session the raw flag froze to false is UPGRADED to alive iff an
 * authoritative signal fires (with that signal as the reason).
 *
 * @param {object|null} session - a claude_sessions-shaped row (or v_active_sessions row). MUST
 *   include `status` for the FR-2 deny-list to take effect — a caller that omits it from its
 *   SELECT silently keeps trusting a stale raw is_alive (see LIVENESS_INPUT_FIELDS below).
...
function isSessionAlive(session, { nowMs = Date.now(), aliveCcPids = null, markerDir = undefined } = {}) {
  ...
  if (session.is_alive === true && !TERMINAL_STATUSES_DENY_RAW_ALIVE.has(session.status)) {
    return { alive: true, reason: 'raw_is_alive' };
  }
...
/**
 * SD-LEO-INFRA-STALE-SESSION-SWEEP-001 FR-5 — the PRODUCER/CONSUMER FIELD-SET CONTRACT.
 *
 * The columns isSessionAlive() actually reads, expressed as OR-groups: each inner array is a set of
 * interchangeable column names, and a caller satisfies the group by supplying AT LEAST ONE of them.
 * The groups are NOT a style preference — they mirror real alternatives in the rungs above:
 *   - heartbeat: hasFreshHeartbeat() prefers heartbeat_age_seconds (:68) and falls back to
 *     heartbeat_at / last_heartbeat (:69). A view row carries the first; a base-table row the second.
 *   - pid: hasPidAlive() resolves from terminal_id OR session_id (:118) — it deliberately does not
 *     bail on a missing terminal_id, because a marker can still be matched by session_id.
 *
 * WHY THIS EXISTS. The JSDoc on shouldHoldClaim already named these fields in prose, and two of the
 * five call sites honoured it (scripts/hooks/reclaim-sd-after-compaction.cjs,
 * scripts/hooks/session-state-sync.cjs — both commented "select the liveness fields the guard
 * reads"). The three seams in scripts/stale-session-sweep.cjs did not, so three rungs could never
 * fire there and a live worker was released on 2026-07-27 while the sweep's own classifier had just
 * called it ALIVE_SOURCE_SIDE. Prose could not fail a build; this constant can.
 *
 * Consumed by tests/unit/fleet/liveness-input-parity.test.js. A caller that adds a new rung MUST add
 * its column here, which turns "someone forgot a column" from an outage into a red test.
 *
 * SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-E added the ['status'] group: the raw_is_alive rung (:169) now
 * reads `status` to deny a stale raw is_alive on a released/stale row. A caller whose SELECT omits
 * `status` gets `session.status === undefined`, which is not in TERMINAL_STATUSES_DENY_RAW_ALIVE,
 * so the rung silently keeps trusting the stale flag — the exact producer-side starvation that left
 * the QF stale-claim sweep unable to see a dead holder for 8.6 hours (scripts/stale-session-sweep.cjs
 * :1281's holderRows query was one of the omitting callers, now fixed alongside this group).
 */
const LIVENESS_INPUT_FIELDS = Object.freeze([
  Object.freeze(['is_alive']),
  Object.freeze(['status']),
  Object.freeze(['heartbeat_age_seconds', 'heartbeat_at', 'last_heartbeat']),
  Object.freeze(['terminal_id', 'session_id']),
  Object.freeze(['process_alive_at']),
  Object.freeze(['expected_silence_until']),
]);
...
module.exports = {
  ...
  LIVENESS_INPUT_FIELDS,
  TERMINAL_STATUSES_DENY_RAW_ALIVE,
};
