 ...
 *
 * SD-LEO-INFRA-HOLD-STATE-CONTRACT-001 (FR-4): an explicit override is a deliberate FLOOR in
 * the hold-state-contract sense (it overrides the computed default the same way a park or an
 * exec_boundary_hold overrides the default flow), so it can OPTIONALLY carry the rest of the
 * contract stamp ({owner, review_at, release_condition}) alongside the pre-existing reason.
 * These 3 fields are purely additive to the return shape -- omitting them (the common,
 * pre-existing call shape) returns EXACTLY the same {min_tier_rank, min_tier_rank_reason?}
 * shape as before this SD. metadata.min_tier_rank itself is NEVER wrapped -- it stays a bare
 * finite integer always (TESTING sub-agent evidence: 3 consumers do Number(metadata.min_tier_rank)
 * and fail OPEN on non-finite, so wrapping it would silently defeat tier-gating fleet-wide).
 * The stamp lives in sibling keys the caller is responsible for merging alongside
 * min_tier_rank in its own metadata patch (this function stays DB-free, matching its
 * pre-existing pure-function contract).
 * @param {object} sd a strategic_directives_v2-shaped row
 * @param {{ explicitRank?: number, explicitReason?: string, explicitOwner?: string, explicitReviewAt?: string, explicitReleaseCondition?: string }} [opts]
 * @returns {{ min_tier_rank: number, min_tier_rank_reason?: string, min_tier_rank_owner?: string, min_tier_rank_review_at?: string, min_tier_rank_release_condition?: string }}
 */
export function stampPayloadForCreation(sd, opts = {}) {
  if (opts.explicitRank != null) {
    ...
    const payload = { min_tier_rank: clamp(opts.explicitRank), min_tier_rank_reason: reason };
    if (opts.explicitOwner || opts.explicitReviewAt || opts.explicitReleaseCondition) {
      const stamped = buildProvenancedStamp(
        { reason, owner: opts.explicitOwner, review_at: opts.explicitReviewAt, release_condition: opts.explicitReleaseCondition },
        opts.writingSessionId
      );
      if (stamped.owner) payload.min_tier_rank_owner = stamped.owner;
      if (stamped.review_at) payload.min_tier_rank_review_at = stamped.review_at;
      if (stamped.release_condition) payload.min_tier_rank_release_condition = stamped.release_condition;
      if (stamped.stamped_by_session) payload.min_tier_rank_stamped_by_session = stamped.stamped_by_session;
    }
    return payload;
  }
...
/**
 * Async companion to stampPayloadForCreation()'s explicit-override path: checks the stamp
 * against the hold-state contract and (observe mode only) logs a violation row. Kept SEPARATE
 * from stampPayloadForCreation() itself, which stays synchronous and DB-free on purpose --
 * many existing call sites invoke it without awaiting. Callers on the explicit-override path
 * that want enforce-mode rejection / observe-mode calibration logging call this first.
 * @param {import('@supabase/supabase-js').SupabaseClient|null} supabase - null skips logging
 * @param {{ explicitReason?: string, explicitOwner?: string, explicitReviewAt?: string, explicitReleaseCondition?: string }} opts
 * @returns {Promise<{ok:boolean, mode:'observe'|'enforce', errors:string[]}>}
 */
export async function checkExplicitTierRankStamp(supabase, opts = {}) {
  const stamp = { reason: opts.explicitReason, owner: opts.explicitOwner, review_at: opts.explicitReviewAt, release_condition: opts.explicitReleaseCondition };
  const holdCheck = checkHoldStamp(stamp);
  if (!holdCheck.ok && holdCheck.mode === 'observe') {
    await logHoldStateViolation(supabase, { surface: 'min_tier_rank', stamp, errors: holdCheck.errors });
  }
  return holdCheck;
}

export default { computeMinTierRank, stampPayload, stampPayloadForCreation, checkExplicitTierRankStamp, FLEET_CLAIMABLE_BASELINE_RANK };
