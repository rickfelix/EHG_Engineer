 ...
 * released_at is selected (not merely filtered) so a caller can distinguish "no seats" from
 * "seats excluded by release" without a second query.
 */
const POPULATION_COLUMNS = 'session_id, status, loop_state, last_tool_at, heartbeat_at, metadata, released_at';
...
async function fetchPopulation(supabase) {
  ...
  // RELEASED SHELLS ARE NOT POPULATION. A retirement/stale-sweep release stamps released_at but does
  // not always change status away from 'active'/'idle' (QF-20260912-784: four seats released
  // 2026-09-06 kept status='idle' with a frozen heartbeat, so the status filter alone let them
  // re-enter the population every tick — cited on 75 consecutive drive-state runs). This predicate is
  // released-state-derived, not claim-derived (sd_key/claimed_at/worktree_path/everClaimed remain
  // banned from this query per the docblock above), so it does not reproduce the everClaimed defect.
