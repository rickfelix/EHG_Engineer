...
const DRAIN_SETS = Object.freeze({
  ...
  // (child G). The michael_handoff kind itself is registered by child G in one PR across
  // PAYLOAD_KINDS, this set, the role_drain_sets rows, declaredAddresseeRole and lane-lint — a
  // partial registration is the untyped-row class, so it is deliberately NOT pre-added here.
