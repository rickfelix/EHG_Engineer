 ...
 * @param {{expectedHolder?: string|null}} [opts] - CAS on a specific dead holder when clearing
...
export async function clearAndReopenQf(supabase, qfId, opts = {}) {
  ...
  // The claim predicate is the ONLY thing that varies. Everything else — the status filter and the
  // real-work exclusions — is identical for both shapes, so neither caller can end up with a
  // weaker guard than the other.
  q = expectedHolder === null
    ? q.is('claiming_session_id', null)
    : q.eq('claiming_session_id', expectedHolder);
