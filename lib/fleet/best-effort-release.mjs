...
// registration for a worktree already provisioned against the claim -- see this helper's doc
// comment for why it only RECORDS the orphan (never removes it inline).
import { recordOrphanedWorktree } from './record-orphaned-worktree.mjs';
import { insertCoordinationRow } from '../coordinator/dispatch.cjs';

/**
 * FR-5 (SD-LEO-INFRA-FIX-CLAIM-EVICTION-001): if the just-released session was sleeping on a
 * claim-wakeup (metadata.wind_down.had_claim === true) AND has not become active again since
 * that stop (heartbeat_at <= wind_down.at), write it a hint on its OWN session_coordination lane
 * so its NEXT /checkin surfaces the removal via pending_directives. This does NOT attempt to
 * preempt/re-arm an already-scheduled ScheduleWakeup mid-sleep -- no such primitive exists in the
 * harness and the gap is already formally risk-accepted (fleet-hibernation-quiet-tick.md:159-171,
 * lib/fleet/self-wake-escalation.cjs:15-22) -- purely a next-checkin hint, mirroring the existing
 * precedent at scripts/stale-session-sweep.cjs's CLAIM_BOUNDARY_PROBE (the "released HOLDER
 * itself gets no notice" fix: a directive-kind row on the released session's OWN lane so its
 * next check-in surfaces it in pending_directives and cannot silently miss it).
 *
 * The heartbeat-vs-wind_down.at check guards against firing on an ORDINARY self-release by a
 * session that is currently awake and working: metadata.wind_down is overwritten on every Stop
 * hook firing (never cleared on wake), so without this check a stale had_claim:true from a PRIOR
 * sleep cycle would fire a confusing hint on a routine /claim release call.
 *
 * Fail-soft by design: never throws, never blocks the release outcome this is called alongside.
 */
async function hintSleepingSeatClaimRemoved(supabase, sessionId, sdKey, reason) {
  try {
    if (!supabase || typeof supabase.from !== 'function' || !sessionId) return;
    const { data: sess, error } = await supabase
      .from('claude_sessions')
      .select('metadata, heartbeat_at')
      .eq('session_id', sessionId)
      .maybeSingle();
    if (error || !sess) return;
    const windDown = sess.metadata && sess.metadata.wind_down;
    if (!windDown || windDown.had_claim !== true) return;
    const windDownAtMs = Date.parse(windDown.at);
    const heartbeatMs = Date.parse(sess.heartbeat_at);
    if (Number.isFinite(heartbeatMs) && Number.isFinite(windDownAtMs) && heartbeatMs > windDownAtMs) {
      return; // woken and active since that stop -- not currently sleeping; stale wind_down
    }
    ...
    await insertCoordinationRow(supabase, {
      ...
      subject: `[CLAIM REMOVED] your claim on ${sdKey || '(unknown)'} was removed while you were sleeping`,
      body: `Your claim on ${sdKey || '(unknown)'} was released (reason: ${reason}) while you were sleeping on a claim-wakeup. Run /checkin to re-claim it (if still open) or pick up a new item -- do not assume you still hold it.`,
      payload: {
        kind: 'coordinator_request',
        chairman_directive: false,
        released_sd: sdKey || null,
        reason: 'sleeping_seat_claim_removed',
        removal_reason: reason,
      },
      expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
    });
  } catch {
    // fail-soft: a hint failure must never affect the release outcome
  }
}
...
export async function bestEffortReleaseSd(supabase, sessionId, reason = 'manual', log = console.error, opts = {}) {
  try {
    ...
    // QF-20260903-936: capture what this session had provisioned before release_sd clears it
    // server-side (release_sd never touches the filesystem/worktree registration itself).
    let worktreePath = null, worktreeBranch = null;
    if (typeof supabase.from === 'function') {
      try {
        const { data: sessRow, error: worktreeReadErr } = await supabase
          .from('claude_sessions')
          .select('sd_key, worktree_path, worktree_branch')
          .eq('session_id', sessionId)
          .maybeSingle();
        // Fail-soft by design: best-effort informational capture, never a release blocker.
        if (!worktreeReadErr) {
          worktreePath = sessRow?.worktree_path || null;
          worktreeBranch = sessRow?.worktree_branch || null;
        }
      } catch { /* fail-soft: this read must never block the release */ }
    }
    ...
    const resolvedSdKey = expectedSdKey || (worktreeBranch ? worktreeBranch.replace(/^feat\//, '') : null);
    recordOrphanedWorktree(supabase, {
      sdKey: resolvedSdKey,
      holder: sessionId, worktreePath, worktreeBranch, reason,
    });
    await hintSleepingSeatClaimRemoved(supabase, sessionId, resolvedSdKey, reason);
...
export async function bestEffortReleaseSdByKey(supabase, sessionId, sdKey, reason = 'manual', log = console.error) {
  try {
    ...
    // QF-20260903-936: capture what this session had provisioned before release_sd_by_key
    // clears it server-side (the RPC never touches the filesystem/worktree registration).
    let worktreePath = null, worktreeBranch = null;
    if (typeof supabase.from === 'function') {
      try {
        const { data: sessRow, error: worktreeReadErr } = await supabase
    ...
    recordOrphanedWorktree(supabase, { sdKey, holder: sessionId, worktreePath, worktreeBranch, reason });
    await hintSleepingSeatClaimRemoved(supabase, sessionId, sdKey, reason);
    return { released: true, error: null };
  } catch (e) {
    const msg = e && e.message ? e.message : String(e);
...
export async function clearAndReopenQf(supabase, qfId, opts = {}) {
  ...
  if (!changed) {
    ...
    try {
      // testing-agent EXEC review (688ca3f5): .from(table) alone has no .eq() on the real
      // postgrest-js client -- only the filter-builder returned by .select()/.update()/etc does.
      // .select('id') must come FIRST, not last, or this throws at runtime (silently swallowed by
      // this try/catch, making guard_refused/no_match_status unreachable in production).
      let lookup = supabase.from('quick_fixes').select('id').eq('id', qfId).in('status', ['in_progress', 'open']);
      lookup = applyClaimPredicate(lookup);
      // count-truncation-diff-lint: bounded by id (a primary key) already, but an explicit
      // limit(1) makes that visible to the scanner without changing the array-shaped response.
      const { data: lookupData, error: lookupError } = await lookup.limit(1);
