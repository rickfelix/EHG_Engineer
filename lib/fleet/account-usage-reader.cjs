...
function identityDisplayMap(env = process.env) {
  ...
  try {
    ...
    for (const [uuid8, name] of Object.entries(parsed)) {
      if (typeof name === 'string' && name.trim()) out.set(uuid8, name.trim().slice(0, 128));
    }
    return out;
  } catch {
...
async function readAllAccounts(opts = {}) {
  ...
  const identities = resolveSlotIdentities(opts);
  ...
  const relabelled = displayMap.size === 0 ? readings : readings.map((r) => {
    ...
    return mapped && mapped !== r.name ? { ...r, name: mapped } : r;
  });
  if (displayMap.size > 0) {
    // Re-key identities under whatever the slots are now called, so collision detection compares
    // the labels the strip will actually show.
    const remapped = new Map();
    for (const [origName, uuid8] of identities) {
      const mapped = uuid8 ? displayMap.get(uuid8) : null;
      remapped.set(mapped || origName, uuid8);
    }
    identities.clear();
    for (const [k, v] of remapped) identities.set(k, v);
  }

  const collisions = findIdentityCollisions(identities);
  if (collisions.length === 0) return relabelled;
