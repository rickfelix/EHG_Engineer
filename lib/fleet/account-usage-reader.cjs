...
async function readOneAccount(entry, opts = {}) {
  ...
  try {
    res = await fetchImpl(USAGE_URL, {
      ...
      // A redirect must never be followed while carrying a bearer token. undici happens to strip
      // the Authorization header cross-origin today, but that is runtime behaviour, not a property
      // of this call — a same-origin 302 DOES forward it. Refusing outright makes the guarantee
      // local to the code that holds the credential, and a redirect from an undocumented endpoint
      // is an anomaly worth surfacing as `unreachable` rather than silently chasing.
      redirect: 'error',
    });
  } catch (err) {
...
/**
 * Whether the hostDefault slot's registry name should be replaced with a measured fallback,
 * instead of a fixed label it cannot vouch for (QF-20260911-082).
 *
 * MEASURED DEFECT: the hostDefault entry's registry name ("Code Street Labs") names whichever
 * account a /login rotation LEAVES BEHIND in the host's default credential directory — an
 * assumption about who normally sits there, not a measurement. Unlike the profile slots, which
 * read a FIXED directory and so cannot silently become a different account, the hostDefault
 * directory's content can change without the registry label changing, and a rotation then
 * mislabels the new account's real usage under the old account's name with no signal anywhere
 * that the label is unverified.
 *
 * The raw name may stand ONLY when something has actually vouched for this identity: the
 * operator-supplied FLEET_ACCOUNT_IDENTITY_MAP names this uuid8 (even a contested one — the
 * existing contested-label path already refuses to render a number for it), or another registry
 * slot's own profile independently resolves to the SAME uuid8 (a real alias, left to the existing
 * collision/DUPLICATE_IDENTITY path). Absent both — and with no uuid8 to show at all — nothing
 * changes, because nothing is derivable either way.
 *
 * @returns {string|null} 'host-default (<uuid8>)', or null when the existing name may stand
 */
function hostDefaultFallbackLabel(hostDefaultName, uuid8, identities, displayMap) {
  if (!uuid8) return null;
  if (displayMap instanceof Map && displayMap.get(uuid8)) return null;
  for (const [otherName, otherUuid8] of identities) {
    if (otherName !== hostDefaultName && otherUuid8 === uuid8) return null;
  }
  return `host-default (${uuid8})`;
}

/**
 * Map each registry slot's ORIGINAL name to the name its reading should actually carry: the
 * operator-supplied FLEET_ACCOUNT_IDENTITY_MAP relabel (FR-1) plus the hostDefault fallback above
 * (QF-20260911-082), computed in ONE place so a reading's name and the identities map a consumer
 * looks it up by (resolveDisplayIdentities / relabelIdentities) can never independently diverge —
 * exactly the defect class "THE KEYING PIN" test already pins for the displayMap-only case.
 */
function computeFinalNames(identities, displayMap, contested) {
  const hostDefaultEntry = ACCOUNT_REGISTRY.find((e) => e.hostDefault);
  const out = new Map();
  for (const [origName, uuid8] of identities) {
    const mapped = displayMap instanceof Map && uuid8 ? displayMap.get(uuid8) : null;
    let finalName = mapped && !contested.has(mapped) ? mapped : origName;
    if (finalName === origName && hostDefaultEntry?.name === origName) {
      finalName = hostDefaultFallbackLabel(origName, uuid8, identities, displayMap) || origName;
    }
    out.set(origName, finalName);
  }
  return out;
}
...
function relabelIdentities(identities, displayMap, contested = contestedDisplayLabels(identities, displayMap)) {
  const finalNames = computeFinalNames(identities, displayMap, contested);
  const remapped = new Map();
  for (const [origName, uuid8] of identities) remapped.set(finalNames.get(origName), uuid8);
  return remapped;
}
...
function contestedDisplayLabels(identities, displayMap) {
  ...
  const out = new Set();
  // (a) the same label claimed by two different accounts.
  for (const [label, uuids] of byLabel) if (uuids.size > 1) out.add(label);
  // (b) a label that collides with a DIFFERENT slot's own registry name. Comparing mapped labels
  // only against other mapped labels misses this: a label claimed by exactly ONE account looks
  // uncontested, gets applied, and lands on the name a slot without a mapping already keeps —
  // producing two readings under one name, which then share a natural key in
  // account_usage_snapshots and hand each other their retained figures. The label namespace
  // includes the raw registry names, so the check has to as well.
  for (const [slotName, ownUuid8] of identities) {
    const claimants = byLabel.get(slotName);
    if (!claimants) continue;
    // Self-mapping is legitimate: a slot whose own account maps to its own name contests nothing,
    // so the slot that rightfully owns the name is never penalised for someone else's config.
    for (const uuid8 of claimants) {
      if (uuid8 !== ownUuid8) { out.add(slotName); break; }
    }
  }
  return out;
}
...
async function readAllAccounts(opts = {}) {
  ...
  // computeFinalNames is the single source of truth readings AND relabelIdentities both consult
  // (QF-20260911-082) — see its header for why that sharing matters.
  const displayMap = identityDisplayMap(opts.env || process.env);
  const contested = contestedDisplayLabels(identities, displayMap);
  const finalNames = computeFinalNames(identities, displayMap, contested);
  const relabelled = readings.map((r) => {
    const finalName = finalNames.get(r.name);
    ...
    return finalName !== r.name ? { ...r, name: finalName } : r;
  });
  ...
  // QF-20260912-509: a uuid8 collision is NOT always genuinely unattributable. When one member
  // of the group is the hostDefault slot (whose registry name is a fixed label it cannot vouch
  // for, per hostDefaultFallbackLabel above) and exactly one OTHER slot's own registry profile
  // independently resolves to the same identity, this IS one real, identifiable account — not
  // two accounts we cannot tell apart. Blank only the hostDefault alias and keep the real
  // profile's live reading, instead of nulling both. A contested display label (two DIFFERENT
  // accounts claiming one name) stays fully blanked — that ambiguity is genuine.
  const hostDefaultName = ACCOUNT_REGISTRY.find((e) => e.hostDefault)?.name;
  const blanked = new Set(contestedSlots);
  for (const names of collisions) {
    const nonHostDefault = names.filter((n) => n !== hostDefaultName);
    if (hostDefaultName && nonHostDefault.length === 1 && nonHostDefault.length < names.length) {
      for (const n of names) if (n !== nonHostDefault[0]) blanked.add(n);
      // eslint-disable-next-line no-console
      console.warn(JSON.stringify({
        event: 'account_usage.duplicate_identity_resolved',
        slots: names,
        attributed_to: nonHostDefault[0],
        detail: 'these registry slots resolve to the SAME account -- attributed to the non-hostDefault profile instead of nulling both',
      }));
    } else {
      for (const n of names) blanked.add(n);
      // eslint-disable-next-line no-console
      console.warn(JSON.stringify({
  ...
  return readingsToMark.map((r) => (blanked.has(r.name)
