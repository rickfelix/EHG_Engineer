...
/**
 * QF-20260903-623: `reason` and `resolved_to` previously assumed EVERY caller shares
 * resolveSeatModel's semantics (an unrecognized/absent model, always resolving to
 * WEAKEST_MODEL). stampRankForWorker's fallback (model recognized, effort missing) is a
 * DIFFERENT failure with a DIFFERENT outcome (deriveWorkerTierRank, not necessarily the
 * weakest) — opts lets a caller supply the MEASURED reason/outcome instead of the assumed
 * default. Omitting opts preserves resolveSeatModel's existing behavior exactly.
 * @param {{reason?: string, resolvedTo?: *}} [opts]
 */
function reportUnresolvedCapability(rawModel, context, opts) {
  const { reason, resolvedTo } = opts || {};
  const event = {
    token: UNRESOLVED_CAPABILITY_TOKEN,
    raw_model: rawModel === undefined ? null : rawModel,
    reason: reason || ((typeof rawModel === 'string' && rawModel.trim()) ? 'unrecognized_model' : 'no_model_stamp'),
    resolved_to: resolvedTo === undefined ? WEAKEST_MODEL : resolvedTo,
 ...
 *
 * SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001 (FR-3, ruling 1B): a floor without provenance
 * (metadata.min_tier_rank_reason) is ADVISORY, not blocking. This is where that rule lives so
 * every caller through this ONE predicate gets it for free -- both current callers (dispatch.cjs,
 * claim-eligibility.cjs's tierAxes) are updated in this same SD to pass `hasProvenance`, and
 * tier-claimable.cjs / merged-pool-self-claim.cjs / worker-checkin.cjs's recoverStrandedFinal all
 * route through tierAxes transitively (via classifyDispatchIneligibility), so they inherit the
 * rule without their own changes. TIER_FLOOR_PROVENANCE_ADVISORY=off restores pre-SD
 * binding-regardless-of-provenance behavior fleet-wide in one flip.
 *
 ...
 * @param {{ hasProvenance?: boolean }} [opts] - hasProvenance: true when the floor carries
 *   metadata.min_tier_rank_reason. Omitted/false is treated as "no provenance" (advisory-eligible),
 *   never as "provenance present" -- an unmigrated caller degrades to the SAFER (more permissive)
 *   advisory reading rather than silently staying binding-by-omission.
 * @returns {'above_worker_tier'|'above_worker_tier_advisory'|'tier_stamp_missing'|null}
 */
// SD-LEO-INFRA-TIER-FLOOR-PROVENANCE-001 scope note (raised as a review question, verified here):
// 'tier_stamp_missing' stays HARD-BINDING regardless of opts.hasProvenance, unconditionally, by
// design -- ruling 1B is about an unjustified SD-side FLOOR (a min_tier_rank with no
// min_tier_rank_reason), not about an unknown WORKER-side capability. A worker with no finite
// tier_rank stamp at all is a different failure mode ("we cannot compare ranks") than "the floor's
// author never justified it" -- provenance on the SD tells you nothing about the worker's own
// stamp, so it must not silently wave through a comparison that literally cannot be made. Only the
// 'above_worker_tier' branch (both ranks ARE known, and the worker is below) is opts.hasProvenance-
// gated. Two-sided coverage: tests/unit/fleet/tier-backlog-reservation.test.js.
function tierRankVerdict(workerTierRank, minTierRank, opts = {}) {
  const minRank = Number(minTierRank);
  if (!Number.isFinite(minRank)) return null; // unscored SD -> no floor to enforce
  if (!Number.isFinite(Number(workerTierRank))) return 'tier_stamp_missing'; // fail closed, scored SD, provenance-independent
  const workerRank = Number(workerTierRank);
  if (minRank > workerRank) {
    const advisoryModeOn = String(process.env.TIER_FLOOR_PROVENANCE_ADVISORY || '').toLowerCase() !== 'off';
    if (advisoryModeOn && !opts.hasProvenance) return 'above_worker_tier_advisory';
    return 'above_worker_tier';
  }
  return null;
}
...
function stampRankForWorker(session, liveFleet) {
  ...
  //
  // QF-20260903-623: MEASURE the cause and the outcome rather than assuming resolveSeatModel's
  // shape. A recognized model with no effort stamp is a DIFFERENT failure than an unrecognized
  // model (distinct reason: no_effort_stamp), and this path's actual return value is
  // deriveWorkerTierRank, which may raise the rank rather than lower it — resolved_to must
  // report what was ACTUALLY returned, never an assumed downgrade.
  const outcome = deriveWorkerTierRank(session, liveFleet);
  const reason = (m && !e && familyFromModelId(m)) ? 'no_effort_stamp' : undefined;
  reportUnresolvedCapability(m, session && session.session_id, { reason, resolvedTo: outcome });
  return outcome;
}
...
/**
 * SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-1): the live fleet's per-worker rank AND
 * model/effort, for lib/fleet/dispatch-suggestions.cjs's advisory ranked-suggestion engine —
 * the one caller that needs more than isTieringActive's boolean. Fail-open to an empty snapshot
 * ({ workers: [], ranks: [] }) on any fault.
 *
 * STATIC RANK SPACE, NOT deriveLiveLadder's dynamic dense rank (live-reproduced by testing-agent
 * evidence 201a3892 against a 5-seat fixture during an earlier iteration of this SD that also
 * wired a defer mechanism into merged-pool-self-claim.cjs — since removed: that mechanism
 * substantially duplicated the already-shipped WORK-DOWN-ONLY-WHEN-LOWER-TIER-BACKLOGGED gate,
 * lib/fleet/tier-backlog.cjs lowerTierBacklog() + claim-eligibility.cjs tierAxes'
 * reserved_no_lower_backlog branch, SD-LEO-INFRA-AUTO-TIERING-ACTIVATION-001-E). The bug this
 * comment preserves for the NEXT reader: every candidate item's floor (metadata.min_tier_rank,
 * computed by lib/fleet/sd-tier-rank.mjs's computeMinTierRank) is calibrated in the FIXED 4-rung
 * STATIC ladder space — the same space classifyDispatchIneligibility's tier axis compares
 * worker_tier_rank against. deriveLiveLadder's dense rank is relative to WHOEVER is currently
 * live (1..K, K = distinct scores present) — a different, dynamically-sized scale that silently
 * disagrees with the static floor once a 3rd+ distinct capability is live. Using
 * rankForModelEffort(model, effort) — the STATIC per-model+effort rank already used by
 * computeMinTierRank's floor and every other tier-axis comparison in this codebase — keeps every
 * value in one fixed, comparable space by construction.
 * @param {object} supabase service-role client
 * @param {{ nowMs?: number }} [opts]
...
async function liveWorkerCapabilitySnapshot(supabase, opts = {}) {
  try {
    const live = await fetchLiveFleetRows(supabase, opts);
    const workers = live.map((s) => ({
      session_id: s.session_id,
      model: s?.metadata?.model,
      effort: s?.metadata?.effort,
      rank: rankForModelEffort(s?.metadata?.model, s?.metadata?.effort),
    }));
    return {
      workers,
      ranks: workers.map((w) => w.rank).filter((r) => Number.isFinite(r)),
    };
  } catch {
