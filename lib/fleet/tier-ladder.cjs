 ...
 * snapshot ({ workers: [], ranks: [], selfRank: null }) on any fault — both callers already
 * treat an empty live set as "no tiering signal, behave as before".
 *
 * ONE LADDER, NOT TWO (caught while writing the FR-2 wiring-pin test): deriveLiveLadder MUST run
 * over the FULL live fleet INCLUDING the caller before any exclusion — dense rank is a function
 * of who else is live, so ranking the caller separately from their peers (e.g. comparing a
 * STAMPED claude_sessions.metadata.tier_rank against a peer-only-derived live rank) compares two
 * incommensurate scales. excludeSessionId only splits the ALREADY-RANKED set afterward, into the
 * caller's own rank (selfRank) and everyone else's (workers/ranks) — both read off the SAME
 * snapshot, so a caller comparing selfRank against ranks is comparing like with like.
 * @param {object} supabase service-role client
 * @param {{ nowMs?: number, excludeSessionId?: string }} [opts]
 * @returns {Promise<{ workers: Array<{session_id:string, model:(string|undefined), effort:(string|undefined), rank:number}>, ranks: number[], selfRank: (number|null) }>}
 */
async function liveWorkerCapabilitySnapshot(supabase, opts = {}) {
  try {
    const live = await fetchLiveFleetRows(supabase, opts);
    const fleetForLadder = live.map((s) => ({
      model: s?.metadata?.model,
      effort: s?.metadata?.effort,
    }));
    const { entries } = deriveLiveLadder(fleetForLadder);
    const allWorkers = live.map((s, i) => ({
      session_id: s.session_id,
      model: entries[i]?.model,
      effort: entries[i]?.effort,
      rank: entries[i]?.rank,
    }));
    const selfEntry = opts.excludeSessionId ? allWorkers.find((w) => w.session_id === opts.excludeSessionId) : null;
    const workers = opts.excludeSessionId ? allWorkers.filter((w) => w.session_id !== opts.excludeSessionId) : allWorkers;
    return {
      workers,
      ranks: workers.map((w) => w.rank).filter((r) => Number.isFinite(r)),
      selfRank: Number.isFinite(selfEntry && selfEntry.rank) ? selfEntry.rank : null,
    };
  } catch {
    return { workers: [], ranks: [], selfRank: null };
  }
}
