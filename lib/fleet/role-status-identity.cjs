...
const ROLE_IDENTITY = Object.freeze({
  ...
  // SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: green is in the FC map and unclaimed by any role.
  michael: { callsign: 'Michael', color: 'green' },
});
 ...
 * @param {string} o.role       'adam' | 'coordinator' | 'solomon' | 'michael'
...
function verdictFromMetadata(metadata) {
  ...
  // QF-20260906-473: the coordinator-election writer (lib/coordinator/resolve.cjs) stamps
  // is_coordinator:true, not role:'coordinator' -- no live coordinator session actually sets
  // the string field the ROLE_IDENTITY.coordinator branch below checks for, so this predicate
  // classified every real coordinator session as WORKER (an absent role key falls through to
  // WORKER at :102) and it kept writing available:true roll_call rows through the same-turn-
  // next-claim path in stop-loop-wakeup-reminder.cjs. Authoritative on its own, mirroring the
  // non_fleet check above.
  if (metadata.is_coordinator === true) return ROLE_VERDICT.ROLE;
...
/**
 * Is this session a role-session (adam | coordinator | solomon | michael)?
 *
