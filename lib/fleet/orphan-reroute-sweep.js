 ...
 *
 * REPEAT-OFFENDER ALARM: once a (role, kind) pair's tally reaches REPEAT_OFFENDER_THRESHOLD,
 * every further occurrence is a re-alarm CANDIDATE, but the durable dedup check (below) only
 * lets a new one through if the most recent unread alarm for that key is older than
 * REPEAT_OFFENDER_REALARM_MS — SD-LEO-INFRA-ACTIVATE-INERT-STALL-001-B / RCA 9a02a76d: a prior
 * exactly-at-threshold-only version fired ONE alarm on 2026-08-23 and never re-raised across the
 * following 9 days, including the exact 6h35m window this SD's incident happened in, because
 * nothing re-evaluates a pattern that already crossed the line once. A repeatedly-orphaning
 * sender IS a live, ongoing bug — silence past the first alarm is the defect, not the feature.
 *
 * SEVERITY-PRESERVING REROUTE: a row carrying `payload.severity === 'high'` is retargeted to
 * REROUTE_TO_KIND_HIGH_SEVERITY ('coordinator_request', this codebase's own existing
 * action-required kind, e.g. this file's repeat-offender alarm) instead of the routine
 * REROUTE_TO_KIND ('coordinator_reminder') — RCA 9a02a76d: the one real alarm that fired during
 * the incident was rescued into 'coordinator_reminder' by this exact sweep, twice, and read as a
 * routine reminder rather than the high-severity page it was.
 *
...
const { TERMINAL_REPLY_KINDS, INFORMATIONAL_KINDS } = workerStatusModule;

export const REROUTE_WINDOW_MS = 14 * 24 * 60 * 60 * 1000; // 14d
export const REROUTE_TO_KIND = 'coordinator_reminder';
export const REROUTE_TO_KIND_HIGH_SEVERITY = 'coordinator_request';
export const REPEAT_OFFENDER_THRESHOLD = 2;
export const REPEAT_OFFENDER_REALARM_MS = 6 * 60 * 60 * 1000; // 6h
...
export function isOrphanCandidate({ kind, recognizedKinds }) {
  ...
  // QF-20260901-097: an INFORMATIONAL_KINDS row (e.g. claim_boundary_released) is a reviewed
  // decision that no owner is needed at ANY role — it is not "undrained by this particular
  // role", it is undrained by design everywhere, so it must never read as an orphan needing
  // reroute (rerouted 16x in 14d against a coordinator role that was never meant to drain it).
  if (INFORMATIONAL_KINDS.includes(kind)) return false;
...
export async function sweepOrphanRows(supabase, {
  ...
  for (const row of rows) {
    ...
    const toKind = row.payload && row.payload.severity === 'high' ? REROUTE_TO_KIND_HIGH_SEVERITY : REROUTE_TO_KIND;
    const mergedPayload = {
      ...row.payload,
      kind: toKind,
      reroute: { from_kind: kind, to_kind: toKind, from_target: row.target_session, to_target: coordinatorTarget, from_role: role, at: nowIso, by_sweep: true },
    };
    ...
    if (priorCount + 1 >= REPEAT_OFFENDER_THRESHOLD) {
      // adversarial-review fix (round 1): the in-memory offenderCounts tally is only a
      // point-in-time snapshot per call, so two overlapping invocations (e.g. a manual CLI run
      // racing the cron) could each independently cross the threshold and double-alarm. A
      // durable check-before-insert on a stable alarm_key narrows that window — same idiom
      // already used for this exact "avoid a duplicate send" concern elsewhere in this codebase
      ...
      //
      // RE-ARM fix (RCA 9a02a76d): the dedup check used to be "any unread alarm at all -> skip
      // forever", so a pattern that crossed the threshold once alarmed exactly once, no matter
      // how many more days it persisted (measured: alarmed 2026-08-23, never again across 9
      // days including this SD's own 6h35m incident window). Narrowing the dedup to a
      // REPEAT_OFFENDER_REALARM_MS freshness window turns "silenced forever" into "silenced
      // until the last alarm goes stale" while still preventing per-row spam within one burst.
      const alarmKey = `orphan-repeat-offender:${role}:${kind}`;
      try {
        const realarmSinceIso = new Date(nowMs - REPEAT_OFFENDER_REALARM_MS).toISOString();
        const { data: existingAlarm } = await supabase
          .from('session_coordination')
          .select('id')
          .eq('payload->>alarm_key', alarmKey)
          .is('read_at', null)
          .gt('created_at', realarmSinceIso)
          .limit(1);
...
export default {
  sweepOrphanRows, isOrphanCandidate, REROUTE_WINDOW_MS, REROUTE_TO_KIND,
  REROUTE_TO_KIND_HIGH_SEVERITY, REPEAT_OFFENDER_THRESHOLD, REPEAT_OFFENDER_REALARM_MS,
};
