...
export function computeSlotDrift({ desired = [], actualByKey = {} } = {}) {
  ...
  for (const d of norm) {
    ...
    // QF-20260821-462: resume_uuid is populated on <0.1% of live rows by design (it is captured
    // once per resumed session, not carried on every heartbeat) -- an absent LIVE value is not
    // evidence of drift, only that this slot hasn't resumed yet. Comparing it like the other
    // fields flagged a spurious mismatch for nearly every desired slot that ever captured one.
    const mismatches = FIELDS.filter((f) => {
      if (f === 'resume_uuid' && !actual[f]) return false;
      return (actual[f] || null) !== (d[f] || null);
    });
