...
// FR-4 (post-merge): imported from the PURE module, not canary-guard.js. origin/main extracted the
// namespace predicate into startup-prompt-selection.js, which imports NOTHING — so this module now
// participates in no cycle, and CANARY_TRIGGER_KEY below can be imported by canary-guard.js and
// spawn-control.js instead of being duplicated into all three. That duplication was mine and it is
// now unnecessary; taking the better shape rather than merging my workaround forward.
import { isCanaryCallsign } from './startup-prompt-selection.js';
