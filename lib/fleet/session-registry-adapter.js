...
export async function loadLiveSlotIdentity(supabase) {
  ...
  for (const s of sessions || []) {
    ...
    slots.push({
      ...
      // Allowlisted in resume-context.test.js (FR-4, SD-LEO-INFRA-FLEET-SESSION-LIFECYCLE-001):
      // metadata.resume_uuid is populated on ~0.06% of rows, so this feeds computeSlotDrift a
      // near-always-null value -- a real, separate, tracked-but-unfixed spurious-mismatch risk.
