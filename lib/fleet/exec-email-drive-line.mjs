...
/**
 * PURE (QF-20260908-073): one ratified leg's trailing values, newest-first, from `rows` (the same
 * shape composeDriveLine already fetches). A row missing this leg (unmeasured that run) maps to
 * null, which driveFlatnessClause's own `usable` filter already excludes -- an absent measurement
 * is honestly reported as a smaller N, never a fabricated value.
 * @param {Array<object>} rows drive_reports rows, newest-first
 * @param {string} legId one of RATIFIED_LEG_IDS
 * @returns {Array<number|null>}
 */
export function trailingLegValues(rows, legId) {
  return (Array.isArray(rows) ? rows : []).map((r) => {
    const legs = Array.isArray(r?.drive_score?.measured_legs) ? r.drive_score.measured_legs : [];
    const m = legs.find((l) => l && l.leg === legId);
...
export async function composeDriveLine(io) {
  ...
  try {
    ...
    // QF-20260908-073: the TOTAL can pass "distinct >= 3" while every individual leg is flat --
    // three unrelated single-leg wobbles manufacture three distinct totals with no leg itself ever
    // varying. The ratified predicate exists to catch a flat signal, so it has to be applied at the
    // level a flat signal actually lives: per leg. Same predicate, same window, no new instrument
    // -- only the DISTINCT COUNT is reported per leg (not the full sentence repeated three times),
    // because this line is shared VERBATIM with the chairman SMS surface's BODY_CEILING and the
    // verbose per-leg sentence pushed real bodies over it, silently dropping the WHOLE line
    // (assembleMorningBody's own ceiling fail-soft) -- the opposite of "visible without a human
    // running the query by hand".
    const perLegClause = RATIFIED_LEG_IDS
      .map((legId) => `L${legId.match(/^leg(\d+)/)?.[1] ?? '?'}=${new Set(trailingLegValues(rows, legId).filter(usable)).size}`)
      .join(' ');
    return {
      line: `${formatDriveBreakdown(facts)}${dateTag} | ${clause} | per-leg distinct: ${perLegClause}`,
