...
const { seatClassFor, policyModelFor } = require('./model-policy.cjs');
...
function buildSessionLaunch(spec = {}, opts = {}) {
  ...
  let profileDir = null;
  // SD-FDBK-INFRA-SESSION-NAMED-ACCOUNT-001 FR-1 (coordinator ruling 1cbade73): distinct from
  // profileDir itself, this records WHETHER a specific named profile was the caller's intent —
  // 'named' when profileDir resolves to a real, non-sentinel directory; 'host-default' when the
  // HOST_DEFAULT_PROFILE sentinel was passed explicitly (a deliberate no-isolation choice); null
  // when buildSessionLaunch was called with no profile/preResolved at all (legacy/no-intent path).
  // session-register.cjs uses this (via FLEET_LAUNCH_PROFILE_INTENT below) to distinguish "a
  // named profile was expected but CLAUDE_CONFIG_DIR is missing" (stay unresolved — the exact
  // QF-20260726-514 hazard) from "no per-profile scope was ever in play" (safe to read the host
  // default). Neither profileDir's truthiness alone nor CLAUDE_CONFIG_DIR's presence alone can
  // make this distinction — HOST_DEFAULT_PROFILE and "no intent given" both leave profileDir null.
  let profileIntent = null;
  if (preResolved) { profileDir = preResolved; profileIntent = 'named'; }
  else if (profile) {
    profileDir = resolveProfileDir(profile, { env });
    profileIntent = profile === HOST_DEFAULT_PROFILE ? 'host-default' : 'named';
  }
  ...
  // SD-FDBK-INFRA-SESSION-NAMED-ACCOUNT-001 FR-1: propagate the intent distinction above so the
  // child's own SessionStart hook can tell a deliberate host-default/no-scope launch apart from
  // a named profile that failed to propagate. Only set when there IS an intent to record —
  // absent (not 'null' as a string) for the legacy no-intent path, matching this module's own
  // "absent, not invented" convention elsewhere.
  if (profileIntent) childEnv.FLEET_LAUNCH_PROFILE_INTENT = profileIntent;
  ...
  // QF-20260911-878 — PIN THE MODEL AT LAUNCH. Before this, a spawned seat carried no --model flag
  // at all and silently took whatever `/model` selection was last active on the machine, which is
  // how all five worker seats ended up on Fable and burned the scarce weekly bucket the ratified
  // model posture reserves for the judgment seats (Solomon/Adam/coordinator). Placed alongside
  // permissionMode above and BEFORE the --resume/--session-id branch for the same reason that one
  // is: both spawn paths (fresh + resume) need it, so it goes on the shared array.
  args.push('--model', policyModelFor(seatClassFor({ role })));
