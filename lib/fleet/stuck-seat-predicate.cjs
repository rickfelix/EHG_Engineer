 ...
 * direction. That is why, on the claim-boundary domain, this module stays advisory-only and drives
 * no claim release, quarantine or handback (see PRECEDENCE below) — a forged clock there would cost
 * a live worker its claim. It is NOT a blanket ban on actuation everywhere: as of
 * SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-C, singleton-role-election consumers actuate on this module's
 * verdict in a different domain, where the same forgery would require the service-role key (see
 * PRECEDENCE for the full scoping).
 */
 ...
 *
 * RULE: on disagreement, STUCK WINS FOR REPORTING and the incumbent wins for ACTUATION *on the
 * claim-boundary domain specifically* — this module is deliberately not wired into runDetectors
 * and drives no claim release, quarantine or handback. A false positive here costs a line of
 * dashboard text; a false positive in the incumbent costs a live worker its claim.
 *
 * SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-C amendment: "advisory-only" does NOT extend to every
 * consumer of this file. lib/fleet/genuine-worker.mjs's isKnownWedged (9 pre-existing production
 * consumers, predating this SD) and, as of this SD, lib/coordinator/adam-identity.cjs /
 * solomon-identity.cjs / singleton-refresh-sequencer.cjs all call classifySeat DIRECTLY to
 * actuate singleton role election, retirement, and worktree cleanup — a different actuation
 * domain from the claim-boundary one this PRECEDENCE block governs. Report loudly, actuate
 * conservatively remains the governing principle; "conservatively" is domain-specific, not a
 * blanket ban on actuation.
 */
