...
/** Extract the SD/QF key segment from a worktree cwd (e.g. '.../.worktrees/QF-2026.../x' ->
 *  'QF-2026...', '.../.worktrees/qf/QF-2026...' -> 'QF-2026...', '.../.worktrees/SD-X-001' ->
 *  'SD-X-001'). Returns null when the cwd carries no /.worktrees/ segment. QF-20260905-060. */
function worktreeKeyOfCwd(cwd) {
  const p = String(cwd == null ? '' : cwd).replace(/\\/g, '/');
  const m = p.match(/\/\.worktrees\/(?:qf\/)?([^/]+)/);
  return m ? m[1] : null;
}
...
function isSdExecutableHere(sd, ctx = {}) {
  try {
    ...
    if (target && (ctx.currentApp || inWorktree)) {
      ...
      // QF-20260905-060: a worktree cwd is committed context ONLY while its own SD/QF still has
      // a LIVE in-progress claim -- a "leftover" worktree (its build already shipped, the session
      // simply never cd'd away) has nothing left to conflict with and must be treated like the
      // bare shared root the comment above already exempts. Default true (byte-identical to
      // pre-fix behavior) so every existing caller that has not resolved this is unaffected; a
      // caller that has POSITIVELY verified the cwd's own worktree is not live (e.g.
      // directed-assignment.cjs, the specimen this QF fixes) passes cwdWorktreeIsLiveClaim:false
      // to skip the check, same as ctx.currentApp bypasses it for an explicit caller assertion.
      const cwdHasCommittedContext = ctx.currentApp ? true : ctx.cwdWorktreeIsLiveClaim !== false;
      if (current && cwdHasCommittedContext && target !== current) {
...
module.exports = { isSdExecutableHere, normalizeAppName, appOfCwd, worktreeKeyOfCwd };
