...
/**
 * QF-20260831-902: a COMMAND-TARGET WORK_ASSIGNMENT has no SD/QF row to target by
 * construction — dispatching /review-subsystem (or any harness that CREATES rows rather than
 * consuming one) was refused as "unresolvable" because the resolver only ever looked for an
 * SD/QF key. Recognized here as a first-class second kind of resolvable target, not a missing
 * SD key: payload.command names the harness/command (e.g. '/review-subsystem harness'), and
 * payload.premise_measured_at (the SAME field/discipline dispatch.cjs already requires on
 * instruction-carrying assignments — lib/coordination/premise-freshness.cjs) proves the
 * dispatcher actually looked before sending. A command with no premise stamp is NOT accepted as
 * command-target — that would let the guard be defeated by omission — it falls through to the
 * ordinary SD/QF resolution (and refuses/warns there, same as before this fix).
 * @param {object} row
 * @returns {boolean}
 */
function isCommandTargetAssignment(row) {
  const p = row && row.payload && typeof row.payload === 'object' ? row.payload : {};
  return typeof p.command === 'string' && p.command.trim().length > 0
    && typeof p.premise_measured_at === 'string' && Number.isFinite(Date.parse(p.premise_measured_at));
}
...
function describeUnreadableAssignment(row) {
  ...
  if (isCommandTargetAssignment(row)) return null; // resolvable as command-target, not a missing SD/QF key
...
/**
 * QF-20260907-463: mirrors isInformationalNudge() in scripts/worker-checkin.cjs — deliberately
 * DUPLICATED, not imported (this module is a documented LEAF with zero requires of
 * worker-checkin.cjs, to avoid a circular dependency with dispatch.cjs), same discipline as that
 * file's own completion-nudge duplication ("keep the two in sync"). A row stamped this way is
 * explicitly informational/broadcast and legitimately carries no structured directed target.
 */
function isInformationalNudgeRow(row) {
  const p = row && row.payload && typeof row.payload === 'object' ? row.payload : {};
  if (p.kind === 'completion_nudge' || p.informational === true) return true;
  return /^Next work available when /.test((row && row.subject) || '');
}

/**
 * QF-20260907-463: measured 58% of live WORK_ASSIGNMENT rows (49/84) carry no field the
 * 'directed' profile (structured-only, no text fallback — extractDirectedSd in
 * worker-checkin.cjs) can read, so the fast/priority directed-claim path can never fire for
 * them regardless of what the body text says — one specimen (a HIGH-severity, live-app defect)
 * sat unclaimed 12 hours despite naming its target in prose.
 *
 * DISTINCT from describeUnreadableAssignment above (which uses the BROADER 'worker' profile,
 * including a text scan): a row can be worker-readable — resolvable via the general claim path —
 * yet still directed-unresolvable, which is exactly the gap this measures. Excludes informational/
 * broadcast nudges and command-target rows, both of which are legitimately keyless by design.
 *
 * @param {object} row - the session_coordination row about to be written
 * @returns {null|{detail: string}} null when the row is fine, informational, command-target, or
 *   not a WORK_ASSIGNMENT.
 */
function describeUndirectableAssignment(row) {
  if (!row || row.message_type !== 'WORK_ASSIGNMENT') return null;
  if (isCommandTargetAssignment(row)) return null;
  if (isInformationalNudgeRow(row)) return null;
  const resolved = resolveAssignmentTarget(row, { profile: 'directed' });
  if (resolved.key) return null;
  return {
    detail: 'no structured directed target (payload.assigned_sd/sd_key/target_sd) — the '
      + 'directed/priority claim path cannot read this row even if a key is named only in prose'
  };
}
...
module.exports = {
  ...
  describeUndirectableAssignment,
  isCommandTargetAssignment,
  WORK_KEY_RE,
  SOURCES,
  PROFILES,
  NEWLY_TAUGHT
