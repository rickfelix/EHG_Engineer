 ...
 * @returns {string} e.g. 'headroom=62%' or 'headroom=--' when no reading exists yet
 */
function formatCapacityChip(identity, store) {
  ...
  if (!entry) return 'headroom=--';
  const headroom = 100 - bindingWeeklyPct(entry);
  // QF-20260902-459: the VALUE is headroom (100 - weekly usage), not the cap itself -- the old
  // `cap=` label read as "% consumed" to a reader and triggered a false wall-in-minutes alarm on
  // a 96%-headroom (4%-used) account. Name the value.
  return `headroom=${Math.round(headroom)}%`;
}
 ...
 *   computedStatus?:string|null, releasedAt?:string|null, role?:string|null, model?:string|null,
 *   effort?:string|null, toolSilentMinutes?:number|null}} row
 * @returns {'WORKING'|'AWAITING INPUT'|'DEEP WORK'|'UNKNOWN'|'MECHANICAL'|'PILOT WK1'|'OFF'}
 */
function computeSessionBadge({
  loopState, pAlive, isSilent, computedStatus, releasedAt, role, model, effort, toolSilentMinutes,
} = {}) {
  ...
  // QF-20260906-553: releasedAt is checked INDEPENDENTLY of computedStatus, never folded into
  // the array above — v_active_sessions's computed_status was measured NOT to flip to
  // 'released' when released_at is set (it stayed 'stale'), so an orphan-heartbeating dead
  // seat rendered WORKING for up to an hour after its sweep-recorded release. A session that
  // has genuinely been released is OFF regardless of what computedStatus or heartbeat age says.
  if (releasedAt) return 'OFF';
