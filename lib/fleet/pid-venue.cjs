...
function countMarkers(dir) {
  try {
    ...
    return fs.readdirSync(dir).filter((f) => /^pid-\d+\.json$/.test(f)).length;
  } catch {
    return null; // unreadable is indistinguishable from absent
  }
}

/**
 * @param {{markerDir?: string, markerDirsFn?: () => string[]}} [opts] an explicit markerDir
 *   pins the read to exactly that one directory (unchanged contract — existing callers that
 *   pass an explicit dir, e.g. tests/unit/fleet/pid-blind-venue-abstains.test.js, are
 *   unaffected). Omitting it aggregates markerCount across markerDirsFn() (defaults to the
 *   real markerDirs) — SD-LEO-INFRA-SESSION-IDENTITY-MARKER-CALLERS-001 (FR-2): a single
 *   directory's marker count is not a reliable capability signal when this host's live markers
 *   may sit entirely in a DIFFERENT git worktree checkout (measured live: one worktree held 1
 *   marker while main held 7 — a thin single-directory read reported false capability on a
 *   population it could see a fraction of).
 ...
 */
function pidVenueCapability({ markerDir, markerDirsFn = markerDirs } = {}) {
  const dirs = markerDir ? [markerDir] : markerDirsFn();
  let present = false;
  let markerCount = 0;
  for (const dir of dirs) {
    const count = countMarkers(dir);
    if (count === null) continue; // this directory is absent/unreadable — try the next
    present = true;
    markerCount += count;
  }
  const reportedMarkerDir = markerDir || dirs.join(',');
  if (!present) return { capable: false, reason: 'marker_dir_absent', markerDir: reportedMarkerDir, markerCount: 0 };
  ...
  if (markerCount === 0) return { capable: false, reason: 'marker_dir_empty', markerDir: reportedMarkerDir, markerCount };
  return { capable: true, reason: 'marker_dir_present', markerDir: reportedMarkerDir, markerCount };
}
