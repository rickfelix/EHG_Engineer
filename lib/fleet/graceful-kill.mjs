...
export function isWorkDurableAfterPrepark(pre, wasDirty) {
  ...
  switch (pre.action) {
    case 'noop':
      // SD-LEO-INFRA-FLEET-SESSION-LIFECYCLE-001 / FR-3. Reached ONLY when wasDirty === true — the
      // genuinely-clean case is the guard immediately above, which already returned. A 'noop' here
      // means decidePrepark declined to act (its own safety rule: NEVER auto-commit on a protected
      // branch, regardless of dirty state) while THIS function's independently-measured wasDirty
      // says the tree has real uncommitted work. That work was never made durable — reporting
      // durable:true here would be false confidence that killing this session loses nothing.
      // decidePrepark's own PROTECTED_BRANCHES rule is untouched; only THIS durability read changes.
      return {
        durable: false,
        why: pre.note ? `dirty tree, but prepark declined to act (${pre.note})` : 'dirty tree, but prepark declined to act',
      };
    case 'commit_only':
...
export async function gracefulKillSession(supabase, sessionId, deps = {}) {
  ...
  try {
    ...
    steps.push('preserve');
    // TESTING-agent finding (EXEC phase, SD-LEO-INFRA-FLEET-SESSION-LIFECYCLE-001 FR-3): a NULL
    // worktree_path is NOT the same fact as "could not verify cleanliness". isWorktreeDirty's
    // fail-closed contract exists for an UNRESOLVABLE check (git failed, timed out) — a session
    // with no worktree at all (9 of 11 live sessions on this host today, including coordinator/
    // Adam/Solomon seats) has NOTHING to preserve, so calling isWorktreeDirty(null) and trusting
    // its fail-closed true would halt every such kill on a false "dirty tree" it can never resolve
    // (runPreparkWip's own no_worktree_path noop, combined with wasDirty=true, hit the SAME
    // protected-branch-dirty durable:false branch this FR just added). Short-circuited BEFORE
    // either function runs, so the "no worktree" case never reaches their machinery at all.
    const hasWorktree = Boolean(session.worktree_path);
    // isWorktreeDirty OMITTED from deps entirely (not merely returning a value) fails CLOSED --
    // the SAME discipline this file's QF-20260728-054 comment already established for verifyGone:
    // absence of a check is not evidence of safety, and defaulting an omitted dep to "clean" is
    // exactly the anti-pattern that made wasDirty unconditionally false in production before this
    // FR wired scripts/fleet-kill.mjs's real isWorktreeDirty.
    const wasDirty = hasWorktree
      ? (typeof isWorktreeDirty === 'function' ? isWorktreeDirty(session.worktree_path) : true)
      : false;
    const pre = hasWorktree
      ? runPreparkWip({ worktreePath: session.worktree_path, sdKey: session.sd_key })
      : { action: 'noop', note: 'no worktree_path recorded for this session — nothing to preserve' };
    ...
    // ---- 7 RECORD ---------------------------------------------------------------------
    // Called for the fleet_verb_stop event, the only durable record that this kill happened —
    // so it runs on EVERY outcome, including refused. But stop() ALSO re-runs release +
    // hand-back, and that re-run was never CAS-guarded (this comment used to say it was). It is
    // only usually inert, because release_sd() nulls claude_sessions.sd_key and stop()'s
    // hand-back no-ops on the null. If our own release leg above failed transiently, sd_key
    // survives, stop()'s retry succeeds, and the item gets handed back with gone === false —
    // undoing step 6's fail-closed decision from a different file, while our return value still
    // reports handback:null. So gone is FORWARDED rather than assumed: the component that
    // learned the process might still be alive is the one that must say so.
    steps.push('record');
    if (recordStop) await recordStop(sessionId, { gone });
