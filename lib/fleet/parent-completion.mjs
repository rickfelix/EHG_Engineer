 ...
 * children terminal, PLAN-TO-LEAD accepted, no accepted LEAD-FINAL yet. A parent_completion
 * WORK_ASSIGNMENT is ~10 minutes of work; this class was previously invisible to both
 * dispatchable_backlog_size (a different supply class by construction) and the belt, and was
 * caught only by the 24h STUCK_WITHOUT_HOLD_REASON breach class (live specimen:
 * SD-LEO-FEAT-MEDIA-PRODUCTION-CAPABILITY-001, 28h unowned).
 *
 ...
 * @returns {Promise<{count: number, oldestAgeMs: number|null, parents: Array<{sd_key: string, age_ms: number}>}>}
 */
export async function countCompletionReadyParents(supabase, { nowMs = Date.now() } = {}) {
  ...
  return { count: ready.length, oldestAgeMs: ages.length ? Math.max(...ages) : null, parents: ready };
}
