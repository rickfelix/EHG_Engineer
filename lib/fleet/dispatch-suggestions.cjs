'use strict';
/**
 * dispatch-suggestions.cjs — SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-1/FR-2/FR-4).
 *
 * ADVISORY ONLY. This module computes ranked (worker, model, effort) fit suggestions for
 * dispatchable belt items and the shared "should I defer to a closer-fit live peer" primitive
 * FR-2 uses. It NEVER assigns: there is no import of tryClaim/claim_sd/sd-start anywhere in this
 * file, and every function here is pure or a read-only DB query. Enforced structurally by
 * tests/unit/fleet/dispatch-suggestions.test.js (TS-1).
 *
 ...
 * from lib/fleet/tier-ladder.cjs's EXISTING dense-rank space (deriveLiveLadder / rankForModelEffort),
 * the same rank space claim-eligibility and dispatch already enforce. No parallel capability-score
 * formula is defined here (no MODEL_STRENGTH/EFFORT_STRENGTH re-declaration) — this module only
 * ever imports ranks, never re-derives them. Registered as a documentation-contract consumer in
 * lib/eval/routing-consumption.mjs's ROUTING_DOCTRINE_CONSUMERS.
 *
...
/** FR-1/FR-2 shared fit primitive: eligible (>= floor) live worker ranks for an item. */
function eligibleFitRanks(minTierRank, liveRanks) {
  const min = Number(minTierRank);
  if (!Number.isFinite(min)) return [];
  return (Array.isArray(liveRanks) ? liveRanks : []).filter((r) => Number.isFinite(r) && r >= min);
}

/**
 * The BEST (closest-to-floor) live+eligible rank for an item, or null when none qualify.
 * "Best fit" = least over-qualified, so scarce top-tier capacity is conserved for items whose
 * floor actually needs it.
 * @param {number} minTierRank
 * @param {number[]} liveRanks
 * @returns {number|null}
 */
function bestFitRankForItem(minTierRank, liveRanks) {
  const eligible = eligibleFitRanks(minTierRank, liveRanks);
  return eligible.length ? Math.min(...eligible) : null;
}

/**
 * FR-2 primitive: should THIS worker defer (skip, not claim) an item because a closer-fit live
 * peer exists? Pure/sync — callers pre-resolve myRank/minTierRank/liveRanks before calling.
 * @param {{ myRank?: number, minTierRank?: number, liveRanks?: number[] }} args
 * @returns {boolean}
 */
function shouldDeferForBetterFit({ myRank, minTierRank, liveRanks } = {}) {
  const my = Number(myRank);
  const min = Number(minTierRank);
  if (!Number.isFinite(my) || !Number.isFinite(min)) return false;
  const best = bestFitRankForItem(min, liveRanks);
  return best !== null && best < my;
}
 ...
 * @param {object} sb
 * @param {{ limit?: number, excludeSessionId?: string }} [opts]
...
async function generateAndPersistSuggestions(sb, opts = {}) {
  const [items, quotaPosture, familiarityBySdType, snapshot] = await Promise.all([
    ...
    liveWorkerCapabilitySnapshot(sb, { excludeSessionId: opts.excludeSessionId }),
  ]);
  const results = [];
  let written = 0;
