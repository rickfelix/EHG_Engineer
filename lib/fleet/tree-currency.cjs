...
function assessTreeCurrency({
  ...
  try {
    ...
    // QF-20260905-611: dirt ALONE no longer disqualifies self-heal — only the branch does.
    // `git pull --ff-only` is itself the safety check: it either fast-forwards cleanly
    // (untracked files are irrelevant to a merge; a local modification survives untouched
    // when the incoming commits do not touch that path) or REFUSES with no partial state
    // when a real collision exists (a tracked-file modification or an untracked file the
    // incoming commit would overwrite) — enforceTreeCurrency's existing try/catch around
    // the pull already converts that refusal into a TreeStaleError. Gating eligibility on
    // `!dirty` pre-empted a decision git already makes safely, and did so on EVERY porcelain
    // line including untracked files with zero relation to the incoming diff — measured: a
    // shared root holding ~1,200 untracked files (scripts/one-off/_*.mjs, .artifacts/*) was
    // permanently non-self-healable even when a fast-forward would converge with zero
    // collisions, refusing spawns whose fix was one already-safe command away.
    const selfHealable = branch === SELF_HEALABLE_BRANCH;
...
function enforceTreeCurrency({
  ...
  // QF-20260905-611: dirt is no longer a blocker here — selfHealable now depends only on
  // branch (see assessTreeCurrency above), so the only way this fallthrough is reached for
  // reason='behind' is an off-branch tree, or a healable tree whose CALLER forbade healing
  // (allowSelfHeal:false). A genuine ff-only collision on a healable tree is NOT this
  // branch — it throws earlier, inside the try/catch around the pull, with its own message
  // naming the git failure.
    ...
    ? `THE FAULT IS BEING BEHIND: ${assessment.behind} commit(s) behind ${baseRef}`
      + (blocker
        ? `. The auto-fast-forward could not run because ${blocker} `
          + `(branch=${assessment.branch}); refusing rather than mutating an off-branch tree, `
          + `which would risk clobbering a peer worktree.`
        // On-branch, so a fast-forward WOULD have been attempted — this path is reached only
        // when the CALLER forbade healing (allowSelfHeal:false, the reaper's deliberate
        // posture: unattended, shared root, refusing costs it nothing). The failed-fast-
        // forward and did-not-converge cases throw earlier with their own messages, so they
        // never land here. Saying "self-heal was permitted" would be exactly backwards for
        // the only caller that reaches this branch.
        : `. A fast-forward would have been attempted (on '${SELF_HEALABLE_BRANCH}') but this `
