...
// for metadata.scheduling_constraint, mirroring isLeadBlockerActive exactly. The one live
// specimen (SD-LEO-FEAT-EVA-VENTURE-IDEATION-001) carries {note, source} narrative prose with no
// structured resolution field (no resolved_at / unblocks_when) — there is nothing to evaluate,
// only presence to detect. Any malformed/legacy shape (a bare string, a number) is still ACTIVE
// unless it is one of the recognized-INACTIVE shapes below; this axis must never throw on an
// unanticipated shape, since the field has zero other readers to have validated it against.
