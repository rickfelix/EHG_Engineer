 ...
 */
async function setHold(sb, sdKey, { setBy, reason, unfenceCondition } = {}) {
  ...
  if (!trimmedSetBy || !trimmedReason || !trimmedCondition) {
    return { set: false, sdKey, error: 'setBy, reason, and unfenceCondition are required (a fence must name who set it, why, and what clears it)' };
  }
  const { mergeMetadataKeys } = await import('../coordinator/safe-metadata-merge.mjs');
