...
async function releaseHold(sb, sdKey, { releaser, reason } = {}) {
  ...
  const merge = await mergeMetadataKeys(sdKey, { unfenced_at: unfencedAt, unfenced_by: releaser, unfenced_reason: reason });
...
module.exports = { draftDepsSatisfied, evaluateDispatchEligibility, baselinedCandidateEligible, classifyDispatchIneligibility, classifyAllDispatchIneligibility, coordinatorReservation, isSeatBusyOnDirectedWork, parentLeadPending, parentLeadPendingVerdict, TEST_FIXTURE_KEY_RE, resolveHoldProvenance, formatHoldProvenance, releaseHold, isDispatchAuthorized, liveClaimWriteFenceReason, CLAIM_WRITE_FENCE_AXES, execBoundaryHoldReason, isLeadBlockerActive, isSchedulingConstraintActive, INELIGIBILITY_AXIS_NAMES };
