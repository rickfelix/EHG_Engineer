...
const INELIGIBILITY_AXES = [
  ...
  function tierAxes(row, ctx) {
    ...
    // is advisory, not blocking -- tierRankVerdict returns 'above_worker_tier_advisory' instead of
    // 'above_worker_tier' in that case, and we let it fall through unblocked below (same branch
    // shape as an unscored SD), never treating it as one of this axis's blocking reasons.
