...
const INELIGIBILITY_AXES = [
  ...
  function tierAxes(row, ctx) {
    ...
    // QF-20260831-419 (chairman order, ratified by adam a78170fa): WORK-DOWN-NEVER-UP and
    // stamp-missing/reserved-backlog claim-side fencing retired to advisory-only, mirroring
    // lib/coordinator/dispatch.cjs assertWorkerTierAllowed. min_tier_rank/tier_rank remain
    // stamped as data; this axis no longer fences a self-claim on either verdict.
    const hasProvenance = !!(row.metadata && row.metadata.min_tier_rank_reason);
    const verdict = tierRankVerdict(ctx.worker_tier_rank, minRank, { hasProvenance });
    if (verdict === 'tier_stamp_missing' || verdict === 'above_worker_tier') {
      console.info(`[claim-eligibility] ADVISORY (not blocking): tier verdict '${verdict}' for ${row.sd_key || row.id} per QF-20260831-419 (tier enforcement retired to advisory).`);
    }
    ...
    if (Number.isFinite(minRank) && minRank < workerRank && ctx.lower_tier_backlog_data) {
      if (!lowerTierBacklog(minRank, ctx.lower_tier_backlog_data)) {
        console.info(`[claim-eligibility] ADVISORY (not blocking): reserved_no_lower_backlog for ${row.sd_key || row.id} per QF-20260831-419 (tier enforcement retired to advisory).`);
      }
    }
    return null;
  },
...
module.exports = { draftDepsSatisfied, evaluateDispatchEligibility, baselinedCandidateEligible, classifyDispatchIneligibility, classifyAllDispatchIneligibility, coordinatorReservation, isSeatBusyOnDirectedWork, parentLeadPending, parentLeadPendingVerdict, TEST_FIXTURE_KEY_RE, resolveHoldProvenance, formatHoldProvenance, isHoldReleased, releaseHold, isDispatchAuthorized, liveClaimWriteFenceReason, CLAIM_WRITE_FENCE_AXES, execBoundaryHoldReason, isLeadBlockerActive, isSchedulingConstraintActive, INELIGIBILITY_AXIS_NAMES };
