...
export async function runRebootRespawnDrill({ supabase, loadFn, rosterFn, spawnFn, logFn, queryEventsFn, queryLifecycleEventsFn, buildInvocationFn, live = false, now, opts = {} } = {}) {
  ...
  const perSlotResumeOk = runResult.results.every((r, i) => {
    // SHIP-REVIEW FIX (adversarial review, PR #7168, finding F7): a skipped slot (SD-LEO-INFRA-
    // FLEET-CANNOT-SELF-001 FR-1/FR-3) has invocation:null -> argv=[] -> !argv.includes('--resume')
    // is vacuously true for a no-resume-token slot, so this check passed and the detail string below
    // claimed "all N slot(s) relaunched" for a slot that was never relaunched at all. A skip is a
    // legitimate, correct outcome (see check 4, which already excludes it from respawn evidence
    // correctly) but it is NOT a successful per-slot relaunch, so it must fail THIS check honestly.
