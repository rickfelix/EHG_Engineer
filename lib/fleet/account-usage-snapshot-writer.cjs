...
async function fetchLastKnown(supabase, accountNames = [], opts = {}) {
  const names = (Array.isArray(accountNames) ? accountNames : []).filter((n) => typeof n === 'string' && n);
  if (!supabase || names.length === 0) return new Map();
  try {
    // Ordered newest-first and de-duplicated in JS rather than with DISTINCT ON, so this needs no
    // SQL dialect assumptions and stays readable. The row count is bounded by the registry size.
    //
    ...
    // just written — percentages NULL, newest fetched_at. Without this filter that row shadows the
    // very reading we are trying to recover and the strip renders "Quota spent" with no number:
    // FR-6 defeated end-to-end by its own write. Reordering the route does NOT fix it either, since
    // a second consecutive exhausted tick shadows it again. "Last KNOWN" means the last row that
    // actually carried a number, so the filter belongs here, at the definition.
    ...
    const out = new Map();
    for (const row of data) if (!out.has(row.account_name)) out.set(row.account_name, row);
    return out;
  } catch {
    return new Map(); // history is a nicety; the live reading is the product
  }
}
