...
/**
 * Is this error "the table isn't there yet" rather than a real fault?
 *
 * MEASURED AGAINST THE CLIENT WE ACTUALLY USE, not against the wording I assumed. Postgres says
 * `relation "x" does not exist`, but nothing here talks to Postgres directly — supabase-js goes
 * through PostgREST, which answers `Could not find the table 'public.x' in the schema cache`. The
 * first version of this check tested only the Postgres phrasing and was fed a hand-written message
 * in its unit test, so it agreed with itself and reported pending_migration:false on a live run
 * where the migration was the only thing pending. The distinction is the entire purpose of the
 * flag: it separates "chairman has not applied the gated migration" from "something is wrong".
 */
function isMissingTableError(msg) {
  return /does not exist|could not find the table|schema cache/i.test(String(msg || ''));
}
  ...
  try {
    ...
    if (error) {
      ...
      logger?.warn?.(JSON.stringify({
        ...
        rows: rows.length,
        pending_migration: isMissingTableError(msg),
...
module.exports = {
  ...
  fetchLastKnown, withLastKnown, isMissingTableError,
};
