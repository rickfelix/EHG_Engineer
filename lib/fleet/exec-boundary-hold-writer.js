 ...
 */
export async function setExecBoundaryHold(supabase, sdId, { reason, owner, reviewAt, releaseCondition, writingSessionId } = {}, mergeMetadataKeysFn = null) {
  ...
  // SD-LEO-FIX-STRATEGIC-DIRECTIVES-UPDATED-001: a full-blob .update({metadata: nextMeta}) here
  // is exactly the hold-flag-resurrection race lib/coordinator/safe-metadata-merge.mjs's own
  // docblock names as the reason it exists — a concurrent coordinator clearing an UNRELATED
  // metadata key between this read and this write would have that clear silently reverted.
  // Still need ONE read for the exec_boundary_hold_reason fallback below; the WRITE is now
  // scoped to only the patch keys via mergeMetadataKeys(), never the whole object.
  ...
  const { merged, error } = await mergeFn(fresh.sd_key, patch);
  if (!merged) throw new Error('[exec-boundary-hold-writer] write failed: ' + (error || 'no row matched'));
 ...
 */
export async function clearExecBoundaryHold(supabase, sdId, { clearedBy } = {}, mergeMetadataKeysFn = null) {
  ...
  // overwrite — see setExecBoundaryHold's comment for the race this closes.
  const mergeFn = mergeMetadataKeysFn || (await import('../coordinator/safe-metadata-merge.mjs')).mergeMetadataKeys;
  const { merged, error } = await mergeFn(fresh.sd_key, {
    exec_boundary_hold: false,
    exec_boundary_hold_cleared_at: new Date().toISOString(),
    exec_boundary_hold_cleared_by: clearedBy,
  });
  if (!merged) throw new Error('[exec-boundary-hold-writer] clear write failed: ' + (error || 'no row matched'));
}
