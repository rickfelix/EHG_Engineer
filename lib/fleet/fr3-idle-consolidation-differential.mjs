 ...
 * stopped calling (isLiveCountableWorker, isDispatchableFleetMember, liveFleetWorkers,
 * isRecentlyReleased) -- none of these were deleted, so this is not a reimplementation. There are
 * TWO exceptions, both because a later fix touched a "pre" baseline this harness depends on:
 *
 * - coordinator-idle-qf-hint.mjs's eligibleIdleWorkers: its internal body WAS overwritten by the
 *   FR-1 migration (its public signature and every exclusion reason were preserved, only the
 *   mechanism moved to seatIdleVerdict) -- `oldEligibleIdleWorkersBody` below is a VERBATIM copy of
 *   the function body as it stood at commit fea73d942fe^ (the parent of the first
 *   seat-idle-predicate.mjs commit, i.e. the last commit before this SD touched this file), cited
 *   so it can be re-diffed against `git show fea73d942fe^:scripts/coordinator-idle-qf-hint.mjs` at
 *   any time.
 * - isBuildForbiddenSession (lib/claim/build-forbidden-session.cjs): used live in `adamPre` to
 *   stand in for pre-migration adam-quiet-tick behavior, on the assumption it would stay untouched.
 *   QF-20260904-968 fixed its is_coordinator check to also accept the JSON-string shape -- correct
 *   for its own purpose (claim authorization), but it would retroactively "fix" this harness's
 *   pre-migration baseline and hide the very TS-2 delta this suite exists to pin. `adamPre` below
 *   uses `frozenPreQF20260904968IsBuildForbiddenSession`, a VERBATIM copy of the boolean-only check
 *   as it stood at commit cd86ff79d7f (the last commit before QF-20260904-968), re-diffable via
 *   `git show cd86ff79d7f:lib/claim/build-forbidden-session.cjs`.
 *
...
// Frozen pre-QF-20260904-968 snapshot of isBuildForbiddenSession's is_coordinator check
// (boolean-only), verbatim as of commit cd86ff79d7f. See the file header for why this cannot be
// the live import.
function frozenPreQF20260904968IsBuildForbiddenSession(metadata) {
  const md = metadata || {};
...
function adamPre(s) {
  return adamQueryLayerOk(s) && !frozenPreQF20260904968IsBuildForbiddenSession(s.metadata) && !s.released_at;
}
function adamPost(s) {
...
export const FROZEN_POPULATION = [
  ...
  // flips on the STRING shape specifically (pre-migration isBuildForbiddenSession's `=== true` check
  // was boolean-only, per TS-2's own note -- since fixed live by QF-20260904-968, frozen here for
  // this harness's own purposes). TS-2's coverage against the raw predicate is real but does not prove the
...
export const MATRIX = {
  ...
  // adam-quiet-tick: pre-migration isBuildForbiddenSession's `=== true` check was boolean-only
  // (TS-2's own documented gap, since fixed by QF-20260904-968 -- see frozenPreQF20260904968...
  // above) -- the bool shape was ALREADY excluded (no change); the STRING shape flips.
