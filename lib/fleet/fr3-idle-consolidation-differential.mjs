...
const FIXTURE_SD_HOLDERS = new Set(['sd-holder-authoritative-1']);
...
export const FROZEN_POPULATION = [
  ...
  // NOTE: stale-is-coordinator (bool and JSON-string shapes) and the active-coordinator-by-id
  // negative control are DELIBERATELY NOT in this population: that regression guard is TS-2,
  // already covered directly against seatIdleVerdict in seat-idle-predicate.test.js (parameterised
  // over both shapes + the active-coordinator control), and "stale-is-coordinator" is not one of
  // FR-3's six named matrix reasons ({fixture-session, directed-work, qf-holder-authoritative,
  // released-shell, spin-up-grace, stale-sd_key-mirror}). Including it here would require a 7th
  // matrix reason FR-3 does not name; TS-2's existing coverage is the right home for it.
];
...
export const MATRIX = {
  ...
  'capacity-inputs': {},
  'fleet-dashboard': {},
  'adam-quiet-tick': { 'fixture-session': true, 'directed-work': true, 'qf-holder-authoritative': true, 'spin-up-grace': true },
};
