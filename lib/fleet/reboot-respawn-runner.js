...
async function reconcileSpawnedSession(supabase, { pid, sessionId } = {}, opts = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-C (FR-1, found by EXEC-phase TESTING evidence
      // 50b3e3c0-0f5f-460d-a3ea-f6472eb0976f as a 4th, unenumerated caller of
      // checkNewSessionHealth): +last_tool_at so a genuinely-stuck respawn (rare, but possible if
      // the new process hangs before its first tool call) can still be caught. A brand-new session
      // legitimately has no last_tool_at yet at this check's ~2.5s-post-spawn timing -- that
      // UNKNOWN case is handled by checkNewSessionHealth folding UNKNOWN to healthy (only a
      // POSITIVELY CONFIRMED STUCK verdict fails), not by omitting the column here.
      const { data: session } = sessionId
        ? await supabase.from('claude_sessions')
          .select('session_id, heartbeat_at, loop_state, last_tool_at').eq('session_id', sessionId).maybeSingle()
        : await supabase.from('claude_sessions')
          .select('session_id, heartbeat_at, loop_state, last_tool_at').eq('pid', pid).maybeSingle();
 ...
 * @param {number}   [opts.coordinatorResolverTimeoutMs] - SD-LEO-INFRA-FLEET-CANNOT-SELF-001 FR-3 (ship-review F1): bounds coordinatorResolverFn so a stalled (never resolves/rejects) read cannot hang this loop; defaults to 5000. A timeout degrades to the SAME fail-toward-spawn outcome as an explicit resolver error.
...
export async function runRebootRespawn(opts = {}) {
  ...
  for (const slot of slots) {
    ...
    if (role === 'coordinator') {
      ...
      // SHIP-REVIEW FIX (adversarial review, PR #7168, finding F3): a non-numeric uptimeFn()
      // (NaN, undefined, a wrongly-async injection) used to make withinBootWindow FALSE, engaging
      // the dedup guard -- the one place in this block that failed TOWARD skipping instead of
      // toward spawning, contradicting every other uncertainty branch here. An unreadable clock is
      // treated as "inside the window" (always attempt), matching the stated intent everywhere else.
      const withinBootWindow = !Number.isFinite(uptimeMs) || uptimeMs < BOOT_WINDOW_MS;
      if (!withinBootWindow) {
        let liveCoordinatorId = null;
        try {
          // SHIP-REVIEW FIX (adversarial review, PR #7168, finding F1): coordinatorResolverFn's
          // production default issues live Supabase queries with no timeout anywhere in that
          // chain -- an unresponsive (not erroring) network leaves this await pending indefinitely,
          // which previously stalled EVERY slot queued behind it in this same for-loop, on the sole
          // post-reboot recovery path, at exactly the moment (just-booted host) a stalled network is
          // most likely. A THROW already failed toward spawning via the catch below; a HANG did not
          // fail toward anything -- it just hung. Bounded so "no answer in time" degrades to the
          // same fail-toward-spawn outcome as an explicit error, never to an indefinite stall.
          liveCoordinatorId = await Promise.race([
            coordinatorResolverFn(supabase),
            new Promise((resolve) => setTimeout(() => resolve(null), opts.coordinatorResolverTimeoutMs ?? 5000)),
          ]);
        } catch (e) {
