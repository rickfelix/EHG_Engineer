...
/**
 * QF-20260724-739 (scope widened per Solomon Mode-B deep-sweep b3c86f61): a fire-and-forget
 * spawnFn() returning without throwing is NOT evidence of a live session -- the code path taken
 * on real recovery and on a silent no-op look IDENTICAL (forgeable-by-construction). Reconcile
 * the spawned pid against a REAL, heartbeating claude_sessions row (bounded poll, never a single
 * point-in-time read) using the SAME health check singleton-refresh-sequencer.cjs already uses
 * to gate retirement decisions -- reused, not reimplemented.
...
export async function runRebootRespawn(opts = {}) {
  ...
  for (const slot of slots) {
    ...
    // QF-20260724-739 (widened): payload.live is derived FROM ground-truth reconciliation, not
    // from spawnFn() having returned without throwing -- see reconcileSpawnedSession() above.
