 ...
 * + pause (pre-existing, session-level) -> fenced-identity check -> write allowlist (write-class
 * actions only) -> outbound-messaging authorization (outbound-class actions only) -> per-session
 * action cap (consumed LAST among guards, so a refusal for any other reason never burns cap budget)
 * -> audit log write -> actionFn. Every refusal is itself audited via auditRefusal() before returning.
 *
...
export async function driveAction(supabase, session, {
  ...
  if (!capResult.allowed) {
    ...
    return { executed: false, reason: 'session_cap_exceeded' };
  }
