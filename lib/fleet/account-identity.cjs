...
/**
 * QF-20260828-376: `prior` is untrusted input — it comes from JSON.parse() of a persisted
 * state file (loadLastAccountIdentity() in adam-quiet-tick.mjs), which can legitimately hold a
 * truthy-but-malformed value (e.g. `{}`) from an older schema or a partial/corrupted write.
 * `current` always comes fresh from getAccountIdentity(), which already guarantees this exact
 * shape or null — so only `prior` needs validating.
 * @param {*} x
 * @returns {boolean}
 */
function isValidIdentity(x) {
  return !!x &&
    typeof x.email === 'string' && x.email.length > 0 &&
    typeof x.orgName === 'string' && x.orgName.length > 0 &&
    typeof x.accountUuid8 === 'string' && x.accountUuid8.length > 0;
}
 ...
 *
 * QF-20260828-376: a `prior` that is truthy but missing/malformed on any of the 3 required
 * fields (e.g. `{}` from a corrupted or pre-schema state file) is treated the SAME as no prior
 * — cold start, no event — rather than being compared field-by-field and producing a notice
 * with `undefined` values (the observed defect: "switched from undefined (undefined) to ...").
 *
...
function detectAccountSwitch(prior, current) {
  if (!isValidIdentity(prior) || !current) return { changed: false, event: null };
  const changed =
...
module.exports = { getAccountIdentity, detectAccountSwitch, resolveRealConfigPath };
