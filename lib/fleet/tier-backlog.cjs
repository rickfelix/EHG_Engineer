...
async function fetchLowerTierBacklogData(supabase) {
  try {
    ...
    const { liveFleetWorkers, isRecentlyReleased } = await import('./genuine-worker.mjs');
    ...
    const [{ data: fleetSds }, { data: fleetSessions }] = await Promise.all([
      ...
      supabase.from('claude_sessions')
        .select('session_id, status, metadata, heartbeat_at, sd_key, claimed_at, worktree_path, continuous_sds_completed, released_at')
    ...
    // QF-20260830-660 (census follow-on, docs/architecture/fleet-liveness-predicate-consumer-census.md):
    // this feeds classifyDispatchIneligibility's dispatch-now capacity, the same class of decision
    // isRecentlyReleased was built for (07:56:44Z incident) — a just-released seat is still "live"
    // per liveFleetWorkers/everClaimed but not actually available, so it must not count as idle
    // capacity here (unlike the reporting-only consumers this census audited, which are SUPPOSED
    // to see it).
    const now = Date.now();
    const idleWorkers = live.filter((w) => !claimedSessionIds.has(w.session_id) && !isRecentlyReleased(w, now));
    return {
