 ...
 * @property {number} [sdHolderFreshnessWindowMs] - QF-20260903-789: opt-in, additive refinement of
 *   the sd-holder-authoritative axis above. A held claim excludes from idle ONLY while the seat is
 *   ACTUALLY ADVANCING (last_tool_at within this window of nowMs) — Solomon's definition: "a seat is
 *   busy when it is advancing work, not when it is holding work; a claim records ownership, not
 *   activity." 0/undefined = no-op — the axis keeps its pre-QF-789 unconditional-exclusion
 *   behaviour, so every existing/future consumer that does not opt in is byte-identical. A holder
 *   with a missing/unparseable last_tool_at fails toward "busy" (still excluded) — never silently
 *   free a real holder's seat over absent data.
 ...
 * @property {Set<string>} [tailInFlightSessionIds] - QF-20260905-755: session_ids that hold NO
 *   current claim (claim released — the SD/QF completion path already clears it) but authored the
 *   just-completed SD's own follow-up "tail" PR (the docs(<SD>)/CHANGELOG pattern /document writes
 *   post-completion), and that tail PR's CI is still pending. Without this axis such a seat is
 *   claim-less AND heartbeat-stale-looking (it is genuinely waiting on CI, not working), so every
 *   OTHER axis reads it as idle and the coordinator fires escalating idle nudges at a seat that is,
 *   in fact, still finishing its own SD (witnessed live: Hotel/#8246, three nudges at 12min/20min/
 *   direct-instruction while blocked on CI for a just-completed SD's tail PR). No-op (empty/
 *   undefined) means this axis is not applied — matches every existing consumer, none of which is
 *   tail-aware today.
...
const AXES = [
  ...
  {
    // QF-20260904-962: role='adam' had an exclusion axis but role='coordinator'/'solomon' had
    // none — a live specimen (session 6ae69e94, role='coordinator', non_fleet=null,
    // is_coordinator absent) already has this exact shape (not currently exploitable: that
    // specimen is released and stale, excluded by other axes). lib/coordinator/self-id-
    // handshake.cjs documents a real post-restart window where a FRESH session of this same
    // shape exists before the coordinator's own is_coordinator flag is set — in that window
    // every consumer of this predicate could count the coordinator's own live seat as idle
    // capacity. Both singleton roles closed here, matching role-adam's own pattern.
    reason: 'role-singleton',
    test: (s) => s.metadata?.role === 'coordinator' || s.metadata?.role === 'solomon',
  },
  {
    reason: 'non-fleet',
  ...
  },
  {
    ...
    test: (s, ctx) => {
      let holds;
      if (ctx.sdHolderSessionIds === undefined) holds = false; // no-op default
      else if (ctx.sdHolderSessionIds === null) holds = !!s.sd_key; // fail-open to the stale mirror
      else holds = !!(s.session_id && ctx.sdHolderSessionIds.has(s.session_id)); // authoritative
      if (!holds) return false;
      // QF-20260903-789: opt-in only. Omitting sdHolderFreshnessWindowMs reproduces the
      // pre-QF-789 unconditional exclusion exactly (holds -> excluded, full stop).
      if (!ctx.sdHolderFreshnessWindowMs) return true;
      const raw = s.last_tool_at;
      if (raw == null) return true; // fail toward "busy" — never free a real holder over absent data
      const ageMs = (ctx.nowMs ?? Date.now()) - Date.parse(raw);
      // fresh (still advancing) -> excluded (busy); stale -> falls through to later axes / idle
      // (HELD: owns produced state but blocked externally, available for other work).
      return Number.isFinite(ageMs) && ageMs < ctx.sdHolderFreshnessWindowMs;
    },
  },
  {
  ...
  {
    // QF-20260905-755: a seat with an open, CI-pending tail PR on its just-completed SD is
    // advancing toward that SD's actual finish line, not idle. See the ctx typedef above for the
    // full rationale; population lives in lib/fleet/idle-ctx-population.mjs (bounded to recently-
    // completed SDs only, so this never scans the whole PR history).
    reason: 'post-completion-tail',
    test: (s, ctx) => !!(ctx.tailInFlightSessionIds && ctx.tailInFlightSessionIds.size && s.session_id && ctx.tailInFlightSessionIds.has(s.session_id)),
  },
  {
