...
import { everClaimed, isFleetWorker, liveFleetWorkers, isKnownWedged } from './genuine-worker.mjs';
 ...
 * string or a claude_sessions row. Returns true when the id carries an EXPLICIT fixture marker
 * (FIXTURE_SESSION_RE), OR — QF-20260903-195 — when metadata.source ends in "-test" (a sentinel
 * id can be an ordinary-looking UUID, e.g. 00000000-0000-0000-0000-fff7000fffff/source=fr7-test,
 * so the id-shape check alone misses it; only checkable when the full row is passed, not a bare
 * id string).
 *
...
export function isFixtureSession(idOrSession) {
  try {
    const source = typeof idOrSession === 'string' ? null : idOrSession?.metadata?.source;
    if (typeof source === 'string' && source.endsWith('-test')) return true;
 ...
 *
 * DO NOT edit this rationale without reading everClaimed's own divergence rationale
 * (lib/fleet/genuine-worker.mjs:163-164, QF-20260728-930) and the full consumer census
...
/**
 * Live population for idle-capacity absorption (coordinator-idle-qf-hint.mjs and any caller
 * with the same need): identity-only membership via isDispatchableFleetMember (which does NOT
 * require prior claim history) combined with the SAME liveness/wedge gating liveFleetWorkers
 * applies (heartbeat freshness + !isKnownWedged).
 *
 * QF-20260903-831: liveFleetWorkers' everClaimed requirement filters out a freshly-spawned,
 * live, never-claimed seat BEFORE eligibleIdleWorkers' own spin-up-grace logic ever sees it --
 * the seat that most needs an idle-capacity hint is exactly the one this excluded. Use this
 * population instead wherever "has this session ever claimed" is not the question being asked.
 * @param {Array<object>} sessions
 * @param {string} coordinatorId
 * @param {number} nowMs
 * @param {number} [windowMs=900000] - heartbeat freshness window (default 15 min)
 */
export function liveDispatchableFleetMembers(sessions, coordinatorId, nowMs, windowMs = 900000) {
  return (sessions || []).filter(
    (s) =>
      isDispatchableFleetMember(s, coordinatorId, nowMs) &&
      s.heartbeat_at &&
