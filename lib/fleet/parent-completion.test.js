...
const { runParentCompletion, STATUS, countCompletionReadyParents } = await import('./parent-completion.mjs');
        ...
        return {
          select: () => ({
            or: () => ({
              ...
              // getLatestPlanToLead chains .eq('handoff_type', ...)[.eq('status', 'accepted')]
              // .order().limit(1) (QF-20260911-593: the accepted-only lookup adds a SECOND .eq()
              // before .order() -- makeChain below supports it as an extra optional step so both
              // the accepted-only query and the plain fallback query work off the same rows).
              // Real supabase-js's query builder is a thenable that also exposes further chain
              // methods -- .order()'s return value here must be BOTH awaitable AND carry a
              // .limit() method, not a bare Promise (a bare Promise has no .limit and throws when
              // chained).
              eq: (_col, val) => {
                const rows = val === 'LEAD-FINAL-APPROVAL' ? lfaHandoffRows : p2lHandoffRows;
                const makeChain = (filteredRows) => ({
                  eq: (col2, val2) => makeChain(filteredRows.filter((r) => r[col2] === val2)),
                  order: () => {
                    const p = Promise.resolve({ data: filteredRows, error: null });
                    p.limit = () => Promise.resolve({ data: filteredRows.slice(0, 1), error: null });
                    return p;
                  },
                });
...
describe('runParentCompletion', () => {
  ...
  it('SD-LEO-INFRA-AUTOMATED-VENTURE-TROUBLESHOOTING-001: a completed parent whose metadata carries a failed journey_walk_result with captured_cause appends a Cause line to the message', async () => {
    claimOrchestratorForRollup.mockResolvedValueOnce({ ok: true, action: 'claimed', sdKey: PARENT_KEY });
    claimOrchestratorForRollup.mockResolvedValueOnce({ ok: true, action: 'released', sdKey: PARENT_KEY });
    const CAUSE_LEG = { producer: 'record_venture_error_feedback', run_id: 'row-id-123456', content_hash: 'h', item_count: 1, items: ['x'], items_truncated: false, absent: false, absent_reason: null };
    const supabase = fullStub({
      parentRow: {
        id: PARENT_ID, sd_key: PARENT_KEY, status: 'pending_approval', created_at: '2026-01-01T00:00:00Z',
        metadata: { journey_walk_result: { status: 'fail', captured_cause: { window: { from: 'a', to: 'b' }, worker_logs: { ...CAUSE_LEG, item_count: 0, items: [], absent: false }, venture_errors: CAUSE_LEG, d1_failures: { ...CAUSE_LEG, item_count: 0, items: [], absent: false }, summary: 'captured cause: venture_errors=1' } } },
      },
      childrenRows: [{ id: 'c1', status: 'completed' }],
      lfaHandoffRows: [],
      p2lHandoffRows: [{ id: 'h1', status: 'accepted', metadata: {}, created_at: '2026-01-01T01:00:00Z' }],
    ...
    const result = await runParentCompletion({ ...baseDeps(), supabase });
    expect(result.status).toBe(STATUS.COMPLETED);
    expect(result.message).toContain('Cause: captured cause: venture_errors=1');
    expect(result.message).toContain('record_venture_error_feedback');
  });

  it('a completed parent with a clean (non-failed) journey_walk_result appends no Cause line -- message byte-identical to the no-walk case', async () => {
    claimOrchestratorForRollup.mockResolvedValueOnce({ ok: true, action: 'claimed', sdKey: PARENT_KEY });
    claimOrchestratorForRollup.mockResolvedValueOnce({ ok: true, action: 'released', sdKey: PARENT_KEY });
    const supabase = fullStub({
      parentRow: { id: PARENT_ID, sd_key: PARENT_KEY, status: 'pending_approval', created_at: '2026-01-01T00:00:00Z', metadata: { journey_walk_result: { status: 'pass' } } },
      childrenRows: [{ id: 'c1', status: 'completed' }],
      lfaHandoffRows: [],
      p2lHandoffRows: [{ id: 'h1', status: 'accepted', metadata: {}, created_at: '2026-01-01T01:00:00Z' }],
    });
...
// QF-20260830-933: completion-ready orchestrator parents.
function stubForCandidates({ candidateRows, childrenRows = [], lfaHandoffRows = [], blockerRows = {} }) {
  return {
    from(table) {
      if (table === 'strategic_directives_v2') {
        return {
          select: () => ({
            eq: (col1, col2) => {
              if (col1 === 'sd_type') {
                // QF-20260830-304: candidate query widened from .eq('status', 'pending_approval')
                // to .not('status', 'in', '(completed,cancelled)') so a parent stuck at any
                // non-terminal status (e.g. 'active') is a candidate, not just pending_approval.
                return { not: () => ({ is: () => ({ limit: () => Promise.resolve({ data: candidateRows, error: null }) }) }) };
              }
              if (col1 === 'parent_sd_id') return Promise.resolve({ data: childrenRows, error: null });
              // QF-20260901-357: holdReasonFor's blocked_by_sd_key live-status re-check.
              if (col1 === 'sd_key') return { maybeSingle: () => Promise.resolve({ data: blockerRows[col2] || null, error: null }) };
              throw new Error(`unexpected eq col: ${col1}`);
            },
          }),
        };
      }
      if (table === 'sd_phase_handoffs') {
        return {
          select: () => ({ or: () => ({ eq: () => ({ order: () => Promise.resolve({ data: lfaHandoffRows, error: null }) }) }) }),
        };
      }
      throw new Error(`unexpected table: ${table}`);
    },
...
describe('countCompletionReadyParents', () => {
  const NOW = Date.parse('2026-08-30T20:00:00Z');
  const candidate = { id: 'p1', sd_key: 'SD-ORCH-P1', status: 'pending_approval', updated_at: '2026-08-30T17:00:00Z' };

  it('completed-children parent, no accepted LEAD-FINAL, unclaimed -> count 1 with age', async () => {
    const supabase = stubForCandidates({
      candidateRows: [candidate],
      childrenRows: [{ id: 'c1', status: 'completed' }],
      lfaHandoffRows: [],
    });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
    expect(result.count).toBe(1);
    expect(result.parents[0].sd_key).toBe('SD-ORCH-P1');
    expect(result.oldestAgeMs).toBe(3 * 60 * 60 * 1000);
  });

  it('same parent with an accepted LEAD-FINAL -> count 0', async () => {
    const supabase = stubForCandidates({
      candidateRows: [candidate],
      childrenRows: [{ id: 'c1', status: 'completed' }],
      lfaHandoffRows: [{ status: 'accepted' }],
    });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
  ...
  it('a parent with one child in_progress -> count 0', async () => {
    const supabase = stubForCandidates({
      candidateRows: [candidate],
      childrenRows: [{ id: 'c1', status: 'in_progress' }],
      lfaHandoffRows: [],
  ...
  // QF-20260830-304: the discriminating fixture the defect class demands -- a completable
  // parent stuck at status='active' (not 'pending_approval') must now count. This is exactly
  // the SD-ALTIFYAI-LEO-ORCH-SPRINT-2026-002 shape: 24h completable, invisible to the old
  // pending_approval-only candidate filter.
  it('a completable parent at status=active counts (the previously-invisible shape)', async () => {
    const activeCandidate = { ...candidate, status: 'active' };
    const supabase = stubForCandidates({
      candidateRows: [activeCandidate],
      childrenRows: [{ id: 'c1', status: 'completed' }],
      lfaHandoffRows: [],
    });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
  ...
  it('a non-completable pending_approval parent (unmet children) does not count', async () => {
    const supabase = stubForCandidates({
      candidateRows: [candidate],
      childrenRows: [{ id: 'c1', status: 'in_progress' }],
      lfaHandoffRows: [],
  ...
  // QF-20260901-357: a documented hold reports as `held`, not `count`/`parents` -- never a
  // dispatch-now interrupt for a parent nobody can legitimately dispatch.
  it('a completable parent with needs_coordinator_review=true is held, not counted', async () => {
    const heldCandidate = { ...candidate, metadata: { needs_coordinator_review: true } };
    const supabase = stubForCandidates({ candidateRows: [heldCandidate], childrenRows: [{ id: 'c1', status: 'completed' }] });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
    expect(result.count).toBe(0);
    expect(result.held).toEqual([{ sd_key: 'SD-ORCH-P1', reason: 'needs_coordinator_review' }]);
  });

  it('a completable parent blocked by a still-non-terminal blocked_by_sd_key is held, not counted', async () => {
    const heldCandidate = { ...candidate, metadata: { blocked_by_sd_key: 'SD-BLOCKER' } };
    const supabase = stubForCandidates({
      candidateRows: [heldCandidate],
      childrenRows: [{ id: 'c1', status: 'completed' }],
      blockerRows: { 'SD-BLOCKER': { status: 'in_progress' } },
    });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
    expect(result.count).toBe(0);
    expect(result.held).toEqual([{ sd_key: 'SD-ORCH-P1', reason: 'blocked_by_sd_key:SD-BLOCKER' }]);
  });

  it('a completable parent whose blocked_by_sd_key blocker already completed is NOT held', async () => {
    const notHeldCandidate = { ...candidate, metadata: { blocked_by_sd_key: 'SD-BLOCKER' } };
    const supabase = stubForCandidates({
      candidateRows: [notHeldCandidate],
      childrenRows: [{ id: 'c1', status: 'completed' }],
      blockerRows: { 'SD-BLOCKER': { status: 'completed' } },
    });
    const result = await countCompletionReadyParents(supabase, { nowMs: NOW });
    expect(result.count).toBe(1);
