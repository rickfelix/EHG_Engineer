...
function appendFindingLine(finding) {
  try {
    ...
    // EXEC SECURITY (non-blocking OBS): ts spread LAST so a finding object carrying its own
    // "ts" field (never happens today, but nothing prevents it) can never override the sink's
    // own append-time timestamp.
    fs.appendFileSync(FINDINGS_LOG_PATH, JSON.stringify({ ...finding, ts: new Date().toISOString() }) + '\n');
  } catch (e) {
...
async function emitFindingAlert(supabase, finding) {
  try {
    ...
    // throw and get mis-caught as a failure. Check landed first.
