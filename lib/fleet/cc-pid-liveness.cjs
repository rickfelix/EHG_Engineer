 ...
 * take. readMarkerDir used to collapse "directory absent" (nothing to read from -- e.g.
 * off-checkout), "directory exists but genuinely holds zero markers" (a real, measured
 * zero), and "directory unreadable" (a read failure) into the same bare `{}`, so a caller
 * could not tell a trustworthy zero from a failure to measure at all. Now discriminated.
 * @param {string} dir
 * @returns {{state: 'absent'|'unreadable'|'empty'|'rows', markers: Object<string, {pid: number, alive: boolean}>, error?: string}}
 * @private
 */
function readMarkerDir(dir) {
  if (!fs.existsSync(dir)) return { state: 'absent', markers: {} };
  let files;
  try {
    files = fs.readdirSync(dir).filter(f => /^pid-\d+\.json$/.test(f));
  } catch (e) {
    return { state: 'unreadable', markers: {}, error: e.message };
  }
  if (files.length === 0) return { state: 'empty', markers: {} };
  const map = {};
  for (const f of files) {
    ...
    } catch { /* skip an individual unreadable marker -- does not make the whole dir unreadable */ }
  }
  return { state: 'rows', markers: map };
}

/**
 * QF-20260911-907: aggregate read-state across every relevant marker directory, for callers
 * that must distinguish "could not determine" from a genuine zero. 'unreadable' if ANY
 * directory failed to scan; 'absent' only if EVERY directory is absent (nothing reachable
 * to measure from at all); otherwise 'ok' (at least one directory was actually scanned,
 * whether or not it held any live rows -- empty is a trustworthy zero, not a failure).
 * @param {string} [markerDir]
 * @param {() => string[]} [markerDirsFn]
 * @returns {'unreadable'|'absent'|'ok'}
 */
function getMarkerReadState(markerDir, markerDirsFn = markerDirs) {
  const dirs = markerDir ? [markerDir] : markerDirsFn();
  const states = dirs.map((d) => readMarkerDir(d).state);
  if (states.some((s) => s === 'unreadable')) return 'unreadable';
...
function getMarkerSessionIds(markerDir, markerDirsFn = markerDirs) {
  if (markerDir) return readMarkerDir(markerDir).markers;
  return mergeAliveBiased(markerDirsFn().map((d) => readMarkerDir(d).markers));
}
...
module.exports = { isProcessRunning, getMarkerSessionIds, getAliveCcPids, getMarkerReadState, MARKER_DIR, markerDirs, mainWorktreeMarkerDir };
