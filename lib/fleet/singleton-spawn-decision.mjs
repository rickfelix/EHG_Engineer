 ...
 *
 * ONE decision function. CORRECTED (SD-LEO-INFRA-FLEET-SESSION-LIFECYCLE-001 / FR-1, AC-4): the
 * ONLY caller is server/routes/fleet-actions.js's addSession route (via resolveSingletonSpawnVerdict)
 * — server/public/fleet-ui/fleet-panel.js is vanilla JS and never imports or re-derives this
 * decision; it renders the uiLabel/uiEnabled fields the route already computed and forwarded in its
 * JSON response. "Consumed by both" was stale narration describing an intended future wiring that
 * had not shipped. The invariant this function exists to guarantee — "every disabled button state
 * corresponds to a route that returns 400 for the same condition" — is enforced by there being
 * exactly ONE decision-maker, not by two implementations agreeing: a UI-only gate is exactly the
 * failure this SD documents as having ALREADY HAPPENED on this page: `role` was once written as
...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A: michael is a refusable singleton like adam/solomon
// (SECURITY evidence 2ca8b0ee: omitting it from the REFUSABLE list would let the spawn route allow
// a duplicate that michael-register.cjs refuses — writer/consumer asymmetry).
export const SINGLETON_ROLES = Object.freeze(['adam', 'solomon', 'coordinator', 'michael']);
/** Coordinator is a singleton but is REPLACEABLE — takeover is how succession works. */
export const REFUSABLE_SINGLETON_ROLES = Object.freeze(['adam', 'solomon', 'michael']);
