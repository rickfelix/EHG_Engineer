...
// TWO write shapes are excluded, not one: `resume_uuid:` (an object key being SET, requiring no
// colon immediately after) and `resume_uuid =` (a direct property assignment, requiring no bare
// `=` immediately after -- QF-20260821-654: the original lookahead excluded only the first shape,
// so `subject.resume_uuid = value` matched as a "read" despite the header comment's own claim).
// The trailing `(?!=)` on the second lookahead is load-bearing: it distinguishes a bare `=`
// (assignment, excluded) from `==`/`===` (comparison, a genuine READ that must still match).
function readPattern(subject) {
  return new RegExp(`\\b${subject}\\s*[.[]\\s*['"]?resume_uuid['"]?\\]?\\s*(?!\\s*:)(?!\\s*=(?!=))`);
}
...
describe('FR3-TOKEN: metadata.resume_uuid is never consulted for RESTART/RESUME decisions', () => {
  ...
  describe('FR-4: the aliased-read detector is neither blind nor uselessly permissive', () => {
    ...
    // QF-20260821-654: the object-literal-key control above is NOT the only write shape. A direct
    // property ASSIGNMENT (`subject.resume_uuid = value`) is a distinct syntax the original
    // exclusion never covered -- this control exercises that shape specifically. Without the
    // `(?!\s*=(?!=))` lookahead this test fails (the assignment matched as a "read").
    it('NEGATIVE CONTROL: does not flag a direct property-assignment WRITE, aliased or not', () => {
      const code = 'const meta = x.metadata; meta.resume_uuid = sessionId;';
      expect(detectsResumeUuidRead(code)).toBe(false);
      const nested = 'session.metadata.resume_uuid = sessionId;';
      expect(detectsResumeUuidRead(nested)).toBe(false);
    });

    // The assignment exclusion must be precise, not blanket: `==`/`===` are comparisons -- genuine
    // READS -- and must still be caught despite starting with the same `=` character.
    it('a resume_uuid COMPARISON (===) is still caught as a read, not swallowed by the write exclusion', () => {
      const code = 'const meta = x.metadata; if (meta.resume_uuid === expected) { /* ... */ }';
      expect(detectsResumeUuidRead(code)).toBe(true);
    });
