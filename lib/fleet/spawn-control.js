...
export function assertSpawnSourceNotExempt(cwd) {
  if (isWorktreeExemptPath(cwd)) {
    ...
    // Tagged so the spawn() call-site can tell a CORRECTNESS violation from a git hiccup. Creation
    // failures there fail SOFT to the prior behaviour; this one must NOT, because a mis-sited tree
    // is silently unguarded and failing soft would hand back exactly the false assurance the guard
    // exists to prevent.
    err.code = SPAWN_SOURCE_SITING_ERROR;
    throw err;
  }
  return cwd;
}
 ...
 */
export const SPAWN_SOURCE_BRANCH = 'spawn-source';
...
export function ensureSpawnSourceWorktree({ repoRoot, exists, runner, baseRef = 'origin/main', env = process.env }) {
  ...
  if (exists(dir)) {
    let refreshed = true;
    try {
      for (const args of buildSpawnSourceUpdateArgs(dir, baseRef)) runner(args);
    } catch {
      refreshed = false;
    }
  ...
  // Freshly created AT the base ref — refreshing it would be a no-op round trip on the spawn path.
