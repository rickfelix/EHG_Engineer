  ...
  if (isSpawnSourceTreeEnabled(opts.currencyEnv || process.env)) {
    ...
    if (repoRoot) {
      ...
      try {
        ...
        // FS-R1 (EXEC SECURITY, correcting MY claim): I said this path was "degraded but still
        // guarded". Measured, that was true only for shared-root spawns. currencyApplies is seeded
        // at :586 as !isWorktreeExemptPath(cwd) and only raised at :616 — so for a spawn issued
        // from INSIDE a worktree it stays false here and the `if (currencyApplies)` block is
        // skipped entirely: unchecked, not degraded. Pin it to the repo root so the fallback is
        // genuinely guarded rather than silently exempt.
        currencyApplies = true;
        currencyDir = repoRoot;
        log(`[spawn-control] spawn-source tree unavailable (${err && err.message ? String(err.message).split('\n')[0] : 'unknown'}) — falling back to the spawning tree, currency enforced against the repo root`);
      }
    } else {
