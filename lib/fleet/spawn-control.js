...
// SD-LEO-INFRA-LEO-APP-LAUNCHER-001 (FR-2) unified resolveClaudeCmd/resolveRepoRoot into the canonical
// build-session-launch.cjs (the pilot fixes from SD-LEO-INFRA-LEO-LAUNCHER-LIVE-ACTIVATION-CHECKPOINT-3-001
// FR-1/FR-2 -- full claude.cmd path resolution + repo-root cwd -- now live there). Re-exported here so
// existing consumers importing these names from spawn-control.js keep working without duplicated logic.
export { resolveClaudeCmd, resolveRepoRoot };
 ...
 * lives here anymore. `resumeUuid` still reattaches a captured session (SD-LEO-INFRA-LEO-COMPLETION-001-D
 * FR-4); `profileDir` is pre-resolved by spawn() and passed through; the returned invocation additionally
 * carries `cwd`+`persistent`.
 */
