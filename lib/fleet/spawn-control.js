...
import { terminalSessionUpdate } from './terminal-session-update.cjs';
// SD-LEO-INFRA-SCHEDULED-WORKTREE-REAPER-001 FR-2/TR-3: the dedicated-source-tree mechanism moved
// to a .cjs home so the CommonJS worktree-reaper tick can share it (its tick() is called
// synchronously, so a dynamic import was not available). The functions below now DELEGATE there —
// spawn keeps its own names, defaults and error code, but there is exactly ONE implementation.
import {
  assertSourceTreeNotExempt as assertSourceTreeNotExempt_,
  isWorktreeExemptPath as isWorktreeExemptPath_,
  resolveSourceTreeDir as resolveSourceTreeDir_,
  buildSourceTreeWorktreeArgs as buildSourceTreeWorktreeArgs_,
  buildSourceTreeUpdateArgs as buildSourceTreeUpdateArgs_,
  ensureSourceTreeWorktree as ensureSourceTreeWorktree_,
  SPAWN_SOURCE_DIRNAME as SPAWN_SOURCE_DIRNAME_,
  SPAWN_SOURCE_BRANCH as SPAWN_SOURCE_BRANCH_,
  scrubGitEnv as scrubGitEnv_,
} from './source-tree-refresh.cjs';
import {
  resolveLiveSession,
...
const SINGLETON_ROLES = new Set(['coordinator', 'adam', 'solomon', 'michael']); // michael: SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A
...
// SD-LEO-INFRA-FLEET-CANNOT-SELF-001 FR-1 — mirrors build-session-launch.cjs's HOST_DEFAULT_PROFILE.
// Kept as an independent literal (not imported) because this file is ESM and that one is CJS with
// no existing cross-import between them; duplicating a one-word constant is cheaper than adding an
// interop seam for it.
export const HOST_DEFAULT_PROFILE = 'host-default';
 ...
 *
 * FAIL-LOUD (SD-LEO-INFRA-FLEET-CANNOT-SELF-001 FR-1): throws on a falsy profileName -- no account
 * intent recorded at all is a caller bug, not a request for no isolation. A caller that deliberately
 * wants no isolation must pass HOST_DEFAULT_PROFILE, which resolves to null (not a path) -- checked
 * BEFORE the regex/baseDir resolution below so it can never silently resolve to a bogus, non-existent
 * "<baseDir>/host-default" directory (it happens to match PROFILE_NAME_RE).
 */
export function resolveProfileDir(profileName, opts = {}) {
  if (!profileName) {
    throw new Error('resolveProfileDir: no account/profile intent provided (pass a profile name or the "host-default" sentinel)');
  }
...
export function isWorktreeExemptPath(cwd) {
  return isWorktreeExemptPath_(cwd); // FR-2/TR-3: one implementation, in source-tree-refresh.cjs
}
 ...
 * to check for.
 */
export const SPAWN_SOURCE_SITING_ERROR = 'SPAWN_SOURCE_SITED_IN_EXEMPT_PATH';

export function assertSpawnSourceNotExempt(cwd) {
  // Delegates (FR-2/TR-3) but KEEPS the spawn-specific error code. That tag is load-bearing:
  // spawn() branches on it to tell a CORRECTNESS violation from a git hiccup. Creation failures
  // there fail SOFT to the prior behaviour; this one must NOT, because a mis-sited tree is
  // silently unguarded and failing soft would hand back exactly the false assurance the guard
  // exists to prevent. (SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-1)
  return assertSourceTreeNotExempt_(cwd, { code: SPAWN_SOURCE_SITING_ERROR, label: 'spawn-source', ref: 'SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-1' });
}
 ...
 */
export const SPAWN_SOURCE_DIRNAME = SPAWN_SOURCE_DIRNAME_; // one literal, in source-tree-refresh.cjs
...
export function resolveSpawnSourceDir(repoRoot, env = process.env) {
  return resolveSourceTreeDir_(repoRoot, {
    dirname: SPAWN_SOURCE_DIRNAME,
    envOverride: 'FLEET_SPAWN_SOURCE_DIR',
    code: SPAWN_SOURCE_SITING_ERROR,
    label: 'spawn-source', ref: 'SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-1',
  }, env);
}
 ...
 *
 * WHY NOT `--detach`, which this shipped with and which was WRONG. The original reasoning was that
 * the spawn source is a read-only snapshot, so a branch checkout would collide with another
 * worktree holding it and would invite commits into a pristine tree. The collision half is true
 * only for a SHARED branch name like `main`; a dedicated name collides with nobody. And the
 * conclusion was fatal: assessTreeCurrency resolves the branch with `rev-parse --abbrev-ref HEAD`,
 * which answers the literal string 'HEAD' for a detached worktree and rejects it as
 * `detached_head` — so the tree would be pristine, exactly on the base ref, every condition the
 * guard demands, and refused anyway. Under FLEET_SPAWN_SOURCE_TREE that refused EVERY spawn.
 * Pinned while it was broken by tests/unit/fleet/spawn-source-detached-incompatibility.test.js.
 *
 * `-B` (create-or-reset) rather than `-b`: the tree is machine-managed and idempotency matters more
 * than protecting a branch nobody is supposed to commit to.
 */
export const SPAWN_SOURCE_BRANCH = SPAWN_SOURCE_BRANCH_; // one literal, in source-tree-refresh.cjs

export function buildSpawnSourceWorktreeArgs(dir, baseRef, branch = SPAWN_SOURCE_BRANCH) {
  return buildSourceTreeWorktreeArgs_(dir, baseRef, branch);
}

/**
 * Build the argv pair that brings an EXISTING spawn-source tree up to the base ref. FR-2's actual
 * mandate — "keep the spawn-source tree current without depending on anyone remembering".
 *
 * This is REQUIRED, not an optimisation. tree-currency self-heals only when the tree is clean AND
 * on SELF_HEALABLE_BRANCH ('main'), and the spawn source is deliberately on neither — so nothing
 * else will ever advance it. Without this the tree is current exactly once, at creation, and
 * refuses from the first merge onward.
 *
 * `-C <dir>` keeps the single-runner contract: the caller's runner is bound to the repo root, and
 * threading a second cwd through would give two ways to say where a command runs.
 *
 * ff-only, never reset/clean/stash — FR-2 forbids those, and nothing should ever be dirty here
 * anyway. If a fast-forward is not possible the tree stays behind and enforceTreeCurrency refuses
 * it with the real reason, which is the correct outcome: one authority decides currency.
 */
export function buildSpawnSourceUpdateArgs(dir, baseRef = 'origin/main') {
  return buildSourceTreeUpdateArgs_(dir, baseRef);
}
 ...
 * REUSE IS NOT A NO-OP. It used to be, and that was half the bug: a tree created once and never
 * advanced is current only until the next merge. On reuse the tree is fast-forwarded to the base
 * ref — best effort, because enforceTreeCurrency is the authority on whether the result is current
 * enough to spawn from. A failed refresh must NOT throw here: it would turn a transient network
 * blip into a spawn outage, when the currency check immediately downstream will refuse with the
 * real reason (and FR-4 renders it).
 *
 * @param {string} [deps.baseRef]
 * @param {object} [deps.env]
 * @returns {{dir:string, created:boolean, refreshed:boolean|null}}
 */
export function ensureSpawnSourceWorktree({ repoRoot, exists, runner, baseRef = 'origin/main', env = process.env }) {
  return ensureSourceTreeWorktree_({
    repoRoot,
    dirname: SPAWN_SOURCE_DIRNAME,
    branch: SPAWN_SOURCE_BRANCH,
    envOverride: 'FLEET_SPAWN_SOURCE_DIR',
    code: SPAWN_SOURCE_SITING_ERROR,
    label: 'spawn-source', ref: 'SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-1',
    exists, runner, baseRef, env,
  });
}

/**
 * Resolve the MAIN repo root — the tree the spawn source belongs beside — from any worktree.
 * SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-2 prerequisite.
 *
 * ASKS GIT RATHER THAN PARSING PATHS, and that is the whole point. `git rev-parse
 * --git-common-dir` returns `<main>/.git` from ANY worktree, so its parent is the main root —
 * the same technique this module already uses to resolve a transcript slug (see the
 * mainRepoRoot block further down, which documents why a `.worktrees/<id>` path silently
 * cold-starts every worktree seat).
 *
 * TWO WRONG ANSWERS I TRIED FIRST, recorded because both look reasonable:
 *   - worktree-manager getRepoRoot(): it execSyncs `git rev-parse --show-toplevel` with NO cwd
 *     option, so it resolves against process.cwd(). For a server-hosted spawn that is wherever
 *     the server was started, not the tree being spawned from.
 *   - unwinding the literal '/.worktrees/' out of the path myself: works only for the one layout
 *     it is written against. Three independent guards in this repo are already keyed to that
 *     literal and each is silently wrong for any other layout — asking git is the fix for that
 *     whole class, not another instance of it.
 *
 * Fails SOFT to null: callers must decide whether an unresolvable root is fatal. Throwing here
 * would turn a git hiccup into a fleet-wide spawn outage.
 *
 * The runner is REQUIRED, not defaulted. This module is ESM, so a default would need either a
 * top-level import of node:child_process (a module-level change to a hot file) or an await
 * (making this async for every caller). Requiring it keeps the function pure and synchronous and
 * matches the injectable-dependency pattern already used throughout this module
 * (opts.currencyRunner, opts.currencyEnv, the exists/runner pair on ensureSpawnSourceWorktree).
 * Callers pass `(args, o) => execFileSync('git', args, { encoding: 'utf8', ...o })`.
 *
 * @param {string} fromDir - a directory inside the repo (any worktree)
 * @param {(args:string[], opts:object)=>string} runner - git runner (required)
 * @returns {string|null}
 */
export function resolveMainRepoRoot(fromDir, runner) {
  try {
    if (typeof runner !== 'function') return null;
    const common = String(runner(['rev-parse', '--path-format=absolute', '--git-common-dir'], { cwd: fromDir })).trim();
...
/**
 * True when spawn should assert currency against the DEDICATED spawn-source tree instead of the
 * tree it happens to be spawning from. SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-2.
 *
 * DEFAULT OFF, deliberately, matching this repo's rollout convention (FLEET_SPAWN_CONTROL_LIVE
 * here, LEO_FR_TRACEABILITY_ENFORCE on the FR gates, INVOCATION_PATH_PROOF_MODE). With the flag
 * unset, spawn behaviour is byte-identical to before this SD: currency is asserted on
 * invocation.cwd under the existing `.worktrees/` exemption. Flipping it on requires a seat that
 * can run a live spawn, because the first enabled spawn performs a real `git worktree add` —
 * which is exactly what FR-3's live-root demonstration exists to validate.
 *
 * Absent-reads-as-OFF is the CONSERVATIVE direction here (unlike the bypass-reason channel
 * documented at the top of this file, where an absent read wrongly meant "inert"): a missing flag
 * leaves the pre-existing guard fully in force rather than silently disabling it.
 */
export function isSpawnSourceTreeEnabled(env = process.env) {
  const v = env.FLEET_SPAWN_SOURCE_TREE;
...
export async function spawn({ role, callsign, accountProfile } = {}, opts = {}) {
  ...
  // SD-LEO-INFRA-FLEET-CANNOT-SELF-001 — NOT widened to unconditional here, deliberately. This
  // guard stays: spawn() is the GENERIC verb used by ordinary, non-singleton worker sessions (UI
  // "Add session", restart()'s fallback for a session with no account_profile stamped), and an
  // absent accountProfile there is long-standing, DOCUMENTED, correct behaviour (see the
  // assertLaunchContract comment below: "spawn-control legitimately builds without a profile when
  // accountProfile is absent... demanding a profile here would silently convert that documented
  // policy into fail-closed — a behaviour change nobody asked for"). The chairman's own decision
  // for this SD scopes the fix to SINGLETONS ONLY ("workers remain operator-started") — the
  // fail-loud requirement lives in reboot-respawn-runner.js's per-slot loop instead, which is the
  // one path handling curated, governance-critical desired_slots rows rather than arbitrary worker
  // spawns. See resolveProfileDir's own doc comment for the 'host-default' sentinel it now accepts.
  let profileDir = null;
  ...
  // SD-FDBK-INFRA-SPAWN-SOURCE-CURRENCY-001 FR-2 (flag-gated, DEFAULT OFF).
  //
  // With FLEET_SPAWN_SOURCE_TREE unset, everything below is byte-identical to before this SD.
  // With it ON, currency is asserted against the DEDICATED always-clean spawn-source tree rather
  // than whatever tree is spawning — which is what makes the EXISTING self-heal reachable. The
  // shared root is chronically dirty (measured: 12 behind, 311 porcelain paths, 25 tracked), and
  // selfHealable (tree-currency.cjs:150) requires a fully clean tree, so the heal never fired
  // there and every fleet merge left operator spawns refused until a human pulled.
  //
  // The child still launches from invocation.cwd either way (coordinator ruling 923be099):
  // starting it from the spawn-source tree would defeat two independent nesting guards keyed to
  // the `.worktrees/` literal (getRepoRoot, sd-start.js:566), and its only benefit is already
  // owned by runStaleBaseGuard.
  let currencyDir = invocation.cwd;
  // The exemption exists because a per-SD worktree is legitimately off-main and behind by
  // construction. It does not apply to the dedicated tree, which is never a per-SD worktree.
  let currencyApplies = !isWorktreeExemptPath(invocation.cwd);

  if (isSpawnSourceTreeEnabled(opts.currencyEnv || process.env)) {
    const { execFileSync } = await import('node:child_process');
    const nodeFs = await import('node:fs');
    // EXEC SECURITY: env scrubbed for the same reason as the reaper tick's runner — GIT_DIR /
    // GIT_WORK_TREE let a bare mkdir pass identity AND redirect the fetch/merge at the shared root.
    const gitRunner = opts.spawnSourceRunner
      // R5-5: the caller object is spread FIRST and the canonical fields set AFTER, so a future
      // caller passing `env` cannot silently un-scrub this runner. Same shape as
      // PAT-PROVENANCE-SPOOF-VIA-SPREAD-ORDER-001: whichever side is spread last wins, and the
      // security-relevant field must be the one that wins.
      || ((args, o) => execFileSync('git', args, {
        ...o,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
        env: scrubGitEnv_({ ...process.env, ...((o && o.env) || {}) }),
      }));
    const repoRoot = opts.repoRoot || resolveMainRepoRoot(invocation.cwd, gitRunner);
    if (repoRoot) {
      // SYMMETRY (independent review, evidence row 6ecbbd8c / C3). ensureSpawnSourceWorktree
      // already fail-softs a failed REFRESH, but a throwing `git worktree add` propagated straight
      // out of spawn() — so under flag-ON one bad creation was a hard fleet-wide spawn outage while
      // the equivalent refresh failure was tolerated. That asymmetry was unintended and untested.
      // Creation failures now fall back to the prior behaviour for the SAME reason the
      // unresolvable-repo-root branch below does: a git hiccup must not be worse than the problem
      // this SD fixes.
      try {
        const ensured = ensureSpawnSourceWorktree({
          repoRoot,
          exists: opts.spawnSourceExists || nodeFs.existsSync,
          runner: (args) => gitRunner(args, { cwd: repoRoot }),
          baseRef: opts.spawnSourceBaseRef || 'origin/main',
          env: opts.currencyEnv || process.env,
        });
        currencyDir = ensured.dir;
        currencyApplies = true;
        if (ensured.created) log(`[spawn-control] created dedicated spawn-source worktree at ${ensured.dir}`);
      } catch (err) {
        // ...with ONE exception that must stay fatal. A tree sited under .worktrees/ is EXEMPT from
        // the currency check, so failing soft there would leave the spawn silently unguarded — the
        // precise false assurance this SD exists to eliminate. Correctness violation, not a hiccup.
        if (err && err.code === SPAWN_SOURCE_SITING_ERROR) throw err;
        // FS-R1, AND ITS REVERSAL — both recorded, because I got this wrong in each direction.
        //
        // First I claimed this path was "degraded but still guarded". EXEC SECURITY measured that
        // as true only for shared-root spawns: currencyApplies is seeded at :586 as
        // !isWorktreeExemptPath(cwd), so for a spawn issued from INSIDE a worktree it stays false
        // and the currency block is skipped — unchecked, not degraded. Correct finding.
        //
        // My fix was to pin currencyDir to repoRoot. That is an AVAILABILITY LANDMINE and the
        // follow-up review measured it: the live shared root carries hundreds of uncommitted paths
        // and is behind, so selfHealable is false and enforceTreeCurrency THROWS. Every spawn
        // hitting this fallback would refuse the moment FLEET_SPAWN_SOURCE_TREE is enabled. Worse,
        // isLinkedWorktreeOf cannot distinguish "not ours" from a transient git failure, so a
        // single index.lock would become a fleet-wide spawn refusal.
        //
        // So it is reverted. The honest position: this fallback lands on EXACTLY the guarding the
        // spawn would have had if the source-tree feature did not exist — worktree paths are
        // deliberately exempt from the currency check, which is pre-existing policy, not something
        // this SD may quietly tighten from inside an error handler. It never executes from the
        // unverified directory either way; the assurance gap is real and belongs to the exemption.
        log(`[spawn-control] spawn-source tree unavailable (${err && err.message ? String(err.message).split('\n')[0] : 'unknown'}) — falling back to the spawning tree (currency policy unchanged: .worktrees/ paths remain exempt)`);
      }
    } else {
      // FAIL SOFT to the prior behaviour rather than refusing the spawn. An unresolvable repo
      // root is a git hiccup, not a currency violation, and turning it into a spawn outage would
      // be a worse failure than the one this SD fixes.
      log('[spawn-control] FLEET_SPAWN_SOURCE_TREE is on but the main repo root did not resolve — falling back to the spawning tree');
    }
  }
...
export async function stop(target, opts = {}) {
  ...
  // QF-20260912-961: default ON — unset LEO_RELEASE_WORKITEM_RESET now enables the hand-back;
  ...
  const { error } = await supabase.from('claude_sessions').update(terminalSessionUpdate('released', {
    released_at: new Date().toISOString(), released_reason: 'manual_stop',
...
export async function restart(target, opts = {}) {
  ...
  const { error } = await supabase.from('claude_sessions').update(terminalSessionUpdate('released', {
    released_at: new Date().toISOString(), released_reason: 'restart',
...
export async function relaunchUnderProfile(target, accountProfile, opts = {}) {
  ...
  const { error } = await supabase.from('claude_sessions').update(terminalSessionUpdate('released', {
    released_at: new Date().toISOString(), released_reason: 'relaunch_under_profile',
  })).eq('session_id', oldIdentity.session_id);
