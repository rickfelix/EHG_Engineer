...
async function readSd(supabase, sdRef) {
  // count-truncation-diff-lint's chainWindow heuristic can't see past a bare closing-paren
  // line, so .maybeSingle() must share a line with the last argument to be recognized as the
  // bound on this select() (purely cosmetic — same query, same behavior).
...
/**
 * SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-E (Child E): resolve the priority-score function.
 * Test-injectable via opts.computePriorityScoreFn (mirrors resolveMergeFn's pattern) —
 * TESTING-AGENT FINDING (evidence e21a99e7): a bare require/import of a not-yet-existing
 * module path throws at RESOLUTION time in vitest, before any mock can intercept it, so an
 * injectable seam is the only way to unit-test the numeric-score branch before Child B's
 * lib/priority/comparator.cjs merges to main. Defaults to an optional dynamic import of that
 * module; absence (Child B not yet merged) degrades to null, never throws.
 */
async function resolveScoreFn(computePriorityScoreFn) {
  if (computePriorityScoreFn) return computePriorityScoreFn;
  try {
    const mod = await import('../priority/comparator.cjs');
    return (mod && mod.computePriorityScore) || null;
  } catch {
    return null;
  }
}

/** Sentinel-safe UNSCORED pick_reason — never a fabricated 0. */
const UNSCORED_PICK_REASON = Object.freeze({ score: 'UNSCORED', components: {}, comparatorVersion: null });

/**
 * Build the pick_reason object attached to a claim_history entry. Never throws; never lets a
 * NaN or non-numeric value reach JSON.stringify as null/NaN (Number.isFinite guards both, per
 * testing-agent finding TR-6 — typeof NaN === 'number' would otherwise pass a naive guard).
 */
function buildPickReason(scoreFn, item) {
  if (typeof scoreFn !== 'function') return UNSCORED_PICK_REASON;
  let result;
  try {
    result = scoreFn(item, {});
  } catch {
    return UNSCORED_PICK_REASON;
  }
  if (!result || typeof result !== 'object') return UNSCORED_PICK_REASON;
  const score = Number.isFinite(result.score) ? result.score : 'UNSCORED';
  const components = {};
  if (result.components && typeof result.components === 'object') {
    for (const [key, value] of Object.entries(result.components)) {
      components[key] = Number.isFinite(value) ? value : 'UNSCORED';
    }
  }
  const comparatorVersion = typeof result.comparatorVersion === 'string' ? result.comparatorVersion : null;
  return { score, components, comparatorVersion };
}

/** A QF-shaped ref (e.g. QF-20260906-123), matching the repo-wide idiom already used at
 * lib/checkin/steps/directed-assignment.cjs:212, lib/coordinator/dispatch.cjs:348/:468, and
 * scripts/release-work-item.mjs:133. */
const QF_ID_RE = /^QF-/;

/**
 * Resolve the QF-side CAS-guarded metadata merge (test-injectable via opts.mergeQfMetadataFn).
 * Defaults to lib/fleet/qf-metadata-merge.mjs's mergeQfMetadataKeys.
 */
async function resolveQfMergeFn(mergeQfMetadataFn) {
  if (mergeQfMetadataFn) return mergeQfMetadataFn;
  return (await import('./qf-metadata-merge.mjs')).mergeQfMetadataKeys;
}

/**
 * Append { session_id, claimed_at[, identity_source], pick_reason } to metadata.claim_history
 * (FIFO cap 20 for the SD path) after a successful claim_sd. Fail-soft: returns the appended
 ...
 *
 * SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-E (Child E): sdRef may be a QF id (e.g. 'QF-20260906-1'),
 * auto-detected via QF_ID_RE — no signature change to the 4 existing positional args, since
 * testing-agent evidence e21a99e7 confirmed position 5 (mergeMetadataKeysFn) is already the SD-
 * side merge-injection seam 8 existing test call sites depend on. `opts` (position 6, purely
 * additive) carries {computePriorityScoreFn, mergeQfMetadataFn} test-injection seams; no
 * production caller passes it today.
 */
async function stampClaim(supabase, sdRef, sessionId, identitySource, mergeMetadataKeysFn = null, opts = {}) {
  try {
    if (!supabase || !sdRef || !sessionId) return null;
    const scoreFn = await resolveScoreFn(opts && opts.computePriorityScoreFn);

    if (QF_ID_RE.test(String(sdRef))) {
      // quick_fixes has no metadata column in production today (chairman-gated migration
      // ships alongside this change) — mergeQfMetadataKeys degrades fail-soft on 42703.
      ...
      entry.pick_reason = buildPickReason(scoreFn, { id: sdRef });
      const mergeQfFn = await resolveQfMergeFn(opts && opts.mergeQfMetadataFn);
      const result = await mergeQfFn(sdRef, sessionId, entry);
      return result && result.merged ? entry : null;
    }
    ...
    entry.pick_reason = buildPickReason(scoreFn, row);
...
module.exports = {
  stampClaim, stampCompletion, stampExecutionContext, buildExecutionContext, CLAIM_HISTORY_CAP,
  buildPickReason, UNSCORED_PICK_REASON, QF_ID_RE,
};
