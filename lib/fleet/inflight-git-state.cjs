...
function collectInflightSnapshot(opts = {}) {
   ...
   * Either one means in-flight."
   *
   * MEASURED, AND THEN PARTITIONED — the partition is the part that matters, and my first pass
   * skipped it. Initial count: draft/in_progress SDs, 24 candidates -> 10 withheld (4 via open PR,
   * 6 branch-only), which I recorded as "branch-only starves ~25% of the belt". That conflated
   * would-withhold with would-FALSELY-withhold. TESTING (aacc101d) ran the partition I had not.
   *
   * POPULATION, stated explicitly because the headline number moves with it: 57 EHG_Engineer
   * items that are neither completed nor cancelled AND not `escalated` — 41 no ref, 9 open PR,
   * 7 branch-only. Counting escalated QFs as non-terminal widens it to ~111 items / 11
   * branch-only (~64% precision). The SPLIT below is the load-bearing part and holds either way;
   * the precision headline does not, so quote the population with it. The 7 branch-only split into
   *
   *   5 never-PR'd, GENUINE in-flight WIP  (including this SD's own branch)
   *   2 ever-merged, stale refs            (SD-EHG-IDEATION-PIPELINE-SEAMS-001, QF-20260726-956)
   *   0 closed-unmerged
   *
   * So branch-only is ~71% precise, not ~25% harmful — my stated harm was overstated by ~2.5x.
   * The stale-ref mechanism is real (this repo squash-merges without pruning, 3059 refs;
   * QF-20260727-703 was merged and its ref survived because --delete-branch failed), it is just
   * far less common than the raw count implied.
   *
   * WHY THE DEFAULT IS STILL PR-ONLY, stated honestly rather than as "branch-only is bad":
   * an open PR is unambiguous (9/9 genuine), and ALL FOUR acceptance criteria are satisfied by the
   * PR signal alone — AC-1 and AC-2 both stage a branch AND an open PR, AC-4 is neither — so this
   * is not a scope reduction against the ACs. A false withhold is also the worse error: a
   * duplicate build wastes one seat once, a false withhold blocks an item indefinitely and looks
   * like an empty queue.
   *
   * WHAT THIS DEFAULT DELIBERATELY DOES NOT COVER: the push-to-open-PR window — which is exactly
   * the correction-delivery window this SD is about. Branch-only is the only signal that sees it.
   * The right fix is branch FRESHNESS (exclude refs already merged into main by ancestry), not
   * discarding the disjunct; that is a follow-on, and because includeBranchOnly is implemented and
   * covered by tests it is a caller-side flag flip rather than a redesign.
   */
