...
class UIValidatorPlaywright {
  ...
  async validateRequirement(req) {
    ...
    try {
      ...
      const screenshotPath = await this.captureEvidence(req);
  ...
  async validateWizard(element) {
    try {
      ...
      const prevButton = await this.page.$('[data-testid="prev-button"], button:has-text("Previous")');
  ...
  async validateScreenshotUploader(element) {
    const fileInput = await this.page.$('input[type="file"]');
    return fileInput !== null;
  }

  async validateSubmissionList(element) {
    // Check if list has items or placeholder
    const items = await element.$$('li, .submission-item, [data-testid*="submission"]');
    return true; // List exists, might be empty
  }
