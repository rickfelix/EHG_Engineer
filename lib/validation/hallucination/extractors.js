...
export function extractFileReferences(output) {
  ...
  // Normalize literal 2-char JSON-escape sequences to a real space, unconditionally --
  // prepareOutputForAnalysis() always returns a plain string before this function ever sees it
  // in the real production chain (validateSubAgentOutput -> prepareOutputForAnalysis ->
  // extractFileReferences), so the JSON.stringify(output) branch above is dead code there; the
  // mangling this fixes is introduced by prepareOutputForAnalysis's own internal
  // JSON.stringify(output.findings) call. Must not be gated behind either ternary branch.
