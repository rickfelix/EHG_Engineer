...
function validateFileReferences(result, content, baseDir, branchContext) {
  ...
  // Built once per call (FR-2/TR-1), before the per-reference loop, and threaded through to
  // checkFileExists/findBasenameMatches -- a same-call shared handle, not a cross-call cache.
  const basenameIndex = buildBasenameIndex(baseDir);

  for (const filePath of fileRefs) {
    if (checkFileExists(filePath, baseDir, branchContext, basenameIndex)) {
