...
function validateFileReferences(result, content, baseDir, branchContext) {
  ...
  // Built at most once per call (FR-2/TR-1), and LAZILY -- only when a bare-basename reference
  // is actually encountered, not unconditionally. SD-FDBK-ENH-RETRO-SUB-AGENT-001 (SECURITY
  // EXEC-TO-PLAN review): an unconditional build paid the full walk cost even for zero or
  // all-full-path reference batches; from a large root that cost is real (see buildBasenameIndex's
  // own comment), not just a worktree-scale rounding error.
  let basenameIndex = null;
  let basenameIndexBuilt = false;

  for (const filePath of fileRefs) {
    const isBareBasename = filePath === path.basename(filePath);
    if (isBareBasename && !basenameIndexBuilt) {
      basenameIndex = buildBasenameIndex(baseDir);
      basenameIndexBuilt = true;
    }

    if (checkFileExists(filePath, baseDir, branchContext, isBareBasename ? basenameIndex : null)) {
      result.file_references.valid++;

      // FR-3: a bare-basename reference matching more than one real repo file is disclosed via
      // warnings, not turned into a new failure mode. checkFileExists stays boolean (TR-4);
      // ambiguity detail comes from calling findBasenameMatches directly, on the same index.
      if (isBareBasename) {
        const matches = findBasenameMatches(filePath, baseDir, basenameIndex);
        if (matches.length > 1) {
          result.warnings.push({
            level: 'L1',
            type: 'ambiguous_basename_match',
            reference: filePath,
            message: `Bare basename '${filePath}' matches ${matches.length} real files: ${matches.slice(0, 5).join(', ')}${matches.length > 5 ? ', ...' : ''}`
          });
        }
      }
    } else {
