...
// LEO v4.4.3: Branch-aware file checking for feature branch validation
 ...
 * @param {Object} branchContext - Optional branch context { branch, repoPath }
 * @returns {boolean|Object} True/false or object with details
 */
function checkFileExists(filePath, baseDir, branchContext = null) {
  ...
  if (branchContext && branchContext.branch && branchContext.repoPath) {
    ...
    // If not on branch, fall through to filesystem check
  }
 ...
 * Uses git cat-file to check without checkout
 */
function checkFileExistsOnBranch(filePath, branchContext) {
  const { branch, repoPath } = branchContext;

  // Normalize the file path relative to repo
  ...
  if (path.isAbsolute(filePath)) {
    ...
    if (filePath.startsWith(repoPath)) {
      ...
      // Path is outside repo, can't check on branch
 ...
 */
function readFileFromBranch(filePath, branchContext) {
  const { branch, repoPath } = branchContext;
 ...
 * LEO v4.4.3: Branch-aware - reads from branch if context provided
 */
function checkSymbolExistsInFile(filePath, symbolName, baseDir, branchContext = null) {
  let content = null;
 ...
 * @param {Object} options.branchContext - Branch context for feature branch validation { branch, repoPath }
...
export async function validateSubAgentOutput(output, options = {}) {
  ...
  // LEO v4.4.3: Branch-aware - check feature branch first if context provided
  if (levels.includes(HallucinationLevel.L1)) {
    ...
    // Add branch context to result for debugging
    if (branchContext) {
    ...
    for (const filePath of fileRefs) {
      if (checkFileExists(filePath, baseDir, branchContext)) {
        ...
        result.file_references.invalid.push({
          ...
          level: 'L1',
          issue: branchContext
            ? 'File does not exist (checked branch and filesystem)'
            : 'File does not exist'
        });
        result.issues.push({
          level: 'L1',
  ...
  // LEO v4.4.3: Branch-aware - read from feature branch if context provided
