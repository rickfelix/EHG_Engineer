...
function extractAllDependencyRefs(sd) {
  ...
  // blocked_by_sd_key mirrors its canonical consumer checkMetadataDependency
  // (scripts/modules/sd-next/dependency-resolver.js): only SD-key-shaped values count —
  // a stray 'N/A'/'TBD'/prose stamp must not become a permanently dangling hard blocker
  // (QF-20260703-999 parity). blocked_on_sd keeps its heritage any-string semantics.
  const bbk = meta.blocked_by_sd_key;
  if (typeof bbk === 'string' && /^SD-[A-Z0-9-]+/.test(bbk) && !NONE_SENTINELS.has(bbk)) refs.push(bbk);
  const bos = meta.blocked_on_sd;
  if (typeof bos === 'string' && bos && !NONE_SENTINELS.has(bos)) refs.push(bos);
  return [...new Set(refs)];
}
