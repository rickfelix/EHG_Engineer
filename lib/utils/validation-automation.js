...
// checkSemanticIndexStatus() below deliberately distinguishes PGRST205/42P01 ("table absent,
// schema problem") from every other error ("unreadable, could not determine") from success
// ("empty, needs indexing") -- three outcomes it inspects error.code itself to tell apart. The
// factory's default throw-on-PGRST205 would collapse that into one generic caught exception.
const supabase = createSupabaseServiceClient({ throwOnSchemaDrift: false });
