...
export function shouldSkipCodeValidation(sd) {
  ...
  // PAT-SD-API-CATEGORY-001: API category indicates backend endpoints that cannot be tested with E2E Playwright tests
  // API implementations produce code but require different validation (unit tests, integration tests, not Playwright E2E)
  if (category === 'api' || category === 'backend') {
    console.log(`   ℹ️  Category='${sd.category}' - API/backend SD detected`);
    console.log('   ℹ️  Skipping E2E-centric validation (TESTING/GITHUB sub-agents)');
    console.log('   💡 API endpoints should be validated via unit tests and integration tests instead');
    return true;
  }
