...
/**
 * Validate and sanitize quick-fix ID to prevent path traversal and command injection
 * SD-SEC-DATA-VALIDATION-001: Input validation for file paths
 * @param {string} qfId - Quick-fix ID to validate
...
function validateQfId(qfId) {
  ...
  // QF IDs follow pattern: QF-YYYYMMDD-NNN or similar alphanumeric with dashes
  const sanitized = qfId.trim();
  const validPattern = /^[a-zA-Z0-9_-]+$/;
  if (!validPattern.test(sanitized) || sanitized.length > 50) {
    throw new Error(`Invalid quick-fix ID format: ${qfId}`);
  }
  // Prevent path traversal
  if (sanitized.includes('..') || sanitized.includes('/') || sanitized.includes('\\')) {
    throw new Error('Quick-fix ID contains invalid characters');
  }
  return sanitized;
}
...
export function initEvidenceDir(qfId) {
  const validatedId = validateQfId(qfId);
  const qfEvidenceDir = path.join(EVIDENCE_DIR, validatedId);
...
export function cleanupEvidence(qfId, options = {}) {
  ...
  if (options.archive) {
    ...
    // SD-SEC-DATA-VALIDATION-001: Use Node.js fs instead of shell commands to prevent injection
    const validatedId = validateQfId(qfId);
    const archiveBaseDir = path.join(process.cwd(), '.quickfix-archive');
    const archivePath = path.join(archiveBaseDir, validatedId);
    try {
      if (!existsSync(archiveBaseDir)) {
        mkdirSync(archiveBaseDir, { recursive: true });
      }
      cpSync(evidenceDir, archivePath, { recursive: true });
  ...
  if (options.delete) {
    try {
      // SD-SEC-DATA-VALIDATION-001: Use Node.js rmSync instead of shell rm -rf
