...
// QF-20260409-720: Raised from 2 → 3 and added Jaccard floor because the
// previous bag-of-words count matched across unrelated QFs that only shared
// generic LEO infra vocabulary (claude, session, team, spawn, execute, …).
const SIMILARITY_KEYWORDS_MIN = 3; // Minimum keyword matches for similarity
const SIMILARITY_JACCARD_MIN = 0.25; // Require ≥25% overlap on stemmed tokens
...
export async function analyzePatterns(issue) {
  ...
  try {
    ...
    for (const qf of recentQuickFixes || []) {
      ...
      if (matchCount >= SIMILARITY_KEYWORDS_MIN && jaccard >= SIMILARITY_JACCARD_MIN) {
        similarIssues.push({
          id: qf.id,
          title: qf.title,
          matchCount,
...
function extractKeywords(text) {
  ...
  const stopWords = new Set([
    ...
    // QF-20260409-720: LEO/infrastructure stoplist — these tokens appear in
    // nearly every LEO platform QF and caused false-positive clustering of
    // unrelated root causes. Filter them from similarity comparisons.
    'claude', 'session', 'sessions', 'execute', 'executes', 'team', 'teams',
    'spawn', 'spawned', 'spawns', 'worker', 'workers', 'supervisor',
    'orchestrator', 'orch', 'leo', 'stage', 'stages', 'ship', 'claim',
    'claims', 'virtual', 'queue', 'protocol', 'coordination', 'handoff',
    'chairman', 'drain', 'agent', 'agents', 'subagent', 'pid', 'process',
    'command', 'script', 'scripts', 'sd', 'prd', 'node', 'windows'
  ]);
...
/**
 * QF-20260409-720: Jaccard similarity on keyword sets
 * Returns intersection/union ratio in [0, 1]. Used alongside the absolute
 * match count to reject loose bag-of-words overlap.
 */
 ...
 */
function generateRootCauseHypothesis(patterns, keywords, _issueType) {
  const hypotheses = [];
