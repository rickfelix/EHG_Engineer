...
import { isPossiblyClippedSmsBody } from './sms-clipped-detector.js';
...
// QF-20260913-173 (SECURITY EXEC-TO-PLAN finding S1): no_open_question is deliberately NOT
// included here. Unlike its no_match/multi-candidate-unclear siblings, it fires only when a
// real candidate decision was found and considered (it just went terminal since) -- a provably
// genuine reply, not evidence of abuse or noise, so it should not feed the auto-suspend counter
// this list gates.
...
// QF-20260913-173: no_open_question added -- the previously-missing THIRD label for "the most
// recent candidate exists but is TERMINAL (already answered/decided)", distinct from expired
// (still pending, token window passed) and no_match (no candidate at all, or a genuine mismatch).
// Parked like every other unresolved outcome so a genuine chairman reply to a closed question is
// never invisible to a human/Adam review.
const PARK_OUTCOMES = ['no_match', 'rate_limited', 'expired', 'ambiguous', 'no_open_question'];
// QF-20260901-633: the subset of PARK_OUTCOMES that ALSO mechanically route to the Adam lane
// (drainSmsRelayStaging below) — a single shared list so the routing gate and the drain script's
// escalation check (scripts/sms-relay-drain.cjs) can never drift apart into per-outcome special
// cases the way the routing gate itself once did (QF-20260831-346).
const ADAM_ROUTABLE_OUTCOMES = ['no_match', 'rate_limited', 'no_open_question'];
...
export async function stageDecisionSmsNotification(supabase, {
  ...
  try {
    ...
    // SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001: the notification row above already landed -- if the
    // token/expiry patch then fails, that row is an orphan (status='queued', no live token backing
    // it, unmatchable by any reply). invalidateDecisionSmsStaging's chairman_decisions clear is
    // scoped to .eq('sms_reply_token', token), so it is a harmless no-op here (the token was never
    // actually written); its chairman_notifications failure-mark is the load-bearing half. Applies
    // to every caller of this shared function, not just one -- the gap was latent here already.
    await invalidateDecisionSmsStaging(supabase, { decisionId, token, notificationId, reason: `staging_partial_failure: ${err.message}` }).catch(() => {});
    throw err;
  }
...
export async function handleInboundSmsReply(supabase, inbound) {
  ...
  if (!decision) {
    ...
    // security-relevant (all three branches equally refuse to resolve anything).
    // QF-20260913-173: a TERMINAL most-recent candidate (status !== 'pending', e.g. already
    // approved/rejected) is a DIFFERENT condition from either 'expired' (still pending, token
    // window passed) or genuine 'no_match' (no candidate, or a real mismatch) -- previously both
    // were reported as 'no_match', indistinguishable from "the chairman's reply matched nothing
    // real". A live chairman reply to an already-decided question must read as "no open question
    // to match", not silently fold into the same bucket as noise/spam.
    const mostRecent = byId.get(candidateIds[0]);
    let outcome;
    if (mostRecent && mostRecent.status === 'pending' && !mostRecent.sms_reply_used_at) {
      outcome = 'expired';
    } else if (mostRecent && mostRecent.status !== 'pending') {
      outcome = 'no_open_question';
    } else {
      outcome = 'no_match';
    }
 ...
 * @returns {Promise<{drained: number, results: Array<{id: string, outcome: string, routedToAdam?: boolean}>}>}
 */
export async function drainSmsRelayStaging(supabase, { limit = 50 } = {}) {
  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      // QF-20260901-633: resultEntry is mutated below once routing is attempted, so the drain
      // script can tell "not routable" (routedToAdam stays undefined) apart from "routable but
      // the route attempt failed" (routedToAdam === false) -- a live specimen (staging row
      // 0ba9b364, 2026-09-01) reached parked_at/resolved_at with routed_at NULL the whole time
      // and NOTHING surfaced the failed route attempt; only a direct staging read caught it.
      const resultEntry = { id: row.id, outcome: outcome.outcome };
      ...
      if (PARK_OUTCOMES.includes(outcome.outcome) && isVerifiedChairman) {
        const isAdamRoutable = ADAM_ROUTABLE_OUTCOMES.includes(outcome.outcome);
        // QF-20260905-781 (recurrence #2 of ratification f6315dbf: a verified chairman sender
        // is structurally UNPARKABLE): routing is a SUBSET of parking, so the old "park first,
        // then maybe route" order wrote parked_at on EVERY Adam-routable verified-chairman row
        // even when the route landed cleanly 40ms later (live specimen sms_relay_staging
        // 5b6c5141, 2026-09-05: parked_at 11:17:25.089, routed_at 11:17:25.132, both quiet-tick
        // alarms fired for the same already-routed row). Attempt routing FIRST; park is now
        // reserved for (a) expired/ambiguous, never Adam-routable, and (b) the fail-soft
        // fallback when a route attempt genuinely fails THIS tick -- never engineered away, so
        // a row can never go silently unmarked anywhere.
        let shouldPark = !isAdamRoutable;

        // QF-20260828-188 (leg 3): a free-form chairman message matching no known pattern must
        // not sit silently unrouted waiting for a human to notice. Route it mechanically to
        // Adam as adam_action_required — the same lane
        // scripts/adam-adherence-staleness-check.mjs already uses.
        //
        // SD-LEO-INFRA-CHAIRMAN-SMS-RELAY-001: stamp routed_at here, NOT resolved_at. Stamping
        // resolved_at at route time (the original QF-20260828-188 shape) made
        // surfaceParkedChairmanSms's QUIET_TICK_SMS_PARKED interrupt (keyed on resolved_at IS
        // NULL, and now ALSO routed_at IS NOT NULL — see that function) stop firing for this
        // row the instant it was mechanically routed -- even though the resulting coordination
        // message could itself go unread. resolved_at is reserved for genuine handling:
        // resolveAllParkedChairmanSmsRows (a confirmed reply actually sent) or explicit
        // disposition via resolve-parked-chairman-sms.cjs / the audit's EVIDENCE_HANDLED class.
        if (isAdamRoutable) {
          // QF-20260901-305: claim routed_at BEFORE delivery (same claim-first shape as the
          // drained_at claim above), not insert-then-stamp -- a landed row could previously
          // read unrouted forever if the stamp failed AFTER the coordination insert already
          // succeeded. A retry finding routed_at already set is now a no-op.
          const { data: claimedRoute } = await supabase
            .from('sms_relay_staging')
            .update({ routed_at: new Date().toISOString() })
            .eq('id', row.id) // unique-key filter -- at most 1 row; limit(1) makes that explicit
            .is('routed_at', null)
            .select('id')
            .limit(1);
          if (!claimedRoute || claimedRoute.length === 0) {
            resultEntry.routedToAdam = true; // already routed — idempotent no-op
            continue;
          }
          try {
            let target = null;
            ...
            await insertCoordinationRow(supabase, {
              ...
              message_type: 'INFO',
              subject: outcome.outcome === 'rate_limited'
                ? 'Chairman SMS rate-limited before reaching a lane'
                // SECURITY EXEC-TO-PLAN finding S8 (QF-20260913-173): no_open_question is a
                // genuine reply to a now-closed question, not a mismatch -- the generic
                // "matched no known pattern" subject was misleading for it.
                : outcome.outcome === 'no_open_question'
                  ? 'Chairman SMS replied to an already-decided question'
                  : 'Chairman free-form SMS matched no known pattern',
              payload: {
                kind: 'adam_action_required',
                // QF-20260830-874: 14 specimens over 8 days landed at exactly 1600 chars,
                // every one ending mid-word -- flag the tell explicitly so a truncated
                // chairman message never reads as complete to this consumer.
                body: `${isPossiblyClippedSmsBody(row.body_raw) ? '[POSSIBLY CLIPPED at 1600 chars] ' : ''}Chairman SMS (staging row ${row.id}, outcome=${outcome.outcome}) was mechanically routed here instead of sitting parked: "${String(row.body_raw || '').slice(0, 320)}"`,
              },
            }, { targetRoleHint: 'adam' });
            resultEntry.routedToAdam = true;
          } catch {
            // Delivery failed AFTER the claim -- roll back so the row stays retriable instead
            // of showing routed_at set with nothing ever delivered.
            try {
              await supabase.from('sms_relay_staging').update({ routed_at: null }).eq('id', row.id);
            } catch { /* best-effort rollback; park is still the durable fallback below */ }
            // fail-soft: park below is the durable fallback if Adam-routing fails.
            // QF-20260901-633: but the drain script must still be able to SEE this failure —
            // routedToAdam=false is the signal it escalates on, instead of the failure being
            // indistinguishable from "not applicable" (undefined) inside the normal tally.
            resultEntry.routedToAdam = false;
            shouldPark = true;
          }
        }
 ...
 * parked (or, since QF-20260905-781, routed-but-never-parked) row. Conditional UPDATE (WHERE
 * (parked_at IS NOT NULL OR routed_at IS NOT NULL) AND resolved_at IS NULL) so a second call on
 * an already-resolved (or never-touched) row is a no-op, never an error — the same
 * claim-by-conditional-UPDATE shape drainSmsRelayStaging uses for drained_at. Once resolved_at
 * is set, surfaceParkedChairmanSms (scripts/adam-quiet-tick.mjs) stops returning the row,
 * silencing the QUIET_TICK_SMS_PARKED interrupt for it exactly once.
 * QF-20260905-781: an Adam-routable verified-chairman row no longer ever gets parked_at (see
 * drainSmsRelayStaging) — it can still surface AND still be resolved, via routed_at.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
...
export async function resolveAllParkedChairmanSmsRows(supabase) {
  const { data } = await supabase
    .from('sms_relay_staging')
    .update({ resolved_at: new Date().toISOString() })
    // QF-20260905-781: OR routed_at — see resolveParkedChairmanSmsRow's docstring.
    .or('parked_at.not.is.null,routed_at.not.is.null')
    .is('resolved_at', null)
    .select('id');
