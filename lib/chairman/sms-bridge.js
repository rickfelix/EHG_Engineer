  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      if (PARK_OUTCOMES.includes(outcome.outcome) && isVerifiedChairman) {
        ...
        // not sit silently parked waiting for a human to notice. Route it mechanically to Adam
        // as adam_action_required — the same lane scripts/adam-adherence-staleness-check.mjs
        // already uses.
        //
        // QF-20260831-346: rate_limited joined this routing set too. The drain path (PARK_
        // OUTCOMES) learned to park rate_limited chairman rows, but this routing step never
        // did — 3 specimens 2026-08-31 showed rate_limited rows getting parked_at, then later
        // resolved_at, with routed_at NULL the whole time: a lane-divergence that let the
        // pipeline mark a row "done" that no consumer ever received. expired/ambiguous remain
        // park-only by design (they are not "unrecognized content" — see PARK_OUTCOMES).
        //
        ...
        // NULL) stop firing for this row the instant it was mechanically routed -- even though
        // the resulting coordination message could itself go unread. A 2026-08-29 21:52Z
        // chairman question hid in exactly this blind window until a manual hand-check found it.
        // resolved_at is now reserved for genuine handling: resolveAllParkedChairmanSmsRows
        // (a confirmed reply actually sent) or explicit disposition via
        // resolve-parked-chairman-sms.cjs / the audit's EVIDENCE_HANDLED classification.
        if (ADAM_ROUTABLE_OUTCOMES.includes(outcome.outcome)) {
          // QF-20260901-305: claim routed_at BEFORE attempting delivery (same claim-first
          // shape as the drained_at claim above), instead of the prior insert-then-stamp
          // order. A landed row could previously read unrouted forever if this stamp step
          // failed AFTER the coordination insert had already succeeded (chairman text
          // 4e0db146, witnessed 2026-09-01 21:18Z: routed only by a coordinator retry at
          // 21:21:49Z, producing a false ESCALATION and requiring a manual routed_at stamp,
          // with real double-delivery risk on that retry). Claiming first means: (a) the row
          // can never read unrouted once delivery lands, and (b) a retry that finds routed_at
          // already set treats this as an idempotent no-op instead of re-delivering.
          ...
          if (!claimedRoute || claimedRoute.length === 0) {
            // Already routed by this drain run or a concurrent one — idempotent no-op.
            ...
            try {
              ...
              // Delivery failed AFTER the claim — roll back so the row is retriable instead of
              // stuck showing routed_at set with nothing ever delivered. Best-effort: the park
              // above remains the durable fallback even if this rollback itself fails.
              try {
                await supabase
                  .from('sms_relay_staging')
                  .update({ routed_at: null })
                  .eq('id', row.id);
              } catch { /* best-effort rollback; park is the durable fallback */ }
              // fail-soft: the park above is the durable fallback if Adam-routing fails.
