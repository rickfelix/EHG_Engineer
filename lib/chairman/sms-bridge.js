...
async function logInbound(supabase, { from, to, body, messageSid, signatureValid, outcome, matchedDecisionId = null, consideredDecisionId = null }) {
  ...
  const { error } = await supabase.from('sms_inbound_log').insert({
    ...
    considered_decision_id: consideredDecisionId,
    outcome,
  });
  if (error) {
    console.warn(`[sms-bridge] logInbound insert failed (audit row lost) for outcome=${outcome}: ${error.message}`);
  }
}
...
export async function drainSmsRelayStaging(supabase, { limit = 50 } = {}) {
  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      // resolved no_match/rate_limited must not terminal-drain unnoticed — nine such messages
      // (a status question, a directive, a decision, a continuity complaint) went invisible to
      // every consumer on 2026-08-10. PARK it (additive parked_at) so the quiet-tick can keep
      // surfacing it (FR-2) until it is explicitly resolved (FR-4c). drained_at above remains
      // the exactly-once claim — this is a separate, orthogonal marker, never a substitute.
      if (
        PARK_OUTCOMES.includes(outcome.outcome)
        && phoneKey(row.from_phone)
