          ...
          try {
            ...
            await insertCoordinationRow(supabase, {
              ...
              payload: {
                ...
                body: `${isPossiblyClippedSmsBody(row.body_raw) ? '[POSSIBLY CLIPPED at 1600 chars] ' : ''}Chairman SMS (staging row ${row.id}) matched no known reply pattern and was mechanically routed here instead of sitting parked: "${String(row.body_raw || '').slice(0, 320)}"`,
              },
            }, { targetRoleHint: 'adam' });
            await supabase
              .from('sms_relay_staging')
