 ...
 *   (a) verified chairman surface — item.source is a surface CONFIRMED chairman-authored, not
 *       merely chairman-ADJACENT.
 ...
 *
 * QF-20260903-023 CORPUS-ERROR CORRECTION: the corpus for (a) used to be session_coordination rows
 * of payload.kind IN ('adam_advisory','solomon_consult'), on the theory (CLAUDE_SOLOMON.md:133)
 * that these were "chairman-authored." MEASURED (Solomon + Adam, independently, 2026-09-03/04, and
 * re-confirmed here 2026-09-04): a 7-day sender_type census of session_coordination shows ZERO rows
 * with sender_type='chairman' — the breakdown is 100% inter-agent traffic (adam/solomon/coordinator/
 * worker/system). session_coordination is the FLEET's own coordination channel; the chairman never
 * writes to it. Scanning it for "missed chairman rulings" therefore measured inter-agent chatter
 * volume, not governance capture, and grew as the fleet talked more — the opposite of the signal it
 * was meant to produce. The real, queryable chairman surface is `sms_relay_staging`: inbound SMS
 * (verified via `phoneKey(from_phone) === phoneKey(CHAIRMAN_PHONE)` plus Twilio `signature_valid`,
 * the same check already used at lib/chairman/sms-bridge.js:1055) is genuinely chairman-authored,
 * unlike anything in session_coordination.
 *
 ...
 * in production, then adds a third `scanEmbeddedMetadataRulings` source here. A separate, ALSO
 * real chairman surface — `chairman_ratifications` itself, and the live in-terminal conversation a
 * seat is scribing FROM — is not a viable corpus source: the former is the capture LEDGER (already
 * captured, by definition, so scanning it for misses is circular), and the latter is not a
 * queryable table at all.
 * Estimated effort: 2-4 hours. Current state: sms_relay_staging (chairman phone, verified
 * signature) + chairman_decisions only.
 */
...
export function evaluatePredicate(item) {
  const text = String(item?.text || '');
  return {
    verifiedSurface: item?.source === 'sms_relay_staging' || item?.source === 'chairman_decisions',
...
export async function scanCorpus(supabase, thresholdHours) {
  ...
  // QF-20260903-023: source (a) is inbound SMS VERIFIED as chairman-authored — his own phone
  // number (phoneKey match against CHAIRMAN_PHONE, the same check already used at
  // sms-bridge.js:1055) AND a valid Twilio signature. session_coordination was measured to
  // contain zero chairman-authored rows (see file header) and is no longer scanned here.
  const chairmanPhoneKey = phoneKey(process.env.CHAIRMAN_PHONE);
  try {
    const { data, error } = chairmanPhoneKey
      ? await supabase
        ...
        .limit(200)
      : { data: null, error: null }; // no CHAIRMAN_PHONE configured — nothing verifiable
    if (!error && Array.isArray(data)) {
      for (const row of data) {
        if (phoneKey(row.from_phone) !== chairmanPhoneKey) continue;
        items.push({
          id: row.id,
          source: 'sms_relay_staging',
          text: row.body_raw || '',
