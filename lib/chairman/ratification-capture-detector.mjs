 ...
 * @param {number} [limitPerSource=200]
 * @returns {Promise<Array<{id:string, source:string, text:string, created_at:string}>>}
 */
export async function scanCorpus(supabase, thresholdHours, limitPerSource = 200) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .limit(limitPerSource); // bounded: candidate-scan window, not a full-table read
...
/**
 * Persist a row to feedback with a bounded, fail-soft insert — a failed persist must never throw
 * out of the caller's fail-soft try/catch (scripts/solomon-advisory.cjs:1294-1300).
 * @param {object} supabase
 * @param {object} row
