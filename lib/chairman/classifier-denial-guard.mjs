 ...
 * chairman_decisions row, decision_type='chairman_approval' — mapped in fn_chairman_decision_value
 * as of database/chairman-gated/20260907_add_chairman_approval_to_decision_value.sql
 * (SD-LEO-INFRA-CHAIRMAN-DECISION-VALUE-001) — instead of leaving the item on a hand-kept board
 * note. REUSE, no new table (76a3c081).
 *
 * CORRECTION (SD-LEO-INFRA-CHAIRMAN-DECISION-VALUE-001): this docstring previously claimed
 * chairman_approval was "an existing, already-mapped arm (see decision-retirement.mjs armOf())".
 * That was wrong and was the root cause of a live defect: armOf() classifies which arm of the
 * chairman_pending_decisions UNION VIEW a row belongs to — an unrelated concept from
 * fn_chairman_decision_value's own (decision_type, action) -> verb CASE mapping, which had no
 * chairman_approval branch until the migration cited above. Every chairman_approval row this
 * module minted before that migration was applied could not be closed via decide (refused
 * UNMAPPED_DECISION_TYPE) — see that migration's header for the live specimen.
 */
  ...
  for (const c of candidates) {
    try {
      ...
      const result = await recordPendingDecision(supabase, {
        ...
        blocking: false,
        raisedBy: 'classifier-denial-guard',
...
export async function resolveAndVerifyClassifierDenial(supabase, { decisionId, action } = {}) {
  ...
  for (const fr of matching) {
    // SD-LEO-INFRA-AUDIT-FIX-FEEDBACK-001-F: supabase-js resolves with {error} rather than
    // throwing on a PostgREST rejection -- this call previously never checked the result at
    // all, so a rejected UPDATE was completely silent.
    ...
    if (error) updateErrors.push({ id: fr.id, error: error.message });
  }
  return { ran: true, verified: true, closed: matching.length - updateErrors.length, updateErrors };
}
