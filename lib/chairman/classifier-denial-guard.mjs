...
/** Is this sd_key+kind already a pending or approved chairman_approval row? Fail-open (a lookup
 *  error must not flood the queue with duplicates on retry, so we skip rather than double-record). */
async function alreadyCovered(supabase, sdKey, kind) {
  const { data, error } = await supabase
    .from('chairman_decisions')
    .select('id')
    .eq('decision_type', 'chairman_approval')
    .contains('brief_data', { context: { sd_key: sdKey, kind } })
    .neq('status', 'rejected')
    .limit(1);
...
export async function runClassifierDenialGuard(supabase) {
  const candidates = [
    ...(await findMigrationApplyWaitCandidates(supabase).catch(() => [])),
    ...(await findClassifierDeniedSignalCandidates(supabase).catch(() => [])),
  ];
  let recorded = 0;
  const errors = [];
  for (const c of candidates) {
    try {
      if (await alreadyCovered(supabase, c.sd_key, c.kind)) continue;
...
export async function resolveAndVerifyClassifierDenial(supabase, { decisionId, action } = {}) {
  ...
  return { ran: true, verified: true, closed: (flagRows || []).length };
}
