...
export async function applyOwedDeliveryTruth(supabase, { messageSid, status, deliveredAt, source }) {
  ...
  // SD-LEO-INFRA-SMS-DELIVERY-STATUS-001 TS-9 (prospective TESTING finding, Q3): a late-arriving
  // staged undelivered/failed callback must never clobber a row that retryOrAlert has since
  // re-armed to 'owed' for a fresh attempt — retryOrAlert's re-arm does NOT change
  // provider_message_id (a new SID is only minted on the actual resend), so the CURRENT-SID-only
  // scope above is insufficient on its own during that window. A genuine terminal confirmation
  // can only ever apply to a row still actively 'sent' on THAT attempt; scoping undelivered/
  // failed to status='sent' closes the window without weakening 'delivered's broader prior-SID
  // resolution (a resend may have already advanced status past 'sent' by the time a genuine
  // delivery confirmation for an earlier attempt arrives, and that case is legitimately honored).
  //
  // Rebuilt as a function (not a single chained builder) so a PGRST204 retry below can re-issue
  // the identical match with a different SET payload — postgrest-js's builder has no re-chainable
  // .update() once .not()/.eq() have already been applied.
  const buildQuery = (updatePatch) => {
    let q = supabase.from('sms_outbound_obligations').update(updatePatch).not('status', 'in', '(delivered,canceled)');
    if (patch.status !== 'delivered') {
      q = q.eq('status', 'sent');
    }
    // count-truncation-diff-lint: bounded by design, not just in practice — this UPDATE targets a
    // single MessageSid (plus, for delivered, its bounded prior_provider_message_ids history), so
    // the affected-row count is inherently a handful at most. The literal cap is a defensive upper
    // bound, never expected to actually engage.
    return q.or(matchFilter).select('id').limit(10);
  };

  let { data, error } = await buildQuery(patch);

  const tableAbsent = Boolean(error) && (error.code === '42P01' || error.code === 'PGRST205');
  const columnAbsent = Boolean(error) && error.code === 'PGRST204';

  // PLAN_VERIFICATION VAL-C2: a missing delivery_status_source column makes PostgREST reject the
  // WHOLE update (PGRST204), not just that one field — so pre-migration, delivered_at/status never
  // get stamped either, silently. Retry the SAME match WITHOUT delivery_status_source so the core
  // delivery-truth write (delivered_at/status) still lands while the column is unapplied; the
  // caller still learns columnAbsent=true so it can withhold marking its own work complete.
  if (columnAbsent) {
    const { delivery_status_source: _omit, ...patchWithoutSource } = patch;
    ({ data, error } = await buildQuery(patchWithoutSource));
  }
