...
export async function runChairmanGatedDecisionRowGuard(supabase, opts = {}) {
  ...
  for (const sd of eligible) {
    ...
    if (excluded) {
      ...
      if (sd.metadata?.gated_guard_prior_hit_at || sd.metadata?.gated_guard_drift_flagged_at) {
        await mergeMetadataKeys(sd.sd_key, { gated_guard_prior_hit_at: null, gated_guard_drift_flagged_at: null });
      }
