...
import { recordPendingDecision, shouldAutoEscalate, escalateChairmanDecision } from './record-pending-decision.mjs';
...
export async function runChairmanGatedDecisionRowGuard(supabase, opts = {}) {
  ...
  const errors = [];
  let hits = 0;
  // QF-20260830-670: one tick can mint several FENCED-SD decisions within milliseconds of each
  // other. Each used to auto-escalate independently, so N hits sent up to min(N,3) IDENTICAL
  // standout emails (every standout renders ALL pending decisions, so a per-row escalation is
  // pure duplication). Record every hit with escalation deferred, then escalate ONCE for the
  // whole batch below — the single email already lists every decision this tick minted.
  const batchEscalationIds = [];
  ...
  for (const sd of eligible) {
    ...
    envelope.skipEscalation = true; // batch-escalated once, below the loop
    ...
    recorded += 1;
    if (result.wouldEscalate) batchEscalationIds.push(result.id);
  ...
  // Escalate ONCE for the whole batch: the primary is the first recorded id, and its standout
  // email already lists every other pending decision (adam-decision-email.mjs renders the full
  // pending set, not just --decision <id>) — see chairman-gated-decision-row-guard.mjs module
  // docstring MECHANISM note.
  if (batchEscalationIds.length) {
    await escalateChairmanDecision(supabase, batchEscalationIds[0]);
  }
