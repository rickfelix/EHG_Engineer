...
// QF-20260912-394: notify the originating seat (not just the chairman's email fallback) that
// its decision packet was carrier-filtered -- previously nothing reached the originator at all.
...
export async function escalateCarrierFiltered(supabase, { sendEmail, logger = console, batchLimit = 25 } = {}) {
  ...
  try {
    ...
    const res = await supabase
      .from('sms_outbound_obligations')
      .select('id, body, last_error, status, decision_id')
  ...
  for (const row of rows) {
    ...
    // QF-20260822-404: SD-LEO-FIX-SMS-OUTBOUND-WORKER-002's discard-class idiom — chain
    // .select('id') and check `error`/zero-rows so a failed or lost-race stamp (row already
    // moved by a concurrent sweep) can't inflate out.escalated for a row that was never marked.
    const { data: stampedRows, error: stampErr } = await supabase
      ...
      .select('id');
    if (stampErr) {
      logger.warn?.(`[sms-channel-health] escalate-stamp UPDATE failed for obligation ${row.id}: ${stampErr.message}`);
    } else if (stampedRows?.length) {
      out.escalated++;
    }
    // QF-20260912-394 (FR-4): best-effort notify the ORIGINATING seat too -- the chairman's
    // email fallback above is the chairman-side channel; the originator (the Adam session that
    // composed this packet) previously had no way to learn its decision silently failed, and
    // read the packet as delivered indefinitely. Resolved via decision_id -> chairman_held_sends
    // .metadata.originator_session_id (the only place this id is recorded — QF-20260905-123).
    // Never blocks/reverts the stamp above: a missing/dead originator or a refused insert (stale
    // session, DISPATCH_TARGET_* errors from insertCoordinationRow) degrades to a logged warning.
    if (row.decision_id) {
      try {
        const { data: heldRow, error: heldErr } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        const originatorSessionId = heldErr ? null : heldRow?.metadata?.originator_session_id;
        if (originatorSessionId) {
          await insertCoordinationRow(supabase, {
            target_session: originatorSessionId,
            message_type: 'INFO',
            payload: {
              kind: 'sms_carrier_filtered_alert',
              subject: `[SMS carrier-filtered] obligation ${row.id} (decision ${row.decision_id})`,
              body: `Your chairman SMS (decision ${row.decision_id}, obligation ${row.id}) was blocked by carrier content filtering and never reached the chairman by text -- it was delivered via the email fallback instead. last_error: ${row.last_error}`,
            },
          }, { logger });
        }
      } catch (e) {
        logger.warn?.(`[sms-channel-health] originator notification failed for obligation ${row.id} (${e?.message || e}) -- the chairman-email fallback above still landed.`);
      }
    }
  }
  return out;
