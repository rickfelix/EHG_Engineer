...
import { writeConsumptionReceipt, describeReceiptOutcome } from '../../consumption/drive-report-receipts.js';
import { DRIVE_REPORT_LANE } from '../../drive-loop/lanes.js';
...
/**
 * SD-LEO-INFRA-DRIVE-LOOP-INSTRUMENT-001-D (FR-3) — read section 2 of the latest drive report.
 *
 * EMBED, NOT RECOMPUTE. chain_to_gate is authored by lib/drive-loop/sections/chain-to-gate.js and
 * stored on the report row. This reads the stored section rather than re-invoking the builder,
 * because re-invoking it here with this file's own waves/items would produce a THIRD representation
 * fed by a third set of inputs — the failure this SD family exists to prevent, one level subtler.
 *
 * Returns null when there is no readable report. The caller renders that absence EXPLICITLY.
 */
export async function fetchChainToGate(supabase) {
  try {
    ...
    const section = data.sections?.chain_to_gate ?? null;
    // reportId travels WITH the section: FR-4 stamps a receipt against the report actually read,
    // and a receipt that cannot name its report is not a receipt.
    return section ? { reportId: data.id, section } : null;
  } catch { return null; }
}

/**
 * FR-3 — render section 2, or say plainly that it is unavailable.
 *
 * THE ABSENCE IS RENDERED, NEVER LEFT BLANK AND NEVER BACKFILLED. drive_reports is on main but the
 * migration is chairman-gated and unapplied, so today this always reports unavailable. That is the
 * intended behaviour per the coordinator ruling: an honest gap is a smaller defect than a confident
 * wrong answer, and a visible gap keeps pressure on the gated migration instead of hiding it behind
 * a substitute computation that looks fine.
 */
/**
 * SD-LEO-INFRA-DRIVE-LOOP-INSTRUMENT-001-D (FR-4) — the chairman_brief consumption receipt.
 *
 * The SHARED writer, not a local insert: the adam lane and this lane must not drift on the one
 * property that matters — a refused write is never reportable as written. Note the lane spelling is
 * chairman_brief with an UNDERSCORE and comes from DRIVE_REPORT_LANES via the writer; every prose
 * message in this SD's thread wrote it hyphenated, and the CHECK constraint rejects that.
 *
 * Never throws: a receipt failure must not cost the chairman his brief. The VERDICT is returned so
 * the caller can render it — fail-open on control flow, truthful on outcome.
 *
 * ── WHAT THIS RECEIPT ATTESTS, STATED ON THE ROW ──────────────────────────────────────────
 * That the section was EMBEDDED INTO THE BRIEF DOCUMENT. Not that the chairman read it, and not
 * that the brief was delivered — delivery truth belongs to lib/chairman/sms-outbound-worker.js
 * (provider accept vs. delivered callback), and the sweep that calls this only ENQUEUES; a brief
 * built inside the quiet window can still be dropped downstream. So the claim is written onto the
 * row rather than left to the lane name to imply. A reader asking "did the chairman get it?" gets
 * a row that answers a narrower question honestly instead of a broader one falsely — a lane named
 * chairman_brief invites the broad reading, and an unqualified receipt would supply it.
 */
export async function stampChairmanBriefReceipt(supabase, reportId) {
  try {
    return await writeConsumptionReceipt(supabase, {
      reportId,
      lane: DRIVE_REPORT_LANE.CHAIRMAN_BRIEF,
      metadata: { attests: 'embedded_in_brief_document', not_attested: 'delivery_to_chairman' },
    });
  } catch { return null; }
}

export function renderChainToGate(section) {
  if (!section) {
    return ['  Chain to gate: UNAVAILABLE — no readable drive report (drive_reports is not live; its migration is chairman-gated and unapplied). Deliberately not substituted by a local computation.'];
  }
  const out = [];
  const gate = section.gate?.value;
  out.push(gate?.title
    ? `  Chain to gate: ${gate.title} (wave ${gate.sequence_rank})`
    : '  Chain to gate: no gate located — see section 2 limitation');
  const chainLen = section.chain_length?.value;
  if (typeof chainLen === 'number') out.push(`      chain: ${chainLen} open item(s) in the gate wave`);
  const b = section.blocker?.value;
  if (b) {
...
async function buildPlanOfRecordSection(supabase, { velocityLookbackDays }) {
  try {
    ...
    const { data: waves, error: wavesErr } = await supabase
      .from('roadmap_waves')
      // SD-LEO-INFRA-DRIVE-LOOP-INSTRUMENT-001-D (FR-3): depends_on_wave_ids + metadata added —
      // the blocker is DERIVED from the dependency chain, which is what "the chain to the next wave
      // gate" actually means. No blocker column exists on roadmap_waves (verified at
      // information_schema, not assumed).
      .select('id, title, sequence_rank, status, confidence_score, depends_on_wave_ids, metadata')
    ...
    const chainToGate = await fetchChainToGate(supabase);
    ...
    const data = {
      ...
      waves: waveSummary,
      chain_to_gate: chainToGate?.section ?? null,
    ...
    // FR-3 (corrected, coordinator ruling 2026-08-04): EMBED section 2 rather than compute a rival.
    // lib/drive-loop/sections/chain-to-gate.js already defines gate / chain / blocker / owner, and
    // this leg's original derivation answered a DIFFERENT question under the same label — its
    // "blocker" was an unmet dependency WAVE, section 2's is the first STUCK ITEM. A brief showing
    // the chairman a dependency-wave blocker where section 2 shows a stuck-item blocker does not
    // degrade gracefully; it points him at the wrong thing with full confidence. Rendering
    // something is only a virtue when the something is the SAME answer.
    //
    // So: embed the STORED section from the latest drive report. Not a re-invocation of the builder
    // with this file's own inputs — that would be a third representation with a third set of inputs.
    lines.push(...renderChainToGate(data.chain_to_gate));
    // FR-4: the chairman_brief lane stamps its OWN receipt, and only for a report it actually
    // embedded. No section means no delivery means no receipt — the same rule FR-2 applies to the
    // adam lane. Failure is REPORTED rather than absorbed: a refused receipt makes the lane read as
    // "never consumed", which is indistinguishable from the producer never publishing.
    if (chainToGate?.reportId) {
      const verdict = await stampChairmanBriefReceipt(supabase, chainToGate.reportId);
      if (!verdict || !verdict.written) {
        lines.push(`  ⚠️  ${describeReceiptOutcome(verdict)}`);
      }
    }
