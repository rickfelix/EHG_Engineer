...
/**
 * SD-LEO-INFRA-DRIVE-LOOP-INSTRUMENT-001-D (FR-3) — what is GATING this wave?
 *
 * PURE, and taking its inputs as parameters, so every branch below is unit-testable without a
 * database — including the ones that only occur on data the live roadmap does not currently have.
 *
 * WHY THE DEPENDENCY CHAIN IS THE SOURCE. roadmap_waves has NO blocker column and NO owner column
 * (checked at information_schema). Measured on the CANONICAL roadmap via resolveCanonicalRoadmap —
 * 8 waves, not the 23 rows a naive query returns, which would have included archived duplicates:
 * depends_on_wave_ids covers 5/8, metadata.chairman_review_gate 1/8. So the blocker is real and
 * partial; the owner lane had no source at all and is deliberately NOT rendered (descoped by
 * coordinator ruling rather than filled with "unknown" for 7 of 8 waves, which would manufacture
 * the appearance of a tracked field).
 *
 * "HAS DEPENDENCIES" IS NOT "IS BLOCKED" — this evaluates the DEPENDED-ON WAVE, never the length of
 * the array. Counting the array reports a blocker on a wave whose dependencies are already
 * satisfied: a false-positive gate on a healthy wave, which is the direction that erodes trust in
 * the whole line.
 *
 * AND "APPROVED" IS NOT "COMPLETE". The only wave statuses that exist are proposed / approved /
 * archived — there is no completed state, so status cannot answer "is the dependency done". The
 * honest completion signal is the item roll-up already computed here: a wave is satisfied when
 * every non-void item is satisfied_elsewhere. A dependency with NO items is not evidence of
 * completion and is reported as unknown rather than silently treated as clear.
 *
 * @param {object} wave              the wave being described
 * @param {Map<string,object>} depIndex   wave_id -> { title, status } for every referenced dependency
 * @param {Map<string,object>} countsByWave wave_id -> { total, promoted }
 * @returns {string|null} a human-readable gate, or null when nothing gates this wave
 */
export function deriveWaveBlocker(wave, depIndex, countsByWave) {
  const gate = wave?.metadata?.chairman_review_gate;
  // A chairman gate outranks a dependency: it is a decision someone must make, not work someone
  // must finish, and it is the one a reader can act on.
  if (gate) return `chairman review gate — ${typeof gate === 'string' ? gate : 'awaiting chairman decision'}`;

  const deps = Array.isArray(wave?.depends_on_wave_ids) ? wave.depends_on_wave_ids : [];
  if (deps.length === 0) return null;

  for (const depId of deps) {
    const dep = depIndex.get(depId);
    // A dependency we cannot resolve is NOT reported as clear. It usually means the depended-on
    // wave is not ratified (proposed/archived) and so never entered the rendered set.
    if (!dep) return 'an unresolved dependency wave (not on the ratified plan of record)';
    if (!RATIFIED_WAVE_STATUSES.includes(dep.status)) return `${dep.title} (status: ${dep.status}, not ratified)`;
    const c = countsByWave.get(depId);
    if (!c || c.total === 0) return `${dep.title} (no items tracked — completion unknown)`;
    if (c.promoted < c.total) return `${dep.title} (${c.promoted}/${c.total} items satisfied)`;
  }
  return null;
}
...
async function buildPlanOfRecordSection(supabase, { velocityLookbackDays }) {
  try {
    ...
    // FR-3: resolve every referenced dependency, DELIBERATELY UNFILTERED BY STATUS. The wave query
    // above is scoped to ratified waves, so a dependency pointing at a proposed or archived wave is
    // absent from `waves_` — and a dependency we cannot see is exactly the case that must NOT read
    // as "nothing is blocking this". Fetching them separately is what lets deriveWaveBlocker tell
    // "not ratified" apart from "unresolvable".
    const depIndex = new Map();
    const referencedDepIds = [...new Set(waves_.flatMap((w) => (Array.isArray(w.depends_on_wave_ids) ? w.depends_on_wave_ids : [])))];
    if (referencedDepIds.length > 0) {
      const { data: depRows } = await supabase
        .from('roadmap_waves')
        .select('id, title, status')
        .in('id', referencedDepIds);
      for (const d of depRows || []) depIndex.set(d.id, d);
    }
    ...
    const waveSummary = waves_.map((w) => ({
      ...
      blocker: deriveWaveBlocker(w, depIndex, countsByWave),
    }));
    ...
    for (const w of data.waves) {
      ...
      // FR-3: ADDITIVE. The counts line above is untouched — a prior SD deliberately removed a
      // misleading "progress N%" clause from it, so the gate goes on its own line rather than being
      // folded in. Emitted only when something actually gates the wave; see the coverage note below
      // for why silence here does not mean "unblocked".
      if (w.blocker) lines.push(`      gated by: ${w.blocker}`);
    }
    // FR-3: SAY WHAT THE LINE CANNOT SEE. depends_on_wave_ids is populated on 5 of 8 waves on the
    // plan of record, so a wave with no "gated by" line may be genuinely clear OR may simply have
    // no dependency recorded. Stating the coverage stops absence from being read as evidence.
    // The OWNER LANE is deliberately absent: no column and no metadata convention carries
    // ownership (verified at information_schema), and printing "unknown" for 7 of 8 waves would
    // manufacture the appearance of a field nobody is tracking. Descoped by coordinator ruling
    // 2026-08-04, recorded rather than silently dropped.
    if (data.waves.length) {
      const withDeps = data.waves.filter((w) => w.blocker !== undefined && w.blocker !== null).length;
      lines.push(`  (gates shown for ${withDeps} of ${data.waves.length} waves — a wave with no gate line has no recorded dependency, which is not the same as being unblocked. Owner lane not tracked.)`);
    }
