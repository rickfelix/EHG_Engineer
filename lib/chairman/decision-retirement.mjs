 ...
 *
 * NARROWLY EXTENDED (SD-LEO-INFRA-CHAIRMAN-DECISION-VALUE-001 FR-4): 'review' added to arm4.
 * chairman_approval was the only chairman_decisions.decision_type this switch recognised, even
 * though arm4 is the WHOLE chairman_decisions table — every other live decision_type in that table
 * (review, migration_apply, gate_decision, session_question, ...) fell through to 'unknown' and
 * could never be retired. Only 'review' is added here (the specimen this SD's acceptance
 * criterion needs, d87a7018) rather than defaulting every unrecognised type to arm4 — measuring
 * whether the other chairman_decisions types genuinely belong here too is a separate, unverified
 * question out of this SD's scope, matching this module's own "measure before widening" discipline.
 */
export function armOf(row) {
  switch (row?.decision_type) {
    case 'chairman_approval': return 'arm4';
    case 'review': return 'arm4';
