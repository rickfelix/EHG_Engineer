...
import { smsOutboundObligationsLive, enqueueChairmanSms, stageDecisionSmsNotification, invalidateDecisionSmsStaging, TOKEN_TTL_MS } from './sms-bridge.js';
import { smsQuietWindowReleaseIso, buildLiveSlotDedupeKeyCandidates } from '../time/chairman-et-wall-clock.js';
import { resolveChairmanZone } from '../comms/adam-outbound/quiet-hours-extension.js';
// SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 (TR-5): reused, not duplicated -- this is the SAME
// DB-first chairman-identity resolver the Adam-outbound gate stages decisions with, imported
// from its OWN small module (not chairman-sms-gate/index.js directly, and re-exported from
// there for backward compatibility). Importing the gate file itself would statically pull in
// its dynamic import of lib/adam/should-consult-solomon.js (the outbound-sink-conformance
// census's CONSULT_GATE) for an entirely unrelated code path, falsely flipping this file (and
// lib/chairman/sms-channel-health.js, which imports this file) to "reaches the consult gate" in
// that census's file-level reachability graph -- confirmed via bisection against the census test.
import { resolveChairmanUserId } from '../comms/adam-outbound/resolve-chairman-user-id.js';

export const DEFAULT_MAX_ATTEMPTS = 3;
export const DEFAULT_BATCH_LIMIT = 25;
// QF-20260912-394: Twilio error codes confirmed (live-measured) to mean a PERMANENT
// carrier-side rejection of this exact body -- resending the identical body will fail
// identically every time. Deliberately starts with only the one code actually measured
// (30007 "carrier violation" / content filtering); extend only once another code is
// similarly confirmed live, never speculatively.
export const TERMINAL_CARRIER_ERROR_CODES = new Set(['30007']);
...
//
// SECURITY (EXEC-TO-PLAN review, SEC-5): this threshold MUST stay well above the widest gap
// scripts/cron/chairman-hourly-heartbeat-backstop-sweep.mjs's own coverage read can span
// (SLOT_HOURS_ET = [6,9,12,15,18,21], max 3h between slots, QF-20260828-650) so a genuinely
// in-flight row is never voided out from under a live dispatcher mid-attempt.
//
// QF-20260911-372 CORRECTION: this comment previously said that sweep's classifier "maps
// status=canceled to do_not_retry" and relied on this threshold staying above the inter-slot
// gap to prevent a voided heartbeat row from suppressing its own backstop replacement. That was
// the live incident (freeze 2026-09-08 to 09-11, 68.4h: three due slots' own backstop rows went
// stale past this exact threshold with no credentialed dispatcher running, were voided here, and
// the classifier then silenced the chairman for 68h reading 'canceled' as "already handled").
// The classifier is now fixed at the root — 'canceled' is never a do_not_retry signal there,
// only 'owed_escalate' is — so a margin between this threshold and the inter-slot gap is no
// longer load-bearing for THAT failure mode. Kept high regardless: voiding too eagerly still
// cancels rows a live dispatcher may be about to genuinely deliver.
...
// QF-20260904-715 guard (3'): these two literal kinds must stay byte-identical to
// scripts/cron/chairman-hourly-heartbeat-backstop-sweep.mjs's own LIVE_KIND/BACKSTOP_KIND. That
// file is import-exempt by its own header ("never via ... a static JS import"), so the two
// strings are duplicated here rather than imported.
const HEARTBEAT_LIVE_KIND = 'heartbeat_status';
const HEARTBEAT_BACKSTOP_KIND = 'heartbeat_status_backstop';
// QF-FOLLOWUP-715-CANCEL-AGE-BOUND: a live row in 'owed'/'sending' has NOT been confirmed
// delivered -- if it gets stuck there (crash, misconfig, never actually dispatched), counting
// it as coverage cancels the backstop forever with nothing having gone out. Mirrors
// STALENESS_GRACE_MS's non-terminal-in-flight window in the sweep file (same "recently
// enqueued, plausibly about to complete" grace), duplicated per the import-exemption above.
const LIVE_ENQUEUED_STALENESS_GRACE_MS = 5 * 60 * 1000;
 ...
 * @param {string} [opts.errorDetail] - QF-20260912-394: when the caller already has a
 *   provider-confirmed error code/message for this attempt (e.g. from checkMessageStatus),
 *   pass it here so it is stamped as last_error on the re-arm branch too (previously ONLY
 *   the final ALERTED stamp ever recorded anything, so every intermediate attempt's real
 *   diagnostic was lost). Falls back to `reason` when absent, matching prior behavior exactly.
 * @returns {Promise<'retried'|'alerted'|'skipped'>}
 */
async function retryOrAlert(supabase, row, { maxAttempts, alert, fromStatuses, reason, errorDetail, now, zone, logger = console }) {
  if ((row.attempts || 0) >= maxAttempts) {
    ...
    const { error: alertErr } = await supabase
      ...
      .update({ status: 'failed', last_error: `ALERTED: ${errorDetail || row.last_error || reason}` })
      .eq('id', row.id)
  ...
  const { error: retryErr } = await supabase
    ...
    .update({
      ...
      claimed_at: null,
      claimed_by: null,
      not_before: smsQuietWindowReleaseIso(now, zone),
      ...(errorDetail ? { last_error: errorDetail } : {}),
    })
    .eq('id', row.id)
    .in('status', fromStatuses)
    .select('id');
 ...
 *
 * SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 (FR-1/FR-2/TR-2/TR-4): the re-ask below now STAGES a
 * fresh reply token + chairman_notifications row (via stageDecisionSmsNotification, the same
 * mechanism chairman-sms-gate/index.js uses for a fresh ask) BEFORE calling enqueueChairmanSms --
 * previously it called enqueueChairmanSms directly with no staging at all, so a chairman reply to
 * the re-emitted question could never match (the original token, if any, is what triggered this
 * branch by having already gone stale). Staging runs strictly before dispatch (fail-closed, TR-2):
 * an identity-resolution or staging failure aborts the RE-ASK entirely -- no fresh token/notification
 * is ever created without a successful stage. chairmanPhone is pinned to row.recipient_phone (TR-4),
 * never an env-var fallback, so a staged notification's matchable phone is BYTE-IDENTICAL to the
 * phone enqueueChairmanSms actually dispatches to two lines below.
 *
 * SECURITY review finding (EXEC-TO-PLAN, evidence row f1855b8f-634f-4047-82d8-7e9c78aaf9af, SEC-A,
 * non-blocking doc correction): a staging failure does NOT prevent Pass 2's pre-existing, unmodified
 * claim+send from later dispatching that SAME still-owed original row through its normal (unstaged)
 * path -- identical, pre-existing behavior to the no-decision_id case (TS-11) and unrelated to this
 * fix's own scope. "Nothing unmatchable is ever sent" describes only the fresh RE-ASK this branch
 * creates, not the original row's fate afterward.
 *
 * @returns {Promise<{voided: number, reEmitted: number, collapsed: number, writeErrors: number}>}
 */
async function voidStaleAndCollapseObligations(supabase, { now, staleThresholdMs, alert, logger, resolveChairmanUserId: resolveChairmanUserIdOverride }) {
  const resolveIdentity = typeof resolveChairmanUserIdOverride === 'function' ? resolveChairmanUserIdOverride : resolveChairmanUserId;
  ...
  for (const row of rows || []) {
    ...
    if (row.kind === DECISION_KIND) {
      if (isStaleAndDue(row) && row.decision_id) {
        // QF-20260829-320 fix (i): the owed-only select above cannot see chairman_decisions, so
        // without this check a stale question keeps re-emitting even after the chairman already
        // decided it. Fail toward re-asking: a lookup error or absent row is treated as still
        // pending so a genuinely unanswered question is never silently swallowed.
        const { data: decisionRows, error: decisionErr } = await supabase
          ...
          .limit(1);
        if (decisionErr) {
          logger.warn?.(`[sms-outbound-worker] decision status lookup failed for ${row.decision_id}: ${decisionErr.message} — treating as pending, proceeding with re-ask`);
        }
        const decisionStatus = decisionRows?.[0]?.status;
        if (decisionStatus && decisionStatus !== 'pending') {
          const { data: voidedRows, error: voidErr } = await supabase
            .from('sms_outbound_obligations')
            .update({ status: CANCELED_STATUS, last_error: `${VOIDED_STALE_PREFIX}decided:${decisionStatus}` })
            .eq('id', row.id)
            .eq('status', 'owed')
            .select('id')
            .limit(1);
          if (voidErr) {
            logger.warn?.(`[sms-outbound-worker] decided-decision void UPDATE failed for ${row.id}: ${voidErr.message}`);
            result.writeErrors++;
        ...
        // FR-1/FR-2/TR-2: stage BEFORE dispatch, fail-closed. TR-4: chairmanPhone is pinned to
        // row.recipient_phone -- the SAME value enqueueChairmanSms below actually sends to -- never
        // process.env.CHAIRMAN_PHONE, so staging and dispatch can never silently diverge.
        let staged;
        let stagedToken;
        try {
          const chairmanUserId = await resolveIdentity(supabase, {});
          stagedToken = crypto.randomBytes(16).toString('hex');
          const expiresAt = new Date(now + TOKEN_TTL_MS).toISOString();
          staged = await stageDecisionSmsNotification(supabase, {
            decisionId: row.decision_id,
            chairmanUserId,
            chairmanEmail: process.env.CHAIRMAN_EMAIL,
            chairmanPhone: row.recipient_phone,
            token: stagedToken,
            expiresAt,
            status: 'queued',
          });
        } catch (stageErr) {
          logger.warn?.(`[sms-outbound-worker] re-ask staging failed for stale decision obligation ${row.id} (decision ${row.decision_id}): ${stageErr.message} -- NOT re-emitting, original stays owed for next sweep`);
          continue;
        }
        ...
        if (reEmitRes.enqueued) {
          ...
          // The staged token/notification is now live but nothing was ever sent referencing it --
          // invalidate so a chairman cannot "reply" to a question that was never actually asked.
          await invalidateDecisionSmsStaging(supabase, {
            decisionId: row.decision_id,
            token: stagedToken,
            notificationId: staged && staged.notificationId,
            reason: `re_emit_enqueue_failed: ${reEmitRes.reason || 'unknown'}`,
          }).catch(() => {});
        }
      }
      continue;
  ...
  // QF-20260913-013: grouped by (kind, decision_id) when a decision_id is present -- two DISTINCT
  // decisions' notices sharing a generic kind (e.g. heartbeat_status) must never collapse into one
  // another (measured: a lost-decision notice for decision A voided a lost-decision notice for
  // decision B as "superseded by newest", so the chairman never saw B's notice at all). A genuine
  // same-decision duplicate (same kind AND same decision_id) still collapses to its newest, as before.
  const byKind = new Map();
  for (const row of surviving) {
    const key = row.decision_id ? `${row.kind}:${row.decision_id}` : row.kind;
 ...
 *   staleThresholdMs?: number, burstCap?: number, resolveChairmanUserId?: Function}} [opts]
 ...
 *   resolveChairmanUserId: injectable (supabase, message) => Promise<string>, default the real
 *   DB-first resolver (SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 TR-5) -- the real resolver calls
 *   supabase.rpc(), which the fake Supabase double used by this file's tests does not expose;
 *   tests inject a stub, matching the identical seam chairman-sms-gate/index.js already uses.
 ...
 *   reEmitted: number, collapsed: number, unconfigured: number, canceledLiveCovered: number}>}
 */
export async function reconcileOutboundSms(supabase, opts = {}) {
  ...
  const summary = { ran: false, claimed: 0, sent: 0, failed: 0, retried: 0, alerted: 0, skipped: 0, reaped: 0, sentTimedOut: 0, escalated: 0, confirmedDelivered: 0, voided: 0, reEmitted: 0, collapsed: 0, unconfigured: 0, canceledLiveCovered: 0 };
  ...
  for (const row of (pass1cProviderConfigured ? (sentRows || []) : [])) {
    ...
    let checkedErrorCode;
    let checkedErrorMessage;
    try {
      ...
      checkedErrorCode = check.errorCode;
      checkedErrorMessage = check.errorMessage;
    } catch (err) {
    ...
    if (checkedStatus === 'delivered') {
      ...
      // QF-20260912-394: a permanent carrier-side content-filter rejection (e.g. Twilio 30007)
      // means the identical body will be refused again identically — resending it up to
      // maxAttempts times just delays the alert and wastes the retry budget on a guaranteed
      // failure. On a known-terminal code, skip the bounded retry/re-arm entirely: alert once
      // and mark failed NOW, with the real code/message recorded so the carrier-filter email
      // fallback (sms-channel-health.js's escalateCarrierFiltered, already wired into the
      // reconcile-sweep cron) can match it on its very next pass.
      const errorDetail = checkedErrorCode
        ? `provider_error_${checkedErrorCode}${checkedErrorMessage ? `: ${checkedErrorMessage}` : ''}`
        : undefined;
      if (checkedErrorCode && TERMINAL_CARRIER_ERROR_CODES.has(checkedErrorCode)) {
        try { await alert(row); } catch { /* alert seam is best-effort */ }
        const { error: terminalErr } = await supabase
          .from('sms_outbound_obligations')
          .update({ status: 'failed', last_error: `ALERTED: ${errorDetail}` })
          .eq('id', row.id)
          .eq('status', 'sent')
          .select('id');
        if (terminalErr) {
          logger.warn?.(`[sms-outbound-worker] terminal carrier-filter stamp UPDATE failed for ${row.id}: ${terminalErr.message}`);
        }
        summary.alerted++;
      } else {
        // Provider-CONFIRMED non-delivery (never blind) — the normal bounded retry/alert path.
        const outcome = await retryOrAlert(supabase, row, { maxAttempts, alert, fromStatuses: ['sent'], reason: 'sent_no_delivery_callback_provider_confirmed', errorDetail, now, zone: chairmanZone, logger });
  ...
  const voidCollapseResult = await voidStaleAndCollapseObligations(supabase, { now, staleThresholdMs, alert, logger, resolveChairmanUserId: opts.resolveChairmanUserId });
  ...
  const OWED_SELECT_COLUMNS = 'id, kind, created_at, dedupe_key, recipient_phone, body, attempts, decision_id, not_before, media_url, provider_message_id, prior_provider_message_ids';
  ...
  // SINCE been sent/delivered, or is itself still owed/sending (i.e. the live path is actively
  // handling this slot right now, in this same pass or not), is CANCELED here -- never sent.
  // Scope: any heartbeat_status row created at/after this backstop row's OWN created_at (the
  // backstop always enqueues within minutes of its due slot; the live send this guards against
  // is always later, per the measured distribution) counts as covering the slot. A row that
  // survives this check is re-verified unfilled AT THIS INSTANT, so its body is recomposed here
  // (never the frozen enqueue-time string, which can go stale across a quiet-hours deferral).
  const stillClaimable = [];
  for (const row of claimable) {
    if (row.kind !== HEARTBEAT_BACKSTOP_KIND) { stillClaimable.push(row); continue; }
    // dedupe_key format is `${HEARTBEAT_BACKSTOP_KIND}:${hourKey}` (hourKey e.g. '2026-07-18T12').
    // QF-20260911-252: also derive this slot's live-side dedupe_key candidates so the ordering
    // check below can be relaxed for an early send (see keyCandidates use further down).
    const hourKey = (row.dedupe_key || '').split(':')[1] || 'unknown-slot';
    const slotMatch = /^\d{4}-\d{2}-\d{2}T\d{2}$/.test(hourKey);
    const keyCandidates = slotMatch
      ? buildLiveSlotDedupeKeyCandidates(Number(hourKey.slice(-2)), hourKey.slice(0, 10))
      : [];
    // No .gte() -- filtered in JS below so this reuses the same eq/in query shape every other
    // fake-supabase test double in this file already models, rather than requiring a new one.
    // order+limit together (not limit alone): heartbeat_status accumulates indefinitely (no
    // pruning), so an unordered .limit(50) could return an arbitrary 50 rows instead of the
    // newest 50 -- the only ones this row-created_at-anchored check can ever actually match.
    const { data: candidateLiveRows, error: liveCheckErr } = await supabase
      .from('sms_outbound_obligations')
      .select('id, created_at, status, dedupe_key')
      .eq('kind', HEARTBEAT_LIVE_KIND)
      .in('status', ['sent', 'delivered', 'owed', 'sending'])
      .order('created_at', { ascending: false })
      .limit(50);
    if (liveCheckErr) {
      logger.warn?.(`[sms-outbound-worker] guard-3' live-coverage check failed for ${row.id}: ${liveCheckErr.message} — failing toward sending (never silently suppress a backstop on a read error)`);
      stillClaimable.push(row);
      continue;
    }
    // TESTING-CRITICAL (adversarial review of QF-20260904-715): 'owed'/'sending' with no age
    // bound let a stuck-forever enqueued row cancel the backstop and then classify as
    // do_not_retry at the sweep, permanently disarming the dead-man for that slot. A terminal
    // status (sent/delivered) always counts; a non-terminal one only counts while still
    // plausibly in flight.
    const now = Date.now();
    const liveRows = (candidateLiveRows || []).filter((r) => {
      const isCurrent = (r.status === 'sent' || r.status === 'delivered')
        || (now - new Date(r.created_at).getTime()) < LIVE_ENQUEUED_STALENESS_GRACE_MS;
      if (!isCurrent) return false;
      if (r.created_at >= row.created_at) return true;
      // QF-20260911-252: an early live send (created BEFORE the backstop's own created_at, so
      // invisible to the ordering check above) still counts as coverage when its dedupe_key
      // matches this exact slot -- the same additive, format-tolerant signal the sweep itself
      // checks before ever enqueueing this row. A row with no/unrecognized dedupe_key on either
      // side falls through unchanged (keyCandidates is empty, or r.dedupe_key never matches).
      return keyCandidates.length > 0 && keyCandidates.includes(r.dedupe_key);
    });
    if (liveRows.length > 0) {
      await supabase.from('sms_outbound_obligations')
        .update({ status: CANCELED_STATUS, last_error: 'live_covered_at_drain' })
        .eq('id', row.id).eq('status', 'owed').select('id').limit(1);
      summary.canceledLiveCovered++;
      continue;
    }
    const freshBody = `[backstop ${hourKey}] Still here — routine heartbeat check-in (no live heartbeat reached for this slot; re-verified at drain).`;
    await supabase.from('sms_outbound_obligations').update({ body: freshBody }).eq('id', row.id).eq('status', 'owed');
    stillClaimable.push(row);
  }
  claimable = stillClaimable;
