...
export async function reconcileOutboundSms(supabase, opts = {}) {
  ...
  for (const row of claimable) {
    ...
    const liveRows = (candidateLiveRows || []).filter((r) => {
      if (r.created_at < row.created_at) return false;
      if (r.status === 'sent' || r.status === 'delivered') return true;
      return (now - new Date(r.created_at).getTime()) < LIVE_ENQUEUED_STALENESS_GRACE_MS;
    });
