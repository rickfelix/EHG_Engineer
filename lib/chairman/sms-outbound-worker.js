 ...
 * an identity-resolution or staging failure aborts the re-emit entirely -- the row stays owed for
 * the next sweep, nothing unmatchable is ever sent. chairmanPhone is pinned to row.recipient_phone
 * (TR-4), never an env-var fallback, so the staged notification's matchable phone is BYTE-IDENTICAL
 * to the phone enqueueChairmanSms actually dispatches to two lines below.
 *
