 ...
 * voids non-decision rows older than DEFAULT_STALE_THRESHOLD_MS (reusing status='canceled',
 * never a new status value — see CANCELED_STATUS) and collapses same-kind duplicates to the
 * newest; decision_question rows are NEVER auto-voided (a stale one with a decision_id is
 * re-emitted as a fresh re-ask instead). Pass 2 additionally slices the oldest-first claimable
 ...
 * TR-1's single spelling authority) instead of being claimed for send.
 *
 ...
 * never deduped against the stale original); a stale one WITHOUT a decision_id (measured live:
 * 71% of decision_question rows) has nothing to re-emit against and is left completely
 * untouched — still owed, still eligible for normal (burst-cap-bounded) sending.
 *
 ...
 *
 * TR-5: every UPDATE here binds and checks `error`. A rejected write is logged and the row is
 * excluded from the voided/collapsed count (left `owed` for the next sweep) rather than being
 * silently believed successful.
 *
 * @returns {Promise<{voided: number, reEmitted: number, collapsed: number, writeErrors: number}>}
 */
async function voidStaleAndCollapseObligations(supabase, { now, staleThresholdMs, logger }) {
  const result = { voided: 0, reEmitted: 0, collapsed: 0, writeErrors: 0 };

  const { data: rows, error: selectErr } = await supabase
    .from('sms_outbound_obligations')
    .select('id, kind, decision_id, recipient_phone, body, created_at')
    .eq('status', 'owed')
  ...
  for (const row of rows || []) {
    // FR-2/VAL-3: decision_question rows are NEVER voided and NEVER collapse-candidates,
    // regardless of staleness — checked FIRST, before the staleness branch below, so a FRESH
    // decision row is excluded from `surviving` (and thus from the by-kind collapse step) just
    // as reliably as a stale one. A stale one WITH a decision_id gets a fresh re-ask; WITHOUT
    // one (measured live: 71% of decision_question rows), or if not stale at all, it is left
    // completely untouched.
    if (row.kind === DECISION_KIND) {
      const isStale = new Date(row.created_at).getTime() < staleCutoff;
      if (isStale && row.decision_id) {
        ...
        if (reEmitRes.enqueued) result.reEmitted++;
        else logger.warn?.(`[sms-outbound-worker] re-ask enqueue failed for stale decision obligation ${row.id}: ${reEmitRes.reason || 'unknown'}`);
      }
      continue;
    }

    const isStale = new Date(row.created_at).getTime() < staleCutoff;
    if (!isStale) { surviving.push(row); continue; }
  ...
  for (const group of byKind.values()) {
    ...
    for (const dup of group.slice(0, -1)) {
      const { error: collapseErr } = await supabase
        .from('sms_outbound_obligations')
 ...
 *   reEmitted: number, collapsed: number}>}
 */
export async function reconcileOutboundSms(supabase, opts = {}) {
  ...
  const summary = { ran: false, claimed: 0, sent: 0, failed: 0, retried: 0, alerted: 0, skipped: 0, reaped: 0, sentTimedOut: 0, escalated: 0, confirmedDelivered: 0, voided: 0, reEmitted: 0, collapsed: 0 };
  ...
  for (const row of sentRows || []) {
    ...
    if (checkedStatus === 'delivered') {
      ...
      const deliveredAtIso = parsedDeliveredAt && !Number.isNaN(parsedDeliveredAt.getTime())
  ...
  const voidCollapseResult = await voidStaleAndCollapseObligations(supabase, { now, staleThresholdMs, logger });
