...
describe("verifyMarkerAcrossTargetContracts — child A's verdict is never second-guessed", () => {
  ...
  // FR-4: the other two of the three fail-open exits, previously untested (only no_manifest was
  // covered above) — same report-not-refuse contract, same row.
  it('returns the base verdict unchanged when section_id is unknown, without reading the row', async () => {
    const sb = makeSupabase(rowWith(['adam']));
    const result = await verifyMarkerAcrossTargetContracts(sb, 'row-1', { ...REF, section_id: '999' }, MARKER, {
  ...
  it('returns the base verdict unchanged when the target file is unreadable, without reading the row', async () => {
    const missingFileRoot = mkdtempSync(join(tmpdir(), 'multi-target-missing-file-'));
    writeFileSync(join(missingFileRoot, 'claude-generation-manifest.json'), JSON.stringify({
      section_digests: { meta: { 94: { target_file: 'CLAUDE_ADAM.md' } } },
    }));
    // CLAUDE_ADAM.md deliberately not written — the manifest names a file this root lacks.
    const sb = makeSupabase(rowWith(['adam']));
    const result = await verifyMarkerAcrossTargetContracts(sb, 'row-1', REF, MARKER, {
      repoRoot: missingFileRoot, driftProbe: freshDrift, deps: deps({}),
    });
    expect(result.verified).toBe(false);
    expect(result.reason).toBe('unreadable_file');
    expect(result.multi_target_checked).toBe(false);
    expect(sb.from).not.toHaveBeenCalled();
  });
