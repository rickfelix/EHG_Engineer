...
// QF-20260903-023: corpus source (a) is now sms_relay_staging gated on a verified chairman
// phone number (see ratification-capture-detector.mjs's header for the measured corpus-error
// this replaces). Fixed for the whole file since every fakeSupabaseFor test needs it.
const CHAIRMAN_PHONE = '+15551234567';
let priorChairmanPhone;
beforeEach(() => { priorChairmanPhone = process.env.CHAIRMAN_PHONE; process.env.CHAIRMAN_PHONE = CHAIRMAN_PHONE; });
...
describe('evaluatePredicate / classifyItem — 3-part predicate', () => {
  ...
  test('verified surface + directive verbs, NO named target -> candidate (never flag)', () => {
    const item = { source: 'sms_relay_staging', text: 'The chairman ratified this approach going forward.' };
    const p = evaluatePredicate(item);
    expect(p.verifiedSurface).toBe(true);
    expect(p.directiveVerbs).toBe(true);
...
describe('row builders — feedback table shape', () => {
  ...
  test('buildCandidateRow: category=ratification_capture_candidate, distinct from flag category', () => {
    const row = buildCandidateRow({ id: 'x2', source: 'sms_relay_staging', text: 'candidate text', created_at: '2026-08-29T00:00:00Z' });
...
function fakeSupabaseFor({ smsRows = [], cdRows = [], crRows = [] } = {}) {
  return {
    from(table) {
        ...
        return {
          select: () => ({
            eq: () => ({
              gte: () => ({
                order: () => ({
                  limit: () => Promise.resolve({ data: smsRows, error: null }),
                }),
              }),
            }),
          }),
...
describe('scanCorpus / detectCaptureMisses — TS-1 (capture miss) and TS-2 (candidate queue)', () => {
  ...
  test('an (a)+(b)-only item routes to the candidate queue, never the flag output', async () => {
    const supabase = fakeSupabaseFor({
      smsRows: [{ id: 's1', from_phone: CHAIRMAN_PHONE, body_raw: 'The chairman ratified moving forward with this direction.', signature_valid: true, received_at: new Date().toISOString() }],
    });
    const result = await detectCaptureMisses(supabase, 24);
