...
describe('detectRatificationRegression — contract coverage (opt-in, non-blocking)', () => {
  ...
  // SUPERSEDED by SD-LEO-FIX-MARKER-RESOLVER-WIRING-001: a stage2-only hit (no manifests passed
  // here, so stage1 is structurally false) is now VETOED by a checked-and-satisfied
  // contractCoverage. Rationale: a marker relocated within an already-satisfied multi-target
  // contract is not a real regression -- only an UNMEASURABLE or SHORT contractCoverage (or a
  // stage1 whole-section removal, see the stage1-always-wins test below) still regresses on a
  // stage2 hit. This title is kept (not renamed) to preserve the historical record of the
  // pre-fix assertion this SD intentionally reverses.
  it('coverage never rescues a regression — a covered row with a deleted marker still regresses [SUPERSEDED]', () => {
    const r = detectRatificationRegression(cleanRow, {
  ...
  // FR-3: the invariant FR-1's fix must never violate. Only stage2 may be vetoed by satisfied
  // coverage; stage1 (whole-section removal) always wins regardless. This deliberately supplies
  // matching newer/olderManifest with the section actually removed, so stage1 is genuinely true
  // here (not just "no manifests supplied" as in the stage2-only cases above) -- a naive fix that
  // gates stage1 on contractCoverageSatisfied too would wrongly report regressed:false here.
  it('stage1 (whole-section removal) always regresses, even with fully satisfied contract coverage', () => {
    const olderManifest = { byId: { '601': 'h1' }, meta: {}, global: 'g1' };
    const newerManifest = { byId: {}, meta: {}, global: 'g2' };
    const r = detectRatificationRegression(cleanRow, {
      newerManifest,
      olderManifest,
      liveFileContent: present.liveFileContent,
      contractCoverage: { checked: true, missing: [] },
    });
    expect(r.stage1).toBe(true);
