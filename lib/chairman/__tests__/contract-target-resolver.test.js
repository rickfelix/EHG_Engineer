...
const EXPECTED = {
  ...
  michael: ['CLAUDE_MICHAEL.md', 'CLAUDE_MICHAEL_MODEL_POSTURE.md'],
  protocol: [
    'CLAUDE.md',
    'CLAUDE_CORE.md',
...
describe('contract-target-resolver — declared file sets pinned by EQUALITY', () => {
  ...
  it('a scalar-returning resolver would FAIL these assertions (the falsifier)', () => {
    ...
    for (const contract of ['adam', 'coordinator', 'solomon', 'michael']) {
      expect(resolveContractTargets(contract, { repoRoot: REPO_ROOT }).length).toBeGreaterThan(1);
    }
    // 8 -> 11 under SD-LEO-FIX-CLAUDE-ADAM-SPLIT-001 (FR-3/FR-4): CLAUDE_EXEC_MANUAL.md,
    // CLAUDE_EXEC_PROVENANCE.md and CLAUDE_CORE_PROVENANCE.md joined the mapping's real file keys.
    expect(resolveContractTargets('protocol', { repoRoot: REPO_ROOT }).length).toBe(11);
  });
...
describe('contract-target-resolver — file-to-contract attribution', () => {
  it.each([
    ...
    ['CLAUDE_MICHAEL.md', 'michael'],
    ['CLAUDE_MICHAEL_MODEL_POSTURE.md', 'michael'],
    ['CLAUDE.md', 'protocol'],
