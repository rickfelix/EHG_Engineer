...
export async function markRatificationEncoded(supabase, ratificationId, { sectionId, manifestHash, encodedRef, markerText, repoRoot, driftProbe, deps } = {}) {
  ...
  // The full rendered contract file (up to ~80KB, e.g. CLAUDE_ADAM.md) lives on markerVerification
  // as `content` so the marker-offset/hash computation above and below has something to work
  // from — it must NEVER be persisted verbatim into `detail`, an INSERT-only, never-deletable
  // jsonb column, which would silently defeat file_sha256's whole purpose (a content-identity
  // hash existing so the raw bytes need not be stored). content is still passed by field to
  // recordVerificationAttempt's own contentRead param, which hashes it; only the hash is written.
  const { content: _rawContentOmittedFromDetail, ...detailForRecord } = markerVerification;
  ...
  if (!markerVerification.verified && markerVerification.reason !== 'not_applicable_for_ref_type') {
    // SECURITY (EXEC-TO-PLAN, evidence f4ae9adf) S2: do not assert unconditional persistence here
    // — recordVerificationAttempt (below, and in the success/refusal paths) degrades gracefully
    // and silently when chairman_ratification_verifications does not exist yet (measured true as
    // of this commit); the return value is the only guarantee, the sibling-table write is best-effort.
    console.warn(`MARKER_UNVERIFIED (${markerVerification.reason}): ratification ${ratificationId} is being marked encoded WITHOUT its marker having been checked against live content. This is recorded in the return value; an attempt is also made to persist it to the verification-verdict table (best-effort — see recordVerificationAttempt's own degrade-on-missing-table behavior).`);
  }
  ...
  if (refusalReason) {
    await recordVerificationAttempt(supabase, {
      ...
      reason: refusalReason, producer: PRODUCER, runId, detail: detailForRecord,
    });
  ...
  await recordVerificationAttempt(supabase, {
    ...
    producer: PRODUCER, runId, detail: detailForRecord,
  });
