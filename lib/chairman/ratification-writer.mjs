...
export function validateEncodedRefShape(ref) {
  ...
  const validator = ENCODED_REF_SHAPES[normalized.type];
  if (!validator) return { valid: false, reason: `unknown encoded_ref.type ${JSON.stringify(normalized.type)} — must be one of ${JSON.stringify(Object.keys(ENCODED_REF_SHAPES))}` };
