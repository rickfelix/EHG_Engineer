 ...
 */
function assertMarkerPresentInLiveSection(ref, markerText, { repoRoot = DEFAULT_REPO_ROOT } = {}) {
  let manifest;
  try {
 ...
 */
export async function markRatificationEncoded(supabase, ratificationId, { sectionId, manifestHash, encodedRef, markerText, repoRoot } = {}) {
  if (!ratificationId) {
  ...
  if (normalizedRef.type === 'section_id') {
    assertMarkerPresentInLiveSection(normalizedRef, trimmedMarker, repoRoot ? { repoRoot } : {});
  }
