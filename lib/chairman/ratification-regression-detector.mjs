 ...
 */
export function detectRatificationRegression(row, { newerManifest, olderManifest, liveFileContent, encodeTimeFileContent } = {}) {
  if (!row || !row.encoded_at) return { regressed: false, stage1: false, stage2: false, markerInvalid: false };
  ...
  return { regressed: (stage1 || stage2) && !markerInvalid, stage1, stage2, markerInvalid };
}
