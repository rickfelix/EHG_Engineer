...
function defaultSpawnEscalationEmail(decisionId) {
  ...
  // on Windows.
  const child = nodeSpawn('node', args, { detached: true, stdio: 'ignore', windowsHide: true });
 ...
 * @param {boolean} [opts.skipEscalation=false] - QF-20260830-670: record without auto-escalating,
 *   for callers that mint several decisions in one batch and escalate ONCE for the whole batch
 *   (see chairman-gated-decision-row-guard.mjs). `wouldEscalate` on the return value still reports
 *   whether shouldAutoEscalate() would have fired, so the caller knows which recorded rows are
 *   batch-escalation candidates.
 ...
 * @returns {Promise<{recorded: boolean, id?: string, error?: string, escalated?: boolean, wouldEscalate?: boolean}>}
 */
export async function recordPendingDecision(supabase, {
  ...
  //
  // QF-20260912-427: the same lookup also snapshots purchase-relevant venture fields into
  // brief_data.venture_snapshot, so a later STALE_CONTEXT refusal (fn_chairman_decide's
  // venture_updated_at > created_at check) can be diffed against what the chairman actually
  // saw, rather than blindly re-checked against "did anything at all move." Widening this
  // existing lookup's select() costs no extra round trip.
  let ventureSnapshot = null;
  if (ventureId && !allowFixture) {
    try {
      const { data: v, error: vErr } = await supabase
        .from('ventures')
        .select('id, name, is_demo, status, current_lifecycle_stage, deployment_url')
        .eq('id', ventureId).maybeSingle();
      ...
      if (!vErr && v) {
        ventureSnapshot = {
          ...
          current_lifecycle_stage: v.current_lifecycle_stage ?? null,
  ...
  // SD-LEO-INFRA-CHAIRMAN-DECIDE-MIGRATION-001 (FR-2): diagnostic-only mint-time probe against
  // the SAME registry fn_chairman_decide's closer reads (fn_chairman_decision_value) -- never
  // enforcement. A confirmed NULL means the type has no close mapping yet (the defect class this
  // SD fixes for migration_apply); the row is stamped and still recorded so it stays visible via
  // the normal decision queue rather than becoming unrecordable. A real try/catch (not a chained
  // .catch()) is required: calling .rpc on a client/mock that lacks it throws synchronously at
  // the call expression, before any promise exists.
  let decisionTypeUnmapped;
  try {
    const { data: mappedValue, error: mapErr } = await supabase.rpc('fn_chairman_decision_value', {
      p_decision_type: decisionType,
      p_action: 'approved',
    });
    if (!mapErr && mappedValue === null) {
      decisionTypeUnmapped = true;
      console.warn(`[record-pending-decision] decision_type "${decisionType}" has no close mapping in fn_chairman_decision_value -- recorded anyway (brief_data.decision_type_unmapped=true)`);
    }
  } catch { /* fail-open: an absent/erroring RPC never blocks a real decision */ }

  const briefData = {
    title,
    ...(recommendation ? { recommendation } : {}),
    ...(raisedBy ? { raised_by: raisedBy } : {}),
    ...(decisionTypeUnmapped ? { decision_type_unmapped: true } : {}),
    ...(ventureSnapshot ? { venture_snapshot: ventureSnapshot } : {}),
  ...
  const wouldEscalate = shouldAutoEscalate({ decisionType, blocking, raisedBy });
  let escalated = false;
  if (id && wouldEscalate && !skipEscalation) {
    const escOpts = {};
  ...
  return { recorded: true, id, escalated, wouldEscalate };
}
