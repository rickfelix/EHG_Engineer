...
// QF-20260829-902: the --decision <primaryId> override was written for a RACE (a row decided-or-
// created microseconds outside the pending query window) but had no clock and no status check --
// it emailed the named row regardless of how long ago it was decided (measured: a 17h-stale
// already-decided row reached the chairman). This is the PURE predicate; the caller (which has
// the DB row and the send/skip decision) logs the refusal loudly per Solomon's amendment.
const DEFAULT_PRIMARY_RACE_WINDOW_MS = 2 * 60_000; // 2 minutes -- tolerates the race the bypass names, refuses a genuinely stale decision.

/**
 * Decide whether a --decision <primaryId> row named OUTSIDE the pending set is still eligible to
 * be emailed: eligible when genuinely pending, or when it left 'pending' within raceWindowMs of
 * now (the race the bypass comment describes). Ineligible (stale) beyond that window.
 * @param {{status?: string, updated_at?: string}} row
 * @param {Date} [now]
 * @param {number} [raceWindowMs]
 * @returns {{eligible: boolean, reason: 'pending'|'race_window'|'stale', ageMs: number|null}}
 */
export function evaluatePrimaryDecisionEligibility(row, now = new Date(), raceWindowMs = DEFAULT_PRIMARY_RACE_WINDOW_MS) {
  if (!row) return { eligible: false, reason: 'stale', ageMs: null };
  if (row.status === 'pending') return { eligible: true, reason: 'pending', ageMs: 0 };
  const updatedAt = row.updated_at ? new Date(row.updated_at).getTime() : NaN;
  const ageMs = Number.isFinite(updatedAt) ? now.getTime() - updatedAt : null;
  if (ageMs != null && ageMs >= 0 && ageMs <= raceWindowMs) return { eligible: true, reason: 'race_window', ageMs };
  return { eligible: false, reason: 'stale', ageMs };
}
