...
export function classifyItem(evidence) {
  ...
  if (!agree) {
    ...
    const surplusUnattributable = !hasApproval && e.live.surplus === true;
...
export function checkManifest(manifest, populationIds, armList) {
  const ids = new Set(populationIds || []);
  const missing = (manifest || []).filter((m) => !ids.has(m.identifier));
  const armsSeeded = new Set((manifest || []).map((m) => m.source_arm));
  const unseededArms = (armList || []).filter((a) => !armsSeeded.has(a));
  return { ok: missing.length === 0 && unseededArms.length === 0, missing, unseededArms };
}
