...
export const EXCLUDED_KEYS = Object.freeze({
  ...
  // NOT an exclusion on the merits - see KNOWN_SCOPE_GAPS below.
  gated_ddl: 'REAL DDL GATE, excluded only because it is unreachable: it sits on a PRD and buildPopulation reads strategic_directives_v2 metadata only. Recorded in KNOWN_SCOPE_GAPS rather than dismissed.',
});
...
export const KNOWN_SCOPE_GAPS = Object.freeze({
  gated_ddl: {
    where: 'product_requirements_v2 metadata (1 row: PRD-SD-LEO-INFRA-LAUNCH-MODE-POLICY-002)',
    why_unreachable: 'buildPopulation reads strategic_directives_v2 metadata only. A gate recorded on a PRD is the same gate on a different row type, and no arm can see it.',
    to_close: 'extend buildPopulation to accept PRD rows as a source, the way quick_fixes and feedback already are. The detector now SEES these (it scans all three tables); the POPULATION still cannot admit them.',
  },
});
 ...
 */
export function findUnconsumedKeys(rows, populationArms, excluded = EXCLUDED_KEYS) {
  const arms = new Set(populationArms || []);
  const inPopulation = new Set();
  for (const r of rows || []) {
    const md = (r && r.metadata) || {};
    for (const a of arms) {
      if (Object.prototype.hasOwnProperty.call(md, a)) { inPopulation.add(r.identifier); break; }
    }
  }
  const byKey = new Map();
  for (const r of rows || []) {
  ...
  return { ok: !findings.some((f) => f.soleReach > 0), findings };
}
