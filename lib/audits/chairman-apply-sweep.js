 ...
 */
export const UNCONSUMED_KEY_CANDIDATE = /chairman/i;
export const UNCONSUMED_KEY_QUALIFIER = /(apply|gate|authoriz|ddl|migration|approv)/i;
/** Keys that qualify without naming a chairman — an apply gate need not say who holds it. */
export const UNCONSUMED_KEY_EXTRA = /^(may_require_ddl|apply_to_prod_requires_user_go)$/i;
...
export function isUnconsumedKeyCandidate(key) {
  ...
  if (UNCONSUMED_KEY_EXTRA.test(key)) return true;
  return UNCONSUMED_KEY_CANDIDATE.test(key) && UNCONSUMED_KEY_QUALIFIER.test(key);
}

/**
 * @param rows            [{ identifier, metadata }] — every SD, not only current members
 * @param populationArms  the arms currently consumed
 * @returns { ok, findings } where a finding is { key, members, soleReach } and soleReach counts
...
export function findUnconsumedKeys(rows, populationArms, excluded = EXCLUDED_KEYS) {
  ...
  for (const r of rows || []) {
    ...
    for (const key of Object.keys(md)) {
      ...
      if (!byKey.has(key)) byKey.set(key, { key, members: 0, soleReach: 0 });
