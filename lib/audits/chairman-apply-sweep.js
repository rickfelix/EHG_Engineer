...
export const EXCLUDED_KEYS = Object.freeze({
  ...
  chairman_gated_items: 'genuinely chairman-gated, but the items are LEGAL/BUSINESS actions (LLC formation, live Stripe account, live-mode payment key provisioning) - not database applies. In scope for a chairman-gate audit generally; OUT of scope for THIS one, which audits what was applied to the database.',
 ...
 */
/**
 * DERIVED, never hardcoded. The previous version was a static object asserting that PRD- and
 * feedback-borne gates were unreachable — which stopped being true the moment buildPopulation read
 * every metadata source, so the run printed "0 unreachable members" and a scope-gap warning seven
 * lines apart. I had also described it as derived when it was not. A stale warning is worse than no
 * warning: it spends the reader's attention on a gap that has closed while implying the check is live.
 */
export function deriveScopeGaps(unreachableMembers) {
  const bySource = {};
  for (const u of unreachableMembers || []) {
    (bySource[u.source] = bySource[u.source] || []).push(u.identifier);
  }
  return Object.entries(bySource).map(([source, identifiers]) => ({
    source,
    count: identifiers.length,
    identifiers,
    why_unreachable: 'carries a consumed arm key but no arm reaches it - the population and the detector disagree about which arms are consumed',
  }));
}
...
export function findUnconsumedKeys(rows, populationArms, excluded = EXCLUDED_KEYS, populationIds = null) {
  ...
  // Seeded from the REAL population: `known` was used only to DEMOTE, never to promote, so a
  // member reached solely by a free-text arm never entered inPopulation and scored sole-reach
  // against a population that does contain it.
  const inPopulation = new Set(known || []);
...
export function approvalTextOf(metadata) {
  ...
  for (const [key, val] of Object.entries(metadata)) {
    ...
    // MATCH ANY ESCAPE FORM, never a list of them. The first version enumerated the eight short
    // escapes and missed \uXXXX -- the only OTHER form JSON.stringify emits (control characters
    // outside \b\t\n\f\r, and lone surrogates left by truncated emoji). A backslash is not in the
    // artifact character class, so the match restarted one character later and the surviving
    // uXXXX FUSED onto the real name: "db/real.sql" became "ud83ddb/real.sql", artifact.present
    // true, pointing at a file that has never existed. That is the same fabrication the comment
    // below already claimed to have closed -- closed for 8 of 9 cases, which is exactly what
    // enumerating buys you instead of covering. Found by adversarial review, reproduced live.
    ...
    const text = typeof val === 'string' ? val : JSON.stringify(val).replace(/\\(u[0-9a-fA-F]{4}|.)/g, ' ');
    out += ' ' + text;
  }
  return out;
}
