 ...
 *
 * buildPopulation EMITS these. NOTHING switches on them any more - buildEvidence was rewritten
 * to have no branch, and the producer stamps evidenceText instead - so these now LABEL report
 * rows rather than remove a seam. Kept because a label that drifts is still a defect.
 * Historically: Previously each side wrote the
...
// The character before `.sql` must be a word character or hyphen - never a dot. With `.` inside
// the class the pattern runs through an elision: "20260722_DOWN_...sql" matched as a path to a file
// that has never existed, while the two REAL migrations named in the same sentence were skipped,
// because .match() returns the first hit. Measured against the live population: exactly 1 row
// changes, 0 rows lose an artifact, the other 36 paths are byte-identical, and on-disk resolution
// goes 36/37 -> 37/37. Same extraction, same failure shape, as the escape-fusion bug fixed in the
// identifier half - fabricating a reference that a future prober will report as MISSING.
export const SQL_ARTIFACT_RE = /[\w./-]*[\w-]\.sql/i;
...
export function buildPopulation(sds, quickFixes, metadataArms, extraMetadataRows = []) {
  ...
  const metadataRows = [
    // sd_key is UNIQUE but NULLABLE and Postgres permits unlimited NULLs, so two null-keyed
    // SDs would MERGE into one row with concatenated arms and the second status discarded.
    // No control can see it: they all derive the same undefined identifier.
    ...(sds || []).map((sd) => ({ identifier: sd.sd_key || sd.id, metadata: sd.metadata, status: sd.status, source: SOURCE.SD })),
    ...(extraMetadataRows || []),
  ];
...
export function addCompletionFlagArm(population, feedbackRows) {
  ...
  for (const row of feedbackRows || []) {
    ...
    if (existing) {
      // MERGE, never skip. A row can be a metadata member AND a completion flag; skipping meant its
      // free text was never read, which is where this class carries its artifact path. Both arms
      // are recorded, and the evidence text is the concatenation so neither half is lost.
      if (!existing.arms.includes('completion_flag_index')) {
        existing.arms.push('completion_flag_index');
        existing.dispositions.push('prose');
      }
      const extra = `${row.title || ''} ${row.description || ''}`;
      existing.freeText = `${existing.freeText || ''} ${extra}`.trim();
      existing.evidenceText = `${existing.evidenceText || ''} ${extra}`.trim();
      continue;
    }
    byId.set(key, {
...
export function buildEvidence(item) {
  ...
  const approvalText = (item && 'evidenceText' in item)
    ? item.evidenceText
    : ((item && item.freeText) || approvalTextOf(item && item.metadata));
