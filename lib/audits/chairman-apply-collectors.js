...
import { membershipOf, approvalTextOf, namesObjects } from './chairman-apply-sweep.js';
...
export function buildPopulation(sds, quickFixes, metadataArms) {
  const byId = new Map();
  for (const sd of sds || []) {
    for (const arm of metadataArms || []) {
      ...
      if (!byId.has(sd.sd_key)) {
        byId.set(sd.sd_key, {
          identifier: sd.sd_key, source: SOURCE.SD,
...
export function buildEvidence(item) {
  ...
  const approvalText = source === SOURCE.SD
    ? approvalTextOf(item.metadata)
    : ((item && item.freeText) || '');
