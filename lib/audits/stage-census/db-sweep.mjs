...
export async function sweepJsonbMetadataPaths(client) {
  const { rows } = await client.query(
    `SELECT attempt_id AS id, 'eva_stage_gate_attempts' AS table_name
     ...
     WHERE metadata::text ~ 'stage[-_]?${RANGE}'`
  );
  return rows;
}

/** pg_proc function bodies mentioning "stage" and an in-range literal. */
...
/** views/matviews whose definition mentions "stage" and an in-range literal. */
export async function sweepViewsAndMatviews(client) {
  const { rows } = await client.query(
    `SELECT viewname AS name, 'view' AS kind FROM pg_views
       WHERE schemaname = 'public' AND definition ~* 'stage' AND definition ~ '${RANGE}'
     UNION ALL
