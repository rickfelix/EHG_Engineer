...
export const VDR_REGISTRY = [
  ...
  // SD-LEO-FEAT-REALIZE-DISTANCE-BROKE-001: PROBE FLIP. Census found distanceToBroke() was
  // already computed AND rendered live in ehg's SurvivabilityCockpit from the live
  // operator_cash_burn_monthly substrate (EHG_Engineer-owned table, written by
  // lib/operator/cash-burn-substrate.js) -- the prior code_grep probe was permanently capped at
  // partial(0.5) regardless of realization, which is why it read 0.5 despite the capability
  // being fully built. This probe now mirrors the render's OWN data dependency: cashLive in
  // distanceToBroke() requires cash_last_synced_at within a 31-day window (ehg's
  // DTB_LIVENESS_WINDOWS_MS.cash), so a recent row here is direct evidence the render can
  // actually show a measurable runway, not merely that the code exists.
  { capability: 'See distance-to-broke', layer: 'application',
    probe: { type: 'db_count', table: 'operator_cash_burn_monthly', filter: { cash_last_synced_at: { gteDaysAgo: 45 } }, min: 1 } },
  { capability: 'Venture-performance read', layer: 'application',
    // SD-LEO-FEAT-REALIZE-VENTURE-PERFORMANCE-001: PROBE FLIP. The prior code_grep pattern
    // (venturePerformance|tile-venture-performance|VenturePerformance) also matched an unrelated
    // same-named function in ehg's survivability-logic.ts/SurvivabilityCockpit.tsx (a different
    // cockpit tile, name collision only) and was permanently capped at partial(0.5) regardless of
    // realization. The real capability is PerformanceGauge.tsx (mounted by BriefingDashboard,
    // routed at DailyBriefingPage.tsx — verified live), fed by useVenturePerformance's live query
    // against ventures.health_score. Flip to db_count reading that same substrate directly
    // (mirrors the distance-to-broke pattern): a non-null health_score is what the render needs
    // to show a real band instead of an honest dash.
    probe: { type: 'db_count', table: 'ventures', filter: { health_score: { not: null } }, min: 1 } },
  ...
  // SD-LEO-INFRA-REALIZE-GATE-CALIBRATION-001: PROBE FLIP. The prior code_grep probe matched
  // lib/discovery/opportunity-discovery-service.js's calibration_cohort stamp -- code
  // presence, capped at partial(0.5) by the anti-inflation rule, forever (a grep can never
  // observe realization). Census confirmed this cohort had ZERO consumers anywhere in the
  // codebase. lib/discovery/calibration-cohort-reader.js is now the named live reader
  // (SD-LEO-INFRA-REALIZE-GATE-CALIBRATION-001 FR-2); it stamps metadata.calibration_read_at
  // on each row it consumes. This probe now reads THAT realized signal -- honest-low
  // (unbuilt/0) if the cohort is empty or has never been read, built once >=1 row has been.
  // SD-LEO-INFRA-SEED-OPPORTUNITY-BLUEPRINTS-001 (FR-5, TESTING finding T-8, evidence
  // 341b6080): the probe's own comment above already warns that "a single stray/seed row
  // must NOT credit built for a capability the vision describes as fed by a real cohort" --
  // seeding exactly the prior min:1 threshold would reach that same inflation from the other
  // end. Raised to min:3 (matching this SD's own >=3 seed floor) and added is_active:true so
  // a future archive event on these seeded rows can never silently re-flip this green.
  { capability: 'Calibrate the gates', layer: 'process',
    probe: { type: 'db_count', table: 'opportunity_blueprints', filter: { 'metadata->>calibration_read_at': { not: null }, is_active: true }, min: 3 } },
  ...
  { capability: 'Cross-stage data contracts', layer: 'process',
    // SD-LEO-INFRA-PHASE-DESIGN-CROSS-001 (FR-2/FR-3): repoint OFF the code_grep call-site probe
    // (permanently capped at partial(0.5) by the anti-inflation rule — a grep can never observe
    // realization) onto the SD-REFILL-00V67JEB telemetry signal, audit_log event_type='contract_coverage',
    // which stamps metadata.valid on each real cross-stage contract check. Fresh EXEC-time measurement
    // (trailing 30 days, 2026-08-31T20:02:34.773Z): total=11525, valid=11474, ratio=0.995575 — within
    // 0.02 of the LEAD-phase baseline (0.9956), so no discrepancy branch. builtAt = round(0.995575 -
    // 0.005, 3) = 0.991 per the deterministic headroom rule. numerFilter is denomFilter plus exactly one
    // key ('metadata->>valid': 'true') — numerator and denominator provably span the same population.
    probe: { type: 'count_ratio', table: 'audit_log', builtAt: 0.991,
      denomFilter: { event_type: 'contract_coverage', created_at: { gteDaysAgo: 30 } },
      numerFilter: { event_type: 'contract_coverage', created_at: { gteDaysAgo: 30 }, 'metadata->>valid': 'true' } } },
  ...
  { capability: 'Decision Filter Engine', layer: 'process',
    // SD-LEO-INFRA-PHASE-DESIGN-DECISION-001: REPOINT from code_grep (presence-only, permanently
    // capped at partial) to advisory-execution coverage telemetry. SD-LEO-INFRA-DFE-CHAIRMAN-FORWARD-
    // GATE-001 wired the engine as an ADVISORY forward filter (lib/eva/forward-gate.js) that runs over
    // each chairman decision and records a chairman_forward_gate_score row to audit_log via
    // recordForwardGateScore — that's a real, measurable realization signal a db_count probe can read
    // honestly. Fresh EXEC-time measurement (2026-08-31T21:17:23.398Z): 3,748 chairman_forward_gate_score
    // rows in the trailing 90-day window (== 3,748 all-time; all rows fall within 90 days). A 30-day
    // ratio probe (the sibling SD's pattern) is unsuitable here: only 13 rows in the trailing 30 days,
    // a degenerate 13/13-true sample with no variance. min=500 is the floor: well below the live 3,748
    // (headroom for natural volume decline) yet requires ~12 days of average volume (3,748/90≈42/day),
    // so a single burst day can never alone credit 'built'.
    //
    // DEFERRED (out of this SD's authority): a deeper accuracy gate over 'would_hold' shadow verdicts
    // (named as a future stage-2 in the enforce-mode SD) needs enforce-canary mode data to compute
    // accuracy from. Measured at the same timestamp above: enforce-canary mode has NEVER been enabled —
    // 0 of 3,748 rows carry metadata.mode='enforce-canary' or would_hold data. Enabling canary mode is a
    // decision-relevant lever change that is chairman-ordered only per this session's own house rule;
    // this comment documents the gap, it does not build the accuracy gate.
    probe: { type: 'db_count', table: 'audit_log',
      filter: { event_type: 'chairman_forward_gate_score', created_at: { gteDaysAgo: 90 } }, min: 500 } },
