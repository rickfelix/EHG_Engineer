...
/**
 * Shared filter-value applicator (SD-LEO-INFRA-REALIZE-GATE-CALIBRATION-001 FR-1): originally
 * countRatioProbe-only, now shared with dbCountProbe so any probe can express the same richer
 * filter shapes, not just flat scalar .eq(). Per column, accepts:
 *   - scalar value          → .eq(col, v)
 *   - array of values       → .in(col, v)
 *   - { not: null }         → .not(col, 'is', null)
 *   - { ne: v }              → .neq(col, v)
 *   - { gteDaysAgo: N }      → .gte(col, now − N days)   (nowMs defaults to Date.now())
 * Byte-identical to the pre-extraction countRatioProbe behavior for every existing shape.
 */
export function applyFilter(q, filter, nowMs = Date.now()) {
...
export async function dbCountProbe(def, io) {
  ...
  try {
    ...
    q = applyFilter(q, def.filter, io && io.nowMs);
