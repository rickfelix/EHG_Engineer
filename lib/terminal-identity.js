  ...
  try {
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      if (proc.name === 'node.exe' || proc.name === 'node') {
        ...
        // or a terminal host — not another node, bash, or intermediary launcher.
        // npx/dotenvx can wrap scripts, adding extra node.exe to the chain.
        if (!parent || !['node.exe', 'node', 'bash.exe', 'bash', 'sh.exe', 'sh', 'powershell.exe', 'pwsh.exe', 'npx', 'npx.exe', 'npx.cmd'].includes(parent.name)) {
          return proc.pid;
        }
      }
    }
