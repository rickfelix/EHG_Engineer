...
export async function createFromPlan(planPath = null, skipConfirmation = false, overrides = {}) {
  ...
  if (parsed.successCriteria && parsed.successCriteria.length > 0) {
    ...
    if (successCriteria.length === 0) {
      // SD-FDBK-ENH-MINT-PIPELINE-WRITES-001 (FR-2): the plan has NEITHER a real Success
      // Criteria section NOR a Steps section to derive from — refuse the mint loudly
      // instead of silently substituting a generic 3-line template (the root cause this
      // SD exists to close). Matches this file's sanctioned exit-to-return convention.
      console.error('\n❌ Cannot derive success_criteria: plan file has no \'## Success Criteria\' (or \'## Acceptance\') section and no derivable implementation steps.');
      console.error(`   Plan: ${targetPath}`);
      console.error('   Add a \'## Success Criteria\' section (numbered or bulleted) to the plan, then re-run.');
      return { ok: false, error: `Plan has no derivable success_criteria: ${targetPath}`, exitCode: 1 };
    }
  }
  ...
  const createOptions = {
    ...
    // SD-LEO-FIX-AUTHORING-TIME-EVIDENCE-001: author-typed plan prose, unlike system-
    // generated provenance strings from other lanes, is a legitimate surface for the
    // uuid-owner check.
    ...
    metadata: {
      ...
      // QF-20260819-415: mirror feedback.js:148 (QF-20260818-873) -- --from-plan never got
      // the GR-DELETION-SAFEGUARD pair the --from-feedback path already has.
      ...(overrides.backupPlan ? { backup_plan: true } : {}),
      ...(overrides.deletionApproved ? { deletion_approved: true } : {}),
