...
// SD-FDBK-ENH-MINT-PIPELINE-WRITES-001 (FR-3): reuse the plan adapter's structured extractor
// against the feedback body instead of always falling to the generic template.
import { extractSuccessCriteria } from '../../../scripts/modules/plan-parser.js';

/**
 * SD-FDBK-ENH-MINT-PIPELINE-WRITES-001 (FR-3): derive success_criteria from the feedback
 * body when structured; default WARN-and-proceed (not refuse) since most feedback is
 * unstructured prose — refuse-by-default broke routine automated ingestion (measured
 * against this codebase's own existing feedback-adapter test suite). --strict-criteria
 * opts INTO the harder refusal for supervised use.
 *
 * @returns {{criteria: string[]|null, refuse: false}|{refuse: true, error: string}}
 */
function deriveFeedbackSuccessCriteria(feedback, options) {
  const extracted = extractSuccessCriteria(feedback.description);
  if (extracted && extracted.length > 0) {
    return { criteria: extracted.map(c => c.criterion), refuse: false };
  }
  if (options.strictCriteria) {
    console.error('\n❌ Cannot derive success_criteria from feedback body: no structured Success/Acceptance Criteria list found.');
    console.error(`   Feedback: ${feedback.id}`);
    console.error('   Add a structured criteria list to the feedback, or omit --strict-criteria to proceed with a generic template.');
    return { refuse: true, error: `Feedback has no derivable success_criteria: ${feedback.id}` };
  }
  console.warn(`   ⚠️  success_criteria: no structured list found in feedback body — falling back to a generic template (feedback ${feedback.id}). Pass --strict-criteria to refuse instead.`);
  return { criteria: null, refuse: false };
}
...
export async function createFromFeedback(feedbackId, options = {}) {
  ...
  // QF-20260817-550: targetRepos was accepted for --from-plan/--child/--from-roadmap-item
  // (plan.js:300) but never destructured here, so a caller-supplied --target-repos was
  // silently swallowed on this route with no error/warning — the governed cross-repo
  // metadata PR_MERGE_VERIFICATION relies on (computeReposForSD) never landed.
  // QF-20260818-873: backupPlan/deletionApproved close the GR-DELETION-SAFEGUARD CLI
  // sibling-parity gap against migrationReviewed/securityReviewed above.
  const { migrationReviewed = false, securityReviewed = false, backupPlan = false, deletionApproved = false, targetRepos = null } = options;
  ...
  const sc = deriveFeedbackSuccessCriteria(feedback, options);
  if (sc.refuse) return { ok: false, error: sc.error, exitCode: 1 };
  ...
  const sd = await createSD({
    ...
    metadata: {
      ...
      // QF-20260818-873: propagate GR-DELETION-SAFEGUARD attestation flags
      ...(backupPlan ? { backup_plan: true } : {}),
      ...(deletionApproved ? { deletion_approved: true } : {}),
      // QF-20260817-550: mirrors plan.js:300 — wire targetRepos through instead of
