...
import { isVentureRepo } from '../../repo-paths.js';
// SD-LEO-INFRA-SINGLE-ESCALATION-WRITER-001: single canonical quick_fixes.status writer.
import { createRequire } from 'module';
const require = createRequire(import.meta.url);
const { setQuickFixStatus } = require('../../quick-fix/status-writer.cjs');
// SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-E (Child E): stamp provenance on the born-claim below.
// sdKey here is a newly minted SD key (never a QF id), so this always takes claim-stamp.cjs's
// SD-side path — no migration dependency.
const { stampClaim } = require('../../fleet/claim-stamp.cjs');
...
// `character varying(500)` — a QF whose own title runs past that (measured here: some
// Adam-seat-minted QFs fold the entire one-sentence finding into the title field, not just a
// short summary) makes createSD's INSERT fail outright with "value too long for type character
// varying(500)", losing the escalation entirely rather than degrading. Bounded the same way
// composeEscalatedDescription bounds the description above: truncate, never fabricate, and
// keep the full original title in metadata (qfOriginBody below) so nothing is discarded.
//
// This same defect was independently found and fixed twice in parallel (QF-20260912-186,
// merged as this function, and separately as part of escalating QF-20260912-959 in this SD's
// own branch, as composeEscalatedTitle()/MAX_TITLE_CHARS) -- converged here on main's version,
// which lands exactly at 500 chars by construction (slice length = SD_TITLE_MAX_CHARS -
// suffix.length) and ships with dedicated coverage in
// tests/unit/leo-create-sd-from-qf-title-length.test.js. The other implementation is dropped
// as a pure duplicate, not a functional loss.
const SD_TITLE_MAX_CHARS = 500;
const TITLE_TRUNCATION_SUFFIX = '… [truncated, see metadata]';
export function truncateTitle(title) {
  const t = title || '';
  if (t.length <= SD_TITLE_MAX_CHARS) return t;
  return `${t.slice(0, SD_TITLE_MAX_CHARS - TITLE_TRUNCATION_SUFFIX.length)}${TITLE_TRUNCATION_SUFFIX}`;
}
...
/**
 * QF-20260902-866: createSD() never received a success_criteria argument from this adapter,
 * so every escalated SD fell through to pipeline.js's buildDefaultSuccessCriteria() -- the
 * generic 3-item type-agnostic template ("Code passes lint and type checks", ...), never the
 * QF's own measured premise. Witnessed live: 4 SDs minted 2026-09-02 12:32Z all carried the
 * template, and the coordinator had to hand-stamp a note that LEAD must re-key from the source
 * QF before LEAD-TO-PLAN.
 *
 * Builds success_criteria FROM the QF's own measured fields (expected/actual behavior, steps
 * to reproduce) -- never fabricated, never the generic template. Falls back to the title only
 * when the QF genuinely has no measured behavior fields at all.
 *
 * SD-LEO-FIX-ESCALATION-COPIES-EXPECTED-001 (escalated from QF-20260906-362): the
 * expected_behavior-derived entry is stamped with an `origin` provenance object
 * ({producer, source_qf_id, content_hash}) so origin-criterion-gate.js can detect it being
 * silently deleted or reworded during PLAN/EXEC. It must be built as a full OBJECT (not a
 * plain string like the other two possible entries) — validate-sd-fields.js's
 * autoEnrichStructure() coerces a plain-string entry into a BRAND-NEW {criterion, measure}
 * object with no room for extra keys, which would silently drop the origin stamp. An object
 * already carrying a `criterion` key survives that enrichment untouched.
 *
 * @param {{title?:string, expected_behavior?:string, actual_behavior?:string, steps_to_reproduce?:string}} qf
 * @returns {Array<string|object>}
 */
export function buildMeasuredSuccessCriteria(qf) {
  const criteria = [];
  if (qf.expected_behavior) {
    const criterion = `Expected behavior achieved: ${qf.expected_behavior}`;
    criteria.push({
      criterion,
      ...
      origin: {
        ...
        source_qf_id: qf.id,
        content_hash: createHash('sha256').update(qf.expected_behavior).digest('hex'),
      },
    });
  }
  if (qf.actual_behavior) {
    criteria.push(`Defective behavior no longer occurs: ${qf.actual_behavior}`);
  }
  if (qf.steps_to_reproduce) {
    criteria.push(`Reproduction steps no longer trigger the defect: ${qf.steps_to_reproduce}`);
  }
  if (criteria.length === 0) {
    // No measured behavior fields at all -- the title is still the QF's own content,
    // not a generated template.
    criteria.push(`Resolves: ${qf.title || 'the escalated quick-fix'}`);
  }
  return criteria;
}

/**
 * SD-LEO-INFRA-CLOSE-REMAINING-CROSS-001-B (FR-1): resolve the createSD() spread for an
 * explicit target_application. Pure and dependency-free so it is unit-testable without
 * mocking createFromQF's DB/pipeline surface (CLAUDE_EXEC.md "Testability-Aware
 * Implementation" -- separate pure logic from side effects). Gated on isVentureRepo(), not
 * mere truthiness, so a platform-targeted QF (the 36/39 majority, measured live) spreads
 * an empty object -- byte-identical to pre-fix behavior, since passing target_application
 * at all also flips pipeline.js's metadata.target_application_explicit stamp.
 *
 * @param {{target_application?: string}} qf
 * @returns {{target_application?: string}}
 */
export function resolveExplicitTargetApplication(qf) {
  return isVentureRepo(qf.target_application) ? { target_application: qf.target_application } : {};
}

/**
 * QF-20260911-484: resolve the --roadmap-link-reason for an SD escalated from a quick-fix.
 * A caller-supplied non-blank opts.roadmapLinkReason always wins (mirrors --from-plan's and
 * --child's identical wiring, plan.js/child.js). When omitted, --from-qf is the one lane where
 * a real default reason is ALWAYS available: the QF's own escalation_reason (its tier-3 routing
 * rationale, stamped at QF creation by create-quick-fix.js), so the register-first exception is
 * never stamped no-reason-supplied for this lane -- unlike --from-plan/--child, where a caller
 * genuinely may have no reason to give. Pure and dependency-free (mirrors
 * resolveExplicitTargetApplication above) so it is unit-testable without mocking createFromQF's
 * DB/pipeline surface.
 *
 * @param {{id: string, escalation_reason?: string|null}} qf
 * @param {{roadmapLinkReason?: string|null}} [opts]
 * @returns {string}
 */
export function resolveQfRoadmapLinkReason(qf, opts = {}) {
  if (typeof opts.roadmapLinkReason === 'string' && opts.roadmapLinkReason.trim()) {
    return opts.roadmapLinkReason;
  }
  const escalationReason = typeof qf.escalation_reason === 'string' && qf.escalation_reason.trim()
    ? qf.escalation_reason
    : 'no reason recorded on the quick-fix';
  return `escalated from ${qf.id}: ${escalationReason}`;
}
...
export async function createFromQF(qfId, opts = {}) {
  ...
  // SD-LEO-INFRA-CLOSE-REMAINING-CROSS-001-B (FR-1): pass target_application
  // explicitly ONLY when the QF's own target is a genuine venture repo. Without
  // this, pipeline.js's createSD falls back to getCurrentVenture() (cwd-keyed),
  // which mints the new SD as target_application='EHG_Engineer' for a
  // venture-targeted QF -- the true target survives only in
  // metadata.qf_target_application below, which nothing re-reads until FR-2.
  // Gated on isVentureRepo() (not mere truthiness) so a platform-targeted QF
  // (the 36/39 majority, measured live) passes NO explicit param and stays
  // byte-identical to pre-fix behavior -- an ungated pass would unconditionally
  // stamp metadata.target_application_explicit=true on every escalated SD,
  // silently disabling the LEAD-TO-PLAN auto-corrector for that majority
  // (validation-agent finding V-8, evidence 41b9d501).
  const sd = await createSD({
    sdKey,
    title: truncateTitle(qf.title),
    description: composeEscalatedDescription(qf),
    type,
    priority,
    success_criteria: buildMeasuredSuccessCriteria(qf),
    ...resolveExplicitTargetApplication(qf),
    // QF-20260911-484: TOP-LEVEL createSD param (pipeline.js destructures it there, not under
    // metadata) -- mirrors plan.js's/child.js's identical roadmap_link_reason wiring, but always
    // set (never omitted) since a real default is always available from the QF itself.
    roadmap_link_reason: resolveQfRoadmapLinkReason(qf, opts),
    // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 (FR-2): criticality verdict, threaded from the
    // --from-qf CLI flags via opts.
    criticality: opts.criticality || null,
    criticalityReason: opts.criticalityReason || null,
    ...
    metadata: {
      ...
      ...(opts.backupPlan ? { backup_plan: true } : {}),
      ...(opts.deletionApproved ? { deletion_approved: true } : {}),
  ...
  try {
    await withRetry(async () => {
      // SD-LEO-INFRA-SINGLE-ESCALATION-WRITER-001: routes via the single canonical writer.
    ...
    try {
      ...
      if (sessErr) {
        ...
        if (claimErr || (claimRes && claimRes.success === false)) {
          ...
          try {
            await stampClaim(supabase, sdKey, qfSession, 'env');
          } catch { /* fail-soft: a provenance-stamp hiccup must never break the born-claim itself */ }
        }
      }
    } catch (claimEx) {
