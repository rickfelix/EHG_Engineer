...
// strategic_directives_v2.title is varchar(500) — a QF's own title has no such bound
// (create-quick-fix.js writes it as free text, same unbounded-narrative shape that made
// composeEscalatedDescription() necessary above) and this path passed it through verbatim,
// so any QF whose title alone runs past 500 chars failed SD creation outright with a raw
// Postgres "value too long" error (found live escalating QF-20260912-959, whose title is
// ~700 chars). Same truncate-with-marker treatment as the description; nothing is
// discarded — see qfOriginBody().
// MAX_TITLE_CHARS is set so the truncated output (title slice + the 51-char
// marker text below) always stays under the 500-char column limit -- a bare
// "truncate to 500" would still overflow once the marker is appended.
const MAX_TITLE_CHARS = 440;
function composeEscalatedTitle(qf) {
  const title = qf.title || 'Escalated quick-fix';
  return title.length > MAX_TITLE_CHARS
    ? `${title.slice(0, MAX_TITLE_CHARS)}… [truncated; full text in metadata.qf_origin_body]`
    : title;
}
...
export async function createFromQF(qfId, opts = {}) {
  ...
  const sd = await createSD({
    sdKey,
    title: composeEscalatedTitle(qf),
