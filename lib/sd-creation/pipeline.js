...
import { resolveArtifactOwnersInFields, annotateOwnerInText } from './authoring-evidence-resolver.js';
import { validateSDFields } from '../../scripts/modules/validate-sd-fields.js';
// SD-LEO-INFRA-SHIFT-LEFT-PRD-001 (FR-1): reuse the existing (previously unwired) authoring-mode
// contract check rather than reimplementing metrics-sufficiency/boilerplate detection.
import { validateArtifact } from '../artifact-contracts/index.js';
...
import { resolveCriticalityVerdict, routeCriticalityLater } from '../governance/criticality-verdict.js';
...
async function createSD(options) {
  const {
    ...
    // SD-LEO-FIX-AUTHORING-TIME-EVIDENCE-001: opt-in only. createSD is the SHARED
    // convergence point for every creation lane (plan, roadmap-item, feedback, child,
    // proposal, direct) -- most lanes' description/rationale carry SYSTEM-GENERATED
    // provenance uuids (e.g. "Promoted from roadmap_wave_items <id>", a copied feedback
    // body) that are not author-cited fixture references and are not on this check's
    // table allowlist. TESTING measured (evidence 20c3546f-c048-42d8-b88c-60657cfddd73)
    // that applying the check unconditionally refuses 100% of --from-roadmap-item mints
    // and ~7% of --from-feedback mints -- lanes this SD never named. Only the two named
    // call sites (plan.js's createFromPlan, child.js's createChild) set this true.
    checkArtifactOwners = false,
    // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 (FR-2): criticality verdict, same contract as
    // FR-1's QF gate. 'later' short-circuits before any strategic_directives_v2 insert.
    criticality = null,
    criticalityReason = null,
  } = options;

  // QF-20260821-313: fail fast here, not 250 lines downstream at a raw Postgres NOT NULL
  // violation on strategic_directives_v2.sd_key. createSD never generates its own key --
  // every caller must call generateSDKey({source,type,title}) first (mirror feedback.js).
  // Returns (not throws): this function's exit->return contract means createSDOrThrow
  // (the historical throwing shim scripts/leo-create-sd.js re-exports as createSD, used by
  // scripts/corrective-triage.mjs and other importers) converts {ok:false} into a thrown
  // Error, and exitFromResult (scripts/leo-create-sd.js) maps {ok:false} to a process.exit
  // via res.exitCode -- both paths already handle this shape correctly.
  if (!sdKey) {
    return {
      ok: false,
      error: 'createSD requires sdKey — generate via generateSDKey({source,type,title}) at the call site',
      code: 'SDKEY_REQUIRED',
      exitCode: 1,
    };
  }

  // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 (FR-2): criticality verdict gate, warn-first via
  // SD_CRITICALITY_GATE_ENFORCE (leo_feature_flags). Same fail-safe read pattern as FR-1.
  let sdCriticalityGateEnabled = false;
  try {
    // SECURITY SEC-1 (EXEC-TO-PLAN): see the identical fix on FR-1's gate in
    // scripts/create-quick-fix.js -- supabase-js does not throw on a PostgREST error, so a
    // query error must be distinguished from a genuine absent/disabled row rather than
    // silently defaulting with zero log output.
    const { data: flagRow, error: flagQueryErr } = await supabase
      .from('leo_feature_flags')
      .select('is_enabled')
      .eq('flag_key', 'SD_CRITICALITY_GATE_ENFORCE')
      .maybeSingle();
    if (flagQueryErr) {
      console.warn(`[createSD] SD_CRITICALITY_GATE_ENFORCE flag query error (defaulting to not enabled): ${flagQueryErr.message}`);
    }
    sdCriticalityGateEnabled = flagRow?.is_enabled === true;
  } catch (flagErr) {
    console.warn(`[createSD] SD_CRITICALITY_GATE_ENFORCE flag read threw (defaulting to not enabled): ${flagErr.message}`);
  }
  const criticalityVerdict = resolveCriticalityVerdict({ criticality, flagEnabled: sdCriticalityGateEnabled });
  if (criticalityVerdict.verdict === 'refuse') {
    return {
      ok: false,
      error: `createSD refused: ${criticalityVerdict.message}`,
      code: 'CRITICALITY_REQUIRED',
      exitCode: 1,
    };
  }
  if (criticalityVerdict.verdict === 'warn_and_file') {
    console.warn(`[createSD] [CRITICALITY_GATE] ${criticalityVerdict.message}`);
  }
  if (criticalityVerdict.verdict === 'route_later') {
    // SECURITY SEC-3 (EXEC-TO-PLAN): sdKey is already generated and guaranteed unique
    // (SDKEY_REQUIRED fail-fast above), so it's a stronger dedup identity than description
    // text -- two different sdKeys never collapse into one harness_backlog row.
    await routeCriticalityLater({
      supabase,
      title: title || sdKey,
      description: description || rationale || '(no description supplied)',
      criticalityReason,
      loggedVia: 'leo-create-sd.js',
      dedupKey: `criticality-later::${sdKey}`,
    });
    return { ok: true, done: true, routedLater: true };
  }
  const criticalitySdMetadata = criticalityVerdict.verdict === 'file_critical'
    ? { criticality: 'critical', criticality_reason: criticalityReason || '' }
    : {};
  ...
  // SD-LEO-INFRA-SHIFT-LEFT-PRD-001 (FR-1/FR-2/FR-3/FR-6): authoring-time shift-left check.
  // MUST run strictly AFTER validateSDFields's enrichment above (TR-2, load-bearing at 100% --
  // buildDefaultKeyChanges/buildDefaultSuccessCriteria emit string arrays that would violate
  // validateArtifact's arrayOfObjects shape check BEFORE enrichment converts them to objects;
  // measured 6 violations pre-enrichment, 0 post-enrichment on the same payload).
  //
  // FR-2 REVISED at PLAN (testing-agent evidence 28adeafe-e976-422a-82ec-30962c59ef1b):
  // createSD() carries ~87.3% of 30d SD creations, so this ships WARN-ONLY for an initial
  // observation window rather than hard-blocking -- promotion to a hard block is a follow-up
  // decision once TS-8 (string-shaped-input regression coverage) has run clean in production.
  // TESTING evidence 28adeafe-e976-422a-82ec-30962c59ef1b (F1, EXEC review): the catch below
  // previously left violation_count/warning_count at their 0/0 initializer, so a THROWING
  // validateArtifact stamped a row indistinguishable from a genuinely clean one -- defeating the
  // whole point of the warn-only observation window (a systematically-crashing check would look
  // like a 100% clean rollout). `errored` makes that failure mode visible in the same summary.
  let sdAuthoringValidation = { violation_count: 0, warning_count: 0 };
  try {
    const authoringCheck = validateArtifact('sd', sdData, { mode: 'authoring' });
    sdAuthoringValidation = {
      violation_count: authoringCheck.violations.length,
      warning_count: authoringCheck.warnings.length,
    };
    for (const v of authoringCheck.violations) {
      console.warn(`   ⚠️  [SD-AUTHORING] ${v.field}: expected ${v.expected}, got ${v.got}${v.hint ? ` (${v.hint})` : ''}`);
    }
    for (const w of authoringCheck.warnings) {
      console.warn(`   ⚠️  [SD-AUTHORING] ${w.field}: ${w.got} -- ${w.hint || w.expected}`);
    }
  } catch (authoringErr) {
    console.warn(`   ⚠️  SD-authoring validation skipped (non-blocking): ${authoringErr.message}`);
    sdAuthoringValidation = { violation_count: 0, warning_count: 0, errored: true, error: authoringErr.message };
  }
  sdData.metadata = {
    ...(sdData.metadata || {}),
    sd_authoring_validated_at: new Date().toISOString(),
    sd_authoring_validation_summary: sdAuthoringValidation,
  };
  ...
  // QF-20260729-725: wire the EXISTING dedup detector (SD-LEO-INFRA-SOURCING-ENGINE-DEDUP-
  // AUTOSTAMP-001's shipped router) into the SD-create choke. Measurement bound resolved before
  // this fix: the module was never called from any SD-create path (an uncalled function, not a
  // plumbing loss between tables) -- 0 of 5453 SDs carried a stamp because nothing here ever ran
  // the matcher. WARN-ONLY by design (acceptance #4): a duplicate must surface, never block --
  // an over-eager blocker on the create path would be a worse failure than the missing warning.
  // dedup_match_sd_key is always stamped (an sd_key on match, 'none' on a checked non-match) so a
  // future null on THIS SD is unambiguously "never checked", never "checked, no duplicate" (#1).
  try {
    const { routeCandidate } = await import('../sourcing-engine/router.js');
    const { fetchAllPaginated } = await import('../db/fetch-all-paginated.mjs');
    const existingSDs = await fetchAllPaginated(() => supabase
      .from('strategic_directives_v2')
      .select('sd_key, title'));
    const existing = existingSDs.map((s) => ({ sd_key: s.sd_key, title: s.title }));
    const routed = routeCandidate(
      { source_id: sdKey, title: sdData.title, description: sdData.description },
      { existing },
    );
    const matchKey = routed.dedup_match_sd_key || 'none';
    sdData.metadata = { ...(sdData.metadata || {}), dedup_match_sd_key: matchKey };
    if (matchKey !== 'none') {
      console.warn(
        `   ⚠️  [DEDUP] possible duplicate of ${matchKey} — review before proceeding `
        + '(this SD is still created; not a hard block).'
      );
    }
  } catch (dedupErr) {
    console.warn(`   ⚠️  dedup-match check skipped (non-blocking): ${dedupErr.message}`);
  }

  // SD-LEO-FIX-AUTHORING-TIME-EVIDENCE-001: resolve uuid-shaped tokens the author cited
  // in description/scope/success_criteria against the live DB BEFORE the insert, so a
  // stale or unowned fixture reference is caught at mint time rather than one
  // investigation round later at LEAD. A genuinely unresolvable token refuses the mint
  // loudly, naming it; a resolved token is annotated with its verified owner in place
  // (the row always wins over stale self-authored prose). OPT-IN (checkArtifactOwners)
  // -- see its destructure comment above for why this cannot run unconditionally for
  // every caller of this shared function.
  if (checkArtifactOwners) try {
    const successCriteriaText = Array.isArray(sdData.success_criteria)
      ? sdData.success_criteria.map((c) => (typeof c === 'string' ? c : c?.criterion || '')).join('\n')
      : '';
    const { resolutions, unresolved } = await resolveArtifactOwnersInFields(
      supabase, [sdData.description, sdData.scope, successCriteriaText],
    );
    if (unresolved.length > 0) {
      const msg = `Unresolved artifact reference(s) in SD prose: ${unresolved.join(', ')} -- cite a real row id in an allowlisted table, or remove the reference.`;
      console.error(msg);
      return { ok: false, error: msg, code: 'UNRESOLVED_ARTIFACT_TOKEN', exitCode: 1 };
    }
    for (const r of resolutions) {
      // A resolved token with ownerKey===null is a real row that simply has no owner
      // set -- nothing meaningful to annotate, and not a refusal case either.
      if (!r.resolved || r.ownerKey == null) continue;
      sdData.description = annotateOwnerInText(sdData.description, r.token, r.ownerKey);
      sdData.scope = annotateOwnerInText(sdData.scope, r.token, r.ownerKey);
      if (Array.isArray(sdData.success_criteria)) {
        sdData.success_criteria = sdData.success_criteria.map((c) => {
          if (typeof c === 'string') return annotateOwnerInText(c, r.token, r.ownerKey);
          if (c && typeof c.criterion === 'string') return { ...c, criterion: annotateOwnerInText(c.criterion, r.token, r.ownerKey) };
          return c;
        });
      }
    }
  } catch (resolverErr) {
    // Fail-open on infra errors ONLY (a genuinely unresolved token returns above, before
    // reaching here) -- mirrors this codebase's convention: an infra blip on a required
    // check must never masquerade as a hard block.
    console.warn(`   ⚠️  artifact-owner resolution skipped (non-blocking, infra error): ${resolverErr.message}`);
  }
