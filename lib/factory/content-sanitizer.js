...
// record_venture_error (live, SECURITY DEFINER, GRANT EXECUTE TO anon) writes rows with
// this source_type from a caller-supplied, length-capped-only p_message. Two more
// anon-callable RPCs writing the same source_type (fn_submit_error_capture,
// fn_submit_venture_error) are chairman-gated and pending apply -- this fix should land
// before either is applied so their exposure arrives pre-mitigated.
const PUBLIC_ORIGIN_SOURCE_TYPES = new Set(['user_feedback', 'error_capture']);
