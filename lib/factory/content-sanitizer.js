...
// 'telegram' added (SD-LEO-GEN-SECURITY-TELEGRAM-BOT-001): a schema-legal source_type
// (feedback_source_type_check permits it) that was absent from this allowlist. NOT a purely
// hypothetical gap -- telegram_bot_insert_feedback was a LIVE, anon-key-reachable PERMISSIVE
// INSERT policy (no venture predicate) from 2026-02-23 until dropped 2026-08-16, two days
// before this fix; the bot integration used the publicly-distributable anon key (never
// authenticatable as chairman-authored), and it produced at least one real row that became
// a real SD (id 4bedbf0d-...). Both write-path axes that historically armed 'error_capture'/
// 'venture_worker' are re-checked here and confirmed closed as of this fix: the RLS axis
// (the named telegram_bot_insert_feedback policy, and the source_type-agnostic
// venture_user_insert_feedback residual path -- the latter's restore is staged/unapplied at
// database/chairman-gated/20260817_restore_feedback_permissive_insert.sql and does not
// discriminate on source_type) AND the SECURITY-DEFINER-RPC-with-anon-EXECUTE axis (no RPC
// grants anon a path to write source_type='telegram'; fn_submit_venture_feedback hard-rejects
// any source_type other than 'venture_worker'). Closes the trust-classification side now,
// so a future reopening of either axis cannot silently default to trusted.
const PUBLIC_ORIGIN_SOURCE_TYPES = new Set(['user_feedback', 'error_capture', 'venture_worker', 'telegram']);
