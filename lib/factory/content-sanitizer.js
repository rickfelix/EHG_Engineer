...
// (feedback_source_type_check permits it) that was absent from this allowlist. No live
// anon-write RLS path to a telegram-sourced row exists today -- both candidate paths (the
// named telegram_bot_insert_feedback policy, and the source_type-agnostic
// venture_user_insert_feedback residual path) are currently closed -- but a staged,
// chairman-gated migration (database/chairman-gated/20260817_restore_feedback_permissive_
// insert.sql) would reopen the residual path if ever applied, and it does not discriminate
// on source_type. Defense-in-depth: closes the trust-classification side now.
