...
/**
 * VentureContextManager — Canonical context provider for DI.
 * Provides venture-scoped context (ventureId, supabase, config) as parameters
 * instead of module-level singletons.
 * Part of SD-MAN-ORCH-VISION-HEAL-SCORE-93-001-05-A
 */
export class VentureContextManager {
  ...
  constructor({ factory, ventureId, config = {} }) {
    this.#factory = factory || getServiceFactory();
    this.#ventureId = ventureId;
    this.#config = config;
  }

  get ventureId() { return this.#ventureId; }
  get config() { return this.#config; }

  async getSupabase() { return this.#factory.getSupabase(); }
  getLLMClient(opts) { return this.#factory.getLLMClient(opts); }

  /**
   * Create a child context with the same factory but different venture.
   */
  forVenture(ventureId) {
    return new VentureContextManager({
  ...
  /**
   * Build context object for passing to services.
   */
  async buildContext() {
    return {
