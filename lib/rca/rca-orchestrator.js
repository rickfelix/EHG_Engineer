...
export async function processTriggerEvent(triggerEvent, options = {}) {
  ...
  if (existingRCR) {
    ...
    const rcrPayload = {
      ...
      scope_id: triggerEvent.sd_id || triggerEvent.module,
      sd_id: triggerEvent.sd_id || null,
  ...
  const issuePatternId = await upsertIssuePattern(supabase, triggerEvent, rcrId, log);
 ...
 */
async function upsertIssuePattern(supabase, triggerEvent, rcrId, log) {
  ...
  if (existing) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        last_seen_sd_id: triggerEvent.sd_id || existing.last_seen_sd_id,
        updated_at: new Date().toISOString()
      })
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      occurrence_count: 1,
      first_seen_sd_id: triggerEvent.sd_id || null,
      last_seen_sd_id: triggerEvent.sd_id || null,
      source: 'auto_rca',
      status: 'active',
      proven_solutions: [],
