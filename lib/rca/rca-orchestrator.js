...
// SD-LEO-INFRA-MINUS-DISPOSITION-RAILS-001 FR-3: governed skipRCA, per trigger_type.
// Conservative by construction: absent an explicit opt-in, RCA sub-agent invocation stays
// skipped. Deliberately NOT keyed off rca_auto_trigger_config.enabled — all 8 rows have
// enabled=true with empty metadata today, so reading that column directly would flip RCA
// invocation on for every trigger_type simultaneously. A NEW metadata key is the opt-in.
const SKIP_RCA_CACHE_TTL_MS = 60 * 1000;
const skipRcaCache = new Map();

/**
 * Pure decision: given the config row's metadata (or null/absent), should RCA be skipped?
 * Exported separately from the DB fetch so the decision logic is unit-testable without a
 * database — mirrors this file's own noise-classifier extraction pattern.
 */
export function computeSkipRCA(configMetadata) {
  return configMetadata?.rca_sub_agent_invocation_enabled !== true;
}

/**
 * Resolve skipRCA for a trigger_type from rca_auto_trigger_config, cached per-process with
 * a bounded TTL (mirrors the table's own rate_limit_per_minute granularity) to avoid a DB
 * round-trip on every handoff-failure/gate-failure/API-failure path.
 */
export async function resolveSkipRCA(supabase, triggerType) {
  const now = Date.now();
  const cached = skipRcaCache.get(triggerType);
  if (cached && cached.expiresAt > now) return cached.value;

  let skip = true; // fail-closed: any lookup problem keeps the conservative default
  try {
    const { data, error } = await supabase
      .from('rca_auto_trigger_config')
      .select('metadata')
      .eq('trigger_type', triggerType)
      .maybeSingle();
    if (!error) skip = computeSkipRCA(data?.metadata);
  } catch {
    skip = true;
  }

  skipRcaCache.set(triggerType, { value: skip, expiresAt: now + SKIP_RCA_CACHE_TTL_MS });
  return skip;
}
 ...
 *
 * SD-LEO-INFRA-MINUS-DISPOSITION-RAILS-001 FR-3: skipRCA is no longer hardcoded true.
 * When the caller (all 3 real call sites, via lib/rca/index.js's triggerRCAOnFailure
 * wrapper, call this with no options) doesn't explicitly pass skipRCA, it is resolved
 * per trigger_type from rca_auto_trigger_config's governed metadata key.
 *
 * The governance lookup's own connection failure is caught and swallowed HERE (falling
 * back to conservative skip=true) rather than being allowed to propagate to the outer
 * try/catch — a regression caught by prospective EXEC-phase testing evidence: if it
 * propagated, processTriggerEvent() would never run at all on a connection hiccup, and
 * NO RCR would be created for a real failure — strictly worse than pre-fix behavior,
 * where triggerQuick unconditionally called processTriggerEvent (which handles its own
 * connection failure internally and still returns gracefully).
 *
 * @param {Object} triggerEvent - From trigger-sdk buildXxxContext()
 * @param {Object} [options]
 * @param {boolean} [options.skipRCA] - Explicit override; omit to use governed resolution
 */
export async function triggerQuick(triggerEvent, options = {}) {
  let skipRCA = options.skipRCA;
  if (skipRCA === undefined) {
    try {
      const { createSupabaseServiceClient } = await import('../../scripts/lib/supabase-connection.js');
      const supabase = await createSupabaseServiceClient('engineer', { verbose: false });
      skipRCA = await resolveSkipRCA(supabase, triggerEvent.trigger_type);
    } catch (err) {
      console.error(`[RCA-Orchestrator] Governed skipRCA lookup failed (falling back to conservative skip=true): ${err.message}`);
      skipRCA = true;
    }
  }
  try {
    await processTriggerEvent(triggerEvent, { skipRCA, verbose: false });
  } catch (err) {
