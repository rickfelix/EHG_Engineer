 ...
 *   Legacy:  "EXPERTISE_GAP: <description>"
 *   Rubric:  "EXPERTISE_GAP: [CATEGORY] — <description>"
 * @param {string[]} seatOutputs - Array of board seat position texts
 * @returns {Array<{category: string|null, description: string}>} Parsed gaps
 */
export function parseExpertiseGaps(seatOutputs) {
  ...
  for (const output of seatOutputs) {
    ...
    for (const match of matches) {
      ...
      // Parse rubric format: "CATEGORY — description"
      const rubricMatch = raw.match(/^(\w+(?:_\w+)?)\s*[—\-]+\s*(.+)$/);
      if (rubricMatch) {
        gaps.push({
          category: rubricMatch[1].toUpperCase(),
          description: rubricMatch[2].trim(),
          raw
        });
      } else {
        gaps.push({ category: null, description: raw, raw });
      }
    }
  }
