...
/**
 * AI-native development context injected into all board deliberations.
 * Without this, board seats assume human dev teams, calendar-time soaks,
 * and meeting-driven coordination — none of which apply to EHG.
 *
 * SD: SD-LEO-ORCH-GSTACK-INTEGRATION-CLEANUP-001-D
 */
const AI_NATIVE_CONTEXT = `
CRITICAL CONTEXT — AI-Native Development Environment:
- EHG has NO human developers. All code is written by Claude Code (AI agents).
- Development velocity is measured in minutes/hours, not days/weeks.
- "Soak periods" mean automated test runs and monitoring, not calendar-time waits.
- Coordination happens through database state (SD claims, handoffs), not meetings.
- Code review is automated through quality gates, not human pull request reviews.
- Deploy cycles are continuous — no sprint boundaries or release trains.
- Cost is measured in API tokens and compute, not developer salaries.
- Risk of "developer turnover" is zero — knowledge persists in code and database.
- Parallelism is limited by session claims, not team size.
Factor this context into ALL evaluations. Do not recommend processes designed for human teams.
`;
