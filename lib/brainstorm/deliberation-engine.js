 ...
 * Falls back to the default invokeAgent if no provider assignment exists.
 *
 * @param {object} providerAssignment - {provider, modelId} from rotation scheduler
 * @param {Function} defaultInvokeAgent - The session's default invokeAgent callback
...
function createProviderInvokeAgent(providerAssignment, defaultInvokeAgent) {
  ...
  return async (systemPrompt, userPrompt) => {
    try {
      ...
      console.warn(`[deliberation-engine] Provider ${providerAssignment.provider} failed, falling back to default:`, err.message);
...
export async function executeDeliberation({
  ...
  // 2.5. Record multi-model provider assignments (if enabled)
  const multiModel = isMultiModelEnabled();
  if (multiModel) {
    const assignments = panel
      .filter(s => s.providerAssignment)
      .map(s => s.providerAssignment);
    if (assignments.length > 0) {
      await recordSeatAssignments(result.debateSessionId, assignments, 1);
      result.providerAssignments = assignments;
    }
  }
  ...
  // 5. Record Round 1 arguments in database (with structured dissent + provider tracking)
  const round1Ids = {};
  for (const pos of result.round1Positions) {
    const citations = extractConstitutionalCitations(pos.position);
    const structuredDissent = parseStructuredDissent(pos.position);
    if (structuredDissent) {
      pos.structuredDissent = structuredDissent;
    }
    // Find provider assignment for this seat
    const seatPanel = panel.find(s => s.code === pos.seatCode);
    const providerInfo = seatPanel?.providerAssignment || null;
    const argId = await recordBoardArgument({
      ...
      structuredDissent: structuredDissent,
      providerUsed: providerInfo?.provider || null,
...
Produce your rebuttal. Reference specific positions from other seats by their code (e.g., "The CRO raises a valid concern about..."). Incorporate specialist testimony where relevant. Refine or defend your position based on new information.`
  ...
  // 10. Advance provider rotation for next session (if multi-model)
  if (multiModel && result.providerAssignments) {
    await advanceRotation(result.providerAssignments);
  }
...
/**
 * Parse structured dissent JSON block from a seat's response.
 * Expects a ```json block with assumption_challenged, counter_evidence, confidence_in_dissent.
 * Returns null with a warning if parsing fails — never throws.
 *
 * @param {string} text - The full seat response
 * @returns {object|null} Parsed dissent or null
 */
function parseStructuredDissent(text) {
  if (!text) return null;
  const jsonMatch = text.match(/```json\s*\n?\s*(\{[\s\S]*?\})\s*\n?\s*```/);
  if (!jsonMatch) return null;
  try {
    const parsed = JSON.parse(jsonMatch[1]);
    if (parsed.assumption_challenged) return parsed;
    return null;
  } catch {
    console.warn('[deliberation-engine] structured_dissent JSON parse failed — storing null');
    return null;
  }
}

// Re-export for convenience
export { QUORUM_PERCENTAGE, DELIBERATION_TIMEOUT_MS, parseStructuredDissent };
