 ...
 * usage log (.claude/logs/context-usage.jsonl). The hook (.claude/context-usage-feed.cjs
 * buildUsageEntry) appends one JSONL snapshot per session: { session_id, timestamp, model_id,
 * context_used, context_size, usage_percent, input_tokens, output_tokens, cache_creation_tokens,
 * cache_read_tokens, status, compaction_detected, working_directory }.
 ...
 *
 * QF-20260729-563: this reader originally looked for { session, ts, input, output, cache_create,
 * cache_read } -- field names that never matched what buildUsageEntry actually writes. Every
 * session's every snapshot silently missed on `e.session === sessionId` (real field: session_id),
 * so captured was false on all 1,904 solomon_advice_outcome_ledger rows despite captureLedgerRow's
 * write path being fully wired. Live-measured against the real .claude/logs/context-usage.jsonl
 * (tail -3) before fixing, not assumed.
 *
...
function snapshotTokens(e) {
  ...
  const sum = ['input_tokens', 'output_tokens', 'cache_creation_tokens', 'cache_read_tokens']
...
function readSessionCostTelemetry({ sessionId, logPath, nowMs = Date.now(), logContent = null } = {}) {
  try {
    ...
    for (const line of String(content).split('\n')) {
      ...
      if (e && e.session_id === sessionId) matching.push(e);
    }
