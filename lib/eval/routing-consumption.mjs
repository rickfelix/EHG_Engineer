...
/** C2 consumption contract — durable anti-fork marker for all doctrine consumers. */
export const ROUTING_DOCTRINE_CONSUMERS = Object.freeze({
  'dispatch-tiering': Object.freeze({
    ...
    // SD-FDBK-INFRA-RETIRE-SEAT-TIER-001 (chairman ratification 20dc072b): assertWorkerTierAllowed
    // -- this doctrine consumer's only production caller of resolveRoutingScore -- was deleted.
    // Left as a retired record (not removed) so the doctrine seam's consumption history stays
    // auditable; a future re-consumer must register a NEW entry, not resurrect this one.
    wired_at: 'RETIRED: was lib/coordinator/dispatch.cjs assertWorkerTierAllowed() (deleted, ratification 20dc072b)',
  }),
  ...
  // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-1): advisory dispatch suggestions. Consumes
  // tier-ladder.cjs's STATIC rank space (rankForModelEffort, liveWorkerCapabilitySnapshot) rather
  // than calling resolveCapabilityRouting directly — ranks are themselves downstream of this seam
  // via resolveRoutingScore, so no second capability-score formula is defined. See
  // lib/fleet/dispatch-suggestions.cjs header for the full rationale (including why this SD's
  // FR-2 was descoped — it duplicated an already-shipped mechanism).
  'dispatch-suggestions': Object.freeze({
    status: 'live',
    entry: 'lib/fleet/dispatch-suggestions.cjs rankCandidatesForItem()',
    wired_at: 'scripts/fleet-dashboard.cjs suggestions (FR-1 reader)',
  }),
});
