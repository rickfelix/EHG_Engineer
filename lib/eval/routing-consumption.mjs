...
export const ROUTING_DOCTRINE_CONSUMERS = Object.freeze({
  ...
  // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-1/FR-2): advisory dispatch suggestions +
  // pickup-fit deferral. Consumes tier-ladder.cjs's dense RANK space (deriveLiveLadder,
  // liveWorkerCapabilitySnapshot) rather than calling resolveCapabilityRouting directly — ranks
  // are themselves downstream of this seam via resolveRoutingScore, so no second capability-score
  // formula is defined. See lib/fleet/dispatch-suggestions.cjs header for the full rationale.
  'dispatch-suggestions': Object.freeze({
    status: 'live',
    entry: 'lib/fleet/dispatch-suggestions.cjs rankCandidatesForItem() / shouldDeferForBetterFit()',
    wired_at: 'lib/checkin/steps/merged-pool-self-claim.cjs (FR-2) + scripts/fleet-dashboard.cjs suggestions (FR-1 reader)',
  }),
});
