...
async function getSDMetrics(since) {
  // Paginated (FR-6 batch 7): completed_this_week is a COUNT over these rows — a
  // capped read silently understates weekly velocity. Errors degrade to [] (the
  // prior destructure-without-error-check behavior).
  const completed = await fetchAllPaginated(() => supabase
    .from('strategic_directives_v2')
    .select('sd_key, title, sd_type, completion_date')
    .eq('status', 'completed')
    .gte('completion_date', since)
    .order('completion_date', { ascending: false })
    .order('sd_key', { ascending: true })).catch(() => []);
...
async function getVentureMetrics() {
  const { data: ventures } = await supabase
    .from('ventures')
    .select('id, name, current_stage, status, updated_at') // schema-lint-disable-line — pre-existing reference on main (ventures.current_stage, untouched line; file entered the diff via FR-6 batch 7)
...
async function getBrainstormMetrics(since) {
  const { data: sessions } = await supabase
    ...
    .select('id, title, outcome_type, score, created_at') // schema-lint-disable-line — pre-existing reference on main (brainstorm_sessions.title, untouched line; file entered the diff via FR-6 batch 7)
