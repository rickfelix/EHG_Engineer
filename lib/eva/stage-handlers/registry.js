...
export async function runStageHandler(completedStage, ctx) {
  ...
  try {
    ...
    // Log string matches the kill-switch Map path verbatim — the two dispatch
    // paths must be observably identical, logs included.
    ctx.logger.log(`[Worker] Post-stage hook fired for S${completedStage} (venture ${ctx.ventureId})`);
  } catch (err) {
    // Fail-soft parity with _runPostStageHooks: warn, never block advancement.
    ctx.logger.warn(`[Worker] Post-stage hook S${completedStage} failed (non-fatal): ${err.message}`);
  }
  return true;
}
