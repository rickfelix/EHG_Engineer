...
export class StageExecutionWorker {
      ...
      while (currentStage <= MAX_STAGE && this._running) {
        ...
        if (isPreExecGate) {
          ...
          if (approvedDecision && approvedDecision.status === 'approved') {
            ...
            if (existingArtifacts && existingArtifacts.length > 0) {
              ...
              // SD-CHAIRMAN-APPROVAL-SIDEEFFECTS-ATOMICITY-ORCH-001-B: Audit-table guard for kill gate side-effects.
              // DB trigger (SD-A) writes to chairman_decision_audit atomically on approval.
              // If audit row exists, side-effects are already applied — skip.
              // If no audit row but approval > 60s old, execute un-archive as TTL fallback.
              if (currentStage === 3 || currentStage === 5) {
                ...
                try {
                  ...
                  if (auditRow) {
                    this._logger.log(
                      `[WorkerGuard] Skipping vision un-archive — trigger side-effects confirmed via audit row (venture=${ventureId}, stage=${currentStage})`
                    );
                    triggerHandled = true;
                  }
                } catch (auditErr) {
                  this._logger.warn(`[WorkerGuard] Audit table query failed (falling through): ${auditErr.message}`);
                }

                if (!triggerHandled) {
                  // TTL fallback: if approval is older than 60s and no audit row, trigger may have failed
                  const approvalAge = approvedDecision.updated_at
                    ? Date.now() - new Date(approvedDecision.updated_at).getTime()
                    : Infinity;
                  const TTL_THRESHOLD_MS = 60_000;

                  if (approvalAge > TTL_THRESHOLD_MS) {
                    this._logger.warn(
                      `[WorkerGuard] WARNING: No audit row found, executing TTL fallback for venture=${ventureId} stage=${currentStage} (approval age=${Math.round(approvalAge / 1000)}s)`
                    );
                  }

                  // SD-MAN-FIX-FIX-ARCHIVED-VISION-001: Un-archive vision on kill gate override
                  // At stages 3/5, eva-orchestrator archives the vision when gateBlocked=true.
                  // Chairman override means the venture should continue with an active vision.
                  ...
                  if (archivedVision) {
                    await this._supabase
                      ...
                      .eq('vision_key', archivedVision.vision_key);
                    this._logger.log(`[Worker] Vision ${archivedVision.vision_key} restored to active (chairman override at stage ${currentStage})`);
                  }
                }
              }
        ...
        if (REVIEW_MODE_STAGES.has(currentStage) && !CHAIRMAN_GATES.BLOCKING.has(currentStage)) {
          ...
          if (reviewAutoApprove) {
            ...
            await this._advanceStage(ventureId, currentStage, nextReviewStage, { durationMs: stageDurationMs, result, advancementType: 'review_approved' });
  ...
  async _postStageHook_S15_StitchProvision(ventureId) {
    ...
    // Fetch Stage 15 advisory_data so provisioner has wireframe screens
    const { data: s15Work } = await this._supabase
      ...
      .limit(1)
      .maybeSingle();
    const result = await postStage15Hook(ventureId, s15Work?.advisory_data || null);
    if (result?.status === 'error') {
      this._logger.error(`[Worker] S15 post-stage hook FAILED: ${result?.error || 'unknown error'}`);
      // Surface error in advisory_data for downstream gate visibility
      await this._supabase
        .from('venture_stage_work')
        .update({
          advisory_data: {
            ...(s15Work?.advisory_data || {}),
            stitch_hook_status: 'error',
            stitch_hook_error: result?.error || 'unknown error',
          },
        })
        .eq('venture_id', ventureId)
        .eq('lifecycle_stage', 15)
      ...
      // Record success in advisory_data
      if (s15Work) {
        await this._supabase
          .from('venture_stage_work')
          .update({
