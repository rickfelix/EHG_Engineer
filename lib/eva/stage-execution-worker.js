        ...
        if (govPost.isReview(currentStage) && !govPost.isBlocking(currentStage) && !(await this._canAutoAdvance(currentStage))) {
          ...
          try {
            ...
            const { id: decisionId, isNew, reason: skipReason } = await createOrReusePendingDecision({
              ...
              decisionType: 'review',
              // SD-LEO-FEAT-MAKE-HIGH-CONSEQUENCE-001 (FR-2): chairman-designated
              // high-consequence stages mint a blocking=true decision.
              blocking: govPost.isHighConsequence(currentStage),
              supabase: this._supabase,
              logger: this._logger,
            });
  ...
  async _handleChairmanGate(ventureId, stageNumber) {
    try {
      ...
      const { id: decisionId, isNew, reason: skipReason } = await createOrReusePendingDecision({
        ...
        // SD-LEO-FEAT-MAKE-HIGH-CONSEQUENCE-001 (FR-2): chairman-designated
        // high-consequence stages mint a blocking=true decision.
        blocking: govGate.isHighConsequence(stageNumber),
        supabase: this._supabase,
        logger: this._logger,
      });
    ...
    try {
      ...
      if (!gov.isHighConsequence(fromStage)) {
        ...
        if (!flagEnabled) {
          ...
          const { data: pendingBlockingDecision } = await this._supabase
            .from('chairman_decisions')
            .select('id')
            .eq('venture_id', ventureId)
