...
export class StageExecutionWorker {
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    //   OFF (default, safe): all 3 checks still RUN (for observability), but a would-have-failed
    //     result from THIS choke-point only logs a structured "would have REFUSED" warning here --
    //     mirrors the LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE precedent's exact mechanism above (TR-2:
    //     reuse, do not invent a parallel logging/table mechanism -- this wrapper itself never
    //     writes a new DB row). SECURITY EXEC-TO-PLAN note (SEC-PATH-004): checkExitGates() has
    //     its OWN independent system_events writes (EXIT_GATE_ANOMALY / EXIT_GATE_OBSERVE_ONLY)
    //     that fire regardless of this flag -- composing it here means _advanceStage() now
    //     inherits those writes on call paths that never reached them before. Advance OUTCOME is
    //     unchanged (44 ventures / 325 failed-gate rows still advance freely with the flag OFF);
    //     the observability side effect is not.
    ...
    //     through 7+ callers here that expect a result object, not an exception).
    //
