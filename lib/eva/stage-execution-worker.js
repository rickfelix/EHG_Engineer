...
export class StageExecutionWorker {
      ...
      while (currentStage <= MAX_STAGE && this._running) {
        ...
        if ((isHcStage || (govPost.isReview(currentStage) && !(await this._canAutoAdvance(currentStage)))) && !govPost.isBlocking(currentStage)) {
          ...
          await this._supabase
            .from('ventures')
            .update({ current_lifecycle_stage: currentStage, stage_write_token: 'stage-execution-worker.js' })
            .eq('id', ventureId);
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    const { error: stageUpdateError } = await this._supabase
      .from('ventures')
      .update({ current_lifecycle_stage: toStage, stage_write_token: 'stage-execution-worker.js' })
      .eq('id', ventureId);
