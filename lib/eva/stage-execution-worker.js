// eva-logger-lint-ignore: this._logger is DI'd (console-shaped) from construction time across
// this whole pre-existing file; retrofitting createLogger/OrchestratorTracer file-wide is out
// of scope for QF-20260913-712's one-line GATE_RETRY_BACKOFF log-line change.
/**
...
import { stageWriteTokenField } from './stage-write-token-probe.js';
import {
...
import { checkGateDebt } from './artifact-persistence-service.js';
import { checkGateRetryCeiling } from './gate-retry-guard.js';
...
// SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-1: the pipeline ceiling is no longer a module-level
// literal — it is derived per-call from getStageGovernance(supabase).maxStageNumber (see
// _pollForWork and _processVenture), so a future venture_stages renumbering cannot silently
// diverge from the worker's own notion of "the last stage".
...
// Gate sets AND the pipeline ceiling are sourced from stage-governance.js (single DB-backed
// source of truth via venture_stages). Only OPERATING_MODES + TASTE_GATE_STAGES remain in
// gate-constants.js (SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-1: MAX_STAGE removed from there too).
...
export class StageExecutionWorker {
  ...
  async _pollForWork() {
    try {
      // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-1: derive the ceiling from the DB-backed
      // stage-governance cache (never a hardcoded literal), so a stage-26 venture is no longer
      // silently excluded from ever picking up the next stage once venture_stages grows to 27+ rows.
      const maxStage = (await getStageGovernance(this._supabase)).maxStageNumber;
      // TESTING evidence 1c5e61ff (SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001): maxStageNumber
      // resolves to 0 (not an error) on an empty venture_stages read, which would silently
      // fail every venture out of `.lt('current_lifecycle_stage', 0)` for the governance
      // cache's whole TTL. Throw loud instead of polling zero ventures in silence.
      if (maxStage < 1) {
        throw new Error(`[Worker] Poll aborted: stage-governance maxStageNumber=${maxStage} (venture_stages read empty or unavailable)`);
      }
      ...
      const data = await fetchAllPaginated(() => this._supabase
        ...
        .lt('current_lifecycle_stage', maxStage)
  ...
  async _processVenture(ventureId) {
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-1: derive the ceiling once per venture-processing
    // pass from the DB-backed stage-governance cache, never a hardcoded literal.
    const maxStage = (await getStageGovernance(this._supabase)).maxStageNumber;
    // TESTING evidence 1c5e61ff: maxStageNumber=0 means the governance read came back empty,
    // not that the pipeline has zero stages. Fail loud (caught by _tick's try/catch and logged)
    // rather than silently treating every venture as already past the ceiling.
    if (maxStage < 1) {
      throw new Error(`[Worker] Venture processing aborted: stage-governance maxStageNumber=${maxStage} (venture_stages read empty or unavailable)`);
    }
    ...
    try {
      ...
      const { data: venture, error: fetchError } = await this._supabase
        .from('ventures')
        .select('current_lifecycle_stage, name, metadata')
        .eq('id', ventureId)
        .single();
      ...
      // Venture-scoped PARK check (SD-LEO-INFRA-APEXNICHE-STAGE-RUNAWAY-001 FR-1, 2nd pass).
      // The first fix (eva-orchestrator.js processStage()) never actually stopped the ApexNiche
      // replay -- live post-merge verification found the "P0 UNIVERSAL pre-execution guard"
      // below (recordGateOverride -> recordGateAttempt) runs unconditionally on every poll,
      // BEFORE processStage() is ever considered, for any venture carrying an approved
      // chairman_decisions row with existing artifacts (the "skip processStage, advancing"
      // shortcut). That is the REAL writer for this incident -- eva_stage_gate_attempts kept
      // growing after both the code merge AND a full leo-stack restart, because this path was
      // never guarded. Checked here, before ANY of that logic runs, so a parked venture is
      // frozen at its true entry point. The processStage() guard stays in place as a second
      // layer for callers/paths that do reach it.
      const gatingDecision = venture.metadata?.gating_decision || null;
      if (gatingDecision?.parked === true) {
        const unparkTrigger = gatingDecision.unpark_trigger || gatingDecision.unpark || '(no unpark trigger recorded)';
        this._logger.log(`[Worker] VENTURE_PARKED venture=${ventureId} stage=${currentStage} decision="${gatingDecision.decision}" by="${gatingDecision.by}" at="${gatingDecision.at}" unpark="${unparkTrigger}" -- skipping all stage processing`);
        releaseState = ORCHESTRATOR_STATES.BLOCKED;
        return { ventureId, stageId: currentStage, status: 'blocked', gate: 'venture_parked' };
      }

      // 4. Sequential stage advancement loop
      while (currentStage <= maxStage && this._running) {
        // Check abort signal
        ...
        // SD-LEO-INFRA-STAGE-GATE-RETRY-001 (FR-1/FR-2): bounded retry ceiling + backoff on gate
        // re-evaluation. Checked fresh (DB-sourced, never cached) every tick, before any gate
        // evaluation for the current stage. A venture that keeps failing/re-evaluating the same
        // stage past GATE_RETRY_CEILING attempts is auto-terminalized (MANUAL_REQUIRED) via the
        // same gating_decision metadata pattern as a manual chairman park; below the ceiling but
        // past GATE_RETRY_BACKOFF_START, an increasing fraction of poll cycles are skipped.
        const retryCeilingCheck = await checkGateRetryCeiling(this._supabase, {
          ventureId, stageNumber: currentStage, logger: this._logger,
        });
        if (retryCeilingCheck.action === 'terminalize') {
          this._logger.log(`[Worker] GATE_RETRY_CEILING_EXCEEDED venture=${ventureId} stage=${currentStage} attempts=${retryCeilingCheck.attemptCount} -- terminalized MANUAL_REQUIRED`);
          releaseState = ORCHESTRATOR_STATES.BLOCKED;
          lastResult = { ventureId, stageId: currentStage, status: 'blocked', gate: 'gate_retry_ceiling_exceeded', attemptCount: retryCeilingCheck.attemptCount };
          break;
        }
        if (retryCeilingCheck.action === 'skip') {
          this._logger.log(`[Worker] GATE_RETRY_BACKOFF venture=${ventureId} stage=${currentStage} attempts=${retryCeilingCheck.attemptCount} residency_start=${retryCeilingCheck.residencyStart ?? 'none'} -- skipping this poll cycle`);
          releaseState = ORCHESTRATOR_STATES.IDLE;
          lastResult = { ventureId, stageId: currentStage, status: 'backoff_skip', attemptCount: retryCeilingCheck.attemptCount };
          break;
        }
          ...
          if (universalApproved) {
            ...
            if (existingArt && existingArt.length > 0) {
              ...
              if (!isBlockingGate) {
                ...
                // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001 (VALIDATION 04f7f256, 4th
                // call site found): universalApproved.status==='approved' was already confirmed
                // above -- a genuine chairman decision (P0 UNIVERSAL guard, covers taste-gate/
                // promotion-gate/other non-BLOCKING stages), advancing without ever reaching
                // _handleChairmanGate() this tick.
                const advanceResult = await this._advanceStage(ventureId, currentStage, nextStage, { advancementType: 'pre_exec_skip', chairmanGateSource: 'chairman_decision', chairmanDecisionId: universalApproved.id });
          ...
          if (approvedDecision && approvedDecision.status === 'approved') {
            ...
            if (currentStage === 3 || currentStage === 5) {
              ...
              if (auditRow) {
                ...
                const nextStage = currentStage + 1;
                // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001: approvedDecision.status==='approved'
                // was already confirmed above -- this IS a genuine chairman decision, just one whose
                // side-effects were already applied by a DB trigger, so _handleChairmanGate() is never
                // reached this tick.
                const advanceResult = await this._advanceStage(ventureId, currentStage, nextStage, { advancementType: 'pre_exec_skip_trigger', chairmanGateSource: 'chairman_decision', chairmanDecisionId: approvedDecision.id });
                if (advanceResult?.blocked) {
                  this._logger.log(`[Worker] Stage ${currentStage} advance blocked by artifact gate: ${advanceResult.reason}`);
                  releaseState = ORCHESTRATOR_STATES.BLOCKED;
                  lastResult = { ventureId, stageId: currentStage, status: 'artifact_precondition_unmet', ...advanceResult };
            ...
            if (existingArtifacts && existingArtifacts.length > 0) {
              ...
              const nextStage = currentStage + 1;
              // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001: approvedDecision.status==='approved'
              // was already confirmed above -- a genuine chairman decision, just re-entered after
              // artifacts already exist, so _handleChairmanGate() is never reached this tick.
              const advanceResult = await this._advanceStage(ventureId, currentStage, nextStage, { advancementType: 'pre_exec_skip', chairmanGateSource: 'chairman_decision', chairmanDecisionId: approvedDecision.id });
              if (advanceResult?.blocked) {
                this._logger.log(`[Worker] Stage ${currentStage} advance blocked by artifact gate: ${advanceResult.reason}`);
                releaseState = ORCHESTRATOR_STATES.BLOCKED;
                lastResult = { ventureId, stageId: currentStage, status: 'artifact_precondition_unmet', ...advanceResult };
        ...
        if (govPost.isReview(currentStage) || govPost.isBlocking(currentStage)) {
          ...
          if (approvedDecision) {
            ...
            // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001: approvedDecision.status==='approved'
            // was already confirmed above -- a genuine chairman decision on worker re-entry;
            // `result` here is a fresh _executeWithRetry() output that never went through
            // _handleChairmanGate(), so the explicit chairmanGateSource flag carries the signal instead.
            const advanceResult = await this._advanceStage(ventureId, currentStage, nextReentryStage, { durationMs: stageDurationMs, result, advancementType: 're_entry', chairmanGateSource: 'chairman_decision', chairmanDecisionId: approvedDecision.id });
            if (advanceResult?.blocked) {
              this._logger.log(`[Worker] Stage ${currentStage} advance blocked by artifact gate: ${advanceResult.reason}`);
              releaseState = ORCHESTRATOR_STATES.BLOCKED;
              lastResult = { ventureId, stageId: currentStage, status: 'artifact_precondition_unmet', ...advanceResult };
        ...
        if ((isHcStage || (govPost.isReview(currentStage) && !(await this._canAutoAdvance(currentStage)))) && !govPost.isBlocking(currentStage)) {
          ...
          // SD-LEO-INFRA-STAGE-WRITER-CHOKE-001: self-stamp for the canonical-writer choke, once
          // the column exists -- stageWriteTokenField degrades to {} until the chairman-gated
          // migration is applied, so this write never breaks on an unapplied migration.
          await this._supabase
            .from('ventures')
            .update({ current_lifecycle_stage: currentStage, ...(await stageWriteTokenField(this._supabase, 'stage-execution-worker.js')) })
            .eq('id', ventureId);
          ...
          if (reviewInsertOk) {
            ...
            await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on review-blocked exit; X1: honest status, not 'completed'
            releaseState = ORCHESTRATOR_STATES.BLOCKED;
        ...
        if (shouldOpenChairmanGate({ isHardGate: isHardcodedGate || isDynamicHardGate, stageFailed, canGovernanceOverrideFailed, isDecisionGate })) {
          ...
          if (gateResult.blocked) {
            ...
            // SD-LEO-INFRA-STAGE-WRITER-CHOKE-001: self-stamp for the canonical-writer choke, once
            // the column exists -- see stage-write-token-probe.js.
            await this._supabase
              .from('ventures')
              .update({ current_lifecycle_stage: currentStage, ...(await stageWriteTokenField(this._supabase, 'stage-execution-worker.js')) })
              .eq('id', ventureId);
            ...
            this._logStageTransition(ventureId, currentStage, 'completed', stageDurationMs, result).catch(() => {});
            await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on chairman-gate-blocked exit; X1: honest status, not 'completed'
            releaseState = ORCHESTRATOR_STATES.BLOCKED;
          ...
          if (gateResult.killed) {
            ...
            await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on killed exit; X1: 'killed' has no venture_stage_work.stage_status enum value, 'blocked' is the accurate non-completed state
          ...
          if (result) {
            result._gateApproved = true;
            // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001 (FR-1a): carry the decision
            // source through to whichever _advanceStage() call this iteration eventually reaches,
            // so FR-1's recordGateAttempt() call can fire ONLY on a genuine chairman decision.
            result._chairmanGateSource = gateResult.source;
            // Adversarial /ship-gate finding #2: also carry the real chairman_decisions.id so
            // recordGateAttempt()'s `reasoning` can reference the actual decision row.
            result._chairmanDecisionId = gateResult.decisionId || null;
          }
        ...
        if (stageFailed) {
          ...
          if (!canGovernanceOverrideFailed) {
            ...
            await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on failed exit; X1: 'failed' has no venture_stage_work.stage_status enum value, 'blocked' is the accurate non-completed state
            releaseState = ORCHESTRATOR_STATES.FAILED;
            break;
          }
        ...
        if (resultStatus === 'BLOCKED' || resultStatus === 'FAILED') {
          ...
          // Only override gate/decision blocks, NOT contract validation failures (missing upstream
          // data) or a deliberate venture-level PARK (SD-LEO-INFRA-APEXNICHE-STAGE-RUNAWAY-001):
          // without this second clause, a venture parked via ventures.metadata.gating_decision.parked
          // at any ordinary (non-hard-gate) stage would silently governance-override past the park —
          // _canAutoAdvance() has no visibility into the park flag, so it would approve advancing
          // exactly the venture this SD's guard exists to freeze, the moment global_auto_proceed is on.
          ...
          const isVenturePark = result?.errors?.some(e => e.code === 'VENTURE_PARKED');
          const governanceOverride = !isContractBlock && !isVenturePark && (result._gateApproved || await this._canAutoAdvance(currentStage));
          if (governanceOverride) {
            ...
            await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on governance-blocked exit; X1: honest status, not 'completed'
            releaseState = ORCHESTRATOR_STATES.BLOCKED;
            break;
          }
        }
        ...
        if (resultStatus === 'HELD') {
          ...
          this._logStageTransition(ventureId, currentStage, 'blocked', stageDurationMs, result).catch(() => {});
          await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // ensure health_score on HELD exit; X1: 'held' has no venture_stage_work.stage_status enum value, 'blocked' is the accurate non-completed state
          releaseState = ORCHESTRATOR_STATES.BLOCKED;
          break;
        }
        ...
        if (result.filterDecision?.action === 'STOP') {
          ...
          await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on filter STOP; X1: honest status, not 'completed'
          releaseState = ORCHESTRATOR_STATES.BLOCKED;
          break;
        }
        ...
        if (result.filterDecision?.action === 'REQUIRE_REVIEW' && !this._waitForReview && currentStage !== 19) {
          ...
          await this._writeHealthScore(ventureId, currentStage, { stageStatus: 'blocked' }); // QF: ensure health_score on filter REQUIRE_REVIEW; X1: honest status, not 'completed'
          releaseState = ORCHESTRATOR_STATES.BLOCKED;
          break;
        }
    ...
    try {
      ...
      if (!isHcGate) {
        ...
        if (autonomy.action === 'auto_approve') {
          ...
          // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001 (FR-1a): tag the decision source so
          // callers can distinguish a genuine chairman decision from an automated bypass -- all 5
          // return points below previously produced this identical shape with no way to tell them
          // apart, which would have mislabeled automated approvals as chairman-adjudicated.
          return { blocked: false, killed: false, approved: true, source: 'autonomy_auto_approve' };
        }
        ...
        if (shouldAutoApprove) {
          ...
          return { blocked: false, killed: false, approved: true, source: 'governance_auto_approve' };
        }
      }
      ...
      if (skipReason === 'fixture_venture') {
        ...
        return { blocked: false, killed: false, approved: true, source: 'fixture_venture_skip' };
      }
      ...
      if (!isNew) {
        ...
        if (existing?.status === 'approved') {
          ...
          return { blocked: false, killed: false, approved: true, source: 'chairman_decision', decisionId };
        }
   ...
   *
   * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-H (X1): this used to unconditionally stamp
   * stage_status='completed' regardless of the caller's real exit outcome, so a
   * review-blocked/chairman-gate-blocked/killed/failed/governance-blocked/HELD/filter-STOP/
   * REQUIRE_REVIEW exit was mislabeled 'completed' in venture_stage_work -- live-verified on
   * AltifyAI (stage_status='completed' at a stage where the venture was actually parked
   * orchestrator_state='blocked'). venture_stage_work.stage_status's CHECK constraint only
   * allows not_started|in_progress|blocked|completed|skipped -- there is no failed/held/killed
   * value in the schema, so every non-advancing exit maps to 'blocked' (the same value this
   * file already uses elsewhere, e.g. lines ~792/~1863/~4231/~4468/~4776, for equivalent
   * non-completed states), never a fabricated new enum value.
   *
   * @param {string} ventureId
   * @param {number} completedStage
   * @param {{stageStatus?: string}} [outcome] - stageStatus defaults to 'completed' (every
   *   pre-existing call site's behavior, byte-identical) unless a caller explicitly names a
   *   non-advancing exit's real status ('blocked' -- the only non-'completed' value this
   *   column's CHECK constraint permits for a non-advancing exit).
   */
  async _writeHealthScore(ventureId, completedStage, outcome = {}) {
    try {
      const { computeHealthScore } = await import('./health-score-computer.js');
      const { data: stageWork } = await this._supabase
        .from('venture_stage_work')
        .select('advisory_data, work_type')
        .eq('venture_id', ventureId)
      ...
      const now = new Date().toISOString();
      const stageStatus = outcome.stageStatus || 'completed';
      // Preserve an existing row's work_type classification (e.g. 'sd_required',
      // 'automated_check') rather than always clobbering it to 'artifact_only' -- work_type
      // describes what KIND of work the stage requires, which a blocked/completed exit does
      // not change.
      const workType = stageWork?.work_type || 'artifact_only';
      await this._supabase
        .from('venture_stage_work')
        .upsert({
          venture_id: ventureId,
      ...
      this._logger.log(`[Worker] S${completedStage} health_score: ${healthScore} (stage_status=${stageStatus})`);
    } catch (err) {
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001 (FR-1a correction, VALIDATION c39db537):
    // an explicit top-level context flag, separate from result._chairmanGateSource, because 3 of
    // this function's 7 call sites (pre_exec_skip, pre_exec_skip_trigger, re_entry) advance a
    // venture off an already-approved chairman_decisions row WITHOUT ever calling
    // _handleChairmanGate() in that tick -- there is no `result` object (or it's a fresh one from
    // _executeWithRetry that was never tagged) to carry the signal on. Each of those call sites
    // independently re-confirms status==='approved' immediately before calling this function.
    const { result = null, durationMs = 0, advancementType = 'normal', chairmanGateSource = null, chairmanDecisionId = null } = context;
    ...
    // paths reached here. Strict on fromStage===PRODUCT_REVIEW_STAGE && toStage===PRODUCT_REVIEW_STAGE+1
    // so all other transitions short-circuit with zero extra queries. The product-review decision
    // is a SEPARATE decision_type from the existing stage-24 kill-gate decision — never merged
    // into one verdict (out of scope: redefining the kill-gate policy). FAIL-OPEN on evaluator
    // error (a transient DB blip must not become fleet-wide denial-of-progress — mirrors the S19
    // backstop's contract).
    //
    // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-H (FR-1): this choke point read fromStage===23 &&
    // toStage===24 and queried lifecycle_stage:23 -- stale against the live producer
    // (chairman-product-review.js's own PRODUCT_REVIEW_STAGE=24), which a prior SD already
    // renumbered. Live-verified: product_review decisions exist only at lifecycle_stage=24, zero
    // at 23 -- the gate could never pass where it queried, and never fired at the stage (24->25)
    // where it needed to. Now consumes the producer's own exported constant so the two halves
    // cannot drift apart again.
    const { PRODUCT_REVIEW_STAGE, PRODUCT_REVIEW_DECISION_TYPE } = await import('./chairman-product-review.js');
    if (fromStage === PRODUCT_REVIEW_STAGE && toStage === PRODUCT_REVIEW_STAGE + 1) {
      let productReviewApproved;
      let productReviewEvalError = null;
      try {
        ...
        productReviewEvalError = e;
        this._logger.warn(`[Worker] Product-review choke-point eval error: ${e.message}`);
        // SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 (FR-4): fail-CLOSED by default (an evaluator
        // error now blocks the advance instead of silently approving it) -- paired with its own
        // kill-switch flag, mirroring FR-1's pattern, so a genuine evaluator bug cannot itself
        // become a fleet-wide publication-blocking incident with no operator escape hatch. Same
        // leo_feature_flags read pattern as LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE (:3029-3036) -- an
        // absent row or read failure defaults to disabled, so fail-CLOSED stays the active
        // default (AC#4 / TS-12) unless an operator has explicitly flipped the flag on.
        let killSwitchEnabled = false;
        try {
          const { data: flagRow } = await this._supabase
            .from('leo_feature_flags')
            .select('is_enabled')
            .eq('flag_key', 'PATH_INTEGRITY_PRODUCT_REVIEW_KILL_SWITCH')
            .maybeSingle();
          killSwitchEnabled = flagRow?.is_enabled === true;
        } catch (flagErr) {
          this._logger.warn(`[Worker] PATH_INTEGRITY_PRODUCT_REVIEW_KILL_SWITCH flag read failed (defaulting to disabled, fail-closed stays active): ${flagErr.message}`);
        }
        if (killSwitchEnabled) {
          this._logger.warn(`[Worker] PATH_INTEGRITY_PRODUCT_REVIEW_KILL_SWITCH is enabled — restoring pre-fix fail-open behavior for venture ${ventureId} on evaluator error.`);
          productReviewApproved = true;
        } else {
          productReviewApproved = false; // fail-closed (active default): evaluator error blocks the advance
        }
      }
      if (!productReviewApproved) {
        this._logger.log(
          `[Worker] _advanceStage PRODUCT-REVIEW CHOKE-POINT (SD-LEO-INFRA-CHAIRMAN-PRODUCT-REVIEW-001): REFUSING advance of venture ${ventureId} from Stage ${PRODUCT_REVIEW_STAGE} to Stage ${PRODUCT_REVIEW_STAGE + 1} — ${productReviewEvalError ? `evaluator error (${productReviewEvalError.message})` : 'no approved chairman product_review decision found'}.`
        );
    ...
    // SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001 (FR-6): fenced synthetic-actor choke-point — the 5th
    // backstop in this function. Strict on fromStage===19 && toStage===20 (mirrors the
    // PRODUCT-REVIEW backstop's scoping — the live signed-in UAT check is inherently a
    // post-deploy concept, and stage 19's own gates already include "Application deployed").
    // Independent, bespoke check: NOT routed through checkStageArtifactPrecondition
    // (venture_stages.required_artifacts is fleet-wide-per-stage, structurally wrong for
    // per-venture opt-in fencing) and NOT routed through checkExitGates() (would newly subject
    // every _advanceStage() caller to 3 unrelated existing binding gates). Fleet-safety
    // short-circuit: a venture that never opted in (metadata.uat_probe_required !== true)
    // always passes with zero GitHub calls — see lib/eva/synthetic-actor-guard.js for the full
    // fail-closed / step-granularity / cached-last-known-good contract.
    if (fromStage === 19 && toStage === 20) {
      const { checkSyntheticActorFencing } = await import('./synthetic-actor-guard.js');
      const fencing = await checkSyntheticActorFencing(this._supabase, ventureId);
      if (fencing.applies && !fencing.satisfied) {
        // SEC-42 (EXEC-TO-PLAN SECURITY finding): FR-6 AC#4 requires this gate ship
        // behind a dedicated, non-global, DEFAULT-OFF flag (observe-only until
        // explicitly enabled) -- the original implementation shipped unconditionally
        // binding, which would have hard-blocked AltifyAI's very next Stage 19->20
        // attempt the moment LEO_ALTIFYAI_UAT_READ_TOKEN was left unprovisioned, with
        // no off switch short of a code revert. Mirrors checkStageArtifactPrecondition's
        // own leo_feature_flags read pattern (flag_key/is_enabled); an absent row
        // defaults to disabled/observe-only, matching AC#4's stated safe default.
        let bindingEnabled = false;
        try {
          const { data: flagRow } = await this._supabase
            .from('leo_feature_flags')
            .select('is_enabled')
            .eq('flag_key', 'LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE')
            .maybeSingle();
          bindingEnabled = flagRow?.is_enabled === true;
        } catch (flagErr) {
          this._logger.warn(`[Worker] LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE flag read failed (defaulting to observe-only, not binding): ${flagErr.message}`);
        }
        if (bindingEnabled) {
          this._logger.error(
            `[Worker] _advanceStage SYNTHETIC-ACTOR CHOKE-POINT (SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001): REFUSING advance of venture ${ventureId} from Stage 19 to 20 — ${fencing.reason}`
          );
          return { advanced: false, blocked: true, reason: 'synthetic_actor_fencing_unmet', details: fencing.reason };
        }
        // VALIDATION V-3 (PLAN_VERIFICATION): AC#14 requires a GitHub API
        // error to log as "cannot determine," distinct from a genuine
        // would-block -- otherwise a transient outage reads identically to
        // the fence actually catching something, corrupting AC#15's
        // promotion-readiness signal (N consecutive clean cycles). Binding
        // behavior is unaffected either way (still fail-closed above).
        if (fencing.indeterminate) {
          this._logger.warn(
            `[Worker] _advanceStage SYNTHETIC-ACTOR CHOKE-POINT (SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001): OBSERVE-ONLY -- CANNOT DETERMINE for venture ${ventureId} from Stage 19 to 20 (${fencing.reason}) -- not a would-block finding, LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE is not enabled regardless -- advancing anyway.`
          );
        } else {
          this._logger.warn(
            `[Worker] _advanceStage SYNTHETIC-ACTOR CHOKE-POINT (SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001): OBSERVE-ONLY -- would have REFUSED advance of venture ${ventureId} from Stage 19 to 20 (${fencing.reason}), but LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE is not enabled -- advancing anyway.`
          );
        }
      }
    }

    // SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 (FR-1): path-integrity choke-point -- the 6th
    // backstop in this function, and the first to COMPOSE 3 existing, independently-flagged
    // checks (checkExitGates, checkThesisKillGate, checkGateDebt) rather than implement a new
    // one. _advanceStage() is the dominant venture-stage advance path (7+ call sites) and, prior
    // to this SD, ran NONE of the three checks that DO run on the OTHER, non-dominant
    // advanceStage()/processLifecycleTerminal() paths -- see lib/eva/lifecycle/gate-conformance.js's
    // doc-block, corrected in this same commit.
    //
    // Gated behind PATH_INTEGRITY_EXIT_GATE_ENFORCE (leo_feature_flags, default OFF --
    // registered by database/migrations/20260823_register_path_integrity_flags.sql):
    //   OFF (default, safe): all 3 checks still RUN (for observability); a would-have-failed
    //     result from THIS choke-point logs a structured "would have REFUSED" warning AND writes a
    //     QUERYABLE system_events row (_emitPathIntegrityEvent, AC#5) -- TESTING EXEC-TO-PLAN
    //     finding C2 corrected an earlier version of this comment: the LEO_SYNTHETIC_ACTOR_FENCE_
    //     ENFORCE precedent cited here really is console-only, but AC#5 explicitly requires
    //     queryability ("so a future promotion decision can be made from real data, not
    //     guesswork"), which a log line alone cannot satisfy. Reuses the SAME system_events
    //     mechanism this module family already uses elsewhere (exit-gate-enforcer.js,
    //     thesis-kill-gate.js, this file's _emitS19HardGateEvent) -- TR-2 compliant (no parallel
    //     mechanism invented). SECURITY EXEC-TO-PLAN note (SEC-PATH-004): checkExitGates() ALSO
    //     has its own independent system_events writes (EXIT_GATE_ANOMALY / EXIT_GATE_OBSERVE_ONLY)
    //     that fire regardless of this flag -- composing it here means _advanceStage() now
    //     inherits those writes on call paths that never reached them before, in addition to this
    //     choke-point's own aggregate PATH_INTEGRITY_WOULD_BLOCK row. Advance OUTCOME is unchanged
    //     with the flag OFF (44 ventures / 325 failed-gate rows still advance freely); the
    //     observability side effect is not.
    //   ON: the same failing result actually BLOCKS, returning this function's OWN local result
    //     shape { advanced:false, blocked:true, reason:<string> } -- NEVER throws (D1 polarity;
    //     advanceStage()'s (artifact-persistence-service.js) THROW polarity would propagate
    //     through 7+ callers here that expect a result object, not an exception). Also writes the
    //     same queryable system_events row (event_type PATH_INTEGRITY_BLOCKED) -- TR-2: "both
    //     [OFF logging and ON block] write to the SAME observable shape."
    //
    // Each of the 3 composed checks carries its OWN independent flag/mode
    // (LEO_S19_EXIT_GATE_ENFORCER, default ON; LEO_THESIS_KILL_GATE, default 'observe') --
    // PATH_INTEGRITY_EXIT_GATE_ENFORCE only controls whether a FAILING RESULT from those checks
    // actually blocks HERE; it never changes their own internal enforcement. `reason` names
    // exactly which limb(s) fired so an operator (or a test) can tell checkGateDebt firing apart
    // from checkExitGates/checkThesisKillGate (D8: with LEO_THESIS_KILL_GATE unset, that limb is
    // silently inert by default -- a check that only proves "something blocked" could pass while
    // 1 of 3 limbs never actually fires).
    let pathIntegrityEnforceOn = false;
    try {
      const { data: flagRow } = await this._supabase
        .from('leo_feature_flags')
        .select('is_enabled')
        .eq('flag_key', 'PATH_INTEGRITY_EXIT_GATE_ENFORCE')
        .maybeSingle();
      pathIntegrityEnforceOn = flagRow?.is_enabled === true;
    } catch (flagErr) {
      this._logger.warn(`[Worker] PATH_INTEGRITY_EXIT_GATE_ENFORCE flag read failed (defaulting to disabled/observe-only): ${flagErr.message}`);
    }

    // SECURITY EXEC-TO-PLAN finding (SEC-PATH-001, round 2 = SEC-PATH-005): checkExitGates/
    // checkThesisKillGate/checkGateDebt are internally guarded against DB/query errors by their
    // own implementations, but checkExitGates dispatches to GATE_VERIFIERS -- an explicitly
    // growable registry -- and an uncaught throw from a future verifier would otherwise escape
    // _advanceStage() entirely (proved empirically: no result object, 7+ callers see an unhandled
    // rejection instead of {advanced,blocked,reason}), which is exactly the polarity D1 exists to
    // prevent. Round 1 wrapped all three calls in ONE shared try/catch -- SECURITY proved this
    // itself introduced a WORSE bug under enforcement (flag ON): a throw from limb 1 aborted the
    // whole block, so limbs 2/3 never ran even when one of them WOULD have genuinely blocked --
    // converting the very gate this SD exists to install into a silent bypass the moment any
    // evaluator errored. Fixed: each limb gets its OWN try/catch so a throw in one can never
    // suppress evaluation of the others, and the two flag states now diverge correctly:
    //   OFF: an errored limb contributes nothing (fail-open, matches "zero behavior change") but
    //     sibling limbs still run and can still surface a would-have-REFUSED record.
    //   ON: ANY limb erroring fails CLOSED on the error itself (matches FR-4's own fail-closed-on-
    //     evaluator-error posture under enforcement above) -- we cannot safely determine the
    //     venture is clear to advance, so block rather than silently proceed, with a reason that
    //     names the error distinctly from a genuine gate finding.
    let exitGateResult = { allowed: true, blocked_by: [] };
    let thesisKillResult = { allowed: true, blocked_by: [] };
    let gateDebtResult = { blocked: false, failedGates: [] };
    const evaluatorErrors = [];
    try {
      exitGateResult = await checkExitGates({ supabase: this._supabase, ventureId, fromStage });
      // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (VALIDATION, PLAN-VERIFY): advisory-only --
      // never blocks -- surfaces exitGateResult.provenance_warnings so the field is observable
      // somewhere rather than computed and silently dropped by this caller.
      if (exitGateResult.provenance_warnings?.length) {
        this._logger.warn(`[Worker] PATH-INTEGRITY provenance warnings (S${fromStage}->S${toStage}, venture ${ventureId}): ${exitGateResult.provenance_warnings.join(' | ')}`);
      }
    } catch (err) {
      evaluatorErrors.push({ limb: 'exit_gates', error: err });
      this._logger.warn(`[Worker] PATH-INTEGRITY checkExitGates evaluator error: ${err.message}`);
    }
    try {
      thesisKillResult = await checkThesisKillGate({ supabase: this._supabase, ventureId, fromStage, toStage });
    } catch (err) {
      evaluatorErrors.push({ limb: 'thesis_kill', error: err });
      this._logger.warn(`[Worker] PATH-INTEGRITY checkThesisKillGate evaluator error: ${err.message}`);
    }
    try {
      gateDebtResult = await checkGateDebt(this._supabase, { ventureId, fromStage });
    } catch (err) {
      evaluatorErrors.push({ limb: 'gate_debt', error: err });
      this._logger.warn(`[Worker] PATH-INTEGRITY checkGateDebt evaluator error: ${err.message}`);
    }

    if (pathIntegrityEnforceOn && evaluatorErrors.length > 0) {
      const errLimbNames = evaluatorErrors.map((e) => e.limb).join(',');
      const errDetail = evaluatorErrors.map((e) => `${e.limb}: evaluator error (${e.error.message})`).join(' | ');
      await this._emitPathIntegrityEvent(ventureId, fromStage, toStage, {
        enforced: true,
        limbs: errLimbNames,
        detail: errDetail,
      });
      this._logger.log(
        `[Worker] _advanceStage PATH-INTEGRITY CHOKE-POINT (SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001): REFUSING advance of venture ${ventureId} from Stage ${fromStage} to ${toStage} — evaluator error on limb(s): ${errLimbNames} (${errDetail}).`
      );
      return { advanced: false, blocked: true, reason: `path_integrity_check_error:${errLimbNames}` };
    }

    const firedLimbs = [];
    if (!exitGateResult.allowed) firedLimbs.push({ limb: 'exit_gates', detail: exitGateResult.blocked_by.join('; ') });
    if (!thesisKillResult.allowed) firedLimbs.push({ limb: 'thesis_kill', detail: thesisKillResult.blocked_by.join('; ') });
    if (gateDebtResult.blocked) firedLimbs.push({ limb: 'gate_debt', detail: (gateDebtResult.failedGates || []).map((g) => `${g.gate_type}(score=${g.overall_score ?? 'n/a'})`).join(', ') });

    // TESTING EXEC-TO-PLAN finding R4: reaching here means pathIntegrityEnforceOn is guaranteed
    // false whenever evaluatorErrors is non-empty (the ON+error case already returned above), so
    // an evaluator error under OFF previously surfaced ONLY as a per-limb logger.warn -- invisible
    // to the queryable AC#5 trail unless a SIBLING limb also happened to fire. A period full of
    // evaluator errors with no sibling firing would read as "clean, nothing would have blocked" to
    // whoever later aggregates PATH_INTEGRITY_WOULD_BLOCK to decide on promoting the flag -- the
    // same undercounting class SEC-PATH-007 closed for a failed insert, here for a failed
    // evaluation. Emit whenever EITHER a limb fired OR a limb errored.
    if (firedLimbs.length > 0 || evaluatorErrors.length > 0) {
      const limbNames = firedLimbs.map((f) => f.limb).join(',');
      const detail = firedLimbs.map((f) => `${f.limb}: ${f.detail}`).join(' | ');
      const errLimbNames = evaluatorErrors.map((e) => e.limb).join(',');
      const errDetail = evaluatorErrors.map((e) => `${e.limb}: evaluator error (${e.error.message})`).join(' | ');
      // TESTING EXEC-TO-PLAN finding C2: AC#5 requires the would-have-REFUSED record be QUERYABLE
      // ("so a future promotion decision can be made from real data, not guesswork") -- a
      // logger.warn line alone does not satisfy that (it's not aggregable without external log
      // infra this repo doesn't have). TR-2's precedent citation (this file's :3054-3062,
      // console-only) was correct as a description of THAT ONE precedent, but the broader,
      // established convention this module family actually uses for queryable observability is a
      // system_events row (exit-gate-enforcer.js's logObserveOnlyEvent/logAnomalyEvent,
      // thesis-kill-gate.js's logThesisKillEvent, this file's own _emitS19HardGateEvent) -- reusing
      // THAT mechanism, not inventing a new one, satisfies TR-2 correctly. Written for BOTH the
      // flag-OFF observe-only case AND the flag-ON block case (TR-2: "both write to the SAME
      // observable shape"), fail-soft so an audit-write failure never destabilizes the gate itself.
      await this._emitPathIntegrityEvent(ventureId, fromStage, toStage, {
        enforced: pathIntegrityEnforceOn,
        limbs: limbNames,
        detail,
        ...(evaluatorErrors.length > 0 ? { errorLimbs: errLimbNames, errorDetail: errDetail } : {}),
      });
      if (firedLimbs.length > 0) {
        if (pathIntegrityEnforceOn) {
          this._logger.log(
            `[Worker] _advanceStage PATH-INTEGRITY CHOKE-POINT (SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001): REFUSING advance of venture ${ventureId} from Stage ${fromStage} to ${toStage} — limb(s) fired: ${limbNames} (${detail}).`
          );
          return { advanced: false, blocked: true, reason: `path_integrity_gate_blocked:${limbNames}` };
        }
        this._logger.warn(
          `[Worker] _advanceStage PATH-INTEGRITY CHOKE-POINT (SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001): OBSERVE-ONLY -- would have REFUSED advance of venture ${ventureId} from Stage ${fromStage} to ${toStage} — limb(s) fired: ${limbNames} (${detail}), but PATH_INTEGRITY_EXIT_GATE_ENFORCE is not enabled -- advancing anyway.`
        );
      } else {
        this._logger.warn(
          `[Worker] _advanceStage PATH-INTEGRITY CHOKE-POINT (SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001): OBSERVE-ONLY -- evaluator error on limb(s) ${errLimbNames} (${errDetail}), no other limb fired -- advancing anyway, recorded for the flag-promotion decision.`
        );
      }
    }

    // SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-C (FR-1): UAT robustness choke-point. Placed LAST
    // among this function's choke-points (immediately before the point of no return below),
    // deliberately AFTER path-integrity's block -- inserting it any earlier would add a NEW
    // venture_stages read into the middle of the existing synthetic-actor/path-integrity call
    // sequence, shifting call-count-based test fixtures that target THOSE checks' own internal
    // reads (confirmed empirically: an earlier placement broke
    // stage-execution-worker-path-integrity-gate.test.js's SEC-PATH-005 case). Mirrors the
    // synthetic-actor block's shape (same feature-flag-gated observe-only-by-default pattern)
    // but is STAGE-AGNOSTIC (TR-1): rather than a hardcoded fromStage/toStage pair,
    // checkUatRobustnessGate looks up whether the CURRENT fromStage's venture_stages row is
    // marked with metadata.gates.uat_robustness_required === true. That marker does not exist
    // on any stage row yet (child B, the stage-key SSOT migration, is what will set it on the
    // new dedicated UAT stage) -- so `applies` is always false today, a safe no-op, matching
    // the fleet-safety short-circuit convention used by the earlier checks in this function.
    {
      const { checkUatRobustnessGate } = await import('./uat-robustness-gate.js');
      const uatGate = await checkUatRobustnessGate(this._supabase, ventureId, fromStage);
      if (uatGate.applies && !uatGate.satisfied) {
        let uatBindingEnabled = false;
        try {
          const { data: flagRow } = await this._supabase
            .from('leo_feature_flags')
            .select('is_enabled')
            .eq('flag_key', 'LEO_UAT_ROBUSTNESS_GATE_ENFORCE')
            .maybeSingle();
          uatBindingEnabled = flagRow?.is_enabled === true;
        } catch (flagErr) {
          this._logger.warn(`[Worker] LEO_UAT_ROBUSTNESS_GATE_ENFORCE flag read failed (defaulting to observe-only, not binding): ${flagErr.message}`);
        }
        if (uatBindingEnabled) {
          this._logger.error(
            `[Worker] _advanceStage UAT-ROBUSTNESS CHOKE-POINT (SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-C): REFUSING advance of venture ${ventureId} from Stage ${fromStage} to ${toStage} — ${uatGate.reason}`
          );
          return { advanced: false, blocked: true, reason: 'uat_robustness_gate_unmet', details: uatGate.reason };
        }
        if (uatGate.indeterminate) {
          this._logger.warn(
            `[Worker] _advanceStage UAT-ROBUSTNESS CHOKE-POINT (SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-C): OBSERVE-ONLY -- CANNOT DETERMINE for venture ${ventureId} from Stage ${fromStage} to ${toStage} (${uatGate.reason}) -- LEO_UAT_ROBUSTNESS_GATE_ENFORCE is not enabled regardless -- advancing anyway.`
          );
        } else {
          this._logger.warn(
            `[Worker] _advanceStage UAT-ROBUSTNESS CHOKE-POINT (SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-C): OBSERVE-ONLY -- would have REFUSED advance of venture ${ventureId} from Stage ${fromStage} to ${toStage} (${uatGate.reason}), but LEO_UAT_ROBUSTNESS_GATE_ENFORCE is not enabled -- advancing anyway.`
          );
        }
      }
    }
    ...
    // SD-LEO-INFRA-STAGE-WRITER-CHOKE-001: self-stamp for the canonical-writer choke, once the
    // column exists -- see stage-write-token-probe.js.
    const { error: stageUpdateError } = await this._supabase
      .from('ventures')
      .update({ current_lifecycle_stage: toStage, ...(await stageWriteTokenField(this._supabase, 'stage-execution-worker.js')) })
      .eq('id', ventureId);

    // Side-effect 1b: Record a chairman-adjudicated attempt row in eva_stage_gate_attempts.
    // SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001 (FR-1/FR-1a/FR-2): fires ONLY when
    // this transition was reached via a genuine chairman decision (source==='chairman_decision'
    // -- tagged by _handleChairmanGate(), never by an autonomy/governance/fixture auto-approval,
    // which would mislabel automated advances as chairman-adjudicated). Placed strictly AFTER
    // the UPDATE above and all of this function's own chokepoints have already passed (this is
    // the function's single point of no return), so a venture polled repeatedly while genuinely
    // blocked at an earlier chokepoint can never reach this line -- avoiding the retry-
    // amplification failure mode already observed in production on a different venture. Non-
    // fatal: a ledger-write failure must never undo or block the real advance already committed
    // above, matching the pattern at this repo's other recordGateAttempt() call sites.
    // Adversarial /ship-gate finding #1: also gated on the UPDATE above actually having
    // succeeded -- a silently-failed UPDATE must never still produce a durable "chairman
    // adjudicated" ledger row for an advance that never actually happened.
    if (!stageUpdateError && (result?._chairmanGateSource === 'chairman_decision' || chairmanGateSource === 'chairman_decision')) {
      try {
        const { recordGateAttempt } = await import('./artifact-persistence-service.js');
        // Adversarial /ship-gate finding #2: thread the real chairman_decisions.id (when
        // available) so `reasoning` references the actual decision row, per FR-1's own spec text.
        const chairmanDecisionRef = result?._chairmanDecisionId || chairmanDecisionId || null;
        await recordGateAttempt(this._supabase, {
          ventureId,
          stageNumber: fromStage,
          gateType: 'stage_gate',
          resolvedOutcome: 'chairman_adjudicated',
          reasoning: chairmanDecisionRef
            ? `Chairman-adjudicated advance from stage ${fromStage} to ${toStage} (advancementType=${advancementType}, chairman_decisions.id=${chairmanDecisionRef}).`
            : `Chairman-adjudicated advance from stage ${fromStage} to ${toStage} (advancementType=${advancementType}, chairman_decisions.id unavailable at this call site).`,
          metadata: chairmanDecisionRef ? { chairman_decision_id: chairmanDecisionRef } : null,
          evaluatedBy: 'chairman',
        });
      } catch (err) {
        this._logger.warn(`[Worker] recordGateAttempt (chairman_adjudicated) failed (non-fatal): ${err.message}`);
      }
    }
  ...
  async _autoApproveCloneVision(ventureId) {
    try {
      ...
      if (active) {
        ...
        // SD-LEO-INFRA-GOVERNANCE-ARTIFACTS-RECORD-001 FR-4: created_by (the row's original author)
        // is no longer overwritten here — an approval action must never destroy author provenance.
        const { error } = await this._supabase
          .from('eva_vision_documents')
          .update({ chairman_approved: true, chairman_approved_at: new Date().toISOString() })
      ...
      const { error } = await this._supabase
        .from('eva_vision_documents')
        .update({ status: 'active', chairman_approved: true, chairman_approved_at: new Date().toISOString() })
  ...
  /**
   * SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 (FR-1, AC#5): queryable audit trail for the
   * path-integrity choke-point -- fires whenever any of the 3 composed checks (checkExitGates/
   * checkThesisKillGate/checkGateDebt) would have refused an advance, regardless of whether
   * PATH_INTEGRITY_EXIT_GATE_ENFORCE actually blocked it (`enforced` distinguishes the two so a
   * future promotion decision can be made from real would-have-REFUSED data, not guesswork).
   * Mirrors _emitS19HardGateEvent's dual payload/details write (system_events has no event_data
   * column) and every other gate/kill backstop's fail-soft, never-throws contract in this module
   * family (exit-gate-enforcer.js, thesis-kill-gate.js).
   * @param {string} ventureId
   * @param {number} fromStage
   * @param {number} toStage
   * @param {{enforced: boolean, limbs: string, detail: string}} body
   */
  async _emitPathIntegrityEvent(ventureId, fromStage, toStage, body) {
    try {
      const payload = { venture_id: ventureId, from_stage: fromStage, to_stage: toStage, ...body };
      const stamp = new Date().toISOString();
      // SECURITY EXEC-TO-PLAN finding SEC-PATH-007: supabase-js does NOT throw on a PostgREST
      // insert failure -- it returns { error } -- so `error` must be bound and checked explicitly,
      // or a failing write is silently swallowed. AC#5 exists so a promotion decision can be made
      // "from real data" -- a silently-empty audit table would read as "never would-block" and be
      // a false green for exactly that decision. exit-gate-enforcer.js fixed this same class as
      // its own "SECURITY finding F2"; applying the same fix here rather than propagating the
      // unbound-error precedent this file's _emitS19HardGateEvent otherwise carries.
      const { error } = await this._supabase.from('system_events').insert({
        event_type: body.enforced ? 'PATH_INTEGRITY_BLOCKED' : 'PATH_INTEGRITY_WOULD_BLOCK',
        venture_id: ventureId,
        idempotency_key: `${body.enforced ? 'PATH_INTEGRITY_BLOCKED' : 'PATH_INTEGRITY_WOULD_BLOCK'}:${ventureId}:${fromStage}:${toStage}:${Date.parse(stamp)}`,
        payload,
        details: payload,
        created_at: stamp,
      });
      if (error) {
        this._logger.warn(`[Worker] PATH-INTEGRITY audit event insert failed (non-fatal): ${error.message}`);
      }
    } catch (err) {
      this._logger.warn(`[Worker] PATH-INTEGRITY audit event emit failed (non-fatal): ${err.message}`);
    }
  }
