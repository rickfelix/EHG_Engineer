  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    if (fromStage === 23 && toStage === 24) {
      ...
      if (!productReviewApproved) {
        this._logger.log(
          `[Worker] _advanceStage PRODUCT-REVIEW CHOKE-POINT (SD-LEO-INFRA-CHAIRMAN-PRODUCT-REVIEW-001): REFUSING advance of venture ${ventureId} from Stage 23 to Stage 24 — ${productReviewEvalError ? `evaluator error (${productReviewEvalError.message})` : 'no approved chairman product_review decision found'}.`
        );
