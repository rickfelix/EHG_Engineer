...
export class StageExecutionWorker {
  ...
  async _reconcileS17Archetypes() {
    try {
      ...
      for (const venture of s17Ventures) {
        // Concurrency guard: skip if generation is already running for this venture
        if (!this._s17ReconcileAttempts) this._s17ReconcileAttempts = new Map();
        if (!this._s17ActiveGenerations) this._s17ActiveGenerations = new Set();
        if (this._s17ActiveGenerations.has(venture.id)) continue;

        // Rate limit: skip if attempted in last 30 minutes (generation takes 15-20 min)
        const lastAttempt = this._s17ReconcileAttempts.get(venture.id);
        if (lastAttempt && Date.now() - lastAttempt < 1_800_000) continue;
        ...
        // SD-LEO-FEAT-GVOS-ACTIVATION-REMEDIATION-001/FR-6: gate legacy archetype-generator
        // behind s17_use_gvos_composer flag. When flag is ON for the venture, skip the
        // legacy Opus-burning generation path — GvosS17Sections renders from venture_gvos_profile instead.
        const { isEnabled: isFlagEnabledReconciler } = await import('../feature-flags/evaluator.js');
        const useGvosComposerReconciler = await isFlagEnabledReconciler('s17_use_gvos_composer', { subjectId: venture.id }).catch(() => false);
        if (useGvosComposerReconciler) {
          this._logger.log(`[Worker] S17 reconciler: ${venture.name} — FLAG ON, skipping legacy archetype-generator`);
          continue;
        }
        ...
        import('./stage-17/archetype-generator.js').then(({ generateArchetypes }) => {
          generateArchetypes(venture.id, this._supabase, reconStrategy ? { strategy: reconStrategy } : undefined)
            ...
            .finally(() => {
              this._s17ActiveGenerations.delete(venture.id);
            });
        }).catch(importErr => {
   ...
   */
  async _postStageHook_S11_GvosProfile(ventureId) {
    const logger = this._logger;

    try {
      // Idempotency: skip if profile row already exists
      const { data: existing, error: existErr } = await this._supabase
  ...
  async _postStageHook_S17_DocGen(ventureId) {
    ...
    // SD-LEO-FEAT-GVOS-ACTIVATION-REMEDIATION-001/FR-6: gate legacy archetype-generator
    // behind s17_use_gvos_composer flag. When flag is ON for the venture, skip the
    // legacy Opus-burning generation path — GvosS17Sections renders from venture_gvos_profile.
    const { isEnabled: isFlagEnabledS17 } = await import('../feature-flags/evaluator.js');
    const useGvosComposerS17 = await isFlagEnabledS17('s17_use_gvos_composer', { subjectId: ventureId }).catch(() => false);
    if (useGvosComposerS17) {
      this._logger.log(`[Worker] S17 post-stage hook: FLAG ON for venture ${ventureId} — skipping legacy archetype-generator`);
      return;
    }
