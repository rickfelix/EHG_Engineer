...
export class StageExecutionWorker {
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    //     result only logs a structured "would have REFUSED" warning -- mirrors the
    //     LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE precedent's exact mechanism above (TR-2: reuse, do
    //     not invent a parallel logging/table mechanism -- that precedent IS a console.warn, not
    //     a new DB write). Zero behavior change for existing ventures (44 ventures / 325 failed-
    //     gate rows currently advance freely under the pre-fix bypass).
    ...
    const exitGateResult = await checkExitGates({ supabase: this._supabase, ventureId, fromStage });
    const thesisKillResult = await checkThesisKillGate({ supabase: this._supabase, ventureId, fromStage, toStage });
    const gateDebtResult = await checkGateDebt(this._supabase, { ventureId, fromStage });
