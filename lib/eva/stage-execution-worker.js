...
export class StageExecutionWorker {
      ...
      while (currentStage <= MAX_STAGE && this._running) {
          ...
          if (universalApproved) {
            ...
            if (existingArt && existingArt.length > 0) {
              ...
              if (!isBlockingGate) {
                ...
                const advanceResult = await this._advanceStage(ventureId, currentStage, nextStage, { advancementType: 'pre_exec_skip', chairmanGateSource: 'chairman_decision' });
          ...
          if (approvedDecision && approvedDecision.status === 'approved') {
            ...
            if (currentStage === 3 || currentStage === 5) {
              ...
              if (auditRow) {
                ...
                // reached this tick.
                const advanceResult = await this._advanceStage(ventureId, currentStage, nextStage, { advancementType: 'pre_exec_skip_trigger', chairmanGateSource: 'chairman_decision' });
                if (advanceResult?.blocked) {
                  this._logger.log(`[Worker] Stage ${currentStage} advance blocked by artifact gate: ${advanceResult.reason}`);
                  releaseState = ORCHESTRATOR_STATES.BLOCKED;
                  lastResult = { ventureId, stageId: currentStage, status: 'artifact_precondition_unmet', ...advanceResult };
        ...
        if (govPost.isReview(currentStage) || govPost.isBlocking(currentStage)) {
          ...
          if (approvedDecision) {
            ...
            const advanceResult = await this._advanceStage(ventureId, currentStage, nextReentryStage, { durationMs: stageDurationMs, result, advancementType: 're_entry', chairmanGateSource: 'chairman_decision' });
            if (advanceResult?.blocked) {
              this._logger.log(`[Worker] Stage ${currentStage} advance blocked by artifact gate: ${advanceResult.reason}`);
              releaseState = ORCHESTRATOR_STATES.BLOCKED;
              lastResult = { ventureId, stageId: currentStage, status: 'artifact_precondition_unmet', ...advanceResult };
    ...
    try {
      ...
      if (!isNew) {
        ...
        if (existing?.status === 'approved') {
          ...
          return { blocked: false, killed: false, approved: true, source: 'chairman_decision' };
        }
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    const { result = null, durationMs = 0, advancementType = 'normal', chairmanGateSource = null } = context;
    ...
    if (result?._chairmanGateSource === 'chairman_decision' || chairmanGateSource === 'chairman_decision') {
      try {
        ...
        await recordGateAttempt(this._supabase, {
          ...
          reasoning: `Chairman-adjudicated advance from stage ${fromStage} to ${toStage} (advancementType=${advancementType}).`,
