...
export class StageExecutionWorker {
  ...
  async _postStageHook_S17_DocGen(ventureId) {
    ...
    await this._ensureS17StrategySelected(ventureId);
  }
  ...
  async _autoApproveCloneVision(ventureId) {
    try {
      ...
      // schema-lint-disable-line: ventures.seeded_from_venture_id exists live (clone-origin column from the
      // seeded-clone migration; lint snapshot is stale/code-ahead — verified live: clones 091f2889/4e710bb2/886d2dc3
      // carry it). Canonical clone marker used throughout EVA.
      const { data: venture } = await this._supabase
        .from('ventures').select('id, seeded_from_venture_id').eq('id', ventureId).maybeSingle(); // schema-lint-disable-line
