...
const DEFAULT_EXECUTION_HEARTBEAT_INTERVAL_MS = 30_000; // 30 seconds
...
export class StageExecutionWorker {
  ...
  constructor(options = {}) {
    ...
    /** @type {number} Lock refresh interval — default 1/3 of stale threshold */
    this._lockRefreshIntervalMs = options.lockRefreshIntervalMs
      ?? Math.floor(this._staleLockThresholdMs / 3);
  ...
  async _processVenture(ventureId) {
    ...
    let executionHeartbeatTimer = null;
    ...
    try {
      ...
      // 3b. Create stage execution record for observability (SD-VW-BACKEND-EXEC-RECORDS-001)
      executionRecordId = await this._createExecutionRecord(ventureId, currentStage);

      // 3c. Start execution heartbeat timer
      if (executionRecordId) {
        executionHeartbeatTimer = setInterval(() => {
          this._updateExecutionHeartbeat(executionRecordId).catch(err =>
            this._logger.warn(`[Worker] Execution heartbeat failed: ${err.message}`)
          );
        }, DEFAULT_EXECUTION_HEARTBEAT_INTERVAL_MS);
      }

      // 3d. Start lock refresh timer (SD-VW-BACKEND-LOCK-HEARTBEAT-001)
      lockRefreshTimer = setInterval(() => {
        this._refreshLockTimestamp(ventureId, lockId, controller).catch(err =>
          this._logger.warn(`[Worker] Lock refresh error: ${err.message}`)
        );
      }, this._lockRefreshIntervalMs);
      ...
      while (currentStage <= MAX_STAGE && this._running) {
        ...
        if (currentStage >= MAX_STAGE || !result.nextStageId) {
          ...
          if (executionRecordId) await this._finalizeExecutionRecord(executionRecordId, 'succeeded', null);
          executionRecordId = null; // Prevent double finalization in finally
      ...
      // Finalize execution record (SD-VW-BACKEND-EXEC-RECORDS-001)
      if (executionRecordId) {
        const finalStatus = releaseState === ORCHESTRATOR_STATES.FAILED ? 'failed' : 'succeeded';
        const errorMsg = releaseState === ORCHESTRATOR_STATES.FAILED
          ? (lastResult?.error || lastResult?.errors?.[0]?.message || 'Unknown error')
          : null;
        await this._finalizeExecutionRecord(executionRecordId, finalStatus, errorMsg);
      }
  ...
  // ── Stage Execution Records (SD-VW-BACKEND-EXEC-RECORDS-001) ──

  /**
   * Create a stage_executions record when beginning to process a venture stage.
   * Non-blocking: returns null on failure so stage processing continues.
   *
   * @param {string} ventureId
   * @param {number} lifecycleStage
   * @returns {Promise<string|null>} Execution record ID or null
   */
  async _createExecutionRecord(ventureId, lifecycleStage) {
    try {
      const { data, error } = await this._supabase
      ...
      if (error) {
        this._logger.warn(`[Worker] Execution record creation failed: ${error.message}`);
        return null;
      }

      this._logger.log(`[Worker] Execution record created: ${data.id} (venture=${ventureId}, stage=${lifecycleStage})`);
      return data.id;
    } catch (err) {
      this._logger.warn(`[Worker] Execution record creation error: ${err.message}`);
      return null;
    }
  }
   ...
   * Non-blocking: errors are logged but do not affect stage processing.
   *
  ...
  async _updateExecutionHeartbeat(executionId) {
    ...
    if (error) {
      this._logger.warn(`[Worker] Execution heartbeat update failed: ${error.message}`);
    }
  }

  /**
   * Finalize an execution record with terminal status and optional error.
   * Non-blocking: errors are logged but do not affect lock release.
   *
   * @param {string} executionId
   * @param {'succeeded'|'failed'|'timed_out'} status
   * @param {string|null} errorMessage
   */
  async _finalizeExecutionRecord(executionId, status, errorMessage) {
    try {
      const update = {
        status,
      ...
      if (error) {
        this._logger.warn(`[Worker] Execution record finalization failed: ${error.message}`);
      } else {
        this._logger.log(`[Worker] Execution record finalized: ${executionId} → ${status}`);
      }
    } catch (err) {
      this._logger.warn(`[Worker] Execution record finalization error: ${err.message}`);
    }
  }
  ...
  /**
   * Refresh the lock timestamp to prevent false stale lock detection.
   * Uses atomic conditional UPDATE with lock_id verification.
   * If the lock was stolen (0 rows matched), aborts processing.
   *
   * @param {string} ventureId
   * @param {string} lockId
   * @param {AbortController} controller
   */
  async _refreshLockTimestamp(ventureId, lockId, controller) {
    try {
      const { data, error } = await this._supabase
        .from('ventures')
      ...
      if (error) {
        this._logger.warn(`[Worker] Lock refresh DB error for ${ventureId}: ${error.message}`);
        return; // Graceful degradation — continue processing
      }

      if (!data || data.length === 0) {
        // Lock was stolen — abort processing
        this._logger.error(`[Worker] Lock stolen for ${ventureId} — aborting processing`);
        controller.abort();
        return;
      }

      // Lock refreshed successfully (silent — too noisy to log every interval)
    } catch (err) {
      this._logger.warn(`[Worker] Lock refresh error for ${ventureId}: ${err.message}`);
