...
export class StageExecutionWorker {
  ...
  async _advanceStage(ventureId, fromStage, toStage, context = {}) {
    ...
    // SECURITY EXEC-TO-PLAN finding (SEC-PATH-001): checkExitGates/checkThesisKillGate/
    // checkGateDebt are internally guarded against DB/query errors by their own implementations,
    // but checkExitGates dispatches to GATE_VERIFIERS -- an explicitly growable registry -- and an
    // uncaught throw from a future verifier would otherwise escape _advanceStage() entirely
    // (proved empirically: no result object, 7+ callers see an unhandled rejection instead of
    // {advanced,blocked,reason}), which is exactly the polarity D1 exists to prevent. Wrap the
    // composition itself so a throw from ANY of the three checks can never escape this function --
    // matching every other backstop above's contract, and matching PATH_INTEGRITY_EXIT_GATE_ENFORCE
    // OFF's own "zero behavior change" promise (an evaluator error must fall through to the advance,
    ...
    } catch (composedCheckErr) {
      this._logger.warn(`[Worker] PATH-INTEGRITY composed-check evaluator error (fail-open, never throws): ${composedCheckErr.message}`);
      // exitGateResult/thesisKillResult/gateDebtResult stay at their safe "nothing fired" defaults.
    }
