...
export class StageExecutionWorker {
  ...
  async _postStageHook_S15_StitchProvision(ventureId) {
    // SD-LEO-FIX-GOOGLE-STITCH-PIPELINE-001-C: Route through adapter with fallback
    const { provision, logStitchEvent } = await import('./bridge/stitch-adapter.js');
    ...
    const result = await provision(ventureId, {}, s15Work?.advisory_data || {});
    ...
    if (result?.status === 'unavailable') {
      ...
      this._logger.warn(`[Worker] S15 Stitch unavailable (${result.reason}), continuing with ASCII wireframes`);
      await this._supabase
        .from('venture_stage_work')
        .update({
          advisory_data: {
  ...
  async _postStageHook_S17_DocGen(ventureId) {
    ...
    // SD-LEO-FIX-GOOGLE-STITCH-PIPELINE-001-C: Export via adapter with fallback
    try {
      const { exportScreens, logStitchEvent } = await import('./bridge/stitch-adapter.js');
      const exportResult = await exportScreens(ventureId, null);
