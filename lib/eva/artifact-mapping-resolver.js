...
import { ARTIFACT_TYPES, ARTIFACT_TYPE_BY_STAGE, getStageForArtifactType } from './artifact-types.js';

// ── Helper: build mapping entry with stage derived from artifact type ─
// SD-RESTRUCTURE-STAGE-15-MOVE-ORCH-001-C: No hardcoded lifecycle_stage values.
function artEntry(artifact_type) {
  return { artifact_type, lifecycle_stage: getStageForArtifactType(artifact_type) };
}

// ── Default Mapping Configuration ────────────────────────────────────
// Universal artifacts attach to every SD regardless of layer
const UNIVERSAL_ARTIFACTS = [
  artEntry(ARTIFACT_TYPES.IDENTITY_PERSONA_BRAND),
  artEntry(ARTIFACT_TYPES.BLUEPRINT_DATA_MODEL),
  artEntry(ARTIFACT_TYPES.BLUEPRINT_TECHNICAL_ARCHITECTURE),
];

// Layer-specific mappings: which artifacts are required for each SD layer
const LAYER_SPECIFIC_ARTIFACTS = {
  data: [
    artEntry(ARTIFACT_TYPES.BLUEPRINT_ERD_DIAGRAM),
    artEntry(ARTIFACT_TYPES.BLUEPRINT_SCHEMA_SPEC),
  ],
  api: [
    artEntry(ARTIFACT_TYPES.BLUEPRINT_API_CONTRACT),
  ],
  ui: [
    artEntry(ARTIFACT_TYPES.BLUEPRINT_WIREFRAMES),
    artEntry(ARTIFACT_TYPES.IDENTITY_BRAND_GUIDELINES),
  ],
  tests: [
    artEntry(ARTIFACT_TYPES.BLUEPRINT_USER_STORY_PACK),
  ],
};

// Supplemental artifacts: useful context but not blocking
const SUPPLEMENTAL_ARTIFACTS = [
  artEntry(ARTIFACT_TYPES.TRUTH_COMPETITIVE_ANALYSIS),
  artEntry(ARTIFACT_TYPES.ENGINE_PRICING_MODEL),
  artEntry(ARTIFACT_TYPES.BLUEPRINT_RISK_REGISTER),
  artEntry(ARTIFACT_TYPES.BLUEPRINT_PRODUCT_ROADMAP),
  artEntry(ARTIFACT_TYPES.IDENTITY_GTM_SALES_STRATEGY),
  artEntry(ARTIFACT_TYPES.TRUTH_FINANCIAL_MODEL),
  artEntry(ARTIFACT_TYPES.ENGINE_BUSINESS_MODEL_CANVAS),
];
