...
/**
 * FR-7 AC-1/AC-4: asserts a query result actually affected rows, not merely that it didn't throw.
 * RISK sub-agent's false-pass mode #1 (zero-row sync): an UPDATE whose WHERE clause matches 0
 * rows (e.g. the id-vs-venture_id key trap, false-pass mode #2) never evaluates a CHECK
 * constraint and reports success -- absence of a thrown error is not evidence anything happened.
 * @param {{rowCount?: number}} result the pg query result object
 * @param {number} [expectedMin=1] minimum acceptable affected-row-count
 * @throws if result.rowCount is not a number >= expectedMin
 */
export function assertRowCount(result, expectedMin = 1) {
  const n = result?.rowCount;
  if (typeof n !== 'number' || n < expectedMin) {
    throw new Error(`ROW_COUNT_ASSERTION_FAILED: expected affected-row-count >= ${expectedMin}, got ${n ?? '(no rowCount on result)'} -- a 0-row UPDATE/DELETE silently evaluates no constraint and proves nothing.`);
  }
  return true;
}
