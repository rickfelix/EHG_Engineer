...
// tokens + motion grammar + derive the audit-instruction block, once per child, for the
// 'ui' leaf.
import { resolveLockedDesignTokens, resolveMotionGrammar } from './bridge/design-token-resolver.js';
import { buildDesignInstructionBlock, loadSharedDesignPrompts } from './bridge/design-input-instructions.js';
import {
  generateSDKey,
  generateChildKey,
...
async function createGrandchildren({
  ...
  // SD-LEO-FEAT-ROUTE-VENTURE-DESIGN-001 (FR-1/FR-3): resolve the venture's locked design
  // tokens + motion grammar ONCE per child (not per grandchild) and derive the instruction
  // block, but only when a 'ui' leaf is actually being created — a venture with no ui layer
  // in this child (e.g. a pure data/api child) pays zero extra DB-lookup cost.
  //
  // TS-2 / AC-2 (zero regression): the instruction checklist enforces conformance against a
  // LOCKED design system -- with no locked source there is nothing for it to check against,
  // so it is gated on hasLockedSource (source !== 'none'), never emitted unconditionally.
  // Without this gate every UI leaf grew by the ~16KB checklist even for token-less ventures.
  let uiDesignInput = null;
  if (cappedLayers.some((l) => l?.key === 'ui')) {
    const { source, tokens } = await resolveLockedDesignTokens(ventureContext?.id || null, supabase);
    const hasLockedSource = source !== 'none';
    const instructionBlock = hasLockedSource ? (buildDesignInstructionBlock(loadSharedDesignPrompts()) || null) : null;
    const motionGrammar = await resolveMotionGrammar(ventureContext?.id || null, supabase);
    uiDesignInput = { designTokens: tokens, designInstructionBlock: instructionBlock, motionGrammar };
  }
