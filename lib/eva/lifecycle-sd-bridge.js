 ...
 */
// eva-logger-lint-ignore: pre-existing dependency-injected `logger` parameter pattern
// (a caller-supplied {log,warn,error} object, not lib/logger.js's createLogger), predates
// the EVA logging standard (docs/reference/eva-logging-standard.md); SD-LEO-INFRA-FILING-
// TOOLS-ENFORCE-001's edits here are metadata stamps on existing insert objects, not new
// logging surface -- retrofitting createLogger/OrchestratorTracer across this file is out
// of scope for that SD.
...
// SD-LEO-INFRA-PRE-BUILD-SUB-001 (U1 FR-001): panel-enriched leaf content. Returns the
...
import { sampleDesignInfluence } from './bridge/design-reference-sampler.js';
...
// SD-LEO-INFRA-VENTURE-JOURNEY-UAT-001 FR-1/FR-1b: orchestrator journey_steps, derived
// from the venture's Stage-15 blueprint_user_journey artifact (never from acceptance criteria).
import { deriveJourneySteps } from './bridge/orchestrator-journey-steps.js';
...
// QF-20260828-605: identify mandatory portfolio-default-capability payloads so the
// amplification cap never silently drops one.
...
// QF-20260828-605: a payload is "mandatory" when its title matches a portfolio-default
// capability name (the same names Stage 19 is instructed to always emit). Everything
// else is "optional" for amplification-cap purposes.
const MANDATORY_CAPABILITY_NAMES = new Set(
  EHG_VENTURE_DEFAULT_CAPABILITIES.map((c) => c.name.trim().toLowerCase()),
);
function isMandatoryPayload(p) {
  return MANDATORY_CAPABILITY_NAMES.has(String(p?.title ?? '').trim().toLowerCase());
}

/**
 * QF-20260828-605: apply the amplification cap, dropping OPTIONAL payloads first.
 * ERRORS (never warns-and-silently-drops) when a mandatory capability payload would
 * have to be cut to meet the cap. Returns { payloads, dropped } — `dropped` is [] when
 * the cap never triggered.
 * @param {Array<{title?:string,type?:string}>} payloads
 * @returns {{payloads: Array, dropped: Array<{title:string,type:string,reason:string}>}}
 */
export function applyAmplificationCap(payloads) {
  if (payloads.length <= MAX_CHILDREN_PER_ORCHESTRATOR) return { payloads, dropped: [] };
  const excess = payloads.length - MAX_CHILDREN_PER_ORCHESTRATOR;
  const optionalIdx = payloads.map((p, i) => (isMandatoryPayload(p) ? -1 : i)).filter((i) => i >= 0);
  if (optionalIdx.length < excess) {
    throw new ServiceError(
      'AMPLIFICATION_CAP_MANDATORY_DROP',
      `Amplification cap (${MAX_CHILDREN_PER_ORCHESTRATOR}) requires dropping ${excess} item(s) but only ` +
      `${optionalIdx.length} optional item(s) exist — a mandatory portfolio-default-capability item would be ` +
      'silently cut. Reduce sprint scope or raise MAX_CHILDREN_PER_ORCHESTRATOR.',
      'LifecycleSDBridge',
    );
  }
  const dropIdx = new Set(optionalIdx.slice(optionalIdx.length - excess));
  const dropped = payloads.filter((p, i) => dropIdx.has(i)).map((p) => ({ title: p.title, type: p.type, reason: 'amplification_cap' }));
  return { payloads: payloads.filter((p, i) => !dropIdx.has(i)), dropped };
}
...
/**
 * SD-LEO-INFRA-VENTURE-JOURNEY-UAT-001 FR-1b: fetch the venture's current
 * blueprint_user_journey artifact (Stage 15 output) for orchestrator journey_steps
 * derivation. `content` is stored as a JSON-encoded STRING on this artifact_type
 * (confirmed against live venture_artifacts rows), not jsonb — must be parsed.
 * Fail-soft: any error or absent artifact returns null (never throws) so a
 * missing/malformed journey artifact can never block orchestrator creation.
 * @param {Object} supabase
 * @param {string} ventureId
 * @returns {Promise<{journeys?: Array}|null>}
 */
export async function fetchCurrentJourneyArtifact(supabase, ventureId) {
  if (!ventureId) return null;
  try {
    const { data, error } = await supabase
...
async function insertSDIdempotent(supabase, row) {
  ...
  // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: identical guarded-stamp pattern as
  // sourced_by above -- the largest-volume bypass path (~30 SDs/30d), never silently
  // uncategorized. Never overwrites an already-set value (mirrors the sourced_by guard).
  if (row.metadata && row.metadata.criticality === undefined) {
    row.metadata.criticality = 'later';
    row.metadata.criticality_reason = 'auto-generated by lifecycle-sd-bridge, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4';
  }
...
export async function convertSprintToSDs(params, deps = {}) {
  ...
  // Amplification cap: limit children (US-004). QF-20260828-605: drop OPTIONAL items
  // first (never rely on array position); ERROR (never warn-and-silently-drop) when a
  // MANDATORY capability payload would be cut.
  const requestedCount = payloads.length;
  const { payloads: cappedPayloads, dropped: droppedPayloads } = applyAmplificationCap(payloads);
  payloads = cappedPayloads;
  if (droppedPayloads.length) {
    logger.warn(`[LifecycleSDBridge] Amplification cap hit: requested ${requestedCount} children, capped at ${MAX_CHILDREN_PER_ORCHESTRATOR}. Dropped (optional-only): ${droppedPayloads.map((d) => d.title).join(', ')}`);
  }
  ...
  try {
    ...
    // SD-LEO-INFRA-VENTURE-JOURNEY-UAT-001 FR-1/FR-1b: journey_steps is orchestrator-level
    // (venture-scoped, not per sprint-item), so it is fetched directly here rather than routed
    // through sd_bridge_payloads. Stage 19 (analyzeStage19) is a pure function with no DB access
    // and cannot be the write site — this insert is the earliest point the orchestrator SD row
    // (and therefore its metadata) exists. Fail-soft: never blocks orchestrator creation.
    let journeySteps = null;
    try {
      const journeyArtifactContent = await (deps.fetchJourneyArtifact || fetchCurrentJourneyArtifact)(supabase, ventureContext?.id);
      journeySteps = deriveJourneySteps(journeyArtifactContent);
    } catch (journeyErr) {
      logger.warn(`[LifecycleSDBridge] FR-1b: journey_steps derivation skipped (${journeyErr.message})`);
    }
    ...
    const { created: orchCreated, error: orchError } = await insertSDIdempotent(supabase, {
        ...
        metadata: {
          ...
          // SD-LEO-INFRA-VENTURE-JOURNEY-UAT-001 FR-1/FR-1b: absent (not []) when the venture has
          // no blueprint_user_journey artifact yet — presence is what FR-3's gate keys on.
          ...(journeySteps ? { journey_steps: journeySteps } : {}),
          ...
          // QF-20260828-605: bridge receipt — which payloads the amplification cap dropped, and why.
          amplification_cap: droppedPayloads.length ? { triggered: true, dropped: droppedPayloads } : { triggered: false, dropped: [] },
    ...
    return {
      ...
      buildReview,
      // QF-20260828-605: amplification-cap drop list (empty when the cap never triggered).
      droppedPayloads,
    };
  } catch (err) {
...
async function createGrandchildren({
  ...
  if (cappedLayers.some((l) => l?.key === 'ui')) {
    ...
    // SD-LEO-FEAT-CLOSE-DISTINCTIVENESS-GAP-001 (FR-1): per-venture award-library influence —
    // seeded, per-dimension, minority weight. sampleDesignInfluence is fail-soft by contract
    // (null on any error/empty library), so the block simply omits the section when absent.
    const awardInfluence = await sampleDesignInfluence({
      ventureId: ventureContext?.id || null,
      archetype: ventureContext?.archetype || null,
      supabase,
    });
    const instructionBlock = buildDesignInstructionBlock(sharedDesignPrompts, undefined, { awardInfluence });
...
async function rollbackCreatedRecords(supabase, createdIds, logger) {
  ...
  for (const id of createdIds) {
    const { error } = await supabase
      .from('strategic_directives_v2')
      .update({ status: 'cancelled', cancellation_reason: 'lifecycle-sd-bridge rollback: hierarchy creation failed', updated_at: new Date().toISOString(), lifecycle_write_token: 'lifecycle-sd-bridge.js' })
      .eq('id', id);
    if (error) {
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      metadata: {
        ...
        created_at: new Date().toISOString(),
        // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: dormant path (0 SDs/30d/180d/all-time
        // measured), stamped for completeness alongside the primary insertSDIdempotent() path.
        criticality: 'later',
        criticality_reason: 'auto-generated by lifecycle-sd-bridge venture-expansion, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4',
      },
    });
