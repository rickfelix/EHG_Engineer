...
export async function wouldCreateCycle(supabase, dependentId, providerId) {
  ...
  // Load all edges at once for efficient traversal. Paginated (FR-6 batch 7): a capped
  // edge read would silently drop graph edges and miss cycles.
  let allEdges;
  try {
    allEdges = await fetchAllPaginated(() => supabase
      .from('venture_dependencies')
      .select('id, dependent_venture_id, provider_venture_id')
      .order('id', { ascending: true }));
  } catch (e) {
    throw new ServiceError('GRAPH_LOAD_FAILED', `Failed to load dependency graph: ${e.message}`, 'DependencyManager');
  }
