...
export async function generateRefinedVariants(ventureId, screenName, selectedHtmls, tokens, supabase, options = {}) {
  ...
  // SD-LEO-FEAT-AUTHOR-VENTURE-DESIGN-001 (FR-4): refined variant HTML generation now runs
  // on the FABLE design-authoring tier via the 'design-generation' purpose (claude-fable-5,
  // env CLAUDE_MODEL_DESIGN_GENERATION). CLAUDE_MODEL_S17_GENERATION keeps highest precedence.
  // The design-fidelity AUDIT slot is untouched (scorer != builder).
  const generationModel = process.env.CLAUDE_MODEL_S17_GENERATION || getClaudeModel('design-generation');
