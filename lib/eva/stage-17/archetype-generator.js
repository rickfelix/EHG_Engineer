...
export async function generateArchetypeVariants(ventureId, screenName, screenId, supabase, options = {}) {
  ...
  // SD-LEO-FEAT-AUTHOR-VENTURE-DESIGN-001 (FR-4): the Fable re-point, live — archetype
  // generation runs on the 'design-generation' purpose (claude-fable-5, env
  // CLAUDE_MODEL_DESIGN_GENERATION). CLAUDE_MODEL_S17_ARCHETYPE_GENERATION keeps precedence.
  const generationModel = process.env.CLAUDE_MODEL_S17_ARCHETYPE_GENERATION || getClaudeModel('design-generation');
