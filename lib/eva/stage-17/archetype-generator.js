...
// SD-LEO-INFRA-STAGE-ARCHETYPE-GENERATION-001 ARMs B + E
import { attemptVariantWithRetry } from './variant-retry.js';
 ...
 */
// SD-MAN-INFRA-LEO-GATE-IMPROVEMENTS-001: Field path verification reminder.
// Before modifying extraction paths below, query real data first:
//   SELECT artifact_data FROM venture_artifacts WHERE artifact_type = 'X' LIMIT 1
// camelCase/snake_case mismatches caused S17 brand color injection to silently fail.
...
export async function generateArchetypes(ventureId, supabase, options = {}) {
  ...
  // Start a 30s heartbeat so the frontend (ARM D) and the resume endpoint
  // (ARM F) can detect a frozen worker. Stopped in the try/finally that
  // wraps the screen loop below.
  const heartbeat = startHeartbeatWriter(supabase, ventureId, {
    initialState: { phase: 'starting', totalScreens },
  });
  ...
  for (let screenIdx = 0; screenIdx < screenList.length; screenIdx++) {
    ...
    // SD-LEO-INFRA-STAGE-ARCHETYPE-GENERATION-001 ARM B/C: track per-screen failures
    let failedVariantCount = 0;
    heartbeat.update({ phase: 'screen', screenIdx: screenIdx + 1, totalScreens, screenTitle });
    ...
    for (let i = 0; i < variantCount; i++) {
      ...
      const llmOpts = { stream: true, timeout: 300000, cacheTTLMs: 0, maxTokens: 32768, purpose: 'content-generation', callerLabel: `s17:${screenTitle}:v${i + 1}` };

      heartbeat.update({ phase: 'variant', screenIdx: screenIdx + 1, variantIdx: i + 1, totalVariants: variantCount });

      // SD-LEO-INFRA-STAGE-ARCHETYPE-GENERATION-001 ARM B: bounded retry around the
      // delivery call. LLMStreamStalled (ARM A) and other transient errors are
      // retried once; on exhaustion we write s17_variant_failed and skip the
      // variant rather than killing the whole screen.
      const variantOutcome = await attemptVariantWithRetry(async () => {
        const r = await client.complete(systemPrompt, userContent, llmOpts);
        return r?.content ?? String(r);
      }, {
        onAttempt: ({ attempt, error, willRetry }) => {
          const stalled = error?.name === 'LLMStreamStalled';
          console.warn(`[archetype-generator] │   v${i + 1}: attempt ${attempt + 1} failed (${stalled ? 'LLMStreamStalled' : (error?.message || 'unknown')}); ${willRetry ? 'retrying' : 'exhausted'}`);
          heartbeat.update({
            phase: willRetry ? 'recovering' : 'variant_failed',
      ...
      if (!variantOutcome.ok) {
        const err = variantOutcome.error;
        const errName = err?.name || 'Error';
        const errMsg = err?.message || 'unknown';
        console.warn(`[archetype-generator] │   v${i + 1}: GIVING UP after ${variantOutcome.attempts} attempts — writing s17_variant_failed (${errName})`);
        await writeArtifact(supabase, {
          ventureId,
          lifecycleStage: 17,
          artifactType: 's17_variant_failed',
          title: `${screenTitle} — Failed Variant ${i + 1}`,
          content: JSON.stringify({ screenName: screenTitle, variantIndex: i + 1, error: { name: errName, message: errMsg } }),
          artifactData: {
            screenId,
            variantIndex: i + 1,
            error: { name: errName, message: errMsg, callerLabel: err?.callerLabel || null },
            attemptsExhausted: variantOutcome.attempts,
          },
          qualityScore: null,
          validationStatus: 'failed',
          source: 'stage-17-archetype-generator',
          metadata: { screenId, variantIndex: i + 1, deviceType, errorName: errName },
        });
        failedVariantCount += 1;
        await updateProgress({ type: 'variant_failed', screen: screenTitle, screenIdx: screenIdx + 1, variant: i + 1, totalVariants: variantCount, errorName: errName, attempts: variantOutcome.attempts, timestamp: new Date().toISOString() });
        continue;
      }

      archetypeHtml = variantOutcome.result;
    ...
    const artifactId = await writeArtifact(supabase, {
      ...
      metadata: {
        ...
        // SD-LEO-INFRA-STAGE-ARCHETYPE-GENERATION-001 ARM C: per-screen failure
        // marker the frontend (ARM D, PR4) reads to decide whether to surface a
        // "regenerate failed variants" affordance. Distinct from per-variant
        // WIP markers (PAT-PERSIST-CHECKPOINT-001) — see stage17-contracts.md §5.1.
        has_failed_variants: failedVariantCount > 0,
  ...
  return { screenCount: totalScreens, artifactIds };
    ...
    // SD-LEO-INFRA-STAGE-ARCHETYPE-GENERATION-001 ARM E: always stop the
    // heartbeat — covers normal completion, mid-run cancellation, and
    // unhandled exceptions out of the variant loop.
