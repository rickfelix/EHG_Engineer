...
function extractPositioning(identity) {
  ...
  if (data.positioning) return data.positioning;
  if (data.tagline) return data.tagline;
  if (data.value_proposition) return data.value_proposition;
  if (data.brand_essence) return data.brand_essence;
  // Check nested brandExpression (identity_naming_visual structure)
  if (data.brandExpression?.elevator_pitch) return data.brandExpression.elevator_pitch;
  if (data.brandExpression?.tagline) return data.brandExpression.tagline;
  // Check decision context
  if (data.decision?.workingTitle) return data.decision.workingTitle;
  return null;
}
...
function extractVoice(identity) {
  ...
  // Check nested brandExpression for messaging pillars
  if (Array.isArray(data.brandExpression?.messaging_pillars)) {
    return data.brandExpression.messaging_pillars.slice(0, 3).join(', ');
  }
  // Check visualIdentity for typography direction
  if (data.visualIdentity?.typography) return `Typography: ${data.visualIdentity.typography}`;
  return null;
}
...
function extractContext(identity, tokenManifest) {
  ...
  // Pull from visualIdentity structure
  if (data.visualIdentity?.colorPalette) {
    const palette = Array.isArray(data.visualIdentity.colorPalette)
      ? data.visualIdentity.colorPalette.map(c => typeof c === 'object' ? (c.hex ?? c.name ?? c.color ?? JSON.stringify(c)) : c).join(', ')
      : String(data.visualIdentity.colorPalette);
    parts.push(`Palette: ${palette}`);
  }
  if (data.visualIdentity?.imageryGuidance) parts.push(`Imagery: ${data.visualIdentity.imageryGuidance}`);
  if (data.namingStrategy?.approach) parts.push(`Brand approach: ${data.namingStrategy.approach}`);
