...
export async function resumeIncompleteArchetypeJobs() {
  ...
  for (const wf of wireframes) {
    ...
    // Only resume for ventures actually at S17 (not past it)
    const { data: venture } = await supabase
      .from('ventures')
      .select('current_lifecycle_stage')
      .eq('id', wf.venture_id)
      .single();
    if (!venture || venture.current_lifecycle_stage !== 17) continue;

    // Count completed S17 archetypes (real ones with screenId, not stage analysis)
    const { data: archetypeArts } = await supabase
      .from('venture_artifacts')
      .select('metadata')
    ...
    const realCount = (archetypeArts ?? []).filter(a => a.metadata?.screenId).length;

    // Only resume if genuinely incomplete (real archetypes < total screens)
    if (realCount < totalScreens) {
      incompleteJobs.push({
