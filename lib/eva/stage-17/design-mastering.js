...
export async function masterDesign(supabase, ventureId, approvedHtml, metadata = {}) {
  ...
  const normalization = analyzeNormalizationNeeds(approvedHtml, designSystem);
  designSystem.normalization = normalization;
  ...
  const artifactId = await writeArtifact(supabase, {
    ...
    metadata: {
      ...
      normalization_issues: normalization.issues.length,
    },
  });
...
/**
 * Analyze normalization needs: detect inconsistencies in nav, buttons, spacing, and colors.
 * Reports what SHOULD be normalized for cross-screen coherence (S18 build can apply these).
 *
 * @param {string} html
 * @param {object} designSystem - extracted design system
 * @returns {{ issues: Array<{type: string, detail: string, recommendation: string}>, coherence_score: number }}
 */
function analyzeNormalizationNeeds(html, designSystem) {
  const issues = [];

  // Nav consistency: check if nav elements use consistent patterns
  const navPatterns = html.match(/<nav[^>]*class="([^"]+)"/gi) ?? [];
  const uniqueNavClasses = new Set(navPatterns.map(n => n.match(/class="([^"]+)"/)?.[1]));
  if (uniqueNavClasses.size > 1) {
    issues.push({ type: 'nav', detail: `${uniqueNavClasses.size} different nav class patterns found`, recommendation: 'Standardize to single nav component class' });
  }

  // Button consistency: check if buttons use consistent sizing/styling
  const btnClasses = html.match(/<button[^>]*class="([^"]+)"/gi) ?? [];
  const uniqueBtnStyles = new Set(btnClasses.map(b => b.match(/class="([^"]+)"/)?.[1]));
  if (uniqueBtnStyles.size > 2) {
    issues.push({ type: 'buttons', detail: `${uniqueBtnStyles.size} different button styles (expected max 2: primary + secondary)`, recommendation: 'Consolidate to primary and secondary button variants' });
  }

  // Spacing consistency: check for non-grid-aligned spacing values
  const nonAligned = designSystem.spacing_patterns.filter(s => !s.grid_aligned);
  if (nonAligned.length > 0) {
    issues.push({ type: 'spacing', detail: `${nonAligned.length} spacing values not on 4px grid: ${nonAligned.map(s => s.value_px + 'px').join(', ')}`, recommendation: 'Snap all spacing to 4px grid multiples' });
  }

  // Color consistency: check for colors used outside token system
  const rawHexColors = html.match(/#[0-9a-fA-F]{3,8}(?=[;}\s"'])/g) ?? [];
  const tokenUsages = designSystem.color_map.reduce((sum, c) => sum + c.usage_count, 0);
  if (rawHexColors.length > 0 && tokenUsages > 0) {
    const rawRatio = rawHexColors.length / (rawHexColors.length + tokenUsages);
    if (rawRatio > 0.2) {
      issues.push({ type: 'colors', detail: `${rawHexColors.length} raw hex colors vs ${tokenUsages} token usages (${Math.round(rawRatio * 100)}% raw)`, recommendation: 'Replace raw hex values with CSS custom property tokens' });
    }
  }

  const maxIssues = 4;
  const coherence_score = Math.round(((maxIssues - Math.min(issues.length, maxIssues)) / maxIssues) * 100);
