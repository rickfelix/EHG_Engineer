// eva-logger-lint-ignore: pure static data (frozen arrays) plus pure lookup/diff
// functions -- no I/O, no async work, no runtime event worth structured logging.
// The one caller that surfaces a finding (wire-check-gate.js) already reports it
// through the gate's own warnings[] channel.
/**
 ...
 *     33 import statements across the 27 files (stage-02/15/16 import more than
 ...
 *
 * (b) CROSS_CUTTING_DISPATCH -- production (non-test) files that import an
 *     analysis-step module directly, outside any stage-NN.js chain. This list is a
 *     BEST-EFFORT enumeration from the discovery scans performed to date, NOT a
 *     provably-exhaustive one -- an adversarial review of this SD found 3 more
 *     entries (server/routes/stage18.js, scripts/backfill-wireframe-screen-
 *     surfaces.mjs, scripts/eva/dry-run-stage24-checklist.mjs) beyond the first 4
 *     found under lib/eva/ alone, by broadening the search to server/ and
 *     scripts/. An incomplete list here is tolerated BY DESIGN: this feeds an
 *     ADVISORY-ONLY report (see module intro above), never a blocking check, and
 *     the coverage limb only ever reports a stray extra entry as MULTIPLE_ENTRIES
 *     (cosmetic), never a false failure. Do not read this list as a completeness
 *     guarantee -- widen it opportunistically as gaps are found, same as any other
 *     living registry.
 *
...
export const CROSS_CUTTING_DISPATCH = Object.freeze([
  ...
  { consumerFile: 'server/routes/stage18.js', analysisStepFile: 'stage-18-marketing-copy.js' },
  { consumerFile: 'scripts/backfill-wireframe-screen-surfaces.mjs', analysisStepFile: 'stage-15-wireframe-generator.js' },
  { consumerFile: 'scripts/eva/dry-run-stage24-checklist.mjs', analysisStepFile: 'stage-23-launch-readiness.js' },
]);
