...
const TEMPLATE = {
  ...
  computeDerived(data, { logger = console } = {}) {
    ...
    // Build readiness decision object
    const hasCriticalBlockers = (data.blockers || []).some(b => b.severity === 'critical');
    let buildReadinessDecision;
    if (hasCriticalBlockers) {
      buildReadinessDecision = 'no_go';
    } else if (blocker_count > 0 || readiness_pct < 100) {
      buildReadinessDecision = 'conditional_go';
    } else {
      buildReadinessDecision = 'go';
    }

    const buildReadiness = {
      decision: buildReadinessDecision,
      rationale: buildReadinessDecision === 'go'
        ? 'All checklist items complete, no blockers'
        : buildReadinessDecision === 'no_go'
          ? `Critical blockers present (${blocker_count} blocker(s))`
          : `Readiness at ${readiness_pct}% with ${blocker_count} non-critical blocker(s)`,
    };
