...
import { analyzeStage15 as analyzeRiskRegister } from './analysis-steps/stage-15-risk-register.js';
  ...
  schema: {
    ...
    risks: {
      ...
      items: {
        ...
        priority: { type: 'enum', values: ['immediate', 'short_term', 'long_term'], required: true },
...
TEMPLATE.analysisStep = async function stage14Multiplexer(ctx) {
  const logger = ctx.logger || console;

  // Sub-step 1: Technical architecture (always runs)
  const archResult = await analyzeStage14(ctx);

  // Sub-step 2: Risk register (moved from S15 — runs after architecture)
  let riskResult = null;
  try {
    riskResult = await analyzeRiskRegister(ctx);
    logger.log('[Stage14] Risk register complete', {
      riskCount: riskResult?.risks?.length || 0,
    });

    // Persist risk register as a separate artifact
    if (riskResult && ctx.supabase && ctx.ventureId) {
      try {
        await writeArtifact(ctx.supabase, {
          ventureId: ctx.ventureId,
          lifecycleStage: 14,
          artifactType: 'risk_register',
          title: 'Risk Register (Stage 14)',
          artifactData: riskResult,
          content: JSON.stringify(riskResult),
          qualityScore: 70,
          validationStatus: 'validated',
          source: 'stage-14-risk-register',
          visionKey: ctx.visionKey || null,
          planKey: ctx.planKey || null,
        });
        logger.log('[Stage14] Risk register artifact persisted');
      } catch (persistErr) {
        logger.warn('[Stage14] Risk register artifact persist failed (non-fatal)', { error: persistErr.message });
      }
    }
  } catch (err) {
    logger.warn('[Stage14] Risk register generation failed (non-fatal)', { error: err.message });
  }

  return { ...archResult, risk_register: riskResult };
};
