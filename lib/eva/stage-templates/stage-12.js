...
const TEMPLATE = {
  ...
  computeDerived(data, prerequisites) {
    ...
    const conversionRates = data.funnel_stages
      .filter(fs => fs.conversionRateEstimate !== undefined && fs.conversionRateEstimate !== null)
      .map(fs => fs.conversionRateEstimate);
    const avgConversionRate = conversionRates.length > 0
      ? Math.round(conversionRates.reduce((sum, r) => sum + r, 0) / conversionRates.length * 100) / 100
      : null;
    const economyCheck = {
      totalPipelineValue,
      avgConversionRate,
