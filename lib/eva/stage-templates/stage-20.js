...
const TEMPLATE = {
  ...
  computeDerived(data) {
    ...
    const totalFailures = critical_failures; // Alias for v2 naming
    const quality_gate_passed = overall_pass_rate === 100 && coverage_pct >= MIN_COVERAGE_PCT;

    // Quality decision object
    let qualityDecisionValue;
    if (quality_gate_passed) {
      qualityDecisionValue = 'pass';
    } else if (overall_pass_rate >= 90 && coverage_pct >= MIN_COVERAGE_PCT * 0.8) {
      qualityDecisionValue = 'conditional_pass';
    } else {
      qualityDecisionValue = 'fail';
    }

    const qualityDecision = {
      decision: qualityDecisionValue,
      rationale: qualityDecisionValue === 'pass'
        ? `All tests passing (${overall_pass_rate}%), coverage ${coverage_pct}%`
        : qualityDecisionValue === 'conditional_pass'
          ? `Near thresholds: ${overall_pass_rate}% pass rate, ${coverage_pct}% coverage`
          : `Below thresholds: ${overall_pass_rate}% pass rate, ${coverage_pct}% coverage`,
    };
