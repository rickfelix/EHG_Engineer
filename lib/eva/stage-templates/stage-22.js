...
const TEMPLATE = {
  ...
  schema: {
    ...
    releaseDecision: { type: 'object', derived: true },
  ...
  computeDerived(data, prerequisites, { logger = console } = {}) {
    ...
    const releaseDecision = {
      decision: all_approved ? 'release' : (approved_items > 0 ? 'hold' : 'cancel'),
      rationale: all_approved
        ? `All ${total_items} release item(s) approved`
        : `${approved_items}/${total_items} release item(s) approved`,
    };
