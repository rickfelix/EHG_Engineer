...
export function evaluatePromotionGate({ stage13, stage14, stage15, stage16 }) {
  ...
  // Stage 15: team members and roles
  const teamCount = stage15?.team_members?.length || 0;
  if (teamCount < MIN_TEAM_MEMBERS) {
    blockers.push(`Resource plan has ${teamCount} team member(s), minimum ${MIN_TEAM_MEMBERS} required`);
    required_next_actions.push(`Add ${MIN_TEAM_MEMBERS - teamCount} more team members with roles and skills`);
  }
  const uniqueRoles = new Set((stage15?.team_members || []).map(tm => tm.role)).size;
  if (uniqueRoles < MIN_ROLES) {
    blockers.push(`Resource plan has ${uniqueRoles} unique role(s), minimum ${MIN_ROLES} required`);
    required_next_actions.push(`Define at least ${MIN_ROLES} distinct roles across team members`);
  }
