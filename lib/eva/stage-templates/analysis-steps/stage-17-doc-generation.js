 ...
 */
// eva-logger-lint-ignore: pre-existing file, already uses an injected `logger` parameter
// (defaulting to console) throughout -- consistent, working DI instrumentation the
// eva-logger-required-lint doesn't recognize (it only detects createLogger/OrchestratorTracer
// imports). SD-LEO-INFRA-ARCHITECTURE-PLANS-GET-001's change here is 2 lines (approved:false
// at two call sites); retrofitting this file's whole logging approach is out of scope.
...
export async function generateDocs({
  ...
  if (visionResult) {
    ...
    const { data: archData, error: archErr } = await upsertArchPlan({
      ...
      brainstormId,
      createdBy: 'stage-17-doc-generation',
      // SD-LEO-INFRA-ARCHITECTURE-PLANS-GET-001 (FR-3): NEVER true here -- requires human
      // approval downstream. Mirrors the vision write above (line 181, QF-20260702-262):
      // a plan claiming alignment to a not-yet-approved vision was an incoherent state this
      // SD closes.
      approved: false,
    });
      ...
      if (archData && archData.quality_checked === false && archData.quality_issues?.length > 0) {
        ...
        for (let retry = 0; retry < 2; retry++) {
          const { data: retryData, error: retryErr } = await upsertArchPlan({
            ...
            createdBy: 'stage-17-doc-generation',
            // SD-LEO-INFRA-ARCHITECTURE-PLANS-GET-001 (FR-3): same as the primary write
            // above -- the retry path must not silently promote the plan to approved.
            approved: false,
          });
