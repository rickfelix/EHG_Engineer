...
/**
 * Fetch Product Hunt UX pattern references for the venture's category.
 * Uses dynamic import with try/catch for graceful fallback.
 *
 * @param {string} category
 * @param {Object} logger
 * @returns {Promise<Array>} Product Hunt products or empty array
 */
async function fetchProductHuntPatterns(category, logger) {
  try {
    const { searchProductHuntByCategory } = await import('../../services/product-hunt-client.js');
    const products = await searchProductHuntByCategory(category, 6);
    logger.log('[Stage15-WF] Product Hunt patterns fetched', { category, count: products.length });
    return products;
  } catch (err) {
    logger.warn?.('[Stage15-WF] Product Hunt fetch failed (non-fatal)', { error: err.message });
    return [];
  }
}
...
function buildExternalContext(phProducts, awwwardsRefs) {
  ...
  if (awwwardsRefs.length > 0) {
    awwwardsContext = `\nAwwwards Visual Quality Benchmarks:\n${awwwardsRefs
      .map(r => `- ${r.site_name} (${r.archetype_category}): Design ${r.score_design ?? 'N/A'}/10, Usability ${r.score_usability ?? 'N/A'}/10. ${r.description ? r.description.substring(0, 100) : ''}`)
      .join('\n')}`;
  }
...
export async function analyzeStage15WireframeGenerator({
  ...
  // ── Fetch external enrichment data in parallel ─────────────────
  const [phProducts, awwwardsRefs] = await Promise.all([
    fetchProductHuntPatterns(category, logger),
    fetchDesignReferences(archetypeCategory, logger),
  ]);
...
Output ONLY valid JSON.`;
  ...
  return {
    ...
    enrichment: {
      product_hunt_count: phProducts.length,
