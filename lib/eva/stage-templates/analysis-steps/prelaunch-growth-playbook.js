...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-2): route growth_playbook /
// growth_optimization_roadmap writes through the unified persistence service (dual-write
// + FR-3 provenance stamp) instead of a raw insert that set neither metadata nor content.
import { writeArtifact } from '../../artifact-persistence-service.js';
import { createLogger } from '../../../logger.js';

// Distinct from the `options.logger`-injected DI pattern this file already uses at call
// sites -- this is the module's OWN structured logger, used additively, never in place of it.
const moduleLogger = createLogger('PrelaunchGrowthPlaybook');
...
export async function persistPrelaunchGrowthPlaybook(supabase, ventureId, result, ventureName, logger = console) {
  ...
  for (const w of writes) {
    ...
    // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-2): writeArtifact() instead of a raw
    // insert; skipDedup:true preserves this file's own mark-stale-then-insert-fresh
    // versioning (see stage-21-visual-assets.js's persistCanonicalPair for the same pattern).
    try {
      const id = await writeArtifact(supabase, {
        ventureId,
        lifecycleStage: PRELAUNCH_STAGE,
        artifactType: w.artifact_type,
        title: w.title, // venture_artifacts.title is NOT NULL
        artifactData: w.artifact_data,
        source: 'worker_sd_leo_feat_post_build_lifecycle_001_d',
        producer: 'prelaunch-growth-playbook',
        isCurrent: true,
        skipDedup: true,
      });
      if (id) persisted.push(w.artifact_type);
    } catch (e2) {
      logger.warn?.(`[S21-PrelaunchGrowth] insert error on ${w.artifact_type}: ${e2.message}`);
      moduleLogger.warn('persistPrelaunchGrowthPlaybook insert error', { ventureId, artifactType: w.artifact_type, error: e2.message });
    }
  }
