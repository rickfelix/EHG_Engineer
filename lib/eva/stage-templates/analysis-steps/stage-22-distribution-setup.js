...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-2): route distribution_channel_config /
// distribution_ad_copy writes through the unified persistence service (dual-write +
...
const moduleLogger = createLogger('Stage22DistributionSetup');
// SD-LEO-INFRA-VENTURE-DEMAND-DISTRIBUTION-001-C FR-2: real production trigger for
// provisionOrganicChannel() — this is the natural point in the pipeline where a
// distribution_channel_config artifact first becomes readable.
import { provisionOrganicChannel } from '../../../marketing/organic-channel-provisioning.js';
import { createStoreSecretDep } from '../../../marketing/channel-secrets.js';
  ...
  for (const w of writes) {
    ...
    try {
      const id = await writeArtifact(supabase, {
        ...
        source: ARTIFACT_SOURCE,
        producer: 'stage-22-distribution-setup',
        isCurrent: true,
        skipDedup: true,
      });
      ...
      moduleLogger.warn('persistCanonicalPair insert error', { ventureId, artifactType: w.artifact_type, error: e2.message });
    }
  }
...
export async function analyzeStage22Distribution(params) {
  ...
  // SD-LEO-INFRA-VENTURE-DEMAND-DISTRIBUTION-001-C FR-2: provision the venture's organic
  // distribution channel now that distribution_channel_config exists. No credentials are
  // available at this planning stage — this seeds the venture_distribution_channels row
  // (idempotent) with credential_ref left null; a later explicit ops step supplies real
  // credentials via storeSecret. Non-blocking: provisioning failure never fails Stage 21.
  try {
    const provisionResult = await provisionOrganicChannel(
      { ventureId },
      { supabase, storeSecret: createStoreSecretDep({ supabase }), logger }
    );
    if (!provisionResult.ok) {
      logger.info?.(`[S21-Distribution] organic-channel provisioning deferred: ${provisionResult.reason}`);
    }
  } catch (err) {
    logger.warn?.(`[S21-Distribution] organic-channel provisioning failed (non-blocking): ${err.message}`);
  }
