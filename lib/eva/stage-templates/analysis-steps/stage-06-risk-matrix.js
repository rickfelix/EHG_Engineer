...
import { RISK_CATEGORIES, RISK_STATUSES } from '../stage-06.js';
...
Output ONLY valid JSON.`;
  ...
  const risks = parsed.risks.map((r, i) => {
    const severity = clamp(r.severity ?? r.consequence, 1, 5);
    const probability = clamp(r.probability, 1, 5);
    const impact = clamp(r.impact ?? r.consequence, 1, 5);
