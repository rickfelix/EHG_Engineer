 ...
 * an unresolvable step, if every RESOLVABLE step is present in order).
...
function computeFlowCoverage(flow, journeys, screens, iaPages) {
  ...
  for (const pageName of flow.steps || []) {
    ...
    resolvableRoutes.push(resolveScreenRoute(screen, iaPages));
  }
  const matchingJourneys = journeys.filter((j) => personaMatches(j.persona_ref, flow.persona));
  const concatenatedRoutes = matchingJourneys.flatMap((j) => j.steps.map((s) => s.route));
