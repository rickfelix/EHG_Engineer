...
// eva-logger-lint-ignore: predates the createLogger/OrchestratorTracer standard
// (SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-145); already uses this directory's established
// injected-logger DI pattern (a `logger` parameter, default console, per analyzeStage17's
// signature below) -- retrofitting the whole file to a different logging mechanism is out
// of scope for SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-F's narrow chairman_decisions fix.
import { ARTIFACT_TYPES } from '../../artifact-types.js';
import { fetchSripSummary } from '../../eva-orchestrator-helpers.js';
import { recordGateResult, recordGateAttempt } from '../../artifact-persistence-service.js';
...
export async function analyzeStage17({
  ...
  try {
    await recordGateResult(supabase, {
      ...
      metadata: JSON.stringify(stage17GateMetadata),
      // SD-MAN-INFRA-VENTURE-CRACK-GATE-001 FR-2 (class b): structured, queryable evidence for
      // the stage17_judgment contract (see crack-gate-evaluator.js's fetchLatestAttestation) --
      // lands in eva_stage_gate_results.gate_criteria jsonb, NOT venture_gate_attestations.
      // Deliberately does NOT bridge-write a venture_gate_attestations row from this call site:
      // that would make Stage-17's own artifact-presence scoring pass self-attest its own
      // stage17_judgment PASS, the exact self-approval pattern
      // database/chairman-gated/20260817_venture_gate_attestations.sql's vga_attester_not_producer
      // CHECK constraint (and its "ARMED AND DISCONNECTED" preamble re: this file's own former
      // chairman_decisions self-approval write, removed by SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-F
      // after it was found to have always silently no-oped) exists to prevent. Per
      // docs/reference/venture-gate-attestations-guide.md line 36, the automated producer role for
      // stage17_judgment is reserved for APA Child E (SD-LEO-INFRA-AUTOMATED-PRODUCT-ASSESSMENT-001-E,
      // separate/unbuilt) -- not this call site.
      criteria: stage17GateCriteria,
    });
  ...
  // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 (T-minus P1): dual-write the durable attempt-history
  // evidence layer. Independent of the recordGateResult call above -- a failure here must never
  // suppress or be suppressed by the primary write's own success/failure.
  try {
    await recordGateAttempt(supabase, {
      ...
      gateType: 'exit',
      passed: gateRecommendation === 'PASS',
      reasoning: gateRationale || null,
      metadata: stage17GateMetadata,
      criteria: stage17GateCriteria,
      evaluatedBy: 'stage-17-blueprint-review',
    });
  } catch (attemptErr) {
    // FAIL LOUD (TESTING F6) -- never a bare logger.warn for a dropped attempt write.
    logger.error(`[Stage17] Gate attempt persist FAILED (venture ${ventureId}): ${attemptErr.message}`);
  }

  // ── Quality-Dependent Chairman Decision Pre-Creation ──
  // Always create/reuse a PENDING decision with quality context, regardless of gate
  // recommendation. SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-F (C4.1): this block used to
  // self-approve (status='approved', decision='approve', resolved_at=...) on PASS, but
  // chairman_decisions has no resolved_at column (PostgREST 42703) -- that write always
  // failed silently (its return value was discarded), while the paired log line still
  // claimed "auto-approved". Removed; a PASS result now logs the same honest pending
  // state as every other recommendation.
  try {
    const briefData = {
      stage: 17,
      gate_recommendation: gateRecommendation,
