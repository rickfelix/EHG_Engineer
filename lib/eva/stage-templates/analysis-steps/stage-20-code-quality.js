...
import { writeFile, lstat, unlink, readFile, readdir } from 'fs/promises';
...
import { FINDING_CATEGORIES, WARN_CAPPED_CATEGORIES } from '../../quality-findings/finding-shape.js';
...
async function runNpmAudit(repoDir, packageManager = 'npm') {
  const findings = [];
  // QF-20260828-487: 'test -f' is POSIX-shell-only and breaks under cmd.exe (win32)
  // -- existsSync is portable and needs no subprocess at all.
  if (!existsSync(path.join(repoDir, 'package.json'))) {
  ...
  // QF-20260828-487: '2>/dev/null || true' is POSIX-shell-only and breaks under
  // cmd.exe (win32) -- the whole command dies before reaching npm at all. `audit`
  // exits nonzero when vulnerabilities are found; that's a real result, not a
  // failure, so read it from the rejected promise's err.stdout instead.
  let stdout = '';
  try {
    ({ stdout } = await execAsync(auditCmd, { cwd: repoDir, env: SANDBOX_ENV, timeout: 30000 }));
  } catch (err) {
    stdout = err.stdout || '';
  }
  try {
 ...
 */
const SECRET_SCAN_MAX_FINDINGS = 50;
const SECRET_SCAN_MAX_FILE_BYTES = 1024 * 1024; // skip huge/binary-shaped files

/**
 * QF-20260828-487: recursive, node-native replacement for the `grep -rn | head`
 * pipeline, which is POSIX-shell-only and breaks under cmd.exe (win32) --
 * the whole command died before grep ever ran, so this check was silently
 * blind (0 findings, always) on Windows. Also removes the shell-injection
 * surface a subprocess pipeline carries.
 */
async function walkForSecrets(dir, repoDir, findings) {
  if (findings.length >= SECRET_SCAN_MAX_FINDINGS) return;
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (SECRET_SCAN_EXCLUDES.includes(entry.name)) continue;
      await walkForSecrets(entryPath, repoDir, findings);
      continue;
    }
    ...
    for (let i = 0; i < lines.length && findings.length < SECRET_SCAN_MAX_FINDINGS; i++) {
      const line = lines[i];
      for (const pattern of SECRET_PATTERNS) {
        pattern.lastIndex = 0; // global regexes are stateful across .test() calls
        if (pattern.test(line)) {
          const relPath = path.relative(repoDir, entryPath).split(path.sep).join('/');
          findings.push({
            check: 'secret_detection',
            title: `Potential secret in ${relPath}:${i + 1}`,
            severity: 'critical',
            detail: line.trim().substring(0, 100) + '...',
          });
          break; // one finding per line, matching the prior grep -n behavior
        }
      }
    }
  }
...
async function runSecretScan(repoDir) {
  const findings = [];
  try {
    await walkForSecrets(repoDir, repoDir, findings);
  } catch { /* best-effort, no findings */ }
  return findings;
}
...
async function runLintCheck(repoDir) {
  ...
  let threw = false;
  // QF-20260828-487: '2>/dev/null || true' is POSIX-shell-only and breaks under
  // cmd.exe (win32) -- the whole command died before eslint ever ran. ESLint
  // exits nonzero when it finds errors/warnings; that's a real, parseable
  // result, not a failure, so read it from the rejected promise's err.stdout.
  // Only a truly empty stdout (spawn failure, deps missing) means "could not run".
  try {
    ({ stdout } = await execAsync(
      'npx eslint . --format json --max-warnings 100',
      { cwd: repoDir, env: SANDBOX_ENV, timeout: 30000 }
    ));
  } catch (err) {
    stdout = err.stdout || '';
    if (!stdout) threw = true;
  }
...
/**
 * Compute the Stage-20 verdict from a legacy findings array (each with a
 * `.check` category name and a `.severity`). Pure, exported for direct unit
 * testing without cloneRepo/sandbox dependencies.
 *
 * SD-LEO-FEAT-STAGE-EXPERIENCE-DESIGN-001 (WARN-capped pilot): findings whose
 * `.check` is in WARN_CAPPED_CATEGORIES are excluded from the FAIL/WARN scan --
 * persisted elsewhere at their true severity (Solomon's raw-findings query sees
 * real signal), but never allowed to drive FAIL/WARN. Every other category's
 * contribution to the verdict is unchanged from before this SD.
 *
 * @param {Array<{check:string, severity:string}>} findings
 * @returns {'PASS'|'WARN'|'FAIL'}
 */
export function computeStage20Verdict(findings) {
  const scan = (findings || []).filter(f => !WARN_CAPPED_CATEGORIES.includes(f.check));
  const hasCritical = scan.some(f => f.severity === 'critical');
  const hasHigh = scan.some(f => f.severity === 'high');
...
export async function analyzeStage20CodeQuality(params) {
  ...
  try {
    ...
    // Determine verdict (pure helper below, exported for direct unit testing).
    const verdict = computeStage20Verdict(allFindings);
...
export { WARN_CAPPED_CATEGORIES };
// QF-20260828-487: exported so cross-platform (win32) regression tests can exercise
// these subprocess-driving checks directly, without mocking child_process.
export { runNpmAudit, runSecretScan, runLintCheck };
