...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-2): route the visual_device_screenshots /
// visual_social_graphics writes through the unified persistence service so they carry
// metadata (and, via FR-3, a provenance stamp) -- the raw insert below never set either.
...
const moduleLogger = createLogger('Stage21VisualAssets');
...
async function persistCanonicalPair(supabase, ventureId, screenshotData, socialData, fullResult, options) {
  ...
  for (const w of writes) {
    ...
    // insert -- gets dual-write (content derived from artifact_data) and the FR-3
    // provenance stamp for free. skipDedup:true preserves this file's own
    // mark-stale-then-insert-fresh versioning (writeArtifact's own dedup would instead
    // UPDATE the prior row in place, which is not this call site's existing behavior).
    try {
    ...
    if (existing && existing.length > 0) {
      ...
      ({ error } = await supabase
        ...
        .insert({
          ...
          // venture_artifacts.title is NOT NULL — omitting it silently fails the insert (0 rows).
          // SD-FDBK-FIX-FIX-RESIDUAL-VENTURE-001 (residual of SD-LEO-FEAT-CONVERT-STAGE-VISUAL-001).
...
export { DEVICE_FRAMES, SOCIAL_SIZES, REQUIRED_UPSTREAM, FEATURE_FLAG_KEY, persistCanonicalPair };
