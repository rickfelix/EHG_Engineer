...
} from '../../quality-model/registry.js';
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): shared venture_artifacts provenance model,
// advisory-only (preflightUpstream's `ok`/`missing` gating -- both of its exit paths already
// fail OPEN by design -- is unaffected; provenance findings land in a separate return field).
...
//
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-B: the 3 arrays below are now GENERATED from
// the Venture Quality Model v1 registry (lib/eva/quality-model/registry.js) via each
// dimension's `stage23_membership` field, instead of being hand-authored here. See
// docs/04_features/venture-quality-model-v1.md. Values are unchanged (regression-
// pinned by tests/unit/eva/stage-templates/stage-23-quality-model-generation.test.js).
const REQUIRED_CATEGORIES = STAGE23_REQUIRED_CATEGORY_IDS;
const ADVISORY_CATEGORIES = STAGE23_ADVISORY_CATEGORY_IDS;
...
const GROWTH_CATEGORIES = STAGE23_GROWTH_CATEGORY_IDS;
...
// SD-LEO-INFRA-VENTURE-WORKFLOW-CREATES-001: chairman rulings 3c20483a/58f5345f. Default
// OFF, same rollout-safety contract as readGrowthPlaybookRequiredFlag/
// readCapabilityChecklistRequiredFlag above -- no in-flight venture is affected until this
// SD's own rollout explicitly enables it (per chairman ruling 212909b9's sequencing, once
// the clean-slate test venture, ratification 3c4a6781, is ready to exercise this path).
const ORGANIZATION_QA_FLAG_KEY = 'LEO_S24_ORGANIZATION_QA_REQUIRED';
const ORGANIZATION_QA_CATEGORIES = ['organization_qa'];

async function readOrganizationQaRequiredFlag(supabase, logger) {
  return readFeatureFlag(supabase, ORGANIZATION_QA_FLAG_KEY, logger);
}

/**
 * Reads the stage-23 organization QA/QC result (venture_artifacts, artifact_type=
 * ORGANIZATION_QA_RESULT, is_current=true) written by stage-23-dedicated-venture-uat.js's
 * createAndCheckOrganization(). Mirrors checkRequiredLegalDocs()'s own dedicated-check
 * pattern (a direct query, not the preflightUpstream anyOf mechanism) -- this category has
 * exactly one producer and no legacy/alternate artifact_type to accept.
 */
async function checkOrganizationQaResult({ supabase, ventureId, logger }) {
  if (!supabase || !ventureId) return { present: false, passed: false, reason: 'missing supabase or ventureId' };
  try {
    const { data, error } = await supabase
      .from('venture_artifacts')
      .select('artifact_data')
      .eq('venture_id', ventureId)
      .eq('artifact_type', ARTIFACT_TYPES.ORGANIZATION_QA_RESULT)
      .eq('is_current', true)
      .maybeSingle();
    if (error) {
      logger?.warn?.(`[S23-LaunchReadiness] organization_qa check DB error (treating as not present): ${error.message}`);
      return { present: false, passed: false, reason: `db error: ${error.message}` };
    }
    if (!data) return { present: false, passed: false, reason: 'no organization_qa_result artifact recorded' };
    const suiteResult = data.artifact_data?.suite_result;
    if (!suiteResult) {
      return { present: true, passed: false, reason: data.artifact_data?.error ? `organization creation/QA-QC failed: ${data.artifact_data.error}` : 'organization QA/QC did not run cleanly' };
    }
    const passed = Number(suiteResult.pass_rate) === 1 && (suiteResult.catch_rate === null || Number(suiteResult.catch_rate) === 1);
    return {
      present: true,
      passed,
      reason: passed ? 'organization acceptance suite passed' : `organization acceptance suite failed (pass_rate=${suiteResult.pass_rate}, catch_rate=${suiteResult.catch_rate})`,
      findings: suiteResult.findings,
    };
  } catch (err) {
    logger?.warn?.(`[S23-LaunchReadiness] organization_qa check threw (treating as not present): ${err.message}`);
    return { present: false, passed: false, reason: `threw: ${err.message}` };
  }
}
 ...
 *
 * @returns {Promise<{ok: boolean, missing: Array<{stage, anyOf}>, present: Set<string>, artifactData: Map<string, object>, provenanceWarnings: string[]}>}
 *   provenanceWarnings (SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G, FR-5): present artifact_types
 *   that fail machine-provenance grading. Advisory-only -- never affects `ok`/`missing`.
 */
async function preflightUpstream({ supabase, ventureId, requirements = UPSTREAM_REQUIREMENTS, logger }) {
  if (!supabase || !ventureId) {
    ...
    return { ok: true, missing: [], present: new Set(), artifactData: new Map(), provenanceWarnings: [] };
  }
  ...
  const { data, error } = await supabase
    .from('venture_artifacts')
    // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): metadata/content/created_at added for
    // provenance grading below.
    .select('lifecycle_stage, artifact_type, is_current, artifact_data, content, metadata, created_at')
    .eq('venture_id', ventureId)
    .eq('is_current', true)
  ...
  const artifactData = new Map(rows.map(r => [r.artifact_type, r.artifact_data]));
  const missing = requirements.filter(r => !r.anyOf.some(t => present.has(t)));
  const provenanceWarnings = rows
    .filter((r) => gradeVentureArtifactProvenance(r).absent)
    .map((r) => r.artifact_type);
  return { ok: missing.length === 0, missing, present, artifactData, provenanceWarnings };
}
 ...
 *
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-H (FR-4): event_source and stage_number both
 * hardcoded the pre-renumber stage 23 — this module is live-dispatched as venture_stages
 * Stage 24 (launch_readiness_gate; live Stage 23 is now dedicated_venture_uat).
 */
async function emitStageSkippedEvent({ supabase, ventureId, missing, logger }) {
  ...
  try {
    const { error } = await supabase.from('eva_orchestration_events').insert({
      event_type: 'custom',
      event_source: 'stage-24-launch-readiness',
...
export async function analyzeStage23LaunchReadiness(params) {
  ...
  // SD-LEO-INFRA-VENTURE-WORKFLOW-CREATES-001: same default-OFF rollout contract as the 2
  // flags above -- organization_qa joins the checklist (as REQUIRED/blocking) ONLY when ON.
  const organizationQaRequired = await readOrganizationQaRequiredFlag(supabase, logger);
  const allCategories = [
    ...CATEGORIES,
    ...(growthRequired ? GROWTH_CATEGORIES : []),
    ...(capabilityChecklistRequired ? CAPABILITY_CATEGORIES : []),
    ...(organizationQaRequired ? ORGANIZATION_QA_CATEGORIES : []),
  ];
  ...
  const capabilityOverrides = capabilityChecklistRequired
    ...
    : new Map();
  // SD-LEO-INFRA-VENTURE-WORKFLOW-CREATES-001: skipped entirely when the flag is OFF --
  // ORGANIZATION_QA_CATEGORIES isn't in allCategories in that case, same skip-avoids-a-
  // required-mock-table contract as capabilityWiredMap/capabilityOverrides above.
  const organizationQaCheck = organizationQaRequired
    ? await checkOrganizationQaResult({ supabase, ventureId, logger })
    : null;
  ...
  const checklist = allCategories.map(cat => {
    ...
    switch (cat) {
      ...
      case 'legal':
        ...
        break;
      // SD-LEO-INFRA-VENTURE-WORKFLOW-CREATES-001: a venture with NO organization_qa_result
      // artifact at all (organizationQaCheck.present === false) must never silently 'pass' --
      // absence is scored 'fail', same as a present-but-failing suite run.
      case 'organization_qa':
        if (organizationQaCheck?.passed) {
          status = 'pass';
          detail = organizationQaCheck.reason;
        } else {
          status = 'fail';
          detail = organizationQaCheck?.reason || 'organization QA/QC result unavailable';
        }
        break;
...
export {
  ...
  ORGANIZATION_QA_CATEGORIES, ORGANIZATION_QA_FLAG_KEY,
  readOrganizationQaRequiredFlag, checkOrganizationQaResult,
  preflightUpstream,
};
