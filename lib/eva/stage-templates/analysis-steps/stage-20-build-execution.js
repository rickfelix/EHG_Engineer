...
export async function analyzeStage20({ stage19Data, stage18Data, ventureName, supabase, ventureId, logger = console }) {
  ...
  const itemsContext = stage19Data.items
    ? `Sprint Items (${stage19Data.items.length}): ${JSON.stringify(stage19Data.items.map(i => ({ title: i.title, type: i.type, story_points: i.story_points })))}`
    : '';

  const readinessContext = stage18Data?.buildReadiness
    ? `Build Readiness: ${stage18Data.buildReadiness.decision}`
    : '';
...
Output ONLY valid JSON.`;
  ...
  const sprintItems = stage19Data.items || [];
