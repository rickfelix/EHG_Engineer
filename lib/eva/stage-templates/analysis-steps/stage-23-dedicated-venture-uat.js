...
async function createAndCheckOrganization({ supabase, ventureId, ventureName, logger }) {
  try {
    ...
    // A QA/QC suite over a silently-truncated organization would evaluate an incomplete
    // membership set -- bound explicitly, just under the PostgREST cap, and fail loud
    // (assertNotCapTruncated) rather than pass a partial org. The caller's try/catch
    // already converts a throw here to {ok:false, reason:'threw'}.
    ...
    assertNotCapTruncated(orgAgents, { cap: 999, site: 'stage-23-dedicated-venture-uat.createAndCheckOrganization#agent_registry' });
