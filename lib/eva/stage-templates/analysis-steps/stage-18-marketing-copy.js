...
import { classifySurface } from './stage-15-wireframe-generator.js';

export { UPSTREAM_ARTIFACT_TYPES };

// ── SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E ────────
// Feature flag re-read per call so vi.stubEnv works in tests.
// Flag-off: all helpers below are pure no-ops / graceful fallbacks.

/**
 * Resolve the venture's surface=marketing Stage 15 wireframe from an array
 * of Stage 15 screens. Returns the first screen whose surface is 'marketing',
 * falling back to classifySurface() for untagged screens. Returns null when
 * no marketing screen exists or input is invalid.
 *
 * Pure function — no I/O, no LLM, no DB.
 *
 * @param {Array|Object|null|undefined} stage15WireframeData
 *   Either an array of screen objects (e.g. from analyzeStage15WireframeGenerator().screens)
 *   or the full wireframe result object (with a .screens property).
 * @returns {{ name: string, key_components: string[], ascii_layout: string[] }|null}
 */
export function resolveMarketingWireframe(stage15WireframeData) {
  // Accept both the raw screens array and the full result object.
  let screens;
  if (Array.isArray(stage15WireframeData)) {
    screens = stage15WireframeData;
  } else if (stage15WireframeData && Array.isArray(stage15WireframeData.screens)) {
    screens = stage15WireframeData.screens;
  } else {
    return null;
  }
  ...
  for (const screen of screens) {
    ...
    // Prefer explicit surface field; fall back to classifySurface for untagged screens.
    const surface = screen.surface ?? classifySurface(screen).surface;
...
/**
 * Build a prompt context block from a marketing wireframe's key_components
 * and ascii_layout. Gracefully handles missing/empty fields.
 *
 * Pure function — no I/O.
 *
 * @param {{ name?: string, key_components?: string[], ascii_layout?: string[] }|null} wireframe
 * @returns {string} A formatted context block for the S18 prompt, or '' when wireframe is null.
 */
export function buildMarketingWireframeContext(wireframe) {
  if (!wireframe || typeof wireframe !== 'object') return '';

  const lines = [];
  lines.push('## Stage 15 Marketing Wireframe (surface=marketing)');

  if (wireframe.name) {
    lines.push(`Screen: ${wireframe.name}`);
  }

  const components = Array.isArray(wireframe.key_components) && wireframe.key_components.length > 0
    ? wireframe.key_components
    : [];
  ...
  if (components.length > 0) {
    ...
    lines.push('Key Components: (none specified)');
  }

  const layout = Array.isArray(wireframe.ascii_layout) && wireframe.ascii_layout.length > 0
    ? wireframe.ascii_layout
    : (typeof wireframe.ascii_layout === 'string' && wireframe.ascii_layout.trim()
        ? wireframe.ascii_layout.split('\n')
        : []);
  ...
  lines.push(
    'Use the above wireframe structure to ground the landing_hero copy: ' +
    'headline and CTA must reflect the visible components and layout hierarchy.',
  );
...
/**
 * Return conversion-copy directives for the marketing surface. These instruct
 * the LLM to emphasise value proposition, benefit framing, and CTA clarity.
 *
 * Pure function — no I/O.
 *
 * @returns {string} A formatted directives block for the S18 prompt.
 */
export function conversionCopyDirectives() {
  return [
    '## Conversion-Copy Directives (surface=marketing)',
    '- VALUE PROPOSITION: Open every marketing section with the single clearest benefit',
    '  the target persona gains — not a feature list. Lead with outcome, not mechanism.',
    '- BENEFIT FRAMING: Translate every feature into a user outcome. Prefer',
    '  "you will [outcome]" over "our product [feature]".',
    '- CTA EMPHASIS: Every section with a call-to-action must use active, specific verbs',
    '  (e.g. "Start your free trial", "See it in action") — never passive or generic',
    '  ("Click here", "Learn more"). CTA text should match the persona\'s primary goal.',
    '- URGENCY WITHOUT PRESSURE: Add time or scarcity cues only when they are factually',
    '  grounded (e.g. launch pricing, limited beta seats). Do not fabricate urgency.',
    '- SOCIAL PROOF: Where testimonials or stats are available from upstream artifacts,',
    '  embed them in landing_hero and social_posts for credibility.',
  ].join('\n');
}
...
/**
 * @param {Object} upstreamData
 * @param {string} ventureName
 * @param {string} [wireframeContextBlock] - Optional: output of buildMarketingWireframeContext()
 * @param {string} [conversionDirectivesBlock] - Optional: output of conversionCopyDirectives()
 */
function buildUserPrompt(upstreamData, ventureName, wireframeContextBlock, conversionDirectivesBlock) {
  const sections = [];
  ...
  // SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E ──────────
  // Inject wireframe context + conversion directives when flag is ON and
  // a marketing wireframe was resolved. Flag-off leaves the prompt identical.
  if (wireframeContextBlock) {
    sections.push(wireframeContextBlock + '\n');
  }
  if (conversionDirectivesBlock) {
    sections.push(conversionDirectivesBlock + '\n');
  }
 ...
 * @param {Array|Object} [params.stage15WireframeData] - Blueprint: Stage 15 wireframe screens
 *   (from analyzeStage15WireframeGenerator output — .screens array or full result object).
 *   SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E: when EVA_SURFACE_AWARE_ENABLED=true
 *   and a surface=marketing screen is found, its key_components + ascii_layout are injected into
 *   the S18 prompt. Flag-off or no marketing screen → identical baseline behavior.
...
export async function analyzeStage18MarketingCopy(params) {
  const {
    ...
    stage15WireframeData = null, // SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E
    ventureName,
    ventureId,
    logger = console,
  ...
  // SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E ──────────
  // Resolve marketing wireframe + build context blocks (flag-gated).
  // Re-read env per call so vi.stubEnv works in tests.
  let wireframeContextBlock;
  let conversionDirectivesBlock;
  let resolvedMarketingWireframe = null;
  if (process.env.EVA_SURFACE_AWARE_ENABLED === 'true' && stage15WireframeData) {
    resolvedMarketingWireframe = resolveMarketingWireframe(stage15WireframeData);
    if (resolvedMarketingWireframe) {
      wireframeContextBlock = buildMarketingWireframeContext(resolvedMarketingWireframe);
      conversionDirectivesBlock = conversionCopyDirectives();
      logger.info?.('[S18-MarketingCopy] Marketing wireframe resolved for surface-aware context injection', {
        screenName: resolvedMarketingWireframe.name,
        componentCount: Array.isArray(resolvedMarketingWireframe.key_components)
          ? resolvedMarketingWireframe.key_components.length : 0,
      });
    } else {
      logger.info?.('[S18-MarketingCopy] No surface=marketing screen found in stage15WireframeData; falling back to baseline prompt');
    }
  }

  // Build LLM prompt
  const userPrompt = buildUserPrompt(upstreamByType, ventureName, wireframeContextBlock, conversionDirectivesBlock);
  ...
  const result = {
    ...
    metadata: {
      ...
      // SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E
      marketing_wireframe_injected: !!resolvedMarketingWireframe,
      marketing_wireframe_screen_name: resolvedMarketingWireframe?.name ?? null,
    },
...
// SD-SURFACEAWARE-WIREFRAME-GENERATION-MARKETING-ORCH-001-E helpers are
// already exported as named exports above:
//   resolveMarketingWireframe, buildMarketingWireframeContext, conversionCopyDirectives
