...
// SD-LEO-INFRA-STAGE-GROUNDING-INJECTOR-001 FR-1: ground architecture cost/feasibility
// claims in the AI-agent operating model (Claude Code + LEO), not hired-team priors.
import { getOperatingModelPromptBlock } from '../../standards/operating-model.js';
// SD-LEO-INFRA-STAGE-GROUNDING-INJECTOR-001 FR-2: inject relevant retro lessons +
// recurring issue patterns (fail-safe: '' when supabase/data unavailable).
import { getRetroPatternGroundingBlock } from '../../standards/retro-pattern-grounding.js';
...
// SD-LEO-ENH-CONSTRAIN-STAGE-EHG-001: house-stack constraint
import { EHG_HOUSE_TECH_STACK, EHG_HOUSE_AUTH_STRATEGY } from '../../config/house-tech-stack.js';
import { validateHouseStackAdherence, HouseStackDeviationError } from '../../utils/validate-house-stack-adherence.js';
...
// SD-LEO-ENH-CONSTRAIN-STAGE-EHG-001: SYSTEM_PROMPT inlines EHG_HOUSE_TECH_STACK
// as a hard constraint, replacing prior free-choice scaffolding strings that
// invited the LLM to pick a stack independently per venture. Belt-and-suspenders:
// post-parse validation in validateHouseStackAdherence enforces the constraint
// independent of whether the LLM honors the prompt-level instruction.
...
The EHG House Tech Stack is fixed across the portfolio. Use these technologies for each layer; do NOT deviate unless an architecture_override_request is provided in the user input. Cross-venture consistency enables shared auth, design system, deployment pipeline, observability, and operational tooling.
...
You MUST output valid JSON with exactly this structure:
{
  ...
  "layers": {
    "presentation": {
      "technology": "${EHG_HOUSE_TECH_STACK.presentation.technology}",
      "components": ["Component 1", "Component 2"],
      "rationale": "Why this technology fits the venture"
    },
    "api": {
      "technology": "${EHG_HOUSE_TECH_STACK.api.technology}",
      "components": ["Endpoint group 1", "Endpoint group 2"],
      "rationale": "Why this API approach fits the venture"
    },
    "business_logic": {
      "technology": "${EHG_HOUSE_TECH_STACK.business_logic.technology}",
      "components": ["Service 1", "Service 2"],
      "rationale": "Why this technology fits the venture"
    },
    "data": {
      "technology": "${EHG_HOUSE_TECH_STACK.data.technology}",
      "components": ["Table 1", "Collection 1"],
      "rationale": "Why this technology fits the venture"
    },
    "infrastructure": {
      "technology": "${EHG_HOUSE_TECH_STACK.infrastructure.technology}",
      "components": ["Service 1", "Service 2"],
      "rationale": "Why this infrastructure fits the venture"
    }
  },
  "security": {
    "authStrategy": "${EHG_HOUSE_AUTH_STRATEGY.technology}",
...
- Use the EHG House Tech Stack values shown above for each layer's "technology" and for security.authStrategy. Do not deviate unless architecture_override_request is set in the user input.
- If architecture_override_request IS set, you MAY deviate, but you MUST populate "override_reason" with a sentence explaining the deviation.
- If no override is set, populate "override_reason" with an empty string.
 ...
 */
export async function analyzeStage14({ stage1Data, stage6Data, stage13Data, ventureName, ventureId, supabase, architecture_override_request, logger = console }) {
  ...
  // SD-LEO-ENH-CONSTRAIN-STAGE-EHG-001: conditional override-permission text
  const overrideRequest = String(architecture_override_request || '').trim();
  const overrideContext = overrideRequest.length > 0
    ? `\nDeviation from the EHG House Tech Stack is permitted for this venture. Reason: ${sanitizeForPrompt(overrideRequest)}\nYou MUST populate "override_reason" in your output explaining why deviation is necessary.\n`
    : '';
...
Output ONLY valid JSON.`;
  ...
  const retroBlock = await getRetroPatternGroundingBlock({ supabase, stageType: 'architecture' });
  const response = await client.complete(SYSTEM_PROMPT + '\n\n' + getOperatingModelPromptBlock() + retroBlock + getFourBucketsPrompt(), userPrompt, { timeout: 120000 });
  ...
  // SD-LEO-ENH-CONSTRAIN-STAGE-EHG-001: post-parse house-stack adherence check.
  // Belt-and-suspenders enforcement: catches deviation independent of the
  // SYSTEM_PROMPT constraint (LLMs frequently ignore single-channel constraints).
  const overrideReason = String(parsed.override_reason || '').trim();
  const allowOverride = overrideRequest.length > 0 && overrideReason.length > 0;
  const adherence = validateHouseStackAdherence({ layers, security }, { allowOverride });
  if (!adherence.valid) {
    logger.warn('[Stage14] House-stack adherence: VIOLATION', { errors: adherence.errors });
    throw new HouseStackDeviationError(
      `Stage 14 architecture deviates from EHG House Tech Stack without override: ${adherence.errors.join('; ')}`,
      adherence.errors
    );
  }
  if (adherence.warnings.length > 0) {
    logger.warn('[Stage14] House-stack adherence: DEVIATION_OVERRIDE', { warnings: adherence.warnings, override_reason: overrideReason });
  } else {
    logger.log('[Stage14] House-stack adherence: PASS');
  }
...
Output ONLY valid JSON.`;
  ...
  const legacyPayload = {
    ...
    integration_points,
    constraints,
    override_reason: overrideReason, // SD-LEO-ENH-CONSTRAIN-STAGE-EHG-001
