...
// SD-MAN-INFRA-VENTURE-CRACK-GATE-001 FR-10 (class j): composeAcquisitionPacket()/
// executeAcquisition() had zero production callers -- this is the mid-flight trigger for the
// packet-creation half, mirroring FR-1's pattern. The post-approval half (executeAcquisition via
// runPostApprovalPipeline) is wired separately, at the actual approval event (scripts/chairman-
// decisions.mjs), not here -- Stage-11 completion only creates the pending decision.
import { composeAcquisitionPacket } from '../../../venture-acquisition/decision-packet.js';
...
Output ONLY valid JSON.`;
  ...
  // ── Domain-Acquisition Packet Trigger (SD-MAN-INFRA-VENTURE-CRACK-GATE-001 FR-10) ──
  // Fires AFTER writeStage11Artifacts so the identity_brand_name domainShortlist it just wrote
  // is readable. Non-fatal (composeAcquisitionPacket's own read/write failures never break Stage
  // 11 itself); idempotent (an existing pending packet is returned as-is, never duplicated); a
  // 'no_shortlist' result (availability seam off, or no candidate registrable) is a normal,
  // honest outcome, not an error.
  let acquisitionPacket = null;
  if (supabase && ventureId) {
    try {
      acquisitionPacket = await composeAcquisitionPacket(supabase, ventureId, { selectedName });
      logger.log('[Stage11] Acquisition packet: ' + acquisitionPacket.status);
    } catch (err) {
      acquisitionPacket = { status: 'error', error: err.message };
      logger.warn('[Stage11] Acquisition packet trigger failed (non-fatal)', { error: err.message });
    }
  }

  // ── SRIP Completion Summary (supplementary, non-fatal) ──────
  let sripSummary = null;
  if (supabase && ventureId) {
    sripSummary = await fetchSripSummary(supabase, ventureId);
    if (sripSummary) {
      logger.log('[Stage11-SRIP] SRIP summary included', { status: sripSummary.status, answers: sripSummary.answer_count });
    }
  }
  ...
  return {
    ...
    ...(sripSummary ? { srip_summary: sripSummary } : {}),
    ...(acquisitionPacket ? { acquisition_packet: acquisitionPacket } : {}),
  };
}
