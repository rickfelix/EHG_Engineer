...
// SD-LEO-FEAT-STAGE-LIVE-ANNOUNCE-001 FR-2: stage 24 now dispatches to canonical go-live
// analyzer (was incorrectly aliased to stage-24-marketing-prep.js, which was a stale Stage 23
// copy exporting analyzeStage23 with module header literally saying "Stage 23 Analysis Step").
// Stale copies archived to docs/archived/orphan-stage-23-modules/.
export { analyzeStage24GoLive as analyzeStage24 } from './stage-24-go-live.js';
...
// SD-LEO-FEAT-STAGE-GROWTH-PLAYBOOK-001 FR-1: stage 26 now dispatches to the
// canonical growth-playbook analyzer. Previously misrouted to
// stage-26-launch-execution.js (re-exporting analyzeStage25 — wrong domain,
// wrong stage). Sibling-pattern fix matching FR-2 of S24/S25 SDs. The
// stage-26-launch-execution.js + stage-26-venture-review.js orphans are
// archived to docs/archived/orphan-stage-26-modules/ in the same PR.
export { analyzeStage26GrowthPlaybook as analyzeStage26 } from './stage-26-growth-playbook.js';
...
export async function getAnalysisStep(stageNumber) {
  const loaders = {
    ...
    // SD-LEO-FEAT-STAGE-LIVE-ANNOUNCE-001 FR-2: dispatch the canonical go-live analyzer
    24: () => import('./stage-24-go-live.js').then(m => m.analyzeStage24GoLive),
    ...
    // SD-LEO-FEAT-STAGE-GROWTH-PLAYBOOK-001 FR-1: dispatch the canonical growth-playbook analyzer
    26: () => import('./stage-26-growth-playbook.js').then(m => m.analyzeStage26GrowthPlaybook),
  };
  const loader = loaders[stageNumber];
  return loader ? loader() : null;
}
