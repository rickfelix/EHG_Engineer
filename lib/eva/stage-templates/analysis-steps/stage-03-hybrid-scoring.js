...
async function getAIScores({ stage1Data, stage2Data, ventureName, logger = console }) {
  ...
  // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-146 (FR-1/FR-2/FR-5): archetype + keyAssumptions are
  // passed as ADVISORY calibration/validation context only (never a numeric weight formula --
  // no archetype-vs-metric-weight benchmark data exists to ground one).
  const archetypeLine = stage1Data?.archetype
    ? `Venture Archetype (calibration context, not a scoring formula): ${sanitizeForPrompt(stage1Data.archetype)}\n`
    : '';
...
${archetypeLine}${keyAssumptionsLine}
