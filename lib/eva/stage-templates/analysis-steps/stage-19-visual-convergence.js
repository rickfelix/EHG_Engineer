...
async function callLLM(systemPrompt, userPrompt, logger) {
  ...
  try {
    ...
    logger.warn?.('[VisualConvergence] LLM returned unexpected structure — falling back to mock');
  } catch {
...
function generateMockResponse(passDef, screens) {
  ...
  const hasLayout = screens.some(s => s.ascii_layout && (Array.isArray(s.ascii_layout) ? s.ascii_layout.length : s.ascii_layout.split('\n').length) >= 5);
...
export async function analyzeStage19VisualConvergence(ventureId, stageData, options = {}) {
  ...
  logger.log('[VisualConvergence] Starting visual convergence analysis', { ventureId });
  ...
  if (!screens || !Array.isArray(screens) || screens.length === 0) {
    logger.warn('[VisualConvergence] No Stage 15 wireframe screens found — returning minimal result');
    return {
      passes: [],
      overall_score: 0,
...
Output ONLY valid JSON.`;
  ...
  for (const passDef of PASS_DEFINITIONS) {
    logger.log(`[VisualConvergence] Running pass: ${passDef.label} (${passDef.expertPersona})`, { domain: passDef.domain });
    ...
    if (!result) {
      ...
      logger.log(`[VisualConvergence] Using mock for ${passDef.domain}`, { score: result.score });
    }
  ...
  logger.log('[VisualConvergence] Visual convergence complete', {
    duration,
    overall_score,
    verdict,
