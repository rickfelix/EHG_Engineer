...
export async function analyzeStage05({ stage1Data, stage3Data, stage4Data, ventureName, ventureId, supabase, logger = console }) {
  ...
  // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-146 (FR-3): keyAssumptions as validation context,
  // completing the Stage 1 -> Stage 3/5 contract for this consumer (Stage 3 already added
  // separately in stage-03-hybrid-scoring.js). Never touches the deterministic ROI/payback
  // kill-gate math below.
  const keyAssumptionsLine = Array.isArray(stage1Data?.keyAssumptions) && stage1Data.keyAssumptions.length > 0
    ? `Key Assumptions (validation context): ${sanitizeForPrompt(stage1Data.keyAssumptions.join('; '))}\n`
    : '';
...
${keyAssumptionsLine}${stage3Data?.overallScore ? `Validation Score: ${stage3Data.overallScore}/100` : ''}
...
Output ONLY valid JSON.`;
  ...
  if (supabase && ventureId) {
    try {
      // Resolve company_id from ventures table (required FK for financial_models)
      ...
      if (!ventureRow?.company_id) {
        logger.warn('[Stage05] Venture missing company_id — skipping financial_models write', { ventureId });
      } else {
      ...
      } // close else block for company_id check
    } catch (err) {
