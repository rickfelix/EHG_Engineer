 ...
 *   → Stage 5: stage5Handoff (avgMarketPrice, pricingModels, priceRange, competitiveDensity)
...
const TEMPLATE = {
  ...
  schema: {
    ...
    stage5Handoff: {
      type: 'object',
      derived: true,
      // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-146: corrected to match the actual shape produced
      // by analysis-steps/stage-04-competitive-landscape.js (buildStage5Handoff / LLM output)
      // and consumed by analysis-steps/stage-05-financial-model.js. Doc-only correction --
      // field is derived:true and excluded from the enforced output contract.
      properties: {
        avgMarketPrice: { type: 'string' },
        pricingModels: { type: 'array', items: { type: 'string' } },
        priceRange: { type: 'object', properties: { low: { type: 'number' }, high: { type: 'number' } } },
        competitiveDensity: { type: 'enum', values: ['low', 'medium', 'high'] },
      },
    },
  },
  defaultData: {
