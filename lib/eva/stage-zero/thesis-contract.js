...
/**
 * SD-LEO-INFRA-MINUS-CARGO-INSTRUMENTS-001 FR-4: kill-fire-readiness precondition. A FIRED
 * verdict may only proceed to mint a chairman decision / block advancement when ALL of:
 *   1. the criterion itself is shape-valid (validateKillCriteria([criterion]) — "frozen
 *      criteria snapshot": the exact object read from ventures.metadata.kill_criteria). The
 *      verdict's own observed/threshold/comparator triple (already logged verbatim to
 *      system_events by every caller) IS the evidence trail — no separate evidence-ref field
 *      is required on top of it.
 *   2. criterion.stage_by references a REAL stage in the canonical stage-governance view
 *      (governance.getStage(criterion.stage_by) !== null) — catches a fabricated/corrupted
 *      stage_by. This is NOT the same check as due-date filtering (stage_by <= toStage,
 *      already correct in evaluateThesisKillCriteria) and must never be confused with
 *      killStages/isKill() membership — AltifyAI's live criteria are stage_by=21, which is
 *      not a kill-type stage in venture_stages, and that is CORRECT and expected.
 *   3. no OPEN blocking finding (gap_class in GATE_CANNOT_FAIL|INSTRUMENT_LIE|GATE_BYPASSED)
 *      exists for this venture at this stage — a known-broken instrument must not be allowed
 *      to fire a kill until the finding is resolved.
 *
 * Pure predicate: zero DB/IO. checkThesisKillGate() is responsible for fetching governance
 * and openBlockingFindings before calling this.
 *
 * @param {Object} criterion - the frozen criterion object as read from kill_criteria
 * @param {Object} verdict - the FIRED verdict from classifyVerdict (unused today; kept in the
 *   signature so a future refinement can inspect it without a call-site change)
 * @param {{getStage: (n:number) => Object|null}} governance - stage-governance's public view
 * @param {Array<{gap_class?: string}>} openBlockingFindings - open findings for this venture/stage
 * @returns {{ ready: boolean, reason?: 'frozen_criteria_missing'|'invalid_stage'|'blocking_finding_open' }}
 */
export function validateKillFireReadiness(criterion, verdict, governance, openBlockingFindings = []) {
  const { valid } = validateKillCriteria([criterion]);
  if (!valid) return { ready: false, reason: 'frozen_criteria_missing' };
  if (!governance?.getStage?.(criterion.stage_by)) return { ready: false, reason: 'invalid_stage' };
  const BLOCKING_GAP_CLASSES = new Set(['GATE_CANNOT_FAIL', 'INSTRUMENT_LIE', 'GATE_BYPASSED']);
  const hasBlockingFinding = Array.isArray(openBlockingFindings) && openBlockingFindings.some((f) => BLOCKING_GAP_CLASSES.has(f?.gap_class));
  if (hasBlockingFinding) return { ready: false, reason: 'blocking_finding_open' };
  return { ready: true };
}
