...
- Supabase infrastructure available (reduces infra overhead, not product complexity)
- Portfolio synergies may reduce CAC for ventures with audience overlap
...
DIMENSION 4: EXECUTION FEASIBILITY (weight: 15%)
  NOTE: EHG uses AI-accelerated development. Score complexity WITHIN that context — not all builds are equal even with AI.
  1 = Requires regulatory approval, novel hardware, or unsolved research problems; high failure risk regardless of tooling
  2 = Needs real-time systems, complex multi-party integrations, or deep domain expertise the team lacks; technical risk is the primary concern
  3 = Non-trivial product: multiple user roles, workflow engines, or complex business logic that requires careful design even with AI assistance
  4 = Straightforward build: well-understood patterns (SaaS dashboards, content platforms, automation tools, marketplaces); AI tooling handles most of the heavy lifting
  5 = Minimal custom logic; thin wrapper over existing APIs/infrastructure; could prototype in days
  Most ventures building standard web/mobile products with AI assistance should score 3-4. Score 2 only for genuinely complex technical challenges.

DIMENSION 5: COMPETITIVE DEFENSIBILITY (weight: 15%)
  Score the POTENTIAL to build defensibility over time, not whether a moat exists today.
  1 = Commodity offering; no path to differentiation (e.g., generic productivity tool in a crowded market)
  2 = Can differentiate on UX or speed but lacks structural advantage (e.g., another chatbot wrapper, a basic automation tool)
  3 = Has a real defensibility path through data accumulation, vertical specialization, or workflow integration (e.g., an industry-specific analytics platform that gets smarter with usage, a compliance tool where switching costs grow over time)
  4 = Defensibility is core to the product design: network effects, data flywheels, or platform ecosystem (e.g., a marketplace connecting specialized buyers/sellers, a B2B tool where each customer's data trains the model)
  5 = Multiple reinforcing moats that compound with scale (e.g., multi-sided marketplace with regulatory advantage and proprietary data)
  IMPORTANT: If the venture targets a specific industry or accumulates user data, it likely scores 3+, not 2. Score 2 is for ventures with NO industry focus and NO data strategy.
