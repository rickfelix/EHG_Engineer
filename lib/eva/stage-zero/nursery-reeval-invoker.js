...
export async function invokeNurseryReeval(opts = {}, deps = {}) {
  ...
  //
  // ⚠ BLAST RADIUS, CORRECTED 2026-08-08 — the previous sentence here rested on a mechanism that
  // does not do that job, and this SD has already retracted one overstated safety claim, so the
  // correction is stated rather than quietly swapped. IT SAID: "a duplicate request, not a
  // duplicate promotion: the queue processor claims rows before draining them." THAT IS FALSE.
  // claimRequest (scripts/stage-zero-queue-processor.js:186-200) is PER-ROW optimistic
  // concurrency — it guarantees one session drains one row. It says nothing about two DISTINCT
  // rows naming the same candidate, which is exactly what this race produces. The processor
  // contains zero occurrences of "nursery" (verified by grep), so it has no candidate-level
  // idempotency at all.
  //
  // ⚠ AND THE RESIDUAL IS WIDER THAN A DUPLICATE REQUEST. The processor's own dedupe is
  // discoveryDedupKey (:274-279) = `${strategy}::${candidateCount}::${constraints}`. This builder
  // always emits nursery_reeval / 1 / {}, so EVERY nursery_reeval request carries the identical
  // key `nursery_reeval::1::{}` and NURSERY_ID IS NOT IN IT. findRecentDiscoveryDuplicate matches
  // same requested_by + discovery_mode + same key within DISCOVERY_DEDUP_MIN (default 10 min), and
  // processRequest (:360-368) then COPIES the matched row's result and marks the new row completed
  // WITHOUT RUNNING THE TRAVERSAL. All these rows share the one service principal. So two requests
  // for DIFFERENT candidates landing inside that window collapse: candidate B is marked completed
  // carrying candidate A's result — a silent skip plus provenance corruption, not a harmless
  // duplicate. Not reachable from the 6-hourly cron alone; reachable via the concurrency hole above
  // and via the stale-row requeue path (:95-150).
  //
  // Fixing the processor's key is OUT OF SCOPE for this SD. Recording it beside the kill switch is
  // not: arming NURSERY_REEVAL_ENABLED without knowing this is arming blind.
  ...
  //
  // PLACEMENT, arrived at by two corrections rather than by design. The first draft put this
  // beside the emptiness check at the top; that broke seven tests which never enqueue at all,
  // which was the code pointing out the precondition belongs next to the write, not at the door.
  // The second draft put it AFTER the dryRun return, reasoning that a dry run writes nothing so
  // it has nothing to protect. THAT WAS WRONG FOR THE REASON DRY RUN EXISTS: --dry-run is the
  // mode an operator uses to ask "is this scheduler healthy", and a dry run that reports success
  // while the registry points at a deleted principal is precisely the false green this whole
  // guard was written to abolish. So it sits here: AFTER the dedupe read (the scheduled tick
  // usually finds nothing due, and an auth round trip on those ticks buys nothing) and BEFORE
  // the dryRun return (so validation actually validates). Only the pure builder call separates
  // it from the insert, so the TOCTOU window stays narrow.
