...
export async function executeCompetitorTeardown({ urls }, deps = {}) {
  ...
  // SD-LEO-INFRA-STAGE0-TRAVERSABILITY-REACH-001: mirrors discovery-mode.js's gate
  // invocation (SD-LEO-INFRA-STAGE0-TRAVERSABILITY-GATE-001, spec R6). This path
  // produces a single candidate, not a ranked array — wrap it in a 1-element array.
  // EnvelopeUnavailableError propagates unhandled: the run fails closed, never
  // silently ungated.
  const gateCandidate = {
    name: deconstruction.suggested_venture_name || '',
    problem_statement: deconstruction.root_customer_problem || '',
    solution: deconstruction.automation_solution || '',
    target_market: deconstruction.target_market || '',
    required_capabilities: deconstruction.required_capabilities,
  };
  ...
  if (gate.failed.length > 0) {
    try {
      await parkFailedCandidate(gate.failed[0], { strategy: 'competitor_teardown' }, { supabase, logger });
    } catch (parkErr) {
  ...
  return createPathOutput({
    ...
    target_market: deconstruction.target_market || '',
    ...(deconstruction.required_capabilities ? { required_capabilities: deconstruction.required_capabilities } : {}),
    metadata: {
...
async function analyzeCompetitor(url, deps = {}) {
  ...
  if (supabase) {
    try {
      ...
      if (sanitizedUrl) {
        ...
        const [byApp, byWeb] = await Promise.all([
          supabase.from('app_rankings')
            .select('chart_position, source, rating, review_count, installs_range, category, scraped_at')
      ...
      // render without their age; no rows renders an explicit NO-DATA marker instead
      // of silent omission.
...
Also identify EVERY factory/external capability this rebuilt venture depends on to launch,
distribute, operate, and collect revenue — form factor/hosting, third-party integrations,
distribution channels, ops surface. Be complete and honest: the factory hard-checks these
against its delivered capability envelope, and a venture requiring an undelivered capability
is parked until that capability ships.
...
Return JSON:
{
  ...
  "required_capabilities": [{ "name": "string", "kind": "form_factor" | "integration" | "channel" | "ops" }]
}`;
