...
// opportunity_blueprints has no required_capabilities data source (no LLM prompt, no
// carry-forward field), so candidates from this path correctly take the gate's existing
// honest no_requirements_declared auto-pass — same as 4 of 5 discovery strategies do today.
...
export async function executeBlueprintBrowse({ blueprintId, category, customizations = {} }, deps = {}) {
  ...
  // Step 4.5: TRAVERSABILITY GATE — hard, in the selection path, not advisory.
  ...
  // No required_capabilities field: opportunity_blueprints has no existing source for
  // it (TR-1) — an undeclared candidate correctly takes the gate's honest auto-pass.
  const gateCandidate = {
    name: template.name || selected.title,
    problem_statement: template.problem_statement || '',
    solution: template.solution_concept || '',
  ...
  if (gate.failed.length > 0) {
    try {
      await parkFailedCandidate(gate.failed[0], { strategy: 'blueprint_browse' }, { supabase, logger });
    } catch (parkErr) {
