...
export async function executeDiscoveryMode({ strategy, constraints = {}, candidateCount = 5 }, deps = {}) {
  ...
  // SD-LEO-INFRA-STAGE0-POSTURE-SUCCESSOR-001 (CH-3): the chairman's anti-goals are a
  // hard PRE-RANKING filter — a pre-excluded candidate never ranks, never wastes a
  // slate slot. Disqualifications are recorded on the run, never silently dropped.
  const antiGoalScreen = applyAntiGoalScreen(candidates, posture);
  if (antiGoalScreen.disqualified.length > 0) {
    logger.log(`   Anti-goal screen: ${antiGoalScreen.disqualified.length}/${candidates.length} candidate(s) disqualified pre-ranking`);
  }
  if (antiGoalScreen.eligible.length === 0) {
    logger.log("   Anti-goal screen: no candidate survives the chairman's anti-goals.");
    return null;
  }

  const ranked = rankCandidates(antiGoalScreen.eligible, { weights: posture.criteria.weights, strategicContext });
  ...
  const flatScores = new Map(
    rankCandidates(antiGoalScreen.eligible, { weights: posture.criteria.weights, strategicContext, flattenAllE0: true })
  ...
  return createPathOutput({
    ...
    raw_material: {
      ...
      // CH-3: anti-goal disqualifications ride to the chairman-review surface.
      anti_goal_disqualified: antiGoalScreen.disqualified.map(d => ({
        name: d.candidate.name,
        anti_goal: d.anti_goal,
    ...
    metadata: {
      ...
      // SD-LEO-INFRA-STAGE0-POSTURE-SUCCESSOR-001 (CH-3): anti-goal screen stamp.
      anti_goal_screen: {
        checked: candidates.length,
        disqualified: antiGoalScreen.disqualified.length,
      },
  ...
  if (supabase) {
    try {
      ...
      const { data: rankings } = await supabase
        .from('app_rankings')
        .select('app_name, source, chart_position, category, rating, scraped_at')
        .gte('scraped_at', sevenDaysAgo)
        .order('chart_position', { ascending: true })
        .limit(150);

      // SD-LEO-INFRA-RETIRE-DEAD-MONITORING-CHAIN-001 (H12/H13): ranking figures never
      // render without their age; empty renders an explicit NO-DATA marker instead of
      // silent omission — the LLM must know when its market context is absent.
      const staleness = stalenessStamp(rankings);
      ...
      // Non-throwing (backward compatible) but honest: explicit NO-DATA, never silence.
      logger.warn(`   Ranking data unavailable: ${rankErr.message}`);
      rankingBlock = `\n## Real Market Data (from app_rankings)\n${NO_DATA_MARKER}\n`;
    }
  }
...
If no ventures are worth reviving, return an empty array: []`;
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      // SD-LEO-INFRA-STAGE0-TRAVERSABILITY-REACH-001 FR-1: nursery_reeval's own
      // re-evaluation prompt does not ask the LLM to re-derive required_capabilities
      // (it is revising an existing pitch, not generating a fresh one). Carry it
      // forward from the original parked record when present, rather than re-prompting
      // for something already declared once.
      const byId = new Map(nurseryItems.map(item => [item.id, item]));
      return parsed.map(c => {
        const original = byId.get(c.nursery_id);
        const requiredCapabilities = original?.metadata?.required_capabilities;
        return {
          ...c,
