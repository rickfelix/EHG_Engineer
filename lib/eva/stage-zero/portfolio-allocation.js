...
export async function recommendProfile(deps, scores, options = {}) {
  const { logger: _logger = console } = deps;
...
export async function updateAllocationCounts(deps) {
  ...
  try {
    // Get all ventures with profile_ids. Paginated (FR-6 batch 7): allocation
    // percentages are computed from rows.length — a capped read silently skews them.
    let ventures;
    try {
      ...
      logger.warn(`   Portfolio allocation: Venture query error: ${e.message}`);
      return { updated: 0, total_ventures: 0 };
    }
