...
// VALID_COMPONENTS import removed - not currently used
...
export async function getOverridesByComponent(deps, component) {
  ...
  try {
    // Paginated (FR-6 batch 7 follow-up): the override log is append-only — a capped
    // read would silently drop older overrides from component analysis. Page errors
    // are converted below, preserving the prior warn-and-return-[] policy.
    let data;
    try {
      ...
      logger.warn(`   Override tracker: Query error: ${e.message}`);
      return [];
    }
...
export async function generateOverrideInsights(deps) {
  ...
  try {
    // Paginated (FR-6 batch 7 follow-up): total_overrides and per-component stats are
    // rows.length gauges over the full append-only log — a capped read would silently
    // clamp them at the PostgREST 1000-row cap once the log grows past it. Page errors
    // are converted below, preserving the prior warn-and-return-empty policy.
    let data;
    try {
      data = await fetchAllPaginated(() => supabase
        .from('chairman_overrides')
