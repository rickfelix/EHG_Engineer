...
/**
 * Run all 13 synthesis components on a PathOutput.
 *
...
export async function runSynthesis(pathOutput, deps = {}) {
  ...
  logger.log('   Running synthesis engine (13/13 components)...');
  ...
  const [crossRef, portfolio, reframing, moat, timeHorizon, archetype, buildCost, virality, design, narrativeRisk, techTrajectory, attentionCapital] = await Promise.all([
  ...
  const componentUsages = [crossRef, portfolio, reframing, moat, timeHorizon, archetype, buildCost, virality, design, narrativeRisk, techTrajectory, attentionCapital]
    .map(c => c.usage)
    .filter(Boolean);
