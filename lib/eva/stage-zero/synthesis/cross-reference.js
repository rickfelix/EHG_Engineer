...
async function loadIntellectualCapital(supabase) {
  ...
  // SD-LEO-INFRA-STAGE0-NURSERY-PARK-PATH-001 (adversarial review of PR #5822): this was a
  // THIRD CH-1 phantom-column read (problem_statement/solution/parked_reason/.eq('status'))
  // that failed SOFT — silently contributing zero nursery intellectual capital. Live shape:
  // parked = not yet promoted; the rich brief rides source_ref (with description fallback
  // for rows parked by other writers, e.g. the traversability gate).
  const { data: nurseryItems } = await supabase
    .from('venture_nursery')
    .select('id, name, description, source_ref')
    .is('promoted_to_venture_id', null)
    .limit(20);

  if (nurseryItems) {
    items.push(...nurseryItems.map(n => {
      const brief = n.source_ref?.brief || {};
      return {
        type: 'nursery_item',
        id: n.id,
        name: n.name,
        content: [brief.problem_statement, brief.solution].filter(Boolean).join(' - ') || n.description || '',
        context: n.source_ref?.park?.parked_reason || null,
      };
    }));
  }
  ...
  // touched this file; NOT introduced or fixed here — fails soft, out of the park-path scope;
  // routed to the coordinator with the venture_synthesis_feedback sibling): 'conclusion' is not
  // a live brainstorm_sessions column.
  const { data: brainstorms } = await supabase
    .from('brainstorm_sessions')
    .select('id, topic, conclusion, created_at') // schema-lint-disable-line
...
async function loadOutcomeHistory(supabase) {
  ...
  // PRE-EXISTING LATENT DRIFT (same class/route as above; NOT introduced or fixed here):
  // 'what_went_wrong' is not a live retrospectives column.
  const { data: retros } = await supabase
    .from('retrospectives')
    .select('id, sd_id, improvements, what_went_well, what_went_wrong') // schema-lint-disable-line
    .order('created_at', { ascending: false })
    .limit(20);
  ...
  // Load issue patterns.
  // PRE-EXISTING LATENT DRIFT (same class/route as above; NOT introduced or fixed here):
  // pattern_name/root_cause/frequency are not live issue_patterns columns.
  const { data: patterns } = await supabase
    .from('issue_patterns')
    .select('id, pattern_name, root_cause, resolution, frequency') // schema-lint-disable-line
