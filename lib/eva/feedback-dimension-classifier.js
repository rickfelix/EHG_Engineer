...
import { resolveDimensionIdentity } from './dimension-ids.js';

// eva-logger-lint-ignore: SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 adds a non-side-
// effecting id field to an existing keyword-classifier loop; no new behavior worth logging,
// and a full retrofit of this pre-existing file is out of scope.
...
async function loadDimensions(supabase) {
  ...
  if (visionResult.data?.extracted_dimensions) {
    for (let i = 0; i < visionResult.data.extracted_dimensions.length; i++) {
      ...
      const { positionalId, stableId, idKind } = resolveDimensionIdentity(dim, i, 'V');
      dimensions.push({
        id: positionalId,
        stable_id: stableId,
        id_kind: idKind,
  ...
  if (archResult.data?.extracted_dimensions) {
    for (let i = 0; i < archResult.data.extracted_dimensions.length; i++) {
      ...
      const { positionalId, stableId, idKind } = resolveDimensionIdentity(dim, i, 'A');
      dimensions.push({
        id: positionalId,
        stable_id: stableId,
        id_kind: idKind,
