...
export async function getAssignmentRecommendations(supabase, params, options = {}) {
  ...
  try {
    // Get active sessions with their claims (paginated — a capped read here would
    // silently hide available sessions; FR-6 batch 7). Page errors throw into the
    // outer catch, which preserves the prior error-return shape.
    const sessions = await fetchAllPaginated(() => supabase
      .from('claude_sessions')
      .select('session_id, sd_key, status, heartbeat_at, metadata')
...
export async function getCapacityOverview(supabase, options = {}) {
  ...
  try {
    // Paginated (FR-6 batch 7): page errors throw into the outer catch below,
    // preserving the prior { overview: emptyOverview(), error } shape.
    const sessions = await fetchAllPaginated(() => supabase
      .from('claude_sessions')
...
function calculateCapacityScore(session, heartbeatAge, isStale) {
  if (isStale) return 0;
  if (session.sd_key != null) return 0;
