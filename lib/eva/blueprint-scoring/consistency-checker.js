...
export function checkConsistency(artifacts) {
  ...
  const apiEndpoints = extractKeys(artifacts.api_contract, ['endpoints', 'routes', 'paths']);
  const wireframeRefs = extractKeys(artifacts.launch_readiness, ['screens', 'pages', 'views']);
  ...
  const stories = extractKeys(artifacts.user_story_pack, ['stories', 'epics', 'features']);
  ...
  const entities = extractKeys(artifacts.data_model, ['entities', 'tables', 'models']);
  const erdEntities = extractKeys(artifacts.erd_diagram, ['entities', 'tables', 'erDiagram']);
