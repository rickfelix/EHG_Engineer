...
const log = createLogger('CrossVentureLearning');
...
async function searchSimilar(supabase, options = {}) {
    ...
    if (tables.includes('venture_artifacts')) {
      vectorPromises.push(
        supabase.rpc('match_venture_artifacts', {
          ...
          if (error) {
            log.warn('match_venture_artifacts RPC error', { error: error.message });
            return;
          }
    ...
    if (tables.includes('issue_patterns')) {
      vectorPromises.push(
        supabase.rpc('match_issue_patterns', {
          ...
          if (error) {
            log.warn('match_issue_patterns RPC error', { error: error.message });
            return;
          }
          for (const row of data || []) {
  ...
  if (query) {
    ...
    if (error) {
      log.warn('keyword_search_fallback RPC error', { error: error.message });
    } else {
