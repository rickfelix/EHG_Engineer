...
export const S19_BRIDGE_OUTCOME = Object.freeze({
  ...
  // SD-LEO-INFRA-S19-BRIDGE-UNBLOCK-SCHEMA-DRIFT-001 FR-4: an INFRA failure (provisioning threw, e.g.
  // an unapplied-migration 42703) — distinct from ZERO_SDS_FAILURE so it is not reported as the
  // "payloads-but-zero-SDs" schism. Still HOLDS at S19, but reports the true cause.
  BRIDGE_THREW: 'bridge_threw',       // the bridge/provisioning threw an infra error (NOT a build schism)
});
...
export function classifyBridgeOutcome(bridgeResult, payloadCount) {
  ...
  // SD-LEO-INFRA-S19-BRIDGE-UNBLOCK-SCHEMA-DRIFT-001 FR-4: an infra throw (provisioning/bridge error,
  // e.g. an unapplied-migration 42703) is reported honestly as BRIDGE_THREW — NOT as the
  // payloads-but-zero-SDs schism. Recognized via an explicit threw flag or a non-vision infra error.
  if (bridgeResult && (bridgeResult.threw === true || bridgeResult.provisioning_failed === true)) {
    return S19_BRIDGE_OUTCOME.BRIDGE_THREW;
  }
