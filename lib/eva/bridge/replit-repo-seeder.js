...
// SD-LEO-INFRA-VENTURE-SCAFFOLD-CODE-001 (FR-2/FR-3): shared with the leo_bridge path's
// 'scaffold_modules_stamped' DEFAULT_STEPS step in venture-provisioner.js.
import { applyVentureScaffoldModules } from './venture-scaffold-modules-writer.js';
...
export async function seedRepo(ventureId, repoUrl, options = {}) {
  ...
  try {
    ...
    // SD-EHG-FEAT-AUTOMATED-RESILIENT-VENTURE-001-A (FR-4): thread optional stack_descriptor
    // from options through to the pure writers. When absent, behavior is byte-identical to today.
    const stackDescriptor = options.stack_descriptor || null;
    const ventureContext = { name: ventureName, screens, stackDescriptor };
    ...
    // docs/deployment-config.md — stack summary; only seeded when a stack_descriptor is present.
    // When absent, behavior is byte-identical to today (no new file).
    if (stackDescriptor) {
      const deploymentConfigLines = [
        '# Deployment Configuration',
        '',
        '> Auto-generated by the EHG venture seeder from the stack descriptor. Do not edit manually.',
        '',
        '| Field | Value |',
        '| --- | --- |',
        `| db_provider | ${stackDescriptor.db_provider || '—'} |`,
        `| deployment_target | ${stackDescriptor.deployment_target || '—'} |`,
        `| storage | ${stackDescriptor.storage || '—'} |`,
        `| region | ${stackDescriptor.region || '—'} |`,
      ];
      writeFileSync(join(docsDir, 'deployment-config.md'), deploymentConfigLines.join('\n') + '\n');
      docsCommitted.push('docs/deployment-config.md');
    }
  ...
  // SD-LEO-INFRA-VENTURE-SCAFFOLD-CODE-001 FR-2/FR-3: stamp the deploy/stack-scan/
  // feedback MODULE_REGISTRY modules + the scaffold manifest, reusing the SAME pure
  // generator + write adapter the leo_bridge path's 'scaffold_modules_stamped' step
  // calls. Must run BEFORE the git add below, or these paths (added to the allowlist
  // there) are silently never staged/pushed (the exact gap TESTING evidence 15ada745
  // found in the original PRD).
  try {
    applyVentureScaffoldModules(ventureName, repoDir);
    docsCommitted.push('scaffold modules (deploy/stack-scan/feedback) + scaffold-manifest.json');
  } catch (err) {
    errors.push(`scaffold modules: ${err.message}`);
  }
  ...
  try {
    // Base allowlist is always present by this point; the FR-2 scaffold paths are
    // only added if applyVentureScaffoldModules actually produced them above — a
    // failed/partial scaffold write must not break committing the OTHER docs (git
    // add throws "pathspec did not match" for a nonexistent path, which would abort
    // this whole add/commit/push and regress already-working behavior).
    const addPaths = ['docs/', 'replit.md', 'CLAUDE.md', '.replit'];
    if (existsSync(join(repoDir, '.github', 'workflows'))) addPaths.push('.github/workflows/');
    if (existsSync(join(repoDir, 'feedback'))) addPaths.push('feedback/');
    if (existsSync(join(repoDir, 'scaffold-manifest.json'))) addPaths.push('scaffold-manifest.json');
    execSync(`git add ${addPaths.join(' ')}`, { cwd: repoDir, encoding: 'utf-8' });
