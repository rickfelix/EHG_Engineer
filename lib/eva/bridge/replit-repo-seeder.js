...
function generateAgentOptimizedReplitMd(ventureName, stitchManifest) {
  const stitchFileMap = stitchManifest ? `
- **docs/stitch/DESIGN.md** — Design tokens: colors, typography, spacing, components
  _Read when_: Building any UI component. This is the canonical design reference.
- **docs/stitch/screens/*.html** — Self-contained HTML for each screen
  _Read when_: Building a specific page. Open the matching HTML file for pixel-accurate reference.
- **docs/stitch/screenshots/*.png** — Visual screenshots of each screen
  _Read when_: Verifying your implementation matches the intended design.
` : '';
  ...
  try {
    ...
    if (stitchArtifact?.data) {
      ...
      if (configRow?.taste_gate_config?.stitch_enabled) {
        stitchManifest = stitchArtifact.data;
      }
    }
  } catch {
