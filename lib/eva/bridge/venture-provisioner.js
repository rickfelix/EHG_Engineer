...
// SD-LEO-INFRA-VENTURE-SCAFFOLD-CODE-001 (FR-2/FR-3): deploy/stack-scan/feedback
// MODULE_REGISTRY modules + the scaffold manifest, shared with the seeded_repo path's
// seedRepo() call site (replit-repo-seeder.js).
import { applyVentureScaffoldModules, checkScaffoldManifest } from './venture-scaffold-modules-writer.js';
...
export const DEFAULT_STEPS = [
  ...
  },
  {
    ...
    execute: async (ctx) => {
      ...
      try {
        ...
        // probe previously filtered .eq('status','active'), so a venture whose
        // normalized_name matched an INACTIVE row took the INSERT branch, hit the
        // uq_applications_normalized_name UNIQUE constraint (23505), and was
        // WARN-logged-and-skipped -- the same silent-skip class FR-4 exists to fix, one
        // branch over. Match across ANY status now; still only local_path is ever
        // updated below (never status), so this does not silently reactivate a
        // deliberately-deactivated venture.
        ...
        // unique index (WHERE deleted_at IS NULL, added by
        // 20260530_applications_soft_delete_reconcile.sql) -- the SEC-5 widening above
        // did not account for tombstones, so a venture matching one of the 4 live
        // soft-deleted rows (MarketLens, CronGenius, Market Modeling SaaS, DataDistill)
        // would take the UPDATE branch onto the tombstone and never get the FR-4
        // INSERT -- the same silent-skip class, resurrected via a different mechanism.
        // .is('deleted_at', null) mirrors the index's own predicate exactly.
        // count-truncation-diff-lint (CI, PR #7482): an unbounded .select() here would
        // silently cap at PostgREST's 1000-row default, missing any collision beyond
        // that page and resurrecting the exact ApexNiche-class bug this block exists to
        // fix -- this probe must see the FULL live population to be correct, so it
        // cannot be capped/limited; fetchAllPaginated (lib/db/fetch-all-paginated.mjs)
        // range-paginates until a short page instead.
        const appRows = await fetchAllPaginated(() => sb.from('applications').select('id, name').is('deleted_at', null));
        ...
        if (match) {
          ...
          // SD-LEO-INFRA-VENTURE-SCAFFOLD-CODE-001 FR-4: the ApexNiche-class gap — a
          // brand-new venture with no pre-existing applications row was previously just
          // WARN-logged and silently skipped forever, never registered in the DB table
          // even though registry.json (above) correctly gained an entry. INSERT it now,
          // matching the schema's NOT NULL/UNIQUE contract (database/migrations/
          // 20260525_venture_build_routing_isolation_additive.sql) and the live INSERT
          // precedent at lib/sd-creation/pipeline.js:1272 (that precedent normalizes to
          // kebab-case, not this collapsed form -- VALIDATION finding V8, evidence
          // bf142c4e: harmless, the live column already holds all 3 formats and the
          // collision probe matches on normalizeAppName for both sides regardless).
          // Still non-fatal -- registry.json already succeeded as the fallback, per
          // this block's original rollback plan.
          // TESTING finding F6 (EXEC-TO-PLAN review, evidence baa1c962, MEDIUM):
          // venture_id + repo_url are both schema-supported columns the live
          // comparable row (ApexNiche AI) populates, and vw_venture_registry
          // exposes repo_url to the resolver -- omitting them left inserted rows
          // materially incomplete versus every shipped row.
          const { error: insErr } = await sb.from('applications').insert({
            name: ctx.venture.name,
            normalized_name: needle,
            kind: 'venture',
            github_repo: `rickfelix/${ctx.venture.repoName}`,
            repo_url: `https://github.com/rickfelix/${ctx.venture.repoName}`,
            local_path: dbLocalPath,
            status: 'active',
            venture_id: ctx.ventureId,
          });
          if (insErr) ctx.log(`[registry_updated] WARN: applications insert failed for "${ctx.venture.name}": ${insErr.message}`);
          else ctx.log(`[registry_updated] applications row inserted for "${ctx.venture.name}" -> ${dbLocalPath}`);
        }
      } catch (dbErr) {
  ...
  },
  {
    ...
    execute: async (ctx) => {
      ...
      // SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-A: throwOnSchemaDrift:false is a DELIBERATE, reviewed
      // opt-out (VAL-A-1, LEAD validation-agent) -- this probe already distinguishes the
      // genuinely-optional stack_descriptor column (42703, degrade below) from a real read
      // error (fail-loud below) itself. The factory's default throw-on-42703 would fire before
      // this code ever saw vErr, re-wedging the exact S19 hard-loop this file's next comment
      // documents fixing.
      const sb = createSupabaseServiceClient({ throwOnSchemaDrift: false });
...
`;
  ...
  {
    // SD-LEO-INFRA-VENTURE-SCAFFOLD-CODE-001 FR-2/FR-3. check() reuses
    // checkScaffoldManifest() (FR-3's shared build-gate predicate) so idempotent
    // re-entry and the standalone gate check can never disagree with each other.
    // execute() does not swallow errors (unlike registry_updated's deliberately
    // non-fatal DB write-through) -- a thrown error here is a genuine step
    // failure, which is how this step blocks provisioning on a broken/missing
    // manifest through the step-runner's own existing failure semantics.
    //
    // Adversarial deep-tier review finding (ship-adversarial-review, PR #7482,
    // WARNING): a `ctx.stepsCompleted.includes(...) return true` shortcut here
    // (removed) let this step be marked permanently 'completed' by
    // executeStepWithRetry the FIRST time execute() ran with no repoPath
    // available (a clean, non-throwing skip -- see execute() below), even
    // though it did nothing. That flag is persisted to
    // venture_provisioning_state.steps_completed, and once set, this check()
    // used to trust it forever -- never re-verifying against the filesystem
    // even after a repo path became available on a later run. This is the
    // same silent-skip-forever class FR-4 exists to eliminate, one layer up.
    // cicd_configured (above) never had this shortcut -- its check() always
    // re-verifies live via existsSync -- despite this step's own prior comment
    // claiming to match it. Removed to actually match: check() now always
    // re-verifies checkScaffoldManifest() against the live filesystem.
    name: 'scaffold_modules_stamped',
    check: async (ctx) => {
      const repoPath = ctx.ventureRepoPath || ctx.venture?.localPath;
      // ctx.ventureRepoPath can be a GitHub URL rather than a local path (a
      // pre-existing, independent condition -- see stage-execution-worker.js);
      // existsSync() safely returns false for a URL string, so this no-ops
      // rather than misbehaving, matching cicd_configured's own guard above.
      if (!repoPath || !existsSync(repoPath)) return false;
      return checkScaffoldManifest(repoPath).ok;
    },
    execute: async (ctx) => {
      if (!ctx.venture) {
        ctx.venture = await resolveVentureMetadata(ctx.ventureId);
      ...
      const repoPath = ctx.ventureRepoPath || ctx.venture?.localPath;
      if (!repoPath || !existsSync(repoPath)) {
        ctx.log('[scaffold_modules_stamped] No local repo path — skipping');
        return;
      }
      applyVentureScaffoldModules(ctx.venture?.name || ctx.ventureId, repoPath, { logger: ctx.log });
    },
  },
  {
