...
export async function generateScreens(projectId, prompts, ventureId) {
  ...
  for (const prompt of prompts) {
    ...
    try {
      ...
      // Socket drops are expected — server is likely still processing.
      // Caller should poll listScreens() to verify completion.
      const isSocketDrop = /fetch failed|socket|ECONNRESET|other side closed|Already connected/i.test(err.message || '');
      if (isSocketDrop) {
        results.push({
          prompt: prompt.slice(0, 60),
          status: 'fired',
        ...
        console.warn(`[stitch-client] generateScreens: socket dropped (server likely still processing): ${err.message}`);
      } else {
