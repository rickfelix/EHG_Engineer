...
async function loadBudget(ventureId) {
  ...
  const budget = { used: data?.data?.used || 0 };
...
function validateResponse(response, requiredFields, context) {
...
async function withCircuitBreaker(fn, label) {
...
// ---------------------------------------------------------------------------
// Stitch Client (uses `stitch` singleton from SDK)
// ---------------------------------------------------------------------------

/**
 * Get the stitch singleton. The SDK reads STITCH_API_KEY from env automatically.
 */
async function getClient() {
  getApiKey(); // validates env var is set
  const sdk = await getSDK();
  // SDK exports a `stitch` singleton that reads STITCH_API_KEY from env
  return sdk.stitch || sdk;
}
...
export async function createProject(options) {
  ...
  return instrumentedCall('createProject', async () => {
    ...
    return {
      ...
      url: `https://stitch.withgoogle.com/project/${projectId}`,
    };
  });
}
