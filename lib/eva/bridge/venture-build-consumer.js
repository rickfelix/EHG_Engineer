...
export async function markConsumed(supabase, topId, { dryRun = false, mergeMetadataKeysFn = null } = {}) {
  if (dryRun) return { marked: false, dryRun: true };
  const { data: fresh } = await supabase.from('strategic_directives_v2').select('sd_key').eq('id', topId).maybeSingle();
  if (!fresh?.sd_key) throw new Error(`build_consumed_at update failed: no row for id=${topId}`);
  // SD-LEO-FIX-STRATEGIC-DIRECTIVES-UPDATED-001: a full-blob .update({ metadata: newMeta })
  // here silently clobbers any key a concurrent writer set between this read and this write.
  // mergeMetadataKeys() writes ONLY the two stamp keys via an atomic JSONB `||` merge.
  ...
  const { merged, error } = await mergeFn(fresh.sd_key, {
    build_consumed_at: new Date().toISOString(),
    build_consumed_by: 'venture-build-consumer',
  });
  if (!merged) throw new Error(`build_consumed_at update failed: ${error || 'no row matched'}`);
  return { marked: true };
}
