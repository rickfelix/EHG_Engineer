 ...
 * SD-EHG-FEAT-AUTOMATED-RESILIENT-VENTURE-001-A (descriptor-driven FR-2)
 *
 ...
 *
 * Stack descriptor dispatch (ctx.stackDescriptor):
 *   ABSENT or deployment_target === 'replit-autoscale'  →  Replit path (DEFAULT, byte-identical)
 *   otherwise (cloudflare-pages/workers/cloud-run)      →  Cloud path
 *
 ...
 * @param {Object} [ctx.stackDescriptor] - optional stack descriptor (see lib/venture-deploy/stack-descriptor.js)
...
export function buildBuildTasks(ctx = {}) {
  ...
  // Dispatch on the deployment-target FAMILY (fail-safe: unknown/invalid => 'replit').
  // 'replit' => DEFAULT path (byte-identical). 'cloudflare'/'cloud-run' => cloud path,
  // which is DESCRIPTOR-DRIVEN (not Cloudflare-hardcoded): labels come from the descriptor.
  const family = deployTargetFamily(sd);
  const useCloud = family !== 'replit';

  // Cloud platform prose label (cloudflare => 'Cloudflare'; cloud-run => 'GCP Cloud Run').
  const platformLabel = family === 'cloud-run' ? 'GCP Cloud Run' : 'Cloudflare';
  const isCloudflare = family === 'cloudflare';

  const dbLabel = resolveDbLabel(sd);
  const storageLabel = resolveStorageLabel(sd);

  // Child-1 heading and backend lines vary by target — descriptor-driven on cloud.
  const child1Heading = useCloud
    ? `### Child 1 — Discover current state & ${platformLabel}-native backend`
    : '### Child 1 — Discover current state & Replit-native backend';

  const line12 = useCloud
    ? `- [ ] **1.2 ${dbLabel}** — ${dbLabel} via ${isCloudflare ? 'a Workers binding / `wrangler d1`' : 'a `DATABASE_URL` connection'}; remove any Supabase coupling.`
    : '- [ ] **1.2 Replit-native data** — Replit Postgres via `DATABASE_URL` (Drizzle or `pg`); remove any Supabase coupling. Schema in code.';

  const line13 = useCloud
    ? `- [ ] **1.3 ${storageLabel}** — ${storageLabel} (bucket + key-prefix namespacing); access via ${isCloudflare ? 'the R2 Workers binding' : 'the platform object-storage binding'}, never a local GCS signer.`
  ...
  // 3.4 Stack-enforcing CI line varies by target — descriptor-driven on cloud.
  const line34 = useCloud
    ? `- [ ] **3.4 Stack-enforcing CI (REQUIRED)** — commit a CI workflow that, on every PR to main, runs (a) the venture-stack compliance test \`tests/stack-compliance.test.js\` (scan \`src/\` + \`lib/\`, fail on forbidden tech found in \`src/\` — no \`@supabase\`, no Replit Auth / OIDC — and assert the REQUIRED stack is present anywhere in \`src/\`+\`lib/\`: Clerk + ${dbLabel} + the shared usage-event RPC; no \`@supabase\`) AND (b) the build. Make the CI check a **required status check** (branch protection) so off-stack code or a broken build cannot merge. Vendor the test from the provided template (\`venture-stack-compliance.test.template.js\`); it is dependency-free (\`node --test\`) so it runs under any toolchain. This is the per-PR code gate the artifact-level stage-19 gate cannot provide.`
    : '- [ ] **3.4 Stack-enforcing CI (REQUIRED)** — commit a CI workflow that, on every PR to main, runs (a) the venture-stack compliance test `tests/stack-compliance.test.js` (scan `src/` + `lib/`, fail on forbidden tech found in `src/` — no `@supabase`, no Replit Auth / OIDC, no CLI-as-product framing — and assert the REQUIRED stack is present anywhere in `src/`+`lib/`: Clerk + Replit Postgres + the shared usage-event RPC) AND (b) the build. Make the CI check a **required status check** (branch protection) so off-stack code or a broken build cannot merge. Vendor the test from the provided template (`venture-stack-compliance.test.template.js`); it is dependency-free (`node --test`) so it runs under any toolchain. This is the per-PR code gate the artifact-level stage-19 gate cannot provide.';
