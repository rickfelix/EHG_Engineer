...
// there are advisory-only, not build-breaking) so calling this via the DEFAULT realIo() cannot
// retroactively hard-fail CI for a venture with pre-existing, previously-unscanned lib/ content.
// SCOPE OF THIS CLAIM (adversarial REGRESSION review): it holds for the default `realIo` path,
// where lib/ genuinely went unwalked before this change. It does NOT extend to a caller who
// already passes scanForStackViolations() an explicit `ioOrFactory` whose `files` include lib/
// paths -- such a caller was previously getting build-breaking violations for those files and
// now gets advisory warnings instead, a real (if currently unreached: 0 such callers exist
// outside this file's own tests) behavior change for that injection point.
 ...
 *
 * ASYMMETRIC lib/ TREATMENT (SD-LEO-GEN-ALL-VENTURES-PRODUCED-001-D): the default realIo() now
 * walks both src/ and lib/ so REQUIRED[] content checks (e.g. usage_events) can see a witness
 * that lives in lib/. FORBIDDEN checks stay build-breaking (violations) for src/-rooted files
 * only; a would-be forbidden finding rooted in lib/ downgrades to the additive warnings array
 * instead, so calling this via the default realIo() cannot retroactively hard-fail CI for a
 * venture with pre-existing, previously-unscanned lib/ content it never had a chance to fix.
 * This claim is scoped to the default `realIo` path -- see the WALK_ROOTS comment above for the
 * caveat when a caller supplies its own `ioOrFactory` already listing lib/ paths.
 */
