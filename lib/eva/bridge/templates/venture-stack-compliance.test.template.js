...
// SEC-D-2 (adversarial SECURITY review, evidence f27688bb-ee56-4e29-8f56-e17f7644d4da): a flat,
// undifferentiated print collapses a B1-class auth/off-stack-tech finding (lib_root_forbidden)
// into the same visual weight as a stylistic stateless-process nit -- print the former with a
// louder, distinct marker so a human skimming CI logs cannot mistake it for routine noise.
test('venture-stack advisory warnings (non-blocking, printed for visibility)', () => {
  const { warnings } = scanForStackViolations(ROOT);
  const forbiddenClass = warnings.filter((w) => w.class === 'lib_root_forbidden');
  const otherClass = warnings.filter((w) => w.class !== 'lib_root_forbidden');
  if (forbiddenClass.length > 0) {
    console.warn(
      `venture-stack-scan: ${forbiddenClass.length} FORBIDDEN-TECH finding(s) in lib/ (non-blocking here, but the same finding in src/ WOULD fail the build):\n`
      + forbiddenClass.map((w) => `  - [${w.class}] ${w.file}: ${w.why}`).join('\n'),
    );
  }
  if (otherClass.length > 0) {
    console.warn(
      `venture-stack-scan: ${otherClass.length} other advisory warning(s) (non-blocking, does not fail CI):\n`
      + otherClass.map((w) => `  - [${w.class}] ${w.file}: ${w.why}`).join('\n'),
    );
  }
});
