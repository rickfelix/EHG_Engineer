...
// PLAN-phase VALIDATION (V-3): this fetch is awaited inside chairman-product-review.js's
// generateReviewPacket, on the S24 choke-point (that file already carries a 5000ms budget
// for the CompetitiveBaselineService read, since Supabase/undici calls have no default
// timeout and a bare await can hang the packet indefinitely). A third-party internet call
// is strictly more stall-prone than that Supabase call, so it needs the identical budget.
...
export async function readStackScanConclusion({
  ...
  try {
    ...
    const res = await fetchImpl(url, { headers, signal: AbortSignal.timeout(GITHUB_API_TIMEOUT_MS) });
    if (!res.ok) {
    ...
    if (err?.name === 'TimeoutError') {
      logger?.warn?.(`[StackScanReader] GitHub API call timed out after ${GITHUB_API_TIMEOUT_MS}ms`);
