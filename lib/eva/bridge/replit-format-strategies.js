...
// Plan Mode sizing is feature-count bounded, not char-count bounded.
// Sonnet's context window (200k–1M tokens) dwarfs anything we'd send, but plan
// QUALITY degrades with too many features at once: Replit's own community guide
// recommends "Must-have features (3 max)" for Plan Mode, and LLMs exhibit
// "lost in the middle" where mid-context items get underweighted. We cap at 5
// (3-max + LEO headroom) and overflow into docs/tasks.md, which the repo seeder
// already writes (replit-repo-seeder.js:651) and the binding contract surfaces.
// The 6000-char budget is a belt-and-suspenders truncation guard — it should
// rarely fire once the feature cap is in place.
const PLAN_MODE_BUDGET = 6000;
const PLAN_MODE_TOP_FEATURES = 5;

const PRIORITY_RANK = { critical: 4, high: 3, medium: 2, low: 1 };

function rankSprintItems(items) {
  return items
    .map((item, index) => ({ item, index }))
    .sort((a, b) => {
      const pa = PRIORITY_RANK[String(a.item.priority || 'medium').toLowerCase()] || 2;
      const pb = PRIORITY_RANK[String(b.item.priority || 'medium').toLowerCase()] || 2;
...
export function formatPlanModePrompt(groups, venture, summary) {
  ...
  // (after GitHub import) doesn't include marketing-copy.md or gtm-strategy.md
  // in its File Map, so we reference them directly here. Per Rule 7 of the
  // pre-approval playbook (validated 2026-04-28), point Replit Agent at
  // files rather than describe their contents inline. Replit cannot fetch
  // from GitHub URLs but reads the imported workspace as a local filesystem.
  ...
  lines.push('- docs/architecture.md — tech stack, data model, API surface. Use these decisions; do not re-derive.');
  lines.push('- docs/gtm-strategy.md — go-to-market positioning context.');
  lines.push('');

  // MVP features from sprint items — capped at PLAN_MODE_TOP_FEATURES per
  // Replit's "3 max" Plan Mode guidance (with LEO headroom). Overflow features
  // live in docs/tasks.md, which the repo seeder writes and the binding
  // contract references.
  if (items.length > 0) {
    const ranked = rankSprintItems(items);
    const planItems = ranked.slice(0, PLAN_MODE_TOP_FEATURES);
    const overflow = ranked.length - planItems.length;
    lines.push(`MVP Features (top ${planItems.length} of ${ranked.length} by priority):`);
    ...
    if (overflow > 0) {
      lines.push(`See docs/tasks.md for the remaining ${overflow} feature${overflow === 1 ? '' : 's'} (full backlog in build order).`);
    }
    lines.push('');
  }

  // Architecture summary previously inlined here; removed 2026-04-28 because
  // the `why_these_decisions` group's first artifact is often a strategic/
  // market-rationale doc, not the technical decisions doc — Replit Agent was
  // anchoring on the wrong context for stack/data-model planning. The binding
  // contract now references docs/architecture.md (written by the repo seeder,
  // replit-repo-seeder.js:652) instead. Doc-driven > inline summary.
  ...
  // Enforce budget: truncate feature descriptions if over limit. The binding
  // contract block is preserved in the truncated fallback because it's the
  // load-bearing part — features can be described tersely, but Replit Agent
  ...
  if (prompt.length > PLAN_MODE_BUDGET) {
    ...
    shortLines.push('- docs/architecture.md (tech stack + data model — use these decisions)');
    shortLines.push('');
    const rankedShort = rankSprintItems(items).slice(0, PLAN_MODE_TOP_FEATURES);
    const overflowShort = items.length - rankedShort.length;
    shortLines.push(`MVP Features (top ${rankedShort.length} of ${items.length}):`);
    for (let i = 0; i < rankedShort.length; i++) {
      shortLines.push(`${i + 1}. ${rankedShort[i].name} (${rankedShort[i].storyPoints} pts)`);
    }
    if (overflowShort > 0) {
      shortLines.push(`See docs/tasks.md for the remaining ${overflowShort} feature${overflowShort === 1 ? '' : 's'}.`);
    }
    shortLines.push('');
    shortLines.push('Build in order listed. Each feature must fulfill the corresponding promise in docs/marketing-copy.md.');
...
export function formatFeaturePrompts(groups, venture, summary) {
  ...
  return items.map((item, index) => {
    ...
    if (index === 0) {
      ...
      lines.push('- Otherwise, generate a simple SVG text logo using the app name and primary brand color from `docs/color-palette.md`');
      lines.push('- Save as `public/logo.svg` and reference it in the header/nav');
    }

    // Instructions — reference docs/* files directly rather than relying on
    // replit.md to act as a pointer index. Replit Agent's auto-generated
    // replit.md (created during GitHub import) doesn't include marketing-copy.md
    // or gtm-strategy.md in its File Map, so we link them explicitly.
    lines.push('');
    ...
    return {
      ...
      // Frontend-render fields (added 2026-04-28 for the Stage 19 unification —
      // BuildMethodSelector.tsx now consumes this directly via the /api/stage19/
      // replit-prompts route instead of duplicating the prompt-building logic).
      title: item.name,
