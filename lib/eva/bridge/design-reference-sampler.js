...
export function blendInfluence(rows, { seedStr, archetype = null }) {
  ...
  for (const dim of TOKEN_DIMENSIONS) {
    ...
    dimensions[dim] = {
      // Library text is external-origin (Awwwards scrape + LLM extraction) and flows into an
      // LLM build prompt — route it through the same sanitization seam the sibling consumer
      // uses (design-directive-generator.js), capped so 7 dimensions stay bounded (W3 fix).
      influence: sanitizeDesignReference(pick.design_tokens[dim], 280),
      source_site: sanitizeDesignReference(pick.site_name, 80),
      source_archetype: sanitizeDesignReference(pick.archetype_category, 40),
    };
  }
...
/** Hard bound on the library read — fail-soft means generation NEVER blocks on grounding. */
const SAMPLER_TIMEOUT_MS = 4000;

/**
 * DB-backed entry point. FAIL-SOFT by contract: returns null on any error, empty library,
 * missing supabase, OR a read that does not settle within SAMPLER_TIMEOUT_MS — a hung
 * client (or a test double whose thenable never resolves) must degrade to "no section",
 * never stall tree creation.
 *
...
export async function sampleDesignInfluence({ ventureId, archetype = null, supabase }) {
  try {
    ...
    // Stable ordering is load-bearing (adversarial W1): blendInfluence picks are row-order
    // sensitive, and Postgres row order without ORDER BY is unstable — without this, the
    // per-venture reproducibility contract silently breaks across vacuum/plan changes.
    const read = supabase
    ...
    const res = await Promise.race([
      ...
      new Promise((resolve) => {
        const t = setTimeout(() => resolve({ __samplerTimeout: true }), SAMPLER_TIMEOUT_MS);
        if (typeof t?.unref === 'function') t.unref();
      }),
    ]);
    if (!res || res.__samplerTimeout || res.error || !res.data?.length) return null;
    return blendInfluence(res.data, { seedStr: String(ventureId), archetype });
  } catch {
    return null;
  }
}
