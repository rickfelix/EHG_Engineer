...
export function applyVentureScaffoldModules(ventureName, repoPath, opts = {}) {
  ...
  // SECURITY finding SEC-2 (EXEC-TO-PLAN review, evidence 6f9eabc9): the raw venture
  // name was interpolated unquoted into generated deploy.yml `run:` lines that execute
  // in a job holding Cloudflare deploy credentials -- a hostile name (e.g. containing
  // backticks or a newline) could inject shell commands or extra YAML steps. Normalize
  // here, the ONE place both entry points (leo_bridge + seeded_repo) funnel through,
  // using the SAME kebab normalization already applied to repoName at
  // venture-provisioner.js:84 for consistency.
  ...
  for (const key of modules) {
    ...
    const files = mod.generate(safeVentureName, repoPath, opts.moduleOptions || {});
