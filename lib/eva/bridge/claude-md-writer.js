...
- Commit a CI workflow that runs on **every PR to main** and is a **required status check** (so it BLOCKS merge): it runs (a) a venture-stack compliance test — \`tests/stack-compliance.test.js\`, which scans \`src/\` + \`lib/\` and fails on forbidden tech found in \`src/\` — no \`@supabase\`, no Replit Auth / OIDC, no CLI-as-product framing — and ${ciStackLine} (checked anywhere in \`src/\`+\`lib/\`, including the shared usage-event RPC) — AND (b) the build. Vendor the dependency-free test from \`venture-stack-compliance.test.template.js\` (runs under \`node --test\`, any toolchain). Without this, off-stack code merges unseen: the platform's stage-19 gate scans planning artifacts, **not** your repo code.
`;
}
