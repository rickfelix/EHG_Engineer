...
function mockSupabase({ artifacts = [], visionUpsertData = null, visionUpsertErr = null, archUpsertData = null, archUpsertErr = null, existingVisions = [] } = {}) {
  ...
  const archUpsertCalls = [];
  return {
    _visionUpsertCalls: visionUpsertCalls,
    _archUpsertCalls: archUpsertCalls,
    from: vi.fn((table) => {
...
describe('generateDocs', () => {
  ...
  // SD-LEO-INFRA-ARCHITECTURE-PLANS-GET-001 (FR-3): the arch-plan-side counterpart of the
  // QF-262 guard above. Prior to this SD, stage-17 wrote a DRAFT/unapproved vision followed
  // immediately by an ACTIVE/chairman-approved architecture plan claiming alignment to it --
  // an incoherent state. Drives BOTH call sites independently (primary write, and the
  // quality-retry write reached only when the primary write returns quality_checked:false
  // with non-empty quality_issues) so a mutant reverting either site's approved:false is
  // caught, not just the primary one.
  test('writes a DRAFT architecture plan (approved:false) at BOTH call sites — never active/chairman_approved', async () => {
    const visionData = {
      id: 'v-uuid', vision_key: 'VISION-TEST-L2-003', level: 'L2',
      version: 1, status: 'draft', quality_checked: true, quality_issues: null,
    };
    // quality_checked:false + non-empty quality_issues forces the primary write's result
    // into the retry loop (lines ~390-400), exercising the SECOND call site too.
    const archData = {
      id: 'a-uuid', plan_key: 'ARCH-TEST-003', version: 1,
      status: 'draft', vision_id: 'v-uuid', quality_checked: false, quality_issues: ['sparse content'],
    };
    const supabase = mockSupabase({ artifacts: [], visionUpsertData: visionData, archUpsertData: archData });

    const result = await generateDocs({ ventureId: 'v-1', ventureName: 'Test Venture', supabase, logger: silentLogger });

    // Both the primary write and the retry-loop write(s) must have fired (vision present ->
    // archResult attempted; quality_checked:false -> retry loop entered).
    expect(supabase._archUpsertCalls.length).toBeGreaterThanOrEqual(2);
    for (const rec of supabase._archUpsertCalls) {
      expect(rec.status).toBe('draft');
      expect(rec.status).not.toBe('active');
      expect(rec.chairman_approved).toBe(false);
    }
