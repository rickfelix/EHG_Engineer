...
// SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 (FR-3/FR-7, TS-3): re-extraction preserves
// stable dimension ids for unchanged names.
describe('upsertVision — stable dimension id assignment (FR-3)', () => {
  test('assigns a fresh id to a dimension with no prior id', async () => {
    const captured = {};
    const supabase = mockSupabase({
      existing: { id: 'u', version: 1, addendums: [], extracted_dimensions: [] },
      upsertResult: { id: 'u', vision_key: 'V-1' },
      captured,
    });
    ...
    await upsertVision({
      ...
      visionKey: 'V-1',
      content: 'test',
      dimensions: [{ name: 'Automation By Default', weight: 0.5, description: 'x' }],
    });

    expect(captured.record.extracted_dimensions).toHaveLength(1);
    expect(captured.record.extracted_dimensions[0].id).toBe('automation-by-default');
  });

  test('preserves the existing id for an unchanged dimension name and assigns a fresh id only to a renamed one', async () => {
    const captured = {};
    const supabase = mockSupabase({
      existing: {
        id: 'u',
        ...
        extracted_dimensions: [
          { name: 'CLI Authoritative Workflow', id: 'cli-authoritative-workflow', weight: 0.5, description: 'x' },
          { name: 'Old Name', id: 'old-name', weight: 0.5, description: 'y' },
        ],
      },
      upsertResult: { id: 'u', vision_key: 'V-1' },
      captured,
    ...
    await upsertVision({
      ...
      dimensions: [
        { name: 'CLI Authoritative Workflow', weight: 0.5, description: 'x' }, // unchanged name, no .id supplied
        { name: 'New Name', weight: 0.5, description: 'y' }, // renamed
      ],
    });

    const dims = captured.record.extracted_dimensions;
    expect(dims.find((d) => d.name === 'CLI Authoritative Workflow').id).toBe('cli-authoritative-workflow');
    expect(dims.find((d) => d.name === 'New Name').id).toBe('new-name');
  });

  test('does not assign ids when dimensions is omitted (carry-forward-on-omit path is untouched)', async () => {
    const captured = {};
    const supabase = mockSupabase({
      existing: { id: 'u', version: 1, addendums: [], extracted_dimensions: ['x'] },
      upsertResult: { id: 'u', vision_key: 'V-1' },
    ...
    expect(captured.record.extracted_dimensions).toEqual(['x']);
  });
});
