...
describe('promoteArchPlan', () => {
  ...
  // TS-6b: NULL created_by case, explicitly decided as ALLOW, not refuse (VERIFY-phase
  // VALIDATION review finding W3 -- an earlier comment here said "refuse-on-null is the safer
  // default," which named the opposite of both this test and the shipped code). The guard
  // clause above requires promotedBy to be truthy, so promotedBy === null can never be true --
  // a NULL created_by row is therefore ALWAYS promotable by any real identity. This is the
  // LEAST protected case in the table (18 live rows have NULL created_by), a known limitation
  // of the provenance-placeholder guard documented in archplan-promote.js's own docblock,
  // pending the deferred approved_by column (FR-6).
  ...
  // TS-10 (updated by SD-LEO-INFRA-GOVERNANCE-ARTIFACTS-RECORD-001 FR-2): the UPDATE payload
  // must contain status/chairman_approved/chairman_approved_at/approved_by/approved_by_at --
  // never created_by, content, or sections. approved_by/approved_by_at were added by FR-2 to
  // persist the approver identity this function already verifies at the self-approval guard;
  // see tests/unit/eva/archplan-promote.test.js for the full FR-2 coverage (distinct-seat
  // write, fail-soft 42703 retry, TOCTOU race).
  test('the UPDATE payload contains status/chairman_approved/chairman_approved_at/approved_by/approved_by_at, never created_by/content/sections', async () => {
    const supabase = mockSupabase({
    ...
    expect(Object.keys(payload).sort()).toEqual(['approved_by', 'approved_by_at', 'chairman_approved', 'chairman_approved_at', 'status']);
