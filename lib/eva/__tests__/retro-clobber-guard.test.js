 ...
 *   - FR-1 cancel-sd.js cancelled_at fix: 3
 *   - Static guard: 9 wire-in sites pinned: 1
 *   - Per-site source-level wiring assertions: 6
 *
...
// ── FR-1 cancel-sd.js cancelled_at fix (3 cases) ──

describe('FR-1: cancel-sd.js cancelled_at column fix', () => {
  const src = readFileSync(path.resolve(__dirname, '../../../scripts/cancel-sd.js'), 'utf8');

  test('cancel-sd.js no longer references cancelled_at in UPDATE payload', () => {
    // Find the strategic_directives_v2 UPDATE block and verify cancelled_at not present
    const updateBlock = src.match(/const updates = \{[\s\S]+?\};/)?.[0] || '';
    expect(updateBlock).not.toMatch(/cancelled_at/);
    expect(updateBlock).toMatch(/cancellation_reason/); // sibling col still present
    expect(updateBlock).toMatch(/updated_at/); // proxy timestamp still present
  });

  test('cancel-sd.js populates cancelled_by from CLAUDE_SESSION_ID', () => {
    expect(src).toMatch(/cancelled_by: process\.env\.CLAUDE_SESSION_ID/);
  });

  test('cancel-sd.js comment cites the SD-FDBK-INFRA-HANDOFF-RETRO-GENERATORS-001 fix rationale', () => {
    expect(src).toMatch(/SD-FDBK-INFRA-HANDOFF-RETRO-GENERATORS-001/);
  });
});

// ── Static guard: 9 wire-in sites pinned ──

describe('Static guard: 9 wire-in sites consult isSafeToWriteRetro', () => {
  const sites = [
    ...
    'scripts/modules/orchestrator-completion-guardian.js',
  ];
  ...
  test('modules/orchestrator-completion-guardian.js wires BOTH INSERT and UPDATE paths', () => {
    const src = readFileSync(
      path.resolve(__dirname, '../../../scripts/modules/orchestrator-completion-guardian.js'),
      'utf8'
    );
    const matches = src.match(/isSafeToWriteRetro/g) || [];
    expect(matches.length).toBeGreaterThanOrEqual(2);
