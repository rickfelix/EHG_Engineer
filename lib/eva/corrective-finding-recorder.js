 ...
 *
 * SD-LEO-INFRA-CORRECTIVE-FINDING-GENERATOR-001 (FR-3): keyed on a STABLE natural
 * tuple -- source_sd_id, dimensions, and a coarse score-band -- not gate_run_id.
 * gate_run_id is a fresh UUID minted on every scoring pass (scripts/eva/vision-scorer.js
 * bare .insert()), so a hash that included it could only ever match a literal re-run of
 * the same score record, never recognize a recurring gap across separate scoring passes.
 * gate_run_id is still recorded on the finding row (top-level column), just excluded
 * from the hash payload so repeated occurrences of the same gap dedupe correctly.
 *
 * @param {string|null} sourceSdId
 * @param {string[]} dimensions
 * @param {number|null} scoreBand - coarse score band (e.g. Math.floor(score/10)*10)
 * @returns {string} 64-char hex sha256
 */
export function computeDedupHash(sourceSdId, dimensions, scoreBand) {
  const dims = Array.isArray(dimensions) ? [...dimensions].sort().join(',') : '';
  const payload = `${sourceSdId ?? 'null'}::${dims}::${scoreBand ?? 'null'}`;
...
export async function recordCorrectiveFinding(supabase, finding) {
  ...
  const scoreBand = typeof score === 'number' ? Math.floor(score / 10) * 10 : null;
  const dedupHash = computeDedupHash(source_sd_id, dimensions, scoreBand);
