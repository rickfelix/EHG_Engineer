...
// SD-LEO-INFRA-UNBLOCK-PORTFOLIO-WIDE-001 (FR-4): the minimum lifecycle stage a venture must
 ...
 * env LEO_TEMPLATE_EXTRACT_MIN_STAGE > DEFAULT_MIN_EXTRACT_STAGE. Non-finite / out-of-range
 * values fall back to the default (no silent gate-disabling).
 * @param {{minStage?: number}} [opts]
 * @param {object} [env]
 * @returns {number}
 */
function resolveMinExtractStage(opts = {}, env = (typeof process !== 'undefined' ? process.env : {})) {
  const candidates = [opts && opts.minStage, env && env.LEO_TEMPLATE_EXTRACT_MIN_STAGE];
  for (const c of candidates) {
    if (c === undefined || c === null || c === '') continue;
    const n = Number(c);
    if (Number.isFinite(n) && n >= 1 && n <= 40) return Math.floor(n);
  }
  return DEFAULT_MIN_EXTRACT_STAGE;
}

/**
 * Extract a reusable template from a venture that has reached the minimum extraction stage.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} ventureId - UUID of the venture
 * @param {{minStage?: number}} [opts] - optional gate override (default: stage 26, prior behavior)
 * @returns {Promise<{id: string, template_name: string, template_version: number}|null>}
 *   Created template record, or null if venture hasn't reached the minimum stage
 */
async function extractTemplate(supabase, ventureId, opts = {}) {
  const minStage = resolveMinExtractStage(opts);
  // 1. Verify venture reached the minimum extraction stage
  ...
  if (!venture || venture.current_lifecycle_stage < minStage) return null;
...
async function extractScoringThresholds(supabase, ventureId) {
  const { data: decisions } = await supabase
    ...
    .select('lifecycle_stage, decision, health_score')
    .eq('venture_id', ventureId)
...
async function extractPricingParams(supabase, ventureId) {
  ...
  const { data: artifacts } = await supabase
    ...
    .eq('venture_id', ventureId)
    .eq('artifact_type', 'engine_revenue_model')
    .eq('is_current', true)
    .limit(1)
    .single();
