...
// advisory-only here (this gate already fails OPEN on error/empty lists by design -- a
// provenance finding never adds a new way to block, matching that existing posture).
import { gradeVentureArtifactProvenance } from './venture-artifact-provenance.js';
import { createLogger } from '../logger.js';

const logger = createLogger('StageArtifactPrecondition');

/**
 * @param {object} supabase - Supabase client
 * @param {string} ventureId
 * @param {number} stage - the FROM stage (artifacts required to leave this stage)
 * @returns {Promise<{ blocked: boolean, missingArtifacts: string[], deviatedArtifacts: string[], provenanceWarnings: string[], source: string }>}
 *   provenanceWarnings (SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G, FR-5): artifact_types that ARE
 *   present but fail machine-provenance grading -- advisory only, never contributes to `blocked`.
 */
export async function checkStageArtifactPrecondition(supabase, ventureId, stage) {
  ...
  if (ventureError) {
    ...
    logger.warn('checkStageArtifactPrecondition failing open on ventures read error', { ventureId, stage, error: ventureError.message });
    return { blocked: false, missingArtifacts: [], provenanceWarnings: [], source: 'check_error_failopen', error: ventureError.message };
  }
  ...
  if (requiredArtifacts.length === 0) {
    return { blocked: false, missingArtifacts: [], provenanceWarnings: [], source };
  }

  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): metadata/artifact_data/content/created_at
  // added so present artifacts can be graded for machine provenance (advisory-only below).
  const { data: presentArtifacts, error: artifactsError } = await supabase
    .from('venture_artifacts')
    .select('artifact_type, metadata, artifact_data, content, created_at')
    .eq('venture_id', ventureId)
    .eq('is_current', true)
    .in('artifact_type', requiredArtifacts)
    // count-truncation-diff-lint: bounded -- at most one is_current row per requiredArtifacts
    // entry per venture, well under this cap.
    .limit(999);
  if (artifactsError) {
    return { blocked: false, missingArtifacts: [], provenanceWarnings: [], source: 'check_error_failopen', error: artifactsError.message };
  }

  const presentRows = presentArtifacts || [];
  const presentSet = new Set(presentRows.map((a) => a.artifact_type));
  const notPresent = requiredArtifacts.filter((a) => !presentSet.has(a));

  // Advisory-only (never blocks): a present-but-unprovenanced artifact is recorded, not refused.
  // A pre-cutover legacy row (the overwhelming majority of the live corpus) is graded leniently
  // and never appears here.
  const provenanceWarnings = presentRows
    .filter((row) => gradeVentureArtifactProvenance(row).absent)
    .map((row) => row.artifact_type);

  if (notPresent.length === 0) {
    return { blocked: false, missingArtifacts: [], deviatedArtifacts: [], provenanceWarnings, source };
  }
  ...
  return { blocked: missingArtifacts.length > 0, missingArtifacts, deviatedArtifacts, provenanceWarnings, source };
}
