 ...
 */
async function fetchVentureForFixtureCheck(supabase, ventureId, logger = console) {
  try {
    const { data } = await supabase.from('ventures').select('is_demo, name').eq('id', ventureId).maybeSingle();
    return data || null;
  } catch (err) {
