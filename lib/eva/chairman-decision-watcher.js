...
import { resetGateAttempts } from './gate-retry-guard.js';
...
export async function createOrReusePendingDecision({
  ...
  // Explicit attemptNumber (caller-supplied, currently unused by any caller) still wins.
  ...
  if (nextAttemptNumber == null) {
    const { data: priorAttempts } = await supabase
      ...
      .eq('lifecycle_stage', stageNumber)
      .eq('decision_type', decisionType);
    const maxAttempt = Array.isArray(priorAttempts)
      ? priorAttempts.reduce((max, r) => Math.max(max, r?.attempt_number ?? 0), 0)
      : 0;
  ...
  // QF-20260913-466 (d): a decision just successfully created means this venture/stage is
  // unblocked again after (potentially) many stale gate-retry attempts accumulated while decision
  // creation was failing (e.g. a cancelled prior attempt colliding forever, fixed above). Those
  // stale attempts must not leave the venture stuck in GATE_RETRY_BACKOFF or push it toward
  // GATE_RETRY_CEILING for a condition that no longer applies. Best-effort / non-fatal — a reset
  // failure must never undo or block the decision already created above.
  try {
    await resetGateAttempts(supabase, { ventureId, stageNumber });
  } catch (e) {
    logger.warn(`[Decision] Gate-retry attempt reset failed for venture ${ventureId} stage ${stageNumber} (non-fatal): ${e.message}`);
  }
