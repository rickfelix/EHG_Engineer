 ...
 */
// eva-logger-lint-ignore: predates the createLogger/OrchestratorTracer standard (SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-145); already uses its own established injected-logger DI pattern (a `logger` parameter, default console, threaded through every exported function here and asserted by this file's own test suite) -- retrofitting the whole file to a different logging mechanism is out of scope for QF-20260913-466's narrow bugfix.
...
export async function createOrReusePendingDecision({
  ...
  // QF-20260913-466: derive attempt_number as max(existing)+1 for this (venture, stage,
  // decision_type) rather than relying on the chairman_decisions column default (1). A CANCELLED
  // prior attempt at attempt_number 1 matches neither the pending-reuse branch above nor the
  // approved/rejected re-entry branch below, so every subsequent insert kept retrying at the same
  // default 1 and colliding on uq_chairman_decision_attempt forever (DECISION_CREATE_FAILED).
  // Explicit attemptNumber still wins (e.g. chairman-product-review.js's requestProductReview()
  // computes its own multi-attempt/re-review number and passes it explicitly).
  let nextAttemptNumber = attemptNumber;
  if (nextAttemptNumber == null) {
    // Fail LOUD on a query error rather than silently falling back to maxAttempt=0 -- a swallowed
    // error here would reproduce the exact bug this fix exists to close (nextAttemptNumber=1
    // again colliding with an existing cancelled attempt 1), just non-deterministically instead
    // of always. Matches this function's own grandfather-lookup precedent a few lines above.
    // Bounded via order+limit(1) (count-truncation-diff-lint) — only the MAX attempt_number is
    // ever needed, never the full row set.
    const { data: latestAttempt, error: priorAttemptsError } = await supabase
      .from('chairman_decisions')
    ...
    if (priorAttemptsError) {
      throw new ServiceError('DECISION_CREATE_FAILED', `Failed to look up prior attempt_number: ${priorAttemptsError.message}`, 'ChairmanDecisionWatcher');
    }
    nextAttemptNumber = (latestAttempt?.attempt_number ?? 0) + 1;
  }
