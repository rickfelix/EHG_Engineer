...
const moduleLogger = createLogger('LegalDocProducer');
...
/**
 * Reduces companies.website (typically a full https:// URL) to a bare domain,
 * for the {{COMPANY_DOMAIN}}/{{CONTACT_EMAIL}} substitution markers.
 *
 * SD-LEO-INFRA-STAGE-LAUNCH-READINESS-001 / security-agent EXEC-phase review
 * (cbfb8391): the prior strip-protocol/path regex did not discard URL userinfo
 * (`user:pass@host`), so a website value like `https://user:pw@evil.com` would
 * persist `legal@user:pw@evil.com` into venture_legal_overrides.generated_content
 * -- a pre-existing defect, newly reachable now that FR-2 wires this producer
 * into automatic per-venture execution instead of manual/CLI-only invocation.
 * The WHATWG URL parser discards userinfo by construction (`.hostname` never
 * includes it), closing the whole class rather than patching the regex for one
 * more pattern. Falls back to the original regex (which still strips protocol/
 * path, just not userinfo) only when the input isn't parseable as a URL at all.
 *
 * @param {string|null|undefined} rawWebsite
 * @returns {string|null}
 */
function extractBareDomain(rawWebsite) {
  // SEC-3 (EXEC-phase SECURITY review): venture.metadata.live_url is JSONB and can
  // hold a non-string value (number, boolean, array) that is still truthy -- e.g.
  // 12345 would otherwise coerce to the hostname-shaped "12345" and land in a
  // chairman-visible legal document's CONTACT_EMAIL field as a silently-wrong domain.
  ...
  try {
    ...
    return rawWebsite.replace(/^https?:\/\//i, '').replace(/^[^/@]*@/, '').replace(/\/.*$/, '') || null;
  }
}
...
async function readVentureContext({ supabase, ventureId, logger }) {
  const { data: venture, error: ventureErr } = await supabase
    .from('ventures')
    .select('id, name, description, value_proposition, solution_approach, company_id, metadata')
    .eq('id', ventureId)
    .maybeSingle();
  ...
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-I (FR-1): companies.website is the
  // primary source, but a venture under a holding company with no marketing
  // site of its own (companies.website NULL) still has its own deployed URL
  // (ventures.metadata.live_url) -- fall back to that before declaring the
  // domain unresolvable, so a real venture in production isn't blocked on a
  // parent-company field that was never meant to describe it.
  const companyDomain = extractBareDomain(company?.website) || extractBareDomain(venture.metadata?.live_url);
 ...
 */
export async function generateLegalDocsForVenture({ supabase, ventureId, logger = moduleLogger }) {
...
export { REQUIRED_TEMPLATE_TYPES, NOT_LEGAL_ADVICE_DISCLAIMER, substituteMarkers, readVentureContext, extractBareDomain };
