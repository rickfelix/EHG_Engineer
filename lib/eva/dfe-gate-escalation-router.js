...
export async function detectPatterns(supabase, options = {}) {
  ...
  try {
    ...
    // Paginated (FR-6 batch 7): a capped read would silently drop gate failures from
    // pattern detection. Page errors throw into the outer catch (same error shape).
...
export async function getEscalationQueue(supabase, options = {}) {
  ...
  try {
    // Paginated (FR-6 batch 7): a capped read would silently hide pending escalations
    // from the chairman queue. Page errors throw into the outer catch (same shape).
    const decisions = await fetchAllPaginated(() => supabase
      .from('chairman_decisions')
