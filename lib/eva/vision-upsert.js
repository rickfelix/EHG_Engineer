...
const logger = createLogger('VisionUpsert');
...
export async function upsertVision({ supabase, visionKey, level = 'L2', content, sections, dimensions, ventureId, brainstormId, createdBy = 'eva-vision-upsert', approved = true, chairmanRatified = false }) {
  ...
  // caller actually supplied dimensions — an explicit `dimensions: null` clear, or the
  // carry-forward-on-omit path above, is left untouched. Existing dimensions (by name) are
  // reused so re-extraction preserves ids for names that did not change.
  ...
  if (Array.isArray(dimensionsWithIds)) {
    ...
    logger.debug('Assigned dimension ids', { visionKey, total: dimensionsWithIds.length, fresh: freshCount, reused: dimensionsWithIds.length - freshCount });
  }
  ...
  const record = {
    ...
    extracted_dimensions: dimensions !== undefined ? dimensionsWithIds : (existing?.extracted_dimensions ?? null),
