...
export async function getConsolidatedContext(supabase) {
  try {
    // Paginated (FR-6 batch 7): totalCount is a rows.length gauge over pending
    // recommendations. Page errors throw into the outer catch (same empty shape).
    const recs = await fetchAllPaginated(() => supabase
      .from('eva_consultant_recommendations')
      .select('id, title, application_domain, priority_score, recommendation_type, description, status, feedback_weight') // schema-lint-disable-line — pre-existing reference on main (eva_consultant_recommendations.feedback_weight was in this select before the FR-6 rewrap); moved into diff by FR-6 batch 7
      .eq('status', 'pending')
