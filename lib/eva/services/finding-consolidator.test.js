...
// getConsolidatedContext now paginates via fetchAllPaginated (chains .order().order()
// then .range()); a chainable+thenable builder satisfies that chain and resolves rows.
...
describe('getConsolidatedContext', () => {
  ...
  it('consolidates mock recommendations by domain', async () => {
    ...
    const mockSupabase = stubClient(mockRecs);
    const result = await getConsolidatedContext(mockSupabase);
