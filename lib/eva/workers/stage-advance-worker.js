  ...
  async execute() {
    ...
    for (const exec of readyExecs) {
      ...
      await advanceStage(this.supabase, {
        ventureId: exec.venture_id,
        fromStage: exec.current_lifecycle_stage,
        toStage: nextStage,
      ...
      if (exec.current_lifecycle_stage >= resolveMinExtractStage()) {
        try {
          await captureVentureStage(this.supabase, exec.venture_id, exec.current_lifecycle_stage);
        } catch (captureErr) {
          this._log('capture-forward-failed', {
            ventureId: exec.venture_id,
            stage: exec.current_lifecycle_stage,
