 ...
 *
 * SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-4/TR-8, individually investigated per this SD's
 * triage requirement — NOT a blanket derive): this worker's whole purpose is gate
 * evaluation ("don't auto-advance past gates"), so per TR-1 the raw gate_type view
 * (stage-governance.js's blockingStagesRaw) is the correct SSOT — closing the same S10/
 * S18/S25 promotion-gate bypass class this SD exists to close (the old hardcoded set
 * above omitted all three). Stages 21/22 (gate_type='none', review_mode='review') are an
 * EXPLICIT, DOCUMENTED carve-out layered on top, not derived from review_mode generically:
 * a generic review_mode derivation would also sweep in stages 7/8/9/11 (also
 * review_mode='review'), which this worker has never gated and this SD did not
 * investigate or intend to change — see docs/architecture/stage-advancement-fr4-tr8-triage.md.
 */
const CARVE_OUT_REVIEW_STAGES = Object.freeze(new Set([21, 22]));

async function getGateStages(supabase) {
  const gov = await getStageGovernance(supabase);
  return new Set([...gov.blockingStagesRaw, ...CARVE_OUT_REVIEW_STAGES]);
}
