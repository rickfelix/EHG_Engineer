  ...
  try {
    ...
    const walk = await withTimeout(
      runVentureJourneyWalk({
        ...
        // QF dcc36266 retro action item: thread the ventureId already resolved above into
        // startSession()'s metadata.venture_id, closing the producer/consumer key mismatch
        // with lib/eva/uat-robustness-gate.js. stageNumber is deliberately omitted here --
        // this producer runs at Stage 20 (quality findings), not the dedicated UAT stage the
        // gate keys runs by, so a real stageNumber value isn't available at this call site.
        ventureId,
        ventureKey,
    ...
    // 'timeout'/'error' (SD-LEO-INFRA-FIX-JOURNEY-WALK-001 FR-1: journey-walk-orchestrator.js
    // now returns status='error' with a reason on any mid-walk exception, instead of
    // throwing past this function's own now-unreachable catch below): the walk could not
    // complete/run at all — a real but non-functional gap (we don't know if the venture
    // itself is broken), so medium, not high.
