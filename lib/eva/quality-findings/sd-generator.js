...
// eva-logger-lint-ignore: this file's operational observability already runs
// through its own audit_log mechanism (writeAuditLog(), every dedup/rate-limit/
// generation decision -- see Family B docs above), a deliberate design choice
// predating SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A. That SD's own edit here
// (isWarnCappedCategoryFinding + the selectPendingFindings exclusion branch)
// already calls writeAuditLog() for its new branch; retrofitting the rest of
// this large, pre-existing file with createLogger is a separate concern, out
// of this SD's scope.
...
import { findRiskKeywordWithContext, findSchemaKeywordWithVerbContext } from '../../utils/work-item-router.js';
...
export const TIER_MAP = Object.freeze({
  ...
  // Experience Design (SD-LEO-FEAT-STAGE-EXPERIENCE-DESIGN-001) — WARN-capped pilot,
  // scope item 7 forbids new authority: capped at Tier 2 (QF/fix) even at critical
  // severity, so an experience finding never auto-escalates to a full remediation SD.
  usability:         { critical: 2, high: 1, medium: 1, low: 1 },
  accessibility:     { critical: 2, high: 1, medium: 1, low: 1 },
  journey_coherence: { critical: 2, high: 1, medium: 1, low: 1 },
  // Venture Quality Baseline (SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A) — WARN-capped,
  // informational-only one-time baseline; the baseline runner itself caps every finding
  // it writes at 'medium' severity, so critical/high rows (and thus sync SD generation)
  // never occur in practice, but every category still needs a tier entry.
  performance: { critical: 2, high: 1, medium: 1, low: 1 },
  responsive:  { critical: 2, high: 1, medium: 1, low: 1 },
});
 ...
 * @param {Object} [opts]
 * @param {string} [opts.criticality] - SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: explicit
 *   verdict from the caller. Defaults to 'later' when omitted -- an automated generator has
 *   no criticality triage upstream, and 'later' is the never-silently-critical default.
 * @param {string} [opts.criticalityReason]
 * @returns {{type: string, tier: number, args: Array<string>, metadata: Object}}
 */
export function buildCreateSdArgs(finding, opts = {}) {
  ...
  // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: default 'later' unless the caller supplies
  // an explicit verdict. Never silently defaults to 'critical'.
  const criticality = opts.criticality || 'later';
  const criticalityReason = opts.criticalityReason
    || 'auto-generated by sd-generator generateRemediationSD, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4';
  ...
  const args = [
    ...
    '--criticality-reason', criticalityReason,
  ];
 ...
 * @param {string} [params.criticality] - SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: passed
 *   through to buildCreateSdArgs; defaults to 'later' there when omitted.
 * @param {string} [params.criticalityReason]
 * @returns {Promise<{sd_key?: string, created: boolean, skipped: boolean, reason?: string}>}
 */
export async function generateRemediationSD({ supabase, finding, dryRun = false, criticality, criticalityReason }) {
  // Idempotency check
  ...
  const { type, tier, args, metadata } = buildCreateSdArgs(finding, { criticality, criticalityReason });
  ...
  return await new Promise((resolve, reject) => {
    ...
    proc.on('close', (exitCode) => {
      ...
      // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001: a 'later' verdict (this function's own default,
      // TR-4) exits 0 with NO SD key printed -- the direct lane's route_later short-circuit
      // never reaches an SD-key-printing branch. Without this guard, exit 0 + no match was
      // reported as created:true/sd_key:null (TESTING finding, EXEC-TO-PLAN): generateBatch
      // then counted a routed-later finding as remediated. created now reflects whether an SD
      // key was actually parsed, never a bare exit-code check.
      const m = stdout.match(/SD Key:\s+(SD-[A-Z0-9-]+)/);
      const sd_key = m ? m[1] : null;
      if (!sd_key) {
        resolve({ sd_key: null, created: false, skipped: true, reason: 'no_sd_key_in_output', tier, type, metadata });
        return;
      }
...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A's own baseline categories --
// deliberately NOT the full WARN_CAPPED_CATEGORIES set. TESTING (EXEC phase)
// found this SD's own findings (writer.js never sets status, so they land at
// the table default 'pending') would be picked up by the FR-C cron generator
// and auto-file a DRAFT remediation SD, contradicting this child's own
// "informational only, no gate change" scope. The fix needs to cover exactly
// this SD's 3 categories -- including 'accessibility', which THIS SD also
// writes via axe-core -- but REGRESSION (VERIFY phase) found that reusing the
// full WARN_CAPPED_CATEGORIES constant silently ALSO exempted 'usability' and
// 'journey_coherence', two categories this SD never touches and does not own
// (SD-LEO-FEAT-STAGE-EXPERIENCE-DESIGN-001's ongoing UX review, whose
// findings exist in order to be fixed -- unlike this SD's one-time baseline,
// they may legitimately need cron-driven remediation). Narrowed to exactly
// this SD's own category set so no other SD's remediation eligibility is
// silently changed. NOTE: 'accessibility' is a category name both SDs write
// findings under (axe-core here, design-agent there) with no producer field
// to disambiguate by -- excluding it here also exempts that SD's
// accessibility-category findings from cron remediation. This overlap is a
// real, deliberately-narrowed-but-unresolved cross-SD scope question, flagged
// for PLAN/chairman-level ratification rather than decided unilaterally here.
const CAPA_001_A_BASELINE_CATEGORIES = Object.freeze(['accessibility', 'performance', 'responsive']);
export function isWarnCappedCategoryFinding(row) {
  return !!row && CAPA_001_A_BASELINE_CATEGORIES.includes(row.finding_category);
}
...
export async function selectPendingFindings(supabase, ventureId = null) {
  // Paginated (FR-6 batch 7): this SELECTS WORK — a capped read would silently leave
  // pending findings unremediated. Page errors rethrow with the prior message shape.
  let data;
  try {
    ...
    throw new Error('selectPendingFindings failed: ' + e.message);
  }
  const rows = data || [];
  ...
  for (const r of rows) {
    ...
    } else if (isWarnCappedCategoryFinding(r)) {
      // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A: WARN-capped categories are
      // informational-only by design (see isWarnCappedCategoryFinding above).
      await writeAuditLog(supabase, 'warn_capped_category_skipped', {
        finding_id: r.id, venture_id: r.venture_id,
        finding_category: r.finding_category, severity: r.severity,
      }, { entityId: r.id, severity: 'info' });
    } else out.push(r);
...
// QF-20260902-265: WITNESSED 2026-09-02 -- Family B minted SD-LEO-FIX-REMEDIATION-* SDs for
// venture findings with target_application resolved from a venture_default heuristic (usually
// literal 'EHG'), which claim-eligibility.cjs:335 correctly refuses (unactionable_venture_remediation
// -- sd-start resolves no applications.local_path for it). Solomon CAPA 9d8d34b3 CA-8: route venture
// findings to the QF lane instead (never teach the SD lane local_path -- broad blast radius).
// Resolves the REAL registered application name for a venture, or null when none is registered
// (falls through to unchanged SD behavior -- this generator never invents a target).
export async function resolveVentureApplication(supabase, ventureId) {
  if (!supabase || !ventureId) return null;
  const { data } = await supabase.from('applications')
    .select('name').eq('venture_id', ventureId).eq('status', 'active').is('deleted_at', null).maybeSingle();
  return data?.name || null;
}

function generateVentureQuickFixId() {
  const now = new Date();
  const y = now.getFullYear(), m = String(now.getMonth() + 1).padStart(2, '0'), d = String(now.getDate()).padStart(2, '0');
  return `QF-${y}${m}${d}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`;
}

/**
 * Mint a venture-targeted quick fix for a single finding, in the fitness-checked QF lane
 * (lib/fleet/qf-repo-fitness.js already refuses an unfit target at claim time). A Tier-3 risk/
 * schema keyword hit GATES the row (owner + release_condition naming the venture owner lane) --
 * never status='escalated', and never an SD (Solomon's ruling: the SD lane cannot check out a
 * venture repo).
 * @returns {Promise<{qfId: string, gated: boolean}>}
 */
export async function mintVentureQuickFix(supabase, { ventureId, targetApp, finding }) {
  const scanText = JSON.stringify(finding.evidence_pointer || {});
  const riskKeyword = findRiskKeywordWithContext(scanText) || findSchemaKeywordWithVerbContext(scanText);
  const qfId = generateVentureQuickFixId();
  const title = `Remediation: ${finding.finding_category} (${finding.severity}) — ${targetApp}`.slice(0, 200);
  const { error } = await supabase.from('quick_fixes').insert({
    id: qfId,
    title,
    type: 'bug',
    severity: finding.severity,
    description: `Auto-filed from Stage 20 quality loop finding ${finding.id} (venture ${ventureId}).`,
    target_application: targetApp,
    status: 'open',
    owner: riskKeyword ? 'venture-owner-lane' : null,
    release_condition: riskKeyword
      ? `Tier-3 keyword "${riskKeyword}" detected on a venture-targeted finding — needs venture-owner review before claim (never escalated, never an SD).`
      : null,
    created_at: new Date().toISOString(),
    // SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4: direct-insert bypass path, unconditional --
    // never 'critical', never absent.
    metadata: {
      criticality: 'later',
      criticality_reason: 'auto-generated by sd-generator mintVentureQuickFix, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4',
    },
  });
  if (error) throw new Error(`mintVentureQuickFix insert failed: ${error.message}`);
  return { qfId, gated: !!riskKeyword };
}
...
export async function insertDraftRemediationSd(supabase, { ventureId, findingCategory, severity, findingIds, sampleFinding }) {
  ...
  const metadata = {
    ...
    // never 'critical', never absent.
    criticality: 'later',
    criticality_reason: 'auto-generated by sd-generator insertDraftRemediationSd, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4',
  };
...
export async function generateRemediationSdsForVenture(ventureId, options = {}) {
  ...
  // QF-20260902-265: resolved ONCE per venture (not per finding) -- every finding in this sweep
  // shares the same venture_id, so the registered application never changes mid-loop.
  const ventureApp = await resolveVentureApplication(supabase, ventureId);

  for (const f of findings) {
    try {
      // A real, registered, non-EHG_Engineer venture app routes to the QF lane instead of the SD
      // lane. No registered app (ventureApp === null) falls through UNCHANGED to the existing SD
      // path -- this generator never invents a target where none is registered.
      if (ventureApp && ventureApp !== 'EHG_Engineer') {
        const { qfId, gated } = await mintVentureQuickFix(supabase, { ventureId, targetApp: ventureApp, finding: f });
        await transitionFindingToSdFiled(supabase, f.id, qfId);
        await writeAuditLog(supabase, gated ? 'venture_qf_filed_gated' : 'venture_qf_filed', {
          venture_id: ventureId, target_application: ventureApp,
          finding_category: f.finding_category, severity: f.severity, finding_id: f.id, qf_id: qfId,
        }, { entityId: f.id });
        mintedQuickFixes.push({ qf_id: qfId, finding_id: f.id, gated });
        continue;
      }
  ...
  return { created, appended, skippedRateLimited, errors, mintedQuickFixes };
}
