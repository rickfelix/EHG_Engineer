...
import { validateFindingShape, WARN_CAPPED_CATEGORIES } from './finding-shape.js';
...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A (TESTING sub-agent finding, EXEC phase):
// WARN_CAPPED_CATEGORIES findings are excluded from the Stage-20 FAIL/WARN verdict
// scan (stage-20-code-quality.js) specifically so they can never drive blocking
// behavior, but selectPendingFindings() had no matching exemption -- a 'medium'
// severity WARN-capped finding (writer.js never sets status, so it lands at the
// table default 'pending') would still be picked up by the FR-C cron generator
// and auto-file a DRAFT remediation SD, contradicting the same "informational
// only" contract WARN_CAPPED_CATEGORIES already encodes for the verdict formula.
export function isWarnCappedCategoryFinding(row) {
  return !!row && WARN_CAPPED_CATEGORIES.includes(row.finding_category);
}
