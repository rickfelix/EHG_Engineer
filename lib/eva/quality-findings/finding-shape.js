...
/**
 * The unified Stage 20 finding categories (count grows additively; see below).
 *
 ...
 *
 * Vision Compliance (chairman mandate — absence is the failure):
 *   - feedback_widget_present: feedback widget (Sentry feedback / LogRocket /
 *                              FullStory / Hotjar / equiv) present on the venture
 *   - error_capture_wired:     error-capture SDK init wired (Sentry / Bugsnag /
 *                              Rollbar / Datadog RUM / equiv) on the venture
 *
 * Experience Design (SD-LEO-FEAT-STAGE-EXPERIENCE-DESIGN-001 — chairman-authorized,
 * WARN-capped pilot: these three categories can never drive a Stage-20 FAIL, see the
 * verdict formula in stage-20-code-quality.js):
 *   - usability:         design-agent findings about interaction/usability defects
 *   - accessibility:     design-agent findings about accessibility defects
 *   - journey_coherence: design-agent findings comparing the live implementation
 *                        against the Stage-15 user-journey artifact (dead UI,
 *                        unreachable screens, broken flows)
 *
 * Venture Quality Baseline (SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-A — WARN-capped,
 * informational-only one-time baseline; this child's own scope explicitly excludes
 * any new gate or blocking behavior):
 *   - performance: Lighthouse baseline findings (performance/best-practice scores)
 *   - responsive:  layout-break findings at the 3 named viewport breakpoints
 */
 ...
 * both the verdict formula and any test asserting the cap.
 */
export const WARN_CAPPED_CATEGORIES = Object.freeze([
  'usability',
  'accessibility',
  'journey_coherence',
  'performance',
