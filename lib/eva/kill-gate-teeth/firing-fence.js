...
export async function evaluateFiringFence(supabase, { ventureId }) {
  ...
  let attestationRows = [];
  try {
    // Bounded: at most a handful of attestation rows could ever exist per venture (one per
    // sealer, in practice ever at most 1-2); 100 is a generous ceiling, not an expectation.
    const { data, error } = await supabase
      .from('system_events')
      .select('id, payload')
