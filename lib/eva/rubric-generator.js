...
// eva-logger-lint-ignore: SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 changes only the
// Map key format used by an existing dimId construction loop; no new behavior worth logging,
...
export async function generateVentureRubrics({ vision, arch, targetPath, llmClient, retries = 1 }) {
  ...
  // SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 (FR-5): key by the dimension's stable id
  // when available (matches vision-evidence-scorer.js's dbDim.rubricKey lookup), falling back
  // to the positional code for a dimension not yet backfilled with one. Namespaced by side
  // (V:/A:) -- a SECURITY review (EXEC-TO-PLAN) found vision and arch dimension NAMES collide
  // in practice (22% of live pairs), so a bare slug would let one side's rubric silently
  // overwrite the other's in this Map.
  for (let i = 0; i < visionDims.length; i++) {
    const { positionalId, stableId } = resolveDimensionIdentity(visionDims[i], i, 'V');
    const dimId = stableId ? `V:${stableId}` : positionalId;
  ...
  for (let i = 0; i < archDims.length; i++) {
    const { positionalId, stableId } = resolveDimensionIdentity(archDims[i], i, 'A');
    const dimId = stableId ? `A:${stableId}` : positionalId;
    const rubric = await generateOneRubric({
