...
export function assertLiveDeploymentBinding({ nonceWriteResult, expectedNonce, deploymentSha }) {
  ...
  if (!nonceWriteResult.echoedNonce || nonceWriteResult.echoedNonce !== expectedNonce) {
    return { passed: false, reason: `write reported success but the echoed nonce ("${nonceWriteResult.echoedNonce ?? 'none'}") does not match the expected nonce ("${expectedNonce}") — refusing a stubbed/mocked transport as evidence` };
  }
  ...
  return { passed: true, reason: `nonce round-trip confirmed against live origin (sha=${deploymentSha})` };
}
