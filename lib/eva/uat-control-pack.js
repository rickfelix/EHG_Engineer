...
/**
 * QF-20260830-666 (Solomon ruling 711cce6d): the four control-pack controls are non-overlapping
 * blind spots, not redundant layers -- a PASS requires ALL FOUR evaluated with real evidence.
 * result-recorder.js's prior `control_pack_evaluated: Boolean(controlPackEvidence)` went true on
 * ANY evidence object regardless of how many controls it actually covered, so an honest 1-of-4
 * partial (e.g. PR #7812's assertion-manifest-only evidence) silently passed the mechanical gate.
 *
 * PURE: derive a per-control status map from the SAME evidence-presence predicates
 * result-recorder.js already uses to decide whether to RUN each check (evaluated = evidence was
 * supplied for that control, independent of whether the check then passed or failed -- pass/fail
 * is tracked separately in control_pack_failures). `waivedControls` lets the chairman explicitly
 * waive named controls (each entry `{control, reason}`); a waived control is REQUIRED-exempt but
 * its reason is always carried in the returned status so the run evidence/receipt cites it.
 *
 * @param {object|null|undefined} evidence - the same controlPackEvidence completeSession() receives
 * @param {Array<{control:string, reason:string}>} [waivedControls]
 * @returns {{status: Record<string,string>, missing: string[], allRequiredEvaluated: boolean}}
 */
export const CONTROL_PACK_CONTROLS = ['minimum_assertion_manifest', 'live_deployment_binding', 'canary_mutation_control', 'fence_two_sidedness'];

export function buildControlPackStatus(evidence, waivedControls = []) {
  const attempted = {
    minimum_assertion_manifest: !!(evidence?.manifest && evidence?.executedJourneys),
    live_deployment_binding: evidence?.nonceWriteResult !== undefined,
    canary_mutation_control: !!(evidence?.canaryJourneyId && evidence?.journeyResults),
    fence_two_sidedness: !!evidence?.fenceEvidence,
  };
  const waivers = new Map(
    (Array.isArray(waivedControls) ? waivedControls : [])
      .filter((w) => w?.control && w?.reason)
      .map((w) => [w.control, w.reason])
  );
  const status = {};
  for (const control of CONTROL_PACK_CONTROLS) {
    if (attempted[control]) status[control] = 'evaluated';
    else if (waivers.has(control)) status[control] = `waived: ${waivers.get(control)}`;
    else status[control] = 'not_attempted';
  }
  const missing = CONTROL_PACK_CONTROLS.filter((c) => status[c] === 'not_attempted');
  return { status, missing, allRequiredEvaluated: missing.length === 0 };
}
