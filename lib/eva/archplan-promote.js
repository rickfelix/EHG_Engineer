...
export async function promoteArchPlan({ supabase, planKey, promotedBy }) {
  ...
  let { data, error } = await supabase
    ...
    // approved_by/approved_by_at ship in this SD's own TIER-1 migration (database/migrations/
    // 20260915_governance_artifact_approver_fields.sql), not yet in the committed
    // schema-reference-snapshot.json -- the 42703 fail-soft retry above is exactly this
    // function's own handling for a DB that hasn't applied it yet.
    .select('id, plan_key, status, chairman_approved, chairman_approved_at, approved_by, approved_by_at') // schema-lint-disable-line
    .single();
