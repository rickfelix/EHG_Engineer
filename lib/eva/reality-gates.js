...
const BOUNDARY_CONFIG = Object.freeze({
  ...
  '17->18': {
    description: 'BLUEPRINT REVIEW → BUILD LOOP',
    required_artifacts: [
      { artifact_type: ARTIFACT_TYPES.BLUEPRINT_REVIEW_SUMMARY, min_quality_score: 0.6, url_verification_required: false },
    ],
  },
  '23->24': {
...
async function evaluateRealityGate(params, deps = {}) {
  ...
  //   2. DB gate_boundary_config (canonical post-FR-1)
  //   3. Hardcoded BOUNDARY_CONFIG (DEPRECATED fallback for DB-unavailable case)
  // Missing-row case at any layer returns NOT_APPLICABLE (advisory pass with warning).
  ...
  if (requiredArtifacts && requiredArtifacts.length > 0) {
    ...
    // DB-driven canonical path (FR-2)
    let dbConfig = null;
    if (supabase) {
      const map = await _loadBoundaryFromDB(supabase, logger);
      dbConfig = map?.get(transitionKey) || null;
    }
    ...
    if (dbConfig) {
      ...
      // Fallback to deprecated hardcoded BOUNDARY_CONFIG (only when DB is unreachable
      // or the boundary has no DB row). Logged as warning so missing-config drift surfaces.
      const config = BOUNDARY_CONFIG[transitionKey];
      if (!config) {
        result.status = 'NOT_APPLICABLE';
        result.passed = true; // NOT_APPLICABLE means gate doesn't block
        (logger ?? console).warn?.(`[reality-gates] No boundary config found for ${transitionKey} in DB or BOUNDARY_CONFIG — advisory pass`);
        return result;
      }
      (logger ?? console).warn?.(`[reality-gates] gate_boundary_config[${transitionKey}] not found in DB — using deprecated BOUNDARY_CONFIG fallback`);
      artifactRequirements = config.required_artifacts;
      result.config_source = 'BOUNDARY_CONFIG_fallback';
    }
  }
