...
export async function getPortfolioRiskPanel(supabase, options = {}) {
  ...
  try {
    // Get all active SDs grouped by venture. Paginated (FR-6 batch 7): draft+planning
    // SDs routinely exceed the PostgREST 1000-row cap, which silently understated
    // portfolio risk. Page errors are converted below, preserving the emptyPanel policy.
    let sds;
    try {
      ...
      logger.warn(`[GovernancePanels] SD query failed: ${e.message}`);
      return emptyPanel('portfolio_risk', e.message);
    }

    // Get latest vision scores for risk assessment
    const { data: scores, error: scoreError } = await supabase
      .from('eva_vision_scores')
      .select('sd_id, total_score, threshold_action, created_at') // schema-lint-disable-line — pre-existing reference on main (eva_vision_scores.created_at, untouched line; file entered the diff via FR-6 batch 7)
...
export async function getDecisionPipelinePanel(supabase, options = {}) {
  ...
  try {
    const { data: decisions, error: decError } = await supabase
      ...
      .select('id, decision_type, status, created_at, resolved_at') // schema-lint-disable-line — pre-existing reference on main (chairman_decisions.resolved_at, untouched line; file entered the diff via FR-6 batch 7)
      .order('created_at', { ascending: false })
      .limit(500);
...
export async function getDecisionEffectivenessPanel(supabase, options = {}) {
  ...
  try {
    const { data: decisions, error: decError } = await supabase
      .from('chairman_decisions')
      .select('id, decision_type, status, created_at, resolved_at, metadata, summary') // schema-lint-disable-line — pre-existing reference on main (chairman_decisions.resolved_at/metadata, untouched line; file entered the diff via FR-6 batch 7)
      .order('created_at', { ascending: false })
      .limit(500);
...
export async function getAuditCompletenessPanel(supabase, options = {}) {
  ...
  try {
    ...
    const { data: events, error: eventError } = await supabase
      .from('eva_event_log')
      .select('event_type, event_data, created_at') // schema-lint-disable-line — pre-existing reference on main (eva_event_log.event_data, untouched line; file entered the diff via FR-6 batch 7)
    ...
    const { data: decisions } = await supabase
      .from('chairman_decisions')
      .select('id, summary, metadata') // schema-lint-disable-line — pre-existing reference on main (chairman_decisions.metadata, untouched line; file entered the diff via FR-6 batch 7)
      .order('created_at', { ascending: false })
      .limit(50);
