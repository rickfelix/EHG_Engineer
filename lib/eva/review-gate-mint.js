...
export async function maybeMintReviewGate(
  ...
  try {
    ...
    // id:null) — reporting minted:true with a null decisionId would be a false success.
    // Unlike the stage-execution-worker.js call sites, no auto-approve/auto-select decision
    // is made here for the non-fixture skip case (isDecisionCreatingStage disagreeing with
    // this caller's own review-stage check) — minted:false is accurate and safe either way,
    // so `res?.skipped` is intentionally checked broadly (not narrowed to reason==='fixture_venture').
    if (res?.skipped) {
      logger.log?.(`[StageEngine] review-gate skipped for fixture venture ${ventureId} at stage ${stageNumber}`);
      return { minted: false, reason: res.reason || 'decision_not_created' };
    }
    logger.log?.(
