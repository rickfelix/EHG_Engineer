...
export async function trackAuthorityCoverage(supabase, options = {}) {
  ...
  try {
    ...
    // Query enforcement events from eva_event_log. Paginated (FR-6 batch 7): a 30-day
    // window of a high-volume event log routinely exceeds the 1000-row cap.
    let events;
    try {
      ...
      logger.warn(`[CLIAuthorityTracker] Event query failed: ${e.message}`);
      return { metrics: emptyMetrics(), coverage: emptyCoverage(), gaps: [], error: e.message };
    }
