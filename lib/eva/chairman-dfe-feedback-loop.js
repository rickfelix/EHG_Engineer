...
export async function analyzeOverridePatterns(supabase, options = {}) {
  ...
  try {
    ...
    // Paginated (FR-6 batch 7): a capped read would silently drop overrides from the
    // pattern-learning window. Page errors are converted below, preserving policy.
    let overrides;
    try {
      overrides = await fetchAllPaginated(() => supabase
      ...
      logger.warn(`[DFEFeedback] Override query failed: ${e.message}`);
      return { patterns: [], totalOverrides: 0, error: e.message };
    }
