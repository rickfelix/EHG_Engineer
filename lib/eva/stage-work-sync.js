...
/**
 * Whether a stage is in the hard-gate set.
 *
 * SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-2: previously ran its OWN separate read of
 * chairman_dashboard_config.hard_gate_stages — a second consumer of that (possibly stale)
 * config row, independent of stage-execution-worker.js's own hard-gate check. Repointed to
 * lib/eva/stage-governance.js's hardGateStages (derived live from venture_stages.gate_type),
 * so this and every other hard-gate consumer see the SAME set from ONE source, instead of two
 * readers that could silently diverge (TESTING sub-agent evidence b57c4b17).
...
export async function isInHardGateStages(supabase, stageNumber) {
  try {
    ...
    return gov.hardGateStages.has(stageNumber);
  } catch {
    return false;
  }
}
