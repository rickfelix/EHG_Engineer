...
export class EvaMasterScheduler {
  ...
  _registerDefaultRounds() {
    ...
    this.registerRound('stage_health', {
      description: 'Check stage template completeness across all 26 lifecycle stages',
      cadence: 'monthly',
      handler: async () => {
  ...
  async _dispatchVenture(venture) {
    ...
    for (let i = 0; i < maxStages; i++) {
      ...
      try {
        ...
        // FR-1 (SD-REFILL-00A88NSU): a CONTEXT_LOAD_FAILED for a venture that no longer exists is a
        // PERMANENT failure — evict the orphaned queue entry so the scheduler stops re-dispatching a
        // deleted venture every poll. That fail-loop (a fixture purge left orphaned pending rows) grew
        // eva_scheduler_metrics unbounded (~666 failure rows/hr). Transient CONTEXT_LOAD_FAILEDs (the
        // venture still exists) are left retryable by the existence check inside the helper.
        if (result.status === 'FAILED' && Array.isArray(result.errors)
            && result.errors.some((e) => e?.code === 'CONTEXT_LOAD_FAILED')) {
          const evicted = await this._evictQueueEntryIfVentureMissing(
            venture.venture_id,
            result.errors.find((e) => e?.code === 'CONTEXT_LOAD_FAILED')?.message
          );
          if (evicted) break; // entry removed; nothing more to dispatch for this absent venture
        }
  ...
  /**
   * FR-1 (SD-REFILL-00A88NSU): evict an eva_scheduler_queue entry IFF its venture no longer exists.
   * processStage() returns CONTEXT_LOAD_FAILED both for a transient DB blip and for a permanently
   * deleted venture (e.g. a fixture purge that left orphaned queue rows — the root cause of the
   * eva_scheduler_metrics unbounded-growth fail-loop). Only the permanent case (venture genuinely
   * absent from `ventures`) is evicted; transient failures stay retryable. The orphaned row is
   * DELETEd — its venture is gone, so the entry is meaningless and `status` has no 'cancelled' value.
   * @param {string} ventureId
   * @param {string} [reason]
   * @returns {Promise<boolean>} true when an orphaned entry was removed
   */
  async _evictQueueEntryIfVentureMissing(ventureId, reason) {
    try {
      const { data: stillExists } = await this.supabase
        ...
        .eq('id', ventureId)
        .maybeSingle();
      if (stillExists) return false; // venture exists -> transient failure, keep retryable
      ...
      if (error) {
        this.logger.error(`[Scheduler] Failed to evict orphaned queue entry for ${ventureId}: ${error.message}`);
        return false;
      }
      this.logger.warn(`[Scheduler] Evicted orphaned queue entry: venture ${ventureId} no longer exists (reason: ${reason || 'venture not found'})`);
      return true;
    } catch (err) {
      this.logger.error(`[Scheduler] Orphan-eviction check failed for ${ventureId}: ${err.message}`);
      return false;
    }
  }
