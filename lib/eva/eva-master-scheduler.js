 ...
 */
// eva-logger-lint-ignore: pre-existing constructor-injected this.logger DI pattern (defaults
// to console) used pervasively across this 900+ line file since SD-EVA-FEAT-SCHEDULER-001,
// not lib/logger.js's createLogger. QF-20260913-788 touched 2 unrelated lines (job cadence);
// a full logging retrofit is out of scope for that fix. Tracked as pre-existing backlog, not
...
const domainRegistry = new ServiceRegistry();

domainRegistry.register('process_stage', async (params) => {
  const { processStage } = await import('./eva-orchestrator.js');
  return processStage(params.stageArgs, params.deps, params.traceCtx);
});

domainRegistry.register('score_sd', async (params) => {
  const { scoreSD } = await import('../../scripts/eva/vision-scorer.js');
  return scoreSD(params);
});

domainRegistry.register('sync_vision_to_patterns', async (params) => {
  const { syncVisionScoresToPatterns } = await import('../../scripts/eva/vision-to-patterns.js');
  return syncVisionScoresToPatterns(params.supabase);
});

domainRegistry.register('okr_monthly_snapshot', async (params) => {
  const { runOkrMonthlySnapshot } = await import('./jobs/okr-monthly-handler.js');
  return runOkrMonthlySnapshot(params);
});

domainRegistry.register('okr_monthly_generation', async (params) => {
  const { runOkrMonthlyGeneration } = await import('./jobs/okr-monthly-generator.js');
  return runOkrMonthlyGeneration(params);
});

domainRegistry.register('okr_mid_month_review', async (params) => {
  const { runOkrMidMonthReview } = await import('./jobs/okr-mid-month-review.js');
  return runOkrMidMonthReview(params);
});

domainRegistry.register('archive_stale_okrs', async (params) => {
  const { archiveStaleOkrs } = await import('./jobs/okr-archive-stale.js');
  return archiveStaleOkrs(params);
});

// SD-LEO-INFRA-OKR-DRIVEN-PRIORITIZATION-001: OKR Day-28 Hard-Stop — Day 28
domainRegistry.register('okr_day28_hardstop', async (params) => {
  const { runOkrDay28HardStop } = await import('./jobs/okr-day28-hardstop.js');
  return runOkrDay28HardStop(params);
});
...
const DEFAULT_MAX_STAGES_PER_CYCLE = 50;
...
// ── Cadence-state rehydration (SD-LEO-INFRA-REVIVE-EVA-HOST-AND-ARM-001 FR-4) ──

/**
 * Parse persisted cadence state from a heartbeat `metadata` blob back into the in-memory
 * Map shape the scheduler tracks ({ roundType|jobName -> lastRunEpochMs }). FAIL-OPEN: any
 * missing/garbled input (null, non-object, array, non-numeric/negative values) yields empty
 * Maps — exactly the cold-start behavior that existed before rehydration — so a corrupt
 * heartbeat row can never crash start(); at worst it loses cadence memory and the catch-up
 * path re-fires, as it always did. Pure (no I/O) so it is unit-testable in isolation.
 */
export function parseHeartbeatCadenceState(metadata) {
  const toEpochMap = (obj) => {
    const out = new Map();
  ...
  return {
    lastRoundRun: toEpochMap(md.last_round_runs),
    jobLastRunAt: toEpochMap(md.job_last_runs),
  };
}
...
export class EvaMasterScheduler {
  ...
  async start() {
    ...
    // SD-LEO-INFRA-REVIVE-EVA-HOST-AND-ARM-001 FR-4: rehydrate cadence memory from the
    // persisted heartbeat BEFORE the first heartbeat write (which would otherwise overwrite
    // last_round_runs/job_last_runs with empty maps) AND before the _runDueJobs catch-up — so
    // a revive does not re-fire every round/job that already ran within its cadence window.
    await this._rehydrateCadenceState();
  ...
  _registerDefaultRounds() {
    ...
    this.registerRound('stage_health', {
      // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-9: this was already stale pre-renumbering (the
      // description said 26 while the loop bound below was 25); now updated to the live 27-stage
      // count. This is a read-only reporting round, not the stage-template dispatch chain itself
      // (which stays excluded per Solomon adjudication) -- reporting stage-27.js as missing here
      // is accurate and does not alter any dispatch behavior.
      description: 'Check stage template completeness across all 27 lifecycle stages',
      cadence: 'monthly',
      handler: async () => {
        ...
        return { totalStages: 27, templatesFound: found.length, templatesMissing: missing.length, found: found.sort((a, b) => a - b), missing };
      },
    });
    ...
    this.registerRound('weekly_consultant_analysis', {
      ...
      cadence: 'weekly',
      handler: async () => {
        const { consultantAnalysisHandler } = await import('../../scripts/eva/consultant-analysis-round.mjs');
        return consultantAnalysisHandler();
      },
    });

    // Weekly management review — generates structured review artifact
    this.registerRound('weekly_management_review', {
      description: 'Generate weekly management review artifact with baseline comparison, OKR snapshots, and pipeline health',
      cadence: 'weekly',
      handler: async () => {
        const { managementReviewHandler } = await import('../../scripts/eva/management-review-round.mjs');
        return managementReviewHandler();
      },
    });
    ...
    this.registerRound('friday_meeting', {
      ...
      cadence: 'weekly',
      handler: async () => {
        const { fridayMeetingHandler } = await import('../../scripts/eva/friday-meeting.mjs');
        return fridayMeetingHandler();
      },
    });

    // Daily burn rate snapshot — captures velocity metrics for trending
    this.registerRound('daily_burn_rate', {
      description: 'Take daily burn rate snapshot for velocity trending',
      cadence: 'daily',
      handler: async () => {
        const { takeSnapshot } = await import('../../scripts/pipeline/burn-rate-worker.js');
        return takeSnapshot();
      },
    });
  }
   ...
   * Auto-triggers assist processing when new items found.
   */
    ...
    this.registerRound('sensemaking_disposition_monitor', {
      ...
      handler: async () => {
        ...
        if (items.length > 0) {
          ...
          // Auto-trigger: flag for assist processing
  ...
  async _createSDFromFeedback(supabase, feedback) {
    ...
    const sd = await createSD({
      ...
      criticality: 'later',
      criticalityReason: 'auto-created from sensemaking-enriched feedback, unsupervised -- routed later by default per SD-LEO-INFRA-FILING-TOOLS-ENFORCE-001 FR-4',
      metadata: {
        source: 'sensemaking-monitor',
    ...
    // with no sd.id. Without this guard the feedback row below would be stamped in_progress with
    // strategic_directive_id=undefined, a dangling link (TESTING finding, EXEC-TO-PLAN). Throw
    // instead -- the caller (processAutoTrigger) already catches per-item errors and logs them,
    // leaving the feedback row untouched (still eligible for the next pass).
    if (sd?.routedLater) {
      throw new Error(`feedback ${feedback.id} routed to harness_backlog (criticality=later), not created as an SD`);
    }
  ...
  async runPeriodicVisionScoring() {
    ...
    for (const sd of sds) {
      try {
        await domainRegistry.dispatch('score_sd', { sdKey: sd.sd_key, supabase: this.supabase });
        scoredCount++;
    ...
    try {
      await domainRegistry.dispatch('sync_vision_to_patterns', { supabase: this.supabase });
  ...
  _registerBuiltinJobs() {
    ...
    this.registerJob({
      ...
      handler: async () => {
        return domainRegistry.dispatch('okr_monthly_snapshot', { supabase: this.supabase, logger: this.logger });
      },
      cadenceDays: 30,
    ...
    this.registerJob({
      ...
      handler: async () => {
        ...
        await domainRegistry.dispatch('archive_stale_okrs', { supabase: this.supabase, logger: this.logger });
        return domainRegistry.dispatch('okr_monthly_generation', { supabase: this.supabase, logger: this.logger });
      },
      cadenceDays: 30,
    ...
    this.registerJob({
      ...
      handler: async () => {
        return domainRegistry.dispatch('okr_mid_month_review', { supabase: this.supabase, logger: this.logger });
      },
      cadenceDays: 15,
    ...
    // SD-LEO-INFRA-OKR-DRIVEN-PRIORITIZATION-001: OKR Day-28 Hard-Stop — Day 28
    // QF-20260913-788: cadenceDays MUST stay 1, not 30. The handler is idempotent per
    // YYYY-MM period (its own chairman_decisions row) and self-guards on day 28 via
    // isDay28OrLater(); a 30-day cadence anchored on an early-month no-op (which still
    // stamps lastRun) drifts the next eligible run past day 28 and the hard-stop window
    // becomes permanently unreachable. Daily cost of a no-op day is one indexed select.
    this.registerJob({
  ...
  /**
   * Rehydrate the in-memory cadence maps (_lastRoundRun / _jobLastRunAt) from the persisted
   * heartbeat metadata. The daemon PERSISTS these maps on every heartbeat (last_round_runs /
   * job_last_runs at _updateHeartbeat) but never READ them back — so every restart began with
   * empty maps and _runDueJobs / _executeScheduledRounds treated every job/round as due,
   * firing them all immediately (a catch-up STORM on each revive). MUST run BEFORE the first
   * _updateHeartbeat('running') write, or the empty in-memory maps clobber the persisted
   * metadata before we can read it. Fail-open: any read/parse error leaves the maps empty
   * (the historical cold-start behavior).
   *
   * SD-LEO-INFRA-REVIVE-EVA-HOST-AND-ARM-001 FR-4.
   */
  async _rehydrateCadenceState() {
    try {
      ...
      const { lastRoundRun, jobLastRunAt } = parseHeartbeatCadenceState(data.metadata);
      if (lastRoundRun.size) this._lastRoundRun = lastRoundRun;
      if (jobLastRunAt.size) this._jobLastRunAt = jobLastRunAt;
      this.logger.log(`[Scheduler] Rehydrated cadence state: ${lastRoundRun.size} round(s), ${jobLastRunAt.size} job(s)`);
    } catch (err) {
      this.logger.warn(`[Scheduler] Cadence rehydration skipped: ${err.message}`);
    }
  }
  ...
  async _runDueJobs() {
    ...
    for (const [name, job] of this._jobRegistry) {
      ...
      // SD-LEO-INFRA-REVIVE-EVA-HOST-AND-ARM-001 FR-3: observe-only must gate JOBS too, not
      // just rounds (line ~311) and dispatch (line ~1084). Builtin jobs run real side effects
      // — incl. okr-monthly-generate and the DESTRUCTIVE archive_stale_okrs — so an unguarded
      // _runDueJobs makes "observe-only" a false safety on the very catch-up path a revive
      // hits first. Mirror the rounds gate: log, stamp lastRun so cadence still advances, skip.
      if (this.observeOnly) {
        this.logger.log(`[Scheduler] OBSERVE: Would run job ${name} (cadence: ${job.cadenceDays}d)`);
  ...
  async _dispatchVenture(venture) {
    ...
    for (let i = 0; i < maxStages; i++) {
      ...
      try {
        const result = await domainRegistry.dispatch('process_stage', {
          stageArgs: { ventureId: venture.venture_id, options: { autoProceed: true } },
          deps: { supabase: this.supabase, logger: this.logger },
        });
