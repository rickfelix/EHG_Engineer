...
// eva-logger-lint-ignore: DI'd `logger` param (console-shaped) is this module's existing
// convention; retrofitting createLogger/OrchestratorTracer is out of scope for this QF.
 ...
 * attempts.
 *
 * QF-20260913-712: a venture that rolls back OUT of a stage and later re-enters it (chairman
 * rollback, e.g. 24 -> 23 -> 24) keeps its OLD attempt rows in eva_stage_gate_attempts -- that
 * table is the append-only attempt ledger and is never edited. Counting every row regardless of
 * residency let a stale prior residency's attempts count toward the ceiling/backoff on the CURRENT
 * residency (measured: 16 of 18 counted rows predated the chairman-sanctioned 08-30 rollback,
 * putting a freshly re-entered venture within 2 evaluations of an automatic MANUAL_REQUIRED park).
 * Both queries are now scoped to attempts at/after the latest venture_stage_transitions row whose
 * to_stage is this stage -- i.e. the latest transition INTO this stage -- so only this residency's
 * attempts count. A venture with no such transition row (legacy data, or a stage the venture has
 * never formally transitioned into) falls back to the epoch as the floor, which is equivalent to
 * counting every attempt ever recorded -- today's behavior, unchanged.
 */
export async function getGateAttemptState(supabase, { ventureId, stageNumber }) {
  const { data: transition, error: transitionErr } = await supabase
    ...
    .eq('to_stage', stageNumber)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (transitionErr) {
    throw new Error(`[gate-retry-guard] residency transition query failed: ${transitionErr.message}`);
  }
  const residencyStart = transition?.created_at ?? null;
  const sinceFloor = residencyStart ?? '1970-01-01T00:00:00.000Z';
  ...
  return { attemptCount: count ?? 0, lastAttemptAt: latest?.created_at ?? null, residencyStart };
}
 ...
 *
 * @returns {Promise<{ action: 'proceed'|'skip'|'terminalize', attemptCount: number, residencyStart: string|null }>}
 */
export async function checkGateRetryCeiling(supabase, { ventureId, stageNumber, logger }) {
  const { attemptCount, lastAttemptAt, residencyStart } = await getGateAttemptState(supabase, { ventureId, stageNumber });
  ...
  if (attemptCount >= GATE_RETRY_CEILING) {
    ...
    return { action: 'terminalize', attemptCount, residencyStart };
  }

  if (shouldSkipForBackoff(attemptCount, lastAttemptAt)) {
    return { action: 'skip', attemptCount, residencyStart };
  }

  return { action: 'proceed', attemptCount, residencyStart };
}
