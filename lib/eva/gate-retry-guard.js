...
/**
 * QF-20260913-466: clears a venture/stage's recorded gate-attempt history once something has
 * genuinely unblocked it (e.g. a chairman decision was just successfully (re)created after a
 * period where creation itself was failing). Without this, stale attempts from the FAILING period
 * keep counting toward GATE_RETRY_BACKOFF/GATE_RETRY_CEILING even after the real defect that
 * caused them is fixed -- observed live: a venture stuck at attemptCount=18 (2 below the ceiling
 * of 20) with a recent lastAttemptAt kept the worker in a ~24h backoff window for a condition
 * that no longer existed. Scoped to exactly (ventureId, stageNumber) -- never touches any other
 * venture's or stage's attempt history.
 * @returns {Promise<number>} rows deleted
 */
export async function resetGateAttempts(supabase, { ventureId, stageNumber }) {
  const { error, count } = await supabase
    ...
    .eq('stage_number', stageNumber);
  if (error) {
    throw new Error(`[gate-retry-guard] attempt reset failed: ${error.message}`);
  }
  return count ?? 0;
}
