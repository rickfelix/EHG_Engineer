...
export async function runCalibration(supabase, options = {}) {
  ...
  // Fetch resolved decisions with DFE context. Paginated (FR-6 batch 7): the
  // calibration corpus must see every resolved decision or the confusion matrix skews.
  let reviewDecisions;
  try {
    reviewDecisions = await fetchAllPaginated(() => {
      let q = supabase
        .from('chairman_decisions')
        .select('id, venture_id, lifecycle_stage, status, decision, dfe_context, created_at')
        .not('dfe_context', 'is', null)
        .in('status', ['approved', 'rejected']);
      if (config.since) q = q.gte('created_at', config.since);
      if (config.until) q = q.lte('created_at', config.until);
      return q.order('created_at', { ascending: true }).order('id', { ascending: true });
    });
  } catch (e) {
    return { error: e.message, total_decisions: 0 };
  }
