...
function _publicView(c) {
  return {
    ...
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-1: the pipeline CEILING (MAX(stage_number)), kept
    // distinct from totalStages (a COUNT). The two are equal only while numbering stays
    // contiguous from 1 — a future gap or non-1-start renumbering would make totalStages a
    // silently-wrong ceiling if reused as one. This is the derivation MAX_STAGE consumers must
    // read instead of a hardcoded literal.
    maxStageNumber: c.stages.size > 0 ? Math.max(...c.stages.keys()) : 0,
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 FR-2: the "does this stage carry a hard gate at all"
    // set for consumers like chairman_dashboard_config.hard_gate_stages / gate-bars.js's
    // CHAIRMAN_GATE_STAGES. Deliberately an alias for blockingStagesRaw (gate_type IN
    // ('kill','promotion'), NOT work_type='decision_gate') — the work_type-filtered view
    // (blockingStages) excludes stages 18/19/26 (sd_required/artifact_only work_type with a real
    // gate_type), and deriving hard-gate membership from that narrower view would silently drop
    // enforcement on them (RISK/TESTING evidence 8bfae0b7/b57c4b17).
    hardGateStages: c.blockingStagesRaw,
