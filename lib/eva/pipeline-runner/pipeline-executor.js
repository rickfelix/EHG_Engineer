...
export class PipelineExecutor {
  ...
  async _runStageZero(ventureId, ventureData) {
    // Map conceptual archetypes to discovery strategies for diversity
    const ARCHETYPE_STRATEGY_MAP = {
      democratizer: 'democratization_finder',
      automator: 'trend_scanner',
      capability_productizer: 'capability_overhang',
      first_principles_rebuilder: 'trend_scanner',
      vertical_specialist: 'trend_scanner',
      portfolio_connector: 'capability_overhang',
      experience_designer: 'democratization_finder',
    };
    const conceptualArchetype = ventureData.synthetic_metadata?.archetype_key || 'automator';
    const strategy = ARCHETYPE_STRATEGY_MAP[conceptualArchetype] || 'trend_scanner';
