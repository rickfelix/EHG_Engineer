 ...
 */
// eva-logger-lint-ignore: pure constant registry + trivial pure lookup helpers
// (getStageForArtifactType/isValidArtifactType/resolveArtifactType are synchronous
// find/has/map operations with no I/O, no error branches, and no side effects worth
// logging -- see docs/reference/eva-logging-standard.md's own "thin helper" exemption.
...
export const ARTIFACT_TYPES = Object.freeze({
  ...
  // S2 falsifiable demand thesis (docs/design/venture-selection-demand-thesis-design.md §1).
  // Hard input of the S21 distribution step (stage-22-distribution-setup.js
  // THESIS_ARTIFACT_TYPE) — the stage binding-blocks without a current row.
  TRUTH_DEMAND_THESIS: 'truth_demand_thesis',
  ...
  // SD-LEO-INFRA-VENTURE-WORKFLOW-CREATES-001: stage 23's organization acceptance-suite
  // result, recorded alongside LAUNCH_UAT_REPORT (same stage, same venture).
  ORGANIZATION_QA_RESULT: 'organization_qa_result',
  ...
  // SD-LEO-GEN-ALL-VENTURES-PRODUCED-001-A: Stage-23 (launch_readiness_gate) kill-gate witness,
  // self-produced by fn_submit_venture_usage_event on successful usage-event ingestion. Not the
  // stage-persist type for this stage (index [0] in ARTIFACT_TYPE_BY_STAGE below remains
  // LAUNCH_READINESS_CHECKLIST) -- appended, not prepended.
  LAUNCH_USAGE_SIGNAL: 'launch_usage_signal',
  ...
  // S21 Distribution Setup block marker (stage-22-distribution-setup.js
  // persistBlockMarker(), LIVE production write) — CORRECTED registration;
  // this value already writes in production and violates the CHECK constraint
  // today (silently persists as {persisted:false}). Registered here per
  // SD-LEO-INFRA-RUN-EVIDENCE-DURABILITY-001 (originally sourced as the
  // nonexistent artifact_type 'demand_thesis_missing', corrected by LEAD
  // validation-agent ground-truth 2026-07-11 — that value is only ever a
  // block_reason string, never an artifact_type). CHECK-widening pending
  // chairman apply.
  DISTRIBUTION_BLOCK_MARKER: 'distribution_block_marker',
});
...
export const ARTIFACT_TYPE_BY_STAGE = Object.freeze({
  ...
  21: [
    ...
    ARTIFACT_TYPES.ORGANIZATION_QA_RESULT,
  ],
  ...
  // SD-LEO-GEN-ALL-VENTURES-PRODUCED-001-A: LAUNCH_USAGE_SIGNAL appended, never prepended --
  // index [0] remains the canonical stage-persist type (stage-execution-engine.js,
  // eva-orchestrator.js read it as such; see this file's own hazard note for stage 2 above).
  23: [ARTIFACT_TYPES.LAUNCH_READINESS_CHECKLIST, ARTIFACT_TYPES.LAUNCH_MARKETING_CHECKLIST, ARTIFACT_TYPES.LAUNCH_USAGE_SIGNAL],
