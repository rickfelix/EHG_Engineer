...
export async function evaluateGateBars(row, opts = {}) {
  ...
  // gates (compose, do NOT replace). G1/G3/G4 ride S3; G2 rides S23/S25. Captured AFTER the
  // evidence bars so all_pass (below) stays the gate-bar regime's OWN verdict — the appended
  // intake bars are recorded for calibration but never flip the existing verdict. Evidence lives
  // in gate_criteria. The gates are pure/fail-open, so this cannot throw the evaluation.
  ...
  if (inScope) {
    ...
    const appended =
      ...
      : stage === 23 || stage === 25 ? [evaluateG2(intakeInput)]
      : [];
