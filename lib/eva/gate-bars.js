...
// SD-LEO-INFRA-VENTURE-INTAKE-GATE-PACK-001 (FR-3): import the ONE canonical venture-intake
// gate definitions; this file COMPOSES them onto S3/S23/S25 (append-not-replace), it does not
// redefine them. Observe-only — the appended bars are recorded but excluded from all_pass.
import { evaluateG1, evaluateG2, evaluateG3, evaluateG4 } from './venture-intake-gates.js';

/**
 * The 9 chairman-gate stages named by the sitting #1 ruling.
 *
 * SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-4/TR-8, individually investigated per this SD's triage
 * requirement — DISPOSITION: KEEP AS-IS, do NOT derive from the SSOT): this set is a chairman
 * RULING, not a gate_type classification. Deriving it from stage-governance.js's
 * promotionStagesRaw/killStagesRaw (kill: 3,5,13,23; promotion: 10,16,17,18,19,24,25) would
 * silently ADD stages 16 and 19 — gate_type='promotion' stages the sitting chairman never named
 * in this ruling. The 9-stage literal below IS this module's source of truth; it does not drift
 * from another SSOT because it was never meant to track one. See
 * docs/architecture/stage-advancement-fr4-tr8-triage.md.
 *
 * SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-B (FR-5): re-anchored from {23,24,25} to {24,25,26} to
 * track the SAME 3 named stages (Launch Readiness kill gate, go_live promotion gate,
 * Post-Launch promotion gate) after the dedicated-venture-UAT stage insert shifts them +1.
 * CONTINGENT on database/chairman-gated/20260825_dedicated_venture_uat_stage_insert_and_renumber.sql
 * having actually been applied — this literal is correct for the POST-renumber database only.
 * Deploying this file's change ahead of that (chairman-gated, staged, unapplied) migration
 * would drop stage 23's live kill gate from advisory observation until apply time (this module
 * is GATE_BARS_OBSERVE_ONLY, never blocking, so the exposure is a temporary calibration-data
 * gap, not an enforcement or security regression) — coordinate this file's release with the
 * migration's chairman-approval ceremony rather than merging it standalone.
 */
export const CHAIRMAN_GATE_STAGES = Object.freeze(new Set([3, 5, 10, 13, 17, 18, 24, 25, 26]));
...
export async function evaluateGateBars(row, opts = {}) {
  ...
  // SD-LEO-INFRA-VENTURE-INTAKE-GATE-PACK-001 (FR-3): APPEND the observe-only venture-intake
  // gates (compose, do NOT replace). G1/G3/G4 ride S3; G2 rides the Launch Readiness / Post-
  // Launch stages (re-anchored 23/25 -> 24/26 by SD-LEO-INFRA-DEDICATED-VENTURE-UAT-001-B FR-5,
  // same contingency note as CHAIRMAN_GATE_STAGES above: correct only once the companion
  // migration has applied). Captured AFTER the evidence bars so all_pass (below) stays the
  // gate-bar regime's OWN verdict — the appended intake bars are recorded for calibration but
  // never flip the existing verdict. Evidence lives in gate_criteria. The gates are pure/fail-
  // open, so this cannot throw the evaluation.
  const evidenceBarCount = bars.length;
  if (inScope) {
    const intakeInput = row?.gate_criteria || {};
    const appended =
      stage === 3 ? [evaluateG1(intakeInput), evaluateG3(intakeInput), evaluateG4(intakeInput)]
      : stage === 24 || stage === 26 ? [evaluateG2(intakeInput)]
      : [];
    for (const b of appended) {
      bars.push({ bar: `intake_${b.gate}`, pass: b.pass, status: b.status, detail: b.detail, observe_only: true, bar_version: b.bar_version });
    }
  }
  ...
  return {
    ...
    in_scope: inScope,
    // all_pass = the EVIDENCE bars' verdict only (intake gates excluded — separate advisory screen).
    all_pass: inScope ? bars.slice(0, evidenceBarCount).every((b) => b.status === 'pass') : true,
    bars,
  };
}
