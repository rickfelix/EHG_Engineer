...
// execFileSync's argv-array form is already immune to shell injection, but a leading-hyphen
// ARGUMENT can still be parsed as a FLAG by the invoked program itself, which is the actual
// (bounded, server-side, coordinator/cron-triggered) risk this closes. VALIDATION sub-agent
// finding (G5): an earlier version of this comment also cited a `--` argv separator as a second
// layer -- that separator was removed (it silently broke vitest's own file-targeting, see
// buildVitestInvocation's docstring below) and no second layer exists; this regex requirement is
// the only defense against flag-shaped extraction, not one of two.
...
// See the TERTIARY classifier below: a premise self-naming its own verification method
// ("confirmed via X", "measured via X") that isn't a file/test citation this checker can execute.
const NAMED_EXTERNAL_VERIFICATION_RE = /\b(?:confirmed|verified|measured)\b[\s\S]{0,60}?\b(?:via|through|by)\b/i;
...
export function checkQuickFixCitation(qf, opts = {}) {
  ...
  try {
    ...
    // TERTIARY classifier (never a resolution -- still INCONCLUSIVE either way): does the premise
    // itself NAME an external verification method this checker has no ability to execute (e.g.
    // "confirmed via branch protection API", "measured via headRefOid...")? QF-20260903-040: two
    // rows were closed with disposition_reason_code='no_deterministic_signal' -- literally false,
    // since each ticket's own text named the exact deterministic check its closure claimed was
    // unavailable. 'no_deterministic_signal' asserts NO signal was named; that assertion must be
    // true before it is written. A distinct, honest code for "a signal was named, this checker
    // cannot attempt it" keeps the outcome conservative (still INCONCLUSIVE) while making the
    // stored claim accurate against the content it was written onto.
    if (NAMED_EXTERNAL_VERIFICATION_RE.test(blob)) {
      return { outcome: OUTCOMES.INCONCLUSIVE, reasonCode: 'named_verification_unattempted', extractedPathCount };
    }
