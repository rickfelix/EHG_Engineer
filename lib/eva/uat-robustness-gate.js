...
export async function checkUatRobustnessGate(supabase, ventureId, fromStage) {
  ...
  if (!run.metadata?.control_pack_evaluated) {
    return {
      applies: true,
      satisfied: false,
      reason: `latest UAT run (${run.id}) reads quality_gate=GREEN but no control-pack evidence was ever evaluated (metadata.control_pack_evaluated is not true) -- a GREEN from pass-rate math alone does not satisfy this gate`,
    };
  }
