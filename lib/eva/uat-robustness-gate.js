...
export async function checkUatRobustnessGate(supabase, ventureId, fromStage) {
  ...
  if (!ventureRow) {
    return { applies: true, satisfied: false, indeterminate: true, reason: `uat-robustness-gate: no ventures row found for id ${ventureId} (fail-closed -- anomalous, matches checkSyntheticActorFencing's SEC-51 polarity)` };
  }
  ...
  return { applies: true, satisfied: true, reason: `latest UAT run (${run.id}) is GREEN with the control pack evaluated` };
}
