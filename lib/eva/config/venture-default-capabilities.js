...
//
// SD-LEO-GEN-ALL-VENTURES-PRODUCED-001-C: bumped from '2026.07'. telemetry-analytics
// description names the concrete sink (venture_usage_events, written via the sibling
// SD-LEO-GEN-ALL-VENTURES-PRODUCED-001-A's fn_submit_venture_usage_event RPC, read via
// this SD's fn_venture_usage_window_summary RPC) rather than the prior "events land
// somewhere" placeholder.
export const DEFAULT_CAPABILITIES_VERSION = '2026.08';
...
export const EHG_VENTURE_DEFAULT_CAPABILITIES = Object.freeze([
  ...
  Object.freeze({
    ...
    description: 'Wire a baseline usage-analytics layer (page/route views, key conversion events, active-user counts) so the Stage-25 post-launch review has real data to assess against rather than running blind. Privacy-respecting (no PII in event payloads); events land in the shared `venture_usage_events` table via fn_submit_venture_usage_event, and are queried for the post-launch review via the fn_venture_usage_window_summary RPC (never a direct table SELECT).',
    story_points: 1,
    priority: 'high',
