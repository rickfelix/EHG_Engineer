...
export async function generateLogoImage(ventureId, logoSpec, options = {}) {
  ...
  const { data: existing } = await supabase.from('venture_artifacts')
    .select('id, content').eq('venture_id', ventureId).eq('artifact_type', ARTIFACT_TYPES.IDENTITY_LOGO_IMAGE).eq('is_current', true).maybeSingle();
  ...
  await writeArtifact(supabase, {
    ventureId, lifecycleStage: 11, artifactType: ARTIFACT_TYPES.IDENTITY_LOGO_IMAGE,
