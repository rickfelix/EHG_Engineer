...
import { hasZeroAutomaticProducers } from './producerless-artifact-registry.js';
import { emitFeedback } from '../governance/emit-feedback.js';
import { createLogger } from '../logger.js';

const moduleLogger = createLogger('GateFailureRecovery');

/**
 * SD-LEO-INFRA-STAGE-LAUNCH-READINESS-001 (FR-4): true only when EVERY reason in the
 * failure is an ARTIFACT_MISSING whose artifact_type has zero automatic producers
 * (lib/eva/producerless-artifact-registry.js). A single REAL failure mixed in
 * (a different reason code, or an ARTIFACT_MISSING for a producer-backed type)
 * fails this check, so a genuine gap still escalates normally via chairman_decisions
 * -- this only redirects the pure automation-gap case.
 * @private
 */
function _isPureProducerlessArtifactGap(reasons) {
  if (!Array.isArray(reasons) || reasons.length === 0) return false;
  return reasons.every(
    (r) => r?.code === REASON_CODES.ARTIFACT_MISSING && hasZeroAutomaticProducers(r.artifact_type)
  );
}
...
export async function markKilledAtGate(supabase, ventureId, opts = {}) {
  const { reasons = [], fromStage, logger = moduleLogger } = opts;
 ...
 * Critical → DFE escalation record (unless FR-4's producerless-artifact-gap
 * carve-out applies, in which case → harness_backlog feedback row instead).
 ...
 * @returns {Promise<{routed: boolean, path: 'dfe'|'auto_track'|'harness_backlog', error?: string}>}
 */
export async function routeGateOutcome(ventureId, severity, context, deps) {
  ...
  if (severity === 'critical') {
    // SD-LEO-INFRA-STAGE-LAUNCH-READINESS-001 (FR-4): a critical failure that is
    // ENTIRELY a producer-less required artifact (e.g. launch_usage_signal -- its
    // sole producer is a post-launch-only RPC insert, so it can never be satisfied
    // by a pre-launch gate) is a structural automation gap, not a venture-specific
    // decision. Route it to the durable harness_backlog channel instead of minting
    // a chairman_decisions row (precedent: automation-defect ruling 1afdeaac).
    if (_isPureProducerlessArtifactGap(reasons)) {
      const missingTypes = reasons.map((r) => r.artifact_type).join(', ');
      try {
        await emitFeedback({
          supabase,
          title: `Producer-less required artifact blocks gate: ${missingTypes}`,
          description: `Gate failure at ${fromStage ?? '?'}->${toStage ?? '?'} for venture ${ventureId}: ${formatReasons(reasons)}. `
            + `${missingTypes} has zero automatic producers in the pipeline (see lib/eva/producerless-artifact-registry.js) -- `
            + 'this is a harness automation gap, not a decision this venture-instance escalation can resolve.',
          category: 'harness_backlog',
          severity: 'medium',
          source_type: 'gate_failure_recovery',
          metadata: { venture_id: ventureId, fromStage, toStage, reasons, sd_origin: 'SD-LEO-INFRA-STAGE-LAUNCH-READINESS-001' },
        });
        logger.info(`[GateRecovery] Producer-less artifact gap for ${ventureId} routed to harness_backlog: ${missingTypes}`);
        return { routed: true, path: 'harness_backlog' };
      } catch (err) {
        logger.error(`[GateRecovery] harness_backlog routing error: ${err.message}`);
        return { routed: false, path: 'harness_backlog', error: err.message };
      }
    }

    // Create DFE escalation record
    try {
      // QF-20260829-634 fold: summary was never populated (always NULL), leaving
      // queue displays blank — all content lived only in `rationale`. Short,
      // single-line version of the same content for list/queue rendering.
      const summaryLine = `Gate failure at ${fromStage ?? '?'}->${toStage ?? '?'}: ${formatReasons(reasons)}`.slice(0, 240);
