  ...
  if (TASTE_GATE_STAGES.includes(resolvedStage) && !options.dryRun) {
    try {
      ...
      if (isEnabled) {
        ...
        // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 (TESTING F11): this call passed `details:` where
        // recordGateResult destructures `criteria:` -- an unrecognized key is silently dropped
        // (no error), so every taste-gate row wrote gate_criteria=null despite tasteGateResult
        // carrying real dimension/reason evidence. Fixed to the recognized param name.
