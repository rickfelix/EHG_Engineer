...
import { writeArtifact, advanceStage } from './artifact-persistence-service.js';
...
export async function processStage({ ventureId, stageId, options = {} }, deps = {}) {
  ...
  try {
    const { data: stageConfig } = await supabase
      ...
      .single();
    if (stageConfig?.required_artifacts?.length > 0) {
  ...
  try {
    ...
    for (const step of steps) {
      try {
        ...
        artifacts.push({
          artifactType: stepResult.artifactType || step.artifactType || (requiredArtifacts[0] || 'stage_output'),
          stageId: resolvedStage,
    ...
    if (requiredArtifacts.length > 1) {
      try {
        const extracted = extractMultiArtifacts(stageOutput, requiredArtifacts, resolvedStage);
        if (extracted.length === requiredArtifacts.length) {
          artifacts = extracted;
          logger.log(`[Eva] Multi-artifact extraction: ${extracted.length} artifacts for stage ${resolvedStage}`);
        } else {
          // Extraction couldn't split by headings — replicate the combined output
          // as each required artifact type so reality gates find all types.
          logger.warn(`[Eva] Multi-artifact extraction partial (${extracted.length}/${requiredArtifacts.length}), replicating to all types`);
          artifacts = requiredArtifacts.map(artifactType => ({
            artifactType,
            stageId: resolvedStage,
            createdAt: new Date().toISOString(),
            payload: artifacts[0]?.payload || stageOutput,
            source: artifacts[0]?.source || 'eva-orchestrator',
          }));
        }
      } catch (extractErr) {
