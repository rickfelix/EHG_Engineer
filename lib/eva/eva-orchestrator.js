    ...
    try {
      ...
      if (isEnabled) {
        ...
        if (tasteGateResult.verdict === TASTE_VERDICT.ESCALATE) {
          // SD-LEO-FIX-GOOGLE-STITCH-PIPELINE-001-C: Route through adapter with structured logging
          try {
            const { tasteGateProvision } = await import('./bridge/stitch-adapter.js');
            const stitchResult = await tasteGateProvision(
              ventureId, {}, {},
              { ventureName: ventureContext?.name, stage: resolvedStage }
            );
            if (stitchResult?.status === 'unavailable') {
              logger.warn(`[Eva] Taste gate ESCALATE → Stitch unavailable (${stitchResult.reason}), skipping provision`);
            } else {
