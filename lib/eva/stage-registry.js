 ...
 * - File-based registration for built-in stages (1-27)
...
import { validateStageKeyBinding } from './stage-templates/stage-key-registry.js';
...
export class StageRegistry {
  ...
  register(stageNumber, template, options = {}) {
    ...
    // SD-LEO-INFRA-STAGE-TEMPLATE-DISPATCH-REMAINDER-001: a template that opts in via
    // TEMPLATE.stageKey must bind to the position it's actually loaded under — this is
    // the check that makes a future renumber fail LOUD (thrown, caught by the caller's
    // existing warn-log path) instead of silently re-binding content, closing the gap
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001 explicitly deferred.
    const keyCheck = validateStageKeyBinding(stageNumber, template);
    if (!keyCheck.valid) {
      throw new Error(
        `StageRegistry.register(${stageNumber}): stage_key mismatch — template declares `
        + `stageKey="${keyCheck.actual}" but position ${stageNumber} expects `
        + `"${keyCheck.expected}". A renumber moved this template's position without `
        + 'updating its content, or vice versa.',
      );
    }
  ...
  /**
   * Register all built-in stage templates (1-27) from file system.
