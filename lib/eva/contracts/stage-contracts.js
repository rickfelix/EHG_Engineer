 ...
 */
export function validatePreStage(stageNumber, upstreamData, { logger = console } = {}) {
  ...
  for (const dep of contract.consumes) {
    ...
    if (!stageData) {
      if (Object.values(dep.fields).some(f => f.required !== false)) {
        warnings.push(`Stage ${stageNumber}: upstream stage-${String(dep.stage).padStart(2, '0')} data not available`);
      }
      continue;
    }
  ...
  if (allErrors.length > 0) {
    logger.warn(`[Eva][Contract] Pre-stage ${stageNumber} validation: ${allErrors.length} error(s)`, { errors: allErrors });
  }
 ...
 * @returns {{ valid: boolean, errors: string[], warnings: string[] }}
 */
export function validatePostStage(stageNumber, stageOutput, { logger = console } = {}) {
  const contract = STAGE_CONTRACTS.get(stageNumber);
  ...
  if (!result.valid) {
    logger.warn(`[Eva][Contract] Post-stage ${stageNumber} validation: ${result.errors.length} error(s)`, { errors: result.errors });
  }
