...
import { resolveVerifier, checkGateProvenance } from './exit-gate-verifiers.js';
import { EXIT_GATE_ANOMALY, EXIT_GATE_OBSERVE_ONLY, EXIT_GATE_OBSERVE_UNRESOLVED } from './exit-gate-event-types.js';
import { createLogger } from '../../logger.js';

const logger = createLogger('ExitGateEnforcer');
 ...
 * @property {string[]} provenance_warnings — SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5):
 *   "<gate string> (<artifact_type>): missing/unverifiable machine provenance" entries from
 *   checkGateProvenance(), for BOTH binding and observe-only gates. Advisory-only — NEVER
 *   affects `allowed`/`blocked_by`/`would_block_by`. Empty for gates not backed by
 *   venture_artifacts (ventures, chairman_decisions, guardrail state, etc).
 */
...
async function logAnomalyEvent({ supabase, ventureId, stageNumber, anomalyKind, gateString = null, reason }) {
  try {
    ...
    // SECURITY finding F2: supabase-js RETURNS PostgREST errors rather than throwing them, so an
    // unbound insert failure here was silently swallowed by the try/catch (never reached it) --
    // the exact "vanishes silently" failure mode FR-3 exists to eliminate, one layer down. Bind
    // and warn on `error` explicitly; still never throw (a logging failure must not affect the
    // already-fail-closed blocking verdict).
    ...
    if (error) {
      console.warn(`[exit-gate-enforcer] anomaly event log failed (non-fatal): ${error.message}`);
    }
  } catch (err) {
...
async function logObserveOnlyEvent({ supabase, eventType, ventureId, stageNumber, gateString, wouldSatisfy, reason }) {
  try {
    const stamp = new Date().toISOString();
    // SECURITY finding F2 (see logAnomalyEvent above for the full rationale): bind `error`
    // explicitly since supabase-js does not throw on a PostgREST insert failure.
    const { error } = await supabase.from('system_events').insert({
    ...
    if (error) {
      console.warn(`[exit-gate-enforcer] observe-only event log failed (non-fatal): ${error.message}`);
    }
  } catch (err) {
...
export async function checkExitGates({ supabase, ventureId, fromStage }) {
  ...
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): advisory-only findings from
  // checkGateProvenance() below -- NEVER affects allowed/blocked_by/would_block_by.
  ...
  if (Array.isArray(gatesExitObserve) && gatesExitObserve.length > 0) {
    for (const gateString of gatesExitObserve) {
      ...
      if (!verifier) {
        // SD-FDBK-FIX-EXIT-GATE-CONFORMANCE-001 (FR-3): previously a bare `continue` here —
        // an unresolvable OBSERVE gate string vanished with only a console.warn, no
        // system_events row, so a typo read as "satisfied" to any would-block-rate consumer.
        // This does NOT change the log-and-allow polarity (observe-mode still never blocks
        // advancement, per the pinned test at tests/unit/eva/lifecycle/exit-gate-enforcer.test.js
        // line ~288) — it only makes the unresolvable case fail LOUD instead of silent.
        const reason = `${gateString}: no verifier registered for an OBSERVE-mode gate`;
        console.warn(`[exit-gate-enforcer] No verifier registered for observe-only gate "${gateString}" at stage ${fromStage} — skipping.`);
        // PINNED (tests/unit/eva/lifecycle/exit-gate-enforcer.test.js "unknown gate strings
        // under gates.exit_observe still log-and-allow"): would_block_by stays untouched here —
        // it means "evaluated and found unsatisfied", which an unresolvable gate was never
        // evaluated for. The new EXIT_GATE_OBSERVE_UNRESOLVED system_events row (not
        // would_block_by) is the fail-loud signal for this case; FR-4's precheck reads that
        // event type directly rather than inferring it from would_block_by.
        await logObserveOnlyEvent({
          supabase, eventType: EXIT_GATE_OBSERVE_UNRESOLVED, ventureId, stageNumber: fromStage,
          gateString, wouldSatisfy: false, reason,
        });
        continue;
      }
      const { satisfied, reason } = await verifier({ supabase, ventureId, fromStage });
      if (!satisfied) would_block_by.push(`${gateString}: ${reason}`);
      const provenanceIssues = await checkGateProvenance(supabase, ventureId, verifier);
      for (const artifactType of provenanceIssues) provenance_warnings.push(`${gateString} (${artifactType}): missing/unverifiable machine provenance`);
      await logObserveOnlyEvent({
  ...
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): advisory-only -- never affects the
  // allowed/blocked_by verdict below, just makes the finding observable in structured logs.
  if (provenance_warnings.length > 0) {
    logger.debug('binding gate check found provenance warnings', { ventureId, fromStage, warnings: provenance_warnings });
  }
