...
// SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001 (FR-6/FR-7): defense-in-depth registration of the
// same bespoke fencing check _advanceStage() runs as its primary, binding choke-point. This
// site is registered under gates.exit_observe (never gates.exit binding) so an unregistered
// or misconfigured entry here fails safe (log-and-allow) rather than causing a fleet-wide
// stage-19 outage — the PRIMARY validation locus is _advanceStage() itself.
import { checkSyntheticActorFencing } from '../synthetic-actor-guard.js';
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): shared venture_artifacts provenance model.
import { gradeVentureArtifactProvenance } from '../venture-artifact-provenance.js';
import { createLogger } from '../../logger.js';

const logger = createLogger('ExitGateVerifiers');
...
/**
 * NON-BINDING DISPOSITION (FR-1 option c, testing-agent-informed; DEMOTED to gates.exit_observe
 * by QF-20260818-010): the following 5 gate strings remain UNRESOLVABLE by design, not oversight.
 * Each was investigated and found to have NO real backing implementation anywhere in the
 * codebase — re-confirmed 2026-08-18 against current main (zero writers/concepts found):
 *   - S1 "Category assigned" — ventures.category column exists but has ZERO known writers.
 *     A verifier would make S1 universally unsatisfiable for every venture ever created (worse
 *     than the current unexercised state) since the column is always NULL.
 ...
 * neither is a QF-scoped change, and writing a rubber-stamp verifier would fabricate a PASS.
 *
 * QF-20260818-010 (FR-4 binding-flip precondition): rather than leave these 5 strings declared
 * on venture_stages.metadata.gates.exit (BINDING — exit-gate-enforcer.js fail-CLOSES on an
 * unresolvable binding verifier per HP-1, meaning any venture that ever hit the two instrumented
 * checkExitGates() call sites at stage 1/2/3 would be PERMANENTLY, unfixably blocked, since none
 * of these 5 concepts can ever become satisfied), the migration
 * database/migrations/20260818_demote_s1s2s3_unresolvable_binding_gates_to_observe.sql moves
 * them to gates.exit_observe. resolveVerifier() still correctly returns null for all 5 — no
 * rubber-stamp verifier is registered — but exit-gate-enforcer.js's observe-mode branch treats
 * that as fail-LOUD (an EXIT_GATE_OBSERVE_UNRESOLVED system_events row) rather than fail-closed,
 * which honestly reflects reality (the concept doesn't exist) without silently hiding it and
 * without ever blocking stage advancement on an unsatisfiable check.
 */

/**
 * Verifier: fenced synthetic-actor exclusion wiring, delegated to the same bespoke check
 * _advanceStage() runs as its binding choke-point (lib/eva/synthetic-actor-guard.js).
 * Backs the gate string "synthetic-actor-fencing-configured" — non-binding here (exit_observe
 * only); see the file-header note above for why this site is defense-in-depth, not primary.
 *
 ...
 * @returns {Promise<VerifierResult>}
 */
async function verifySyntheticActorFencingConfigured({ supabase, ventureId }) {
  const result = await checkSyntheticActorFencing(supabase, ventureId);
  if (!result.applies) {
    // Venture never opted in — same fleet-safety short-circuit as the primary choke-point.
    return { satisfied: true, reason: '' };
  }
  return { satisfied: result.satisfied, reason: result.satisfied ? '' : result.reason };
}

/**
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-D (C3.2): Stage 15 design-fidelity observe-only gate.
 * Backs the gate string 'design fidelity reviewed'.
 *
 * Reads the venture's current stitch_qa_report artifact, written by
 * lib/eva/qa/stitch-wireframe-qa.js's scoreWireframeFidelity(). Fail-closed on every absent
 * or non-completed path: no artifact, no wireframe_fidelity field, or a status other than
 * 'completed' (e.g. 'no_screens' -- expected today for every venture, since no venture
 * fleet-wide has yet produced the stitch_design_export artifact scoreWireframeFidelity's
 * getExportedScreens() reads) all return satisfied:false with a distinct reason, never a
 * fabricated pass. A completed report additionally satisfies only when aggregate.fail_count
 * is exactly 0 -- any failing screen keeps the gate unsatisfied.
 *
 * @param {VerifierContext} ctx
 * @returns {Promise<VerifierResult>}
 */
async function verifyDesignFidelityReviewed({ supabase, ventureId }) {
  const { data, error } = await supabase
    .from('venture_artifacts')
    .select('metadata')
    .eq('venture_id', ventureId)
    ...
    .maybeSingle();
  if (error) return { satisfied: false, reason: `venture_artifacts query failed: ${error.message} (fail-closed)` };
  if (!data) return { satisfied: false, reason: 'stitch_qa_report artifact missing or not is_current (fail-closed)' };
  const fidelity = data.metadata?.wireframe_fidelity;
  if (!fidelity) return { satisfied: false, reason: 'stitch_qa_report.metadata.wireframe_fidelity missing (fail-closed)' };
  if (fidelity.status !== 'completed') {
    return { satisfied: false, reason: `wireframe_fidelity.status='${fidelity.status}' (not 'completed') -- fail-closed` };
  }
  const failCount = fidelity.aggregate?.fail_count;
  if (typeof failCount !== 'number') {
    return { satisfied: false, reason: 'wireframe_fidelity.aggregate.fail_count missing (fail-closed)' };
  }
  return failCount === 0
    ? { satisfied: true, reason: '' }
    : { satisfied: false, reason: `${failCount} screen(s) failed wireframe-fidelity scoring` };
}

/**
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-5): the venture_artifacts artifact_type(s) each
 * venture_artifacts-backed verifier function reads, keyed by the SAME function reference used in
 * GATE_VERIFIERS below (declared once there is no risk of the two lists drifting on the verifier
 * side -- only a new artifact-type addition needs a matching entry here).
 *
 * Deliberately NOT touching any of the 10 verifier functions' own internals (each has several
 * return branches; a per-branch edit across all of them is a much larger, higher-risk diff for
 * the same outcome) -- provenance is graded via a SEPARATE, additive re-query dispatched by
 * exit-gate-enforcer.js alongside the existing verifier call, never inside it. A verifier whose
 * gate does not read venture_artifacts at all (ventures, chairman_decisions, guardrail state,
 * stack_descriptor, synthetic-actor-guard delegation) has no entry and is correctly untouched.
 */
const VERIFIER_ARTIFACT_TYPES = new Map([
  [verifyBuildMvpBuildPresent, ['build_mvp_build']],
  [verifyLaunchTriggered, ['launch_metrics', 'launch_launch_metrics']],
  [verifyAllChannelsActivated, ['launch_metrics', 'launch_launch_metrics']],
  [verifyGrowthPlaybookPresent, ['growth_playbook']],
  [verifyGrowthOptimizationRoadmapPresent, ['growth_optimization_roadmap', 'launch_optimization_roadmap']],
  [verifyDescriptionValidated, ['truth_idea_brief']],
  [verifyFinancialModelComplete, ['truth_financial_model']],
  [verifyUnitEconomicsViable, ['truth_financial_model']],
  [verifyLaunchReadinessGreen, ['launch_readiness_checklist']],
  // readCodeQualityReport is a SHARED reader for these 3 verifiers -- same artifact_type.
  [verifyNoCriticalSecurityIssues, ['code_quality_report']],
  [verifyNoExposedSecrets, ['code_quality_report']],
  [verifyLintPasses, ['code_quality_report']],
]);

/**
 * Re-query venture_artifacts for the given types and grade each present row's machine
 * provenance. Advisory-only by construction -- the caller (exit-gate-enforcer.js) records the
 * result separately from `satisfied`/`blocked_by` and never lets it affect the gate verdict.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} ventureId
 * @param {(ctx: VerifierContext) => Promise<VerifierResult>} verifierFn - a GATE_VERIFIERS entry
 * @returns {Promise<string[]>} artifact_types present but failing provenance grading (empty if
 *   the verifier is not venture_artifacts-backed, or none fail)
 */
export async function checkGateProvenance(supabase, ventureId, verifierFn) {
  const artifactTypes = VERIFIER_ARTIFACT_TYPES.get(verifierFn);
  if (!artifactTypes || !supabase || !ventureId) return [];
  // Advisory-only: ANY failure here (query error, a client/mock that doesn't implement a method
  // used, a malformed row) must degrade to "no findings", never throw into the caller's binding
  // gate evaluation.
  try {
    const { data, error } = await supabase
      .from('venture_artifacts')
      .select('artifact_type, metadata, content, artifact_data, created_at')
      .eq('venture_id', ventureId)
      .eq('is_current', true)
      .in('artifact_type', artifactTypes)
      .limit(20);
    if (error || !data) return [];
    return data.filter((row) => gradeVentureArtifactProvenance(row).absent).map((row) => row.artifact_type);
  } catch (err) {
    logger.debug('checkGateProvenance degraded to no-findings', { ventureId, error: err?.message });
    return [];
  }
}
...
export const GATE_VERIFIERS = Object.freeze([
  ...
  // SD-LEO-INFRA-ALTIFYAI-TEST-IDENTITY-001 FR-6/FR-7 — defense-in-depth only (gates.exit_observe).
  { match: 'synthetic-actor-fencing-configured', verifier: verifySyntheticActorFencingConfigured },
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-D (C3.2) — Stage 15 observe-only design-fidelity gate.
  { match: 'design fidelity reviewed', verifier: verifyDesignFidelityReviewed },
]);
