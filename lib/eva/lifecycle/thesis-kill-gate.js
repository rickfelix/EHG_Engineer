...
import { evaluateThesisKillCriteria, createMetadataResolver } from './thesis-kill-evaluator.js';
import { createOrReusePendingDecision } from '../chairman-decision-watcher.js';
import { getStageGovernance } from '../stage-governance.js';
import { validateKillFireReadiness } from '../stage-zero/thesis-contract.js';
 ...
 *   defaults to a real one bound to this venture's metadata (createMetadataResolver) when omitted
 * @param {Object} [args.logger]
 * @returns {Promise<{ allowed: boolean, would_kill_by: string[], blocked_by: string[], fired: Array, held: Array, flag_enforced: boolean }>}
 */
export async function checkThesisKillGate({ supabase, ventureId, fromStage, toStage, resolveObservedValue, logger = console }) {
  ...
  try {
    ...
    ventureMetadata = venture?.metadata ?? null;
    killCriteria = ventureMetadata?.kill_criteria ?? null;
  } catch (err) {
  ...
  // SD-LEO-INFRA-MINUS-CARGO-INSTRUMENTS-001 FR-1: when the caller does not inject a resolver
  // (every production call site today), build a real one bound to this venture's metadata
  // instead of falling back to the permanent no-op default — this is the seam that makes
  // K1-K3 actually measurable. A caller-injected resolveObservedValue (tests, future DI)
  // still wins unchanged.
  const resolver = resolveObservedValue || createMetadataResolver(ventureMetadata);
  ...
  // SD-LEO-INFRA-MINUS-CARGO-INSTRUMENTS-001 FR-3: per-criterion resolver isolation now lives
  // INSIDE evaluateThesisKillCriteria (thesis-kill-evaluator.js) — a single throwing resolver
  // classifies only that criterion as HOLD/resolver_error and sibling criteria still evaluate
  // normally. This outer catch remains as the last-resort guard for a genuinely unexpected
  // failure (e.g. a malformed criteria array crashing the filter itself), which should now
  // rarely if ever fire for a plain per-metric resolver error.
  let fired = [];
  let held = [];
  try {
    ({ fired, held } = await evaluateThesisKillCriteria({ killCriteria, toStage, resolveObservedValue: resolver }));
  } catch (err) {
  ...
  for (const verdict of held) {
    await logThesisKillEvent({ supabase, eventType: 'THESIS_KILL_CANNOT_EVALUATE', ventureId, fromStage, stageNumber: toStage, verdict });
    // FR-5: a resolver that THREW (not merely "no data yet") indicates a broken instrument,
    // not a legitimate no-data gate outcome — mint a factory_defect finding (best-effort,
    // never blocks or throws).
    if (verdict.errorClass === 'resolver_error') {
      try {
        await recordFactoryDefect(supabase, {
          source_sd_id: null,
          gap_class: GAP_CLASS.INSTRUMENT_LIE,
          title: `Thesis-kill resolver threw for criterion "${verdict.criterionId}"`,
          description: verdict.errorMessage || '(no error message captured)',
          metadata: { venture_id: ventureId, stage_number: toStage, criterion_id: verdict.criterionId, metric: verdict.metric },
        });
      } catch (err) {
        logger.warn?.(`[thesis-kill-gate] factory_defect record failed (non-fatal): ${err.message}`);
      }
    }
  }
  ...
  for (const verdict of fired) {
    // SD-LEO-INFRA-MINUS-CARGO-INSTRUMENTS-001 FR-4: a FIRED verdict lacking frozen criteria,
    // an evidence ref, a valid canonical stage, or with an open blocking finding is downgraded
    // BEFORE it ever reaches would_kill_by/blocked_by or mints a decision — never persisted as
    // a kill. Zero-DB pure check; governance + openBlockingFindings are fetched here.
    const criterion = killCriteria.find((c) => c?.id === verdict.criterionId);
    let readiness = { ready: true };
    try {
      const governance = await getStageGovernance(supabase);
      // count-truncation-diff-lint: bounded — a single venture legitimately having more than 50
      // concurrently OPEN factory_defect findings would itself be a separate, alarming problem;
      // this only needs to know "is there at least one blocking finding", not enumerate all of them.
      const { data: openFindings } = await supabase
        ...
        .limit(50);
      const openBlockingFindings = (openFindings || [])
        .map((f) => ({ gap_class: f.metadata?.gap_class }))
        .filter((f) => f.gap_class);
      readiness = validateKillFireReadiness(criterion, verdict, governance, openBlockingFindings);
    } catch (err) {
      logger.warn?.(`[thesis-kill-gate] fire-readiness check failed (fail-closed, downgrading): ${err.message}`);
      readiness = { ready: false, reason: 'readiness_check_failed' };
    }

    if (!readiness.ready) {
      const downgraded = { ...verdict, errorClass: 'unobservable_input', errorMessage: `kill_validity_precondition_failed: ${readiness.reason}` };
      await logThesisKillEvent({ supabase, eventType: 'THESIS_KILL_CANNOT_EVALUATE', ventureId, fromStage, stageNumber: toStage, verdict: downgraded });
      if (readiness.reason !== 'blocking_finding_open') {
        const gapClass = readiness.reason === 'invalid_stage' ? GAP_CLASS.GATE_BYPASSED : GAP_CLASS.GATE_CANNOT_FAIL;
        try {
          await recordFactoryDefect(supabase, {
            source_sd_id: null,
            gap_class: gapClass,
            title: `Thesis-kill fire-readiness precondition failed for criterion "${verdict.criterionId}"`,
            description: `reason=${readiness.reason}`,
            metadata: { venture_id: ventureId, stage_number: toStage, criterion_id: verdict.criterionId },
          });
        } catch (err) {
          logger.warn?.(`[thesis-kill-gate] factory_defect record failed (non-fatal): ${err.message}`);
        }
