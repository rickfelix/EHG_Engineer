...
export function evaluateExitGateCriterion(rows, marketLensVentureIds = MARKETLENS_VENTURE_IDS) {
  ...
  const marketLensFalseReject = marketLensRows.some((r) => r.would_satisfy === false);
...
/**
 * FR-2/TR-2: fetches ALL EXIT_GATE_OBSERVE_ONLY rows in one query, filtered by event_type at
 * the database layer (never a post-fetch JS filter on the unfiltered 141k-row table). Per-gate
 * grouping happens in JS afterward (groupRowsByGateString) -- cheap, since event_type alone
 * narrows the live table to single-digit rows today.
 *
...
export async function fetchVentureStackObserveRows(supabase) {
  const { data, error } = await supabase
    .from('system_events')
    .select('payload, created_at')
    .eq('event_type', 'VENTURE_STACK_OBSERVE_ONLY');
  if (error) throw error;
  return (data || []).map((r) => ({
