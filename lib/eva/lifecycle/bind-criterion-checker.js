 ...
 */
// eva-logger-lint-ignore: pure, side-effect-free evaluators (no I/O) except
// crossCheckCandidateGateStrings, whose only I/O boundary already surfaces its error via the
// returned {error} field by design (its own docstring: "returns a diff for the CLI to print as a
// warning") -- the CLI caller displays it, matching this file's caller-surfaces-it convention
// rather than a module-level structured logger.
...
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-H (FR-6): the three "Go Live" exit_observe strings
// below were pinned to the pre-renumber stage 24 -- live-verified (venture_stages.metadata.
// gates.exit_observe) that they now live at stage 25 (go_live); stage 24 is now
// launch_readiness_gate and carries no exit_observe strings at all.
export const CANDIDATE_GATE_STRINGS = Object.freeze([
  ...
  Object.freeze({ stage_number: 25, gate_string: 'pages url live' }),
  Object.freeze({ stage_number: 25, gate_string: 'compute deployed' }),
  Object.freeze({ stage_number: 25, gate_string: 'publish evidence recorded' }),
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-D (C3.2): tracks graduation-readiness for the new
  // Stage 15 observe-only design-fidelity gate (lib/eva/lifecycle/exit-gate-verifiers.js's
  // verifyDesignFidelityReviewed). No venture has produced real observation data yet -- this
  // entry is registered ahead of that data so the checker starts counting the moment any
  // venture's stitch_qa_report exists, rather than requiring a second follow-up SD later.
  Object.freeze({ stage_number: 15, gate_string: 'design fidelity reviewed' }),
]);

// SD-LEO-INFRA-ARM-BINDING-EXIT-001 FR-3: exported so the new crack-gate criterion (lib/eva/
// lifecycle/crack-gate-criterion.js) reuses these values via import rather than re-typing the
// literals. Values unchanged (25, 48); evaluateExitGateCriterion/evaluateVentureStackCriterion
 ...
 * config for stage 19/25, so a future SD adding/removing an observe-only string doesn't leave
