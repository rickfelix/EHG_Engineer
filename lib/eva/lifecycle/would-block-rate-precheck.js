 ...
 * -- see the DISCLOSURE paragraph immediately below. Exercised directly by
 * tests/unit/eva/lifecycle/would-block-rate-precheck.test.js (6 tests, including a source-pinned
 * mock proving the event-type filter is genuinely applied); unwired, not untested.
 *
 ...
 *
 * SECURITY finding F1: the original hand-rolled pagination loop had no `.order()` and no
 * maxPages guard. Postgres does not guarantee row order across statements without one — an
 * unordered `.range()` re-query can return the same row twice or skip another, silently
 * mis-measuring the rate (see lib/git/branch-owner.js's ".order() IS LOAD-BEARING, NOT TIDINESS"
 * precedent). Reuses the repo's existing fetchAllPaginated() (lib/db/fetch-all-paginated.mjs)
 * rather than re-deriving the same maxPages/short-page logic a second time — it already converts
 * a query builder that ignores .range() into a diagnosable error instead of an infinite loop.
 * `.order('id', ...)` gives a stable (not chronological — id is a random UUID) total order,
 * which is all pagination correctness requires.
 *
...
export async function computeWouldBlockRate({ supabase }) {
  ...
  try {
    ...
    throw new Error(`system_events query failed: ${err.message}`);
  }
