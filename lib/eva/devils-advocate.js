 ...
 *
 * Integration points (SD-LEO-INFRA-MINUS-GATE-SSOT-001 FR-1/TR-1/TR-7: SSOT-derived from
 * lib/eva/stage-governance.js's raw gate_type view, not hardcoded literals):
 *   - Kill gates: stages 3, 5, 13, 23
 *   - Promotion gates: stages 10, 16, 17, 18, 19, 24, 25
 *
...
// SD-LEO-INFRA-KILLGATE-DETERMINISM-ALL-GATES-001: the single source of truth so the
// orchestrator's atKillGateStage / isRouteToReview predicates cover ALL kill gates (not a local literal).
// SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-1/TR-7): converted from static arrays to async accessors —
// the SSOT (lib/eva/stage-governance.js) requires a DB round-trip, so these can no longer be
// synchronous module-level constants. Callers (eva-orchestrator.js, already async) await them.
export async function getKillGates(supabase) {
  const gov = await getStageGovernance(supabase);
  return [...gov.killStagesRaw].sort((a, b) => a - b);
}

export async function getPromotionGates(supabase) {
  const gov = await getStageGovernance(supabase);
  return [...gov.promotionStagesRaw].sort((a, b) => a - b);
}

export async function getAllGates(supabase) {
  const [kill, promotion] = await Promise.all([getKillGates(supabase), getPromotionGates(supabase)]);
  return [...kill, ...promotion];
}
...
const SECTION_BUDGETS = {
  ...
  // QF-20260816-546: system_architecture/implementation_approach were entirely omitted from
  // prdSections -- the critique never saw them at all. Sized against a live corpus scan
  // (n≈930, 2026-08-16), not a single fixture like the four above: p90/p99/max were
  // 3,190/4,030/6,311 (system_architecture) and 2,387/3,115/7,217 (implementation_approach).
  // Budgeted above p90 with real headroom, not the corpus max -- consistent with this file's
  // existing precedent (the four fixed budgets above are also sized with headroom over a
  // single measurement, not a guaranteed-never-truncate ceiling).
  system_architecture: 6000, // p90 3,190 — 1.9x headroom
  implementation_approach: 5000, // p90 2,387 — 2.1x headroom
};

/**
 * Check if a stage has a Devil's Advocate gate.
 * SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-1/TR-7): SSOT-derived, now requires `supabase`.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {number} stageId
 * @returns {Promise<{ isGate: boolean, gateType: 'kill'|'promotion'|null }>}
 */
export async function isDevilsAdvocateGate(supabase, stageId) {
  const gov = await getStageGovernance(supabase);
  if (gov.isKillRaw(stageId)) return { isGate: true, gateType: 'kill' };
  if (gov.isPromotionRaw(stageId)) return { isGate: true, gateType: 'promotion' };
 ...
 * no recorded adversarial input. Only PROMOTION gates may bypass production, and (since
 * SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 FR-5 below) only on 'auto_approve'.
 * Pure decision used by the EVA orchestrator's §5b branch (and unit-tested directly).
 *
 * SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 (FR-5, descoped per O1): the SD's original ask was to
 * disable the skip bypass specifically for an undefined "W3-class evaluation" concept, which has
 * ZERO code referents anywhere in lib/eva — it is an Adam roadmap WAVE identifier, not an EVA
 * evaluation classification the code can branch on. EXEC sent one non-stalling request for a
 * code-expressible mapping at the start of EXEC and, per the pre-authorized descope path (no
 * answer by the time FR-1..FR-4 were complete), implements the broader, honest, code-expressible
 * claim instead: disable the 'skip' bypass for L3/L4 autonomy levels GENERALLY. The autonomy
 * matrix (autonomy-model.js) gives devils_advocate:'skip' ONLY at L3/L4, so removing 'skip' from
 * the bypass set below is exactly that. 'auto_approve' (L2) is a SEPARATE autonomy action and is
 * left untouched by design (D9) — this fix has teeth only on promotion gates (kill gates were
 * already unconditional above).
...
export function mustProduceDevilsAdvocate(gateType, autonomyAction) {
  ...
  const bypass = autonomyAction === 'auto_approve'; // FR-5: 'skip' no longer bypasses
  return !bypass; // promotion gates produce unless L2 auto_approve legitimately bypasses
}
...
/**
 * FR-4: SHA-256 of the FULL, pre-truncation prdContent+archContent (prdRawText/archRawText from
 * buildCritiqueUserPrompt — NOT the truncated prdText/archText actually sent to the LLM), PLUS
 * the requested model (adapter.defaultModel — NEVER response.model, T12: OpenAI can silently
 ...
 *
 * SECURITY (EXEC-phase evidence + re-verification, SEC-HIGH-1, live-reachable: 11/253
 * eva_architecture_plans already exceed MAX_CRITIQUE_ANALYSIS_CHARS, 50 SDs affected): hashing
 * only the SENT (truncated) text — even with a charsTotal length appended, an earlier version of
 * this fix — leaves any LENGTH-PRESERVING edit made entirely beyond the truncation boundary
 * (swapping a date, a same-length identifier, a threshold value) hashing identically to the
 * original, letting an audited override keep binding, or the FR-4 cache return a hit, against
 * content that was never actually re-reviewed. Hashing the FULL content closes the defect class
 * completely rather than narrowing it to length-changing edits. This is not the added cost it
 * first appears: buildBudgetedPrdText already fully materializes all 5 sections to compute its
 * own fast-path total-size check, and archRaw is already the full arch string before its own
 * substring cut — nothing extra is read or serialized to hash it (measured live: 0.425ms/call
 * over 284,492 chars). MAX_CRITIQUE_ANALYSIS_CHARS bounds LLM TOKEN spend, not hashing cost;
 * hashing the full content doesn't spend what the budget exists to save.
 *
 * SECURITY (2nd re-verification, LOW): since sent text is no longer hashed, content_hash is now
 * invariant to MAX_CRITIQUE_ANALYSIS_CHARS/SECTION_BUDGETS themselves — the harmful direction is a
 * future budget INCREASE landing inside a live 14-day override window, making content that was
 * genuinely invisible when a human recorded an override suddenly visible (and any new BLOCK
 * findings it produces auto-downgraded by an override that never actually saw them). Folded the
 * budget constants in alongside model, extending the SAME "critic version" role FR-4 already gives
 * model — a budget change is as much a critic-version change as a model upgrade.
 */
export function computeContentHash({ prdRawText, archRawText, archLoadStatus, model }) {
  const input = JSON.stringify({
    prdRawText: prdRawText || '',
    archRawText: archRawText || '',
    archLoadStatus: archLoadStatus || 'ok',
    model: model || null,
    maxCritiqueAnalysisChars: MAX_CRITIQUE_ANALYSIS_CHARS,
...
export async function critiquePlanProposal(input, deps = {}) {
  ...
  const { prompt: userPrompt, truncated, prdRawText, archRawText } = buildCritiqueUserPrompt({ prdContent, archContent, sdContext });
  // SEC-HIGH-1 (re-verification): hash the FULL pre-truncation content, not the sent/truncated
  // pair — see computeContentHash's docstring for why a length-only guard was insufficient.
  const contentHash = computeContentHash({ prdRawText, archRawText, archLoadStatus, model: adapter.defaultModel });
...
Severity rules — anchored to DECISION-AUTHORITY COST, not to whether a gap exists at all:
- "block": ONLY a finding that PLAN/EXEC cannot resolve unilaterally while building — a genuine
  contradiction between two claims in the SD's own spine (title/scope/objectives disagree about
  what is being built), or a missing rollback/safety plan for a step that is irreversible or
  destructive (data deletion, a one-way migration, an external side-effect that cannot be undone).
  A missing or thin acceptance criterion that PLAN/EXEC could simply ADD without further guidance
  is NOT "block" — it is "warn". Ask yourself: "would a competent PLAN/EXEC agent need to escalate
  this to LEAD or the chairman to proceed, or could they just fix it and keep going?" Only the
  first case is "block". A "block"-severity finding's category MUST be "contradiction" or
  "missing_rollback" — those are the only two categories that meet the decision-authority bar
  above. If a genuine escalation-worthy gap does not fit either label, it is not actually "block":
  reclassify it as "warn" rather than reaching for "other". Do not use category "other" for a
  "block"-severity finding.
- "warn": gaps that will cause rework but not immediate failure — including most missing or
  under-specified acceptance criteria, scope ambiguity a reader could resolve by asking one
  clarifying question, or a criterion that's present but vague.
...
- Do not inflate "warn"-level gaps to "block" just to be thorough — a real PRD almost always has
  SOME addressable gap; reserve "block" for the rare case that genuinely cannot proceed without
  a decision above PLAN/EXEC's own authority`;
}
...
/**
 * FR-2: section-aware budget replacing the single whole-blob substring cut. The six small,
 ...
 *
 * QF-20260816-546: system_architecture and implementation_approach were previously omitted from
 * prdSections entirely (0 chars, not merely truncated) despite the SELECT at the call site already
 * fetching both columns — the critique never saw architecture or approach content at all.
 *
 * Accepts either the structured {executive_summary, functional_requirements, acceptance_criteria,
 * test_scenarios, risks, system_architecture, implementation_approach} object (enables
 * section-aware budgeting) or a plain string (legacy/back-compat — falls back to a flat cut
 * against MAX_CRITIQUE_ANALYSIS_CHARS, old behavior with
 * the new, larger budget).
 */
function joinSections({ es, fr, ac, ts, rk, sa, ia }) {
  return [
    `EXECUTIVE_SUMMARY:\n${es}`,
...
function buildBudgetedPrdText(prdSections) {
  if (typeof prdSections === 'string' || prdSections == null) {
    ...
    return { text: raw.substring(0, MAX_CRITIQUE_ANALYSIS_CHARS), truncated: charsRead < charsTotal, charsRead, charsTotal, rawText: raw };
  }
  ...
  const saRaw = serializeSection(prdSections.system_architecture);
  const iaRaw = serializeSection(prdSections.implementation_approach);
  const frRawForTotal = serializeSection(prdSections.functional_requirements);
  const rawTotal = esRaw.length + acRaw.length + tsRaw.length + rkRaw.length + saRaw.length + iaRaw.length + frRawForTotal.length;
  // SECURITY (EXEC-phase re-verification, SEC-HIGH-1 residual): the full, untruncated
  // concatenation is ALREADY fully materialized right here to compute rawTotal for the fast-path
  // check above — reusing it as the content_hash input costs nothing extra to serialize (measured
  // live: 0.425ms/call over 284,492 chars) and closes the defect class completely, not just the
  // length-changing subset a charsTotal-only hash would leave open (a length-preserving edit
  // entirely beyond the truncation boundary — e.g. swapping a date or a same-length identifier —
  // would still collide under charsTotal alone). The MAX_CRITIQUE_ANALYSIS_CHARS budget bounds
  // LLM token spend, not hashing cost; hashing the full content doesn't spend what the budget
  // exists to save.
  const rawText = joinSections({ es: esRaw, fr: frRawForTotal, ac: acRaw, ts: tsRaw, rk: rkRaw, sa: saRaw, ia: iaRaw });
  ...
  const sa = budgetField(prdSections.system_architecture, SECTION_BUDGETS.system_architecture);
  const ia = budgetField(prdSections.implementation_approach, SECTION_BUDGETS.implementation_approach);

  const fixedRead = es.charsRead + ac.charsRead + ts.charsRead + rk.charsRead + sa.charsRead + ia.charsRead;
  ...
  return {
    text: joinSections({ es: es.text, fr: fr.text, ac: ac.text, ts: ts.text, rk: rk.text, sa: sa.text, ia: ia.text }),
    truncated: es.truncated || ac.truncated || ts.truncated || rk.truncated || sa.truncated || ia.truncated || fr.truncated,
    charsRead: es.charsRead + fr.charsRead + ac.charsRead + ts.charsRead + rk.charsRead + sa.charsRead + ia.charsRead,
    charsTotal: es.charsTotal + fr.charsTotal + ac.charsTotal + ts.charsTotal + rk.charsTotal + sa.charsTotal + ia.charsTotal,
    rawText, // the FULL, untruncated concatenation — computed once above, reused for content_hash
  };
}

export function buildCritiqueUserPrompt({ prdContent, archContent, sdContext }) {
...
Critique this plan adversarially. Return JSON only.`;
  ...
  // prdText/archText: the exact post-truncation/chunking text actually SENT to the LLM -- the
  // SAME values embedded in the prompt above, exposed separately so the caller never has to
  // re-derive or re-parse them out of the assembled prompt string.
  // prdRawText/archRawText: the FULL, untruncated content (SEC-HIGH-1 re-verification residual —
  // computeContentHash hashes THESE, not the sent/truncated pair, so an edit made entirely beyond
  // the truncation boundary always changes the hash, not only when it happens to change length).
  return { prompt, truncated, prdText: prdBudget.text, archText, prdRawText: prdBudget.rawText, archRawText: archRaw };
}
...
export const _internal = {
  ...
  // SD-LEO-INFRA-MINUS-GATE-SSOT-001 (FR-1/TR-7): ALL_GATES/KILL_GATES/PROMOTION_GATES are no
  // longer static constants — use the exported getAllGates(supabase)/getKillGates(supabase)/
  // getPromotionGates(supabase) accessors above instead.
  getAllGates,
  getKillGates,
  getPromotionGates,
  MAX_ANALYSIS_CHARS,
