 ...
 * that rotation change the hash on byte-identical content, orphaning every existing override),
 * archLoadStatus (PT-8 — so a transient eva_architecture_plans read failure never hashes the same
 * as a genuine "no arch plan" absence), AND prdCharsTotal/archCharsTotal (SECURITY EXEC-phase
 * finding SEC-HIGH-1, live-reachable: 11/253 eva_architecture_plans already exceed
 * MAX_CRITIQUE_ANALYSIS_CHARS, 50 SDs affected). Without the real pre-truncation length, any edit
 * made ENTIRELY beyond the truncation boundary — extending an over-budget section, growing an
 * existing FR's body past the cut, editing arch content past 64,000 chars (archText has NO
 * integrity marker at all, unlike prdText's [FR ids present] guard) — leaves the hashed PREFIX
 * byte-identical while the actual reviewed content differs. That would let an audited override
 * keep binding for the full 14-day lookback, and the FR-4 cache return a hit, against content
 * that was never actually re-reviewed — the exact defect class SECURITY MEDIUM-1 (e77d1c4b)
 * already fixed once, recurring one layer below where FR-4's own fix landed. Same PT-8 principle,
 * applied consistently: a truncated prefix must never hash the same as a DIFFERENT truncated
 * prefix just because the visible portion happens to match. This does not defend against a
 * length-preserving edit made entirely past the truncation boundary (which is the true value of
 * hashing over full content — not attempted here, since that requires reading the full content
 * regardless of budget, the exact cost the budget exists to avoid); that residual is accepted and
 * documented, not silently assumed away.
 * Becomes findActiveOverride's binding predicate.
 */
function computeContentHash({ prdText, archText, archLoadStatus, model, prdCharsTotal, archCharsTotal }) {
  const input = JSON.stringify({
    prdText: prdText || '',
    archText: archText || '',
    prdCharsTotal: Number.isFinite(prdCharsTotal) ? prdCharsTotal : null,
    archCharsTotal: Number.isFinite(archCharsTotal) ? archCharsTotal : null,
...
export async function critiquePlanProposal(input, deps = {}) {
  ...
  // SEC-HIGH-1: real pre-truncation lengths are hash inputs (see computeContentHash's docstring)
  // so an edit made entirely beyond the truncation boundary changes the hash even when the
  // truncated prefix sent to the LLM happens to stay byte-identical.
  const contentHash = computeContentHash({
    prdText, archText, archLoadStatus, model: adapter.defaultModel,
    prdCharsTotal: truncated.prd.charsTotal, archCharsTotal: truncated.arch.charsTotal,
  });
