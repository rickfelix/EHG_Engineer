...
import { registerDefaultSchemas, getSchemaCount, initPersistence, loadSchemasFromDB, syncSchemasToDB } from './event-schema-registry.js';
import { registerHookObserver, getHookObserverCount, SUBSCRIBER_ERROR_EVENT } from './vision-events.js';
...
export async function initializeEventBus(supabase) {
  ...
  syncSchemasToDB(supabase).catch((err) => {
    logger.warn('Schema sync failed', { error: err.message });
  });
  ...
  if (getHookObserverCount() === 0) {
    registerHookObserver(function governanceObserver(eventType, payload) {
      // PAT-LES-77572983b741: a subscriber failure arrives here as SUBSCRIBER_ERROR_EVENT
      // (see vision-events.js's notifySubscriberError) — surfaced at error level, distinct from
      // the routine per-publish log below, so it is visible without active log monitoring.
      if (eventType === SUBSCRIBER_ERROR_EVENT) {
      ...
      logger.info('Vision event published', { eventType, sdKey: payload?.sdKey || payload?.scoreId || 'n/a' });
    });
  }
