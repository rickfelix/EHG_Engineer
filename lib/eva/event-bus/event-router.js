...
export async function checkEscalationThreshold(supabase, options = {}) {
  ...
  // Table sub_agent_queue does not exist yet
  const staleItems = [];
  const error = null;
  ...
  for (const item of staleItems) {
    try {
      ...
      if (processResult.success) {
        // Table sub_agent_queue does not exist yet — skip status update
...
async function enqueueToPriorityQueue(supabase, event) {
  // Table sub_agent_queue does not exist yet — return mock success
  const data = null;
  const error = { message: 'sub_agent_queue table not yet created' };
  if (true) {
    return { success: false, status: 'queue_unavailable' };
  }
  // Dead code below preserved for future table creation
  const _unused = {
