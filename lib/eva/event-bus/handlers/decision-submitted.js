...
const log = createLogger('DecisionSubmitted');
...
export async function handleDecisionSubmitted(payload, context) {
  ...
  if (venture.status === 'blocked' || venture.status === 'paused' || venture.status === 'pending_review') {
    ...
    if (decision.status === 'approved' || decision.status === 'rejected') {
      ...
      log.info('Venture unblocked', { ventureId, previousStatus: venture.status, newStatus, decisionId, decisionStatus: decision.status });
      return { outcome: 'unblocked', ventureId, decisionId, previousStatus: venture.status, newStatus };
    }

    // Decision is still pending - no change
    log.info('Decision still pending', { decisionId, ventureId, status: venture.status });
  ...
  log.info('Venture not blocked, no unblocking needed', { ventureId, status: venture.status });
