...
const log = createLogger('GateEvaluated');
...
async function handleProceed(supabase, ventureId, gateId) {
  ...
  if (!nextStage) {
    log.info('Gate passed but no next stage', { gateId, ventureId });
  ...
  log.info('Gate passed - advancing venture', { gateId, ventureId, nextStage: nextStage.name });
...
async function handleBlock(supabase, ventureId, gateId, reason) {
  ...
  log.warn('Gate blocked venture', { gateId, ventureId, reason: reason || 'no reason' });
...
async function handleKill(supabase, ventureId, gateId) {
  ...
  log.warn('Gate KILLED venture', { gateId, ventureId });
