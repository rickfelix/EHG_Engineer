...
function notifySubscriberError(eventType, handlerName, error, originalPayload) {
  ...
  for (const observer of _hookObservers) {
    try {
      ...
      if (result && typeof result.catch === 'function') {
        result.catch((obsErr) => {
          console.error(`[VisionBus] Hook observer error while reporting subscriber error: ${obsErr.message}`);
        });
      }
    } catch (obsErr) {
      console.error(`[VisionBus] Hook observer sync error while reporting subscriber error: ${obsErr.message}`);
    }
  }
}
