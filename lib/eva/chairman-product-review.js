...
const GUIDED_TOUR_STOPS = [
  ...
  { key: 'core_action', label: 'The core thing this product does', artifactType: null, screenId: 'screen-5' },
...
export function buildGuidedTour(artifactsByType, reconciliationByScreen = {}) {
  return GUIDED_TOUR_STOPS.map(({ label, artifactType, screenId }) => {
    const artifact = artifactType ? artifactsByType[artifactType] : null;
    const note = extractHumanText(artifact)
      || (screenId ? describeReconciliationRow(reconciliationByScreen[screenId]) : null)
