 ...
 *
 * SD-LEO-FIX-TRIAGE-THREE-FALSE-001: `schedulerObserveOnly` adds a THIRD, purely informational
 * signal — whether EvaMasterScheduler's own heartbeat reports it is running in deliberate
 * observe-only mode (env EVA_SCHEDULER_OBSERVE_ONLY). This does NOT change `healthy` or
 * `lineSilent` — a silent line is still worth a human seeing, whether deliberate or not. It
 * only adds `observeOnlySilence` + a reason string so a human (or future dedup tooling)
 * triaging a red run can immediately distinguish "expected, scheduler intentionally not
 * dispatching" from "investigate a crash" — the exact ambiguity a 2026-09-01 false-completion
 * investigation of SD-LEO-FIX-TRIAGE-THREE-FAILING-001 could not resolve without touching the
 * live scheduler. Unknown/unreadable `schedulerObserveOnly` (null/undefined) fail-quiets to
 * `observeOnlySilence: false`, matching this module's existing "cannot tell != alarm" stance.
 *
 * QF-20260901-840: `observeOnlySilence` STOPPED being purely informational -- a silent line the
 * scheduler is deliberately not dispatching to is expected, not a stall, so it must not fail
 * `healthy` either. `lineSilent` and its reason string are still reported unchanged (the
 * silence duration is a real fact worth tracking); only the alarm verdict for THIS cause is
 * suppressed. `workerAbsent` is untouched -- observe-only mode is a scheduler-dispatch decision
 * and says nothing about whether the worker process itself is alive.
 *
 * @returns {{healthy: boolean, workerAbsent: boolean, lineSilent: boolean, observeOnlySilence: boolean, reasons: string[]}}
 */
export function classifyStageLine({ pid, pidAlive, lastExecutionAt, now, silentHours, schedulerObserveOnly } = {}) {
  const workerAbsent = isWorkerAbsent({ pid, pidAlive });
  const lineSilent = isLineSilent({ lastExecutionAt, now, silentHours });
  const observeOnlySilence = lineSilent && schedulerObserveOnly === true;
  const reasons = [];
  ...
  if (observeOnlySilence) {
    reasons.push('scheduler running in deliberate observe-only mode (EVA_SCHEDULER_OBSERVE_ONLY) -- silence is expected, not a stall');
  }
  return { healthy: !workerAbsent && (!lineSilent || observeOnlySilence), workerAbsent, lineSilent, observeOnlySilence, reasons };
}
