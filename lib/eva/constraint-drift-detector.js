...
function compareCategories(baseline, currentCategories, currentStage) {
  ...
  for (const category of allKeys) {
    ...
    // Final stage must always check vision if present in baseline.
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001: was stage 26 (comment already said "25" and
    // disagreed with the code even before this SD); now 27, the 2026-08-28 renumbering's
    // final stage.
    if (currentStage === 27 && category === 'vision') {
      if (!baselineVal) {
        findings.push({
          category: 'vision',
          baselineValue: null,
