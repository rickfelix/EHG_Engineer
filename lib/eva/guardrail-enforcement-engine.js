...
const DEFAULT_ENFORCEMENT_POLICY = Object.freeze({
  ...
  'GR-STAKEHOLDER-RESPONSE': 'blocking',  // V02: mandatory stakeholder acknowledgment gate
});
...
export async function getEnforcementSummary(supabase, options = {}) {
  ...
  try {
    ...
    // Get enforcement events. Paginated (FR-6 batch 7): summary counts must see the
    // whole window. Page errors throw into the outer catch (same error shape).
    const events = await fetchAllPaginated(() => supabase
      .from('eva_event_log')
      .select('id, payload, created_at') // schema-lint-disable-line — pre-existing reference on main (eva_event_log.payload was in this select before the FR-6 rewrap); moved into diff by FR-6 batch 7
    ...
    // Get override decisions (paginated; prior policy was warn-and-continue with [],
    // preserved via .catch)
    const overrideList = await fetchAllPaginated(() => supabase
      .from('chairman_decisions')
      ...
      .order('id', { ascending: true }))
      .catch((e) => {
        logger.warn(`[GuardrailEnforcement] Override query failed: ${e.message}`);
        return [];
      });
