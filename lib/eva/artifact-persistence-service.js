...
export async function recordGateAttempt(supabase, opts) {
  ...
  if (openError) {
    throw new Error(`[artifact-persistence-service] recordGateAttempt open failed: ${openError.message}`);
  }
  ...
  if (finalizeError) {
    throw new Error(`[artifact-persistence-service] recordGateAttempt finalize failed for attempt ${attemptId}: ${finalizeError.message}`);
  }
  if (finalized !== true) {
