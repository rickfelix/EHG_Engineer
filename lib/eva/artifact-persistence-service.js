...
export async function writeArtifact(supabase, opts) {
  ...
  // Use upsert for is_current=true rows to prevent duplicates on stage re-runs.
  // The partial unique index idx_venture_artifacts_idempotent enforces
  // (venture_id, lifecycle_stage, artifact_type) WHERE is_current = true.
  const useUpsert = isCurrent;
  let { data, error } = useUpsert
    ? await supabase
        .from('venture_artifacts')
        .upsert(row, { onConflict: 'venture_id,lifecycle_stage,artifact_type' })
        .select('id')
        .single()
    : await supabase
        .from('venture_artifacts')
