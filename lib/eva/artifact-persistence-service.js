...
const logger = createLogger('ArtifactPersistenceService');
...
/**
 * SD-LEO-INFRA-MINUS-PATH-INTEGRITY-001 (FR-2, TR-3): decision verbs that count as genuine
 * advance-intent for checkGateDebt() below. A single named, exported source of truth so FR-3's
 * eva-decisions.js alignment work and any future audit reference the SAME list.
 *   - 'proceed', 'approve', 'override': the three verbs fn_chairman_decision_value(<type>,
 *     'approved') can emit (verified live, 2026-08-23).
 *   - 'override': the chairman's documented, auditable build-out-forcing escape hatch --
 *     see advanceStage()'s GATE_BLOCKED remediation text below. MUST remain allow-listed.
 *   - 'go': LEGACY-COMPAT only -- 45 live approved rows, never emitted by the canonical writer,
 *     included so the pre-existing legitimate case does not regress.
 *   - 'conditional_pass': REGRESSION EXEC-TO-PLAN finding (2026-08-23, C1) -- the original
 *     blast-radius measurement was a LIVE-ROW CENSUS (287 approved rows), which cannot see a
 *     verb no venture has hit yet even though production code actively writes it.
 ...
 *     decision:'conditional_pass'} -- a first-class chairman approval, genuinely advance-intent,
 *     that would otherwise silently stop resolving gate debt the moment any venture hits it.
 * NOT allow-listed (measured live, 2026-08-23, n=287 approved rows): 'pending', 'sunset',
 * 'complete', 'continue' -- an approved decision carrying one of these must NOT resolve gate debt.
 * Also NOT allow-listed (REGRESSION finding, code-census not live-row): 'advisory' --
 * lib/eva/chairman-decision-watcher.js:650 writes {status:'approved', decision:'advisory',
 * blocking:false} for a non-binding notification checkpoint. Deliberately EXCLUDED: an advisory
 * FYI stamped as approved for bookkeeping is not a deliberate act of clearing a gate, and
 * historically (pre-FR-2) exactly this kind of row could accidentally satisfy an unrelated
 * blocking gate at the same (venture,stage) -- the precise defect class FR-2 exists to close.
 */
export const ADVANCE_INTENT_VERBS = ['proceed', 'approve', 'override', 'go', 'conditional_pass'];

/**
 * SD-FDBK-ENH-APPLIES-FALSE-DECLINATION-001: detect the {applies:false, satisfied:true, reason}
 * declination shape (a gate/check reporting "not applicable here, vacuously satisfied") mirrored
 * verbatim in lib/eva/uat-robustness-gate.js:92-94 and lib/eva/synthetic-actor-guard.js:181.
 * A declination payload is NOT a real, validated artifact — writeArtifact's hardcoded
 * qualityScore=70/validationStatus='validated' defaults previously mislabeled it as one.
 * @param {*} payload - parsed artifactData or JSON-parsed content
 * @returns {boolean}
 */
export function isDeclinationPayload(payload) {
  return !!(
    payload &&
...
/**
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G: generalizes the chairman-ratified
 * gate-evidence-provenance principle (ratification 6c263823) to venture_artifacts.
 * Rows written before this timestamp are graded leniently by provenance-aware readers
 * (0 of 7972 live venture_artifacts rows carry any provenance field as of the SD's LEAD
 * phase, including all 6772 of AltifyAI's) -- mirrors PROVENANCE_CUTOVER_AT
 * (lib/sub-agent-executor/evidence-provenance.js) and DELIVERABLES_PROVENANCE_CUTOVER
 * (scripts/modules/handoff/validation/semantic-gate-utils.js). No backfill: historical
 * NULLs stand, per the policy already documented below at writeArtifactBatch.
 *
 * VALIDATION (PLAN-VERIFY) caught a real race: an earlier same-day value was already in the
 * past relative to this unmerged branch, so a row written by main's still-unstamped code
 * between that timestamp and merge would become permanently, unfixably ABSENT post-cutover.
 * Set comfortably past the expected merge window instead of pinned to authoring time.
 */
export const VENTURE_ARTIFACT_PROVENANCE_CUTOVER_AT = '2026-09-21T00:00:00.000Z';

/**
 * Builds the metadata.machine_provenance stamp for a venture_artifacts write.
 *
 * Hashes content ?? artifactData, NOT content alone -- confirmed live that content is
 * NULL on a majority of is_current rows for the visual/distribution/growth-playbook
 * artifact_type families (their payloads live in artifact_data), so a content-only hash
 * would constant-hash null across every venture. hash_source records which field was
 * actually hashed so a verifier can reproduce the exact hash without guessing.
 *
 * producer defaults to `source` (already a producing-module label, e.g.
 * 'stage-23-dedicated-venture-uat') rather than the lifecycle_stage number, so a
 * stage-identity offset in a caller (e.g. stage-21-visual-assets.js recording
 * lifecycle_stage:22) cannot corrupt the recorded producer identity.
 *
 * @param {Object} params
 * @param {string} params.source
 * @param {string|null} [params.producer]
 * @param {string|null} [params.runId]
 * @param {*} params.content
 * @param {*} params.artifactData
 * @returns {{producer: string, run_id: string, content_hash: string, hash_source: 'content'|'artifact_data'}}
 */
export function buildMachineProvenance({ source, producer = null, runId = null, content, artifactData }) {
  const hashSource = content != null ? 'content' : 'artifact_data';
  const hashInput = content ?? artifactData;
  return {
 ...
 * @param {string|null} [opts.producer] - Producing module identity for the provenance stamp
 *   (defaults to `source`); SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G
 * @param {string|null} [opts.runId] - Run identifier for the provenance stamp (defaults to a
 *   fresh UUID per write); SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G
 * @returns {Promise<string>} Inserted artifact ID
 *
 * Every write path (dedup-UPDATE, fresh-INSERT, unique-violation fallback) stamps
 * metadata.machine_provenance = {producer, run_id, content_hash, hash_source} — see
 * buildMachineProvenance() above. Readers grading against VENTURE_ARTIFACT_PROVENANCE_CUTOVER_AT
 * treat a pre-cutover row with no stamp leniently, never ABSENT.
 */
export async function writeArtifact(supabase, opts) {
  ...
  let { qualityScore = 70, validationStatus = 'validated' } = opts;

  // SD-FDBK-ENH-APPLIES-FALSE-DECLINATION-001: a declination payload ({applies:false,
  // satisfied:true, reason}) is not a validated artifact — override the defaults BEFORE any
  // of the three write paths below (dedup-UPDATE, fresh-INSERT, unique-violation fallback)
  // read qualityScore/validationStatus, so all three are covered by one check.
  const declinationCandidate = artifactData ?? (content ? tryParse(content) : null);
  if (isDeclinationPayload(declinationCandidate)) {
    qualityScore = 0;
  ...
  if (isCurrent && !skipDedup) {
    // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-1): widened from 'id' to 'id, metadata' so
    // the provenance stamp below can be SPREAD-MERGED under existing metadata (screenId, the
    // unique-index discriminator; version/content_hash from artifact-versioning.js; future
    // editorial-provenance keys from SD-LEO-INFRA-VENTURE-FACTORY-HANDOFF-001-C) instead of
    // being silently dropped (metadata was never in this UPDATE's payload before this fix) or
    // clobbered by a wholesale overwrite.
    ...
    if (existing) {
      ...
      const machineProvenance = buildMachineProvenance({
        source, producer, runId, content: resolvedContent, artifactData: resolvedArtifactData,
      });
  ...
  // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-1, FR-3): unconditional now (was `if (metadata)
  // row.metadata = metadata;`), so a caller passing no metadata still receives the provenance
  // stamp -- a conditional assignment would silently skip stamping most callers.
  row.metadata = {
    ...(metadata || {}),
    machine_provenance: buildMachineProvenance({
      source, producer, runId, content: resolvedContent, artifactData: resolvedArtifactData,
    }),
  };
  ...
  if (isUniqueViolation) {
    ...
    // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-1): widened from 'id' to 'id, metadata' --
    // this fallback previously did `metadata: row.metadata` wholesale after a bare 'id' SELECT,
    // a pre-existing clobber bug (fixed here, not just for the new provenance stamp).
    let query = supabase
      .from('venture_artifacts')
      .select('id, metadata')
      .eq('venture_id', ventureId)
    ...
    if (existing) {
      const mergedMetadata = { ...(existing.metadata || {}), ...(row.metadata || {}) };
      await supabase.from('venture_artifacts').update({
        title: row.title, artifact_data: row.artifact_data, content: row.content,
        metadata: mergedMetadata, source: row.source, quality_score: row.quality_score,
...
export async function writeArtifactBatch(supabase, ventureId, lifecycleStage, artifacts, idempotencyKey = null, { visionKey = null, planKey = null } = {}) {
  ...
  for (const art of artifacts) {
    const id = await writeArtifact(supabase, {
      ...
      // SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G (FR-6): art.metadata/art.runId were previously
      // never read here -- silently dropped, even after eva-orchestrator.js's own typed_artifacts
      // branch was fixed to carry them this far. A stage template's declared metadata (e.g.
      // stage-23-dedicated-venture-uat.js's uat_test_run_id hash-link) now actually reaches the
      // written row instead of being lost one hop from the DB.
...
/**
 * Record a gate evaluation as a durable, immutable-once-finalized ATTEMPT on
 * eva_stage_gate_attempts (SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001, T-minus P1 evidence layer).
 * Dual-writes alongside recordGateResult() -- this function NEVER touches eva_stage_gate_results
 * and its failure must never suppress the primary write (call sites call both independently).
 *
 * NOTE ON "in-flight" VISIBILITY: at every currently-known call site, the verdict (`passed`) is
 * already computed by the time this is called (evaluation runs to completion first, then results
 * are persisted) -- so this opens and finalizes the attempt in the same call, back-to-back. A
 * crash DURING evaluation (before this function is ever reached) is not represented as an
 * in-flight row here; that guarantee is explicitly descoped for these call sites per the PRD's
 * FR-4 correction (TESTING F1), rather than silently claimed and left unmet. What IS delivered:
 * durable, attributable, immutable-once-finalized attempt history, with a genuine attempt_number
 * per retry/re-evaluation cycle.
 *
 * A failed write here throws -- callers MUST log any caught error loudly (never a bare
 * logger.warn), per TESTING F6: a silently-dropped attempt write is the exact failure class this
 ...
 * @param {string} opts.gateType - code-level gate type (same vocabulary as recordGateResult)
 * @param {boolean} opts.passed
 * @param {string} [opts.resolvedOutcome] - one of the 7 canonical terms; defaults to
 *   passed ? 'machine_pass' : 'machine_fail'
 * @param {string|null} [opts.reasoning]
 * @param {Object|null} [opts.metadata] - kept DISTINCT from reasoning, never clobbering it
 *   (the defect this table exists to avoid inheriting from recordGateResult's notes column)
 * @param {Object|null} [opts.criteria]
 * @param {string|null} [opts.evaluatedBy]
 * @returns {Promise<string>} the new attempt_id
 */
// SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 (VALIDATION VAL-B2): the eva_stage_gate_attempts
// migration is chairman-gated. CORRECTION (SD-LEO-INFRA-ALTIFYAI-INSTRUMENTATION-RETROFIT-001,
// live-verified 2026-08-24): the migration IS applied -- a direct RPC probe against
// open_eva_gate_attempt returned a foreign-key violation (23503), not PGRST202, and the table
// holds 1000+ live rows. The comment below was stale. describeGateAttemptError still recognizes
// PGRST202 defensively (a re-deployment could theoretically regress this), but it should not be
// read as evidence the migration is currently unapplied.
function describeGateAttemptError(error) {
  if (error?.code === 'PGRST202') {
    return `eva_stage_gate_attempts migration not yet applied (see database/chairman-gated/README.md) — ${error.message}`;
  }
  return error?.message;
}

export async function recordGateAttempt(supabase, opts) {
  const {
    ventureId,
    stageNumber,
    gateType,
    passed,
    resolvedOutcome = passed ? 'machine_pass' : 'machine_fail',
    reasoning = null,
    metadata = null,
    criteria = null,
    evaluatedBy = null,
  ...
  const { data: opened, error: openError } = await supabase.rpc('open_eva_gate_attempt', {
    p_venture_id: ventureId,
    p_stage_number: stageNumber,
    p_gate_type: mappedGateType,
    p_evaluator: evaluatedBy,
  });
  if (openError) {
    throw new Error(`[artifact-persistence-service] recordGateAttempt open failed: ${describeGateAttemptError(openError)}`);
  }
  const attemptId = Array.isArray(opened) ? opened[0]?.attempt_id : opened?.attempt_id;
  if (!attemptId) {
    throw new Error('[artifact-persistence-service] recordGateAttempt open returned no attempt_id');
  }

  const { data: finalized, error: finalizeError } = await supabase.rpc('finalize_eva_gate_attempt', {
    p_attempt_id: attemptId,
    p_resolved_outcome: resolvedOutcome,
    p_passed: resolvedOutcome === 'machine_pass' ? true : resolvedOutcome === 'machine_fail' ? false : null,
    p_reasoning: reasoning,
    p_metadata: metadata && typeof metadata === 'object' ? metadata : null,
    p_gate_criteria: criteria && typeof criteria === 'object' ? criteria : null,
  });
  if (finalizeError) {
    throw new Error(`[artifact-persistence-service] recordGateAttempt finalize failed for attempt ${attemptId}: ${describeGateAttemptError(finalizeError)}`);
  }
  if (finalized !== true) {
    throw new Error(`[artifact-persistence-service] recordGateAttempt finalize did not update attempt ${attemptId} (already finalized or missing)`);
  }
...
export async function recordGateOverride(supabase, opts) {
  ...
  // SD-LEO-INFRA-STAGE-GATE-RETRY-001 (FR-3): idempotent short-circuit. Every caller of this
  // function (the P0 UNIVERSAL pre-execution guard in stage-execution-worker.js) re-invokes it
  // unconditionally on EVERY poll cycle for a venture sitting on an already-approved decision --
  // before this fix that meant a fresh eva_stage_gate_attempts row every ~30s forever (ApexNiche
  // S21: 1900+ rows for one decision). A genuinely NEW decision_id for the same gate still falls
  // through and is recorded normally.
  //
  // Gated on attempt_recorded=true, not merely decision_id matching (SECURITY finding SEC-2,
  // evidence 7b1758b7-fb56-45bc-8bbf-57402c8b20cf): gate_criteria.override is written FIRST,
  // recordGateAttempt below SECOND. If decision_id alone were the short-circuit key, a transient
  // recordGateAttempt failure would permanently suppress that decision's audit-trail row on every
  // future poll (the pre-fix code accidentally self-healed this by retrying both writes every
  // poll). attempt_recorded is only set true after recordGateAttempt actually succeeds, so a
  // failed attempt write is retried on the next poll instead of being silently lost forever.
  const alreadyFullyRecorded = existing.gate_criteria?.override?.decision_id === override.decision_id
    && existing.gate_criteria?.override?.attempt_recorded === true;
  if (alreadyFullyRecorded) {
    return existing.id;
  }

  const merged = { ...(existing.gate_criteria || {}), override: { ...override, attempt_recorded: false } };
  ...
  // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 (T-minus P1, FR-6 partial): a chairman override is a
  // resolved_outcome='override' attempt in its own right, distinct from the machine_pass/
  // machine_fail attempts recordGateAttempt already writes for automated evaluations. This is a
  // fire-and-forget side effect -- the return value stays the pre-existing plain gate-row id
  // (tests/unit/eva/kill-gate-evidence.test.js pins that contract, and the one production caller,
  // stage-execution-worker.js:852, already discards the return value entirely). Stamping the
  // resulting attempt_id into chairman_decisions.context / venture_artifacts.metadata is NOT done
  // here because neither table is written by this function today (recordGateOverride only ever
  // touches eva_stage_gate_results.gate_criteria; the chairman_decisions INSERT for
  // override.decision_id happens in a different, not-yet-located call path) -- documented as a
  // follow-up in the FR-8 reader census rather than silently left undone.
  try {
    await recordGateAttempt(supabase, {
      ...
      gateType,
      passed: null,
      resolvedOutcome: 'override',
      reasoning: override.rationale || null,
      metadata: { decision_id: override.decision_id, decided_by: override.decided_by || null, at: override.at || null },
      criteria: merged,
      evaluatedBy: override.decided_by || 'chairman-override',
    });
    // Stamp attempt_recorded=true ONLY after the attempt write above actually succeeded, so a
    // future poll's idempotency check (above) does not short-circuit on a decision whose
    // audit-trail row was never durably written.
    const { error: stampErr } = await supabase
      .from('eva_stage_gate_results')
      .update({ gate_criteria: { ...merged, override: { ...merged.override, attempt_recorded: true } } })
      .eq('id', existing.id);
    if (stampErr) {
      console.error(`[artifact-persistence-service] recordGateOverride: attempt_recorded stamp FAILED (non-fatal, will retry next poll): ${stampErr.message}`);
    }
  } catch (attemptErr) {
    console.error(`[artifact-persistence-service] recordGateOverride: attempt-evidence write FAILED (venture ${ventureId}, S${stageNumber}/${gateType}, decision ${override.decision_id}): ${attemptErr.message}`);
  }
 ...
 * (no approved chairman_decisions row for the venture+stage carrying an
 * advance-intent verb -- see ADVANCE_INTENT_VERBS above, SD-LEO-INFRA-MINUS-
 * PATH-INTEGRITY-001 FR-2. Resolves on ANY approved row with an allow-listed
 * verb, not "the latest" -- the query below has no ORDER BY. 'override' is the
 * chairman's auditable record of deliberate build-out forcing and remains
 * allow-listed; a non-advance-intent verb like 'sunset' or 'pending' no longer
 * silently resolves debt just by being approved).
 *
 * FR-2 AC#5 (TESTING EXEC-TO-PLAN finding C5): the decision-value filter above is NOT the same
 * thing as true attempt/run-scoping (resolving debt per advancement ATTEMPT rather than per
 * venture+stage forever). That requires eva_stage_gate_attempts, which is CONFIRMED STILL ABSENT
 * live (a real .select().limit(1) returns PGRST205 -- a head:true count on this table returns
 * count=null with NO error and must never be used to prove absence, TR-8). True attempt-scoping
 * is an explicit follow-on blocked on the chairman applying that table's staged migration -- not
 * fixed here, not silently dropped.
 *
...
export async function checkGateDebt(supabase, { ventureId, fromStage }) {
  try {
    ...
    // for this venture+stage carrying an advance-intent verb (decision='override' is
    // the deliberate-forcing record; see ADVANCE_INTENT_VERBS, FR-2).
    const { data: decisions, error: decErr } = await supabase
      ...
      .eq('status', 'approved')
      .in('decision', ADVANCE_INTENT_VERBS)
      .limit(1);
...
export async function advanceStage(supabase, opts) {
  ...
  // never blocks -- surfaces gateResult.provenance_warnings so the field is observable
  // somewhere rather than computed and silently dropped by this caller.
  if (gateResult.provenance_warnings?.length) {
    logger.warn('advanceStage provenance warnings', { ventureId, fromStage, toStage, warnings: gateResult.provenance_warnings });
  }
