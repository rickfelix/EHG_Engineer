...
export class VentureMonitor {
  ...
  async _cronTick() {
    ...
    // Venture decision execution (every 6 hours — post-Stage-25 decisions)
    if (this._isDue(now, 'ventureDecisionExec', this.config.opsCycleIntervalHours)) {
      await this._runCronJob('venture_decision_execution', () => this._ventureDecisionExecution());
    }
  }
  ...
  async _portfolioHealthSweep(correlationId) {
    // Paginated (FR-6 batch 7): portfolio sweeps must see every active venture.
    const ventures = await fetchAllPaginated(() => this.supabase
      .from('eva_ventures')
      .select('id')
      .eq('status', 'active')
      .order('id', { ascending: true }))
      .catch((e) => { throw new Error(`Health sweep query failed: ${e.message}`); });
  ...
  async _opsCycleCheck(correlationId) {
    // Paginated (FR-6 batch 7)
    const ventures = await fetchAllPaginated(() => this.supabase
      .from('eva_ventures')
      .select('id, metadata') // schema-lint-disable-line — pre-existing reference on main (eva_ventures.metadata was in this select before the FR-6 rewrap); moved into diff by FR-6 batch 7
      .eq('status', 'active')
      .order('id', { ascending: true }))
      .catch((e) => { throw new Error(`Ops cycle query failed: ${e.message}`); });
    if (!ventures?.length) return;
    ...
    for (const venture of ventures) {
      ...
      try {
        ...
        await this.supabase
          .from('eva_ventures')
          .update({ metadata: updatedMetadata }) // schema-lint-disable-line — pre-existing reference on main (eva_ventures.metadata update, untouched by FR-6 batch 7)
          .eq('id', venture.id);
      } catch (err) {
  ...
  async _releaseScheduling(correlationId) {
    ...
    const ventures = await fetchAllPaginated(() => this.supabase
      ...
      .eq('status', 'active')
      .order('id', { ascending: true }))
      .catch((e) => { throw new Error(`Release scheduling query failed: ${e.message}`); });
    if (!ventures?.length) return;
  ...
  async _nurseryReEvaluation(correlationId) {
    ...
    const ventures = await fetchAllPaginated(() => this.supabase
      ...
      .eq('status', 'active')
      .order('id', { ascending: true }))
      .catch((e) => { throw new Error(`Nursery re-eval query failed: ${e.message}`); });
    if (!ventures?.length) return;
  ...
  /**
   * Execute pending venture decisions from Stage 27 venture reviews (SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001: updated 26 -> 27).
   * Queries ventures at stage >= 27 with unprocessed advisory decisions,
   * then routes to appropriate handler based on ventureDecision.recommendation.
   */
  async _ventureDecisionExecution(correlationId) {
    // Paginated (FR-6 batch 7): this SELECTS WORK — a capped read would silently
    // strand pending venture decisions.
    const decisions = await fetchAllPaginated(() => this.supabase
      .from('chairman_decisions')
      .select('id, venture_id, brief_data, lifecycle_stage')
      .eq('decision_type', 'advisory')
      .eq('status', 'info')
      .gte('lifecycle_stage', 27)
      .order('created_at', { ascending: true })
      .order('id', { ascending: true }))
      .catch((e) => { throw new Error(`Decision execution query failed: ${e.message}`); });
    if (!decisions?.length) return;

    this.logger.log(`[VentureMonitor] Found ${decisions.length} pending venture decision(s)`);
    const maxRetries = this.config.maxProcessStageRetries;

    for (const decision of decisions) {
      if (this._shutdownRequested) break;

      const recommendation = decision.brief_data?.ventureDecision?.recommendation;
      if (!recommendation) {
        this.logger.warn(`[VentureMonitor] Decision ${decision.id} has no recommendation, skipping`);
        continue;
      }
      ...
      for (let attempt = 1; attempt <= maxRetries && !success; attempt++) {
        try {
          await this._routeDecision(decision.venture_id, recommendation, correlationId);
          success = true;

          // Mark decision as processed
          await this.supabase
            .from('chairman_decisions')
            .update({ status: 'processed', decision: recommendation })
            .eq('id', decision.id);

          await this._logEvent({
            eventType: 'venture_decision_executed',
            triggerSource: 'cron',
            ventureId: decision.venture_id,
            correlationId,
            status: 'succeeded',
            jobName: 'venture_decision_execution',
            metadata: { decision_id: decision.id, recommendation, attempt },
          });
        } catch (err) {
          this.logger.warn(`[VentureMonitor] Decision ${decision.id} attempt ${attempt}/${maxRetries} failed: ${err.message}`);
          if (attempt === maxRetries) {
            await this._logEvent({
              ...
              jobName: 'venture_decision_execution',
              errorMessage: err.message,
              metadata: { decision_id: decision.id, recommendation, attempts: attempt },
            });
          }
        }
      }
  ...
  /**
   * Route a venture decision to the appropriate handler.
   * @param {string} ventureId
   * @param {string} recommendation - One of: continue, pivot, expand, sunset, exit
   * @param {string} correlationId
   */
  async _routeDecision(ventureId, recommendation, correlationId) {
    switch (recommendation) {
      case 'continue':
        // Re-run current stage to advance normally
        await this.processStage(
          { ventureId, options: { parentTraceId: correlationId } },
          { supabase: this.supabase }
        );
        break;
      case 'expand':
        // Treat like continue but flag for resource scaling
        await this.processStage(
          { ventureId, options: { parentTraceId: correlationId, expandMode: true } },
          { supabase: this.supabase }
        );
        break;
      case 'pivot':
      case 'sunset':
      case 'exit':
        // Create a chairman decision requiring manual review
        await this.supabase
          .from('chairman_decisions')
          .insert({
            ...
            decision_type: 'gate',
            blocking: true,
            summary: `Venture review recommends: ${recommendation}. Chairman approval required.`,
            brief_data: { recommendation, requires_manual_review: true },
          });
        this.logger.log(`[VentureMonitor] Created blocking gate for ${recommendation} on venture ${ventureId}`);
        break;
      default:
        this.logger.warn(`[VentureMonitor] Unknown recommendation: ${recommendation}`);
    }
  }
  ...
  _jobNameToKey(jobName) {
    const map = {
      ...
      venture_decision_execution: 'ventureDecisionExec',
    };
