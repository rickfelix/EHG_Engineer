...
import { resolveEvaKeys } from './stage-execution-engine.js';
    ...
    return {
      ...
      analysisSteps: [{
        ...
        execute: async (ctx) => {
          ...
          // SD-LEO-INFRA-STAGE-PER-TYPE-001 contract: when the analysis step
          // returns the typed-array form ({ artifacts: [{artifactType, payload,
          // source, gaps?}, ...] }), surface that array at the top level so the
          // orchestrator's detection (`Array.isArray(stepResult.artifacts)`) fires.
          // Burying it inside `payload` (legacy path) caused every projected
          // artifact to be silently dropped — e.g. S14 emits 5 typed artifacts but
          // only blueprint_technical_architecture was persisted, and S26 likewise.
          // RCA: NicheMetrics S14 (4 of 5 blueprint artifacts missing).
...
async function persistArtifacts(supabase, ventureId, stageId, artifacts, idempotencyKey, { visionKey = null, planKey = null } = {}) {
  ...
  // SD-LEO-INFRA-STREAM-VENTURE-EVA-002-C: Auto-resolve EVA keys when not explicitly provided
  let resolvedVisionKey = visionKey;
  let resolvedPlanKey = planKey;
  if (!resolvedVisionKey && !resolvedPlanKey && stageId >= 1 && stageId <= 15) {
    try {
      const keys = await resolveEvaKeys(supabase, ventureId, stageId);
      resolvedVisionKey = keys.visionKey;
      resolvedPlanKey = keys.planKey;
    } catch (_evaErr) {
      // Non-blocking: artifact persistence continues without EVA keys
    }
  }
  return writeArtifactBatch(supabase, ventureId, stageId, artifacts, idempotencyKey, { visionKey: resolvedVisionKey, planKey: resolvedPlanKey });
}
 ...
 * @param {string[]} requiredArtifacts - Artifact types from stage_artifact_requirements
 * @param {number} stageId - Current stage number
 * @param {string[]} [existingArtifactTypes=[]] - Already-persisted artifact types to skip (dedup)
 * @returns {Array} Extracted artifacts array (one per matched type)
 */
function extractMultiArtifacts(stageOutput, requiredArtifacts, stageId, existingArtifactTypes = []) {
  // SD-ARTIFACT-SYSTEM-RECONCILIATION-MULTIARTIFACT-ORCH-001-A: dedup check
  const existingSet = new Set(existingArtifactTypes.map(t => t.toLowerCase()));
  const filteredRequired = requiredArtifacts.filter(
    t => !existingSet.has(t.toLowerCase())
  );
  if (filteredRequired.length === 0) return [];

  const content = typeof stageOutput === 'string'
    ? stageOutput
    : stageOutput.description || stageOutput.content || JSON.stringify(stageOutput);

  // Strategy 1: Split by ## headings (markdown)
  ...
  // Strategy 2: JSON key-matching fallback
  // When LLM returns structured JSON instead of markdown, match top-level keys
  let jsonKeys = {};
  if (Object.keys(sections).length === 0) {
    try {
      const parsed = typeof stageOutput === 'string' ? JSON.parse(stageOutput) : stageOutput;
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        for (const [key, value] of Object.entries(parsed)) {
          if (value && typeof value === 'object') {
            jsonKeys[key.toLowerCase().replace(/[^a-z0-9_]/g, '_')] = JSON.stringify(value);
          } else if (value && typeof value === 'string' && value.length > 10) {
            jsonKeys[key.toLowerCase().replace(/[^a-z0-9_]/g, '_')] = value;
          }
        }
      }
    } catch {
      // Not valid JSON — sections-only path
    }
  }

  const lookupSources = Object.keys(sections).length > 0 ? sections : jsonKeys;
  const extractionMethod = Object.keys(sections).length > 0 ? 'section' : 'json_key';

  // Map required artifacts to extracted sections/keys
  const result = [];
  for (const artifactType of filteredRequired) {
    const normalized = artifactType.toLowerCase().replace(/[^a-z0-9_]/g, '_');
    let matched = null;

    for (const [heading, body] of Object.entries(lookupSources)) {
    ...
    if (matched) {
      result.push({
        ...
        payload: { content: matched, extractedFrom: extractionMethod },
...
/**
 * Fetch aggregated SRIP summary across all 4 SRIP tables for a venture.
 * Returns a structured summary or null if no SRIP data exists.
 *
 * Queries: srip_site_dna, srip_brand_interviews, srip_synthesis_prompts,
 * srip_quality_checks — aggregating status and quality from each.
 *
 * @param {Object} supabase - Supabase client
 * @param {string} ventureId - Venture UUID
 * @returns {Promise<Object|null>} SRIP summary or null
 */
async function fetchSripSummary(supabase, ventureId) {
  try {
    // Query all 4 SRIP tables in parallel
    const [dnaResult, interviewResult, synthesisResult, qcResult] = await Promise.all([
      supabase.from('srip_site_dna')
        ...
        .maybeSingle()
        .catch(() => ({ data: null })),
      supabase.from('srip_brand_interviews')
        .select('id, status, answers, updated_at')
        .eq('venture_id', ventureId)
        .order('created_at', { ascending: false })
        .limit(1)
        .maybeSingle()
        .catch(() => ({ data: null })),
      supabase.from('srip_synthesis_prompts')
        .select('id, version, token_count')
        .eq('venture_id', ventureId)
        .order('version', { ascending: false })
        .limit(1)
        .maybeSingle()
        .catch(() => ({ data: null })),
      supabase.from('srip_quality_checks')
        .select('id, overall_score, passed, domain_scores')
        .eq('venture_id', ventureId)
        .order('created_at', { ascending: false })
        .limit(1)
        .maybeSingle()
    ...
    // If no data exists in any table, return null
    if (!dna && !interview && !synthesis && !qc) return null;

    const answerCount = interview?.answers && typeof interview.answers === 'object'
      ? Object.keys(interview.answers).length
      : 0;

    return {
      hasSrip: true,
      dna_status: dna?.status ?? null,
      dna_quality: dna?.quality_score ?? null,
      interview_complete: answerCount >= 12,
      interview_count: answerCount,
      synthesis_exists: !!synthesis,
      qc_score: qc?.overall_score ?? null,
      qc_passed: qc?.passed ?? null,
      // Backward-compatible fields
      interview_id: interview?.id ?? null,
      status: interview?.status ?? (dna ? 'partial' : null),
      answer_count: answerCount,
      completed_at: interview?.updated_at ?? null,
      has_data: !!(dna || answerCount > 0 || synthesis || qc),
    };
  } catch {
    return null;
  }
