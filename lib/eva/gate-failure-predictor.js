...
export async function predictGateFailures(supabase, params, options = {}) {
  ...
  try {
    ...
    // Query historical handoffs for same SD type. Paginated (FR-6 batch 7): a 90-day
    // handoff window routinely exceeds the 1000-row cap and skews failure rates.
    // Page errors throw into the outer catch (same error-return shape).
...
export async function getGatePerformanceSummary(supabase, options = {}) {
  ...
  try {
    ...
    // Paginated (FR-6 batch 7): summary rates must see the whole window. Page errors
    // throw into the outer catch (same emptySummary error shape).
    const handoffs = await fetchAllPaginated(() => supabase
      .from('sd_phase_handoffs')
