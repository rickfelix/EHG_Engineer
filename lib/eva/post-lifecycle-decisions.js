...
export async function handlePostLifecycleDecision(params, deps = {}) {
  ...
  logger.log(`[PostLifecycle] Stage 26 completed for venture ${ventureId} (${ventureContext?.name || 'unknown'})`);
  ...
  if (!decision) {
    ...
    return {
      ...
      requiresReview: true,
      decisionOptions,
      summary: `Venture "${ventureContext?.name || ventureId}" has completed all 26 lifecycle stages. Chairman decision required.`,
    };
  }
  ...
  // restart an ALREADY-COMPLETED 26-stage traversal at an earlier stage", not "recover a
