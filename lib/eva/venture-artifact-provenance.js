...
const logger = createLogger('VentureArtifactProvenance');
...
export async function verifyLaunchUatReportLink(supabase, artifactRow) {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    logger.warn('verifyLaunchUatReportLink lookup error', { runId, error: error.message });
    return { linked: false, reason: `uat_test_runs lookup error: ${error.message}`, runId };
  }
