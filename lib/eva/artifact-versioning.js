...
const logger = createLogger('ArtifactVersioning');
// SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-G: computeContentHash moved to its own module so
// artifact-persistence-service.js can reuse it without a circular import (this file already
// imports writeArtifact from there). Re-exported here for backward compatibility.
...
export async function createVersionedArtifact(supabase, { ventureId, artifactType, stageId, content, existingMetadata = {} }) {
  ...
  try {
    ...
    logger.error('createVersionedArtifact failed', { ventureId, artifactType, error: err.message });
...
export async function updateArtifactVersion(supabase, { artifactId, content, additionalMetadata = {} }) {
  ...
  try {
    ...
    logger.error('updateArtifactVersion failed, restored prior is_current', { artifactId, error: err.message });
