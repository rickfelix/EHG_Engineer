...
const moduleLogger = createLogger('ContractValidator');
...
export async function validateContracts(options = {}) {
  ...
  if (requiredStages.length === 0 && targetStage > 1) {
    ...
    // QF/SD-LEO-INFRA-STAGE-LAUNCH-READINESS-001 FR-5: previously a second, unexported,
    // function-local copy of this map lived here (8 of 27 stages, no stage-24 entry) and
    // silently disagreed with the canonical export below for every OTHER stage. Import the
    // single source of truth instead of maintaining a local fork.
  ...
  if (ventureId) {
    try {
      ...
      // Non-blocking -- validateContracts' own result is unaffected either way.
      moduleLogger.warn('contract_validation_completed event emit failed', {
        ventureId, targetStage, error: eventErr.message,
      });
    }
  }
...
export async function getVentureProgression(supabase, ventureId) {
  try {
    ...
    if (error) {
      moduleLogger.warn('getVentureProgression query failed', { ventureId, error: error.message });
    ...
    moduleLogger.warn('getVentureProgression threw', { ventureId, error: err.message });
    return { stages: [], totalStages: 8, completedStages: 0, error: true };
  }
}
