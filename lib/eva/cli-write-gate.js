...
import { slugifyDimensionName } from './dimension-ids.js';

// eva-logger-lint-ignore: SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 adds one static
// metadata field (dimension_slug) to a zero-caller info export; no new behavior worth
// logging, and a full retrofit of this pre-existing file is out of scope.
...
export async function trackWriteSource(supabase, options = {}) {
  ...
  try {
    await supabase.from('eva_event_log').insert({ // schema-lint-disable-line — pre-existing reference on main (eva_event_log.event_data insert, untouched by FR-6 batch 7)
...
export async function getWriteSourceCoverage(supabase, options = {}) {
  ...
  try {
    ...
    // Paginated (FR-6 batch 7): a capped read would silently understate write coverage
    // and hide violations beyond the 1000-row cap.
    let events;
    try {
      events = await fetchAllPaginated(() => supabase
        .from('eva_event_log')
        .select('id, event_type, event_data, created_at') // schema-lint-disable-line — pre-existing reference on main (eva_event_log.event_data was in this select before the FR-6 rewrap); moved into diff by FR-6 batch 7
        .in('event_type', ['cli_write_tracked', 'cli_write_violation'])
        .gte('created_at', cutoff)
        .order('created_at', { ascending: false })
        .order('id', { ascending: true }));
    } catch (e) {
      logger.warn(`[CLIWriteGate] Query failed: ${e.message}`);
      return { coverage: emptyCoverage(), violations: [], error: e.message };
    }
...
export function getWriteGateInfo() {
  return {
    dimension: 'V06',
    // SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 (FR-5): additive stable companion —
    // the 'V06' literal above is a positional code with zero current callers (grepped
    // repo-wide) and is left unchanged; dimension_slug is the stable reference.
    dimension_slug: slugifyDimensionName('CLI Authority'),
