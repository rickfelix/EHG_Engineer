...
export async function getPortfolioEconomicLens({ supabase }) {
  // Paginated (FR-6 batch 7): portfolio-wide aggregation must see every venture's
  // current analysis. Page errors rethrow with the prior message shape.
  let data;
  try {
    data = await fetchAllPaginated(() => supabase
      .from('venture_artifacts')
      .select('id, venture_id, artifact_data, metadata, created_at, ventures!inner(name)') // schema-lint-disable-line — pre-existing reference on main + lint misparse: ventures!inner(name) is PostgREST embedded-join syntax, not a column; moved into diff by FR-6 batch 7 rewrap
      .eq('artifact_type', 'economic_lens')
      .eq('is_current', true)
      .order('created_at', { ascending: false })
      .order('id', { ascending: true }));
  } catch (e) {
    throw new Error(`Failed to fetch portfolio data: ${e.message}`);
  }
