...
// eva-logger-lint-ignore: pre-existing file already uses console.warn for its retry/fail-soft
// path; SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 adds one call to an existing, already-
// tested helper (assignDimensionIds) -- a full createLogger retrofit of this file's unrelated
// existing logging is out of this SD's scope.
...
${truncated}`;
  ...
  try {
    ...
    // SD-LEO-INFRA-VISION-ARCHITECTURE-DIMENSION-001 (FR-3): the canonical extractor assigns
    // a stable id to its own output so callers that don't route through upsertVision/
    // upsertArchPlan (e.g. the eager-synthesis path) still persist ids going forward.
    return assignDimensionIds(dimensions);
  } catch (err) {
    if (retryCount === 0) {
