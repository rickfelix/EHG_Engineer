...
export class VisionGovernanceService {
  ...
  async getGaps(filters = {}) {
    ...
    const supabase = await this._getSupabase();
    // Paginated (FR-6 batch 7): gap governance must see every open gap.
    const data = await fetchAllPaginated(() => {
      ...
      if (sdId) q = q.eq('sd_id', sdId);
      return q;
    }).catch((e) => { throw new Error(`Gap query failed: ${e.message}`); });
    return data || [];
  }
  ...
  async getActiveCorrectiveSDs() {
    const supabase = await this._getSupabase();
    // Paginated (FR-6 batch 7): active corrective-SD accounting must be complete.
    const data = await fetchAllPaginated(() => supabase
      ...
      .catch((e) => { throw new Error(`Corrective SD query failed: ${e.message}`); });
    return data || [];
  }
  ...
  async getVentureProgression(ventureId) {
    ...
    // SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001: the loop bound (previously 25) and pendingCount's
    // denominator (previously 26) disagreed with each other even before this SD -- both now
    // consistently use 27 (the 2026-08-28 renumbering's total stage count).
    ...
    return {
      ...
      pendingCount: 27 - completedStages.size,
      stages,
    };
  }
}
