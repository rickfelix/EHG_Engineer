...
export async function promoteVisionDocument({ supabase, visionKey, promotedBy }) {
  ...
  // schema-reference-snapshot.json -- the 42703 fail-soft retry below is exactly this
  // function's own handling for a DB that hasn't applied it yet.
  let { data, error } = await supabase
    .from('eva_vision_documents')
    .update({ // schema-lint-disable-line
      status: 'active',
    ...
    .select('id, vision_key, status, chairman_approved, chairman_approved_at, approved_by, approved_by_at') // schema-lint-disable-line
    .single();
