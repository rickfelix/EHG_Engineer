...
/**
 * Mark a venture's orchestrator as completed (terminal state).
 * Transitions processing → completed and sets completed_at timestamp.
 *
 * @param {Object} supabase - Supabase client
 * @param {string} ventureId - Venture UUID
 * @param {Object} [options]
 ...
 */
export async function markCompleted(supabase, ventureId, options = {}) {
  const { lockId, logger = console } = options;

  if (!supabase || !ventureId) {
    return { completed: false, error: 'Missing db client or ventureId' };
  }

  try {
    // Accept both 'processing' and 'idle' states — the lock may have already
    // been released to 'idle' before markCompleted is called. (Bug 3 fix: QF-20260320-509)
    let query = supabase
      .from('ventures')
      .update({
      ...
      .eq('id', ventureId)
      .in('orchestrator_state', [ORCHESTRATOR_STATES.PROCESSING, ORCHESTRATOR_STATES.IDLE]);
    ...
    if (error || !data) {
      logger.warn(`[StateMachine] markCompleted failed for ${ventureId}: lockId=${lockId}`);
      return { completed: false, error: 'Completion failed - state or lockId mismatch' };
    }

    logger.log(`[StateMachine] Venture ${ventureId} marked COMPLETED`);
    return { completed: true };
  } catch (err) {
    logger.error(`[StateMachine] markCompleted error: ${err.message}`);
