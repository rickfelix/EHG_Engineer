 ...
 *
 * wired=true iff event_count > 0 -- independent of actor_hash nullability, per FR-3 AC-2.
 *
 ...
 * @param {string} ventureId - ventures.id to check
 * @returns {Promise<{ wired: boolean, reason: string, active_users?: number }>}
 */
async function verifyTelemetryAnalyticsWired(supabase, ventureId) {
  ...
  return eventCount > 0
    ? { wired: true, reason: `${eventCount} usage event(s) in the last ${TELEMETRY_ANALYTICS_DEFAULT_WINDOW_DAYS} days`, active_users: activeUsers }
    : { wired: false, reason: `no usage events in the last ${TELEMETRY_ANALYTICS_DEFAULT_WINDOW_DAYS} days`, active_users: 0 };
}
