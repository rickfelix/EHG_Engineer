...
const moduleLogger = createLogger('ValidateVentureDefaultCapabilities');
...
/**
 * Shared, single-source override-reason validation: trims and fail-closes on
 * empty/whitespace-only. Extracted (SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-I
 * FR-3) so the Stage-24 ground-truth capability checklist's override path
 * reuses this exact logic rather than re-implementing a second, divergent
 * copy of the same rule.
 * @param {unknown} overrideReasonRaw
 * @returns {{valid: boolean, trimmed: string}}
 */
export function validateOverrideReason(overrideReasonRaw) {
  const trimmed = typeof overrideReasonRaw === 'string' ? overrideReasonRaw.trim() : '';
...
export function validateVentureDefaultCapabilities(parsedSprintPlan, opts = {}) {
  ...
  for (const capability of EHG_VENTURE_DEFAULT_CAPABILITIES) {
    ...
    const { valid, trimmed } = validateOverrideReason(override?.override_reason);
    if (!valid) {
    ...
    warnings.push(`Capability ${capability.capability_id} intentionally omitted: ${trimmed}`);
  }
...
/**
 * SD-LEO-INFRA-VENTURE-QUALITY-CAPA-001-I FR-3: record (upsert) a written-reason
 * override for a default capability with no ground-truth wiring signal. Reuses
 * validateOverrideReason so this is the identical trim+fail-closed rule already
 * enforced at Stage-19 declaration time -- never a second, divergent copy.
 * @returns {Promise<{ok: boolean, id?: string, reason?: string}>}
 */
export async function recordCapabilityOverride({ supabase, ventureId, capabilityId, overrideReason, logger = moduleLogger }) {
  if (!supabase || !ventureId || !capabilityId) {
    return { ok: false, reason: 'missing_required_fields' };
  }
  const { valid, trimmed } = validateOverrideReason(overrideReason);
  if (!valid) {
    return { ok: false, reason: 'empty_or_whitespace_reason' };
  }

  const { data: existing } = await supabase
    .from('venture_capability_overrides')
    .select('id')
    .eq('venture_id', ventureId)
    .eq('capability_id', capabilityId)
    .maybeSingle();

  const row = { venture_id: ventureId, capability_id: capabilityId, override_reason: trimmed, updated_at: new Date().toISOString() };

  if (existing?.id) {
    const { error } = await supabase.from('venture_capability_overrides').update(row).eq('id', existing.id);
    if (error) {
      logger?.warn?.(`[validate-venture-default-capabilities] override update failed for ${capabilityId}: ${error.message}`);
  ...
  const { data: inserted, error: insertErr } = await supabase
    ...
    .single();
  if (insertErr) {
    logger?.warn?.(`[validate-venture-default-capabilities] override insert failed for ${capabilityId}: ${insertErr.message}`);
...
/**
 * Reads all recorded capability overrides for a venture, keyed by capability_id.
 * @returns {Promise<Map<string, {override_reason: string}>>}
 */
export async function readCapabilityOverrides({ supabase, ventureId, logger = moduleLogger }) {
  if (!supabase || !ventureId) return new Map();
  const { data, error } = await supabase
    .from('venture_capability_overrides')
    .select('capability_id, override_reason')
    .eq('venture_id', ventureId)
    .limit(50);
  if (error) {
    logger?.warn?.(`[validate-venture-default-capabilities] override read error (treating as none): ${error.message}`);
    return new Map();
  }
  return new Map((data || []).map((row) => [row.capability_id, { override_reason: row.override_reason }]));
}
