...
describe('parseFourBuckets', () => {
  ...
  describe('malformed entries', () => {
    ...
    // SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-153 FR-1 (TS-1/TS-2): these two silent-drop paths
    // previously dropped entries with no log call at all, unlike the other two malformed-input
    // paths above (which already log). Now matched to the same logged behavior.
    it('logs a warning when skipping an entry without a claim', () => {
      const logger = { warn: vi.fn() };
      const result = parseFourBuckets({
        epistemicClassification: [{ bucket: 'fact', evidence: 'no claim' }],
      }, { logger });
      expect(result.classifications).toHaveLength(0);
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('no claim field')
      );
    });

    it('logs a warning when skipping a null/non-object entry', () => {
      const logger = { warn: vi.fn() };
      const result = parseFourBuckets({
        epistemicClassification: [null, 'not-an-object', 42],
      }, { logger });
      expect(result.classifications).toHaveLength(0);
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('non-object')
      );
      // 3 malformed entries -> 3 warnings, one per skipped entry
      expect(logger.warn).toHaveBeenCalledTimes(3);
    });

    it('does not log for a well-formed entry', () => {
      const logger = { warn: vi.fn() };
      parseFourBuckets({
        epistemicClassification: [{ claim: 'Valid', bucket: 'fact', evidence: '' }],
      }, { logger });
