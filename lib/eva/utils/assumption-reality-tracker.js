...
// docs/reference/eva-logging-standard.md: every function below accepts an injectable `logger`
// param (tests pass a silent stub) -- this module-level instance is only the DEFAULT when no
// logger is supplied, satisfying the standard's createLogger requirement without disturbing the
// existing dependency-injection pattern this file shares with token-tracker.js.
const moduleLogger = createLogger('RealityTracker');
 ...
 * @param {Object} [logger=moduleLogger] - Logger
 * @returns {Promise<Object|null>} Reality measurements keyed by category, or null on error
 */
export async function collectRealityMeasurements(supabase, ventureId, stageId, logger = moduleLogger) {
 ...
 */
export async function buildCalibrationReport(supabase, ventureId, logger = moduleLogger) {
  try {
...
/**
 * PAT-LES-6d8e6a986931: the fire-and-forget write below previously only logged failures via
 * `logger.warn` -- console-only output that a background pipeline write has no guarantee
 * anyone is watching, so a silent DB failure was genuinely invisible in production (the
 * retrospective's exact complaint). This routes the same failure through emitFeedback's
 * existing durable, queryable sink (the same harness_backlog channel gate-failure-recovery.js
 * already uses for EVA-pipeline runtime failures) so it survives beyond the process's stdout.
 * Deliberately fire-and-forget itself (never awaited, errors swallowed) to preserve this
 * function's own non-blocking contract -- a feedback-emission failure must never surface as a
 * pipeline error.
 *
 * @param {Object} supabase - Supabase client
 * @param {string} ventureId - Venture UUID
 * @param {string} errorMessage - The underlying write failure's message
 * @param {Object} [logger=moduleLogger] - Logger
 */
function reportWriteFailure(supabase, ventureId, errorMessage, logger) {
  emitFeedback({
    supabase,
    title: 'Fire-and-forget assumption_sets write failed',
    // validation-learn154 F3 + security-learn154 R2: description feeds emit-feedback.js's
    // dedup_hash (sha256(today::description::dedup_key) -- see its own header comment on the
    // fleet_dormancy/66-row incident this exact mistake caused elsewhere), so it must stay
    // STABLE per venture per day -- no raw error text here. ventureId itself is deliberately
    // left OUT of description too (it already lives in dedup_key and metadata.venture_id):
    // both this function and updateAssumptionSetStatus are exported, so a future caller
    // reaching this code without the orchestrator's implicit venture-id-is-a-real-UUID
    // guarantee would otherwise get that value echoed into description unvalidated.
    description: "assumption-reality-tracker.js's updateAssumptionSetStatus() failed to persist reality_data/calibration_report/status. See metadata.venture_id and metadata.error_message for detail.",
    category: 'harness_backlog',
    severity: 'medium',
    // 'assumption_reality_tracker' is NOT a member of the live feedback_source_type_check
    // CHECK constraint (verified via pg_get_constraintdef) -- using it here would make
    // emitFeedback itself throw on every call, silently swallowed by the .catch() below,
    // reproducing the exact "silent failure" bug this function exists to close. 'auto_capture'
    // is a real, valid value for an automated (non-human) feedback-table writer; the specific
    // producer is preserved in metadata.source for provenance instead of the enum column.
    source_type: 'auto_capture',
    dedup_key: `assumption-sets-write-failure:${ventureId}`,
    metadata: {
      venture_id: ventureId,
      source: 'assumption_reality_tracker',
      // security-learn154 R1: bound the raw DB error text before it lands in a durable row --
      // this is a brand-new metadata key (0 pre-existing feedback rows carry it), so it is also
      // the one place a future redaction policy would need to target.
      error_message: String(errorMessage).slice(0, 500),
      sd_origin: 'SD-LEARN-FIX-ADDRESS-PATTERN-LEARN-154',
    },
  }).catch((err) => {
    // security-learn154 W1: this is the ONE path where the fix's own durability can silently
    // regress (e.g. a future caller reaching reportWriteFailure with a non-service-role client
    // gets RLS-denied here) -- error, not warn, so a swallowed feedback-emission failure stays
    // severity-visible in log aggregation rather than blending into routine warnings.
    logger.error(`[RealityTracker] emitFeedback failed (non-fatal): ${err.message}`);
  });
}
 ...
 * @param {Object} [logger=moduleLogger] - Logger
 */
export function updateAssumptionSetStatus(supabase, ventureId, stageId, calibrationReport, logger = moduleLogger) {
  ...
  supabase
    ...
    .then(({ error }) => {
      if (error) {
        ...
        reportWriteFailure(supabase, ventureId, error.message, logger);
      }
    })
    .catch((err) => {
      logger.warn(`[RealityTracker] Update error (non-fatal): ${err.message}`);
      reportWriteFailure(supabase, ventureId, err.message, logger);
    });
}
...
export async function runRealityTracking({ ventureId, stageId }, deps = {}) {
  const { supabase, logger = moduleLogger } = deps;
