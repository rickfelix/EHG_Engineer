...
function reportWriteFailure(supabase, ventureId, errorMessage, logger) {
  emitFeedback({
    ...
    // validation-learn154 F3: description feeds emit-feedback.js's dedup_hash
    // (sha256(today::description::dedup_key) -- see its own header comment on the
    // fleet_dormancy/66-row incident this exact mistake caused elsewhere). A raw error
    // message varies per failure and would defeat the per-venture-per-day dedup this
    // dedup_key is meant to provide. Keep description STABLE per venture per day; the
    // volatile message goes in metadata only (never hashed).
    description: `assumption-reality-tracker.js's updateAssumptionSetStatus() failed to persist reality_data/calibration_report/status for venture ${ventureId}. See metadata.error_message for detail.`,
    ...
    logger.warn(`[RealityTracker] emitFeedback failed (non-fatal): ${err.message}`);
  });
}
