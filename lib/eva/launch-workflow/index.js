...
export async function getLaunchStatus(ventureId, deps = {}) {
  ...
  // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 / FR-1: eva_stage_gate_results has no `reasoning`
  // column (real column is `notes`); this query reproduced a live 42703 error on every call.
  const { data: gateResults, error: gateError } = await supabase
    .from('eva_stage_gate_results')
    .select('stage_number, passed, gate_type, notes, created_at')
    .eq('venture_id', ventureId)
    // SD-LEO-FEAT-POST-BUILD-LIFECYCLE-001-A: the spend_approval launch-blocking gate moved
    // from stage 22 to stage 21 (Distribution Setup), so include 21 in the readiness scope.
    .gte('stage_number', 21)
    .order('stage_number', { ascending: true })
    // Bounded by construction: a single venture spans <=40 lifecycle stages x a handful of
    // gate_types each, so this is provably well under the cap (count-truncation-diff-lint).
    .limit(100);

  if (gateError) {
    throw new Error(`[launch-workflow] getLaunchStatus gate query failed: ${gateError.message}`);
  }
  ...
  return {
    ...
    inLaunchPhase,
    launchReadiness: {
      // SD-LEO-FEAT-POST-BUILD-LIFECYCLE-001-A: spend_approval gate moved 22->21, field
      // renamed to follow it (breaking; EHG-app type ehg/src/types/workflowStages.ts is a
      // cross-repo follow-up — no runtime consumers found at rename time).
      stage21Gate: launchGates.stage_21 || null,
      stage24Gate: launchGates.stage_24 || null,
      allGatesPassed: !!(launchGates.stage_21?.passed && launchGates.stage_24?.passed),
    },
...
export async function getChecklist(ventureId, deps = {}) {
  ...
  // Chairman blocking gates for launch: 21 (spend_approval, moved from 22) and 24
  // Advisory gates: 23
  // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 / FR-1: eva_stage_gate_results has no `reasoning`/
  // `score` columns (real columns are `notes`/`overall_score`); this query reproduced a live
  // 42703 error on every call, silently swallowed (no `error` binding), computing checklist
  // readiness over an always-empty gate set.
  const { data: gates, error: gatesError } = await supabase
    .from('eva_stage_gate_results')
    .select('stage_number, passed, gate_type, notes, overall_score, created_at')
    .eq('venture_id', ventureId)
    .gte('stage_number', 20)
    .order('stage_number', { ascending: true })
    // Bounded by construction: a single venture spans <=40 lifecycle stages x a handful of
  ...
  if (gatesError) {
    throw new Error(`[launch-workflow] getChecklist gate query failed: ${gatesError.message}`);
  }
...
export async function getTimeline(ventureId, deps = {}) {
  ...
  // SD-LEO-INFRA-MINUS-EVIDENCE-LAYER-001 / FR-1: eva_stage_gate_results has no `score`
  // column (real column is `overall_score`); this query reproduced a live 42703 error.
  const { data: gates, error: gatesError } = await supabase
    .from('eva_stage_gate_results')
    .select('stage_number, passed, gate_type, overall_score, created_at')
    .eq('venture_id', ventureId)
  ...
  if (gatesError) {
    throw new Error(`[launch-workflow] getTimeline gate query failed: ${gatesError.message}`);
  }
...
async function getStageLabels(supabase) {
  ...
  try {
    ...
    // stage_number 1..maxStageNumber via getStage(). SD-LEO-INFRA-STAGE-RENUMBER-DRIFT-001:
    // now DB-derived (was a hardcoded 26 literal), so this can never silently diverge again.
    for (let n = 1; n <= gov.maxStageNumber; n += 1) {
...
// Chairman blocking gates (SD-LEO-FEAT-POST-BUILD-LIFECYCLE-001-A: the spend_approval
// blocking gate moved from stage 22 to stage 21 with the Distribution Setup swap).
const BLOCKING_GATES = [3, 10, 21, 24];
