...
export async function checkSyntheticActorFencing(supabase, ventureId, opts = {}) {
  ...
  try {
    ...
    // VALIDATION V-3 (PLAN_VERIFICATION): AC#14 requires this case read as
    // "cannot determine," distinct from a genuine not-verified result -- a
    // transient GitHub outage is not the fence catching a real problem, and
    // conflating the two would corrupt AC#15's promotion-readiness signal
    // (N consecutive clean observe cycles becomes meaningless if an API
    // blip looks identical to a true would-block). satisfied:false is
    // UNCHANGED -- binding behavior still fails closed either way, matching
    // this module's own fail-closed contract; only the indeterminate flag
    // lets a caller distinguish the two for logging/measurement purposes.
    return { applies: true, satisfied: false, indeterminate: true, reason: `GitHub API error and no fresh cached result (fail-closed, cannot determine): ${e.message}` };
  }
}
