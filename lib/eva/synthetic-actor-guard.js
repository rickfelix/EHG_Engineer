...
// 7 days is a STARTING value, not a data-derived one: measured against deploy.yml's actual
// history on main at the time of this fix (34 completed runs, 2026-08-17 to 2026-08-21,
// median inter-run gap 0.01 days, p90 0.38, max 1.76 -- 7 days is ~4x the observed max), but
// four days of history cannot characterise a genuinely quiet period. Expected to be revisited
// once the observe-only window (LEO_SYNTHETIC_ACTOR_FENCE_ENFORCE default-off) produces more
// data -- calibrating this is exactly what that window exists to measure, not something to
// defer past promotion to binding. LAST_KNOWN_GOOD_TTL_MS's 5-minute cache-reuse-on-API-error
// window is negligible against this bound (a cached result is at most 5 minutes stale relative
// to whatever the underlying check would have found).
const STALENESS_WINDOW_MS = 7 * 24 * 60 * 60 * 1000; // 7 days
