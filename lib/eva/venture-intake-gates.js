 ...
 *     calibration cohort exists (currently 1 completed run — DataDistill — i.e. a cohort
 *     of one; isPackBinding(1) is false). The binding-enforcement branch is
 *     authored-but-DORMANT (guarded behind BOTH seams, never reached this SD).
 *
...
export const COMPLETION_STAGE = 27;
...
/**
 * First present non-null OWN value among field names, checking input + input.metadata.
 * Own-property only (hasOwnProperty) so an inherited/poisoned-prototype field can't
 * fabricate a false-positive observation in the calibration record.
 */
function firstField(input, names) {
  const own = (obj, n) => obj != null && typeof obj === 'object' && Object.prototype.hasOwnProperty.call(obj, n) && obj[n] != null;
  for (const n of names) {
    if (own(input, n)) return input[n];
    if (own(input?.metadata, n)) return input.metadata[n];
  }
  return undefined;
}
 ...
 * fabricate a cohort (current live count is 1 — DataDistill — i.e. cohort of one).
 *
