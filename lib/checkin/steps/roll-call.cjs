...
const { verdictFromMetadata, ROLE_VERDICT } = require('../../fleet/role-status-identity.cjs');
...
module.exports = {
  ...
  async run(ctx) {
    ...
    // QF-20260904-935: role seats (Adam, Solomon, coordinator) are non-fleet and never hold a
    // claim, so a roll_call row from one is a false availability record, not a real worker
    // check-in. Census (session_coordination filtered on the coordinator's own sender_session +
    // payload.kind=roll_call) found no consumer reading these rows for a liveness signal --
    // coordinator-quiet-tick.mjs already marks roll_call COORDINATOR_UNACKABLE, and the reserve
    // picker already excludes role sessions via isFleetWorker -- so the write is skipped
    // entirely rather than kept-with-a-fixed-availability-literal.
    const isRoleSession = verdictFromMetadata(ctx.sessionMetadata) === ROLE_VERDICT.ROLE;
    // 3. register availability (idempotent)
    const rollCall = isRoleSession
      ? { id: null, deduped: false, skipped: true }
      : await registerRollCall(sb, { sessionId, coordinatorId, callsign: ctx.callsign, mySd: ctx.mySd });
