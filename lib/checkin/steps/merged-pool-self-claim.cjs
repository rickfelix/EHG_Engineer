...
module.exports = {
  ...
  async run(ctx) {
    ...
    // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-2): the shared pure fit-defer primitive.
    const { shouldDeferForBetterFit } = require('../../fleet/dispatch-suggestions.cjs');
    const {
    ...
    try {
      ...
      if (tierCtx.tiering_active === true) {
        ...
        // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-2): the OTHER live workers' ranks, once per
        // tick, so the claim loop below can defer an over-qualified pick to a closer-fit live peer
        // (shouldDeferForBetterFit). Gated on tiering_active exactly like every other tier-axis fetch
        // in this function (degrade-to-1 leaves this [] — a no-op, byte-identical to pre-FR-2
        // behavior). Uses live_self_rank (THIS worker's rank in the SAME live-ladder snapshot as
        // live_peer_ranks) rather than the STAMPED tierCtx.worker_tier_rank — the two are
        // incommensurate scales (one is a cached/possibly-stale stamp, the other a fresh
        // dense-rank over whoever is live right now); comparing across them would compare apples
        // to oranges. When live_self_rank is unavailable (e.g. this session's own row filtered
        // out of the live-fleet query), shouldDeferForBetterFit's Number.isFinite guard makes the
        // defer check a no-op — same fail-open contract as before.
        try {
          const { liveWorkerCapabilitySnapshot } = require('../../fleet/tier-ladder.cjs');
          const snap = await liveWorkerCapabilitySnapshot(sb, { excludeSessionId: sessionId });
          tierCtx.live_peer_ranks = snap.ranks;
          tierCtx.live_self_rank = snap.selfRank;
        } catch { /* fail-open: no peer-fit signal, defer check stays inert */ }
      }
      /*
      ...
      // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-2): hoisted out of the try block below so the
      // claim loop's defer check can look up each candidate's min_tier_rank without a second fetch.
      // Empty when the try below fails or finds nothing — the defer check degrades to a no-op then.
      let poolBySdKey = new Map();
      try {
        ...
        poolBySdKey = new Map(pool.map((r) => [r.sd_key, r]));
      ...
      for (const x of ranked) {
        ...
        // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-2): pickup intelligence. Only when tiering
        // is active AND this candidate's min_tier_rank is known does the defer check evaluate;
        // otherwise it is a byte-identical no-op (shouldDeferForBetterFit returns false on any
        // non-finite input), which is exactly the single-seat / unscored-item fallback path.
        // Does NOT touch classifyDispatchIneligibility/tryClaim — x is already past every
        // eligibility gate below; this only changes whether THIS worker claims it THIS tick, never
        // whether it is claimable at all.
        if (tierCtx.tiering_active === true) {
          const poolRow = poolBySdKey.get(x.key);
          const minTierRank = poolRow && poolRow.metadata && poolRow.metadata.min_tier_rank;
          if (shouldDeferForBetterFit({ myRank: tierCtx.live_self_rank, minTierRank, liveRanks: tierCtx.live_peer_ranks })) {
            continue;
          }
        }
        if (x.kind === 'baselined') {
