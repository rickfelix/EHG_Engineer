...
module.exports = {
  ...
  async run(ctx) {
    ...
    // producer steps (this one, critical-qf-jump.cjs, self-claim-qf.cjs). FAIL-CLOSED on a read
    // error (skip), unlike most of this file's fail-open convention -- an uncertain read
    ...
    try {
      ...
      // QF-20260903-977 — CAP BEFORE PREDICATE. `ranked` is the union of FOUR CAPPED WINDOWS:
      // v_sd_next_candidates .limit(SELF_CLAIM_CANDIDATE_LIMIT=5), the oldest-N and newest-N draft
      // windows (DRAFT_CANDIDATE_LIMIT=10 each), and fleet_critical. Its length is therefore a SAMPLE
      // SIZE, not a population, and every eligibility predicate below runs on that sample. So when the
      // sample yields nothing claimable, "none of the ones I looked at" and "none exist" are
      // INDISTINGUISHABLE to the caller — which is the measured symptom this QF was filed for: a seat
      // parked for ten minutes while 33 unfenced SDs existed, then claimed one by hand seconds later.
      //
      // Counting the population under the same filter set makes exhaustion and emptiness separable, so
      // idle.cjs can report "examined N of M" instead of an unmeasured negative.
      //
      // THIS BLOCK IS REPORTING ONLY. It admits and excludes nothing; the caps, the candidate set and
      // every claim decision are byte-identical with or without it. Widening the windows or paging until
      // exhaustion are real fixes to the same defect, but they change dispatch BEHAVIOUR rather than
      // dispatch REPORTING and are deliberately out of scope here.
      //
      // EXTENT OF THE DENOMINATOR, stated so a reader can refute it: this counts the DRAFT-WINDOW
      // population — unclaimed, non-orchestrator, status in (draft, active) — which is exactly what the
      // two draft windows sample. It does NOT count the v_sd_next_candidates view or the fleet_critical
      // source, which draw from different populations. So belt_population_total is the honest
      // denominator for the draft windows and a LOWER BOUND for the merged pool, never an over-count.
      //
      // Fail-open leaves the total NULL, and a null total must read as UNDETERMINED downstream — never
      // as a belt that is known to be empty. An unmeasured negative is the defect, not the remedy.
      ctx.base.belt_sample_examined = ranked.length;
      ctx.base.belt_population_total = null;
      try {
        ...
        if (typeof count === 'number') ctx.base.belt_population_total = count;
      } catch { /* fail-open: total stays null => UNDETERMINED, never a false "belt is empty" */ }
