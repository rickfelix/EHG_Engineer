  ...
  async run(ctx) {
    ...
    try {
      ...
      if (tierCtx.tiering_active === true) {
        ...
        // behavior). excludeSessionId so this worker never "defers to itself".
        try {
      ...
      for (const x of ranked) {
        ...
        if (tierCtx.tiering_active === true) {
          ...
          if (shouldDeferForBetterFit({ myRank: tierCtx.worker_tier_rank, minTierRank, liveRanks: tierCtx.live_peer_ranks })) {
            continue;
          }
        }
        if (x.kind === 'baselined') {
