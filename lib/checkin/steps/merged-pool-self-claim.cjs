  ...
  async run(ctx) {
    ...
    try {
      ...
      try {
        ...
        // SD-LEO-ORCH-CAPA-RECORD-TRUTH-002-D (FR-1): `parent_sd_id` and `dependencies` added so the
        // eligibility predicate below can run IN MEMORY. Without them the gauge counted a raw pool.
        const cols = 'sd_key,id,sd_type,status,description,title,metadata,target_application,claiming_session_id,parent_sd_id,dependencies';
        let pool = [];
        if (allKeys.length) {
        ...
        if (missingRefs.length) {
          ...
          const fetches = [];
          if (uuidRefs.length) fetches.push(sb.from('strategic_directives_v2').select(pcols).in('id', uuidRefs));
          if (keyRefs.length) fetches.push(sb.from('strategic_directives_v2').select(pcols).in('sd_key', keyRefs));
