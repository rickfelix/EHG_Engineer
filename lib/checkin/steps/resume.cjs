...
module.exports = {
  ...
  async run(ctx) {
    ...
    if (ctx.mySd) {
      ...
      if (!isQf) {
        try {
          const { data: sdRow, error: sdErr } = await sb.from('strategic_directives_v2').select('status, progress').eq('sd_key', ctx.mySd).maybeSingle();
          ...
          if (sdRow) {
            ...
            else if ((sdRow.progress || 0) === 0) strayReason = await detectStrayUnworkedClaim(sb, sessionId, ctx.mySd);
