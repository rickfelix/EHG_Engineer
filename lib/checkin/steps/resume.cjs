...
module.exports = {
  ...
  async run(ctx) {
    ...
    const { ws, confirmRowGone, selfHealStaleClaim, findOwnSdClaim, healOwnClaimPointer, extractDirectedSd, extractSdFromAssignment, isInformationalNudge, ASSIGNMENT_RECENCY_WINDOW_MS, ackMessage } = ctx.helpers;
      ...
      if (staleTerminal || staleDeleted) {
        ...
        try {
          ...
          if (wa) {
            ...
            // branch changes.
            if (waSd && waSd !== ctx.mySd && justRehydrated) {
