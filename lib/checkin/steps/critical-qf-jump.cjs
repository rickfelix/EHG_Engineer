...
async function withheldFilteredQfs(qfs, ctx) {
  ...
  try {
    ...
    for (const [app, group] of byApp) {
      ...
      // forApp mirrors fallback-queue.js / merged-pool-self-claim.cjs. Harmless to omit today
      // (the cache keys on repo), but an asymmetry between three producers of the same snapshot
      // is how the next reader concludes one of them meant something different.
      const r = filterOutInFlight(group, (qf) => qf.id, { repo, forApp: app, ...(ctx?.inflightProbe || {}) });
      kept.push(...r.kept);
    ...
    if (withheld.length) {
      ...
      // QF-20260911-388: expose the withheld list on the returned array (an extra, non-
      // iteration-affecting property) so a caller building a per-candidate self-claim trace can
      // answer "why not X" even when X never reached the post-filter candidate list. Existing
      // callers that only iterate/read the array elements are unaffected.
      kept.withheld = withheld.map((w) => ({ id: w.id, reason: w.reason }));
...
module.exports = {
  ...
  async run(ctx) {
    ...
    // producer steps (this one, merged-pool-self-claim.cjs, self-claim-qf.cjs). FAIL-CLOSED on a
    // read error (skip), unlike most of this file's fail-open convention -- an uncertain read
