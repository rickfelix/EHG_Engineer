...
// QF-20260905-060: verify whether THIS cwd's own worktree SD/QF is a LIVE in-progress claim
// before the repo-match axis (sd-executable-here.cjs) treats a worktree cwd as committed
// context -- a leftover worktree purged directed venture-targeted assignments unconditionally.
const { isCwdWorktreeLiveClaim } = require('../../fleet/cwd-worktree-liveness.cjs');
...
module.exports = {
  ...
  async run(ctx) {
    ...
    // QF-20260830-452: claim-row-less directed work (a UAT walk, ceremony, review — anything
    // that is neither an SD nor a QF) has no assigned_sd/sd_key for extractSdFromAssignment to
    // resolve, so it fell through as "unclaimable" and the seat self-claimed something else
    // instead — twice, on a chairman-ordered item. payload.assignment_type is the carrier a
    // keyless dispatch uses to say "this IS directed work, act on the payload itself".
    const isKeylessDirectedWork = (m) =>
      !!(m && m.payload && typeof m.payload.assignment_type === 'string' && m.payload.assignment_type);
    // QF-20260907-369: a transiently-deferred assignment (not_before gate, or a fitness-check
    // query failure below) is legitimately seen and processed by this code EVERY tick, but
    // neither branch calls ackWithReceipt -- by design, since the row must keep re-surfacing
    ...
    // read_at+acknowledged_at TOGETHER (scripts/worker-checkin.cjs's ackMessage) -- so a row
    // stuck in either transient state never gets read_at set at all, no matter how many times it
    // is correctly seen and deferred (measured: QF-20260903-936's assignment, deferred to
    // 2026-09-11, unread past 33+ minutes across multiple checkins that all correctly deferred
    // it). Mirrors the DELIVERED-only stamp surfaceCoordinatorMessages and ackMessage's own
    // adam-directive branch already use: read_at ONLY, never acknowledged_at, so the row keeps
    // re-surfacing and re-attempting exactly as before -- this is purely a visibility fix.
    const markDeliveredOnly = async (m) => {
      if (m.read_at) return; // already delivered -- idempotent, no redundant write
      try { await sb.from('session_coordination').update({ read_at: new Date().toISOString() }).eq('id', m.id); } catch { /* best-effort */ }
    };
    const pickClaimable = (rows) => {
      const skipped = [];
      for (const m of (rows || [])) {
        if (m.message_type !== 'WORK_ASSIGNMENT' || isInformationalNudge(m)) continue;
        if (extractSdFromAssignment(m) || isKeylessDirectedWork(m)) return { picked: m, skipped };
        skipped.push(m);
      }
    ...
    try {
      ...
      // FR-2: nothing claimable survived either pull -- check (report-only) whether an EXPIRED row
      // is the reason, so the printed verdict can say 'expired' instead of the generic 'none' a
      // seat with truly nothing pending also reports. Never claims an expired row; excludeExpired
      // above is unchanged for the actual claim path.
      if (!assignment) {
        try {
          const withExpired = await ws.getMessagesForSession(sb, sessionId, { sinceIso: assignmentSinceIso });
          const expiredHit = (withExpired || []).find(m => m.message_type === 'WORK_ASSIGNMENT' && !isInformationalNudge(m)
            && (extractSdFromAssignment(m) || isKeylessDirectedWork(m)));
          if (expiredHit) {
            ctx.base.directed_lane_verdict = { outcome: 'expired', id: expiredHit.id, sd_key: extractSdFromAssignment(expiredHit) || null, reason: 'excluded as expired' };
          }
        } catch { /* report-only; never blocks the claim path */ }
      }
    } catch { /* fail-open */ }
    if (assignment) {
       ...
       *
       * WRAPPING ackMessage RATHER THAN ADDING RECEIPT BLOCKS PER-BRANCH IS THE POINT. This file
       * has five distinct ack branches (stale-purge, ineligible-purge, claimed, terminal-claim-
       * purge, and QF-20260830-452's keyless directed-work) added at different times by different
       * SDs. Copies of a receipt call are chances to add a later branch and forget one — which is
       * precisely how this lane, and the contract enumerating it, ended up half-wired in the first
       * place. One door makes "acked but unrecorded" hard to write by accident. Defined here
       * (above the sdKey branch, not inside it) so the keyless carrier can share it too.
       *
      ...
      if (!sdKey && isKeylessDirectedWork(assignment)) {
        // No sd_key/assigned_sd to claim via claim_sd -- the payload itself IS the work spec.
        // Ack it (same terminal action as a fulfilled keyed assignment) so it stops being
        // re-selected, and hand the payload back for the worker to act on immediately.
        await ackWithReceipt(DISPOSITIONS.ACTIONED, { assignment_type: assignment.payload.assignment_type });
        ctx.base.directed_lane_verdict = { outcome: 'consumed', id: assignment.id, sd_key: null, reason: `keyless directed work: ${assignment.payload.assignment_type}` };
        return { ...ctx.base, action: 'directed_work', assignment_type: assignment.payload.assignment_type,
          work_spec: assignment.payload,
          message: `Claimed keyless directed work (assignment_type=${assignment.payload.assignment_type}) via WORK_ASSIGNMENT ${assignment.id}. No SD/QF claim row exists for this item — act on the payload as the work spec, then re-run /checkin.` };
      }
      if (sdKey) {
        ...
        // QF-20260905-060: resolve BEFORE classifying, so a leftover (non-live) harness worktree
        // cwd does not fail the repo-match axis for a directed venture-targeted assignment. A
        // GENUINELY live conflicting worktree still refuses -- this only removes the false positive.
        const cwdWorktreeIsLiveClaim = (!terminalStatus && assignedSdRow)
          ? await isCwdWorktreeLiveClaim(sb, process.cwd())
          : true;
        const allIneligibleReasons = (!terminalStatus && assignedSdRow)
          ? classifyAllDispatchIneligibility(assignedSdRow, { cwd: process.cwd(), cwdWorktreeIsLiveClaim })
          : [];
        ...
        if (assignedSdFetchFailed) {
          ...
          await markDeliveredOnly(assignment);
          ctx.base.assignment_claim_error = 'fitness_check_query_failed';
          ctx.base.directed_lane_verdict = { outcome: 'deferred', id: assignment.id, sd_key: sdKey, reason: 'fitness_check_query_failed' };
          ...
          ctx.base.directed_lane_verdict = { outcome: 'skipped', id: assignment.id, sd_key: sdKey, reason: `target_terminal:${terminalStatus}` };
          ...
          ctx.base.directed_lane_verdict = { outcome: 'skipped', id: assignment.id, sd_key: sdKey, reason: `ineligible:${ineligibleReason}` };
          ...
          ctx.base.directed_lane_verdict = { outcome: 'deferred', id: assignment.id, sd_key: sdKey, reason: `not_before:${qfDeferredUntil}` };
        } else {
          ...
          if (claimed.ok) {
            ...
            ctx.base.directed_lane_verdict = { outcome: 'consumed', id: assignment.id, sd_key: sdKey, reason: null };
          ...
          if (TERMINAL_CLAIM_ERRORS.has(claimed.error)) {
            ...
            ctx.base.directed_lane_verdict = { outcome: 'skipped', id: assignment.id, sd_key: sdKey, reason: `claim_terminal:${claimed.error}` };
          } else {
            // could not claim the assigned SD -> fall through to self-claim
            await markDeliveredOnly(assignment);
            ctx.base.assignment_claim_error = claimed.error;
            ctx.base.directed_lane_verdict = { outcome: 'deferred', id: assignment.id, sd_key: sdKey, reason: claimed.error };
          }
        }
      }
    }
