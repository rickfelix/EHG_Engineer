 ...
 * 7.05. resume-yield-fallback  (rung 5.02 — SD-LEO-INFRA-CHECKIN-DIRECTED-BEFORE-RESUME-001: if
 *                                resume yielded a rediscovered claim to a directed row this tick
 *                                and the claim attempt did NOT succeed, restore ctx.mySd and stop
 *                                here -- never let an unclaimed yield fall through to a self-claim
 *                                tier that could silently evict the still-held real claim)
 * 7.1. directed-assignment-    (rung 5.05 — QF-20260831-947 fork (b): a WORK_ASSIGNMENT just
 *      outranks-self-claim                purged as orchestrator_parent-ineligible, with the
 *                                parent independently completable, stops here instead of
 *                                falling through to self-claim)
...
module.exports = [
  ...
  // self-claim-gates (rung 5.9) so it takes effect on THIS SAME check-in tick.
  require('./dedicated-seat-standdown.cjs'),
  ...
  // rediscovered claim to a directed WORK_ASSIGNMENT this tick and the claim attempt did NOT
  // succeed (ctx.mySd still null -- a successful claim short-circuits before this step runs),
  // restore ctx.mySd and stop here. Positioned BEFORE directed-assignment-outranks-self-claim:
  // restoring a real, still-DB-held claim always takes priority over that narrower advisory hold.
  require('./resume-yield-fallback.cjs'),
  // QF-20260831-947 (fork b): if directed-assignment JUST purged a WORK_ASSIGNMENT as
  // orchestrator_parent-ineligible and the parent is independently completable, stop here
  // instead of falling through to the self-claim tiers below.
  require('./directed-assignment-outranks-self-claim.cjs'),
