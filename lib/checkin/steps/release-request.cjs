  ...
  async run(ctx) {
    ...
    try {
      ...
      for (const row of held || []) {
        ...
        // SD-LEO-FIX-STRATEGIC-DIRECTIVES-UPDATED-001: this used to read metadata, delete the
        // key from a LOCAL copy, then full-blob .update({metadata: cleared}) -- the
        // claiming_session_id guard protects against claim-stealing, but a DIFFERENT concurrent
        // stamper's key (claim_history, execution_context, ...) landing between this read and
        // this write would still be silently clobbered. removeMetadataKeyIfClaimedBy() deletes
        // ONLY release_request server-side under the same guard, touching no other key.
        const honored = row.metadata && row.metadata.release_request;
        const { removeMetadataKeyIfClaimedBy } = await import('../../coordinator/safe-metadata-merge.mjs');
        const removeFn = ctx.removeMetadataKeyIfClaimedByFn || removeMetadataKeyIfClaimedBy;
        const upd = await removeFn(row.id, 'release_request', sessionId);
        if (!upd.removed) continue;
        const { bestEffortReleaseSdByKey } = await import('../../fleet/best-effort-release.mjs');
        // SD-SCOPED (QF-20260726-593), CLOSED not narrowed (SD-LEO-INFRA-RELEASE-KEY-SESSION-001
        // FR-3). release_sd takes NO SD argument — it releases whatever claude_sessions.sd_key
        // currently points at. This loop walks up to 5 SDs claimed by this session, so an unscoped
        // call could clear the release_request flag on THIS row while releasing a DIFFERENT, live
        // SD. release_sd_by_key(session, row.sd_key, reason) moves that guard into a SQL-level CAS
        // under a row lock — the check and the release are one atomic statement, not the app-layer
        // read-then-act (expectedSdKey) this replaces, which only narrowed the race.
        const rel = await bestEffortReleaseSdByKey(sb, sessionId, row.sd_key,
          `release_request:${honored.reason || 'unspecified'}`, console.error);
