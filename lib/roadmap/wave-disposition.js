...
export async function applyWaveDisposition(supabase, { waveDisposition, sourceKey, title, dispositionSource }) {
  ...
  // QF-20260911-229 (preventive): sourceKey is sometimes an SD key that already exists at call
  // time (create-orchestrator-from-plan.js and sd-creation/pipeline.js both pass the just-created
  // SD's own sd_key). Stamping promoted_to_sd_key in THIS SAME write, rather than relying on a
  // later pass to link it, is what closes the register-first gap -- six live items proved the
  // later-pass assumption false (metadata.source_key named a real, even completed, SD while
  // promoted_to_sd_key sat NULL indefinitely). A sourceKey that is a workstream/question_key
  // label (decision-binding/disposition.js's callers) never matches an sd_key, so this is a
  // no-op for those -- the link is only ever made when the target SD genuinely exists.
  const { data: linkedSd } = await supabase
    .from('strategic_directives_v2').select('sd_key').eq('sd_key', sourceKey).maybeSingle();
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      ...(linkedSd ? { promoted_to_sd_key: linkedSd.sd_key } : {}),
