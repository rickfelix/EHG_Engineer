...
export async function computePlanCheckStatus(supabase, { windowHours = 48 } = {}) {
  ...
  if (waveIds.length > 0) {
    const { data: itemsData, error: itemsErr } = await supabase
      ...
      .select('id, wave_id, title, promoted_to_sd_key, item_disposition, priority_rank')
      .in('wave_id', waveIds);
    if (itemsErr) throw new Error(`plan-check-status: roadmap_wave_items query failed: ${itemsErr.message}`);
    items = itemsData || [];
  }
