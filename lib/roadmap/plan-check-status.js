...
export async function computePlanCheckStatus(supabase, { windowHours = 48 } = {}) {
  ...
  if (linkedSdKeys.length) {
    ...
    //
    // DELIBERATELY NOT SELECTED: unmet_dependencies, blocked_on_decision, owner_lane. MEASURED
    // 2026-08-07 they exist NOWHERE — not as columns on strategic_directives_v2 (all three
    // return 42703 undefined_column) and not as metadata keys (sampled 60 rows carrying
    // metadata; zero occurrences). Selecting a phantom column poisons the WHOLE projection, so
    // naming them here would have turned a working join into an empty one. QF-20260807-032
    // removed the drive-loop sections' dead read branches for all three (belt-diagnosis.js,
    // chain-to-gate.js, next-acts.js) rather than adding producers, so this exclusion is now
    // simply correct rather than a known gap — see the comment below.
  ...
  return {
    ...
    // untouched.
    //
    // RESOLVED (QF-20260807-032): the drive-loop sections used to also read sd.unmet_dependencies,
    // sd.owner_lane and sd.blocked_on_decision, guarded with Array.isArray()/optional-chaining/
    // ternary so the branches never fired — a guard that tolerates an absent input makes the
    // absence invisible. Decision: prune the dead branches (belt-diagnosis.js's classifyItem,
    // chain-to-gate.js's resolveOwner/resolveChain, next-acts.js's nextAct/ownerOf/ACTS) rather
    // than add producers for fields that exist nowhere (42703 as columns; absent from metadata
    // across a 60-row sample) — adding columns/a derived view is schema work, out of QF scope
    // and a bigger blast radius than three sections quietly implying a capability they lack.
    // This join supplies every field that actually exists.
