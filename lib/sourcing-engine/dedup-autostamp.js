 ...
 * @param {object} params
 * @param {object} params.supabase - service-role client
 * @returns {Promise<{ existingQfs:Array<{sd_key:string,title:string}>, completedQfKeys:Set<string> }>}
 */
export async function loadQfDedupContext({ supabase } = {}) {
  ...
  // Reuses findDedupMatch's `{sd_key, title}` shape (router.js) rather than a second matcher --
  // `sd_key` here holds a QF id (e.g. "QF-20260902-724"), not an SD key.
  const existingQfs = qfs.map((q) => ({ sd_key: q.id, title: q.title }));
