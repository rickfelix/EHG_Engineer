 ...
 *   FR-2: derive `outcomeRealizedKeys` — an SD's outcome is realized iff it has shipped
 *         (status==='completed') AND has at least one entry in its own delivers_capabilities
 *         column. SD-LEO-FIX-SOURCING-DEDUP-PRIORITY-001 replaced an earlier VDR-gauge-based
 *         derivation here with this simpler, directly-verifiable check: the gauge's ~20-entry
 *         portfolio-vision label vocabulary never overlaps with any per-SD capability key by
 *         construction, so the gauge-based join was always empty in production (see
 *         loadDedupContext's own inline comment for the full measurement). deriveOutcomeRealizedKeys
 *         below is UNCHANGED and still gauge-based — it remains the correct helper for
 *         lib/sourcing-engine/gauge-gap-miner.js's genuinely portfolio-level use; loadDedupContext
 *         no longer calls it.
 *   FR-3: persist lane + dedup_match_sd_key on conversion_ledger (columns already shipped).
 *
 * HONESTY / SAFE DEFAULT (FR-2): when an SD has no delivers_capabilities entry (true for most SDs
 * today, since capability registration is opt-in per SD type), it is treated as NOT realized, so
 * re_emit fires. That is the intent-aligned direction: never falsely CLOSE still-open realization work.
 *
 ...
 */
export async function loadDedupContext({ supabase } = {}) {
  let sds;
  try {
    sds = await fetchAllPaginated(() => supabase
      .from('strategic_directives_v2')
      .select('sd_key, title, status, delivers_capabilities')
  ...
  // SD-LEO-FIX-SOURCING-DEDUP-PRIORITY-001 (from QF-20260905-631): this safeguard's
  // outcomeRealizedKeys was ALWAYS empty in production. It read metadata.delivers_capabilities
  // (populated on 0 of 6213 SD rows -- the real signal is the TOP-LEVEL delivers_capabilities
  // column, populated on 121 of 6213, maintained by trigger fn_handle_capability_lifecycle) and
  // only accepted string entries (the real shape is objects), then joined the result against the
  // VDR portfolio-vision gauge's ~20-entry human-readable label vocabulary (e.g. "The cockpit") --
  // verified live to have ZERO overlap with any of the 206 distinct sd_capabilities.capability_key
  // slugs (e.g. "gate0-workflow-enforcement"): two different namespaces that were never meant to
  // join by string equality. deriveOutcomeRealizedKeys/computeBuildGauge are left completely
  // untouched here -- lib/sourcing-engine/gauge-gap-miner.js:245 depends on the same gauge-based
  // contract for an unrelated, genuinely portfolio-level purpose. For THIS safeguard, "realized" is
  // simpler than a gauge lookup: a shipped SD (status='completed') that has at least one entry in
  // its own real, populated delivers_capabilities column.
  const outcomeRealizedKeys = new Set(
    sds
      .filter((s) => s.status === 'completed' && Array.isArray(s.delivers_capabilities) && s.delivers_capabilities.length > 0)
      .map((s) => s.sd_key),
  );
