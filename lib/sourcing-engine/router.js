...
export const DEFAULT_JACCARD_THRESHOLD = 0.8;
...
// QF-20260906-868: exported so a caller (checkAlreadyBuilt's cite-or-NOT-FOUND guard) can
// validate a match's score against the SAME type-specific floor this module already enforces,
// rather than duplicating the number or -- worse -- checking a semantic match's looser score
// against the stricter jaccard floor (DEFAULT_JACCARD_THRESHOLD), which would reject legitimate
// semantic matches this module already deems trustworthy (measured regression risk: 0.76-score
// semantic matches this repo's own pinned tests rely on).
export const SEMANTIC_OVERLAP_COEFF_THRESHOLD = 0.6; // >=60% of the smaller problem-key's tokens must be shared
 ...
 *
 * QF-20260903-254: exported so a caller can reuse this SAME title/semantic matcher against a
 * different corpus shaped as `{sd_key, title}` (e.g. the quick_fixes lane, keyed by qf id) --
 * the matcher itself has no SD-specific behavior, only the field name is a holdover.
 * @returns {{ sd_key:string, reason:'source_id'|'exact_title'|'jaccard'|'semantic', score?:number }|null}
 */
export function findDedupMatch(item, existing, threshold) {
