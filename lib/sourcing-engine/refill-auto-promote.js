...
// SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-3): reuse the SAME work-class classifier Fable's
// own claim fence already uses (lib/fleet/work-class.cjs) — "Fable-heavy" here means
// deriveWorkClass(row) === 'creative_design', the exact class Fable is fenced TO. No new heuristic.
import { deriveWorkClass } from '../fleet/dispatch-suggestions.cjs';
...
/**
 * SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-3): reorder valid candidates so creative_design
 * ("Fable-heavy") rows sink BELOW non-creative_design rows in a lean-Fable week — applied BEFORE
 * the `.slice(0, limit)` cap below, so a lean week's cap is spent on work Fable is not fenced to.
 *
 * PURE + TOTAL, mirroring selectRefillBatch's own contract (:14-15) — no DB/fs/clock, and it must
 * degrade byte-identically to today's order when `quotaPosture` is absent (TR-3 fail-safe): a
 * caller that never opts in sees no behavior change, exactly like `opts.demand`'s existing idiom.
 * Stable partition (Array.prototype.sort is stable in V8/Node), so within each partition rows keep
 * their pre-existing relative order — only creative_design rows move, and only relative to
 * non-creative_design rows.
 * @param {Array<Object>} rows
 * @param {{leanFable?: boolean}|null|undefined} quotaPosture
 * @returns {Array<Object>}
 */
export function reorderByQuotaPosture(rows, quotaPosture) {
  const list = Array.isArray(rows) ? rows : [];
  if (!quotaPosture || quotaPosture.leanFable !== true) return list;
  return [...list].sort((a, b) => {
    const aHeavy = deriveWorkClass(a) === 'creative_design' ? 1 : 0;
    const bHeavy = deriveWorkClass(b) === 'creative_design' ? 1 : 0;
    return aHeavy - bHeavy; // non-heavy (0) sorts before heavy (1); stable within each group
  });
}
...
export function selectRefillBatch(rows, opts = {}) {
  ...
  const report = verifyStagedCandidates(rows, { shippedTitleSet: opts.shippedTitleSet, distilledOnly: opts.distilledOnly === true, acceptedFingerprintSet: opts.acceptedFingerprintSet });
  // SD-LEO-INFRA-INTELLIGENT-ROUTING-RANK-001 (FR-3): reorder BEFORE the cap so a lean-Fable week's
  // limited slice is spent on non-Fable-heavy work first. opts.quotaPosture is the SAME
  // injected-value idiom as opts.demand above — absent by default, so every existing caller/test is
  // byte-identical (TR-3).
  const ordered = reorderByQuotaPosture(report.valid, opts.quotaPosture);
  return {
