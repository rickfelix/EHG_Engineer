...
// SD-LEO-INFRA-PRIORITY-RECORD-ONE-001-D — the non-terminal scope split.
//
// WHY THIS EXISTS: countRoadmapLinkExceptions above tallies the FULL historical corpus (6,141+
// rows, 455+ completed). A CI gate asserting without_reason===0 against that denominator can never
// pass — most of the corpus predates this module and will never be revisited. The live, actionable
// surface is the NON-TERMINAL belt (draft/active/in_progress/pending_approval/etc): rows that can
// still be acted on. This function delegates to countRoadmapLinkExceptions TWICE — once over all
// rows, once over the non-terminal subset — so the two figures can never diverge in tally logic;
// it never reimplements the reason_supplied===true predicate.
export const TERMINAL_SD_STATUSES = [
  'completed', 'cancelled', 'archived', 'superseded', 'deferred', 'rejected',
];

/**
 * Classify the SHAPE of one row's roadmap_link_exception value. Distinct from
 * countRoadmapLinkExceptions' tally, this names WHICH writer defect produced a reasonless
 * reading, so a health probe or CI failure can say why, not just how many:
 *   - 'canonical_reasoned'    : {reason_supplied:true, ...} — nothing wrong.
 *   - 'no_reason_marker'      : {reason_supplied:false, ...} — a genuine no-reason mint.
 *   - 'bare_string'           : the value is a plain string, not the builder's object shape
 *                                (e.g. .artifacts/michael-002-fences-20260905.mjs:30) — a REAL
 *                                reason the strict reader cannot see.
 *   - 'malformed_object'      : an object missing the reason_supplied key entirely.
 */
export function classifyExceptionShape(ex) {
  if (typeof ex === 'string') return ex.length > 0 ? 'bare_string' : 'malformed_object';
  if (ex && typeof ex === 'object') {
    if (ex.reason_supplied === true) return 'canonical_reasoned';
    if (ex.reason_supplied === false) return 'no_reason_marker';
...
/**
 * Split countRoadmapLinkExceptions into {all, non_terminal}, plus a shapes breakdown scoped to
 * non_terminal only (the actionable set). `terminalStatuses` is injectable for tests; production
 * callers should omit it and take TERMINAL_SD_STATUSES.
 */
export function countRoadmapLinkExceptionsByScope(rows, { terminalStatuses = TERMINAL_SD_STATUSES } = {}) {
  const allRows = rows || [];
  const terminalSet = new Set(terminalStatuses);
  const nonTerminalRows = allRows.filter((row) => !terminalSet.has(row && row.status));

  const shapes = { canonical_reasoned: 0, no_reason_marker: 0, bare_string: 0, malformed_object: 0 };
  for (const row of nonTerminalRows) {
    const ex = row && row.metadata && row.metadata.roadmap_link_exception;
    if (!ex) continue;
    shapes[classifyExceptionShape(ex)] += 1;
  }

  return {
    all: countRoadmapLinkExceptions(allRows),
    non_terminal: { ...countRoadmapLinkExceptions(nonTerminalRows), shapes },
  };
}

export default {
  buildRoadmapLinkException, countRoadmapLinkExceptions, NO_REASON_MARKER, REASON_MAX_CHARS,
  TERMINAL_SD_STATUSES, classifyExceptionShape, countRoadmapLinkExceptionsByScope,
};
