...
import { findDedupMatch, DEFAULT_JACCARD_THRESHOLD, SEMANTIC_OVERLAP_COEFF_THRESHOLD } from './router.js';

/**
 * QF-20260906-868 (cite-or-NOT-FOUND, fail-closed): is a findDedupMatch() result trustworthy
 * enough to publish a hard ALREADY-BUILT? Two independent guards, either failing means NOT-FOUND:
 *   (1) the match must actually be citable by key (defensive -- findDedupMatch never returns a
 *       keyless match today, but a citation-less "match" must never silently pass as confident).
 *   (2) a SCORED match (jaccard/semantic) must clear the SAME type-specific floor findDedupMatch
 *       itself already enforces -- checked here TYPE-AWARE, never a single blanket number, because
 *       the semantic path's own floor (0.6) is deliberately looser than the jaccard path's (0.8;
 *       see router.js's PRECISION-FIRST comment) and a legitimate 0.6-0.79 semantic match must
 *       never be rejected just for failing the STRICTER jaccard bar. An unscored match
 *       (source_id/exact_title -- deterministic, "exact beats fuzzy") is always citable.
 * Pure + exported so this is directly unit-testable on hand-built match objects, independent of
 * whether the real matcher can currently produce a below-floor result (it cannot today -- this is
 * defense-in-depth against either threshold ever being loosened, not a workaround for a reachable
 * bug in findDedupMatch itself).
 * @param {{sd_key:string, reason:string, score?:number}|null} match
 * @returns {boolean}
 */
export function isCitableMatch(match) {
  if (!match || !match.sd_key) return false;
  if (typeof match.score !== 'number') return true; // source_id / exact_title: deterministic
  const floor = match.reason === 'semantic' ? SEMANTIC_OVERLAP_COEFF_THRESHOLD : DEFAULT_JACCARD_THRESHOLD;
 ...
 * that QF via `citedQfKey` -- if the SD lane also had a non-terminal re-emit signal for the same
 * candidate, its `citedSdKey` is preserved alongside (informational: the QF decides result/re_emit
 * here, but the related SD reference is not silently discarded). Both lane reads are FAIL-LOUD
 * (loadDedupContext/loadQfDedupContext both throw on a read error) rather than silently returning
 * a clean NOT-FOUND -- a lane that cannot be searched must never look identical to a lane that was
 ...
 *   reason: (string|null),
 *   re_emit: boolean,
 *   nearMiss: ({key:string, score:(number|undefined), reason:string}|null)
 * }>}
 */
export async function checkAlreadyBuilt({ supabase, io, title, description } = {}) {
  ...
  if (sdResult && sdResult.result === 'ALREADY-BUILT') {
    return { predicate: 'was-this-built', citedQfKey: null, nearMiss: null, ...sdResult };
  }
  ...
  const qfMatchCitable = isCitableMatch(qfMatch);
  if (qfMatch && completedQfKeys.has(qfMatch.sd_key) && qfMatchCitable) {
    // QF-FOLLOWUP-254-DROPPED-REEMIT (adversarial review of QF-20260903-254): this branch used
    // to hardcode citedSdKey:null, discarding a live SD re_emit signal whenever a completed QF
    // ALSO matched -- converting "re-emit as outcome work against this SD, do not mint new" into
    // a bare terminal "already built", with no trace of the SD the outcome work belongs to.
    // Preserve it as informational context; the QF match still decides result/re_emit here.
    return {
      predicate: 'was-this-built',
      result: 'ALREADY-BUILT',
      citedSdKey: sdResult ? sdResult.citedSdKey : null,
  ...
  // A completed-QF match existed but failed the citation floor above -- fail closed to
  // NOT-FOUND, but surface it as nearMiss so a human still sees what the router found.
  const nearMiss = (qfMatch && completedQfKeys.has(qfMatch.sd_key) && !qfMatchCitable)
    ? { key: qfMatch.sd_key, score: qfMatch.score, reason: qfMatch.reason }
    : null;
  ...
  return sdResult
    ? { predicate: 'was-this-built', citedQfKey: null, nearMiss, ...sdResult }
    : { predicate: 'was-this-built', result: 'NOT-FOUND', citedSdKey: null, citedQfKey: null, reason: null, re_emit: false, nearMiss };
}
