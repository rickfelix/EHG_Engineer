...
export async function addToBurstGroup(error, burstGroup) {
  ...
  // Update the group metadata. QF-20260912-316: `title` is intentionally excluded here --
  // it is a genuine content column the append-only freeze trigger
  // (database/chairman-gated/20260907_feedback_immutability_trigger.sql) keeps frozen
  // regardless of the separate lifecycle-column allowlist migration's outcome
  // (SD-LEO-INFRA-FEEDBACK-LIFECYCLE-UPDATE-ALLOWLIST-001), so writing it here would always
  // fail this UPDATE. burst_count/burst_last_seen/grouped_item_ids already track occurrence
  // count in metadata -- no consumer parses the "(Nx)" suffix out of title.
