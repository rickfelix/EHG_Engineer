 ...
 */
async function createIgnorePattern(pattern) {
...
async function createIgnorePatternsTable() {
  const { error } = await supabase.rpc('exec_sql', {
    sql: `
      CREATE TABLE IF NOT EXISTS feedback_ignore_patterns (
      ...
      CREATE INDEX IF NOT EXISTS idx_fip_active ON feedback_ignore_patterns(is_active) WHERE is_active = true;
    `
  });

  if (error) {
    console.warn('Could not create table via RPC, table may already exist:', error.message);
  }
}
 ...
 */
async function getActivePatterns(forceRefresh = false) {
  const now = Date.now();
 ...
 */
async function matchesIgnorePattern(feedback) {
 ...
 */
function checkMatch(value, type, pattern) {
 ...
 */
async function autoIgnoreFeedback(feedbackId, pattern) {
  const { data, error } = await supabase
    .from('feedback')
    .update({
      status: 'ignored',
 ...
 */
async function processFeedbackForIgnore(feedback) {
