...
import { evaluateQuarantine, createQuarantineRecord, QUARANTINE_STATUS } from './quarantine-engine.js';
...
export async function processQuality(feedback, options = {}) {
  ...
  try {
    ...
    // Step 1: Sanitization
    if (!options.skipSanitization && config.enable_sanitization) {
    ...
    if (!options.skipQuarantine && config.enable_quarantine && result.sanitization) {
