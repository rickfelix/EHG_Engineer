...
// QF-LEO-INFRA-VENTURE-JOURNEY-UAT-001-adjacent fix: this was constructed eagerly at module
// scope, so merely IMPORTING this file (not calling anything in it) threw whenever Supabase
// env vars were absent -- newly reachable from lib/eva/stage-templates/index.js's barrel via
// SD-LEO-INFRA-VENTURE-JOURNEY-UAT-001's result-recorder.js -> journey-walk-orchestrator.js
// -> db-sourced-findings.js import chain, which the barrel's credential-free smoke check caught.
let _supabase;
...
export async function updateFeedbackPriority(feedbackId) {
  ...
  const { data: feedback, error: fetchError } = await getSupabase()
    .from('feedback')
    .select('*')
    .eq('id', feedbackId)
    .single();
...
export async function recalculateAllPriorities() {
  let items;
  try {
    items = await fetchAllPaginated(() => getSupabase()
      .from('feedback')
      .select('id')
