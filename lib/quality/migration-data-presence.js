...
// QF-20260905-161: 22P02 (invalid_text_representation) / 22023 (invalid_parameter_value) mean
// the regex-extracted "literal" doesn't match the target column's real type -- a signal the
// naive INSERT parse matched the wrong kind of SQL (e.g. a plpgsql function body), not that the
// query itself failed to execute. Fail open on these, same as the other genuine parser-shape
// limitations above, instead of throwing.
const PARSER_LIMITATION_SQLSTATES = new Set(['22P02', '22023']);
...
export async function checkMigrationDataPresent(supabase, migrationRelPath) {
  ...
  if (error) {
    if (PARSER_LIMITATION_SQLSTATES.has(error.code)) return null;
    throw error;
  }
