...
async function getSnoozedItems(options = {}) {
  const buildQuery = () => {
    // metadata->snooze->>active = 'true' scopes this to rows THIS mechanism snoozed -- never a
    // row sharing snoozed_until for an unrelated reason (see wakeExpiredSnoozes above).
    let query = supabase
      .from('feedback')
      .select('*')
      .eq('metadata->snooze->>active', 'true')
