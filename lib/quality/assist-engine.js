...
import { NON_CODE_FIXABLE_CATEGORIES, filterIssuesExcludingNonCodeFixable } from '../governance/non-code-fixable-categories.cjs';
...
/**
 * SD-FDBK-ENH-LEO-ASSIST-PHASE-001: fleet-ops telemetry categories that write
 * type='enhancement' feedback rows but are not genuine, schedulable enhancement
 * requests -- routine fleet-operations records (worker wind-down surveys, coordinator/
 * Adam/Solomon review and self-assessment output, deferred chairman decisions).
 *
 * First pass measured live 2026-08-19 (v_feedback_with_sensemaking, type=enhancement,
 * unresolved, LIMIT 5000 with no ORDER BY -- an unordered, non-representative "sample"
 * of the first 1000 rows Postgres happened to return) after Golf-6 signal 2cb34b71
 * reported the Phase 2 enhancements stream flooded to 3477 rows: wind_down_survey=902
 * (dominant in that sample), coordinator_review=7, coordinator_adam_review=2,
 * adam_self_assessment=2, solomon_self_assessment=2, solomon_adherence_drift=1,
 * chairman_decision_deferred=1.
 *
 * SECOND PASS (same day, EXEC-phase live smoke check per this SD's own TS-7/FR-4
 * requirement): the unordered first-pass sample undercounted -- a post-fix live re-run
 * against the FULL population showed 854 rows still remaining, 95% of which were 5
 * categories entirely absent from the first pass: fleet_retro=652 ("Fleet retro —
 * <session>:<timestamp>"), fleet_dormancy=71 ("Fleet dormancy: N worker(s) armed a
 * wakeup that never fired"), model_capability_baseline=40 ("[SEALED FABLE5 RUN]
 * FABLE5-BASELINE-..."), feature_flag_governance=36 ("Stale feature flags (N) —
 * <date>"), solomon_forecast_basis=15 ("Solomon forecast basis vNN - ..."). Each
 * confirmed by direct title inspection to be automated fleet-ops/governance telemetry,
 * the same defect class as the first-pass 7, not genuine enhancement requests --
 * shipping only the first-pass 7 would have left the SD's own success metric
 * ("reduced to a small double/low-triple-digit count") unmet (854, not double/low-
 * triple-digit). Added rather than deferred to a follow-up, since the fix mechanism
 * (add to this Set) is identical and the evidence was already in hand.
 *
 * Deliberately separate from NON_CODE_FIXABLE_CATEGORIES (lib/governance/non-code-
 * fixable-categories.cjs): that Set's filterIssuesExcludingNonCodeFixable() is hard-gated
 * to type==='issue' by design (SD-LEO-INFRA-EXCLUDE-MONITORING-TELEMETRY-001 scoped
 * itself to the issues stream only), so it structurally cannot apply here regardless of
 * caller. Static list, not a dynamic taxonomy -- same drift-and-patch residual risk the
 * sibling issues-stream filter already carries (see QF-20260818-390), and this SD's own
 * second pass is a live instance of that exact drift-and-patch cycle occurring same-day.
 */
const FLEET_OPS_TELEMETRY_CATEGORIES = new Set([
  'wind_down_survey',
  'coordinator_review',
...
/**
 * SD-FDBK-ENH-LEO-ASSIST-PHASE-001: exclude fleet-ops telemetry categories from the
 * enhancements stream, chained immediately after splitEnhancementsExcludingHarnessBacklog
 * so the two exclusions compose rather than race. Toggleable via
 * LEO_ASSIST_ENH_OPSFILTER=false for emergency rollback, matching the
 * LEO_ASSIST_NONCODE_FILTER precedent already used for the issues-stream filter.
 *
 * The type==='enhancement' gate is re-asserted here rather than trusted from the caller
 * (matching lib/governance/non-code-fixable-categories.cjs's stated principle: "the type
 * gate is enforced here, not left to caller discipline") -- even though the current call
 * site only ever passes the already-enhancements-only output of
 * splitEnhancementsExcludingHarnessBacklog, this keeps the function safe to call directly.
 *
 * @param {Array<object>} enriched - Sensemaking-enriched feedback rows (or an already
 *   type='enhancement'-filtered subset, e.g. splitEnhancementsExcludingHarnessBacklog's output)
 * @returns {{enhancements: Array<object>, skippedFleetOps: number}}
 */
function filterEnhancementsExcludingFleetOps(enriched) {
  const all = (enriched || []).filter(i => i.type === 'enhancement');
  const kept = all.filter(i => !FLEET_OPS_TELEMETRY_CATEGORIES.has(i.category));
  const skippedFleetOps = all.length - kept.length;
  return { enhancements: kept, skippedFleetOps };
}
  ...
  async loadInboxItems() {
    ...
    try {
      items = await fetchAllPaginated(() => supabase
        ...
        // SD-LEO-INFRA-AUDIT-FIX-FEEDBACK-001-C: lib/quality/snooze-manager.js's /inbox snooze
        // never touches status (only snoozed_until + metadata.snooze), so an actively-snoozed
        // item was invisible to the exclude-list above and resurfaced here immediately. Exclude
        // any row whose snoozed_until is still in the future; an EXPIRED snooze (past, not yet
        // woken by wakeExpiredSnoozes()) is deliberately NOT excluded -- it behaves as unsnoozed.
    ...
    const { issues: decisionFiltered, skippedNeedsDecision } = filterIssuesExcludingNeedsDecision(enriched);
    ...
    // SD-LEO-INFRA-EXCLUDE-MONITORING-TELEMETRY-001: exclude non-code-fixable monitoring/
    // telemetry categories from the issues stream. CHAINED after filterIssuesExcludingNeedsDecision
    // (never run independently/in parallel) -- decisionFiltered is already all type==='issue', so
    // this second filter's own type gate is an idempotent pass-through, and completion_flag rows
    // already excluded above cannot be reintroduced. Toggleable via LEO_ASSIST_NONCODE_FILTER=false
    // for emergency fleet-wide rollback without a code revert.
    let issues = decisionFiltered;
    if (process.env.LEO_ASSIST_NONCODE_FILTER !== 'false') {
      const { issues: nonCodeFiltered, skippedNonCodeFixable } = filterIssuesExcludingNonCodeFixable(decisionFiltered);
      issues = nonCodeFiltered;
      if (skippedNonCodeFixable > 0) {
        console.log(`[assist-engine] Excluded ${skippedNonCodeFixable} non-code-fixable monitoring/telemetry row(s) from Phase 1 autonomous-fix stream (SD-LEO-INFRA-EXCLUDE-MONITORING-TELEMETRY-001)`);
      }
    } else {
      console.log('[assist-engine] non-code-fixable filter disabled via LEO_ASSIST_NONCODE_FILTER');
    }

    const { enhancements: harnessFiltered, skippedHarnessBacklog } = splitEnhancementsExcludingHarnessBacklog(enriched);
    ...
    // SD-FDBK-ENH-LEO-ASSIST-PHASE-001: exclude fleet-ops telemetry categories from the
    // enhancements stream. CHAINED after splitEnhancementsExcludingHarnessBacklog (never
    // run independently/in parallel), matching the issues-stream filter-chaining convention
    // a few lines above. Toggleable via LEO_ASSIST_ENH_OPSFILTER=false for emergency
    // fleet-wide rollback without a code revert.
    let enhancements = harnessFiltered;
    if (process.env.LEO_ASSIST_ENH_OPSFILTER !== 'false') {
      const { enhancements: opsFiltered, skippedFleetOps } = filterEnhancementsExcludingFleetOps(harnessFiltered);
      enhancements = opsFiltered;
      if (skippedFleetOps > 0) {
        console.log(`[assist-engine] Excluded ${skippedFleetOps} fleet-ops telemetry row(s) from enhancements stream (SD-FDBK-ENH-LEO-ASSIST-PHASE-001)`);
      }
    } else {
      console.log('[assist-engine] fleet-ops filter disabled via LEO_ASSIST_ENH_OPSFILTER');
    }
