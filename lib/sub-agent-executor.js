...
export async function executeSubAgent(code, sdId, options = {}) {
  ...
  try {
    ...
    if (useContract) {
      ...
      const objective = options.objective || `Execute ${subAgent.name} analysis for SD ${sdId}. ${subAgent.description || ''}`;
      ...
      try {
        taskContract = await createTaskContract(code, objective, {
          ...
          input_summary: `Sub-agent ${code} for ${sdId} | Phase: ${sdPhase || 'unknown'} | Model: ${recommendedModel}`,
          constraints,
          priority: subAgent.priority || 50,
          max_tokens: 4000
        });
    ...
    if (taskContract) {
      ...
      console.log('────────────────────────────────────────────────────────────────');
      console.log(`Target: ${code} | SD: ${sdId} | Phase: ${sdPhase || 'unknown'}`);
    ...
    try {
      ...
      if (subAgentModule.execute) {
        ...
        const execOptions = taskContract
          ? { ...options, taskContract, inputArtifacts }
          : options;
        results = await subAgentModule.execute(sdId, subAgent, execOptions);
      } else {
  ...
  if (PRD_LINKABLE_SUBAGENTS.includes(code)) {
    try {
      ...
      if (!prdErr && prd) {
        ...
        if (!updateErr) {
          console.log(`   🔗 Linked ${code} results to PRD metadata.${metadataField}`);
        } else {
