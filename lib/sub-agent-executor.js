...
const VALIDATION_MAX_RETRIES = parseInt(process.env.LEO_VALIDATION_MAX_RETRIES || '2', 10);
...
const SUB_AGENT_CATEGORY_MAPPING = {
  ...
  'REGRESSION': ['refactoring', 'testing', 'code_structure']  // LEO v4.3.3
};
...
export async function storeSubAgentResults(code, sdId, subAgent, results, options = {}) {
  ...
  const safeMetadata = (() => {
    ...
    const { findings, sub_agent_results, ...rest } = results.metadata;
