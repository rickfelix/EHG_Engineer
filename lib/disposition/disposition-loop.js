...
/**
 * Wire the loop's dependencies to real, existing primitives. Not invoked automatically by
 * this SD — production auto-invocation wiring (deciding WHEN a defect enters the loop) is a
 * separate integration decision left for the caller / a follow-on SD; this factory only
 * proves the loop composes correctly over real primitives.
 *
...
export function createDefaultDispositionDeps(supabase, sdId) {
  return {
    ...
    revise: async () => {
      // No generic "fix" primitive exists — revision is domain-specific and left to the
      // caller to inject when wiring this loop to a concrete defect class.
    },
