...
/**
 * Pure: did `content` actually land under `## sectionName` in `markdown`? Used by
 * updateSectionVerified() to assert a write succeeded instead of trusting the boolean
 * updateSection() returns, which reports true even when the regex silently fails to
 * match anything to replace (QF-20260903-992 -- a prepare step that silently writes
 * nothing is worse than no prepare at all).
 */
export function sectionLanded(markdown, sectionName, content) {
  const escaped = String(sectionName).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // QF-20260907-330: no 'm' flag. With 'm', bare $ matches before EVERY internal newline, so
  // this lazy [\s\S]*? group stopped at the section's FIRST line for any multi-paragraph
  // content instead of the true next-section boundary or end of file -- a multi-line write
  // that landed correctly still read back as "truncated" (only line 1 captured).
  const match = String(markdown || '').match(new RegExp(`## ${escaped}\\n([\\s\\S]*?)(?=\\n## |$)`));
  const expected = String(content || '').trim();
  if (!match) return expected.length === 0;
  return match[1].trim().startsWith(expected.slice(0, 200));
}
...
class MemoryManager {
  ...
  async updateSection(sectionName, content) {
    try {
      ...
      // QF-20260907-330: no 'm' flag -- same bug as sectionLanded() above. With 'm', bare $
      // matches before every internal newline, so replacing an EXISTING multi-line section
      // only cleared its first line, leaving the rest of the old section as orphaned,
      // untitled leftover text below the new write.
  ...
  /**
   * Like updateSection(), but re-reads the file afterward and THROWS if the content did
   * not actually land, rather than returning a boolean the caller may not check. For a
   * prepare step ahead of context compaction, an unverified write is indistinguishable
   * from a silent no-op -- this makes that failure loud (QF-20260903-992).
   */
  async updateSectionVerified(sectionName, content) {
    // updateSection() requires the file to already exist (startSession() usually creates
    // it); bootstrap a minimal skeleton first rather than failing "write failed" on the
    // ordinary case of a worktree that never called startSession().
    try {
      await fs.access(this.sessionFile);
    } catch {
      await fs.mkdir(this.memoryDir, { recursive: true });
      await fs.writeFile(this.sessionFile, `# LEO Protocol Session State\n\n**Last Updated**: ${new Date().toISOString()}\n`, 'utf8');
    }
    const ok = await this.updateSection(sectionName, content);
    if (!ok) throw new Error(`updateSectionVerified: write failed for section "${sectionName}"`);
    const readBack = await fs.readFile(this.sessionFile, 'utf8');
    if (!sectionLanded(readBack, sectionName, content)) {
      throw new Error(`updateSectionVerified: read-back check failed for section "${sectionName}" -- write did not land`);
    }
    return true;
  }
