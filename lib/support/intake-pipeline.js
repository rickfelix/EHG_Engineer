 ...
 *
 * contentId is NOT ticket.ticket_id (SECURITY finding, EXEC-TO-PLAN evidence 41066290-0688-
 * 474b-b63c-4f4d4bc5e4c3): normalizeSupportTicket() prefers a CALLER-SUPPLIED ticket_id/id over
 * the derived content hash (for legitimate webhook-redelivery idempotency on the DB row) -- using
 * that raw value here would let a submitter reuse an already-`accepted` content_ref for entirely
 * different content, resolving to allowed:true for a reply the chairman never actually approved.
 * contentId is instead derived from the ACTUAL staged content (resolutionNotes) + venture_id, so
 * two different replies can never collide on the same accepted-approval lookup regardless of what
 * ticket_id the caller supplied.
 */
async function stageReplyDelivery(supabase, ticket, resolutionNotes) {
  if (!ticket.venture_id) return { staged: false, reason: 'no venture_id' };
  try {
    const contentId = createHash('sha256').update(`${ticket.venture_id}|${resolutionNotes}`).digest('hex').slice(0, 32);
...
export async function disposeSupportTicket(supabase, ticket, triage) {
  ...
  if (triage.routing_decision === 'auto_resolve') {
    ...
    if (recordedId) {
      ...
      const staging = await stageReplyDelivery(supabase, ticket, resolutionNotes);
