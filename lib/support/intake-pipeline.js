...
export async function craftSupportReply(ticket = {}, triage = {}) {
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error(`craftSupportReply: exceeded ${CRAFT_TIMEOUT_MS}ms`)), CRAFT_TIMEOUT_MS)),
    ]);
  } catch {
    return null;
  }
 ...
 * ZERO send-capable code exists in this module or its imports (no SMTP, resend, twilio, or
 * sendgrid call anywhere). This holds regardless of what checkPublishAuthorization() itself returns (it
 * CAN return allowed:true for an autonomous/pre-accepted channel) -- there is nothing downstream
 * in this module that acts on that result to actually deliver anything to a real human.
 *
