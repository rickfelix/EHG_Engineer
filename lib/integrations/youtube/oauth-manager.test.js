...
describe('oauth-manager encrypt-at-rest', () => {
  ...
  it('storeTokens never writes a plaintext token-shaped string to source_metadata', async () => {
    ...
    // TESTING mutation-pass finding (row 5fc91c90-b504-4adf-939c-3559ee2c5fcb): a nondeterministic-
    // but-fake "encryptor" (random nonce + base64, no real key/auth-tag) would still satisfy every
    // assertion above AND the different-ciphertext-per-call test below, since randomness alone
    // produces varying output without providing any real confidentiality. Pinning the reported
    // algorithm closes that residual gap without needing to inspect raw ciphertext bytes.
    expect(stored.source_metadata.encrypted_tokens.metadata.algorithm).toBe('aes-256-gcm');
