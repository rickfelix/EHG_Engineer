...
export function mapVideoToIntakeRow(item, videoDetail = {}) {
  ...
  // SECURITY sub-agent finding C-1 (EXEC-TO-PLAN): the chairman's rider promotes the playlist
  // ID to low-grade-secret status, but a playlistItems.list snippet always includes it -- storing
  // `item` verbatim would persist it in plaintext into a table any authenticated JWT can SELECT.
  // No consumer reads snippet.playlistId (the target playlist is already known from config), so
  // it is dropped rather than the whole raw_data payload being restricted.
  const { playlistId: _playlistId, ...snippetWithoutPlaylistId } = snippet;
  const sanitizedItem = { ...item, snippet: snippetWithoutPlaylistId };
  return {
    ...
    raw_data: { playlistItem: sanitizedItem, videoDetail }
  };
}
...
export async function syncYouTube(options = {}) {
  ...
  try {
    ...
    if (!playlist) {
      ...
      // SECURITY sub-agent finding C-2 (EXEC-TO-PLAN): without this, a privacy flip-back to
      // Private is invisible outside CI -- totalErrors stays 0 and the circuit breaker (which
      // keys on consecutive_failures) never arms for a manual `npm run eva:ideas:sync` run.
      if (!dryRun) {
        await updateSyncState(supabase, TARGET_PLAYLIST_NAME, 0, 'playlist not found / not key-readable');
      }
      return results;
    }
