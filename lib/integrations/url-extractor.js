...
export function extractYouTubeVideoId(text) {
  ...
  // Require at least one digit or special char to avoid matching English words
  // (e.g., "dynamically", "appropriate" are 11 chars but not video IDs)
  const lower = s.toLowerCase();
  if (lower.includes('youtube') || lower.includes('youtu')) {
    const bare = s.match(/\b([A-Za-z0-9_-]{11})\b/g);
    if (bare) {
      const validId = bare.find(id => /[\d_-]/.test(id) && !/^[a-zA-Z]+$/.test(id));
