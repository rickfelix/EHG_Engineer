 ...
 *
 * Used by the FULL-CREATE path (draft roadmap from scratch) only — `limit` here is a
 * deliberate, explicit sampling cap (combined, todoist-first, oldest-first), not a safety
 * net. Do NOT reuse this for incremental assignment: see loadNewIntakeItems, which must see
 * every classified row regardless of table size (QF-20260911-117).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {Object} [options]
 * @param {string} [options.application] - Filter by target_application
 * @param {number} [options.limit] - Max items to return (default: 500), applied AFTER
 *   concatenating both tables oldest-first (todoist, then youtube) — a deliberate cap for
 *   the full-create path, not a truncation bug.
...
const INTAKE_SELECT_COLUMNS = 'id, title, description, target_application, target_aspects, chairman_intent, classification_confidence, created_at';
 ...
 *
 * QF-20260911-117: must page ALL classified rows per table (fetchAllPaginated, no 500-row
 * cap) before filtering — loadClassifiedIntakeItems's combined-then-slice(0, 500) silently
 * dropped every classified row past the oldest 500 combined, so once eva_todoist_intake +
 * eva_youtube_intake crossed 500 rows, newly-classified items (all YouTube, since it's the
 * newer table) never reached this loader at all. Also excludes chairman_intent='reference'
 * (and null/unreviewed) items — the distill skill's own contract says reference rows are
 * "stored, excluded from wave clustering," but neither loader previously enforced that.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {Object} [options]
 * @param {string} [options.application] - Filter by target_application
 * @param {number} [options.limit] - Max items to return (default: 500), applied AFTER the
 *   assigned/reference filter, newest-first (so a caller-side cap keeps the most recent
 *   unassigned work, not an arbitrary oldest-first slice).
 * @returns {Promise<Array>}
 */
export async function loadNewIntakeItems(supabase, options = {}) {
  const limit = options.limit || 500;

  // Get all source_ids already assigned to a wave (uncapped — this already used
  // fetchAllPaginated before this fix, so it was never the source of the truncation).
  let assigned;
  try {
  ...
  const buildQuery = (table) => () => {
    let q = supabase
      .from(table)
      .select(INTAKE_SELECT_COLUMNS)
      .not('classified_at', 'is', null)
      .order('created_at', { ascending: true });
    if (options.application) q = q.eq('target_application', options.application);
    return q;
  };

  let todoist = [];
  try {
    todoist = await fetchAllPaginated(buildQuery('eva_todoist_intake'));
  } catch (err) {
    console.warn(`  Warning: Todoist query error: ${err.message}`);
  }

  let youtube = [];
  try {
    youtube = await fetchAllPaginated(buildQuery('eva_youtube_intake'));
  } catch (err) {
    console.warn(`  Warning: YouTube query error: ${err.message}`);
  }

  const allItems = [
    ...todoist.map((row) => ({ ...row, source_type: 'todoist' })),
    ...youtube.map((row) => ({ ...row, source_type: 'youtube' })),
  ];

  const newItems = allItems.filter((item) =>
    !assignedIds.has(item.id) && item.chairman_intent && item.chairman_intent !== 'reference'
  );
