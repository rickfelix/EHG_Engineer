...
async function updateSyncState(supabase, projectName, syncedCount, error = null) {
  ...
  // never threw either, and this function runs inside the per-project loop's own catch block —
  // a throw here would escape the loop and abort every remaining project's sync, masking the
  // REAL per-project sync error with an unrelated RPC error.
  const { error: rpcError } = await supabase.rpc('eva_sync_state_record_sync_result', {
    p_source_type: 'todoist',
    p_source_identifier: projectName,
