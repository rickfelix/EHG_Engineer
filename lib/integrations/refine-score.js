...
/**
 * Extract scoring context for Claude Code inline analysis.
 * Outputs structured data that Claude Code can evaluate using the 4-persona rubric.
 * @param {Array} items - Wave items to score
 * @param {Object} waveInfo - { title, description }
 * @returns {{ items: Array, rubric: Object, thresholds: Object }}
 */
export function extractScoringContext(items, waveInfo = {}) {
  const contextItems = items.map((item, i) => ({
    index: i + 1,
    title: (item.title || '').slice(0, 200),
    description: (item.description || '').slice(0, 300),
    source_type: item.source_type || 'unknown',
    target_application: item.target_application || 'unknown',
    chairman_intent: item.chairman_intent || 'unknown',
  }));
  ...
  return {
    ...
    items: contextItems,
    rubric: {
      optimist: { weight: SCORE_CONFIG.PERSONAS.optimist.weight, instruction: 'Score potential upside, innovation value, opportunity. Be generous but honest.' },
      pragmatist: { weight: SCORE_CONFIG.PERSONAS.pragmatist.weight, instruction: 'Score feasibility, effort-to-value ratio, achievability with current resources.' },
      devils_advocate: { weight: SCORE_CONFIG.PERSONAS.devils_advocate.weight, instruction: 'Score risk level (INVERTED: 100 = low risk, 0 = extremely risky). Consider hidden dependencies, technical debt.' },
      strategist: { weight: SCORE_CONFIG.PERSONAS.strategist.weight, instruction: 'Score vision alignment, strategic importance, timing. Does this move the product forward?' },
    },
    thresholds: SCORE_CONFIG.THRESHOLDS,
    response_format: {
      description: 'For each item, provide composite score and per-persona breakdown',
      example: {
        item_scores: [
          {
            item_index: 1,
            ...
            persona_scores: {
              ...
              pragmatist: { score: 70, reasoning: 'Feasible with...' },
      ...
      rules: [
        'item_index is 1-based matching the items list',
        'composite = (optimist × 0.15) + (pragmatist × 0.35) + (devils_advocate × 0.25) + (strategist × 0.25)',
        'recommendation: "promote" if composite >= 70, "review" if 40-69, "defer" if < 40',
        'Every item must have exactly one score entry',
