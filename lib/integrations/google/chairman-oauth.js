...
// expires_at is the GRANT (refresh-token) expiry the §9 gauge watches — NOT tokens.expiry_date, which is the
// ~1h access-token lifetime google-auth-library sets. Under the Testing-status posture (D4) a grant lasts
// seven days from consent; it is stamped at consent and left untouched by access-token refreshes.
...
export function oauthHealth(row, now = Date.now()) {
  ...
  if (!hasRequiredScopes(row)) return 'partial_scope';
  const t = row.expires_at ? Date.parse(row.expires_at) : NaN;
  if (Number.isFinite(t) && t < now + EXPIRING_MS) return 'expiring';
  return 'ok';
}

/** The scopes Google actually granted (granular consent can drop one); the requested list only when the response carries none. */
export function grantedScopes(tokens) {
  const s = tokens && typeof tokens.scope === 'string' ? tokens.scope.trim().split(/\s+/).filter(Boolean) : [];
  return s.length ? s : [...SCOPES];
}

/** True when every scope Michael needs was granted; the status surfaces use it so a partial grant never reads as healthy. */
export function hasRequiredScopes(row) {
  const have = new Set(Array.isArray(row && row.scopes) ? row.scopes : []);
 ...
 * `consent: true` (the consent flow) stamps expires_at = grant expiry (refresh_token_expires_in when Google sends it,
 * else now + GRANT_TTL_MS); a refresh omits expires_at so the upsert leaves the grant expiry untouched.
 */
export async function storeTokens(tokens, { sb, enc, env = process.env, now = new Date(), consent = false } = {}) {
  assertHostVenue(env);
  enc = enc || new HostKeyEncryption(env);
  const key_fingerprint = await enc.fingerprint();
  const { encrypted, metadata } = await enc.encrypt(tokens, TOKEN_VAULT_APP_ID);
  const row = {
    identifier: CREDENTIAL_IDENTIFIER, encrypted_blob: encrypted, encryption_metadata: metadata, key_fingerprint, scopes: grantedScopes(tokens),
    last_refreshed_at: now.toISOString(), last_error: null,
  };
  const rtSeconds = tokens && Number(tokens.refresh_token_expires_in);
  if (Number.isFinite(rtSeconds) && rtSeconds > 0) row.expires_at = new Date(now.getTime() + rtSeconds * 1000).toISOString();
  else if (consent) row.expires_at = new Date(now.getTime() + GRANT_TTL_MS).toISOString();
  // one row by unique identifier: select().maybeSingle() is the provably bounded read (count-truncation-diff-lint)
  const w = await writeRows(sb || createMichaelClient(), 'michael_credentials', (t) => t.upsert(row, { onConflict: 'identifier' }).select('identifier').maybeSingle());
  if (!w.ok) throw codedError(w.refusal || 'WRITE_FAILED', `storeTokens: failed to persist encrypted tokens: ${w.error}`);
  return { identifier: CREDENTIAL_IDENTIFIER, key_fingerprint, expires_at: row.expires_at || null };
}

/** Writes the gauge source column; a lost write is logged, never silent (a null last_error would read as healthy). */
export async function recordLastError(sb, last_error, { warn = console.warn } = {}) {
  const w = await writeRows(sb, 'michael_credentials', (t) => t.update({ last_error }).eq('identifier', CREDENTIAL_IDENTIFIER));
  if (!w.ok) warn(`[chairman-oauth] could not record last_error=${last_error} (${w.refusal}: ${w.error}); the oauth-health gauge will NOT see it`);
  return w;
}
...
export async function runConsentFlow({ sb, enc, env = process.env, client, openBrowser = defaultOpenBrowser, createServer = http.createServer, timeoutMs = 300000, log = console.log } = {}) {
  ...
  const code = await new Promise((resolve, reject) => {
    const ignore = (res) => { res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' }); res.end(page('Ignored', 'This request did not match the consent in progress.')); };
    const server = createServer((req, res) => {
      let url, authCode, error, matches;
      try {
        url = new URL(req.url, `http://${REDIRECT_HOST}:${REDIRECT_PORT}`);
        authCode = url.searchParams.get('code');
        error = url.searchParams.get('error');
        matches = url.pathname === REDIRECT_PATH && safeEqual(url.searchParams.get('state') || '', state) && (authCode || error);
      } catch { matches = false; }
      if (!matches) { ignore(res); return; }
  ...
  return storeTokens(tokens, { sb, enc, env, consent: true });
}
