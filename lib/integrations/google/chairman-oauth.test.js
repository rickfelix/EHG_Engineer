...
import {
  assertHostVenue, keyFingerprint, readHostKey, HostKeyEncryption, oauthHealth, hoursToExpiry, grantedScopes, hasRequiredScopes,
...
// expiry_date is what google-auth-library sets: the ~1h ACCESS-token lifetime, never the grant expiry.
const TOKENS = { access_token: 'ya29.fixture-access', refresh_token: '1//fixture-refresh', id_token: 'eyJ.fixture', expiry_date: Date.parse('2026-09-06T13:00:00Z'), scope: SCOPES.join(' '), token_type: 'Bearer' };
...
function fakeSb({ row = null, missing = false, writeError = null } = {}) {
  ...
  const from = (table) => {
    ...
    const q = {
      ...
      eq() { return q; }, limit() { return q; }, maybeSingle() { return q; },
...
describe('storage (FR-3): ciphertext only, fingerprint stamped and compared', () => {
  it('TS-2: real AES-256-GCM write carries no token property and no fixture string', async () => {
    ...
    const out = await storeTokens(TOKENS, { sb, enc, env: HOST_ENV, now: new Date('2026-09-06T12:00:00Z'), consent: true });
    ...
    // GRANT expiry (consent + 7 days), NOT the ~1h access-token expiry_date (adversarial review 8346 W-1)
    expect(payload.expires_at).toBe('2026-09-13T12:00:00.000Z');
    expect(payload.expires_at).not.toBe(new Date(TOKENS.expiry_date).toISOString());
...
describe('refresh (FR-3): gauge predicates written by the module', () => {
  ...
  it('TS-5: a successful refresh re-stores the blob, clears last_error and leaves the grant expiry untouched', async () => {
    const sb = fakeSb({ row });
    const set = [];
    ...
    expect(sb.writes[0].payload).not.toHaveProperty('expires_at'); // grant expiry is stamped at consent only
    expect(sb.writes[0].payload.last_refreshed_at).toMatch(/T/);
    expect(sb.writes[0].payload.last_error).toBeNull();
    expect(set[1].access_token).toBe('new');
  });
  it('refresh_token_expires_in from Google wins over the 7-day posture when present', async () => {
    const sb = fakeSb();
    await storeTokens({ ...stored, refresh_token_expires_in: 3600 }, { sb, enc, env: HOST_ENV, now: new Date('2026-09-06T12:00:00Z'), consent: true });
    expect(sb.writes[0].payload.expires_at).toBe('2026-09-06T13:00:00.000Z');
  });
  it('stores the scopes Google granted, not the requested constant (adversarial review 8351 W-1)', async () => {
    const sb = fakeSb();
    await storeTokens({ ...stored, scope: `${SCOPES[1]} ${SCOPES[2]}` }, { sb, enc, env: HOST_ENV, consent: true });
    expect(sb.writes[0].payload.scopes).toEqual([SCOPES[1], SCOPES[2]]);
    expect(oauthHealth({ ...sb.writes[0].payload, last_error: null })).toBe('partial_scope');
  });
  it('a lost last_error write is warned, never silent (adversarial review 8346 I-1)', async () => {
    const warns = [];
    const sb = fakeSb({ row, writeError: { message: 'boom' } });
    const client = { setCredentials() {}, refreshAccessToken: async () => { throw new Error('invalid_grant'); } };
    await expect(getAuthenticatedClient({ sb, enc, env: HOST_ENV, client, now: 2000 })).rejects.toThrow(/invalid_grant/);
    const { recordLastError } = await import('./chairman-oauth.js');
    await recordLastError(sb, 'invalid_grant', { warn: (m) => warns.push(m) });
    expect(warns[0]).toMatch(/could not record last_error=invalid_grant/);
  });
...
describe('oauthHealth (spec §9) is the single predicate', () => {
  ...
  it('five row shapes', () => {
    ...
    expect(oauthHealth({ last_error: 'invalid_grant', expires_at: '2026-09-20T00:00:00Z', scopes: SCOPES }, now)).toBe('invalid_grant');
    expect(oauthHealth({ last_error: null, expires_at: '2026-09-07T12:00:00Z', scopes: SCOPES }, now)).toBe('expiring');
    expect(oauthHealth({ last_error: null, expires_at: '2026-09-13T12:00:00Z', scopes: SCOPES }, now)).toBe('ok');
    expect(oauthHealth({ last_error: null, expires_at: null, scopes: SCOPES }, now)).toBe('ok'); // DATABASE note: NULL expires_at is not "expiring"
    // adversarial review 8351 W-1: a granular-consent grant missing gmail.modify must never read as healthy
    expect(oauthHealth({ last_error: null, expires_at: '2026-09-13T12:00:00Z', scopes: SCOPES.slice(1) }, now)).toBe('partial_scope');
    expect(grantedScopes({ scope: `${SCOPES[1]}  ${SCOPES[2]}` })).toEqual([SCOPES[1], SCOPES[2]]);
    expect(grantedScopes({})).toEqual(SCOPES);
    expect(hasRequiredScopes({ scopes: [...SCOPES, 'extra'] })).toBe(true);
    expect(hasRequiredScopes({ scopes: SCOPES.slice(0, 2) })).toBe(false);
  ...
  it('binds 127.0.0.1 before opening the browser; wrong state is 400 and never exchanged; PKCE verifier reaches getToken', async () => {
    ...
    const http = fakeHttp({
      drive(handler, events) {
        ...
        // same character count, different byte length (adversarial review 8346 W-2): must be 400, never a throw
        const odd = res(); handler({ url: `/oauth2callback?code=x&state=${encodeURIComponent('é' + state.slice(1))}` }, odd); events.push(['odd', odd.statuses[0]]);
        const malformed = res(); handler({ url: 'http://%' }, malformed); events.push(['malformed', malformed.statuses[0]]);
    ...
    for (const k of ['bad', 'empty', 'odd', 'malformed']) expect(http.events).toContainEqual([k, 400]);
    expect(http.events).toContainEqual(['good', 200]);
...
describe('SCOPES (FR-6, child J non-goal): exactly the three read-only scopes, no youtube, no write surface', () => {
  it('is pinned to exactly the three read-only scopes in this order — a silent addition here is a silent grant widening', () => {
  ...
  it('carries no youtube scope of any kind (readonly or write) — v1.1 youtube-digest reads public RSS, never this grant', () => {
    expect(SCOPES.some((s) => /youtube/i.test(s))).toBe(false);
    // every scope already present is .readonly except gmail.modify (spec-required for triage labelling)
    expect(SCOPES.filter((s) => !/\.readonly$/.test(s))).toEqual(['https://www.googleapis.com/auth/gmail.modify']);
  });
});
