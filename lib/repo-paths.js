 ...
 * to ENGINEER_ROOT -- registry entries store the sibling-directory convention
 * explicitly (e.g. `../ehg`), since every venture/app repo lives alongside
 * EHG_Engineer under one parent. An already-absolute value is returned unchanged
 * (explicit-override escape hatch for a genuinely nonstandard layout) --
...
function resolveLocalPath(localPath) {
  ...
  return path.resolve(ENGINEER_ROOT, localPath);
}
      ...
      if (!error && Array.isArray(data)) {
        ...
        if (hit) {
          ...
          if (isActive && hit.local_path) {
            return { path: path.resolve(hit.local_path), source: 'db', reason: 'active' };
          }
          if (!isActive) {
