...
import { withSchemaDriftDetection } from './supabase-client-schema-drift.cjs';
import { resolveActorIdentity, withActorHeader } from './db-actor-identity.js';
 ...
 * @param {boolean} [options.throwOnSchemaDrift=true] - SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-A:
 *   set false ONLY when a caller already inspects error.code itself to distinguish a
 *   genuinely-optional, not-yet-migrated column/relation from a real fault (see
 *   lib/eva/bridge/venture-provisioner.js's stack_descriptor probe for the documented
 *   precedent -- SD-LEO-INFRA-S19-BRIDGE-UNBLOCK-SCHEMA-DRIFT-001). Default stays true:
 *   this is an explicit, reviewed opt-out, not a general escape hatch.
 ...
 * @param {boolean} [options.throwOnSchemaDrift=true] - SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-A: see
 *   createSupabaseClient's matching option doc -- same explicit, reviewed opt-out, same default.
...
export function createSupabaseServiceClient(options = {}) {
  ...
  const { fetchTimeoutMs, throwOnSchemaDrift } = options;
  const applyWrap = (client) => (throwOnSchemaDrift === false ? client : withSchemaDriftDetection(client));
  ...
  if (!fetchTimeoutMs) {
    const global = withActorHeader(undefined, actorId);
    return applyWrap(createClient(supabaseUrl, supabaseKey, global ? { global } : undefined));
  }
  ...
  return applyWrap(createClient(supabaseUrl, supabaseKey, { global: withActorHeader({ fetch: boundedFetch }, actorId) }));
}
