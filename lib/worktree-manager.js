...
export function createWorktree({ sdKey, session, branch, force = false, repoRoot: repoRootOverride }) {
  ...
  if (branchExists) {
    ...
    // QF-20260902-780: trace pid+cwd+op right before the only index-touching git subcommand
    // in the worktree-creation path, so a future orphaned-lock incident can be correlated
    // against this log line instead of only the lock file's own bare mtime.
    logWorktreeEvent('worktree.git_add_start', { pid: process.pid, cwd: repoRoot, branch });
...
export function createWorkTypeWorktree({ workType, workKey, branch, force = false, repoRoot: repoRootOverride }) {
  ...
  if (workType === 'SD') {
    ...
    if (fs.existsSync(legacyPath) && !fs.existsSync(worktreePath)) {
      ...
      // QF-20260912-665: refresh a stale propagated .env on EVERY reuse return of this shared
      // factory (quick-fix / team-spawner / plan-to-exec / lead-final / sd-start), not just the
      // resolve-sd-workdir.js re-attach path — a reused QF tree never went through that path.
      try { refreshPropagatedEnv(repoRoot, legacyPath); } catch { /* non-fatal, matches propagateEnvFile */ }
      return {
        mode: 'worktree',
        path: legacyPath,
  ...
  try {
    ...
    if (fs.existsSync(worktreePath)) {
      ...
      if (existingBranch === resolvedBranch || !force) {
        ...
        // QF-20260912-665: same refresh as the legacy-reuse branch above.
        try { refreshPropagatedEnv(repoRoot, worktreePath); } catch { /* non-fatal, matches propagateEnvFile */ }
        return {
          mode: 'worktree',
          path: worktreePath,
    ...
    if (branchExists) {
      ...
      logWorktreeEvent('worktree.git_add_start', { pid: process.pid, cwd: repoRoot, branch: resolvedBranch });
 ...
 * QF-20260901-296: stamps a `# propagated-from-root sha256=<hash>` marker line so a
 * later root secret rotation can be detected (see refreshPropagatedEnv) without ever
 * comparing/printing values -- only the content hash.
 * @param {string} repoRoot - Repo root path
 * @param {string} worktreePath - Worktree destination path
 * @param {{force?: boolean}} [opts] - force=true re-copies even if destEnv already exists
 */
function propagateEnvFile(repoRoot, worktreePath, { force = false } = {}) {
  const sourceEnv = path.join(repoRoot, '.env');
  const destEnv = path.join(worktreePath, '.env');
  try {
    if (fs.existsSync(sourceEnv) && (force || !fs.existsSync(destEnv))) {
      const content = fs.readFileSync(sourceEnv);
      const hash = crypto.createHash('sha256').update(content).digest('hex');
      fs.writeFileSync(destEnv, Buffer.concat([content, Buffer.from(`\n# propagated-from-root sha256=${hash}\n`)]));
    }
  } catch (err) {
...
const PROPAGATED_ENV_MARKER_RE = /^# propagated-from-root sha256=([0-9a-f]{64})$/m;

/**
 * QF-20260901-296: detect-and-repair a worktree .env snapshot that has gone stale after a
 * root secret rotation. Called at claim/checkin (worktree reuse), not just creation.
 *
 * Only a .env carrying the propagated-from-root marker is ever touched -- an unmarked .env
 * (venture-owned or hand-authored) is left alone by design. mtime is deliberately never
 * used (it lies after checkouts); only a content-hash comparison decides staleness, and
 * values are never read/printed, only hashed.
 * @param {string} repoRoot - Repo root path (source of truth)
 * @param {string} worktreePath - Worktree to check/refresh
 * @returns {{refreshed: boolean, reason: 'missing'|'unmarked'|'current'|'stale'|'no-root-env'}}
 */
export function refreshPropagatedEnv(repoRoot, worktreePath) {
  const sourceEnv = path.join(repoRoot, '.env');
  const destEnv = path.join(worktreePath, '.env');
  if (!fs.existsSync(sourceEnv)) return { refreshed: false, reason: 'no-root-env' };
  if (!fs.existsSync(destEnv)) {
    propagateEnvFile(repoRoot, worktreePath);
    return { refreshed: true, reason: 'missing' };
  }
  const marker = fs.readFileSync(destEnv, 'utf8').match(PROPAGATED_ENV_MARKER_RE);
  if (!marker) return { refreshed: false, reason: 'unmarked' };
  const rootHash = crypto.createHash('sha256').update(fs.readFileSync(sourceEnv)).digest('hex');
  if (rootHash === marker[1]) return { refreshed: false, reason: 'current' };
  propagateEnvFile(repoRoot, worktreePath, { force: true });
