...
function defaultGitRunner(args, cwd) {
  try {
    // QF-20260912-698: explicit maxBuffer -- execFileSync's default (1 MiB) silently
    // truncated `git diff --cached` on 5 real trees whose cached diffs ran
    // 19.8-58.8 MB, handing the secret scanner a truncated first megabyte with no
    // error surfaced. 256 MiB comfortably covers those measured sizes; the caller
    // (readAndScanStagedFilesForSecrets) additionally bounds risk by reading one
    // file's diff at a time rather than the whole staged set in one call.
    const out = execFileSync('git', args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], maxBuffer: 256 * 1024 * 1024 });
...
/**
 * Enumerate the staged file set and read + scan each file's diff individually
 * (QF-20260912-698, hardening QF-20260912-495's per-file withhold): a single
 * `git diff --cached` read of the WHOLE staged set overflowed execFileSync's
 * default 1 MiB maxBuffer on 5 real trees (19.8-58.8 MB cached diffs) -- the read
 * silently truncated to the first megabyte, scanStagedFilesForSecrets only ever
 * saw the early files, withheld stayed 0, and the commit proceeded to fail at
 * husky's own pre-commit secret check instead of being caught here.
 *
 * FAIL-CLOSED: a nonzero code or empty stdout for any path the name-only list says
 * is staged aborts scanning entirely -- this stage must never commit a set it could
 * not fully read, even under a runner error it cannot classify further.
 *
 * @param {Function} run - the bound `(args) => gitRunner(args, wtPath)` helper
 * @param {Function} logger
 * @returns {{ok: true, perFile: Array<{path: string, held: boolean, findings: Array}>} | {ok: false, reason: string}}
 */
function readAndScanStagedFilesForSecrets(run, logger) {
  const nameListRes = run(['diff', '--cached', '--name-only']);
  if (nameListRes.code !== 0) {
    const reason = `name-only list failed (code ${nameListRes.code}): ${nameListRes.stderr || nameListRes.stdout}`;
    logger(`[preserve-stage] ${reason} -- failing closed`);
    return { ok: false, reason };
  }
  const paths = String(nameListRes.stdout || '').split('\n').map((l) => l.trim()).filter(Boolean);
  const perFile = [];
  for (const p of paths) {
    const fileDiff = run(['diff', '--cached', '--', p]);
    if (fileDiff.code !== 0 || !fileDiff.stdout) {
      const reason = `diff read failed/empty for staged path "${p}" (code ${fileDiff.code}): ${fileDiff.stderr || '(empty stdout)'}`;
      logger(`[preserve-stage] ${reason} -- failing closed`);
      return { ok: false, reason };
    }
    const { held, findings } = scanStagedDiffForSecrets(fileDiff.stdout);
    perFile.push({ path: p, held, findings });
  }
...
export async function runPreserveStage({ wtPath, key, ownerSessionId }, opts = {}) {
  ...
  if (hasStagedChanges) {
    ...
    const scan = readAndScanStagedFilesForSecrets(run, logger);
    if (!scan.ok) {
      // Fail-closed (QF-20260912-698): could not safely read every staged file's
      // diff -- never risk committing an unscanned set, hold the whole tree exactly
      // as the original whole-diff-read failure path did.
      run(['reset']);
      return { verdict: PRESERVE_VERDICT.HELD_SECRET, ref: null, sha: null, pushed: false, findings: [], error: scan.reason };
    }
    const heldFiles = scan.perFile.filter((f) => f.held);
