...
export async function runPreserveStage({ wtPath, key, ownerSessionId }, opts = {}) {
  ...
  if (hasStagedChanges) {
    ...
    // 3. Commit, with owner session + worktree path as trailers. Capture the tree's
    // OWN pre-commit tip first -- QF-20260904-596: this commit must never become
    // reachable from the tree's checked-out branch (that is the whole bug this fixes:
    // a stash-commit here previously became an ancestor of the worker's own next
    // commit/push on this exact branch, and rode along into that worker's own PR).
    const originalHeadRes = run(['rev-parse', 'HEAD']);
    const originalHead = String(originalHeadRes.stdout || '').trim();
    ...
    return { verdict: PRESERVE_VERDICT.PUSHED, ref, sha: committedSha, pushed: true };
  }

  // No staged changes at all (tree carries only already-committed, unpushed work) --
  // nothing to commit or reset, push the tree's current HEAD unchanged.
