 ...
 *  QF-20260912-495 adds DB-CONN-STRING: a live postgresql://user:pass@ connection string
 *  (mirrors .husky/pre-commit's own SECRET_PATTERNS entry exactly) -- test-evidence JSON
...
export async function runPreserveStage({ wtPath, key, ownerSessionId }, opts = {}) {
  ...
  if (hasStagedChanges) {
    ...
    // same class of content the whole-diff scanStagedDiffForSecrets check does not
    // cover (a postgresql://user:pass@ string) -- so these trees never reached
    // scan.held at all, they failed at the commit step below as PUSH_FAILED. Both
