...
export async function fetchTerminalStatusKeys(supabase, keys = []) {
  ...
  try {
    if (sdKeys.length) {
      const { data } = await supabase.from('strategic_directives_v2').select('sd_key, status').in('sd_key', sdKeys);
      for (const row of data || []) {
        if (row.status === 'completed' || row.status === 'cancelled') terminal.add(row.sd_key);
      }
    }
    if (qfKeys.length) {
      const { data } = await supabase.from('quick_fixes').select('id, status').in('id', qfKeys);
      for (const row of data || []) {
