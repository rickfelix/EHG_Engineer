...
export function evaluateReclaimEligibility(input, opts = {}) {
  const { contentSafe, holder, auditAccepted, nowMs = Date.now(), freezeCutMinutes = FREEZE_CUT_MINUTES } = input;
...
export default { isHolderPidResident, evaluateReclaimEligibility, RECLAIM_VERDICT };
