...
// QF-20260829-726: the manually-created ceremony class (a tree outside .worktrees/ that
// sd-start/qf-start can never marker) sits idle for chairman-paced gaps of ~2h between queue
// answers, well past the 30min default -- specimen 3 was reaped mid-sitting. 240min (4h) gives
// margin above that pacing without weakening the guard for the .worktrees/-resident pool the
// 30min default was tuned for.
export const CEREMONY_RESIDENCY_WINDOW_MIN = 240;
...
/** Is `wtPath` outside the `.worktrees/` directory -- the manually-created ceremony class
 *  (no sd-start/qf-start marker) rather than the pool sd-start/qf-start manage? Path-shape-tolerant
 *  (checks a normalized, forward-slashed form) so it agrees with sameDir()'s separator handling. */
function isOutsideWorktreesDir(wtPath) {
  const normalized = norm(wtPath).replace(/\\/g, '/');
...
// QF-20260829-726: precedence is explicit param > env override > location-aware default. The
// location-aware step (ceremony trees outside .worktrees/ get the longer default) sits BELOW the
// env override on purpose -- WORKTREE_RESIDENCY_WINDOW_MIN, once set, must win over both defaults
// unconditionally, matching every other reaper knob's "explicit beats default" contract.
function resolveWindowMin(explicit, wtPath) {
  if (Number.isFinite(explicit) && explicit > 0) return explicit;
  const env = Number(process.env.WORKTREE_RESIDENCY_WINDOW_MIN);
  if (Number.isFinite(env) && env > 0) return env;
  return isOutsideWorktreesDir(wtPath) ? CEREMONY_RESIDENCY_WINDOW_MIN : DEFAULT_RESIDENCY_WINDOW_MIN;
}
...
export function treeResidencyBlocksRemoval(wtPath, options = {}) {
  ...
  const windowMs = resolveWindowMin(windowMin, wtPath) * 60 * 1000;
