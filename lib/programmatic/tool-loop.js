...
import { getClaudeModel, getGoogleModel } from '../config/model-config.js';

const DEFAULT_MODEL = getClaudeModel('generation');
 ...
 *
 * QF-20260903-508: exported so a caller can name WHICH backend produced a non-conforming
 * response (e.g. "no JSON in output") -- the fallback is silent and automatic from the
 * caller's point of view, and the only prior evidence it engaged was a console.log line
 * inside runProgrammaticTask itself, invisible once the caller's own error surfaces instead.
 * Safe to call a second time: it is a pure function of env vars, which do not change mid-process,
 * so it always agrees with whichever branch runProgrammaticTask actually took.
...
async function runGeminiLoop(prompt, tools, options) {
  ...
  const model = getGoogleModel('generation');
