...
/**
 * Factory that returns a domain knowledge service instance with cross-venture pattern search.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {Object} [options]
 * @param {Object} [options.logger=console]
 * @returns {{ findCrossVenturePatterns: Function, getByIndustry: Function }}
 */
export function createDomainKnowledgeService(supabase, { logger = console } = {}) {
  return {
    getByIndustry: (industry, opts) => getByIndustry(supabase, industry, opts),
    getHierarchy: (industry) => getHierarchy(supabase, industry),
    upsert: (entry) => upsert(supabase, entry),

    /**
     * Find cross-venture patterns by searching tags and problem areas.
     *
     * @param {string[]} tags - Industry/market tags to match
     * @param {string[]} problemAreas - Problem area keywords to match
     * @param {number} [limit=10] - Max results
     * @returns {Promise<Object[]>} Matching domain knowledge entries
     */
    async findCrossVenturePatterns(tags, problemAreas, limit = 10) {
      try {
        // Search by tags overlap using Supabase's contains/overlaps operators
        let query = supabase
          .from('domain_knowledge')
          .select('*')
          .limit(limit);
        ...
        const { data, error } = await query;
        if (error) {
          logger.log(`   Domain knowledge query error: ${error.message}`);
          return [];
        }

        // Also search by problem_area text match
        let problemMatches = [];
        if (problemAreas.length > 0) {
          for (const area of problemAreas.slice(0, 3)) {
            const { data: areaData } = await supabase
              .from('domain_knowledge')
              .select('*')
              .ilike('problem_area', `%${area}%`)
              .limit(5);
            if (areaData) problemMatches.push(...areaData);
          }
        }

        // Deduplicate and enrich with freshness
        const seen = new Set();
        const combined = [...(data || []), ...problemMatches].filter(entry => {
        ...
        logger.log(`   Domain knowledge search failed: ${err.message}`);
        return [];
      }
    },
  };
