...
// QF-20260906-235: a flag whose kill-switch READ was deliberately removed from a JS chokepoint
// (a graduation) leaves behind a marker comment naming the flag, e.g. "QF-XXXXX-XXX (...):
// GRADUATED. The leo_feature_flags.<FLAG_KEY> ... read was removed...". Unlike
// buildLiveReaderIndex, this scans RAW source (comments included) — the marker IS a comment.
const GRADUATED_MARKER_RE = /GRADUATED\.[\s\S]{0,200}?leo_feature_flags\.([A-Z][A-Z0-9_]+)/g;

/**
 * Build a predicate: has this flag key been marked GRADUATED-in-code anywhere in the source tree?
 * @param {string} repoRoot absolute repo root
 * @returns {(flagKey:string)=>boolean}
 */
export function buildGraduatedMarkerIndex(repoRoot) {
  const found = new Set();
  const files = [];
  for (const d of SOURCE_DIRS) walk(path.join(repoRoot, d), files);
  for (const file of files) {
    let src;
    try { src = fs.readFileSync(file, 'utf8'); } catch { continue; }
    if (!src.includes('GRADUATED')) continue;
    for (const m of src.matchAll(GRADUATED_MARKER_RE)) found.add(m[1]);
  }
  return (flagKey) => found.has(flagKey);
}

// QF-20260906-235 perf: buildLiveReaderIndex + buildGraduatedMarkerIndex each do their own full
// tree-walk + full-file read over the same SOURCE_DIRS — timed in isolation at ~1.8s + ~6s on a
// cold cache, which doubled the pre-existing single-walk cost and produced a timeout on the
// production cron. This combined builder does ONE walk + ONE read pass and computes both
// predicates together; production callers should use this instead of calling both separately.
// The two single-purpose builders above stay exported as-is (still directly unit-tested).
export function buildFlagCodeIndices(repoRoot, flagKeys = []) {
  const keys = [...new Set((flagKeys || []).filter((k) => typeof k === 'string' && k.length >= 3))];
  const liveFound = new Set();
  const graduatedFound = new Set();
  const files = [];
  for (const d of SOURCE_DIRS) walk(path.join(repoRoot, d), files);
  for (const file of files) {
    let src;
    ...
    if (src.includes('GRADUATED')) {
      for (const m of src.matchAll(GRADUATED_MARKER_RE)) graduatedFound.add(m[1]);
    }

    if (keys.length && liveFound.size < keys.length) {
      const rel = path.relative(repoRoot, file).replace(/\\/g, '/');
      if (!EXCLUDE_FILE_RE.test(rel)) {
        const remaining = keys.filter((k) => !liveFound.has(k));
        if (remaining.some((k) => src.includes(k))) {
          const code = stripComments(src);
          for (const k of remaining) if (code.includes(k)) liveFound.add(k);
        }
      }
    }
  }
  return {
    hasLiveReaders: (flagKey) => liveFound.has(flagKey),
    isGraduatedInCode: (flagKey) => graduatedFound.has(flagKey)
  };
}
