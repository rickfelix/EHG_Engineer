 ...
 * @param {object} [opts] - { env, hasLiveReaders, isGraduatedInCode } — env: runtime snapshot for
 *   registry-vs-runtime drift detection (QF-20260610-863); hasLiveReaders(flagKey): optional
 *   predicate that returns true when the flag still has live code readers, so a KILL is
 *   downgraded to KEEP (QF-20260721-951); isGraduatedInCode(flagKey): optional predicate that
 *   returns true when a "GRADUATED" marker comment names this flag, so an enabled-never-rolled-
 *   out flag reports GRADUATED (informational) instead of re-recommending graduation daily
 *   (QF-20260906-235). All injected, never read globally (stays pure).
 * @returns {{flag_key:string, reasons:string[], recommendation:'enable'|'graduate'|'graduated'|'kill'|'keep'|'extend'|'review'|'reconcile', detail:string}|null}
 */
export function classifyFlag(flag, now, opts = {}) {
  ...
  // QF-20260906-235 ADDENDUM: a disabled flag the operator has DELIBERATELY held off (its own
  // registry text carries an [OPERATOR_HOLD] marker) is a decision, not staleness — killing it
  // would re-recommend an action nobody can act on (alert fatigue). Downgrade KILL -> KEEP,
  // mirroring the live-readers downgrade above.
  const operatorHold = /\[OPERATOR_HOLD/i.test(flag.enablement_criteria || '') || /\[OPERATOR_HOLD/i.test(flag.description || '');
  if (recommendation === 'kill' && operatorHold) {
    recommendation = 'keep';
    reasons.push('operator-hold');
  }

  // QF-20260906-235: an enabled-never-rolled-out flag that ALSO carries a "GRADUATED" marker
  // comment (its JS chokepoint read was already removed by a prior graduation QF) is DONE in
  // code, not a fresh graduation candidate — re-recommending GRADUATE daily is the exact
  // alert-fatigue shape this ticket exists to close. Report GRADUATED instead; any residual
  // DB-side reader (e.g. an RPC function) is a separate, chairman-ceremony surface, named here
  // so the digest stays informative without re-nagging.
  if (recommendation === 'graduate' && typeof opts.isGraduatedInCode === 'function' && opts.isGraduatedInCode(flag.flag_key)) {
    recommendation = 'graduated';
    reasons.push('graduated-in-code');
    return {
      flag_key: flag.flag_key,
      lifecycle_state: lifecycle,
      is_enabled: !!flag.is_enabled,
      reasons,
      recommendation,
      detail: `${reasons.join(', ')} — JS chokepoint read already removed; any residual DB-side reader is a separate chairman-ceremony surface`
    };
  }
