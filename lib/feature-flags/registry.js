...
async function logAudit({ flagKey, action, previousState, newState, changedBy, environment }) {
  ...
  // SD-LEO-FIX-CHAIRMAN-APPROVED-ARMING-001: leo_feature_flag_audit_log is an updatable VIEW
  // that rejects any insert naming its `environment` column at all -- "cannot insert into
  // column 'environment' of view", even when the value is NULL. Every caller in this file
  // unconditionally included `environment: environment || null` in the insert payload, so
  // EVERY logAudit() call in this file has been silently failing since inception (the error
  // was never inspected -- see the check below, added by this same fix). Only include the
  // key when a real value was actually supplied, which is the common case for every caller
  // except setPolicy(); setPolicy's own always-on environment value still hits the view's
  // restriction and is a separate, pre-existing defect, not fixed here.
  const insertPayload = {
    flag_key: flagKey,
    action,
  ...
  if (environment) {
    insertPayload.environment = environment;
  }

  const { error } = await supabase.from('leo_feature_flag_audit_log').insert(insertPayload);
  if (error) {
    console.warn(`[feature-flags/registry] logAudit failed for '${flagKey}' action='${action}': ${error.message}`);
  }
}
...
export async function createFlag({ flagKey, displayName, description, isEnabled = false, changedBy, ownerType, ownerId, riskTier = 'medium', isTemporary = false, expiryAt }) {
  ...
  // chk_action_type admits create/update/transition/approval/rollback/expire/delete only --
  // 'created' (past tense) is not among them; every mutator in this file used its own past-tense
  // verb and every one of them silently failed the same way (see logAudit()'s doc comment).
  await logAudit({
    flagKey,
    action: 'create',
    previousState: null,
...
export async function transitionLifecycleState(flagKey, newState, options = {}) {
  const supabase = getSupabase();
  // QF-20260912-959: reason/actorId/actorType were accepted but silently discarded (prefixed
  // `_` to satisfy lint) -- the sole mutator in this file with no audit trail at all, unlike
  // every other write below. A governed transition (e.g. arming a HIGH-risk flag off a chairman
  // decision) must leave the same kind of citable record createFlag/updateFlag/deleteFlag/
  // setPolicy already do.
  const { reason, actorId = 'system', actorType = 'system' } = options;
  ...
  // chk_action_type on leo_feature_flag_audit_log admits create/update/transition/approval/
  // rollback/expire/delete only -- 'transition' is the admissible value; the specific reason/
  // actor citation still rides in new_state so the chairman decision/ratification is captured
  // in the row instead of being lost to a rejected insert (found live: 'lifecycle_transitioned'
  // silently failed the CHECK, and logAudit never surfaced it).
  await logAudit({
    flagKey,
    action: 'transition',
    previousState: flag,
    newState: { ...data, transition_reason: reason || null, actor_type: actorType },
    changedBy: actorId
  });
...
/**
 * Stamp a flag as rolled out (graduated). governance-review.js's classifyFlag() treats a
 * non-null rolled_out_at as the authoritative graduation signal (`enabledNeverRolledOut`
 * checks `!flag.rolled_out_at`) -- this is the governed write for that column; before
 * QF-20260912-959, nothing but a one-off backfill script (scripts/backfill-pending-
 * enablement-registry.mjs) ever wrote it, so an enabled flag actually graduated in code kept
 * re-triggering a daily GRADUATE recommendation until a separate marker-comment workaround
 * (QF-20260906-235) papered over the missing stamp instead of setting it.
 * @param {string} flagKey - Flag key
 * @param {Date|string} rolledOutAt - When the flag was rolled out / graduated
 * @param {string} changedBy - Who is making the change (e.g. a chairman_decisions citation)
 * @returns {Promise<Object>} Updated flag
 */
export async function markRolledOut(flagKey, rolledOutAt, changedBy) {
  const supabase = getSupabase();
  ...
  const rolledOutDate = rolledOutAt instanceof Date ? rolledOutAt.toISOString() : rolledOutAt;

  const { data, error } = await supabase
    .from('leo_feature_flags')
    .update({ rolled_out_at: rolledOutDate, updated_at: new Date().toISOString() })
    .eq('flag_key', flagKey)
    .select()
    .single();

  if (error) {
    throw new Error(`Failed to mark '${flagKey}' rolled out: ${error.message}`);
  }

  // chk_action_type admits create/update/transition/approval/rollback/expire/delete only --
  // 'rolled_out' is not among them and silently failed the CHECK before this fix. rolled_out_at
  // is a plain column update, so 'update' is both admissible and semantically accurate.
  await logAudit({
    flagKey,
    action: 'update',
    previousState: currentFlag,
...
export async function setPolicy({ flagKey, environment = 'production', rolloutPercentage = 100, userTargeting = {}, changedBy }) {
  ...
  // 'policy_created'/'policy_updated' are not admissible action_type values either (same bug
  // class as create/update/delete above). The per-environment detail is not lost by dropping the
  // top-level `environment` argument -- it already rides in previousState.environment /
  // newState.environment (a real column on leo_feature_flag_policies), and passing it here would
  // still hit the view's environment-column restriction regardless of the action_type fix.
  await logAudit({
    flagKey,
...
export async function activateKillSwitch(switchKey, activatedBy) {
  ...
  // Activating a kill switch is an emergency-disable action -- 'rollback' is the admissible
  // value the CHECK constraint provides for exactly this case (same bug class as above:
  // 'kill_switch_activated' is not admitted).
  await logAudit({
    flagKey: switchKey,
    action: 'rollback',
