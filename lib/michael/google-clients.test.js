...
import { listCalendarEvents, listDriveFiles, readDriveFileText, driveQuery, driveLiteral, CALENDAR_MAX_RESULTS, DRIVE_FILE_FIELDS } from './google-clients.mjs';
...
describe('listCalendarEvents', () => {
  it('maps calendarId/timeMin/timeMax with singleEvents, orderBy startTime and maxResults 250', async () => {
    ...
    expect(r).toEqual({ ok: true, events: [{ id: 'e1' }], calendarId: 'primary', truncated: false });
...
describe('listDriveFiles and readDriveFileText', () => {
  it('lists files in a folder by exact name with the bounded field set', async () => {
    ...
    expect(r).toEqual({ ok: true, files: [{ id: 'f1', name: "o'brien.json", parents: ['FOLDER'] }], truncated: false });
    ...
    // backslash is escaped BEFORE the quote so a trailing backslash cannot terminate the literal early
    expect(driveLiteral('x\\')).toBe("'x\\\\'");
    expect(driveLiteral("x\\' or name contains 'a")).toBe("'x\\\\\\' or name contains \\'a'");
