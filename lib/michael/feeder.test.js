...
describe('pure helpers', () => {
  it('registry lists the eleven feeder ids with venues and inclusive windows', () => {
    expect(FEEDER_IDS).toEqual(['tasks-classifier', 'calendar-read', 'gmail-triage', 'todoist-brief', 'seat-classify', 'retention', 'brief-assemble', 'oracle-extract', 'health-sync', 'youtube-digest', 'checkpoint-send']);
    // QF-20260911-282: calendar-read/gmail-triage/todoist-brief now carry an ARRAY of windows
    // (pre-dawn + midday 12:00-12:30 + evening 18:00-18:30 ET, chairman ratification 2026-09-11).
    expect(FEEDERS['gmail-triage']).toEqual({ venue: 'task_scheduler', window: [{ start: '04:30', end: '05:30' }, { start: '12:00', end: '12:30' }, { start: '18:00', end: '18:30' }], intervalMinutes: 15 });
    // QF-20260911-110 / ratification 00f696f1: moved from 'gha' to 'task_scheduler' -- pinned
    // explicitly (not just covered by the generic enum check below) so a future edit can't
    // silently regress these two back onto a GHA cron without this test catching it.
    expect(FEEDERS['todoist-brief']).toEqual({ venue: 'task_scheduler', window: [{ start: '04:45', end: '05:30' }, { start: '12:00', end: '12:30' }, { start: '18:00', end: '18:30' }], intervalMinutes: 15 });
    expect(FEEDERS['brief-assemble']).toEqual({ venue: 'task_scheduler', window: { start: '05:15', end: '06:00' }, intervalMinutes: 15 });
    expect(FEEDERS['oracle-extract']).toEqual({ venue: 'task_scheduler', window: { start: '06:00', end: '06:30' }, intervalMinutes: 15 });
    expect(FEEDERS['health-sync']).toEqual({ venue: 'task_scheduler', window: { start: '06:00', end: '06:30' }, intervalMinutes: 15 });
    expect(FEEDERS['youtube-digest']).toEqual({ venue: 'gha', window: { start: '06:00', end: '06:30' }, intervalMinutes: 15 });
    // SD-LEO-INFRA-MICHAEL-TIER2-CHECKPOINT-SEND-001: the four ratification-named windows,
    // pinned explicitly (not just covered by the generic enum check below).
    expect(FEEDERS['checkpoint-send']).toEqual({ venue: 'task_scheduler', window: [{ start: '06:00', end: '06:15' }, { start: '10:00', end: '10:15' }, { start: '14:00', end: '14:15' }, { start: '18:00', end: '18:15' }], intervalMinutes: 15 });
  ...
  // PLAN-TO-EXEC TESTING finding F3 (testing-agent:aa363409899c1c035): the registry invariant
  // itself was asserted nowhere before this -- "every feeder window ends on a fire minute" is
  // documented at the top of this file (F1) but never checked.
  it.each(FEEDER_IDS)('%s: well-formed registry entry (venue enum, every window ends on a fire minute)', (id) => {
    const entry = FEEDERS[id];
    expect(Object.isFrozen(entry)).toBe(true);
    expect(['task_scheduler', 'gha', 'seat']).toContain(entry.venue);
    if (entry.window === null) {
      expect(entry.intervalMinutes).toBeNull();
      return;
    }
    // QF-20260911-282: window is a single {start,end} object OR a frozen array of them -- check
    // every window in either shape (the array itself, and each entry, must be frozen).
    expect(Object.isFrozen(entry.window)).toBe(true);
    const windows = Array.isArray(entry.window) ? entry.window : [entry.window];
    for (const w of windows) {
      ...
      if (entry.intervalMinutes) {
        expect(end % entry.intervalMinutes).toBe(0);
      }
    }
  });

  it('v1.1 enrichment feeders (oracle-extract, health-sync, youtube-digest) are never brief-blocking', () => {
    for (const id of ['oracle-extract', 'health-sync', 'youtube-digest']) {
      expect(READINESS_REQUIREMENTS).not.toContain(id);
    }
  });
  ...
  it('QF-20260911-282: inWindow accepts an array of windows, true inside ANY of them (pre-dawn, midday, evening)', () => {
    const windows = FEEDERS['gmail-triage'].window;
    expect(windows.length).toBe(3);
    // pre-dawn 04:30-05:30
    expect(inWindow(hhmmToMinutes('04:30'), windows)).toBe(true);
    expect(inWindow(hhmmToMinutes('05:30'), windows)).toBe(true);
    // midday 12:00-12:30
    expect(inWindow(hhmmToMinutes('12:00'), windows)).toBe(true);
    expect(inWindow(hhmmToMinutes('12:30'), windows)).toBe(true);
    // evening 18:00-18:30
    expect(inWindow(hhmmToMinutes('18:00'), windows)).toBe(true);
    expect(inWindow(hhmmToMinutes('18:30'), windows)).toBe(true);
    // between windows -- outside all three
    expect(inWindow(hhmmToMinutes('06:00'), windows)).toBe(false);
    expect(inWindow(hhmmToMinutes('11:59'), windows)).toBe(false);
    expect(inWindow(hhmmToMinutes('12:31'), windows)).toBe(false);
    expect(inWindow(hhmmToMinutes('17:59'), windows)).toBe(false);
    expect(inWindow(hhmmToMinutes('18:31'), windows)).toBe(false);
    expect(inWindow(hhmmToMinutes('23:00'), windows)).toBe(false);
  });
...
describe('runFeeder inert paths (TS-1)', () => {
  ...
  it('QF-20260911-282: runFeeder fires in the new midday window, not just the pre-dawn one (calendar-read at 12:00 ET)', async () => {
    const { sb, calls } = scripted();
    const run = vi.fn(async () => ({ status: 'ok', counts: { events: 0 } }));
    const NOON_ET = new Date('2026-09-06T16:00:00.000Z'); // 12:00 ET (EDT = UTC-4)
    const r = await runFeeder({ feeder: 'calendar-read', run }, { sb, env, now: NOON_ET, logger: quiet });
  ...
  it('the object form { feeder, accept } narrows what counts as ready', async () => {
    const up = scripted({ reads: [{ data: [], error: null }, { data: [{ feeder: 'calendar-read', status: 'degraded' }], error: null }] });
    const r = await runFeeder({ feeder: 'todoist-brief', upstream: [{ feeder: 'calendar-read', accept: ['ok'] }], run: vi.fn() }, { sb: up.sb, env, now: FIVE_AM_ET, logger: quiet });
    expect(r).toMatchObject({ reason: 'upstream_not_ready', upstream_not_ready: ['calendar-read'] });
  });
  it('a malformed upstream entry is refusal UPSTREAM_INVALID before any read (never a throw)', async () => {
    for (const bad of [{ feeder: 'calendar-read', accept: {} }, null, { feeder: 'calendar-read', accept: 'ok' }, { feeder: 'calendar-read', accept: [] }, { feeder: 'calendar-read', accept: ['running'] }, { accept: ['ok'] }]) {
      const { sb, calls } = scripted();
      const r = await runFeeder({ feeder: 'todoist-brief', upstream: [bad], run: vi.fn() }, { sb, env, now: FIVE_AM_ET, logger: quiet });
      expect(r).toMatchObject({ ok: false, refusal: 'UPSTREAM_INVALID' });
      expect(calls).toHaveLength(0);
    }
  });
...
describe('single-flight and attempt (TS-2)', () => {
  ...
  it('writes the start row as status skipped / phase started with ISO started_at, then finishes with the real status', async () => {
    ...
    expect(ins).toEqual({ feeder: 'gmail-triage', et_date: '2026-09-06', attempt: 1, venue: 'task_scheduler', status: 'skipped', counts: { phase: 'started', window: '04:30' }, started_at: '2026-09-06T09:00:00.000Z' });
  ...
  // QF-20260912-290: brief-assemble's readiness-not-ready outcome is status 'ok' (counts.decision
  // 'wait') because 'wait' is not in RUN_STATUSES -- it must not read as already_ok, or the first
  // wait of the day defeats the deadline fire for the whole rest of the day.
  it('QF-20260912-290: a status ok row with counts.decision "wait" is NOT already_ok (it is not done)', () => {
    expect(inertReasonFor([{ status: 'ok', counts: { decision: 'wait' } }], 'brief-assemble', FIVE_AM_ET)).toBe(null);
    // A genuine ok/imported row with no decision field (every other feeder) is unaffected.
    expect(inertReasonFor([{ status: 'ok', counts: {} }], 'gmail-triage', FIVE_AM_ET)).toBe('already_ok');
    expect(inertReasonFor([{ status: 'ok', counts: { decision: 'assemble' } }], 'brief-assemble', FIVE_AM_ET)).toBe('already_ok');
  });
  it('QF-20260912-290: a day whose first attempt waited still runs the deadline fire (not suppressed as already_ok)', async () => {
    const waitRow = [{ attempt: 1, status: 'ok', counts: { decision: 'wait', window: '05:15' }, finished_at: '2026-09-06T09:20:00.000Z' }];
    const deadline = scripted({ reads: [{ data: waitRow, error: null }] });
    const run = vi.fn(async () => ({ status: 'degraded', counts: { decision: 'assemble_degraded' } }));
    const r = await runFeeder({ feeder: 'brief-assemble', run }, { sb: deadline.sb, env, now: new Date('2026-09-06T09:50:00.000Z'), logger: quiet }); // 05:50 ET
    expect(r).toMatchObject({ ok: true, action: 'run', status: 'degraded' });
    expect(run).toHaveBeenCalledTimes(1);
  });
  // QF-20260912-163: a multi-window feeder's already_ok is scoped to the window that actually ran,
  // so a pre-dawn success no longer silently suppresses the midday/evening re-runs the schedule added.
  it('QF-20260912-163: an ok row from the pre-dawn window does not suppress the scheduled noon window, but does suppress a second pre-dawn run', async () => {
    const NOON_ET = new Date('2026-09-06T16:00:00.000Z'); // 12:00 ET (EDT = UTC-4)
    const okFromPreDawn = [{ attempt: 1, status: 'ok', counts: { window: '04:30' }, finished_at: '2026-09-06T09:05:00.000Z' }];
    const noon = scripted({ reads: [{ data: okFromPreDawn, error: null }] });
    const run = vi.fn(async () => ({ status: 'ok', counts: { threads: 1 } }));
    const rNoon = await runFeeder({ feeder: 'gmail-triage', run }, { sb: noon.sb, env, now: NOON_ET, logger: quiet });
    expect(rNoon).toMatchObject({ ok: true, action: 'run' });
    expect(run).toHaveBeenCalledTimes(1);
    const preDawnAgain = scripted({ reads: [{ data: okFromPreDawn, error: null }] });
    const rPreDawn = await runFeeder({ feeder: 'gmail-triage', run: vi.fn() }, { sb: preDawnAgain.sb, env, now: FIVE_AM_ET, logger: quiet });
    expect(rPreDawn).toMatchObject({ ok: true, action: 'inert', reason: 'already_ok' });
  });
  it('QF-20260912-163: a call outside every scheduled window is inert outside_et_window, naming the window(s) checked', async () => {
    const THREE_PM_ET = new Date('2026-09-06T19:00:00.000Z'); // 15:00 ET (EDT = UTC-4), outside pre-dawn/noon/evening
    const { sb, calls } = scripted();
    const r = await runFeeder({ feeder: 'gmail-triage', run: vi.fn() }, { sb, env, now: THREE_PM_ET, logger: quiet });
    expect(r).toMatchObject({ ok: true, action: 'inert', reason: 'outside_et_window', window: FEEDERS['gmail-triage'].window });
    expect(calls).toHaveLength(0);
  });
...
// QF-20260911-110 / ratification 00f696f1: the class of regression this move exists to close --
// a task_scheduler-venue feeder with a GitHub Actions cron file left behind would mean two
// producers writing the same brief window's rows (the exact bug todoist-brief/brief-assemble had
// until this QF). Generic over every id in FEEDERS, not just the two moved here, so a future
// feeder moved onto task_scheduler is covered automatically.
describe('QF-20260911-110: no task_scheduler-venue feeder has a leftover GitHub Actions workflow file', () => {
  const WORKFLOWS_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '.github', 'workflows');

  it.each(Object.entries(FEEDERS).filter(([, reg]) => reg.venue === 'task_scheduler').map(([id]) => id))(
    '%s (task_scheduler venue) has no michael-%s-cron.yml under .github/workflows',
    (id) => {
      const candidate = path.join(WORKFLOWS_DIR, `michael-${id}-cron.yml`);
