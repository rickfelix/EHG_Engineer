...
import {
  ...
  hhmmToMinutes, inWindow, etMinuteOfDay, isUniqueViolation, staleThresholdMinutes, exitCodeFor,
...
describe('single-flight and attempt (TS-2)', () => {
  ...
  it('retries exactly once on a 23505 and mints attempt max+1 from the re-read', async () => {
    const { sb, calls } = scripted({ reads: [{ data: [], error: null }, { data: [{ attempt: 1, status: 'skipped', counts: {}, started_at: '2026-09-06T08:59:59.000Z', finished_at: null }], error: null }], inserts: [DUP] });
