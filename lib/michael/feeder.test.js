...
describe('pure helpers', () => {
  it('registry lists the ten feeder ids with venues and inclusive windows', () => {
    ...
    expect(FEEDERS['todoist-brief']).toEqual({ venue: 'task_scheduler', window: { start: '04:45', end: '05:30' }, intervalMinutes: 15 });
  ...
  it.each(FEEDER_IDS)('%s: well-formed registry entry (venue enum, window ends on a fire minute)', (id) => {
    ...
    const start = hhmmToMinutes(entry.window.start);
    const end = hhmmToMinutes(entry.window.end);
