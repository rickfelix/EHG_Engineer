...
export async function writeRows(sb, table, op) {
  try {
    ...
    if (error) {
      ...
      // code/details pass through so a caller can branch on 23505 without regex-sniffing the message
      // (SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-D, DATABASE DB-D3); additive — no prior caller reads them.
      return { ok: false, data: null, tables_absent: false, refusal: 'WRITE_FAILED', code: error.code ?? null, details: error.details ?? null, error: `${table}: ${error.message || error.code || 'write failed'}` };
    }
    ...
    return { ok: false, data: null, tables_absent: false, refusal: 'WRITE_FAILED', code: (e && e.code) ?? null, details: (e && e.details) ?? null, error: `${table}: ${e && e.message ? e.message : String(e)}` };
  }
}
...
export function parseArgs(argv) {
  const out = { _: [] };
  // QF-20260907-847: a repeated --flag used to silently overwrite, so a caller passing the SAME
  // flag N times (e.g. `--disposition '{...}' --disposition '{...}'`) got only the last one, with
  // no error and no signal that anything was discarded. A flag's first occurrence stays a scalar
  // (unchanged for the overwhelming majority of single-occurrence callers); the second+ occurrence
  // converts it to an array so a caller that DOES pass repeats gets all of them, matching what
  // feedback-append.mjs's own array branch already assumed the parser provided.
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    const key = a.slice(2);
    const next = argv[i + 1];
    if (next !== undefined && !next.startsWith('--')) { setFlag(key, next); i++; } else { setFlag(key, true); }
  }
  return out;
}
