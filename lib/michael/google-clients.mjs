...
export async function listCalendarEvents({ calendarId, timeMin, timeMax } = {}, { auth, calendarFactory = defaultCalendarFactory, sb, enc, env = process.env } = {}) {
  ...
  try {
    ...
    return { ok: true, events: data && Array.isArray(data.items) ? data.items : [], calendarId: String(calendarId) };
  } catch (e) {
    return failure(e);
  }
}
...
export function driveQuery({ folderId, name }) {
  const parts = [`'${String(folderId).replace(/'/g, "\\'")}' in parents`, 'trashed = false'];
...
export async function listDriveFiles({ folderId, name } = {}, { auth, driveFactory = defaultDriveFactory, sb, enc, env = process.env } = {}) {
  ...
  try {
    ...
    return { ok: true, files: data && Array.isArray(data.files) ? data.files : [] };
  } catch (e) {
    return failure(e);
  }
}
...
export default { listCalendarEvents, listDriveFiles, readDriveFileText, driveQuery };
