...
export async function listThreads({ q, maxResults = THREADS_MAX_RESULTS } = {}, { auth, gmailFactory = defaultGmailFactory, sb, enc, env = process.env } = {}) {
  ...
  const bound = Math.min(Number(maxResults) || THREADS_MAX_RESULTS, THREADS_MAX_RESULTS);
  try {
    const gmail = await gmailFor({ auth, gmailFactory, sb, enc, env });
    ...
    return { ok: true, threads, truncated: threads.length >= bound, q: String(q) };
  } catch (e) {
    return failure(e);
  }
}
...
export async function getThreadMeta({ threadId } = {}, { auth, gmailFactory = defaultGmailFactory, sb, enc, env = process.env } = {}) {
  ...
  try {
    const gmail = await gmailFor({ auth, gmailFactory, sb, enc, env });
    const { data } = await gmail.users.threads.get({ userId: 'me', id: String(threadId), format: 'metadata', metadataHeaders: [...META_HEADERS] });
...
export default { modifyThread, forbiddenLabel, FORBIDDEN_LABELS, listThreads, getThreadMeta, listLabels, headerValue, META_HEADERS };
