...
export async function listThreads({ q, maxResults = THREADS_MAX_RESULTS } = {}, { auth, gmailFactory = defaultGmailFactory, sb, enc, env = process.env } = {}) {
  ...
  // A positive integer is honoured up to 200; anything else (null, 0, true, NaN, 1.5) means the default.
  const n = typeof maxResults === 'number' || typeof maxResults === 'string' ? Number(maxResults) : NaN;
  const bound = Number.isInteger(n) && n >= 1 ? Math.min(n, THREADS_MAX_RESULTS) : THREADS_MAX_RESULTS;
  try {
    const gmail = await gmailFor({ auth, gmailFactory, sb, enc, env });
