 ...
 *
 * @param {string} text
 * @param {{domain?: string, sectionHeadingRe?: RegExp|RegExp[]}} opts - when either is omitted,
 *   this file has no known heading-per-rule convention (confirmed live for body-section.md and
 *   CLAUDE.md) -- the whole text is reported as unparsed rather than guessing a structure the
 *   corpus doesn't use. sectionHeadingRe accepts an array (QF-20260907-610): a file's rule-shaped
 *   "### " headings do not always sit under one "## " parent -- measured live, gmail.md/todoist.md
 *   each have exactly one, but morning-brief-distillation.md's 8 rule-shaped headings are spread
 *   across three separate "## " sections. Each regex extracts its own bounded section (mirrors
 *   parseDoctrine's DOCTRINE_SECTIONS pattern below) and their blocks are unioned.
 */
export function parseRuleFile(text, { domain, sectionHeadingRe } = {}) {
  ...
  const rules = [];
  const unparsed = [];
  for (const re of Array.isArray(sectionHeadingRe) ? sectionHeadingRe : [sectionHeadingRe]) {
    const section = extractSection(text, re);
    if (section === null) { unparsed.push(`section not found: ${re}`); continue; }
