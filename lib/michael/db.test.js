...
describe('parseArgs / refusal', () => {
  ...
  it('a single occurrence of a flag stays a scalar (QF-20260907-847 regression guard)', () => {
    expect(parseArgs(['--disposition', '{"a":1}']).disposition).toBe('{"a":1}');
  });
  it('QF-20260907-847: repeated --key value flags collect into an array instead of the last one silently winning', () => {
    const out = parseArgs(['--disposition', '{"a":1}', '--disposition', '{"a":2}', '--disposition', '{"a":3}']);
    expect(out.disposition).toEqual(['{"a":1}', '{"a":2}', '{"a":3}']);
  });
  it('QF-20260907-847: repeated --key=value flags collect the same way', () => {
    expect(parseArgs(['--tag=x', '--tag=y']).tag).toEqual(['x', 'y']);
  });
  it('QF-20260907-847: repeated bare boolean flags collect into an array of true, not silently deduped', () => {
    expect(parseArgs(['--acted', '--acted']).acted).toEqual([true, true]);
  });
...
describe('writeRows code passthrough (child D, DB-D3)', () => {
  it('surfaces error.code and details beside the message so a caller can branch on 23505', async () => {
    const sb = stubClient(() => ({ data: null, error: { code: '23505', details: 'Key (et_date, feeder, attempt)=(...) already exists.', message: 'duplicate key value violates unique constraint "x"' } }));
    const w = await writeRows(sb, 'michael_feeder_runs', (t) => t.insert({ feeder: 'gmail-triage' }));
    expect(w).toMatchObject({ ok: false, refusal: 'WRITE_FAILED', code: '23505', tables_absent: false });
    expect(w.details).toContain('already exists');
    expect(w.error).toContain('michael_feeder_runs: duplicate key');
  });
  it('a thrown error without a code yields code null', async () => {
    const sb = { from: () => { throw new Error('network down'); } };
    const w = await writeRows(sb, 'michael_feeder_runs', (t) => t.insert({}));
    expect(w).toMatchObject({ ok: false, refusal: 'WRITE_FAILED', code: null, details: null });
  });
});
