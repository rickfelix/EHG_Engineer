...
import { modifyThread, forbiddenLabel, FORBIDDEN_LABELS, listThreads, getThreadMeta, listLabels, headerValue, META_HEADERS, THREADS_MAX_RESULTS } from './gmail-client.mjs';
...
describe('read legs (child D FR-2, TS-16)', () => {
  ...
  it('getThreadMeta requests format metadata with exactly the four headers and never a body; reads the last message', async () => {
    ...
    expect(calls[1]).toEqual(['threads.get', { userId: 'me', id: 't9', format: 'metadata', metadataHeaders: ['From', 'Subject', 'List-Id', 'Date'] }]);
