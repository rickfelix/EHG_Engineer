...
// QF-20260908-903: prose describing HISTORICAL work is not a live functional dependency on the
// _Cowork folder. scripts/one-off/** are fleet-wide, one-time retro/evidence write-ups (a new one
// lands with nearly every Michael-child SD and will keep mentioning "_Cowork" in past tense
// indefinitely); scripts/temp/** is .gitignore'd scratch output that was never committed; and
// docs/plans/archived/** holds superseded plan snapshots. None of these are reachable from a live
// code path the way scripts/michael/**.mjs is.
const ALLOW_PREFIXES = Object.freeze(['docs/michael/', 'scripts/one-off/', 'scripts/temp/', 'docs/plans/archived/']);
...
export function isAllowListed(relPath) {
  ...
  return ALLOW_PREFIXES.some((prefix) => norm.startsWith(prefix)) || ALLOW_LIST.includes(norm);
}
