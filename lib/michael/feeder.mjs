...
export function exitCodeFor(result) {
  ...
  if (result.action === 'run' && (result.status === 'failed' || result.finish_write_ok === false)) return 1;
  return 0;
}
...
export async function runFeeder(config = {}, deps = {}) {
  ...
  // upstream accepts feeder ids or FR-1(f) objects { feeder, accept: ['ok','degraded'] }.
  const ups = upstream.map((u) => (typeof u === 'string' ? { feeder: u, accept: ['ok', 'degraded'] } : { feeder: u.feeder, accept: u.accept || ['ok', 'degraded'] }));
  if (ups.length) {
