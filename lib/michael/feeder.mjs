...
export const FEEDERS = Object.freeze({
  ...
  // Child E (FR-2): the GHA assembler. Same window assembleReadiness's BRIEF_DEADLINE_ET (05:45) sits
  // inside — a mismatch here would silently make runFeeder's own window gate drop the last retry.
  'brief-assemble': Object.freeze({ venue: 'gha', window: Object.freeze({ start: '05:15', end: '06:00' }), intervalMinutes: 15 }),
  ...
  // credential needed, so this runs on GHA like todoist-brief/brief-assemble, not task_scheduler.
 ...
 * review of PR 8364).
 */
export function inertReasonFor(rows, feeder, now) {
  if (rows.some((r) => r.status === 'ok' || r.status === 'imported')) return 'already_ok';
...
async function claimAttempt(sb, { feeder, etDate, venue, now, prior }, { maxRetries = 1 } = {}) {
  ...
  for (let i = 0; i <= maxRetries; i++) {
    if (i > 0) {
      ...
      rows = again.rows;
      const reason = inertReasonFor(rows, feeder, now);
...
export async function runFeeder(config = {}, deps = {}) {
  ...
  const priorReason = inertReasonFor(prior.rows, feeder, now);
