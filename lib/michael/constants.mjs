...
export const MICHAEL_CONSTANTS = Object.freeze({
  ...
  MICHAEL_GMAIL_MODIFY_CEILING: Object.freeze({ required: false, default: 60, parse: 'positiveInt' }),
});

function parseValue(name, raw, spec) {
  if (spec.parse === 'positiveInt') {
    const n = Number(raw);
    if (!Number.isInteger(n) || n < 1) return { ok: false, refusal: 'CONSTANT_INVALID', variable: name, message: `${name} must be a positive integer (got ${JSON.stringify(raw)})` };
...
export function resolveConstant(name, env = process.env) {
  const spec = MICHAEL_CONSTANTS[name];
