...
/**
 * Delete a disposition row outright (FR-4, QF-20260912-746 / SD-LEO-FIX-FIX-DOMAIN-REGISTRAR-001).
 * Used ONLY to clear a stuck `awaiting_disposition` row once the registrar itself has
 * independently confirmed the underlying purchase did NOT happen — never to mark a
 * disposition "resolved" by trusting a caller's claim alone. Deleting (rather than
 * transitioning to some new status) lets a subsequent recordDisposition() create a
 * genuinely fresh row, matching the idempotency-key unique-insert path every other
 * caller already relies on.
 *
 * @returns {Promise<{deleted: boolean}>}
 */
export async function deleteDisposition(supabase, questionKey) {
  assertSupabase(supabase);
  const { error } = await supabase
    ...
    .eq('idempotency_key', questionKey);
  if (error) {
    throw new Error(`deleteDisposition: delete failed: ${error.message}`);
  }
  return { deleted: true };
}
