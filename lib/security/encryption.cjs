...
class CredentialEncryption {
   ...
   * Decrypt data
   */
  async decrypt(encryptedData, _metadata = {}) {
    try {
      const masterKey = await this.getMasterKey();
...
// SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-C: also export the class so a caller can bind its own
// getMasterKey() (host .env key, refuse-never-generate) without duplicating the AES-GCM/PBKDF2 code.
module.exports.CredentialEncryption = CredentialEncryption;
