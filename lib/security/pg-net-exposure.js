 ...
 *
 * REACHABILITY HAS TWO AXES, BOTH PROBED (SECURITY sub-agent finding C1, EXEC-TO-PLAN):
 * object-level privileges (has_function_privilege/has_table_privilege, checked above) are
 * necessary but NOT sufficient — Postgres also requires schema-level USAGE on `net`
 * itself. A probe that only checked object ACLs would keep reporting today's exposure
 * FOREVER even after a successful fix, if that fix revokes schema USAGE (the single most
 * likely remediation shape) rather than touching each object individually — and schema
 * USAGE is exactly what Supabase's own `extensions.grant_pg_net_access()` event trigger
 * re-grants (`GRANT USAGE ON SCHEMA net TO ... anon, authenticated, service_role`) on
 * every `CREATE EXTENSION`/`ALTER EXTENSION pg_net UPDATE` — the strongest reason this
 * probe needs to watch that axis, not the weakest.
 *
...
/**
 * Schema-USAGE query — SECURITY sub-agent finding C1 (EXEC-TO-PLAN review). Object-level
 * privileges (has_function_privilege/has_table_privilege above) are necessary but NOT
 * sufficient for reachability: Postgres also requires USAGE on the containing SCHEMA. A
 * probe that only checks object ACLs would keep reporting "10 functions, 3 relations
 * exposed" FOREVER even after a successful fix, if that fix revokes schema USAGE rather
 * than touching every object individually — the single most likely remediation shape,
 * and the exact vector Supabase's own `extensions.grant_pg_net_access()` event trigger
 * re-grants (`GRANT USAGE ON SCHEMA net TO ... anon, authenticated, service_role`) on
 * every `CREATE EXTENSION`/`ALTER EXTENSION pg_net UPDATE`. Checking this axis is what
 * lets the sentinel actually observe that re-grant if it ever fires again.
 */
export const PG_NET_SCHEMA_EXPOSURE_SQL = `
 ...
 * @returns {Promise<{probe_ran: boolean, reason: string|null, functions_at_risk: number|null, relations_at_risk: number|null, functions: Array<object>, relations: Array<object>, schema_usage: {anon: boolean, authenticated: boolean}|null}>}
 */
export async function probePgNetExposure({ connect } = {}) {
  ...
  const failure = (reason) => ({
    probe_ran: false, reason, functions_at_risk: null, relations_at_risk: null, functions: [], relations: [], schema_usage: null,
  });
  ...
  try {
    ...
    const schemaRes = await client.query(PG_NET_SCHEMA_EXPOSURE_SQL);

    // Uninterpretable payload on ANY of the three axes must not render as a clean result
    // on the others — matches the definer-exposure.js precedent's own "the fix for this
    // fix" note.
    ...
    if (!schemaRes || !Array.isArray(schemaRes.rows) || schemaRes.rows.length !== 1) {
      return failure(`catalog query returned an uninterpretable payload (expected exactly 1 row from the schema-usage query, got ${schemaRes ? typeof schemaRes.rows : 'no result'})`);
    }
    const schemaRow = schemaRes.rows[0];
    // Row-CONTENT validation, not just row-count: has_schema_privilege() always returns a
    // real boolean for the anon/authenticated roles, which always exist in Supabase — a row
    // present but missing/mistyped these keys (future column-alias typo, driver change) is
    // exactly as uninterpretable as a wrong row count, and must not silently coerce to
    // `false` (a fabricated "not exposed" on the newest, most safety-relevant axis).
    if (typeof schemaRow?.anon_usage !== 'boolean' || typeof schemaRow?.authenticated_usage !== 'boolean') {
      return failure(`catalog query returned an uninterpretable payload (expected boolean anon_usage/authenticated_usage columns from the schema-usage query, got keys: ${schemaRow ? Object.keys(schemaRow).join(', ') || 'none' : 'no row'})`);
    }
    ...
    return {
      ...
      functions, relations,
      schema_usage: { anon: schemaRow.anon_usage, authenticated: schemaRow.authenticated_usage },
    };
  } catch (error) {
