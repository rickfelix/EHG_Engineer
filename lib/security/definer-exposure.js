 ...
 * from here.
 *
 * SCHEMA SCOPE covers every application schema that carries this class, measured rather than
 * assumed: public (44), governance (10), portfolio (5) = 59.
 *
 * This started as `public` only, which under-reported the class by 25%. The narrowing was mine
 * and it was wrong: adversarial review measured 15 further members in governance/portfolio, and
 * `anon` AND `authenticated` hold USAGE on BOTH those schemas, so all 15 are genuinely
 * reachable — not a theoretical tail. Shipping a blind-spot fix with a self-inflicted 25% blind
 * spot would have committed the exact defect class the SD is about. Verified when widening:
 * every other non-system schema contributes ZERO rows, so this list is complete today and adds
 * no Supabase-managed vendor noise. Re-measure the schema list rather than trusting this comment
 * if a new application schema is introduced.
 *
...
export const DEFINER_EXPOSURE_SQL = `
  ...
  WHERE n.nspname IN ('public', 'governance', 'portfolio')
...
export function classifyDefinerExposure(rows = []) {
  return (Array.isArray(rows) ? rows : [])
    // All three axes use strict === true. The middle axis previously read
    // `security_definer !== false`, which admits null/undefined/'false' while its two neighbours
    // demanded strict true — an asymmetry that made the "two-sided by construction" claim in the
    // docstring true of only two of the three axes. Conservative in direction (it over-reported
    // rather than under-reported), but an inconsistent predicate is how a real hole gets argued
...
export async function probeDefinerExposure({ connect } = {}) {
  ...
  try {
    ...
    // THE FIX FOR THIS FIX. A query that RESOLVES with a payload we cannot interpret is NOT a
    // clean catalog. Without this check, `classifyDefinerExposure(undefined)` returns [] and the
    // probe reports { probe_ran:true, functions_at_risk:0 } — byte-identical to a genuinely clean
    // result, which is precisely the defect class this whole SD exists to remove. Caught in
    // adversarial review of this very commit: the repair had reproduced its own bug one layer up.
    // Unreachable through node-postgres (which always yields an array), but `connect` is a
    // first-class injectable seam (lib/sub-agents/security.js passes options.catalog_connect), so
    // the malformed-success path is real for any non-pg caller.
    if (!res || !Array.isArray(res.rows)) {
      return {
        probe_ran: false,
        reason: `catalog query returned an uninterpretable payload (expected rows[], got ${res ? typeof res.rows : 'no result'})`,
        functions_at_risk: null,
        functions: []
      };
    }
