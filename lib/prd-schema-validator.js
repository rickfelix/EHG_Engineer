...
export const PRD_SCHEMA = {
  ...
  // SD ID SCHEMA CLEANUP (2025-12-12): sd_id is now the canonical FK to strategic_directives_v2.id
  id: { type: 'VARCHAR', required: true, maxLength: 100 },
  directive_id: { type: 'VARCHAR', required: false, maxLength: 50, deprecated: true, note: 'Legacy column - use sd_id instead' },
  sd_id: { type: 'VARCHAR', required: true, maxLength: 50, note: 'Foreign key to strategic_directives_v2.id - CRITICAL for handoffs' },
  ...
  failure_modes: { type: 'JSONB', required: false, default: '[]', note: 'US-004: Document anticipated failure scenarios and rollback strategies' },
...
export function validatePRDSchema(prdData) {
  ...
  Object.entries(prdData).forEach(([field, _value]) => {
  ...
  // Critical check: sd_id must be present (required for handoffs)
  // SD ID SCHEMA CLEANUP (2025-12-12): sd_id references strategic_directives_v2.id
  if (!prdData.sd_id && !prdData.directive_id) {
    errors.push('CRITICAL: Missing sd_id field (required for handoff validation)');
    suggestions.sd_id = 'Use strategic_directives_v2.id as the foreign key value';
  }
