...
// SD-LEO-INFRA-CLAIM-LIFECYCLE-RELEASE-002 (FR-9): exported as a test seam. FR-9 states this
// function has "its client pinned to a singleton" — it does NOT: `supabase` is already the first
// parameter, so it was always injectable and only the export was missing. Recording the correction
// because it makes FR-9 smaller here than specified, not larger.
//
// This is also the site FR-6 targets: the QF branch below routes through the shared fail-closed CAS,
// while the SD branch issues a bare update with no compare-and-set and will overwrite a foreign
// holder. Exporting it is what lets that be asserted in CI rather than argued.
export async function reaffirmClaimColumns(supabase, sdKey, sessionId) {
  if (sdKey.startsWith('QF-')) {
    ...
    // UNCONDITIONALLY overwrote whoever held the row — including a live peer. The QF branch above
    // was hardened against exactly this and the SD branch was left behind.
    //
    // REAFFIRM IS NOT CLAIM. Its only legitimate outcomes are "re-assert the claim I already hold"
    // and "take a row nobody holds". Stealing is never one of them, so the CAS is null-OR-self,
    // matching the QF branch's guard rather than inventing a second rule.
    const { data: reaffirmed, error: reaffirmErr } = await supabase
    ...
    //
    // SD-LEO-INFRA-CHECKER-READBACK-WRITE-001 (FR-5) disposition: this projection off the SAME
    // UPDATE...RETURNING statement is NOT an independent re-read (a fresh SELECT would observe a
    // post-statement trigger; RETURNING would not — see lib/checkers/readback-checker.mjs TR-1).
    // Left as-is here deliberately — this function's own null-OR-self CAS + WRITE_FAULT/NOT_HELD/
    // READBACK_MISMATCH state machine is out of this SD's scope — but flagged as a candidate for a
    // follow-on checker-skills-program SD to adopt the shared verifyReadback() primitive.
...
export async function claimGuard(sdKey, sessionId, options = {}) {
  ...
  const { data: sdStatusRow, error: sdStatusError } = await supabase
    .from('strategic_directives_v2')
    .select('status, cancellation_reason, metadata')
    .eq('sd_key', sdKey)
    .maybeSingle();
  ...
  if (!sdStatusError && sdStatusRow && (sdStatusRow.status === 'completed' || sdStatusRow.status === 'deferred')) {
    // QF-20260912-346: carry the park provenance through so a caller (sd-start.js) can tell
    // a coordinator/role-seat park apart from one the SAME session set -- printing the
    // unpark remedy unconditionally let a worker undo a coordinator's park by following the
    // tool's own printed instruction.
    const md = (sdStatusRow.metadata && typeof sdStatusRow.metadata === 'object') ? sdStatusRow.metadata : {};
    return {
      success: false,
      error: 'sd_terminal_status',
      status: sdStatusRow.status,
      sdKey,
      parkedBy: md.parked_by ?? null,
      parkedBySession: md.stamped_by_session ?? null,
      parkReleaseCondition: md.park_release_condition ?? null,
      parkReviewAt: md.park_review_at ?? null,
    };
  }
  ...
  // SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-C: filter on sd_id, NOT sd_key. sd_baseline_items has no
  // sd_key column — the filter here returned 42703 and wedged the claim path for every orchestrator
  // parent once SCHEMA-TRUTH-001-A made the client factory throw on a missing column instead of
  // returning an empty result. The throw is correct and stays; this query was the bug it exposed.
  //
  // sd_id IS the sd_key STRING, confusingly named. Documented in two places that already sat next to
  // the right answer: lib/fleet/ensure-active-baseline.cjs:19-20 (v_sd_next_candidates JOINs
  // sd_baseline_items.sd_id = strategic_directives_v2.sd_key) and lib/sd-baseline/build-item.js:75
  // ("sd_id is ALWAYS sd.sd_key, never sd.id"). The two sibling call sites in
  // lib/session-conflict-checker.mjs:256,294 already filter on sd_id correctly — this was the outlier.
  // sdKey is the key string here: it is passed straight to claim_sd as p_sd_id just below.
...
/**
 * SD-LEO-INFRA-DEFERRED-STATE-ENTRANCE-001 (FR-1/FR-7): the exact `sd:unpark` command
 * line, named in exactly ONE place so claim-guard's banner and sd-start.js's
 * TARGET_ALREADY_TERMINAL message can never drift out of sync with each other.
 * @param {string} sdKey
 * @returns {string}
 */
export function buildUnparkExitCommand(sdKey) {
  return `npm run sd:unpark -- ${sdKey} --reason "<why>"`;
}

/**
 * Pure, unit-testable refusal text for claim-guard's own status='deferred' (parked)
 * banner — deferred is NOT finished/closed, it has a live exit. scripts/sd-start.js's
 * TARGET_ALREADY_TERMINAL message is a SEPARATE, independently-authored block (static-pin
 * tested, see tests/unit/sd-start-terminal-target-deferred-branch.test.js) that shares
 * only the actual exit command via buildUnparkExitCommand above -- an earlier draft tried
 * to also share this banner's prose via a common function, but that broke an established
 * regression test (QF-20260704-825) pinning sd-start.js's own source text, so the two
 * surfaces' wording is independently maintained, not unified.
 * TS-5 (PLAN-TO-EXEC TESTING review, evidence e03cc24e-6d29-4dd5-ba23-777ffd66c88f).
 * @param {string} sdKey
 * @returns {string[]}
 */
export function buildDeferredRefusalLines(sdKey) {
  return [
    '╔══════════════════════════════════════════════════════════╗',
    '║  CLAIM GUARD: ITEM IS DEFERRED — CANNOT CLAIM DIRECTLY  ║',
    '╚══════════════════════════════════════════════════════════╝',
    '  Status:    deferred',
    '',
    '  This item is PARKED (deferred), not finished. Restore it with:',
    '',
    `    ${buildUnparkExitCommand(sdKey)}`,
    '',
    '  Deferred items cannot be claimed directly until unparked.',
    '',
  ];
}
...
export function formatClaimFailure(result) {
  ...
  if (result.error === 'sd_terminal_status') {
    ...
    // it has a live, allowlisted exit (npm run sd:unpark). completed (and, via the claim_sd
    // RPC guard, cancelled/escalated QFs) remain genuinely terminal and keep the unchanged
    // banner below (TS-7 regression guard).
    if (st === 'deferred') {
      return buildDeferredRefusalLines(result.sdKey || '<SD-KEY>').join('\n');
    }
    return [
      '╔══════════════════════════════════════════════════════════╗',
