...
export async function claimGuard(sdKey, sessionId, options = {}) {
  ...
  // SD-LEO-INFRA-BLOCK-CLAIMS-CANCELLED-001 FR-1: Refuse claims on cancelled SDs.
  // Three-layer defense: this is layer 1 (claim-guard pre-acquire). Layers 2+3 are
  // assertValidClaim cancelled-status assertion (FR-2) and a BEFORE-UPDATE PG
  // trigger (FR-5). The check runs AFTER fetchClaimTTL to avoid an extra round-trip
  // on the cold-start path, but BEFORE any claim_sd RPC or write — fail-closed.
  // QFs use a different table (quick_fixes); only SDs need the cancelled refusal.
  if (!sdKey.startsWith('QF-')) {
    const { data: sdRow, error: sdLookupErr } = await supabase
      .from('strategic_directives_v2')
      .select('status, cancellation_reason')
      .eq('sd_key', sdKey)
      .maybeSingle();
    if (!sdLookupErr && sdRow && sdRow.status === 'cancelled') {
      return {
        success: false,
        error: `sd_cancelled: ${sdRow.cancellation_reason || 'no reason recorded'}`,
        owner: undefined,
        cancellation_reason: sdRow.cancellation_reason || null
      };
    }
    // Fail-open on lookup error — claim-validity-gate (FR-2) and PG trigger (FR-5)
...
export function formatClaimFailure(result) {
  ...
  // SD-LEO-INFRA-BLOCK-CLAIMS-CANCELLED-001 FR-1: render a distinct banner for
  // the sd_cancelled refusal so operators see the cancellation_reason instead
  // of the generic "claimed by another session" message.
  if (typeof result.error === 'string' && result.error.startsWith('sd_cancelled')) {
    return [
      '╔══════════════════════════════════════════════════════════╗',
      '║  CLAIM GUARD: SD HAS BEEN CANCELLED                      ║',
      '╚══════════════════════════════════════════════════════════╝',
      `  Reason: ${result.cancellation_reason || 'no reason recorded'}`,
      '',
      '  This SD has been strategically cancelled. Pick a different SD.',
      '  If this is unexpected, check the cancellation history.',
      ''
    ].join('\n');
  }
