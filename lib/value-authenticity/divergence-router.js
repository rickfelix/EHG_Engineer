...
/**
 * research-engine.js's runResearch() concatenates `context` directly into a
 * template literal (`\`## Context\n\n${context}\``) -- it expects a STRING,
 * not an object. Passing an object silently renders as the literal text
 * "[object Object]", losing the actual context (adversarial-review finding:
 * exactly the decorative-grounding failure mode this SD exists to prevent,
 * applied to the research INPUT rather than its output).
 */
function stringifyResearchContext(context) {
  if (!context || typeof context !== 'object' || Object.keys(context).length === 0) return undefined;
 ...
 * @param {object} params
 * @param {string} params.question
 * @param {string} [params.reviewId] - required for the 'judgment' route; scopes
 *   the decision-binding question_key so two different reviews that happen to
 *   share generic question text never collide onto the same disposition row
...
export async function routeDivergence(classification, {
  ...
  switch (classification.type) {
    case 'factual':
      return { action: 'deep_research', result: await runResearchFn({ question, context: stringifyResearchContext(panelContext) }) };

    case 'judgment': {
      if (!supabase) throw new Error('routeDivergence: supabase client is required for the judgment (chairman) route');
      if (!reviewId) throw new Error('routeDivergence: reviewId is required for the judgment (chairman) route -- prevents cross-review question_key collisions');
      // subject includes reviewId (not just question) so two DIFFERENT
      // reviews with coincidentally identical question text get distinct
      // question_keys; repeated escalations of the SAME review+question
      // still correctly dedup onto the same disposition row.
      const { row, created } = await recordDispositionFn(supabase, {
        decisionType: 'ratification',
        subject: { question, reviewId },
        decisionKey: `value-authenticity-judgment:${reviewId}`,
      });
...
export { STAKES_LEVELS, stringifyResearchContext };
