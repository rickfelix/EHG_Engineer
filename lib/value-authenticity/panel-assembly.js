 ...
 * @param {object} params
 * @param {string} params.question
 * @param {string} params.reviewId - required, e.g. "<sdKey>:<frId>". Scopes
 *   decision-binding escalations so two DIFFERENT reviews that happen to
 *   share generic question text (adversarial-review finding: plausible with
 *   phrasing like "system generates a personalized pricing recommendation
 *   score") never collide onto the same disposition row and silently drop
 *   each other's context. Required, not defaulted -- a silent fallback here
 *   would just relocate the collision risk, not close it.
 * @param {object} [params.context]
...
export async function runIterativeReview({
  ...
  if (!reviewId) throw new Error('runIterativeReview: reviewId is required (scopes chairman escalations to this specific review)');

  if (!shouldRunFullPanel(question)) {
    const research = await runResearchFn({ question, context: stringifyResearchContext(context) });
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    if (round === maxRounds) {
      ...
      const { row } = await recordDisposition(supabase, {
        ...
        decisionKey: `value-authenticity-round-cap:${reviewId}`,
    ...
    routing = await routeDivergenceFn({ type: routeType }, {
      question, reviewId, panelContext: workingContext, supabase,
    });
