...
// QF-20260903-936: shared with lib/fleet/best-effort-release.mjs -- see that module's doc comment.
import { recordOrphanedWorktree } from '../fleet/record-orphaned-worktree.mjs';
...
export async function releaseClaimBothSurfaces(sb, opts = {}) {
  // QF-20260912-175: 'released' retires the WHOLE session (terminalSessionUpdate stamps
  // is_alive:false), which is correct only when the holder is actually gone (dead, or itself
  // exiting/reconciling). The safe default is 'idle' — clear this claim, keep the seat — so a
  // caller that forgets to pass sessionStatus never silently evicts a still-working holder from
  // the roster/capacity/idle gauges. Callers that genuinely mean "this holder is retired" must
  // say so explicitly.
  const { sdKey, reason = 'release', sessionStatus = 'idle', tryRpc = false, readback = true } = opts;
  ...
  // QF-20260903-936: capture what THIS holder had provisioned before it is cleared below --
  // neither release path reads these columns before nulling them.
  let worktreePath = null, worktreeBranch = null;
  try {
    const { data: sessRow, error: worktreeReadErr } = await sb
      .from('claude_sessions')
      .select('worktree_path, worktree_branch')
      .eq('session_id', holder)
      .maybeSingle();
    // Fail-soft by design: this capture is best-effort informational context for the
    // finding below, never a release blocker. A read error just means the finding (if any)
    // is emitted without a path -- still logged via safeQuery's own error surface elsewhere
    // in this module, not silently discarded here.
    if (!worktreeReadErr) {
  ...
  if (tryRpc && sessionStatus === 'released') {
    ...
    if (!sessReadErr && sess && sess.sd_key === sdKey) {
      ...
      if (!rpcErr && !(rpcRes && rpcRes.success === false)) {
        ...
        recordOrphanedWorktree(sb, { sdKey, holder, worktreePath, worktreeBranch, reason });
  ...
  // SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-E (FR-1): sessionStatus is a genuine either/or (retire vs
  // keep-alive unclaim) -- only 'released' routes through the terminal-status chokepoint (which
  // adds is_alive:false); 'idle' is not a terminal state and must never carry it.
  const sessionUpdatePayload = sessionStatus === 'released'
    ? terminalSessionUpdate('released', {
        sd_key: null,
    ...
    : {
        ...
        status: sessionStatus,   // 'idle' (keep-alive unclaim)
        released_at: new Date().toISOString(),
        released_reason: reason,
      };
...
async function verifyOldHolderGone(sb, sdKey, holder, result) {
  try {
    // SECURITY-RELEVANT: a query fault must never read as "row absent" here -- that would
    // FALSELY CONFIRM the old holder is gone on both surfaces when the readback simply could
    // not check. safeQuery throws on a real fault, and this function's own catch below already
    // sets result.ok = false on any throw -- never a false-positive release confirmation.
    const sd = await safeQuery(
      sb
        ...
        .eq('sd_key', sdKey)
        .maybeSingle(),
      { site: 'release-claim-both-surfaces:readback_sd' }
    );
    const sess = await safeQuery(
      sb
        ...
        .eq('sd_key', sdKey)
        .maybeSingle(),
      { site: 'release-claim-both-surfaces:readback_session' }
    );
