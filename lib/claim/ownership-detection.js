...
export async function getClaimHolder(sdKey, supabase) {
  ...
  const { data: session, error: sessErr } = await supabase
    ...
    // QF-20260902-724: was last_heartbeat, a phantom column (claude_sessions has heartbeat_at)
    // that made this select error on every call, so getClaimHolder always returned null and
    // every SD read unclaimed to it. Fixed to the real column.
 ...
 *
 * CORRECTION (SD-LEO-ORCH-CAPA-RECORD-TRUTH-001-B, 2026-09-06): this docstring previously
 * claimed "Used by sweep and dashboard to render active workers." That was false -- a
 * repo-wide audit found zero production call sites: scripts/stale-session-sweep.cjs never
 * imports this function, and the module's only two production importers
 * (scripts/fleet-dashboard.cjs, scripts/modules/handoff/executors/BaseExecutor.js) use
 * isClaimedBy/getClaimHolder only. This function currently has no production call site --
 * it is defensive/future-use code, not wired into the sweep or dashboard as previously
 * documented. (The stale claim already misled SD-LEO-INFRA-COUNT-TRUNCATION-DISCIPLINE-001
 * FR-6 batch 8, which justified pagination hardening here on the same false premise.)
 *
 * @param {Object} supabase
...
export async function getLiveClaimHolders(supabase) {
  ...
  let sessions;
  try {
    // QF-20260902-724: same phantom-column fix as getClaimHolder above.
...
export function classifyHoldingStatus(session) {
  ...
  const heartbeatSec = heartbeatAgeSeconds(session.heartbeat_at);
