...
function isBuildForbiddenSession(metadata) {
  ...
  // QF-20260904-968: is_coordinator can be written as either boolean true or the JSON
  // string 'true' (claude_sessions.metadata storage shape) -- both must be forbidden,
  // matching lib/coordinator/resolve.cjs's election filter, role-comms-guard.cjs, and
  // seat-idle-predicate.mjs's coordinator-flag axis, which already accept both shapes.
  return md.non_fleet === true || md.role === 'adam' || md.is_coordinator === true || String(md.is_coordinator) === 'true';
}
