...
async function getNextWorkableSD(supabase, excludeKeys = []) {
  const { safeQuery } = await import('../db/safe-query.mjs');
  ...
  const activeSessions = await safeQuery(
    ...
    { site: 'queue-resolver:active_sessions' }
  );
  ...
  const candidates = await safeQuery(
    supabase
      ...
      .limit(20),
    { site: 'queue-resolver:candidates' }
  );
  ...
  if (error || !data || data.length === 0) {
    ...
    const parent = await safeQuery(
      supabase
        ...
        .eq('sd_key', sdKeyOrId)
        .single(),
      { site: 'queue-resolver:orchestrator_children_parent_resolve' }
    );
...
async function getActiveClaimSessionIds(supabase) {
  ...
  const data = await safeQuery(
    supabase
      ...
      .not('sd_key', 'is', null)
      .in('status', ['active', 'idle']),
    { site: 'queue-resolver:active_claim_session_ids' }
  );
...
async function findUnclaimedChild(supabase, parentSdKey, { selectNextReadyChild } = {}) {
  ...
  const { safeQuery } = await import('../db/safe-query.mjs');
  const parentRow = await safeQuery(
    supabase
      .from('strategic_directives_v2')
      .select('id')
      .eq('sd_key', parentSdKey)
      .single(),
    { site: 'queue-resolver:find_unclaimed_child_parent_row' }
  );
  if (parentRow) {
