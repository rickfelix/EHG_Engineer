 ...
 */
export async function writeConsumptionReceipt(client, { reportId, lane, metadata = {} } = {}) {
  ...
  try {
    ...
    return { written: true, reason: RECEIPT_OUTCOME.WRITTEN, lane, id: data?.id ?? null };
  } catch (e) {
    // A throw is still a refusal, never a success. Callers get one shape regardless of how the
    // write failed, so no caller needs its own try/catch to decide what happened.
