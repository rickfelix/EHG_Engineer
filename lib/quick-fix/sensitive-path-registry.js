 ...
 */
/**
 * Template/reference env files carry no real secrets and are conventionally safe to edit —
 * excluded so `.env*` doesn't false-positive on them (EXEC-phase prospective TESTING
 * evidence, C5).
 */
export const ENV_TEMPLATE_ALLOWLIST = Object.freeze([
  /(^|\/)\.env\.example$/i,
  /(^|\/)\.env\.sample$/i,
  /(^|\/)\.env\.template$/i
]);
...
export const SENSITIVE_PATH_PATTERNS = Object.freeze([
  ...
  '**/policy/**',
  'config/*policy*',
  // hooks (SECURITY EXEC-phase evidence, 59bc9c16: .claude/hooks/** and scripts/hooks/**
  // are NOT the git-enforced hooks path on this repo — core.hooksPath=.husky/_, so a QF
  // modifying .husky/pre-commit ran on every future commit while passing the old pattern set)
  '.husky/**',
  '.githooks/**',
  'hooks/**',
  'lib/hooks/**',
...
export function isSensitivePath(filePath, patterns = SENSITIVE_PATH_PATTERNS) {
  ...
  if (ENV_TEMPLATE_ALLOWLIST.some((re) => re.test(normalized))) return false;
  // nocase: true (SECURITY EXEC-phase evidence, 59bc9c16) — without it, a tracked file like
  // docs/SECRETS-SETUP-CHECKLIST.txt silently evades **/*secret* purely by casing, a fail-open
  // on this win32 repo where the working tree is case-insensitive even though git is not.
  return patterns.some((p) => minimatch(normalized, p, { dot: true, matchBase: false, nocase: true }));
}
