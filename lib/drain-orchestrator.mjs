...
export class DrainOrchestrator {
   ...
   */
  async loadQueue() {
    // Active-SD predicate (status + is_active + archived_at) is sourced from the shared
    // lib/sd/active-sd-predicate.js module (FR-6 of SD-EVA-SUPPORT-CLI-SKILL-ORCH-001-C).
    // Drain-specific filter `claiming_session_id IS NULL` remains here — that is consumer-specific
    // (drain only wants unclaimed SDs) and not part of the shared "active SD" definition.
    //
    // Behavior change vs prior inline filter: `is_active = true` → `is_active IS NULL OR = true`.
    // As of CP-1 ship date, 0 rows in (draft|in_progress|active) have is_active=NULL, so this is
    // forward-compatible — no immediate row-set change.
    const baseQuery = this.supabase
      .from('strategic_directives_v2')
      .select('sd_key, title, sd_type, priority, current_phase, category, parent_sd_id, dependencies, metadata');
    const { data, error } = await getActiveSDFilter(baseQuery)
