...
export async function loadKeySet(supabase) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('sd_key');
    if (error) return fail(OWNER_REASON.KEY_SET_UNAVAILABLE, error.message || 'query failed');
    const keys = new Set(
      (Array.isArray(data) ? data : [])
        .map((r) => (r && typeof r.sd_key === 'string' ? r.sd_key.trim() : ''))
        .filter(Boolean),
    );
    if (keys.size === 0) {
      return fail(OWNER_REASON.KEY_SET_UNAVAILABLE, 'key set loaded but empty — treated as unavailable, not as "nothing matches"');
    }
    return { ok: true, keys, reason: OWNER_REASON.RESOLVED, error: null };
  } catch (e) {
