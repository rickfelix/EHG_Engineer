  ...
  try {
    ...
    if (keys.size < count) {
      return fail(
        ...
        `key set INCOMPLETE: ${keys.size} distinct keys for ${count} rows. Either the read was truncated or pages overlapped; both silently resolve real branches to "no matching key", which the gate reads as "no open PRs" and PASSES.`,
      );
    }
