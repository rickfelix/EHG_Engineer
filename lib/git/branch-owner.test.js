 ...
 */
function pagedSb({ total, capPages = Infinity, pageSize = 1000 }) {
  let pagesServed = 0;
  return {
    from: () => ({
      select: (_cols, opts) => {
        if (opts && opts.head) return Promise.resolve({ count: total, error: null });
        return {
          range: async (from, to) => {
            if (pagesServed >= capPages) return { data: [], error: null };
            pagesServed += 1;
            const end = Math.min(to, total - 1, from + pageSize - 1);
            const data = [];
...
describe('loadKeySet fails CLOSED — the hazard the substrate change introduces', () => {
  ...
  it('query returns an error', async () => {
    const sb = { from: () => ({ select: (_c,o) => (o&&o.head ? Promise.resolve({count:5,error:null}) : { range: async () => ({ data: null, error: { message: 'boom' } }) }) }) };
  ...
  it('a TRUNCATED read is unavailable, not a smaller key set', async () => {
    ...
    expect(res.ok).toBe(false);
    expect(res.reason).toBe(OWNER_REASON.KEY_SET_UNAVAILABLE);
    expect(res.error).toMatch(/TRUNCATED/);
    expect(res.error).toMatch(/1000 of 5000/);
  });
