...
describe('loadKeySet fails CLOSED — the hazard the substrate change introduces', () => {
  ...
  it('a TRUNCATED read is unavailable, not a smaller key set', async () => {
    ...
    expect(res.error).toMatch(/1000 distinct keys against 5000 rows/);
    // The refusal must distinguish itself from the benign concurrent-delete case, or an operator
    // reading it cannot tell a broken read from a busy minute.
    expect(res.error).toMatch(/not explained by concurrent deletes/);
  });

  // A ROW DELETED MID-LOAD IS NOT A BROKEN READ. Deletes run continuously here (240-1162/day, worst
  // minute 104), and failing on a shortfall against the OPENING count blocks every completion in
  // the fleet — the SECURITY sub-agent put it near 49% during that burst. The re-read distinguishes
  // them: a truncated or overlapped read stays short against the new count; a delete does not.
  it('a concurrent DELETE mid-load reconciles instead of blocking the fleet', async () => {
    let counts = 0;
    const ranged = {
      order: () => ranged,
      range: async (from) => {
        ...
        const data = [];
        for (let i = 0; i < 999; i += 1) data.push({ sd_key: `SD-KEY-${i}` });
        return { data, error: null }; // 999 keys — one row vanished mid-load
      },
    };
    const sb = {
      from: () => ({
        select: (_c, o) => {
          if (o && o.head) { counts += 1; return Promise.resolve({ count: counts === 1 ? 1000 : 999, error: null }); }
          return ranged;
        },
      }),
    };
    const res = await loadKeySet(sb);
    expect(res.ok).toBe(true);
    expect(res.keys.size).toBe(999);
    expect(res.deletedDuringLoad).toBe(1);
  });
  ...
  it('detects page OVERLAP, where rows fetched looks correct but keys are short', async () => {
    ...
    expect(res.ok).toBe(false);
    expect(res.error).toMatch(/1000 distinct keys against 2000 rows/);
  });
...
describe('SD-LEO-INFRA-WIDEN-BRANCH-TYPE-001 — the sd/ prefix widen', () => {
  // TS-2: reproduces the incident this SD closes. Before the widen, this branch resolved
  // UNSUPPORTED_BRANCH_TYPE (belongs: false) and PR_MERGE_VERIFICATION's Scan A/B/C could not see
  // the PR, false-blocking SD-LEO-FIX-CHAIRMAN-DECISION-CAPTURE-001's LEAD-FINAL-APPROVAL.
  it('an sd/ branch now resolves to its owning SD key (was previously unsupported_branch_type)', () => {
    const r = branchBelongsToSd(
      'sd/SD-LEO-FIX-CHAIRMAN-DECISION-CAPTURE-001',
      'SD-LEO-FIX-CHAIRMAN-DECISION-CAPTURE-001',
      new Set(['SD-LEO-FIX-CHAIRMAN-DECISION-CAPTURE-001']),
    );
    expect(r.belongs).toBe(true);
    expect(r.owner).toBe('SD-LEO-FIX-CHAIRMAN-DECISION-CAPTURE-001');
    expect(r.reason).toBe(OWNER_REASON.RESOLVED);
  });

  // TS-3: assertTypeTokensPrefixFree already runs unconditionally at module import (this file
  // importing branch-owner.js at all already exercised it once) -- this test pins the shipped set
  // itself so a future edit that breaks prefix-freeness fails here, not only at import.
  it('the widened token set (including sd) is still prefix-free', () => {
    expect(assertTypeTokensPrefixFree(BRANCH_TYPE_TOKENS)).toBe(true);
    expect(BRANCH_TYPE_TOKENS).toContain('sd');
  });
});
