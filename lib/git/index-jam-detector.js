 ...
 *
 * SD-LEO-INFRA-ACTIVATE-INERT-STALL-001-A: the JAMMED line previously asserted the tree "has been
 * unwritable" — a writability FACT the detector structurally cannot know (see module header: it
 * never opens, writes, or tests the lock). What it actually knows is that the SAME lock identity
 * has persisted across the dwell window. The prose now names that observed symptom instead of a
 * claim the detector never checked.
 */
export function formatVerdict(repoPath, result) {
  if (result.verdict === VERDICT.JAMMED) {
    ...
    return `JAMMED GIT INDEX — ${repoPath} has held the SAME index.lock identity for ${mins} min `
      + '(no observed release or replacement). Every git index operation in this tree is blocked '
      + 'while that lock stands. This is a SHARED-RESOURCE condition, not a fault in your command.';
  }
