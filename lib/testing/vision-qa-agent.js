...
// SD-LEO-ORCH-GEMINI-MODEL-SCAN-001-D: Gemini model selection has exactly one SSOT
// (lib/config/model-config.js's getGoogleModel(purpose)). autoSelectModel() below used to
// return hardcoded 'gemini-2.5-flash'/'gemini-2.5-pro' literals; those are now resolved via
// getGoogleModel('vision')/getGoogleModel('reasoning') -- byte-identical effective models
// today (MODEL_DEFAULTS.google.vision='gemini-2.5-flash', .reasoning='gemini-2.5-pro'), but
// now env-overridable and centrally upgradable like every other purpose-keyed caller.
...
class VisionQAAgent {
   ...
   */
  async testApplication(appId, testGoal, _options = {}) {
    console.log(`
╔══════════════════════════════════════════════╗
...
Cost Limit: $${this.config.costLimit}
  ...
  autoSelectModel(config) {
    ...
    if (goal.includes('critical') || goal.includes('payment') || goal.includes('security') || goal.includes('compliance')) {
      ...
      return getGoogleModel('reasoning');
    }
