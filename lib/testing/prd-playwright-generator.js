...
${scenarios.map(scenario => this.generateTestCase(scenario, requirementId)).join('\n\n')}
   ...
   */
  generateTestCase(scenario, requirementId) {
...
`;
  ...
  generateCleanupSteps(requirement) {
    return [
      {
        action: 'clearStorage',
        target: 'localStorage'
  ...
  generateInvalidData(requirement) {
