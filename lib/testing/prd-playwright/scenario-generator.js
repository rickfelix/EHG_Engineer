/**
 * PRD Playwright Generator - Scenario Generation
 */

import { mapPriority, requiresValidation, determineNavigationUrl, extractSelector } from './utils.js';
...
export function parseAcceptanceCriteria(criteria, startStep) {
  ...
  if (lowerText.includes('click') || lowerText.includes('button') || lowerText.includes('submit')) {
    ...
    steps.push({
      ...
      target: selector,
      data: null,
      assertion: { type: 'visible', selector: selector }
    });
  }
