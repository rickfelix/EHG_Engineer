...
export const REAP_REASONS = Object.freeze({
  ...
  // QF-20260906-756: distinct from UNPUSHED — the fetch that should have refreshed the local
  // upstream ref before this check failed, so a non-zero cherry count cannot be trusted as real
  // unpushed work vs. a fetch-lagged ref. Still not reapable (fail-closed), but honestly labeled.
...
// QF-20260906-756: memoized per PROCESS (one reap run), not per worktree — countUnpushedCommitsResult's
// `git cherry <upstream> HEAD` reads whatever local upstream ref the last fetch left behind (its own
// naming note above), and nothing on the reap path ever fetched, so a squash-merged worktree read
// unsafe-to-reap until an UNRELATED fetch happened to refresh the ref (PAT-LES-9256aa38dfc8).
const _upstreamFetchCache = new Map();

/**
 * Fetch `upstream`'s remote once per process and report provenance. Call this ONCE at a reap RUN's
 * entry, before iterating worktrees — repeat calls with the same `upstream` return the cached
 * result instead of re-fetching. Fail-closed by construction: on a fetch failure, `sha`/`age_s` fall
 * back to whatever the local ref already held (reading it via plain rev-parse/show, no network), so
 * a failed fetch degrades to exactly today's pre-fix behavior rather than silently trusting a stale
 * value as fresh.
 * @returns {{ref: string, sha: string|null, age_s: number|null, fetched: boolean, error: string|null}}
 */
export function fetchUpstreamOnce(upstream = 'origin/main', { gitRunner = runGit, cwd = process.cwd() } = {}) {
  if (_upstreamFetchCache.has(upstream)) return _upstreamFetchCache.get(upstream);
  const slash = upstream.indexOf('/');
  const remote = slash === -1 ? upstream : upstream.slice(0, slash);
  const branch = slash === -1 ? 'main' : upstream.slice(slash + 1);
  const fetchRes = gitRunner(['fetch', remote, branch], cwd);
  const shaRes = gitRunner(['rev-parse', upstream], cwd);
  const sha = shaRes.code === 0 ? String(shaRes.stdout || '').trim() || null : null;
  let ageS = null;
  if (sha) {
    const timeRes = gitRunner(['show', '-s', '--format=%ct', sha], cwd);
    const ts = timeRes.code === 0 ? parseInt(String(timeRes.stdout || '').trim(), 10) : NaN;
    if (Number.isFinite(ts)) ageS = Math.max(0, Math.round(Date.now() / 1000 - ts));
  }
  const result = { ref: upstream, sha, age_s: ageS, fetched: fetchRes.code === 0, error: fetchRes.code === 0 ? null : String(fetchRes.stderr || 'fetch failed').trim() };
  _upstreamFetchCache.set(upstream, result);
  return result;
}

/** Test-only seam: clear the per-process fetch memoization between test cases. */
export function _resetUpstreamFetchCache() { _upstreamFetchCache.clear(); }
