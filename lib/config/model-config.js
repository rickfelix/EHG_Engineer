...
const MODEL_DEFAULTS = {
  openai: {
    validation: 'gpt-5.4',           // SD/PRD/quality validation (verified 2026-04-04)
    classification: 'gpt-5.4',       // Type classification, categorization
    generation: 'gpt-5.4',           // Content generation, PRD writing
    fast: 'gpt-5.4-mini',            // Quick operations, low latency needed (verified 2026-04-04)
    vision: 'gpt-5.4',               // Native multimodal — replaces dedicated gpt-4o (verified 2026-04-04)
    reasoning: 'gpt-5.4',            // Reserved deep-reasoning tier (security/critical)
    'image-generation': 'dall-e-3',  // SD-LEO-FIX-MIGRATE-RUNTIME-MODEL-001: OpenAI fallback for image generation (visualization-provider.js)
  },
  claude: {
    validation: 'claude-sonnet-4-6',
    classification: 'claude-haiku-4-5-20251001',
    // REVISIT-IF closed 2026-08-19 (QF-20260818-343, chairman commission): Gemini 3.7 Flash
    // (2026-08-13) qualifies as the superseding release for this marker's condition -- live
    // A/B test (google.generation/validation swapped to gemini-3.7-flash below) confirmed the
    // Google-default/Claude-fallback split still holds; Gemini's quality gap narrowed further,
    // reinforcing rather than reversing the 2026-04-19 downgrade. No change to Claude generation.
    ...
    // REVISIT-IF(condition=Fable-class API access becomes fleet-default) owner=solomon provenance=SD-LEO-INFRA-SOLOMON-CONSULT-001C note=oracle pinned to opus-4-8 for availability; env-swappable via CLAUDE_MODEL_SOLOMON, re-pin when the stronger tier is durably available
    ...
    // SD-LEO-ORCH-MICHAEL-ROLE-FORMALIZATION-002-A (spec §3, chairman on Solomon Q5, ratification 2b14e48d):
    // the Michael morning conversation runs on Opus at medium effort, with Sonnet as the fallback under
    // account quota. Max-plan seat models only — nothing in Michael's path bills an API key.
    michael: 'claude-opus-4-8',
    'michael-fallback': 'claude-sonnet-4-6',
    'premium-generation': 'claude-opus-4-8', // SD-LEO-FIX-MIGRATE-RUNTIME-MODEL-001: shared Opus-tier pin for callers that explicitly need Opus over the cost-controlled 'generation' default (board deliberation, design-fidelity refinement, EVA-support orchestrator). Independent of 'solomon' (distinct feature, distinct env var).
    'design-generation': 'claude-fable-5', // SD-LEO-FEAT-AUTHOR-VENTURE-DESIGN-001 (FR-4): the S17 venture-design GENERATION executor runs on Fable (design authoring/judgment tier). Claude-only purpose like 'solomon'. The design-fidelity AUDIT slot stays on premium-generation (scorer != builder).
  },
  google: {
    // COST CONTROL (2026-06-05, SD-LEO-INFRA Gemini cost-reduction):
    // gate/validation/generation/vision downgraded gemini-2.5-pro → gemini-2.5-flash.
    // Flash output ~$2.50/M vs Pro $10/M and runs with thinking disabled (see
    // provider-adapters.js GoogleAdapter). Deep reasoning is preserved ONLY via the
    // 'reasoning' purpose below, which client-factory routes for security/critical work.
    //
    // UPGRADE (2026-08-19, QF-20260818-343, chairman commission): validation/generation
    // moved gemini-2.5-flash → gemini-3.7-flash after a live per-role A/B test (not just
    // vendor benchmarks, which only publish 3.6-vs-3.7 deltas -- no public 2.5-vs-3.7
    // baseline exists). Evidence: generation showed a real, concrete quality gain (specific
    // status codes/error payloads/audit-log fields vs generic phrasing) that justifies the
    // ~2x cost; validation showed a smaller but real gain (more concrete, +1 edge case) on a
    // quality-gate role where precision matters. classification/fast/vision were tested (or,
    // for vision, had no supporting evidence either way) and intentionally NOT upgraded:
    // classification produced byte-identical output at 2.4x cost and ~10x latency; fast was
    // MORE verbose than the 2.5 answer (worse fit for a low-latency purpose) at ~5.7x cost and
    // ~5x latency; vision has zero 3.7-flash-specific evidence (Google's launch benchmarks are
    // coding/agent-focused, not multimodal) so it stays put absent a reason to move it.
    // REQUIRES provider-adapters.js's gemini-3.x thinking-config default (see GoogleAdapter.
    // complete()) -- gemini-3.x has no thinking-off state (min=LOW, unlike 2.5's
    // thinkingBudget:0), so an uncapped default would reproduce the June cost-spike class.
    validation: 'gemini-3.7-flash',
    classification: 'gemini-2.5-flash',
    generation: 'gemini-3.7-flash',
    fast: 'gemini-2.5-flash',
    vision: 'gemini-2.5-flash',
    reasoning: 'gemini-2.5-pro',       // Reserved: security/critical gates only
    'image-generation': 'gemini-3-pro-image-preview', // SD-LEO-FIX-MIGRATE-RUNTIME-MODEL-001: Nano Banana Pro image generation (image-generator.js, visualization-provider.js)
    'creative-image-generation': 'gemini-2.5-flash-image', // SD-LEO-ORCH-GEMINI-MODEL-SCAN-001-E: lib/creative/providers/gemini.js's chairman-ratified (2026-07-10) image fallback path. Distinct from 'image-generation' above because the ratified default here differs (see venture-hosting-standard.md).
  }
};
...
const ENV_VARS = {
  openai: {
    ...
    'image-generation': 'OPENAI_MODEL_IMAGE_GENERATION',
  },
  claude: {
    ...
    michael: 'CLAUDE_MODEL_MICHAEL',
    'michael-fallback': 'CLAUDE_MODEL_MICHAEL_FALLBACK',
    'premium-generation': 'CLAUDE_MODEL_PREMIUM_GENERATION',
    'design-generation': 'CLAUDE_MODEL_DESIGN_GENERATION',
  },
  google: {
    ...
    'image-generation': 'GEMINI_MODEL_IMAGE_GENERATION',
    'creative-image-generation': 'GEMINI_MODEL_CREATIVE_IMAGE_GENERATION',
  }
};
 ...
 */
const VALID_PURPOSES = ['validation', 'classification', 'generation', 'fast', 'vision', 'reasoning', 'solomon', 'michael', 'michael-fallback', 'premium-generation', 'design-generation', 'image-generation', 'creative-image-generation'];
 ...
 *
 * @example
 * const model = getOpenAIModel('validation'); // 'gpt-5.4' or env override
 * const fastModel = getOpenAIModel('fast');   // 'gpt-5.4-mini' or env override
 */
...
function getAllModels() {
  return {
    openai: {
      ...
      reasoning: getOpenAIModel('reasoning'),
    },
    claude: {
      ...
      reasoning: getClaudeModel('reasoning'),
    },
    google: {
      ...
      vision: getGoogleModel('vision'),
      reasoning: getGoogleModel('reasoning'),
    }
  };
}
