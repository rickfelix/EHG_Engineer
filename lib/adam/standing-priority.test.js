...
import {
  SETTINGS_KEY, TABLE, SOURCES, IN_FLIGHT_STATUSES, classifyServed, isRoadmapAnchored,
...
describe('TS-9 IN-FLIGHT VOCABULARY — active (EXEC) counts as work being routed', () => {
  // QF-20260907-607: 'active' is the status a strategic directive carries while in EXEC.
  // Its absence here made a chairman-set priority read UNSERVED while its linked SDs were
  // being actively worked, and the tick's own remedy text ("route work in, or clear it")
  // is destructive when applied to an override that is already being served.
  it('matches the working reference in scripts/adam-coordinator-health.mjs', () => {
    const reference = readFileSync(resolve(REPO_ROOT, 'scripts/adam-coordinator-health.mjs'), 'utf8');
    const m = reference.match(/export const IN_FLIGHT_STATUSES = \[([^\]]+)\];/);
    expect(m).not.toBeNull();
    const referenceStatuses = m[1].split(',').map((s) => s.trim().replace(/'/g, ''));
    expect([...IN_FLIGHT_STATUSES].sort()).toEqual([...referenceStatuses].sort());
  });

  it('a linked SD with status active is observed as in-flight end-to-end', async () => {
    const priority = { ...ROADMAP_PRIORITY, linked_sd_keys: ['SD-A'] };
    const sb = makeStore({
      settingsRow: priority,
      sds: [{ sd_key: 'SD-A', status: 'active' }],
    });
    const r = await evaluateStandingPriority(sb);
    expect(r.status).toBe('served');
    expect(r.servedBy).toEqual(['SD-A']);
  });

  it('draft and deferred are NOT treated as in-flight (not yet, or no longer, being worked)', () => {
    expect(IN_FLIGHT_STATUSES).not.toContain('draft');
    expect(IN_FLIGHT_STATUSES).not.toContain('deferred');
    expect(IN_FLIGHT_STATUSES).not.toContain('completed');
    expect(IN_FLIGHT_STATUSES).not.toContain('cancelled');
  });
});
