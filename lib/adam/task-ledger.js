...
/**
 * QF-20260911-447: bind-at-mint works (every Adam mint creates a source_kind='sourced_sd' child
 * via createOrUpsertNode), but nothing marks that child DONE once its source (a QF or SD)
 * reaches a terminal status — the board only ever grows, and every census/stall-probe/chairman
 * open-count reads the residue as live work. Idempotent: a row already 'done'/'cancelled' is
 * excluded from the match, so re-calling with the same args after the first call is a no-op.
 * Never touches source_kind='manual' rows (the filter only ever matches 'sourced_sd').
 *
 * count-truncation-diff-lint (this SD's own EXEC review): UNIQUE(source_kind, source_ref) on
 * adam_task_ledger (database/migrations/20260701_adam_task_ledger.sql) means at most ONE row can
 * ever match this filter, so the read uses maybeSingle() -- a lint-recognized bounded form --
 * rather than an unbounded select().
 * @param {object} supabase
 * @param {string} sourceRef - the QF id or SD key the child was bound to at mint time
 * @param {string} terminalStatus - the source's terminal status, stamped into the blocker text
 * @returns {Promise<{closed: number}>}
 */
export async function closeSourcedChild(supabase, sourceRef, terminalStatus) {
  if (!sourceRef) throw new Error('closeSourcedChild: sourceRef is required');
  const { data: row, error: selErr } = await supabase
    .from(TABLE)
    .select('id')
    .eq('source_kind', 'sourced_sd')
    .eq('source_ref', sourceRef)
    .not('status', 'in', '(done,cancelled)')
    .maybeSingle();
  if (selErr) throw new Error(`closeSourcedChild select failed: ${selErr.message}`);
  if (!row) return { closed: 0 };

  const blocker = `CLOSED AT SOURCE: ${sourceRef} ${terminalStatus} ${new Date().toISOString()}`;
  const { error: updErr } = await supabase
    .from(TABLE)
    .update({ status: 'done', blocker })
    .eq('id', row.id);
  if (updErr) throw new Error(`closeSourcedChild update failed: ${updErr.message}`);
  return { closed: 1 };
}
 ...
 * the caller's tick.
 *
 * QF-20260912-408: the original shape queried ALL live parents first (uncapped, silently
 * truncated by PostgREST's 1,000-row default once the mechanical `advisory_thread` parent
 * population passed that count), then filtered children by a `.in('parent_id', parentIds)`
 * built from up to 1,000 ids — PostgREST rejects an oversized `.in()` filter outright, and the
 * failure was swallowed by the caller. Only a handful of parents ever have children, so this
 * inverts the order: read children first (paginated, no cap risk), group by parent_id in
 * memory, then fetch only the (few) parents that actually have children.
 * @param {object} supabase
...
export async function syncParentRollupStatus(supabase) {
  ...
  try {
    const children = await fetchAllPaginated(() =>
      supabase.from(TABLE).select('id, parent_id, status').eq('tier', 'child').not('parent_id', 'is', null)
    );
    ...
    const parentIdsWithChildren = [...childrenByParent.keys()];
    if (!parentIdsWithChildren.length) return result;
