 ...
 * @returns {Promise<{snapshot: object, alerted: Array<{id: string, title: string, escalated: boolean}>, suppressed: Array<{id: string, tier: string, source_kind: string, reason: string}>}>}
 */
export async function checkAndAlertStalls(supabase, parents, prevSnapshot = {}, opts = {}) {
  const snapshot = {};
  const stalls = [];
  const suppressed = []; // QF-20260725-639: non-work rows excluded from the stall class, recorded for audit
  for (const node of Array.isArray(parents) ? parents : []) {
    ...
    // QF-20260725-639 (unshipped half of QF-20260725-638, whose venture half merged while the
    // anchor half did not): two NON-WORK classes were alarming every tick forever.
    //   - tier=parent rows are forward-list ANCHORS / rollups — task-ledger.js rolls child status
    //     up into them, so a parent that "never moves" is the board working as designed, and
    //     alarming on it double-counts the child that actually carries the work.
    //   - advisory_thread rows are comms threads, not work at all.
    // The discriminator is not invented here: lib/adam/adherence-probes.js's pm_board probe
    // already excludes advisory_thread and keys on source_kind, so this reuses proven vocabulary.
    // Suppression is RECORDED, never silent — same audit contract QF-638 shipped for parked
    // ventures (QUIET_TICK_VENTURE_PARK_SUPPRESSED), so an exclusion can always be read back.
    // SCOPED TO PARENT TIER ON PURPOSE — narrower than the QF's literal wording, which asked to
    // exclude "parent-tier AND advisory_thread" rows. A BLANKET advisory_thread exclusion would
    // silently revoke QF-20260704-319: a child advisory_thread whose correlation is genuinely
    // open (no reply, no ratified decision) is an awaited answer that HAS gone unanswered, and
    // that contract is pinned by an existing test. It is also unnecessary — all six rows the QF
    // measured are tier=parent (verified live: 20 open parent rows, 9 advisory_thread / 6
    // sourced_sd / 5 manual), so the parent rule alone delivers the stated acceptance. A
    // parent-tier advisory_thread is still suppressed here, as an anchor.
    const nonWorkReason = node.tier === 'parent' ? 'parent_tier_anchor' : null;
    if (nonWorkReason) {
      suppressed.push({
        id: node.id,
        title: node.title,
        tier: node.tier || 'unknown',
        source_kind: node.source_kind || 'unknown',
  ...
  return { snapshot, alerted, suppressed };
}
