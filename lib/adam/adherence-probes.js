...
/**
 * What a probe's verdict is ACTUALLY a claim about.
 * SD-LEO-INFRA-ROLE-SESSION-SELF-001 FR-2.
 *
 *   DUTY    — "the duty is wired" (a set-membership / presence check)
 *   CONDUCT — "behaviour complied" (a check that read live behaviour)
 *
 * These render identically today: a green is a green. The originating incident was a role-session
 * adherence review returning CLEAN on the night of a self-reported execution breach — its green was
 * honest and IRRELEVANT, because it had only ever checked that duties were listed. A reader cannot
 * tell those two greens apart, and the weaker one is the one that reassures.
 */
// The vocabulary is shared with Solomon (and any future role) via lib/governance/check-class.js —
// re-exported here so existing importers keep working. Both roles must use the same two words or
// their rows are not comparable, which is the whole point of persisting the class.
import { CHECK_CLASS as SHARED_CHECK_CLASS, assertCheckClass } from '../governance/check-class.js';

export const CHECK_CLASS = SHARED_CHECK_CLASS;

/**
 * Build one probe verdict. THROWS on a missing or unrecognised checkClass.
 *
 * Deliberately loud, and deliberately HERE rather than at the DB writer: recordAdherence is
 * fail-open by construction (it warns and returns null), so a throw down there would become a
 * MISSING ROW — silent coverage loss, which is this SD's own defect. Probes run in the audit's
 * main flow, outside that catch, so an unlabelled probe fails the run visibly instead of writing
 * a row whose meaning nobody can recover.
 *
 ...
 * image of the inflated denominator FR-1 fixes. Neither silence is safe, so silence is refused.
 */
export function bar(probe, duty, verdict, detail, checkClass) {
  assertCheckClass(probe, checkClass);
  return { probe, duty, verdict, detail, check_class: checkClass };
}

/** Every Adam probe reads live behaviour facts, so all eight are CONDUCT claims. */
const conductBar = (probe, duty, verdict, detail) => bar(probe, duty, verdict, detail, CHECK_CLASS.CONDUCT);
...
export function probeSourcingCadence(facts = {}) {
  ...
  if (unresolved(n) || !Number.isFinite(Number(n))) {
    return conductBar('sourcing_cadence', duty, VERDICT.UNKNOWN, 'sourcedInWindow unresolved — cannot confirm sourcing cadence (not a pass)');
  }
  const count = Number(n);
  return count > 0
    ? conductBar('sourcing_cadence', duty, VERDICT.PASS, `${count} item(s) sourced in the last ${facts.windowDays ?? '?'}d`)
    : conductBar('sourcing_cadence', duty, VERDICT.FAIL, `0 items sourced in the last ${facts.windowDays ?? '?'}d — sourcing cadence stalled`);
}
 ...
 *
 * QF-20260728-818: when the gauge's own INPUT is absent (visionGaugeInputAvailable===false — no
 * vision doc, no DB source), a zero read-count is unpassable-by-construction, not a diligence
 * failure — Adam cannot read a gauge that cannot compute. That is a distinct, named condition
 * from "gauge was readable but Adam didn't check", checked FIRST so it can never be
 * misclassified as a measured FAIL.
 * @param {{ visionGaugeReadInWindow?: boolean|null, visionGaugeInputAvailable?: boolean|null, visionGaugeUnavailableNote?: string|null }} facts
 */
export function probeVisionMonitoring(facts = {}) {
  ...
  if (facts.visionGaugeInputAvailable === false) {
    return conductBar('vision_monitoring', duty, VERDICT.UNKNOWN,
      `vision gauge input unavailable (${facts.visionGaugeUnavailableNote || 'no vision doc / DB source'}) — duty cannot be evidenced, not a diligence failure`);
  }
  const read = facts.visionGaugeReadInWindow;
  if (unresolved(read)) {
    return conductBar('vision_monitoring', duty, VERDICT.UNKNOWN, 'visionGaugeReadInWindow unresolved — cannot confirm monitoring (not a pass)');
  }
  return read === true
    ? conductBar('vision_monitoring', duty, VERDICT.PASS, 'vision gauge was read within the window')
    : conductBar('vision_monitoring', duty, VERDICT.FAIL, 'vision gauge NOT read within the window — monitoring lapsed');
}
...
export function probeFrictionSignaling(facts = {}) {
  ...
  if (unresolved(recur) || unresolved(signals)) {
    return conductBar('friction_signaling', duty, VERDICT.UNKNOWN, 'recurrences/signals unresolved — cannot confirm signaling (not a pass)');
  }
  if (Number(recur) === 0) {
    return conductBar('friction_signaling', duty, VERDICT.PASS, 'no recurrences in the window — nothing to signal');
  }
  return Number(signals) > 0
    ? conductBar('friction_signaling', duty, VERDICT.PASS, `${recur} recurrence(s) and ${signals} signal(s) — signaling kept up`)
    : conductBar('friction_signaling', duty, VERDICT.FAIL, `${recur} recurrence(s) but 0 signals — friction went unsignalled`);
}
...
export function probeProposeOnly(facts = {}) {
  ...
  if (unresolved(builds) || !Number.isFinite(Number(builds))) {
    return conductBar('propose_only', duty, VERDICT.UNKNOWN, 'adamAuthoredBuildsInWindow unresolved — cannot confirm propose-only (not a pass)');
  }
  return Number(builds) === 0
    ? conductBar('propose_only', duty, VERDICT.PASS, 'no Adam-role session holds a build claim — propose-only upheld')
    : conductBar('propose_only', duty, VERDICT.FAIL, `${builds} Adam-role session(s) hold a build claim (sd_key) — CONST-002 propose-only violated`);
}
...
export function probeBeltStarvation(facts = {}) {
  ...
  if (unresolved(belt) || !Number.isFinite(Number(belt))
    ...
    return conductBar('belt_starvation', duty, VERDICT.UNKNOWN, 'claimableBelt/idleWorkers/sourceableBacklogCount unresolved — cannot confirm belt health (not a pass)');
  }
  const b = Number(belt), i = Number(idle), k = Number(backlog);
  return (b === 0 && i > 0 && k > 0)
    ? conductBar('belt_starvation', duty, VERDICT.FAIL, `belt=0 while ${i} worker(s) idle and ${k} sourceable backlog item(s) exist — sourcing starved the belt`)
    : conductBar('belt_starvation', duty, VERDICT.PASS, `belt=${b}, idle=${i}, sourceable backlog=${k} — no starvation`);
}
 ...
 *
 * QF-20260906-775 — twice-witnessed false FAIL (self-adherence runs 3025f07e, c6c723d0): "the
 * Deploy token lacks Workers AI scope" is Cloudflare Workers AI (a product name), not fleet
 * dispatch, but "deploy ... Workers" already matched the ambiguous-verb clause and NOT_A_CAPACITY_
 * SENSE had no entry for the domain noun that follows Workers here ("AI"/"secret(s)") — extended
 * below, the same mechanism the three guards above already use.
 *
...
const NOT_A_CAPACITY_SENSE =
  ...
  '|events?|messages?|names?|counts?|timestamps?|uuids?' +
  // QF-20260906-775: Cloudflare product terms — "Workers AI", "Worker secret(s)/binding(s)" — are
  // technical noun-phrases naming a Cloudflare Workers product feature, not fleet capacity.
  '|ai|secrets?|bindings?)\\b)';
const VERB_NOT_HYPHENATED = '(?<!-)';
const NOUN_NOT_IDENTIFIER = '(?![-\\w])';
/**
 * QF-20260908-140 — the remedy shape enumerating exception words has failed three times
 * ('add real session attribution', 'Deploy token lacks Workers', and now 'a false DISPATCH
 * costs a worker's context'). This case is unreachable by any addition to that list: the
 * disambiguating evidence sits to the LEFT of the verb (a determiner, optionally one
 * adjective) and BETWEEN the verb and the noun (a finite verb filling the skip-gap). Two
 * grammatical guards instead of a longer denylist:
 */
// (ii) a determiner (+ optional single adjective) immediately left of the ambiguous-verb
// token marks it a NOUN ("a false DISPATCH", "the recent ADD"), never the imperative verb.
const NOT_PRECEDED_BY_DETERMINER =
  '(?<!\\b(?:a|the|this|that|these|those|any|every|no|one|two|three)\\s+(?:\\w+\\s+)?)';
// (i) the 0-2 word filler between the verb and FLEET_NOUN must not itself be a finite verb
// ("DISPATCH COSTS a worker") — that shape is the verb's own PREDICATE, not its object.
const NOT_FINITE_VERB_FILLER =
  '(?!(?:costs?|cost|takes?|took|is|was|were|means?|meant|buys?|bought|burns?|burnt|wastes?|saves?)\\b)\\w+\\s+';
const DISPATCH_LANGUAGE = new RegExp(
  '\\b(' +
    `(?:spin|stand)\\s+(?:up|down)\\s+(?:a\\s+|the\\s+|\\w+\\s+){0,3}${FLEET_NOUN}` +
    `|${NOT_PRECEDED_BY_DETERMINER}${VERB_NOT_HYPHENATED}(?:assign|dispatch|deploy|reallocat\\w*|add|remove)` +
      `\\s+(?:a\\s+|the\\s+|${NOT_FINITE_VERB_FILLER}){0,2}${FLEET_NOUN}${NOUN_NOT_IDENTIFIER}${NOT_A_CAPACITY_SENSE}` +
 ...
 *
 * QF-20260908-140 (b): the corpus is scanned for ALL matches (not just the first), deduped by
 * matched text, and fingerprinted the same way decision_rubric's over-asks are (fingerprintOverAsk +
 * encode/parseFingerprintsTail). A match whose fingerprint already carries a remediation_ref on a
 * prior ledger row (resolvedDispatchFingerprints, supplied by the review script) is EXCLUDED — a
 * remediated false-fail no longer re-files every run for the life of the lookup window.
 * @param {{ advisoryBody?: string|null, resolvedDispatchFingerprints?: string[]|null }} facts
 */
export function probeDispatchBoundary(facts = {}) {
  ...
  if (unresolved(body)) {
    return conductBar('dispatch_boundary', duty, VERDICT.UNKNOWN, 'advisoryBody unresolved — cannot confirm boundary (not a pass)');
  }
  const resolved = new Set(Array.isArray(facts.resolvedDispatchFingerprints) ? facts.resolvedDispatchFingerprints : []);
  const globalRe = new RegExp(DISPATCH_LANGUAGE.source, 'gi');
  const seen = new Set();
  const matches = [];
  let m;
  while ((m = globalRe.exec(String(body))) !== null) {
    const norm = m[0].toLowerCase();
    if (!seen.has(norm)) { seen.add(norm); matches.push(m[0]); }
    if (m.index === globalRe.lastIndex) globalRe.lastIndex++; // never loop on a zero-length match
  }
  const withFps = matches.map((match) => ({ match, fp: fingerprintOverAsk(match) }));
  const fpsTail = encodeFingerprintsTail(withFps.map((x) => x.fp));
  const fresh = withFps.filter((x) => !resolved.has(x.fp));
  const excluded = withFps.length - fresh.length;
  const exclNote = excluded > 0 ? `, ${excluded} excluded as already-remediated` : '';
  if (fresh.length === 0) {
    return conductBar('dispatch_boundary', duty, VERDICT.PASS,
      `no NEW fleet-dispatch language in the advisory body — boundary upheld${exclNote}${fpsTail}`);
  }
  return conductBar('dispatch_boundary', duty, VERDICT.FAIL,
    `advisory body contains fleet-dispatch language ("${fresh[0].match}") — Adam crossed into the coordinator's capacity lane${exclNote}${fpsTail}`);
}
...
export function probeDecisionRubric(facts = {}) {
  ...
  if (unresolved(items) || !Array.isArray(items) || unresolved(machineNoise) || !Array.isArray(machineNoise)) {
    return conductBar('decision_rubric', duty, VERDICT.UNKNOWN, 'adamChairmanDecisionQuestionsInWindow / adamMachineRaisedNoiseInWindow unresolved — cannot assess over-asks (not a pass)');
  }
  ...
  if (fresh.length === 0) {
    return conductBar('decision_rubric', duty, VERDICT.PASS,
  ...
  return conductBar('decision_rubric', duty, VERDICT.FAIL,
...
// QF-20260823-016: the daily plan-check forward-list snapshot (scripts/adam-adherence-staleness
// -check.mjs) stamps its anchor row source_ref='plan-check-forward-list-<date>' by convention — an
// anchor never advances by design, so it is not a finding awaiting sourcing/deferral. lib/adam/
// stall-alert.js already carves this class out of the stall alarm (QF-20260725-639); this probe
// did not, and re-indicted the same correctly-maintained anchor row on every run.
export const ANCHOR_REF_RE = /^plan-check-forward-list-/i;

/** A finding-class ledger row terminates as 'sourced' | 'deferred' | 'anchor' | 'orphan' (the drift state). */
export function classifyFindingRow(row = {}) {
  const ref = String(row.source_ref ?? '').trim();
  if (FILED_KEY_RE.test(ref)) return 'sourced';
  if (ANCHOR_REF_RE.test(ref)) return 'anchor';
...
export function probePmBoard(facts = {}) {
  ...
  if (unresolved(findings) || !Array.isArray(findings)) {
    return conductBar('pm_board', duty, VERDICT.UNKNOWN,
  ...
  if (orphans.length > 0) {
    return conductBar('pm_board', duty, VERDICT.FAIL,
  ...
  if (unresolved(current) || !Array.isArray(current)) {
    return conductBar('pm_board', duty, VERDICT.UNKNOWN, 'pmBoardSnapshot unresolved — cannot confirm board discipline (not a pass)');
  }
  ...
  if (current.length === 0) {
    return conductBar('pm_board', duty, VERDICT.PASS, `0 open child-tier item(s) — clean board${tail}`);
  }
  if (unresolved(prior)) {
    return conductBar('pm_board', duty, VERDICT.UNKNOWN, `${current.length} open child-tier item(s), but no prior recorded check to compare against — cannot confirm discipline yet (not a pass)${tail}`);
  }
  ...
  if (prior.size === 0) {
    return conductBar('pm_board', duty, VERDICT.PASS, `${current.length} open child-tier item(s), 0 at the prior check — board newly in use${tail}`);
  }
  let completed = 0;
  let changed = 0;
  let stale = 0;
  ...
  if (completed > 0 || changed > 0) {
    return conductBar('pm_board', duty, VERDICT.PASS,
...
export function runCardinalProbes(facts = {}) {
  return CARDINAL_ACTION_TIME_PROBES.map((p) => {
    try { return p(facts); }
    catch (e) { return conductBar(p.name || 'unknown_probe', 'unknown', VERDICT.UNKNOWN, `probe errored (fail-open to unknown): ${e?.message ?? e}`); }
  });
}
...
export function runAdherenceProbes(facts = {}) {
  return ADHERENCE_PROBES.map((p) => {
    try {
      ...
      return conductBar(p.name || 'unknown_probe', 'unknown', VERDICT.UNKNOWN, `probe errored (fail-open to unknown): ${e?.message ?? e}`);
    }
  });
}
