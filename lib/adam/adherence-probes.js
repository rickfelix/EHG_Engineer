...
// SD-LEO-INFRA-UPSCALE-ADAM-PROJECT-MANAGEMENT-DISCIPLINE-001-C: generalized keyed tail codec.
// A ledger row's `detail` text field carries zero or more machine-readable `::key=value` tails
// (the ledger has no jsonb column) so a probe can persist state for its OWN next run to read back.
// decision_rubric's `::fps=` fingerprint tail (below) and pm_board's `::pmsnap=` snapshot tail
// (further below) both go through this single codec instead of two bespoke regexes.
