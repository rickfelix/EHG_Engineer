...
/**
 * Pure: split rows into those targeting a currently-live session and phantoms targeting a
 * retired/dead one.
 *
 * QF-20260816-532: rawBacklogCount previously counted EVERY row addressed to any HISTORICAL
 * role=adam/adam_retired session id (resolveAdamSessionIds resolves all of them, by design, for
 * the handoff-stranding reason documented there). Live-measured 2026-08-17: 19 resolved ids, 18
 * `status='released'`, 1 `status='active'` — 120 of 182 raw rows targeted a retired session and
 * NONE of them ever breach (isBreaching only tests age/ack, not liveness), so they padded the
 * printed gauge (backlog=170-181) forever without ever becoming actionable — nobody polls a dead
 * session_id. The floor is real (phantoms never resolve on their own) but must be visible as its
 * own count, not silently merged into "backlog", per the SD's own SSOT-by-construction goal for
 * this module.
 *
 * @param {Array<{target_session?:string}>} rows
 * @param {Array<string>} liveSessionIds
 * @returns {{liveRows: Array, phantomRows: Array}}
 */
export function partitionByLiveness(rows, liveSessionIds) {
  const live = new Set((liveSessionIds || []).filter(Boolean));
  const liveRows = [];
  const phantomRows = [];
  for (const r of (rows || [])) {
    (live.has(r && r.target_session) ? liveRows : phantomRows).push(r);
  }
  return { liveRows, phantomRows };
}
