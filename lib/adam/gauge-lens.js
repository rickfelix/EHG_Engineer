...
export async function readCapabilityGaps(io = {}, deps = {}) {
  ...
  try {
    ...
    for (const c of gauge.components) {
      ...
      // SD-LEO-INFRA-GAUGE-BUILDABLE-VS-OPERATIONAL-001 (FR-4): exclude operational-nature
      // capabilities from the capability_gap sourcing signal. An operational proof (real income, a
      // running venture, live governance enforcement, a chairman ratification, an OBSERVED competitor
      // baseline) is NOT something Adam can source an SD to complete — only a venture/operation flips
      // it. Leaving it in the gaps map would let the sourcing scorer rank an un-sourceable proof as a
      // build target. Excluded here => capabilityGapTerm returns active:false (no boost, no-op).
      if (c.nature === 'operational') continue;
