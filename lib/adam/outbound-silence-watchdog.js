...
// QF-20260904-790 (SD-LEO-INFRA-LOOP-LIVENESS-DISCRIMINATOR-001 FR-3): the shared,
// fail-open loop-liveness discriminator -- see the widened query + filter below.
import { classifyLoopLiveness } from '../fleet/genuine-worker.mjs';
...
export async function runOutboundSilenceWatchdog(supabase, opts = {}) {
  ...
  // SECURITY sub-agent finding SEC-3 (EXEC-TO-PLAN): excluding a DEAD-LOOP session from liveIds
  // silently drops it from probing, escalation AND laneHealth counting all at once, with no
  // residual signal that anything was suppressed -- reproducing this SD's own "report loudly"
  // defect class one layer over. deadLoopSuppressed makes the suppression itself observable.
  const result = { probed: [], escalated: [], deadLoopSuppressed: [], laneHealth: { unactionedCount: 0, maxAgeMs: 0 } };
  try {
    ...
    const { data: outbound } = await supabase
      ...
      .limit(200);
    // QF-20260904-790 FR-3: widened from `session_id, heartbeat_at` alone -- last_tool_at,
    // loop_state and metadata are the columns classifyLoopLiveness needs to tell a genuinely
    // live seat from a heartbeat-fresh DEAD LOOP (a dead loop keeps heartbeating; heartbeat_at
    // is stamped by a daemon separate from the loop). Fail-open by construction: a session
    // missing these columns classifies UNKNOWN, not DEAD-LOOP, and stays in liveIds exactly as
    // before this fix -- only a POSITIVELY confirmed dead loop is now excluded.
    const { data: sessions } = await supabase
      .from('claude_sessions')
      .select('session_id, heartbeat_at, last_tool_at, loop_state, metadata')
      .gte('heartbeat_at', new Date(now - HEARTBEAT_FRESH_MS).toISOString())
      .limit(200);
    const deadLoopIds = new Set(
      (sessions || [])
        .filter((s) => classifyLoopLiveness(s, { nowMs: now }) === 'DEAD-LOOP')
        .map((s) => s.session_id)
    );
    const liveIds = new Set((sessions || []).map((s) => s.session_id).filter((id) => !deadLoopIds.has(id)));
    ...
    // SEC-3: name every reply-expected, otherwise-breaching row that landed at a dead-loop
    // target, so the suppression is visible instead of indistinguishable from "nothing to see".
    // Uses the SAME correlationRows as the real classification below, so a row answered by a
    // cross-actor reply is correctly excluded here too, not double-counted as suppressed.
    if (deadLoopIds.size > 0) {
      result.deadLoopSuppressed = (outbound || [])
        .filter((r) => r && deadLoopIds.has(r.target_session) && isReplyExpected(r) && isBreaching(r, now, correlationRows))
        .map((r) => ({ id: r.id, target_session: r.target_session }));
    }
