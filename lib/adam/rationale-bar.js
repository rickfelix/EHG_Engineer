...
//
// QF-20260905-267 — same QF-20260727-129 class recurrence: lib/drive-loop/score/leg4-capacity.js
// VERDICTS (chairman ratification be6e9d73, SD-LEO-INFRA-DRIVE-SCORE-LEG4-001) added the live
// enum value 'DEFICIT-URGENT' AFTER the 'urgent' bare-block above was verified enum-free — the
// hyphen is a word boundary, so 'DEFICIT-URGENT' matched and refused two chairman-ruling relays
// (decision f508c4ee, ratification be6e9d73) until respelled. Only 'DEFICIT-URGENT' among the
// four VERDICTS ('DEFICIT-URGENT', 'DEFICIT', 'TIGHT', 'SURPLUS') contains a flagged token, so a
// single targeted lookbehind (not a broader enum exemption) exempts exactly that compound —
// 'act urgently' and every other 'urgent' construction still block.
export const MANIPULATIVE_PATTERNS =
  /\b((?<!not-)(?<!not )(?<!non-)(?<!non )(?<!nothing )(?<!DEFICIT-)urgent(?:ly)?|act now|act fast|must act|(?:act|respond|reply|decide|approve|escalate)\s+immediately|critical(?:ly)?\s+(?:important|urgent)|(?:it'?s|it is|is)\s+critical\s+that|guaranteed|certainly|definitely|absolutely|you (?:need|have) to|don'?t miss|last chance)\b/i;
