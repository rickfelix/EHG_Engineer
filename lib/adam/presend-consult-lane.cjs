 ...
 * @param {string|number} [input.expiresAt] - ISO string or epoch ms (a number is normalized to
 *   an ISO string internally; session_coordination.expires_at is timestamptz)
...
async function runPreSendConsultLane(input = {}, deps = {}) {
  const {
    subject = '', body = '', senderCallsign, sessionId, repo, expiresAt: expiresAtInput,
    isChairmanTargeted = false,
    // QF-20260912-901: per-number provenance stamps ({value, instrument, row_ref, measured_at})
    // the composer attached to this packet. Threaded through unchanged; checked below, only for
    // a body that would otherwise open a consult.
    measuredBy = null,
    // QF-20260905-123: the raising session, kept separate from `sessionId` (the lane's own
    // sender_session, which may be a documented non-session sentinel for lanes with no live
    // session threaded to them). Optional -- an omitted value reproduces the previous payload
    // envelope exactly (no origin_session key at all).
    originatorSessionId = null,
  ...
  // QF-20260905-194: session_coordination.expires_at is timestamptz -- a caller passing a
  // NUMBER (epoch ms, e.g. Date.now() + ttl) makes the insert below fail Postgres's
  // date/time parsing, so no consult row is ever written and a chairman-targeted hold
  // becomes permanently unresolvable. Normalize here so no caller can reintroduce the bug,
  // matching the fix at the one known call site (chairman-sms-gate/index.js).
  const expiresAt = typeof expiresAtInput === 'number'
  ...
  // QF-20260828-255: same-action write+readback verification for the hold-and-surface path
  // (Solomon advisory f02c00e7, held-send 5ce0c4b5) -- a held chairman send previously trusted
  // insertCoordinationRow's own return value with no independent confirmation, so a silently
  // failed/absent consult insert produced a hold with nothing to ever resolve it. Injectable
  // (mirrors randomUUID above); defaults to the real readback checker in production.
  const verifyConsultReadback = typeof deps.verifyConsultReadback === 'function'
    ? deps.verifyConsultReadback
    : async (id) => {
      const { verifyReadback } = await import('../checkers/readback-checker.mjs');
      return verifyReadback({ table: 'session_coordination', match: { id } });
    };
  ...
  // QF-20260912-901 (FIX SHAPE (a) of QF-20260912-079): a first-use number with no measured_by[]
  // stamp refuses the consult outright -- mechanical, before the oracle grade, naming the number
  // so the composer can measure and re-send. Checked ONLY for a body that would otherwise consult
  // (routine/proceed-path sends are out of scope; the measured incident was entirely held packets).
  const { checkMeasuredByProvenance } = require('./measured-number-provenance.cjs');
  const provenanceCheck = checkMeasuredByProvenance(body, measuredBy);
  if (!provenanceCheck.ok) {
    return { action: 'refuse', reason: 'unstamped_number', unstampedNumber: provenanceCheck.unstampedNumber };
  }
  ...
  return performBoundedConsult(gateInput, {
    ...
    consult: async () => {
      ...
      const cp = buildSolomonConsultPayload({
        ...
        originSession: originatorSessionId,
        // isAwait deliberately OMITTED => reply_class 'reply-needed' + reply_expected_by (FR-1).
      });
      // SD-LEO-INFRA-CHAIRMAN-SMS-DECISION-002 (FR-1): readback-verify the consult insert
      // (Explore defect 1 — this call previously discarded insertCoordinationRow's return
      // value entirely, so a silent insert failure here left every downstream held row with
      // no consult_row_id anchor to readback-verify against). `select`/`single` are already
      // supported by the shared choke (lib/coordinator/dispatch.cjs insertCoordinationRow) —
      // no change needed there, just requesting the id back. Best-effort: a failed readback
      // does not change this lane's non-blocking contract (see module header) — it only means
      // consultRowId comes back null, same as before this fix.
      const insertResult = await insertCoordinationRow({
      ...
      }, { targetRoleHint: 'solomon', select: 'id', single: true });
      const consultRowId = insertResult && !insertResult.error && insertResult.data && insertResult.data.id
        ? insertResult.data.id
        : null;
      // QF-20260828-255: only the chairman-targeted (hold-and-surface-capable) path needs this
      // extra read -- a non-chairman consult that degrades just proceeds, so an unresolvable hold
      // is not a risk there. Fail LOUD (console.error), not silent, on either a missing row id or
      // a failed readback -- this is the exact gap that let held-send 5ce0c4b5 park forever.
      let consultRowVerified = false;
      if (isChairmanTargeted) {
        if (consultRowId) {
          try {
            await verifyConsultReadback(consultRowId);
            consultRowVerified = true;
          } catch (err) {
            console.error(`[presend-consult-lane] QF-20260828-255: consult row ${consultRowId} FAILED readback verification (${err.message}) -- a hold entered on this outcome may be UNRESOLVABLE.`);
          }
        } else {
          console.error('[presend-consult-lane] QF-20260828-255: consult insert produced NO row id -- a hold entered on this outcome will be UNRESOLVABLE.');
        }
      }
      return { correlationId, pending: true, consultRowId, consultRowVerified };
    },
    recordLedger,
