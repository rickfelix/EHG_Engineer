...
export async function performBoundedConsult(input = {}, deps = {}) {
  ...
  //
  // QF-20260830-762: this row is CONSULT STATE, not a decision_rubric duty verdict — filing it
  // under probe='decision_rubric' polluted the real audit's pass rate (904/1000 rows measured
  // 'unknown' this way, 30d, vs 47 pass / 0 fail on the actual audit). probe='pre_send_consult' +
  // check_class='consult' give it its own home so readers that filter on probe or check_class
  // never see it as a decision_rubric outcome. verdict stays 'unknown' (genuinely not known yet)
  // pending the reconciler wiring, which needs the chairman-delegated CHECK-widen migration
  // (database/migrations/*_adam_adherence_ledger_consult_verdicts_STAGED.sql) applied first — see
  // that file for the 'reconciled'/'oracle_unreachable' close-out, shipped as a follow-up once
  // Adam applies it under §3b. The prior KNOWN GAP reasoning (writing 'pass' would inflate the
  // rate) still holds and is why this arm still never guesses a verdict.
