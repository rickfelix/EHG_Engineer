 ...
 * @param {boolean}[input.isChairmanTargeted]
 * @param {Object} deps
 * @param {Function} deps.consult        - async (payload) => verdict | null, OR (FR-0) an envelope
 *   { verdict?, correlationId?, pending? }. The envelope form is required for the non-blocking
 *   lane: it resolves immediately with { correlationId, pending: true } and NO verdict, so the
 *   correlation can be stamped onto the degraded-proceed ledger row and a late verdict reconciled
 *   back to it. Bare-verdict returns remain fully supported. May throw/hang.
...
export async function performBoundedConsult(input = {}, deps = {}) {
  ...
  // FR-0 + FR-7 (SD-LEO-INFRA-SOLOMON-CONSULT-CANNOT-DELIVER-001): deps.consult may now return
  // EITHER the legacy bare verdict OR an envelope { verdict, correlationId, pending }. The envelope
  // is what makes late-verdict reconciliation possible at all: the correlationId used to be created
  // inside the deps.consult closure and never escaped this function, so the degraded-proceed ledger
  // row below had nothing to join on (run_id is random per insert).
  //
  // CRITICAL (FR-7): `gotVerdict` MUST test the VERDICT, not mere non-nullness of `result`. Once the
  // consult goes non-blocking it resolves IMMEDIATELY with an envelope carrying no verdict yet — the
  // old `result != null` test would read that as success, return action:'send', and write NO ledger
  // row. FR-0's "every degraded-proceed row carries a correlation id" would then be vacuously true
  // over an EMPTY SET, the reconciler would have no anchor, and the whole suite would still pass.
  // Discriminator is DELIBERATELY narrow: `correlationId` / `pending` ONLY. A legacy verdict is
  // frequently itself an object carrying a `verdict` key (see should-consult-solomon.test.js:89,
  // which passes { verdict: 'looks-good' } and expects it back WHOLE), so keying on 'verdict' would
  // silently unwrap real verdicts and change what callers receive. Only the two envelope-exclusive
  // fields are safe markers.
  const envelope = (result !== TIMEOUT && result != null && typeof result === 'object'
    && ('correlationId' in result || 'pending' in result))
    ? result
    : null;
  const verdict = envelope ? (envelope.verdict ?? null) : (result !== TIMEOUT ? result : null);
  const correlationId = envelope ? (envelope.correlationId ?? null) : null;
  const pending = envelope ? envelope.pending === true : false;

  const gotVerdict = verdict != null && !pending;
  if (gotVerdict) {
    ...
    const amended = detectVerdictDelta(verdict);
    ...
    return { action: 'send', consultRecorded: true, consequence: 'high', verdict, nearMissCaptured: amended };
  }

  // FAIL-OPEN degradation. Adam is never hard-blocked on Solomon.
  if (isChairmanTargeted) {
    // SD-LEO-INFRA-CHAIRMAN-DECISION-LANE-001 (FR-1): correlationId MUST be handed back here —
    // it was previously dropped on this arm while the proceed arm (below) kept it, which made every
    // held row unreconcilable by construction (no anchor to join a later Solomon verdict against).
    // `reason` also now discriminates a genuine pending consult (an envelope arrived) from a real
    // timeout/throw/no-dep (no envelope) — the two were previously collapsed into one hardcoded
    // 'timeout' label even though the pending case is the only one ever observed in production.
    return {
      action: 'hold-and-surface',
      degraded: true,
      consequence: 'high',
      reason: pending ? 'solomon-consult-async::chairman-hold-pending-reconcile' : 'solomon-consult-timeout::chairman-hold-and-surface',
      ...(correlationId ? { correlationId } : {}),
      // SD-LEO-INFRA-CHAIRMAN-SMS-DECISION-002 (FR-1): forward the consult row id the same way
      // correlationId is forwarded above — envelope.consultRowId is NOT auto-propagated by the
      // generic envelope handling further up (only 'correlationId'/'pending' are read there),
      // so without this line chairman-sms-gate's hold insert would always see consult_row_id
      // as null even after presend-consult-lane.cjs started returning it.
      ...(envelope && envelope.consultRowId ? { consultRowId: envelope.consultRowId } : {}),
      // QF-20260828-255: forwarded the same way as consultRowId above -- generic envelope
      // handling further up does not auto-propagate this field either.
      ...(envelope ? { consultRowVerified: envelope.consultRowVerified === true } : {}),
    };
  }

  // FR-0/FR-7: the PENDING (non-blocking emit) case and the TIMEOUT case both land here, on the
  // SAME proceed+ledger arm — deliberately, so the FR-5 "only two exits" invariant is untouched and
  // hasConsultEvidence() stays satisfied without a third exit. They are distinguished by `detail`,
  // and the pending case carries the correlationId in remediation_ref so a late verdict can be
  // joined back to THIS row. verdict stays 'unknown' in both cases (it is genuinely not known yet),
  // which also keeps the rung-health catch-rate trend (rung-health-convergence.mjs:126) unaffected.
  //
  // KNOWN GAP, stated here so nobody reads this arm as more finished than it is: reconcileLateVerdicts
  // records the arriving verdict as a DISPOSITION (system_events) and does NOT append a row to
  // adam_adherence_ledger. So Solomon's acceptance criterion (a) — "solomon-cited ledger rows with a
  // NON-unknown verdict must be greater than zero" — is NOT satisfied by this SD alone; that count
  // stays at 0. Closing it needs a decision this code cannot make unilaterally:
  //   - the verdict CHECK constraint (adam_adherence_ledger_verdict_check) admits only
  //     pass/fail/unknown — VERIFIED live, so Solomon's proposed `oracle_unreachable` value needs a
  //     chairman-gated migration; and
  //   - writing 'pass' on a concurring late verdict would satisfy (a) but collides with his
  //     criterion (c), "no probe pass RATE rises as a side effect".
  // Escalated to the coordinator rather than guessed. An earlier revision of this comment claimed the
  // reconciler "APPENDS a row referencing this one" — it does not, and that claim is removed.
