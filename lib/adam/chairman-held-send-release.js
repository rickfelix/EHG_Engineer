 ...
 * returning BOTH the payload and the answering sender_session together — closing the TOCTOU window
 * a two-query design would leave between "fetch the verdict" and "check who sent it" (S-3).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} correlationId
 * @returns {Promise<{answerRowId: string|null, senderSession: string|null, payload: *|null}>}
 */
async function fetchAnswerRow(supabase, correlationId) {
  const { data, error } = await supabase
    .from('session_coordination')
    .select('id, sender_session, payload')
    .eq('payload->>reply_to', correlationId)
    .eq('payload->>kind', ANSWER_KIND)
    .order('created_at', { ascending: true })
    .order('id', { ascending: true })
    .limit(1);
  if (error || !Array.isArray(data) || data.length === 0) return { answerRowId: null, senderSession: null, payload: null };
  const row = data[0];
  return { answerRowId: row.id, senderSession: row.sender_session ?? null, payload: row.payload ?? null };
}

/**
 * ALLOWLIST check (S-2): is `sessionId` a session this repo's own role-assignment system has
 * marked role='solomon'? Deliberately NOT "is it fresh/currently active" (Solomon role handoffs
 * mean a legitimate historical answerer may have since rotated out) and deliberately NOT "does it
 * differ from the asker" (that denylist is what let S-2 through) — it must be a session that was
 * genuinely, identifiably Solomon, full stop. Fail-closed: any DB error or absent row is NOT Solomon.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
...
/**
 * Combine the answer-row fetch with the Solomon-identity check into one verified-answer result.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} correlationId
 * @param {object} [deps]
 * @returns {Promise<{found: boolean, isGenuineSolomon: boolean, answerRowId: string|null, verdict: *}>}
 */
export async function resolveVerifiedAnswer(supabase, correlationId, deps = {}) {
  const { answerRowId, senderSession, payload } = await (deps.fetchAnswerRow || fetchAnswerRow)(supabase, correlationId);
  if (!payload) return { found: false, isGenuineSolomon: false, answerRowId: null, verdict: null };
  const isGenuineSolomon = await (deps.isSolomonSession || isSolomonSession)(supabase, senderSession);
  return { found: true, isGenuineSolomon, answerRowId, verdict: payload.body ?? payload.verdict ?? null };
}
...
export async function releaseHeldSend(supabase, heldRow, deps = {}) {
  ...
  const verified = await (deps.resolveVerifiedAnswer || resolveVerifiedAnswer)(supabase, heldRow.consult_correlation_id);
  ...
  if (sendThrew || !sendResult || sendResult.sent !== true) {
    ...
    // watch 'releasing' either). unclaimedRows.length===0 makes that case observable.
    const { data: unclaimedRows, error: unclaimErr } = await supabase
      .from('chairman_held_sends')
      .update({
      ...
      .select('id');
    const unclaimFailed = Boolean(unclaimErr) || !Array.isArray(unclaimedRows) || unclaimedRows.length === 0;
    return {
