...
/**
 * QF-20260913-013 (FIX a): is `decisionId`'s chairman_decisions row ALREADY decided? Mirrors the
 * established precedent in lib/chairman/sms-outbound-worker.js's voidStaleAndCollapseObligations
 * (the identical "decided-decision void" check already shipped for sms_outbound_obligations,
 * QF-20260829-320) -- same status vocabulary ('pending'|'approved'|'rejected'|'cancelled'), same
 * fail-toward-pending posture on error/absence. Also returns `summary` (the decision's own
 * human-readable subject, e.g. chairman_decisions.summary) so a genuinely-lost-decision notice can
 * name what was actually decided instead of a held row's generic subject (FIX c).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string|null} decisionId
 * @returns {Promise<{status: string, decidedAt: string|null, summary: string|null}|null>}
 */
export async function getChairmanDecisionStatus(supabase, decisionId) {
  ...
  return { status: data[0].status, decidedAt: data[0].updated_at ?? null, summary: data[0].summary ?? null };
}

/**
 * QF-20260913-013 (FIX a): close a held row whose decision was ALREADY decided through some other
 * channel before this hold's own consult/expiry path ran (RETRY-FORKS-STATE: a retried chairman
 * packet forks a new hold+consult; the older fork sits `held` until it expires a day later even
 * after the decision is answered elsewhere). Deliberately NO chairman notice is enqueued here --
 * the decision already reached the chairman through another channel, so a "never reached you" text
 * would repeat the exact false claim measured in this QF's own incident (rows 98218e4d/6c85a2c9).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {object} heldRow
 * @param {{status: string, decidedAt: string|null}} decisionInfo
 * @param {object} [deps]
 * @returns {Promise<object>} outcome descriptor
 */
export async function closeSupersededHold(supabase, heldRow, decisionInfo, deps = {}) {
  const now = new Date().toISOString();
  const voidReason = `closed by ${deps.claimedBy || 'chairman-held-sends-release-sweep'}: decision_id ${heldRow.decision_id} was already decided (chairman_decisions.status=${decisionInfo.status}) through another channel before this hold's own consult/expiry path ran -- a stale retry fork, not a lost decision (QF-20260913-013, RETRY-FORKS-STATE).`;
  const { data: claimed, error: claimErr } = await supabase
    ...
    .update({
      ...
      metadata: {
        ...(heldRow.metadata || {}),
        void_reason: 'superseded_by_decision',
        superseding_decision_status: decisionInfo.status,
        superseding_decision_decided_at: decisionInfo.decidedAt,
        voided_at: now,
        voided_by: 'QF-20260913-013',
      },
    })
  ...
  return { action: 'abandoned', reason: 'superseded_by_decision', heldSendId: heldRow.id, noticeEnqueued: false };
}
...
export async function releaseHeldSend(supabase, heldRow, deps = {}) {
  // QF-20260913-013 (FIX a, sweep's pre-release classification): runs on EVERY sweep pass for
  // EVERY held row, not only at this row's own hold_expires_at -- a fork left behind by a retried
  // chairman packet (RETRY-FORKS-STATE) is closed within one ~15-min sweep cadence of its decision
  // being answered through a DIFFERENT fork or channel, instead of sitting immortal until its own
  // 24h expiry. Checked before the no_consult_anchor guard so it also covers a decision that was
  // answered before any consult was ever opened on this particular fork.
  if (heldRow.decision_id) {
    const decisionInfo = await (deps.getDecisionStatus || getChairmanDecisionStatus)(supabase, heldRow.decision_id);
    if (decisionInfo && decisionInfo.status && decisionInfo.status !== 'pending') {
      return (deps.closeSupersededHold || closeSupersededHold)(supabase, heldRow, decisionInfo, deps);
    }
  }
...
export async function deferExpiredConsultHoldForAbsentOracle(supabase, heldRow, deps = {}) {
  ...
  // SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 (FR-4): intentionally OUT OF SCOPE for that SD's
  // reply-token staging fix -- kind='heartbeat_status' is a one-way status notice, not a
  // decision_question expecting a reply, so it has no reply-matching requirement.
...
export async function abandonExpiredConsultHold(supabase, heldRow, deps = {}) {
  // QF-20260913-013 (FIX a, defense-in-depth): this function is independently exported/callable,
  // so it re-checks chairman_decisions.status itself rather than trusting that every caller already
  // ran releaseHeldSend's own upstream check. Also carries `summary` through for FIX c below.
  const decisionInfo = heldRow.decision_id
    ? await (deps.getDecisionStatus || getChairmanDecisionStatus)(supabase, heldRow.decision_id)
    : null;
  if (decisionInfo && decisionInfo.status && decisionInfo.status !== 'pending') {
    return (deps.closeSupersededHold || closeSupersededHold)(supabase, heldRow, decisionInfo, deps);
  }
  ...
  // SD-LEO-INFRA-CHAIRMAN-SMS-DURABLE-001 (FR-4): intentionally OUT OF SCOPE, same reasoning as
  // the deferral notice above -- kind='heartbeat_status', not a reply-expecting decision_question.
  const notify = deps.enqueueChairmanSms || enqueueChairmanSms;
  ...
  // FIX c: name the decision's OWN subject (chairman_decisions.summary) when available, never the
  // literal held-row subject placeholder '[CHAIRMAN SMS]' the measured incident's notice carried.
  const subjectLabel = ((decisionInfo && decisionInfo.summary) || heldRow.subject || '(no subject)').slice(0, 60);
  const notice = await notify(supabase, {
    recipientPhone: heldRow.recipient_phone,
    kind: 'heartbeat_status',
    body: `A held chairman decision (${subjectLabel}) never reached you -- ${noticeDetail} and the hold has expired. It was NOT sent.`,
    decisionId: heldRow.decision_id || null,
