...
export async function abandonExpiredConsultHold(supabase, heldRow, deps = {}) {
  ...
  const voidReason = `abandoned by ${deps.claimedBy || 'chairman-held-sends-release-sweep'}: consult correlation ${heldRow.consult_correlation_id} produced no genuine Solomon answer before hold_expires_at (${heldRow.hold_expires_at}) -- FIX 2, QF-20260905-746 (consult-bearing holds previously had no expiry path).`;
  const { data: claimed, error: claimErr } = await supabase
    .from('chairman_held_sends')
    .update({
      status: 'abandoned',
  ...
  const notice = await notify(supabase, {
    ...
    body: `A held chairman decision (${(heldRow.subject || '(no subject)').slice(0, 60)}) never reached you -- its Solomon consult never answered and the hold has expired. It was NOT sent.`,
