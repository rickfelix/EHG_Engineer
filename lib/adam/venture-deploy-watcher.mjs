...
export async function runVentureDeployWatcher(supabase, deps = {}) {
  ...
  const { data: apps, error: appsErr } = await supabase
    .from('applications').select('id, name, repo_url').not('repo_url', 'is', null);
  if (appsErr) { result.errors.push(appsErr.message); return result; }

  const { data: pending } = await supabase
    .from('chairman_decisions').select('brief_data').eq('status', 'pending').eq('decision_type', DECISION_TYPE);
