...
export async function resolveAdamSessionIds(supabase) {
  try {
    ...
    const all = rows || [];
    return {
      ids: all.map((r) => r.session_id).filter(Boolean),
      // QF-20260816-532: the live subset of the same resolved ids, from the same query (no extra
      // round-trip, no race between two separate reads of claude_sessions).
      liveIds: all.filter((r) => r.status === 'active').map((r) => r.session_id).filter(Boolean),
      error: null,
    };
  } catch (e) {
    return { ids: [], liveIds: [], error: e && e.message ? e.message : String(e) };
  }
}
...
export async function runInboundBacklogWatchdog(supabase, opts = {}) {
  ...
  const result = {
    ...
    breachingCount: 0,
    rawBacklogCount: 0,
    // QF-20260816-532: rows addressed to a retired/dead role=adam session — never actionable
    // (nobody polls a dead session_id), reported separately so the real backlog floor is visible
    // instead of silently padding rawBacklogCount.
    phantomCount: 0,
    error: null,
  };
  ...
  try {
    const { ids, liveIds, error: idError } = await resolveAdamSessionIds(supabase);
    ...
    // QF-20260816-532: classify only rows targeting a LIVE session — a phantom row can never
    // breach meaningfully (nobody will ever read/ack it), so folding it into the real verdict
    // only ever pads rawBacklogCount, never adds real signal.
    //
    // ADVERSARIAL REVIEW FINDING (PR #7169): if liveIds is EMPTY — no Adam session is currently
    // active at all — there is nothing to partition AGAINST, and blindly partitioning against an
    // empty live set would send every historical row to phantomRows, silently reporting a clean
    // backlog while Adam is entirely dark. That is precisely the scenario this module's own
    // pre-existing design deliberately has NO liveness gate for (see the module docstring: "no
    // live Adam session is the WORST backlog condition, not an exemption" — SD criterion 4). Only
    // partition when a live session actually exists to distinguish actionable from phantom;
    // otherwise fall back to treating every historical row as real, unaddressed backlog.
    const { liveRows, phantomRows } = liveIds.length
      ? partitionByLiveness(rows, liveIds)
      : { liveRows: rows, phantomRows: [] };
    ...
    result.phantomCount = phantomRows.length;
    // undrainedKinds is a drain-set REGISTRATION gap (is this kind known at all?), not a liveness
    // concern — computed over the FULL row set (not just liveRows) so a kind that happens to first
    // appear on a phantom-targeted row still alarms, rather than being silently narrowed away by
    // the liveness partition above.
    result.undrainedKinds = partitionUndrained(rows).undrainedKinds;

    const scopes = [];
    if (verdict.breaching) scopes.push(SCOPE_BACKLOG);
    if (result.undrainedKinds.length > 0) scopes.push(SCOPE_UNDRAINED); // the full-set value, not verdict's liveRows-scoped one
    ...
    for (const scope of scopes) {
      ...
      const emitted = await emitFeedback({
        ...
        metadata: {
          ...
          phantom_count: result.phantomCount,
