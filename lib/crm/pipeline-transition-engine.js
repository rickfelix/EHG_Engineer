...
const INITIAL_STAGE = { pipeline: 'inbound', support: 'inbound' };

export async function createPipelineCase(supabase, { contactId, ventureId, caseType = 'pipeline' }) {
  ...
  const { data, error } = await supabase
    ...
    .insert({ contact_id: contactId, venture_id: ventureId, case_type: caseType, current_stage: initialStage })
    .select('id, contact_id, venture_id, case_type, current_stage')
    .single();
