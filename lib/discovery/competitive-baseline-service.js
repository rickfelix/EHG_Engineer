...
class CompetitiveBaselineService {
  ...
  /**
   * Write a researched (or fallback) result for venture_id + competitor_name, updating the
   * existing COMPETITOR row for that pair if one exists rather than always inserting. Without
   * this, the daily refresh (runRefresh -> researchAndCreate for every venture whose
   * getFreshOrNull() is null) would insert one new row per venture per tick indefinitely
   * whenever search is disabled or returns zero results -- unbounded row growth with no dedupe
   * or pruning path (adversarial review finding on this SD's own PR).
   * @param {Object} payload
   * @returns {Promise<Object>} Created or updated baseline row
   */
  async _writeResearchResult(payload) {
    const { data: existing, error: findError } = await this.supabase
      .from('competitive_baselines')
      .select('id')
      .eq('venture_id', payload.venture_id)
      .eq('competitor_name', payload.competitor_name)
      .eq('baseline_type', 'COMPETITOR')
      .maybeSingle();

    if (findError) throw new Error(`Failed to look up existing baseline: ${findError.message}`);
  ...
  async researchAndCreate(ventureId, competitorName, opts = {}) {
    ...
    if (!isSearchEnabled()) {
      return this._writeResearchResult({
