...
// SD-LEO-INFRA-VENTURE-INTAKE-GATE-PACK-001 (FR-3): compose the observe-only venture-intake
// gates onto the Stage-1 intake surface — G5 (non-commodity moat) + G6 (Option-B monetization
// shape). Imported from the ONE canonical module (no duplicate definition).
import { evaluateG5, evaluateG6 } from '../eva/venture-intake-gates.js';
...
export function evaluateIntakeBar(idea = {}) {
  ...
  // SD-LEO-INFRA-VENTURE-INTAKE-GATE-PACK-001 (FR-3): the venture-intake gates ride the intake
  // surface as a SEPARATE observe-only block (G5 the non-commodity-moat criterion, G6 the
  // Option-B monetization-shape filter) — NOT folded into the ratified 7-point score, so the
  // existing 7 criteria still fire unchanged (FR-6) and intake is never blocked.
  const intakeGates = { G5: evaluateG5(idea), G6: evaluateG6(idea) };
  return {
    ...
    checks,
    failures,
    intake_gates: intakeGates,
    intake_gates_observe_only: true,
