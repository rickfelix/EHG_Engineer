...
async function gatherIntakeStreams() {
  ...
  const [{ data: yt }, { data: td }] = await Promise.all([
    ...
    supabase.from('eva_todoist_intake').select('title, description, created_at').order('created_at', { ascending: false }).limit(30),
  ]);
  return shapePayload('intake_streams', 'Chairman intake streams (YouTube + Todoist)', {
    market: {
      ...
      todoist_signals: (td || []).map((t) => `${t.title}: ${String(t.description || '').slice(0, 200)}`),
    },
  });
}
