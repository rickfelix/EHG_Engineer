...
class OpportunityDiscoveryService {
  ...
  async runScan(params) {
    ...
    try {
      ...
      // Step 1: Gather market intelligence via the mission-anchored source
      // registry (SD-MAN-INFRA-STAGE-REVIVAL-PLUMBING-001 FR-1). The legacy
      // competitor path is preserved as the 'competitor_teardown' source.
      // Pipeline stays GATED: explicit invocation only, no cron/auto-dispatch.
      let competitorData;
      if (scanType === 'competitor' && targetUrl) {
        competitorData = await analyzeCompetitorCanonical(targetUrl, { serviceConfig: this.config });
      } else {
        const { gatherFromSource } = await import('./source-registry.js');
        competitorData = await gatherFromSource(scanType, { targetUrl, targetMarket, serviceConfig: this.config });
      }
  ...
  /**
   * Pure row builder — exported via class for the calibration-stamp pin test.
   * Every pipeline-created row carries (SD-MAN-INFRA-STAGE-REVIVAL-PLUMBING-001):
   * - metadata.intake_bar: 7-point advisory evaluation (FR-3, observe-first)
   * - metadata.calibration_cohort=true + cohort_number=1 (FR-4): gate ground
   *   truth for SD-MAN-INFRA-GATE-BAR-REGIME-001, NOT revenue; no launches.
   *   Metadata stamp by design — no calibration_cohort column exists; schema
   *   choice is coordinated with sibling GATE-BAR-REGIME-001 before any migration.
   */
  buildBlueprintRow(blueprint, evaluateBar) {
    const intakeBar = evaluateBar(blueprint);
    return {
      metadata: {
        ...(blueprint.metadata || {}),
        intake_bar: intakeBar,
  ...
  async saveBlueprintsToDatabase(blueprints) {
    ...
    const { evaluateIntakeBar } = await import('./intake-bar.js');
    for (const blueprint of blueprints) {
      try {
        const { error } = await supabase
          .from('opportunity_blueprints')
          .insert({
            ...this.buildBlueprintRow(blueprint, evaluateIntakeBar),
