...
/**
 * CANONICAL migration root directories, repo-relative and POSIX.
 *
 * SINGLE REPRESENTATION (SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-B): this list is the one
 * definition. scripts/verify-migration-apply-state.mjs imports its DEFAULT_EXTRA_ROOTS
 * from here rather than repeating the literal — two lists over the same corpus is the
 * drift class this workstream exists to remove, and the verifier's own scan set and this
 * reader's comparison key MUST agree or a file is scanned under a root the key cannot
 * anchor on.
 *
 * Order is irrelevant to correctness: normalizeMigrationPath anchors on the LAST match
 * across all roots, not on the first root that happens to hit.
 */
export const MIGRATION_ROOTS = Object.freeze([
  'database/migrations',
  'database/functions',
...
export function normalizeMigrationPath(inputPath) {
  ...
  // Already repo-relative under a recognized root — pass through unchanged.
  for (const rootDir of MIGRATION_ROOTS) {
    if (p.startsWith(`${rootDir}/`)) return p;
  }

  // LOCATION-INVARIANT KEY: anchor on the LAST recognized migration root, so the key
  // is identical no matter which checkout recorded the row. This closes BOTH previously
  // unmatchable shapes with one mechanism (SD-LEO-ORCH-CAPA-SCHEMA-TRUTH-001-B FR-1):
  //   (a) WORKTREE — '<root>/.worktrees/<name>/database/migrations/x.sql' DID start with
  //       the repo root, so the old prefix-strip fired and left '.worktrees/<name>/...',
  //       which still matched nothing. Stripped, and still unmatchable.
  //   (b) SIBLING CHECKOUT — '.../_EHG/.adam-scribe-fr5/database/migrations/x.sql' does
  //       NOT start with `${root}/`, so the old guard never fired at all and the value
  //       passed through ABSOLUTE AND UNMODIFIED. This is the shape a '.worktrees/'
  //       special-case would silently leave live, and it is the harder one to notice
  //       precisely because the value is not mis-shaped — it is untouched.
  // The SUBDIRECTORY IS PRESERVED and never collapsed to a basename: this function's
  // contract above states that basename is too weak because database/migrations/x.sql and
  // database/chairman-gated/x.sql would collide. That collision guard still holds here.
  let anchor = -1;
  for (const rootDir of MIGRATION_ROOTS) {
    const idx = p.lastIndexOf(`/${rootDir}/`);
    if (idx > anchor) anchor = idx;
  }
  if (anchor !== -1) return p.slice(anchor + 1);

  // Unrecognized root: previous behaviour, byte-identical. Like the getLatestSuccessForPath
  // fallback, this change can only ADD matches, never remove a previously-working one.
  if (p.startsWith(`${root}/`)) return p.slice(root.length + 1);
  return p;
}

/**
 * QF-20260903-622: true iff `rawPath` (normalized to a repo-relative comparison key via
 * normalizeMigrationPath) resolves to a file git actually tracks. Five schema_migrations_applied
 * rows were found asserting success=true for paths that never existed at that path — none stale
 * (moved/deleted), all simply phantom. This is the write-time guard: a caller recording success
 * for a migration_path MUST pass this check first, so a new phantom row can no longer be written.
 *
 * `execFn` is injectable so callers/tests can avoid a real git subprocess (pure by default via
 * child_process.execSync, matching apply-migration.js's own isMigrationCommittedToGit pattern).
 *
 * QF-20260913-250: `repoRoot` is injectable (defaults to getRepoRoot(), byte-identical to
 * previous behaviour for every existing caller). getRepoRoot() deliberately resolves a
 * .worktrees/<sd> checkout to the MAIN repo (lib/repo-paths.js), so a caller running from a
 * worktree whose pre-DDL check already passed against ITS OWN checkout root would otherwise
 * have this write-time guard check a DIFFERENT root (main) that may not yet have the
 * branch-only file — disagreeing with the check that just passed, by construction. Passing
 * the invoking checkout's actual root here makes both guards read one checkout.
 *
 * @param {string} rawPath
 * @param {{execFn?: Function, repoRoot?: string}} [opts]
 * @returns {boolean}
 */
export function isTrackedMigrationPath(rawPath, { execFn = execSync, repoRoot } = {}) {
  if (!rawPath || typeof rawPath !== 'string') return false;
  const relPath = normalizeMigrationPath(rawPath);
  try {
    execFn(`git ls-files --error-unmatch -- "${relPath}"`, {
      cwd: repoRoot || getRepoRoot(),
