  ...
  const subscription = supabase
    ...
    .on(
      ...
      async (payload) => {
        ...
        // sd_id attributes the RCR to the SD whose run is FAILING now, not the SD whose
        // (unrelated, earlier) run happened to pass -- priorRuns deliberately excludes the
        // current run (line 149), so it is never a source of the current run's own sd_id.
        const { data: currentRun, error: currentRunError } = await supabase
          .from('test_runs')
          .select('sd_id')
          .eq('id', failure.test_run_id)
          .maybeSingle();

        if (currentRunError) {
          console.warn(
            '[rca-runtime-triggers] current test_run lookup failed -- skipping',
            { id: failure.id, error: currentRunError }
          );
          return;
        }

        const sd_id = currentRun?.sd_id ?? null;
