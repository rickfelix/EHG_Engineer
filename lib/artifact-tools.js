...
/**
 * Read a task contract by ID (with expanded artifact contents)
 *
 * @param {string} contractId - The contract UUID
 * @returns {Promise<Object>} Contract details with input artifacts expanded
 */
export async function readTaskContract(contractId) {
  const db = getSupabase();

  const { data, error } = await db.rpc('read_task_contract', {
    p_contract_id: contractId
  });

  if (error) {
    console.error('Failed to read task contract:', error);
    throw new Error(`Task contract read failed: ${error.message}`);
  }
