...
const DISPOSITIONS = Object.freeze({
  ...
  // SD-LEO-INFRA-SIGNAL-LANE-PER-001 (FR-1): the signal lane's per-item disposition machinery
  // needs two genuinely new outcomes the original 3-value set cannot express — "promoted" (this
  // signal was the one that tipped an aggregation into a feedback promotion) and "deferred"
  // (dispositioned, but revisit when a named trigger fires — not a permanent close).
  PROMOTED: 'promoted',
  DEFERRED: 'deferred',
});

/**
 * SD-LEO-INFRA-SIGNAL-LANE-PER-001 (FR-1): the signal lane's 5-value CALLER-FACING vocabulary
 * (actioned / promoted / duplicate-of / rejected-with-reason / deferred-with-trigger) maps onto
 * this ledger's DISPOSITIONS, explicitly — never silently. 'duplicate-of' and 'rejected-with-reason'
 * reuse SUPERSEDED/DECLINED (the existing values already capture that semantic) with the specific
 * target/reason carried in metadata, rather than growing the enum for a distinction the ledger
 * itself doesn't need to make. 'requires' names the extra field the CLI must supply and reject on
 * absence — see resolveSignalDisposition().
 */
const SIGNAL_LANE_DISPOSITIONS = Object.freeze({
  actioned: Object.freeze({ ledger: DISPOSITIONS.ACTIONED }),
  promoted: Object.freeze({ ledger: DISPOSITIONS.PROMOTED }),
  'duplicate-of': Object.freeze({ ledger: DISPOSITIONS.SUPERSEDED, requires: 'duplicateOf' }),
  'rejected-with-reason': Object.freeze({ ledger: DISPOSITIONS.DECLINED, requires: 'reason' }),
  'deferred-with-trigger': Object.freeze({ ledger: DISPOSITIONS.DEFERRED, requires: 'trigger' }),
});

/**
 * Resolve a signal-lane caller-facing disposition value into a ledger disposition + metadata,
 * enforcing mandatory linkage (mirrors coordinator-ack-adam.cjs's LINKAGE_REQUIRED_DISPOSITIONS /
 * defer-trigger pattern). PURE — no I/O, testable without a database.
 *
 * @param {string} value - one of SIGNAL_LANE_DISPOSITIONS' keys
 * @param {{reason?: string, trigger?: string, duplicateOf?: string}} [extra]
 * @returns {{ok: true, ledgerDisposition: string, metadata: object} | {ok: false, error: string}}
 */
function resolveSignalDisposition(value, extra = {}) {
  const spec = SIGNAL_LANE_DISPOSITIONS[value];
  if (!spec) return { ok: false, error: `unknown signal-lane disposition: ${value}` };
  if (spec.requires) {
    const suppliedValue = extra[spec.requires];
    if (typeof suppliedValue !== 'string' || suppliedValue.trim().length === 0) {
      return { ok: false, error: `disposition '${value}' requires a non-empty '${spec.requires}'` };
    }
  }
  const metadata = {};
  if (spec.requires) metadata[spec.requires] = extra[spec.requires].trim();
  return { ok: true, ledgerDisposition: spec.ledger, metadata };
}
...
module.exports = { LANES, STATES, DISPOSITIONS, SIGNAL_LANE_DISPOSITIONS, resolveSignalDisposition, buildReceipt, recordReceipt };
