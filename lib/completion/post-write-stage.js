...
export async function runPostWriteStage(name, fn, opts = {}) {
  ...
  //
  // QF-20260913-273: fn() is invoked INSIDE .then(), not called directly -- a caller may pass a
  // thenable with no .catch method (e.g. a raw Supabase PostgrestFilterBuilder, which is
  // PromiseLike but not a real Promise), or fn itself may throw synchronously. Calling fn()
  // directly (the prior `const fnPromise = fn();`) let a synchronous throw escape this function
  // entirely (uncaught, before the try block below even starts), and let a non-Promise thenable
  // reach `.catch()` two lines down and throw 'fnPromise.catch is not a function'. Promise.resolve()
  // .then(fn) defers the fn() call into a microtask callback, so the Promise machinery converts
  // ANY outcome -- a normal value, a thenable, a thrown error -- into one genuine native Promise
  // (with a real .catch) before it is ever touched here.
