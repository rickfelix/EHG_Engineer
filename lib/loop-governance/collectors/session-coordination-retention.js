 ...
 * @param {Date} [now] - evaluation instant (defaults to the real clock; callers that pin
 *   `now` for evaluateLoopBatch/evaluateLoopClosure MUST thread the SAME instant here too
 *   — a split clock between this collector and the evaluator is exactly the defect class
 *   that produced a 29-day-fused CI time bomb, see the collector-registry seam in index.js
 *   and verifier.js:35).
 ...
 */
export async function collectSessionCoordinationRetentionEvidence(supabase, now = new Date()) {
