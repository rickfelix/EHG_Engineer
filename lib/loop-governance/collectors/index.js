...
/** loop_key -> (supabase, now?) => Promise<Object> */
 ...
 * @returns {(loop:Object, now?:Date)=>Promise<Object>} collectEvidence, matching runClosureVerifier's expected signature
 */
export function createCollectEvidence(supabase) {
  return async function collectEvidence(loop, now = new Date()) {
    const collector = COLLECTORS[loop?.loop_key];
    if (!collector) return {};
    return collector(supabase, now);
  };
}
