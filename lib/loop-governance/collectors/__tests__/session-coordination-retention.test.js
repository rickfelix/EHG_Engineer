...
// The collector now takes an injected `now` (defaults to the real clock in production —
// lib/loop-governance/collectors/session-coordination-retention.js). Every fixture below
// threads the SAME `now` into both the collector and evaluateLoopClosure, so the two legs
// agree by construction. Previously (QF-20260717-737) this suite faked ONLY `Date` in
// beforeAll to work around the collector's un-injectable Date.now() — that workaround
// never actually worked (describe-body fixtures below evaluate at collection time, before
// beforeAll runs, so the "pinned" clock and the fixtures disagreed) and this suite was
// quarantined 2026-07-17 when it failed anyway. See closure-engine.js's
// CLOCK_SKEW_TOLERANCE_MS for the companion fix bounding `fresh()` against future-dated
// evidence — the exact mechanism that let a split clock false-CLOSE for 29 days in the
// sibling registry.test.js suite.
...
describe('collectSessionCoordinationRetentionEvidence (TS-1..TS-3)', () => {
  it('returns a fresh edgeAt when the reaper fired and the backlog is at/under threshold (measured decline)', async () => {
    ...
    const evidence = await collectSessionCoordinationRetentionEvidence(supabase, NOW);
  ...
  it('throws when the retention_archive query errors (never swallows into a fabricated verdict)', async () => {
    ...
    await expect(collectSessionCoordinationRetentionEvidence(supabase, NOW)).rejects.toThrow(/retention_archive query failed/);
  });
  ...
  it('throws when the eligible-backlog count query errors (never swallows into a fabricated verdict)', async () => {
    ...
    await expect(collectSessionCoordinationRetentionEvidence(supabase, NOW)).rejects.toThrow(/eligible-backlog count query failed/);
  });
  ...
  it('pins the eligible-backlog predicate to cleanup_expired_coordination\'s own delete filter (not the raw expires_at < now() count)', async () => {
    ...
    await collectSessionCoordinationRetentionEvidence(supabase, NOW);
  ...
  it('adversarial-review fix: a reaper that fired once, long ago (400 days), never produces edgeAt even if the current backlog happens to be zero', async () => {
    ...
    const archivedAt = new Date(NOW.getTime() - 400 * 86400 * 1000).toISOString();
    const orSpy = vi.fn();
  ...
  it('boundary: retention_archive round exactly 30 days old still counts as recent enough', async () => {
    const archivedAt = new Date(NOW.getTime() - 30 * 86400 * 1000 + 1000).toISOString(); // 1s inside the window
  ...
  it('boundary: retention_archive round just past 30 days old is too stale for edgeAt', async () => {
    const archivedAt = new Date(NOW.getTime() - 30 * 86400 * 1000 - 1000).toISOString(); // 1s outside the window
    const orSpy = vi.fn();
    const supabase = stubSupabase({ archiveData: [{ archived_at: archivedAt }], count: 0, orSpy });
    const evidence = await collectSessionCoordinationRetentionEvidence(supabase, NOW);
    expect(evidence).toEqual({ upstreamFiredAt: archivedAt, edgeAt: null });
...
describe('GT-1 golden regression: liveness alone never closes L30, measured decline does (TS-5)', () => {
  ...
  // Both legs (collector + evaluator) take the SAME injected `now` explicitly — no
  // reliance on two independent clock reads (real or otherwise) staying in sync.
  const now = NOW;
  ...
  it.each([
    ...
    const evidence = await collectSessionCoordinationRetentionEvidence(
      stubSupabase({ archiveData: [{ archived_at: upstreamFiredAt }], count: 100 }), now
    );
  ...
  it.each([
    ...
    const evidence = await collectSessionCoordinationRetentionEvidence(
      stubSupabase({ archiveData: [{ archived_at: upstreamFiredAt }], count: 0 }), now
    );
    const verdict = evaluateLoopClosure(loop, evidence, now);
  ...
  it('boundary: backlog exactly at threshold (5) counts as measured decline', async () => {
    const evidence = await collectSessionCoordinationRetentionEvidence(
      stubSupabase({ archiveData: [{ archived_at: ago(0) }], count: 5 }), now
    );
  ...
  it('boundary: backlog one over threshold (6) does NOT count as measured decline', async () => {
    const evidence = await collectSessionCoordinationRetentionEvidence(
      stubSupabase({ archiveData: [{ archived_at: ago(0) }], count: 6 }), now
    );
  ...
  it('status is starved when the collector finds no evidence at all', async () => {
    const evidence = await collectSessionCoordinationRetentionEvidence(stubSupabase({ archiveData: [] }), now);
