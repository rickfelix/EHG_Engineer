...
describe('createCollectEvidence (TS-6)', () => {
  ...
  it('delegates to the registered L30 collector, passing supabase through (liveness only, high backlog)', async () => {
    ...
    const collectEvidence = createCollectEvidence(supabase);
    // Pin `now` near archivedAt so this exercises the intended high-backlog rejection path
    // rather than short-circuiting on the collector's (unrelated) upstream-staleness guard —
    // both produce edgeAt: null, so an un-pinned real clock would pass this assertion for the
    // wrong reason once real time ages past archivedAt + RETENTION_ARCHIVE_STALE_MS (30d).
    const evidence = await collectEvidence({ loop_key: 'L30' }, new Date('2026-07-16T12:00:00.000Z'));
