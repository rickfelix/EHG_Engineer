...
// A small allowance for real clock/DB skew — never a license for future-dated evidence.
// Without this bound, an edge/witness timestamp minutes (or days) ahead of `now` reads as
// negative age, which satisfies ANY window forever: a split clock between an evidence
// collector and the evaluator (e.g. one using a pinned test `now`, the other reading the
// real clock) silently produces a false CLOSED instead of failing loud. Confirmed root
// cause of a 29-day-fused CI time bomb (lib/loop-governance/collectors/session-coordination-
// retention.js previously read Date.now() directly while its evaluator took an injected
// `now`); this bound is the fix that would have caught it on day one.
...
export function evaluateLoopClosure(loop, evidence = {}, now = new Date()) {
  ...
  const fresh = (iso) => {
    ...
    const ageMs = nowMs - t;
    if (ageMs < -CLOCK_SKEW_TOLERANCE_MS) return false; // future-dated beyond tolerable skew — never fresh
    return windowMs == null ? true : ageMs <= windowMs;
  };
