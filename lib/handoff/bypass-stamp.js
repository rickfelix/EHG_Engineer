 ...
 * @param {{source: string, reason: string|null, gate: string, gates?: string[], issues?: string[], patternId?: string|null, followupSdKey?: string|null, ledgerId?: string|null}} params
 * @returns {object}
 */
export function buildBypassStamp({ source, reason = null, gate, gates, issues = [], patternId = null, followupSdKey = null, ledgerId = null, selfAuthorshipCheckStatus = null }) {
  ...
  return {
    ...
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-B (FR-B1): the bypass_ledger row id this bypass
    // attempt originated from, so HandoffRecorder can join it back to whichever sd_phase_handoffs
    // row (accepted or rejected) this attempt produces. Threaded from BOTH bypass call sites --
    // the ledger row is written unconditionally whenever --bypass-validation is passed, regardless
    // of which fall-through site ultimately fires.
    ledgerId: ledgerId || null,
    // SD-LEO-ORCH-CAPA-GATE-EVIDENCE-001-B (FR-B2, SECURITY finding HIGH-2): one of 'cleared'
    // (checked, no match), 'skipped_no_actor_session', 'skipped_no_evidence_session_id',
    // 'not_applicable_no_evidence_detail', 'not_applicable_authority_fence', or null when this
    // stamp predates FR-B2. Never let "the check cleared it" and "the check could not run" read
    // as the same thing on a persisted row.
    selfAuthorshipCheckStatus: selfAuthorshipCheckStatus || null,
  };
}
...
export function applyBypassToResult(baseResult, bypassInfo) {
  ...
  return {
    ...
    bypassLedgerId: bypassInfo.ledgerId,
    bypassSelfAuthorshipCheckStatus: bypassInfo.selfAuthorshipCheckStatus,
  };
}
...
export function buildPersistedBypassMetadata(result, { actor, nowIso } = {}) {
  return {
    ...
    self_authorship_check_status: result?.bypassSelfAuthorshipCheckStatus || null,
  };
}
