 ...
 *
 * TR-4: calls executeSubAgent()/storeSubAgentResults() DIRECTLY via lib/sub-agent-executor.js
 * — this file deliberately does NOT import lib/error-triggered-sub-agent-invoker.js, which is
 * a structurally distinct, chairman-provisional-gated mechanism (error-pattern-triggered, with
 * its own circuit breaker). TS-7 asserts this via static source inspection.
 */
 ...
 *   `executeSubAgentFn` is injected (defaults to the real executeSubAgent) so callers/tests
 *   never need a live DB/agent to exercise the bounded-invocation and scope-restriction logic.
