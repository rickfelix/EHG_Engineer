...
export const SD_TYPE_ADDRESSABLE_DIMENSIONS = {
  ...
  // QF-20260816-696: the 'fix' entry that used to sit here was a phantom duplicate of
  // 'bugfix' above -- this docstring's own line 16-17 already said it "was removed", but it
  // was only dropped from SD_TYPE_THRESHOLDS, not here. Deleted (not renamed: 'bugfix' below
...
/**
 * A dimension key that is a DURATION, not a 0-100 quality score. Exported (SD-LEO-INFRA-LEARN-
 * VISION-GAP-RUBRIC-CLASSIFY-001) so a per-key consumer (vision-to-patterns.js) can exclude this
 * specific key from a 0-100 threshold comparison without duplicating the 'elapsed_ms' literal --
 * a latency-3dim row may legitimately mix this key with otherwise-comparable ones, so the
 * exclusion this key drives must be per-key, not per-row (unlike EXACT_RUBRICS members, whose
 * exact-match membership makes whole-row exclusion safe -- see EXACT_RUBRICS above).
 */
export const LATENCY_KEYS = new Set(['elapsed_ms']);
