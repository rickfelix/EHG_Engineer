...
export function buildWaitResult({
  ...
  // SD-LEO-INFRA-FIX-JOURNEY-WALK-001 FR-3: per-wait-reason exemption from the 24h
  // wall-clock ceiling (ValidationOrchestrator.js's WAIT_MAX_WALL_CLOCK_MS). Distinct from
  // the existing gate-level `gate.exemptFromWaitCeiling` flag (exec-boundary-hold.js) --
  // this one applies to a SPECIFIC wait reason within a gate that emits several, so setting
  // it here never exempts a DIFFERENT wait reason from the same gate.
  if (exemptFromWaitCeiling === true) result.exemptFromWaitCeiling = true;
  return result;
}
