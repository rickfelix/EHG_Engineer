...
export async function completeSession(testRunId, controlPackEvidence = null) {
  ...
  // SECURITY sub-agent finding S5 (EXEC-TO-PLAN evidence): computeSubstantiveEvidenceHash was
  // defined and unit-tested but had zero callers -- wired here as an actually-exercised
  // control. Only meaningful once a PRIOR run's hash is available for comparison (the caller,
  // e.g. lib/eva/uat-robustness-gate.js, is what knows how to look up the prior run for a
  // venture/stage); the FIRST run for a venture/stage establishes a baseline, not a failure.
