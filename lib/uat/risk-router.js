...
export async function notifyHighSeverityFeedback(feedbackRecord) {
  ...
  if (feedbackRecord.priority === 'P0') {
    ...
    if (assessment.riskLevel === 'HIGH') {
      ...
      //
      // SD-LEO-INFRA-AUDIT-FIX-FEEDBACK-001-G: this used to merge into feedbackRecord.metadata,
      // the pre-insert in-memory snapshot passed into this function -- NOT a fresh read. That
      // raced with lib/eva/event-bus/handlers/feedback-quality-updated.js, which fires on the
      // same insert via a fire-and-forget vision event and DOES read metadata fresh immediately
      // before its own write. Whichever write landed second won wholesale, silently dropping the
      // other's fields (dimension_codes/dimension_classifications vs risk_assessment/
      // escalation_actions/escalated_at). Read fresh here too, mirroring that handler's pattern,
      // so both writers' fields survive regardless of ordering.
      try {
        const { data: freshRow, error: readError } = await db
          ...
          .single();
        if (readError) {
          console.error('[RiskRouter] Failed to read fresh feedback metadata before escalation update:', readError.message);
        } else {
          const { error: updateError } = await db
            ...
            .eq('id', feedbackRecord.id);
          // supabase-js resolves with {error} rather than throwing on a PostgREST rejection --
          // an un-destructured await here silently swallowed every rejected UPDATE (this is why
          // the 2026-09-08 append-only-trigger break was invisible on this call site for 4 days).
          if (updateError) {
