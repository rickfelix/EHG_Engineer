...
/**
 * Convert one derived journey step (lib/eva/bridge/orchestrator-journey-steps.js's
 * deriveJourneySteps() output) into a GWT-shaped scenario, for FR-2's journey walker.
 *
 * Real Stage-15 journey steps carry `action`/`expected_outcome` as goal-level user-intent
 * prose (e.g. "define my primary niche and target audience"), not concrete UI actions --
 * confirmed against a live blueprint_user_journey artifact. `when` is therefore the step's
 * own action/goal text verbatim, not a synthesized UI instruction the way storyToScenario()
 * synthesizes one from a user story's free-text description.
 *
 * @param {{step_id: string, journey_id: string|null, persona_ref: string|null, goal: string|null, action: string|null, expected_outcome: string|null, route: string|null, screen_ref: string|null}} step
 * @returns {Object} Test scenario
 */
function journeyStepToScenario(step) {
  const persona = step.persona_ref || 'a user';
  const goal = step.goal || step.action || 'complete the step';
  ...
  return {
    ...
    title: `Journey: ${goal}`,
    // Journey steps carry no priority field of their own -- these are a venture's core
    // Stage-15 user journeys (not optional/edge-case flows), so default HIGH rather than
    // MEDIUM the way an unscored PRD criterion would.
    priority: 'HIGH',
    priorityScore: priorityToNumber('HIGH'),

    given: `${persona} is attempting to ${goal}`,
    when: step.action || goal,
    then: step.expected_outcome || 'the step completes successfully',

    steps: [
      { step: 1, action: goal, detail: step.screen_ref ? `Expected on screen ${step.screen_ref}` : 'No screen_ref recorded' },
    ],

    passCriteria: [step.expected_outcome || 'step completes without error'],
    storyPoints: 1,
    labels: ['journey-step', step.journey_id].filter(Boolean),
    routeContext: { path: step.route, screenRef: step.screen_ref },
  };
}

/**
 * Generate GWT-shaped scenarios directly from derived journey steps (FR-1's
 * deriveJourneySteps() output) -- for FR-2's journey walker, not the SD/user_stories path
 * generateScenarios() below uses. Pure: no DB access.
 *
 * @param {Array|null|undefined} journeySteps
 * @returns {Array} scenarios, one per step, in input order (never throws on empty input)
 */
export function generateJourneyScenarios(journeySteps) {
  if (!Array.isArray(journeySteps)) return [];
  return journeySteps.map(journeyStepToScenario);
}
