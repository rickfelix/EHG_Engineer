...
// SD-FDBK-INFRA-AUTO-PUSH-WIP-001 (FR-2): read the FULL PostToolUse stdin payload
// ONCE and return the parsed object (or null). stdin can only be consumed once, so
// a hook that needs BOTH session_id AND tool_input (e.g. the ScheduleWakeup hook
// reading tool_input.delaySeconds to arm a capped expected_silence_until) must read
// the whole payload here instead of calling readSessionIdFromStdin (which discards
// everything but session_id). Resolve the session id from the returned object via
// `payload?.session_id` (validate with isValidSessionId) and fall back to env /
// marker resolvers — those do NOT touch stdin.
function readHookStdinPayload(timeoutMs = 250) {
  return new Promise((resolve) => {
    let buf = '';
    const timer = setTimeout(() => resolve(null), timeoutMs);
    try {
