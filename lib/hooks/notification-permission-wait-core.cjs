...
const BLOCKED_ACTION_DETAIL_MAX = 200;

// QF-20260911-078: the hook hardcoded kind='notification_permission_wait' for EVERY
// Notification event, regardless of payload.notification_type -- measured live (retention_archive,
// the only place any of these 16+ rows still exist, since the live table has archived all of
// them): 'permission_prompt' (the real blocking case), 'idle_prompt' ("Claude is waiting for your
// input" -- a RESUMING seat, not blocked) and 'quota_auto_resume_fired' ("Usage limit reset" -- also
// resuming). Lumping all three under one name over-counted frozen seats ~5.3x (13 of 16 observed
// rows were not permission waits) and, worse, made a resuming seat read as blocked. Claude Code's
// own hook payload already carries notification_type distinctly; this just stops discarding it.
const NOTIFICATION_KIND_BY_TYPE = Object.freeze({
  permission_prompt: 'notification_permission_wait',
  idle_prompt: 'notification_idle_prompt',
  quota_auto_resume_fired: 'notification_usage_limit_reset',
});

/** Map a Notification hook's notification_type to its session_coordination payload.kind.
 *  An unrecognized/absent type falls back to 'notification_other' -- NOT to
 *  notification_permission_wait, which would silently reintroduce the over-count this exists to
 *  fix for any future Claude Code notification sub-type not yet seen. */
function classifyNotificationKind(notificationType) {
  return NOTIFICATION_KIND_BY_TYPE[notificationType] || 'notification_other';
}

// QF-20260905-884 (second appended half): the row previously carried only the generic
// "Claude needs your permission" message, so nobody reading it could name the blocked
// command without opening the transcript by hand. The Notification hook payload carries
// transcript_path; walk its tail (same reader stop-loop-wakeup-reminder.cjs already uses
// for arm-evidence scanning) for the LAST assistant tool_use block and surface its name +
// command/file_path so the row -- and the pager line built from it -- is self-describing.
// `deps.readTailEntries` is injectable (mirrors resolveTargetSession's `readPointerFile`
// DI) so tests exercise the real branch logic against a synthetic entries array instead of
// a real transcript file on disk.
//
// ADVERSARIAL REVIEW FINDING (deep-tier /ship gate, this SD): a blocked Bash command can embed
// a live credential (`curl -H "Authorization: Bearer sk-..."`, `export SUPABASE_SERVICE_ROLE_KEY=...`)
// -- an ops/debugging harness routinely runs exactly that shape. Without redaction, that raw text
// would land unredacted in the persisted session_coordination row AND get broadcast over the
// chairman's SMS (an external channel, per buildStuckPermissionWaitMessage in
// scripts/fleet-down-alert.mjs). `redact()` is the same general-purpose 6-pattern secret
// redactor scripts/worker-signal.cjs already applies to every outbound signal body (AWS keys,
// GitHub/provider tokens, JWTs, password=, postgres conn strings, credential-keyword fallback)
// -- one policy, reused, not a second hand-rolled pattern set.
function extractBlockedAction(transcriptPath, deps = {}) {
  if (typeof transcriptPath !== 'string' || !transcriptPath) return null;
  try {
    const fs = require('fs');
    const { redact } = require('../shared/body-cap.cjs');
    const readTail = deps.readTailEntries || require('../../scripts/hooks/print-before-park.cjs').readTailEntries;
    if (!deps.readTailEntries && !fs.existsSync(transcriptPath)) return null;
    const entries = readTail(transcriptPath) || [];
    for (let i = entries.length - 1; i >= 0; i--) {
      const e = entries[i];
      if (!e || e.type !== 'assistant' || !e.message || !Array.isArray(e.message.content)) continue;
      for (let j = e.message.content.length - 1; j >= 0; j--) {
        const block = e.message.content[j];
        if (block && block.type === 'tool_use') {
          const rawDetail = (block.input && (block.input.command || block.input.file_path || block.input.path)) || '';
          return {
            tool: block.name || 'unknown',
            detail: redact(String(rawDetail)).slice(0, BLOCKED_ACTION_DETAIL_MAX),
          };
        }
      }
    }
    ...
    return null; // fail-open: transcript parsing must never break the write path
  }
}
...
function writeNotificationRow(payload, deps = {}) {
  try {
    ...
    const blockedAction = extractBlockedAction(payload.transcript_path, deps);
    ...
    const body = JSON.stringify({
      ...
      sender_type: 'worker',
      payload: {
        kind: classifyNotificationKind(payload.notification_type),
...
module.exports = {
  writeNotificationRow, drainUndiciPool, resolveTargetSession, resolveCredentials, extractBlockedAction,
  classifyNotificationKind, NOTIFICATION_KIND_BY_TYPE,
};
